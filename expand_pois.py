...
poi_data = {
    ...
    "animal-habitat-nagyerdei-kulturpark-extra": {
        ...
        "facts": [
            ...
            "Es werden über 160 Tierarten gezeigt, darunter seltene Giraffen, Tiger und Rote Pandas.",
    ...
    "industry-bmw-debrecen-extra": {
        ...
        "facts": [
            ...
            "Das Gelände des Werks umfasst eine Fläche von über 400 Hektar.",
...
for poi_id, data in poi_data.items():
    poi_start = content.find(f'id: "{poi_id}"')
    if poi_start == -1:
        print(f"POI {poi_id} not found")
        continue
    
    # Find the closing brace of the POI object
    # We look for the next '},' that matches the indentation of the start
    # But for simplicity, let's find the 'facts:' block and append after it
    
    facts_block_end = content.find('    }', content.find('facts:', poi_start)) + 5
    
    # Check if descriptionAdvanced already exists
    if content.find('descriptionAdvanced:', poi_start, facts_block_end + 500) != -1:
        # It exists, but it was empty/wrong in my previous run attempt (actually it didn't exist)
        # So we skip adding and do replacement if needed, but here we assume it doesn't exist
        pass
    ...
    # Insert before the closing brace of the POI object
    # The POI object ends with a line that has '  },'
    poi_end = content.find('  },', poi_start)
    if poi_end != -1:
        content = content[:poi_end] + advanced_content + "\n" + content[poi_end:]

with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("Added advanced content to 25 POIs successfully.")
