...
with open('romania_ro_data.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

with open('lib/visualLab/data/romaniaPoi.ts', 'r', encoding='utf-8') as f:
    content = f.read()

# Strategy: Find all POI blocks, and if the ID matches one we generated, we replace the `ro` values.
...
with open('lib/visualLab/data/romaniaPoi.ts', 'w', encoding='utf-8', newline='\n') as f:
    f.write(new_content)
