...
function generateGeneratorsTyping(lang = "en", seed = 0): CurriculumTyping[] {
  return [
    ...
    createTyping("electromagnetism", "generators", q4("Viszontjelentés Elektromágnes?", "Back EMF (counter-EMF)?", "Visszahatás EMF?", "EMF contrare?", lang),
...
function generateTransformersTyping(lang = "en", seed = 0): CurriculumTyping[] {
  return [
    ...
    createTyping("electromagnetism", "transformers", q4("Zajtartalomtartamány Transzformátor?", "Transformer noise?", "Transzformátor zaj?", "Zgomot transformator?", lang),
      [q4("~50 Hz alapharmónikus (2 × 50 Hz = 100 Hz); magnetostrikció (vas molekulák vibrálnak); olaj hűtésben buborék okoz", "~50 Hz fundamental + harmonics (100 Hz); magnetostriction (iron vibration); oil cooling bubbles", "~50 Hz + harmonikusok; magnetostrikció (vas vibrálás); olaj hűtésben buborék", "~50 Hz + armonici; magnetostricție (oscilație fier); bule în ulei", lang)]),
  ];
}
