...
function generateResistanceCalcTyping(lang = "en", seed = 0): CurriculumTyping[] {
  return [
    ...
    createTyping("electricity", "resistance_calc", q4("L verdoppelt → R?", "L doubled → R?", "L megduplázódott → R?", "L dublat → R?", lang),
      [q4("R verdoppelt auch (linear proportional zu L)", "R also doubled (linear proportional to L)", "R szintén megduplázódott (lineárisan arányos L-hez)", "R și se-nduplează (linear proporțional cu L)", lang)]),
    createTyping("electricity", "resistance_calc", q4("A halbe → R?", "A halved → R?", "A felezése → R?", "A jumătate → R?", lang),
      [q4("R verdoppelt (vékonyabb Draht → höhere R)", "R doubled (thinner wire → higher R)", "R megduplázódik (vékonyabb drót → nagyobb R)", "R se-nduplează (fir mai subțire → R mai mare)", lang)]),
