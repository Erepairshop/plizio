...
export const AI_K5_CURRICULUM: KemiaTheme[] = Array.from({length: 9}, (_, th) => {
  const i = th + 1;
  const poolData = generateAiPool(5, i);
  return {
    id: `ai_k5_th${i}`,
    ...
    subtopics: Array.from({length: 15}, (_, sub) => {
      ...
      return {
        ...
        questions: poolData.pool[sub].quiz ? [{
          type: "mcq",
          topic: "ai",
          subtopic: `ai_k5_t${i}_${topicIndex}`,
