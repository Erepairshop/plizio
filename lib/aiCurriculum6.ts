...
import t01 from "@/data/ai/class-6/hu/1.json";
...
export const AI_K6_CURRICULUM: KemiaTheme[] = Array.from({length: 6}, (_, th) => ({
  id: `ai_k6_th${th+1}`, name: `AI Téma ${th+1}`, icon: "🧠", color: "#10B981",
  subtopics: Array.from({length: 5}, (_, sub) => ({
    id: `ai_k6_t${th*5 + sub + 1}`,
    name: { de: `Thema ${th*5 + sub + 1}`, en: `Topic ${th*5 + sub + 1}`, hu: `Téma ${th*5 + sub + 1}`, ro: `Tema ${th*5 + sub + 1}` },
    questions: jsonToQuestions(`ai_k6_t${th*5 + sub + 1}`, e(t01)),
    hasGenerator: false
  }))
}));

export function getAIK6Questions(subtopicIds: string[], count?: number): KemiaQuestion[] {
  const pool: KemiaQuestion[] = [];
  for (const theme of AI_K6_CURRICULUM) {
