...
export const AI_K6_CURRICULUM: KemiaTheme[] = Array.from({length: 9}, (_, th) => {
  const i = th + 1;
  const poolData = generateAiPool(6, i);
  return {
    id: `ai_k6_th${i}`,
    ...
    subtopics: Array.from({length: 15}, (_, sub) => {
      ...
      return {
        ...
        questions: poolData.pool[sub].quiz ? [{
          type: "mcq",
          topic: "ai",
          subtopic: `ai_k6_t${i}_${topicIndex}`,
