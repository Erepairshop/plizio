...
function generateImperialismus(seed: number = 801): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Was meinte man mit 'Sendungsbewusstsein' im Imperialismus?", "Der Glaube, die eigene 'zivilisierte' Kultur in die Welt tragen zu müssen", ["Die Absicht, alle Briefe per Post zu schicken", "Die Idee, dass alle Menschen reich sein sollten", "Kein Begriff des Imperialismus"]],
    ...
    ["Was passierte mit der Kultur der Kolonisierten?", "Sie wurde oft unterdrückt und durch europäische Werte ersetzt", ["Sie wurde weltweit Vorbild", "Sie wurde unter Naturschutz gestellt", "Nichts geschah"]],
    ["Was war die 'Zweibund' Allianz?", "Ein Bündnis zwischen Deutschland und Österreich-Ungarn", ["Frankreich und Russland", "England und USA", "Italien und Spanien"]],
    ["Welche Krise in Marokko zeigte die Spannungen zwischen den Mächten?", "Die Marokkokrisen (1905 und 1911)", ["Die Weizenkrise", "Die Goldkrise", "Die Ölkrise"]],
    ...
    ["Was war die 'Faschoda-Krise' (1898)?", "Ein Beinahe-Krieg zwischen England und Frankreich in Afrika", ["Eine Tropenkrankheit", "Ein Modestil", "Ein Musikstück"]],
...
function generateErsterWeltkriegVerlauf(seed: number = 803): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Was passierte mit der deutschen Monarchie 1918?", "Kaiser Wilhelm II. dankte ab und floh ins Exil", ["Er blieb Kaiser mit mehr Macht", "Er wurde hingerichtet", "Er wurde zum Präsidenten gewählt"]],
    ...
    ["Was war der 'Friede von Brest-Litowsk' (1918)?", "Der Friedensvertrag zwischen Deutschland und Russland 1918", ["Das Ende des Krieges im Westen", "Ein Vertrag mit den USA", "Ein Handelsabkommen"]],
    ...
    ["Was war die 'Diktatur der OHL'?", "Die faktische Herrschaft der Heerführer Hindenburg und Ludendorff", ["Die Macht des Kaisers allein", "Die Herrschaft des Parlaments", "Ein Gesetz der SPD"]],
    ...
    ["Welches Gebiet musste Deutschland nach 1918 an Frankreich zurückgeben?", "Elsass-Lothringen", ["Bayern", "Sachsen", "Schlesien", "Ostpreußen"]]
  ];
...
function generateNSMachtuebernahme(seed: number = 813): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Was geschah im 'Röhm-Putsch' 1934?", "Die Ausschaltung der SA-Führung durch die SS", ["Ein Aufstand der Arbeiter", "Die Wahl des Kaisers", "Die Gründung der Hitlerjugend"]],
...
function generateNSPropagandaAlltag(seed: number = 815): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Was passierte bei den Bücherverbrennungen 1933?", "Schriften jüdischer, marxistischer und pazifistischer Autoren wurden verbrannt", ["Alle Schulbücher wurden verbrannt", "Nur Kochbücher wurden verbrannt", "Nichts passierte"]],
    ...
    ["Was war das Ziel der Erziehung im NS?", "Abhärtung, Gehorsam und ideologische Schulung", ["Kritisches Denken", "Künstlerische Freiheit", "Friedensliebe"]],
...
function generateHolocaust(seed: number = 816): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Was geschah in den Ghettos (z.B. Warschau)?", "Juden wurden dort auf engstem Raum eingesperrt und hungerten", ["Dort gab es Luxushotels", "Es waren Ferienorte", "Sie dienten der Bildung"]],
    ...
    ["Was geschah bei der 'Selektion' an der Rampe?", "Arbeitsfähige wurden von Kindern, Alten und Kranken getrennt", ["Es wurden Geschenke verteilt", "Niemand wurde getrennt", "Man wählte eine Partei"]],
...
function generateZweiterWeltkrieg(seed: number = 817): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Wer waren die 'Alliierten'?", "Großbritannien, USA, Sowjetunion (und Frankreich)", ["Deutschland und Italien", "Japan und China", "Österreich und Ungarn"]],
    ["Was versteht man unter 'Blitzkrieg'?", "Schnelle Siege durch massiven Einsatz von Panzern und Flugzeugen", ["Krieg mit Taschenlampen", "Ein Krieg, der nur bei Gewitter stattfand", "Ein sehr langsamer Krieg"]],
    ...
    ["Was war der 'Hitler-Stalin-Pakt'?", "Ein Nichtangriffspakt zwischen Deutschland und der Sowjetunion 1939", ["Ein Handelsvertrag über Öl", "Die Aufteilung Amerikas", "Ein Sportbündnis"]],
    ...
    ["Wie nannte man die Luftschlacht um England?", "Battle of Britain", ["Materialschlacht", "D-Day", "Blitzkrieg"]],
...
function generateWiderstandNS(seed: number = 818): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Warum scheiterte das Attentat von Stauffenberg?", "Die Bombe stand hinter einem schweren Tischbein und Hitler überlebte", ["Hitler war nicht da", "Die Bombe explodierte nicht", "Stauffenberg wurde vorher gefasst"]],
    ...
    ["Welche religiöse Gruppe verweigerte konsequent den Hitlergruß und den Kriegsdienst?", "Zeugen Jehovas", ["Katholiken allgemein", "Protestanten allgemein", "Heilsarmee"]],
    ...
    ["Wer war Dietrich Bonhoeffer?", "Ein evangelischer Theologe und Widerstandskämpfer", ["Ein katholischer Papst", "Ein General der Wehrmacht", "Ein Erfinder"]],
    ...
    ["Was war die 'Rote Kapelle'?", "Ein Spionagenetzwerk und Widerstandsgruppe gegen den NS", ["Ein Musikchor", "Ein Zirkus", "Ein Kloster"]],
    ["Warum war Widerstand im NS-Staat extrem lebensgefährlich?", "Wegen der totalen Überwachung durch die Gestapo und grausamer Strafen", ["Wegen schlechten Wetters", "Wegen der Steuern", "Wegen der vielen Autos"]],
    ["Was forderte die 'Weiße Rose' in ihren Flugblättern?", "Freiheit, Wahrheit und ein Ende des Krieges", ["Die Weltherrschaft Deutschlands", "Mehr Macht für Hitler", "Die Vernichtung Englands"]],
    ["Was passierte nach dem Scheitern des 20. Juli?", "Tausende wurden verhaftet und hingerichtet (Sippenhaft)", ["Hitler trat zurück", "Nichts geschah", "Es gab Wahlen"]],
    ...
    ["Gab es während des Krieges einen großen Volksaufstand gegen Hitler?", "Nein, der Terror und die Propaganda verhinderten dies weitgehend", ["Ja, 1944 revoltierte das ganze Volk", "Ja, in Berlin gab es tägliche Demos", "Nur in Bayern"]]
  ];
...
function generateWiedervereinigung(seed: number = 821): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Was passierte am 1. Juli 1990 (Wirtschafts-, Währungs- und Sozialunion)?", "Die D-Mark wurde in der DDR eingeführt", ["Die DDR wurde abgeschafft", "Die Mauer wurde gebaut", "Gar nichts"]],
...
function generateEuropaEinigung(seed: number = 822): CurriculumQuestion[] {
  ...
  const mcqPool: [string, string, string[]][] = [
    ...
    ["Was symbolisieren die Sterne auf der Flagge?", "Einheit, Solidarität und Harmonie", ["Die Anzahl der Länder", "Die Anzahl der Flüsse", "Die Anzahl der Könige"]],
    ["Welche Rolle hat das Europäische Parlament?", "Vertretung der Bürger und Gesetzgebung", ["Oberbefehl über das Heer", "Verwaltung der Steuern allein", "Gar keine"]],
    ["In welcher Stadt tagt das Europäische Parlament meist?", "Straßburg", ["Brüssel", "Bonn", "Den Haag", "London"]],
    ["Was versteht man unter den 'Vier Freiheiten' im EU-Binnenmarkt?", "Freier Verkehr von Waren, Personen, Dienstleistungen und Kapital", ["Freiheit zu rauchen, zu trinken, zu tanzen und zu singen", "Gar nichts"]],
    ["Welches Land trat 2020 als erstes Mitglied aus der EU aus (Brexit)?", "Großbritannien", ["Frankreich", "Italien", "Polen", "Ungarn"]],
    ["Was ist der Europäische Rat?", "Die Versammlung der Staats- und Regierungschefs", ["Ein Gerichtshof", "Eine Bank", "Ein Sportverein"]],
    ...
    ["Was war der Friedensnobelpreis für die EU 2012?", "Ehrung für über 60 Jahre Beitrag zu Frieden und Versöhnung", ["Eine Geldspende für Banken", "Ein Preis für das beste Auto", "Gar nichts"]],
