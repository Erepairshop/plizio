...
import t01 from "@/data/ai/class-8/hu/1.json";
...
export const AI_K8_CURRICULUM: KemiaTheme[] = Array.from({length: 6}, (_, th) => ({
  id: `ai_k8_th${th+1}`, name: `AI Téma ${th+1}`, icon: "🚀", color: "#EF4444",
  subtopics: Array.from({length: 5}, (_, sub) => ({
    id: `ai_k8_t${th*5 + sub + 1}`,
    name: { de: `Thema ${th*5 + sub + 1}`, en: `Topic ${th*5 + sub + 1}`, hu: `Téma ${th*5 + sub + 1}`, ro: `Tema ${th*5 + sub + 1}` },
    questions: jsonToQuestions(`ai_k8_t${th*5 + sub + 1}`, e(t01)),
    hasGenerator: false
  }))
}));

export function getAIK8Questions(subtopicIds: string[], count?: number): KemiaQuestion[] {
  const pool: KemiaQuestion[] = [];
  for (const theme of AI_K8_CURRICULUM) {
