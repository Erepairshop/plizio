...
export const AI_K8_CURRICULUM: KemiaTheme[] = Array.from({length: 9}, (_, th) => {
  const i = th + 1;
  const poolData = generateAiPool(8, i);
  return {
    id: `ai_k8_th${i}`,
    ...
    subtopics: Array.from({length: 15}, (_, sub) => {
      ...
      return {
        ...
        questions: poolData.pool[sub].quiz ? [{
          type: "mcq",
          topic: "ai",
          subtopic: `ai_k8_t${i}_${topicIndex}`,
