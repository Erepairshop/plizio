import type { POI } from "./poi";

export const poiExtraJamaicaHistoryV2: POI[] = [
  {
    id: "seville-la-nueva-ruins-history-v2",
    type: "historical",
    parent: "JM-08",
    coords: [-77.2163, 18.4281],
    name: {
      de: "Ruinen von Sevilla la Nueva",
      hu: "Sevilla la Nueva romjai",
      ro: "Ruinele Sevilla la Nueva",
      en: "Seville la Nueva Ruins",
    },
    description: {
      de: "Die Überreste der ersten spanischen Hauptstadt auf Jamaika, die Anfang des 16. Jahrhunderts gegründet wurde. Die Stätte gibt Einblicke in die frühe Kolonialzeit und die Begegnung mit den Taíno.",
      hu: "Jamaica első spanyol fővárosának maradványai, amelyet a 16. század elején alapítottak. A helyszín bepillantást enged a korai gyarmati időszakba és a taíno néppel való találkozásba.",
      ro: "Rămășițele primei capitale spaniole din Jamaica, fondată la începutul secolului al XVI-lea. Situl oferă o perspectivă asupra perioadei coloniale timpurii și a întâlnirii cu poporul Taíno.",
      en: "The remains of the first Spanish capital in Jamaica, founded in the early 16th century. The site offers insights into the early colonial period and the encounter with the Taíno people.",
    },
    facts: {
      de: ["Gegründet um 1509 von Juan de Esquivel.", "Eine der frühesten europäischen Siedlungen in der Neuen Welt.", "Die Stätte enthält spanische und Taíno-Artefakte.", "Wurde später zugunsten von Spanish Town aufgegeben."],
      hu: ["Kb. 1509-ben alapította Juan de Esquivel.", "Az Újvilág egyik legkorábbi európai települése.", "A helyszín spanyol és taíno leleteket is tartalmaz.", "Később elhagyták Spanish Town javára."],
      ro: ["Fondată în jurul anului 1509 de Juan de Esquivel.", "Una dintre cele mai vechi așezări europene din Lumea Nouă.", "Situl conține artefacte spaniole și Taíno.", "A fost abandonată ulterior în favoarea Spanish Town."],
      en: ["Founded around 1509 by Juan de Esquivel.", "One of the earliest European settlements in the New World.", "The site contains Spanish and Taíno artifacts.", "Was later abandoned in favor of Spanish Town."],
    },
  },
  {
    id: "taino-middens-white-marl-history-v2",
    type: "historical",
    parent: "JM-13",
    coords: [-77.01, 17.9],
    name: {
      de: "Taíno-Abfallhaufen bei White Marl",
      hu: "Taíno szemétdombok White Marlnál",
      ro: "Gropile de gunoi Taíno de la White Marl",
      en: "Taíno Middens at White Marl",
    },
    description: {
      de: "Eine bedeutende archäologische Stätte, die aus großen Haufen von Muschelschalen, Töpferwaren und anderen Artefakten besteht und das Leben der indigenen Taíno-Bevölkerung vor Kolumbus dokumentiert.",
      hu: "Jelentős régészeti lelőhely, amely nagy kagylóhéj-, kerámia- és egyéb lelethalmokból áll, dokumentálva a Kolumbusz előtti őslakos taíno lakosság életét.",
      ro: "Un sit arheologic important format din mormane mari de scoici, ceramică și alte artefacte, documentând viața populației indigene Taíno pre-columbiene.",
      en: "A significant archaeological site consisting of large mounds of shells, pottery, and other artifacts, documenting the life of the indigenous Taíno people before Columbus.",
    },
    facts: {
      de: ["Wichtige Quelle für das Verständnis der Taíno-Kultur.", "Enthält Überreste von Mahlzeiten und zerbrochenen Werkzeugen.", "Zeigt die Abhängigkeit der Taíno von Meeresressourcen.", "Ein Museum vor Ort zeigt viele der Funde."],
      hu: ["A taíno kultúra megértésének fontos forrása.", "Ételmaradványokat és törött szerszámokat tartalmaz.", "Bemutatja a taínók tengeri erőforrásoktól való függőségét.", "Egy helyi múzeum mutatja be a leletek nagy részét."],
      ro: ["Sursă importantă pentru înțelegerea culturii Taíno.", "Conține resturi de mese și unelte sparte.", "Demonstrează dependența poporului Taíno de resursele marine.", "Un muzeu local expune multe dintre descoperiri."],
      en: ["Important source for understanding Taíno culture.", "Contains remains of meals and broken tools.", "Shows the Taíno's dependence on marine resources.", "A local museum displays many of the findings."],
    },
  },
  {
    id: "nanny-town-ruins-history-v2",
    type: "historical",
    parent: "JM-10",
    coords: [-76.602, 18.093],
    name: {
      de: "Ruinen von Nanny Town",
      hu: "Nanny Town romjai",
      ro: "Ruinele Nanny Town",
      en: "Nanny Town Ruins",
    },
    description: {
      de: "Die legendäre Siedlung der Windward Maroons, angeführt von der Nationalheldin Nanny. Die schwer zugängliche Stätte in den Blue Mountains war ein Symbol des Widerstands gegen die britische Kolonialmacht.",
      hu: "A Windward Maroons legendás települése, amelyet Nanny nemzeti hősnő vezetett. A Blue Mountains-ben nehezen megközelíthető hely a brit gyarmati uralom elleni ellenállás szimbóluma volt.",
      ro: "Așezarea legendară a maronilor Windward, condusă de eroina națională Nanny. Locul greu accesibil din Munții Albaștri a fost un simbol al rezistenței împotriva puterii coloniale britanice.",
      en: "The legendary settlement of the Windward Maroons, led by National Heroine Nanny. The hard-to-reach site in the Blue Mountains was a symbol of resistance against British colonial rule.",
    },
    facts: {
      de: ["Gegründet im 18. Jahrhundert.", "Mehrfach von den Briten angegriffen und zerstört.", "Nanny of the Maroons ist eine jamaikanische Nationalheldin.", "Die genaue Lage war lange Zeit ein Geheimnis."],
      hu: ["A 18. században alapították.", "A britek többször megtámadták és elpusztították.", "Nanny a maroonok közül Jamaica nemzeti hőse.", "Pontos helye sokáig titok volt."],
      ro: ["Fondată în secolul al XVIII-lea.", "Atacată și distrusă de mai multe ori de britanici.", "Nanny a Maronilor este o eroină națională jamaicană.", "Locația exactă a fost un secret pentru mult timp."],
      en: ["Founded in the 18th century.", "Attacked and destroyed multiple times by the British.", "Nanny of the Maroons is a Jamaican National Heroine.", "The exact location was a secret for a long time."],
    },
  },
  {
    id: "morant-bay-courthouse-history-v2",
    type: "historical",
    parent: "JM-04",
    coords: [-76.4061, 17.8817],
    name: {
      de: "Gerichtsgebäude von Morant Bay",
      hu: "Morant Bay-i bíróság",
      ro: "Tribunalul din Morant Bay",
      en: "Morant Bay Courthouse",
    },
    description: {
      de: "Der historische Ort des Morant Bay Aufstands von 1865, eines entscheidenden Ereignisses in der jamaikanischen Geschichte, das zu bedeutenden politischen Veränderungen führte. Das Gebäude ist ein Symbol des Kampfes für Gerechtigkeit.",
      hu: "Az 1865-ös Morant Bay-i lázadás történelmi helyszíne, amely Jamaica történelmének meghatározó eseménye volt és jelentős politikai változásokhoz vezetett. Az épület az igazságért folytatott harc szimbóluma.",
      ro: "Locul istoric al Rebeliunii din Morant Bay din 1865, un eveniment crucial în istoria Jamaicăi care a dus la schimbări politice semnificative. Clădirea este un simbol al luptei pentru justiție.",
      en: "The historic site of the 1865 Morant Bay Rebellion, a pivotal event in Jamaican history that led to significant political changes. The building is a symbol of the struggle for justice.",
    },
    facts: {
      de: ["Anführer des Aufstands war der Nationalheld Paul Bogle.", "Der Aufstand führte zur Abschaffung der alten Hausversammlung.", "Das ursprüngliche Gebäude wurde durch einen Brand zerstört und wieder aufgebaut.", "Eine Statue von Paul Bogle steht vor dem Gebäude."],
      hu: ["A felkelés vezetője Paul Bogle nemzeti hős volt.", "A lázadás a régi képviselőház eltörléséhez vezetett.", "Az eredeti épület leégett, de újjáépítették.", "Paul Bogle szobra áll az épület előtt."],
      ro: ["Liderul rebeliunii a fost eroul național Paul Bogle.", "Rebeliunea a dus la abolirea vechii Case a Adunării.", "Clădirea originală a fost distrusă de un incendiu și reconstruită.", "O statuie a lui Paul Bogle se află în fața clădirii."],
      en: ["The leader of the rebellion was National Hero Paul Bogle.", "The rebellion led to the abolition of the old House of Assembly.", "The original building was destroyed by fire and rebuilt.", "A statue of Paul Bogle stands in front of the building."],
    },
  },
  {
    id: "halse-hall-great-house-history-v2",
    type: "historical",
    parent: "JM-13",
    coords: [-77.2625, 17.9372],
    name: {
      de: "Halse Hall Great House",
      hu: "Halse Hall udvarház",
      ro: "Conacul Halse Hall",
      en: "Halse Hall Great House",
    },
    description: {
      de: "Eines der ältesten Herrenhäuser Jamaikas, dessen Ursprünge bis ins späte 17. Jahrhundert zurückreichen. Es wurde auf dem Land errichtet, das einem Major der britischen Invasionsarmee von 1655 gewährt wurde.",
      hu: "Jamaica egyik legrégebbi udvarháza, amelynek eredete a 17. század végére nyúlik vissza. Az 1655-ös brit inváziós hadsereg egyik őrnagyának adományozott földön épült.",
      ro: "Unul dintre cele mai vechi conace din Jamaica, cu origini datând de la sfârșitul secolului al XVII-lea. A fost construit pe un teren acordat unui maior din armata de invazie britanică din 1655.",
      en: "One of the oldest great houses in Jamaica, with origins dating back to the late 17th century. It was built on land granted to a major in the 1655 British invasion army.",
    },
    facts: {
      de: ["Ursprünglich eine Zuckerplantage.", "Hat im Laufe der Jahrhunderte viele architektonische Veränderungen erfahren.", "Heute im Besitz eines Bauxitunternehmens.", "Ein Beispiel für frühe Kolonialarchitektur."],
      hu: ["Eredetileg cukornádültetvény volt.", "Az évszázadok során számos építészeti változáson ment keresztül.", "Ma egy bauxitcég tulajdonában van.", "A korai gyarmati építészet egyik példája."],
      ro: ["Inițial o plantație de zahăr.", "A suferit multe modificări arhitecturale de-a lungul secolurilor.", "Astăzi este deținut de o companie de bauxită.", "Un exemplu de arhitectură colonială timpurie."],
      en: ["Originally a sugar plantation.", "Has undergone many architectural changes over the centuries.", "Today it is owned by a bauxite company.", "An example of early colonial architecture."],
    },
  },
  {
    id: "st-peters-church-port-royal-history-v2",
    type: "historical",
    parent: "JM-01",
    coords: [-76.8408, 17.9358],
    name: {
      de: "St. Peter's Church, Port Royal",
      hu: "Szent Péter-templom, Port Royal",
      ro: "Biserica Sf. Petru, Port Royal",
      en: "St. Peter's Church, Port Royal",
    },
    description: {
      de: "Eine historische anglikanische Kirche, die 1726 erbaut wurde, um die beim Erdbeben von 1692 zerstörte Kirche zu ersetzen. Sie beherbergt das kunstvolle Grab von Lewis Galdy, der das Erdbeben überlebte.",
      hu: "Történelmi anglikán templom, amelyet 1726-ban építettek az 1692-es földrengésben elpusztult templom helyére. Itt található Lewis Galdy díszes sírja, aki túlélte a földrengést.",
      ro: "O biserică anglicană istorică construită în 1726 pentru a înlocui biserica distrusă în cutremurul din 1692. Adăpostește mormântul elaborat al lui Lewis Galdy, care a supraviețuit cutremurului.",
      en: "A historic Anglican church built in 1726 to replace the one destroyed in the 1692 earthquake. It houses the elaborate tomb of Lewis Galdy, who survived the earthquake.",
    },
    facts: {
      de: ["Enthält eine Orgel aus dem 18. Jahrhundert.", "Das silberne Abendmahlsgeschirr wurde angeblich von Henry Morgan gestiftet.", "Galdys Grab erzählt seine wundersame Überlebensgeschichte.", "Eines der wenigen verbliebenen historischen Gebäude in Port Royal."],
      hu: ["Egy 18. századi orgonát tartalmaz.", "Az ezüst úrvacsorai kelyhet állítólag Henry Morgan adományozta.", "Galdy sírja elmeséli csodálatos túlélésének történetét.", "Port Royal kevés megmaradt történelmi épületének egyike."],
      ro: ["Conține o orgă din secolul al XVIII-lea.", "Vesela de împărtășanie din argint ar fi fost donată de Henry Morgan.", "Mormântul lui Galdy povestește supraviețuirea sa miraculoasă.", "Una dintre puținele clădiri istorice rămase în Port Royal."],
      en: ["Contains an 18th-century organ.", "The silver communion set was supposedly donated by Henry Morgan.", "Galdy's tomb tells his miraculous survival story.", "One of the few remaining historic buildings in Port Royal."],
    },
  },
  {
    id: "colbeck-castle-history-v2",
    type: "historical",
    parent: "JM-14",
    coords: [-77.1208, 18.0683],
    name: {
      de: "Colbeck Castle",
      hu: "Colbeck-kastély",
      ro: "Castelul Colbeck",
      en: "Colbeck Castle",
    },
    description: {
      de: "Die imposanten Ruinen eines riesigen Gebäudes aus dem 17. Jahrhundert, das eher einer Festung als einem Wohnhaus glich. Es wurde von Colonel John Colbeck erbaut, einem Offizier in der Armee, die Jamaika eroberte.",
      hu: "Egy hatalmas, 17. századi épület impozáns romjai, amely inkább erődre, mint lakóházra hasonlított. John Colbeck ezredes építtette, aki a Jamaicát meghódító hadsereg tisztje volt.",
      ro: "Ruinele impunătoare ale unei clădiri uriașe din secolul al XVII-lea, care semăna mai mult cu o fortăreață decât cu o reședință. A fost construită de colonelul John Colbeck, un ofițer din armata care a cucerit Jamaica.",
      en: "The imposing ruins of a huge 17th-century building that resembled a fortress more than a residence. It was built by Colonel John Colbeck, an officer in the army that conquered Jamaica.",
    },
    facts: {
      de: ["Eines der größten Gebäude seiner Zeit auf Jamaika.", "Gebaut mit Ziegeln und Steinen in einem symmetrischen Design.", "Es wurde wahrscheinlich nie vollständig fertiggestellt oder bewohnt.", "Die Ruinen sind ein nationales Denkmal."],
      hu: ["Korának egyik legnagyobb épülete volt Jamaicában.", "Téglából és kőből épült, szimmetrikus kialakítással.", "Valószínűleg soha nem fejezték be teljesen, és nem is lakták.", "A romok nemzeti műemlékek."],
      ro: ["Una dintre cele mai mari clădiri din Jamaica la vremea sa.", "Construită din cărămidă și piatră într-un design simetric.", "Probabil nu a fost niciodată complet finalizată sau locuită.", "Ruinele sunt un monument național."],
      en: ["One of the largest buildings of its time in Jamaica.", "Built with bricks and stones in a symmetrical design.", "It was likely never fully completed or inhabited.", "The ruins are a national monument."],
    },
  },
  {
    id: "stewart-castle-ruins-history-v2",
    type: "historical",
    parent: "JM-07",
    coords: [-77.675, 18.461],
    name: {
      de: "Ruinen von Stewart Castle",
      hu: "Stewart-kastély romjai",
      ro: "Ruinele Castelului Stewart",
      en: "Stewart Castle Ruins",
    },
    description: {
      de: "Die Überreste einer befestigten Plantagenresidenz aus der Mitte des 18. Jahrhunderts. Die Burg wurde als Verteidigungsanlage gegen Sklavenaufstände und mögliche Angriffe von See gebaut.",
      hu: "Egy 18. század közepéről származó, erődített ültetvényesi rezidencia maradványai. A kastélyt a rabszolgafelkelések és az esetleges tengeri támadások elleni védelemként építették.",
      ro: "Rămășițele unei reședințe fortificate de pe o plantație de la mijlocul secolului al XVIII-lea. Castelul a fost construit ca o structură defensivă împotriva revoltelor sclavilor și a posibilelor atacuri de pe mare.",
      en: "The remains of a fortified plantation residence from the mid-18th century. The castle was built as a defensive structure against slave rebellions and potential attacks from the sea.",
    },
    facts: {
      de: ["Erbaut von James Stewart, einem lokalen Pflanzer.", "Hatte Schießscharten und eine robuste Konstruktion.", "Ein Beispiel für die defensive Architektur auf Plantagen.", "Die Ruinen sind von Zuckerrohrfeldern umgeben."],
      hu: ["James Stewart, egy helyi ültetvényes építtette.", "Lőrésekkel és masszív szerkezettel rendelkezett.", "Példa az ültetvények védekező építészetére.", "A romokat cukornádültetvények veszik körül."],
      ro: ["Construit de James Stewart, un plantator local.", "Avea ambrazuri și o construcție robustă.", "Un exemplu de arhitectură defensivă pe plantații.", "Ruinele sunt înconjurate de câmpuri de trestie de zahăr."],
      en: ["Built by James Stewart, a local planter.", "Had gun loops and a sturdy construction.", "An example of defensive architecture on plantations.", "The ruins are surrounded by sugarcane fields."],
    },
  },
  {
    id: "good-hope-great-house-history-v2",
    type: "historical",
    parent: "JM-07",
    coords: [-77.604, 18.406],
    name: {
      de: "Good Hope Great House",
      hu: "Good Hope udvarház",
      ro: "Conacul Good Hope",
      en: "Good Hope Great House",
    },
    description: {
      de: "Ein wunderschön restauriertes Herrenhaus aus dem 18. Jahrhundert, das einst das Herz einer wohlhabenden Zuckerplantage war. Es ist ein erstklassiges Beispiel für die georgianische Architektur Jamaikas.",
      hu: "Egy gyönyörűen felújított, 18. századi udvarház, amely egykor egy virágzó cukorültetvény központja volt. Jamaica györgy korabeli építészetének elsőrangú példája.",
      ro: "Un conac frumos restaurat din secolul al XVIII-lea, care a fost odată inima unei plantații de zahăr prospere. Este un exemplu de primă clasă al arhitecturii georgiene din Jamaica.",
      en: "A beautifully restored 18th-century great house that was once the heart of a prosperous sugar plantation. It is a prime example of Jamaican Georgian architecture.",
    },
    facts: {
      de: ["Die Plantage umfasste einst ein ganzes Dorf für die Arbeiter.", "Umgewandelt in ein Boutique-Hotel und einen Abenteuerpark.", "Das Anwesen erstreckt sich über 2000 Hektar.", "Gehörte einst John Tharp, einem der reichsten Pflanzer Jamaikas."],
      hu: ["Az ültetvény egykor egy egész falut foglalt magában a munkások számára.", "Butikhotellé és kalandparkká alakították át.", "A birtok több mint 2000 hektáron terül el.", "Egykor John Tharp, Jamaica egyik leggazdagabb ültetvényesének tulajdonában volt."],
      ro: ["Plantația a inclus odată un întreg sat pentru muncitori.", "Transformat într-un hotel de tip boutique și parc de aventuri.", "Proprietatea se întinde pe 2000 de acri.", "A aparținut odată lui John Tharp, unul dintre cei mai bogați plantatori din Jamaica."],
      en: ["The plantation once included an entire village for workers.", "Converted into a boutique hotel and adventure park.", "The estate covers 2,000 acres.", "Once owned by John Tharp, one of Jamaica's wealthiest planters."],
    },
  },
  {
    id: "fort-haldane-history-v2",
    type: "historical",
    parent: "JM-06",
    coords: [-77.128, 18.468],
    name: {
      de: "Fort Haldane",
      hu: "Haldane-erőd",
      ro: "Fortul Haldane",
      en: "Fort Haldane",
    },
    description: {
      de: "Eine kleine Küstenfestung aus dem 18. Jahrhundert mit Blick auf Port Maria. Es wurde gebaut, um den Hafen vor Piraten und feindlichen Schiffen zu schützen und beherbergte eine kleine Garnison.",
      hu: "Egy kis, 18. századi tengerparti erőd, amely Port Mariára néz. A kalózok és ellenséges hajók elleni védelem céljából épült, és egy kis helyőrségnek adott otthont.",
      ro: "O mică fortăreață de coastă din secolul al XVIII-lea, cu vedere la Port Maria. A fost construită pentru a proteja portul de pirați și nave inamice și a găzduit o mică garnizoană.",
      en: "A small 18th-century coastal fort overlooking Port Maria. It was built to protect the harbor from pirates and enemy ships and housed a small garrison.",
    },
    facts: {
      de: ["Benannt nach General George Haldane, dem damaligen Gouverneur von Jamaika.", "Zwei Kanonen sind noch heute auf dem Gelände zu sehen.", "Bietet einen Panoramablick auf die Bucht von Port Maria.", "In der Nähe befindet sich das 'Firefly', das Haus von Sir Noël Coward."],
      hu: ["George Haldane tábornokról, Jamaica akkori kormányzójáról nevezték el.", "Két ágyú ma is látható a területen.", "Panorámás kilátást nyújt a Port Maria-öbölre.", "A közelben található a 'Firefly', Sir Noël Coward háza."],
      ro: ["Numit după generalul George Haldane, guvernatorul de atunci al Jamaicăi.", "Două tunuri sunt încă vizibile pe teren astăzi.", "Oferă o vedere panoramică a golfului Port Maria.", "În apropiere se află 'Firefly', casa lui Sir Noël Coward."],
      en: ["Named after General George Haldane, then Governor of Jamaica.", "Two cannons are still visible on the site today.", "Offers a panoramic view of Port Maria Bay.", "Nearby is 'Firefly', the home of Sir Noël Coward."],
    },
  },
  {
    id: "edinburgh-castle-ruins-jamaica-history-v2",
    type: "historical",
    parent: "JM-08",
    coords: [-77.108, 18.288],
    name: {
      de: "Ruinen von Edinburgh Castle (Jamaika)",
      hu: "Edinburgh-kastély romjai (Jamaica)",
      ro: "Ruinele Castelului Edinburgh (Jamaica)",
      en: "Edinburgh Castle Ruins (Jamaica)",
    },
    description: {
      de: "Die unheimlichen Ruinen eines Hauses aus dem 18. Jahrhundert, das von Lewis Hutchinson, Jamaikas berüchtigstem Serienmörder, dem 'verrückten Doktor', erbaut wurde. Die Legende besagt, dass er Reisende aus dem Hinterhalt überfiel und in ein Erdloch warf.",
      hu: "Egy 18. századi ház kísérteties romjai, amelyet Lewis Hutchinson, Jamaica leghírhedtebb sorozatgyilkosa, az 'őrült doktor' épített. A legenda szerint lesből támadt az utazókra, és egy verembe dobta őket.",
      ro: "Ruinele sinistre ale unei case din secolul al XVIII-lea, construită de Lewis Hutchinson, cel mai notoriu criminal în serie din Jamaica, 'doctorul nebun'. Legenda spune că acesta le întindea ambuscade călătorilor și îi arunca într-o groapă.",
      en: "The eerie ruins of an 18th-century house built by Lewis Hutchinson, Jamaica's most notorious serial killer, the 'Mad Doctor'. Legend has it that he ambushed travelers and threw them into a sinkhole.",
    },
    facts: {
      de: ["Hutchinson wurde 1773 gehängt.", "Das 'Schloss' war ein zweistöckiges befestigtes Haus.", "Der Ort ist abgelegen und schwer zu erreichen.", "Gilt als einer der am meisten heimgesuchten Orte Jamaikas."],
      hu: ["Hutchinsont 1773-ban felakasztották.", "A 'kastély' egy kétszintes, erődített ház volt.", "A hely eldugott és nehezen megközelíthető.", "Jamaica egyik leginkább kísértetjárta helyének tartják."],
      ro: ["Hutchinson a fost spânzurat în 1773.", "'Castelul' era o casă fortificată cu două etaje.", "Locul este izolat și greu de accesat.", "Considerat unul dintre cele mai bântuite locuri din Jamaica."],
      en: ["Hutchinson was hanged in 1773.", "The 'castle' was a two-story fortified house.", "The site is remote and difficult to access.", "Considered one of the most haunted places in Jamaica."],
    },
  },
  {
    id: "rockfort-mineral-bath-history-v2",
    type: "historical",
    parent: "JM-01",
    coords: [-76.75, 17.971],
    name: {
      de: "Rockfort Mineralbad",
      hu: "Rockfort ásványfürdő",
      ro: "Băile minerale Rockfort",
      en: "Rockfort Mineral Bath",
    },
    description: {
      de: "Ein historisches Mineralbad, das nach dem Erdbeben von 1907 entstand, als eine Mineralquelle aus dem Boden sprudelte. Es befindet sich neben den Ruinen von Fort Rock, einer alten britischen Festung.",
      hu: "Történelmi ásványfürdő, amely az 1907-es földrengés után jött létre, amikor egy ásványvízforrás tört a felszínre. A Fort Rock, egy régi brit erőd romjai mellett található.",
      ro: "O baie minerală istorică care a apărut după cutremurul din 1907, când un izvor mineral a țâșnit din pământ. Este situată lângă ruinele Fortului Rock, o veche fortăreață britanică.",
      en: "A historic mineral bath that emerged after the 1907 earthquake when a mineral spring gushed from the ground. It is located next to the ruins of Fort Rock, an old British fort.",
    },
    facts: {
      de: ["Das Wasser gilt als therapeutisch.", "Fort Rock wurde im 17. Jahrhundert erbaut, um Kingston zu verteidigen.", "Der Komplex umfasst private Bäder und einen Swimmingpool.", "Ein beliebter Ort für Einheimische zur Entspannung."],
      hu: ["A vizet gyógyhatásúnak tartják.", "A Fort Rockot a 17. században építették Kingston védelmére.", "A komplexum privát fürdőket és egy úszómedencét foglal magában.", "A helyiek körében népszerű pihenőhely."],
      ro: ["Apa este considerată terapeutică.", "Fortul Rock a fost construit în secolul al XVII-lea pentru a apăra Kingstonul.", "Complexul include băi private și o piscină.", "Un loc popular pentru relaxare printre localnici."],
      en: ["The water is considered therapeutic.", "Fort Rock was built in the 17th century to defend Kingston.", "The complex includes private baths and a swimming pool.", "A popular spot for locals to relax."],
    },
  },
  {
    id: "old-naval-hospital-port-royal-history-v2",
    type: "historical",
    parent: "JM-01",
    coords: [-76.843, 17.937],
    name: {
      de: "Altes Marinekrankenhaus Port Royal",
      hu: "Régi haditengerészeti kórház, Port Royal",
      ro: "Vechiul Spital Naval din Port Royal",
      en: "Old Naval Hospital, Port Royal",
    },
    description: {
      de: "Ein historisches Gebäude aus dem 19. Jahrhundert, das als eines der ersten Fertigbau-Gebäude der Welt gilt. Die gusseisernen Rahmenelemente wurden in England hergestellt und vor Ort montiert.",
      hu: "Egy 19. századi történelmi épület, amelyet a világ egyik első előregyártott épületeként tartanak számon. Az öntöttvas váz elemeit Angliában gyártották és a helyszínen szerelték össze.",
      ro: "O clădire istorică din secolul al XIX-lea, considerată una dintre primele clădiri prefabricate din lume. Elementele cadrului din fontă au fost fabricate în Anglia și asamblate la fața locului.",
      en: "A historic 19th-century building, considered one of the world's first pre-fabricated buildings. The cast-iron frame elements were made in England and assembled on site.",
    },
    facts: {
      de: ["Erbaut um 1819.", "Diente zur Versorgung von Seeleuten der Royal Navy.", "Die Struktur hat mehreren Hurrikanen und Erdbeben standgehalten.", "Heute ein nationales Denkmal und Forschungszentrum."],
      hu: ["Kb. 1819-ben épült.", "A Királyi Haditengerészet tengerészeinek ellátására szolgált.", "A szerkezet több hurrikánnak és földrengésnek is ellenállt.", "Ma nemzeti műemlék és kutatóközpont."],
      ro: ["Construit în jurul anului 1819.", "A servit la îngrijirea marinarilor din Marina Regală.", "Structura a rezistat mai multor uragane și cutremure.", "Astăzi este un monument național și un centru de cercetare."],
      en: ["Built around 1819.", "Served to care for Royal Navy sailors.", "The structure has withstood several hurricanes and earthquakes.", "Today it is a national monument and research center."],
    },
  },
  {
    id: "rodney-memorial-spanish-town-history-v2",
    type: "historical",
    parent: "JM-14",
    coords: [-76.953, 17.995],
    name: {
      de: "Rodney-Denkmal, Spanish Town",
      hu: "Rodney-emlékmű, Spanish Town",
      ro: "Memorialul Rodney, Spanish Town",
      en: "Rodney Memorial, Spanish Town",
    },
    description: {
      de: "Ein Denkmal in einem tempelartigen Gebäude am Spanish Town Square, das Admiral Lord Rodney ehrt, der die britische Flotte 1782 zum Sieg über die Franzosen in der Schlacht von Les Saintes führte.",
      hu: "Egy templomszerű épületben található emlékmű a Spanish Town téren, amely Lord Rodney admirálist tiszteli, aki 1782-ben a Les Saintes-i csatában győzelemre vezette a brit flottát a franciák felett.",
      ro: "Un memorial într-o clădire asemănătoare unui templu din Piața Spanish Town, în onoarea amiralului Lord Rodney, care a condus flota britanică la victorie împotriva francezilor în Bătălia de la Saintes în 1782.",
      en: "A memorial in a temple-like building in Spanish Town Square, honoring Admiral Lord Rodney who led the British fleet to victory over the French at the Battle of the Saintes in 1782.",
    },
    facts: {
      de: ["Die Statue wurde vom berühmten Bildhauer John Bacon geschaffen.", "Die Kanonen neben dem Denkmal stammen aus der Schlacht.", "Das Denkmal war umstritten, da Rodney ein Verfechter der Sklaverei war.", "Teil des historischen Ensembles des Platzes."],
      hu: ["A szobrot a híres szobrász, John Bacon készítette.", "Az emlékmű melletti ágyúk a csatából származnak.", "Az emlékmű ellentmondásos volt, mivel Rodney a rabszolgaság szószólója volt.", "A tér történelmi együttesének része."],
      ro: ["Statuia a fost creată de faimosul sculptor John Bacon.", "Tunurile de lângă memorial provin din bătălie.", "Memorialul a fost controversat, deoarece Rodney era un susținător al sclaviei.", "Parte a ansamblului istoric al pieței."],
      en: ["The statue was created by the famous sculptor John Bacon.", "The cannons beside the memorial are from the battle.", "The memorial was controversial as Rodney was a proponent of slavery.", "Part of the square's historic ensemble."],
    },
  },
  {
    id: "hibbert-house-kingston-history-v2",
    type: "historical",
    parent: "JM-01",
    coords: [-76.792, 17.973],
    name: {
      de: "Hibbert House, Kingston",
      hu: "Hibbert-ház, Kingston",
      ro: "Casa Hibbert, Kingston",
      en: "Hibbert House, Kingston",
    },
    description: {
      de: "Ein elegantes georgianisches Stadthaus aus dem Jahr 1755, das als Hauptquartier der jamaikanischen Nationalen Erb-Stiftung dient. Es wurde von einem wohlhabenden Kaufmann erbaut und war einst Sitz der Legislative Jamaikas.",
      hu: "Egy elegáns, 1755-ben épült György-korabeli városi ház, amely a Jamaica-i Nemzeti Örökségvédelmi Alapítvány székhelyeként szolgál. Egy gazdag kereskedő építtette, és egykor Jamaica törvényhozásának székhelye volt.",
      ro: "O casă elegantă georgiană din 1755, care servește drept sediu al Trustului Patrimoniului Național din Jamaica. A fost construită de un negustor bogat și a fost odată sediul legislativului din Jamaica.",
      en: "An elegant Georgian townhouse from 1755, which serves as the headquarters of the Jamaica National Heritage Trust. It was built by a wealthy merchant and was once the seat of Jamaica's legislature.",
    },
    facts: {
      de: ["Auch bekannt als Headquarters House.", "Ein herausragendes Beispiel für jamaikanische georgianische Architektur.", "Thomas Hibbert, der Erbauer, war ein führender Sklavenhändler.", "Befindet sich in der Duke Street, einer historischen Straße in Kingston."],
      hu: ["Headquarters House néven is ismert.", "A jamaicai György-korabeli építészet kiemelkedő példája.", "Thomas Hibbert, az építtető, vezető rabszolga-kereskedő volt.", "A Duke Streeten található, Kingston egyik történelmi utcájában."],
      ro: ["Cunoscută și sub numele de Casa Cartierului General.", "Un exemplu remarcabil de arhitectură georgiană jamaicană.", "Thomas Hibbert, constructorul, a fost un important comerciant de sclavi.", "Situată pe strada Duke, o stradă istorică din Kingston."],
      en: ["Also known as Headquarters House.", "An outstanding example of Jamaican Georgian architecture.", "Thomas Hibbert, the builder, was a leading slave trader.", "Located on Duke Street, a historic street in Kingston."],
    },
  },
  {
    id: "old-iron-bridge-spanish-town-history-v2",
    type: "historical",
    parent: "JM-14",
    coords: [-76.958, 18.001],
    name: {
      de: "Alte Eisenbrücke, Spanish Town",
      hu: "Régi vashíd, Spanish Town",
      ro: "Vechiul pod de fier, Spanish Town",
      en: "Old Iron Bridge, Spanish Town",
    },
    description: {
      de: "Eine der ersten Gusseisenbrücken der Welt, die 1801 über den Rio Cobre errichtet wurde. Sie wurde in England entworfen und in Teilen nach Jamaika verschifft, um vor Ort montiert zu werden.",
      hu: "A világ egyik első öntöttvas hídja, amelyet 1801-ben építettek a Rio Cobre folyón. Angliában tervezték, és darabokban szállították Jamaicába, hogy a helyszínen szereljék össze.",
      ro: "Unul dintre primele poduri din fontă din lume, ridicat peste râul Rio Cobre în 1801. A fost proiectat în Anglia și expediat în piese în Jamaica pentru a fi asamblat la fața locului.",
      en: "One of the first cast-iron bridges in the world, erected over the Rio Cobre in 1801. It was designed in England and shipped in parts to Jamaica for assembly.",
    },
    facts: {
      de: ["Ein Meisterwerk der frühen Industriearchitektur.", "Heute nur noch für Fußgänger zugänglich.", "Steht auf der Beobachtungsliste des World Monuments Fund.", "Ein Symbol für die Ingenieurskunst des frühen 19. Jahrhunderts."],
      hu: ["A korai ipari építészet remekműve.", "Ma már csak gyalogosok számára használható.", "A World Monuments Fund figyelőlistáján szerepel.", "A 19. század eleji mérnöki tudomány szimbóluma."],
      ro: ["O capodoperă a arhitecturii industriale timpurii.", "Astăzi este deschis doar pentru pietoni.", "Se află pe lista de supraveghere a World Monuments Fund.", "Un simbol al ingineriei de la începutul secolului al XIX-lea."],
      en: ["A masterpiece of early industrial architecture.", "Today it is only open to pedestrians.", "It is on the World Monuments Fund watch list.", "A symbol of early 19th-century engineering."],
    },
  },
  {
    id: "fort-augusta-history-v2",
    type: "historical",
    parent: "JM-14",
    coords: [-76.866, 17.961],
    name: {
      de: "Fort Augusta",
      hu: "Augusta-erőd",
      ro: "Fortul Augusta",
      en: "Fort Augusta",
    },
    description: {
      de: "Eine große Festung aus dem 18. Jahrhundert, die an der Mündung des Hafens von Kingston erbaut wurde, um die Stadt vor Seeangriffen zu schützen. Sie war eine der wichtigsten militärischen Anlagen der Insel.",
      hu: "Egy nagy, 18. századi erőd, amelyet a kingstoni kikötő torkolatánál építettek a város tengeri támadások elleni védelmére. A sziget egyik legfontosabb katonai létesítménye volt.",
      ro: "O mare fortăreață din secolul al XVIII-lea, construită la gura portului Kingston pentru a proteja orașul de atacurile navale. A fost una dintre cele mai importante instalații militare de pe insulă.",
      en: "A large 18th-century fortress built at the mouth of Kingston Harbour to protect the city from naval attacks. It was one of the most important military installations on the island.",
    },
    facts: {
      de: ["Der Bau begann in den 1740er Jahren.", "Wurde später als Gefängnis genutzt.", "Hatte über 80 Kanonen.", "Benannt nach der Mutter von König Georg III."],
      hu: ["Az építkezés az 1740-es években kezdődött.", "Később börtönként használták.", "Több mint 80 ágyúja volt.", "III. György király anyjáról nevezték el."],
      ro: ["Construcția a început în anii 1740.", "A fost folosit ulterior ca închisoare.", "Avea peste 80 de tunuri.", "Numit după mama regelui George al III-lea."],
      en: ["Construction began in the 1740s.", "Was later used as a prison.", "Had over 80 cannons.", "Named after the mother of King George III."],
    },
  },
  {
    id: "falmouth-historic-courthouse-history-v2",
    type: "historical",
    parent: "JM-07",
    coords: [-77.658, 18.494],
    name: {
      de: "Historisches Gerichtsgebäude von Falmouth",
      hu: "Falmouth-i történelmi bíróság",
      ro: "Tribunalul istoric din Falmouth",
      en: "Falmouth Historic Courthouse",
    },
    description: {
      de: "Ein prächtiges Gerichtsgebäude aus dem Jahr 1815, das die georgianische Pracht von Falmouth zur Zeit seines Wohlstands als Zuckerhafen widerspiegelt. Das Gebäude ist ein zentrales Element des historischen Stadtbildes.",
      hu: "Egy csodálatos, 1815-ben épült bírósági épület, amely Falmouth György-korabeli pompáját tükrözi a cukorkikötőként való virágzása idején. Az épület a történelmi városkép központi eleme.",
      ro: "O clădire magnifică a tribunalului din 1815, care reflectă splendoarea georgiană a Falmouth-ului în timpul prosperității sale ca port de zahăr. Clădirea este un element central al peisajului urban istoric.",
      en: "A magnificent courthouse from 1815, reflecting the Georgian splendor of Falmouth during its prosperity as a sugar port. The building is a central feature of the historic townscape.",
    },
    facts: {
      de: ["Ein Brand im Jahr 1926 zerstörte das Original, es wurde aber originalgetreu wieder aufgebaut.", "Zeugt vom ehemaligen Reichtum der Stadt.", "Es ist immer noch ein funktionierendes Gerichtsgebäude.", "Ein hervorragendes Beispiel für jamaikanische georgianische öffentliche Architektur."],
      hu: ["Az 1926-os tűzvész elpusztította az eredetit, de hűen újjáépítették.", "A város egykori gazdagságáról tanúskodik.", "Ma is működő bírósági épület.", "A jamaicai György-korabeli középítészet kiváló példája."],
      ro: ["Un incendiu din 1926 a distrus originalul, dar a fost reconstruit cu fidelitate.", "Mărturie a bogăției de odinioară a orașului.", "Este încă o clădire de tribunal funcțională.", "Un exemplu superb de arhitectură publică georgiană jamaicană."],
      en: ["A fire in 1926 destroyed the original, but it was faithfully rebuilt.", "Testament to the former wealth of the town.", "It is still a functioning courthouse.", "A superb example of Jamaican Georgian public architecture."],
    },
  },
  {
    id: "liberty-hall-legacy-of-marcus-garvey-history-v2",
    type: "historical",
    parent: "JM-01",
    coords: [-76.787, 17.979],
    name: {
      de: "Liberty Hall: Das Erbe von Marcus Garvey",
      hu: "Liberty Hall: Marcus Garvey öröksége",
      ro: "Liberty Hall: Moștenirea lui Marcus Garvey",
      en: "Liberty Hall: The Legacy of Marcus Garvey",
    },
    description: {
      de: "Das ehemalige Hauptquartier der Universal Negro Improvement Association (UNIA) in Kingston. Es ist ein Museum, das dem Leben und Werk des jamaikanischen Nationalhelden Marcus Garvey gewidmet ist.",
      hu: "Az Universal Negro Improvement Association (UNIA) egykori kingstoni székhelye. Ez egy múzeum, amelyet Marcus Garvey jamaicai nemzeti hős életének és munkásságának szenteltek.",
      ro: "Fostul sediu al Asociației Universale pentru Îmbunătățirea Situației Negrilor (UNIA) din Kingston. Este un muzeu dedicat vieții și operei eroului național jamaican Marcus Garvey.",
      en: "The former headquarters of the Universal Negro Improvement Association (UNIA) in Kingston. It is a museum dedicated to the life and work of Jamaican National Hero Marcus Garvey.",
    },
    facts: {
      de: ["Garvey war ein einflussreicher panafrikanischer Führer.", "Die Halle war ein Zentrum für politische und kulturelle Aktivitäten.", "Das Museum bietet Bildungsprogramme an.", "Garveys Lehren beeinflussten Bewegungen weltweit."],
      hu: ["Garvey befolyásos pán-afrikai vezető volt.", "A csarnok politikai és kulturális tevékenységek központja volt.", "A múzeum oktatási programokat kínál.", "Garvey tanításai világszerte befolyásoltak mozgalmakat."],
      ro: ["Garvey a fost un lider panafrican influent.", "Sala a fost un centru pentru activități politice și culturale.", "Muzeul oferă programe educaționale.", "Învățăturile lui Garvey au influențat mișcări din întreaga lume."],
      en: ["Garvey was an influential Pan-African leader.", "The hall was a center for political and cultural activities.", "The museum offers educational programs.", "Garvey's teachings influenced movements worldwide."],
    },
  },
  {
    id: "blenheim-birthplace-alexander-bustamante-history-v2",
    type: "historical",
    parent: "JM-09",
    coords: [-77.943, 18.359],
    name: {
      de: "Blenheim, Geburtsort von Alexander Bustamante",
      hu: "Blenheim, Alexander Bustamante szülőhelye",
      ro: "Blenheim, locul de naștere al lui Alexander Bustamante",
      en: "Blenheim, Birthplace of Alexander Bustamante",
    },
    description: {
      de: "Die bescheidene Stätte in Hanover, wo Sir Alexander Bustamante, der erste Premierminister Jamaikas und Nationalheld, geboren wurde. Eine Nachbildung seines Geburtshauses steht heute dort.",
      hu: "Szerény hely Hanoverben, ahol Sir Alexander Bustamante, Jamaica első miniszterelnöke és nemzeti hőse született. Szülőházának másolata ma ott áll.",
      ro: "Situl modest din Hanover unde s-a născut Sir Alexander Bustamante, primul prim-ministru al Jamaicăi și erou național. O replică a casei sale natale se află astăzi acolo.",
      en: "The humble site in Hanover where Sir Alexander Bustamante, the first Prime Minister of Jamaica and a National Hero, was born. A replica of his birthplace stands there today.",
    },
    facts: {
      de: ["Bustamante war ein führender Arbeiterführer.", "Gründer der Jamaica Labour Party (JLP).", "Führte Jamaika 1962 in die Unabhängigkeit.", "Die Stätte ist ein nationales Denkmal."],
      hu: ["Bustamante vezető munkásvezér volt.", "A Jamaica Labour Party (JLP) alapítója.", "1962-ben vezette Jamaicát a függetlenséghez.", "A helyszín nemzeti emlékmű."],
      ro: ["Bustamante a fost un lider muncitoresc de frunte.", "Fondator al Partidului Laburist din Jamaica (JLP).", "A condus Jamaica la independență în 1962.", "Situl este un monument național."],
      en: ["Bustamante was a leading labour leader.", "Founder of the Jamaica Labour Party (JLP).", "Led Jamaica to independence in 1962.", "The site is a national monument."],
    },
  },
  {
    id: "lover-s-leap-history-v2",
    type: "historical",
    parent: "JM-11",
    coords: [-77.654, 17.873],
    name: {
      de: "Lover's Leap",
      hu: "Szerelmesek ugrása",
      ro: "Saltul Iubiților",
      en: "Lover's Leap",
    },
    description: {
      de: "Eine dramatische Klippe mit einer Höhe von 520 Metern, die nach der Legende zweier Sklavenliebender benannt ist, die lieber in den Tod sprangen, als getrennt zu werden. Die Stätte hat auch eine historische Bedeutung als Navigationspunkt.",
      hu: "Egy drámai, 520 méter magas szikla, amely két rabszolgaszerelmes legendájáról kapta a nevét, akik inkább a halálba ugrottak, minthogy elválasszák őket egymástól. A hely történelmi jelentőséggel is bír mint navigációs pont.",
      ro: "O stâncă dramatică de 520 de metri, numită după legenda a doi iubiți sclavi care au sărit în gol în loc să fie despărțiți. Locul are și o importanță istorică ca punct de navigație.",
      en: "A dramatic 520-meter cliff named after the legend of two slave lovers who chose to jump to their deaths rather than be separated. The site also has historical importance as a navigation point.",
    },
    facts: {
      de: ["Die Legende ist eine beliebte jamaikanische Folklore.", "Ein Leuchtturm wurde 1954 in der Nähe gebaut.", "Bietet eine atemberaubende Aussicht auf die Küste.", "Der Leuchtturm ist der höchste des westlichen Hemispheres."],
      hu: ["A legenda egy népszerű jamaicai népmese.", "1954-ben világítótornyot építettek a közelben.", "Lélegzetelállító kilátást nyújt a tengerpartra.", "A világítótorony a nyugati félteke legmagasabbja."],
      ro: ["Legenda este o poveste populară din folclorul jamaican.", "Un far a fost construit în apropiere în 1954.", "Oferă o priveliște uluitoare a coastei.", "Farul este cel mai înalt din emisfera vestică."],
      en: ["The legend is a popular Jamaican folklore.", "A lighthouse was built nearby in 1954.", "Offers a breathtaking view of the coastline.", "The lighthouse is the highest in the Western Hemisphere."],
    },
  },
  {
    id: "trench-town-culture-yard-history-v2",
    type: "historical",
    parent: "JM-01",
    coords: [-76.809, 17.986],
    name: {
      de: "Trench Town Culture Yard",
      hu: "Trench Town Kulturális Udvar",
      ro: "Curtea Culturală Trench Town",
      en: "Trench Town Culture Yard",
    },
    description: {
      de: "Ein Regierungshof in Trenchtown, der in den 1960er Jahren die Heimat von Bob Marley war. Hier schrieb er einige seiner berühmtesten Lieder. Heute ist es ein Museum und eine Gemeinschaftseinrichtung.",
      hu: "Egy kormányzati udvar Trenchtownban, amely az 1960-as években Bob Marley otthona volt. Itt írta néhány leghíresebb dalát. Ma múzeum és közösségi létesítmény.",
      ro: "O curte guvernamentală din Trenchtown care a fost casa lui Bob Marley în anii 1960. Aici a scris unele dintre cele mai faimoase cântece ale sale. Astăzi este un muzeu și un centru comunitar.",
      en: "A government yard in Trenchtown that was Bob Marley's home in the 1960s. It was here that he wrote some of his most famous songs. Today it is a museum and community facility.",
    },
    facts: {
      de: ["Gilt als die Wiege des Reggae.", "Marleys alter VW-Bus steht noch im Hof.", "Viele andere berühmte Musiker kamen aus Trenchtown.", "Ein nationales Denkmal seit 2006."],
      hu: ["A reggae bölcsőjének tartják.", "Marley régi VW busza még mindig az udvaron áll.", "Sok más híres zenész származott Trenchtownból.", "2006 óta nemzeti emlékmű."],
      ro: ["Considerat leagănul muzicii reggae.", "Vechiul autobuz VW al lui Marley este încă în curte.", "Mulți alți muzicieni celebri au provenit din Trenchtown.", "Monument național din 2006."],
      en: ["Considered the cradle of reggae music.", "Marley's old VW bus is still in the yard.", "Many other famous musicians came from Trenchtown.", "A national monument since 2006."],
    },
  },
  {
    id: "lacovia-tombstones-history-v2",
    type: "historical",
    parent: "JM-11",
    coords: [-77.783, 18.15],
    name: {
      de: "Grabsteine von Lacovia",
      hu: "Lacovia sírkövei",
      ro: "Pietrele funerare din Lacovia",
      en: "Lacovia Tombstones",
    },
    description: {
      de: "Zwei alte Grabsteine am Straßenrand, die an einen lokalen Gutsherrn und seinen Duellgegner aus dem 17. Jahrhundert erinnern. Die Legende besagt, dass beide Männer im Duell starben.",
      hu: "Két régi sírkő az út mentén, amelyek egy helyi földesúrnak és 17. századi párbajellenfelének állítanak emléket. A legenda szerint mindkét férfi a párbajban halt meg.",
      ro: "Două pietre funerare vechi de pe marginea drumului, care comemorează un moșier local și adversarul său la duel din secolul al XVII-lea. Legenda spune că ambii bărbați au murit în duel.",
      en: "Two old tombstones on the roadside, commemorating a local squire and his 17th-century dueling opponent. Legend has it that both men died in the duel.",
    },
    facts: {
      de: ["Eine lokale Kuriosität und ein Stück Folklore.", "Die Inschriften sind kaum noch lesbar.", "Einer der Steine markiert das Grab von Thomas Jordan.", "Ein seltenes erhaltenes Denkmal aus dieser frühen Zeit."],
      hu: ["Helyi kuriózum és egy darab folklór.", "A feliratok már alig olvashatók.", "Az egyik kő Thomas Jordan sírját jelöli.", "Ritka fennmaradt emlék ebből a korai időszakból."],
      ro: ["O curiozitate locală și o bucată de folclor.", "Inscripțiile sunt abia lizibile.", "Una dintre pietre marchează mormântul lui Thomas Jordan.", "Un monument rar păstrat din această perioadă timpurie."],
      en: ["A local curiosity and a piece of folklore.", "The inscriptions are barely legible.", "One of the stones marks the grave of Thomas Jordan.", "A rare surviving monument from this early period."],
    },
  },
  {
    id: "georg-liele-memorial-history-v2",
    type: "historical",
    parent: "JM-01",
    coords: [-76.791, 17.975],
    name: {
      de: "George Liele Denkmal",
      hu: "George Liele emlékmű",
      ro: "Memorialul George Liele",
      en: "George Liele Memorial",
    },
    description: {
      de: "Ein Denkmal in Kingston, das George Liele ehrt, einen befreiten amerikanischen Sklaven, der 1782 nach Jamaika kam und die erste baptistische Kirche auf der Insel gründete.",
      hu: "Egy emlékmű Kingstonban, amely George Liele-nek, egy felszabadított amerikai rabszolgának állít emléket, aki 1782-ben érkezett Jamaicába és megalapította az első baptista gyülekezetet a szigeten.",
      ro: "Un memorial în Kingston care îl onorează pe George Liele, un sclav american eliberat care a venit în Jamaica în 1782 și a fondat prima biserică baptistă de pe insulă.",
      en: "A memorial in Kingston honoring George Liele, a freed American slave who came to Jamaica in 1782 and founded the first Baptist church on the island.",
    },
    facts: {
      de: ["Liele gilt als Pionier der baptistischen Bewegung in Jamaika.", "Seine Arbeit trug zur späteren Abschaffung der Sklaverei bei.", "Das Denkmal befindet sich auf dem Gelände der East Queen Street Baptist Church.", "Er predigte sowohl zu Sklaven als auch zu Freien."],
      hu: ["Liele-t a jamaicai baptista mozgalom úttörőjének tartják.", "Munkássága hozzájárult a rabszolgaság későbbi eltörléséhez.", "Az emlékmű az East Queen Street-i baptista templom területén található.", "Prédikált rabszolgáknak és szabadoknak egyaránt."],
      ro: ["Liele este considerat un pionier al mișcării baptiste din Jamaica.", "Munca sa a contribuit la abolirea ulterioară a sclaviei.", "Memorialul se află pe terenul Bisericii Baptiste de pe strada East Queen.", "A predicat atât sclavilor, cât și oamenilor liberi."],
      en: ["Liele is considered a pioneer of the Baptist movement in Jamaica.", "His work contributed to the eventual abolition of slavery.", "The memorial is located on the grounds of the East Queen Street Baptist Church.", "He preached to both enslaved and free people."],
    },
  },
  {
    id: "national-heroes-park-history-v2",
    type: "historical",
    parent: "JM-01",
    coords: [-76.789, 17.981],
    name: {
      de: "Nationalheldenpark",
      hu: "Nemzeti Hősök Parkja",
      ro: "Parcul Eroilor Naționali",
      en: "National Heroes Park",
    },
    description: {
      de: "Einst ein Rennplatz, heute ein botanischer Garten und die Begräbnisstätte der Nationalhelden Jamaikas sowie vieler ehemaliger Premierminister. Es ist ein heiliger Ort nationaler Bedeutung.",
      hu: "Egykor versenypálya, ma botanikus kert és Jamaica nemzeti hőseinek, valamint számos volt miniszterelnökének temetkezési helye. Ez egy szent, nemzeti jelentőségű hely.",
      ro: "Odinioară un hipodrom, acum o grădină botanică și locul de înmormântare al eroilor naționali ai Jamaicăi, precum și al multor foști prim-miniștri. Este un loc sacru de importanță națională.",
      en: "Once a racecourse, now a botanical garden and the burial site for Jamaica's National Heroes and many former Prime Ministers. It is a sacred space of national importance.",
    },
    facts: {
      de: ["Enthält Denkmäler für Marcus Garvey, Norman Manley und Alexander Bustamante.", "Ein Kriegerdenkmal ehrt die in den Weltkriegen gefallenen Jamaikaner.", "Der Park erstreckt sich über 50 Hektar.", "Früher bekannt als George VI Memorial Park."],
      hu: ["Itt találhatók Marcus Garvey, Norman Manley és Alexander Bustamante emlékművei.", "Egy háborús emlékmű tiszteleg a világháborúkban elesett jamaicaiak előtt.", "A park 50 hektáron terül el.", "Korábban VI. György Emlékparkként ismerték."],
      ro: ["Conține monumente pentru Marcus Garvey, Norman Manley și Alexander Bustamante.", "Un memorial de război îi onorează pe jamaicanii căzuți în războaiele mondiale.", "Parcul se întinde pe 50 de acri.", "Cunoscut anterior ca Parcul Memorial George al VI-lea."],
      en: ["Contains monuments to Marcus Garvey, Norman Manley, and Alexander Bustamante.", "A war memorial honors Jamaicans who died in the World Wars.", "The park covers 50 acres.", "Formerly known as George VI Memorial Park."],
    },
  },
  {
    id: "the-cage-montego-bay-history-v2",
    type: "historical",
    parent: "JM-02",
    coords: [-77.923, 18.473],
    name: {
      de: "Der Käfig, Montego Bay",
      hu: "A ketrec, Montego Bay",
      ro: "Cușca, Montego Bay",
      en: "The Cage, Montego Bay",
    },
    description: {
      de: "Ein kleines, weiß getünchtes Gebäude aus dem Jahr 1806, das als vorübergehendes Gefängnis für weggelaufene Sklaven, betrunkene Seeleute und andere Kleinkriminelle diente. Es steht mitten auf dem Sam Sharpe Square.",
      hu: "Egy kis, 1806-ban épült, fehérre meszelt épület, amely ideiglenes börtönként szolgált szökevény rabszolgák, részeg tengerészek és más apró bűnözők számára. A Sam Sharpe téren áll.",
      ro: "O clădire mică, văruită în alb, datând din 1806, care a servit ca închisoare temporară pentru sclavii fugari, marinarii beți și alți infractori mărunți. Se află în mijlocul Pieței Sam Sharpe.",
      en: "A small, whitewashed building from 1806 that served as a temporary jail for runaway slaves, drunken sailors, and other petty criminals. It stands in the middle of Sam Sharpe Square.",
    },
    facts: {
      de: ["Ursprünglich aus Holz gebaut.", "Es hat zwei Zellen.", "Ein deutliches Erinnerungsstück an die soziale Ordnung der Kolonialzeit.", "Heute ein kleines historisches Wahrzeichen."],
      hu: ["Eredetileg fából épült.", "Két cellája van.", "A gyarmati kor társadalmi rendjének markáns emléke.", "Ma egy kis történelmi nevezetesség."],
      ro: ["Construită inițial din lemn.", "Are două celule.", "O amintire pregnantă a ordinii sociale din perioada colonială.", "Astăzi este un mic punct de reper istoric."],
      en: ["Originally built of wood.", "It has two cells.", "A stark reminder of the colonial-era social order.", "Now a small historical landmark."],
    },
  },
  {
    id: "sam-sharpe-square-history-v2",
    type: "historical",
    parent: "JM-02",
    coords: [-77.923, 18.473],
    name: {
      de: "Sam Sharpe Square",
      hu: "Sam Sharpe tér",
      ro: "Piața Sam Sharpe",
      en: "Sam Sharpe Square",
    },
    description: {
      de: "Der Hauptplatz von Montego Bay, benannt nach dem Nationalhelden Samuel Sharpe, der hier 1832 wegen seiner Rolle im 'Baptistenkrieg', einem großen Sklavenaufstand, gehängt wurde.",
      hu: "Montego Bay főtere, amelyet Samuel Sharpe nemzeti hős után neveztek el, akit itt akasztottak fel 1832-ben a 'baptista háborúban', egy nagy rabszolgafelkelésben játszott szerepe miatt.",
      ro: "Piața principală din Montego Bay, numită după eroul național Samuel Sharpe, care a fost spânzurat aici în 1832 pentru rolul său în 'Războiul Baptist', o mare revoltă a sclavilor.",
      en: "The main square of Montego Bay, named after National Hero Samuel Sharpe, who was hanged here in 1832 for his role in the 'Baptist War', a major slave rebellion.",
    },
    facts: {
      de: ["Der Aufstand beschleunigte die Abschaffung der Sklaverei.", "Der Platz war früher als Charles Square bekannt.", "Ein Denkmal ehrt Sam Sharpe und seine Anhänger.", "Ein lebendiger öffentlicher Raum im Herzen der Stadt."],
      hu: ["A felkelés felgyorsította a rabszolgaság eltörlését.", "A teret korábban Charles térnek hívták.", "Egy emlékmű tiszteleg Sam Sharpe és követői előtt.", "Élénk közterület a város szívében."],
      ro: ["Revolta a accelerat abolirea sclaviei.", "Piața era cunoscută anterior ca Piața Charles.", "Un monument îi onorează pe Sam Sharpe și pe adepții săi.", "Un spațiu public vibrant în inima orașului."],
      en: ["The rebellion hastened the abolition of slavery.", "The square was previously known as Charles Square.", "A monument honors Sam Sharpe and his followers.", "A vibrant public space in the heart of the city."],
    },
  },
  {
    id: "tryall-great-house-history-v2",
    type: "historical",
    parent: "JM-09",
    coords: [-78.053, 18.461],
    name: {
      de: "Tryall Great House",
      hu: "Tryall udvarház",
      ro: "Conacul Tryall",
      en: "Tryall Great House",
    },
    description: {
      de: "Ein Herrenhaus aus dem 19. Jahrhundert auf einem Hügel mit Blick auf das Karibische Meer. Es ist das Herzstück des exklusiven Tryall Club und bewahrt die Geschichte der ehemaligen Zuckerplantage.",
      hu: "Egy 19. századi udvarház egy dombon, kilátással a Karib-tengerre. Az exkluzív Tryall Club központja, és megőrzi az egykori cukorültetvény történetét.",
      ro: "Un conac din secolul al XIX-lea, cocoțat pe un deal cu vedere la Marea Caraibelor. Este piesa centrală a exclusivistului Tryall Club și păstrează istoria fostei plantații de zahăr.",
      en: "A 19th-century great house perched on a hill overlooking the Caribbean Sea. It is the centerpiece of the exclusive Tryall Club and preserves the history of the former sugar plantation.",
    },
    facts: {
      de: ["Eine alte Wassermühle auf dem Gelände ist noch intakt.", "Das Anwesen ist heute ein Luxusresort und Golfclub.", "Die Kanone vor dem Haus stammt von Fort Tryall.", "Ein Ort von historischer und moderner Eleganz."],
      hu: ["Egy régi vízimalom a területen még mindig ép.", "A birtok ma luxusüdülőhely és golfklub.", "A ház előtti ágyú a Tryall erődből származik.", "A történelmi és modern elegancia helyszíne."],
      ro: ["O veche moară de apă de pe proprietate este încă intactă.", "Proprietatea este acum o stațiune de lux și un club de golf.", "Tunul din fața casei provine de la Fortul Tryall.", "Un loc de eleganță istorică și modernă."],
      en: ["An old water wheel on the property is still intact.", "The estate is now a luxury resort and golf club.", "The cannon in front of the house is from Fort Tryall.", "A place of historic and modern elegance."],
    },
  },
  {
    id: "cinnamon-hill-great-house-history-v2",
    type: "historical",
    parent: "JM-02",
    coords: [-77.81, 18.49],
    name: {
      de: "Cinnamon Hill Great House",
      hu: "Cinnamon Hill udvarház",
      ro: "Conacul Cinnamon Hill",
      en: "Cinnamon Hill Great House",
    },
    description: {
      de: "Ein historisches Plantagenhaus, das einst der Familie von Elizabeth Barrett Browning gehörte und später das jamaikanische Zuhause der Country-Musik-Legende Johnny Cash war. Es ist voller Geschichte und persönlicher Erinnerungsstücke.",
      hu: "Történelmi ültetvényház, amely egykor Elizabeth Barrett Browning családjának tulajdonában volt, később pedig Johnny Cash country-legenda jamaicai otthona lett. Tele van történelemmel és személyes emléktárgyakkal.",
      ro: "O casă istorică de plantație, care a aparținut cândva familiei lui Elizabeth Barrett Browning și care a devenit ulterior casa jamaicană a legendei muzicii country Johnny Cash. Este plină de istorie și suveniruri personale.",
      en: "A historic plantation house once owned by the family of Elizabeth Barrett Browning, and later became the Jamaican home of country music legend Johnny Cash. It is filled with history and personal memorabilia.",
    },
    facts: {
      de: ["Johnny Cash und June Carter sind hier oft aufgetreten.", "Angeblich spukt die 'Weiße Hexe' Annie Palmer hier.", "Das Haus bietet Touren an, die das Leben von Cash beleuchten.", "Ein wunderschönes Beispiel für die Architektur des 18. Jahrhunderts."],
      hu: ["Johnny Cash és June Carter gyakran felléptek itt.", "Állítólag a 'Fehér Boszorkány', Annie Palmer kísért itt.", "A házban túrákat tartanak, amelyek bemutatják Cash életét.", "A 18. századi építészet gyönyörű példája."],
      ro: ["Johnny Cash și June Carter au concertat aici adesea.", "Se presupune că 'Vrăjitoarea Albă' Annie Palmer bântuie aici.", "Casa oferă tururi care evidențiază viața lui Cash.", "Un exemplu frumos de arhitectură din secolul al XVIII-lea."],
      en: ["Johnny Cash and June Carter often performed here.", "Allegedly haunted by the 'White Witch' Annie Palmer.", "The house offers tours highlighting Cash's life.", "A beautiful example of 18th-century architecture."],
    },
  },
  {
    id: "stokes-hall-great-house-ruins-history-v2",
    type: "historical",
    parent: "JM-04",
    coords: [-76.284, 17.886],
    name: {
      de: "Ruinen des Stokes Hall Great House",
      hu: "Stokes Hall udvarház romjai",
      ro: "Ruinele conacului Stokes Hall",
      en: "Stokes Hall Great House Ruins",
    },
    description: {
      de: "Die Ruinen eines der ältesten und am stärksten befestigten Herrenhäuser Jamaikas. Es wurde im 17. Jahrhundert erbaut und hatte vier Türme, um sich gegen Angriffe zu verteidigen.",
      hu: "Jamaica egyik legrégebbi és legerősebben erődített udvarházának romjai. A 17. században épült, és négy tornya volt a támadások elleni védelem érdekében.",
      ro: "Ruinele unuia dintre cele mai vechi și mai puternic fortificate conace din Jamaica. A fost construit în secolul al XVII-lea și avea patru turnuri pentru a se apăra împotriva atacurilor.",
      en: "The ruins of one of the oldest and most heavily fortified great houses in Jamaica. It was built in the 17th century and had four towers to defend against attacks.",
    },
    facts: {
      de: ["Erbaut von der Familie Stokes nach der englischen Eroberung.", "Ein seltenes Beispiel für ein befestigtes Haus aus dieser Zeit.", "Durch das Erdbeben von 1907 schwer beschädigt.", "Ein Zeugnis der unsicheren frühen Kolonialzeit."],
      hu: ["A Stokes család építtette az angol hódítás után.", "Ritka példája egy erődített háznak ebből az időszakból.", "Az 1907-es földrengés súlyosan megrongálta.", "A bizonytalan korai gyarmati időszak tanúja."],
      ro: ["Construit de familia Stokes după cucerirea engleză.", "Un exemplu rar de casă fortificată din acea perioadă.", "Grav avariat de cutremurul din 1907.", "O mărturie a perioadei coloniale timpurii nesigure."],
      en: ["Built by the Stokes family after the English conquest.", "A rare example of a fortified house from that period.", "Severely damaged by the 1907 earthquake.", "A testament to the insecure early colonial era."],
    },
  },
  {
    id: "black-river-spa-mineral-spring-history-v2",
    type: "historical",
    parent: "JM-11",
    coords: [-77.85, 18.025],
    name: {
      de: "Black River Spa Mineralquelle",
      hu: "Black River gyógyfürdő ásványforrás",
      ro: "Izvorul mineral Black River Spa",
      en: "Black River Spa Mineral Spring",
    },
    description: {
      de: "Eine historische Mineralquelle, die im 18. Jahrhundert als Heilbad populär wurde. Das Wasser, das in ein steinernes Becken fließt, sollte bei verschiedenen Krankheiten helfen.",
      hu: "Történelmi ásványforrás, amely a 18. században gyógyfürdőként vált népszerűvé. A kőmedencébe folyó víz állítólag különböző betegségeken segített.",
      ro: "Un izvor mineral istoric care a devenit popular ca stațiune balneară în secolul al XVIII-lea. Se spunea că apa, care curge într-un bazin de piatră, ajută la diverse afecțiuni.",
      en: "A historic mineral spring that became popular as a spa in the 18th century. The water, which flows into a stone basin, was said to help with various ailments.",
    },
    facts: {
      de: ["Das Spa war ein gesellschaftlicher Treffpunkt für die Pflanzerelite.", "Das ursprüngliche Gebäude ist nicht mehr erhalten.", "Die Quelle ist immer noch für die Öffentlichkeit zugänglich.", "Zeugt von den frühen Gesundheitspraktiken auf der Insel."],
      hu: ["A fürdő az ültetvényes elit társadalmi találkozóhelye volt.", "Az eredeti épület már nem létezik.", "A forrás ma is látogatható.", "Tanúskodik a sziget korai egészségügyi gyakorlatairól."],
      ro: ["Stațiunea era un loc de întâlnire socială pentru elita plantatorilor.", "Clădirea originală nu mai există.", "Izvorul este încă accesibil publicului.", "Mărturie a practicilor timpurii de sănătate de pe insulă."],
      en: ["The spa was a social gathering place for the planter elite.", "The original building no longer exists.", "The spring is still accessible to the public.", "A testament to early health practices on the island."],
    },
  },
  {
    id: "lyssons-beach-public-park-history-v2",
    type: "historical",
    parent: "JM-04",
    coords: [-76.38, 17.88],
    name: {
      de: "Lyssons Beach Öffentlicher Park",
      hu: "Lyssons Beach nyilvános park",
      ro: "Parcul Public Lyssons Beach",
      en: "Lyssons Beach Public Park",
    },
    description: {
      de: "Ein Strand in der Nähe von Morant Bay, der eine Rolle im Aufstand von 1865 spielte. Rebellen zogen durch dieses Gebiet auf ihrem Marsch zum Gerichtsgebäude von Morant Bay.",
      hu: "Egy strand Morant Bay közelében, amely szerepet játszott az 1865-ös felkelésben. A lázadók ezen a területen vonultak keresztül a Morant Bay-i bírósághoz vezető útjukon.",
      ro: "O plajă lângă Morant Bay care a jucat un rol în rebeliunea din 1865. Rebelii au trecut prin această zonă în marșul lor spre tribunalul din Morant Bay.",
      en: "A beach near Morant Bay that played a role in the 1865 rebellion. Rebels passed through this area on their march to the Morant Bay courthouse.",
    },
    facts: {
      de: ["Teil des Weges, den Paul Bogle und seine Anhänger nahmen.", "Heute ein beliebter Erholungsort für Einheimische.", "Der Name stammt von einer ehemaligen Zuckerplantage.", "Ein Ort mit ruhiger Schönheit und turbulenter Geschichte."],
      hu: ["Része volt az útvonalnak, amelyet Paul Bogle és követői bejártak.", "Ma a helyiek kedvelt üdülőhelye.", "A nevét egy egykori cukorültetvényről kapta.", "A nyugodt szépség és a viharos történelem helyszíne."],
      ro: ["Parte a traseului parcurs de Paul Bogle și adepții săi.", "Astăzi este un loc popular de recreere pentru localnici.", "Numele provine de la o fostă plantație de zahăr.", "Un loc de o frumusețe liniștită și o istorie turbulentă."],
      en: ["Part of the route taken by Paul Bogle and his followers.", "Today a popular recreational spot for locals.", "The name comes from a former sugar plantation.", "A place of tranquil beauty and turbulent history."],
    },
  },
];
