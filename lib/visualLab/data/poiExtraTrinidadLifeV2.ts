// @ts-nocheck
import type { POI } from "./poi";

export const poiExtraTrinidadLifeV2: POI[] = [
  {
    id: "tt-caroni-bird-sanctuary-life-v2",
    type: "animal-habitat",
    parent: "TT-CTT",
    coords: [-61.45, 10.58],
    name: { de: "Caroni Vogelreservat", hu: "Caroni madarrezervátum", ro: "Sanctuarul de păsări Caroni", en: "Caroni Bird Sanctuary"},
    image: "/poi-images/tt-caroni-bird-sanctuary-life-v2.webp",
    description: { de: "Ein Mangrovengebiet, das für die roten Ibisse bekannt ist.", hu: "Mangrovevidék, amely a skarlát íbiszekről ismert.", ro: "O zonă de mangrove cunoscută pentru ibișii stacojii.", en: "A mangrove wetland known for its scarlet ibises." },
    facts: {
      de: ["Heimat des Nationalvogels", "Beliebte Sonnenuntergangsboote", "Wichtiges Mangroven-Ökosystem"],
      hu: ["A nemzeti madár otthona", "Napsütötte hajótúrák", "Fontos mangrove ökoszisztéma"],
      ro: ["Casa păsării naționale", "Tururi cu barca la apus", "Ecosistem important de mangrove"],
      en: ["Home to the national bird", "Popular sunset boat tours", "Important mangrove ecosystem"]
    }, image: "/poi-images/tt-caroni-bird-sanctuary-life-v2.webp",
    descriptionAdvanced: {
      de: "Caroni Vogelreservat ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein Mangrovengebiet, das für die roten Ibisse bekannt ist. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Heimat des Nationalvogels. Hervorzuheben ist außerdem: Beliebte Sonnenuntergangsboote. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Caroni madarrezervátum a karibi Trinidad sziget egyik figyelemre méltó helye. Mangrovevidék, amely a skarlát íbiszekről ismert. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: A nemzeti madár otthona. További fontos jellemző: Napsütötte hajótúrák. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Sanctuarul de păsări Caroni este un loc remarcabil de pe insula caraibiană Trinidad. O zonă de mangrove cunoscută pentru ibișii stacojii. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Casa păsării naționale. Un alt aspect important: Tururi cu barca la apus. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Caroni Bird Sanctuary is a notable site on the Caribbean island of Trinidad. A mangrove wetland known for its scarlet ibises. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Home to the national bird. Another point of interest: Popular sunset boat tours. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Caroni Vogelreservat.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Caroni madarrezervátum.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Sanctuarul de păsări Caroni.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Caroni Bird Sanctuary.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-asa-wright-life-v2",
    type: "animal-habitat",
    parent: "TT-TUP",
    coords: [-61.2833, 10.7167],
    name: { de: "Asa Wright Naturzentrum", hu: "Asa Wright természetközpont", ro: "Centrul de natură Asa Wright", en: "Asa Wright Nature Centre"},
    image: "/poi-images/tt-asa-wright-life-v2.webp",
    description: { de: "Ein weltbekanntes Vogelbeobachtungszentrum in den Bergen.", hu: "Világhírű madármegfigyelő központ a hegyekben.", ro: "Un centru celebru de observare a păsărilor în munți.", en: "A world-famous birdwatching center in the hills." },
    facts: {
      de: ["Berühmt für Tukane", "Ehemalige Kakao- und Kaffeeplantage", "Biodiversitäts-Hotspot"],
      hu: ["Híres a tukánokról", "Egykori kakaó- és kávéültetvény", "Biodiverzitás-gazdag hely"],
      ro: ["Faimos pentru tucani", "Fosta plantație de cacao și cafea", "Punct fierbinte al biodiversității"],
      en: ["Famous for toucans", "Former cocoa and coffee estate", "Biodiversity hotspot"]
    }, image: "/poi-images/tt-asa-wright-life-v2.webp",
    descriptionAdvanced: {
      de: "Asa Wright Naturzentrum ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein weltbekanntes Vogelbeobachtungszentrum in den Bergen. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Berühmt für Tukane. Hervorzuheben ist außerdem: Ehemalige Kakao- und Kaffeeplantage. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Asa Wright természetközpont a karibi Trinidad sziget egyik figyelemre méltó helye. Világhírű madármegfigyelő központ a hegyekben. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Híres a tukánokról. További fontos jellemző: Egykori kakaó- és kávéültetvény. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Centrul de natură Asa Wright este un loc remarcabil de pe insula caraibiană Trinidad. Un centru celebru de observare a păsărilor în munți. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Faimos pentru tucani. Un alt aspect important: Fosta plantație de cacao și cafea. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Asa Wright Nature Centre is a notable site on the Caribbean island of Trinidad. A world-famous birdwatching center in the hills. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Famous for toucans. Another point of interest: Former cocoa and coffee estate. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Asa Wright Naturzentrum.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Asa Wright természetközpont.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Centrul de natură Asa Wright.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Asa Wright Nature Centre.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-yerette-life-v2",
    type: "animal-habitat",
    parent: "TT-TUP",
    coords: [-61.4, 10.67],
    name: { de: "Yerette Kolibrimenedek", hu: "Yerette kolibrimenedék", ro: "Sanctuarul de colibri Yerette", en: "Yerette Hummingbird Sanctuary"},
    image: "/poi-images/tt-yerette-life-v2.webp",
    description: { de: "Ein privates Refugium für Kolibris im Maracas-Tal.", hu: "Magán kolibrimenedékhely a Maracas-völgyben.", ro: "Un refugiu privat pentru colibri în Valea Maracas.", en: "A private hummingbird sanctuary in Maracas Valley." },
    facts: {
      de: ["13 Kolibriarten", "Geführte Naturbesuche", "Fotografie-Standort"],
      hu: ["13 kolibrifaj", "Vezetett természetlátogatások", "Fotográfiai helyszín"],
      ro: ["13 specii de colibri", "Vizite ghidate în natură", "Loc bun pentru fotografie"],
      en: ["13 hummingbird species", "Guided nature visits", "Great for photography"]
    },
    descriptionAdvanced: {
      de: "Yerette Kolibrimenedek ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein privates Refugium für Kolibris im Maracas-Tal. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: 13 Kolibriarten. Hervorzuheben ist außerdem: Geführte Naturbesuche. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Yerette kolibrimenedék a karibi Trinidad sziget egyik figyelemre méltó helye. Magán kolibrimenedékhely a Maracas-völgyben. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: 13 kolibrifaj. További fontos jellemző: Vezetett természetlátogatások. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Sanctuarul de colibri Yerette este un loc remarcabil de pe insula caraibiană Trinidad. Un refugiu privat pentru colibri în Valea Maracas. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: 13 specii de colibri. Un alt aspect important: Vizite ghidate în natură. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Yerette Hummingbird Sanctuary is a notable site on the Caribbean island of Trinidad. A private hummingbird sanctuary in Maracas Valley. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: 13 hummingbird species. Another point of interest: Guided nature visits. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Yerette Kolibrimenedek.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Yerette kolibrimenedék.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Sanctuarul de colibri Yerette.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Yerette Hummingbird Sanctuary.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-emperor-valley-zoo-life-v2",
    type: "kid-landmark",
    parent: "TT-POS",
    coords: [-61.52, 10.665],
    name: { de: "Emperor Valley Zoo", hu: "Emperor Valley állatkert", ro: "Grădina Zoologică Emperor Valley", en: "Emperor Valley Zoo"},
    image: "/poi-images/tt-emperor-valley-zoo-life-v2.webp",
    description: { de: "Der bekannteste Zoo von Trinidad und Tobago.", hu: "Trinidad és Tobago legismertebb állatkertje.", ro: "Cea mai cunoscută grădină zoologică din Trinidad și Tobago.", en: "The best-known zoo in Trinidad and Tobago." },
    facts: {
      de: ["In Port of Spain", "Beliebt bei Familien", "Verbindet Lernen und Freizeit"],
      hu: ["Port of Spainben", "Családok kedvence", "Tanulás és szórakozás"],
      ro: ["În Port of Spain", "Popular pentru familii", "Îmbină învățarea și distracția"],
      en: ["In Port of Spain", "Popular with families", "Combines learning and fun"]
    }, image: "/poi-images/tt-emperor-valley-zoo-life-v2.webp",
    descriptionAdvanced: {
      de: "Emperor Valley Zoo ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Der bekannteste Zoo von Trinidad und Tobago. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: In Port of Spain. Hervorzuheben ist außerdem: Beliebt bei Familien. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Emperor Valley állatkert a karibi Trinidad sziget egyik figyelemre méltó helye. Trinidad és Tobago legismertebb állatkertje. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Port of Spainben. További fontos jellemző: Családok kedvence. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Grădina Zoologică Emperor Valley este un loc remarcabil de pe insula caraibiană Trinidad. Cea mai cunoscută grădină zoologică din Trinidad și Tobago. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: În Port of Spain. Un alt aspect important: Popular pentru familii. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Emperor Valley Zoo is a notable site on the Caribbean island of Trinidad. The best-known zoo in Trinidad and Tobago. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: In Port of Spain. Another point of interest: Popular with families. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Emperor Valley Zoo.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Emperor Valley állatkert.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Grădina Zoologică Emperor Valley.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Emperor Valley Zoo.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-queens-park-savannah-life-v2",
    type: "kid-landmark",
    parent: "TT-POS",
    coords: [-61.5147, 10.6686],
    name: { de: "Queens Park Savannah", hu: "Queens Park Savannah", ro: "Queen's Park Savannah", en: "Queen's Park Savannah"},
    image: "/poi-images/tt-queens-park-savannah-life-v2.webp",
    description: { de: "Die größte offene Fläche in Port of Spain.", hu: "Port of Spain legnagyobb nyitott tere.", ro: "Cel mai mare spațiu deschis din Port of Spain.", en: "The largest open space in Port of Spain." },
    facts: {
      de: ["Ort für Karneval", "Freizeit und Sport", "Umgeben von Kolonialhäusern"],
      hu: ["A karnevál helyszíne", "Szabadidős és sportos terület", "Gyarmati házak veszik körül"],
      ro: ["Loc pentru Carnaval", "Timp liber și sport", "Înconjurată de case coloniale"],
      en: ["Carnival venue", "Recreation and sports", "Surrounded by colonial mansions"]
    }, image: "/poi-images/tt-queens-park-savannah-life-v2.webp",
    descriptionAdvanced: {
      de: "Queens Park Savannah ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Die größte offene Fläche in Port of Spain. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Ort für Karneval. Hervorzuheben ist außerdem: Freizeit und Sport. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Queens Park Savannah a karibi Trinidad sziget egyik figyelemre méltó helye. Port of Spain legnagyobb nyitott tere. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: A karnevál helyszíne. További fontos jellemző: Szabadidős és sportos terület. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Queen's Park Savannah este un loc remarcabil de pe insula caraibiană Trinidad. Cel mai mare spațiu deschis din Port of Spain. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Loc pentru Carnaval. Un alt aspect important: Timp liber și sport. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Queen's Park Savannah is a notable site on the Caribbean island of Trinidad. The largest open space in Port of Spain. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Carnival venue. Another point of interest: Recreation and sports. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Queens Park Savannah.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Queens Park Savannah.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Queen's Park Savannah.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Queen's Park Savannah.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-maracas-bay-life-v2",
    type: "kid-landmark",
    parent: "TT-SJL",
    coords: [-61.4392, 10.7583],
    name: { de: "Maracas Bay", hu: "Maracas-öböl", ro: "Golful Maracas", en: "Maracas Bay"},
    image: "/poi-images/tt-maracas-bay-life-v2.webp",
    description: { de: "Trinidads bekanntester Strand und ein beliebter Wochenendort.", hu: "Trinidad legismertebb strandja és kedvelt hétvégi helyszín.", ro: "Cea mai cunoscută plajă din Trinidad și o destinație de weekend.", en: "Trinidad's most famous beach and a popular weekend spot." },
    facts: {
      de: ["Bekannt für Bake and Shark", "Nordkustenfahrt", "Breiter Sandstrand"],
      hu: ["Híres a Bake and Sharkról", "Északi parti út", "Széles homokos part"],
      ro: ["Faimos pentru Bake and Shark", "Drum pitoresc pe coasta de nord", "Plajă lată cu nisip"],
      en: ["Famous for Bake and Shark", "Scenic North Coast drive", "Wide sandy beach"]
    }, image: "/poi-images/tt-maracas-bay-life-v2.webp",
    descriptionAdvanced: {
      de: "Maracas Bay ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Trinidads bekanntester Strand und ein beliebter Wochenendort. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Bekannt für Bake and Shark. Hervorzuheben ist außerdem: Nordkustenfahrt. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Maracas-öböl a karibi Trinidad sziget egyik figyelemre méltó helye. Trinidad legismertebb strandja és kedvelt hétvégi helyszín. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Híres a Bake and Sharkról. További fontos jellemző: Északi parti út. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Golful Maracas este un loc remarcabil de pe insula caraibiană Trinidad. Cea mai cunoscută plajă din Trinidad și o destinație de weekend. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Faimos pentru Bake and Shark. Un alt aspect important: Drum pitoresc pe coasta de nord. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Maracas Bay is a notable site on the Caribbean island of Trinidad. Trinidad's most famous beach and a popular weekend spot. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Famous for Bake and Shark. Another point of interest: Scenic North Coast drive. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Maracas Bay.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Maracas-öböl.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Golful Maracas.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Maracas Bay.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-cleaver-woods-life-v2",
    type: "kid-landmark",
    parent: "TT-TUP",
    coords: [-61.45, 10.63],
    name: { de: "Cleaver Woods", hu: "Cleaver Woods", ro: "Cleaver Woods", en: "Cleaver Woods"},
    image: "/poi-images/tt-cleaver-woods-life-v2.webp",
    description: { de: "Ein Wald- und Erholungspark in Ost-Trinidad.", hu: "Erdős és pihenő park Kelet-Trinidadban.", ro: "Un parc forestier și de recreere în estul Trinidadului.", en: "A forest and recreation park in eastern Trinidad." },
    facts: {
      de: ["Naturpfade", "Beliebt bei Schulgruppen", "Kleines Besucherzentrum"],
      hu: ["Természeti utak", "Iskolai csoportok kedvence", "Kis látogatóközpont"],
      ro: ["Trasee în natură", "Popular printre grupurile școlare", "Mic centru pentru vizitatori"],
      en: ["Nature trails", "Popular for school trips", "Small visitor centre"]
    },
    descriptionAdvanced: {
      de: "Cleaver Woods ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein Wald- und Erholungspark in Ost-Trinidad. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Naturpfade. Hervorzuheben ist außerdem: Beliebt bei Schulgruppen. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Cleaver Woods a karibi Trinidad sziget egyik figyelemre méltó helye. Erdős és pihenő park Kelet-Trinidadban. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Természeti utak. További fontos jellemző: Iskolai csoportok kedvence. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Cleaver Woods este un loc remarcabil de pe insula caraibiană Trinidad. Un parc forestier și de recreere în estul Trinidadului. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Trasee în natură. Un alt aspect important: Popular printre grupurile școlare. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Cleaver Woods is a notable site on the Caribbean island of Trinidad. A forest and recreation park in eastern Trinidad. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Nature trails. Another point of interest: Popular for school trips. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Cleaver Woods.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Cleaver Woods.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Cleaver Woods.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Cleaver Woods.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-pointe-a-pierre-wildfowl-life-v2",
    type: "animal-habitat",
    parent: "TT-CTT",
    coords: [-61.45, 10.33],
    name: { de: "Pointe a Pierre Wildvogelpark", hu: "Pointe a Pierre vadmadárpark", ro: "Rezervația de păsări sălbatice Pointe a Pierre", en: "Pointe-a-Pierre Wildfowl Trust"},
    image: "/poi-images/tt-pointe-a-pierre-wildfowl-life-v2.webp",
    description: { de: "Ein Schutzgebiet mit Lagunen, Vögeln und ruhigen Wegen.", hu: "Védett terület lagúnákkal, madarakkal és csendes sétányokkal.", ro: "O rezervație cu lagune, păsări și alei liniștite.", en: "A sanctuary with lagoons, birds, and quiet walking paths." },
    facts: {
      de: ["Wichtige Vogelstation", "Lagunen und Mangroven", "Beliebt bei Naturfreunden"],
      hu: ["Fontos madárállomás", "Lagúnák és mangrovek", "Kedvelt a természetbarátok körében"],
      ro: ["Stație importantă pentru păsări", "Lagune și mangrove", "Populară pentru iubitorii de natură"],
      en: ["Important bird site", "Lagoons and mangroves", "Popular with nature lovers"]
    }, image: "/poi-images/tt-pointe-a-pierre-wildfowl-life-v2.webp",
    descriptionAdvanced: {
      de: "Pointe a Pierre Wildvogelpark ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein Schutzgebiet mit Lagunen, Vögeln und ruhigen Wegen. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Wichtige Vogelstation. Hervorzuheben ist außerdem: Lagunen und Mangroven. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Pointe a Pierre vadmadárpark a karibi Trinidad sziget egyik figyelemre méltó helye. Védett terület lagúnákkal, madarakkal és csendes sétányokkal. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Fontos madárállomás. További fontos jellemző: Lagúnák és mangrovek. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Rezervația de păsări sălbatice Pointe a Pierre este un loc remarcabil de pe insula caraibiană Trinidad. O rezervație cu lagune, păsări și alei liniștite. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Stație importantă pentru păsări. Un alt aspect important: Lagune și mangrove. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Pointe-a-Pierre Wildfowl Trust is a notable site on the Caribbean island of Trinidad. A sanctuary with lagoons, birds, and quiet walking paths. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Important bird site. Another point of interest: Lagoons and mangroves. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Pointe a Pierre Wildvogelpark.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Pointe a Pierre vadmadárpark.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Rezervația de păsări sălbatice Pointe a Pierre.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Pointe-a-Pierre Wildfowl Trust.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-caroni-sugarcane-belt-life-v2",
    type: "agriculture",
    parent: "TT-CTT",
    coords: [-61.42, 10.5],
    name: { de: "Caroni Zuckerrohrgürtel", hu: "Caroni cukornádöv", ro: "Centura de trestie Caroni", en: "Caroni Sugarcane Belt"},
    image: "/poi-images/tt-caroni-sugarcane-belt-life-v2.webp",
    description: { de: "Eine landwirtschaftliche Zone mit traditionellem Zuckerrohranbau.", hu: "Hagyományos cukornád-termesztési régió.", ro: "O zonă agricolă cu cultivare tradițională a trestiei de zahăr.", en: "An agricultural zone with traditional sugarcane farming." },
    facts: {
      de: ["Fruchtbare Ebenen", "Wichtiger Teil der Agrargeschichte", "Offene Felder und Kanäle"],
      hu: ["Termékeny síkságok", "A mezőgazdasági történelem fontos része", "Nyitott mezők és csatornák"],
      ro: ["Câmpii fertile", "Parte importantă a istoriei agricole", "Câmpuri deschise și canale"],
      en: ["Fertile plains", "Part of agricultural history", "Open fields and canals"]
    },
    descriptionAdvanced: {
      de: "Caroni Zuckerrohrgürtel ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Eine landwirtschaftliche Zone mit traditionellem Zuckerrohranbau. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Fruchtbare Ebenen. Hervorzuheben ist außerdem: Wichtiger Teil der Agrargeschichte. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Caroni cukornádöv a karibi Trinidad sziget egyik figyelemre méltó helye. Hagyományos cukornád-termesztési régió. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Termékeny síkságok. További fontos jellemző: A mezőgazdasági történelem fontos része. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Centura de trestie Caroni este un loc remarcabil de pe insula caraibiană Trinidad. O zonă agricolă cu cultivare tradițională a trestiei de zahăr. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Câmpii fertile. Un alt aspect important: Parte importantă a istoriei agricole. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Caroni Sugarcane Belt is a notable site on the Caribbean island of Trinidad. An agricultural zone with traditional sugarcane farming. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Fertile plains. Another point of interest: Part of agricultural history. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Caroni Zuckerrohrgürtel.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Caroni cukornádöv.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Centura de trestie Caroni.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Caroni Sugarcane Belt.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-central-range-cocoa-life-v2",
    type: "agriculture",
    parent: "TT-CTT",
    coords: [-61.35, 10.42],
    name: { de: "Central Range Kakaoanbau", hu: "Central Range kakaó", ro: "Plantații de cacao din Central Range", en: "Central Range Cocoa Farms"},
    image: "/poi-images/tt-central-range-cocoa-life-v2.webp",
    description: { de: "Kakaofarmen an den Hängen der Central Range.", hu: "Kakaófarmok a Central Range lejtőin.", ro: "Ferme de cacao pe pantele din Central Range.", en: "Cocoa farms on the slopes of the Central Range." },
    facts: {
      de: ["Kakaotradition", "Schattenanbau unter Bäumen", "Kleinskalige Farmen"],
      hu: ["Kakaóhagyomány", "Fák alatt, árnyékban növekvő termesztés", "Kisméretű farmok"],
      ro: ["Tradiție în cacao", "Cultivare la umbră sub arbori", "Ferme de mici dimensiuni"],
      en: ["Cocoa tradition", "Shade-grown under trees", "Small-scale farms"]
    },
    descriptionAdvanced: {
      de: "Central Range Kakaoanbau ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Kakaofarmen an den Hängen der Central Range. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Kakaotradition. Hervorzuheben ist außerdem: Schattenanbau unter Bäumen. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Central Range kakaó a karibi Trinidad sziget egyik figyelemre méltó helye. Kakaófarmok a Central Range lejtőin. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Kakaóhagyomány. További fontos jellemző: Fák alatt, árnyékban növekvő termesztés. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Plantații de cacao din Central Range este un loc remarcabil de pe insula caraibiană Trinidad. Ferme de cacao pe pantele din Central Range. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Tradiție în cacao. Un alt aspect important: Cultivare la umbră sub arbori. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Central Range Cocoa Farms is a notable site on the Caribbean island of Trinidad. Cocoa farms on the slopes of the Central Range. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Cocoa tradition. Another point of interest: Shade-grown under trees. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Central Range Kakaoanbau.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Central Range kakaó.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Plantații de cacao din Central Range.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Central Range Cocoa Farms.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-aripo-savannas-life-v2",
    type: "animal-habitat",
    parent: "TT-SGE",
    coords: [-61.22, 10.5],
    name: { de: "Aripo Savannen", hu: "Aripo szavannák", ro: "Savanele Aripo", en: "Aripo Savannas"},
    image: "/poi-images/tt-aripo-savannas-life-v2.webp",
    description: { de: "Ein einzigartiges Savannen- und Feuchtgebietssystem.", hu: "Egyedülálló szavanna és vizesélőhely-rendszer.", ro: "Un sistem unic de savană și zone umede.", en: "A unique savanna and wetland system." },
    facts: {
      de: ["Seltene Pflanzen", "Wissenschaftlich wichtig", "Empfindlicher Lebensraum"],
      hu: ["Ritka növények", "Tudományosan fontos", "Érzékeny élőhely"],
      ro: ["Plante rare", "Important științific", "Habitat sensibil"],
      en: ["Rare plants", "Scientifically important", "Sensitive habitat"]
    },
    descriptionAdvanced: {
      de: "Aripo Savannen ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein einzigartiges Savannen- und Feuchtgebietssystem. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Seltene Pflanzen. Hervorzuheben ist außerdem: Wissenschaftlich wichtig. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Aripo szavannák a karibi Trinidad sziget egyik figyelemre méltó helye. Egyedülálló szavanna és vizesélőhely-rendszer. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Ritka növények. További fontos jellemző: Tudományosan fontos. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Savanele Aripo este un loc remarcabil de pe insula caraibiană Trinidad. Un sistem unic de savană și zone umede. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Plante rare. Un alt aspect important: Important științific. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Aripo Savannas is a notable site on the Caribbean island of Trinidad. A unique savanna and wetland system. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Rare plants. Another point of interest: Scientifically important. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Aripo Savannen.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Aripo szavannák.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Savanele Aripo.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Aripo Savannas.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-nariva-swamp-life-v2",
    type: "animal-habitat",
    parent: "TT-MRC",
    coords: [-61.02, 10.32],
    name: { de: "Nariva Sumpf", hu: "Nariva mocsár", ro: "Mlaștina Nariva", en: "Nariva Swamp"},
    image: "/poi-images/tt-nariva-swamp-life-v2.webp",
    description: { de: "Trinidads größtes Süßwasser-Feuchtgebiet.", hu: "Trinidad legnagyobb édesvízi vizes élőhelye.", ro: "Cea mai mare zonă umedă de apă dulce din Trinidad.", en: "The largest freshwater wetland in Trinidad." },
    facts: {
      de: ["RAMSAR-Gebiet", "Heimat von Manatis", "Reich an Vogelwelt"],
      hu: ["RAMSAR terület", "Manátuszok otthona", "Gazdag madárvilág"],
      ro: ["Sit RAMSAR", "Casa manatinilor", "Bogată în păsări"],
      en: ["RAMSAR site", "Home to manatees", "Rich birdlife"]
    }, image: "/poi-images/tt-nariva-swamp-life-v2.webp",
    descriptionAdvanced: {
      de: "Nariva Sumpf ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Trinidads größtes Süßwasser-Feuchtgebiet. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: RAMSAR-Gebiet. Hervorzuheben ist außerdem: Heimat von Manatis. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Nariva mocsár a karibi Trinidad sziget egyik figyelemre méltó helye. Trinidad legnagyobb édesvízi vizes élőhelye. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: RAMSAR terület. További fontos jellemző: Manátuszok otthona. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Mlaștina Nariva este un loc remarcabil de pe insula caraibiană Trinidad. Cea mai mare zonă umedă de apă dulce din Trinidad. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Sit RAMSAR. Un alt aspect important: Casa manatinilor. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Nariva Swamp is a notable site on the Caribbean island of Trinidad. The largest freshwater wetland in Trinidad. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: RAMSAR site. Another point of interest: Home to manatees. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Nariva Sumpf.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Nariva mocsár.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Mlaștina Nariva.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Nariva Swamp.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-bush-bush-sanctuary-life-v2",
    type: "animal-habitat",
    parent: "TT-MRC",
    coords: [-61.0, 10.33],
    name: { de: "Bush Bush Schutzgebiet", hu: "Bush Bush védelmi terület", ro: "Sanctuarul Bush Bush", en: "Bush Bush Sanctuary"},
    image: "/poi-images/tt-bush-bush-sanctuary-life-v2.webp",
    description: { de: "Ein geschütztes Sumpf- und Waldgebiet bei Nariva.", hu: "Védett mocsaras és erdős terület Nariva mellett.", ro: "O zonă protejată de mlaștină și pădure lângă Nariva.", en: "A protected swamp and forest area near Nariva." },
    facts: {
      de: ["Wilde Mangroven", "Naturkundliche Bootsfahrt", "Teil des Nariva-Gebiets"],
      hu: ["Vad mangrovek", "Természeti hajókirándulás", "A Nariva-terület része"],
      ro: ["Mangrove sălbatice", "Tur cu barca în natură", "Parte din zona Nariva"],
      en: ["Wild mangroves", "Nature boat rides", "Part of the Nariva area"]
    }, image: "/poi-images/tt-bush-bush-sanctuary-life-v2.webp",
    descriptionAdvanced: {
      de: "Bush Bush Schutzgebiet ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein geschütztes Sumpf- und Waldgebiet bei Nariva. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Wilde Mangroven. Hervorzuheben ist außerdem: Naturkundliche Bootsfahrt. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Bush Bush védelmi terület a karibi Trinidad sziget egyik figyelemre méltó helye. Védett mocsaras és erdős terület Nariva mellett. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Vad mangrovek. További fontos jellemző: Természeti hajókirándulás. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Sanctuarul Bush Bush este un loc remarcabil de pe insula caraibiană Trinidad. O zonă protejată de mlaștină și pădure lângă Nariva. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Mangrove sălbatice. Un alt aspect important: Tur cu barca în natură. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Bush Bush Sanctuary is a notable site on the Caribbean island of Trinidad. A protected swamp and forest area near Nariva. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Wild mangroves. Another point of interest: Nature boat rides. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Bush Bush Schutzgebiet.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Bush Bush védelmi terület.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Sanctuarul Bush Bush.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Bush Bush Sanctuary.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-trinity-hills-life-v2",
    type: "animal-habitat",
    parent: "TT-PRT",
    coords: [-61.16, 10.08],
    name: { de: "Trinity Hills Wildnisgebiet", hu: "Trinity Hills vadonvédelmi terület", ro: "Sanctuarul Trinity Hills", en: "Trinity Hills Wildlife Sanctuary"},
    image: "/poi-images/tt-trinity-hills-life-v2.webp",
    description: { de: "Ein hügeliges Waldschutzgebiet im Südosten Trinidads.", hu: "Dombos erdős védelmi terület Trinidad délkeleti részén.", ro: "O zonă forestieră deluroasă protejată în sud-estul Trinidadului.", en: "A hilly forest sanctuary in southeastern Trinidad." },
    facts: {
      de: ["Wichtiger Wassereinzugsbereich", "Reiches Tierleben", "Beliebt zum Wandern"],
      hu: ["Fontos vízgyűjtő terület", "Gazdag állatvilág", "Kedvelt túrázóhely"],
      ro: ["Bazin hidrografic important", "Faună bogată", "Popular pentru drumeții"],
      en: ["Important watershed", "Rich wildlife", "Popular for hiking"]
    }, image: "/poi-images/tt-trinity-hills-life-v2.webp",
    descriptionAdvanced: {
      de: "Trinity Hills Wildnisgebiet ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein hügeliges Waldschutzgebiet im Südosten Trinidads. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Wichtiger Wassereinzugsbereich. Hervorzuheben ist außerdem: Reiches Tierleben. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Trinity Hills vadonvédelmi terület a karibi Trinidad sziget egyik figyelemre méltó helye. Dombos erdős védelmi terület Trinidad délkeleti részén. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Fontos vízgyűjtő terület. További fontos jellemző: Gazdag állatvilág. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Sanctuarul Trinity Hills este un loc remarcabil de pe insula caraibiană Trinidad. O zonă forestieră deluroasă protejată în sud-estul Trinidadului. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Bazin hidrografic important. Un alt aspect important: Faună bogată. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Trinity Hills Wildlife Sanctuary is a notable site on the Caribbean island of Trinidad. A hilly forest sanctuary in southeastern Trinidad. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Important watershed. Another point of interest: Rich wildlife. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Trinity Hills Wildnisgebiet.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Trinity Hills vadonvédelmi terület.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Sanctuarul Trinity Hills.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Trinity Hills Wildlife Sanctuary.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-matura-turtle-beach-life-v2",
    type: "animal-habitat",
    parent: "TT-SGE",
    coords: [-61.0, 10.67],
    name: { de: "Matura Schildkrötenstrand", hu: "Matura teknősös strand", ro: "Plaja Matura pentru țestoase", en: "Matura Turtle Beach"},
    image: "/poi-images/tt-matura-turtle-beach-life-v2.webp",
    description: { de: "Ein bekannter Nistplatz für Meeresschildkröten.", hu: "Ismert fészkelőhely a tengeri teknősök számára.", ro: "Un loc cunoscut de cuibărit pentru țestoasele marine.", en: "A well-known nesting beach for sea turtles." },
    facts: {
      de: ["Lederschildkröten", "Nächtliche Naturbeobachtung", "Schutz wichtiger Nester"],
      hu: ["Bőrösteknősök", "Éjszakai természetmegfigyelés", "Fontos fészkelőhely védelme"],
      ro: ["Țestoase piele", "Observare nocturnă a naturii", "Protejarea cuiburilor"],
      en: ["Leatherback turtles", "Night nature watching", "Protects important nests"]
    }, image: "/poi-images/tt-matura-turtle-beach-life-v2.webp",
    descriptionAdvanced: {
      de: "Matura Schildkrötenstrand ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein bekannter Nistplatz für Meeresschildkröten. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Lederschildkröten. Hervorzuheben ist außerdem: Nächtliche Naturbeobachtung. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Matura teknősös strand a karibi Trinidad sziget egyik figyelemre méltó helye. Ismert fészkelőhely a tengeri teknősök számára. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Bőrösteknősök. További fontos jellemző: Éjszakai természetmegfigyelés. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Plaja Matura pentru țestoase este un loc remarcabil de pe insula caraibiană Trinidad. Un loc cunoscut de cuibărit pentru țestoasele marine. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Țestoase piele. Un alt aspect important: Observare nocturnă a naturii. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Matura Turtle Beach is a notable site on the Caribbean island of Trinidad. A well-known nesting beach for sea turtles. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Leatherback turtles. Another point of interest: Night nature watching. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Matura Schildkrötenstrand.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Matura teknősös strand.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Plaja Matura pentru țestoase.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Matura Turtle Beach.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-grande-riviere-turtle-beach-life-v2",
    type: "animal-habitat",
    parent: "TT-SGE",
    coords: [-61.03, 10.7],
    name: { de: "Grande Riviere Schildkrötenstrand", hu: "Grande Riviere teknősös strand", ro: "Plaja Grande Riviere pentru țestoase", en: "Grande Riviere Turtle Beach"},
    image: "/poi-images/tt-grande-riviere-turtle-beach-life-v2.webp",
    description: { de: "Einer der wichtigsten Strände für Lederschildkröten in der Karibik.", hu: "A Karib-térség egyik fontos bőrösteknősös partja.", ro: "Una dintre cele mai importante plaje pentru țestoasele marine din Caraibe.", en: "One of the Caribbean's most important leatherback beaches." },
    facts: {
      de: ["Starke Schildkrötenpopulation", "Geführte Nachtbesuche", "Schutzgebiet am Meer"],
      hu: ["Erős teknősállomány", "Vezetett éjszakai látogatások", "Tengeri védett terület"],
      ro: ["Populație puternică de țestoase", "Vizite ghidate noaptea", "Zonă protejată de coastă"],
      en: ["Strong turtle population", "Guided night visits", "Coastal protected area"]
    }, image: "/poi-images/tt-grande-riviere-turtle-beach-life-v2.webp",
    descriptionAdvanced: {
      de: "Grande Riviere Schildkrötenstrand ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Einer der wichtigsten Strände für Lederschildkröten in der Karibik. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Starke Schildkrötenpopulation. Hervorzuheben ist außerdem: Geführte Nachtbesuche. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Grande Riviere teknősös strand a karibi Trinidad sziget egyik figyelemre méltó helye. A Karib-térség egyik fontos bőrösteknősös partja. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Erős teknősállomány. További fontos jellemző: Vezetett éjszakai látogatások. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Plaja Grande Riviere pentru țestoase este un loc remarcabil de pe insula caraibiană Trinidad. Una dintre cele mai importante plaje pentru țestoasele marine din Caraibe. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Populație puternică de țestoase. Un alt aspect important: Vizite ghidate noaptea. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Grande Riviere Turtle Beach is a notable site on the Caribbean island of Trinidad. One of the Caribbean's most important leatherback beaches. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Strong turtle population. Another point of interest: Guided night visits. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Grande Riviere Schildkrötenstrand.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Grande Riviere teknősös strand.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Plaja Grande Riviere pentru țestoase.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Grande Riviere Turtle Beach.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-toco-coconut-groves-life-v2",
    type: "agriculture",
    parent: "TT-SGE",
    coords: [-61.0, 10.78],
    name: { de: "Toco Kokospalmenhaine", hu: "Toco kókuszligetek", ro: "Livezi de cocotieri Toco", en: "Toco Coconut Groves"},
    image: "/poi-images/tt-toco-coconut-groves-life-v2.webp",
    description: { de: "Kokosplantagen entlang der Ostküste.", hu: "Kókuszültetvények a keleti part mentén.", ro: "Plantații de cocotieri de-a lungul coastei de est.", en: "Coconut plantations along the east coast." },
    facts: {
      de: ["Küstenlandwirtschaft", "Schattige Haine", "Wichtige Dorfkultur"],
      hu: ["Parti mezőgazdaság", "Árnyékos ligetek", "Fontos falusi kultúra"],
      ro: ["Agricultură de coastă", "Livezi umbroase", "Cultură locală importantă"],
      en: ["Coastal farming", "Shady groves", "Important village culture"]
    }, image: "/poi-images/tt-toco-coconut-groves-life-v2.webp",
    descriptionAdvanced: {
      de: "Toco Kokospalmenhaine ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Kokosplantagen entlang der Ostküste. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Küstenlandwirtschaft. Hervorzuheben ist außerdem: Schattige Haine. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Toco kókuszligetek a karibi Trinidad sziget egyik figyelemre méltó helye. Kókuszültetvények a keleti part mentén. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Parti mezőgazdaság. További fontos jellemző: Árnyékos ligetek. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Livezi de cocotieri Toco este un loc remarcabil de pe insula caraibiană Trinidad. Plantații de cocotieri de-a lungul coastei de est. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Agricultură de coastă. Un alt aspect important: Livezi umbroase. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Toco Coconut Groves is a notable site on the Caribbean island of Trinidad. Coconut plantations along the east coast. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Coastal farming. Another point of interest: Shady groves. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Toco Kokospalmenhaine.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Toco kókuszligetek.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Livezi de cocotieri Toco.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Toco Coconut Groves.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-pigeon-point-life-v2",
    type: "kid-landmark",
    parent: "TT-TOB",
    coords: [-60.8353, 11.1733],
    name: { de: "Pigeon Point", hu: "Pigeon Point", ro: "Pigeon Point", en: "Pigeon Point"},
    image: "/poi-images/tt-pigeon-point-life-v2.webp",
    description: { de: "Tobagos ikonischster Strand mit dem bekannten Holzsteg.", hu: "Tobago ikonikus strandja a híres fahíddal.", ro: "Cea mai iconică plajă din Tobago, cu celebrul ponton din lemn.", en: "Tobago's iconic beach with the famous wooden jetty." },
    facts: {
      de: ["Weißer Sand", "Türkisfarbenes Wasser", "Beliebt für Familien"],
      hu: ["Fehér homok", "Türkiz víz", "Családok kedvence"],
      ro: ["Nisip alb", "Apă turcoaz", "Popular pentru familii"],
      en: ["White sand", "Turquoise water", "Popular with families"]
    }, image: "/poi-images/tt-pigeon-point-life-v2.webp",
    descriptionAdvanced: {
      de: "Pigeon Point ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Tobagos ikonischster Strand mit dem bekannten Holzsteg. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Weißer Sand. Hervorzuheben ist außerdem: Türkisfarbenes Wasser. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Pigeon Point a karibi Trinidad sziget egyik figyelemre méltó helye. Tobago ikonikus strandja a híres fahíddal. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Fehér homok. További fontos jellemző: Türkiz víz. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Pigeon Point este un loc remarcabil de pe insula caraibiană Trinidad. Cea mai iconică plajă din Tobago, cu celebrul ponton din lemn. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Nisip alb. Un alt aspect important: Apă turcoaz. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Pigeon Point is a notable site on the Caribbean island of Trinidad. Tobago's iconic beach with the famous wooden jetty. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: White sand. Another point of interest: Turquoise water. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Pigeon Point.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Pigeon Point.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Pigeon Point.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Pigeon Point.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-buccoo-reef-life-v2",
    type: "animal-habitat",
    parent: "TT-TOB",
    coords: [-60.82, 11.18],
    name: { de: "Buccoo Riff", hu: "Buccoo zátony", ro: "Reciful Buccoo", en: "Buccoo Reef"},
    image: "/poi-images/tt-buccoo-reef-life-v2.webp",
    description: { de: "Ein geschütztes Korallenriff mit großer mariner Vielfalt.", hu: "Védett korallzátony nagy tengeri sokféleséggel.", ro: "Un recif de corali protejat cu mare diversitate marină.", en: "A protected coral reef with rich marine life." },
    facts: {
      de: ["Glasbodenboote", "Farbenfrohe Fische", "Wichtig für den Tourismus"],
      hu: ["Üvegfenekű hajók", "Színes halak", "Fontos a turizmusnak"],
      ro: ["Bărci cu fund de sticlă", "Pești colorați", "Important pentru turism"],
      en: ["Glass-bottom boats", "Colorful fish", "Important for tourism"]
    }, image: "/poi-images/tt-buccoo-reef-life-v2.webp",
    descriptionAdvanced: {
      de: "Buccoo Riff ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein geschütztes Korallenriff mit großer mariner Vielfalt. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Glasbodenboote. Hervorzuheben ist außerdem: Farbenfrohe Fische. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Buccoo zátony a karibi Trinidad sziget egyik figyelemre méltó helye. Védett korallzátony nagy tengeri sokféleséggel. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Üvegfenekű hajók. További fontos jellemző: Színes halak. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Reciful Buccoo este un loc remarcabil de pe insula caraibiană Trinidad. Un recif de corali protejat cu mare diversitate marină. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Bărci cu fund de sticlă. Un alt aspect important: Pești colorați. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Buccoo Reef is a notable site on the Caribbean island of Trinidad. A protected coral reef with rich marine life. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Glass-bottom boats. Another point of interest: Colorful fish. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Buccoo Riff.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Buccoo zátony.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Reciful Buccoo.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Buccoo Reef.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
      {
    id: "tt-little-tobago-bird-sanctuary-life-v2",
    type: "animal-habitat",
    parent: "TT-TOB",
    coords: [-60.52, 11.35],
    name: { de: "Little Tobago Vogelreservat", hu: "Little Tobago madárrezervátum", ro: "Sanctuarul de păsări Little Tobago", en: "Little Tobago Bird Sanctuary"},
    image: "/poi-images/tt-little-tobago-bird-sanctuary-life-v2.webp",
    description: { de: "Eine kleine Insel mit wichtigem Brutgebiet für Seevögel.", hu: "Kis sziget fontos tengeri madár fészkelőhellyel.", ro: "O insulă mică cu un loc important de cuibărit pentru păsări marine.", en: "A small island with an important seabird nesting site." },
    facts: {
      de: ["Seevogelkolonien", "Beliebt bei Bootsausflügen", "Unbewohnt"],
      hu: ["Tengeri madár kolóniák", "Népszerű hajókirándulás", "Lakatlan"],
      ro: ["Colonii de păsări marine", "Populară pentru excursii cu barcă", "Nelocuită"],
      en: ["Seabird colonies", "Popular boat trips", "Uninhabited"]
    }, image: "/poi-images/tt-little-tobago-bird-sanctuary-life-v2.webp",
    descriptionAdvanced: {
      de: "Little Tobago Vogelreservat ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Eine kleine Insel mit wichtigem Brutgebiet für Seevögel. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Seevogelkolonien. Hervorzuheben ist außerdem: Beliebt bei Bootsausflügen. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Little Tobago madárrezervátum a karibi Trinidad sziget egyik figyelemre méltó helye. Kis sziget fontos tengeri madár fészkelőhellyel. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Tengeri madár kolóniák. További fontos jellemző: Népszerű hajókirándulás. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Sanctuarul de păsări Little Tobago este un loc remarcabil de pe insula caraibiană Trinidad. O insulă mică cu un loc important de cuibărit pentru păsări marine. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Colonii de păsări marine. Un alt aspect important: Populară pentru excursii cu barcă. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Little Tobago Bird Sanctuary is a notable site on the Caribbean island of Trinidad. A small island with an important seabird nesting site. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Seabird colonies. Another point of interest: Popular boat trips. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Little Tobago Vogelreservat.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Little Tobago madárrezervátum.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Sanctuarul de păsări Little Tobago.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Little Tobago Bird Sanctuary.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-scarborough-botanical-gardens-life-v2",
    type: "kid-landmark",
    parent: "TT-TOB",
    coords: [-60.7333, 11.1833],
    name: { de: "Scarborough Botanischer Garten", hu: "Scarborough botanikus kert", ro: "Grădina botanică Scarborough", en: "Scarborough Botanical Gardens"},
    image: "/poi-images/tt-scarborough-botanical-gardens-life-v2.webp",
    description: { de: "Ein ruhiger Garten mit tropischen Pflanzen in Scarborough.", hu: "Csendes kert trópusi növényekkel Scarboroughban.", ro: "O grădină liniștită cu plante tropicale în Scarborough.", en: "A quiet garden with tropical plants in Scarborough." },
    facts: {
      de: ["Schattenwege", "Tropische Pflanzen", "Gute Familienpause"],
      hu: ["Árnyékos sétányok", "Trópusi növények", "Kellemes családi pihenő"],
      ro: ["Alei umbroase", "Plante tropicale", "Pauză bună pentru familii"],
      en: ["Shaded paths", "Tropical plants", "Nice family stop"]
    }, image: "/poi-images/tt-scarborough-botanical-gardens-life-v2.webp",
    descriptionAdvanced: {
      de: "Scarborough Botanischer Garten ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein ruhiger Garten mit tropischen Pflanzen in Scarborough. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Schattenwege. Hervorzuheben ist außerdem: Tropische Pflanzen. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Scarborough botanikus kert a karibi Trinidad sziget egyik figyelemre méltó helye. Csendes kert trópusi növényekkel Scarboroughban. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Árnyékos sétányok. További fontos jellemző: Trópusi növények. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Grădina botanică Scarborough este un loc remarcabil de pe insula caraibiană Trinidad. O grădină liniștită cu plante tropicale în Scarborough. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Alei umbroase. Un alt aspect important: Plante tropicale. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Scarborough Botanical Gardens is a notable site on the Caribbean island of Trinidad. A quiet garden with tropical plants in Scarborough. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Shaded paths. Another point of interest: Tropical plants. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Scarborough Botanischer Garten.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Scarborough botanikus kert.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Grădina botanică Scarborough.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Scarborough Botanical Gardens.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-castara-fishing-village-life-v2",
    type: "kid-landmark",
    parent: "TT-TOB",
    coords: [-60.6833, 11.2667],
    name: { de: "Castara Fischerdorf", hu: "Castara halászfalu", ro: "Satul pescăresc Castara", en: "Castara Fishing Village"},
    image: "/poi-images/tt-castara-fishing-village-life-v2.webp",
    description: { de: "Ein ruhiges Fischerdorf mit kleinem Strand und lokaler Küche.", hu: "Csendes halászfalu kis stranddal és helyi konyhával.", ro: "Un sat pescăresc liniștit, cu plajă mică și bucătărie locală.", en: "A quiet fishing village with a small beach and local food." },
    facts: {
      de: ["Bucht mit Dorfcharakter", "Lokale Fischerboote", "Beliebt für Natururlaub"],
      hu: ["Falu jellegű öböl", "Helyi halászcsónakok", "Népszerű természetközeli nyaralásra"],
      ro: ["Golf cu aer de sat", "Bărci de pescari locale", "Popular pentru vacanțe în natură"],
      en: ["Village-style bay", "Local fishing boats", "Popular for nature stays"]
    }, image: "/poi-images/tt-castara-fishing-village-life-v2.webp",
    descriptionAdvanced: {
      de: "Castara Fischerdorf ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein ruhiges Fischerdorf mit kleinem Strand und lokaler Küche. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Bucht mit Dorfcharakter. Hervorzuheben ist außerdem: Lokale Fischerboote. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Castara halászfalu a karibi Trinidad sziget egyik figyelemre méltó helye. Csendes halászfalu kis stranddal és helyi konyhával. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Falu jellegű öböl. További fontos jellemző: Helyi halászcsónakok. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Satul pescăresc Castara este un loc remarcabil de pe insula caraibiană Trinidad. Un sat pescăresc liniștit, cu plajă mică și bucătărie locală. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Golf cu aer de sat. Un alt aspect important: Bărci de pescari locale. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Castara Fishing Village is a notable site on the Caribbean island of Trinidad. A quiet fishing village with a small beach and local food. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Village-style bay. Another point of interest: Local fishing boats. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Castara Fischerdorf.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Castara halászfalu.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Satul pescăresc Castara.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Castara Fishing Village.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-arima-cocoa-estate-life-v2",
    type: "agriculture",
    parent: "TT-TUP",
    coords: [-61.28, 10.62],
    name: { de: "Arima Kakaoanbau", hu: "Arima kakaó", ro: "Plantații de cacao Arima", en: "Arima Cocoa Estate"},
    image: "/poi-images/tt-arima-cocoa-estate-life-v2.webp",
    description: { de: "Kleine Kakaoanbauflächen in der Umgebung von Arima.", hu: "Kis kakaótermesztési területek Arima környékén.", ro: "Mici suprafețe de cacao în zona Arima.", en: "Small cocoa growing areas around Arima." },
    facts: {
      de: ["Traditionelles Kakaoerbe", "Familienbetriebe", "Teil der Ost-Trinidad-Landschaft"],
      hu: ["Hagyományos kakaóörökség", "Családi gazdaságok", "Kelet-Trinidad tájának része"],
      ro: ["Moștenire tradițională de cacao", "Ferme de familie", "Parte din peisajul estic"],
      en: ["Traditional cocoa heritage", "Family-run farms", "Part of the eastern landscape"]
    },
    descriptionAdvanced: {
      de: "Arima Kakaoanbau ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Kleine Kakaoanbauflächen in der Umgebung von Arima. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Traditionelles Kakaoerbe. Hervorzuheben ist außerdem: Familienbetriebe. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Arima kakaó a karibi Trinidad sziget egyik figyelemre méltó helye. Kis kakaótermesztési területek Arima környékén. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Hagyományos kakaóörökség. További fontos jellemző: Családi gazdaságok. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Plantații de cacao Arima este un loc remarcabil de pe insula caraibiană Trinidad. Mici suprafețe de cacao în zona Arima. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Moștenire tradițională de cacao. Un alt aspect important: Ferme de familie. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Arima Cocoa Estate is a notable site on the Caribbean island of Trinidad. Small cocoa growing areas around Arima. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Traditional cocoa heritage. Another point of interest: Family-run farms. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Arima Kakaoanbau.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Arima kakaó.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Plantații de cacao Arima.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Arima Cocoa Estate.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-freeport-rice-fields-life-v2",
    type: "agriculture",
    parent: "TT-CTT",
    coords: [-61.4, 10.45],
    name: { de: "Freeport Reisfelder", hu: "Freeport rizsföldek", ro: "Câmpurile de orez Freeport", en: "Freeport Rice Fields"},
    image: "/poi-images/tt-freeport-rice-fields-life-v2.webp",
    description: { de: "Reisanbauflächen in Zentral-Trinidad.", hu: "Rizstermesztési területek Közép-Trinidadban.", ro: "Zone de cultivare a orezului în centrul Trinidadului.", en: "Rice growing areas in central Trinidad." },
    facts: {
      de: ["Wichtige Nahrungspflanze", "Flache Bewässerungsfelder", "Ländliche Wirtschaftsgrundlage"],
      hu: ["Fontos élelmiszernövény", "Sekély öntözött táblák", "Vidéki gazdasági alap"],
      ro: ["Cultură alimentară importantă", "Câmpuri irigate de mică adâncime", "Bază economică rurală"],
      en: ["Important food crop", "Shallow irrigated fields", "Rural economic base"]
    }, image: "/poi-images/tt-freeport-rice-fields-life-v2.webp",
    descriptionAdvanced: {
      de: "Freeport Reisfelder ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Reisanbauflächen in Zentral-Trinidad. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Wichtige Nahrungspflanze. Hervorzuheben ist außerdem: Flache Bewässerungsfelder. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Freeport rizsföldek a karibi Trinidad sziget egyik figyelemre méltó helye. Rizstermesztési területek Közép-Trinidadban. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Fontos élelmiszernövény. További fontos jellemző: Sekély öntözött táblák. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Câmpurile de orez Freeport este un loc remarcabil de pe insula caraibiană Trinidad. Zone de cultivare a orezului în centrul Trinidadului. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Cultură alimentară importantă. Un alt aspect important: Câmpuri irigate de mică adâncime. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Freeport Rice Fields is a notable site on the Caribbean island of Trinidad. Rice growing areas in central Trinidad. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Important food crop. Another point of interest: Shallow irrigated fields. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Freeport Reisfelder.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Freeport rizsföldek.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Câmpurile de orez Freeport.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Freeport Rice Fields.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-siparia-dairy-farms-life-v2",
    type: "agriculture",
    parent: "TT-SIP",
    coords: [-61.5, 10.13],
    name: { de: "Siparia Milchfarmen", hu: "Siparia tejfarmok", ro: "Ferme de lapte Siparia", en: "Siparia Dairy Farms"},
    image: "/poi-images/tt-siparia-dairy-farms-life-v2.webp",
    description: { de: "Weidebetriebe im Südwesten, die Milch und Käse liefern.", hu: "Legelő gazdaságok a délnyugaton, tejjel és sajttal.", ro: "Ferme de pășunat în sud-vest care produc lapte și brânză.", en: "Pasture farms in the southwest producing milk and cheese." },
    facts: {
      de: ["Viehzucht", "Lokale Molkereiprodukte", "Grüne Weideflächen"],
      hu: ["Szarvasmarha tartás", "Helyi tejtermékek", "Zöld legelők"],
      ro: ["Creșterea bovinelor", "Produse lactate locale", "Pășuni verzi"],
      en: ["Cattle farming", "Local dairy products", "Green pastures"]
    }, image: "/poi-images/tt-siparia-dairy-farms-life-v2.webp",
    descriptionAdvanced: {
      de: "Siparia Milchfarmen ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Weidebetriebe im Südwesten, die Milch und Käse liefern. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Viehzucht. Hervorzuheben ist außerdem: Lokale Molkereiprodukte. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Siparia tejfarmok a karibi Trinidad sziget egyik figyelemre méltó helye. Legelő gazdaságok a délnyugaton, tejjel és sajttal. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Szarvasmarha tartás. További fontos jellemző: Helyi tejtermékek. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Ferme de lapte Siparia este un loc remarcabil de pe insula caraibiană Trinidad. Ferme de pășunat în sud-vest care produc lapte și brânză. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Creșterea bovinelor. Un alt aspect important: Produse lactate locale. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Siparia Dairy Farms is a notable site on the Caribbean island of Trinidad. Pasture farms in the southwest producing milk and cheese. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Cattle farming. Another point of interest: Local dairy products. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Siparia Milchfarmen.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Siparia tejfarmok.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Ferme de lapte Siparia.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Siparia Dairy Farms.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
  {
    id: "tt-point-fortin-beach-park-life-v2",
    type: "kid-landmark",
    parent: "TT-PTF",
    coords: [-61.68, 10.18],
    name: { de: "Point Fortin Strandpark", hu: "Point Fortin strandpark", ro: "Parcul de plajă Point Fortin", en: "Point Fortin Beach Park"},
    image: "/poi-images/tt-point-fortin-beach-park-life-v2.webp",
    description: { de: "Ein lokaler Freizeitstrand an der Südwestküste.", hu: "Helyi szabadidős strand a délnyugati parton.", ro: "O plajă de agrement locală pe coasta de sud-vest.", en: "A local leisure beach on the southwest coast." },
    facts: {
      de: ["Familienfreundlich", "Sonnenuntergangsblick", "Kleiner Küstenpark"],
      hu: ["Családbarát", "Naplemente kilátás", "Kis parti park"],
      ro: ["Prietenos pentru familii", "Vederi la apus", "Mic parc de coastă"],
      en: ["Family-friendly", "Sunset views", "Small coastal park"]
    }, image: "/poi-images/tt-point-fortin-beach-park-life-v2.webp",
    descriptionAdvanced: {
      de: "Point Fortin Strandpark ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Ein lokaler Freizeitstrand an der Südwestküste. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Familienfreundlich. Hervorzuheben ist außerdem: Sonnenuntergangsblick. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Point Fortin strandpark a karibi Trinidad sziget egyik figyelemre méltó helye. Helyi szabadidős strand a délnyugati parton. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Családbarát. További fontos jellemző: Naplemente kilátás. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Parcul de plajă Point Fortin este un loc remarcabil de pe insula caraibiană Trinidad. O plajă de agrement locală pe coasta de sud-vest. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Prietenos pentru familii. Un alt aspect important: Vederi la apus. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Point Fortin Beach Park is a notable site on the Caribbean island of Trinidad. A local leisure beach on the southwest coast. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Family-friendly. Another point of interest: Sunset views. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Point Fortin Strandpark.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Point Fortin strandpark.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Parcul de plajă Point Fortin.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Point Fortin Beach Park.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  },
    {
    id: "tt-saut-deau-island-life-v2",
    type: "animal-habitat",
    parent: "TT-DMN",
    coords: [-61.55, 10.77],
    name: { de: "Saut d Eau Insel", hu: "Saut d Eau sziget", ro: "Insula Saut d Eau", en: "Saut d'Eau Island"},
    image: "/poi-images/tt-saut-deau-island-life-v2.webp",
    description: { de: "Eine kleine bewaldete Insel als Schutzgebiet für Seevögel.", hu: "Kis erdős sziget, amely tengeri madarak védett területe.", ro: "O mică insulă împădurită, sanctuar pentru păsări marine.", en: "A small wooded island that protects seabirds." },
    facts: {
      de: ["Vogelschutz", "Unbewohnt", "Tropische Küstenvegetation"],
      hu: ["Madárvédelem", "Lakatlan", "Trópusi parti növényzet"],
      ro: ["Protejarea păsărilor", "Nelocuită", "Vegetație tropicală de coastă"],
      en: ["Bird protection", "Uninhabited", "Tropical coastal vegetation"]
    },
    descriptionAdvanced: {
      de: "Saut d Eau Insel ist ein bemerkenswerter Ort auf der Karibikinsel Trinidad. Eine kleine bewaldete Insel als Schutzgebiet für Seevögel. Die Lage spiegelt zugleich die geographische Vielfalt und das kulturelle Erbe der Insel wider. Bemerkenswert ist dabei: Vogelschutz. Hervorzuheben ist außerdem: Unbewohnt. Für Schülerinnen und Schüler bietet dieser Ort einen anschaulichen Bezug zwischen Lehrplan und realer Welt. Geographie K7 — Inselgeographie und kulturelles Erbe.",
      hu: "Saut d Eau sziget a karibi Trinidad sziget egyik figyelemre méltó helye. Kis erdős sziget, amely tengeri madarak védett területe. Elhelyezkedése egyszerre tükrözi a sziget földrajzi változatosságát és gazdag kulturális örökségét. Említésre méltó, hogy: Madárvédelem. További fontos jellemző: Lakatlan. A tanítás során jól szemlélteti a tananyag és a való világ közötti kapcsolatot. Földrajz K7 — sziget-földrajz és kulturális örökség.",
      ro: "Insula Saut d Eau este un loc remarcabil de pe insula caraibiană Trinidad. O mică insulă împădurită, sanctuar pentru păsări marine. Așezarea reflectă deopotrivă diversitatea geografică și moștenirea culturală a insulei. Este de remarcat: Protejarea păsărilor. Un alt aspect important: Nelocuită. Pentru elevi, locul oferă o legătură concretă între conținutul programei școlare și lumea reală. Geografie K7 — geografia insulară și patrimoniul cultural.",
      en: "Saut d'Eau Island is a notable site on the Caribbean island of Trinidad. A small wooded island that protects seabirds. Its setting reflects both the geographical diversity and the cultural heritage of the island. Notably: Bird protection. Another point of interest: Uninhabited. For students, the location offers a concrete bridge between curriculum content and the real world. Geography K7 — island geography and cultural heritage."
    },
    factsAdvanced: {
      de: ["Bekannt unter dem Namen Saut d Eau Insel.", "Liegt auf der Karibikinsel Trinidad.", "Teil des kulturellen und geographischen Erbes Trinidads.", "Eingebettet in die tropische Klimazone der südlichen Karibik."],
      hu: ["A helyszín neve: Saut d Eau sziget.", "A karibi Trinidad szigetén található.", "Trinidad kulturális és földrajzi örökségének része.", "A déli Karib-térség trópusi éghajlatú terletén fekszik."],
      ro: ["Este cunoscut sub numele de Insula Saut d Eau.", "Este situat pe insula caraibiană Trinidad.", "Face parte din moștenirea culturală și geografică a Trinidadului.", "Se află în zona climatică tropicală a Caraibelor de Sud."],
      en: ["Known by the name Saut d'Eau Island.", "Located on the Caribbean island of Trinidad.", "Part of Trinidad’s cultural and geographical heritage.", "Set within the tropical climate zone of the southern Caribbean."]
    }
  }
];

