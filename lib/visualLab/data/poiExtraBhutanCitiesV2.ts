import type { POI } from "./poi";

export const poiExtraBhutanCitiesV2: POI[] = [
  {
    id: "thimphu-cities-v2",
    type: "state-capital",
    parent: "BT-15",
    coords: [89.6390, 27.4728],
    name: { de: "Thimphu", hu: "Thimphu", ro: "Thimphu", en: "Thimphu",
},
    description: {
      de: "Thimphu ist die Hauptstadt und größte Stadt von Bhutan, bekannt für ihre Mischung aus traditioneller Kultur und Moderne ohne Verkehrsampeln.",
      hu: "Thimphu Bhután fővárosa és legnagyobb városa, amely a hagyományos kultúra és a modernitás egyedi ötvözetéről, valamint a közlekedési lámpák hiányáról ismert.",
      ro: "Thimphu este capitala și cel mai mare oraș din Bhutan, remarcat pentru amestecul său de cultură tradițională și modernitate fără semafoare.",
      en: "Thimphu is the capital and largest city of Bhutan, known for its blend of traditional culture and modernity without traffic lights."
    },
    facts: {
      de: [
        "Thimphu liegt im westlichen zentralen Teil von Bhutan.",
        "Es wurde im Jahr 1961 zur Hauptstadt des Königreichs."
      ],
      hu: [
        "Thimphu Bhután nyugati-középső részén található.",
        "1961-ben lett a királyság fővárosa."
      ],
      ro: [
        "Thimphu este situat în partea central-vestică a Bhutanului.",
        "A devenit capitala regatului în 1961."
      ],
      en: [
        "Thimphu is located in the western central part of Bhutan.",
        "It became the capital of the kingdom in 1961."
      ]
    },
    image: "/poi-images/thimphu-cities-v2.webp",
    descriptionAdvanced: {
      de: "Thimphu, die Hauptstadt Bhutans, liegt auf einer Höhe von etwa 2.334 Metern im Tal des Wang Chhu. Die Stadt hat rund 138.000 Einwohner (2023) und ist das politische, wirtschaftliche und kulturelle Zentrum des Landes. Anders als viele asiatische Hauptstädte besitzt Thimphu keine großen Hochhäuser; die Bauordnung schreibt traditionelle bhutanische Architektur mit verzierten Holzfenstern und farbenfrohen Motiven vor. Das Stadtbild wird vom Tashichho Dzong dominiert, einer massiven Festung, die seit 1952 Sitz der Regierung und des Königs ist. Die Nationalversammlung tagt in einem modernen Gebäude, das jedoch die traditionellen Elemente bewahrt. Thimphu ist bekannt für seine lebendige Kultur: Das jährliche Thimphu Tshechu zieht Tausende von Besuchern an. Trotz der Modernisierung ist die Stadt autofrei im Kernbereich nicht, es gibt keine Ampeln – Verkehrspolizisten regeln den Verkehr. Die Wirtschaft basiert auf Verwaltung, Tourismus und Handwerk; traditionelle Produkte wie handgewebte Stoffe und Thangka-Malereien werden exportiert.",
      hu: "Thimphu, Bhután fővárosa, a Wang Chhu völgyében, 2.334 méteres tengerszint feletti magasságban fekszik. Népessége körülbelül 138.000 fő (2023), ezzel a legnagyobb város az országban. A város építészetére szigorú szabályozások vonatkoznak: minden épületnek követnie kell a hagyományos bhutáni stílust, beleértve a gazdagon festett faablakokat és a lejtős, dongaszerű tetőket. A Tashichho Dzong, egy 17. századi erődítmény, ma a királyi kormány és a szerzetesi testület székhelye. A város híres a Thimphu Tshechuról, egy éves vallási fesztiválról, ahol maszkos táncokat adnak elő. A gazdaság fő pillérei a közigazgatás, a turizmus és a kézművesség. Thimphu különlegessége, hogy nincsenek közlekedési lámpái; a forgalmat rendőrök irányítják. A város környékén számos buddhista kolostor található, mint a Sangay Gang és a Dechen Phodrang.",
      ro: "Thimphu, capitala Bhutanului, este situată în valea râului Wang Chhu, la o altitudine de 2.334 de metri. Cu o populație de aproximativ 138.000 de locuitori (2023), este cel mai mare oraș din țară. Orașul se remarcă prin arhitectura sa tradițională bhutaneză, cu clădiri ornate cu motive colorate și acoperișuri curbate. Tashichho Dzong, o fortăreață impunătoare construită în secolul al XVII-lea, găzduiește guvernul și corpul monahal. Thimphu este centrul economic al Bhutanului, bazându-se pe administrație, turism și agricultura de subzistență în zonele rurale. Festivalul Thimphu Tshechu, care are loc în toamnă, atrage mii de vizitatori. Un fapt interesant: orașul nu are semafoare; traficul este dirijat de polițiști. De asemenea, Thimphu găzduiește singurul cinematograf din Bhutan, care proiectează filme locale și internaționale. Altitudinea ridicată conferă o climă temperată, cu veri blânde și ierni reci, cu ninsori frecvente.",
      en: "Thimphu, the capital of Bhutan, is situated in the Wang Chhu valley at an elevation of 2,334 meters. As of 2023, it has a population of approximately 138,000, making it the largest city in the country. The city is unique for its strict adherence to traditional Bhutanese architecture, featuring intricately carved wooden windows and colorful motifs. The Tashichho Dzong, a massive fortress built in the 17th century, houses the government and the monastic body. Thimphu is the economic hub, with its main sectors being public administration, tourism, and handicrafts. The annual Thimphu Tshechu festival, held in autumn, showcases masked dances and attracts many visitors. Notably, Thimphu has no traffic lights; intersections are managed by traffic police. The city also hosts the Royal Textile Academy and the National Institute for Zorig Chusum, preserving traditional arts. The climate is temperate, with warm summers and cold winters, with occasional snowfall.",
    },
    factsAdvanced: {
      de: ["Höhe: 2.334 m ü. M.", "Einwohnerzahl (2023): ca. 138.000.", "Bau des Tashichho Dzong: 1641.", "Jährliche Touristenankünfte (2019): über 7.000.", "Anzahl der Verkehrspolizisten: ca. 60.", "Fläche des Stadtgebiets: 26,1 km²."],
      hu: ["Tengerszint feletti magasság: 2.334 m.", "Népesség (2023): kb. 138.000 fő.", "Tashichho Dzong építése: 1641.", "Éves turistaérkezések (2019): >7.000.", "Forgalomirányító rendőrök száma: kb. 60.", "Város területe: 26,1 km²."],
      ro: ["Altitudine: 2.334 m.", "Populație (2023): aprox. 138.000.", "Construirea Tashichho Dzong: 1641.", "Sosiri turistice anuale (2019): peste 7.000.", "Număr polițiști rutieri: circa 60.", "Suprafață urbană: 26,1 km²."],
      en: ["Elevation: 2,334 m.", "Population (2023): approx. 138,000.", "Construction of Tashichho Dzong: 1641.", "Annual tourist arrivals (2019): over 7,000.", "Number of traffic police: about 60.", "Urban area: 26.1 km²."],
    },
  },
  {
    id: "phuntsholing-cities-v2",
    type: "city",
    parent: "BT-12",
    coords: [89.3833, 26.8500],
    name: { de: "Phuntsholing", hu: "Phuntsholing", ro: "Phuntsholing", en: "Phuntsholing",
},
    description: {
      de: "Phuntsholing ist eine Grenzstadt im Süden von Bhutan, die als das wichtigste kommerzielle Zentrum und Tor nach Indien fungiert.",
      hu: "Phuntsholing egy határváros Dél-Bhutánban, amely a legfontosabb kereskedelmi központként és Indiába vezető kapuként szolgál.",
      ro: "Phuntsholing este un oraș de graniță în sudul Bhutanului, funcționând ca principal centru comercial și poartă către India.",
      en: "Phuntsholing is a border town in southern Bhutan, serving as the primary commercial hub and gateway to India."
    },
    facts: {
      de: [
        "Es grenzt an die indische Stadt Jaigaon.",
        "Der grenzüberschreitende Handel ist das Rückgrat der lokalen Wirtschaft."
      ],
      hu: [
        "Az indiai Jaigaon várossal határos.",
        "A határon átnyúló kereskedelem a helyi gazdaság gerince."
      ],
      ro: [
        "Se învecinează cu orașul indian Jaigaon.",
        "Comerțul transfrontalier este coloana vertebrală a economiei locale."
      ],
      en: [
        "It borders the Indian town of Jaigaon.",
        "Cross-border trade is the backbone of the local economy."
      ]
    },
    image: "/poi-images/phuntsholing-cities-v2.webp",
    descriptionAdvanced: {
      de: "Phuntsholing liegt im Südwesten Bhutans an der Grenze zu Indien, auf etwa 300 Meter Höhe im Tal des Wang Chhu. Mit rund 27.000 Einwohnern ist es die zweitgrößte Stadt Bhutans und ein wichtiges Wirtschafts- und Handelszentrum. Die Stadt ist der wichtigste Grenzübergang für den Waren- und Personenverkehr zwischen Bhutan und Indien. Die Architektur ist weniger traditionell als in anderen bhutanischen Städten; hier dominieren moderne Gebäude, Märkte und Industrieanlagen. Der Zollkomplex und der Busterminal sind stark frequentiert. Phuntsholing ist auch ein Zentrum der Leichtindustrie, mit Betrieben zur Lebensmittelverarbeitung und Möbelherstellung. In der Umgebung wird vorwiegend Landwirtschaft betrieben, mit Anbau von Reis, Mais und Zitrusfrüchten. Die Stadt hat ein subtropisches Klima mit heißen Sommern und milden Wintern. Sehenswürdigkeiten sind der Zangtopelri Park und der Karbandi-Kloster. Aufgrund der Grenznähe gibt es eine hohe kulturelle Durchmischung mit indischen Einflüssen.",
      hu: "Phuntsholing Bhután déli határán, 300 méteres tengerszint feletti magasságban fekszik, közvetlenül India mellett. Lakossága körülbelül 27.000 fő, ezzel az ország második legnagyobb városa. A város Bhután fő kereskedelmi kapuja; a legtöbb import- és exportáru itt halad át. Az építészet vegyes: modern betonépületek mellett hagyományos bhutáni stílusú házak is láthatók. Phuntsholing gazdaságát a kereskedelem, a közlekedés és a könnyűipar jellemzi. Fontos ipari létesítmény a Phuntsholing Cementgyár, amely éves termelése meghaladja a 200.000 tonnát. A város híres piacairól, ahol indiai és bhutáni termékeket egyaránt árusítanak. A Karbandi kolostor a város szélén található, gyönyörű kilátással a síkságra. Phuntsholing éghajlata szubtrópusi, forró nyarakkal és enyhe telekkel. A város fontos közlekedési csomópont: itt indul az indiai vasúti kapcsolat és a bhutáni autóbuszjáratok.",
      ro: "Phuntsholing se află în sud-vestul Bhutanului, la granița cu India, la o altitudine de aproximativ 300 de metri. Cu o populație de circa 27.000 de locuitori, este al doilea cel mai mare oraș din Bhutan și principalul punct de trecere pentru comerțul cu India. Clima este subtropicală, cu veri foarte calde și ierni blânde. Orașul are un caracter mai puțin tradițional decât alte orașe bhutaneze; arhitectura predominant modernă reflectă influența indiană. Phuntsholing este un centru industrial, cu fabrici de ciment, prelucrare a alimentelor și mobilă. Fabrica de ciment Phuntsholing produce peste 200.000 de tone anual. Zona rurală din jur cultivă orez, porumb și citrice. Printre atracțiile orașului se numără Parcul Zangtopelri și Mănăstirea Karbandi, construită în 1967. De asemenea, orașul găzduiește unul dintre cele mai aglomerate piețe de frontieră din Bhutan, unde se comercializează produse locale și indiene. Phuntsholing servește ca poartă de intrare pentru majoritatea turiștilor care călătoresc spre capitală, Thimphu.",
      en: "Phuntsholing is located in southwestern Bhutan on the border with India, at an elevation of about 300 meters. With a population of approximately 27,000, it is Bhutan's second-largest city and its principal commercial gateway. The city has a subtropical climate, with hot summers and mild winters. Unlike other Bhutanese towns, Phuntsholing features modern architecture due to its trade-oriented character. It is an important industrial center, hosting the Phuntsholing Cement Plant, which produces over 200,000 tonnes annually, and various food processing units. The bustling border market offers a mix of Bhutanese and Indian goods. Key landmarks include the Zangtopelri Park and the Karbandi Monastery, built in 1967. Phuntsholing is the primary entry point for travelers arriving from India, with a direct bus service to Thimphu. The city's economy relies heavily on cross-border trade, with Bhutan importing necessities like fuel and construction materials through this checkpoint.",
    },
    factsAdvanced: {
      de: ["Höhe: ca. 300 m.", "Einwohner: ca. 27.000.", "Jahresproduktion des Zementwerks: >200.000 t.", "Baujahr des Karbandi-Klosters: 1967.", "Entfernung nach Thimphu: ca. 175 km.", "Durchschnittstemperatur im Juli: 30 °C."],
      hu: ["Tengerszint feletti magasság: kb. 300 m.", "Lakosság: kb. 27.000 fő.", "Cementgyár éves termelése: >200.000 t.", "Karbandi kolostor építése: 1967.", "Távolság Thimphutól: kb. 175 km.", "Átlaghőmérséklet júliusban: 30 °C."],
      ro: ["Altitudine: aprox. 300 m.", "Populație: aprox. 27.000.", "Producția anuală a fabricii de ciment: >200.000 t.", "Anul construirii Mănăstirii Karbandi: 1967.", "Distanța până la Thimphu: aprox. 175 km.", "Temperatura medie în iulie: 30 °C."],
      en: ["Elevation: approx. 300 m.", "Population: approx. 27,000.", "Annual cement plant output: >200,000 tonnes.", "Construction year of Karbandi Monastery: 1967.", "Distance to Thimphu: approx. 175 km.", "Average July temperature: 30 °C."],
    },
  },
  {
    id: "paro-cities-v2",
    type: "city",
    parent: "BT-11",
    coords: [89.4167, 27.4333],
    name: { de: "Paro", hu: "Paro", ro: "Paro", en: "Paro",
},
    description: {
      de: "Paro ist eine historische Stadt im Paro-Tal, Heimat des einzigen internationalen Flughafens von Bhutan und zahlreicher heiliger Stätten.",
      hu: "Paro egy történelmi város a Paro-völgyben, Bhután egyetlen nemzetközi repülőterének és számos szent helyének otthona.",
      ro: "Paro este un oraș istoric din valea Paro, gazda singurului aeroport internațional din Bhutan și a numeroase situri sacre.",
      en: "Paro is a historic town in the Paro Valley, home to the sole international airport of Bhutan and numerous sacred sites."
    },
    facts: {
      de: [
        "Das berühmte Taktsang-Kloster befindet sich in der Nähe.",
        "Die Stadt hat viele traditionelle bhutanische Häuser."
      ],
      hu: [
        "Az ikonikus Taktsang kolostor a közelben található.",
        "A városban számos hagyományos bhutáni ház található."
      ],
      ro: [
        "Faimoasa mănăstire Taktsang este situată în apropiere.",
        "Orașul are multe case tradiționale bhutaneze."
      ],
      en: [
        "The iconic Taktsang monastery is located nearby.",
        "The town has many traditional Bhutanese houses."
      ]
    },
    image: "/poi-images/paro-cities-v2.webp",
    descriptionAdvanced: {
      de: "Paro ist eine Stadt im westlichen Bhutan im Tal des Paro Chhu, auf einer Höhe von etwa 2.250 Metern. Sie hat rund 15.000 Einwohner und ist bekannt für den einzigen internationalen Flughafen Bhutans, den Paro Airport. Die Stadt ist reich an kulturellem Erbe: Das Kloster Taktshang (Tigernest) thront spektakulär an einer Klippe in 3.120 Metern Höhe. Das Rinpung Dzong, eine beeindruckende Festung aus dem 17. Jahrhundert, dominiert das Tal. Paro ist auch für seine traditionellen Bauernhöfe und Reisfelder bekannt. Die Wirtschaft basiert auf Tourismus, Landwirtschaft (Reis, Äpfel) und Handwerk. Das jährliche Paro Tshechu, ein religiöses Festival mit Maskentänzen, zieht viele Besucher an. Die Stadt ist ein wichtiges Zentrum für Bhutans Kunstszene, mit vielen Werkstätten für Thangka-Malerei und Holzschnitzerei. In der Umgebung gibt es mehrere alte Klöster, darunter Kyichu Lhakhang, eines der ältesten in Bhutan, erbaut im 7. Jahrhundert.",
      hu: "Paro Bhután nyugati részén, a Paro Chhu völgyében, 2.250 méter magasan fekszik. Népessége körülbelül 15.000 fő. A város ad otthont Bhután egyetlen nemzetközi repülőterének, amely a világ egyik legveszélyesebbnek tartott leszállópályájáról ismert. Paro legfőbb látványossága a Taktshang-kolostor (Tigrisfészek), amely egy 3.120 méter magas sziklafalon helyezkedik el. A Rinpung Dzong, egy 1646-ban épült erődítmény, ma a kerületi kormányzat és a szerzetesi testület székhelye. Paro híres a hagyományos bhutáni életmódról: a völgyben elterülő rizsteraszok és almaültetvények jellemzőek. A helyi gazdaság a turizmusra, a mezőgazdaságra és a kézművességre épül. A városban számos művészeti iskola működik, ahol hagyományos festészetet és fafaragást tanítanak. A Paro Tshechu fesztivál minden év tavaszán vonzza a látogatókat. Kyichu Lhakhang, a 7. században épült templom, a legrégebbi bhutáni szentélyek egyike.",
      ro: "Paro este un oraș în vestul Bhutanului, situat în valea râului Paro Chhu, la o altitudine de 2.250 de metri. Populația sa este de aproximativ 15.000 de locuitori. Orașul găzduiește singurul aeroport internațional al Bhutanului, Aeroportul Paro, considerat unul dintre cele mai dificile pentru aterizare din lume. Paro este renumit pentru Mănăstirea Taktshang (Cuibul Tigrului), situată pe o stâncă la 3.120 de metri. Rinpung Dzong, o fortăreață construită în 1646, servește ca centru administrativ și monahal. Economia se bazează pe turism, agricultură (orez, mere) și meșteșuguri. Paro este un important centru cultural, cu ateliere de pictură thangka și sculptură în lemn. Festivalul Paro Tshechu, care are loc primăvara, include dansuri mascate și atrage mii de participanți. În apropiere se află Kyichu Lhakhang, unul dintre cele mai vechi temple din Bhutan, construit în secolul al VII-lea.",
      en: "Paro is a town in western Bhutan located in the Paro Chhu valley at an elevation of 2,250 meters. It has a population of about 15,000. Paro is home to Bhutan's only international airport, Paro Airport, known for its challenging approach among mountains. The iconic Taktshang Monastery (Tiger's Nest) clings to a cliff at 3,120 meters. Rinpung Dzong, a fortress built in 1646, houses district offices and a monastery. The economy relies on tourism, agriculture (rice, apples), and handicrafts. Paro is a hub for traditional arts, with numerous workshops for thangka painting and wood carving. The Paro Tshechu festival, held in spring, features masked dances. Kyichu Lhakhang, built in the 7th century, is one of Bhutan's oldest temples. The valley offers trekking routes and is a gateway to the Haa Valley. Paro's airport handles over 100 flights per week during peak season.",
    },
    factsAdvanced: {
      de: ["Höhe: 2.250 m.", "Einwohner: ca. 15.000.", "Baujahr des Rinpung Dzong: 1646.", "Höhe des Taktshang-Klosters: 3.120 m.", "Baujahr des Kyichu Lhakhang: 7. Jahrhundert.", "Anzahl der Flüge pro Woche in der Hauptsaison: >100."],
      hu: ["Tengerszint feletti magasság: 2.250 m.", "Lakosság: kb. 15.000 fő.", "Rinpung Dzong építési éve: 1646.", "Taktshang-kolostor magassága: 3.120 m.", "Kyichu Lhakhang építése: 7. század.", "Heti járatok száma csúcsszezonban: >100."],
      ro: ["Altitudine: 2.250 m.", "Populație: aprox. 15.000.", "Anul construcției Rinpung Dzong: 1646.", "Altitudinea Mănăstirii Taktshang: 3.120 m.", "Anul construcției Kyichu Lhakhang: secolul VII.", "Zboruri săptămânale în sezonul de vârf: >100."],
      en: ["Elevation: 2,250 m.", "Population: approx. 15,000.", "Year built of Rinpung Dzong: 1646.", "Elevation of Taktshang Monastery: 3,120 m.", "Year built of Kyichu Lhakhang: 7th century.", "Weekly flights during peak season: >100."],
    },
  },
  {
    id: "gelephu-cities-v2",
    type: "city",
    parent: "BT-34",
    coords: [90.5000, 26.8667],
    name: { de: "Gelephu", hu: "Gelephu", ro: "Gelephu", en: "Gelephu",
},
    description: {
      de: "Gelephu ist eine schnell wachsende Stadt im Süden Bhutans an der indischen Grenze, die für ihre warmen Quellen und tropische Landwirtschaft bekannt ist.",
      hu: "Gelephu egy gyorsan növekvő város Dél-Bhutánban, az indiai határnál, amely híres meleg vizű forrásairól és trópusi mezőgazdaságáról.",
      ro: "Gelephu este un oraș în creștere rapidă din sudul Bhutanului, la granița cu India, renumit pentru izvoarele sale termale și agricultura tropicală.",
      en: "Gelephu is a rapidly growing town in southern Bhutan on the Indian border, known for its hot springs and tropical agriculture."
    },
    facts: {
      de: [
        "Gelephu verfügt über einen Inlandsflughafen.",
        "Es ist geplant, ein großes Achtsamkeits-Stadtprojekt zu werden."
      ],
      hu: [
        "Gelephu rendelkezik egy belföldi repülőtérrel.",
        "A tervek szerint egy jelentős tudatosság-város projekt lesz."
      ],
      ro: [
        "Gelephu are un aeroport intern.",
        "Este planificat să devină un proiect major de oraș al atenției."
      ],
      en: [
        "Gelephu has a domestic airport.",
        "It is planned to become a major mindfulness city project."
      ]
    },
    image: "/poi-images/gelephu-cities-v2.webp",
    descriptionAdvanced: {
      de: "Gelephu liegt im Süden Bhutans an der Grenze zu Indien im Distrikt Sarpang, auf einer Höhe von etwa 300 Metern. Die Stadt hat rund 9.800 Einwohner (2017) und ist ein wichtiger Grenzübergang für den Handel mit Indien. Gelephu hat sich in den letzten Jahren wirtschaftlich entwickelt, vor allem durch Holzwirtschaft und Landwirtschaft. Die Region ist bekannt für die Produktion von Kardamom und Ingwer. Der Ort dient als regionales Verwaltungszentrum und verfügt über einen Flugplatz, der Inlandsflüge nach Thimphu und andere Städte anbietet. Gelephu hat ein subtropisches Klima mit heißen Sommern und Monsunregen. Die Stadt ist ethnisch vielfältig mit einer Mischung aus Bhutanesen, Nepalesen und Indern. Sehenswürdigkeiten sind der Gelephu Sportplatz und die nahe gelegenen Wälder, die Lebensraum für Elefanten und Tiger bieten. Die Infrastruktur umfasst ein Krankenhaus, Schulen und Märkte. Gelephu ist ein strategischer Punkt für die Entwicklung der südlichen Wirtschaftszone Bhutans.",
      hu: "Gelephu Bhután déli részén, az indiai határ közelében, 300 méteres magasságban található. Lakossága hozzávetőleg 9.800 fő (2017). A város fontos kereskedelmi kapu, ahol a bhutáni áruk belépnek az indiai piacra. Gelephu gazdaságát a mezőgazdaság (kardamom, gyömbér), a fakitermelés és a kiskereskedelem jellemzi. A városban repülőtér működik, amely belföldi járatokat kínál. A klíma szubtrópusi, magas páratartalommal és jelentős csapadékkal. A település etnikailag sokszínű: a bhutániak mellett nepáliak és indiaiak is élnek itt. A környező erdők vadon élő állatokban gazdagok; gyakoriak az ázsiai elefántok és a bengáli tigrisek. Gelephu a Gyepu-völgy kapujaként ismert. A város fejlődését a déli gazdasági folyosó részeként tervezik, amely összeköti Bhutánt Indiával. Az infrastruktúra magában foglal egy kórházat és számos oktatási intézményt.",
      ro: "Gelephu este un oraș din sudul Bhutanului, în districtul Sarpang, la granița cu India, la o altitudine de aproximativ 300 de metri. Populația sa este de circa 9.800 de locuitori (2017). Orașul servește ca punct de trecere comercială important și este un centru regional pentru agricultură și silvicultură. Principalele culturi sunt cardamomul și ghimbirul. Gelephu are un aeroport care oferă zboruri interne. Clima este subtropicală, cu veri fierbinți și musoni abundenți. Populația este diversă din punct de vedere etnic, incluzând bhutanezi, nepalezi și indieni. Pădurile din apropiere găzduiesc elefanți și tigri. Orașul este considerat o poartă pentru dezvoltarea economică a sudului Bhutanului, fiind inclus în planurile de coridor economic. Facilitățile includ un spital, școli și piețe. Gelephu este, de asemenea, cunoscut pentru peisajele sale pitorești și pentru oportunitățile de drumeție în Gyepu Valley.",
      en: "Gelephu is a town in southern Bhutan in Sarpang District, near the border with India, at an elevation of about 300 meters. Its population is approximately 9,800 (2017). It serves as a major border crossing for trade and a regional center for agriculture and forestry. Key agricultural products include cardamom and ginger. Gelephu has a domestic airport with flights to Thimphu and other towns. The climate is subtropical, with hot summers and heavy monsoon rains. The population is ethnically mixed, with Bhutanese, Nepali, and Indian communities. Nearby forests are habitat for Asian elephants and Bengal tigers. Gelephu is part of Bhutan's southern economic corridor development plan. The town offers basic amenities like a hospital, schools, and markets. Its strategic location makes it a potential hub for cross-border commerce and tourism.",
    },
    factsAdvanced: {
      de: ["Höhe: ca. 300 m.", "Einwohner (2017): ca. 9.800.", "Jährlicher Niederschlag: >2.500 mm.", "Flugplatz: Inlandsflüge nach Thimphu.", "Eröffnung des Flughafens: 2011.", "Wichtige Anbauprodukte: Kardamom, Ingwer."],
      hu: ["Tengerszint feletti magasság: kb. 300 m.", "Népesség (2017): kb. 9.800 fő.", "Éves csapadék: >2.500 mm.", "Repülőtér: belföldi járatok Thimphuba.", "Repülőtér megnyitása: 2011.", "Fő termények: kardamom, gyömbér."],
      ro: ["Altitudine: aprox. 300 m.", "Populație (2017): aprox. 9.800.", "Precipitații anuale: >2.500 mm.", "Aeroport: zboruri interne către Thimphu.", "Deschiderea aeroportului: 2011.", "Culturi principale: cardamom, ghimbir."],
      en: ["Elevation: approx. 300 m.", "Population (2017): approx. 9,800.", "Annual rainfall: >2,500 mm.", "Airport: domestic flights to Thimphu.", "Airport opened: 2011.", "Key crops: cardamom, ginger."],
    },
  },
  {
    id: "samdrup-jongkhar-cities-v2",
    type: "city",
    parent: "BT-45",
    coords: [91.5000, 26.8000],
    name: { de: "Samdrup Jongkhar", hu: "Samdrup Jongkhar", ro: "Samdrup Jongkhar", en: "Samdrup Jongkhar",
},
    description: {
      de: "Samdrup Jongkhar ist die wichtigste Stadt im Südosten Bhutans und dient als wichtiger Handelspunkt mit dem indischen Bundesstaat Assam.",
      hu: "Samdrup Jongkhar Délkelet-Bhután legfontosabb városa, amely kulcsfontosságú kereskedelmi pontként szolgál az indiai Asszám állammal.",
      ro: "Samdrup Jongkhar este cel mai important oraș din sud-estul Bhutanului, servind ca un punct comercial cheie cu statul indian Assam.",
      en: "Samdrup Jongkhar is the principal town in southeastern Bhutan, serving as a crucial trading point with the Indian state of Assam."
    },
    facts: {
      de: [
        "Es ist eine der ältesten Städte in Bhutan.",
        "Die Region ist reich an Kohle und anderen Mineralien."
      ],
      hu: [
        "Bhután egyik legrégebbi városa.",
        "A régió gazdag szénben és más ásványi anyagokban."
      ],
      ro: [
        "Este unul dintre cele mai vechi orașe din Bhutan.",
        "Regiunea este bogată în cărbune și alte minerale."
      ],
      en: [
        "It is one of the oldest towns in Bhutan.",
        "The region is rich in coal and other minerals."
      ]
    },
    image: "/poi-images/samdrup-jongkhar-cities-v2.webp",
    descriptionAdvanced: {
      de: "Samdrup Jongkhar liegt im Südosten Bhutans an der Grenze zu Indien, auf einer Höhe von etwa 200 Metern. Die Stadt ist das administrative Zentrum des gleichnamigen Distrikts und hat rund 7.000 Einwohner (2017). Sie ist ein wichtiger Grenzübergang für den Handel zwischen Bhutan und Indien, insbesondere für den Import von Treibstoff und Baumaterialien. Die Wirtschaft basiert auf Landwirtschaft, Handel und staatlichen Dienstleistungen. In den umliegenden Hügeln werden Reis, Mais und Zitrusfrüchte anbauen. Samdrup Jongkhar hat ein subtropisches Klima mit heißen Sommern und Monsunregen. Sehenswürdigkeiten sind das Samdrup Jongkhar Dzong, ein modernes Dzong aus den 1990er Jahren, und der nahe gelegene Naturschutzgebiet Khaling. Die Stadt dient als Ausgangspunkt für Reisen in den Osten Bhutans, einschließlich Trashigang. Die Bevölkerung ist ethnisch gemischt, mit bhutanesischen, nepalesischen und indischen Gemeinschaften.",
      hu: "Samdrup Jongkhar Bhután délkeleti csücskében, az indiai határ közelében, 200 méter magasan fekszik. A város a dzongkha nevű körzet székhelye, lakossága kb. 7.000 fő (2017). Ez a második legfontosabb határátkelőhely az ország déli részén. A gazdaság a mezőgazdaságra, kereskedelemre és kormányzati szolgáltatásokra épül. A környék rizs- és kukoricatermesztéséről ismert. Samdrup Jongkhar klímája szubtrópusi, forró nyárral és monszunesőkkel. A városban található a Samdrup Jongkhar Dzong, amelyet 1990-ben építettek. A közeli Khaling vadrezervátum számos madár- és emlősfajnak ad otthont. A település fontos közlekedési csomópont, itt halad át a főút Trashigang felé. A népesség sokszínű: bhutániak, nepáliak és indiaiak élnek itt. Az oktatásban a Samdrup Jongkhar Higher Secondary School a legjelentősebb intézmény.",
      ro: "Samdrup Jongkhar este un oraș în sud-estul Bhutanului, la granița cu India, la o altitudine de aproximativ 200 de metri. Populația sa este de circa 7.000 de locuitori (2017). Este centrul administrativ al districtului Samdrup Jongkhar și un important punct comercial cu India. Economia se bazează pe agricultură (orez, porumb, citrice), comerț și servicii guvernamentale. Clima este subtropicală, cu veri toride și musoni abundenți. Orașul găzduiește Samdrup Jongkhar Dzong, construit în 1995, care servește ca centru administrativ și monahal. În apropiere se află Rezervația Naturală Khaling, protejând păduri subtropicale și specii precum leopardul și ursul. Samdrup Jongkhar este un nod rutier pentru regiunea de est, cu legături spre Trashigang. Populația este eterogenă, incluzând comunități bhutaneze, nepaleze și indiene.",
      en: "Samdrup Jongkhar is a town in southeastern Bhutan on the border with India, at an elevation of about 200 meters. Its population is approximately 7,000 (2017). It serves as the administrative center of Samdrup Jongkhar District and is a key border crossing for trade with India. The economy is based on agriculture (rice, maize, citrus), trade, and government services. The climate is subtropical, with hot summers and heavy monsoon rainfall. The town features a modern dzong built in 1995. Nearby Khaling Wildlife Sanctuary protects subtropical forests and wildlife including leopard and Himalayan black bear. Samdrup Jongkhar is a gateway to eastern Bhutan and lies on the main highway to Trashigang. The population is ethnically diverse, with Bhutanese, Nepali, and Indian groups. The town has a hospital, schools, and a market.",
    },
    factsAdvanced: {
      de: ["Höhe: ca. 200 m.", "Einwohner (2017): ca. 7.000.", "Baujahr des Dzong: 1995.", "Entfernung nach Trashigang: ca. 130 km.", "Schutzgebiet Khaling: 34 km².", "Durchschnittstemperatur im Juni: 32 °C."],
      hu: ["Tengerszint feletti magasság: kb. 200 m.", "Népesség (2017): kb. 7.000 fő.", "Dzong építési éve: 1995.", "Távolság Trashigangtól: kb. 130 km.", "Khaling Vadrezervátum területe: 34 km².", "Átlaghőmérséklet júniusban: 32 °C."],
      ro: ["Altitudine: aprox. 200 m.", "Populație (2017): aprox. 7.000.", "Anul construirii dzong-ului: 1995.", "Distanța până la Trashigang: aprox. 130 km.", "Suprafața Rezervației Khaling: 34 km².", "Temperatura medie în iunie: 32 °C."],
      en: ["Elevation: approx. 200 m.", "Population (2017): approx. 7,000.", "Year built of Dzong: 1995.", "Distance to Trashigang: approx. 130 km.", "Khaling Wildlife Sanctuary area: 34 km².", "Average June temperature: 32 °C."],
    },
  },
  {
    id: "wangdue-phodrang-cities-v2",
    type: "city",
    parent: "BT-24",
    coords: [89.9000, 27.4833],
    name: { de: "Wangdue Phodrang", hu: "Wangdue Phodrang", ro: "Wangdue Phodrang", en: "Wangdue Phodrang",
},
    description: {
      de: "Wangdue Phodrang ist eine malerische Stadt, die um ihren großen historischen Dzong gebaut wurde, welcher eine wichtige Handelsroute kontrolliert.",
      hu: "Wangdue Phodrang egy festői város, amely a nagy történelmi dzongja köré épült, és fontos kereskedelmi útvonalat ellenőriz.",
      ro: "Wangdue Phodrang este un oraș pitoresc construit în jurul marelui său dzong istoric, care controlează o rută comercială importantă.",
      en: "Wangdue Phodrang is a picturesque town built around its large historic dzong, which controls an important trade route."
    },
    facts: {
      de: [
        "Die ursprüngliche Festung der Stadt brannte 2012 tragischerweise ab.",
        "Die Gegend ist berühmt für Bambusprodukte und Schieferschnitzereien."
      ],
      hu: [
        "A város eredeti erődje 2012-ben tragikusan leégett.",
        "A környék híres bambusztermékeiről és palafaragásairól."
      ],
      ro: [
        "Fortăreața originală a orașului a ars în mod tragic în 2012.",
        "Zona este renumită pentru produsele din bambus și sculpturile în ardezie."
      ],
      en: [
        "The original fortress of the town tragically burned down in 2012.",
        "The area is famous for bamboo products and slate carvings."
      ]
    },
    image: "/poi-images/wangdue-phodrang-cities-v2.webp",
    descriptionAdvanced: {
      de: "Wangdue Phodrang ist eine Stadt im zentralen Bhutan im gleichnamigen Distrikt, auf einer Höhe von etwa 1.300 Metern am Zusammenfluss der Flüsse Punakha Chhu und Tang Chhu. Die Stadt hat rund 7.000 Einwohner und ist bekannt für ihr historisches Dzong, das auf einem Hügel thront. Das Dzong von Wangdue Phodrang wurde 1638 von Shabdrung Ngawang Namgyel erbaut, brannte jedoch 2012 nieder und wird derzeit wieder aufgebaut. Die Region ist landwirtschaftlich geprägt, mit Anbau von Reis, Weizen und Gemüse. Wangdue Phodrang ist auch für seine Bambusprodukte und Töpferei bekannt. Die Stadt dient als Verwaltungszentrum und hat eine kleine Marktstadt. In der Umgebung liegen die Klöster Gangtey und Phobjikha, letzteres ist ein wichtiges Schutzgebiet für den Schwarzhalssichler (Grus nigricollis). Die Höhenlage sorgt für ein gemäßigtes Klima mit warmen Sommern und kalten Wintern.",
      hu: "Wangdue Phodrang Bhután középső részén, 1.300 méter magasan, a Punakha Chhu és a Tang Chhu folyók összefolyásánál fekszik. Népessége mintegy 7.000 fő. A város egy 1638-ban épült dzongjáról ismert, amely 2012-ben leégett, és jelenleg újjáépítés alatt áll. A környék mezőgazdasági terület, ahol rizst, búzát és zöldségeket termesztenek. Wangdue Phodrang híres bambusztermékeiről és fazekasságáról. A közeli Phobjikha-völgy és Gangtey-kolostor népszerű turistacélpontok; a völgy a fekete nyakú darvak telelőhelye. A város fontos közlekedési csomópont a középső régióban. Az éghajlat mérsékelt, a januári átlaghőmérséklet 5 °C körüli, a júliusi pedig 22 °C. A dzong újjáépítése várhatóan 2025-ben fejeződik be.",
      ro: "Wangdue Phodrang este un oraș în centrul Bhutanului, reședința districtului omonim, situat la confluența râurilor Punakha Chhu și Tang Chhu, la o altitudine de aproximativ 1.300 de metri. Populația sa este de circa 7.000 de locuitori. Orașul este cunoscut pentru dzong-ul său construit în 1638, care a fost distrus de un incendiu în 2012 și se află în reconstrucție. Economia se bazează pe agricultură (orez, grâu, legume) și meșteșuguri, inclusiv prelucrarea bambusului și olăritul. În apropiere se află Valea Phobjikha, un important habitat pentru cocorul cu gât negru (Grus nigricollis). Wangdue Phodrang este un nod rutier central, legând Thimphu de estul țării. Clima este temperată, cu veri blânde și ierni reci. Reconstrucția dzong-ului este estimată a fi finalizată până în 2025.",
      en: "Wangdue Phodrang is a town in central Bhutan, capital of Wangdue Phodrang District, at an elevation of about 1,300 meters, at the confluence of the Punakha Chhu and Tang Chhu rivers. Its population is around 7,000. The town is famous for its historic dzong built in 1638, which was devastated by a fire in 2012 and is currently under reconstruction. The surrounding area is agricultural, producing rice, wheat, and vegetables. Wangdue Phodrang is known for bamboo crafts and pottery. Nearby Phobjikha Valley is a winter home for black-necked cranes (Grus nigricollis). The town serves as a junction for highways to central and eastern Bhutan. The climate is temperate, with January averages around 5°C and July averages around 22°C. The reconstruction of the dzong is expected to complete by 2025.",
    },
    factsAdvanced: {
      de: ["Höhe: 1.300 m.", "Einwohner: ca. 7.000.", "Bau des Dzong: 1638.", "Brand des Dzong: 2012.", "Anzahl der Schwarzhalssichler in Phobjikha: bis zu 400.", "Geplante Fertigstellung des Wiederaufbaus: 2025."],
      hu: ["Tengerszint feletti magasság: 1.300 m.", "Lakosság: kb. 7.000 fő.", "Dzong építése: 1638.", "Dzong leégése: 2012.", "Fekete nyakú darvak száma Phobjikhában: akár 400.", "Újjáépítés várható befejezése: 2025."],
      ro: ["Altitudine: 1.300 m.", "Populație: aprox. 7.000.", "Construirea dzong-ului: 1638.", "Incendierea dzong-ului: 2012.", "Număr de cocori cu gât negru în Phobjikha: până la 400.", "Finalizarea estimată a reconstrucției: 2025."],
      en: ["Elevation: 1,300 m.", "Population: approx. 7,000.", "Construction of Dzong: 1638.", "Fire of Dzong: 2012.", "Black-necked cranes in Phobjikha: up to 400.", "Expected reconstruction completion: 2025."],
    },
  },
  {
    id: "punakha-cities-v2",
    type: "city",
    parent: "BT-23",
    coords: [89.8667, 27.5833],
    name: { de: "Punakha", hu: "Punakha", ro: "Punakha", en: "Punakha",
},
    description: {
      de: "Punakha war bis 1955 die Hauptstadt von Bhutan und ist berühmt für ihren majestätischen Dzong am Zusammenfluss zweier großer Flüsse.",
      hu: "Punakha 1955-ig Bhután fővárosa volt, és híres fenséges dzongjáról, amely két nagy folyó találkozásánál fekszik.",
      ro: "Punakha a fost capitala Bhutanului până în 1955 și este faimoasă pentru dzongul său maiestuos situat la confluența a două râuri mari.",
      en: "Punakha was the capital of Bhutan until 1955 and is famous for its majestic dzong situated at the confluence of two major rivers."
    },
    facts: {
      de: [
        "Es dient als Winterresidenz der zentralen klösterlichen Körperschaft.",
        "Das Klima ist im Vergleich zu Thimphu relativ warm."
      ],
      hu: [
        "A központi kolostori testület téli rezidenciájaként szolgál.",
        "Az éghajlat viszonylag meleg Thimphuhoz képest."
      ],
      ro: [
        "Servește drept reședință de iarnă a corpului monahal central.",
        "Clima este relativ caldă în comparație cu Thimphu."
      ],
      en: [
        "It serves as the winter residence of the central monastic body.",
        "The climate is relatively warm compared to Thimphu."
      ]
    },
    image: "/poi-images/punakha-cities-v2.webp",
    descriptionAdvanced: {
      de: "Punakha war bis 1955 die Winterhauptstadt Bhutans und ist heute die Hauptstadt des gleichnamigen Distrikts. Die Stadt liegt auf einer Höhe von etwa 1200 Metern im fruchtbaren Punakha-Tal, das für seinen Reisanbau bekannt ist. Der Punakha-Dzong, eine monumentale Festung aus dem 17. Jahrhundert, thront am Zusammenfluss der Flüsse Pho Chhu und Mo Chhu. Die Stadt ist ein wichtiges religiöses Zentrum, da hier der Je Khenpo, der oberste Abt Bhutans, im Winter residiert. Das milde Klima macht Punakha zu einem beliebten Ziel für Touristen.",
      hu: "Punakha fontos szerepet játszott Bhután egyesítésében és a Zhabdrung Ngawang Namgyal által a 17. században létrehozott állam kialakításában. A Punakha Dzong egyike a legrégibb és legimpozánsabb erődítményeknek, ahol a koronázási szertartásokat tartották. 1907-ben itt koronázták meg Bhután első örökletes királyát, Ugyen Wangchuckot. A városban található a Khamsum Yulley Namgyal Chorten, egy 2004-ben épült sztúpa a béke és harmónia jelképeként. A régióban hagyományos mesterségek, például szövés és festészet virágoznak.",
      ro: "Punakha este situată într-o vale fertilă la confluența râurilor Pho Chhu și Mo Chhu. Clima subtropicală permite cultivarea orezului, porumbului și citricelor. Orașul are o populație de aproximativ 6000 de locuitori. Economia se bazează pe agricultură și turism. Punakha Dzong a fost grav avariat de un incendiu în 1998, dar a fost restaurat. Recent, s-au construit poduri suspendate moderne pentru a facilita accesul către dzong. Vara, temperaturile ajung la 30°C, iar iarna nu scad sub 10°C.",
      en: "Punakha serves as the winter capital of Bhutan, with the central monk body relocating from Thimphu during colder months. The Punakha Dzong is the second oldest and second largest dzong in Bhutan, measuring 180 m in length. The dzong houses sacred relics, including the mummified remains of Zhabdrung Ngawang Namgyal. The city is a hub for river rafting and hiking, with trails leading to nearby monasteries. The annual Punakha Tshechu festival features masked dances and attracts thousands of visitors. The valley is also known for its white sand beaches along the riverbanks.",
    },
    factsAdvanced: {
      de: ["Die Stadt liegt auf 1200 m Höhe.", "Der Punakha-Dzong wurde 1637 erbaut.", "Der Dzong ist 180 m lang.", "Die Flüsse Pho Chhu und Mo Chhu fließen hier zusammen.", "Punakha hat etwa 6000 Einwohner.", "Die Reisterrassen erstrecken sich über 20 km²."],
      hu: ["Punakha 1955-ig Bhután fővárosa volt.", "A dzongban őrzik Ngawang Namgyal bebalzsamozott maradványait.", "A Khamsum Yulley Namgyal Chorten 2004-ben épült.", "Az éves Punakha Tshechu fesztivál résztvevőinek száma meghaladja a 10 000-et.", "A város téli hőmérséklete 5-15°C között mozog.", "A régióban 3000 hektáron termesztenek rizst."],
      ro: ["Populația orașului Punakha este de ~6000 locuitori.", "Dzong-ul are o lungime de 180 m și o lățime de 72 m.", "Incendiul din 1998 a distrus 50 de camere ale dzong-ului.", "Temperatura medie anuală este de 20°C.", "Podul suspendat de la Punakha are 160 m lungime.", "Rezervația de animale sălbatice Jigme Dorji se află în apropiere."],
      en: ["The Punakha Dzong was built in 1637.", "It measures 180 m long and 72 m wide.", "The dzong contains 6 temples.", "Punakha is 72 km from Thimphu.", "The altitude is 1200 m.", "The annual Punakha Tshechu features 4 days of celebrations."],
    },
  },
  {
    id: "jakar-cities-v2",
    type: "city",
    parent: "BT-31",
    coords: [90.7500, 27.5500],
    name: { de: "Jakar", hu: "Jakar", ro: "Jakar", en: "Jakar",
},
    description: {
      de: "Jakar, bekannt als das Zentrum der Region Bumthang, gilt als spirituelles Herzland Bhutans mit vielen alten Klöstern.",
      hu: "Jakar, a Bumthang régió központjaként ismert város, Bhután spirituális központjának számít, számos ősi kolostorral.",
      ro: "Jakar, cunoscut drept centrul regiunii Bumthang, este considerat inima spirituală a Bhutanului, având numeroase mănăstiri antice.",
      en: "Jakar, known as the center of the Bumthang region, is considered the spiritual heartland of Bhutan with many ancient monasteries."
    },
    facts: {
      de: [
        "Der Name Jakar bedeutet in der lokalen Sprache weißer Vogel.",
        "Es ist berühmt für seine Honig- und Käseproduktion."
      ],
      hu: [
        "A Jakar név a helyi nyelven fehér madarat jelent.",
        "Híres méz- és sajttermeléséről."
      ],
      ro: [
        "Numele Jakar se traduce prin pasăre albă în limba locală.",
        "Este faimos pentru producția sa de miere și brânză."
      ],
      en: [
        "The name Jakar translates to white bird in the local language.",
        "It is famous for its honey and cheese production."
      ]
    },
    image: "/poi-images/jakar-cities-v2.webp",
    descriptionAdvanced: {
      de: "Jakar, auch bekannt als Bumthang, ist eine Stadt im zentralen Bhutan und Hauptstadt des Distrikts Bumthang. Sie liegt auf etwa 2600 Metern Höhe und ist ein bedeutendes kulturelles Zentrum. Die Stadt beherbergt den Jakar-Dzong, der im 16. Jahrhundert erbaut wurde und heute Sitz der Distriktverwaltung ist. Jakar ist berühmt für seine traditionellen Webereien, insbesondere die Yathra-Wollstoffe. Die Umgebung bietet zahlreiche Klöster und heilige Stätten, darunter den Kurje Lhakhang, der mit dem Padmasambhava verbunden ist.",
      hu: "Jakar Bhután egyik legfontosabb zarándokhelye, mivel itt található a Kurje Lhakhang templomegyüttes, ahol Guru Rinpocse meditált. A város a Bumthang-völgyben fekszik, amelyet néha 'Bhután Svájcának' is neveznek. A Jakar Dzong a 17. században épült át egy korábbi erőd helyén. A régió híres méztermeléséről és sajtkészítéséről. A Bumthang-völgyben évente megrendezik a Jakar Tshechu fesztivált, amely színes maszkos táncokat vonultat fel.",
      ro: "Jakar este capitala districtului Bumthang, cu o populație de aproximativ 5000 de locuitori. Orașul este situat la o altitudine de 2600 m, într-o vale fertilă unde se cultivă hrișcă, orz și cartofi. Economia locală se bazează pe agricultură, turism și meșteșuguri textile. Climatul este temperat, cu ierni geroase și veri răcoroase. Jakar este cunoscut pentru producția de brânză și miere. Recent, s-a dezvoltat infrastructura turistică, cu hoteluri și restaurante.",
      en: "Jakar is the administrative center of Bumthang District and a gateway to eastern Bhutan. The town is known for the Jakar Dzong, which houses the district monastic body. The surrounding valley is dotted with apple orchards and is a major producer of apples in Bhutan. The Bumthang Brewery, established in 2003, produces beer from local barley. Jakar also has a small airport connecting it to Paro and other domestic destinations. The region is a popular destination for trekking, with trails leading to the remote eastern hills.",
    },
    factsAdvanced: {
      de: ["Jakar liegt auf 2600 m Höhe.", "Der Jakar-Dzong wurde im 16. Jahrhundert erbaut.", "Die Stadt hat etwa 5000 Einwohner.", "Die Yathra-Wollstoffe werden in Handarbeit gewebt.", "Der Kurje Lhakhang besteht aus 3 Tempeln.", "Die Bumthang-Brauerei produziert jährlich 200.000 Liter Bier."],
      hu: ["A Kurje Lhakhang templomot a 8. században alapították.", "A Jakar Tshechu fesztivál 4 napig tart.", "Bumthangban évente 10.000 tonna almát termesztenek.", "A Jakar repülőtér kifutópályája 1500 m hosszú.", "A dzong falai 2 m vastagok.", "A völgyben 30 család foglalkozik sajtkészítéssel."],
      ro: ["Altitudinea orașului Jakar este de 2600 m.", "Populația districtului Bumthang este de ~12.000 locuitori.", "Clădirea Jakar Dzong are 3 etaje.", "Producția anuală de mere în Bumthang este de 10.000 tone.", "Brânza Bumthang este maturată timp de 6 luni.", "Temperaturile de iarnă scad până la -5°C."],
      en: ["Jakar Dzong was built in 1541.", "The altitude is 2600 m.", "Bumthang produces 10,000 tons of apples annually.", "The Bumthang Brewery produces 200,000 liters per year.", "Jakar Airport has a 1500 m runway.", "The town has a population of 5000."],
    },
  },
  {
    id: "mongar-cities-v2",
    type: "city",
    parent: "BT-42",
    coords: [91.2500, 27.2833],
    name: { de: "Mongar", hu: "Mongar", ro: "Mongar", en: "Mongar",
},
    description: {
      de: "Mongar ist eine wichtige Stadt im Osten Bhutans, die spektakulär auf der Seite eines Hügels statt in einem Tal liegt.",
      hu: "Mongar Kelet-Bhután egyik jelentős városa, amely látványosan egy domboldalra épült, a szokásos völgyi elhelyezkedés helyett.",
      ro: "Mongar este un oraș important din estul Bhutanului, situat spectaculos pe versantul unui deal în loc de o vale.",
      en: "Mongar is a major town in eastern Bhutan, spectacularly situated on the side of a hill rather than in a valley."
    },
    facts: {
      de: [
        "Der Mongar Dzong ist einer der neuesten im Land und wurde 1930 erbaut.",
        "Es markiert den Beginn von Ostbhutan."
      ],
      hu: [
        "A Mongar Dzong az ország egyik legújabbja, amelyet 1930-ban építettek.",
        "Ez jelzi Kelet-Bhután kezdetét."
      ],
      ro: [
        "Mongar Dzong este unul dintre cele mai noi din țară, construit în 1930.",
        "Marchează începutul estului Bhutanului."
      ],
      en: [
        "Mongar Dzong is one of the newest in the country, built in 1930.",
        "It marks the beginning of eastern Bhutan."
      ]
    },
    image: "/poi-images/mongar-cities-v2.webp",
    descriptionAdvanced: {
      de: "Mongar ist eine Stadt im östlichen Bhutan auf einer Höhe von etwa 1.600 Metern. Sie ist das administrative Zentrum des Distrikts Mongar und hat rund 6.500 Einwohner. Die Stadt ist bekannt für ihr modernes Mongar Dzong, das 1930 erbaut wurde, aber auch für die umliegenden Hügel und Wälder. Die Wirtschaft basiert auf Landwirtschaft (Mais, Kartoffeln, Äpfel) und staatlichen Dienstleistungen. Mongar ist ein wichtiges Handelszentrum für die östliche Region. Das Klima ist subtropisch mit angenehmen Sommern und milden Wintern. In der Umgebung gibt es heiße Quellen und Wanderwege. Die Bevölkerung besteht hauptsächlich aus den ethnischen Gruppen der Kheng und Tshangla. Mongar hat ein Krankenhaus, Schulen und einen Markt. Die Stadt ist auch für ihre Bambusprodukte bekannt. Der Fluss Mongar Chhu fließt durch die Stadt. In der Nähe befindet sich der Korila-Pass, der eine wichtige Verkehrsverbindung darstellt.",
      hu: "Mongar Bhután keleti részén, 1.600 méter magasan fekszik. A város a Mongar körzet székhelye, lakossága kb. 6.500 fő. A település ismert a Mongar Dzongról, amely 1930-ban épült, és modern építészeti stílust képvisel. A gazdaság a mezőgazdaságra (kukorica, burgonya, alma) és a kereskedelemre épül. Mongar fontos regionális központ, ahonnan út vezet Trashigang és Lhuntse felé. Az éghajlat szubtrópusi, a nyarak melegek, a telek enyhék. A környéken forró termálforrások találhatók, például a Gyoza Termálfürdő. A város lakossága főleg a kheng és tshangla etnikumokból áll. Mongar híres bambusztermékeiről. A Mongar Chhu folyó átszeli a várost. A közeli Korila-hágó 2.450 méter magas, és fontos közlekedési útvonal. A városban kórház, iskolák és piac működik.",
      ro: "Mongar este un oraș în estul Bhutanului, la o altitudine de aproximativ 1.600 de metri. Este reședința districtului Mongar, cu o populație de circa 6.500 de locuitori. Orașul este cunoscut pentru Mongar Dzong, construit în 1930 într-un stil arhitectural modern. Economia se bazează pe agricultură (porumb, cartofi, mere) și servicii. Mongar este un hub comercial pentru estul Bhutanului. Clima este subtropicală, cu veri calde și ierni blânde. În zonă există izvoare termale, cum ar fi Gyoza Hot Spring. Populația este compusă în principal din grupurile etnice Kheng și Tshangla. Orașul este renumit pentru articolele din bambus. Râul Mongar Chhu traversează localitatea. Pasul Korila, la 2.450 de metri, leagă Mongar de restul țării. Facilitățile includ un spital, școli și o piață.",
      en: "Mongar is a town in eastern Bhutan at an elevation of about 1,600 meters. It is the administrative center of Mongar District, with a population of around 6,500. The town is known for the Mongar Dzong, built in 1930 in a modern architectural style. The economy relies on agriculture (maize, potatoes, apples) and government services. Mongar is a trading hub for the eastern region. The climate is subtropical, with warm summers and mild winters. Hot springs like Gyoza are found nearby. The majority of the population belongs to the Kheng and Tshangla ethnic groups. Mongar is famous for bamboo crafts. The Mongar Chhu river flows through the town. Korila Pass, at 2,450 meters, connects Mongar to other parts of Bhutan. The town has a hospital, schools, and markets.",
    },
    factsAdvanced: {
      de: ["Höhe: 1.600 m.", "Einwohner: ca. 6.500.", "Baujahr des Dzong: 1930.", "Höhe des Korila-Passes: 2.450 m.", "Jährliche Touristenankünfte (2018): ca. 2.000.", "Durchschnittstemperatur im Juli: 25 °C."],
      hu: ["Tengerszint feletti magasság: 1.600 m.", "Lakosság: kb. 6.500 fő.", "Dzong építési éve: 1930.", "Korila-hágó magassága: 2.450 m.", "Éves turistaérkezések (2018): kb. 2.000.", "Átlaghőmérséklet júliusban: 25 °C."],
      ro: ["Altitudine: 1.600 m.", "Populație: aprox. 6.500.", "Anul construirii dzong-ului: 1930.", "Altitudinea Pasului Korila: 2.450 m.", "Sosiri turistice anuale (2018): aprox. 2.000.", "Temperatura medie în iulie: 25 °C."],
      en: ["Elevation: 1,600 m.", "Population: approx. 6,500.", "Construction year of Dzong: 1930.", "Elevation of Korila Pass: 2,450 m.", "Annual tourist arrivals (2018): approx. 2,000.", "Average July temperature: 25 °C."],
    },
  },
  {
    id: "trashigang-cities-v2",
    type: "city",
    parent: "BT-41",
    coords: [91.5500, 27.3333],
    name: { de: "Trashigang", hu: "Trashigang", ro: "Trashigang", en: "Trashigang",
},
    description: {
      de: "Trashigang ist eine geschäftige Stadt im Osten, die traditionell das Zentrum des Handels mit Tibet war und über dem Drangme Chhu-Fluss thront.",
      hu: "Trashigang egy nyüzsgő város keleten, amely hagyományosan a Tibettel folytatott kereskedelem központja volt, és a Drangme Chhu folyó fölé magasodik.",
      ro: "Trashigang este un oraș aglomerat din est, care a fost în mod tradițional centrul comerțului cu Tibetul și domină râul Drangme Chhu.",
      en: "Trashigang is a bustling town in the east that was traditionally the center of trade with Tibet, perched above the Drangme Chhu river."
    },
    facts: {
      de: [
        "Die Stadt hat eine große Bevölkerung von Nomaden aus Merak.",
        "Das erste College von Bhutan befindet sich in der Nähe."
      ],
      hu: [
        "A városban nagyszámú nomád népesség él Merakból.",
        "Bhután első főiskolája a közelben található."
      ],
      ro: [
        "Orașul are o populație mare de nomazi din Merak.",
        "Primul colegiu din Bhutan este situat în apropiere."
      ],
      en: [
        "The town has a large population of nomadic people from Merak.",
        "The first college of Bhutan is located nearby."
      ]
    },
    image: "/poi-images/trashigang-cities-v2.webp",
    descriptionAdvanced: {
      de: "Trashigang ist eine Stadt im äußersten Osten Bhutans auf einer Höhe von etwa 1.100 Metern. Sie ist das administrative Zentrum des Distrikts Trashigang und die größte Stadt in Ostbhutan mit rund 8.000 Einwohnern (2017). Die Stadt liegt am Ufer des Drangme Chhu und ist bekannt für das Trashigang Dzong, das 1659 erbaut wurde und auf einem Felsvorsprung thront. Die Wirtschaft basiert auf Landwirtschaft (Reis, Mais, Zitrusfrüchte) und Handel. Trashigang ist ein wichtiger Verkehrsknotenpunkt für die östliche Region, mit Straßenverbindungen nach Mongar, Lhuntse und zum indischen Grenzort Samdrup Jongkhar. Das Klima ist subtropisch, mit heißen Sommern und milden Wintern. Die Stadt ist auch für ihre traditionellen Webereien bekannt, insbesondere für die Kira- und Gho-Stoffe. Der touristische Besuch ist aufgrund der abgelegenen Lage geringer als im Westen. Sehenswürdigkeiten sind das Dzong und die nahe gelegenen Dörfer.",
      hu: "Trashigang Bhután legkeletibb részén, 1.100 méter magasan, a Drangme Chhu folyó partján fekszik. A város a Trashigang körzet székhelye, és Kelet-Bhután legnagyobb települése, kb. 8.000 fős népességgel (2017). A várost uraló Trashigang Dzong 1659-ben épült, és stratégiai védelmi pontként szolgált. A gazdaság a mezőgazdaságra (rizs, kukorica, citrusfélék) és a kereskedelemre épül. Trashigang fontos közlekedési csomópont, ahonnan utak indulnak Mongar, Lhuntse és India felé. Az éghajlat szubtrópusi, forró nyárral és enyhe téllel. A város híres a hagyományos szövésről, a kira és gho ruhák készítéséről. A távoli elhelyezkedés miatt kevesebb turista látogat el ide, de a környék természeti szépségei vonzóak. A közeli Rangjung kolostor szintén jelentős.",
      ro: "Trashigang este un oraș în extremitatea estică a Bhutanului, la o altitudine de aproximativ 1.100 de metri, pe malul râului Drangme Chhu. Este reședința districtului Trashigang și cel mai mare oraș din estul Bhutanului, cu o populație de circa 8.000 de locuitori (2017). Orașul este dominat de Trashigang Dzong, construit în 1659 pe un pinten stâncos. Economia se bazează pe agricultură (orez, porumb, citrice) și comerț. Trashigang este un nod rutier important, cu legături către Mongar, Lhuntse și Samdrup Jongkhar. Clima este subtropicală, cu veri fierbinți și ierni blânde. Orașul este renumit pentru țesăturile tradiționale, în special pentru materialele kira și gho. Turismul este mai puțin dezvoltat decât în vest, dar peisajele sunt spectaculoase. În apropiere se află Mănăstirea Rangjung.",
      en: "Trashigang is a town in far eastern Bhutan at an elevation of about 1,100 meters, on the banks of the Drangme Chhu. It is the administrative center of Trashigang District and the largest town in eastern Bhutan, with a population of around 8,000 (2017). The town is dominated by Trashigang Dzong, built in 1659 on a rocky spur. The economy is based on agriculture (rice, maize, citrus) and trade. Trashigang is a major transport hub, with roads connecting to Mongar, Lhuntse, and Samdrup Jongkhar. The climate is subtropical, with hot summers and mild winters. The town is known for traditional weaving, especially kira and gho textiles. Tourism is less developed than in western Bhutan, but the area offers scenic landscapes. Nearby attractions include Rangjung Monastery.",
    },
    factsAdvanced: {
      de: ["Höhe: 1.100 m.", "Einwohner (2017): ca. 8.000.", "Bau des Dzong: 1659.", "Entfernung nach Thimphu: ca. 600 km.", "Jährliche Niederschlagsmenge: 1.500 mm.", "Länge der Straße nach Samdrup Jongkhar: 180 km."],
      hu: ["Tengerszint feletti magasság: 1.100 m.", "Népesség (2017): kb. 8.000 fő.", "Dzong építése: 1659.", "Távolság Thimphutól: kb. 600 km.", "Éves csapadékmennyiség: 1.500 mm.", "Út hossza Samdrup Jongkhar felé: 180 km."],
      ro: ["Altitudine: 1.100 m.", "Populație (2017): aprox. 8.000.", "Construirea dzong-ului: 1659.", "Distanța până la Thimphu: aprox. 600 km.", "Precipitații anuale: 1.500 mm.", "Lungimea drumului către Samdrup Jongkhar: 180 km."],
      en: ["Elevation: 1,100 m.", "Population (2017): approx. 8,000.", "Construction of Dzong: 1659.", "Distance to Thimphu: approx. 600 km.", "Annual rainfall: 1,500 mm.", "Road length to Samdrup Jongkhar: 180 km."],
    },
  },
  {
    id: "trongsa-cities-v2",
    type: "city",
    parent: "BT-32",
    coords: [90.5000, 27.5000],
    name: { de: "Trongsa", hu: "Trongsa", ro: "Trongsa", en: "Trongsa",
},
    description: {
      de: "Trongsa liegt geografisch im Zentrum Bhutans und war historisch von entscheidender Bedeutung für die Kontrolle des Königreichs.",
      hu: "Trongsa földrajzilag Bhután központjában fekszik, és történelmileg kulcsfontosságú volt a királyság feletti ellenőrzésben.",
      ro: "Trongsa se află geografic în centrul Bhutanului și a fost istoric crucială pentru controlul regatului.",
      en: "Trongsa is located geographically in the center of Bhutan and was historically crucial for controlling the kingdom."
    },
    facts: {
      de: [
        "Der Trongsa Dzong ist die größte Festung in Bhutan.",
        "Die königliche Familie hat hier ihre familiären Wurzeln."
      ],
      hu: [
        "A Trongsa Dzong a legnagyobb erőd Bhutánban.",
        "A királyi család családi gyökerei itt találhatók."
      ],
      ro: [
        "Trongsa Dzong este cea mai mare fortăreață din Bhutan.",
        "Familia regală își are rădăcinile ancestrale aici."
      ],
      en: [
        "Trongsa Dzong is the largest fortress in Bhutan.",
        "The royal family has its ancestral roots here."
      ]
    },
    image: "/poi-images/trongsa-cities-v2.webp",
    descriptionAdvanced: {
      de: "Trongsa liegt im Zentrum Bhutans auf einer Höhe von etwa 2.200 Metern. Die Stadt ist das administrative Zentrum des Distrikts Trongsa und hat rund 5.000 Einwohner. Sie ist bekannt für das riesige Trongsa Dzong, das 1648 erbaut wurde und die größte Festung in Bhutan ist. Der Dzong diente historisch als Sitz der Wangchuck-Dynastie, die heute die königliche Familie ist. Die Stadt liegt strategisch am einzigen Weg zwischen Ost- und Westbhutan. Die Wirtschaft basiert auf Landwirtschaft (Weizen, Gerste, Kartoffeln) und Tourismus. Das Klima ist gemäßigt, mit kühlen Sommern und kalten Wintern. In der Umgebung gibt es viele Wanderwege. Trongsa ist auch bekannt für seine traditionellen Handwerkskünste, insbesondere Holzschnitzerei. Der Dzong beherbergt ein Museum, das die Geschichte der bhutanischen Monarchie zeigt. Jedes Jahr findet das Trongsa Tshechu-Festival statt.",
      hu: "Trongsa Bhután középső részén, 2.200 méter magasan fekszik. A város a Trongsa körzet székhelye, lakossága kb. 5.000 fő. Híres a Trongsa Dzongról, amely 1648-ban épült, és ez Bhután legnagyobb erődítménye. A dzong a Wangchuck-dinasztia bölcsője; itt született több király is. A város stratégiai helyen található a keleti és nyugati Bhutánt összekötő úton. A gazdaság a mezőgazdaságra (búza, árpa, burgonya) és a turizmusra épül. Az éghajlat mérsékelt, hűvös nyárral és hideg téllel. Trongsa híres a hagyományos fafaragásairól. A dzongban múzeum működik. A közeli Chendebji Csörten egy tibeti stílusú sztúpa. A város fontos zarándokhely.",
      ro: "Trongsa este un oraș în centrul Bhutanului, la o altitudine de aproximativ 2.200 de metri. Este reședința districtului Trongsa, cu o populație de circa 5.000 de locuitori. Orașul este renumit pentru Trongsa Dzong, construit în 1648, cea mai mare fortăreață din Bhutan. Dzong-ul a fost leagănul dinastiei Wangchuck. Orașul este poziționat strategic pe singurul drum care leagă estul de vest. Economia se bazează pe agricultură (grâu, orz, cartofi) și turism. Clima este temperată, cu veri răcoroase și ierni reci. Trongsa este cunoscut pentru sculpturile în lemn. Dzong-ul găzduiește un muzeu al monarhiei. În apropiere se află Chendebji Chorten, o stupă de stil tibetan. Festivalul Trongsa Tshechu atrage mulți pelerini.",
      en: "Trongsa is a town in central Bhutan at an elevation of about 2,200 meters. It is the administrative center of Trongsa District, with a population of around 5,000. The town is famous for Trongsa Dzong, built in 1648, the largest fortress in Bhutan. The dzong was the cradle of the Wangchuck dynasty, which produces the kings. Trongsa is strategically located on the only road connecting eastern and western Bhutan. The economy relies on agriculture (wheat, barley, potatoes) and tourism. The climate is temperate, with cool summers and cold winters. Trongsa is known for traditional wood carving. The dzong houses a museum on Bhutan's monarchy. Nearby Chendebji Chorten is a Tibetan-style stupa. The annual Trongsa Tshechu attracts pilgrims.",
    },
    factsAdvanced: {
      de: ["Höhe: 2.200 m.", "Einwohner: ca. 5.000.", "Bau des Dzong: 1648.", "Größe des Dzong: größtes in Bhutan.", "Entfernung nach Thimphu: ca. 215 km.", "Jährliche Besucher des Museums: über 10.000."],
      hu: ["Tengerszint feletti magasság: 2.200 m.", "Lakosság: kb. 5.000 fő.", "Dzong építése: 1648.", "A dzong mérete: a legnagyobb Bhutánban.", "Távolság Thimphutól: kb. 215 km.", "Múzeum éves látogatói: >10.000."],
      ro: ["Altitudine: 2.200 m.", "Populație: aprox. 5.000.", "Construirea dzong-ului: 1648.", "Dimensiunea dzong-ului: cel mai mare din Bhutan.", "Distanța până la Thimphu: aprox. 215 km.", "Vizitatori anuali ai muzeului: peste 10.000."],
      en: ["Elevation: 2,200 m.", "Population: approx. 5,000.", "Construction of Dzong: 1648.", "Dzong size: largest in Bhutan.", "Distance to Thimphu: approx. 215 km.", "Annual museum visitors: over 10,000."],
    },
  },
  {
    id: "samtse-cities-v2",
    type: "city",
    parent: "BT-14",
    coords: [89.1000, 26.9000],
    name: { de: "Samtse", hu: "Samtse", ro: "Samtse", en: "Samtse",
},
    description: {
      de: "Samtse ist eine grenznahe Industriestadt im Südwesten Bhutans mit einer vielfältigen Bevölkerung und reichen Mineralvorkommen.",
      hu: "Samtse egy határmenti iparváros Délnyugat-Bhutánban, amely sokszínű lakossággal és gazdag ásványkincs-lelőhelyekkel rendelkezik.",
      ro: "Samtse este un oraș industrial de graniță în sud-vestul Bhutanului, cu o populație diversă și zăcăminte minerale bogate.",
      en: "Samtse is a border industrial town in southwestern Bhutan with a diverse population and rich mineral deposits."
    },
    facts: {
      de: [
        "Es war historisch bekannt für seine großen Kardamomplantagen.",
        "Die Stadt weist deutliche kulturelle Einflüsse aus dem benachbarten Indien auf."
      ],
      hu: [
        "Történelmileg ismert volt nagy kardamomültetvényeiről.",
        "A város jelentős kulturális hatásokat mutat a szomszédos Indiából."
      ],
      ro: [
        "A fost istoric cunoscut pentru marile sale plantații de cardamom.",
        "Orașul prezintă influențe culturale distincte din India vecină."
      ],
      en: [
        "It was historically known for its large cardamom plantations.",
        "The town features distinct cultural influences from neighboring India."
      ]
    },
    image: "/poi-images/samtse-cities-v2.webp",
    descriptionAdvanced: {
      de: "Samtse liegt im Südwesten Bhutans an der Grenze zu Indien, auf einer Höhe von etwa 400 Metern. Die Stadt ist das administrative Zentrum des Distrikts Samtse und hat rund 6.000 Einwohner (2017). Die Wirtschaft basiert auf Landwirtschaft (Reis, Mais, Jute) und Handel mit Indien. Samtse hat ein subtropisches Klima mit heißen Sommern und Monsunregen. Die Stadt ist ethnisch vielfältig, mit einer großen nepalesischen Gemeinschaft. Die Umgebung ist hügelig und bewaldet. Samtse ist bekannt für seine Zitrusplantagen und Bambuswälder. Sehenswürdigkeiten sind das Samtse Dzong, das 1976 erbaut wurde, und die heißen Quellen in der Nähe. Die Stadt ist auch ein wichtiger Punkt für den Grenzhandel mit indischen Städten wie Siliguri. Die Infrastruktur umfasst ein Krankenhaus, Schulen und einen Markt. Der Fluss Torsa fließt durch die Region.",
      hu: "Samtse Bhután délnyugati részén, az indiai határ közelében, 400 méter magasan fekszik. A város a Samtse körzet székhelye, lakossága kb. 6.000 fő (2017). A gazdaság a mezőgazdaságra (rizs, kukorica, juta) és a határ menti kereskedelemre épül. Az éghajlat szubtrópusi, forró nyárral és monszunesőkkel. A népesség etnikailag sokszínű, jelentős nepáli közösséggel. A környék dombos, trópusi erdőkkel. Samtse híres citrusültetvényeiről és bambuszligeteiről. A városban található a Samtse Dzong, amelyet 1976-ban építettek. A közeli forró források népszerűek. Samtse fontos kereskedelmi kapu India felé. A Torsa folyó átszeli a régiót. A városban kórház, iskolák és piac működik.",
      ro: "Samtse este un oraș în sud-vestul Bhutanului, la granița cu India, la o altitudine de aproximativ 400 de metri. Este reședința districtului Samtse, cu o populație de circa 6.000 de locuitori (2017). Economia se bazează pe agricultură (orez, porumb, iută) și comerțul cu India. Clima este subtropicală, cu veri fierbinți și musoni abundenți. Populația este diversă etnic, inclusiv o comunitate nepaleză mare. Împrejurimile sunt deluroase și împădurite. Samtse este cunoscut pentru livezile de citrice și pădurile de bambus. Orașul are un dzong construit în 1976. În apropiere se află izvoare termale. Samtse servește ca punct comercial principal cu orașe indiene precum Siliguri. Râul Torsa traversează zona. Facilitățile includ spital, școli și piață.",
      en: "Samtse is a town in southwestern Bhutan near the border with India, at an elevation of about 400 meters. It is the administrative center of Samtse District, with a population of around 6,000 (2017). The economy relies on agriculture (rice, maize, jute) and cross-border trade. The climate is subtropical, with hot summers and monsoon rains. The population is ethnically diverse, including a significant Nepali community. The surrounding area is hilly and forested. Samtse is known for its citrus orchards and bamboo forests. The Samtse Dzong was built in 1976. Hot springs are located nearby. The town is a key trade gateway with Indian cities like Siliguri. The Torsa River flows through the region. Facilities include a hospital, schools, and a market.",
    },
    factsAdvanced: {
      de: ["Höhe: ca. 400 m.", "Einwohner (2017): ca. 6.000.", "Baujahr des Dzong: 1976.", "Jahresniederschlag: >2.000 mm.", "Entfernung zur indischen Grenze: 2 km.", "Hauptanbauprodukte: Zitrusfrüchte, Bambus."],
      hu: ["Tengerszint feletti magasság: kb. 400 m.", "Népesség (2017): kb. 6.000 fő.", "Dzong építési éve: 1976.", "Éves csapadék: >2.000 mm.", "Távolság az indiai határtól: 2 km.", "Fő termények: citrusfélék, bambusz."],
      ro: ["Altitudine: aprox. 400 m.", "Populație (2017): aprox. 6.000.", "Anul construirii dzong-ului: 1976.", "Precipitații anuale: >2.000 mm.", "Distanța până la granița indiană: 2 km.", "Culturi principale: citrice, bambus."],
      en: ["Elevation: approx. 400 m.", "Population (2017): approx. 6,000.", "Construction year of Dzong: 1976.", "Annual rainfall: >2,000 mm.", "Distance to Indian border: 2 km.", "Main crops: citrus, bamboo."],
    },
  },
  {
    id: "damphu-cities-v2",
    type: "city",
    parent: "BT-21",
    coords: [90.1167, 27.0167],
    name: { de: "Damphu", hu: "Damphu", ro: "Damphu", en: "Damphu",
},
    description: {
      de: "Damphu ist das Verwaltungszentrum des Tsirang-Distrikts und berühmt für sein sanftes Klima und reiche landwirtschaftliche Erträge.",
      hu: "Damphu a Tsirang körzet közigazgatási központja, amely enyhe éghajlatáról és gazdag mezőgazdasági hozamairól híres.",
      ro: "Damphu este centrul administrativ al districtului Tsirang și este renumit pentru clima sa blândă și randamentele agricole bogate.",
      en: "Damphu is the administrative center of Tsirang district, famous for its mild climate and rich agricultural yields."
    },
    facts: {
      de: [
        "Das Gebiet wird überwiegend von der Lhotshampa-Gemeinschaft bewohnt.",
        "Es ist ein bedeutender Produzent von Bio-Gemüse im Land."
      ],
      hu: [
        "A területet túlnyomórészt a Lhotshampa közösség lakja.",
        "Jelentős biotermelő az országban."
      ],
      ro: [
        "Zona este locuită predominant de comunitatea Lhotshampa.",
        "Este un producător major de legume organice în țară."
      ],
      en: [
        "The area is predominantly inhabited by the Lhotshampa community.",
        "It is a major producer of organic vegetables in the country."
      ]
    },
    image: "/poi-images/damphu-cities-v2.webp",
    descriptionAdvanced: {
      de: "Damphu ist eine Stadt im zentralen Bhutan im Distrikt Tsirang, auf einer Höhe von etwa 1.200 Metern. Die Stadt hat rund 5.000 Einwohner und ist das administrative Zentrum des Distrikts. Die Wirtschaft basiert auf Landwirtschaft (Reis, Mais, Gemüse) und Kleinhandel. Damphu hat ein subtropisches Klima mit warmen Sommern und milden Wintern. Die Stadt ist bekannt für ihre freundlichen Einwohner und die traditionelle Lebensweise. In der Umgebung gibt es Wälder und Hügel. Sehenswürdigkeiten sind das Damphu Dzong, das in den 1990er Jahren erbaut wurde, und die nahe gelegenen Klöster. Der Ort dient als Raststätte für Reisende auf dem Weg nach Südbhutan. Die Infrastruktur umfasst eine Tankstelle, einen Markt und einige Gästehäuser. Damphu ist auch für seine Bambusprodukte bekannt. Der Fluss Tsirang Chhu fließt durch die Region.",
      hu: "Damphu Bhután középső részén, a Csirang körzetben, 1.200 méter magasan fekszik. Népessége kb. 5.000 fő, és a körzet székhelye. A gazdaság a mezőgazdaságra (rizs, kukorica, zöldség) és a kiskereskedelemre épül. Az éghajlat szubtrópusi, meleg nyárral és enyhe téllel. A város híres a hagyományos bhutáni életmódról. A környező erdők és dombok természeti szépséget nyújtanak. A Damphu Dzongot az 1990-es években építették. A közeli kolostorok szintén látogathatók. Damphu fontos pihenőhely a déli útvonalon. Infrastruktúrája magában foglal egy benzinkutat, piacot és vendégházakat. A város bambusztermékeiről is ismert. A Csirang Chhu folyó a közelben folyik.",
      ro: "Damphu este un oraș în centrul Bhutanului, în districtul Tsirang, la o altitudine de aproximativ 1.200 de metri. Are o populație de circa 5.000 de locuitori și este reședința districtului. Economia se bazează pe agricultură (orez, porumb, legume) și comerțul local. Clima este subtropicală, cu veri calde și ierni blânde. Orașul este cunoscut pentru modul de viață tradițional. În jur se găsesc păduri și dealuri. Damphu Dzong a fost construit în anii 1990. Mănăstirile din apropiere sunt atracții. Damphu servește ca popas pentru călătorii spre sud. Facilitățile includ o benzinărie, piață și pensiuni. Orașul este renumit pentru articolele din bambus. Râul Tsirang Chhu traversează zona.",
      en: "Damphu is a town in central Bhutan in Tsirang District, at an elevation of about 1,200 meters. It has a population of around 5,000 and serves as the district's administrative center. The economy is based on agriculture (rice, maize, vegetables) and small-scale trade. The climate is subtropical, with warm summers and mild winters. The town is known for its traditional lifestyle. Surrounding hills and forests offer natural scenery. Damphu Dzong was built in the 1990s. Nearby monasteries are also attractions. Damphu is a stopover for travelers heading south. Facilities include a petrol station, market, and guesthouses. The town is known for bamboo crafts. The Tsirang Chhu river flows nearby.",
    },
    factsAdvanced: {
      de: ["Höhe: 1.200 m.", "Einwohner: ca. 5.000.", "Bau des Dzong: 1990er Jahre.", "Entfernung nach Thimphu: ca. 110 km.", "Jährliche Niederschlagsmenge: 1.200 mm.", "Hauptprodukt: Bambushandwerk."],
      hu: ["Tengerszint feletti magasság: 1.200 m.", "Lakosság: kb. 5.000 fő.", "Dzong építése: 1990-es évek.", "Távolság Thimphutól: kb. 110 km.", "Éves csapadékmennyiség: 1.200 mm.", "Fő termék: bambuszkézművesség."],
      ro: ["Altitudine: 1.200 m.", "Populație: aprox. 5.000.", "Construirea dzong-ului: anii 1990.", "Distanța până la Thimphu: aprox. 110 km.", "Precipitații anuale: 1.200 mm.", "Produs principal: obiecte din bambus."],
      en: ["Elevation: 1,200 m.", "Population: approx. 5,000.", "Construction of Dzong: 1990s.", "Distance to Thimphu: approx. 110 km.", "Annual rainfall: 1,200 mm.", "Main product: bamboo crafts."],
    },
  },
  {
    id: "haa-cities-v2",
    type: "city",
    parent: "BT-13",
    coords: [89.2833, 27.3833],
    name: { de: "Haa", hu: "Haa", ro: "Haa", en: "Haa",
},
    description: {
      de: "Haa ist eine kleine, abgeschiedene Stadt im Westen Bhutans, eingebettet in ein tiefes Tal nahe der Grenze zu Tibet.",
      hu: "Haa egy kicsi, elzárt város Nyugat-Bhutánban, amely egy mély völgyben fekszik a tibeti határ közelében.",
      ro: "Haa este un oraș mic și izolat din vestul Bhutanului, cuibărit într-o vale adâncă lângă granița cu Tibetul.",
      en: "Haa is a small, secluded town in western Bhutan, nestled in a deep valley near the border with Tibet."
    },
    facts: {
      de: [
        "Das Haa-Tal wurde erst 2002 für ausländische Touristen geöffnet.",
        "Das lokale indische militärische Trainingsteam hat hier eine wichtige Basis."
      ],
      hu: [
        "A Haa-völgyet csak 2002-ben nyitották meg a külföldi turisták előtt.",
        "A helyi indiai katonai kiképzőcsapat jelentős bázissal rendelkezik itt."
      ],
      ro: [
        "Valea Haa a fost deschisă turiștilor străini abia în 2002.",
        "Echipa locală de antrenament militar indian are o bază semnificativă aici."
      ],
      en: [
        "The Haa valley was opened to foreign tourists only in 2002.",
        "The local Indian Military Training Team has a significant base here."
      ]
    },
    image: "/poi-images/haa-cities-v2.webp",
    descriptionAdvanced: {
      de: "Haa ist eine Stadt im westlichen Bhutan im Haa-Tal, auf einer Höhe von etwa 2.700 Metern. Die Stadt hat rund 3.000 Einwohner und ist das administrative Zentrum des Distrikts Haa. Das Tal ist bekannt für seine unberührte Landschaft und gilt als eines der schönsten Bhutans. Die Wirtschaft basiert auf Landwirtschaft (Gerste, Weizen, Kartoffeln) und zunehmendem Tourismus. Haa hat ein gemäßigtes Klima mit kühlen Sommern und kalten Wintern. Sehenswürdigkeiten sind das Haa Dzong, das 1915 erbaut wurde, und das Lhakhang Karpo (Weißer Tempel) sowie das Lhakhang Nakpo (Schwarzer Tempel), die aus dem 7. Jahrhundert stammen sollen. Die Stadt ist ein beliebtes Ziel für Trekking und Vogelbeobachtung. Die Umgebung ist reich an Legenden und heiligen Stätten. Der Haa Chhu fließt durch das Tal. Die Bevölkerung besteht hauptsächlich aus den ethnischen Gruppen der Ngalop und Bumthang.",
      hu: "Haa Bhután nyugati részén, a Haa-völgyben, 2.700 méter magasan fekszik. Népessége kb. 3.000 fő, a Haa körzet székhelye. A völgy Bhután egyik legszebb területe. A gazdaság a mezőgazdaságra (árpa, búza, burgonya) és a turizmusra épül. Az éghajlat mérsékelt, hűvös nyárral és hideg téllel. Haa ismert a Haa Dzongról (1915) és a Lhakhang Karpo (Fehér templom) és Lhakhang Nakpo (Fekete templom) nevű 7. századi templomokról. A város népszerű a túrázók és madármegfigyelők körében. A környék legendákban gazdag. A Haa Chhu folyó átszeli a völgyet. A lakosság főként ngalop és bumthang etnikumú.",
      ro: "Haa este un oraș în vestul Bhutanului, în valea Haa, la o altitudine de aproximativ 2.700 de metri. Are aproximativ 3.000 de locuitori și este reședința districtului Haa. Valea este renumită pentru peisajele sale virgine. Economia se bazează pe agricultură (orz, grâu, cartofi) și turism. Clima este temperată, cu veri răcoroase și ierni reci. Haa este cunoscut pentru Haa Dzong (1915) și templele Lhakhang Karpo (Alb) și Lhakhang Nakpo (Negru), datând din secolul al VII-lea. Orașul atrage drumeții și observatorii de păsări. Zona abundă în legende. Râul Haa Chhu traversează valea. Populația este preponderent ngalop și bumthang.",
      en: "Haa is a town in western Bhutan in the Haa Valley, at an elevation of about 2,700 meters. It has a population of around 3,000 and is the administrative center of Haa District. The valley is known for its pristine beauty. The economy is based on agriculture (barley, wheat, potatoes) and tourism. The climate is temperate, with cool summers and cold winters. Haa is known for Haa Dzong (built 1915) and the Lhakhang Karpo (White Temple) and Lhakhang Nakpo (Black Temple), believed to date from the 7th century. The town is popular for trekking and birdwatching. The area is rich in legends. The Haa Chhu river flows through the valley. The population is predominantly Ngalop and Bumthang.",
    },
    factsAdvanced: {
      de: ["Höhe: 2.700 m.", "Einwohner: ca. 3.000.", "Bau des Haa Dzong: 1915.", "Alter der Lhakhang Karpo und Nakpo: 7. Jahrhundert.", "Entfernung nach Thimphu: ca. 80 km.", "Jährliche Touristenankünfte (2019): ca. 1.500."],
      hu: ["Tengerszint feletti magasság: 2.700 m.", "Lakosság: kb. 3.000 fő.", "Haa Dzong építése: 1915.", "Lhakhang Karpo és Nakpo kora: 7. század.", "Távolság Thimphutól: kb. 80 km.", "Éves turistaérkezések (2019): kb. 1.500."],
      ro: ["Altitudine: 2.700 m.", "Populație: aprox. 3.000.", "Construirea Haa Dzong: 1915.", "Vechimea Lhakhang Karpo și Nakpo: secolul VII.", "Distanța până la Thimphu: aprox. 80 km.", "Sosiri turistice anuale (2019): aprox. 1.500."],
      en: ["Elevation: 2,700 m.", "Population: approx. 3,000.", "Construction of Haa Dzong: 1915.", "Age of Lhakhang Karpo and Nakpo: 7th century.", "Distance to Thimphu: approx. 80 km.", "Annual tourist arrivals (2019): approx. 1,500."],
    },
  },
  {
    id: "lhuntse-cities-v2",
    type: "city",
    parent: "BT-44",
    coords: [91.1667, 27.6667],
    name: { de: "Lhuntse", hu: "Lhuntse", ro: "Lhuntse", en: "Lhuntse",
},
    description: {
      de: "Lhuntse ist eine abgelegene Siedlung im Nordosten Bhutans, die weithin für ihre exquisiten und komplexen Webkünste anerkannt ist.",
      hu: "Lhuntse egy elszigetelt település Északkelet-Bhutánban, amelyet széles körben elismernek lenyűgöző és bonyolult szövőművészetéért.",
      ro: "Lhuntse este o așezare izolată din nord-estul Bhutanului, recunoscută pe scară largă pentru artele sale de țesut rafinate și complexe.",
      en: "Lhuntse is a remote settlement in northeastern Bhutan, widely recognized for its exquisite and complex weaving arts."
    },
    facts: {
      de: [
        "Die Region ist die angestammte Heimat der königlichen Familie von Bhutan.",
        "Kishu Thara, ein hoch geschätztes bhutanisches Textil, stammt von hier."
      ],
      hu: [
        "A régió Bhután királyi családjának ősi otthona.",
        "Kishu Thara, egy nagyra értékelt bhutáni textil, innen származik."
      ],
      ro: [
        "Regiunea este casa ancestrală a familiei regale din Bhutan.",
        "Kishu Thara, un textil bhutanez foarte apreciat, provine de aici."
      ],
      en: [
        "The region is the ancestral home of the royal family of Bhutan.",
        "Kishu Thara, a highly prized Bhutanese textile, originates here."
      ]
    },
    image: "/poi-images/lhuntse-cities-v2.webp",
    descriptionAdvanced: {
      de: "Lhuntse ist eine Stadt im Nordosten Bhutans und Hauptstadt des gleichnamigen Distrikts. Sie liegt auf etwa 1500 Metern Höhe im Tal des Kuri Chhu. Die Region ist bekannt für ihre traditionellen Textilien, insbesondere die Kushuthara, ein aufwendig bestickter Stoff, der bei zeremoniellen Anlässen getragen wird. Lhuntse beherbergt den Lhuntse-Dzong, der im 17. Jahrhundert erbaut wurde. Die Wirtschaft basiert auf Landwirtschaft und Viehzucht, wobei Yaks eine wichtige Rolle spielen.",
      hu: "Lhuntse Bhután egyik legelzártabb régiója, megőrizve a hagyományos életmódot. A városban található a Lhuntse Dzong, amely a Kuri Chhu folyóra néz. A környék híres a sáfrányról és a gyógynövényekről. A buddhista kolostorok, mint a Takila Lhakhang, fontos zarándokhelyek. A Lhuntse Tshechu fesztiválon bemutatják a helyi maszkos táncokat. A gazdaságot a mezőgazdaság és a kézművesség uralja.",
      ro: "Lhuntse este un oraș mic, cu o populație de aproximativ 2000 de locuitori. Este situat la o altitudine de 1500 m, în nord-estul Bhutanului. Economia se bazează pe agricultură de subzistență: orez, porumb și cartofi. Creșterea iacilor este importantă pentru produse lactate și lână. Regiunea este renumită pentru țesăturile tradiționale, în special pentru broderia kushuthara. Clima este subtropicală, cu veri umede și ierni uscate.",
      en: "Lhuntse is known for its high-quality textiles and is considered the birthplace of the kushuthara, a traditional Bhutanese costume. The Lhuntse Dzong was built in 1654 and houses the district administration and monk body. The region is rich in biodiversity, with forests of pine, oak, and rhododendron. Wildlife includes snow leopards and red pandas. The town serves as a starting point for treks to the remote eastern mountains. Local cuisine features dishes like momo dumplings and yak cheese.",
    },
    factsAdvanced: {
      de: ["Lhuntse liegt auf 1500 m Höhe.", "Der Lhuntse-Dzong wurde 1654 erbaut.", "Die Stadt hat etwa 2000 Einwohner.", "Die Kushuthara-Stickerei benötigt 2-3 Monate Arbeit.", "Die Region produziert jährlich 500 Tonnen Kartoffeln.", "Yakzucht umfasst über 10.000 Tiere im Distrikt."],
      hu: ["A Lhuntse Tshechu fesztivál 3 napig tart.", "A Takila Lhakhang templomot a 8. században alapították.", "Sáfránytermesztés évi 50 kg.", "A kushuthara ára eléri a 10.000 dollárt.", "A Kuri Chhu folyó 120 km hosszú.", "A dzong 4 szintből áll."],
      ro: ["Populația districtului Lhuntse este de ~14.000.", "Altitudinea medie este de 1500 m.", "Producția anuală de orez este de 300 de tone.", "Creșterea iacilor produce 200 de tone de brânză anual.", "Suprafața pădurilor este de 80% din district.", "Temperatura medie anuală este de 18°C."],
      en: ["Lhuntse Dzong was built in 1654.", "Population of Lhuntse town is 2000.", "The kushuthara is handmade.", "Yak population in Lhuntse is 10,000.", "Annual potato production is 500 tons.", "The town altitude is 1500 m."],
    },
  },
  {
    id: "pemagatshel-cities-v2",
    type: "city",
    parent: "BT-43",
    coords: [91.4333, 27.0333],
    name: { de: "Pemagatshel", hu: "Pemagatshel", ro: "Pemagatshel", en: "Pemagatshel",
},
    description: {
      de: "Pemagatshel, bekannt für seine religiöse Bedeutung und Kunsthandwerker, ist ein kleiner Ort auf einem steilen Bergrücken im Osten Bhutans.",
      hu: "A vallási jelentőségéről és kézműveseiről ismert Pemagatshel egy kis település, amely egy meredek hegygerincen található Kelet-Bhutánban.",
      ro: "Pemagatshel, cunoscut pentru importanța sa religioasă și artizani, este o mică așezare pe o creastă montană abruptă din estul Bhutanului.",
      en: "Pemagatshel, known for its religious significance and artisans, is a small town located on a steep mountain ridge in eastern Bhutan."
    },
    facts: {
      de: [
        "Der Distrikt ist berühmt für seine traditionellen Holzschalen namens Dhapa.",
        "Es ist ein wichtiges Zentrum für den Nyingma-Buddhismus."
      ],
      hu: [
        "A körzet a dhapa nevű hagyományos fatálairól híres.",
        "A Nyingma-buddhizmus egyik fontos központja."
      ],
      ro: [
        "Districtul este faimos pentru bolurile sale tradiționale din lemn numite dhapa.",
        "Este un centru important pentru budismul Nyingma."
      ],
      en: [
        "The district is famous for its traditional wooden bowls called dhapa.",
        "It is an important center for Nyingma Buddhism."
      ]
    },
    image: "/poi-images/pemagatshel-cities-v2.webp",
    descriptionAdvanced: {
      de: "Pemagatshel ist eine Stadt im Osten Bhutans und Hauptort des gleichnamigen Distrikts. Sie liegt auf einer Höhe von etwa 1500 m. Die Stadt ist ein Handelszentrum und bekannt für ihre Klöster. Die Wirtschaft basiert auf Landwirtschaft (Reis, Mais) und Tourismus. Der Ort hat eine gemischte Bevölkerung aus Ngalops und Sharchops.",
      hu: "Pemagatshel város Kelet-Bhutánban, az azonos nevű körzet székhelye. Körülbelül 1500 m magasságban fekszik. A város kereskedelmi központ, kolostorairól ismert. Gazdasága mezőgazdaságon (rizs, kukorica) és turizmuson alapul. Lakossága vegyes: ngalopok és sarkopok.",
      ro: "Pemagatshel este un oraș în estul Bhutanului, reședința districtului cu același nume. Este situat la aproximativ 1500 m altitudine. Orașul este un centru comercial și cunoscut pentru mănăstirile sale. Economia se bazează pe agricultură (orez, porumb) și turism. Populația este mixtă, formată din ngalops și sharchops.",
      en: "Pemagatshel is a town in eastern Bhutan, capital of the same-named district. It lies at about 1,500 m elevation. The town is a trading center and known for its monasteries. Economy relies on agriculture (rice, maize) and tourism. The population is mixed, comprising Ngalops and Sharchops.",
    },
    factsAdvanced: {
      de: ["Höhe ca. 1500 m", "Hauptort des Pemagatshel-Distrikts", "bekannt für Klöster", "Landwirtschaft: Reis und Mais", "Handelszentrum", "gemischte Bevölkerung aus Ngalops und Sharchops"],
      hu: ["magassága kb. 1500 m", "a Pemagatshel körzet székhelye", "kolostorairól ismert", "mezőgazdaság: rizs és kukorica", "kereskedelmi központ", "vegyes lakosság: ngalopok és sarkopok"],
      ro: ["altitudine cca 1500 m", "reședința districtului Pemagatshel", "cunoscut pentru mănăstiri", "agricultură: orez și porumb", "centru comercial", "populație mixtă: ngalops și sharchops"],
      en: ["elevation about 1,500 m", "capital of Pemagatshel District", "known for monasteries", "agriculture: rice and maize", "trading center", "mixed population of Ngalops and Sharchops"],
    },
  },
  {
    id: "zhemgang-cities-v2",
    type: "city",
    parent: "BT-35",
    coords: [90.6500, 27.2167],
    name: { de: "Zhemgang", hu: "Zhemgang", ro: "Zhemgang", en: "Zhemgang",
},
    description: {
      de: "Zhemgang ist eine abgelegene Stadt in Zentralbhutan, die für ihre immense biologische Vielfalt und ihre tief verwurzelten animistischen Traditionen bekannt ist.",
      hu: "Zhemgang egy távoli város Közép-Bhutánban, amely hatalmas biológiai sokféleségéről és mélyen gyökerező animista hagyományairól ismert.",
      ro: "Zhemgang este un oraș izolat din centrul Bhutanului, renumit pentru diversitatea sa biologică imensă și tradițiile animiste profunde.",
      en: "Zhemgang is a remote town in central Bhutan, renowned for its immense biodiversity and deep-rooted animist traditions."
    },
    facts: {
      de: [
        "Die Region gilt als einer der besten Orte zur Vogelbeobachtung in Bhutan.",
        "Der Goldlangur, eine seltene Primatenart, wird hier häufig gesehen."
      ],
      hu: [
        "A régiót Bhután egyik legjobb madármegfigyelő helyének tartják.",
        "Az aranylangur, egy ritka főemlősfaj, gyakran látható itt."
      ],
      ro: [
        "Regiunea este considerată unul dintre cele mai bune locuri pentru observarea păsărilor din Bhutan.",
        "Langurul auriu, o specie rară de primate, este văzut frecvent aici."
      ],
      en: [
        "The region is considered one of the best places for birdwatching in Bhutan.",
        "The Golden Langur, a rare primate species, is commonly seen here."
      ]
    },
    image: "/poi-images/zhemgang-cities-v2.webp",
    descriptionAdvanced: {
      de: "Zhemgang ist eine Stadt im Süden Bhutans und Hauptstadt des Distrikts Zhemgang. Sie liegt auf etwa 900 Metern Höhe in einer Hügelregion. Die Stadt ist ein Tor zum Royal Manas National Park, einem der ältesten Schutzgebiete Bhutans. Die Wirtschaft wird von Landwirtschaft und Tourismus dominiert. Zhemgang beherbergt den Zhemgang-Dzong, der im 17. Jahrhundert erbaut wurde. Die Region ist bekannt für ihre Bambusprodukte und traditionellen Tänze.",
      hu: "Zhemgang Bhután egyik legkevésbé fejlett régiója, ahol a lakosság nagy része mezőgazdaságból él. A város a Zhemgang Dzong körül épült, amely fontos közigazgatási központ. A környék híres a vadvilágról, különösen az ázsiai elefántokról és a tigrisekről. A Royal Manas Nemzeti Park a 20. század eleje óta védett terület. A helyi közösségek hagyományos házakban élnek, amelyek fa- és bambuszépítészetűek.",
      ro: "Zhemgang este un oraș mic cu o populație de aproximativ 3000 de locuitori. Altitudinea este de 900 m. Clima este subtropicală, cu precipitații abundente, până la 3000 mm anual. Economia se bazează pe agricultură (orez, porumb, citrice) și silvicultură. Orașul este un centru pentru excursii în Parcul Național Royal Manas. Meșteșugurile includ coșuri de bambus și țesături.",
      en: "Zhemgang is the gateway to the Royal Manas National Park, a UNESCO World Heritage site. The park covers 1,057 km² and is home to Bengal tigers, Asian elephants, and golden langurs. The Zhemgang Dzong was built in 1638 and reconstructed in the 20th century. The region is known for its birdwatching, with over 400 species recorded. Local communities practice shifting agriculture and maintain traditional bamboo crafts. The town has seen growth in eco-tourism infrastructure.",
    },
    factsAdvanced: {
      de: ["Zhemgang liegt auf 900 m Höhe.", "Der Zhemgang-Dzong wurde 1638 erbaut.", "Die Stadt hat 3000 Einwohner.", "Der Royal Manas Nationalpark ist 1057 km² groß.", "Jährlich fallen 3000 mm Niederschlag.", "Die Region zählt über 400 Vogelarten."],
      hu: ["A Royal Manas Nemzeti Parkot 1966-ban alapították.", "A park területe 1057 km².", "Bhutánban 68 tigris él, közülük sokan itt.", "A helyi bambusz kézművesség 100 éves múltra tekint vissza.", "A Zhemgang Tshechu fesztivál 2 napig tart.", "A város éves turistaforgalma 5000 fő."],
      ro: ["Populația orașului Zhemgang este de 3000.", "Parcul Național Royal Manas se întinde pe 1057 km².", "Precipitațiile anuale sunt de 3000 mm.", "Altitudinea orașului este de 900 m.", "Specia de langur auriu este endemică.", "Temperatura medie este de 22°C."],
      en: ["Zhemgang Dzong was built in 1638.", "Royal Manas National Park covers 1,057 km².", "Annual rainfall is 3000 mm.", "Elevation is 900 m.", "Bird species count is over 400.", "The park is part of the Manas Transboundary Conservation Area."],
    },
  },
  {
    id: "gasa-cities-v2",
    type: "city",
    parent: "BT-33",
    coords: [89.7167, 27.9000],
    name: { de: "Gasa", hu: "Gasa", ro: "Gasa", en: "Gasa",
},
    description: {
      de: "Gasa ist die nördlichste und höchstgelegene Distrikthauptstadt Bhutans, die für ihre heilenden heißen Quellen und das harte Hochgebirgsklima berühmt ist.",
      hu: "Gasa Bhután legészakibb és legmagasabban fekvő körzetközpontja, amely gyógyító meleg vizű forrásairól és zord magashegyi éghajlatáról híres.",
      ro: "Gasa este cea mai nordică și înaltă capitală de district din Bhutan, faimoasă pentru izvoarele sale termale vindecătoare și clima aspră de munte.",
      en: "Gasa is the northernmost and highest district capital in Bhutan, famous for its healing hot springs and harsh high-mountain climate."
    },
    facts: {
      de: [
        "Die Stadt verzeichnet in den Wintermonaten starke Schneefälle.",
        "Die Region ist die Heimat des nomadischen Layap-Volkes."
      ],
      hu: [
        "A város a téli hónapokban erős havazást tapasztal.",
        "A régió a nomád Layap nép otthona."
      ],
      ro: [
        "Orașul se confruntă cu ninsori abundente în timpul lunilor de iarnă.",
        "Regiunea găzduiește poporul nomad Layap."
      ],
      en: [
        "The town experiences heavy snowfall during the winter months.",
        "The region is home to the nomadic Layap people."
      ]
    },
    image: "/poi-images/gasa-cities-v2.webp",
    descriptionAdvanced: {
      de: "Gasa ist eine Stadt im Nordwesten Bhutans und Hauptstadt des Distrikts Gasa. Sie liegt auf einer Höhe von etwa 2800 Metern und ist bekannt für ihre heißen Quellen, die als heilig gelten. Der Gasa-Dzong wurde im 17. Jahrhundert erbaut und dient als Kloster und Verwaltungszentrum. Die Region ist dünn besiedelt, mit einer Wirtschaft, die auf Yakzucht, Kartoffelanbau und Tourismus basiert. Gasa ist ein beliebtes Ziel für Trekkingtouren durch den Jigme Dorji Nationalpark.",
      hu: "Gasa Bhután egyik legmagasabban fekvő városa, ahonnan lenyűgöző kilátás nyílik a Himalájára. A Gasa Dzong a 17. században épült, és a helyiek szerint egy szent sárkány helyén áll. A város híres a gyógyhatású termálforrásairól, amelyeket a buddhista zarándokok évszázadok óta használnak. A környék számos kolostornak ad otthont, mint a Tarachok Gompa. A gazdaság a mezőgazdaságra és a turizmusra épül.",
      ro: "Gasa este un oraș mic, cu o populație de aproximativ 1000 de locuitori. Este situat la altitudinea de 2800 m, în nord-vestul Bhutanului. Clima este montană, cu ierni reci și veri răcoroase. Economia se bazează pe creșterea iacilor, cultivarea cartofilor și turism. Băile termale naturale atrag vizitatori. Orașul este poarta de intrare în Parcul Național Jigme Dorji. Construcțiile sunt tradiționale, din piatră și lemn.",
      en: "Gasa is the smallest dzongkhag capital in Bhutan and a starting point for the famous Snowman Trek. The Gasa Dzong was built in 1640 and is one of the most isolated dzongs. The region is known for its pristine wilderness and snow-capped peaks. Wildlife includes blue sheep, Himalayan musk deer, and the elusive takin. The hot springs at Gasa are believed to have medicinal properties. The local economy relies heavily on tourism, with homestays and camping.",
    },
    factsAdvanced: {
      de: ["Gasa liegt auf 2800 m Höhe.", "Der Gasa-Dzong wurde 1640 erbaut.", "Die Stadt hat 1000 Einwohner.", "Es gibt 3 heiße Quellen mit Temperaturen bis 80°C.", "Der Distrikt ist 1600 km² groß.", "Die Yak-Population beträgt 5000 Tiere."],
      hu: ["A Snowman Trek 24 napos túra.", "A Gasa Dzongot 1640-ben építették.", "A termálforrások hőmérséklete 60-80°C.", "A Jigme Dorji Nemzeti Park 4316 km².", "A város évi csapadékmennyisége 1500 mm.", "A lakosság 80%-a mezőgazdaságból él."],
      ro: ["Altitudinea este de 2800 m.", "Populația orașului este de 1000.", "Parcul Național Jigme Dorji are 4316 km².", "Temperaturile de iarnă scad la -10°C.", "Producția anuală de cartofi este de 200 de tone.", "Cele 3 izvoare termale au temperaturi între 60-80°C."],
      en: ["Gasa Dzong was built in 1640.", "Elevation is 2800 m.", "Jigme Dorji National Park covers 4316 km².", "Hot spring temperature is 60-80°C.", "Population is 1000.", "The Snowman Trek is 24 days long."],
    },
  },
  {
    id: "daga-cities-v2",
    type: "city",
    parent: "BT-22",
    coords: [89.8667, 27.0667],
    name: { de: "Daga", hu: "Daga", ro: "Daga", en: "Daga",
},
    description: {
      de: "Daga, auch bekannt als Dagana, ist eine historische Festungsstadt, die im 17. Jahrhundert zur Verteidigung gegen feindliche Truppen gegründet wurde.",
      hu: "Daga, más néven Dagana, egy történelmi erődváros, amelyet a 17. században alapítottak az ellenséges csapatok elleni védekezés céljából.",
      ro: "Daga, cunoscută și sub numele de Dagana, este un oraș-fortăreață istoric fondat în secolul al XVII-lea pentru apărarea împotriva trupelor inamice.",
      en: "Daga, also known as Dagana, is a historic fortress town founded in the 17th century to defend against hostile forces."
    },
    facts: {
      de: [
        "Die Region zeichnet sich durch steiles Gelände und tiefe Schluchten aus.",
        "Die Landwirtschaft, insbesondere Orangen und Kardamom, ist eine wichtige wirtschaftliche Aktivität."
      ],
      hu: [
        "A régiót meredek terep és mély szurdokok jellemzik.",
        "A mezőgazdaság, különösen a narancs és a kardamom, létfontosságú gazdasági tevékenység."
      ],
      ro: [
        "Regiunea se caracterizează prin terenuri abrupte și chei adânci.",
        "Agricultura, în special portocalele și cardamomul, este o activitate economică vitală."
      ],
      en: [
        "The region is characterized by steep terrains and deep gorges.",
        "Agriculture, particularly oranges and cardamom, is a vital economic activity."
      ]
    },
    image: "/poi-images/daga-cities-v2.webp",
    descriptionAdvanced: {
      de: "Daga ist eine Stadt im Süden Bhutans und Hauptstadt des Distrikts Dagana. Sie liegt auf etwa 1500 Metern Höhe in einer bergigen Region. Der Daga-Dzong wurde im 17. Jahrhundert erbaut und dient als religiöses und administratives Zentrum. Die Wirtschaft basiert auf Landwirtschaft und Handel. Die Region ist bekannt für ihre Orangenplantagen und den Anbau von Kardamom. Daga ist auch für seine traditionelle Musik und Tänze bekannt.",
      hu: "Daga Bhután egyik fontos kereskedelmi csomópontja, amely összeköti a déli és északi régiókat. A Daga Dzong a 17. századi építészet remeke. A város híres a narancsáról, amelyet Indiába exportálnak. A környék lakói főként a lhotsampa etnikai csoporthoz tartoznak. A kultúra sokszínű, a hindu és buddhista hagyományok keverednek. A gazdaság a mezőgazdaságra és a turizmusra támaszkodik.",
      ro: "Daga este un oraș cu o populație de aproximativ 4000 de locuitori. Altitudinea este de 1500 m. Clima este subtropicală. Economia se bazează pe agricultură: orez, portocale și cardamom. Orașul are o piață locală importantă. Daga Dzong-ul este o fortăreață impresionantă. Regiunea este cunoscută pentru pădurile de sal și tec. Recent, s-a dezvoltat apicultura.",
      en: "Daga is the capital of Dagana District, known for its orange production and subtropical climate. The Daga Dzong was built in 1651 and houses the district administration. The town is a hub for trade with India, exporting agricultural products. The population is ethnically diverse, including Lhotsampas and Bhutias. The region is part of the Himalayan subtropical pine forests ecoregion. Local festivals blend Buddhist and Hindu elements.",
    },
    factsAdvanced: {
      de: ["Daga liegt auf 1500 m Höhe.", "Der Daga-Dzong wurde 1651 erbaut.", "Die Stadt hat 4000 Einwohner.", "Die Orangenplantagen erstrecken sich über 500 ha.", "Der Distrikt produziert jährlich 2000 Tonnen Kardamom.", "Die jährliche Niederschlagsmenge beträgt 2000 mm."],
      hu: ["A Daga Dzong 1651-ben épült.", "A narancsexport évi 10 millió dollár.", "A lhotsampa lakosság aránya 40% a környéken.", "A város piaca hetente 2 napon tart nyitva.", "A kardamomtermelés évi 2000 tonna.", "A Daga Tshechu fesztivál 3 napig tart."],
      ro: ["Populația orașului Daga este de 4000.", "Altitudinea este de 1500 m.", "Producția anuală de portocale este de 10.000 de tone.", "Suprafața pădurilor este de 70% din district.", "Temperatura medie anuală este de 20°C.", "Daga Dzong-ul are 2 etaje."],
      en: ["Daga Dzong was built in 1651.", "Elevation is 1500 m.", "Annual orange production is 10,000 tons.", "Population is 4000.", "Cardamom production is 2000 tons per year.", "Rainfall is 2000 mm annually."],
    },
  },
  {
    id: "nganglam-cities-v2",
    type: "city",
    parent: "BT-43",
    coords: [91.2500, 26.8333],
    name: { de: "Nganglam", hu: "Nganglam", ro: "Nganglam", en: "Nganglam",
},
    description: {
      de: "Nganglam ist eine schnell wachsende Handelsstadt an der südlichen Grenze Bhutans, die von der Zementindustrie und neuen Verkehrsverbindungen angetrieben wird.",
      hu: "Nganglam egy gyorsan növekvő kereskedelmi város Bhután déli határánál, amelyet a cementipar és az új közlekedési kapcsolatok hajtanak.",
      ro: "Nganglam este un oraș comercial cu o creștere rapidă la granița de sud a Bhutanului, impulsionat de industria cimentului și de noile conexiuni de transport.",
      en: "Nganglam is a rapidly growing commercial town on the southern border of Bhutan, driven by the cement industry and new transport links."
    },
    facts: {
      de: [
        "Eine große Zementfabrik ist in der Nähe der Stadt in Betrieb.",
        "Das Klima ist das ganze Jahr über heiß und feucht."
      ],
      hu: [
        "A város közelében egy nagy cementgyár működik.",
        "Az éghajlat egész évben meleg és párás."
      ],
      ro: [
        "O fabrică mare de ciment funcționează în apropierea orașului.",
        "Clima este caldă și umedă pe tot parcursul anului."
      ],
      en: [
        "A major cement plant operates near the town.",
        "The climate is hot and humid year-round."
      ]
    },
    image: "/poi-images/nganglam-cities-v2.webp",
    descriptionAdvanced: {
      de: "Nganglam ist eine Stadt im Osten Bhutans nahe der Grenze zu Indien. Sie liegt im Distrikt Pemagatshel. Die Stadt ist ein wichtiger Grenzübergang für den Handel mit Indien. Die Wirtschaft wird vom grenzüberschreitenden Handel und der Landwirtschaft bestimmt. In der Umgebung gibt es heiße Quellen.",
      hu: "Nganglam város Kelet-Bhutánban, India határának közelében. A Pemagatshel körzetben található. A város fontos határátkelőhely India felé. Gazdaságát a határon átnyúló kereskedelem és a mezőgazdaság határozza meg. A környéken termálforrások találhatók.",
      ro: "Nganglam este un oraș în estul Bhutanului, aproape de granița cu India. Se află în districtul Pemagatshel. Orașul este un punct important de trecere a frontierei pentru comerțul cu India. Economia este dominată de comerțul transfrontalier și agricultură. În zonă există izvoare termale.",
      en: "Nganglam is a town in eastern Bhutan near the border with India, in Pemagatshel District. It is a key border crossing for trade with India. The economy is driven by cross-border trade and agriculture. There are hot springs in the vicinity.",
    },
    factsAdvanced: {
      de: ["liegt im Distrikt Pemagatshel", "Grenzübergang zu Indien", "Handelsstadt", "Landwirtschaft wichtig", "heiße Quellen in der Nähe", "strategische Lage an der Grenze"],
      hu: ["a Pemagatshel körzetben található", "határátkelő India felé", "kereskedőváros", "mezőgazdaság fontos", "termálforrások a közelben", "stratégiai elhelyezkedés a határon"],
      ro: ["situat în districtul Pemagatshel", "trecere de frontieră în India", "oraș comercial", "agricultură importantă", "izvoare termale în apropiere", "locație strategică la graniță"],
      en: ["located in Pemagatshel District", "border crossing to India", "trading town", "agriculture important", "hot springs nearby", "strategic border location"],
    },
  },
  {
    id: "sarpang-cities-v2",
    type: "city",
    parent: "BT-34",
    coords: [90.2667, 26.8667],
    name: { de: "Sarpang", hu: "Sarpang", ro: "Sarpang", en: "Sarpang",
},
    description: {
      de: "Sarpang ist das Verwaltungszentrum des Sarpang-Distrikts und liegt am Rande der indischen Ebenen, umgeben von dichtem Dschungel.",
      hu: "Sarpang a Sarpang körzet közigazgatási központja, amely az indiai síkságok szélén fekszik, sűrű dzsungellel körülvéve.",
      ro: "Sarpang este centrul administrativ al districtului Sarpang, situat la marginea câmpiilor indiene, înconjurat de junglă deasă.",
      en: "Sarpang is the administrative center of the Sarpang district, situated at the edge of the Indian plains and surrounded by dense jungle."
    },
    facts: {
      de: [
        "Das Gebiet ist während der Monsunzeit sehr hochwassergefährdet.",
        "Die ursprüngliche Stadt musste aufgrund schwerer Überschwemmungen in der Vergangenheit verlegt werden."
      ],
      hu: [
        "A terület a monszun idején nagyon árvízveszélyes.",
        "Az eredeti várost a múltban súlyos árvizek miatt át kellett költöztetni."
      ],
      ro: [
        "Zona este predispusă la inundații în timpul sezonului musonic.",
        "Orașul original a trebuit să fie mutat din cauza inundațiilor severe din trecut."
      ],
      en: [
        "The area is prone to flooding during the monsoon season.",
        "The original town had to be relocated due to severe floods in the past."
      ]
    },
    image: "/poi-images/sarpang-cities-v2.webp",
    descriptionAdvanced: {
      de: "Sarpang ist eine Stadt im Süden Bhutans und Hauptstadt des Distrikts Sarpang. Sie liegt auf etwa 400 Metern Höhe im subtropischen Tiefland. Die Stadt ist ein wichtiges wirtschaftliches Zentrum mit Industrie und Handel. Der Sarpang-Dzong wurde im 17. Jahrhundert erbaut. Die Region ist bekannt für ihre Reisfelder und Kautschukplantagen. Sarpang hat eine ethnisch vielfältige Bevölkerung mit Nepalesen und Bhutia.",
      hu: "Sarpang Bhután déli kapuja, közvetlenül az indiai határ mellett. A város fontos tranzitpont a nemzetközi kereskedelemben. A Sarpang Dzong a 17. században épült. A gazdaságot a mezőgazdaság és a könnyűipar jellemzi. A régió híres a gumiültetvényekről és a rizstermesztésről. A lakosság nagy része nepalai származású. A kultúra a hindu és buddhista hagyományok keveréke.",
      ro: "Sarpang este un oraș de frontieră cu o populație de aproximativ 10.000 de locuitori. Altitudinea este de 400 m. Clima este tropicală, cu veri fierbinți și ierni blânde. Economia se bazează pe comerț, agricultură (orez, trestie de zahăr) și industria ușoară. Sarpang Dzong-ul este un monument istoric. Orașul are un spital și școli. Recent, s-a dezvoltat sectorul serviciilor.",
      en: "Sarpang is the economic hub of southern Bhutan, with a thriving industrial estate. The town is home to the Sarpang Dzong, built in 1654. The region has a high concentration of rubber and tea plantations. The population is predominantly Nepali-speaking, with a mix of other ethnic groups. Sarpang serves as a transit point for goods from India. The local economy is diversifying into manufacturing and logistics.",
    },
    factsAdvanced: {
      de: ["Sarpang liegt auf 400 m Höhe.", "Der Sarpang-Dzong wurde 1654 erbaut.", "Die Stadt hat 10.000 Einwohner.", "Die Gummibaumplantagen erstrecken sich über 2000 ha.", "Das Industriegebiet umfasst 50 Fabriken.", "Die jährliche Durchschnittstemperatur beträgt 25°C."],
      hu: ["A Sarpang Dzong 1654-ben épült.", "A város lakossága 10.000 fő.", "A gumiültetvények 2000 hektáron terülnek el.", "Az ipari park 50 hektáron fekszik.", "Az évi átlaghőmérséklet 25°C.", "A rizstermelés évi 5000 tonna."],
      ro: ["Populația orașului Sarpang este de 10.000.", "Altitudinea este de 400 m.", "Suprafața plantațiilor de cauciuc este de 2000 ha.", "Temperatura medie anuală este de 25°C.", "Producția anuală de orez este de 5000 de tone.", "Sarpang Dzong-ul datează din 1654."],
      en: ["Sarpang Dzong was built in 1654.", "Population is 10,000.", "Rubber plantations cover 2000 ha.", "Industrial estate has 50 factories.", "Average temperature is 25°C.", "Annual rice production is 5000 tons."],
    },
  },
  {
    id: "tsimasham-cities-v2",
    type: "city",
    parent: "BT-12",
    coords: [89.5333, 27.0833],
    name: { de: "Tsimasham", hu: "Tsimasham", ro: "Tsimasham", en: "Tsimasham",
},
    description: {
      de: "Tsimasham ist das Hauptquartier des Chukha-Distrikts und liegt strategisch günstig an der Hauptautobahn zwischen Thimphu und der indischen Grenze.",
      hu: "Tsimasham a Chukha körzet központja, amely stratégiai fekvésű a Thimphut és az indiai határt összekötő főútvonal mentén.",
      ro: "Tsimasham este sediul districtului Chukha, fiind situat strategic de-a lungul autostrăzii principale dintre Thimphu și granița indiană.",
      en: "Tsimasham is the headquarters of Chukha district, strategically located along the main highway between Thimphu and the Indian border."
    },
    facts: {
      de: [
        "Das nahegelegene Chukha-Wasserkraftwerk ist eine enorme nationale Einnahmequelle.",
        "Die Stadt liegt hoch auf einem Kamm und bietet weite Ausblicke."
      ],
      hu: [
        "A közeli Chukha vízerőmű hatalmas nemzeti bevételi forrás.",
        "A város magasan fekszik egy hegygerincen, és széles kilátást nyújt."
      ],
      ro: [
        "Centrala hidroelectrică Chukha din apropiere este o sursă masivă de venit național.",
        "Orașul se află sus pe o creastă oferind vederi întinse."
      ],
      en: [
        "The nearby Chukha hydropower plant is a massive source of national income.",
        "The town sits high on a ridge offering expansive views."
      ]
    },
    image: "/poi-images/tsimasham-cities-v2.webp",
    descriptionAdvanced: {
      de: "Tsimasham ist ein Ort im Distrikt Chukha in Bhutan, der für seine strategische Lage an der Hauptstraße zwischen Phuentsholing und Thimphu bekannt ist. Der Ort dient als wichtiger Haltepunkt für Reisende und bietet einen Einblick in das ländliche Leben Bhutans. In der Umgebung befinden sich mehrere Klöster und traditionelle Bauernhöfe. Die Wirtschaft basiert hauptsächlich auf Landwirtschaft und Kleinhandel.",
      hu: "Tsimasham Bhután Chukha körzetében található település, amely a Phuentsholing és Thimphu közötti főút mentén fekszik. A település fontos pihenőhely az utazók számára, és betekintést nyújt Bhután vidéki életébe. A környéken több kolostor és hagyományos gazdaság található. A gazdaság főként mezőgazdaságon és kiskereskedelmen alapul.",
      ro: "Tsimasham este o localitate din districtul Chukha, Bhutan, situată pe drumul principal dintre Phuentsholing și Thimphu. Servește ca un important punct de oprire pentru călători și oferă o privire asupra vieții rurale din Bhutan. În împrejurimi se află mai multe mănăstiri și ferme tradiționale. Economia se bazează în principal pe agricultură și comerțul cu amănuntul.",
      en: "Tsimasham is a settlement in Chukha District, Bhutan, located along the main highway connecting Phuentsholing and Thimphu. It serves as a key stopover for travelers and offers insight into rural Bhutanese life. The surrounding area features monasteries and traditional farms. The local economy relies on agriculture and small-scale retail.",
    },
    factsAdvanced: {
      de: ["Höhe: ca. 1.200 m", "Entfernung nach Thimphu: etwa 60 km", "Bevölkerung: ca. 1.500 (Schätzung)", "Jährliche Niederschlagsmenge: über 2.000 mm", "Gründungsjahr der ersten Siedlung: 17. Jahrhundert", "Anzahl der Klöster in der Umgebung: 3"],
      hu: ["Tengerszint feletti magasság: kb. 1200 m", "Távolság Thimphutól: kb. 60 km", "Lakosság: kb. 1500 fő (becslés)", "Éves csapadékmennyiség: több mint 2000 mm", "Az első település alapításának éve: 17. század", "A környéken található kolostorok száma: 3"],
      ro: ["Altitudine: aproximativ 1.200 m", "Distanța până la Thimphu: circa 60 km", "Populație: aproximativ 1.500 (estimare)", "Precipitații anuale: peste 2.000 mm", "Anul înființării primei așezări: secolul al XVII-lea", "Numărul mănăstirilor din zonă: 3"],
      en: ["Elevation: approximately 1,200 m", "Distance to Thimphu: about 60 km", "Population: approx. 1,500 (estimate)", "Annual rainfall: over 2,000 mm", "Year of first settlement: 17th century", "Number of monasteries in the area: 3"],
    },
  },
  {
    id: "deothang-cities-v2",
    type: "city",
    parent: "BT-45",
    coords: [91.4667, 26.8667],
    name: { de: "Deothang", hu: "Deothang", ro: "Deothang", en: "Deothang",
},
    description: {
      de: "Deothang ist eine malerische Stadt im Südosten Bhutans, die für ihre starke militärische Präsenz und Bildungseinrichtungen bekannt ist.",
      hu: "Deothang egy festői város Délkelet-Bhutánban, amely erős katonai jelenlétéről és oktatási intézményeiről ismert.",
      ro: "Deothang este un oraș pitoresc din sud-estul Bhutanului, remarcat pentru o puternică prezență militară și instituții de învățământ.",
      en: "Deothang is a scenic town in southeastern Bhutan noted for a strong military presence and educational institutions."
    },
    facts: {
      de: [
        "Es war der Schauplatz des Duar-Krieges zwischen Bhutan und Britisch-Indien im Jahr 1865.",
        "Das Jigme Namgyel Engineering College befindet sich hier."
      ],
      hu: [
        "Ez volt a bhutáni és a brit-indiai csapatok közötti 1865-ös duar-háború helyszíne.",
        "A Jigme Namgyel Mérnöki Főiskola itt található."
      ],
      ro: [
        "A fost locul războiului Duar dintre Bhutan și India Britanică în 1865.",
        "Colegiul de Inginerie Jigme Namgyel este situat aici."
      ],
      en: [
        "It was the site of the Duar War between Bhutan and British India in 1865.",
        "The Jigme Namgyel Engineering College is located here."
      ]
    },
    image: "/poi-images/deothang-cities-v2.webp",
    descriptionAdvanced: {
      de: "Deothang (auch Dewathang) ist eine Stadt im Osten Bhutans, im Distrikt Samdrup Jongkhar. Die Stadt liegt in einer Tiefebene nahe dem Fluss Dangme. Sie hat eine große Militärbasis und ist ein Zentrum für Handel mit Indien. Die Wirtschaft umfasst Landwirtschaft und den Verkauf von Textilien.",
      hu: "Deothang (vagy Dewathang) város Kelet-Bhutánban, a Szamdrup Dzsongkhar körzetben. A város sík területen fekszik a Dangme folyó közelében. Nagy katonai bázis található itt, India felé irányuló kereskedelem központja. Gazdaságában mezőgazdaság és textíliák értékesítése szerepel.",
      ro: "Deothang (sau Dewathang) este un oraș în estul Bhutanului, în districtul Samdrup Jongkhar. Este situat într-o câmpie lângă râul Dangme. Are o bază militară mare și este un centru comercial cu India. Economia include agricultură și vânzarea de textile.",
      en: "Deothang (also Dewathang) is a town in eastern Bhutan, in Samdrup Jongkhar District. It lies in a lowland plain near the Dangme River. It hosts a major military base and is a trading hub with India. Economy includes agriculture and textile sales.",
    },
    factsAdvanced: {
      de: ["liegt im Distrikt Samdrup Jongkhar", "nahe dem Dangme-Fluss", "große Militärbasis", "Handelszentrum mit Indien", "Landwirtschaft und Textilhandel", "Tiefebene"],
      hu: ["a Szamdrup Dzsongkhar körzetben található", "a Dangme folyó közelében", "nagy katonai bázis", "kereskedelmi központ Indiával", "mezőgazdaság és textilkereskedelem", "síkság"],
      ro: ["situat în districtul Samdrup Jongkhar", "lângă râul Dangme", "bază militară mare", "centru comercial cu India", "agricultură și comerț textil", "câmpie"],
      en: ["located in Samdrup Jongkhar District", "near Dangme River", "large military base", "trading hub with India", "agriculture and textile trade", "lowland plain"],
    },
  },
  {
    id: "kanglung-cities-v2",
    type: "city",
    parent: "BT-41",
    coords: [91.5167, 27.2667],
    name: { de: "Kanglung", hu: "Kanglung", ro: "Kanglung", en: "Kanglung",
},
    description: {
      de: "Kanglung ist eine bedeutende Bildungsstadt im Trashigang-Distrikt und beherbergt das renommierte Sherubtse College, das erste seiner Art in Bhutan.",
      hu: "Kanglung egy jelentős oktatási város a Trashigang körzetben, amely a híres Sherubtse College-nak, Bhután első főiskolájának ad otthont.",
      ro: "Kanglung este un oraș educațional important din districtul Trashigang, găzduind renumitul Colegiu Sherubtse, primul de acest fel din Bhutan.",
      en: "Kanglung is a significant educational town in Trashigang district, home to the renowned Sherubtse College, the first of its kind in Bhutan."
    },
    facts: {
      de: [
        "Die Stadt hat eine lebendige studentische Gemeinschaft und akademische Atmosphäre.",
        "Sie liegt an der primären östlichen Autobahn von Bhutan."
      ],
      hu: [
        "A város nyüzsgő diák közösséggel és akadémiai légkörrel rendelkezik.",
        "Bhután elsődleges keleti autópályája mentén fekszik."
      ],
      ro: [
        "Orașul are o comunitate studențească vibrantă și o atmosferă academică.",
        "Este situat de-a lungul autostrăzii primare estice a Bhutanului."
      ],
      en: [
        "The town has a vibrant student community and academic atmosphere.",
        "It is located along the primary eastern highway of Bhutan."
      ]
    },
    image: "/poi-images/kanglung-cities-v2.webp",
    descriptionAdvanced: {
      de: "Kanglung ist ein Ort im Distrikt Trashigang in Ostbhutan, bekannt als Standort des Sherubtse College, der ersten Hochschule des Landes. Der Ort spielt eine zentrale Rolle in der Bildung und bildet viele Jungendliche aus. Die Umgebung ist geprägt von Reisterrassen und dichten Wäldern. Die Wirtschaft ist stark von der Hochschule und der Landwirtschaft abhängig.",
      hu: "Kanglung Bhután Trashigang körzetében található, és a Sherubtse College-nak ad otthont, amely az ország első főiskolája. A település központi szerepet játszik az oktatásban, számos fiatalt képezve. A környéket rizsteraszok és sűrű erdők jellemzik. A gazdaság nagymértékben függ a főiskolától és a mezőgazdaságtól.",
      ro: "Kanglung este o localitate din districtul Trashigang, estul Bhutanului, cunoscută ca sediul Sherubtse College, prima instituție de învățământ superior din țară. Localitatea joacă un rol central în educație, formând mulți tineri. Împrejurimile sunt caracterizate de terase de orez și păduri dese. Economia depinde în mare măsură de colegiu și de agricultură.",
      en: "Kanglung is a settlement in Trashigang District, eastern Bhutan, best known as the site of Sherubtse College, the country's first tertiary institution. It plays a central role in education, training many young Bhutanese. The area features rice terraces and dense forests. The economy relies heavily on the college and agriculture.",
    },
    factsAdvanced: {
      de: ["Bevölkerung: ca. 2.000 (einschließlich Studenten)", "Höhe: ca. 1.800 m", "Gründung des Sherubtse College: 1966", "Anzahl der Studenten: über 1.000", "Entfernung nach Trashigang: etwa 20 km", "Jährliche Durchschnittstemperatur: 15°C"],
      hu: ["Lakosság: kb. 2000 fő (diákokkal együtt)", "Tengerszint feletti magasság: kb. 1800 m", "A Sherubtse College alapításának éve: 1966", "Diákok száma: több mint 1000", "Távolság Trashigangtól: kb. 20 km", "Éves átlaghőmérséklet: 15°C"],
      ro: ["Populație: aproximativ 2.000 (inclusiv studenți)", "Altitudine: aproximativ 1.800 m", "Anul înființării Sherubtse College: 1966", "Numărul de studenți: peste 1.000", "Distanța până la Trashigang: circa 20 km", "Temperatura medie anuală: 15°C"],
      en: ["Population: approx. 2,000 (including students)", "Elevation: approximately 1,800 m", "Year Sherubtse College was founded: 1966", "Number of students: over 1,000", "Distance to Trashigang: about 20 km", "Average annual temperature: 15°C"],
    },
  },
  {
    id: "gyalpozhing-cities-v2",
    type: "city",
    parent: "BT-42",
    coords: [91.1833, 27.2667],
    name: { de: "Gyalpozhing", hu: "Gyalpozhing", ro: "Gyalpozhing", en: "Gyalpozhing",
},
    description: {
      de: "Gyalpozhing ist eine warme Stadt im Osten Bhutans, die sich aufgrund des Baus des riesigen Kurichhu-Wasserkraftwerks schnell entwickelt hat.",
      hu: "Gyalpozhing egy meleg éghajlatú város Kelet-Bhutánban, amely a hatalmas Kurichhu vízerőmű építésének köszönhetően indult gyors fejlődésnek.",
      ro: "Gyalpozhing este un oraș cald din estul Bhutanului care s-a dezvoltat rapid datorită construcției uriașei centrale hidroelectrice Kurichhu.",
      en: "Gyalpozhing is a warm town in eastern Bhutan that developed rapidly due to the construction of the massive Kurichhu hydroelectric plant."
    },
    facts: {
      de: [
        "Es dient als moderner regionaler Knotenpunkt im Mongar-Distrikt.",
        "Die Stadt beherbergt ein wichtiges College für Informationstechnologie."
      ],
      hu: [
        "A Mongar körzet modern regionális központjaként szolgál.",
        "A város ad otthont egy fontos informatikai főiskolának."
      ],
      ro: [
        "Servește ca un hub regional modern în districtul Mongar.",
        "Orașul găzduiește un important colegiu de Tehnologia Informației."
      ],
      en: [
        "It serves as a modern regional hub in the Mongar district.",
        "The town houses an important college of Information Technology."
      ]
    },
    image: "/poi-images/gyalpozhing-cities-v2.webp",
    descriptionAdvanced: {
      de: "Gyalpozhing ist eine Planstadt in Ostbhutan, im Distrikt Mongar. Sie wurde 2014 offiziell eingeweiht und ist eine der ersten modernen Städte Bhutans. Die Stadt ist als Handels- und Dienstleistungszentrum konzipiert. Sie liegt in einem Tal auf etwa 400 m Höhe. Die Bevölkerung besteht aus verschiedenen ethnischen Gruppen, darunter Sharchops.",
      hu: "Gyalpozhing egy tervezett város Kelet-Bhutánban, a Mongar körzetben. 2014-ben avatták fel hivatalosan, Bhutan egyik első modern városa. Kereskedelmi és szolgáltatási központnak tervezték. Egy völgyben fekszik, körülbelül 400 m magasságban. Lakossága különböző etnikai csoportokból áll, köztük sarkopokból.",
      ro: "Gyalpozhing este un oraș planificat în estul Bhutanului, în districtul Mongar. A fost inaugurat oficial în 2014, fiind unul dintre primele orașe moderne din Bhutan. Este conceput ca centru comercial și de servicii. Este situat într-o vale la aproximativ 400 m altitudine. Populația include diverse grupuri etnice, printre care sharchops.",
      en: "Gyalpozhing is a planned town in eastern Bhutan, in Mongar District. Officially inaugurated in 2014, it is one of Bhutan's first modern towns. It is designed as a trade and service hub. Located in a valley at about 400 m elevation, its population comprises various ethnic groups, including Sharchops.",
    },
    factsAdvanced: {
      de: ["offiziell eingeweiht 2014", "Planstadt im Distrikt Mongar", "Höhe ca. 400 m", "als Handels- und Dienstleistungszentrum konzipiert", "eine der ersten modernen Städte Bhutans", "ethnische Vielfalt (Sharchops)"],
      hu: ["2014-ben avatták fel", "tervezett város a Mongar körzetben", "magassága kb. 400 m", "kereskedelmi és szolgáltatási központnak tervezték", "Bhutan egyik első modern városa", "etnikai sokszínűség (sarkopok)"],
      ro: ["inaugurat oficial în 2014", "oraș planificat în districtul Mongar", "altitudine cca 400 m", "conceput ca centru comercial și de servicii", "unul dintre primele orașe moderne din Bhutan", "diversitate etnică (sharchops)"],
      en: ["officially inaugurated in 2014", "planned town in Mongar District", "elevation about 400 m", "designed as trade and service hub", "one of Bhutan's first modern towns", "ethnic diversity (Sharchops)"],
    },
  },
  {
    id: "wamrong-cities-v2",
    type: "city",
    parent: "BT-41",
    coords: [91.5667, 27.1333],
    name: { de: "Wamrong", hu: "Wamrong", ro: "Wamrong", en: "Wamrong",
},
    description: {
      de: "Wamrong ist eine malerische Siedlung auf einem Bergrücken, die eine wichtige Raststätte auf der langen Reise durch Ostbhutan bietet.",
      hu: "Wamrong egy festői hegygerincen fekvő település, amely fontos pihenőhelyként szolgál a Kelet-Bhutánon átívelő hosszú utazás során.",
      ro: "Wamrong este o așezare pitorească pe o creastă montană care servește drept oprire importantă pe drumul lung prin estul Bhutanului.",
      en: "Wamrong is a picturesque ridge-top settlement serving as an important rest stop on the long journey through eastern Bhutan."
    },
    facts: {
      de: [
        "Die lokale Wirtschaft ist stark vom Autobahnverkehr abhängig.",
        "Es verfügt über dichte Wälder, die typisch für den unteren Himalaya sind."
      ],
      hu: [
        "A helyi gazdaság nagymértékben függ az autópálya-forgalomtól.",
        "Az alsó-Himalájára jellemző sűrű erdőkkel rendelkezik."
      ],
      ro: [
        "Economia locală se bazează puternic pe traficul pe autostradă.",
        "Prezintă păduri dense tipice pentru Himalaya inferioară."
      ],
      en: [
        "The local economy relies heavily on highway traffic.",
        "It features dense forests typical of the lower Himalayas."
      ]
    },
    image: "/poi-images/wamrong-cities-v2.webp",
    descriptionAdvanced: {
      de: "Wamrong ist ein kleines Dorf im Distrikt Trashigang, bekannt für den Wamrong-Tempel und seine traditionelle Handwerkskunst. Der Ort liegt abseits der Hauptstraße und bietet einen ruhigen Rückzugsort. Die lokale Wirtschaft basiert auf Landwirtschaft, insbesondere dem Anbau von Kartoffeln und Mais. Wamrong ist auch ein Ausgangspunkt für Wanderungen in die umliegenden Hügel.",
      hu: "Wamrong egy kis falu Bhután Trashigang körzetében, ismert a Wamrong-templomról és hagyományos kézművességéről. A település a főúttól távol fekszik, csendes pihenőhelyet kínálva. A helyi gazdaság a mezőgazdaságon alapul, különösen a burgonya és a kukorica termesztésén. Wamrong egyben kiindulópont a környező dombokba tett túrákhoz.",
      ro: "Wamrong este un mic sat din districtul Trashigang, Bhutan, cunoscut pentru templul Wamrong și meșteșugurile tradiționale. Satul este situat departe de drumul principal, oferind un refugiu liniștit. Economia locală se bazează pe agricultură, în special pe cultivarea cartofilor și a porumbului. Wamrong este, de asemenea, un punct de plecare pentru drumeții în dealurile din jur.",
      en: "Wamrong is a small village in Trashigang District, Bhutan, known for the Wamrong Temple and traditional handicrafts. The village is located off the main road, offering a quiet retreat. The local economy relies on agriculture, especially potato and maize cultivation. Wamrong also serves as a starting point for hikes into the surrounding hills.",
    },
    factsAdvanced: {
      de: ["Bevölkerung: ca. 500", "Höhe: ca. 2.000 m", "Entfernung nach Trashigang: etwa 30 km", "Jährliche Niederschlagsmenge: ca. 1.500 mm", "Anzahl der Haushalte: etwa 100", "Gründungsjahr des Tempels: 18. Jahrhundert"],
      hu: ["Lakosság: kb. 500 fő", "Tengerszint feletti magasság: kb. 2000 m", "Távolság Trashigangtól: kb. 30 km", "Éves csapadékmennyiség: kb. 1500 mm", "Háztartások száma: kb. 100", "A templom alapításának éve: 18. század"],
      ro: ["Populație: aproximativ 500", "Altitudine: aproximativ 2.000 m", "Distanța până la Trashigang: circa 30 km", "Precipitații anuale: circa 1.500 mm", "Numărul de gospodării: aproximativ 100", "Anul construirii templului: secolul al XVIII-lea"],
      en: ["Population: approx. 500", "Elevation: approximately 2,000 m", "Distance to Trashigang: about 30 km", "Annual rainfall: about 1,500 mm", "Number of households: around 100", "Year the temple was built: 18th century"],
    },
  },
  {
    id: "rangjung-cities-v2",
    type: "city",
    parent: "BT-41",
    coords: [91.6667, 27.3500],
    name: { de: "Rangjung", hu: "Rangjung", ro: "Rangjung", en: "Rangjung",
},
    description: {
      de: "Rangjung ist eine lebendige Stadt im Radhi-Tal, die für ihr spektakuläres Kloster und als Zentrum der Webarbeit bekannt ist.",
      hu: "Rangjung egy élénk város a Radhi-völgyben, amely látványos kolostoráról és a szövőmesterség központjaként ismert.",
      ro: "Rangjung este un oraș plin de viață din valea Radhi, renumit pentru mănăstirea sa spectaculoasă și ca un centru al meșteșugului de țesut.",
      en: "Rangjung is a lively town in the Radhi Valley, known for its spectacular monastery and as a center of weaving craftsmanship."
    },
    facts: {
      de: [
        "Das lokale Kloster ist im tibetischen Architekturstil erbaut.",
        "Es fungiert als Tor zu den nomadischen Dörfern Merak und Sakteng."
      ],
      hu: [
        "A helyi kolostor tibeti építészeti stílusban épült.",
        "Kapuként szolgál Merak és Sakteng nomád falvaihoz."
      ],
      ro: [
        "Mănăstirea locală este construită în stil arhitectural tibetan.",
        "Acționează ca poartă către satele nomade Merak și Sakteng."
      ],
      en: [
        "The local monastery is built in the Tibetan architectural style.",
        "It acts as the gateway to the nomadic villages of Merak and Sakteng."
      ]
    },
    image: "/poi-images/rangjung-cities-v2.webp",
    descriptionAdvanced: {
      de: "Rangjung ist ein Ort im Distrikt Trashigang, der für sein Kloster, das Rangjung Chokhorling, bekannt ist. Der Ort ist ein wichtiges religiöses Zentrum in Ostbhutan. Die Umgebung bietet atemberaubende Ausblicke auf die Berge und Täler. Die Wirtschaft wird von Landwirtschaft und Klosteraktivitäten geprägt.",
      hu: "Rangjung Bhután Trashigang körzetében található, és a Rangjung Chokhorling kolostoráról ismert. A település fontos vallási központ Kelet-Bhutánban. A környező hegyek és völgyek lenyűgöző kilátást nyújtanak. A gazdaságot a mezőgazdaság és a kolostori tevékenységek határozzák meg.",
      ro: "Rangjung este o localitate din districtul Trashigang, Bhutan, cunoscută pentru mănăstirea sa, Rangjung Chokhorling. Localitatea este un important centru religios în estul Bhutanului. Împrejurimile oferă priveliști impresionante asupra munților și văilor. Economia este modelată de agricultură și activitățile monahale.",
      en: "Rangjung is a settlement in Trashigang District, Bhutan, known for its monastery, Rangjung Chokhorling. It is an important religious center in eastern Bhutan. The surrounding area offers stunning views of mountains and valleys. The economy is shaped by agriculture and monastic activities.",
    },
    factsAdvanced: {
      de: ["Bevölkerung: ca. 1.200", "Höhe: ca. 2.100 m", "Gründungsjahr des Klosters: 1985", "Anzahl der Mönche: etwa 150", "Entfernung nach Trashigang: etwa 15 km", "Fläche des Klostergeländes: 5 Hektar"],
      hu: ["Lakosság: kb. 1200 fő", "Tengerszint feletti magasság: kb. 2100 m", "A kolostor alapításának éve: 1985", "Szerzetesek száma: kb. 150", "Távolság Trashigangtól: kb. 15 km", "A kolostor területe: 5 hektár"],
      ro: ["Populație: aproximativ 1.200", "Altitudine: aproximativ 2.100 m", "Anul înființării mănăstirii: 1985", "Numărul de călugări: aproximativ 150", "Distanța până la Trashigang: circa 15 km", "Suprafața mănăstirii: 5 hectare"],
      en: ["Population: approx. 1,200", "Elevation: approximately 2,100 m", "Year monastery was established: 1985", "Number of monks: about 150", "Distance to Trashigang: about 15 km", "Area of monastery grounds: 5 hectares"],
    },
  },
  {
    id: "khuruthang-cities-v2",
    type: "city",
    parent: "BT-23",
    coords: [89.8833, 27.5333],
    name: { de: "Khuruthang", hu: "Khuruthang", ro: "Khuruthang", en: "Khuruthang",
},
    description: {
      de: "Khuruthang ist eine planmäßig angelegte Neustadt, die als Handelszentrum für den Punakha-Distrikt dient, etwas südlich des historischen Dzongs.",
      hu: "Khuruthang egy tervezett újváros, amely a Punakha körzet kereskedelmi központjaként szolgál, kissé délre a történelmi dzongtól.",
      ro: "Khuruthang este un oraș nou planificat care servește drept centru comercial pentru districtul Punakha, chiar la sud de dzongul istoric.",
      en: "Khuruthang is a planned new town serving as the commercial hub for the Punakha district, just south of the historic dzong."
    },
    facts: {
      de: [
        "Die Stadt bietet moderne Annehmlichkeiten für die lokale Bevölkerung.",
        "Sie liegt im fruchtbaren Tal des Puna Tsang Chhu-Flusses."
      ],
      hu: [
        "A város modern kényelmi szolgáltatásokat kínál a helyi lakosság számára.",
        "A Puna Tsang Chhu folyó termékeny völgyében fekszik."
      ],
      ro: [
        "Orașul oferă facilități moderne pentru populația locală.",
        "Se află în valea fertilă a râului Puna Tsang Chhu."
      ],
      en: [
        "The town provides modern amenities for the local population.",
        "It lies in the fertile valley of the Puna Tsang Chhu river."
      ]
    },
    image: "/poi-images/khuruthang-cities-v2.webp",
    descriptionAdvanced: {
      de: "Khuruthang ist eine Siedlung im Distrikt Punakha, die sich entlang des Flusses Mo Chhu erstreckt. Der Ort dient als Tor zum berühmten Punakha Dzong und ist bekannt für seine Reisfelder und traditionellen Bauernhäuser. Die Landwirtschaft, insbesondere der Reisanbau, ist der wichtigste Wirtschaftszweig. Khuruthang ist auch ein beliebter Ort für Vogelbeobachtungen.",
      hu: "Khuruthang Bhután Punakha körzetében található település, amely a Mo Chhu folyó mentén húzódik. A település kapuként szolgál a híres Punakha Dzonghoz, és rizsföldjeiről és hagyományos parasztházairól ismert. A mezőgazdaság, különösen a rizstermesztés, a legfontosabb gazdasági ág. Khuruthang népszerű madármegfigyelő hely is.",
      ro: "Khuruthang este o așezare în districtul Punakha, Bhutan, situată de-a lungul râului Mo Chhu. Localitatea servește ca poartă de intrare către faimosul Punakha Dzong și este cunoscută pentru orezăriile și casele tradiționale țărănești. Agricultura, în special cultivarea orezului, este principala ramură economică. Khuruthang este, de asemenea, un loc popular pentru observarea păsărilor.",
      en: "Khuruthang is a settlement in Punakha District, Bhutan, stretching along the Mo Chhu River. It serves as a gateway to the famous Punakha Dzong and is known for its rice paddies and traditional farmhouses. Agriculture, especially rice cultivation, is the main economic activity. Khuruthang is also a popular birdwatching spot.",
    },
    factsAdvanced: {
      de: ["Bevölkerung: ca. 800", "Höhe: ca. 1.300 m", "Entfernung zum Punakha Dzong: etwa 3 km", "Jährliche Ernte von Reis: ca. 500 Tonnen", "Anzahl der Vogelarten in der Gegend: über 100", "Gründungsjahr der Siedlung: 19. Jahrhundert"],
      hu: ["Lakosság: kb. 800 fő", "Tengerszint feletti magasság: kb. 1300 m", "Távolság a Punakha Dzongtól: kb. 3 km", "Éves rizstermés: kb. 500 tonna", "A környéken található madárfajok száma: több mint 100", "A település alapításának éve: 19. század"],
      ro: ["Populație: aproximativ 800", "Altitudine: aproximativ 1.300 m", "Distanța până la Punakha Dzong: circa 3 km", "Recolta anuală de orez: circa 500 de tone", "Numărul de specii de păsări din zonă: peste 100", "Anul înființării așezării: secolul al XIX-lea"],
      en: ["Population: approx. 800", "Elevation: approximately 1,300 m", "Distance to Punakha Dzong: about 3 km", "Annual rice harvest: about 500 tons", "Number of bird species in the area: over 100", "Year the settlement was founded: 19th century"],
    },
  },
  {
    id: "trashi-yangtse-cities-v2",
    type: "city",
    parent: "BT-46",
    coords: [91.5000, 27.6000],
    name: { de: "Trashi Yangtse", hu: "Trashi Yangtse", ro: "Trashi Yangtse", en: "Trashi Yangtse",
},
    description: {
      de: "Trashi Yangtse ist eine kleine, traditionelle Stadt im Nordosten Bhutans, die für ihre Holzarbeiten und Papiermacherei berühmt ist.",
      hu: "Trashi Yangtse egy kis, hagyományos város Északkelet-Bhutánban, amely famegmunkálásáról és papírkészítéséről híres.",
      ro: "Trashi Yangtse este un mic oraș tradițional din nord-estul Bhutanului, faimos pentru prelucrarea lemnului și fabricarea hârtiei.",
      en: "Trashi Yangtse is a small traditional town in northeastern Bhutan, famous for its woodwork and paper making."
    },
    facts: {
      de: [
        "Der majestätische Chorten Kora ist hier ein bedeutender Pilgerort.",
        "Es wurde im Jahr 1992 ein eigenständiger Distrikt."
      ],
      hu: [
        "A fenséges Chorten Kora egy jelentős zarándokhely itt.",
        "1992-ben vált külön körzetté."
      ],
      ro: [
        "Maiestuosul Chorten Kora este un loc de pelerinaj major aici.",
        "A devenit un district separat în 1992."
      ],
      en: [
        "The majestic Chorten Kora is a major pilgrimage site here.",
        "It became a separate district in 1992."
      ]
    },
    image: "/poi-images/trashi-yangtse-cities-v2.webp",
    descriptionAdvanced: {
      de: "Trashi Yangtse ist eine Stadt im gleichnamigen Distrikt im Osten Bhutans. Sie ist bekannt für die Trashi Yangtse Dzong, die administrative und religiöse Bedeutung hat. Die Stadt liegt in einem malerischen Tal und ist ein Zentrum für den Anbau von Kardamom und anderen Gewürzen. Der Tourismus wächst langsam, da die Region abgelegen ist.",
      hu: "Trashi Yangtse város Kelet-Bhutánban, az azonos nevű körzet székhelye. Ismert a Trashi Yangtse Dzongról, amely közigazgatási és vallási jelentőséggel bír. A város festői völgyben fekszik, és a kardamom és más fűszerek termesztésének központja. A turizmus lassan növekszik, mivel a régió elszigetelt.",
      ro: "Trashi Yangtse este un oraș în estul Bhutanului, reședința districtului omonim. Este cunoscut pentru Trashi Yangtse Dzong, care are semnificație administrativă și religioasă. Orașul este situat într-o vale pitorească și este un centru pentru cultivarea cardamomului și a altor condimente. Turismul crește încet, deoarece regiunea este izolată.",
      en: "Trashi Yangtse is a town in eastern Bhutan, capital of the eponymous district. It is known for the Trashi Yangtse Dzong, which holds administrative and religious significance. The town is set in a scenic valley and is a center for cardamom and other spice cultivation. Tourism is slowly growing as the region is remote.",
    },
    factsAdvanced: {
      de: ["Bevölkerung: ca. 3.000", "Höhe: ca. 1.800 m", "Gründung des Dzong: 15. Jahrhundert", "Jährliche Kardamomproduktion: ca. 50 Tonnen", "Entfernung nach Trashigang: etwa 80 km", "Anzahl der Schulen: 5"],
      hu: ["Lakosság: kb. 3000 fő", "Tengerszint feletti magasság: kb. 1800 m", "A Dzong alapításának éve: 15. század", "Éves kardamomtermelés: kb. 50 tonna", "Távolság Trashigangtól: kb. 80 km", "Iskolák száma: 5"],
      ro: ["Populație: aproximativ 3.000", "Altitudine: aproximativ 1.800 m", "Anul construirii Dzong: secolul al XV-lea", "Producția anuală de cardamom: circa 50 de tone", "Distanța până la Trashigang: circa 80 km", "Numărul de școli: 5"],
      en: ["Population: approx. 3,000", "Elevation: approximately 1,800 m", "Year the Dzong was built: 15th century", "Annual cardamom production: about 50 tons", "Distance to Trashigang: about 80 km", "Number of schools: 5"],
    },
  },
  {
    id: "lhamoizingkha-cities-v2",
    type: "city",
    parent: "BT-22",
    coords: [89.8167, 26.7167],
    name: { de: "Lhamoizingkha", hu: "Lhamoizingkha", ro: "Lhamoizingkha", en: "Lhamoizingkha",
},
    description: {
      de: "Lhamoizingkha ist eine subtropische Grenzstadt in Bhutan, die am Ufer des Sunkosh-Flusses in einem Gebiet mit üppiger Vegetation liegt.",
      hu: "Lhamoizingkha egy szubtrópusi határváros Bhutánban, amely a Sunkosh folyó partján fekszik egy dús növényzetű területen.",
      ro: "Lhamoizingkha este un oraș de graniță subtropical din Bhutan, situat pe malurile râului Sunkosh într-o zonă cu vegetație luxuriantă.",
      en: "Lhamoizingkha is a subtropical border town in Bhutan, situated on the banks of the Sunkosh River in an area with lush vegetation."
    },
    facts: {
      de: [
        "Es war früher als Kalikhola bekannt.",
        "Das lokale Klima ist die meiste Zeit des Jahres sehr heiß und feucht."
      ],
      hu: [
        "Korábban Kalikhola néven volt ismert.",
        "A helyi éghajlat az év nagy részében nagyon meleg és párás."
      ],
      ro: [
        "A fost cunoscut anterior sub numele de Kalikhola.",
        "Clima locală este foarte caldă și umedă în cea mai mare parte a anului."
      ],
      en: [
        "It was formerly known as Kalikhola.",
        "The local climate is very hot and humid for most of the year."
      ]
    },
    image: "/poi-images/lhamoizingkha-cities-v2.webp",
    descriptionAdvanced: {
      de: "Lhamoizingkha ist eine Kleinstadt im Dagana-Distrikt von Bhutan. Sie wurde in den späten 1960er Jahren als Verwaltungszentrum gegründet. Die Stadt liegt auf einer Höhe von etwa 600 Metern und dient als wichtiger Marktort für die umliegenden Dörfer. Die Bevölkerungszahl beträgt rund 2.500 Einwohner. Historisch war die Region von der Landwirtschaft geprägt, insbesondere dem Anbau von Reis, Mais und Kartoffeln. In den letzten Jahrzehnten hat sich Lhamoizingkha zu einem kleinen Handelszentrum entwickelt, das die abgelegenen Gemeinden des Distrikts versorgt. Die Stadt verfügt über eine Grundschule, ein kleines Krankenhaus und einen Busbahnhof, der sie mit der Hauptstadt Thimphu verbindet. Die Entwicklung wurde durch den Bau der Straße von Thimphu nach Dagana in den 1970er Jahren begünstigt. Heute ist Lhamoizingkha ein ruhiger Ort, der die traditionelle bhutanische Lebensweise bewahrt hat.",
      hu: "Lhamoizingkha városa Bhutan délnyugati részén, Dagana körzetben található. A település építészeti stílusa a hagyományos bhutáni építészetet tükrözi, ahol a fa- és vályogszerkezetek dominálnak. A városka utcái szűkek, a házak általában két-három szintesek, jellegzetes bhutáni tetőkkel. A központban található a helyi piac, ahol a gazdák termékeiket árusítják. A város fejlődését a 20. század végén felgyorsult infrastrukturális beruházások segítették, mint az utak javítása és az elektromos hálózat kiépítése. A helyi templom, a Lhamoizingkha Lhakhang, a 20. század közepén épült, és a buddhista vallás fontos gyülekezőhelye. A városközpontban található egy kis park, ahol a fák és virágágyások a környék zöldellő táját idézik. A település rendezettsége és nyugalma jellemző a vidéki Bhutánra.",
      ro: "Lhamoizinghka este un oraș mic situat în districtul Dagana din Bhutan, la o altitudine de aproximativ 600 de metri. Clima este subtropicală, cu veri calde și ierni blânde. Economia locală se bazează pe agricultură, în special culturi de orez, porumb și cartofi. De asemenea, creșterea animalelor, cum ar fi bovinele și caprele, este o sursă importantă de venit. Recent, turismul a început să se dezvolte, vizitatorii fiind atrași de peisajele pitorești și de autenticitatea vieții rurale. Orașul este deservit de un drum principal care leagă Thimphu de Dagana, facilitând transportul de mărfuri și persoane. În ciuda dimensiunilor sale mici, Lhamoizinghka are o școală primară, un centru de sănătate și o piață săptămânală. Populația este de aproximativ 2.500 de locuitori, majoritatea de etnie Ngalop. Resursele de apă sunt abundente datorită râurilor din apropiere.",
      en: "Lhamoizingkha is a small town in Dagana District, Bhutan, with a population of around 2,500. It serves as an administrative and commercial hub for the surrounding rural communities. The town's cultural life is centered on Buddhism, with the Lhamoizingkha Lhakhang temple being a focal point for religious festivals. Annual tshechus (religious dances) attract locals and visitors. The town's demographic is predominantly Ngalop, and the primary language is Dzongkha. Traditional attire, such as the gho for men and kira for women, is commonly worn. The local market offers fresh produce, textiles, and handicrafts. Despite modernization, the town retains a strong sense of community and tradition, with many residents engaged in farming. The government has invested in education and healthcare, resulting in improved literacy rates and life expectancy.",
    },
    factsAdvanced: {
      de: ["Bevölkerung: ca. 2.500 (Schätzung 2022)", "Höhe: 600 m über dem Meeresspiegel", "Gründung: 1968", "Fläche: 5 km²", "Anzahl der Haushalte: 400", "Entfernung nach Thimphu: 65 km"],
      hu: ["Népesség: kb. 2.500 (2022-es becslés)", "Tengerszint feletti magasság: 600 m", "Alapítás éve: 1968", "Terület: 5 km²", "Háztartások száma: 400", "Távolság Thimphutól: 65 km"],
      ro: ["Populație: aproximativ 2.500 (estimare 2022)", "Altitudine: 600 m deasupra nivelului mării", "Anul înființării: 1968", "Suprafață: 5 km²", "Număr de gospodării: 400", "Distanța până la Thimphu: 65 km"],
      en: ["Population: approx. 2,500 (2022 estimate)", "Elevation: 600 m above sea level", "Founded: 1968", "Area: 5 km²", "Number of households: 400", "Distance to Thimphu: 65 km"],
    },
  }
,
  {
    id: "bhutan-gedu-cities-v2",
    type: "town",
    name: { de: "Gedu", hu: "Gedu", ro: "Gedu", en: "Gedu" },
    coords: [89.525, 26.921],
    description: {
      de: "Gedu ist ein Bildungszentrum im Distrikt Chhukha, bekannt für seine Wirtschaftshochschule. Es ist ein wichtiger Halt an der Straße nach Indien.",
      hu: "Gedu egy oktatási központ Chhukha körzetben, amely híres gazdasági főiskolájáról. Fontos pihenőhely az indiai határ felé vezető főúton.",
      ro: "Gedu este un centru educațional în districtul Chhukha, cunoscut pentru colegiul său de afaceri. Este un punct de oprire pe drumul spre India.",
      en: "Gedu is an educational hub in Chhukha District, known for its business college. It is a key stop on the highway to the southern Indian border.",
    },
    facts: {
      de: ["Sitz des Gedu College of Business Studies", "Liegt auf etwa 2000 Metern Höhe", "Wichtiger Verkehrsknotenpunkt in Chhukha", "Kühles Bergklima das ganze Jahr über"],
      hu: ["A Gedu Üzleti Főiskola székhelye", "Körülbelül 2000 méter magasságban fekszik", "Fontos közlekedési csomópont Chhukhában", "Egész évben hűvös hegyi éghajlat jellemzi"],
      ro: ["Sediul Colegiului de Afaceri Gedu", "Situat la o altitudine de aproximativ 2000 m", "Nod rutier important în districtul Chhukha", "Climat montan răcoros pe tot parcursul anului"],
      en: ["Home to the Gedu College of Business Studies", "Located at an altitude of about 2000 meters", "Major transit point in Chhukha District", "Features a cool mountain climate year-round"],
    },
    imageHint: "Gedu town and business college buildings",
    descriptionAdvanced: {
      de: "Gedu ist eine Stadt im Südwesten Bhutans, im Distrikt Chukha, nahe der Grenze zu Indien. Sie liegt auf etwa 300 Metern Höhe und hat ein subtropisches Klima. Gedu ist ein wichtiger Handels- und Industriestandort, vor allem durch das nahegelegene Wasserkraftwerk Tala. Die Stadt wächst schnell, mit Einwohnern aus verschiedenen Teilen Bhutans. Die Wirtschaft basiert auf Landwirtschaft, Holzwirtschaft und Energieproduktion. Gedu hat mehrere Schulen und Gesundheitszentren. Die Straße nach Phuentsholing ist eine wichtige Verbindung.",
      hu: "Gedu város Bhután délnyugati részén, Chukha körzetben, India határához közel. Tengerszint feletti magassága kb. 300 méter, szubtrópusi éghajlatú. Gedu fontos kereskedelmi és ipari központ, elsősorban a közeli Tala vízerőmű miatt. A város gyorsan növekszik, lakosai az ország különböző részeiből érkeztek. Gazdasága mezőgazdaságon, fafeldolgozáson és energiatermelésen alapul. Több iskola és egészségügyi központ található itt. A Phuentsholing felé vezető út fontos összeköttetés.",
      ro: "Gedu este un oraș în sud-vestul Bhutanului, în districtul Chukha, aproape de granița cu India. Se află la o altitudine de aproximativ 300 de metri, cu climă subtropicală. Gedu este un important centru comercial și industrial, datorită centralei hidroelectrice Tala din apropiere. Orașul este în creștere rapidă, cu locuitori din toate colțurile Bhutanului. Economia se bazează pe agricultură, exploatare forestieră și producție de energie. Există mai multe școli și centre de sănătate. Drumul către Phuentsholing este o legătură vitală.",
      en: "Gedu is a rapidly growing town in southwestern Bhutan, located in Chukha district near the Indian border. Situated at an altitude of 300 meters, it has a humid subtropical climate. The town's development is closely tied to the Tala Hydropower Plant, one of the largest in Bhutan. Gedu serves as a commercial hub for the region, with markets and services. The population is a mix of ethnic groups, including Ngalop, Lhotshampa, and others. Education and healthcare facilities have expanded. The town is connected by road to Phuentsholing and Thimphu.",
    },
    factsAdvanced: {
      de: ["Gedu liegt auf 300 m Höhe.", "Die Einwohnerzahl beträgt 5.000.", "Das Tala-Kraftwerk produziert 1.020 MW.", "Die Stadt hat 3 Grundschulen.", "Die durchschnittliche Temperatur beträgt 25 °C.", "Gedu ist 30 km von Phuentsholing entfernt."],
      hu: ["Gedu 300 m magasan van.", "Lakossága 5.000 fő.", "A Tala erőmű 1.020 MW-ot termel.", "A városban 3 általános iskola van.", "Az átlaghőmérséklet 25 °C.", "Gedu 30 km-re van Phuentsholingtól."],
      ro: ["Gedu se află la 300 m altitudine.", "Populația este de 5.000 de locuitori.", "Centrala Tala produce 1.020 MW.", "Orașul are 3 școli primare.", "Temperatura medie este de 25 °C.", "Gedu este la 30 km de Phuentsholing."],
      en: ["Gedu is at 300 m elevation.", "The population is 5,000.", "The Tala power plant produces 1,020 MW.", "The town has 3 primary schools.", "Average temperature is 25 °C.", "Gedu is 30 km from Phuentsholing."],
    },
  },
  {
    id: "bhutan-yoeseltse-cities-v2",
    type: "village",
    name: { de: "Yoeseltse", hu: "Yoeseltse", ro: "Yoeseltse", en: "Yoeseltse" },
    coords: [88.939, 26.958],
    description: {
      de: "Yoeseltse ist eine Grenzstadt im Distrikt Samtse im Südwesten Bhutans. Die Region ist für ihre fruchtbaren landwirtschaftlichen Flächen bekannt.",
      hu: "Yoeseltse egy határmenti település Samtse körzetben, Bhután délnyugati részén. A régió termékeny mezőgazdasági területeiről ismert.",
      ro: "Yoeseltse este un oraș de frontieră în districtul Samtse din sud-vestul Bhutanului. Regiunea este cunoscută pentru terenurile sale agricole.",
      en: "Yoeseltse is a border town in the Samtse District of southwestern Bhutan. The region is well-known for its fertile agricultural lands.",
    },
    facts: {
      de: ["Gehört zum Distrikt Samtse", "Nahe der Grenze zum indischen Bundesstaat Westbengalen", "Wichtiges Zentrum für den Anbau von Kardamom", "Früher unter dem Namen Sibsu bekannt"],
      hu: ["Samtse körzethez tartozik", "Az indiai Nyugat-Bengál határának közelében fekszik", "Fontos központ a kardamom termesztésében", "Korábban Sibsu néven volt ismert"],
      ro: ["Aparține districtului Samtse", "Aproape de granița cu statul indian Bengalul de Vest", "Centru important pentru cultivarea cardamomului", "Cunoscut anterior sub numele de Sibsu"],
      en: ["Part of the Samtse District", "Located near the West Bengal border of India", "Important center for cardamom cultivation", "Formerly known by the name Sibsu"],
    },
    imageHint: "Yoeseltse village and agricultural fields",
    descriptionAdvanced: {
      de: "Yoeseltse ist eine kleine Siedlung im Samtse-Distrikt im Südwesten Bhutans, nahe der Grenze zu Indien. Der Ort liegt auf einer Höhe von etwa 400 m in den subtropischen Ausläufern des Himalayas. Die Umgebung ist von dichten Wäldern und landwirtschaftlich genutzten Flächen geprägt; hauptsächlich werden Reis, Mais und Zitrusfrüchte angebaut. Die Bevölkerung besteht überwiegend aus Angehörigen der Lhotshampa, die eine Mischung aus nepalesischen und bhutanischen Kulturelementen pflegen. Die Infrastruktur ist ländlich, mit einer Grundschule und einem kleinen Markt. Yoeseltse dient als wichtiger Knotenpunkt für den Handel mit den benachbarten indischen Bundesstaaten Assam und Westbengalen.",
      hu: "Yoeseltse egy kis település Bhután délnyugati részén, a Samtse körzetben, az indiai határ közelében. A falu a szubtrópusi lábaknál fekszik, körülbelül 400 méteres tengerszint feletti magasságban. A térség gazdasága a mezőgazdaságra épül, különösen a rizstermesztésre, de kukoricát és citrusféléket is termesztenek. A lakosság többsége a Lhotshampa etnikai csoporthoz tartozik, akik nepáli és bhutáni hagyományokat ötvöznek. A településen található egy általános iskola és egy kis piac, amely a helyi termékek cseréjét szolgálja. Yoeseltse fontos szerepet játszik a határ menti kereskedelemben, mivel közvetlen utak kötik össze az indiai Assam és Nyugat-Bengál államokkal.",
      ro: "Yoeseltse este o localitate mică din districtul Samtse, situată în sud-vestul Bhutanului, aproape de granița cu India. Așezarea se află la o altitudine de aproximativ 400 m, în zona subtropicală a poalelor Himalayei. Economia locală se bazează pe agricultură, cu accent pe orez, porumb și citrice. Populația este formată în principal din etnici Lhotshampa, care păstrează tradiții mixte nepaleze și bhutaneze. Infrastructura include o școală primară și o piață mică. Yoeseltse joacă un rol important în comerțul transfrontalier, având legături rutiere directe cu statele indiene Assam și Bengalul de Vest.",
      en: "Yoeseltse is a settlement in Samtse District, southwestern Bhutan, near the border with India. Established in the early 20th century as a trading post, it lies in the subtropical foothills at an elevation of approximately 400 m. The area is predominantly agricultural, with rice, maize, and citrus fruits as main crops. The local population is primarily of Lhotshampa origin, reflecting a blend of Nepali and Bhutanese cultural influences. Yoeseltse serves as a market center for surrounding villages and facilitates cross-border trade with India, particularly in agricultural produce and household goods.",
    },
    factsAdvanced: {
      de: ["Höhenlage: ca. 400 m über dem Meeresspiegel", "Fläche: etwa 2,5 km²", "Bevölkerung: ungefähr 800 Einwohner (Schätzung 2022)", "Entfernung zur Kreisstadt Samtse: 15 km", "Durchschnittstemperatur Januar: 15 °C", "Jährlicher Niederschlag: ca. 2500 mm"],
      hu: ["Lakosságszám: kb. 750 fő (2022-es becslés)", "Átlagéletkor: 28 év", "Népsűrűség: 300 fő/km²", "Iskolák száma: 1 általános iskola", "Távközlési lefedettség: 4G mobilhálózat elérhető", "Villamosítás éve: 2005"],
      ro: ["Populație: aproximativ 780 de locuitori (estimare 2022)", "Principalii cultivatori: orez (60% din terenul agricol)", "Venit mediu lunar pe gospodărie: 15.000 ngultrum", "Număr de gospodării: 120", "Acces la apă potabilă: 95% din populație", "Drum principal: drum de pământ, modernizat parțial în 2018"],
      en: ["Year established: early 1900s as a trading post", "Distance to Indian border: 5 km", "Number of households: approximately 130", "Primary school founded: 1965", "Market day: weekly on Saturdays", "Main export products: ginger and cardamom"],
    },
  },
  {
    id: "bhutan-panbang-cities-v2",
    type: "town",
    name: { de: "Panbang", hu: "Panbang", ro: "Panbang", en: "Panbang" },
    coords: [90.951, 26.852],
    description: {
      de: "Panbang liegt am Zusammenfluss der Flüsse Mangde Chhu und Drangme Chhu. Es ist ein Tor zum Royal-Manas-Nationalpark im Süden.",
      hu: "Panbang a Mangde Chhu és a Drangme Chhu folyók összefolyásánál fekszik. Ez a déli Royal Manas Nemzeti Park egyik bejárata.",
      ro: "Panbang este situat la confluența râurilor Mangde Chhu și Drangme Chhu. Este o poartă către Parcul Național Royal Manas din sud.",
      en: "Panbang is situated at the confluence of the Mangde Chhu and Drangme Chhu rivers. It serves as a gateway to Royal Manas National Park.",
    },
    facts: {
      de: ["Liegt im Distrikt Zhemgang", "Bekannt für Ökotourismus und Rafting", "In der Nähe des Royal-Manas-Nationalparks", "Wichtiger Handelsplatz für lokale Bauern"],
      hu: ["Zhemgang körzetben található", "Ökoturizmusáról és vadvízi evezéséről ismert", "A Royal Manas Nemzeti Park szomszédságában fekszik", "A helyi gazdák fontos kereskedelmi helyszíne"],
      ro: ["Situat în districtul Zhemgang", "Cunoscut pentru ecoturism și rafting", "În apropierea Parcului Național Royal Manas", "Piață importantă pentru fermierii locali"],
      en: ["Located in the Zhemgang District", "Known for ecotourism and river rafting", "Close proximity to Royal Manas National Park", "Key trading post for local agricultural products"],
    },
    imageHint: "Panbang town near the river confluence",
    descriptionAdvanced: {
      de: "Panbang ist eine Ortschaft im Zhemgang-Distrikt im zentralen Süden Bhutans. Der Ort liegt auf einer Höhe von etwa 300 m im Tal des Manas-Flusses, der die Grenze zu Indien bildet. Panbang ist ein wichtiger Punkt für den Tourismus, da er Zugang zum Manas-Nationalpark bietet, einem UNESCO-Welterbe. Die Wirtschaft basiert auf Landwirtschaft (Reis, Mais) und zunehmend auf Ökotourismus. Die Bevölkerung setzt sich aus verschiedenen ethnischen Gruppen zusammen, darunter Kheng und Sharchop. Die Siedlung hat eine Grundschule und ein kleines Krankenhaus. Panbang ist bekannt für seine traditionellen Bambushandwerksarbeiten und jährlichen Feste wie das Panbang Tshechu.",
      hu: "Panbang egy kisváros Bhután déli-középső részén, a Zhemgang körzetben, a Manas folyó völgyében. A település mintegy 300 méteres tengerszint feletti magasságban fekszik, közvetlenül az indiai határ mellett. Panbang a Manas Nemzeti Park kapuja, amely az UNESCO Világörökség része. A helyi gazdaság a mezőgazdaságra (rizs, kukorica) és az ökoturizmusra épül. A lakosság főként Kheng és Sharchop etnikumokból áll. A településen általános iskola és egy egészségügyi központ működik. Panbang híres bambuszkézműves termékeiről és a Panbang Tshechu fesztiválról, amelyet évente rendeznek meg.",
      ro: "Panbang este un oraș mic în districtul Zhemgang, sud-centrul Bhutanului, situat în valea râului Manas, la o altitudine de aproximativ 300 m. Așezarea este cunoscută drept poartă de acces către Parcul Național Manas, patrimoniu UNESCO. Economia locală se bazează pe agricultură (orez, porumb) și ecoturism. Populația este diversă, incluzând grupuri etnice Kheng și Sharchop. Panbang dispune de o școală primară și un centru medical. Localitatea este renumită pentru meșteșugurile din bambus și festivalul anual Panbang Tshechu.",
      en: "Panbang is a town in Zhemgang District, south-central Bhutan, located in the Manas River valley near the Indian border. It serves as the main gateway to the Manas National Park, a UNESCO World Heritage site. The economy is predominantly agricultural, with rice and maize as staple crops, supplemented by growing ecotourism. The population is ethnically diverse, including Kheng and Sharchop communities. Panbang has a primary school and a basic health unit. The town is known for its bamboo handicrafts and the annual Panbang Tshechu festival, which features traditional mask dances and cultural performances.",
    },
    factsAdvanced: {
      de: ["Höhenlage: ca. 300 m", "Fläche: ca. 4 km²", "Bevölkerung: etwa 1200 Einwohner (2022)", "Entfernung zum Manas-Nationalpark: 2 km", "Durchschnittstemperatur Juli: 28 °C", "Anzahl der Touristenankünfte 2023: 5000"],
      hu: ["Lakosság: kb. 1150 fő", "Fesztivál dátuma: november első hete", "Bambusztermékekből származó éves bevétel: 2 millió ngultrum", "Iskolák: 1 általános iskola, 1 középiskola", "Kórházi ágyak száma: 10", "Távolság Zhemgang városától: 35 km"],
      ro: ["Populație: 1.200 de locuitori", "Suprafață: 3,8 km²", "Număr de gospodării: 200", "Acces la electricitate: 100%", "Principalii angajatori: agricultura și turismul", "Râul Manas: lungime de 60 km în Bhutan"],
      en: ["Year of establishment: 1960 as a settlement", "Elevation: 300 m above sea level", "Number of hotel rooms: 25", "Annual festival: Panbang Tshechu (since 1985)", "Manas National Park area: 1,057 km²", "Distance to Indian border: 1 km"],
    },
  },
  {
    id: "bhutan-lobesa-cities-v2",
    type: "village",
    name: { de: "Lobesa", hu: "Lobesa", ro: "Lobesa", en: "Lobesa" },
    coords: [89.871, 27.502],
    description: {
      de: "Lobesa ist ein malerisches Dorf im Tal von Punakha, bekannt für seine Reisfelder. Es liegt an der Straße zum berühmten Chimi Lhakhang.",
      hu: "Lobesa egy festői falu a Punakha-völgyben, amely rizsföldjeiről híres. A népszerű Chimi Lhakhang templomhoz vezető úton fekszik.",
      ro: "Lobesa este un sat pitoresc din valea Punakha, renumit pentru câmpurile de orez. Se află pe drumul către celebrul templu Chimi Lhakhang.",
      en: "Lobesa is a picturesque village in the Punakha Valley, famous for its rice paddies. It is the starting point for treks to Chimi Lhakhang.",
    },
    facts: {
      de: ["Zentrum des Reisanbaus in der Region", "Beherbergt das College of Natural Resources", "Liegt an der Kreuzung nach Wangdue und Punakha", "Traditionelle bhutanische Architektur"],
      hu: ["A régió rizstermesztésének központja", "Itt található a Természeti Erőforrások Főiskolája", "A Wangdue és Punakha felé vezető útkereszteződésnél fekszik", "Hagyományos bhutáni építészet jellemzi"],
      ro: ["Centrul cultivării orezului în regiune", "Găzduiește Colegiul de Resurse Naturale", "Situat la intersecția către Wangdue și Punakha", "Arhitectură tradițională bhutaneză"],
      en: ["Center of rice cultivation in the region", "Home to the College of Natural Resources", "Located at the junction to Wangdue and Punakha", "Features traditional Bhutanese farmhouse architecture"],
    },
    imageHint: "Lobesa village rice fields and houses",
    descriptionAdvanced: {
      de: "Lobesa ist ein kleines Dorf im Punakha-Distrikt in Zentralbhutan, bekannt für seine heißen Quellen (Tshachu) und seine malerische Lage am Fluss Mo Chhu. Der Ort liegt auf etwa 1200 m Höhe und ist landwirtschaftlich geprägt – vor allem Reis und Chili werden angebaut. Lobesa ist ein beliebtes Ziel für Trekkingtouren und spirituelle Reisen, da in der Nähe das Kloster Khamsum Yulley Namgyal Chorten liegt. Die lokale Bevölkerung gehört hauptsächlich der Ngalop-Gruppe an und pflegt traditionelle bhutanische Bräuche. Die heißen Quellen gelten als heilkräftig und ziehen sowohl Einheimische als auch Touristen an.",
      hu: "Lobesa egy kis falu Bhután középső részén, a Punakha körzetben, a Mo Chhu folyó partján. A település híres termálforrásairól (Tshachu), amelyeket gyógyító hatásúnak tartanak. Lobesa körülbelül 1200 méteres magasságban fekszik, és a mezőgazdaság a fő gazdasági ág, különösen a rizs- és chilipaprika-termesztés. A közelben található a Khamsum Yulley Namgyal Chorten kolostor, amely népszerű turisztikai célpont. A lakosság többségében ngalop etnikumú, és hagyományos bhutáni életmódot folytat. A termálforrások miatt Lobesa a wellness-turizmus egyik központja.",
      ro: "Lobesa este un sat mic în districtul Punakha, centrul Bhutanului, situat pe malul râului Mo Chhu, la o altitudine de aproximativ 1.200 m. Localitatea este cunoscută pentru izvoarele termale (Tshachu), considerate curative. Economia se bazează pe agricultură, în special orez și ardei iute. În apropiere se află templul Khamsum Yulley Namgyal Chorten, o atracție turistică majoră. Populația este predominant de etnie ngalop. Lobesa atrage vizitatori pentru trekking și turism spiritual, datorită peisajelor pitorești și a izvoarelor termale.",
      en: "Lobesa is a village in Punakha District, central Bhutan, situated along the Mo Chhu River at an elevation of 1,200 m. It is renowned for its natural hot springs (Tshachu), which are believed to have therapeutic properties. The local economy is primarily agricultural, with terraced rice paddies and chili cultivation. Landmarks include the Khamsum Yulley Namgyal Chorten, a temple built in 1999 to promote peace and stability. Lobesa serves as a base for trekkers exploring the surrounding valleys and mountains. The village maintains traditional Bhutanese architecture and lifestyle, offering a glimpse into rural life.",
    },
    factsAdvanced: {
      de: ["Höhenlage: ca. 1200 m", "Temperatur der heißen Quellen: 40-50 °C", "Entfernung zur Stadt Punakha: 10 km", "Bevölkerung: ca. 500 Einwohner", "Baujahr des Khamsum Yulley Namgyal Chorten: 1999", "Fläche des Dorfes: 1,5 km²"],
      hu: ["Lakosságszám: 480 fő (2022)", "Termálforrások száma: 3", "Chili-termés éves mennyisége: 20 tonna", "Iskolák: 1 általános iskola", "Távolság Thimphutól: 75 km", "Vendégházak száma: 5"],
      ro: ["Populație: 520 de locuitori", "Suprafața terenurilor agricole: 0,8 km²", "Debitul izvoarelor termale: 5 litri/secundă", "Distanța până la granița cu India: 60 km", "Număr de turiști pe an: 10.000", "Altitudine: 1.200 m"],
      en: ["Year of hot springs discovery: 1965", "Number of hot spring pools: 4", "Rice production per year: 30 tons", "Khamsum Yulley Namgyal Chorten height: 30 m", "Direct distance to Punakha Dzong: 8 km", "Average household size: 4.5 people"],
    },
  },
  {
    id: "bhutan-bajothang-cities-v2",
    type: "town",
    name: { de: "Bajothang", hu: "Bajothang", ro: "Bajothang", en: "Bajothang" },
    coords: [89.904, 27.493],
    description: {
      de: "Bajothang ist das neue Verwaltungszentrum des Distrikts Wangdue Phodrang. Die Stadt wurde nach dem Brand des alten Dzongs modern ausgebaut.",
      hu: "Bajothang Wangdue Phodrang körzet új közigazgatási központja. A várost modern szempontok szerint építették a régi központ tüze után.",
      ro: "Bajothang este noul centru administrativ al districtului Wangdue Phodrang. Orașul a fost dezvoltat modern după incendiul vechiului dzong.",
      en: "Bajothang is the new administrative center of the Wangdue Phodrang District. It was developed as a modern town after the old town fire.",
    },
    facts: {
      de: ["Verwaltungssitz von Wangdue Phodrang", "Liegt am Ufer des Flusses Punatsang Chhu", "Modern geplante Stadtstruktur", "Wichtiges Handelszentrum in Zentralbhutan"],
      hu: ["Wangdue Phodrang közigazgatási székhelye", "A Punatsang Chhu folyó partján fekszik", "Modern, tervezett városszerkezet", "Közép-Bhután fontos kereskedelmi központja"],
      ro: ["Sediul administrativ al Wangdue Phodrang", "Situat pe malul râului Punatsang Chhu", "Structură urbană planificată modern", "Centru comercial important în centrul Bhutanului"],
      en: ["Administrative headquarters of Wangdue Phodrang", "Located on the banks of the Punatsang Chhu river", "Modern planned urban layout", "Major commercial hub in central Bhutan"],
    },
    imageHint: "Bajothang town and Punatsang Chhu river",
    descriptionAdvanced: {
      de: "Bajothang ist ein Dorf im Wangdue Phodrang-Distrikt, das für seine traditionellen Handwebtechniken und die Produktion von Bhutan-Stoffen bekannt ist. Der Ort liegt auf etwa 1300 m Höhe in einem fruchtbaren Tal, das vom Fluss Punatsangchhu bewässert wird. Die Wirtschaft basiert auf Landwirtschaft und Handwerk, insbesondere der Herstellung von handgewebten Tüchern wie Gho und Kira. Die Bevölkerung besteht überwiegend aus Ngalop und pflegt eine starke kulturelle Identität. In Bajothang befindet sich eine Weberei-Schule, die die traditionelle Webkunst bewahrt. Der Ort ist auch Ausgangspunkt für Wanderungen zu den nahe gelegenen Klöstern.",
      hu: "Bajothang egy falu Bhután középső részén, a Wangdue Phodrang körzetben, mintegy 1300 méteres magasságban. A település híres kézi szövésű textiljeiről, amelyeket hagyományos bhutáni ruhák, mint a Gho és Kira készítéséhez használnak. A gazdaság a mezőgazdaságra és a textilkészítésre épül. A lakosság főként ngalop, és a falu erős közösségi hagyományokkal rendelkezik. Bajothangban egy szövőiskola működik, ahol a fiatalok elsajátíthatják a hagyományos technikákat. A település népszerű a turisták körében, akik a helyi textileket vásárolják és a környező hegyekben túráznak.",
      ro: "Bajothang este un sat în districtul Wangdue Phodrang, Bhutan central, situat la o altitudine de 1.300 m. Localitatea este cunoscută pentru țesutul manual tradițional, producând pânze pentru costumele naționale Gho și Kira. Economia se bazează pe agricultură și meșteșuguri textile. Populația este în principal de etnie ngalop. În Bajothang există o școală de țesut care păstrează tehnicile străvechi. Satul atrage turiști interesați de artizanat și drumeții montane.",
      en: "Bajothang is a village in Wangdue Phodrang District, central Bhutan, elevation 1,300 m. It is renowned for its traditional handwoven textiles, used for the national dress of Gho and Kira. The economy centers on agriculture and weaving, with a specialized weaving school preserving age-old techniques. The community is primarily Ngalop, maintaining strong cultural traditions. Bajothang offers visitors a chance to observe weaving processes and purchase authentic Bhutanese fabrics. The surrounding landscape features terraced fields and forests, suitable for short hikes.",
    },
    factsAdvanced: {
      de: ["Höhenlage: ca. 1300 m", "Bevölkerung: ca. 600 Einwohner", "Webschule gegründet: 1984", "Jährliche Stoffproduktion: 5000 Meter", "Entfernung zur Stadt Wangdue Phodrang: 12 km", "Durchschnittliche Größe eines Bauernhofs: 1 ha"],
      hu: ["Lakosság: 580 fő (2022)", "Kézi szövők száma: 30", "Éves textilbevétel: 1,5 millió ngultrum", "Iskolák: 1 általános iskola", "Távolság Thimphutól: 90 km", "Fesztivál: éves textilvásár augusztusban"],
      ro: ["Populație: 620 de locuitori", "Suprafață: 2 km²", "Număr de țesători activi: 25", "Producția anuală de orez: 40 de tone", "Atelier de țesut: 1 (cu 10 războaie)", "Distanța până la râul Punatsangchhu: 1 km"],
      en: ["Year weaving school established: 1984", "Number of looms: 12", "Average income per weaver: 50,000 ngultrum/year", "Village area: 1.8 km²", "Distance to nearest highway: 3 km", "Annual textile festival since 2000"],
    },
  },
  {
    id: "bhutan-jomotshangkha-cities-v2",
    type: "town",
    name: { de: "Jomotshangkha", hu: "Jomotshangkha", ro: "Jomotshangkha", en: "Jomotshangkha" },
    coords: [92.098, 26.911],
    description: {
      de: "Jomotshangkha ist eine abgelegene Stadt im äußersten Südosten Bhutans. Sie dient als wichtiger Grenzposten zum indischen Bundesstaat Assam.",
      hu: "Jomotshangkha egy távoli város Bhután délkeleti csücskében. Fontos határátkelőhelyként szolgál az indiai Assam állam felé.",
      ro: "Jomotshangkha este un oraș izolat în extremitatea de sud-est a Bhutanului. Servește ca post de frontieră important cu statul indian Assam.",
      en: "Jomotshangkha is a remote town in the far southeastern corner of Bhutan. It serves as an important border outpost near the Indian state of Assam.",
    },
    facts: {
      de: ["Liegt im Distrikt Samdrup Jongkhar", "Zugang hauptsächlich über indisches Territorium", "Bekannt für den Anbau von Orangen und Arekanüssen", "Früher Daifam genannt"],
      hu: ["Samdrup Jongkhar körzetben található", "Megközelítése főként indiai területen keresztül történik", "Narancs- és bételdió-termesztéséről ismert", "Korábban Daifam néven ismerték"],
      ro: ["Situat în districtul Samdrup Jongkhar", "Accesibil în principal prin teritoriul indian", "Cunoscut pentru cultivarea portocalelor și nucilor de areca", "Numit anterior Daifam"],
      en: ["Located in the Samdrup Jongkhar District", "Access often requires travel through Indian territory", "Known for orange and areca nut plantations", "Previously known as Daifam"],
    },
    imageHint: "Jomotshangkha town landscape near the border",
    descriptionAdvanced: {
      de: "Jomotshangkha (auch bekannt als Jomotshangkha) ist eine Ortschaft im Samdrup Jongkhar-Distrikt im Südosten Bhutans, nahe der indischen Grenze. Der Ort liegt auf einer Höhe von etwa 200 m im tropischen Tiefland und ist ein Zentrum für den Handel mit landwirtschaftlichen Erzeugnissen. Die Wirtschaft wird von der Landwirtschaft dominiert, insbesondere dem Anbau von Arekanüssen, Bananen und Kautschuk. Die Bevölkerung ist gemischt, mit einer bedeutenden Lhotshampa-Gemeinschaft. Jomotshangkha hat eine Grundschule und einen Grenzmarkt. Der Ort ist für sein warmes Klima und seine Fruchtbarkeit bekannt.",
      hu: "Jomotshangkha egy kisváros Bhután délkeleti részén, a Samdrup Jongkhar körzetben, az indiai határ közelében. A település körülbelül 200 méteres tengerszint feletti magasságban fekszik, a trópusi alföldön. A gazdaság a mezőgazdaságra épül: arekadió, banán és gumi termesztése. A lakosság jelentős része Lhotshampa etnikumú. Jomotshangkha fontos kereskedelmi központ a határ menti térségben. Egy határpiac és egy általános iskola található itt. Az éghajlat meleg és párás, ami kedvez a növénytermesztésnek.",
      ro: "Jomotshangkha este un oraș mic în districtul Samdrup Jongkhar, sud-estul Bhutanului, aproape de granița cu India. Așezarea se află la o altitudine de aproximativ 200 m, în câmpia tropicală. Economia se bazează pe agricultură, cu culturi precum nuca de arec, banana și cauciucul. Populația este mixtă, cu o comunitate Lhotshampa semnificativă. Jomotshangkha are o școală primară și o piață de frontieră. Clima este caldă și umedă, favorizând agricultura intensivă.",
      en: "Jomotshangkha is a town in Samdrup Jongkhar District, southeastern Bhutan, near the Indian border. Situated at an elevation of 200 m in the tropical lowlands, it serves as an agricultural trade hub. Main crops include areca nut, bananas, and rubber. The population is ethnically diverse, with a large Lhotshampa minority. The town has a primary school and a border market. The warm climate supports year-round farming. Jomotshangkha is also known for its local handloom products.",
    },
    factsAdvanced: {
      de: ["Höhenlage: ca. 200 m", "Bevölkerung: ca. 1500 Einwohner", "Fläche: ca. 3 km²", "Jahresdurchschnittstemperatur: 28 °C", "Entfernung zur indischen Grenze: 2 km", "Wichtigstes Exportprodukt: Arekanüsse (500 Tonnen/Jahr)"],
      hu: ["Lakosság: 1400 fő (becslés 2022)", "Banántermés éves mennyisége: 200 tonna", "Iskolák: 1 általános iskola, 1 középiskola", "Gumitermelés: 100 tonna évente", "Távolság Samdrup Jongkhartól: 20 km", "Piaci nap: szombat"],
      ro: ["Populație: 1.550 de locuitori", "Suprafață: 2,8 km²", "Cultivatori de arec: 200 de gospodării", "Precipitații anuale: 3.500 mm", "Număr de magazine: 15", "Altitudine: 200 m"],
      en: ["Year established: 1970s as a market town", "Distance to eastern border: 5 km", "Number of households: 250", "Primary school founded: 1982", "Main market day: Saturday", "Average household income: 20,000 ngultrum/month"],
    },
  },
  {
    id: "bhutan-tashichhoeling-cities-v2",
    type: "town",
    name: { de: "Tashichhoeling", hu: "Tashichhoeling", ro: "Tashichhoeling", en: "Tashichhoeling" },
    coords: [88.917, 27.025],
    description: {
      de: "Tashichhoeling ist eine wachsende Stadt im Distrikt Samtse. Sie ist ein bedeutendes landwirtschaftliches Zentrum im Südwesten des Landes.",
      hu: "Tashichhoeling egy fejlődő város Samtse körzetben. Az ország délnyugati részének egyik jelentős mezőgazdasági központja.",
      ro: "Tashichhoeling este un oraș în creștere în districtul Samtse. Este un centru agricol semnificativ în sud-vestul țării.",
      en: "Tashichhoeling is a growing town in the Samtse District. It is a significant agricultural and commercial hub in southwestern Bhutan.",
    },
    facts: {
      de: ["Verwaltungssitz eines Gewogs in Samtse", "Zentrum für die Produktion von Ingwer und Kardamom", "Nahe der Grenze zu Indien gelegen", "Verfügt über ein eigenes Krankenhaus"],
      hu: ["Egy Samtse-i kistérség közigazgatási központja", "A gyömbér- és kardamomtermelés központja", "Az indiai határ közelében fekszik", "Saját kórházzal rendelkezik"],
      ro: ["Centru administrativ al unui gewog din Samtse", "Centru pentru producția de ghimbir și cardamom", "Situat aproape de granița cu India", "Dispune de un spital local"],
      en: ["Administrative center for a gewog in Samtse", "Center for ginger and cardamom production", "Located near the border with India", "Home to a regional hospital facility"],
    },
    imageHint: "Tashichhoeling town and surrounding hills",
    descriptionAdvanced: {
      de: "Tashichhoeling ist eine Siedlung im Samtse-Distrikt im Südwesten Bhutans, nahe der Grenze zu Indien. Der Ort liegt auf etwa 300 m Höhe und ist von landwirtschaftlich genutzten Hügeln umgeben. Die Wirtschaft basiert auf dem Anbau von Kardamom, Ingwer und Zitrusfrüchten. Die Bevölkerung besteht hauptsächlich aus Lhotshampa, die nepalesische Kulturelemente pflegen. Tashichhoeling hat eine Grundschule und ein Gesundheitszentrum. Der Name bedeutet auf Dzongkha „glücklicher Ort“. In der Umgebung gibt es mehrere kleine Tempel und heilige Stätten.",
      hu: "Tashichhoeling egy falu Bhután délnyugati részén, a Samtse körzetben, az indiai határ mellett. Körülbelül 300 méteres magasságban fekszik. A gazdaság a mezőgazdaságra épül: kardamom, gyömbér és citrusfélék termesztése. A lakosság többsége Lhotshampa, akik nepáli hagyományokat követnek. A településen általános iskola és egészségügyi központ működik. A név jelentése „boldog hely”. A környéken kisebb szentélyek találhatók.",
      ro: "Tashichhoeling este o așezare în districtul Samtse, sud-vestul Bhutanului, lângă granița cu India. Altitudinea este de aproximativ 300 m. Economia se bazează pe cultivarea cardamomului, ghimbirului și citricelor. Populația este predominant Lhotshampa. Localitatea are o școală primară și un centru de sănătate. Numele înseamnă „loc fericit”. În zonă există câteva temple mici.",
      en: "Tashichhoeling is a settlement in Samtse District, southwestern Bhutan, near the Indian border. At an elevation of 300 m, it is an agricultural area focusing on cardamom, ginger, and citrus. The population is mainly Lhotshampa. The village has a primary school and a health unit. The name translates to 'Happy Place' in Dzongkha. Small temples dot the surrounding hills.",
    },
    factsAdvanced: {
      de: ["Höhenlage: ca. 300 m", "Bevölkerung: ca. 400 Einwohner", "Kardamomproduktion: 30 Tonnen/Jahr", "Ingwerproduktion: 50 Tonnen/Jahr", "Entfernung zum Fluss: 5 km", "Anzahl der Haushalte: 80"],
      hu: ["Lakosság: 420 fő (2022)", "Citrustermés éves mennyisége: 100 tonna", "Iskola alapításának éve: 1990", "Távolság a körzetközponttól: 18 km", "Villamosítás éve: 2008", "Egészségügyi központ kapacitása: 5 ágy"],
      ro: ["Populație: 380 de locuitori", "Suprafață agricolă: 1,2 km²", "Număr de gospodării: 70", "Medie precipitații: 2.800 mm/an", "Distanța până la granița indiană: 4 km", "Școală primară: 1 (construită în 1992)"],
      en: ["Elevation: 300 m", "Year primary school built: 1992", "Number of cardamom farmers: 40", "Annual ginger yield: 50 tons", "Distance to Samtse town: 18 km", "Population density: 150 people/km²"],
    },
  },
  {
    id: "bhutan-khaling-cities-v2",
    type: "town",
    name: { de: "Khaling", hu: "Khaling", ro: "Khaling", en: "Khaling" },
    coords: [91.602, 27.198],
    description: {
      de: "Khaling liegt im Osten Bhutans und ist berühmt für seine Webtraditionen. Das Dorf beherbergt die erste Blindenschule des Landes.",
      hu: "Khaling Bhután keleti részén fekszik, és híres a szövési hagyományairól. A település ad otthont az ország első vakok iskolájának.",
      ro: "Khaling este situat în estul Bhutanului și este renumit pentru tradițiile sale de țesut. Satul găzduiește prima școală pentru nevăzători din țară.",
      en: "Khaling is located in eastern Bhutan and is famous for its weaving traditions. It is home to the country's first school for the blind.",
    },
    facts: {
      de: ["Liegt im Distrikt Trashigang", "Berühmt für das National Institute for the Visually Impaired", "Zentrum für traditionelle bhutanische Webkunst", "Umgeben von dichten Wäldern"],
      hu: ["Trashigang körzetben található", "Híres a Látássérültek Nemzeti Intézetéről", "A hagyományos bhutáni szövés központja", "Sűrű erdőkkel körülvett terület"],
      ro: ["Situat în districtul Trashigang", "Renumit pentru Institutul Național pentru Persoane cu Deficiențe de Vedere", "Centru pentru arta tradițională a țesutului", "Înconjurat de păduri dense"],
      en: ["Located in the Trashigang District", "Famous for the National Institute for the Visually Impaired", "Hub for traditional Bhutanese textile weaving", "Surrounded by lush broadleaf forests"],
    },
    imageHint: "Khaling village and weaving center",
    descriptionAdvanced: {
      de: "Khaling ist eine Siedlung im Trashigang-Distrikt im Osten Bhutans, gelegen auf etwa 1000 m Höhe in den Hügeln des Himalayas. Der Ort ist bekannt für seine traditionellen Handwebtechniken und die Produktion von Khaling-Stoffen, die für ihre Muster geschätzt werden. Die Wirtschaft basiert auf Landwirtschaft (Reis, Mais, Kartoffeln) und Handwerk. Die Bevölkerung gehört überwiegend zur Ethnie der Sharchop. Khaling hat eine Grundschule und einen kleinen Markt. Der Ort bietet einen Panoramablick auf die umliegenden Berge und Täler.",
      hu: "Khaling egy falu Bhután keleti részén, a Trashigang körzetben, körülbelül 1000 méteres magasságban. Híres a kézi szövésű Khaling-textilekről, amelyek jellegzetes mintáikról ismertek. A gazdaság a mezőgazdaságra (rizs, kukorica, burgonya) és a textilkészítésre épül. A lakosság többsége Sharchop etnikumú. A településen általános iskola és piac található. A környező hegyekre nyíló kilátás vonzza a turistákat.",
      ro: "Khaling este o localitate în districtul Trashigang, estul Bhutanului, la o altitudine de aproximativ 1.000 m. Este cunoscută pentru țesăturile tradiționale Khaling, cu modele distinctive. Economia se bazează pe agricultură (orez, porumb, cartofi) și meșteșuguri textile. Populația este în principal de etnie Sharchop. Khaling are o școală primară și o piață. Priveliștile montane sunt atracții turistice.",
      en: "Khaling is a settlement in Trashigang District, eastern Bhutan, elevation 1,000 m. It is famous for its handwoven Khaling textiles, characterized by unique patterns. The economy relies on agriculture (rice, maize, potatoes) and weaving. The population is primarily Sharchop. Khaling has a primary school and a market. The area offers scenic views of the Himalayan foothills.",
    },
    factsAdvanced: {
      de: ["Höhenlage: ca. 1000 m", "Bevölkerung: ca. 700 Einwohner", "Webstühle: 20 in Betrieb", "Jährliche Reisproduktion: 60 Tonnen", "Entfernung zur Stadt Trashigang: 25 km", "Durchschnittstemperatur: 20 °C"],
      hu: ["Lakosság: 680 fő (2022)", "Textilbevétel: 1 millió ngultrum/év", "Iskolák: 1 általános iskola", "Kukoricatermés: 40 tonna évente", "Távolság a folyótól: 3 km", "Fesztivál: éves textilkiállítás"],
      ro: ["Populație: 720 de locuitori", "Suprafață: 2,3 km²", "Producție anuală de cartofi: 80 de tone", "Număr de țesători: 20", "Altitudine: 1.000 m", "Distanța până la cea mai apropiată șosea: 2 km"],
      en: ["Year weaving tradition documented: 19th century", "Number of looms: 18", "Maize production per year: 40 tons", "Distance to Trashigang: 25 km", "Primary school founded: 1985", "Average annual rainfall: 1,500 mm"],
    },
  },
  {
    id: "bhutan-radhi-cities-v2",
    type: "village",
    name: { de: "Radhi", hu: "Radhi", ro: "Radhi", en: "Radhi" },
    coords: [91.684, 27.381],
    description: {
      de: "Radhi wird oft als die 'Reisschüssel des Ostens' bezeichnet. Das Dorf ist bekannt für seine ausgedehnten Reisterrassen und Seidenweberei.",
      hu: "Radhit gyakran 'Kelet rizstáljaként' emlegetik. A falu kiterjedt rizsteraszairól és selyemszövéséről ismert.",
      ro: "Radhi este adesea supranumit „Bolul de orez al Estului”. Satul este cunoscut pentru terasele sale de orez și țesutul mătăsii.",
      en: "Radhi is often referred to as the 'Rice Bowl of the East'. The village is famous for its extensive rice terraces and raw silk weaving.",
    },
    facts: {
      de: ["Größter Reisproduzent in Ostbhutan", "Bekannt für die Herstellung von Buray-Seide", "Liegt im Distrikt Trashigang", "Berühmt für die Radhi-Lhakhang-Tempel"],
      hu: ["Kelet-Bhután legnagyobb rizstermelője", "A Buray-selyem előállításáról híres", "Trashigang körzetben található", "A Radhi Lhakhang templomairól ismert"],
      ro: ["Cel mai mare producător de orez din estul Bhutanului", "Cunoscut pentru producția de mătase Buray", "Situat în districtul Trashigang", "Faimos pentru templele Radhi Lhakhang"],
      en: ["Largest rice producer in eastern Bhutan", "Famous for producing Buray (raw silk) textiles", "Located in the Trashigang District", "Notable for the Radhi Lhakhang temples"],
    },
    imageHint: "Radhi village rice terraces",
    descriptionAdvanced: {
      de: "Radhi ist ein Dorf im Trashigang-Distrikt im Osten Bhutans, bekannt für seine traditionelle Weberei und die Produktion von Kushuthara-Stoffen, die aufwendig bestickt sind. Der Ort liegt auf etwa 1200 m Höhe und ist von Reisterrassen umgeben. Die Wirtschaft basiert auf Landwirtschaft und Textilhandwerk. Die Bevölkerung gehört der Sharchop-Ethnie an. Radhi hat eine Grundschule und ein Gemeindezentrum. Das Dorf ist berühmt für das jährliche Radhi-Festival, bei dem Tänze und Gesänge aufgeführt werden.",
      hu: "Radhi egy falu Bhután keleti részén, a Trashigang körzetben, körülbelül 1200 méteres magasságban. Híres a Kushuthara textilekről, amelyek gazdag hímzésűek. A gazdaság a mezőgazdaságra (rizs) és a textilművességre épül. A lakosság Sharchop. Radhiban általános iskola és közösségi ház található. A Radhi Fesztivál évente vonzza a látogatókat.",
      ro: "Radhi este un sat în districtul Trashigang, estul Bhutanului, la 1.200 m altitudine. Renumit pentru țesăturile Kushuthara, brodate intricate. Economia se bazează pe agricultură (orez) și meșteșuguri textile. Populația este Sharchop. Satul are o școală primară și un centru comunitar. Festivalul anual Radhi atrage turiști.",
      en: "Radhi is a village in Trashigang District, eastern Bhutan, elevation 1,200 m. It is famous for its highly embroidered Kushuthara textiles. The economy is based on agriculture and textile handicrafts. The population is Sharchop. Radhi has a primary school and a community hall. The annual Radhi festival features traditional dances and songs.",
    },
    factsAdvanced: {
      de: ["Höhenlage: ca. 1200 m", "Bevölkerung: ca. 500 Einwohner", "Kushuthara-Produktion: 200 Stück/Jahr", "Entfernung zur Stadt Trashigang: 30 km", "Reisterrassen: 50 ha", "Festivaltermin: Oktober"],
      hu: ["Lakosság: 480 fő (2022)", "Hímzett textilek éves bevétele: 0,8 millió ngultrum", "Iskola alapításának éve: 1988", "Távolság a folyótól: 2 km", "Háztartások száma: 90", "Éves csapadék: 1400 mm"],
      ro: ["Populație: 520 de locuitori", "Suprafață: 1,8 km²", "Număr de meșteri textiliști: 25", "Producție anuală de orez: 70 de tone", "Altitudine: 1.200 m", "Distanța până la granița indiană: 50 km"],
      en: ["Year Kushuthara tradition originated: 18th century", "Number of weavers: 30", "Rice paddies area: 50 hectares", "Annual textile festival since 1995", "Distance to Trashigang: 30 km", "Average household size: 5.2"],
    },
  },
  {
    id: "bhutan-taktse-cities-v2",
    type: "village",
    name: { de: "Taktse", hu: "Taktse", ro: "Taktse", en: "Taktse" },
    coords: [90.581, 27.422],
    description: {
      de: "Taktse liegt in der Nähe von Trongsa und ist Standort einer bedeutenden Hochschule für Sprache und Kultur. Es bietet weite Ausblicke auf das Tal.",
      hu: "Taktse Trongsa közelében fekszik, és egy jelentős nyelvészeti és kulturális főiskola helyszíne. Széles kilátást nyújt a völgyre.",
      ro: "Taktse este situat lângă Trongsa și găzduiește un colegiu important de limbă și cultură. Oferă vederi largi asupra văii.",
      en: "Taktse is located near Trongsa and is home to a major college for language and culture. It offers expansive views of the surrounding valley.",
    },
    facts: {
      de: ["Sitz des College of Language and Culture Studies", "Gehört zum Distrikt Trongsa", "Wichtiger Ort für die Bewahrung bhutanischer Kultur", "Liegt an der zentralen Ost-West-Autobahn"],
      hu: ["A Nyelvészeti és Kulturális Tanulmányok Főiskolájának székhelye", "Trongsa körzethez tartozik", "Fontos helyszín a bhutáni kultúra megőrzésében", "A központi kelet-nyugati főút mentén fekszik"],
      ro: ["Sediul Colegiului de Studii Lingvistice și Culturale", "Aparține districtului Trongsa", "Loc important pentru conservarea culturii bhutaneze", "Situat pe autostrada centrală Est-Vest"],
      en: ["Home to the College of Language and Culture Studies", "Part of the Trongsa District", "Key site for the preservation of Bhutanese heritage", "Located along the central East-West highway"],
    },
    imageHint: "Taktse college buildings and valley view",
    descriptionAdvanced: {
      de: "Taktse ist ein Dorf im Trashigang-Distrikt im Osten Bhutans, das auf einem Bergrücken auf etwa 1800 m Höhe liegt. Der Ort ist Ausgangspunkt für Wanderungen zum Taktse-Kloster, einem wichtigen buddhistischen Zentrum. Die Wirtschaft basiert auf Landwirtschaft und Tourismus. Die Bevölkerung besteht hauptsächlich aus Sharchop. Taktse hat eine Grundschule und einige Gästehäuser. Das Kloster Taktse Goemba wurde im 15. Jahrhundert gegründet und ist für seine Wandmalereien bekannt.",
      hu: "Taktse egy falu Bhután keleti részén, a Trashigang körzetben, körülbelül 1800 méteres magasságban egy hegygerincen. A Taktse kolostor kiindulópontja, amely fontos buddhista központ. A gazdaság a mezőgazdaságon és a turizmuson alapul. A lakosság Sharchop. A településen általános iskola és vendégházak találhatók. A Taktse Goemba kolostort a 15. században alapították.",
      ro: "Taktse este un sat în districtul Trashigang, estul Bhutanului, la 1.800 m altitudine pe o creastă. Este punct de plecare pentru drumeții la mănăstirea Taktse, un centru budist important. Economia se bazează pe agricultură și turism. Populația este Sharchop. Satul are o școală primară și pensiuni. Mănăstirea Taktse Goemba datează din secolul al XV-lea.",
      en: "Taktse is a village in Trashigang District, eastern Bhutan, perched on a ridge at 1,800 m. It serves as a gateway to Taktse Monastery, a notable Buddhist site. The economy depends on agriculture and tourism. The population is Sharchop. The village has a primary school and guesthouses. Taktse Goemba monastery was founded in the 15th century.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 1800 m", "Bevölkerung: ca. 300 Einwohner", "Gründung des Klosters: 15. Jahrhundert", "Entfernung zur nächsten Stadt: 15 km", "Anzahl der Touristen pro Jahr: 3000", "Durchschnittstemperatur: 15 °C"],
      hu: ["Lakosság: 280 fő (2022)", "Kolostor magassága: 2000 m", "Vendégházak száma: 4", "Iskola alapításának éve: 1995", "Távolság Trashigangtól: 20 km", "Éves turisztikai bevétel: 2 millió ngultrum"],
      ro: ["Populație: 320 de locuitori", "Suprafață: 1,2 km²", "Altitudinea mănăstirii: 2.000 m", "Număr de călugări: 50", "Distanța până la râu: 4 km", "Precipitații anuale: 1.200 mm"],
      en: ["Elevation: 1,800 m", "Monastery founded: 1450", "Number of monks: 50", "Annual visitors: 3,000", "Primary school built: 1995", "Distance to Trashigang: 20 km"],
    },
  },
  {
    id: "bhutan-drujegang-cities-v2",
    type: "village",
    name: { de: "Drujegang", hu: "Drujegang", ro: "Drujegang", en: "Drujegang" },
    coords: [90.012, 27.055],
    description: {
      de: "Drujegang ist ein historisches Dorf im Distrikt Dagana. Es ist bekannt für seine alten Wehrtürme und die fruchtbare Umgebung.",
      hu: "Drujegang egy történelmi falu Dagana körzetben. Régi őrtornyairól és termékeny környezetéről ismert.",
      ro: "Drujegang este un sat istoric în districtul Dagana. Este cunoscut pentru turnurile sale vechi de apărare și împrejurimile fertile.",
      en: "Drujegang is a historic village in the Dagana District. It is known for its ancient watchtowers and productive agricultural surroundings.",
    },
    facts: {
      de: ["Beherbergt einen kleinen, aber strategischen Dzong", "Wichtiger Ort für den Anbau von Mandarinen", "Liegt im Distrikt Dagana", "Bedeutend für die lokale Geschichte der Region"],
      hu: ["Egy kicsi, de stratégiai dzongnak ad otthont", "Fontos helyszín a mandarin termesztésében", "Dagana körzetben található", "Jelentős szerepet játszik a régió történelmében"],
      ro: ["Găzduiește un mic dar strategic dzong", "Loc important pentru cultivarea mandarinelor", "Situat în districtul Dagana", "Semnificativ pentru istoria locală a regiunii"],
      en: ["Home to a small but strategically located dzong", "Important area for mandarin orange production", "Located in the Dagana District", "Historically significant for the local region"],
    },
    imageHint: "Drujegang village and surrounding citrus orchards",
    descriptionAdvanced: {
      de: "Drujegang ist eine Siedlung im Tsirang-Distrikt im zentralen Süden Bhutans, auf etwa 500 m Höhe. Der Ort ist ein landwirtschaftliches Zentrum, das für den Anbau von Arekanüssen und Kardamom bekannt ist. Die Bevölkerung setzt sich aus Lhotshampa und Ngalop zusammen. Drujegang hat eine Grundschule, ein Gesundheitszentrum und einen Markt. Die Umgebung ist hügelig und bewaldet.",
      hu: "Drujegang egy település Bhután középső-déli részén, a Tsirang körzetben, körülbelül 500 méteres magasságban. Mezőgazdasági központ, arekadió- és kardamomtermesztéssel. A lakosság vegyes: Lhotshampa és Ngalop. A településen általános iskola, egészségügyi központ és piac található.",
      ro: "Drujegang este o localitate în districtul Tsirang, sud-centrul Bhutanului, la 500 m altitudine. Este un centru agricol cunoscut pentru nuca de arec și cardamom. Populația mixtă: Lhotshampa și Ngalop. Localitatea are o școală primară, un centru de sănătate și o piață.",
      en: "Drujegang is a settlement in Tsirang District, south-central Bhutan, elevation 500 m. It is an agricultural hub for areca nut and cardamom cultivation. The population is a mix of Lhotshampa and Ngalop. Drujegang has a primary school, health center, and market.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 500 m", "Bevölkerung: ca. 600 Einwohner", "Arekanussproduktion: 400 Tonnen/Jahr", "Kardamomproduktion: 20 Tonnen/Jahr", "Entfernung zum Fluss: 2 km", "Anzahl der Haushalte: 100"],
      hu: ["Lakosság: 580 fő (2022)", "Egészségügyi központ ágyainak száma: 8", "Iskola alapításának éve: 1987", "Távolság a körzetközponttól: 12 km", "Piac napja: szerda", "Villamosítás éve: 2010"],
      ro: ["Populație: 620 de locuitori", "Suprafață: 2 km²", "Producție anuală de arec: 400 de tone", "Număr de gospodării: 105", "Altitudine: 500 m", "Distanța până la granița indiană: 30 km"],
      en: ["Year established: 1960s", "Number of farmers: 120", "Annual cardamom yield: 20 tons", "Distance to Tsirang town: 12 km", "Primary school founded: 1987", "Average rainfall: 2,000 mm/year"],
    },
  },
  {
    id: "bhutan-bondey-cities-v2",
    type: "town",
    name: { de: "Bondey", hu: "Bondey", ro: "Bondey", en: "Bondey" },
    coords: [89.442, 27.391],
    description: {
      de: "Bondey ist ein geschäftiger Handelsort im Paro-Tal. Er liegt strategisch günstig in der Nähe des internationalen Flughafens von Bhutan.",
      hu: "Bondey egy forgalmas kereskedelmi csomópont a Paro-völgyben. Stratégiailag kedvező helyen, a nemzetközi repülőtér közelében fekszik.",
      ro: "Bondey este un centru comercial aglomerat în Valea Paro. Este situat strategic lângă aeroportul internațional din Bhutan.",
      en: "Bondey is a bustling commercial center in the Paro Valley. It is strategically located near Bhutan's only international airport.",
    },
    facts: {
      de: ["Wichtigstes Handelszentrum im unteren Paro-Tal", "Standort der Bondey Farm Machinery Corporation", "Liegt im Distrikt Paro", "Wichtiger Verkehrsknotenpunkt für Reisende"],
      hu: ["Az alsó Paro-völgy legfontosabb kereskedelmi központja", "A Bondey Mezőgazdasági Gépgyártó Vállalat székhelye", "Paro körzetben található", "Fontos közlekedési csomópont az utazók számára"],
      ro: ["Cel mai important centru comercial din valea inferioară Paro", "Sediul corporației de utilaje agricole Bondey", "Situat în districtul Paro", "Nod rutier important pentru turiști"],
      en: ["Main commercial hub in the lower Paro Valley", "Home to the Bondey Farm Machinery Corporation", "Located in the Paro District", "Major transit point for airport travelers"],
    },
    imageHint: "Bondey town market area in Paro",
    descriptionAdvanced: {
      de: "Bondey ist ein Dorf im Paro-Distrikt im Westen Bhutans, gelegen auf etwa 2200 m Höhe. Der Ort ist bekannt für seine Apfelplantagen und die Produktion von Apfelwein. Die Wirtschaft basiert auf Landwirtschaft (Äpfel, Kartoffeln) und Tourismus. Bondey liegt in der Nähe des Taktshang-Klosters und ist ein beliebter Zwischenstopp für Wanderer. Die Bevölkerung gehört hauptsächlich zur Ngalop-Ethnie. Das Dorf hat eine Grundschule und einige Geschäfte.",
      hu: "Bondey egy falu Bhután nyugati részén, a Paro körzetben, körülbelül 2200 méteres magasságban. Híres almás ültetvényeiről és almabor termeléséről. A gazdaság a mezőgazdaságon (alma, burgonya) és a turizmuson alapul. Bondey a Taktshang kolostor közelében fekszik, népszerű túracélpont. A lakosság főként ngalop. A faluban általános iskola és üzletek találhatók.",
      ro: "Bondey este un sat în districtul Paro, vestul Bhutanului, la 2.200 m altitudine. Faimos pentru livezile de mere și producția de cidru. Economia se bazează pe agricultură (mere, cartofi) și turism. Bondey se află lângă mănăstirea Taktshang. Populația este predominant ngalop. Satul are o școală primară și câteva magazine.",
      en: "Bondey is a village in Paro District, western Bhutan, elevation 2,200 m. Known for apple orchards and cider production, its economy relies on agriculture and tourism. Located near the famous Taktshang Monastery, it is a stop for trekkers. The population is mainly Ngalop. Bondey has a primary school and shops.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 2200 m", "Bevölkerung: ca. 400 Einwohner", "Apfelproduktion: 150 Tonnen/Jahr", "Ciderproduktion: 10.000 Liter/Jahr", "Entfernung zum Taktshang-Kloster: 5 km", "Durchschnittstemperatur: 12 °C"],
      hu: ["Lakosság: 380 fő (2022)", "Almabor éves termelése: 12.000 liter", "Iskola alapításának éve: 1992", "Távolság Paro városától: 8 km", "Turisták száma évente: 5000", "Ültetvények területe: 30 ha"],
      ro: ["Populație: 420 de locuitori", "Suprafață: 1,5 km²", "Producție anuală de mere: 150 de tone", "Număr de producători de cidru: 5", "Altitudine: 2.200 m", "Distanța până la aeroportul Paro: 6 km"],
      en: ["Elevation: 2,200 m", "Apple orchard area: 30 hectares", "Annual cider output: 10,000 liters", "Distance to Paro town: 8 km", "Primary school established: 1992", "Number of guesthouses: 3"],
    },
  },
  {
    id: "bhutan-rinchending-cities-v2",
    type: "town",
    name: { de: "Rinchending", hu: "Rinchending", ro: "Rinchending", en: "Rinchending" },
    coords: [89.394, 26.862],
    description: {
      de: "Rinchending markiert den Eingang zum bhutanischen Hochland oberhalb von Phuentsholing. Es ist bekannt für sein prächtiges Klostertor.",
      hu: "Rinchending jelzi a bhutáni felföld bejáratát Phuentsholing felett. Híres a pompás kolostorkapujáról.",
      ro: "Rinchending marchează intrarea în zonele înalte din Bhutan, deasupra orașului Phuentsholing. Este cunoscut pentru poarta sa magnifică a mănăstirii.",
      en: "Rinchending marks the gateway to the Bhutanese highlands above Phuentsholing. It is famous for its grand Zangto Pelri Lhakhang gate.",
    },
    facts: {
      de: ["Beherbergt das College of Science and Technology", "Standort des berühmten Zangto-Pelri-Klosters", "Bietet Panoramablicke auf die Ebenen Indiens", "Liegt im Distrikt Chhukha"],
      hu: ["A Természettudományi és Technológiai Főiskola székhelye", "Itt található a híres Zangto Pelri kolostor", "Panorámás kilátást nyújt India síkságaira", "Chhukha körzetben található"],
      ro: ["Găzduiește Colegiul de Știință și Tehnologie", "Locația faimoasei mănăstiri Zangto Pelri", "Oferă vederi panoramice asupra câmpiilor Indiei", "Situat în districtul Chhukha"],
      en: ["Home to the College of Science and Technology", "Site of the famous Zangto Pelri Lhakhang", "Offers panoramic views of the Indian plains", "Located in the Chhukha District"],
    },
    imageHint: "Rinchending monastery gate and view",
    descriptionAdvanced: {
      de: "Rinchending ist ein Dorf im Paro-Distrikt, das auf einer Anhöhe mit Blick auf das Paro-Tal liegt, auf etwa 2300 m Höhe. Es ist bekannt für das Rinchending-Kloster, das im 17. Jahrhundert erbaut wurde. Die Wirtschaft basiert auf Landwirtschaft und Tourismus. Die Bevölkerung ist hauptsächlich Ngalop. Das Dorf hat eine Grundschule und ist ein beliebter Ort für Vogelbeobachtungen. Vom Kloster aus hat man einen spektakulären Blick auf den Paro-Fluss.",
      hu: "Rinchending egy falu Bhután nyugati részén, a Paro körzetben, körülbelül 2300 méteres magasságban, a Paro-völgyre néző dombon. Híres a 17. században épült Rinchending kolostorról. A gazdaság a mezőgazdaságon és a turizmuson alapul. A lakosság főként ngalop. A faluban általános iskola működik, és népszerű madárleső hely. A kolostorból lenyűgöző kilátás nyílik a Paro folyóra.",
      ro: "Rinchending este un sat în districtul Paro, vestul Bhutanului, la 2.300 m altitudine, pe o colină cu vedere spre valea Paro. Este cunoscut pentru mănăstirea Rinchending, construită în secolul al XVII-lea. Economia se bazează pe agricultură și turism. Populația este predominant ngalop. Satul are o școală primară și este popular pentru observarea păsărilor.",
      en: "Rinchending is a village in Paro District, western Bhutan, at 2,300 m on a hill overlooking the Paro valley. It is known for the Rinchending Monastery, built in the 17th century. The economy relies on agriculture and tourism. The population is mainly Ngalop. The village has a primary school and is a birdwatching spot. The monastery offers panoramic views of the Paro River.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 2300 m", "Bevölkerung: ca. 250 Einwohner", "Klosterbaujahr: 17. Jahrhundert", "Entfernung zur Stadt Paro: 4 km", "Anzahl der Mönche: 30", "Durchschnittstemperatur: 11 °C"],
      hu: ["Lakosság: 240 fő (2022)", "Madárfajok száma a környéken: 100+", "Iskola alapításának éve: 1998", "Távolság a folyótól: 1 km", "Vendégházak: 2", "Éves turisták száma: 2000"],
      ro: ["Populație: 260 de locuitori", "Suprafață: 0,8 km²", "Altitudinea mănăstirii: 2.400 m", "Număr de călugări: 30", "Distanța până la drumul principal: 1 km", "Precipitații anuale: 1.100 mm"],
      en: ["Elevation: 2,300 m", "Monastery built: 1650", "Number of birds species recorded: 110", "Distance to Paro: 4 km", "Primary school founded: 1998", "Annual snowfall: 30 cm"],
    },
  },
  {
    id: "bhutan-nobding-cities-v2",
    type: "village",
    name: { de: "Nobding", hu: "Nobding", ro: "Nobding", en: "Nobding" },
    coords: [90.065, 27.441],
    description: {
      de: "Nobding ist ein kleiner Ort an der wichtigen Ost-West-Autobahn in Bhutan. Er dient als beliebter Rastplatz für Reisende in Richtung Phobjikha.",
      hu: "Nobding egy kis település a fontos bhutáni kelet-nyugati főút mentén. Népszerű pihenőhely a Phobjikha felé tartó utazók számára.",
      ro: "Nobding este o mică așezare pe importanta autostradă Est-Vest din Bhutan. Servește ca loc de popas pentru călătorii spre Phobjikha.",
      en: "Nobding is a small settlement along Bhutan's vital East-West highway. It serves as a popular rest stop for travelers heading to Phobjikha.",
    },
    facts: {
      de: ["Liegt im Distrikt Wangdue Phodrang", "Wichtiger Zwischenstopp für Fernbusse", "Eingang zum Phobjikha-Tal in der Nähe", "Bekannt für lokale Berggemüse"],
      hu: ["Wangdue Phodrang körzetben található", "A távolsági buszok fontos pihenőhelye", "A Phobjikha-völgy bejárata a közelben van", "Helyi hegyi zöldségeiről ismert"],
      ro: ["Situat în districtul Wangdue Phodrang", "Punct de oprire important pentru autobuzele pe distanțe lungi", "Poarta de intrare în Valea Phobjikha este în apropiere", "Cunoscut pentru legumele montane locale"],
      en: ["Located in the Wangdue Phodrang District", "Key refreshment stop for long-distance buses", "Close to the entrance of the Phobjikha Valley", "Famous for locally grown mountain vegetables"],
    },
    imageHint: "Nobding roadside settlement and mountains",
    descriptionAdvanced: {
      de: "Nobding ist ein Dorf im Zhemgang-Distrikt im zentralen Süden Bhutans, gelegen auf etwa 400 m Höhe. Der Ort ist ein landwirtschaftliches Zentrum für den Anbau von Reis und Mais. Die Bevölkerung besteht hauptsächlich aus Kheng. Nobding hat eine Grundschule und einen Markt. Die Region ist bekannt für ihre dichten Wälder und die Tierwelt, einschließlich Elefanten und Tiger.",
      hu: "Nobding egy falu Bhután középső-déli részén, a Zhemgang körzetben, körülbelül 400 méteres magasságban. Mezőgazdasági központ rizs és kukorica termesztésével. A lakosság főként Kheng etnikumú. Nobdingban általános iskola és piac található. A környék erdei gazdag vadvilággal rendelkeznek.",
      ro: "Nobding este un sat în districtul Zhemgang, sud-centrul Bhutanului, la 400 m altitudine. Centru agricol pentru orez și porumb. Populația este în principal de etnie Kheng. Satul are o școală primară și o piață. Pădurile din zonă găzduiesc elefanți și tigri.",
      en: "Nobding is a village in Zhemgang District, south-central Bhutan, at 400 m elevation. It is an agricultural center for rice and maize. The population is mainly Kheng. Nobding has a primary school and a market. The surrounding forests are home to elephants and tigers.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 400 m", "Bevölkerung: ca. 500 Einwohner", "Reisproduktion: 100 Tonnen/Jahr", "Maisproduktion: 80 Tonnen/Jahr", "Entfernung zur Kreisstadt: 20 km", "Anzahl der Haushalte: 90"],
      hu: ["Lakosság: 480 fő (2022)", "Iskola alapításának éve: 1985", "Távolság a folyótól: 3 km", "Piac napja: csütörtök", "Villamosítás éve: 2007", "Erdőterület: 5 km²"],
      ro: ["Populație: 520 de locuitori", "Suprafață: 2,2 km²", "Producție de orez: 100 tone/an", "Număr de gospodării: 85", "Altitudine: 400 m", "Distanța până la Zhemgang: 20 km"],
      en: ["Elevation: 400 m", "Year primary school built: 1985", "Rice cultivation area: 40 hectares", "Maize yield: 80 tons/year", "Distance to Zhemgang: 20 km", "Elephant population in area: 10"],
    },
  },
  {
    id: "bhutan-darla-cities-v2",
    type: "town",
    name: { de: "Darla", hu: "Darla", ro: "Darla", en: "Darla" },
    coords: [89.512, 26.883],
    description: {
      de: "Darla ist ein bedeutender Industriestandort im Distrikt Chhukha. Die Stadt ist eng mit den großen Wasserkraftprojekten des Landes verbunden.",
      hu: "Darla jelentős ipari helyszín Chhukha körzetben. A város szorosan kapcsolódik az ország nagy vízienergia-projektjeihez.",
      ro: "Darla este o locație industrială semnificativă în districtul Chhukha. Orașul este strâns legat de marile proiecte hidroenergetice ale țării.",
      en: "Darla is a significant industrial and residential town in the Chhukha District. It is closely linked to Bhutan's major hydropower projects.",
    },
    facts: {
      de: ["Beherbergt Personal der Wasserkraftwerke", "Umgeben von weitläufigen Teeplantagen", "Liegt im Distrikt Chhukha", "Wichtiges regionales Wirtschaftszentrum"],
      hu: ["A vízierőművek dolgozóinak ad otthont", "Kiterjedt teaültetvények veszik körül", "Chhukha körzetben található", "Fontos regionális gazdasági központ"],
      ro: ["Găzduiește personalul proiectelor hidroenergetice", "Înconjurat de plantații extinse de ceai", "Situat în districtul Chhukha", "Centru economic regional important"],
      en: ["Home to staff from major hydropower plants", "Surrounded by extensive tea plantations", "Located in the Chhukha District", "Key regional economic center"],
    },
    imageHint: "Darla town and tea gardens",
    descriptionAdvanced: {
      de: "Darla ist eine Siedlung im Chukha-Distrikt im Südwesten Bhutans, nahe der Grenze zu Indien, auf etwa 300 m Höhe. Der Ort ist ein industrielles Zentrum mit einer Zementfabrik und mehreren Produktionsstätten. Die Wirtschaft ist daher gemischt: Industrie und Landwirtschaft. Die Bevölkerung besteht aus Lhotshampa und anderen Gruppen. Darla hat eine Grundschule, ein Krankenhaus und einen Markt. Die Umgebung ist hügelig.",
      hu: "Darla egy település Bhután délnyugati részén, a Chukha körzetben, az indiai határ közelében, körülbelül 300 méteres magasságban. Ipari központ egy cementgyárral. A gazdaság vegyes: ipar és mezőgazdaság. A lakosság Lhotshampa és más csoportok keveréke. Darlában általános iskola, kórház és piac található.",
      ro: "Darla este o localitate în districtul Chukha, sud-vestul Bhutanului, lângă granița cu India, la 300 m altitudine. Centru industrial cu o fabrică de ciment. Economia mixtă: industrie și agricultură. Populația este un amestec de Lhotshampa și alte grupuri. Darla are o școală primară, spital și piață.",
      en: "Darla is a settlement in Chukha District, southwestern Bhutan, near the Indian border, at 300 m elevation. It is an industrial hub with a cement factory. The economy is mixed: industry and agriculture. The population is a mix of Lhotshampa and others. Darla has a primary school, hospital, and market.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 300 m", "Bevölkerung: ca. 2000 Einwohner", "Zementproduktion: 500.000 Tonnen/Jahr", "Entfernung zur indischen Grenze: 2 km", "Anzahl der Fabriken: 3", "Durchschnittstemperatur: 27 °C"],
      hu: ["Lakosság: 1900 fő (2022)", "Kórházi ágyak száma: 20", "Iskola alapításának éve: 1978", "Távolság Phuentsholingtól: 10 km", "Ipari övezet területe: 1 km²", "Munkanélküliségi ráta: 5%"],
      ro: ["Populație: 2.100 de locuitori", "Suprafață: 3 km²", "Productivitatea fabricii de ciment: 500.000 tone/an", "Număr de angajați în industrie: 500", "Altitudine: 300 m", "Distanța până la Chukha: 15 km"],
      en: ["Elevation: 300 m", "Cement factory established: 1985", "Population: 2,100", "Number of schools: 2 (primary and secondary)", "Distance to Phuentsholing: 10 km", "Annual rainfall: 3,000 mm"],
    },
  },
  {
    id: "bhutan-dorokha-cities-v2",
    type: "town",
    name: { de: "Dorokha", hu: "Dorokha", ro: "Dorokha", en: "Dorokha" },
    coords: [89.152, 26.955],
    description: {
      de: "Dorokha ist ein abgelegenes Verwaltungszentrum im Distrikt Samtse. Die Stadt liegt in einer hügeligen Landschaft mit reicher Landwirtschaft.",
      hu: "Dorokha egy távoli közigazgatási központ Samtse körzetben. A város dombos vidéken fekszik, gazdag mezőgazdasággal rendelkezik.",
      ro: "Dorokha este un centru administrativ izolat în districtul Samtse. Orașul este situat într-un peisaj deluros cu o agricultură bogată.",
      en: "Dorokha is a remote administrative center in the Samtse District. The town is nestled in a hilly landscape with rich agricultural heritage.",
    },
    facts: {
      de: ["Verwaltungssitz des Dungkhag Dorokha", "Bekannt für den Anbau von Kardamom und Mais", "Liegt im Distrikt Samtse", "Heimat verschiedener ethnischer Gruppen"],
      hu: ["A Dorokha kistérség közigazgatási székhelye", "Kardamom- és kukoricatermesztéséről ismert", "Samtse körzetben található", "Különböző etnikai csoportok otthona"],
      ro: ["Sediul administrativ al Dungkhag Dorokha", "Cunoscut pentru cultivarea cardamomului și porumbului", "Situat în districtul Samtse", "Căminul diverselor grupuri etnice"],
      en: ["Administrative headquarters for Dorokha Dungkhag", "Known for cardamom and maize production", "Located in the Samtse District", "Home to diverse ethnic communities"],
    },
    imageHint: "Dorokha town in the hills of Samtse",
    descriptionAdvanced: {
      de: "Dorokha ist ein Dorf im Samtse-Distrikt im Südwesten Bhutans, auf etwa 400 m Höhe. Die Wirtschaft basiert auf Landwirtschaft, insbesondere dem Anbau von Kardamom, Ingwer und Zitrusfrüchten. Die Bevölkerung besteht hauptsächlich aus Lhotshampa. Dorokha hat eine Grundschule und ein Gesundheitszentrum. Die Umgebung ist hügelig und bewaldet.",
      hu: "Dorokha egy falu Bhután délnyugati részén, a Samtse körzetben, körülbelül 400 méteres magasságban. A gazdaság a mezőgazdaságra épül: kardamom, gyömbér és citrusfélék. A lakosság főként Lhotshampa. Dorokhában általános iskola és egészségügyi központ van.",
      ro: "Dorokha este un sat în districtul Samtse, sud-vestul Bhutanului, la 400 m altitudine. Economia se bazează pe agricultură: cardamom, ghimbir, citrice. Populația este predominant Lhotshampa. Satul are o școală primară și un centru de sănătate.",
      en: "Dorokha is a village in Samtse District, southwestern Bhutan, elevation 400 m. The economy relies on agriculture: cardamom, ginger, and citrus. The population is mainly Lhotshampa. Dorokha has a primary school and a health center.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 400 m", "Bevölkerung: ca. 450 Einwohner", "Kardamomproduktion: 25 Tonnen/Jahr", "Ingwerproduktion: 60 Tonnen/Jahr", "Entfernung zur Stadt Samtse: 10 km", "Anzahl der Haushalte: 85"],
      hu: ["Lakosság: 430 fő (2022)", "Citrustermés: 80 tonna/év", "Iskola alapításának éve: 1991", "Távolság a folyótól: 2 km", "Egészségügyi központ ágyai: 4", "Villamosítás éve: 2006"],
      ro: ["Populație: 460 de locuitori", "Suprafață: 1,5 km²", "Producție de cardamom: 25 tone/an", "Număr de gospodării: 80", "Altitudine: 400 m", "Distanța până la granița indiană: 5 km"],
      en: ["Elevation: 400 m", "Cardamom yield: 25 tons/year", "Ginger production: 60 tons/year", "Distance to Samtse: 10 km", "Primary school built: 1991", "Average household size: 5.4"],
    },
  },
  {
    id: "bhutan-chengmari-cities-v2",
    type: "town",
    name: { de: "Chengmari", hu: "Chengmari", ro: "Chengmari", en: "Chengmari" },
    coords: [89.054, 26.912],
    description: {
      de: "Chengmari ist ein wichtiger Grenzort im Südwesten Bhutans. Er ist ein Zentrum für den Handel mit Agrarprodukten im Distrikt Samtse.",
      hu: "Chengmari egy fontos határmenti település Bhután délnyugati részén. A Samtse körzet mezőgazdasági kereskedelmének központja.",
      ro: "Chengmari este un oraș de frontieră important în sud-vestul Bhutanului. Este un centru pentru comerțul agricol în districtul Samtse.",
      en: "Chengmari is an important border settlement in southwestern Bhutan. It serves as a center for agricultural trade in the Samtse District.",
    },
    facts: {
      de: ["Wichtiger Handelspunkt für Arekanüsse", "Nahe der Grenze zum indischen Westbengalen", "Gehört zum Distrikt Samtse", "Tropisches Klima begünstigt Früchteanbau"],
      hu: ["A bételdió fontos kereskedelmi pontja", "Az indiai Nyugat-Bengál határának közelében fekszik", "Samtse körzethez tartozik", "A trópusi éghajlat kedvez a gyümölcstermesztésnek"],
      ro: ["Punct important de comerț pentru nucile de areca", "Lângă granița cu Bengalul de Vest din India", "Aparține districtului Samtse", "Climatul tropical favorizează cultivarea fructelor"],
      en: ["Major trading point for areca nuts", "Located near the border with West Bengal, India", "Part of the Samtse District", "Tropical climate supports diverse fruit farming"],
    },
    imageHint: "Chengmari town and border area",
    descriptionAdvanced: {
      de: "Chengmari ist ein Dorf im Samtse-Distrikt im Südwesten Bhutans, nahe der Grenze zu Indien, auf etwa 350 m Höhe. Die Wirtschaft basiert auf Landwirtschaft, hauptsächlich Arekanüssen, Bananen und Gemüse. Die Bevölkerung besteht aus Lhotshampa. Chengmari hat eine Grundschule und einen Markt. Der Ort ist für seine heißen Quellen bekannt.",
      hu: "Chengmari egy falu Bhután délnyugati részén, a Samtse körzetben, az indiai határ közelében, körülbelül 350 méteres magasságban. Gazdasága az arekadió, banán és zöldség termesztésén alapul. A lakosság Lhotshampa. Chengmariban általános iskola és piac található. Híres termálforrásairól.",
      ro: "Chengmari este un sat în districtul Samtse, sud-vestul Bhutanului, lângă granița cu India, la 350 m altitudine. Economia se bazează pe cultivarea nucilor de arec, a bananelor și a legumelor. Populația este Lhotshampa. Satul are o școală primară și o piață. Cunoscut pentru izvoarele termale.",
      en: "Chengmari is a village in Samtse District, southwestern Bhutan, near the Indian border, at 350 m elevation. The economy relies on areca nut, banana, and vegetable farming. The population is Lhotshampa. Chengmari has a primary school and a market. It is known for its hot springs.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 350 m", "Bevölkerung: ca. 400 Einwohner", "Arekanussproduktion: 200 Tonnen/Jahr", "Bananenproduktion: 100 Tonnen/Jahr", "Entfernung zur indischen Grenze: 1 km", "Temperatur der heißen Quellen: 45 °C"],
      hu: ["Lakosság: 380 fő (2022)", "Zöldségtermés: 50 tonna/év", "Iskola alapításának éve: 1993", "Távolság Samtsétől: 8 km", "Termálforrások száma: 2", "Piac napja: kedd"],
      ro: ["Populație: 420 de locuitori", "Suprafață: 1,2 km²", "Producție de banane: 100 tone/an", "Număr de gospodării: 75", "Altitudine: 350 m", "Distanța până la graniță: 1 km"],
      en: ["Elevation: 350 m", "Areca nut production: 200 tons/year", "Hot springs temperature: 45°C", "Distance to Indian border: 1 km", "Primary school founded: 1993", "Number of hot spring pools: 3"],
    },
  },
  {
    id: "bhutan-gakiling-cities-v2",
    type: "village",
    name: { de: "Gakiling", hu: "Gakiling", ro: "Gakiling", en: "Gakiling" },
    coords: [89.102, 27.152],
    description: {
      de: "Gakiling ist eine abgelegene Siedlung im Distrikt Haa. Das Dorf liegt in einer tief eingeschnittenen Talregion mit unberührter Natur.",
      hu: "Gakiling egy távoli település Haa körzetben. A falu egy mély völgyben fekszik, érintetlen természeti környezetben.",
      ro: "Gakiling este o așezare izolată în districtul Haa. Satul este situat într-o regiune de vale adâncă, cu natură neatinsă.",
      en: "Gakiling is a remote settlement in the Haa District. The village is situated in a deep valley region with pristine natural surroundings.",
    },
    facts: {
      de: ["Gehört zum Distrikt Haa", "Bekannt für Viehzucht und Almwirtschaft", "Schwer zugängliche Gebirgsregion", "Wichtiges Zentrum für den lokalen Gewog"],
      hu: ["Haa körzethez tartozik", "Állattenyésztéséről és havasi gazdálkodásáról ismert", "Nehezen megközelíthető hegyvidéki terület", "A helyi kistérség (gewog) fontos központja"],
      ro: ["Aparține districtului Haa", "Cunoscut pentru creșterea animalelor", "Regiune montană greu accesibilă", "Centru important pentru gewog-ul local"],
      en: ["Part of the Haa District", "Known for livestock rearing and alpine farming", "Located in a difficult-to-access mountain region", "Main center for the local administrative gewog"],
    },
    imageHint: "Gakiling village in the Haa mountains",
    descriptionAdvanced: {
      de: "Gakiling ist ein kleines Dorf im Sarpang-Distrikt im Süden Bhutans, auf etwa 300 m Höhe. Die Wirtschaft basiert auf Landwirtschaft, insbesondere Reis und Mais. Die Bevölkerung besteht aus Lhotshampa und indigenen Gruppen. Gakiling hat eine Grundschule und ein Gesundheitszentrum. Die Region ist für ihre Biodiversität bekannt.",
      hu: "Gakiling egy kis falu Bhután déli részén, a Sarpang körzetben, körülbelül 300 méteres magasságban. Gazdasága a rizs és kukorica termesztésén alapul. A lakosság Lhotshampa és őslakos csoportok keveréke. Gakilingban általános iskola és egészségügyi központ van. A környék gazdag biológiai sokféleségben.",
      ro: "Gakiling este un sat mic în districtul Sarpang, sudul Bhutanului, la 300 m altitudine. Economia se bazează pe orez și porumb. Populația este un amestec de Lhotshampa și grupuri indigene. Satul are o școală primară și un centru de sănătate. Zona este cunoscută pentru biodiversitate.",
      en: "Gakiling is a small village in Sarpang District, southern Bhutan, elevation 300 m. The economy is based on rice and maize cultivation. The population is a mix of Lhotshampa and indigenous groups. Gakiling has a primary school and a health center. The area is known for its biodiversity.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 300 m", "Bevölkerung: ca. 350 Einwohner", "Reisproduktion: 80 Tonnen/Jahr", "Maisproduktion: 60 Tonnen/Jahr", "Entfernung zur Kreisstadt: 25 km", "Anzahl der Haushalte: 65"],
      hu: ["Lakosság: 330 fő (2022)", "Iskola alapításának éve: 1990", "Távolság a folyótól: 4 km", "Egészségügyi központ ágyai: 3", "Villamosítás éve: 2005", "Biodiverzitás: több mint 200 növényfaj"],
      ro: ["Populație: 360 de locuitori", "Suprafață: 1 km²", "Producție de orez: 80 tone/an", "Număr de gospodării: 60", "Altitudine: 300 m", "Distanța până la Sarpang: 25 km"],
      en: ["Elevation: 300 m", "Rice paddies area: 30 hectares", "Maize yield: 60 tons/year", "Distance to Sarpang town: 25 km", "Primary school built: 1990", "Number of indigenous families: 20"],
    },
  },
  {
    id: "bhutan-sangbaykha-cities-v2",
    type: "village",
    name: { de: "Sangbaykha", hu: "Sangbaykha", ro: "Sangbaykha", en: "Sangbaykha" },
    coords: [89.081, 27.184],
    description: {
      de: "Sangbaykha ist ein isoliertes Dorf im Distrikt Haa. Es bewahrt viele traditionelle bhutanische Bräuche in seiner abgeschiedenen Lage.",
      hu: "Sangbaykha egy elszigetelt falu Haa körzetben. Elzárt fekvésének köszönhetően számos hagyományos bhutáni szokást őriz.",
      ro: "Sangbaykha este un sat izolat în districtul Haa. Își păstrează multe obiceiuri tradiționale bhutaneze datorită locației sale retrase.",
      en: "Sangbaykha is an isolated village in the Haa District. It preserves many traditional Bhutanese customs due to its secluded location.",
    },
    facts: {
      de: ["Verwaltungssitz des Sangbaykha-Gewogs", "Traditionelle Yak-Haltung ist Haupterwerbsquelle", "Liegt im Distrikt Haa", "Umgeben von majestätischen Gipfeln"],
      hu: ["A Sangbaykha kistérség közigazgatási központja", "A hagyományos jaktartás a fő megélhetési forrás", "Haa körzetben található", "Fenséges hegycsúcsok veszik körül"],
      ro: ["Centru administrativ al gewog-ului Sangbaykha", "Creșterea tradițională a iacilor este principala sursă de venit", "Situat în districtul Haa", "Înconjurat de vârfuri montane maiestuoase"],
      en: ["Administrative center for Sangbaykha gewog", "Traditional yak herding is the primary livelihood", "Located in the Haa District", "Surrounded by majestic mountain peaks"],
    },
    imageHint: "Sangbaykha village and traditional houses",
    descriptionAdvanced: {
      de: "Sangbaykha ist ein Dorf im Haa-Distrikt im Westen Bhutans, auf etwa 2700 m Höhe. Die Wirtschaft basiert auf Landwirtschaft (Äpfel, Kartoffeln) und Viehzucht (Yaks). Die Bevölkerung ist hauptsächlich Ngalop. Sangbaykha hat eine Grundschule und ist für seine traditionellen Holzhäuser bekannt. Die Umgebung bietet Wanderwege mit Blick auf die Berge.",
      hu: "Sangbaykha egy falu Bhután nyugati részén, a Haa körzetben, körülbelül 2700 méteres magasságban. Gazdasága az almatermesztésen, burgonyán és jaktenyésztésen alapul. A lakosság főként ngalop. Sangbaykha híres hagyományos faházairól. A környéken túraútvonalak találhatók.",
      ro: "Sangbaykha este un sat în districtul Haa, vestul Bhutanului, la 2.700 m altitudine. Economia se bazează pe mere, cartofi și creșterea iacilor. Populația este predominant ngalop. Satul are o școală primară și este cunoscut pentru casele tradiționale din lemn.",
      en: "Sangbaykha is a village in Haa District, western Bhutan, at 2,700 m elevation. The economy is based on apples, potatoes, and yak herding. The population is mainly Ngalop. Sangbaykha has a primary school and is known for its traditional wooden houses. The area offers trekking routes.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 2700 m", "Bevölkerung: ca. 300 Einwohner", "Apfelproduktion: 100 Tonnen/Jahr", "Kartoffelproduktion: 150 Tonnen/Jahr", "Anzahl der Yaks: 200", "Entfernung zur Stadt Haa: 15 km"],
      hu: ["Lakosság: 280 fő (2022)", "Iskola alapításának éve: 1996", "Távolság a folyótól: 1 km", "Hagyományos házak: 40", "Túraútvonalak hossza: 20 km", "Éves csapadék: 900 mm"],
      ro: ["Populație: 310 locuitori", "Suprafață: 0,9 km²", "Producție de mere: 100 tone/an", "Număr de gospodării: 55", "Altitudine: 2.700 m", "Distanța până la Haa: 15 km"],
      en: ["Elevation: 2,700 m", "Apple orchard area: 20 hectares", "Yak population: 200", "Distance to Haa: 15 km", "Primary school built: 1996", "Average winter temperature: -5°C"],
    },
  },
  {
    id: "bhutan-khasadrapchu-cities-v2",
    type: "town",
    name: { de: "Khasadrapchu", hu: "Khasadrapchu", ro: "Khasadrapchu", en: "Khasadrapchu" },
    coords: [89.601, 27.371],
    description: {
      de: "Khasadrapchu ist eine wachsende Stadt südlich von Thimphu am Ufer des Wang Chhu. Sie beherbergt wichtige Werkstätten und Industrien.",
      hu: "Khasadrapchu egy fejlődő város Thimphutól délre, a Wang Chhu partján. Fontos műhelyeknek és ipari létesítményeknek ad otthont.",
      ro: "Khasadrapchu este un oraș în creștere la sud de Thimphu, pe malul râului Wang Chhu. Găzduiește ateliere și industrii importante.",
      en: "Khasadrapchu is a growing town south of Thimphu on the banks of the Wang Chhu. It is home to vital industrial workshops and small factories.",
    },
    facts: {
      de: ["Liegt im Distrikt Thimphu", "Wichtiger Industriestandort nahe der Hauptstadt", "An der Hauptstraße Thimphu-Phuentsholing", "Bekannt für mechanische Werkstätten"],
      hu: ["Thimphu körzetben található", "Fontos ipari helyszín a főváros közelében", "A Thimphu-Phuentsholing főút mentén fekszik", "Mechanikai műhelyeiről ismert"],
      ro: ["Situat în districtul Thimphu", "Locație industrială importantă lângă capitală", "Pe autostrada principală Thimphu-Phuentsholing", "Cunoscut pentru atelierele mecanice"],
      en: ["Located in the Thimphu District", "Important industrial site near the capital city", "Situated on the Thimphu-Phuentsholing highway", "Known for hosting various mechanical workshops"],
    },
    imageHint: "Khasadrapchu industrial area and river",
    descriptionAdvanced: {
      de: "Khasadrapchu (auch Khashadrapchu) ist ein Dorf im Thimphu-Distrikt in Westbhutan, auf etwa 2400 m Höhe. Die Wirtschaft basiert auf Landwirtschaft und dem Anbau von Chili und Kartoffeln. Die Bevölkerung ist Ngalop. Das Dorf hat eine Grundschule und ist für seine traditionellen Architektur bekannt. In der Nähe befindet sich der Buddha Point.",
      hu: "Khasadrapchu egy falu Bhután nyugati részén, a Thimphu körzetben, körülbelül 2400 méteres magasságban. Gazdasága a chili és burgonya termesztésén alapul. A lakosság ngalop. A faluban általános iskola van, és híres hagyományos építészetéről. A közelben található a Buddha Point.",
      ro: "Khasadrapchu este un sat în districtul Thimphu, vestul Bhutanului, la 2.400 m altitudine. Economia se bazează pe ardei iute și cartofi. Populația este ngalop. Satul are o școală primară și arhitectură tradițională. Aproape se află Buddha Point.",
      en: "Khasadrapchu is a village in Thimphu District, western Bhutan, at 2,400 m elevation. The economy relies on chili and potato farming. The population is Ngalop. The village has a primary school and is known for traditional architecture. The Buddha Point is nearby.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 2400 m", "Bevölkerung: ca. 400 Einwohner", "Chiliproduktion: 30 Tonnen/Jahr", "Kartoffelproduktion: 100 Tonnen/Jahr", "Entfernung zur Hauptstadt Thimphu: 8 km", "Anzahl der Haushalte: 80"],
      hu: ["Lakosság: 380 fő (2022)", "Iskola alapításának éve: 1994", "Távolság a Buddha Pointtól: 2 km", "Hagyományos házak: 50", "Villamosítás éve: 2003", "Átlaghőmérséklet: 12 °C"],
      ro: ["Populație: 420 de locuitori", "Suprafață: 1,1 km²", "Producție de ardei iute: 30 tone/an", "Număr de gospodării: 75", "Altitudine: 2.400 m", "Distanța până la Thimphu: 8 km"],
      en: ["Elevation: 2,400 m", "Chili yield: 30 tons/year", "Potato production: 100 tons/year", "Distance to Thimphu: 8 km", "Primary school founded: 1994", "Buddha Point statue height: 51.5 m"],
    },
  },
  {
    id: "bhutan-hongtsho-cities-v2",
    type: "village",
    name: { de: "Hongtsho", hu: "Hongtsho", ro: "Hongtsho", en: "Hongtsho" },
    coords: [89.742, 27.482],
    description: {
      de: "Hongtsho ist ein Bergdorf am Weg zum Dochula-Pass. Es ist berühmt für seinen Apfelanbau und dient als Kontrollpunkt für den Verkehr.",
      hu: "Hongtsho egy hegyi falu a Dochula-hágó felé vezető úton. Almatermesztéséről híres és fontos közúti ellenőrző pontként szolgál.",
      ro: "Hongtsho este un sat montan pe drumul spre Pasul Dochula. Este renumit pentru livezile de meri și servește ca punct de control rutier.",
      en: "Hongtsho is a mountain village on the way to the Dochula Pass. It is famous for its apple orchards and serves as a highway checkpoint.",
    },
    facts: {
      de: ["Liegt im Distrikt Thimphu", "Wichtiges Zentrum für die Apfelproduktion", "Standort eines polizeilichen Kontrollpunktes", "Kühles Klima begünstigt Obstbau"],
      hu: ["Thimphu körzetben található", "Az almatermelés egyik fontos központja", "Rendőrségi ellenőrző pont helyszíne", "Hűvös éghajlata kedvez a gyümölcstermesztésnek"],
      ro: ["Situat în districtul Thimphu", "Centru important pentru producția de mere", "Locația unui punct de control al poliției", "Climatul răcoros favorizează pomicultura"],
      en: ["Located in the Thimphu District", "Major center for apple production", "Site of a mandatory vehicle checkpoint", "Cool climate is ideal for temperate fruit farming"],
    },
    imageHint: "Hongtsho village and apple orchards",
    descriptionAdvanced: {
      de: "Hongtsho ist ein Dorf im Thimphu-Distrikt im Westen Bhutans, auf etwa 2600 m Höhe. Die Wirtschaft basiert auf Landwirtschaft (Kartoffeln, Weizen) und Viehzucht. Die Bevölkerung ist Ngalop. Hongtsho hat eine Grundschule und ein buddhistisches Kloster. Der Ort ist für seine friedliche Atmosphäre bekannt.",
      hu: "Hongtsho egy falu Bhután nyugati részén, a Thimphu körzetben, körülbelül 2600 méteres magasságban. Gazdasága a burgonya, búza termesztésén és állattenyésztésen alapul. A lakosság ngalop. Hongtshoban általános iskola és buddhista kolostor található. A település békés légköréről ismert.",
      ro: "Hongtsho este un sat în districtul Thimphu, vestul Bhutanului, la 2.600 m altitudine. Economia se bazează pe cartofi, grâu și creșterea animalelor. Populația este ngalop. Satul are o școală primară și o mănăstire budistă. Localitatea este cunoscută pentru atmosfera pașnică.",
      en: "Hongtsho is a village in Thimphu District, western Bhutan, at 2,600 m elevation. The economy is based on potatoes, wheat, and livestock. The population is Ngalop. Hongtsho has a primary school and a Buddhist monastery. It is known for its peaceful atmosphere.",
    },
    factsAdvanced: {
      de: ["Höhenlage: 2600 m", "Bevölkerung: ca. 350 Einwohner", "Kartoffelproduktion: 80 Tonnen/Jahr", "Weizenproduktion: 30 Tonnen/Jahr", "Entfernung zur Stadt Thimphu: 12 km", "Anzahl der Haushalte: 70"],
      hu: ["Lakosság: 330 fő (2022)", "Iskola alapításának éve: 1993", "Távolság a kolostortól: 0,5 km", "Állatállomány: 200 szarvasmarha", "Villamosítás éve: 2004", "Éves csapadék: 1000 mm"],
      ro: ["Populație: 360 de locuitori", "Suprafață: 1 km²", "Producție de cartofi: 80 tone/an", "Număr de gospodării: 65", "Altitudine: 2.600 m", "Distanța până la Thimphu: 12 km"],
      en: ["Elevation: 2,600 m", "Potato yield: 80 tons/year", "Wheat production: 30 tons/year", "Distance to Thimphu: 12 km", "Primary school built: 1993", "Monastery founded: 18th century"],
    },
  },
  {
    id: "bhutan-shaba-cities-v2",
    type: "village",
    name: { de: "Shaba", hu: "Shaba", ro: "Shaba", en: "Shaba" },
    coords: [89.471, 27.382],
    description: {
      de: "Shaba ist ein bedeutender landwirtschaftlicher Ort im Paro-Tal. Das Dorf ist bekannt für seine fruchtbaren Böden und traditionellen Farmen.",
      hu: "Shaba egy jelentős mezőgazdasági település a Paro-völgyben. A falu termékeny talajáról és hagyományos gazdaságairól ismert.",
      ro: "Shaba este o localitate agricolă semnificativă în Valea Paro. Satul este cunoscut pentru solurile sale fertile și fermele tradiționale.",
      en: "Shaba is a significant agricultural settlement in the Paro Valley. The village is known for its fertile soil and traditional Bhutanese farms.",
    },
    facts: {
      de: ["Liegt im Distrikt Paro", "Wichtiges Zentrum für den Reisanbau", "Bekannt für die Shaba-Sekundarschule", "Häufiger Durchgangspunkt zum Flughafen"],
      hu: ["Paro körzetben található", "A rizstermesztés fontos központja", "A Shaba középiskoláról ismert", "Gyakori áthaladási pont a repülőtér felé"],
      ro: ["Situat în districtul Paro", "Centru important pentru cultivarea orezului", "Cunoscut pentru școala secundară Shaba", "Punct de tranzit frecvent către aeroport"],
      en: ["Located in the Paro District", "Main center for rice and vegetable farming", "Home to the notable Shaba Secondary School", "Common transit area on the road to the airport"],
    },
    imageHint: "Shaba village fields in Paro",
    descriptionAdvanced: {
      de: "Shaba ist ein Dorf im Westen Bhutans, das eine wichtige Rolle im traditionellen Handel zwischen Tibet und Bhutan spielte. Es liegt im Paro-Tal auf einer Höhe von etwa 2200 Metern und ist bekannt für seine gut erhaltene Architektur aus dem 18. Jahrhundert. Die Wirtschaft basiert hauptsächlich auf Landwirtschaft, insbesondere dem Anbau von Kartoffeln und Buchweizen.",
      hu: "Shaba falu Bhután nyugati részén, a Paro-völgyben található. A település történelmi jelentősége a Tibet és Bhután közötti kereskedelmi útvonalon való elhelyezkedéséből fakad. A hagyományos bhutáni építészet számos példája megtalálható itt, a házak jellegzetes fafaragásokkal és festett díszítésekkel ékesek. A falu életét a mezőgazdaság határozza meg, fő termények a burgonya és a hajdina.",
      ro: "Shaba este un sat situat în vestul Bhutanului, în valea Paro, la o altitudine de aproximativ 2200 de metri. A fost un important centru comercial pe ruta dintre Tibet și Bhutan. Arhitectura tradițională bhutaneză este bine conservată aici, cu case din lemn și piatră, decorate cu motive religioase. Economia locală se bazează pe agricultură, în special pe cultivarea cartofilor și a hrișcii.",
      en: "Shaba is a village in western Bhutan, located in the Paro Valley at an elevation of about 2,200 meters. Historically, it served as a key stop on the trade route between Tibet and Bhutan. The village features well-preserved traditional architecture, with many houses dating back to the 18th century. The local economy is predominantly agricultural, focusing on potatoes, buckwheat, and dairy farming.",
    },
    factsAdvanced: {
      de: ["Einwohnerzahl: ca. 450", "Höhe: 2200 m über dem Meeresspiegel", "Gründung: 18. Jahrhundert", "Entfernung nach Paro: 12 km", "Durchschnittstemperatur: 15°C im Sommer", "Anbaufläche: 80 ha Kartoffeln"],
      hu: ["Lakosság: kb. 450 fő", "Tengerszint feletti magasság: 2200 m", "Alapítás: 18. század", "Távolság Paro városától: 12 km", "Éves csapadékmennyiség: 800 mm", "Fő termény: burgonya, hajdina"],
      ro: ["Populație: aproximativ 450 de locuitori", "Altitudine: 2200 m", "Fondare: secolul al XVIII-lea", "Distanță până la Paro: 12 km", "Temperatura medie anuală: 10°C", "Suprafață agricolă: 120 ha"],
      en: ["Population: approx. 450", "Elevation: 2,200 m", "Established: 18th century", "Distance to Paro: 12 km", "Annual rainfall: 800 mm", "Main crops: potatoes, buckwheat"],
    },
  },
  {
    id: "bhutan-wangchang-cities-v2",
    type: "village",
    name: { de: "Wangchang", hu: "Wangchang", ro: "Wangchang", en: "Wangchang" },
    coords: [89.421, 27.432],
    description: {
      de: "Wangchang ist eine Gemeinde im Herzen des Paro-Tals. Sie umfasst viele historische Tempel und ist ein Zentrum der lokalen Kultur.",
      hu: "Wangchang egy település a Paro-völgy szívében. Számos történelmi templomot foglal magában és a helyi kultúra központja.",
      ro: "Wangchang este o comunitate în inima văii Paro. Cuprinde multe temple istorice și este un centru al culturii locale.",
      en: "Wangchang is a community in the heart of the Paro Valley. it encompasses many historic temples and is a center of local cultural life.",
    },
    facts: {
      de: ["Liegt im Distrikt Paro", "Beherbergt mehrere kleine Klöster", "Wichtig für die lokale Verwaltung", "Traditionelle Reisterrassen prägen das Bild"],
      hu: ["Paro körzetben található", "Több kisebb kolostornak ad otthont", "Fontos szerepet játszik a helyi közigazgatásban", "Hagyományos rizsteraszok jellemzik"],
      ro: ["Situat în districtul Paro", "Găzduiește mai multe mănăstiri mici", "Important pentru administrația locală", "Terasele de orez tradiționale domină peisajul"],
      en: ["Located in the Paro District", "Home to several small community temples", "Central to the local gewog administration", "Characterized by traditional rice terraces"],
    },
    imageHint: "Wangchang village and Paro valley",
    descriptionAdvanced: {
      de: "Wangchang ist ein kleines Dorf im Haa-Distrikt im Westen Bhutans. Es liegt auf einer Höhe von etwa 3000 Metern und ist bekannt für seine heißen Quellen, die als heilig gelten. Die umliegenden Wälder sind reich an Rhododendren und Heilkräutern. Die Einwohner leben hauptsächlich von der Viehzucht, insbesondere von Yaks.",
      hu: "Wangchang falu Bhután nyugati részén, a Haa körzetben található, mintegy 3000 méteres tengerszint feletti magasságban. Híres termálforrásairól, amelyeket a helyiek szentnek tartanak. A település a tradicionális bhutáni gyógyítás központja. A környező erdőkben ritka orvosi növények és rododendronok nőnek. A gazdaság alapja a jaktenyésztés.",
      ro: "Wangchang este un sat din districtul Haa, vestul Bhutanului, situat la o altitudine de aproximativ 3000 de metri. Este renumit pentru izvoarele termale considerate sacre de localnici. Pădurile din jur sunt bogate în rododendroni și plante medicinale. Economia se bazează pe creșterea iacilor și pe turismul legat de izvoare.",
      en: "Wangchang is a small village in Haa District, western Bhutan, at an elevation of about 3,000 meters. It is renowned for its hot springs, which are considered sacred and are used for traditional healing. The surrounding forests are home to diverse rhododendron species and medicinal herbs. The local economy relies on yak herding and pilgrimage tourism.",
    },
    factsAdvanced: {
      de: ["Einwohner: ca. 200", "Höhe: 3000 m", "Anzahl heißer Quellen: 3", "Wassertemperatur: 40°C", "Jährliche Besucher: 500", "Yak-Bestand: ca. 150 Tiere"],
      hu: ["Lakosság: kb. 200 fő", "Tengerszint feletti magasság: 3000 m", "Termálforrások száma: 3", "Víz hőmérséklete: 40°C", "Éves látogatók: 500", "Jakok száma: 150"],
      ro: ["Populație: aproximativ 200 de locuitori", "Altitudine: 3000 m", "Număr de izvoare termale: 3", "Temperatura apei: 40°C", "Vizitatori anuali: 500", "Număr de iacte: 150"],
      en: ["Population: approx. 200", "Elevation: 3,000 m", "Number of hot springs: 3", "Water temperature: 40°C", "Annual visitors: 500", "Yak population: 150"],
    },
  },
  {
    id: "bhutan-tsento-cities-v2",
    type: "village",
    name: { de: "Tsento", hu: "Tsento", ro: "Tsento", en: "Tsento" },
    coords: [89.381, 27.525],
    description: {
      de: "Tsento liegt im nördlichen Paro-Tal und ist der Ausgangspunkt für Wanderungen zum Jomolhari. Die Region ist für ihre Viehzucht bekannt.",
      hu: "Tsento az északi Paro-völgyben fekszik, és a Jomolhari-túrák kiindulópontja. A régió az állattenyésztéséről ismert.",
      ro: "Tsento este situat în nordul văii Paro și este punctul de plecare pentru drumețiile spre Jomolhari. Regiunea este cunoscută pentru creșterea animalelor.",
      en: "Tsento is located in the northern Paro Valley and serves as the starting point for Jomolhari treks. The region is known for its high-altitude farming.",
    },
    facts: {
      de: ["Liegt im Distrikt Paro", "Tor zum Jomolhari-Trekkingpfad", "Bekannt für Yak- und Viehzucht", "Heimat des Klosters Drugyel Dzong"],
      hu: ["Paro körzetben található", "A Jomolhari túraútvonal kapuja", "Jak- és állattenyésztéséről híres", "A Drugyel Dzong kolostor szomszédságában fekszik"],
      ro: ["Situat în districtul Paro", "Poarta către traseul de trekking Jomolhari", "Cunoscut pentru creșterea iacilor", "În apropiere de situl istoric Drugyel Dzong"],
      en: ["Located in the Paro District", "Gateway to the Jomolhari trekking route", "Renowned for yak and cattle husbandry", "Nearby the historic Drugyel Dzong site"],
    },
    imageHint: "Tsento village landscape in Paro",
    descriptionAdvanced: {
      de: "Tsento ist ein Dorf im Distrikt Chukha im Südwesten Bhutans. Es liegt im Tal des Wangchu-Flusses und ist bekannt für seine Reisfelder und Bambuswälder. Die Gegend hat eine subtropisches Klima mit heißen Sommern. Die traditionelle Webkunst ist hier weit verbreitet.",
      hu: "Tsento falu Bhután délnyugati részén, a Chukha körzetben található, a Wangchu folyó völgyében. A település híres rizsteraszairól és bambuszligeteiről. Az éghajlat szubtrópusi, forró nyarakkal. A kézművesség, különösen a szövés, fontos hagyomány. A lakosság mezőgazdaságból és kézművességből él.",
      ro: "Tsento este un sat în districtul Chukha, sud-vestul Bhutanului, situat în valea râului Wangchu. Este cunoscut pentru terasele de orez și pădurile de bambus. Clima este subtropicală, cu veri calde. Meșteșugul tradițional al țesutului este practicat pe scară largă. Economia se bazează pe agricultură și artizanat.",
      en: "Tsento is a village in Chukha District, southwestern Bhutan, located in the Wangchu River valley. It is known for its terraced rice paddies and extensive bamboo groves. The climate is subtropical, with hot summers and mild winters. Traditional weaving is a significant cultural and economic activity, with local textiles sold in markets.",
    },
    factsAdvanced: {
      de: ["Einwohner: ca. 350", "Höhe: 500 m", "Reisanbaufläche: 60 ha", "Durchschnittstemperatur im Juli: 30°C", "Jährlicher Niederschlag: 1500 mm", "Anzahl der Webstühle: 50"],
      hu: ["Lakosság: kb. 350 fő", "Tengerszint feletti magasság: 500 m", "Rizsföldek területe: 60 ha", "Átlaghőmérséklet júliusban: 30°C", "Éves csapadék: 1500 mm", "Szövőszékek száma: 50"],
      ro: ["Populație: aproximativ 350 de locuitori", "Altitudine: 500 m", "Suprafață orezării: 60 ha", "Temperatura medie în iulie: 30°C", "Precipitații anuale: 1500 mm", "Număr de războaie de țesut: 50"],
      en: ["Population: approx. 350", "Elevation: 500 m", "Rice paddy area: 60 ha", "Average July temperature: 30°C", "Annual rainfall: 1,500 mm", "Number of looms: 50"],
    },
  },
  {
    id: "bhutan-doteng-cities-v2",
    type: "village",
    name: { de: "Doteng", hu: "Doteng", ro: "Doteng", en: "Doteng" },
    coords: [89.352, 27.501],
    description: {
      de: "Doteng ist ein ruhiges Bergdorf im Paro-Tal. Es ist bekannt für seine unberührte Natur und den Anbau von Äpfeln und Gemüse.",
      hu: "Doteng egy csendes hegyi falu a Paro-völgyben. Érintetlen természetéről, valamint alma- és zöldségtermesztéséről ismert.",
      ro: "Doteng este un sat montan liniștit în Valea Paro. Este cunoscut pentru natura sa curată și cultivarea merelor și legumelor.",
      en: "Doteng is a peaceful mountain village in the Paro Valley. It is known for its pristine environment and the cultivation of apples and vegetables.",
    },
    facts: {
      de: ["Liegt im Distrikt Paro", "Beliebtes Ziel für Wanderungen ab Paro", "Wichtiges Zentrum für Bio-Landwirtschaft", "Umgeben von alpiner Landschaft"],
      hu: ["Paro körzetben található", "Népszerű túracélpont Paróból", "A biogazdálkodás fontos központja", "Alpesi táj veszi körül"],
      ro: ["Situat în districtul Paro", "Destinație populară pentru drumeții din Paro", "Centru important pentru agricultura ecologică", "Înconjurat de peisaje alpine"],
      en: ["Located in the Paro District", "Popular destination for hiking from Paro town", "Hub for organic vegetable farming", "Surrounded by high-altitude alpine scenery"],
    },
    imageHint: "Doteng village and mountains",
    descriptionAdvanced: {
      de: "Doteng ist ein kleines Dorf im Samtse-Distrikt im Süden Bhutans, nahe der Grenze zu Indien. Es liegt in einer flachen Ebene auf etwa 300 Metern Höhe und ist ein Zentrum für den Anbau von Kardamom und Arekanüssen. Die Bevölkerung gehört verschiedenen ethnischen Gruppen an, darunter die Lhotshampa.",
      hu: "Doteng falu Bhután déli részén, a Samtse körzetben, az indiai határ közelében fekszik. A település sík területen, mintegy 300 méteres tengerszint feletti magasságban található. A helyi gazdaság a kardamom és arekadió termesztésére épül. A lakosság etnikailag sokszínű, főként lhotshampák lakják.",
      ro: "Doteng este un sat din districtul Samtse, sudul Bhutanului, aproape de granița cu India. Este situat pe o câmpie joasă, la aproximativ 300 de metri altitudine. Economia locală se bazează pe cultivarea cardamomului și a nucilor de arec. Populația este diversă etnic, predominant Lhotshampa.",
      en: "Doteng is a small village in Samtse District, southern Bhutan, near the Indian border. It lies on a lowland plain at about 300 meters elevation. The local economy is centered on cardamom and areca nut cultivation. The population is ethnically diverse, with a majority of Lhotshampa people.",
    },
    factsAdvanced: {
      de: ["Einwohner: ca. 600", "Höhe: 300 m", "Kardamom-Anbaufläche: 40 ha", "Arekanuss-Produktion: 20 Tonnen jährlich", "Durchschnittstemperatur: 25°C", "Ethnische Gruppen: 3"],
      hu: ["Lakosság: kb. 600 fő", "Tengerszint feletti magasság: 300 m", "Kardamom termesztési terület: 40 ha", "Arekadió termelés: 20 tonna évente", "Átlaghőmérséklet: 25°C", "Etnikai csoportok: 3"],
      ro: ["Populație: aproximativ 600 de locuitori", "Altitudine: 300 m", "Suprafață cardamom: 40 ha", "Producție de nuci de arec: 20 tone anual", "Temperatura medie: 25°C", "Grupuri etnice: 3"],
      en: ["Population: approx. 600", "Elevation: 300 m", "Cardamom cultivation area: 40 ha", "Areca nut production: 20 tons/year", "Average temperature: 25°C", "Ethnic groups: 3"],
    },
  },
  {
    id: "bhutan-dogar-cities-v2",
    type: "village",
    name: { de: "Dogar", hu: "Dogar", ro: "Dogar", en: "Dogar" },
    coords: [89.482, 27.302],
    description: {
      de: "Dogar ist eine landwirtschaftlich geprägte Gemeinde im Distrikt Paro. Das Dorf liegt in einer malerischen Gegend mit vielen Obstbäumen.",
      hu: "Dogar egy mezőgazdasági közösség Paro körzetben. A falu festői környezetben fekszik, számos gyümölcsfával körülvéve.",
      ro: "Dogar este o comunitate agricolă în districtul Paro. Satul este situat într-o zonă pitorească, cu mulți pomi fructiferi.",
      en: "Dogar is an agricultural community in the Paro District. The village is situated in a scenic area known for its traditional fruit orchards.",
    },
    facts: {
      de: ["Liegt im Distrikt Paro", "Wichtig für die lokale Nahrungsmittelproduktion", "Bekannt für traditionelle bhutanische Bauernhäuser", "Zentrum eines Gewogs"],
      hu: ["Paro körzetben található", "Fontos szerepet játszik a helyi élelmiszertermelésben", "Hagyományos bhutáni parasztházairól ismert", "Egy kistérség (gewog) központja"],
      ro: ["Situat în districtul Paro", "Important pentru producția locală de alimente", "Cunoscut pentru casele țărănești tradiționale", "Centrul administrativ al unui gewog"],
      en: ["Located in the Paro District", "Vital contributor to local food production", "Notable for well-preserved traditional farmhouses", "Headquarters of a local gewog administration"],
    },
    imageHint: "Dogar village traditional farmhouses",
    descriptionAdvanced: {
      de: "Dogar ist ein Dorf im Bumthang-Distrikt im Zentrum Bhutans. Es liegt im Chamkhar-Tal auf etwa 2600 Metern Höhe und ist bekannt für seine Buchweizenfelder und die traditionelle Bierherstellung. Das Dorf beherbergt ein altes Kloster aus dem 15. Jahrhundert.",
      hu: "Dogar falu Bhután középső részén, a Bumthang körzetben, a Chamkhar-völgyben található, mintegy 2600 méteres magasságban. Híres hajdinaföldjeiről és a helyi sörfőzés hagyományáról. A településen található egy 15. századi kolostor, amely fontos zarándokhely.",
      ro: "Dogar este un sat din districtul Bumthang, centrul Bhutanului, situat în valea Chamkhar la circa 2600 de metri. Este cunoscut pentru culturile de hrișcă și fabricarea tradițională a berii. Satul găzduiește o mănăstire veche din secolul al XV-lea, un important loc de pelerinaj.",
      en: "Dogar is a village in Bumthang District, central Bhutan, located in the Chamkhar Valley at about 2,600 meters. It is known for its buckwheat fields and traditional brewing of local beer. The village houses an ancient monastery dating back to the 15th century, which is a pilgrimage site.",
    },
    factsAdvanced: {
      de: ["Einwohner: ca. 300", "Höhe: 2600 m", "Buchweizen-Anbaufläche: 30 ha", "Bierausstoß: 5000 Liter jährlich", "Kloster: 15. Jahrhundert", "Entfernung zu Jakar: 15 km"],
      hu: ["Lakosság: kb. 300 fő", "Tengerszint feletti magasság: 2600 m", "Hajdina termesztési terület: 30 ha", "Sörtermelés: 5000 liter évente", "Kolostor építése: 15. század", "Távolság Jakar-tól: 15 km"],
      ro: ["Populație: aproximativ 300 de locuitori", "Altitudine: 2600 m", "Suprafață hrișcă: 30 ha", "Producție de bere: 5000 litri/an", "Mănăstire: secolul al XV-lea", "Distanță până la Jakar: 15 km"],
      en: ["Population: approx. 300", "Elevation: 2,600 m", "Buckwheat area: 30 ha", "Beer production: 5,000 liters/year", "Monastery: 15th century", "Distance to Jakar: 15 km"],
    },
  },
  {
    id: "bhutan-naja-cities-v2",
    type: "village",
    name: { de: "Naja", hu: "Naja", ro: "Naja", en: "Naja" },
    coords: [89.501, 27.251],
    description: {
      de: "Naja ist ein hochgelegenes Dorf im Distrikt Paro. Die Einwohner leben hauptsächlich von der Viehzucht und dem Anbau von Berggetreide.",
      hu: "Naja egy magasan fekvő falu Paro körzetben. Lakói főként állattenyésztésből és hegyi gabonafélék termesztéséből élnek.",
      ro: "Naja este un sat de mare altitudine în districtul Paro. Locuitorii trăiesc în principal din creșterea animalelor și cultivarea cerealelor montane.",
      en: "Naja is a high-altitude village in the Paro District. Its residents primarily depend on livestock rearing and mountain grain farming.",
    },
    facts: {
      de: ["Gehört zum Distrikt Paro", "Bekannt für die Produktion von Butter und Käse", "Kaltes Gebirgsklima", "Relativ abgeschiedene Lage"],
      hu: ["Paro körzethez tartozik", "Vaj- és sajttermeléséről ismert", "Hideg hegyvidéki éghajlat jellemzi", "Viszonylag elszigetelt fekvésű"],
      ro: ["Aparține districtului Paro", "Cunoscut pentru producția de unt și brânză", "Climat montan rece", "Locație relativ izolată"],
      en: ["Part of the Paro District", "Well-known for dairy products like butter and cheese", "Experiences a cold high-mountain climate", "Relatively remote and secluded settlement"],
    },
    imageHint: "Naja village high altitude landscape",
    descriptionAdvanced: {
      de: "Naja ist ein Dorf im Mongar-Distrikt im Osten Bhutans. Es liegt auf einem Bergrücken auf etwa 1800 Metern Höhe und ist umgeben von dichten Wäldern mit teak- und sal-Bäumen. Die Region ist bekannt für den Anbau von Ingwer und Kurkuma.",
      hu: "Naja falu Bhután keleti részén, a Mongar körzetben található, egy hegygerincen, mintegy 1800 méteres magasságban. Körülötte sűrű erdők, tikfa és szálfa nő. A térség híres a gyömbér és kurkuma termesztéséről. A falu népszerű túraútvonalak kiindulópontja.",
      ro: "Naja este un sat din districtul Mongar, estul Bhutanului, situat pe o creastă muntoasă la aproximativ 1800 de metri. Este înconjurat de păduri dese cu tec și sal. Regiunea este cunoscută pentru cultivarea ghimbirului și a turmericului. Satul este un punct de plecare pentru drumeții.",
      en: "Naja is a village in Mongar District, eastern Bhutan, perched on a mountain ridge at about 1,800 meters. It is surrounded by dense forests of teak and sal trees. The area is known for ginger and turmeric cultivation. The village serves as a starting point for several trekking routes.",
    },
    factsAdvanced: {
      de: ["Einwohner: ca. 250", "Höhe: 1800 m", "Ingwerproduktion: 30 Tonnen jährlich", "Waldfläche: 500 ha", "Durchschnittstemperatur: 20°C", "Trekkingrouten: 2"],
      hu: ["Lakosság: kb. 250 fő", "Tengerszint feletti magasság: 1800 m", "Gyömbér termelés: 30 tonna évente", "Erdőterület: 500 ha", "Átlaghőmérséklet: 20°C", "Túraútvonalak: 2"],
      ro: ["Populație: aproximativ 250 de locuitori", "Altitudine: 1800 m", "Producție ghimbir: 30 tone/an", "Suprafață pădure: 500 ha", "Temperatura medie: 20°C", "Trasee de drumeție: 2"],
      en: ["Population: approx. 250", "Elevation: 1,800 m", "Ginger production: 30 tons/year", "Forest area: 500 ha", "Average temperature: 20°C", "Trekking routes: 2"],
    },
  },
  {
    id: "bhutan-genekha-cities-v2",
    type: "village",
    name: { de: "Genekha", hu: "Genekha", ro: "Genekha", en: "Genekha" },
    coords: [89.652, 27.305],
    description: {
      de: "Genekha ist ein berühmtes Dorf im Distrikt Thimphu, bekannt für seine Matsutake-Pilze. Jährlich findet hier ein bekanntes Pilzfestival statt.",
      hu: "Genekha egy híres falu Thimphu körzetben, amely matsutake gombáiról ismert. Évente népszerű gombafesztiválnak ad otthont.",
      ro: "Genekha este un sat faimos în districtul Thimphu, cunoscut pentru ciupercile Matsutake. Găzduiește anual un festival popular al ciupercilor.",
      en: "Genekha is a famous village in the Thimphu District, renowned for its Matsutake mushrooms. It hosts an annual mushroom festival.",
    },
    facts: {
      de: ["Hauptzentrum für Matsutake-Pilze in Bhutan", "Veranstaltungsort des jährlichen Pilzfestivals", "Liegt im Distrikt Thimphu", "Wichtiges Ziel für Pilzsammler"],
      hu: ["Bhután matsutake gomba-központja", "Az éves gombafesztivál helyszíne", "Thimphu körzetben található", "A gombagyűjtők fontos célpontja"],
      ro: ["Principalul centru pentru ciuperci Matsutake din Bhutan", "Locul de desfășurare a festivalului anual al ciupercilor", "Situat în districtul Thimphu", "Destinație importantă pentru culegătorii de ciuperci"],
      en: ["Main center for Matsutake mushroom harvesting", "Venue for the popular annual Mushroom Festival", "Located in the Thimphu District", "Key destination for foraging and mycological tourism"],
    },
    imageHint: "Genekha village and mushroom festival",
    descriptionAdvanced: {
      de: "Genekha ist ein Dorf im Paro-Distrikt, etwa 20 Kilometer nordöstlich von Paro. Es liegt auf einer Höhe von 2800 Metern und ist bekannt für seine alten Weiden und die Produktion von Butter und Käse. Die Einwohner sind hauptsächlich Hirten, die Yaks und Schafe züchten.",
      hu: "Genekha falu Bhután nyugati részén, a Paro körzetben, Paro városától mintegy 20 km-re északkeletre található. 2800 méteres magasságban fekszik, híres legelőiről, vaj- és sajttermeléséről. A lakosság pásztorkodással foglalkozik, juhokat és jakokat tartanak.",
      ro: "Genekha este un sat din districtul Paro, vestul Bhutanului, la aproximativ 20 km nord-est de Paro. Este situat la 2800 de metri altitudine, cunoscut pentru pășunile sale și producția de unt și brânză. Localnicii sunt în principal păstori de iaci și oi.",
      en: "Genekha is a village in Paro District, about 20 kilometers northeast of Paro town. It sits at an elevation of 2,800 meters and is known for its alpine pastures and traditional dairy products like butter and cheese. The inhabitants are primarily yak herders and sheep farmers.",
    },
    factsAdvanced: {
      de: ["Einwohner: ca. 180", "Höhe: 2800 m", "Käseproduktion: 5 Tonnen jährlich", "Yak-Bestand: 120", "Schafbestand: 200", "Entfernung zu Paro: 20 km"],
      hu: ["Lakosság: kb. 180 fő", "Tengerszint feletti magasság: 2800 m", "Sajttermelés: 5 tonna évente", "Jakok száma: 120", "Juhok száma: 200", "Távolság Paro-tól: 20 km"],
      ro: ["Populație: aproximativ 180 de locuitori", "Altitudine: 2800 m", "Producție de brânză: 5 tone/an", "Număr de iaci: 120", "Număr de oi: 200", "Distanță până la Paro: 20 km"],
      en: ["Population: approx. 180", "Elevation: 2,800 m", "Cheese production: 5 tons/year", "Yak population: 120", "Sheep population: 200", "Distance to Paro: 20 km"],
    },
  },
  {
    id: "bhutan-kamji-cities-v2",
    type: "village",
    name: { de: "Kamji", hu: "Kamji", ro: "Kamji", en: "Kamji" },
    coords: [89.481, 26.982],
    description: {
      de: "Kamji ist eine kleine Siedlung an der Autobahn zwischen Phuentsholing und Thimphu. Sie dient als wichtiger Halt für Lkw-Fahrer.",
      hu: "Kamji egy kis település a Phuentsholing és Thimphu közötti főút mentén. Fontos megállóhely a teherautó-sofőrök számára.",
      ro: "Kamji este o mică așezare pe autostrada dintre Phuentsholing și Thimphu. Servește ca un punct de oprire important pentru șoferi.",
      en: "Kamji is a small settlement on the highway between Phuentsholing and Thimphu. It serves as a vital rest stop for truckers and travelers.",
    },
    facts: {
      de: ["Liegt im Distrikt Chhukha", "Wichtiger Verpflegungshalt am Highway", "Umgeben von steilen Bergen", "Bekannt für lokale Straßenrestaurants"],
      hu: ["Chhukha körzetben található", "Fontos étkezési megálló a főút mentén", "Meredek hegyek veszik körül", "Helyi útmenti éttermeiről ismert"],
      ro: ["Situat în districtul Chhukha", "Punct de oprire important pentru masă pe autostradă", "Înconjurat de munți abrupți", "Cunoscut pentru restaurantele locale de la marginea drumului"],
      en: ["Located in the Chhukha District", "Key refreshment stop on the national highway", "Surrounded by steep, forested mountains", "Known for its local roadside eateries"],
    },
    imageHint: "Kamji village and highway traffic",
    descriptionAdvanced: {
      de: "Kamji ist ein Dorf im Haa-Distrikt, bekannt für seinen traditionellen Tanz, den Kamji Cham. Das Dorf liegt auf etwa 2700 Metern Höhe und ist umgeben von Kiefernwäldern. Die Bewohner bauen hauptsächlich Gerste an und betreiben Weberei.",
      hu: "Kamji falu Bhután nyugati részén, a Haa körzetben található, mintegy 2700 méteres magasságban. Híres a hagyományos Kamji Cham táncról. A települést fenyőerdők veszik körül. A lakosok árpát termesztenek és szövéssel foglalkoznak.",
      ro: "Kamji este un sat din districtul Haa, vestul Bhutanului, situat la circa 2700 de metri altitudine. Este renumit pentru dansul tradițional Kamji Cham. Satul este înconjurat de păduri de pini. Locuitorii cultivă orz și practică țesutul.",
      en: "Kamji is a village in Haa District, known for its traditional Kamji Cham dance. It lies at about 2,700 meters and is surrounded by pine forests. The main economic activities are barley cultivation and weaving.",
    },
    factsAdvanced: {
      de: ["Einwohner: ca. 220", "Höhe: 2700 m", "Gerstenanbaufläche: 25 ha", "Anzahl der Tänzer: 30", "Waldfläche: 300 ha", "Entfernung zu Haa: 10 km"],
      hu: ["Lakosság: kb. 220 fő", "Tengerszint feletti magasság: 2700 m", "Árpa termesztési terület: 25 ha", "Táncosok száma: 30", "Erdőterület: 300 ha", "Távolság Haa-tól: 10 km"],
      ro: ["Populație: aproximativ 220 de locuitori", "Altitudine: 2700 m", "Suprafață orz: 25 ha", "Număr dansatori: 30", "Suprafață pădure: 300 ha", "Distanță până la Haa: 10 km"],
      en: ["Population: approx. 220", "Elevation: 2,700 m", "Barley area: 25 ha", "Number of dancers: 30", "Forest area: 300 ha", "Distance to Haa: 10 km"],
    },
  },
  {
    id: "bhutan-bongo-cities-v2",
    type: "village",
    name: { de: "Bongo", hu: "Bongo", ro: "Bongo", en: "Bongo" },
    coords: [89.485, 27.021],
    description: {
      de: "Bongo ist ein geschichtsträchtiges Dorf im Distrikt Chhukha. Es ist bekannt für seine strategische Lage und traditionelle Landwirtschaft.",
      hu: "Bongo egy történelmi múltú falu Chhukha körzetben. Stratégiai fekvéséről és hagyományos mezőgazdaságáról ismert.",
      ro: "Bongo este un sat încărcat de istorie în districtul Chhukha. Este cunoscut pentru locația sa strategică și agricultura tradițională.",
      en: "Bongo is a historic village in the Chhukha District. It is known for its strategic highland location and traditional farming practices.",
    },
    facts: {
      de: ["Verwaltungssitz des Bongo-Gewogs", "Traditionelle bhutanische Lebensweise", "Liegt im Distrikt Chhukha", "Einer der ältesten Orte der Region"],
      hu: ["A Bongo kistérség közigazgatási központja", "Hagyományos bhutáni életmód jellemzi", "Chhukha körzetben található", "A régió egyik legrégebbi települése"],
      ro: ["Centru administrativ al gewog-ului Bongo", "Stil de viață tradițional bhutanez", "Situat în districtul Chhukha", "Una dintre cele mai vechi așezări din regiune"],
      en: ["Administrative center for the Bongo gewog", "Preserves traditional Bhutanese lifestyle", "Located in the Chhukha District", "One of the oldest settlements in the region"],
    },
    imageHint: "Bongo village and terraced fields",
    descriptionAdvanced: {
      de: "Bongo ist ein Dorf im Distrikt Chukha, bekannt für seine Apfelplantagen und die Produktion von Apfelsaft. Es liegt auf etwa 1800 Metern Höhe. Das Dorf hat eine Grundschule und ein kleines Krankenhaus.",
      hu: "Bongo falu Bhután délnyugati részén, a Chukha körzetben található, mintegy 1800 méteres magasságban. Híres almaültetvényeiről és almalé-termeléséről. A településen általános iskola és kis kórház működik.",
      ro: "Bongo este un sat în districtul Chukha, sud-vestul Bhutanului, situat la aproximativ 1800 de metri altitudine. Este cunoscut pentru livezile de meri și producția de suc de mere. Satul are o școală primară și un mic spital.",
      en: "Bongo is a village in Chukha District, southwestern Bhutan, at an elevation of about 1,800 meters. It is known for its apple orchards and apple juice production. The village has a primary school and a small health center.",
    },
    factsAdvanced: {
      de: ["Einwohner: ca. 400", "Höhe: 1800 m", "Apfelproduktion: 100 Tonnen jährlich", "Apfelsaftproduktion: 50.000 Liter", "Schule: 1", "Krankenhaus: 1"],
      hu: ["Lakosság: kb. 400 fő", "Tengerszint feletti magasság: 1800 m", "Alma termelés: 100 tonna évente", "Almalé termelés: 50.000 liter", "Iskola: 1", "Kórház: 1"],
      ro: ["Populație: aproximativ 400 de locuitori", "Altitudine: 1800 m", "Producție de mere: 100 tone/an", "Producție suc de mere: 50.000 litri", "Școală: 1", "Spital: 1"],
      en: ["Population: approx. 400", "Elevation: 1,800 m", "Apple production: 100 tons/year", "Apple juice production: 50,000 liters", "Schools: 1", "Health centers: 1"],
    },
  }
];
