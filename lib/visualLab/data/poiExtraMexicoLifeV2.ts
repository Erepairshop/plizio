// @ts-nocheck
import type { POI } from "./poi";

export const poiExtraMexicoLifeV2: POI[] = [
  {
    id: "chapultepec-zoo-life-v2", "sights": {
      "de": [
            {
                  "name": "Papalote•Museo del Niño",
                  "text": "Das Papalote Museo del Niño ist ein Museum in Chapultepec Zoo Life v2, eines der größten Kindermuseen Lateinamerikas.",
                  "category": "museum",
                  "coords": [
                        -99.19463,
                        19.411946
                  ]
            },
            {
                  "name": "Casa Luis Barragán",
                  "text": "Die Casa Luis Barragán ist ein Museum in Chapultepec Zoo Life v2, ein UNESCO-Weltkulturerbe als ehemaliges Wohnhaus des Architekten Luis Barragán.",
                  "category": "museum",
                  "coords": [
                        -99.192433,
                        19.411114
                  ]
            },
            {
                  "name": "Casa del Lago UNAM",
                  "text": "Die Casa del Lago UNAM ist ein Museum in Chapultepec Zoo Life v2, ein Kulturzentrum am See im Chapultepec-Park.",
                  "category": "museum",
                  "coords": [
                        -99.185836,
                        19.42237
                  ]
            },
            {
                  "name": "Complejo Cultural Los Pinos",
                  "text": "Der Complejo Cultural Los Pinos ist ein Museum in Chapultepec Zoo Life v2, die ehemalige Präsidentenresidenz, heute ein Kulturkomplex.",
                  "category": "museum",
                  "coords": [
                        -99.191481,
                        19.415573
                  ]
            },
            {
                  "name": "Auditorio Nacional",
                  "text": "Das Auditorio Nacional ist ein Theater in Chapultepec Zoo Life v2, einer der größten Konzertsäle der Welt.",
                  "category": "cultural",
                  "coords": [
                        -99.19484,
                        19.424806
                  ]
            },
            {
                  "name": "Cárcamo de Dolores",
                  "text": "Der Cárcamo de Dolores ist eine Attraktion in Chapultepec Zoo Life v2, mit Wandmalereien von Diego Rivera.",
                  "category": "landmark",
                  "coords": [
                        -99.197989,
                        19.412849
                  ]
            },
            {
                  "name": "Zona arqueológica de Mixcoac",
                  "text": "Die Zona arqueológica de Mixcoac ist eine Attraktion in Chapultepec Zoo Life v2, eine archäologische Stätte einer vorspanischen Siedlung.",
                  "category": "landmark",
                  "coords": [
                        -99.189851,
                        19.386303
                  ]
            },
            {
                  "name": "Polyforum Cultural Siqueiros",
                  "text": "Das Polyforum Cultural Siqueiros ist eine Attraktion in Chapultepec Zoo Life v2, ein Kulturzentrum mit Wandmalereien von David Alfaro Siqueiros.",
                  "category": "landmark",
                  "coords": [
                        -99.17344,
                        19.393316
                  ]
            },
            {
                  "name": "Parque Lira",
                  "text": "Der Parque Lira ist ein Park in Chapultepec Zoo Life v2, ein öffentlicher Park in der Gegend.",
                  "category": "park",
                  "coords": [
                        -99.189934,
                        19.405958
                  ]
            },
            {
                  "name": "Aztlán Parque Urbano",
                  "text": "Der Aztlán Parque Urbano ist ein Themenpark in Chapultepec Zoo Life v2, ein Vergnügungspark mit Fahrgeschäften und Attraktionen.",
                  "category": "recreational",
                  "coords": [
                        -99.195634,
                        19.416062
                  ]
            },
            {
                  "name": "Casa Presidencial Lázaro Cárdenas",
                  "text": "Die Casa Presidencial Lázaro Cárdenas ist ein Museum in Chapultepec Zoo Life v2, das dem ehemaligen Präsidenten gewidmet ist.",
                  "category": "museum",
                  "coords": [
                        -99.191433,
                        19.41383
                  ]
            },
            {
                  "name": "Zoológico de Chapultepec",
                  "text": "Der Zoológico de Chapultepec ist ein Zoo in Chapultepec Zoo Life v2, der viele Tierarten beherbergt.",
                  "category": "recreational",
                  "coords": [
                        -99.189527,
                        19.422993
                  ]
            },
            {
                  "name": "LagoAlgo",
                  "text": "LagoAlgo Museum in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.199773,
                        19.417143
                  ]
            },
            {
                  "name": "Galería Enrique Guerrero",
                  "text": "Galería Enrique Guerrero (Kunstgalerie) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.185524,
                        19.411015
                  ]
            },
            {
                  "name": "Patricia Conde Galería",
                  "text": "Patricia Conde Galería (Kunstgalerie) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.185034,
                        19.412771
                  ]
            },
            {
                  "name": "Parque Galería",
                  "text": "Parque Galería (Parkgalerie) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.183736,
                        19.41564
                  ]
            },
            {
                  "name": "Galería de Arte Mexicano GAM",
                  "text": "Galería de Arte Mexicano GAM (Galerie für mexikanische Kunst) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.186982,
                        19.413308
                  ]
            },
            {
                  "name": "Cencalli",
                  "text": "Museum Cencalli im Zoo, eröffnet im September 2021.",
                  "category": "museum",
                  "coords": [
                        -99.192028,
                        19.417172
                  ]
            },
            {
                  "name": "Bernardini",
                  "text": "Die Bernardini ist eine Galerie in Chapultepec Zoo Life v2.",
                  "category": "museum",
                  "coords": [
                        -99.191611,
                        19.429424
                  ]
            },
            {
                  "name": "Museo de la Policía Federal",
                  "text": "Museo de la Policía Federal (Museum der Bundespolizei) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.218055,
                        19.393062
                  ]
            },
            {
                  "name": "Espacio CDMX",
                  "text": "Der Espacio CDMX ist eine Galerie in Chapultepec Zoo Life v2.",
                  "category": "museum",
                  "coords": [
                        -99.202804,
                        19.41119
                  ]
            },
            {
                  "name": "Museo Nacional de la Cartografía",
                  "text": "Museo Nacional de la Cartografía (Nationalmuseum für Kartographie) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.191798,
                        19.404457
                  ]
            },
            {
                  "name": "Museo de Historia Natural y Cultura Ambiental",
                  "text": "Naturkundemuseum im Zoo, gegründet 1964.",
                  "category": "museum",
                  "coords": [
                        -99.2015,
                        19.409971
                  ]
            },
            {
                  "name": "Mapoteca Manuel Orozco y Berra",
                  "text": "Mapoteca Manuel Orozco y Berra (Kartenbibliothek) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.196764,
                        19.404265
                  ]
            },
            {
                  "name": "Museo de Geofísica de la UNAM",
                  "text": "Museo de Geofísica de la UNAM (Geophysikmuseum der UNAM) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.195251,
                        19.404514
                  ]
            },
            {
                  "name": "Centro de Cultura Ambiental",
                  "text": "Umweltkulturzentrum eröffnet 2023, entworfen von Rafael Ponce Ortiz, im Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.198258,
                        19.410848
                  ]
            },
            {
                  "name": "Museo Casa de la Bola",
                  "text": "Museo Casa de la Bola (Haus des Balls Museum) in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.188831,
                        19.404728
                  ]
            },
            {
                  "name": "Casa Gilardi, Luis Barragán 1976",
                  "text": "Casa Gilardi, ein Haus von Architekt Luis Barragán von 1976, in Zooviertel.",
                  "category": "museum",
                  "coords": [
                        -99.186235,
                        19.41424
                  ]
            },
            {
                  "name": "FUENTE",
                  "text": "FUENTE (historischer Brunnen) in Zooviertel.",
                  "category": "historical",
                  "coords": [
                        -99.203921,
                        19.387904
                  ]
            },
            {
                  "name": "Tomáš Garrigue Masaryk 1850-1937",
                  "text": "Das Denkmal Tomáš Garrigue Masaryk 1850-1937 ist ein Denkmal in Chapultepec Zoo Life v2.",
                  "category": "historical",
                  "coords": [
                        -99.191177,
                        19.431534
                  ]
            },
            {
                  "name": "Molino de Santo Domingo",
                  "text": "Der Molino de Santo Domingo ist ein Denkmal in Chapultepec Zoo Life v2.",
                  "category": "historical",
                  "coords": [
                        -99.193917,
                        19.402823
                  ]
            },
            {
                  "name": "Auditorio Adrián Gibert",
                  "text": "Das Auditorio Adrián Gibert ist ein Theater in Chapultepec Zoo Life v2.",
                  "category": "cultural",
                  "coords": [
                        -99.181253,
                        19.407138
                  ]
            },
            {
                  "name": "Teatro Hipódromo Condesa",
                  "text": "Teatro Hipódromo Condesa (Theater im Stadtteil Condesa) in Zooviertel.",
                  "category": "cultural",
                  "coords": [
                        -99.18468,
                        19.406563
                  ]
            },
            {
                  "name": "Centro Cultural del Bosque",
                  "text": "Centro Cultural del Bosque (Kulturzentrum im Wald) in Zooviertel.",
                  "category": "cultural",
                  "coords": [
                        -99.195433,
                        19.423727
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Papalote•Museo del Niño",
                  "text": "A Papalote Museo del Niño egy múzeum a Chapultepec Zoo Life v2-ben, Latin-Amerika egyik legnagyobb gyermekmúzeuma.",
                  "category": "museum",
                  "coords": [
                        -99.19463,
                        19.411946
                  ]
            },
            {
                  "name": "Casa Luis Barragán",
                  "text": "A Casa Luis Barragán egy múzeum a Chapultepec Zoo Life v2-ben, UNESCO világörökségi helyszín, Luis Barragán építész egykori otthona.",
                  "category": "museum",
                  "coords": [
                        -99.192433,
                        19.411114
                  ]
            },
            {
                  "name": "Casa del Lago UNAM",
                  "text": "A Casa del Lago UNAM egy múzeum a Chapultepec Zoo Life v2-ben, egy kulturális központ a Chapultecep Park tavánál.",
                  "category": "museum",
                  "coords": [
                        -99.185836,
                        19.42237
                  ]
            },
            {
                  "name": "Complejo Cultural Los Pinos",
                  "text": "A Complejo Cultural Los Pinos egy múzeum a Chapultepec Zoo Life v2-ben, egykori elnöki rezidencia, ma kulturális komplexum.",
                  "category": "museum",
                  "coords": [
                        -99.191481,
                        19.415573
                  ]
            },
            {
                  "name": "Auditorio Nacional",
                  "text": "Az Auditorio Nacional egy színház a Chapultepec Zoo Life v2-ben, a világ egyik legnagyobb koncertterme.",
                  "category": "cultural",
                  "coords": [
                        -99.19484,
                        19.424806
                  ]
            },
            {
                  "name": "Cárcamo de Dolores",
                  "text": "A Cárcamo de Dolores egy látványosság a Chapultepec Zoo Life v2-ben, Diego Rivera falfestményeivel.",
                  "category": "landmark",
                  "coords": [
                        -99.197989,
                        19.412849
                  ]
            },
            {
                  "name": "Zona arqueológica de Mixcoac",
                  "text": "A Zona arqueológica de Mixcoac egy látványosság a Chapultepec Zoo Life v2-ben, egy prehispán település régészeti lelőhelye.",
                  "category": "landmark",
                  "coords": [
                        -99.189851,
                        19.386303
                  ]
            },
            {
                  "name": "Polyforum Cultural Siqueiros",
                  "text": "A Polyforum Cultural Siqueiros egy látványosság a Chapultepec Zoo Life v2-ben, David Alfaro Siqueiros falfestményeivel rendelkező kulturális központ.",
                  "category": "landmark",
                  "coords": [
                        -99.17344,
                        19.393316
                  ]
            },
            {
                  "name": "Parque Lira",
                  "text": "A Parque Lira egy park a Chapultepec Zoo Life v2-ben, egy nyilvános park a környéken.",
                  "category": "park",
                  "coords": [
                        -99.189934,
                        19.405958
                  ]
            },
            {
                  "name": "Aztlán Parque Urbano",
                  "text": "Az Aztlán Parque Urbano egy vidámpark a Chapultepec Zoo Life v2-ben, játékokkal és attrakciókkal rendelkező szórakoztató park.",
                  "category": "recreational",
                  "coords": [
                        -99.195634,
                        19.416062
                  ]
            },
            {
                  "name": "Casa Presidencial Lázaro Cárdenas",
                  "text": "A Casa Presidencial Lázaro Cárdenas egy múzeum a Chapultepec Zoo Life v2-ben, amely az egykori elnöknek állít emléket.",
                  "category": "museum",
                  "coords": [
                        -99.191433,
                        19.41383
                  ]
            },
            {
                  "name": "Zoológico de Chapultepec",
                  "text": "A Zoológico de Chapultepec egy állatkert a Chapultepec Zoo Life v2-ben, számos állatfaj otthona.",
                  "category": "recreational",
                  "coords": [
                        -99.189527,
                        19.422993
                  ]
            },
            {
                  "name": "LagoAlgo",
                  "text": "LagoAlgo múzeum az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.199773,
                        19.417143
                  ]
            },
            {
                  "name": "Galería Enrique Guerrero",
                  "text": "Galería Enrique Guerrero (művészeti galéria) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.185524,
                        19.411015
                  ]
            },
            {
                  "name": "Patricia Conde Galería",
                  "text": "Patricia Conde Galería (művészeti galéria) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.185034,
                        19.412771
                  ]
            },
            {
                  "name": "Parque Galería",
                  "text": "Parque Galería (park galéria) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.183736,
                        19.41564
                  ]
            },
            {
                  "name": "Galería de Arte Mexicano GAM",
                  "text": "Galería de Arte Mexicano GAM (mexikói művészeti galéria) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.186982,
                        19.413308
                  ]
            },
            {
                  "name": "Cencalli",
                  "text": "Cencalli múzeum az Állatkertben, 2021 szeptemberében nyílt meg.",
                  "category": "museum",
                  "coords": [
                        -99.192028,
                        19.417172
                  ]
            },
            {
                  "name": "Bernardini",
                  "text": "A Bernardini egy galéria a Chapultepec Zoo Life v2-ben.",
                  "category": "museum",
                  "coords": [
                        -99.191611,
                        19.429424
                  ]
            },
            {
                  "name": "Museo de la Policía Federal",
                  "text": "Museo de la Policía Federal (Szövetségi Rendőrség Múzeuma) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.218055,
                        19.393062
                  ]
            },
            {
                  "name": "Espacio CDMX",
                  "text": "Az Espacio CDMX egy galéria a Chapultepec Zoo Life v2-ben.",
                  "category": "museum",
                  "coords": [
                        -99.202804,
                        19.41119
                  ]
            },
            {
                  "name": "Museo Nacional de la Cartografía",
                  "text": "Museo Nacional de la Cartografía (Nemzeti Térképészeti Múzeum) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.191798,
                        19.404457
                  ]
            },
            {
                  "name": "Museo de Historia Natural y Cultura Ambiental",
                  "text": "Természetrajzi múzeum az Állatkertben, alapítva 1964-ben.",
                  "category": "museum",
                  "coords": [
                        -99.2015,
                        19.409971
                  ]
            },
            {
                  "name": "Mapoteca Manuel Orozco y Berra",
                  "text": "Mapoteca Manuel Orozco y Berra (térképkönyvtár) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.196764,
                        19.404265
                  ]
            },
            {
                  "name": "Museo de Geofísica de la UNAM",
                  "text": "Museo de Geofísica de la UNAM (az UNAM Geofizikai Múzeuma) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.195251,
                        19.404514
                  ]
            },
            {
                  "name": "Centro de Cultura Ambiental",
                  "text": "Környezeti kultúraközpont 2023-ban nyílt meg, Rafael Ponce Ortiz tervezte, az Állatkertben.",
                  "category": "museum",
                  "coords": [
                        -99.198258,
                        19.410848
                  ]
            },
            {
                  "name": "Museo Casa de la Bola",
                  "text": "Museo Casa de la Bola (labdaház múzeum) az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.188831,
                        19.404728
                  ]
            },
            {
                  "name": "Casa Gilardi, Luis Barragán 1976",
                  "text": "Casa Gilardi, Luis Barragán építész 1976-os háza az Állatkert közelében.",
                  "category": "museum",
                  "coords": [
                        -99.186235,
                        19.41424
                  ]
            },
            {
                  "name": "FUENTE",
                  "text": "FUENTE (történelmi szökőkút) az Állatkert közelében.",
                  "category": "historical",
                  "coords": [
                        -99.203921,
                        19.387904
                  ]
            },
            {
                  "name": "Tomáš Garrigue Masaryk 1850-1937",
                  "text": "A Tomáš Garrigue Masaryk 1850-1937 emlékmű egy emlékmű a Chapultepec Zoo Life v2-ben.",
                  "category": "historical",
                  "coords": [
                        -99.191177,
                        19.431534
                  ]
            },
            {
                  "name": "Molino de Santo Domingo",
                  "text": "A Molino de Santo Domingo egy emlékmű a Chapultepec Zoo Life v2-ben.",
                  "category": "historical",
                  "coords": [
                        -99.193917,
                        19.402823
                  ]
            },
            {
                  "name": "Auditorio Adrián Gibert",
                  "text": "Az Auditorio Adrián Gibert egy színház a Chapultepec Zoo Life v2-ben.",
                  "category": "cultural",
                  "coords": [
                        -99.181253,
                        19.407138
                  ]
            },
            {
                  "name": "Teatro Hipódromo Condesa",
                  "text": "Teatro Hipódromo Condesa (színház a Condesa negyedben) az Állatkert közelében.",
                  "category": "cultural",
                  "coords": [
                        -99.18468,
                        19.406563
                  ]
            },
            {
                  "name": "Centro Cultural del Bosque",
                  "text": "Centro Cultural del Bosque (erdei kulturális központ) az Állatkert közelében.",
                  "category": "cultural",
                  "coords": [
                        -99.195433,
                        19.423727
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Papalote•Museo del Niño",
                  "text": "Papalote Museo del Niño este un muzeu în Chapultepec Zoo Life v2, unul dintre cele mai mari muzee pentru copii din America Latină.",
                  "category": "museum",
                  "coords": [
                        -99.19463,
                        19.411946
                  ]
            },
            {
                  "name": "Casa Luis Barragán",
                  "text": "Casa Luis Barragán este un muzeu în Chapultepec Zoo Life v2, un sit UNESCO ca fosta casă a arhitectului Luis Barragán.",
                  "category": "museum",
                  "coords": [
                        -99.192433,
                        19.411114
                  ]
            },
            {
                  "name": "Casa del Lago UNAM",
                  "text": "Casa del Lago UNAM este un muzeu în Chapultepec Zoo Life v2, un centru cultural pe lacul din Parcul Chapultepec.",
                  "category": "museum",
                  "coords": [
                        -99.185836,
                        19.42237
                  ]
            },
            {
                  "name": "Complejo Cultural Los Pinos",
                  "text": "Complejo Cultural Los Pinos este un muzeu în Chapultepec Zoo Life v2, fosta reședință prezidențială, acum un complex cultural.",
                  "category": "museum",
                  "coords": [
                        -99.191481,
                        19.415573
                  ]
            },
            {
                  "name": "Auditorio Nacional",
                  "text": "Auditorio Nacional este un teatru în Chapultepec Zoo Life v2, una dintre cele mai mari săli de concerte din lume.",
                  "category": "cultural",
                  "coords": [
                        -99.19484,
                        19.424806
                  ]
            },
            {
                  "name": "Cárcamo de Dolores",
                  "text": "Cárcamo de Dolores este o atracție în Chapultepec Zoo Life v2, cu picturi murale de Diego Rivera.",
                  "category": "landmark",
                  "coords": [
                        -99.197989,
                        19.412849
                  ]
            },
            {
                  "name": "Zona arqueológica de Mixcoac",
                  "text": "Zona arqueológica de Mixcoac este o atracție în Chapultepec Zoo Life v2, un sit arheologic al unei așezări prehispanice.",
                  "category": "landmark",
                  "coords": [
                        -99.189851,
                        19.386303
                  ]
            },
            {
                  "name": "Polyforum Cultural Siqueiros",
                  "text": "Polyforum Cultural Siqueiros este o atracție în Chapultepec Zoo Life v2, un centru cultural cu picturi murale de David Alfaro Siqueiros.",
                  "category": "landmark",
                  "coords": [
                        -99.17344,
                        19.393316
                  ]
            },
            {
                  "name": "Parque Lira",
                  "text": "Parque Lira este un parc în Chapultepec Zoo Life v2, un parc public în zonă.",
                  "category": "park",
                  "coords": [
                        -99.189934,
                        19.405958
                  ]
            },
            {
                  "name": "Aztlán Parque Urbano",
                  "text": "Aztlán Parque Urbano este un parc tematic în Chapultepec Zoo Life v2, un parc de distracții cu atracții.",
                  "category": "recreational",
                  "coords": [
                        -99.195634,
                        19.416062
                  ]
            },
            {
                  "name": "Casa Presidencial Lázaro Cárdenas",
                  "text": "Casa Presidencial Lázaro Cárdenas este un muzeu în Chapultepec Zoo Life v2, dedicat fostului președinte.",
                  "category": "museum",
                  "coords": [
                        -99.191433,
                        19.41383
                  ]
            },
            {
                  "name": "Zoológico de Chapultepec",
                  "text": "Zoológico de Chapultepec este o grădină zoologică în Chapultepec Zoo Life v2, găzduind multe specii de animale.",
                  "category": "recreational",
                  "coords": [
                        -99.189527,
                        19.422993
                  ]
            },
            {
                  "name": "LagoAlgo",
                  "text": "Muzeul LagoAlgo lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.199773,
                        19.417143
                  ]
            },
            {
                  "name": "Galería Enrique Guerrero",
                  "text": "Galería Enrique Guerrero (galerie de artă) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.185524,
                        19.411015
                  ]
            },
            {
                  "name": "Patricia Conde Galería",
                  "text": "Patricia Conde Galería (galerie de artă) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.185034,
                        19.412771
                  ]
            },
            {
                  "name": "Parque Galería",
                  "text": "Parque Galería (galerie-parc) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.183736,
                        19.41564
                  ]
            },
            {
                  "name": "Galería de Arte Mexicano GAM",
                  "text": "Galería de Arte Mexicano GAM (galerie de artă mexicană) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.186982,
                        19.413308
                  ]
            },
            {
                  "name": "Cencalli",
                  "text": "Muzeul Cencalli din Zoo, deschis în septembrie 2021.",
                  "category": "museum",
                  "coords": [
                        -99.192028,
                        19.417172
                  ]
            },
            {
                  "name": "Bernardini",
                  "text": "Bernardini este o galerie în Chapultepec Zoo Life v2.",
                  "category": "museum",
                  "coords": [
                        -99.191611,
                        19.429424
                  ]
            },
            {
                  "name": "Museo de la Policía Federal",
                  "text": "Museo de la Policía Federal (Muzeul Poliției Federale) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.218055,
                        19.393062
                  ]
            },
            {
                  "name": "Espacio CDMX",
                  "text": "Espacio CDMX este o galerie în Chapultepec Zoo Life v2.",
                  "category": "museum",
                  "coords": [
                        -99.202804,
                        19.41119
                  ]
            },
            {
                  "name": "Museo Nacional de la Cartografía",
                  "text": "Museo Nacional de la Cartografía (Muzeul Național de Cartografie) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.191798,
                        19.404457
                  ]
            },
            {
                  "name": "Museo de Historia Natural y Cultura Ambiental",
                  "text": "Muzeul de Istorie Naturală din Zoo, fondat în 1964.",
                  "category": "museum",
                  "coords": [
                        -99.2015,
                        19.409971
                  ]
            },
            {
                  "name": "Mapoteca Manuel Orozco y Berra",
                  "text": "Mapoteca Manuel Orozco y Berra (bibliotecă de hărți) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.196764,
                        19.404265
                  ]
            },
            {
                  "name": "Museo de Geofísica de la UNAM",
                  "text": "Museo de Geofísica de la UNAM (Muzeul de Geofizică al UNAM) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.195251,
                        19.404514
                  ]
            },
            {
                  "name": "Centro de Cultura Ambiental",
                  "text": "Centru de cultură ecologică deschis în 2023, proiectat de Rafael Ponce Ortiz, în Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.198258,
                        19.410848
                  ]
            },
            {
                  "name": "Museo Casa de la Bola",
                  "text": "Museo Casa de la Bola (muzeul casei mingii) lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.188831,
                        19.404728
                  ]
            },
            {
                  "name": "Casa Gilardi, Luis Barragán 1976",
                  "text": "Casa Gilardi, o casă din 1976 de arhitectul Luis Barragán, lângă Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.186235,
                        19.41424
                  ]
            },
            {
                  "name": "FUENTE",
                  "text": "FUENTE (fântână istorică) lângă Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.203921,
                        19.387904
                  ]
            },
            {
                  "name": "Tomáš Garrigue Masaryk 1850-1937",
                  "text": "Monumentul Tomáš Garrigue Masaryk 1850-1937 este un monument în Chapultepec Zoo Life v2.",
                  "category": "historical",
                  "coords": [
                        -99.191177,
                        19.431534
                  ]
            },
            {
                  "name": "Molino de Santo Domingo",
                  "text": "Molino de Santo Domingo este un monument în Chapultepec Zoo Life v2.",
                  "category": "historical",
                  "coords": [
                        -99.193917,
                        19.402823
                  ]
            },
            {
                  "name": "Auditorio Adrián Gibert",
                  "text": "Auditorio Adrián Gibert este un teatru în Chapultepec Zoo Life v2.",
                  "category": "cultural",
                  "coords": [
                        -99.181253,
                        19.407138
                  ]
            },
            {
                  "name": "Teatro Hipódromo Condesa",
                  "text": "Teatro Hipódromo Condesa (teatru în cartierul Condesa) lângă Zoo.",
                  "category": "cultural",
                  "coords": [
                        -99.18468,
                        19.406563
                  ]
            },
            {
                  "name": "Centro Cultural del Bosque",
                  "text": "Centro Cultural del Bosque (centru cultural în pădure) lângă Zoo.",
                  "category": "cultural",
                  "coords": [
                        -99.195433,
                        19.423727
                  ]
            }
      ],
      "en": [
            {
                  "name": "Papalote•Museo del Niño",
                  "text": "The Papalote Museo del Niño is a museum in Chapultepec Zoo Life v2, one of the largest children's museums in Latin America.",
                  "category": "museum",
                  "coords": [
                        -99.19463,
                        19.411946
                  ]
            },
            {
                  "name": "Casa Luis Barragán",
                  "text": "The Casa Luis Barragán is a museum in Chapultepec Zoo Life v2, a UNESCO World Heritage site as the former home of architect Luis Barragán.",
                  "category": "museum",
                  "coords": [
                        -99.192433,
                        19.411114
                  ]
            },
            {
                  "name": "Casa del Lago UNAM",
                  "text": "The Casa del Lago UNAM is a museum in Chapultepec Zoo Life v2, a cultural center on the lake in Chapultepec Park.",
                  "category": "museum",
                  "coords": [
                        -99.185836,
                        19.42237
                  ]
            },
            {
                  "name": "Complejo Cultural Los Pinos",
                  "text": "The Complejo Cultural Los Pinos is a museum in Chapultepec Zoo Life v2, the former presidential residence now a cultural complex.",
                  "category": "museum",
                  "coords": [
                        -99.191481,
                        19.415573
                  ]
            },
            {
                  "name": "Auditorio Nacional",
                  "text": "The Auditorio Nacional is a theatre in Chapultepec Zoo Life v2, one of the largest concert halls in the world.",
                  "category": "cultural",
                  "coords": [
                        -99.19484,
                        19.424806
                  ]
            },
            {
                  "name": "Cárcamo de Dolores",
                  "text": "The Cárcamo de Dolores is an attraction in Chapultepec Zoo Life v2, featuring murals by Diego Rivera.",
                  "category": "landmark",
                  "coords": [
                        -99.197989,
                        19.412849
                  ]
            },
            {
                  "name": "Zona arqueológica de Mixcoac",
                  "text": "The Zona arqueológica de Mixcoac is an attraction in Chapultepec Zoo Life v2, an archaeological site of a pre-Hispanic settlement.",
                  "category": "landmark",
                  "coords": [
                        -99.189851,
                        19.386303
                  ]
            },
            {
                  "name": "Polyforum Cultural Siqueiros",
                  "text": "The Polyforum Cultural Siqueiros is an attraction in Chapultepec Zoo Life v2, a cultural center with murals by David Alfaro Siqueiros.",
                  "category": "landmark",
                  "coords": [
                        -99.17344,
                        19.393316
                  ]
            },
            {
                  "name": "Parque Lira",
                  "text": "The Parque Lira is a park in Chapultepec Zoo Life v2, a public park in the area.",
                  "category": "park",
                  "coords": [
                        -99.189934,
                        19.405958
                  ]
            },
            {
                  "name": "Aztlán Parque Urbano",
                  "text": "The Aztlán Parque Urbano is a theme park in Chapultepec Zoo Life v2, an amusement park with rides and attractions.",
                  "category": "recreational",
                  "coords": [
                        -99.195634,
                        19.416062
                  ]
            },
            {
                  "name": "Casa Presidencial Lázaro Cárdenas",
                  "text": "The Casa Presidencial Lázaro Cárdenas is a museum in Chapultepec Zoo Life v2, dedicated to the former president.",
                  "category": "museum",
                  "coords": [
                        -99.191433,
                        19.41383
                  ]
            },
            {
                  "name": "Zoológico de Chapultepec",
                  "text": "The Zoológico de Chapultepec is a zoo in Chapultepec Zoo Life v2, housing many animal species.",
                  "category": "recreational",
                  "coords": [
                        -99.189527,
                        19.422993
                  ]
            },
            {
                  "name": "LagoAlgo",
                  "text": "LagoAlgo museum near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.199773,
                        19.417143
                  ]
            },
            {
                  "name": "Galería Enrique Guerrero",
                  "text": "Galería Enrique Guerrero (art gallery) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.185524,
                        19.411015
                  ]
            },
            {
                  "name": "Patricia Conde Galería",
                  "text": "Patricia Conde Galería (art gallery) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.185034,
                        19.412771
                  ]
            },
            {
                  "name": "Parque Galería",
                  "text": "Parque Galería (park gallery) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.183736,
                        19.41564
                  ]
            },
            {
                  "name": "Galería de Arte Mexicano GAM",
                  "text": "Galería de Arte Mexicano GAM (Mexican art gallery) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.186982,
                        19.413308
                  ]
            },
            {
                  "name": "Cencalli",
                  "text": "Cencalli museum in Zoo, opened in September 2021.",
                  "category": "museum",
                  "coords": [
                        -99.192028,
                        19.417172
                  ]
            },
            {
                  "name": "Bernardini",
                  "text": "The Bernardini is a gallery in Chapultepec Zoo Life v2.",
                  "category": "museum",
                  "coords": [
                        -99.191611,
                        19.429424
                  ]
            },
            {
                  "name": "Museo de la Policía Federal",
                  "text": "Museo de la Policía Federal (Federal Police Museum) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.218055,
                        19.393062
                  ]
            },
            {
                  "name": "Espacio CDMX",
                  "text": "The Espacio CDMX is a gallery in Chapultepec Zoo Life v2.",
                  "category": "museum",
                  "coords": [
                        -99.202804,
                        19.41119
                  ]
            },
            {
                  "name": "Museo Nacional de la Cartografía",
                  "text": "Museo Nacional de la Cartografía (national cartography museum) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.191798,
                        19.404457
                  ]
            },
            {
                  "name": "Museo de Historia Natural y Cultura Ambiental",
                  "text": "Natural history museum in Zoo, founded in 1964.",
                  "category": "museum",
                  "coords": [
                        -99.2015,
                        19.409971
                  ]
            },
            {
                  "name": "Mapoteca Manuel Orozco y Berra",
                  "text": "Mapoteca Manuel Orozco y Berra (map library) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.196764,
                        19.404265
                  ]
            },
            {
                  "name": "Museo de Geofísica de la UNAM",
                  "text": "Museo de Geofísica de la UNAM (geophysics museum of UNAM) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.195251,
                        19.404514
                  ]
            },
            {
                  "name": "Centro de Cultura Ambiental",
                  "text": "Environmental culture center opened 2023, designed by Rafael Ponce Ortiz, located in the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.198258,
                        19.410848
                  ]
            },
            {
                  "name": "Museo Casa de la Bola",
                  "text": "Museo Casa de la Bola (house of the ball museum) near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.188831,
                        19.404728
                  ]
            },
            {
                  "name": "Casa Gilardi, Luis Barragán 1976",
                  "text": "Casa Gilardi, a 1976 house by architect Luis Barragán, near the Zoo.",
                  "category": "museum",
                  "coords": [
                        -99.186235,
                        19.41424
                  ]
            },
            {
                  "name": "FUENTE",
                  "text": "FUENTE (historical fountain) near the Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.203921,
                        19.387904
                  ]
            },
            {
                  "name": "Tomáš Garrigue Masaryk 1850-1937",
                  "text": "The Tomáš Garrigue Masaryk 1850-1937 is a monument in Chapultepec Zoo Life v2.",
                  "category": "historical",
                  "coords": [
                        -99.191177,
                        19.431534
                  ]
            },
            {
                  "name": "Molino de Santo Domingo",
                  "text": "The Molino de Santo Domingo is a monument in Chapultepec Zoo Life v2.",
                  "category": "historical",
                  "coords": [
                        -99.193917,
                        19.402823
                  ]
            },
            {
                  "name": "Auditorio Adrián Gibert",
                  "text": "The Auditorio Adrián Gibert is a theatre in Chapultepec Zoo Life v2.",
                  "category": "cultural",
                  "coords": [
                        -99.181253,
                        19.407138
                  ]
            },
            {
                  "name": "Teatro Hipódromo Condesa",
                  "text": "Teatro Hipódromo Condesa (theater in the Condesa neighborhood) near the Zoo.",
                  "category": "cultural",
                  "coords": [
                        -99.18468,
                        19.406563
                  ]
            },
            {
                  "name": "Centro Cultural del Bosque",
                  "text": "Centro Cultural del Bosque (forest cultural center) near the Zoo.",
                  "category": "cultural",
                  "coords": [
                        -99.195433,
                        19.423727
                  ]
            }
      ]
},
    type: "kid-landmark",
    parent: "MX-CMX",
    coords: [-99.1867, 19.4165],
    name: { de: "Chapultepec Zoo", hu: "Chapultepec allatkert", ro: "Gradina Zoologica Chapultepec", en: "Chapultepec Zoo"},
    image: "/poi-images/chapultepec-zoo-life-v2.webp",
    descriptionAdvanced: {
      de: "Der Zoo Chapultepec im Herzen von Mexiko-Stadt ist eine der bedeutendsten zoologischen Einrichtungen Lateinamerikas. Er wurde 1923 vom Biologen Alfonso L. Herrera gegründet und beherbergt heute über 1.200 Tiere in naturnah gestalteten Gehegen. Besonders berühmt ist der Zoo für sein erfolgreiches Zuchtprogramm für Große Pandas, das als eines der ersten außerhalb Chinas große Erfolge feierte. Neben exotischen Tieren liegt ein starker Fokus auf der Erhaltung einheimischer Arten wie dem Mexikanischen Wolf und dem Vulkankaninchen. Der Zoo bietet eine grüne Oase inmitten der Metropole und ist ein wichtiges Zentrum für Umweltbildung und Artenschutzforschung. Ein Besuch verbindet Bildung mit dem Erlebnis der faszinierenden Tierwelt Mexikos und der Welt. Geschichte K8 - Artenschutz und Evolution.",
      hu: "Chapultepec allatkert egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Chapultepec allatkert kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Gradina Zoologica Chapultepec este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Gradina Zoologica Chapultepec este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Chapultepec Zoo, nestled within the sprawling Bosque de Chapultepec in Mexico City, is one of the most visited and beloved zoological institutions in Latin America. Founded in 1923 by biologist Alfonso L. Herrera, it was designed to showcase native Mexican species alongside exotic animals from around the globe. The zoo is famously known for its successful giant panda breeding program, being the first institution outside of China to achieve this feat. Beyond being a popular family destination, it serves as a critical center for scientific research and the conservation of endangered species like the Mexican wolf and the volcano rabbit. Visitors can explore various biomes, from tropical forests to arid deserts, all within the heart of the metropolis. This institution plays a vital role in environmental education, connecting urban residents with the wonders of the natural world.",
      es: "El Zoológico de Chapultepec, en el corazón de la Ciudad de México, es una de las instituciones zoológicas más importantes de América Latina. Fundado en 1923 por el biólogo Alfonso L. Herrera, hoy alberga a más de 1.200 animales en recintos diseñados de forma natural. El zoológico es particularmente famoso por su exitoso programa de reproducción de pandas gigantes, uno de los primeros en lograr grandes éxitos fuera de China. Además de los animales exóticos, se enfoca fuertemente en la conservación de especies nativas como el lobo mexicano y el conejo de los volcanes. El zoológico ofrece un oasis verde en medio de la metrópoli y es un centro vital para la educación ambiental y la investigación de especies. Una visita combina el aprendizaje con la experiencia de la fascinante fauna de México y del mundo.",
      pt: "O Zoológico de Chapultepec, no coração da Cidade do México, é uma das instituições zoológicas mais importantes da América Latina. Foi fundado em 1923 pelo biólogo Alfonso L. Herrera e abriga hoje mais de 1.200 animais em recintos de design natural. O zoológico é especialmente famoso pelo seu bem-sucedido programa de reprodução de pandas-gigantes, um dos primeiros a ter grande sucesso fora da China. Além de animais exóticos, há um forte foco na conservação de espécies nativas, como o lobo mexicano e o coelho-dos-vulcões. O zoológico oferece um oásis verde no meio da metrópole e é um centro importante para a educação ambiental e pesquisa de conservação. Uma visita combina educação com a experiência da fascinante vida selvagem do México e do mundo.",
      fr: "Le zoo de Chapultepec, au cœur de Mexico, est l'une des institutions zoologiques les plus importantes d'Amérique latine. Fondé en 1923 par le biologiste Alfonso L. Herrera, il abrite aujourd'hui plus de 1 200 animaux dans des enclos au design naturel. Le zoo est particulièrement célèbre pour son programme réussi de reproduction des pandas géants, l'un des premiers à avoir connu un grand succès hors de Chine. Outre les animaux exotiques, l'accent est mis sur la conservation des espèces indigènes telles que le loup mexicain et le lapin des volcans. Le zoo offre une oasis de verdure au milieu de la métropole et constitue un centre important pour l'éducation environnementale et la recherche sur la protection des espèces. Une visite allie éducation et découverte de la faune fascinante du Mexique et du monde.",
    },
    factsAdvanced: {
      de: ["Wurde am 6. Juli 1923 nach dem Vorbild des Jardin des Plantes in Paris eröffnet.", "Beherbergt mehr als 220 verschiedene Tierarten auf einer Fläche von 17 Hektar.", "Erfolgreiches Panda-Zuchtprogramm seit den 1980er Jahren.", "Zentrum für die Wiederauswilderung des gefährdeten Mexikanischen Wolfs.", "Teil des weitläufigen Chapultepec-Parks, der grünen Lunge der Hauptstadt.", "Eintritt ist für die Öffentlichkeit traditionell kostenlos.", "Verfügt über spezialisierte Bereiche für Wüsten-, Tropen- und gemäßigte Zonen."],
      hu: ["Chapultepec allatkert sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Chapultepec allatkert egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Gradina Zoologica Chapultepec ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Gradina Zoologica Chapultepec este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The zoo was inaugurated on July 6, 1923, inspired by the Jardin des Plantes in Paris.",
        "It is home to over 1,200 animals representing more than 220 different species.",
        "Chapultepec Zoo is one of the few places in the world that houses giant pandas not owned by China.",
        "The facility covers approximately 17 hectares of land within the first section of Chapultepec Park.",
        "It successfully reintroduced the Mexican wolf into its natural habitat after decades of captive breeding."
      ],
      es: ["Se inauguró el 6 de julio de 1923, inspirado en el Jardin des Plantes de París.", "Alberga más de 220 especies diferentes de animales en una superficie de 17 hectáreas.", "Exitoso programa de reproducción de pandas desde la década de 1980.", "Centro para la reintroducción del lobo mexicano, especie en peligro de extinción.", "Parte del extenso Bosque de Chapultepec, el pulmón verde de la capital.", "La entrada es tradicionalmente gratuita para el público.", "Cuenta con áreas especializadas para zonas desérticas, tropicales y templadas."],
      pt: ["Foi inaugurado em 6 de julho de 1923, inspirado no Jardin des Plantes de Paris.", "Abriga mais de 220 espécies diferentes de animais em uma área de 17 hectares.", "Programa de reprodução de pandas bem-sucedido desde a década de 1980.", "Centro para a reintrodução do lobo mexicano, espécie ameaçada de extinção.", "Parte do extenso Parque Chapultepec, o pulmão verde da capital.", "A entrada é tradicionalmente gratuita para o público.", "Possui áreas especializadas para zonas desérticas, tropicais e temperadas."],
      fr: ["Inauguré le 6 juillet 1923 sur le modèle du Jardin des Plantes de Paris.", "Abrite plus de 220 espèces animales différentes sur une surface de 17 hectares.", "Programme de reproduction des pandas réussi depuis les années 1980.", "Centre de réintroduction du loup mexicain, espèce menacée.", "Fait partie du vaste parc de Chapultepec, le poumon vert de la capitale.", "L'entrée est traditionnellement gratuite pour le public.", "Dispose de zones spécialisées pour les zones désertiques, tropicales et tempérées."],
    }, image: "/poi-images/chapultepec-zoo-life-v2.webp",
    description: {
      de: "Der größte Zoo Mexikos, gelegen im weitläufigen Chapultepec-Park in Mexiko-Stadt.",
      hu: "Mexikóváros legrégebbi állatkertje, amely a Chapultepec park szívében található.",
      ro: "Cea mai veche grădină zoologică din Mexic, situată în parcul istoric Chapultepec din inima capitalei.",
      en: "The oldest and most visited zoo in Mexico, located within the historic Chapultepec Park in the heart of Mexico City.",
      es: "El zoológico más grande de México, ubicado en el extenso Bosque de Chapultepec en la Ciudad de México.",
      pt: "O maior zoológico do México, localizado no extenso Parque Chapultepec, na Cidade do México.",
      fr: "Le plus grand zoo du Mexique, situé dans le vaste parc de Chapultepec à Mexico.",
    },
    facts: {
      de: ["Eröffnet im Jahr 1923", "Berühmt für seine Großen Pandas", "Über 1.200 Tiere zu sehen"],
      hu: ["1923-ban alapították", "Több mint 1200 állat otthona", "Ingyenesen látogatható park"],
      ro: ["Fondată în anul 1923", "Găzduiește peste 1200 de animale", "Celebră pentru urșii panda uriași"],
      en: ["Opened in 1923", "Famous for its giant pandas", "Home to over 1,200 animals"],
      es: ["Inaugurado en 1923", "Famoso por sus pandas gigantes", "Más de 1.200 animales para ver"],
      pt: ["Inaugurado em 1923", "Famoso por seus pandas-gigantes", "Mais de 1.200 animais para ver"],
      fr: ["Ouvert en 1923", "Célèbre pour ses pandas géants", "Plus de 1 200 animaux à voir"],
    }
  },
  {
    id: "san-juan-de-aragon-zoo-life-v2", "sights": {
      "de": [
            {
                  "name": "Las Torres",
                  "text": "Ein Denkmal im Stadtteil San Juan de Aragón von Mexiko-Stadt.",
                  "category": "historical",
                  "coords": [
                        -99.045858,
                        19.475183
                  ]
            },
            {
                  "name": "Templo de la Ciudad de México",
                  "text": "Ein Tempel der Kirche Jesu Christi der Heiligen der Letzten Tage im Gebiet San Juan de Aragón von Mexiko-Stadt.",
                  "category": "religious",
                  "coords": [
                        -99.086824,
                        19.465894
                  ]
            },
            {
                  "name": "Bosque de San Juan de Aragón",
                  "text": "Ein großer Stadtpark im Gebiet San Juan de Aragón von Mexiko-Stadt.",
                  "category": "landmark",
                  "coords": [
                        -99.07338,
                        19.460408
                  ]
            },
            {
                  "name": "Peñón de los baños",
                  "text": "Ein kleiner Hügel im Gebiet San Juan de Aragón von Mexiko-Stadt mit prähispanischer Bedeutung.",
                  "category": "natural",
                  "coords": [
                        -99.082743,
                        19.441046
                  ]
            },
            {
                  "name": "Alameda Oriente",
                  "text": "Ein öffentlicher Park im Osten von Mexiko-Stadt, nahe San Juan de Aragón.",
                  "category": "park",
                  "coords": [
                        -99.054209,
                        19.435696
                  ]
            },
            {
                  "name": "Proyecto Ecológico Lago de Texcoco (PELT)",
                  "text": "Ein ökologisches Parkprojekt um den Texcoco-See im Raum Mexiko-Stadt.",
                  "category": "park",
                  "coords": [
                        -98.985819,
                        19.481469
                  ]
            },
            {
                  "name": "Zoológico de San Juan de Aragón",
                  "text": "Ein Zoo im Gebiet San Juan de Aragón von Mexiko-Stadt mit einer Vielzahl von Tierarten.",
                  "category": "recreational",
                  "coords": [
                        -99.083555,
                        19.461538
                  ]
            },
            {
                  "name": "Jardín de las Campanas",
                  "text": "Eine Galerie im Gebiet San Juan de Aragón von Mexiko-Stadt.",
                  "category": "museum",
                  "coords": [
                        -99.073597,
                        19.463918
                  ]
            },
            {
                  "name": "C47 - El Mexicano",
                  "text": "Ein Denkmal im Stadtteil San Juan de Aragón von Mexiko-Stadt.",
                  "category": "historical",
                  "coords": [
                        -99.074524,
                        19.423332
                  ]
            },
            {
                  "name": "Torre 8",
                  "text": "Torre 8 (Turm 8) im Juan-De-Aragon-Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.088311,
                        19.454203
                  ]
            },
            {
                  "name": "Torre VI",
                  "text": "Torre VI ist einer der nummerierten historischen Türme im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.088075,
                        19.462116
                  ]
            },
            {
                  "name": "Torre XIII",
                  "text": "Torre XIII steht als historischer Turm im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.074216,
                        19.451278
                  ]
            },
            {
                  "name": "Torre XVI",
                  "text": "Torre XVI zählt zu den nummerierten historischen Türmen im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.06862,
                        19.448543
                  ]
            },
            {
                  "name": "Torre XV",
                  "text": "Torre XV ist ein historischer Turm im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.06914,
                        19.452551
                  ]
            },
            {
                  "name": "Torre XIV",
                  "text": "Torre XIV ist ein nummerierter historischer Turm im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.07043,
                        19.455723
                  ]
            },
            {
                  "name": "Torre V",
                  "text": "Torre V gehört zu den historischen Türmen mit Nummer im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.089388,
                        19.458651
                  ]
            },
            {
                  "name": "Torre IV",
                  "text": "Torre IV ist ein historischer Turm im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.091086,
                        19.454194
                  ]
            },
            {
                  "name": "Torre VII",
                  "text": "Torre VII ist einer der nummerierten historischen Türme im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.089759,
                        19.450356
                  ]
            },
            {
                  "name": "Torre I",
                  "text": "Torre I ist der erste der nummerierten historischen Türme im Zoo Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.095812,
                        19.456316
                  ]
            },
            {
                  "name": "Sor Juana Ines de la Cruz",
                  "text": "Denkmal zu Ehren der mexikanischen Dichterin und Nonne Sor Juana Inés de la Cruz.",
                  "category": "historical",
                  "coords": [
                        -99.014712,
                        19.405658
                  ]
            },
            {
                  "name": "Auditorio la Salle",
                  "text": "Kulturzentrum in San Juan de Aragón für Aufführungen und Veranstaltungen.",
                  "category": "cultural",
                  "coords": [
                        -99.016317,
                        19.422449
                  ]
            },
            {
                  "name": "Marbet",
                  "text": "Marbet ist ein kulturelles Wahrzeichen im Zoo Juan de Aragón.",
                  "category": "cultural",
                  "coords": [
                        -99.016664,
                        19.413906
                  ]
            },
            {
                  "name": "Centro Cultural \"El Castillito\"",
                  "text": "Das Centro Cultural „El Castillito“ ist ein Kulturzentrum im Zoo Juan de Aragón.",
                  "category": "cultural",
                  "coords": [
                        -99.009774,
                        19.403373
                  ]
            },
            {
                  "name": "Teatro José Vasconcelos",
                  "text": "Theater benannt nach dem mexikanischen Schriftsteller und Politiker José Vasconcelos.",
                  "category": "cultural",
                  "coords": [
                        -99.046331,
                        19.476404
                  ]
            },
            {
                  "name": "Baños Medicinales del Peñón",
                  "text": "Die Baños Medicinales del Peñón sind medizinische Bäder an einem Felsen im Zoo Juan de Aragón.",
                  "category": "landmark",
                  "coords": [
                        -99.086881,
                        19.438244
                  ]
            },
            {
                  "name": "Gallina de Guinea",
                  "text": "Ein Perlhuhn als Attraktion im Zoo von San Juan de Aragón.",
                  "category": "landmark",
                  "coords": [
                        -99.086341,
                        19.463408
                  ]
            },
            {
                  "name": "León Africano",
                  "text": "Der León Africano ist ein Wahrzeichen, das einen afrikanischen Löwen darstellt, im Zoo Juan de Aragón.",
                  "category": "landmark",
                  "coords": [
                        -99.085613,
                        19.464345
                  ]
            },
            {
                  "name": "Elefante Asiático",
                  "text": "Der Elefante Asiático ist ein Wahrzeichen, das einen asiatischen Elefanten zeigt, im Zoo Juan de Aragón.",
                  "category": "landmark",
                  "coords": [
                        -99.084629,
                        19.46364
                  ]
            },
            {
                  "name": "EL PULPO ACUATICO",
                  "text": "Eine Attraktion im Zoo von San Juan de Aragón mit dem Namen „EL PULPO ACUATICO“.",
                  "category": "landmark",
                  "coords": [
                        -99.034013,
                        19.402114
                  ]
            },
            {
                  "name": "Playa de Aves",
                  "text": "Ein Aussichtspunkt auf die Vogelwelt im Zoo von San Juan de Aragón.",
                  "category": "natural",
                  "coords": [
                        -99.070916,
                        19.461213
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Las Torres",
                  "text": "Emlékmű Mexikóváros San Juan de Aragón negyedében.",
                  "category": "historical",
                  "coords": [
                        -99.045858,
                        19.475183
                  ]
            },
            {
                  "name": "Templo de la Ciudad de México",
                  "text": "Az Utolsó Napok Szentjeinek Jézus Krisztus Egyháza temploma Mexikóváros San Juan de Aragón negyedében.",
                  "category": "religious",
                  "coords": [
                        -99.086824,
                        19.465894
                  ]
            },
            {
                  "name": "Bosque de San Juan de Aragón",
                  "text": "Nagy városi park Mexikóváros San Juan de Aragón negyedében.",
                  "category": "landmark",
                  "coords": [
                        -99.07338,
                        19.460408
                  ]
            },
            {
                  "name": "Peñón de los baños",
                  "text": "Kicsi domb Mexikóváros San Juan de Aragón negyedében, prehispán jelentőséggel.",
                  "category": "natural",
                  "coords": [
                        -99.082743,
                        19.441046
                  ]
            },
            {
                  "name": "Alameda Oriente",
                  "text": "Közpark Mexikóváros keleti részén, San Juan de Aragón közelében.",
                  "category": "park",
                  "coords": [
                        -99.054209,
                        19.435696
                  ]
            },
            {
                  "name": "Proyecto Ecológico Lago de Texcoco (PELT)",
                  "text": "Ökológiai parkprojekt a Texcoco-tó körül Mexikóváros térségében.",
                  "category": "park",
                  "coords": [
                        -98.985819,
                        19.481469
                  ]
            },
            {
                  "name": "Zoológico de San Juan de Aragón",
                  "text": "Állatkert Mexikóváros San Juan de Aragón negyedében, sokféle állatfajjal.",
                  "category": "recreational",
                  "coords": [
                        -99.083555,
                        19.461538
                  ]
            },
            {
                  "name": "Jardín de las Campanas",
                  "text": "Galéria Mexikóváros San Juan de Aragón negyedében.",
                  "category": "museum",
                  "coords": [
                        -99.073597,
                        19.463918
                  ]
            },
            {
                  "name": "C47 - El Mexicano",
                  "text": "Emlékmű Mexikóváros San Juan de Aragón negyedében.",
                  "category": "historical",
                  "coords": [
                        -99.074524,
                        19.423332
                  ]
            },
            {
                  "name": "Torre 8",
                  "text": "Torre 8 (8-as torony) a Juan De Aragon Állatkertben.",
                  "category": "historical",
                  "coords": [
                        -99.088311,
                        19.454203
                  ]
            },
            {
                  "name": "Torre VI",
                  "text": "A Torre VI a Juan de Aragón Állatkert számozott történelmi tornyainak egyike.",
                  "category": "historical",
                  "coords": [
                        -99.088075,
                        19.462116
                  ]
            },
            {
                  "name": "Torre XIII",
                  "text": "A Torre XIII történelmi torony a Juan de Aragón Állatkertben.",
                  "category": "historical",
                  "coords": [
                        -99.074216,
                        19.451278
                  ]
            },
            {
                  "name": "Torre XVI",
                  "text": "A Torre XVI a Juan de Aragón Állatkert számozott történelmi tornyai közé tartozik.",
                  "category": "historical",
                  "coords": [
                        -99.06862,
                        19.448543
                  ]
            },
            {
                  "name": "Torre XV",
                  "text": "A Torre XV történelmi torony a Juan de Aragón Állatkertben.",
                  "category": "historical",
                  "coords": [
                        -99.06914,
                        19.452551
                  ]
            },
            {
                  "name": "Torre XIV",
                  "text": "A Torre XIV számozott történelmi torony a Juan de Aragón Állatkertben.",
                  "category": "historical",
                  "coords": [
                        -99.07043,
                        19.455723
                  ]
            },
            {
                  "name": "Torre V",
                  "text": "A Torre V a Juan de Aragón Állatkert számozott történelmi tornyai közé tartozik.",
                  "category": "historical",
                  "coords": [
                        -99.089388,
                        19.458651
                  ]
            },
            {
                  "name": "Torre IV",
                  "text": "A Torre IV történelmi torony a Juan de Aragón Állatkertben.",
                  "category": "historical",
                  "coords": [
                        -99.091086,
                        19.454194
                  ]
            },
            {
                  "name": "Torre VII",
                  "text": "A Torre VII a Juan de Aragón Állatkert számozott történelmi tornyainak egyike.",
                  "category": "historical",
                  "coords": [
                        -99.089759,
                        19.450356
                  ]
            },
            {
                  "name": "Torre I",
                  "text": "A Torre I az első a Juan de Aragón Állatkert számozott történelmi tornyai közül.",
                  "category": "historical",
                  "coords": [
                        -99.095812,
                        19.456316
                  ]
            },
            {
                  "name": "Sor Juana Ines de la Cruz",
                  "text": "A mexikói költőnő és apáca, Sor Juana Inés de la Cruz tiszteletére állított emlékmű.",
                  "category": "historical",
                  "coords": [
                        -99.014712,
                        19.405658
                  ]
            },
            {
                  "name": "Auditorio la Salle",
                  "text": "Kulturális központ San Juan de Aragónban előadások és események számára.",
                  "category": "cultural",
                  "coords": [
                        -99.016317,
                        19.422449
                  ]
            },
            {
                  "name": "Marbet",
                  "text": "A Marbet kulturális nevezetesség a Juan de Aragón Állatkertben.",
                  "category": "cultural",
                  "coords": [
                        -99.016664,
                        19.413906
                  ]
            },
            {
                  "name": "Centro Cultural \"El Castillito\"",
                  "text": "A Centro Cultural „El Castillito” egy kulturális központ a Juan de Aragón Állatkertben.",
                  "category": "cultural",
                  "coords": [
                        -99.009774,
                        19.403373
                  ]
            },
            {
                  "name": "Teatro José Vasconcelos",
                  "text": "José Vasconcelos mexikói író és politikus nevét viselő színház.",
                  "category": "cultural",
                  "coords": [
                        -99.046331,
                        19.476404
                  ]
            },
            {
                  "name": "Baños Medicinales del Peñón",
                  "text": "A Baños Medicinales del Peñón gyógyfürdők egy sziklánál a Juan de Aragón Állatkertben.",
                  "category": "landmark",
                  "coords": [
                        -99.086881,
                        19.438244
                  ]
            },
            {
                  "name": "Gallina de Guinea",
                  "text": "Egy gyöngytyúk látványosság a San Juan de Aragón-i állatkertben.",
                  "category": "landmark",
                  "coords": [
                        -99.086341,
                        19.463408
                  ]
            },
            {
                  "name": "León Africano",
                  "text": "A León Africano egy afrikai oroszlánt ábrázoló nevezetesség a Juan de Aragón Állatkertben.",
                  "category": "landmark",
                  "coords": [
                        -99.085613,
                        19.464345
                  ]
            },
            {
                  "name": "Elefante Asiático",
                  "text": "Az Elefante Asiático egy ázsiai elefántot ábrázoló nevezetesség a Juan de Aragón Állatkertben.",
                  "category": "landmark",
                  "coords": [
                        -99.084629,
                        19.46364
                  ]
            },
            {
                  "name": "EL PULPO ACUATICO",
                  "text": "Egy látványosság a San Juan de Aragón-i állatkertben „EL PULPO ACUATICO” néven.",
                  "category": "landmark",
                  "coords": [
                        -99.034013,
                        19.402114
                  ]
            },
            {
                  "name": "Playa de Aves",
                  "text": "Egy kilátópont a madárvilágra a San Juan de Aragón-i állatkertben.",
                  "category": "natural",
                  "coords": [
                        -99.070916,
                        19.461213
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Las Torres",
                  "text": "Un monument în districtul San Juan de Aragón din Ciudad de México.",
                  "category": "historical",
                  "coords": [
                        -99.045858,
                        19.475183
                  ]
            },
            {
                  "name": "Templo de la Ciudad de México",
                  "text": "Un templu al Bisericii lui Isus Hristos a Sfinților din Zilele din Urmă în zona San Juan de Aragón din Ciudad de México.",
                  "category": "religious",
                  "coords": [
                        -99.086824,
                        19.465894
                  ]
            },
            {
                  "name": "Bosque de San Juan de Aragón",
                  "text": "Un parc urban mare în zona San Juan de Aragón din Ciudad de México.",
                  "category": "landmark",
                  "coords": [
                        -99.07338,
                        19.460408
                  ]
            },
            {
                  "name": "Peñón de los baños",
                  "text": "Un deal mic în zona San Juan de Aragón din Ciudad de México cu semnificație prehispanică.",
                  "category": "natural",
                  "coords": [
                        -99.082743,
                        19.441046
                  ]
            },
            {
                  "name": "Alameda Oriente",
                  "text": "Un parc public în estul Ciudad de México, aproape de San Juan de Aragón.",
                  "category": "park",
                  "coords": [
                        -99.054209,
                        19.435696
                  ]
            },
            {
                  "name": "Proyecto Ecológico Lago de Texcoco (PELT)",
                  "text": "Un proiect de parc ecologic în jurul lacului Texcoco în zona Ciudad de México.",
                  "category": "park",
                  "coords": [
                        -98.985819,
                        19.481469
                  ]
            },
            {
                  "name": "Zoológico de San Juan de Aragón",
                  "text": "O grădină zoologică în zona San Juan de Aragón din Ciudad de México cu o varietate de specii animale.",
                  "category": "recreational",
                  "coords": [
                        -99.083555,
                        19.461538
                  ]
            },
            {
                  "name": "Jardín de las Campanas",
                  "text": "O galerie în zona San Juan de Aragón din Ciudad de México.",
                  "category": "museum",
                  "coords": [
                        -99.073597,
                        19.463918
                  ]
            },
            {
                  "name": "C47 - El Mexicano",
                  "text": "Un monument în districtul San Juan de Aragón din Ciudad de México.",
                  "category": "historical",
                  "coords": [
                        -99.074524,
                        19.423332
                  ]
            },
            {
                  "name": "Torre 8",
                  "text": "Torre 8 (Turnul 8) la Grădina Zoologică Juan De Aragon.",
                  "category": "historical",
                  "coords": [
                        -99.088311,
                        19.454203
                  ]
            },
            {
                  "name": "Torre VI",
                  "text": "Torre VI este unul dintre turnurile istorice numerotate din Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.088075,
                        19.462116
                  ]
            },
            {
                  "name": "Torre XIII",
                  "text": "Torre XIII este un turn istoric în Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.074216,
                        19.451278
                  ]
            },
            {
                  "name": "Torre XVI",
                  "text": "Torre XVI face parte din turnurile istorice numerotate din Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.06862,
                        19.448543
                  ]
            },
            {
                  "name": "Torre XV",
                  "text": "Torre XV este un turn istoric în Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.06914,
                        19.452551
                  ]
            },
            {
                  "name": "Torre XIV",
                  "text": "Torre XIV este un turn istoric numerotat în Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.07043,
                        19.455723
                  ]
            },
            {
                  "name": "Torre V",
                  "text": "Torre V face parte din turnurile istorice numerotate din Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.089388,
                        19.458651
                  ]
            },
            {
                  "name": "Torre IV",
                  "text": "Torre IV este un turn istoric în Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.091086,
                        19.454194
                  ]
            },
            {
                  "name": "Torre VII",
                  "text": "Torre VII este unul dintre turnurile istorice numerotate din Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.089759,
                        19.450356
                  ]
            },
            {
                  "name": "Torre I",
                  "text": "Torre I este primul dintre turnurile istorice numerotate din Grădina Zoologică Juan de Aragón.",
                  "category": "historical",
                  "coords": [
                        -99.095812,
                        19.456316
                  ]
            },
            {
                  "name": "Sor Juana Ines de la Cruz",
                  "text": "Monument în onoarea poetei și călugăriței mexicane Sor Juana Inés de la Cruz.",
                  "category": "historical",
                  "coords": [
                        -99.014712,
                        19.405658
                  ]
            },
            {
                  "name": "Auditorio la Salle",
                  "text": "Centru cultural în San Juan de Aragón pentru spectacole și evenimente.",
                  "category": "cultural",
                  "coords": [
                        -99.016317,
                        19.422449
                  ]
            },
            {
                  "name": "Marbet",
                  "text": "Marbet este un reper cultural în Grădina Zoologică Juan de Aragón.",
                  "category": "cultural",
                  "coords": [
                        -99.016664,
                        19.413906
                  ]
            },
            {
                  "name": "Centro Cultural \"El Castillito\"",
                  "text": "Centro Cultural „El Castillito” este un centru cultural în Grădina Zoologică Juan de Aragón.",
                  "category": "cultural",
                  "coords": [
                        -99.009774,
                        19.403373
                  ]
            },
            {
                  "name": "Teatro José Vasconcelos",
                  "text": "Teatru numit după scriitorul și politicianul mexican José Vasconcelos.",
                  "category": "cultural",
                  "coords": [
                        -99.046331,
                        19.476404
                  ]
            },
            {
                  "name": "Baños Medicinales del Peñón",
                  "text": "Baños Medicinales del Peñón sunt băi medicinale la o stâncă în Grădina Zoologică Juan de Aragón.",
                  "category": "landmark",
                  "coords": [
                        -99.086881,
                        19.438244
                  ]
            },
            {
                  "name": "Gallina de Guinea",
                  "text": "O bibilică ca atracție în grădina zoologică din San Juan de Aragón.",
                  "category": "landmark",
                  "coords": [
                        -99.086341,
                        19.463408
                  ]
            },
            {
                  "name": "León Africano",
                  "text": "León Africano este un reper care reprezintă un leu african în Grădina Zoologică Juan de Aragón.",
                  "category": "landmark",
                  "coords": [
                        -99.085613,
                        19.464345
                  ]
            },
            {
                  "name": "Elefante Asiático",
                  "text": "Elefante Asiático este un reper care ilustrează un elefant asiatic în Grădina Zoologică Juan de Aragón.",
                  "category": "landmark",
                  "coords": [
                        -99.084629,
                        19.46364
                  ]
            },
            {
                  "name": "EL PULPO ACUATICO",
                  "text": "O atracție în grădina zoologică din San Juan de Aragón numită „EL PULPO ACUATICO”.",
                  "category": "landmark",
                  "coords": [
                        -99.034013,
                        19.402114
                  ]
            },
            {
                  "name": "Playa de Aves",
                  "text": "Un punct de belvedere asupra păsărilor în grădina zoologică din San Juan de Aragón.",
                  "category": "natural",
                  "coords": [
                        -99.070916,
                        19.461213
                  ]
            }
      ],
      "en": [
            {
                  "name": "Las Torres",
                  "text": "A monument in the San Juan de Aragón district of Mexico City.",
                  "category": "historical",
                  "coords": [
                        -99.045858,
                        19.475183
                  ]
            },
            {
                  "name": "Mexico City Mexico Temple",
                  "text": "A Latter-day Saint temple in the San Juan de Aragón area of Mexico City.",
                  "category": "religious",
                  "coords": [
                        -99.086824,
                        19.465894
                  ]
            },
            {
                  "name": "Bosque de San Juan de Aragón",
                  "text": "A large urban park in the San Juan de Aragón area of Mexico City.",
                  "category": "landmark",
                  "coords": [
                        -99.07338,
                        19.460408
                  ]
            },
            {
                  "name": "Peñón de los baños",
                  "text": "A small hill in the San Juan de Aragón area of Mexico City with pre-Hispanic significance.",
                  "category": "natural",
                  "coords": [
                        -99.082743,
                        19.441046
                  ]
            },
            {
                  "name": "Alameda Oriente",
                  "text": "A public park in eastern Mexico City, near San Juan de Aragón.",
                  "category": "park",
                  "coords": [
                        -99.054209,
                        19.435696
                  ]
            },
            {
                  "name": "Proyecto Ecológico Lago de Texcoco (PELT)",
                  "text": "An ecological park project around Lake Texcoco in the Mexico City area.",
                  "category": "park",
                  "coords": [
                        -98.985819,
                        19.481469
                  ]
            },
            {
                  "name": "Zoológico de San Juan de Aragón",
                  "text": "A zoo in the San Juan de Aragón area of Mexico City housing a variety of animal species.",
                  "category": "recreational",
                  "coords": [
                        -99.083555,
                        19.461538
                  ]
            },
            {
                  "name": "Jardín de las Campanas",
                  "text": "A gallery in the San Juan de Aragón area of Mexico City.",
                  "category": "museum",
                  "coords": [
                        -99.073597,
                        19.463918
                  ]
            },
            {
                  "name": "C47 - El Mexicano",
                  "text": "A monument in the San Juan de Aragón district of Mexico City.",
                  "category": "historical",
                  "coords": [
                        -99.074524,
                        19.423332
                  ]
            },
            {
                  "name": "Torre 8",
                  "text": "Torre 8 (Tower 8) at Juan De Aragon Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.088311,
                        19.454203
                  ]
            },
            {
                  "name": "Torre VI",
                  "text": "Torre VI is one of the numbered historical towers in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.088075,
                        19.462116
                  ]
            },
            {
                  "name": "Torre XIII",
                  "text": "Torre XIII is a historical tower in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.074216,
                        19.451278
                  ]
            },
            {
                  "name": "Torre XVI",
                  "text": "Torre XVI is among the numbered historical towers in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.06862,
                        19.448543
                  ]
            },
            {
                  "name": "Torre XV",
                  "text": "Torre XV is a historical tower in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.06914,
                        19.452551
                  ]
            },
            {
                  "name": "Torre XIV",
                  "text": "Torre XIV is a numbered historical tower in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.07043,
                        19.455723
                  ]
            },
            {
                  "name": "Torre V",
                  "text": "Torre V is one of the numbered historical towers in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.089388,
                        19.458651
                  ]
            },
            {
                  "name": "Torre IV",
                  "text": "Torre IV is a historical tower in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.091086,
                        19.454194
                  ]
            },
            {
                  "name": "Torre VII",
                  "text": "Torre VII is one of the numbered historical towers in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.089759,
                        19.450356
                  ]
            },
            {
                  "name": "Torre I",
                  "text": "Torre I is the first of the numbered historical towers in the Juan de Aragón Zoo.",
                  "category": "historical",
                  "coords": [
                        -99.095812,
                        19.456316
                  ]
            },
            {
                  "name": "Sor Juana Ines de la Cruz",
                  "text": "Monument honoring the Mexican poet and nun Sor Juana Inés de la Cruz.",
                  "category": "historical",
                  "coords": [
                        -99.014712,
                        19.405658
                  ]
            },
            {
                  "name": "Auditorio la Salle",
                  "text": "Arts centre in San Juan de Aragón for performances and events.",
                  "category": "cultural",
                  "coords": [
                        -99.016317,
                        19.422449
                  ]
            },
            {
                  "name": "Marbet",
                  "text": "Marbet is a cultural landmark in the Juan de Aragón Zoo.",
                  "category": "cultural",
                  "coords": [
                        -99.016664,
                        19.413906
                  ]
            },
            {
                  "name": "Centro Cultural \"El Castillito\"",
                  "text": "Centro Cultural \"El Castillito\" is a cultural center in the Juan de Aragón Zoo.",
                  "category": "cultural",
                  "coords": [
                        -99.009774,
                        19.403373
                  ]
            },
            {
                  "name": "Teatro José Vasconcelos",
                  "text": "Theatre named after Mexican writer and politician José Vasconcelos.",
                  "category": "cultural",
                  "coords": [
                        -99.046331,
                        19.476404
                  ]
            },
            {
                  "name": "Baños Medicinales del Peñón",
                  "text": "Baños Medicinales del Peñón are medicinal baths at a rock formation in the Juan de Aragón Zoo.",
                  "category": "landmark",
                  "coords": [
                        -99.086881,
                        19.438244
                  ]
            },
            {
                  "name": "Gallina de Guinea",
                  "text": "A guinea fowl as an attraction in the San Juan de Aragón zoo.",
                  "category": "landmark",
                  "coords": [
                        -99.086341,
                        19.463408
                  ]
            },
            {
                  "name": "León Africano",
                  "text": "León Africano is a landmark depicting an African lion in the Juan de Aragón Zoo.",
                  "category": "landmark",
                  "coords": [
                        -99.085613,
                        19.464345
                  ]
            },
            {
                  "name": "Elefante Asiático",
                  "text": "Elefante Asiático is a landmark showing an Asian elephant in the Juan de Aragón Zoo.",
                  "category": "landmark",
                  "coords": [
                        -99.084629,
                        19.46364
                  ]
            },
            {
                  "name": "EL PULPO ACUATICO",
                  "text": "An attraction in the San Juan de Aragón zoo called „EL PULPO ACUATICO”.",
                  "category": "landmark",
                  "coords": [
                        -99.034013,
                        19.402114
                  ]
            },
            {
                  "name": "Playa de Aves",
                  "text": "A viewpoint over the birdlife in the San Juan de Aragón zoo.",
                  "category": "natural",
                  "coords": [
                        -99.070916,
                        19.461213
                  ]
            }
      ]
},
    type: "kid-landmark",
    parent: "MX-CMX",
    coords: [-99.0595, 19.4535],
    name: { de: "San Juan de Aragon Zoo", hu: "San Juan de Aragon allatkert", ro: "Gradina Zoologica San Juan de Aragon", en: "San Juan de Aragon Zoo"},
    image: "/poi-images/san-juan-de-aragon-zoo-life-v2.webp",
    descriptionAdvanced: {
      de: "Der Zoo San Juan de Aragón im Nordosten von Mexiko-Stadt ist flächenmäßig der zweitgrößte Zoo der Hauptstadt und ein wichtiges Erholungsgebiet. Seit seiner Eröffnung im Jahr 1964 verfolgt er das Konzept, Tieren weitläufige Freiluftgehege zu bieten, die ihren natürlichen Lebensräumen nachempfunden sind. Nach einer umfassenden Modernisierung Anfang der 2000er Jahre hat sich der Zoo zu einem führenden Zentrum für Umweltbildung entwickelt. Ein besonderer Schwerpunkt liegt auf den im Tal von Mexiko heimischen Arten sowie auf großen Säugetieren und einer beeindruckenden Vogelsammlung. Die ruhigen, schattigen Wege laden zu ausgedehnten Spaziergängen ein und bieten eine friedliche Alternative zum Stadttrubel. Der Zoo spielt eine entscheidende Rolle im lokalen Naturschutz und bietet zahlreiche Workshops für Besucher an. Biologie K7 - Ökosysteme und Lebensräume.",
      hu: "San Juan de Aragon allatkert egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. San Juan de Aragon allatkert kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Gradina Zoologica San Juan de Aragon este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Gradina Zoologica San Juan de Aragon este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The San Juan de Aragón Zoo, located in the northeastern part of Mexico City, is the city's second-largest zoo and a vital green lung for the surrounding neighborhoods. Opened in 1964, it was designed with a focus on providing spacious, open-air enclosures that mimic the natural habitats of the animals more closely than traditional zoos. The park underwent a major renovation in the early 2000s to modernize its facilities and enhance its role as an educational center for environmental conservation. It is particularly noted for its work with species native to the Valley of Mexico and its impressive collection of birds and large mammals. The zoo's layout encourages long walks through shaded paths, making it a peaceful alternative to the more crowded Chapultepec Zoo. It remains a key site for community engagement, offering workshops and guided tours that emphasize the importance of biodiversity.",
      es: "El Zoológico de San Juan de Aragón, en el noreste de la Ciudad de México, es por superficie el segundo zoológico más grande de la capital y una importante área recreativa. Desde su apertura en 1964, sigue el concepto de ofrecer a los animales recintos amplios al aire libre que imitan sus hábitats naturales. Tras una profunda modernización a principios de la década de 2000, el zoológico se ha convertido en un centro líder para la educación ambiental. Se enfoca especialmente en las especies nativas del Valle de México, así como en grandes mamíferos y una impresionante colección de aves. Los senderos tranquilos y sombreados invitan a largos paseos y ofrecen una alternativa pacífica al bullicio de la ciudad. El zoológico desempeña un papel crucial en la conservación local y ofrece numerosos talleres para visitantes. Biología K7 - Ecosistemas y hábitats.",
      pt: "O Jardim Zoológico San Juan de Aragón, no nordeste da Cidade do México, é o segundo maior jardim zoológico da capital em termos de área e uma importante zona de lazer. Desde a sua abertura em 1964, tem seguido o conceito de oferecer aos animais recintos amplos ao ar livre, moldados de acordo com os seus habitats naturais. Após uma modernização abrangente no início dos anos 2000, o jardim zoológico tornou-se um centro líder para a educação ambiental. É dado um enfoque especial às espécies nativas do Vale do México, bem como a grandes mamíferos e a uma impressionante coleção de aves. Os caminhos tranquilos e sombreados convidam a longos passeios e oferecem uma alternativa pacífica à agitação da cidade. O jardim zoológico desempenha um papel crucial na conservação local e oferece inúmeros workshops para os visitantes. Biologia K7 - Ecossistemas e habitats.",
      fr: "Le zoo de San Juan de Aragón, dans le nord-est de Mexico, est le deuxième plus grand zoo de la capitale en termes de superficie et une zone de loisirs importante. Depuis son ouverture en 1964, il suit le concept consistant à offrir aux animaux de vastes enclos en plein air reproduisant leurs habitats naturels. Après une modernisation complète au début des années 2000, le zoo est devenu un centre de premier plan pour l'éducation environnementale. Un accent particulier est mis sur les espèces indigènes de la vallée de Mexico ainsi que sur les grands mammifères et une impressionnante collection d'oiseaux. Les allées calmes et ombragées invitent à de longues promenades et offrent une alternative paisible à l'agitation de la ville. Le zoo joue un rôle crucial dans la protection de la nature locale et propose de nombreux ateliers aux visiteurs. Biologie K7 - Écosystèmes et habitats.",
    },
    factsAdvanced: {
      de: ["Erstreckt sich über eine beeindruckende Fläche von etwa 31 Hektar.", "Beherbergt ein modernes Krankenhaus für Wildtiere und ein Forschungszentrum.", "Verfügt über einen Bereich, der das antike Seensystem von Xochimilco nachbildet.", "Wichtiger Standort für die Erhaltung und Zucht des Mexikanischen Grauwolfs.", "Der zooeigene See dient als Rastplatz für zahlreiche Zugvogelarten.", "Bietet spezialisierte Bildungsprogramme für Schulen der Region.", "Fokus auf die Vermittlung der Biodiversität des mexikanischen Hochlandes."],
      hu: ["San Juan de Aragon allatkert sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "San Juan de Aragon allatkert egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Gradina Zoologica San Juan de Aragon ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Gradina Zoologica San Juan de Aragon este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The zoo spans an area of approximately 31 hectares near the San Juan de Aragón Forest.",
        "It features a specialized hospital for wild animals and a research center for species preservation.",
        "The 'Xochimilco' area of the zoo replicates the ancient lake system of the Valley of Mexico.",
        "It houses a significant population of Mexican gray wolves, a species that was once extinct in the wild.",
        "The zoo's lake is a seasonal stop for various migratory bird species traveling across North America."
      ],
      es: ["Se extiende sobre una superficie impresionante de unas 31 hectáreas.", "Alberga un moderno hospital para vida silvestre y un centro de investigación.", "Cuenta con un área que recrea el antiguo sistema de lagos de Xochimilco.", "Lugar importante para la conservación y cría del lobo gris mexicano.", "El lago propio del zoológico sirve como lugar de descanso para numerosas aves migratorias.", "Ofrece programas educativos especializados para escuelas de la región.", "Se enfoca en transmitir el conocimiento sobre la biodiversidad del altiplano mexicano."],
      pt: ["Estende-se por uma área impressionante de cerca de 31 hectares.", "Abriga um moderno hospital para animais selvagens e um centro de investigação.", "Possui uma área que recria o antigo sistema de lagos de Xochimilco.", "Local importante para a conservação e reprodução do lobo-cinzento-mexicano.", "O lago do próprio zoológico serve como local de repouso para inúmeras espécies de aves migratórias.", "Oferece programas educativos especializados para escolas da região.", "Foca-se na transmissão da biodiversidade do planalto mexicano."],
      fr: ["S'étend sur une superficie impressionnante d'environ 31 hectares.", "Abrite un hôpital moderne pour la faune sauvage et un centre de recherche.", "Dispose d'une zone reproduisant l'ancien système lacustre de Xochimilco.", "Site important pour la conservation et l'élevage du loup gris mexicain.", "Le lac du zoo sert de halte à de nombreuses espèces d'oiseaux migrateurs.", "Propose des programmes éducatifs spécialisés pour les écoles de la région.", "Se concentre sur la sensibilisation à la biodiversité des hauts plateaux mexicains."],
    }, image: "/poi-images/san-juan-de-aragon-zoo-life-v2.webp",
    description: {
      de: "Ein wichtiger Zoo im Nordosten von Mexiko-Stadt, der sich auf bedrohte Arten konzentriert.",
      hu: "A főváros második legnagyobb állatkertje, amely elsősorban a mexikói farkas védelmére összpontosít.",
      ro: "A doua cea mai mare grădină zoologică din Ciudad de México, axată pe specii native și conservarea lupului mexican.",
      en: "The second largest zoo in Mexico City, focused on the conservation of native and endangered species like the Mexican wolf.",
      es: "Un importante zoológico en el noreste de la Ciudad de México que se concentra en especies amenazadas.",
      pt: "Um importante jardim zoológico no nordeste da Cidade do México, que se foca em espécies ameaçadas.",
      fr: "Un zoo important du nord-est de Mexico qui se concentre sur les espèces menacées.",
    },
    facts: {
      de: ["Fokus auf mexikanische Fauna", "Zentrum für Artenschutz", "Großzügige natürliche Gehege"],
      hu: ["1964-ben nyitották meg", "31 hektáros területen fekszik", "Különleges madárházzal rendelkezik"],
      ro: ["Inaugurată în anul 1964", "Suprafață de 31 de hectare", "Include o replică a lacului Xochimilco"],
      en: ["Inaugurated in 1964", "Spans 31 hectares", "Includes a replica of Xochimilco"],
      es: ["Enfoque en la fauna mexicana", "Centro de protección de especies", "Recintos naturales amplios"],
      pt: ["Foco na fauna mexicana", "Centro para proteção de espécies", "Recintos naturais generosos"],
      fr: ["Accent sur la faune mexicaine", "Centre pour la protection des espèces", "Enclos naturels spacieux"],
    }
  },
  {
    id: "guadalajara-zoo-life-v2", "sights": {
      "de": [
            {
                  "name": "Selva Magica",
                  "text": "Selva Mágica ist ein beliebter Vergnügungspark in Guadalajara mit Fahrgeschäften und Attraktionen.",
                  "category": "recreational",
                  "coords": [
                        -103.311196,
                        20.724322
                  ]
            },
            {
                  "name": "Auditorio Telmex",
                  "text": "Auditorio Telmex ist ein modernes Veranstaltungszentrum im Zoogelände von Guadalajara.",
                  "category": "cultural",
                  "coords": [
                        -103.381243,
                        20.73336
                  ]
            },
            {
                  "name": "Centro Cultural Constitución",
                  "text": "Centro Cultural Constitución ist ein Kulturzentrum auf dem Zoogelände in Guadalajara.",
                  "category": "cultural",
                  "coords": [
                        -103.368066,
                        20.727411
                  ]
            },
            {
                  "name": "Zoológico Guadalajara",
                  "text": "Der Zoo von Guadalajara ist ein großer Zoo mit einer Vielzahl von Tierarten.",
                  "category": "recreational",
                  "coords": [
                        -103.307654,
                        20.728364
                  ]
            },
            {
                  "name": "Museo de Ciencias Ambientales",
                  "text": "Umweltwissenschaftliches Museum in Guadalajara mit Schwerpunkt auf Ökologie und Natur.",
                  "category": "museum",
                  "coords": [
                        -103.381161,
                        20.735241
                  ]
            },
            {
                  "name": "Torre Eiffel -Escala-",
                  "text": "Die Torre Eiffel -Escala- ist eine maßstabsgetreue Nachbildung des Eiffelturms im Zoo.",
                  "category": "historical",
                  "coords": [
                        -103.271686,
                        20.694046
                  ]
            },
            {
                  "name": "Ruinas del Planetario del Municipio de Guadalajara Severo Díaz Galindo",
                  "text": "Die Ruinas del Planetario del Municipio de Guadalajara Severo Díaz Galindo sind die Ruinen eines Planetariums im Zoo.",
                  "category": "historical",
                  "coords": [
                        -103.309434,
                        20.71886
                  ]
            },
            {
                  "name": "El Grillo",
                  "text": "Die archäologische Stätte El Grillo in Guadalajara, eine prähispanische Siedlung.",
                  "category": "historical",
                  "coords": [
                        -103.369594,
                        20.73354
                  ]
            },
            {
                  "name": "Calle 2",
                  "text": "Calle 2 ist eine Straße im kulturellen Bereich des Zoos.",
                  "category": "cultural",
                  "coords": [
                        -103.376515,
                        20.734986
                  ]
            },
            {
                  "name": "Foro Ichitan Río Verde",
                  "text": "Das Foro Ichitan Río Verde ist ein Veranstaltungsort im Zoo, benannt nach Ichitan und dem Río Verde.",
                  "category": "cultural",
                  "coords": [
                        -103.289025,
                        20.698589
                  ]
            },
            {
                  "name": "Ágora Jenkins",
                  "text": "Die Ágora Jenkins ist ein kultureller Versammlungsort im Zoo, benannt nach Jenkins.",
                  "category": "cultural",
                  "coords": [
                        -103.381026,
                        20.73721
                  ]
            },
            {
                  "name": "Sala 2",
                  "text": "Sala 2 ist ein kultureller Raum im Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.382035,
                        20.736455
                  ]
            },
            {
                  "name": "Teatro Estudio Cavaret",
                  "text": "Das Teatro Estudio Cavaret ist ein Theaterstudio im Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.378299,
                        20.736814
                  ]
            },
            {
                  "name": "Auditorio Central",
                  "text": "Das Auditorio Central ist das Hauptauditorium im Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.380751,
                        20.741605
                  ]
            },
            {
                  "name": "Anfiteatro Independencia",
                  "text": "Das Anfiteatro Independencia ist ein Amphitheater im Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.310856,
                        20.741421
                  ]
            },
            {
                  "name": "Centro Holístico Macuilli Tepec",
                  "text": "Das Centro Holístico Macuilli Tepec ist ein ganzheitliches Zentrum im Zoo.",
                  "category": "landmark",
                  "coords": [
                        -103.235632,
                        20.693025
                  ]
            },
            {
                  "name": "Mirador Del Cañon Escarpado",
                  "text": "Der Mirador Del Cañon Escarpado bietet einen Aussichtspunkt auf eine steile Schlucht im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.374984,
                        20.75823
                  ]
            },
            {
                  "name": "Parque Mirador Dr. Atl",
                  "text": "Der Parque Mirador Dr. Atl ist ein Aussichtspark, benannt nach dem Künstler Dr. Atl, im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.344203,
                        20.795063
                  ]
            },
            {
                  "name": "Mirador \"Las Cañadas\"",
                  "text": "Der Mirador „Las Cañadas“ ist ein Aussichtspunkt auf die Schluchten im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.372884,
                        20.766212
                  ]
            },
            {
                  "name": "El Riachelo",
                  "text": "El Riachelo ist ein kleiner Bach im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.371669,
                        20.765382
                  ]
            },
            {
                  "name": "Mirador De Los Eucaliptos",
                  "text": "Der Mirador De Los Eucaliptos ist ein Aussichtspunkt unter Eukalyptusbäumen im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.379035,
                        20.766707
                  ]
            },
            {
                  "name": "Mirador \"Lago El Centinela\"",
                  "text": "Der Mirador „Lago El Centinela“ bietet einen Blick auf den Lago El Centinela im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.378634,
                        20.765317
                  ]
            },
            {
                  "name": "Piedras Bola",
                  "text": "Piedras Bola ist ein Aussichtspunkt mit großen runden Felsformationen in Guadalajara Zoo Life.",
                  "category": "natural",
                  "coords": [
                        -103.375593,
                        20.766298
                  ]
            },
            {
                  "name": "Observatoire Du Roi Dakk",
                  "text": "Das Observatoire Du Roi Dakk ist eine Sternwarte im Zoo, benannt nach König Dakk.",
                  "category": "natural",
                  "coords": [
                        -103.376414,
                        20.762115
                  ]
            },
            {
                  "name": "Pont De Rivière Profonde",
                  "text": "Der Pont De Rivière Profonde ist eine Brücke über einen tiefen Fluss im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.37803,
                        20.763883
                  ]
            },
            {
                  "name": "Trône Du Roi Dakk",
                  "text": "Der Trône Du Roi Dakk ist ein Thron im Zoo, der König Dakk gewidmet ist.",
                  "category": "natural",
                  "coords": [
                        -103.393722,
                        20.792451
                  ]
            },
            {
                  "name": "Mirador Del Ficus Insipida",
                  "text": "Der Mirador Del Ficus Insipida ist ein Aussichtspunkt an einem Ficus Insipida-Baum im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.346266,
                        20.817251
                  ]
            },
            {
                  "name": "Mirador Del Jabalí",
                  "text": "Der Mirador Del Jabalí ist ein Aussichtspunkt mit Blick auf Wildschweine im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.349135,
                        20.820468
                  ]
            },
            {
                  "name": "Mirador Del Buitre",
                  "text": "Der Mirador Del Buitre ist ein Aussichtspunkt auf Geier im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.349187,
                        20.822644
                  ]
            },
            {
                  "name": "Área de Protección Estatal Barrancas de los Ríos Santiago y Verde",
                  "text": "Das Área de Protección Estatal Barrancas de los Ríos Santiago y Verde ist ein Naturschutzgebiet mit tiefen Schluchten in Guadalajara.",
                  "category": "natural",
                  "coords": [
                        -103.244946,
                        20.713193
                  ]
            },
            {
                  "name": "Área de Protección Hidrológica Municipal Bosque el Nixticuil - San Estebán - el Diente",
                  "text": "Das Área de Protección Hidrológica Municipal Bosque el Nixticuil - San Estebán - el Diente ist ein geschütztes Waldgebiet im Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.391785,
                        20.79446
                  ]
            },
            {
                  "name": "Mundo Marino Guadalajara",
                  "text": "Mundo Marino Guadalajara ist ein Meerespark im Zoo von Guadalajara.",
                  "category": "recreational",
                  "coords": [
                        -103.310317,
                        20.723397
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Selva Magica",
                  "text": "A Selva Mágica egy népszerű vidámpark Guadalajara-ban, hullámvasutakkal és attrakciókkal.",
                  "category": "recreational",
                  "coords": [
                        -103.311196,
                        20.724322
                  ]
            },
            {
                  "name": "Auditorio Telmex",
                  "text": "Auditorio Telmex egy modern rendezvényközpont a guadalajarai állatkert területén.",
                  "category": "cultural",
                  "coords": [
                        -103.381243,
                        20.73336
                  ]
            },
            {
                  "name": "Centro Cultural Constitución",
                  "text": "Centro Cultural Constitución egy kulturális központ az állatkert területén Guadalajarában.",
                  "category": "cultural",
                  "coords": [
                        -103.368066,
                        20.727411
                  ]
            },
            {
                  "name": "Zoológico Guadalajara",
                  "text": "A Guadalajarai Állatkert egy nagy állatkert, amely sokféle állatfajnak ad otthont.",
                  "category": "recreational",
                  "coords": [
                        -103.307654,
                        20.728364
                  ]
            },
            {
                  "name": "Museo de Ciencias Ambientales",
                  "text": "Környezettudományi Múzeum Guadalajara-ban, amely az ökológiára és a természetre összpontosít.",
                  "category": "museum",
                  "coords": [
                        -103.381161,
                        20.735241
                  ]
            },
            {
                  "name": "Torre Eiffel -Escala-",
                  "text": "A Torre Eiffel -Escala- az Eiffel-torony méretarányos mása az Állatkertben.",
                  "category": "historical",
                  "coords": [
                        -103.271686,
                        20.694046
                  ]
            },
            {
                  "name": "Ruinas del Planetario del Municipio de Guadalajara Severo Díaz Galindo",
                  "text": "A Ruinas del Planetario del Municipio de Guadalajara Severo Díaz Galindo egy planetárium romjai az Állatkertben.",
                  "category": "historical",
                  "coords": [
                        -103.309434,
                        20.71886
                  ]
            },
            {
                  "name": "El Grillo",
                  "text": "El Grillo régészeti lelőhely Guadalajara-ban, egy prehispán település.",
                  "category": "historical",
                  "coords": [
                        -103.369594,
                        20.73354
                  ]
            },
            {
                  "name": "Calle 2",
                  "text": "A Calle 2 egy utca az Állatkert kulturális területén.",
                  "category": "cultural",
                  "coords": [
                        -103.376515,
                        20.734986
                  ]
            },
            {
                  "name": "Foro Ichitan Río Verde",
                  "text": "A Foro Ichitan Río Verde egy rendezvényhelyszín az Állatkertben, amelyet az Ichitanról és a Río Verdéről neveztek el.",
                  "category": "cultural",
                  "coords": [
                        -103.289025,
                        20.698589
                  ]
            },
            {
                  "name": "Ágora Jenkins",
                  "text": "Az Ágora Jenkins egy kulturális gyülekezőhely az Állatkertben, amelyet Jenkinsről neveztek el.",
                  "category": "cultural",
                  "coords": [
                        -103.381026,
                        20.73721
                  ]
            },
            {
                  "name": "Sala 2",
                  "text": "A Sala 2 egy kulturális terem az Állatkertben.",
                  "category": "cultural",
                  "coords": [
                        -103.382035,
                        20.736455
                  ]
            },
            {
                  "name": "Teatro Estudio Cavaret",
                  "text": "A Teatro Estudio Cavaret egy színházstúdió az Állatkertben.",
                  "category": "cultural",
                  "coords": [
                        -103.378299,
                        20.736814
                  ]
            },
            {
                  "name": "Auditorio Central",
                  "text": "Az Auditorio Central a fő auditorium az Állatkertben.",
                  "category": "cultural",
                  "coords": [
                        -103.380751,
                        20.741605
                  ]
            },
            {
                  "name": "Anfiteatro Independencia",
                  "text": "Az Anfiteatro Independencia egy amfiteátrum az Állatkertben.",
                  "category": "cultural",
                  "coords": [
                        -103.310856,
                        20.741421
                  ]
            },
            {
                  "name": "Centro Holístico Macuilli Tepec",
                  "text": "A Centro Holístico Macuilli Tepec egy holisztikus központ az Állatkertben.",
                  "category": "landmark",
                  "coords": [
                        -103.235632,
                        20.693025
                  ]
            },
            {
                  "name": "Mirador Del Cañon Escarpado",
                  "text": "A Mirador Del Cañon Escarpado kilátópont egy meredek szurdokra az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.374984,
                        20.75823
                  ]
            },
            {
                  "name": "Parque Mirador Dr. Atl",
                  "text": "A Parque Mirador Dr. Atl egy kilátópark, amelyet Dr. Atl művészről neveztek el, az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.344203,
                        20.795063
                  ]
            },
            {
                  "name": "Mirador \"Las Cañadas\"",
                  "text": "A Mirador „Las Cañadas” kilátópont a szurdokokra az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.372884,
                        20.766212
                  ]
            },
            {
                  "name": "El Riachelo",
                  "text": "Az El Riachelo egy kis patak az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.371669,
                        20.765382
                  ]
            },
            {
                  "name": "Mirador De Los Eucaliptos",
                  "text": "A Mirador De Los Eucaliptos egy kilátópont eukaliptuszfák között az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.379035,
                        20.766707
                  ]
            },
            {
                  "name": "Mirador \"Lago El Centinela\"",
                  "text": "A Mirador „Lago El Centinela” kilátást nyújt a Lago El Centinela-tóra az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.378634,
                        20.765317
                  ]
            },
            {
                  "name": "Piedras Bola",
                  "text": "A Piedras Bola egy kilátópont nagy kerek sziklaformációkkal a Guadalajara Zoo Life-ban.",
                  "category": "natural",
                  "coords": [
                        -103.375593,
                        20.766298
                  ]
            },
            {
                  "name": "Observatoire Du Roi Dakk",
                  "text": "Az Observatoire Du Roi Dakk egy csillagvizsgáló az Állatkertben, amelyet Dakk királyról neveztek el.",
                  "category": "natural",
                  "coords": [
                        -103.376414,
                        20.762115
                  ]
            },
            {
                  "name": "Pont De Rivière Profonde",
                  "text": "A Pont De Rivière Profonde egy híd egy mély folyó felett az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.37803,
                        20.763883
                  ]
            },
            {
                  "name": "Trône Du Roi Dakk",
                  "text": "A Trône Du Roi Dakk egy trón az Állatkertben, amelyet Dakk királynak szenteltek.",
                  "category": "natural",
                  "coords": [
                        -103.393722,
                        20.792451
                  ]
            },
            {
                  "name": "Mirador Del Ficus Insipida",
                  "text": "A Mirador Del Ficus Insipida egy kilátópont egy Ficus Insipida fa mellett az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.346266,
                        20.817251
                  ]
            },
            {
                  "name": "Mirador Del Jabalí",
                  "text": "A Mirador Del Jabalí egy kilátópont vaddisznókra az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.349135,
                        20.820468
                  ]
            },
            {
                  "name": "Mirador Del Buitre",
                  "text": "A Mirador Del Buitre egy kilátópont keselyűkre az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.349187,
                        20.822644
                  ]
            },
            {
                  "name": "Área de Protección Estatal Barrancas de los Ríos Santiago y Verde",
                  "text": "Az Área de Protección Estatal Barrancas de los Ríos Santiago y Verde egy természetvédelmi terület mély szurdokokkal Guadalajarában.",
                  "category": "natural",
                  "coords": [
                        -103.244946,
                        20.713193
                  ]
            },
            {
                  "name": "Área de Protección Hidrológica Municipal Bosque el Nixticuil - San Estebán - el Diente",
                  "text": "Az Área de Protección Hidrológica Municipal Bosque el Nixticuil - San Estebán - el Diente egy védett erdőterület az Állatkertben.",
                  "category": "natural",
                  "coords": [
                        -103.391785,
                        20.79446
                  ]
            },
            {
                  "name": "Mundo Marino Guadalajara",
                  "text": "A Mundo Marino Guadalajara egy tengeri park a Guadalajarai Állatkertben.",
                  "category": "recreational",
                  "coords": [
                        -103.310317,
                        20.723397
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Selva Magica",
                  "text": "Selva Mágica este un parc de distracții popular în Guadalajara, cu curse și atracții.",
                  "category": "recreational",
                  "coords": [
                        -103.311196,
                        20.724322
                  ]
            },
            {
                  "name": "Auditorio Telmex",
                  "text": "Auditorio Telmex este un modern centru de evenimente pe terenul grădinii zoologice din Guadalajara.",
                  "category": "cultural",
                  "coords": [
                        -103.381243,
                        20.73336
                  ]
            },
            {
                  "name": "Centro Cultural Constitución",
                  "text": "Centro Cultural Constitución este un centru cultural pe terenul grădinii zoologice din Guadalajara.",
                  "category": "cultural",
                  "coords": [
                        -103.368066,
                        20.727411
                  ]
            },
            {
                  "name": "Zoológico Guadalajara",
                  "text": "Grădina Zoologică Guadalajara este o grădină zoologică mare cu o varietate de specii de animale.",
                  "category": "recreational",
                  "coords": [
                        -103.307654,
                        20.728364
                  ]
            },
            {
                  "name": "Museo de Ciencias Ambientales",
                  "text": "Muzeul de Științe ale Mediului din Guadalajara, concentrându-se pe ecologie și natură.",
                  "category": "museum",
                  "coords": [
                        -103.381161,
                        20.735241
                  ]
            },
            {
                  "name": "Torre Eiffel -Escala-",
                  "text": "Torre Eiffel -Escala- este o reproducere la scară a Turnului Eiffel din Grădina Zoologică.",
                  "category": "historical",
                  "coords": [
                        -103.271686,
                        20.694046
                  ]
            },
            {
                  "name": "Ruinas del Planetario del Municipio de Guadalajara Severo Díaz Galindo",
                  "text": "Ruinas del Planetario del Municipio de Guadalajara Severo Díaz Galindo sunt ruinele unui planetariu din Grădina Zoologică.",
                  "category": "historical",
                  "coords": [
                        -103.309434,
                        20.71886
                  ]
            },
            {
                  "name": "El Grillo",
                  "text": "Situl arheologic El Grillo în Guadalajara, o așezare prehispanică.",
                  "category": "historical",
                  "coords": [
                        -103.369594,
                        20.73354
                  ]
            },
            {
                  "name": "Calle 2",
                  "text": "Calle 2 este o stradă în zona culturală a Grădinii Zoologice.",
                  "category": "cultural",
                  "coords": [
                        -103.376515,
                        20.734986
                  ]
            },
            {
                  "name": "Foro Ichitan Río Verde",
                  "text": "Foro Ichitan Río Verde este un loc de evenimente în Grădina Zoologică, numit după Ichitan și Río Verde.",
                  "category": "cultural",
                  "coords": [
                        -103.289025,
                        20.698589
                  ]
            },
            {
                  "name": "Ágora Jenkins",
                  "text": "Ágora Jenkins este un loc de întrunire culturală în Grădina Zoologică, numit după Jenkins.",
                  "category": "cultural",
                  "coords": [
                        -103.381026,
                        20.73721
                  ]
            },
            {
                  "name": "Sala 2",
                  "text": "Sala 2 este o sală culturală în Grădina Zoologică.",
                  "category": "cultural",
                  "coords": [
                        -103.382035,
                        20.736455
                  ]
            },
            {
                  "name": "Teatro Estudio Cavaret",
                  "text": "Teatro Estudio Cavaret este un studio de teatru în Grădina Zoologică.",
                  "category": "cultural",
                  "coords": [
                        -103.378299,
                        20.736814
                  ]
            },
            {
                  "name": "Auditorio Central",
                  "text": "Auditorio Central este auditoriul principal din Grădina Zoologică.",
                  "category": "cultural",
                  "coords": [
                        -103.380751,
                        20.741605
                  ]
            },
            {
                  "name": "Anfiteatro Independencia",
                  "text": "Anfiteatro Independencia este un amfiteatru în Grădina Zoologică.",
                  "category": "cultural",
                  "coords": [
                        -103.310856,
                        20.741421
                  ]
            },
            {
                  "name": "Centro Holístico Macuilli Tepec",
                  "text": "Centro Holístico Macuilli Tepec este un centru holistic în Grădina Zoologică.",
                  "category": "landmark",
                  "coords": [
                        -103.235632,
                        20.693025
                  ]
            },
            {
                  "name": "Mirador Del Cañon Escarpado",
                  "text": "Mirador Del Cañon Escarpado oferă un punct de belvedere asupra unui canion abrupt în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.374984,
                        20.75823
                  ]
            },
            {
                  "name": "Parque Mirador Dr. Atl",
                  "text": "Parque Mirador Dr. Atl este un parc de belvedere, numit după artistul Dr. Atl, în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.344203,
                        20.795063
                  ]
            },
            {
                  "name": "Mirador \"Las Cañadas\"",
                  "text": "Mirador „Las Cañadas” este un punct de belvedere asupra canioanelor din Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.372884,
                        20.766212
                  ]
            },
            {
                  "name": "El Riachelo",
                  "text": "El Riachelo este un pârâu în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.371669,
                        20.765382
                  ]
            },
            {
                  "name": "Mirador De Los Eucaliptos",
                  "text": "Mirador De Los Eucaliptos este un punct de belvedere printre arbori de eucalipt în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.379035,
                        20.766707
                  ]
            },
            {
                  "name": "Mirador \"Lago El Centinela\"",
                  "text": "Mirador „Lago El Centinela” oferă o vedere asupra Lacului El Centinela în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.378634,
                        20.765317
                  ]
            },
            {
                  "name": "Piedras Bola",
                  "text": "Piedras Bola este un punct de belvedere cu formațiuni stâncoase rotunde mari în Guadalajara Zoo Life.",
                  "category": "natural",
                  "coords": [
                        -103.375593,
                        20.766298
                  ]
            },
            {
                  "name": "Observatoire Du Roi Dakk",
                  "text": "Observatoire Du Roi Dakk este un observator în Grădina Zoologică, numit după regele Dakk.",
                  "category": "natural",
                  "coords": [
                        -103.376414,
                        20.762115
                  ]
            },
            {
                  "name": "Pont De Rivière Profonde",
                  "text": "Pont De Rivière Profonde este un pod peste un râu adânc în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.37803,
                        20.763883
                  ]
            },
            {
                  "name": "Trône Du Roi Dakk",
                  "text": "Trône Du Roi Dakk este un tron în Grădina Zoologică, dedicat regelui Dakk.",
                  "category": "natural",
                  "coords": [
                        -103.393722,
                        20.792451
                  ]
            },
            {
                  "name": "Mirador Del Ficus Insipida",
                  "text": "Mirador Del Ficus Insipida este un punct de belvedere lângă un arbore Ficus Insipida în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.346266,
                        20.817251
                  ]
            },
            {
                  "name": "Mirador Del Jabalí",
                  "text": "Mirador Del Jabalí este un punct de belvedere asupra mistreților în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.349135,
                        20.820468
                  ]
            },
            {
                  "name": "Mirador Del Buitre",
                  "text": "Mirador Del Buitre este un punct de belvedere asupra vulturilor în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.349187,
                        20.822644
                  ]
            },
            {
                  "name": "Área de Protección Estatal Barrancas de los Ríos Santiago y Verde",
                  "text": "Área de Protección Estatal Barrancas de los Ríos Santiago y Verde este o rezervație naturală cu canioane adânci în Guadalajara.",
                  "category": "natural",
                  "coords": [
                        -103.244946,
                        20.713193
                  ]
            },
            {
                  "name": "Área de Protección Hidrológica Municipal Bosque el Nixticuil - San Estebán - el Diente",
                  "text": "Área de Protección Hidrológica Municipal Bosque el Nixticuil - San Estebán - el Diente este o zonă forestieră protejată în Grădina Zoologică.",
                  "category": "natural",
                  "coords": [
                        -103.391785,
                        20.79446
                  ]
            },
            {
                  "name": "Mundo Marino Guadalajara",
                  "text": "Mundo Marino Guadalajara este un parc marin în Grădina Zoologică din Guadalajara.",
                  "category": "recreational",
                  "coords": [
                        -103.310317,
                        20.723397
                  ]
            }
      ],
      "en": [
            {
                  "name": "Selva Magica",
                  "text": "Selva Mágica is a popular amusement park in Guadalajara with rides and attractions.",
                  "category": "recreational",
                  "coords": [
                        -103.311196,
                        20.724322
                  ]
            },
            {
                  "name": "Auditorio Telmex",
                  "text": "Auditorio Telmex is a modern event venue on the zoo grounds in Guadalajara.",
                  "category": "cultural",
                  "coords": [
                        -103.381243,
                        20.73336
                  ]
            },
            {
                  "name": "Centro Cultural Constitución",
                  "text": "Centro Cultural Constitución is a cultural center on the zoo grounds in Guadalajara.",
                  "category": "cultural",
                  "coords": [
                        -103.368066,
                        20.727411
                  ]
            },
            {
                  "name": "Zoológico Guadalajara",
                  "text": "Guadalajara Zoo is a major zoo housing diverse animal species.",
                  "category": "recreational",
                  "coords": [
                        -103.307654,
                        20.728364
                  ]
            },
            {
                  "name": "Museo de Ciencias Ambientales",
                  "text": "Environmental Sciences Museum in Guadalajara focusing on ecology and nature.",
                  "category": "museum",
                  "coords": [
                        -103.381161,
                        20.735241
                  ]
            },
            {
                  "name": "Torre Eiffel -Escala-",
                  "text": "Torre Eiffel -Escala- is a scale replica of the Eiffel Tower in the Zoo.",
                  "category": "historical",
                  "coords": [
                        -103.271686,
                        20.694046
                  ]
            },
            {
                  "name": "Ruinas del Planetario del Municipio de Guadalajara Severo Díaz Galindo",
                  "text": "Ruinas del Planetario del Municipio de Guadalajara Severo Díaz Galindo are the ruins of a planetarium in the Zoo.",
                  "category": "historical",
                  "coords": [
                        -103.309434,
                        20.71886
                  ]
            },
            {
                  "name": "El Grillo",
                  "text": "El Grillo archaeological site in Guadalajara, a pre-Hispanic settlement.",
                  "category": "historical",
                  "coords": [
                        -103.369594,
                        20.73354
                  ]
            },
            {
                  "name": "Calle 2",
                  "text": "Calle 2 is a street in the cultural area of the Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.376515,
                        20.734986
                  ]
            },
            {
                  "name": "Foro Ichitan Río Verde",
                  "text": "Foro Ichitan Río Verde is an event venue in the Zoo, named after Ichitan and the Río Verde.",
                  "category": "cultural",
                  "coords": [
                        -103.289025,
                        20.698589
                  ]
            },
            {
                  "name": "Ágora Jenkins",
                  "text": "Ágora Jenkins is a cultural gathering place in the Zoo, named after Jenkins.",
                  "category": "cultural",
                  "coords": [
                        -103.381026,
                        20.73721
                  ]
            },
            {
                  "name": "Sala 2",
                  "text": "Sala 2 is a cultural room in the Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.382035,
                        20.736455
                  ]
            },
            {
                  "name": "Teatro Estudio Cavaret",
                  "text": "Teatro Estudio Cavaret is a theater studio in the Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.378299,
                        20.736814
                  ]
            },
            {
                  "name": "Auditorio Central",
                  "text": "Auditorio Central is the main auditorium in the Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.380751,
                        20.741605
                  ]
            },
            {
                  "name": "Anfiteatro Independencia",
                  "text": "Anfiteatro Independencia is an amphitheater in the Zoo.",
                  "category": "cultural",
                  "coords": [
                        -103.310856,
                        20.741421
                  ]
            },
            {
                  "name": "Centro Holístico Macuilli Tepec",
                  "text": "Centro Holístico Macuilli Tepec is a holistic center in the Zoo.",
                  "category": "landmark",
                  "coords": [
                        -103.235632,
                        20.693025
                  ]
            },
            {
                  "name": "Mirador Del Cañon Escarpado",
                  "text": "Mirador Del Cañon Escarpado is a viewpoint over a steep canyon in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.374984,
                        20.75823
                  ]
            },
            {
                  "name": "Parque Mirador Dr. Atl",
                  "text": "Parque Mirador Dr. Atl is a viewpoint park named after artist Dr. Atl in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.344203,
                        20.795063
                  ]
            },
            {
                  "name": "Mirador \"Las Cañadas\"",
                  "text": "Mirador \"Las Cañadas\" is a viewpoint over the canyons in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.372884,
                        20.766212
                  ]
            },
            {
                  "name": "El Riachelo",
                  "text": "El Riachelo is a small stream in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.371669,
                        20.765382
                  ]
            },
            {
                  "name": "Mirador De Los Eucaliptos",
                  "text": "Mirador De Los Eucaliptos is a viewpoint among eucalyptus trees in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.379035,
                        20.766707
                  ]
            },
            {
                  "name": "Mirador \"Lago El Centinela\"",
                  "text": "Mirador \"Lago El Centinela\" offers a view of Lake El Centinela in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.378634,
                        20.765317
                  ]
            },
            {
                  "name": "Piedras Bola",
                  "text": "Piedras Bola is a viewpoint featuring large spherical rock formations in Guadalajara Zoo Life.",
                  "category": "natural",
                  "coords": [
                        -103.375593,
                        20.766298
                  ]
            },
            {
                  "name": "Observatoire Du Roi Dakk",
                  "text": "Observatoire Du Roi Dakk is an observatory in the Zoo, named after King Dakk.",
                  "category": "natural",
                  "coords": [
                        -103.376414,
                        20.762115
                  ]
            },
            {
                  "name": "Pont De Rivière Profonde",
                  "text": "Pont De Rivière Profonde is a bridge over a deep river in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.37803,
                        20.763883
                  ]
            },
            {
                  "name": "Trône Du Roi Dakk",
                  "text": "Trône Du Roi Dakk is a throne in the Zoo, dedicated to King Dakk.",
                  "category": "natural",
                  "coords": [
                        -103.393722,
                        20.792451
                  ]
            },
            {
                  "name": "Mirador Del Ficus Insipida",
                  "text": "Mirador Del Ficus Insipida is a viewpoint beside a Ficus Insipida tree in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.346266,
                        20.817251
                  ]
            },
            {
                  "name": "Mirador Del Jabalí",
                  "text": "Mirador Del Jabalí is a viewpoint overlooking wild boars in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.349135,
                        20.820468
                  ]
            },
            {
                  "name": "Mirador Del Buitre",
                  "text": "Mirador Del Buitre is a viewpoint over vultures in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.349187,
                        20.822644
                  ]
            },
            {
                  "name": "Área de Protección Estatal Barrancas de los Ríos Santiago y Verde",
                  "text": "Área de Protección Estatal Barrancas de los Ríos Santiago y Verde is a nature reserve known for its deep canyons in Guadalajara.",
                  "category": "natural",
                  "coords": [
                        -103.244946,
                        20.713193
                  ]
            },
            {
                  "name": "Área de Protección Hidrológica Municipal Bosque el Nixticuil - San Estebán - el Diente",
                  "text": "Área de Protección Hidrológica Municipal Bosque el Nixticuil - San Estebán - el Diente is a protected forest area in the Zoo.",
                  "category": "natural",
                  "coords": [
                        -103.391785,
                        20.79446
                  ]
            },
            {
                  "name": "Mundo Marino Guadalajara",
                  "text": "Mundo Marino Guadalajara is a marine park in the Guadalajara Zoo.",
                  "category": "recreational",
                  "coords": [
                        -103.310317,
                        20.723397
                  ]
            }
      ]
},
    type: "kid-landmark",
    parent: "MX-JAL",
    coords: [-103.315, 20.742],
    name: { de: "Guadalajara Zoo", hu: "Guadalajara allatkert", ro: "Gradina Zoologica Guadalajara", en: "Guadalajara Zoo"},
    image: "/poi-images/guadalajara-zoo-life-v2.webp",
    descriptionAdvanced: {
      de: "Der Zoo von Guadalajara gilt als einer der modernsten und schönsten Tierparks in ganz Mexiko und liegt spektakulär am Rande der Huentitán-Schlucht. Er bietet Besuchern einzigartige Erlebnisse wie die 'SkyZoo'-Seilbahn, die einen atemberaubenden Blick über die Tiergehege und die Schlucht ermöglicht. Der Zoo zeichnet sich durch seine Vielfalt aus, die von einem großen Aquarium über ein Tropenhaus bis hin zu einer afrikanischen Safari-Tour reicht. Besonders hervorzuheben ist das Engagement des Zoos bei der Zucht bedrohter Arten und der Forschung. Die Architektur der Anlage fügt sich harmonisch in die natürliche Topographie der Region ein. Für Familien ist der Park ein absolutes Highlight im Westen Mexikos, da er Bildung und Abenteuer auf höchstem Niveau verbindet. Sachkunde K4 - Tiere in verschiedenen Klimazonen.",
      hu: "Guadalajara allatkert egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Guadalajara allatkert kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Gradina Zoologica Guadalajara este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Gradina Zoologica Guadalajara este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Guadalajara Zoo is widely considered one of the finest and most innovative zoological parks in Mexico, offering a diverse range of experiences that go beyond simple animal viewing. Situated on the edge of the breathtaking Huentitán Canyon, the zoo uses its dramatic topography to create unique exhibits like the 'Monkeyland' and a cable car system that provides stunning aerial views of the animals and the gorge. It features a world-class aquarium, a specialized safari area where visitors can see African wildlife from a secure vehicle, and an Antarctic exhibit with penguins. The zoo is a leader in animal welfare and conservation education in western Mexico, attracting millions of visitors annually. Its commitment to creating immersive environments allows guests to feel as though they have traveled to different continents without leaving the city. The integration of technology and nature makes it a standout destination for both locals and international tourists.",
      es: "El zoológico de Guadalajara es considerado uno de los parques de animales más modernos y hermosos de todo México y está espectacularmente situado al borde de la Barranca de Huentitán. Ofrece a los visitantes experiencias únicas como el teleférico 'SkyZoo', que permite una vista impresionante de los recintos de animales y la barranca. El zoológico se distingue por su diversidad, que va desde un gran acuario hasta un aviario tropical y un recorrido de safari africano. Cabe destacar especialmente el compromiso del zoológico con la cría de especies en peligro de extinción y la investigación. La arquitectura de las instalaciones se integra armoniosamente en la topografía natural de la región. Para las familias, el parque es un punto culminante absoluto en el occidente de México, ya que combina educación y aventura al más alto nivel.",
      pt: "O zoológico de Guadalajara é considerado um dos parques de animais mais modernos e bonitos de todo o México e está espetacularmente localizado na borda da Barranca de Huentitán. Oferece aos visitantes experiências únicas, como o teleférico 'SkyZoo', que proporciona uma vista deslumbrante dos recintos dos animais e do desfiladeiro. O zoológico destaca-se pela sua diversidade, que vai desde um grande aquário e uma casa tropical até um passeio de safári africano. Destaca-se especialmente o compromisso do zoológico com a reprodução de espécies ameaçadas e a pesquisa. A arquitetura do complexo integra-se harmoniosamente na topografia natural da região. Para as famílias, o parque é um destaque absoluto no oeste do México, pois combina educação e aventura ao mais alto nível.",
      fr: "Le zoo de Guadalajara est considéré comme l'un des parcs animaliers les plus modernes et les plus beaux du Mexique. Il est spectaculairement situé au bord du canyon de Huentitán. Il propose aux visiteurs des expériences uniques comme le téléphérique 'SkyZoo', qui offre une vue imprenable sur les enclos des animaux et le canyon. Le zoo se distingue par sa diversité, allant d'un grand aquarium à un dôme tropical en passant par un safari africain. L'engagement du zoo dans l'élevage d'espèces menacées et la recherche est particulièrement remarquable. L'architecture du site s'intègre harmonieusement dans la topographie naturelle de la région. Pour les familles, le parc est un incontournable de l'ouest du Mexique, alliant éducation et aventure au plus haut niveau.",
    },
    factsAdvanced: {
      de: ["Wurde 1988 eingeweiht und umfasst heute rund 50 Hektar Fläche.", "Die SkyZoo-Seilbahn schwebt in 15 Metern Höhe über den Gehegen.", "Beherbergt das größte Aquarium im Landesinneren von Mexiko.", "Die 'Masai Mara Safari' bietet Begegnungen mit Nashörnern und Giraffen.", "Verfügt über ein spezielles Forschungszentrum für Primaten.", "Hält eine der bedeutendsten Sammlungen mexikanischer Reptilien.", "Bietet eine antarktische Pinguin-Anlage mit kontrollierter Temperatur."],
      hu: ["Guadalajara allatkert sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Guadalajara allatkert egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Gradina Zoologica Guadalajara ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Gradina Zoologica Guadalajara este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The zoo was inaugurated on March 25, 1988, by the President of Mexico.",
        "Its 'SkyZoo' cable car travels at a height of 15 meters, spanning 3.5 kilometers over the exhibits.",
        "The zoo's aquarium holds over 1.2 million liters of water and features a shark tunnel.",
        "It covers 50 hectares of land and houses more than 3,800 animals of 390 different species.",
        "The Safari Masai Mara offers an immersive 20-minute journey through an 8-hectare African savanna exhibit."
      ],
      es: ["Fue inaugurado en 1988 y hoy abarca unas 50 hectáreas de superficie.", "El teleférico SkyZoo flota a 15 metros de altura sobre los recintos.", "Alberga el acuario más grande del interior de México.", "El 'Safari Masai Mara' ofrece encuentros con rinocerontes y jirafas.", "Cuenta con un centro de investigación especializado para primates.", "Mantiene una de las colecciones más importantes de reptiles mexicanos.", "Ofrece una exhibición de pingüinos antárticos con temperatura controlada."],
      pt: ["Foi inaugurado em 1988 e hoje abrange cerca de 50 hectares de área.", "O teleférico SkyZoo flutua a 15 metros de altura sobre os recintos.", "Abriga o maior aquário no interior do México.", "O 'Masai Mara Safari' oferece encontros com rinocerontes e girafas.", "Possui um centro de pesquisa especializado para primatas.", "Detém uma das coleções mais importantes de répteis mexicanos.", "Oferece uma instalação de pinguins antárticos com temperatura controlada."],
      fr: ["Inauguré en 1988, il couvre aujourd'hui environ 50 hectares.", "Le téléphérique SkyZoo survole les enclos à 15 mètres de hauteur.", "Abrite le plus grand aquarium de l'intérieur des terres du Mexique.", "Le 'Masai Mara Safari' propose des rencontres avec des rhinocéros et des girafes.", "Dispose d'un centre de recherche spécialisé pour les primates.", "Possède l'une des plus importantes collections de reptiles mexicains.", "Propose un espace pour manchots antarctiques avec température contrôlée."],
    }, image: "/poi-images/guadalajara-zoo-life-v2.webp",
    description: {
      de: "Ein moderner Zoo in Guadalajara, der für seine innovative Tierhaltung und Forschung bekannt ist.",
      hu: "Mexikó egyik legmodernebb állatkertje, amely a Huentitán-kanyon peremén helyezkedik el.",
      ro: "O grădină zoologică modernă situată spectaculos pe marginea canionului Huentitán, oferind experiențe unice de safari.",
      en: "One of the most modern zoos in Latin America, spectacularly situated on the edge of the Huentitán Canyon in Jalisco.",
      es: "Un zoológico moderno en Guadalajara, conocido por su innovadora gestión animal e investigación.",
      pt: "Um zoológico moderno em Guadalajara, conhecido por seu manejo inovador de animais e pesquisa.",
      fr: "Un zoo moderne à Guadalajara, connu pour sa gestion animale innovante et sa recherche.",
    },
    facts: {
      de: ["Eigener Safaripark", "Spektakuläres Aquarium", "Blick in den Huentitán-Canyon"],
      hu: ["50 hektáros terület", "Saját akváriummal rendelkezik", "Antarktiszi pingvin-bemutató"],
      ro: ["Include un sistem de telecabină", "Găzduiește un acvariu de clasă mondială", "Expoziție rară de pinguini antarctici"],
      en: ["Features a 3.5 km cable car", "Home to a world-class aquarium", "Includes an Antarctic penguin exhibit"],
      es: ["Parque de safari propio", "Acuario espectacular", "Vista al Cañón de Huentitán"],
      pt: ["Parque de safári próprio", "Aquário espetacular", "Vista para o Canyon Huentitán"],
      fr: ["Propre parc de safari", "Aquarium spectaculaire", "Vue sur le canyon de Huentitán"],
    }
  },
  {
    id: "acuario-inbursa-life-v2", "sights": {
      "de": [
            {
                  "name": "Árbol de Gernika",
                  "text": "Der Árbol de Gernika ist eine Nachbildung des symbolischen Baums des Baskenlandes in Mexiko-Stadt.",
                  "category": "landmark",
                  "coords": [
                        -99.201886,
                        19.433359
                  ]
            },
            {
                  "name": "Petróleos Mexicanos",
                  "text": "Das Petróleos Mexicanos ist ein Monument zu Ehren der staatlichen Ölgesellschaft in Mexiko-Stadt.",
                  "category": "historical",
                  "coords": [
                        -99.203107,
                        19.427455
                  ]
            },
            {
                  "name": "Taller de Arquitectura",
                  "text": "Der Taller de Arquitectura ist eine Architekturwerkstatt in Inbursa.",
                  "category": "landmark",
                  "coords": [
                        -99.235064,
                        19.41508
                  ]
            },
            {
                  "name": "Centro Banamex",
                  "text": "Das Centro Banamex ist ein Kulturzentrum in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.224082,
                        19.440158
                  ]
            },
            {
                  "name": "Omega Dance",
                  "text": "Omega Dance ist ein zeitgenössisches Tanzstudio in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.266997,
                        19.447038
                  ]
            },
            {
                  "name": "Casa de Música",
                  "text": "Casa de Música ist ein Konzerthaus in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.26524,
                        19.440625
                  ]
            },
            {
                  "name": "Hospital Del Niño Dios",
                  "text": "Hospital Del Niño Dios ist ein nach dem Jesuskind benanntes Kulturzentrum in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.259483,
                        19.450098
                  ]
            },
            {
                  "name": "Academia Real",
                  "text": "Academia Real ist eine Kunstakademie in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.277186,
                        19.46111
                  ]
            },
            {
                  "name": "Edy Forever Dance",
                  "text": "Edy Forever Dance ist eine Tanzschule in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.280228,
                        19.458228
                  ]
            },
            {
                  "name": "Payasos Ejecutivos",
                  "text": "Payasos Ejecutivos ist ein kultureller Ort für Clownshows in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.285027,
                        19.454644
                  ]
            },
            {
                  "name": "Academia Feel Infinity",
                  "text": "Academia Feel Infinity ist eine Tanz- und Fitnessakademie in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.287505,
                        19.459679
                  ]
            },
            {
                  "name": "Zariñana Academy",
                  "text": "Zariñana Academy ist ein Kunst- und Kulturzentrum in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.282787,
                        19.438732
                  ]
            },
            {
                  "name": "Reparacion De Niños Dios",
                  "text": "Reparacion De Niños Dios ist eine Werkstatt zur Restaurierung von Jesuskind-Figuren in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.281642,
                        19.446663
                  ]
            },
            {
                  "name": "Mundo Fantastico",
                  "text": "Mundo Fantastico ist ein Vergnügungspark in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.280914,
                        19.447779
                  ]
            },
            {
                  "name": "Clases De Pintura De Cerámica",
                  "text": "Clases De Pintura De Cerámica ist ein Keramikmalerei-Workshop in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.277998,
                        19.444396
                  ]
            },
            {
                  "name": "Mirador de lluvia",
                  "text": "Mirador de lluvia ist ein Aussichtspunkt über Inbursa.",
                  "category": "natural",
                  "coords": [
                        -99.219715,
                        19.400183
                  ]
            },
            {
                  "name": "Centro Cultural Nelson Mandela",
                  "text": "Centro Cultural Nelson Mandela ist ein nach Nelson Mandela benanntes Kulturzentrum in Inbursa.",
                  "category": "museum",
                  "coords": [
                        -99.196206,
                        19.429576
                  ]
            },
            {
                  "name": "Galería Oscar Román",
                  "text": "Galería Oscar Román ist eine Kunstgalerie in Inbursa.",
                  "category": "museum",
                  "coords": [
                        -99.197566,
                        19.427661
                  ]
            },
            {
                  "name": "Granja las Américas",
                  "text": "Granja las Américas ist ein Bauernhof mit Tierbegegnungen in Inbursa.",
                  "category": "recreational",
                  "coords": [
                        -99.225416,
                        19.435628
                  ]
            },
            {
                  "name": "Santa Muerte",
                  "text": "Santa Muerte ist eine Attraktion, die mit der gleichnamigen Volksheiligen verbunden ist, in Mexiko-Stadt.",
                  "category": "landmark",
                  "coords": [
                        -99.243481,
                        19.45263
                  ]
            },
            {
                  "name": "Parque Duraznos",
                  "text": "Parque Duraznos ist ein Park mit Pfirsichbäumen in Inbursa.",
                  "category": "natural",
                  "coords": [
                        -99.240118,
                        19.408899
                  ]
            },
            {
                  "name": "Bosque de las Lomas",
                  "text": "Waldpark in Inbursa, eröffnet im August 1994.",
                  "category": "natural",
                  "coords": [
                        -99.244548,
                        19.403408
                  ]
            },
            {
                  "name": "Auditorio Cri Crí",
                  "text": "Das Auditorio Cri Crí ist ein Theater, benannt nach der mexikanischen Kinderfigur, in Mexiko-Stadt.",
                  "category": "cultural",
                  "coords": [
                        -99.220417,
                        19.398046
                  ]
            },
            {
                  "name": "Glorieta Bosques",
                  "text": "Glorieta Bosques ist ein Kreisverkehr mit Waldlandschaft in Inbursa.",
                  "category": "natural",
                  "coords": [
                        -99.2333,
                        19.40704
                  ]
            },
            {
                  "name": "Auditorio Alfonso Reyes",
                  "text": "Das Auditorio Alfonso Reyes ist ein Theater, benannt nach dem mexikanischen Schriftsteller, in Mexiko-Stadt.",
                  "category": "cultural",
                  "coords": [
                        -99.222232,
                        19.397944
                  ]
            },
            {
                  "name": "Monumento Magno Conmemorativo del Centenario del Ejército Mexicano",
                  "text": "Das Monumento Magno Conmemorativo del Centenario del Ejército Mexicano ist ein großes Monument zum hundertjährigen Bestehen der mexikanischen Armee in Mexiko-Stadt.",
                  "category": "historical",
                  "coords": [
                        -99.197247,
                        19.42606
                  ]
            },
            {
                  "name": "Teatro Angela Peralta",
                  "text": "Das Teatro Angela Peralta ist ein Theater in Acuario Inbursa Life v2.",
                  "category": "cultural",
                  "coords": [
                        -99.19446,
                        19.429159
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Árbol de Gernika",
                  "text": "Az Árbol de Gernika a baszk függetlenség szimbolikus fájának másolata Mexikóvárosban.",
                  "category": "landmark",
                  "coords": [
                        -99.201886,
                        19.433359
                  ]
            },
            {
                  "name": "Petróleos Mexicanos",
                  "text": "A Petróleos Mexicanos egy emlékmű az állami olajtársaság tiszteletére Mexikóvárosban.",
                  "category": "historical",
                  "coords": [
                        -99.203107,
                        19.427455
                  ]
            },
            {
                  "name": "Taller de Arquitectura",
                  "text": "A Taller de Arquitectura egy építészeti műhely Inbursában.",
                  "category": "landmark",
                  "coords": [
                        -99.235064,
                        19.41508
                  ]
            },
            {
                  "name": "Centro Banamex",
                  "text": "A Centro Banamex egy kulturális központ Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.224082,
                        19.440158
                  ]
            },
            {
                  "name": "Omega Dance",
                  "text": "Omega Dance egy kortárs táncstúdió Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.266997,
                        19.447038
                  ]
            },
            {
                  "name": "Casa de Música",
                  "text": "Casa de Música egy koncertterem Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.26524,
                        19.440625
                  ]
            },
            {
                  "name": "Hospital Del Niño Dios",
                  "text": "Hospital Del Niño Dios egy a Jézusgyermekről elnevezett kulturális központ Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.259483,
                        19.450098
                  ]
            },
            {
                  "name": "Academia Real",
                  "text": "Academia Real egy művészeti akadémia Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.277186,
                        19.46111
                  ]
            },
            {
                  "name": "Edy Forever Dance",
                  "text": "Edy Forever Dance egy tánciskola Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.280228,
                        19.458228
                  ]
            },
            {
                  "name": "Payasos Ejecutivos",
                  "text": "Payasos Ejecutivos egy bohócműsoroknak helyet adó kulturális helyszín Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.285027,
                        19.454644
                  ]
            },
            {
                  "name": "Academia Feel Infinity",
                  "text": "Academia Feel Infinity egy tánc- és fitnesz akadémia Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.287505,
                        19.459679
                  ]
            },
            {
                  "name": "Zariñana Academy",
                  "text": "Zariñana Academy egy művészeti és kulturális központ Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.282787,
                        19.438732
                  ]
            },
            {
                  "name": "Reparacion De Niños Dios",
                  "text": "Reparacion De Niños Dios egy Jézusgyermek-szobrok restaurálására szakosodott műhely Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.281642,
                        19.446663
                  ]
            },
            {
                  "name": "Mundo Fantastico",
                  "text": "Mundo Fantastico egy vidámpark Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.280914,
                        19.447779
                  ]
            },
            {
                  "name": "Clases De Pintura De Cerámica",
                  "text": "Clases De Pintura De Cerámica egy kerámiafestő műhely Inbursában.",
                  "category": "cultural",
                  "coords": [
                        -99.277998,
                        19.444396
                  ]
            },
            {
                  "name": "Mirador de lluvia",
                  "text": "Mirador de lluvia egy kilátóhely Inbursa fölött.",
                  "category": "natural",
                  "coords": [
                        -99.219715,
                        19.400183
                  ]
            },
            {
                  "name": "Centro Cultural Nelson Mandela",
                  "text": "Centro Cultural Nelson Mandela egy Nelson Mandeláról elnevezett kulturális központ Inbursában.",
                  "category": "museum",
                  "coords": [
                        -99.196206,
                        19.429576
                  ]
            },
            {
                  "name": "Galería Oscar Román",
                  "text": "Galería Oscar Román egy művészeti galéria Inbursában.",
                  "category": "museum",
                  "coords": [
                        -99.197566,
                        19.427661
                  ]
            },
            {
                  "name": "Granja las Américas",
                  "text": "Granja las Américas egy állattalálkozós farm Inbursában.",
                  "category": "recreational",
                  "coords": [
                        -99.225416,
                        19.435628
                  ]
            },
            {
                  "name": "Santa Muerte",
                  "text": "Santa Muerte egy látványosság, amely a névadó népi szenthez kapcsolódik Mexikóvárosban.",
                  "category": "landmark",
                  "coords": [
                        -99.243481,
                        19.45263
                  ]
            },
            {
                  "name": "Parque Duraznos",
                  "text": "Parque Duraznos egy őszibarackfás park Inbursában.",
                  "category": "natural",
                  "coords": [
                        -99.240118,
                        19.408899
                  ]
            },
            {
                  "name": "Bosque de las Lomas",
                  "text": "Erdőpark Inbursában, 1994 augusztusában nyílt meg.",
                  "category": "natural",
                  "coords": [
                        -99.244548,
                        19.403408
                  ]
            },
            {
                  "name": "Auditorio Cri Crí",
                  "text": "Az Auditorio Cri Crí egy színház, amelyet a mexikói gyermekfiguráról neveztek el Mexikóvárosban.",
                  "category": "cultural",
                  "coords": [
                        -99.220417,
                        19.398046
                  ]
            },
            {
                  "name": "Glorieta Bosques",
                  "text": "Glorieta Bosques egy erdei körforgalom Inbursában.",
                  "category": "natural",
                  "coords": [
                        -99.2333,
                        19.40704
                  ]
            },
            {
                  "name": "Auditorio Alfonso Reyes",
                  "text": "Az Auditorio Alfonso Reyes egy színház, amelyet a mexikói íróról neveztek el Mexikóvárosban.",
                  "category": "cultural",
                  "coords": [
                        -99.222232,
                        19.397944
                  ]
            },
            {
                  "name": "Monumento Magno Conmemorativo del Centenario del Ejército Mexicano",
                  "text": "A Monumento Magno Conmemorativo del Centenario del Ejército Mexicano egy nagy emlékmű a mexikói hadsereg centenáriumára Mexikóvárosban.",
                  "category": "historical",
                  "coords": [
                        -99.197247,
                        19.42606
                  ]
            },
            {
                  "name": "Teatro Angela Peralta",
                  "text": "A Teatro Angela Peralta egy színház az Acuario Inbursa Life v2-ben.",
                  "category": "cultural",
                  "coords": [
                        -99.19446,
                        19.429159
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Árbol de Gernika",
                  "text": "Árbol de Gernika este o replică a copacului simbolic al Țării Bascilor din Mexico City.",
                  "category": "landmark",
                  "coords": [
                        -99.201886,
                        19.433359
                  ]
            },
            {
                  "name": "Petróleos Mexicanos",
                  "text": "Petróleos Mexicanos este un monument în cinstea companiei petroliere de stat din Mexico City.",
                  "category": "historical",
                  "coords": [
                        -99.203107,
                        19.427455
                  ]
            },
            {
                  "name": "Taller de Arquitectura",
                  "text": "Taller de Arquitectura este un atelier de arhitectură în Inbursa.",
                  "category": "landmark",
                  "coords": [
                        -99.235064,
                        19.41508
                  ]
            },
            {
                  "name": "Centro Banamex",
                  "text": "Centro Banamex este un centru cultural în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.224082,
                        19.440158
                  ]
            },
            {
                  "name": "Omega Dance",
                  "text": "Omega Dance este un studio de dans contemporan în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.266997,
                        19.447038
                  ]
            },
            {
                  "name": "Casa de Música",
                  "text": "Casa de Música este o sală de concerte în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.26524,
                        19.440625
                  ]
            },
            {
                  "name": "Hospital Del Niño Dios",
                  "text": "Hospital Del Niño Dios este un centru cultural numit după Pruncul Isus în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.259483,
                        19.450098
                  ]
            },
            {
                  "name": "Academia Real",
                  "text": "Academia Real este o academie de artă în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.277186,
                        19.46111
                  ]
            },
            {
                  "name": "Edy Forever Dance",
                  "text": "Edy Forever Dance este o școală de dans în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.280228,
                        19.458228
                  ]
            },
            {
                  "name": "Payasos Ejecutivos",
                  "text": "Payasos Ejecutivos este un loc cultural pentru spectacole de clovni în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.285027,
                        19.454644
                  ]
            },
            {
                  "name": "Academia Feel Infinity",
                  "text": "Academia Feel Infinity este o academie de dans și fitness în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.287505,
                        19.459679
                  ]
            },
            {
                  "name": "Zariñana Academy",
                  "text": "Zariñana Academy este un centru de artă și cultură în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.282787,
                        19.438732
                  ]
            },
            {
                  "name": "Reparacion De Niños Dios",
                  "text": "Reparacion De Niños Dios este un atelier de restaurare a figurinelor Pruncului Isus în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.281642,
                        19.446663
                  ]
            },
            {
                  "name": "Mundo Fantastico",
                  "text": "Mundo Fantastico este un parc de distracții în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.280914,
                        19.447779
                  ]
            },
            {
                  "name": "Clases De Pintura De Cerámica",
                  "text": "Clases De Pintura De Cerámica este un atelier de pictură pe ceramică în Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.277998,
                        19.444396
                  ]
            },
            {
                  "name": "Mirador de lluvia",
                  "text": "Mirador de lluvia este un punct de belvedere deasupra Inbursei.",
                  "category": "natural",
                  "coords": [
                        -99.219715,
                        19.400183
                  ]
            },
            {
                  "name": "Centro Cultural Nelson Mandela",
                  "text": "Centro Cultural Nelson Mandela este un centru cultural numit după Nelson Mandela în Inbursa.",
                  "category": "museum",
                  "coords": [
                        -99.196206,
                        19.429576
                  ]
            },
            {
                  "name": "Galería Oscar Román",
                  "text": "Galería Oscar Román este o galerie de artă în Inbursa.",
                  "category": "museum",
                  "coords": [
                        -99.197566,
                        19.427661
                  ]
            },
            {
                  "name": "Granja las Américas",
                  "text": "Granja las Américas este o fermă cu întâlniri cu animale în Inbursa.",
                  "category": "recreational",
                  "coords": [
                        -99.225416,
                        19.435628
                  ]
            },
            {
                  "name": "Santa Muerte",
                  "text": "Santa Muerte este o atracție asociată cu sfânta populară omonimă în Mexico City.",
                  "category": "landmark",
                  "coords": [
                        -99.243481,
                        19.45263
                  ]
            },
            {
                  "name": "Parque Duraznos",
                  "text": "Parque Duraznos este un parc cu piersici în Inbursa.",
                  "category": "natural",
                  "coords": [
                        -99.240118,
                        19.408899
                  ]
            },
            {
                  "name": "Bosque de las Lomas",
                  "text": "Parcul forestier din Inbursa, deschis în august 1994.",
                  "category": "natural",
                  "coords": [
                        -99.244548,
                        19.403408
                  ]
            },
            {
                  "name": "Auditorio Cri Crí",
                  "text": "Auditorio Cri Crí este un teatru numit după personajul pentru copii mexican în Mexico City.",
                  "category": "cultural",
                  "coords": [
                        -99.220417,
                        19.398046
                  ]
            },
            {
                  "name": "Glorieta Bosques",
                  "text": "Glorieta Bosques este un sens giratoriu cu peisaj forestier în Inbursa.",
                  "category": "natural",
                  "coords": [
                        -99.2333,
                        19.40704
                  ]
            },
            {
                  "name": "Auditorio Alfonso Reyes",
                  "text": "Auditorio Alfonso Reyes este un teatru numit după scriitorul mexican în Mexico City.",
                  "category": "cultural",
                  "coords": [
                        -99.222232,
                        19.397944
                  ]
            },
            {
                  "name": "Monumento Magno Conmemorativo del Centenario del Ejército Mexicano",
                  "text": "Monumento Magno Conmemorativo del Centenario del Ejército Mexicano este un monument mare pentru centenarul Armatei Mexicane în Mexico City.",
                  "category": "historical",
                  "coords": [
                        -99.197247,
                        19.42606
                  ]
            },
            {
                  "name": "Teatro Angela Peralta",
                  "text": "Teatrul Angela Peralta este un teatru în Acuario Inbursa Life v2.",
                  "category": "cultural",
                  "coords": [
                        -99.19446,
                        19.429159
                  ]
            }
      ],
      "en": [
            {
                  "name": "Árbol de Gernika",
                  "text": "Árbol de Gernika is a replica of the symbolic tree of Basque freedom in Mexico City.",
                  "category": "landmark",
                  "coords": [
                        -99.201886,
                        19.433359
                  ]
            },
            {
                  "name": "Petróleos Mexicanos",
                  "text": "Petróleos Mexicanos is a monument honoring the state oil company in Mexico City.",
                  "category": "historical",
                  "coords": [
                        -99.203107,
                        19.427455
                  ]
            },
            {
                  "name": "Taller de Arquitectura",
                  "text": "Taller de Arquitectura is an architecture workshop in Inbursa.",
                  "category": "landmark",
                  "coords": [
                        -99.235064,
                        19.41508
                  ]
            },
            {
                  "name": "Centro Banamex",
                  "text": "Centro Banamex is a cultural center in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.224082,
                        19.440158
                  ]
            },
            {
                  "name": "Omega Dance",
                  "text": "Omega Dance is a contemporary dance studio in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.266997,
                        19.447038
                  ]
            },
            {
                  "name": "Casa de Música",
                  "text": "Casa de Música is a concert venue in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.26524,
                        19.440625
                  ]
            },
            {
                  "name": "Hospital Del Niño Dios",
                  "text": "Hospital Del Niño Dios is a cultural center named after the Child Jesus in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.259483,
                        19.450098
                  ]
            },
            {
                  "name": "Academia Real",
                  "text": "Academia Real is an art academy in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.277186,
                        19.46111
                  ]
            },
            {
                  "name": "Edy Forever Dance",
                  "text": "Edy Forever Dance is a dance school in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.280228,
                        19.458228
                  ]
            },
            {
                  "name": "Payasos Ejecutivos",
                  "text": "Payasos Ejecutivos is a cultural venue for clown performances in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.285027,
                        19.454644
                  ]
            },
            {
                  "name": "Academia Feel Infinity",
                  "text": "Academia Feel Infinity is a dance and fitness academy in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.287505,
                        19.459679
                  ]
            },
            {
                  "name": "Zariñana Academy",
                  "text": "Zariñana Academy is an arts and culture center in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.282787,
                        19.438732
                  ]
            },
            {
                  "name": "Reparacion De Niños Dios",
                  "text": "Reparacion De Niños Dios is a workshop for restoring Child Jesus figurines in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.281642,
                        19.446663
                  ]
            },
            {
                  "name": "Mundo Fantastico",
                  "text": "Mundo Fantastico is an amusement park in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.280914,
                        19.447779
                  ]
            },
            {
                  "name": "Clases De Pintura De Cerámica",
                  "text": "Clases De Pintura De Cerámica is a ceramic painting workshop in Inbursa.",
                  "category": "cultural",
                  "coords": [
                        -99.277998,
                        19.444396
                  ]
            },
            {
                  "name": "Mirador de lluvia",
                  "text": "Mirador de lluvia is a viewpoint overlooking Inbursa.",
                  "category": "natural",
                  "coords": [
                        -99.219715,
                        19.400183
                  ]
            },
            {
                  "name": "Centro Cultural Nelson Mandela",
                  "text": "Centro Cultural Nelson Mandela is a cultural center named after Nelson Mandela in Inbursa.",
                  "category": "museum",
                  "coords": [
                        -99.196206,
                        19.429576
                  ]
            },
            {
                  "name": "Galería Oscar Román",
                  "text": "Galería Oscar Román is an art gallery in Inbursa.",
                  "category": "museum",
                  "coords": [
                        -99.197566,
                        19.427661
                  ]
            },
            {
                  "name": "Granja las Américas",
                  "text": "Granja las Américas is a farm with animal encounters in Inbursa.",
                  "category": "recreational",
                  "coords": [
                        -99.225416,
                        19.435628
                  ]
            },
            {
                  "name": "Santa Muerte",
                  "text": "Santa Muerte is an attraction associated with the folk saint of the same name in Mexico City.",
                  "category": "landmark",
                  "coords": [
                        -99.243481,
                        19.45263
                  ]
            },
            {
                  "name": "Parque Duraznos",
                  "text": "Parque Duraznos is a park with peach trees in Inbursa.",
                  "category": "natural",
                  "coords": [
                        -99.240118,
                        19.408899
                  ]
            },
            {
                  "name": "Bosque de las Lomas",
                  "text": "Forest park in Inbursa, opened in August 1994.",
                  "category": "natural",
                  "coords": [
                        -99.244548,
                        19.403408
                  ]
            },
            {
                  "name": "Auditorio Cri Crí",
                  "text": "Auditorio Cri Crí is a theatre named after the Mexican children's character in Mexico City.",
                  "category": "cultural",
                  "coords": [
                        -99.220417,
                        19.398046
                  ]
            },
            {
                  "name": "Glorieta Bosques",
                  "text": "Glorieta Bosques is a roundabout with forest landscaping in Inbursa.",
                  "category": "natural",
                  "coords": [
                        -99.2333,
                        19.40704
                  ]
            },
            {
                  "name": "Auditorio Alfonso Reyes",
                  "text": "Auditorio Alfonso Reyes is a theatre named after the Mexican writer in Mexico City.",
                  "category": "cultural",
                  "coords": [
                        -99.222232,
                        19.397944
                  ]
            },
            {
                  "name": "Monumento Magno Conmemorativo del Centenario del Ejército Mexicano",
                  "text": "Monumento Magno Conmemorativo del Centenario del Ejército Mexicano is a large monument for the centennial of the Mexican Army in Mexico City.",
                  "category": "historical",
                  "coords": [
                        -99.197247,
                        19.42606
                  ]
            },
            {
                  "name": "Teatro Angela Peralta",
                  "text": "The Teatro Angela Peralta is a theatre in Acuario Inbursa Life v2.",
                  "category": "cultural",
                  "coords": [
                        -99.19446,
                        19.429159
                  ]
            }
      ]
},
    type: "kid-landmark",
    parent: "MX-CMX",
    coords: [-99.2088, 19.4326],
    name: { de: "Acuario Inbursa", hu: "Acuario Inbursa", ro: "Acuario Inbursa", en: "Acuario Inbursa"},
    image: "/poi-images/acuario-inbursa-life-v2.webp",
    descriptionAdvanced: {
      de: "Das Acuario Inbursa in Mexiko-Stadt ist ein architektonisches und biologisches Wunderwerk, das sich größtenteils unter der Erdoberfläche befindet. Mit über 14.000 Exemplaren aus 350 verschiedenen Arten bietet es einen tiefen Einblick in die marinen Ökosysteme der Weltmeere. Das Aquarium ist in verschiedene Themenbereiche unterteilt, die von Korallenriffen bis hin zu den dunklen Tiefen der Tiefsee reichen. Besonders beeindruckend sind die Haifischbecken und die Pinguinanlage, die Besuchern das Gefühl geben, mitten im Ozean zu sein. Die Einrichtung legt großen Wert auf den Schutz der Meere und informiert über die Gefahren durch Umweltverschmutzung und Überfischung. Es ist ein Ort des Staunens, der zeigt, wie zerbrechlich und schützenswert das Leben unter Wasser ist. Biologie K6 - Marines Leben und Umweltschutz.",
      hu: "Acuario Inbursa egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Acuario Inbursa kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Acuario Inbursa este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Acuario Inbursa este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Acuario Inbursa is a modern subterranean marvel located in the upscale Polanco district of Mexico City, right across from the Soumaya Museum. Designed by architect Gerardo García, the aquarium spans four underground levels, taking visitors on a journey from the ocean floor to the coastal mangroves. It is the largest aquarium in Mexico, housing thousands of marine creatures from over 300 different species, including sharks, rays, and exotic jellyfish. The facility is not just a tourist attraction but also a center for marine conservation and education, supported by the Blau Life Foundation. Its state-of-the-art life support systems and immersive exhibits, such as the sunken shipwreck and the penguinarium, provide a captivating look into the mysteries of the deep. This urban oasis serves as a crucial platform for raising awareness about the threats facing our oceans and the importance of marine protection.",
      es: "El Acuario Inbursa en la Ciudad de México es una maravilla arquitectónica y biológica que se encuentra mayoritariamente bajo la superficie terrestre. Con más de 14.000 ejemplares de 350 especies diferentes, ofrece una visión profunda de los ecosistemas marinos de los océanos del mundo. El acuario se divide en varias áreas temáticas, que van desde arrecifes de coral hasta las oscuras profundidades del mar. Son especialmente impresionantes los tanques de tiburones y la zona de pingüinos, que dan a los visitantes la sensación de estar en medio del océano. La institución pone gran énfasis en la protección de los mares e informa sobre los peligros de la contaminación y la sobrepesca. Es un lugar de asombro que muestra lo frágil y digna de protección que es la vida bajo el agua.",
      pt: "O Aquário Inbursa, na Cidade do México, é uma maravilha arquitetónica e biológica localizada maioritariamente abaixo da superfície terrestre. Com mais de 14.000 exemplares de 350 espécies diferentes, oferece uma visão profunda dos ecossistemas marinhos dos oceanos do mundo. O aquário está dividido em várias áreas temáticas, que vão desde recifes de coral até às profundezas escuras do mar profundo. São especialmente impressionantes os tanques de tubarões e a área dos pinguins, que dão aos visitantes a sensação de estarem no meio do oceano. A instituição dá grande importância à proteção dos mares e informa sobre os perigos da poluição ambiental e da sobrepesca. É um lugar de espanto que mostra como a vida subaquática é frágil e digna de proteção.",
      fr: "L'aquarium Inbursa de Mexico est une merveille architecturale et biologique située en grande partie sous la surface de la terre. Avec plus de 14 000 spécimens de 350 espèces différentes, il offre un aperçu approfondi des écosystèmes marins des océans du monde. L'aquarium est divisé en plusieurs zones thématiques allant des récifs coralliens aux abysses sombres. Les bassins de requins et l'espace des pingouins sont particulièrement impressionnants, donnant aux visiteurs l'impression d'être au milieu de l'océan. L'établissement accorde une grande importance à la protection des mers et informe sur les dangers de la pollution et de la surpêche. C'est un lieu d'émerveillement qui montre à quel point la vie sous-marine est fragile et précieuse.",
    },
    factsAdvanced: {
      de: ["Das Aquarium erstreckt sich über vier unterirdische Ebenen.", "Es werden mehr als 22 Millionen Liter Meerwasser aus dem Golf von Mexiko verwendet.", "Beherbergt seltene Arten wie Sandtigerhaie und verschiedene Rochenarten.", "Verfügt über ein hochmodernes System zur Wasserfilterung und Sauerstoffanreicherung.", "Das Gebäude wurde vom Architekten Fernando Romero entworfen.", "Bietet interaktive 'Touch Pools' für den direkten Kontakt mit Meeresbewohnern.", "Teil des Projekts ist ein spezialisiertes Zentrum für die Zucht von Korallen."],
      hu: ["Acuario Inbursa sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Acuario Inbursa egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Acuario Inbursa ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Acuario Inbursa este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The aquarium was opened in 2014 and required an investment of over 250 million pesos.",
        "It contains over 1.6 million liters of seawater, which is transported from the Gulf of Mexico.",
        "The facility is located 26 meters below ground level to preserve the aesthetic of the surrounding plaza.",
        "It is home to the first gentoo and chinstrap penguins born in a controlled environment in Mexico.",
        "The 'Sunken Ship' exhibit features several species of large sharks and giant manta rays."
      ],
      es: ["El acuario se extiende a lo largo de cuatro niveles subterráneos.", "Se utilizan más de 22 millones de litros de agua marina del Golfo de México.", "Alberga especies raras como tiburones toro y diversas especies de rayas.", "Cuenta con un sistema ultramoderno de filtración de agua y oxigenación.", "El edificio fue diseñado por el arquitecto Fernando Romero.", "Ofrece 'Touch Pools' interactivos para el contacto directo con habitantes marinos.", "Parte del proyecto es un centro especializado en el cultivo de corales."],
      pt: ["O aquário estende-se por quatro níveis subterrâneos.", "São utilizados mais de 22 milhões de litros de água do mar do Golfo do México.", "Abriga espécies raras, como tubarões-touro e várias espécies de raias.", "Possui um sistema ultramoderno de filtragem de água e oxigenação.", "O edifício foi projetado pelo arquiteto Fernando Romero.", "Oferece 'Touch Pools' interativos para contacto direto com os seres marinhos.", "Parte do projeto é um centro especializado para a criação de corais."],
      fr: ["L'aquarium s'étend sur quatre niveaux souterrains.", "Plus de 22 millions de litres d'eau de mer du golfe du Mexique sont utilisés.", "Abrite des espèces rares comme les requins-taureaux et diverses raies.", "Dispose d'un système ultra-moderne de filtration et d'oxygénation de l'eau.", "Le bâtiment a été conçu par l'architecte Fernando Romero.", "Propose des bassins tactiles interactifs pour un contact direct avec les animaux.", "Le projet comprend un centre spécialisé pour la culture des coraux."],
    }, image: "/poi-images/acuario-inbursa-life-v2.webp",
    description: {
      de: "Eines der größten Aquarien Lateinamerikas, direkt im schicken Viertel Polanco in Mexiko-Stadt.",
      hu: "Mexikóváros modern, föld alatti akváriuma, amely négy szinten mutatja be a tengerek élővilágát.",
      ro: "Un acvariu subteran de ultimă generație situat în Polanco, care adăpostește mii de creaturi marine pe patru niveluri.",
      en: "A state-of-the-art subterranean aquarium in Polanco, housing thousands of marine creatures across four underground levels.",
      es: "Uno de los acuarios más grandes de Latinoamérica, ubicado en el exclusivo barrio de Polanco en la Ciudad de México.",
      pt: "Um dos maiores aquários da América Latina, localizado no bairro chique de Polanco, na Cidade do México.",
      fr: "L'un des plus grands aquariums d'Amérique latine, situé dans le quartier chic de Polanco à Mexico.",
    },
    facts: {
      de: ["Unterirdische Anlage", "Über 14.000 Meeresbewohner", "Interaktive Ausstellungen"],
      hu: ["26 méter mélyen a föld alatt", "350 különböző faj látható", "Interaktív érintőmedencék"],
      ro: ["Situat la 26 de metri sub pământ", "Peste 14.000 de exemplare expuse", "Adăpostește pinguini Gentoo și Chinstrap"],
      en: ["Located 26 meters underground", "Features over 14,000 specimens", "Home to gentoo and chinstrap penguins"],
      es: ["Instalación subterránea", "Más de 14.000 habitantes marinos", "Exposiciones interactivas"],
      pt: ["Instalação subterrânea", "Mais de 14.000 habitantes marinhos", "Exposições interativas"],
      fr: ["Installation souterraine", "Plus de 14 000 créatures marines", "Expositions interactives"],
    }
  },
  {
    id: "xcaret-park-life-v2", "sights": {
      "de": [
            {
                  "name": "Mulchí",
                  "text": "Mulchí ist eine präkolumbische Maya-Stätte mit einem Tempel in einer parkähnlichen Umgebung.",
                  "category": "historical",
                  "coords": [
                        -87.123898,
                        20.579263
                  ]
            },
            {
                  "name": "Xcaret",
                  "text": "Öko-archäologischer Themenpark in der Riviera Maya.",
                  "category": "recreational",
                  "coords": [
                        -87.119388,
                        20.580774
                  ]
            },
            {
                  "name": "XPLOR Park",
                  "text": "XPLOR Park ist ein Abenteuerpark mit Seilrutschen und unterirdischen Flüssen in der Riviera Maya.",
                  "category": "recreational",
                  "coords": [
                        -87.122751,
                        20.593327
                  ]
            },
            {
                  "name": "Residencia Art Project",
                  "text": "Residencia Art Project ist eine Künstlerresidenz in Park.",
                  "category": "museum",
                  "coords": [
                        -87.06589,
                        20.635217
                  ]
            },
            {
                  "name": "Frida Kahlo",
                  "text": "Das Frida-Kahlo-Museum ist ein der Künstlerin gewidmetes Museum in Park.",
                  "category": "museum",
                  "coords": [
                        -87.072757,
                        20.625921
                  ]
            },
            {
                  "name": "El Acuario de Playa",
                  "text": "Aquarium Beach ist ein Aquariumsmuseum in Park.",
                  "category": "museum",
                  "coords": [
                        -87.071468,
                        20.628367
                  ]
            },
            {
                  "name": "Alamo Sakahua",
                  "text": "Monument eines Baumes mit 18 Metern Durchmesser im Xcaret-Gebiet.",
                  "category": "historical",
                  "coords": [
                        -87.064815,
                        20.634554
                  ]
            },
            {
                  "name": "Playalingua",
                  "text": "Playalingua ist eine Sprachschule in Park.",
                  "category": "cultural",
                  "coords": [
                        -87.069882,
                        20.63089
                  ]
            },
            {
                  "name": "Teatro Comunale",
                  "text": "Teatro Comunale ist ein Stadttheater in Park.",
                  "category": "cultural",
                  "coords": [
                        -87.10547,
                        20.655872
                  ]
            },
            {
                  "name": "Casa de la Cultura",
                  "text": "Casa de la Cultura ist ein Kulturhaus in Park.",
                  "category": "cultural",
                  "coords": [
                        -87.083521,
                        20.651359
                  ]
            },
            {
                  "name": "Teatro De La Ciudad Solidaridad",
                  "text": "Teatro De La Ciudad Solidaridad ist ein Stadttheater in Park.",
                  "category": "cultural",
                  "coords": [
                        -87.08288,
                        20.652628
                  ]
            },
            {
                  "name": "Rio Secreto",
                  "text": "Rio Secreto ist ein unterirdischer Fluss in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.13398,
                        20.588231
                  ]
            },
            {
                  "name": "La Casa De Las Hamacas",
                  "text": "La Casa De Las Hamacas ist ein Hängemattenladen in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.068071,
                        20.641773
                  ]
            },
            {
                  "name": "Playa mamitas",
                  "text": "Playa mamitas ist ein Strandabschnitt in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.064977,
                        20.63074
                  ]
            },
            {
                  "name": "The Grand Sol de America Mariachi",
                  "text": "The Grand Sol de America Mariachi bietet Mariachi-Musikdarbietungen im Xcaret-Park-Gebiet.",
                  "category": "landmark",
                  "coords": [
                        -87.070552,
                        20.629164
                  ]
            },
            {
                  "name": "3D Museum",
                  "text": "Das 3D Museum ist ein Museum mit dreidimensionalen Ausstellungen im Xcaret-Park-Gebiet.",
                  "category": "landmark",
                  "coords": [
                        -87.073352,
                        20.626961
                  ]
            },
            {
                  "name": "Skydive Playa",
                  "text": "Skydive Playa bietet Fallschirmsprünge im Xcaret-Park-Gebiet an.",
                  "category": "landmark",
                  "coords": [
                        -87.077991,
                        20.622084
                  ]
            },
            {
                  "name": "Chaak Tun",
                  "text": "Chaak Tun ist eine Cenote in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.12547,
                        20.652126
                  ]
            },
            {
                  "name": "Dog rescue center volunteering",
                  "text": "Dog rescue center volunteering ist ein Hundeschutzzentrum in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.105852,
                        20.633978
                  ]
            },
            {
                  "name": "Mayan village",
                  "text": "Mayan village ist ein traditionelles Maya-Dorf in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.121094,
                        20.579685
                  ]
            },
            {
                  "name": "parque fundadores",
                  "text": "Settlers park ist ein Park in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.074981,
                        20.622004
                  ]
            },
            {
                  "name": "El gran Tlachco",
                  "text": "El gran Tlachco ist eine Show über das antike Maya-Ballspiel im Xcaret-Park-Gebiet.",
                  "category": "landmark",
                  "coords": [
                        -87.119035,
                        20.584006
                  ]
            },
            {
                  "name": "Flying Men",
                  "text": "Flying Men ist eine Aufführung des Voladores-Rituals im Xcaret-Park-Gebiet.",
                  "category": "landmark",
                  "coords": [
                        -87.117791,
                        20.581419
                  ]
            },
            {
                  "name": "Juego de pelota",
                  "text": "Juego de pelota ist ein Maya-Ballspielplatz in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.120855,
                        20.580759
                  ]
            },
            {
                  "name": "Capilla de Guadalupe",
                  "text": "Capilla de Guadalupe ist eine Kapelle, die Unserer Lieben Frau von Guadalupe gewidmet ist, im Xcaret-Park-Gebiet.",
                  "category": "landmark",
                  "coords": [
                        -87.11982,
                        20.582351
                  ]
            },
            {
                  "name": "Korolevskii grif",
                  "text": "Korolevskii grif ist ein Aussichtspunkt mit einer Geierskulptur in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.122026,
                        20.579114
                  ]
            },
            {
                  "name": "Salida del Río Subterráneo",
                  "text": "Salida del Río Subterráneo ist der Ausgang eines unterirdischen Flusses im Xcaret-Park-Gebiet.",
                  "category": "landmark",
                  "coords": [
                        -87.121349,
                        20.577085
                  ]
            },
            {
                  "name": "Zona arqueologica",
                  "text": "Zona arqueologica ist eine archäologische Stätte in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.119435,
                        20.579033
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Mulchí",
                  "text": "Mulchí egy prekolumbiánus maja lelőhely templommal, parkos környezetben.",
                  "category": "historical",
                  "coords": [
                        -87.123898,
                        20.579263
                  ]
            },
            {
                  "name": "Xcaret",
                  "text": "Öko-régészeti tematikus park a Riviera Mayán.",
                  "category": "recreational",
                  "coords": [
                        -87.119388,
                        20.580774
                  ]
            },
            {
                  "name": "XPLOR Park",
                  "text": "XPLOR Park egy kalandpark kötélpályákkal és földalatti folyókkal a Riviéra Mayán.",
                  "category": "recreational",
                  "coords": [
                        -87.122751,
                        20.593327
                  ]
            },
            {
                  "name": "Residencia Art Project",
                  "text": "Residencia Art Project egy művészrezidencia Parkban.",
                  "category": "museum",
                  "coords": [
                        -87.06589,
                        20.635217
                  ]
            },
            {
                  "name": "Frida Kahlo",
                  "text": "A Frida Kahlo Múzeum egy a művésznek szentelt múzeum Parkban.",
                  "category": "museum",
                  "coords": [
                        -87.072757,
                        20.625921
                  ]
            },
            {
                  "name": "El Acuario de Playa",
                  "text": "Aquarium Beach egy akváriummúzeum Parkban.",
                  "category": "museum",
                  "coords": [
                        -87.071468,
                        20.628367
                  ]
            },
            {
                  "name": "Alamo Sakahua",
                  "text": "Egy 18 méter átmérőjű fa emlékműve a Xcaret területén.",
                  "category": "historical",
                  "coords": [
                        -87.064815,
                        20.634554
                  ]
            },
            {
                  "name": "Playalingua",
                  "text": "Playalingua egy nyelviskola Parkban.",
                  "category": "cultural",
                  "coords": [
                        -87.069882,
                        20.63089
                  ]
            },
            {
                  "name": "Teatro Comunale",
                  "text": "Teatro Comunale egy városi színház Parkban.",
                  "category": "cultural",
                  "coords": [
                        -87.10547,
                        20.655872
                  ]
            },
            {
                  "name": "Casa de la Cultura",
                  "text": "Casa de la Cultura egy kultúrház Parkban.",
                  "category": "cultural",
                  "coords": [
                        -87.083521,
                        20.651359
                  ]
            },
            {
                  "name": "Teatro De La Ciudad Solidaridad",
                  "text": "Teatro De La Ciudad Solidaridad egy városi színház Parkban.",
                  "category": "cultural",
                  "coords": [
                        -87.08288,
                        20.652628
                  ]
            },
            {
                  "name": "Rio Secreto",
                  "text": "Rio Secreto egy föld alatti folyó Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.13398,
                        20.588231
                  ]
            },
            {
                  "name": "La Casa De Las Hamacas",
                  "text": "La Casa De Las Hamacas egy függőágybolt Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.068071,
                        20.641773
                  ]
            },
            {
                  "name": "Playa mamitas",
                  "text": "Playa mamitas egy strand Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.064977,
                        20.63074
                  ]
            },
            {
                  "name": "The Grand Sol de America Mariachi",
                  "text": "A The Grand Sol de America Mariachi Mariachi zenei előadásokat kínál a Xcaret park területén.",
                  "category": "landmark",
                  "coords": [
                        -87.070552,
                        20.629164
                  ]
            },
            {
                  "name": "3D Museum",
                  "text": "A 3D Múzeum egy háromdimenziós kiállításokkal rendelkező múzeum a Xcaret park területén.",
                  "category": "landmark",
                  "coords": [
                        -87.073352,
                        20.626961
                  ]
            },
            {
                  "name": "Skydive Playa",
                  "text": "A Skydive Playa ejtőernyős ugrásokat kínál a Xcaret park területén.",
                  "category": "landmark",
                  "coords": [
                        -87.077991,
                        20.622084
                  ]
            },
            {
                  "name": "Chaak Tun",
                  "text": "Chaak Tun egy cenote Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.12547,
                        20.652126
                  ]
            },
            {
                  "name": "Dog rescue center volunteering",
                  "text": "Dog rescue center volunteering egy kutyamentő központ Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.105852,
                        20.633978
                  ]
            },
            {
                  "name": "Mayan village",
                  "text": "Mayan village egy hagyományos maja falu Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.121094,
                        20.579685
                  ]
            },
            {
                  "name": "parque fundadores",
                  "text": "Settlers park egy park Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.074981,
                        20.622004
                  ]
            },
            {
                  "name": "El gran Tlachco",
                  "text": "El gran Tlachco egy show az ősi maja labdajátékról a Xcaret park területén.",
                  "category": "landmark",
                  "coords": [
                        -87.119035,
                        20.584006
                  ]
            },
            {
                  "name": "Flying Men",
                  "text": "A Flying Men a Voladores szertartás előadása a Xcaret park területén.",
                  "category": "landmark",
                  "coords": [
                        -87.117791,
                        20.581419
                  ]
            },
            {
                  "name": "Juego de pelota",
                  "text": "Juego de pelota egy maja labdapálya Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.120855,
                        20.580759
                  ]
            },
            {
                  "name": "Capilla de Guadalupe",
                  "text": "Capilla de Guadalupe egy kápolna, amely a Guadalupei Szűzanyának van szentelve, a Xcaret park területén.",
                  "category": "landmark",
                  "coords": [
                        -87.11982,
                        20.582351
                  ]
            },
            {
                  "name": "Korolevskii grif",
                  "text": "Korolevskii grif egy kilátóhely keselyűszoborral Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.122026,
                        20.579114
                  ]
            },
            {
                  "name": "Salida del Río Subterráneo",
                  "text": "A Salida del Río Subterráneo egy földalatti folyó kijárata a Xcaret park területén.",
                  "category": "landmark",
                  "coords": [
                        -87.121349,
                        20.577085
                  ]
            },
            {
                  "name": "Zona arqueologica",
                  "text": "Zona arqueologica egy régészeti lelőhely Parkban.",
                  "category": "landmark",
                  "coords": [
                        -87.119435,
                        20.579033
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Mulchí",
                  "text": "Mulchí este un sit maya precolumbian cu un templu, situat într-un cadru asemănător unui parc.",
                  "category": "historical",
                  "coords": [
                        -87.123898,
                        20.579263
                  ]
            },
            {
                  "name": "Xcaret",
                  "text": "Parcul tematic eco-arheologic pe Riviera Maya.",
                  "category": "recreational",
                  "coords": [
                        -87.119388,
                        20.580774
                  ]
            },
            {
                  "name": "XPLOR Park",
                  "text": "XPLOR Park este un parc de aventură cu tiroliane și râuri subterane în Riviera Maya.",
                  "category": "recreational",
                  "coords": [
                        -87.122751,
                        20.593327
                  ]
            },
            {
                  "name": "Residencia Art Project",
                  "text": "Residencia Art Project este o reședință pentru artiști în Park.",
                  "category": "museum",
                  "coords": [
                        -87.06589,
                        20.635217
                  ]
            },
            {
                  "name": "Frida Kahlo",
                  "text": "Muzeul Frida Kahlo este un muzeu dedicat artistei în Park.",
                  "category": "museum",
                  "coords": [
                        -87.072757,
                        20.625921
                  ]
            },
            {
                  "name": "El Acuario de Playa",
                  "text": "Aquarium Beach este un muzeu-acvariu în Park.",
                  "category": "museum",
                  "coords": [
                        -87.071468,
                        20.628367
                  ]
            },
            {
                  "name": "Alamo Sakahua",
                  "text": "Monument al unui copac cu diametrul de 18 metri în zona Xcaret.",
                  "category": "historical",
                  "coords": [
                        -87.064815,
                        20.634554
                  ]
            },
            {
                  "name": "Playalingua",
                  "text": "Playalingua este o școală de limbi străine în Park.",
                  "category": "cultural",
                  "coords": [
                        -87.069882,
                        20.63089
                  ]
            },
            {
                  "name": "Teatro Comunale",
                  "text": "Teatro Comunale este un teatru municipal în Park.",
                  "category": "cultural",
                  "coords": [
                        -87.10547,
                        20.655872
                  ]
            },
            {
                  "name": "Casa de la Cultura",
                  "text": "Casa de la Cultura este o casă de cultură în Park.",
                  "category": "cultural",
                  "coords": [
                        -87.083521,
                        20.651359
                  ]
            },
            {
                  "name": "Teatro De La Ciudad Solidaridad",
                  "text": "Teatro De La Ciudad Solidaridad este un teatru al orașului în Park.",
                  "category": "cultural",
                  "coords": [
                        -87.08288,
                        20.652628
                  ]
            },
            {
                  "name": "Rio Secreto",
                  "text": "Rio Secreto este un râu subteran în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.13398,
                        20.588231
                  ]
            },
            {
                  "name": "La Casa De Las Hamacas",
                  "text": "La Casa De Las Hamacas este un magazin de hamace în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.068071,
                        20.641773
                  ]
            },
            {
                  "name": "Playa mamitas",
                  "text": "Playa mamitas este o plajă în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.064977,
                        20.63074
                  ]
            },
            {
                  "name": "The Grand Sol de America Mariachi",
                  "text": "The Grand Sol de America Mariachi oferă spectacole de muzică mariachi în zona parcului Xcaret.",
                  "category": "landmark",
                  "coords": [
                        -87.070552,
                        20.629164
                  ]
            },
            {
                  "name": "3D Museum",
                  "text": "Muzeul 3D este un muzeu cu exponate tridimensionale în zona parcului Xcaret.",
                  "category": "landmark",
                  "coords": [
                        -87.073352,
                        20.626961
                  ]
            },
            {
                  "name": "Skydive Playa",
                  "text": "Skydive Playa oferă parașutism în zona parcului Xcaret.",
                  "category": "landmark",
                  "coords": [
                        -87.077991,
                        20.622084
                  ]
            },
            {
                  "name": "Chaak Tun",
                  "text": "Chaak Tun este o cenotă în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.12547,
                        20.652126
                  ]
            },
            {
                  "name": "Dog rescue center volunteering",
                  "text": "Dog rescue center volunteering este un centru de salvare a câinilor în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.105852,
                        20.633978
                  ]
            },
            {
                  "name": "Mayan village",
                  "text": "Mayan village este un sat tradițional mayaș în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.121094,
                        20.579685
                  ]
            },
            {
                  "name": "parque fundadores",
                  "text": "Settlers park este un parc în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.074981,
                        20.622004
                  ]
            },
            {
                  "name": "El gran Tlachco",
                  "text": "El gran Tlachco este un spectacol despre jocul de minge mayaș antic în zona parcului Xcaret.",
                  "category": "landmark",
                  "coords": [
                        -87.119035,
                        20.584006
                  ]
            },
            {
                  "name": "Flying Men",
                  "text": "Flying Men este o reprezentație a ritualului Voladores în zona parcului Xcaret.",
                  "category": "landmark",
                  "coords": [
                        -87.117791,
                        20.581419
                  ]
            },
            {
                  "name": "Juego de pelota",
                  "text": "Juego de pelota este un teren de joc cu mingea mayaș în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.120855,
                        20.580759
                  ]
            },
            {
                  "name": "Capilla de Guadalupe",
                  "text": "Capilla de Guadalupe este o capelă dedicată Maicii Domnului din Guadalupe, în zona parcului Xcaret.",
                  "category": "landmark",
                  "coords": [
                        -87.11982,
                        20.582351
                  ]
            },
            {
                  "name": "Korolevskii grif",
                  "text": "Korolevskii grif este un punct de belvedere cu o sculptură de vultur în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.122026,
                        20.579114
                  ]
            },
            {
                  "name": "Salida del Río Subterráneo",
                  "text": "Salida del Río Subterráneo este ieșirea unui râu subteran în zona parcului Xcaret.",
                  "category": "landmark",
                  "coords": [
                        -87.121349,
                        20.577085
                  ]
            },
            {
                  "name": "Zona arqueologica",
                  "text": "Zona arqueologica este un sit arheologic în Park.",
                  "category": "landmark",
                  "coords": [
                        -87.119435,
                        20.579033
                  ]
            }
      ],
      "en": [
            {
                  "name": "Mulchí",
                  "text": "Mulchí is a pre-Columbian Maya site with a temple, situated in a park-like setting.",
                  "category": "historical",
                  "coords": [
                        -87.123898,
                        20.579263
                  ]
            },
            {
                  "name": "Xcaret",
                  "text": "Eco-archaeological theme park in the Riviera Maya.",
                  "category": "recreational",
                  "coords": [
                        -87.119388,
                        20.580774
                  ]
            },
            {
                  "name": "XPLOR Park",
                  "text": "XPLOR Park is an adventure park with zip lines and underground rivers in the Riviera Maya.",
                  "category": "recreational",
                  "coords": [
                        -87.122751,
                        20.593327
                  ]
            },
            {
                  "name": "Residencia Art Project",
                  "text": "Residencia Art Project is an artist residency in Park.",
                  "category": "museum",
                  "coords": [
                        -87.06589,
                        20.635217
                  ]
            },
            {
                  "name": "Frida Kahlo",
                  "text": "Frida Kahlo is a museum dedicated to the artist in Park.",
                  "category": "museum",
                  "coords": [
                        -87.072757,
                        20.625921
                  ]
            },
            {
                  "name": "Aquarium Beach",
                  "text": "Aquarium Beach is an aquarium museum in Park.",
                  "category": "museum",
                  "coords": [
                        -87.071468,
                        20.628367
                  ]
            },
            {
                  "name": "tree of 18m diameter",
                  "text": "Monument of a tree with an 18-meter diameter in the Xcaret area.",
                  "category": "historical",
                  "coords": [
                        -87.064815,
                        20.634554
                  ]
            },
            {
                  "name": "Playalingua",
                  "text": "Playalingua is a language school in Park.",
                  "category": "cultural",
                  "coords": [
                        -87.069882,
                        20.63089
                  ]
            },
            {
                  "name": "Teatro Comunale",
                  "text": "Teatro Comunale is a municipal theater in Park.",
                  "category": "cultural",
                  "coords": [
                        -87.10547,
                        20.655872
                  ]
            },
            {
                  "name": "Casa de la Cultura",
                  "text": "Casa de la Cultura is a cultural center in Park.",
                  "category": "cultural",
                  "coords": [
                        -87.083521,
                        20.651359
                  ]
            },
            {
                  "name": "Teatro De La Ciudad Solidaridad",
                  "text": "Teatro De La Ciudad Solidaridad is a city theater in Park.",
                  "category": "cultural",
                  "coords": [
                        -87.08288,
                        20.652628
                  ]
            },
            {
                  "name": "Rio Secreto",
                  "text": "Rio Secreto is an underground river in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.13398,
                        20.588231
                  ]
            },
            {
                  "name": "La Casa De Las Hamacas",
                  "text": "La Casa De Las Hamacas is a hammock store in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.068071,
                        20.641773
                  ]
            },
            {
                  "name": "Playa mamitas",
                  "text": "Playa mamitas is a beach in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.064977,
                        20.63074
                  ]
            },
            {
                  "name": "The Grand Sol de America Mariachi",
                  "text": "The Grand Sol de America Mariachi offers mariachi music performances in the Xcaret park area.",
                  "category": "landmark",
                  "coords": [
                        -87.070552,
                        20.629164
                  ]
            },
            {
                  "name": "3D Museum",
                  "text": "3D Museum is a museum with three-dimensional exhibits in the Xcaret park area.",
                  "category": "landmark",
                  "coords": [
                        -87.073352,
                        20.626961
                  ]
            },
            {
                  "name": "Skydive Playa",
                  "text": "Skydive Playa offers skydiving experiences in the Xcaret park area.",
                  "category": "landmark",
                  "coords": [
                        -87.077991,
                        20.622084
                  ]
            },
            {
                  "name": "Chaak Tun",
                  "text": "Chaak Tun is a cenote in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.12547,
                        20.652126
                  ]
            },
            {
                  "name": "Dog rescue center volunteering",
                  "text": "Dog rescue center volunteering is a dog rescue center in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.105852,
                        20.633978
                  ]
            },
            {
                  "name": "Mayan village",
                  "text": "Mayan village is a traditional Maya village in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.121094,
                        20.579685
                  ]
            },
            {
                  "name": "settlers park",
                  "text": "Settlers park is a park in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.074981,
                        20.622004
                  ]
            },
            {
                  "name": "El gran Tlachco",
                  "text": "El gran Tlachco is a show about the ancient Mayan ball game in the Xcaret park area.",
                  "category": "landmark",
                  "coords": [
                        -87.119035,
                        20.584006
                  ]
            },
            {
                  "name": "Flying Men",
                  "text": "Flying Men is a performance of the Voladores ritual in the Xcaret park area.",
                  "category": "landmark",
                  "coords": [
                        -87.117791,
                        20.581419
                  ]
            },
            {
                  "name": "Juego de pelota",
                  "text": "Juego de pelota is a Maya ball court in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.120855,
                        20.580759
                  ]
            },
            {
                  "name": "Capilla de Guadalupe",
                  "text": "Capilla de Guadalupe is a chapel dedicated to Our Lady of Guadalupe in the Xcaret park area.",
                  "category": "landmark",
                  "coords": [
                        -87.11982,
                        20.582351
                  ]
            },
            {
                  "name": "Korolevskii grif",
                  "text": "Korolevskii grif is a viewpoint with a vulture sculpture in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.122026,
                        20.579114
                  ]
            },
            {
                  "name": "Salida del Río Subterráneo",
                  "text": "Salida del Río Subterráneo is the exit of an underground river in the Xcaret park area.",
                  "category": "landmark",
                  "coords": [
                        -87.121349,
                        20.577085
                  ]
            },
            {
                  "name": "Zona arqueologica",
                  "text": "Zona arqueologica is an archaeological site in Park.",
                  "category": "landmark",
                  "coords": [
                        -87.119435,
                        20.579033
                  ]
            }
      ]
},
    type: "kid-landmark",
    parent: "MX-ROO",
    coords: [-87.0713, 20.5818],
    name: { de: "Xcaret Park", hu: "Xcaret park", ro: "Parcul Xcaret", en: "Xcaret Park"},
    image: "/poi-images/xcaret-park-life-v2.webp",
    descriptionAdvanced: {
      de: "Xcaret ist weit mehr als nur ein Freizeitpark; es ist ein öko-archäologisches Paradies an der Riviera Maya, das die reiche Kultur und Natur Mexikos feiert. Die Besucher können in unterirdischen Flüssen schwimmen, Maya-Ruinen erkunden und die prachtvolle Flora und Fauna der Region bewundern. Ein absoluter Höhepunkt ist die abendliche Show 'Xcaret México Espectacular', die die Geschichte des Landes mit Hunderten von Künstlern eindrucksvoll inszeniert. Der Park engagiert sich stark für den Naturschutz, insbesondere für die Rettung von Meeresschildkröten und die Zucht von roten Aras. Die harmonische Verbindung von traditioneller Kultur, archäologischen Schätzen und kristallklarem Wasser macht Xcaret zu einem der beliebtesten Reiseziele weltweit. Geographie K7 - Tourismus und Naturschutz.",
      hu: "Xcaret park egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Xcaret park kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Parcul Xcaret este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Parcul Xcaret este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Xcaret Park is a world-renowned eco-archaeological destination located on the Riviera Maya, blending natural beauty, Mexican culture, and history into a single immersive experience. Built on the site of a former Maya trading port, the park features authentic archaeological ruins alongside underground rivers, lagoons, and a pristine coastline. Visitors can swim through crystal-clear freshwater caves, visit a massive coral reef aquarium, and walk through one of the world's largest butterfly pavilions. The park's highlight is the spectacular 'Xcaret México Espectacular' evening show, which tells the history of Mexico through music, dance, and ancient ball games. Xcaret is also a major player in conservation, notably for its sea turtle nesting program and the reproduction of scarlet macaws. It is a place where the vibrant traditions of Mexico are celebrated daily, making it a must-visit for anyone seeking to understand the country's diverse heritage.",
      es: "Xcaret es mucho más que un simple parque de diversiones; es un paraíso eco-arqueológico en la Riviera Maya que celebra la rica cultura y naturaleza de México. Los visitantes pueden nadar en ríos subterráneos, explorar ruinas mayas y admirar la espléndida flora y fauna de la región. Un punto culminante absoluto es el espectáculo nocturno 'Xcaret México Espectacular', que escenifica de forma impresionante la historia del país con cientos de artistas. El parque está fuertemente comprometido con la conservación de la naturaleza, especialmente con el rescate de tortugas marinas y la cría de guacamayas rojas. La armoniosa combinación de cultura tradicional, tesoros arqueológicos y aguas cristalinas convierte a Xcaret en uno de los destinos turísticos más populares del mundo.",
      pt: "Xcaret é muito mais do que apenas um parque de diversões; é um paraíso eco-arqueológico na Riviera Maya que celebra a rica cultura e natureza do México. Os visitantes podem nadar em rios subterrâneos, explorar ruínas maias e admirar a esplêndida flora e fauna da região. Um destaque absoluto é o espetáculo noturno 'Xcaret México Espectacular', que encena a história do país de forma impressionante com centenas de artistas. O parque está fortemente empenhado na conservação da natureza, especialmente no resgate de tartarugas marinhas e na criação de araras vermelhas. A combinação harmoniosa de cultura tradicional, tesouros arqueológicos e águas cristalinas torna Xcaret um dos destinos de viagem mais populares do mundo.",
      fr: "Xcaret est bien plus qu'un simple parc d'attractions ; c'est un paradis éco-archéologique sur la Riviera Maya qui célèbre la richesse de la culture et de la nature mexicaines. Les visiteurs peuvent nager dans des rivières souterraines, explorer des ruines mayas et admirer la splendide flore et faune de la région. Le clou du spectacle est le show nocturne 'Xcaret México Espectacular', qui met en scène l'histoire du pays avec des centaines d'artistes. Le parc s'engage fortement dans la protection de la nature, notamment pour le sauvetage des tortues marines et l'élevage des aras rouges. L'alliance harmonieuse de la culture traditionnelle, des trésors archéologiques et des eaux cristallines fait de Xcaret l'une des destinations les plus prisées au monde.",
    },
    factsAdvanced: {
      de: ["Der Park wurde 1990 auf dem Gelände einer antiken Maya-Hafenstadt eröffnet.", "Verfügt über drei verschiedene unterirdische Flüsse zum Schwimmen.", "Beherbergt einen der weltweit erfolgreichsten Korallenriff-Aquarien.", "Das Schutzprogramm für Meeresschildkröten hat bereits Millionen Tiere gerettet.", "Die Abendshow umfasst über 300 Schauspieler und Musiker.", "Xcaret wurde mehrfach als bester Themenpark der Welt ausgezeichnet.", "Im Park befindet sich eine traditionelle mexikanische Hacienda und ein Orchideenhaus."],
      hu: ["Xcaret park sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Xcaret park egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Parcul Xcaret ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Parcul Xcaret este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Xcaret opened in 1990 and has since become one of the most famous tourist attractions in Mexico.",
        "The park is home to a traditional Maya village where ancient rituals and crafts are demonstrated.",
        "Its Coral Reef Aquarium is one of the few in the world where visitors can see reef ecosystems in a semi-natural state.",
        "The evening show features over 300 artists and covers 3,000 years of Mexican history.",
        "Xcaret holds a Guinness World Record for its scarlet macaw conservation and breeding efforts."
      ],
      es: ["El parque fue inaugurado en 1990 en el sitio de una antigua ciudad portuaria maya.", "Cuenta con tres ríos subterráneos diferentes para nadar.", "Alberga uno de los acuarios de arrecife de coral más exitosos del mundo.", "El programa de protección de tortugas marinas ha salvado ya a millones de animales.", "El espectáculo nocturno cuenta con más de 300 actores y músicos.", "Xcaret ha sido premiado varias veces como el mejor parque temático del mundo.", "En el parque hay una hacienda tradicional mexicana y un orquideario."],
      pt: ["O parque foi inaugurado em 1990 no local de uma antiga cidade portuária maia.", "Possui três rios subterrâneos diferentes para nadar.", "Abriga um dos aquários de recifes de coral mais bem-sucedidos do mundo.", "O programa de proteção de tartarugas marinhas já salvou milhões de animais.", "O show noturno inclui mais de 300 atores e músicos.", "Xcaret foi premiado várias vezes como o melhor parque temático do mundo.", "No parque encontra-se uma fazenda mexicana tradicional e um orquidário."],
      fr: ["Le parc a ouvert ses portes en 1990 sur le site d'une ancienne cité portuaire maya.", "Il dispose de trois rivières souterraines différentes pour la baignade.", "Il abrite l'un des aquariums de récifs coralliens les plus réussis au monde.", "Le programme de protection des tortues marines a déjà sauvé des millions d'animaux.", "Le spectacle du soir réunit plus de 300 acteurs et musiciens.", "Xcaret a été élu à plusieurs reprises meilleur parc à thème au monde.", "Le parc comprend une hacienda mexicaine traditionnelle et une serre d'orchidées."],
    }, image: "/poi-images/xcaret-park-life-v2.webp",
    description: {
      de: "Ein ökologischer Themenpark an der Riviera Maya, der Natur, Kultur und Archäologie verbindet.",
      hu: "A Riviéra Maya híres öko-régészeti parkja, ahol a maja kultúra és a természet találkozik.",
      ro: "Un parc eco-arheologic renumit pe Riviera Maya, care celebrează cultura mexicană și frumusețea naturală prin râuri subterane.",
      en: "A premier eco-archaeological park on the Riviera Maya that celebrates Mexican culture, history, and natural beauty.",
      es: "Un parque temático ecológico en la Riviera Maya que combina naturaleza, cultura y arqueología.",
      pt: "Um parque temático ecológico na Riviera Maya que combina natureza, cultura e arqueologia.",
      fr: "Un parc à thème écologique sur la Riviera Maya, alliant nature, culture et archéologie.",
    },
    facts: {
      de: ["Unterirdische Flüsse", "Abendliche Folklore-Shows", "Zentrum für Meeresschildkröten"],
      hu: ["Maja romok a park területén", "Hatalmas korallzátony-akvárium", "Esti folklór előadás 300 művésszel"],
      ro: ["Găzduiește spectacole folclorice nocturne", "Include ruine arheologice mayașe", "Centru major pentru protecția țestoaselor"],
      en: ["Features underground rivers", "Famous for its nightly folklore show", "Major sea turtle nesting site"],
      es: ["Ríos subterráneos", "Espectáculos de folklore nocturnos", "Centro de tortugas marinas"],
      pt: ["Rios subterrâneos", "Shows de folclore noturnos", "Centro de tartarugas marinhas"],
      fr: ["Rivières souterraines", "Spectacles folkloriques nocturnes", "Centre pour les tortues marines"],
    }
  },
  {
    id: "africam-safari-life-v2", "sights": {
      "de": [
            {
                  "name": "Africam Safari",
                  "text": "Africam Safari ist ein Safaripark mit frei laufenden Tieren in Puebla.",
                  "category": "recreational",
                  "coords": [
                        -98.133038,
                        18.936376
                  ]
            },
            {
                  "name": "Parque Estatal Humedal de Valsequillo",
                  "text": "Parque Estatal Humedal de Valsequillo ist ein staatlicher Feuchtgebietspark im Gebiet von Africam Safari.",
                  "category": "natural",
                  "coords": [
                        -98.176392,
                        18.921775
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Africam Safari",
                  "text": "Africam Safari egy szafaripark szabadon kóborló állatokkal Pueblában.",
                  "category": "recreational",
                  "coords": [
                        -98.133038,
                        18.936376
                  ]
            },
            {
                  "name": "Parque Estatal Humedal de Valsequillo",
                  "text": "A Parque Estatal Humedal de Valsequillo egy állami vizes élőhely park az Africam Safari területén.",
                  "category": "natural",
                  "coords": [
                        -98.176392,
                        18.921775
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Africam Safari",
                  "text": "Africam Safari este un parc safari cu animale care se plimbă liber în Puebla.",
                  "category": "recreational",
                  "coords": [
                        -98.133038,
                        18.936376
                  ]
            },
            {
                  "name": "Parque Estatal Humedal de Valsequillo",
                  "text": "Parque Estatal Humedal de Valsequillo este un parc de zone umede de stat în zona Africam Safari.",
                  "category": "natural",
                  "coords": [
                        -98.176392,
                        18.921775
                  ]
            }
      ],
      "en": [
            {
                  "name": "Africam Safari",
                  "text": "Africam Safari is a safari park with free-roaming animals in Puebla.",
                  "category": "recreational",
                  "coords": [
                        -98.133038,
                        18.936376
                  ]
            },
            {
                  "name": "Parque Estatal Humedal de Valsequillo",
                  "text": "Parque Estatal Humedal de Valsequillo is a state park wetland in the Africam Safari area.",
                  "category": "natural",
                  "coords": [
                        -98.176392,
                        18.921775
                  ]
            }
      ]
},
    type: "animal-habitat",
    parent: "MX-PUE",
    coords: [-98.2195, 18.9012],
    name: { de: "Africam Safari", hu: "Africam Safari", ro: "Africam Safari", en: "Africam Safari"},
    image: "/poi-images/africam-safari-life-v2.webp",
    descriptionAdvanced: {
      de: "Africam Safari bei Puebla bietet das außergewöhnliche Erlebnis einer echten Safari, bei der die Besucher in ihren eigenen Fahrzeugen durch weite Flächen fahren, in denen Tiere frei umherstreifen. Gegründet von Captain Carlos Camacho Espíritu, verfolgt der Park die Philosophie, dass Tiere in Freiheit glücklicher und gesünder sind. Man kann Giraffen, Löwen, Elefanten und Nashörner aus nächster Nähe beobachten, während sie sich in einer Landschaft bewegen, die ihren natürlichen Lebensräumen nachempfunden ist. Africam Safari ist international für seine erfolgreichen Erhaltungsprogramme bekannt, insbesondere für den Schutz des Afrikanischen Elefanten. Die Anlage bietet zudem nächtliche Safaris an, die einen faszinierenden Einblick in das Verhalten dämmerungsaktiver Tiere ermöglichen. Sachkunde K5 - Wildtiere und ihre Lebensräume.",
      hu: "Africam Safari egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Africam Safari kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Africam Safari este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Africam Safari este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Africam Safari, located just outside the city of Puebla, offers a unique opportunity to witness wild animals in expansive, open-range habitats that closely resemble their native environments. Unlike a traditional zoo, visitors drive through various 'continents' in their own vehicles or on park buses, allowing for close but safe encounters with giraffes, rhinos, lions, and elephants. Founded in 1972 by Captain Carlos Camacho Espíritu, the park was a pioneer in the 'safari' concept in Mexico and remains a leader in wildlife conservation. The park is famous for its successful breeding programs for endangered species, including the northern white rhino and the Mexican gray wolf. In addition to the drive-through safari, the park features a walking area with bird aviaries, insectariums, and nocturnal animal exhibits. Africam Safari provides a thrilling and educational experience that emphasizes the majesty of nature and the urgent need to protect it.",
      es: "Africam Safari, cerca de Puebla, ofrece la experiencia extraordinaria de un safari real, donde los visitantes conducen sus propios vehículos por vastas áreas donde los animales deambulan libremente. Fundado por el Capitán Carlos Camacho Espíritu, el parque sigue la filosofía de que los animales son más felices y sanos en libertad. Se pueden observar jirafas, leones, elefantes y rinocerontes de cerca mientras se mueven en un paisaje que emula sus hábitats naturales. Africam Safari es conocido internacionalmente por sus exitosos programas de conservación, especialmente para la protección del elefante africano. El recinto ofrece además safaris nocturnos que brindan una visión fascinante del comportamiento de los animales crepusculares.",
      pt: "O Africam Safari, perto de Puebla, oferece a experiência extraordinária de um safari real, onde os visitantes conduzem os seus próprios veículos por vastas áreas onde os animais circulam livremente. Fundado pelo Capitão Carlos Camacho Espíritu, o parque segue a filosofia de que os animais são mais felizes e saudáveis em liberdade. Pode-se observar girafas, leões, elefantes e rinocerontes de perto enquanto se movem numa paisagem que imita os seus habitats naturais. O Africam Safari é conhecido internacionalmente pelos seus programas de conservação bem-sucedidos, especialmente para a proteção do elefante africano. A instalação oferece também safaris noturnos, que proporcionam uma visão fascinante do comportamento dos animais crepusculares.",
      fr: "Africam Safari, près de Puebla, offre l'expérience extraordinaire d'un véritable safari où les visiteurs circulent dans leurs propres véhicules à travers de vastes zones où les animaux errent librement. Fondé par le capitaine Carlos Camacho Espíritu, le parc suit la philosophie selon laquelle les animaux sont plus heureux et en meilleure santé en liberté. On peut y observer de près des girafes, des lions, des éléphants et des rhinocéros évoluant dans un paysage qui imite leurs habitats naturels. Africam Safari est internationalement reconnu pour ses programmes de conservation réussis, notamment pour la protection de l'éléphant d'Afrique. Le site propose également des safaris nocturnes offrant un aperçu fascinant du comportement des animaux crépusculaires.",
    },
    factsAdvanced: {
      de: ["Beherbergt etwa 2.500 Tiere aus 300 verschiedenen Arten weltweit.", "Besucher legen die Safari-Route in ihrem eigenen Auto oder im Parkbus zurück.", "Erfolgreiches Zuchtprogramm für den bedrohten Mexikanischen Wolf.", "Der Park umfasst ein riesiges Areal von über 200 Hektar.", "Bietet eine einzigartige 'Aventura Nocturna' für Nachtbeobachtungen an.", "Verfügt über eine spezialisierte Klinik für die Behandlung von Wildtieren.", "Engagiert sich aktiv in internationalen Projekten zum Schutz von Nashörnern."],
      hu: ["Africam Safari sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Africam Safari egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Africam Safari ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Africam Safari este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The park covers more than 200 hectares and is divided into several thematic zones.",
        "It is the only park in Mexico to successfully breed the endangered African elephant.",
        "The facility was named 'Africam' by combining 'Africa' with the founder's surname, 'Camacho'.",
        "Visitors can participate in 'Night Safaris' during certain times of the year for a different perspective on wildlife.",
        "The park's conservation foundation actively works on projects across Mexico and even in Africa."
      ],
      es: ["Alberga unos 2.500 animales de 300 especies diferentes de todo el mundo.", "Los visitantes realizan la ruta del safari en su propio coche o en el autobús del parque.", "Exitoso programa de cría para el amenazado lobo mexicano.", "El parque abarca un área inmensa de más de 200 hectáreas.", "Ofrece una 'Aventura Nocturna' única para observaciones de noche.", "Cuenta con una clínica especializada para el tratamiento de animales salvajes.", "Participa activamente en proyectos internacionales para la protección de rinocerontes."],
      pt: ["Abriga cerca de 2.500 animais de 300 espécies diferentes de todo o mundo.", "Os visitantes percorrem a rota do safari no seu próprio carro ou no autocarro do parque.", "Programa de reprodução bem-sucedido para o ameaçado lobo mexicano.", "O parque abrange uma área enorme de mais de 200 hectares.", "Oferece uma 'Aventura Nocturna' única para observações noturnas.", "Possui uma clínica especializada para o tratamento de animais selvagens.", "Envolve-se ativamente em projetos internacionais para a proteção de rinocerontes."],
      fr: ["Abrite environ 2 500 animaux de 300 espèces différentes du monde entier.", "Les visiteurs parcourent l'itinéraire du safari dans leur propre voiture ou en bus.", "Programme de reproduction réussi pour le loup mexicain menacé.", "Le parc s'étend sur une zone immense de plus de 200 hectares.", "Propose une 'Aventura Nocturna' unique pour les observations nocturnes.", "Dispose d'une clinique spécialisée pour le traitement des animaux sauvages.", "S'engage activement dans des projets internationaux pour la protection des rhinocéros."],
    }, image: "/poi-images/africam-safari-life-v2.webp",
    description: {
      de: "Ein Safaripark in der Nähe von Puebla, wo Tiere in großen Freigehegen fast wie in Freiheit leben.",
      hu: "Puebla közelében található vadaspark, ahol az állatok szabadon mozognak a hatalmas kifutókban.",
      ro: "Un parc de viață sălbatică lângă Puebla, unde animalele trăiesc în libertate în habitate vaste explorate cu mașina.",
      en: "A renowned wildlife park near Puebla where animals roam freely in large habitats that visitors explore by car or bus.",
      es: "Un parque de safari cerca de Puebla, donde los animales viven en grandes recintos casi como en libertad.",
      pt: "Um parque de safari perto de Puebla, onde os animais vivem em grandes recintos quase como em liberdade.",
      fr: "Un parc safari près de Puebla, où les animaux vivent dans de grands enclos presque comme en liberté.",
    },
    facts: {
      de: ["Erkundung mit dem eigenen Auto", "Über 2.500 Tiere", "Engagement für den Artenschutz"],
      hu: ["Több mint 2500 vadállat", "Sikeres elefánttenyésztési program", "Éjszakai szafari lehetőségek"],
      ro: ["Peste 2500 de animale în semi-libertate", "Suprafață de peste 200 de hectare", "Program de succes pentru elefanții africani"],
      en: ["First safari-style park in Mexico", "Covers over 200 hectares", "Successful African elephant breeding program"],
      es: ["Exploración con el coche propio", "Más de 2.500 animales", "Compromiso con la protección de especies"],
      pt: ["Exploração com o próprio carro", "Mais de 2.500 animais", "Compromisso com a proteção das espécies"],
      fr: ["Exploration avec sa propre voiture", "Plus de 2 500 animaux", "Engagement pour la protection des espèces"],
    }
  },
  {
    id: "bioparque-estrella-life-v2", "sights": {
      "de": [
            {
                  "name": "Campo De Futbol San Pablo",
                  "text": "Campo De Futbol San Pablo ist ein Fußballplatz in Estrella.",
                  "category": "landmark",
                  "coords": [
                        -99.492189,
                        19.965785
                  ]
            },
            {
                  "name": "La hoguera tattoo studio",
                  "text": "La hoguera tattoo studio ist ein Tätowierstudio in Estrella.",
                  "category": "museum",
                  "coords": [
                        -99.564698,
                        19.973136
                  ]
            },
            {
                  "name": "Museo de Jilotepec",
                  "text": "Museo de Jilotepec ist ein Regionalmuseum in Estrella.",
                  "category": "museum",
                  "coords": [
                        -99.533201,
                        19.950825
                  ]
            },
            {
                  "name": "Casa de Cultura",
                  "text": "Casa de Cultura ist ein Kulturzentrum in Estrella.",
                  "category": "cultural",
                  "coords": [
                        -99.532661,
                        19.949748
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Campo De Futbol San Pablo",
                  "text": "Campo De Futbol San Pablo egy futballpálya Estrellában.",
                  "category": "landmark",
                  "coords": [
                        -99.492189,
                        19.965785
                  ]
            },
            {
                  "name": "La hoguera tattoo studio",
                  "text": "La hoguera tattoo studio egy tetoválószalon Estrellában.",
                  "category": "museum",
                  "coords": [
                        -99.564698,
                        19.973136
                  ]
            },
            {
                  "name": "Museo de Jilotepec",
                  "text": "Museo de Jilotepec egy regionális múzeum Estrellában.",
                  "category": "museum",
                  "coords": [
                        -99.533201,
                        19.950825
                  ]
            },
            {
                  "name": "Casa de Cultura",
                  "text": "Casa de Cultura egy kulturális központ Estrellában.",
                  "category": "cultural",
                  "coords": [
                        -99.532661,
                        19.949748
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Campo De Futbol San Pablo",
                  "text": "Campo De Futbol San Pablo este un teren de fotbal în Estrella.",
                  "category": "landmark",
                  "coords": [
                        -99.492189,
                        19.965785
                  ]
            },
            {
                  "name": "La hoguera tattoo studio",
                  "text": "La hoguera tattoo studio este un studio de tatuaje în Estrella.",
                  "category": "museum",
                  "coords": [
                        -99.564698,
                        19.973136
                  ]
            },
            {
                  "name": "Museo de Jilotepec",
                  "text": "Museo de Jilotepec este un muzeu regional în Estrella.",
                  "category": "museum",
                  "coords": [
                        -99.533201,
                        19.950825
                  ]
            },
            {
                  "name": "Casa de Cultura",
                  "text": "Casa de Cultura este un centru cultural în Estrella.",
                  "category": "cultural",
                  "coords": [
                        -99.532661,
                        19.949748
                  ]
            }
      ],
      "en": [
            {
                  "name": "Campo De Futbol San Pablo",
                  "text": "Campo De Futbol San Pablo is a soccer field in Estrella.",
                  "category": "landmark",
                  "coords": [
                        -99.492189,
                        19.965785
                  ]
            },
            {
                  "name": "La hoguera tattoo studio",
                  "text": "La hoguera tattoo studio is a tattoo studio in Estrella.",
                  "category": "museum",
                  "coords": [
                        -99.564698,
                        19.973136
                  ]
            },
            {
                  "name": "Museo de Jilotepec",
                  "text": "Museo de Jilotepec is a regional museum in Estrella.",
                  "category": "museum",
                  "coords": [
                        -99.533201,
                        19.950825
                  ]
            },
            {
                  "name": "Casa de Cultura",
                  "text": "Casa de Cultura is a cultural center in Estrella.",
                  "category": "cultural",
                  "coords": [
                        -99.532661,
                        19.949748
                  ]
            }
      ]
},
    type: "animal-habitat",
    parent: "MX-MEX",
    coords: [-99.5305, 19.9213],
    name: { de: "Bioparque Estrella", hu: "Bioparque Estrella", ro: "Bioparcul Estrella", en: "Bioparque Estrella"},
    image: "/poi-images/bioparque-estrella-life-v2.webp",
    descriptionAdvanced: {
      de: "Der Bioparque Estrella ist ein riesiger Safari- und Erlebnispark, der Bildung, Naturschutz und Abenteuer in einem einzigartigen Konzept vereint. Das Herzstück ist die 'Safari Serengeti', bei der Besucher Giraffen und andere afrikanische Großtiere füttern können, während sie in offenen Fahrzeugen durch das Gelände fahren. Der Park bietet zudem prähistorische Attraktionen wie das 'Rio Jurásico', wo lebensgroße Dinosaurier-Modelle in einer dschungelartigen Umgebung zu sehen sind. Für Kinder gibt es zahlreiche interaktive Stationen, die den Respekt vor der Natur und den Tieren vermitteln. Die weitläufige Anlage in der Nähe von Mexiko-Stadt ist ein ideales Ziel für einen Tagesausflug mit der ganzen Familie. Der Bioparque Estrella zeigt eindrucksvoll, wie wichtig der Erhalt der Biodiversität für unseren Planeten ist. Biologie K4 - Artenvielfalt und Erdgeschichte.",
      hu: "Bioparque Estrella egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Bioparque Estrella kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Bioparcul Estrella este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Bioparcul Estrella este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Bioparque Estrella, situated in the State of Mexico near Jilotepec, is a massive theme park and wildlife preserve that combines safari adventures with interactive attractions. It is best known for the 'Serengeti Safari,' where guests board specialized trucks to feed giraffes, zebras, and antelopes in a vast open field. The park also features the 'Subterranium,' an underground mining tour, and various water-based activities like 'Rio Loco.' With a strong focus on family fun and outdoor education, Bioparque Estrella provides a full day of entertainment while fostering a respect for animals and the environment. Its Jurassic-themed area, featuring life-sized animatronic dinosaurs, is a major draw for younger children. The park's wide-open spaces and diverse collection of animals make it one of the most popular weekend getaways for residents of the Mexico City metropolitan area.",
      es: "El Bioparque Estrella es un inmenso parque de safari y aventuras que une educación, conservación y aventura en un concepto único. El corazón es el 'Safari Serengeti', donde los visitantes pueden alimentar a jirafas y otros grandes animales africanos mientras recorren el terreno en vehículos abiertos. El parque ofrece además atracciones prehistóricas como el 'Río Jurásico', donde se pueden ver modelos de dinosaurios a tamaño real en un entorno selvático. Para los niños hay numerosas estaciones interactivas que transmiten el respeto por la naturaleza y los animales. El extenso recinto cerca de la Ciudad de México es un destino ideal para una excursión de un día con toda la familia. Bioparque Estrella muestra de forma impresionante la importancia de preservar la biodiversidad para nuestro planeta.",
      pt: "O Bioparque Estrella é um enorme parque de safari e aventura que combina educação, conservação e aventura num conceito único. O coração é o 'Safari Serengeti', onde os visitantes podem alimentar girafas e outros grandes animais africanos enquanto percorrem o terreno em veículos abertos. O parque oferece também atrações pré-históricas como o 'Rio Jurásico', onde se podem ver modelos de dinossauros em tamanho real num ambiente de selva. Para as crianças, existem inúmeras estações interativas que transmitem o respeito pela natureza e pelos animais. A extensa instalação perto da Cidade do México é um destino ideal para um passeio de um dia com toda a família. O Bioparque Estrella mostra de forma impressionante como é importante a preservação da biodiversidade para o nosso planeta.",
      fr: "Bioparque Estrella est un immense parc safari et d'aventures qui allie éducation, conservation et aventure dans un concept unique. Le cœur du parc est le 'Safari Serengeti', où les visiteurs peuvent nourrir des girafes et d'autres grands animaux africains à bord de véhicules ouverts. Le parc propose également des attractions préhistoriques comme le 'Rio Jurásico', où des modèles de dinosaures grandeur nature sont visibles dans un environnement de jungle. Pour les enfants, de nombreuses stations interactives transmettent le respect de la nature et des animaux. Ce vaste complexe situé près de Mexico est une destination idéale pour une excursion d'une journée en famille. Bioparque Estrella démontre de manière impressionnante l'importance de préserver la biodiversité pour notre planète.",
    },
    factsAdvanced: {
      de: ["Ist mit über 300 Hektar Fläche einer der größten Erlebnisparks Mexikos.", "Die Safari Serengeti ermöglicht direkten Kontakt zu Giraffen und Zebras.", "Beherbergt über 500 Tiere aus verschiedenen Kontinenten.", "Das 'Rio Jurásico' bietet eine Floßfahrt vorbei an animatronischen Dinosauriern.", "Verfügt über einen künstlichen See für Bootsfahrten und Wasseraktivitäten.", "Bietet spezialisierte Umweltbildungsprogramme für Kindergärten und Schulen.", "Der Park legt großen Wert auf die Nachzucht gefährdeter Pflanzenarten."],
      hu: ["Bioparque Estrella sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Bioparque Estrella egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Bioparcul Estrella ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Bioparcul Estrella este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The park spans over 300 hectares, making it one of the largest of its kind in the country.",
        "It features a unique 'Lion's Kingdom' where visitors can observe large predators from secure bridges.",
        "The 'Paraíso de Jirafas' (Giraffe Paradise) is one of the largest giraffe enclosures in Mexico.",
        "Bioparque Estrella is home to over 500 animals representing 50 different species.",
        "The park includes an environmental education center called 'Earth' aimed at school groups."
      ],
      es: ["Con más de 300 hectáreas, es uno de los parques de aventuras más grandes de México.", "El Safari Serengeti permite el contacto directo con jirafas y cebras.", "Alberga más de 500 animales de diferentes continentes.", "El 'Río Jurásico' ofrece un paseo en balsa pasando por dinosaurios animatrónicos.", "Cuenta con un lago artificial para paseos en bote y actividades acuáticas.", "Ofrece programas especializados de educación ambiental para escuelas.", "El parque pone gran énfasis en la reproducción de especies de plantas amenazadas."],
      pt: ["Com mais de 300 hectares de área, é um dos maiores parques de aventura do México.", "O Safari Serengeti permite o contacto direto com girafas e zebras.", "Abriga mais de 500 animais de diferentes continentes.", "O 'Rio Jurásico' oferece um passeio de balsa passando por dinossauros animatrónicos.", "Possui um lago artificial para passeios de barco e atividades aquáticas.", "Oferece programas especializados de educação ambiental para jardins de infância e escolas.", "O parque dá grande importância à propagação de espécies de plantas ameaçadas."],
      fr: ["Avec plus de 300 hectares, c'est l'un des plus grands parcs d'aventures du Mexique.", "Le Safari Serengeti permet un contact direct avec les girafes et les zèbres.", "Abrite plus de 500 animaux provenant de différents continents.", "Le 'Rio Jurásico' propose une promenade en radeau devant des dinosaures animatroniques.", "Dispose d'un lac artificiel pour des balades en bateau et des activités nautiques.", "Propose des programmes d'éducation environnementale spécialisés pour les écoles.", "Le parc accorde une grande importance à la reproduction d'espèces végétales menacées."],
    }, image: "/poi-images/bioparque-estrella-life-v2.webp",
    description: {
      de: "Ein Erlebnispark und Safari-Zoo im Bundesstaat Mexiko, ideal für Familienausflüge.",
      hu: "Mexikó államban fekvő szafari- és élménypark, amely interaktív állatbemutatókat kínál.",
      ro: "Un parc tematic și rezervație naturală în statul México, oferind experiențe interactive de safari și expoziții preistorice.",
      en: "A massive theme park and safari preserve in the State of Mexico, offering interactive animal encounters and prehistoric exhibits.",
      es: "Un parque de aventuras y zoológico safari en el estado de México, ideal para excursiones familiares.",
      pt: "Um parque de aventuras e jardim zoológico safari no estado do México, ideal para passeios em família.",
      fr: "Un parc d'aventures et zoo safari dans l'État du Mexique, idéal pour les sorties en famille.",
    },
    facts: {
      de: ["Große Safari-Tour", "Interaktive Dino-Ausstellung", "Fokus auf Umweltbildung"],
      hu: ["300 hektáros vadaspark", "Dinoszaurusz-kiállítás", "Zsiráfetetési lehetőség"],
      ro: ["Include safari în stil Serengeti", "Expoziție cu dinozauri animatronici", "Peste 500 de animale din 50 de specii"],
      en: ["Includes a Serengeti-style safari", "Features life-sized animatronic dinosaurs", "Home to more than 500 animals"],
      es: ["Gran tour de safari", "Exposición interactiva de dinosaurios", "Enfoque en educación ambiental"],
      pt: ["Grande tour de safari", "Exposição interativa de dinossauros", "Foco na educação ambiental"],
      fr: ["Grand tour de safari", "Exposition interactive de dinosaures", "Accent sur l'éducation environnementale"],
    }
  },
  {
    id: "monarch-butterfly-reserve-life-v2", "sights": {
      "de": [
            {
                  "name": "Mariposas Monarca",
                  "text": "Butterflies ist ein Schmetterlingsgehege in Butterfly Reserve.",
                  "category": "landmark",
                  "coords": [
                        -100.294558,
                        19.672218
                  ]
            },
            {
                  "name": "Santuario de la Mariposa Monarca Chincua",
                  "text": "Ein Heiligtum für Monarchfalter in Mexiko, Teil des UNESCO-Biosphärenreservats.",
                  "category": "natural",
                  "coords": [
                        -100.278939,
                        19.669893
                  ]
            },
            {
                  "name": "Monarch butterfly reserve",
                  "text": "Monarch butterfly reserve ist ein Schutzgebiet für Monarchfalter in Butterfly Reserve.",
                  "category": "recreational",
                  "coords": [
                        -100.267171,
                        19.588473
                  ]
            },
            {
                  "name": "Zona Núcleo",
                  "text": "Die Kernzone des Biosphärenreservats der Monarchfalter.",
                  "category": "natural",
                  "coords": [
                        -100.257081,
                        19.552982
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Mariposas Monarca",
                  "text": "Butterflies egy pillangóház Butterfly Reserveben.",
                  "category": "landmark",
                  "coords": [
                        -100.294558,
                        19.672218
                  ]
            },
            {
                  "name": "Santuario de la Mariposa Monarca Chincua",
                  "text": "Egy monarch lepkék szentélye Mexikóban, az UNESCO bioszféra-rezervátum része.",
                  "category": "natural",
                  "coords": [
                        -100.278939,
                        19.669893
                  ]
            },
            {
                  "name": "Monarch butterfly reserve",
                  "text": "Monarch butterfly reserve egy uralkodólepke-rezervátum Butterfly Reserveben.",
                  "category": "recreational",
                  "coords": [
                        -100.267171,
                        19.588473
                  ]
            },
            {
                  "name": "Zona Núcleo",
                  "text": "A monarch lepkék bioszféra-rezervátumának magterülete.",
                  "category": "natural",
                  "coords": [
                        -100.257081,
                        19.552982
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Mariposas Monarca",
                  "text": "Butterflies este o seră cu fluturi în Butterfly Reserve.",
                  "category": "landmark",
                  "coords": [
                        -100.294558,
                        19.672218
                  ]
            },
            {
                  "name": "Santuario de la Mariposa Monarca Chincua",
                  "text": "Un sanctuar pentru fluturii monarh din Mexic, parte a Rezervației Biosferei UNESCO.",
                  "category": "natural",
                  "coords": [
                        -100.278939,
                        19.669893
                  ]
            },
            {
                  "name": "Monarch butterfly reserve",
                  "text": "Monarch butterfly reserve este o rezervație de fluturi monarh în Butterfly Reserve.",
                  "category": "recreational",
                  "coords": [
                        -100.267171,
                        19.588473
                  ]
            },
            {
                  "name": "Zona Núcleo",
                  "text": "Zona nucleu a Rezervației Biosferei Fluturilor Monarh.",
                  "category": "natural",
                  "coords": [
                        -100.257081,
                        19.552982
                  ]
            }
      ],
      "en": [
            {
                  "name": "Butterflies",
                  "text": "Butterflies is a butterfly house in Butterfly Reserve.",
                  "category": "landmark",
                  "coords": [
                        -100.294558,
                        19.672218
                  ]
            },
            {
                  "name": "Santuario de la Mariposa Monarca Chincua",
                  "text": "A sanctuary for Monarch butterflies in Mexico, part of the UNESCO Biosphere Reserve.",
                  "category": "natural",
                  "coords": [
                        -100.278939,
                        19.669893
                  ]
            },
            {
                  "name": "Monarch butterfly reserve",
                  "text": "Monarch butterfly reserve is a monarch butterfly sanctuary in Butterfly Reserve.",
                  "category": "recreational",
                  "coords": [
                        -100.267171,
                        19.588473
                  ]
            },
            {
                  "name": "Zona Núcleo",
                  "text": "The core zone of the Monarch Butterfly Biosphere Reserve.",
                  "category": "natural",
                  "coords": [
                        -100.257081,
                        19.552982
                  ]
            }
      ]
},
    type: "forest",
    parent: "MX-MIC",
    coords: [-100.25, 19.6],
    name: { de: "Mariposa-Monarca-Reservat", hu: "Monarch pillango-rezervatum", ro: "Rezervatia fluturelui monarh", en: "Monarch Butterfly Reserve"},
    image: "/poi-images/monarch-butterfly-reserve-life-v2.webp",
    descriptionAdvanced: {
      de: "Das Biosphärenreservat Mariposa Monarca ist ein Naturwunder von globalem Ausmaß, in dem jedes Jahr Millionen von Monarchfaltern aus Kanada und den USA überwintern. Die orange-schwarzen Schmetterlinge bedecken die Oyamel-Tannen so dicht, dass sich die Äste unter ihrem Gewicht biegen. Es ist eine der spektakulärsten Tierwanderungen der Erde und wurde von der UNESCO zum Weltnaturerbe erklärt. Die Stille in den Wäldern wird nur durch das leise Rauschen von Millionen Schmetterlingsflügeln unterbrochen, wenn die Sonne die Tiere erwärmt. Der Schutz dieses empfindlichen Ökosystems ist von entscheidender Bedeutung, da der Klimawandel und die Abholzung die Wanderung gefährden. Ein Besuch in diesem Reservat ist ein tief bewegendes Erlebnis, das die Zerbrechlichkeit und Schönheit der Natur vor Augen führt. Biologie K8 - Migration und Ökosysteme.",
      hu: "Monarch pillango-rezervatum egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Monarch pillango-rezervatum kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Rezervatia fluturelui monarh este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Rezervatia fluturelui monarh este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Monarch Butterfly Biosphere Reserve is a breathtaking natural sanctuary located in the rugged mountains of Michoacán and the State of Mexico. Every year, millions of monarch butterflies travel over 4,000 kilometers from Canada and the United States to spend the winter in these high-altitude oyamel fir forests. The sight of thousands of butterflies clustering on tree branches, often bending them with their collective weight, is one of nature's most spectacular phenomena. Designated as a UNESCO World Heritage site, the reserve is critical for the survival of the monarch's unique migratory cycle. Visitors can hike or ride horses up the steep mountain trails to reach the colonies, where the air hums with the sound of millions of wings. The reserve also plays a vital role in protecting the regional watershed and providing a habitat for numerous other mountain species. Conservation efforts here are a collaboration between local communities, international organizations, and the Mexican government.",
      es: "La Reserva de la Biosfera de la Mariposa Monarca es una maravilla natural de escala global, donde cada año millones de mariposas monarca de Canadá y EE. UU. pasan el invierno. Las mariposas de color naranja y negro cubren los abetos oyamel tan densamente que las ramas se doblan bajo su peso. Es una de las migraciones animales más espectaculares de la Tierra y fue declarada Patrimonio de la Humanidad por la UNESCO. El silencio en los bosques solo se ve interrumpido por el suave susurro de millones de alas de mariposa cuando el sol calienta a los animales. La protección de este ecosistema frágil es de vital importancia, ya que el cambio climático y la deforestación amenazan la migración. Una visita a esta reserva es una experiencia profundamente conmovedora que muestra la fragilidad y la belleza de la naturaleza. Biología K8 - Migración y ecosistemas.",
      pt: "A Reserva da Biosfera da Borboleta Monarca é uma maravilha natural de escala global, onde todos os anos milhões de borboletas monarca do Canadá e dos EUA passam o inverno. As borboletas cor-de-laranja e pretas cobrem os abetos oyamel de forma tão densa que os ramos se curvam sob o seu peso. É uma das migrações de animais mais espetaculares da Terra e foi declarada Património Mundial pela UNESCO. O silêncio nas florestas só é interrompido pelo suave sussurro de milhões de asas de borboleta quando o sol aquece os animais. A proteção deste ecossistema frágil é de importância crucial, pois as alterações climáticas e a desflorestação ameaçam a migração. Uma visita a esta reserva é uma experiência profundamente comovente que revela a fragilidade e a beleza da natureza. Biologia K8 - Migração e ecossistemas.",
      fr: "La réserve de biosphère du papillon monarque est une merveille naturelle d'envergure mondiale, où chaque année des millions de papillons monarques du Canada et des États-Unis hivernent. Les papillons orange et noir recouvrent les sapins oyamel si densément que les branches ploient sous leur poids. C'est l'une des migrations animales les plus spectaculaires de la planète et elle a été classée au patrimoine mondial par l'UNESCO. Le silence des forêts n'est interrompu que par le léger bruissement de millions d'ailes de papillons lorsque le soleil réchauffe les animaux. La protection de cet écosystème fragile est d'une importance cruciale, car le changement climatique et la déforestation menacent la migration. Une visite dans cette réserve est une expérience profondément émouvante qui met en lumière la fragilité et la beauté de la nature. Biologie K8 - Migration et écosystèmes.",
    },
    factsAdvanced: {
      de: ["Die Falter legen eine Strecke von über 4.000 Kilometern aus dem Norden zurück.", "Das Reservat umfasst rund 56.000 Hektar in den Bundesstaaten Michoacán und Mexiko.", "Die Überwinterung findet in Höhenlagen zwischen 2.400 und 3.600 Metern statt.", "Monarchfalter sind die einzige Insektenart, die eine so weite Wanderung unternimmt.", "Wurde 2008 in die Liste des UNESCO-Weltnaturerbes aufgenommen.", "Die Generation, die wandert, lebt bis zu acht Monate, viel länger als normale Falter.", "Tourismus wird streng kontrolliert, um die Ruheplätze der Falter nicht zu stören."],
      hu: ["Monarch pillango-rezervatum sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Monarch pillango-rezervatum egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Rezervatia fluturelui monarh ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Rezervatia fluturelui monarh este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The reserve was designated a UNESCO World Heritage site in 2008.",
        "The butterflies arrive in November and stay until March before beginning their journey back north.",
        "The forests are located at an elevation of 2,400 to 3,600 meters above sea level.",
        "Only a few specific areas of the total 56,000-hectare reserve are open to public tourism.",
        "The monarch migration is considered an 'endangered phenomenon' due to habitat loss and climate change."
      ],
      es: ["Las mariposas recorren una distancia de más de 4.000 kilómetros desde el norte.", "La reserva abarca unas 56.000 hectáreas en los estados de Michoacán y México.", "La hibernación ocurre en altitudes de entre 2.400 y 3.600 metros.", "Las mariposas monarca son la única especie de insecto que realiza una migración tan larga.", "Fue incluida en la lista del Patrimonio Mundial de la UNESCO en 2008.", "La generación que migra vive hasta ocho meses, mucho más que las mariposas normales.", "El turismo se controla estrictamente para no perturbar los lugares de descanso de las mariposas."],
      pt: ["As borboletas percorrem uma distância de mais de 4.000 quilómetros vindo do norte.", "A reserva abrange cerca de 56.000 hectares nos estados de Michoacán e México.", "A hibernação ocorre em altitudes entre 2.400 e 3.600 metros.", "As borboletas monarca são a única espécie de inseto que realiza uma migração tão longa.", "Foi incluída na lista do Património Mundial da UNESCO em 2008.", "A geração que migra vive até oito meses, muito mais do que as borboletas normais.", "O turismo é estritamente controlado para não perturbar os locais de descanso das borboletas."],
      fr: ["Les papillons parcourent une distance de plus de 4 000 kilomètres depuis le nord.", "La réserve couvre environ 56 000 hectares dans les États du Michoacán et de Mexico.", "L'hivernage a lieu à des altitudes comprises entre 2 400 et 3 600 mètres.", "Les monarques sont la seule espèce d'insecte à entreprendre une migration aussi longue.", "A été inscrite sur la liste du patrimoine mondial de l'UNESCO en 2008.", "La génération qui migre vit jusqu'à huit mois, bien plus longtemps que les papillons normaux.", "Le tourisme est strictement contrôlé pour ne pas perturber les sites de repos des papillons."],
    }, image: "/poi-images/monarch-butterfly-reserve-life-v2.webp",
    description: {
      de: "Ein UNESCO-Welterbe, in dem Millionen von Monarchfaltern aus Kanada überwintern.",
      hu: "Az UNESCO világörökség része, ahol telente monarch lepkék milliói gyűlnek össze.",
      ro: "Sanctuar natural protejat de UNESCO unde milioane de fluturi monarh migrează anual din Canada pentru iarnă.",
      en: "A UNESCO World Heritage site in the mountains of Michoacán where millions of monarch butterflies overwinter every year.",
      es: "Un Patrimonio de la Humanidad de la UNESCO donde millones de mariposas monarca de Canadá pasan el invierno.",
      pt: "Um Património Mundial da UNESCO onde milhões de borboletas monarca vindas do Canadá passam o inverno.",
      fr: "Un site classé au patrimoine mondial de l'UNESCO où des millions de papillons monarques du Canada hivernent.",
    },
    facts: {
      de: ["Gelegen in Michoacán", "Spektakuläre Massenansammlung", "Schutzgebiet in den Bergen"],
      hu: ["UNESCO világörökségi helyszín", "2400-3600 méteres magasságban", "4000 kilométeres vándorlási útvonal"],
      ro: ["Sit UNESCO din anul 2008", "Fluturii parcurg peste 4000 km", "Altitudine între 2400 și 3600 metri"],
      en: ["Butterflies travel over 4,000 km", "Located at high altitude (up to 3,600m)", "UNSECO site since 2008"],
      es: ["Situada en Michoacán", "Espectacular concentración masiva", "Área protegida en las montañas"],
      pt: ["Situada em Michoacán", "Espetacular concentração em massa", "Área protegida nas montanhas"],
      fr: ["Située dans le Michoacán", "Rassemblement de masse spectaculaire", "Zone protégée dans les montagnes"],
    }
  },
  {
    id: "sian-kaan-life-v2", "sights": {
      "de": [
            {
                  "name": "Biosfera de Sian Kaan",
                  "text": "UNESCO-Biosphärenreservat mit über 500.000 Hektar Küsten- und Dschungelgebieten.",
                  "category": "landmark",
                  "coords": [
                        -87.604288,
                        19.475378
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Biosfera de Sian Kaan",
                  "text": "UNESCO bioszféra-rezervátum, több mint 500 000 hektárnyi partvidéki és dzsungel területtel.",
                  "category": "landmark",
                  "coords": [
                        -87.604288,
                        19.475378
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Biosfera de Sian Kaan",
                  "text": "Rezervație UNESCO a biosferei, ce se întinde pe peste 500.000 de hectare de coastă și jungle.",
                  "category": "landmark",
                  "coords": [
                        -87.604288,
                        19.475378
                  ]
            }
      ],
      "en": [
            {
                  "name": "Biosfera de Sian Kaan",
                  "text": "UNESCO biosphere reserve spanning over 500,000 hectares of coastal and jungle habitats.",
                  "category": "landmark",
                  "coords": [
                        -87.604288,
                        19.475378
                  ]
            }
      ]
},
    type: "forest",
    parent: "MX-ROO",
    coords: [-87.65, 19.42],
    name: { de: "Sian Ka'an", hu: "Sian Ka'an", ro: "Sian Ka'an", en: "Sian Ka'an"},
    image: "/poi-images/sian-kaan-life-v2.webp",
    descriptionAdvanced: {
      de: "Sian Ka'an, was in der Sprache der Maya 'Wo der Himmel geboren wurde' bedeutet, ist ein riesiges Biosphärenreservat an der Karibikküste Mexikos. Es umfasst tropische Regenwälder, Mangroven, Sümpfe und einen bedeutenden Teil des mittelamerikanischen Barriereriffs. Das Gebiet beherbergt eine unglaubliche Artenvielfalt, darunter Jaguare, Pumas, Seekühe und hunderte Vogelarten. Besucher können die kristallklaren Kanäle der Maya erkunden, die einst als Handelswege dienten, und in den natürlichen Strömungen treiben. Das Reservat ist ein Musterbeispiel für nachhaltigen Tourismus und den Schutz unberührter Wildnis. Als UNESCO-Welterbe bewahrt Sian Ka'an nicht nur die Natur, sondern auch archäologische Schätze der Maya-Zivilisation. Es ist ein Ort der absoluten Ruhe und unberührten Schönheit. Geographie K8 - Ökosysteme und nachhaltige Entwicklung.",
      hu: "Sian Ka'an egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Sian Ka'an kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Sian Ka'an este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Sian Ka'an este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Sian Ka'an, which means 'Gate of Heaven' or 'Where the Sky is Born' in the Maya language, is a massive biosphere reserve on the Caribbean coast of Quintana Roo. This UNESCO World Heritage site encompasses tropical forests, mangroves, marshes, and a large section of the Mesoamerican Barrier Reef. It is one of the most ecologically diverse places in Mexico, providing a home to hundreds of species of birds, monkeys, jaguars, and sea turtles. The reserve is also rich in cultural history, containing over 20 archaeological sites, including the ancient Maya trading center of Muyil. Visitors can explore the reserve through eco-tours that involve floating down crystal-clear freshwater canals built by the Maya or kayaking through the labyrinthine mangroves. Sian Ka'an serves as a global model for sustainable development, where conservation and the needs of local communities are carefully balanced. Its pristine beauty remains largely untouched by the massive tourism developments found further north.",
      es: "Sian Ka'an, que en lengua maya significa 'Donde nace el cielo', es una enorme reserva de la biosfera en la costa caribeña de México. Abarca selvas tropicales, manglares, pantanos y una parte importante del Arrecife Mesoamericano. El área alberga una increíble biodiversidad, incluyendo jaguares, pumas, manatíes y cientos de especies de aves. Los visitantes pueden explorar los canales mayas de aguas cristalinas, que alguna vez sirvieron como rutas comerciales, y dejarse llevar por las corrientes naturales. La reserva es un ejemplo modelo de turismo sostenible y protección de la naturaleza virgen. Como Patrimonio de la Humanidad de la UNESCO, Sian Ka'an conserva no solo la naturaleza, sino también tesoros arqueológicos de la civilización maya. Es un lugar de absoluta tranquilidad y belleza intacta. Geografía K8 - Ecosistemas y desarrollo sostenible.",
      pt: "Sian Ka'an, que na língua maia significa 'Onde nasce o céu', é uma enorme reserva da biosfera na costa das Caraíbas mexicana. Abrange florestas tropicais, manguezais, pântanos e uma parte significativa da barreira de recifes da América Central. A área abriga uma incrível biodiversidade, incluindo jaguares, pumas, peixes-boi e centenas de espécies de aves. Os visitantes podem explorar os canais maias de águas cristalinas, que outrora serviram como rotas comerciais, e flutuar nas correntes naturais. A reserva é um modelo de turismo sustentável e proteção da natureza selvagem intocada. Como Património Mundial da UNESCO, Sian Ka'an preserva não só a natureza, mas também tesouros arqueológicos da civilização maia. É um lugar de tranquilidade absoluta e beleza intocada. Geografia K8 - Ecossistemas e desenvolvimento sustentável.",
      fr: "Sian Ka'an, qui signifie 'Là où le ciel est né' en langue maya, est une immense réserve de biosphère sur la côte caraïbe du Mexique. Elle comprend des forêts tropicales, des mangroves, des marécages et une partie importante de la barrière de corail méso-américaine. La zone abrite une biodiversité incroyable, notamment des jaguars, des pumas, des lamentins et des centaines d'espèces d'oiseaux. Les visiteurs peuvent explorer les canaux mayas aux eaux cristallines, qui servaient autrefois de routes commerciales, et se laisser porter par les courants naturels. La réserve est un modèle de tourisme durable et de protection d'une nature sauvage préservée. En tant que site classé à l'UNESCO, Sian Ka'an préserve non seulement la nature, mais aussi des trésors archéologiques de la civilisation maya. C'est un lieu de calme absolu et de beauté vierge. Géographie K8 - Écosystèmes et développement durable.",
    },
    factsAdvanced: {
      de: ["Ist das größte Schutzgebiet im mexikanischen Teil der Halbinsel Yucatán.", "Umfasst eine Fläche von etwa 5.280 Quadratkilometern Land und Meer.", "Beherbergt over 300 Vogelarten und 100 verschiedene Säugetierarten.", "Wurde bereits im Jahr 1987 zum UNESCO-Weltnaturerbe erklärt.", "Enthält 23 bekannte archäologische Stätten der Maya-Kultur.", "Das Barriereriff vor der Küste ist das zweitgrößte der Welt.", "Süßwasserquellen, sogenannte Cenoten, sind im gesamten Reservat zu finden."],
      hu: ["Sian Ka'an sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Sian Ka'an egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Sian Ka'an ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Sian Ka'an este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Sian Ka'an was established as a biosphere reserve in 1986 and became a UNESCO site in 1987.",
        "It covers approximately 1.3 million acres, representing about 10% of the state of Quintana Roo.",
        "The reserve's coastal waters are home to the second-largest coral reef system in the world.",
        "More than 300 species of birds have been recorded within the reserve's boundaries.",
        "The freshwater canals were used by the ancient Maya for over 1,000 years for trade and navigation."
      ],
      es: ["Es el área protegida más grande de la parte mexicana de la península de Yucatán.", "Abarca una superficie de unos 5.280 kilómetros cuadrados de tierra y mar.", "Alberga más de 300 especies de aves y 100 especies diferentes de mamíferos.", "Fue declarada Patrimonio de la Humanidad por la UNESCO en 1987.", "Contiene 23 sitios arqueológicos conocidos de la cultura maya.", "El arrecife de coral frente a la costa es el segundo más grande del mundo.", "Se pueden encontrar fuentes de agua dulce, llamadas cenotes, en toda la reserva."],
      pt: ["É a maior área protegida na parte mexicana da península de Yucatán.", "Abrange uma área de cerca de 5.280 quilómetros quadrados de terra e mar.", "Abriga mais de 300 espécies de aves e 100 espécies diferentes de mamíferos.", "Foi declarada Património Mundial pela UNESCO já em 1987.", "Contém 23 sítios arqueológicos conhecidos da cultura maia.", "A barreira de recifes ao largo da costa é a segunda maior do mundo.", "Fontes de água doce, chamadas cenotes, encontram-se em toda a reserva."],
      fr: ["Est la plus grande zone protégée de la partie mexicaine de la péninsule du Yucatán.", "Couvre une superficie d'environ 5 280 kilomètres carrés de terre et de mer.", "Abrite plus de 300 espèces d'oiseaux et 100 espèces différentes de mammifères.", "A été déclarée patrimoine mondial de l'UNESCO dès 1987.", "Contient 23 sites archéologiques connus de la culture maya.", "La barrière de corail au large de la côte est la deuxième plus grande au monde.", "Des sources d'eau douce, appelées cénotes, se trouvent dans toute la réserve."],
    }, image: "/poi-images/sian-kaan-life-v2.webp",
    description: {
      de: "Ein riesiges Biosphärenreservat an der Karibikküste, bekannt für seine Artenvielfalt und Lagunen.",
      hu: "Hatalmas bioszféra-rezervátum a karibi tengerparton, amely trópusi erdőket és korallzátonyokat védelmez.",
      ro: "O rezervație uriașă a biosferei pe coasta Caraibelor, protejând păduri tropicale, mangrove și recife de corali virgine.",
      en: "A vast biosphere reserve on the Caribbean coast, encompassing tropical forests, mangroves, and pristine coral reefs.",
      es: "Una enorme reserva de la biosfera en la costa del Caribe, conocida por su biodiversidad y lagunas.",
      pt: "Uma enorme reserva da biosfera na costa das Caraíbas, conhecida pela sua biodiversidade e lagoas.",
      fr: "Une immense réserve de biosphère sur la côte caraïbe, connue pour sa biodiversité et ses lagunes.",
    },
    facts: {
      de: ["UNESCO-Weltnaturerbe", "Maya-Kanäle im Dschungel", "Lebensraum für Jaguare und Seekühe"],
      hu: ["UNESCO világörökségi helyszín", "23 maja régészeti lelőhely", "A világ második legnagyobb zátonyrendszere"],
      ro: ["Inclusă în patrimoniul UNESCO din 1987", "Conține peste 20 de situri arheologice", "Al doilea cel mai mare recif din lume"],
      en: ["UNESCO World Heritage site", "Contains over 20 archaeological sites", "Second-largest reef system in the world"],
      es: ["Patrimonio Natural de la UNESCO", "Canales mayas en la selva", "Hábitat para jaguares y manatíes"],
      pt: ["Património Mundial da UNESCO", "Canais maias na selva", "Habitat para jaguares e peixes-boi"],
      fr: ["Patrimoine mondial naturel de l'UNESCO", "Canaux mayas dans la jungle", "Habitat pour jaguars et lamentins"],
    }
  },
  {
    id: "calakmul-reserve-life-v2", "sights": {
      "de": [
            {
                  "name": "Zona arqueológica de Balamkú",
                  "text": "Die archäologische Zone Balamkú im Calakmul-Reservat ist berühmt für ihren gut erhaltenen Stuckfries.",
                  "category": "landmark",
                  "coords": [
                        -89.945061,
                        18.557415
                  ]
            },
            {
                  "name": "Casa de los cuatro reyes",
                  "text": "Historisches Haus, benannt nach vier Königen, gelegen in einem Naturschutzgebiet.",
                  "category": "historical",
                  "coords": [
                        -89.945331,
                        18.560205
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Zona arqueológica de Balamkú",
                  "text": "A Balamkú régészeti övezet a Calakmul-rezervátumban híres jól megőrzött stukkófrízéről.",
                  "category": "landmark",
                  "coords": [
                        -89.945061,
                        18.557415
                  ]
            },
            {
                  "name": "Casa de los cuatro reyes",
                  "text": "Történelmi ház, négy királyról elnevezve, egy természetvédelmi területen található.",
                  "category": "historical",
                  "coords": [
                        -89.945331,
                        18.560205
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Zona arqueológica de Balamkú",
                  "text": "Zona arheologică Balamkú din Rezervația Calakmul este renumită pentru friza sa de stuc bine conservată.",
                  "category": "landmark",
                  "coords": [
                        -89.945061,
                        18.557415
                  ]
            },
            {
                  "name": "Casa de los cuatro reyes",
                  "text": "Casă istorică numită după patru regi, situată într-o rezervație naturală.",
                  "category": "historical",
                  "coords": [
                        -89.945331,
                        18.560205
                  ]
            }
      ],
      "en": [
            {
                  "name": "Zona arqueológica de Balamkú",
                  "text": "The Balamkú archaeological zone in the Calakmul Reserve is famous for its well-preserved stucco frieze.",
                  "category": "landmark",
                  "coords": [
                        -89.945061,
                        18.557415
                  ]
            },
            {
                  "name": "Casa de los cuatro reyes",
                  "text": "Historical house named after four kings, located in a nature reserve.",
                  "category": "historical",
                  "coords": [
                        -89.945331,
                        18.560205
                  ]
            }
      ]
},
    type: "forest",
    parent: "MX-CAM",
    coords: [-89.95, 18.62],
    name: { de: "Calakmul Biosphaerenreservat", hu: "Calakmul bioszfera-rezervatum", ro: "Rezervatia biosferei Calakmul", en: "Calakmul Biosphere Reserve"},
    image: "/poi-images/calakmul-reserve-life-v2.webp",
    descriptionAdvanced: {
      de: "Das Biosphärenreservat Calakmul ist eines der größten tropischen Waldgebiete Mexikos und beherbergt die gleichnamige, gigantische Maya-Metropole. Tief im Dschungel von Campeche gelegen, bietet es einen Rückzugsort für bedrohte Großkatzen wie den Jaguar und den mittelamerikanischen Tapir. Die Verbindung von unberührter Natur und monumentalen archäologischen Strukturen macht Calakmul zu einem weltweit einzigartigen Kulturerbe der UNESCO. Von den Spitzen der hohen Pyramiden aus hat man einen endlosen Blick über das grüne Kronendach des Waldes, in dem Brüllaffen und bunte Tukane leben. Die Abgeschiedenheit des Ortes sorgt für eine mystische Atmosphäre, die an die Zeit der großen Maya-Könige erinnert. Der Schutz dieses Reservats ist entscheidend für den Erhalt der biologischen Vielfalt in Mittelamerika. Geschichte K6 - Maya-Zivilisation und tropische Ökosysteme.",
      hu: "Calakmul bioszfera-rezervatum egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Calakmul bioszfera-rezervatum kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Rezervatia biosferei Calakmul este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Rezervatia biosferei Calakmul este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Calakmul Biosphere Reserve is a vast expanse of tropical forest in the heart of the Yucatán Peninsula, standing as one of the largest protected areas in Mexico. It is famous for housing the ancient Maya city of Calakmul, once a powerful rival to Tikal, whose massive pyramids rise high above the jungle canopy. The reserve is a critical corridor for large felines, including the jaguar and puma, and is home to two species of monkeys and over 350 species of birds. The dense vegetation hides countless undiscovered ruins and provides a glimpse into the wild landscape that once dominated the Maya world. Conservation in Calakmul is vital for maintaining the region's biodiversity and protecting the endemic flora and fauna of the southern Yucatán. Due to its remote location, it remains one of the most tranquil and authentic destinations for eco-tourism in Mexico. The integration of high-density archaeological remains within a primary forest makes it a unique 'mixed' UNESCO World Heritage site.",
      es: "La Reserva de la Biosfera de Calakmul es una de las mayores áreas de bosque tropical en México y alberga la gigantesca metrópoli maya del mismo nombre. Situada en lo profundo de la selva de Campeche, ofrece un refugio para felinos amenazados como el jaguar y el tapir centroamericano. La combinación de naturaleza virgen y estructuras arqueológicas monumentales convierte a Calakmul en un patrimonio cultural único de la UNESCO a nivel mundial. Desde las cimas de las altas pirámides se tiene una vista infinita sobre el dosel verde del bosque, donde viven monos aulladores y coloridos tucanes. El aislamiento del lugar crea una atmósfera mística que recuerda la época de los grandes reyes mayas. La protección de esta reserva es crucial para mantener la biodiversidad en Centroamérica.",
      pt: "A Reserva da Biosfera de Calakmul é uma das maiores áreas de floresta tropical no México e abriga a gigantesca metrópole maia com o mesmo nome. Situada nas profundezas da selva de Campeche, oferece um refúgio para grandes felinos ameaçados, como o jaguar, e o tapir centro-americano. A combinação de natureza intocada e estruturas arqueológicas monumentais faz de Calakmul um Património Mundial da UNESCO único no mundo. Do topo das altas pirâmides, tem-se uma vista interminável sobre a copa verde da floresta, onde vivem macacos-uivadores e tucanos coloridos. O isolamento do local cria uma atmosfera mística que recorda o tempo dos grandes reis maias. A proteção desta reserva é crucial para a preservação da biodiversidade na América Central.",
      fr: "La réserve de la biosphère de Calakmul est l'une des plus grandes zones de forêt tropicale du Mexique et abrite la gigantesque métropole maya du même nom. Située au cœur de la jungle de Campeche, elle offre un refuge aux grands félins menacés comme le jaguar et au tapir d'Amérique centrale. L'alliance d'une nature intacte et de structures archéologiques monumentales fait de Calakmul un patrimoine mondial de l'UNESCO unique au monde. Depuis le sommet des hautes pyramides, on a une vue infinie sur la canopée verte de la forêt où vivent singes hurleurs et toucans colorés. L'isolement du lieu crée une atmosphère mystique rappelant l'époque des grands rois mayas. La protection de cette réserve est cruciale pour le maintien de la biodiversité en Amérique centrale.",
    },
    factsAdvanced: {
      de: ["Das Reservat umfasst mehr als 700.000 Hektar geschützten Tropenwald.", "Beherbergt die höchste Dichte an Jaguaren in ganz Mexiko.", "Calakmul war eine der mächtigsten Städte der klassischen Maya-Zeit.", "Wurde als 'Mixed Property' (Natur und Kultur) in das UNESCO-Welterbe aufgenommen.", "Die Pyramide Struktur II gehört mit 45 Metern zu den höchsten der Maya-Welt.", "Im Reservat wurden über 350 Vogelarten wissenschaftlich dokumentiert.", "Dient als wichtiger biologischer Korridor zwischen Nord- und Südamerika."],
      hu: ["Calakmul bioszfera-rezervatum sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Calakmul bioszfera-rezervatum egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Rezervatia biosferei Calakmul ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Rezervatia biosferei Calakmul este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Calakmul is the largest tropical forest reserve in Mexico, covering over 7,200 square kilometers.",
        "The Great Pyramid of Calakmul is one of the tallest Maya structures, reaching 45 meters in height.",
        "It was designated as a UNESCO World Heritage site for both its natural and cultural significance in 2014.",
        "The reserve is part of the Selva Maya, the largest tropical rainforest in the Americas after the Amazon.",
        "It is one of the few places in Mexico where all five species of native cats can still be found."
      ],
      es: ["La reserva abarca más de 700.000 hectáreas de selva tropical protegida.", "Alberga la mayor densidad de jaguares en todo México.", "Calakmul fue una de las ciudades más poderosas del periodo maya clásico.", "Fue incluida como 'Bien Mixto' (natural y cultural) en el Patrimonio de la UNESCO.", "La pirámide Estructura II es una de las más altas del mundo maya con 45 metros.", "En la reserva se han documentado científicamente más de 350 especies de aves.", "Sirve como importante corredor biológico entre Norte y Sudamérica."],
      pt: ["A reserva abrange mais de 700.000 hectares de floresta tropical protegida.", "Abriga a maior densidade de jaguares em todo o México.", "Calakmul foi uma das cidades mais poderosas do período maia clássico.", "Foi incluída como 'Mixed Property' (natureza e cultura) no Património Mundial da UNESCO.", "A pirâmide Estrutura II, com 45 metros, é uma das mais altas do mundo maia.", "Mais de 350 espécies de aves foram documentadas cientificamente na reserva.", "Serve como um importante corredor biológico entre a América do Norte e a América do Sul."],
      fr: ["La réserve comprend plus de 700 000 hectares de forêt tropicale protégée.", "Elle abrite la plus forte densité de jaguars de tout le Mexique.", "Calakmul était l'une des cités les plus puissantes de l'époque maya classique.", "Inscrite comme 'bien mixte' (naturel et culturel) au patrimoine de l'UNESCO.", "La pyramide Structure II est l'une des plus hautes du monde maya avec 45 mètres.", "Plus de 350 espèces d'oiseaux ont été documentées scientifiquement dans la réserve.", "Sert de corridor biologique majeur entre l'Amérique du Nord et du Sud."],
    }, image: "/poi-images/calakmul-reserve-life-v2.webp",
    description: {
      de: "Ein riesiges Tropenwaldreservat im Bundesstaat Campeche, das bedeutende Maya-Ruinen beherbergt.",
      hu: "Campeche államban található trópusi erdőrezervátum, amely egy hatalmas maja várost rejt.",
      ro: "Cea mai mare rezervație de pădure tropicală din Mexic, care adăpostește ruinele unei puternice metropole mayașe.",
      en: "A massive tropical forest reserve in Campeche that protects one of the most powerful ancient Maya cities ever discovered.",
      es: "Una enorme reserva de selva tropical en el estado de Campeche, que alberga importantes ruinas mayas.",
      pt: "Uma enorme reserva de floresta tropical no estado de Campeche, que abriga importantes ruínas maias.",
      fr: "Une immense réserve de forêt tropicale dans l'État de Campeche, abritant d'importantes ruines mayas.",
    },
    facts: {
      de: ["UNESCO-Weltkulturerbe", "Zentrum der Maya-Kultur", "Lebensraum für Jaguare"],
      hu: ["700 000 hektárnyi védett erdő", "Vegyes világörökségi helyszín", "45 méter magas maja piramisok"],
      ro: ["Sit UNESCO mixt (natură și cultură)", "Cea mai mare densitate de jaguari din țară", "Piramida principală are 45 de metri înălțime"],
      en: ["Largest tropical forest reserve in Mexico", "Highest density of jaguars in the country", "Mixed UNESCO Heritage site"],
      es: ["Patrimonio de la Humanidad UNESCO", "Centro de la cultura maya", "Hábitat para jaguares"],
      pt: ["Património Mundial da UNESCO", "Centro da cultura maia", "Habitat para jaguares"],
      fr: ["Patrimoine mondial de l'UNESCO", "Centre de la culture maya", "Habitat pour les jaguars"],
    }
  },
  {
    id: "lacandon-jungle-life-v2", "sights": {
      "de": [
            {
                  "name": "Reserva de la Biosfera Montes Azules",
                  "text": "Das Biosphärenreservat Montes Azules ist ein Schutzgebiet im Lacandon-Dschungel.",
                  "category": "natural",
                  "coords": [
                        -91.138813,
                        16.471932
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Reserva de la Biosfera Montes Azules",
                  "text": "A Montes Azules Bioszféra Rezervátum egy védett terület a Lacandon-őserdőben.",
                  "category": "natural",
                  "coords": [
                        -91.138813,
                        16.471932
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Reserva de la Biosfera Montes Azules",
                  "text": "Rezervația Biosferei Montes Azules este o arie protejată în Pădurea Lacandon.",
                  "category": "natural",
                  "coords": [
                        -91.138813,
                        16.471932
                  ]
            }
      ],
      "en": [
            {
                  "name": "Reserva de la Biosfera Montes Azules",
                  "text": "Montes Azules Biosphere Reserve is a protected area in the Lacandon Jungle.",
                  "category": "natural",
                  "coords": [
                        -91.138813,
                        16.471932
                  ]
            }
      ]
},
    type: "forest",
    parent: "MX-CHP",
    coords: [-91.15, 16.55],
    name: { de: "Lacandon-Dschungel", hu: "Lacandon dzsungel", ro: "Jungla Lacandon", en: "Lacandon Jungle"},
    image: "/poi-images/lacandon-jungle-life-v2.webp",
    descriptionAdvanced: {
      de: "Die Selva Lacandona im Bundesstaat Chiapas ist der größte tropische Regenwald Mexikos und ein Ort von unglaublicher biologischer und kultureller Bedeutung. Sie ist die Heimat der Lacandon-Maya, die ihre traditionelle Lebensweise und tiefe spirituelle Verbindung zum Wald bis heute bewahrt haben. Der Dschungel ist von reißenden Flüssen, spektakulären Wasserfällen wie Agua Azul und verborgenen Maya-Ruinen wie Bonampak durchzogen. Die immense Artenvielfalt reicht von bunten Aras über seltene Orchideen bis hin zu den majestätischen Jaguaren. Trotz massiver Abholzung in der Vergangenheit bemühen sich lokale Gemeinschaften und Naturschützer heute verstärkt um den Erhalt dieses grünen Schatzes. Ein Besuch in der Lacandona ist eine Reise in das Herz der mexikanischen Wildnis. Biologie K7 - Tropischer Regenwald und indigene Kulturen.",
      hu: "Lacandon dzsungel egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Lacandon dzsungel kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Jungla Lacandon este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Jungla Lacandon este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Lacandon Jungle, or Selva Lacandona, is a dense and mysterious rainforest located in the eastern part of Chiapas, along the border with Guatemala. It is the ancestral home of the Lacandon Maya, a group that maintained their traditional way of life deep within the forest for centuries. This incredibly biodiverse region is characterized by soaring mahogany and cedar trees, hidden waterfalls, and the ruins of ancient cities like Bonampak and Yaxchilán. The jungle is a haven for rare wildlife, including the harpy eagle, the scarlet macaw, and the elusive jaguar. Exploring the Lacandon Jungle often involves guided treks led by local community members, who share their deep knowledge of medicinal plants and forest lore. Despite its beauty, the region faces significant threats from deforestation and agricultural expansion, making sustainable tourism and conservation efforts more critical than ever. It remains one of the last true wilderness areas in North America.",
      es: "La Selva Lacandona, en el estado de Chiapas, es la selva tropical más grande de México y un lugar de increíble importancia biológica y cultural. Es el hogar de los Mayas Lacandones, quienes han conservado su estilo de vida tradicional y su profunda conexión espiritual con el bosque hasta hoy. La selva está surcada por ríos caudalosos, cascadas espectaculares como Agua Azul y ruinas mayas ocultas como Bonampak. La inmensa biodiversidad abarca desde guacamayas coloridas hasta orquídeas raras y los majestuosos jaguares. A pesar de la deforestación masiva en el pasado, las comunidades locales y los conservacionistas se esfuerzan hoy intensamente por preservar este tesoro verde. Una visita a la Lacandona es un viaje al corazón de la naturaleza salvaje mexicana. Biología K7 - Selva tropical y culturas indígenas.",
      pt: "A Selva Lacandona, no estado de Chiapas, é a maior floresta tropical do México e um lugar de incrível importância biológica e cultural. É o lar dos Maias Lacandones, que preservaram seu estilo de vida tradicional e sua profunda conexão espiritual com a floresta até hoje. A selva é atravessada por rios caudalosos, cachoeiras espetaculares como Agua Azul e ruínas maias escondidas como Bonampak. A imensa biodiversidade varia de araras coloridas a orquídeas raras e os majestosos jaguares. Apesar do desmatamento massivo no passado, as comunidades locais e os conservacionistas hoje se esforçam intensamente para preservar este tesouro verde. Uma visita à Lacandona é uma viagem ao coração da natureza selvagem mexicana. Biologia K7 - Floresta tropical e culturas indígenas.",
      fr: "La Selva Lacandona, dans l'État du Chiapas, est la plus grande forêt tropicale du Mexique et un lieu d'une importance biologique et culturelle incroyable. Elle est la terre des Mayas Lacandons, qui ont préservé leur mode de vie traditionnel et leur lien spirituel profond avec la forêt jusqu'à aujourd'hui. La jungle est sillonnée par des rivières impétueuses, des cascades spectaculaires comme Agua Azul et des ruines mayas cachées comme Bonampak. L'immense biodiversité va des aras colorés aux orchidées rares en passant par les majestueux jaguars. Malgré une déforestation massive par le passé, les communautés locales et les défenseurs de la nature s'efforcent aujourd'hui de préserver ce trésor vert. Une visite dans la Lacandona est un voyage au cœur de la vie sauvage mexicaine. Biologie K7 - Forêt tropicale et cultures indigènes.",
    },
    factsAdvanced: {
      de: ["Stellt etwa 10 % der gesamten biologischen Vielfalt Mexikos auf kleiner Fläche.", "Ist der Lebensraum des seltenen Harpyien-Adlers, des mächtigsten Greifvogels.", "Wichtige archäologische Stätten wie Palenque liegen am Rande dieses Dschungels.", "Die Lacandonen sind direkte Nachfahren der klassischen Maya-Bevölkerung.", "Der Usumacinta-Fluss bildet eine natürliche Grenze und Lebensader des Waldes.", "Enthält über 3.000 verschiedene Pflanzenarten, viele davon endemisch.", "Das Gebiet ist ein wichtiges Reservoir für Süßwasser und Kohlenstoffspeicherung."],
      hu: ["Lacandon dzsungel sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Lacandon dzsungel egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Jungla Lacandon ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Jungla Lacandon este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The jungle is named after the Lacandon people, who call themselves 'Hach Winik' (the true people).",
        "It is estimated to contain up to 20% of all the biological species found in Mexico.",
        "The Montes Azules Biosphere Reserve is the most strictly protected core area within the jungle.",
        "The Lacandon Jungle is the northernmost tropical rainforest in the Western Hemisphere.",
        "The ruins of Bonampak, hidden within the jungle, are famous for their incredibly preserved Maya murals."
      ],
      es: ["Representa aproximadamente el 10% de toda la biodiversidad de México en una superficie pequeña.", "Es el hábitat del raro águila arpía, el ave de presa más poderosa.", "Sitios arqueológicos importantes como Palenque se encuentran en el borde de esta selva.", "Los Lacandones son descendientes directos de la población maya clásica.", "El río Usumacinta forma una frontera natural y es el eje vital de la selva.", "Contiene más de 3.000 especies de plantas diferentes, muchas de ellas endémicas.", "El área es un importante reservorio de agua dulce y almacenamiento de carbono."],
      pt: ["Representa cerca de 10% de toda a biodiversidade do México em uma área pequena.", "É o habitat da rara águia-harpia, a ave de rapina mais poderosa.", "Sítios arqueológicos importantes como Palenque situam-se na borda desta selva.", "Os Lacandones são descendentes diretos da população maia clássica.", "O rio Usumacinta forma uma fronteira natural e é a artéria vital da floresta.", "Contém mais de 3.000 espécies de plantas diferentes, muitas delas endêmicas.", "A área é um importante reservatório de água doce e armazenamento de carbono."],
      fr: ["Représente environ 10 % de toute la biodiversité du Mexique sur une petite surface.", "Est l'habitat de la rare harpie féroce, le rapace le plus puissant.", "Des sites archéologiques importants comme Palenque se trouvent en bordure de cette jungle.", "Les Lacandons sont des descendants directs de la population maya classique.", "Le fleuve Usumacinta forme une frontière naturelle et constitue l'artère vitale de la forêt.", "Contient plus de 3 000 espèces végétales différentes, dont beaucoup sont endémiques.", "La zone est un réservoir important d'eau douce et de stockage de carbone."],
    }, image: "/poi-images/lacandon-jungle-life-v2.webp",
    description: {
      de: "Einer der letzten großen Regenwälder Nordamerikas, gelegen im Grenzgebiet zu Guatemala.",
      hu: "Chiapas államban fekvő hatalmas esőerdő, amely a lakandon maják otthona.",
      ro: "Una dintre ultimele păduri tropicale mari din America de Nord, cămin al poporului mayaș lacandon și al unei biodiversități uriașe.",
      en: "One of North America's largest remaining rainforests, home to the Lacandon Maya and an incredible variety of tropical wildlife.",
      es: "Una de las últimas grandes selvas tropicales de América del Norte, ubicada en la frontera con Guatemala.",
      pt: "Uma das últimas grandes florestas tropicais da América do Norte, localizada na fronteira com a Guatemala.",
      fr: "L'une des dernières grandes forêts tropicales d'Amérique du Nord, située à la frontière avec le Guatemala.",
    },
    facts: {
      de: ["Enorme Artenvielfalt", "Heimat der Lakandonen-Maya", "Bedeutende Flusssysteme"],
      hu: ["Mexikó legnagyobb esőerdője", "Ritka hárpiasasok élőhelye", "Bonampak maja romjai a területén"],
      ro: ["Adăpostește rari vulturi harpie", "Include ruinele mayașe de la Bonampak", "Acoperă estul statului Chiapas"],
      en: ["Covers eastern Chiapas", "Habitat for the rare harpy eagle", "Contains the ruins of Bonampak"],
      es: ["Enorme biodiversidad", "Hogar de los Mayas Lacandones", "Sistemas fluviales importantes"],
      pt: ["Enorme biodiversidade", "Lar dos Maias Lacandones", "Sistemas fluviais importantes"],
      fr: ["Biodiversité énorme", "Terre des Mayas Lacandons", "Systèmes fluviaux importants"],
    }
  },
  {
    id: "el-vizcaino-life-v2",
    type: "animal-habitat",
    parent: "MX-BCS",
    coords: [-114.2, 27.7],
    name: { de: "El Vizcaino", hu: "El Vizcaino", ro: "El Vizcaino", en: "El Vizcaino"},
    image: "/poi-images/el-vizcaino-life-v2.webp",
    descriptionAdvanced: {
      de: "Das Biosphärenreservat El Vizcaíno auf der Halbinsel Baja California ist eines der größten Schutzgebiete der Erde und ein Ort von rauer, wüstenhafter Schönheit. Es ist weltberühmt für seine Lagunen, in denen Grauwale jedes Jahr ihre Kälber zur Welt bringen und aufziehen. Die Begegnung mit diesen sanften Riesen in den Lagunen von Ojo de Liebre und San Ignacio gehört zu den eindrucksvollsten Naturerlebnissen weltweit. Neben den Meeresriesen beherbergt das Reservat die bedrohte Berber-Antilope und prähistorische Höhlenmalereien in der Sierra de San Francisco. Die Landschaft ist geprägt von bizarren Kakteenwäldern und weiten Salzebenen, die einen extremen Kontrast zum tiefblauen Pazifik bilden. El Vizcaíno ist ein Refugium für das Leben unter extremen Bedingungen und UNESCO-Weltnaturerbe. Geographie K8 - Wüstenökosysteme und Meeressäuger.",
      hu: "El Vizcaino egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. El Vizcaino kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "El Vizcaino este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. El Vizcaino este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The El Vizcaíno Biosphere Reserve is a vast and varied protected area located in the central part of the Baja California Peninsula. Spanning from the Pacific Ocean to the Sea of Cortez, it encompasses sandy deserts, rugged mountains, and pristine coastal lagoons. The reserve is most famous for the Ojo de Liebre and San Ignacio lagoons, which serve as the primary calving grounds for the eastern North Pacific gray whale. Every winter, these gentle giants migrate thousands of miles to these shallow, protected waters to give birth and nurse their young. Beyond its marine significance, the reserve is home to the endangered Peninsular pronghorn and features remarkable ancient rock paintings in the Sierra de San Francisco. The stark, arid beauty of the landscape and its unique endemic flora, such as the giant cardon cactus, make it a place of profound scientific and aesthetic value. It is one of the largest and most ecologically significant reserves in all of Latin America.",
      es: "La Reserva de la Biosfera El Vizcaíno, en la península de Baja California, es una de las áreas protegidas más grandes del mundo y un lugar de belleza desértica y ruda. Es mundialmente famosa por sus lagunas, donde cada año las ballenas grises dan a luz y crían a sus ballenatos. El encuentro con estos gigantes gentiles en las lagunas de Ojo de Liebre y San Ignacio es una de las experiencias naturales más impresionantes del mundo. Además de los gigantes marinos, la reserva alberga al berrendo de Baja California, en peligro de extinción, y pinturas rupestres prehistóricas en la Sierra de San Francisco. El paisaje se caracteriza por extraños bosques de cactus y vastas salinas que forman un contraste extremo con el azul profundo del Pacífico. El Vizcaíno es un refugio para la vida en condiciones extremas y es Patrimonio Natural de la Humanidad por la UNESCO.",
      pt: "A Reserva da Biosfera El Vizcaíno, na península da Baixa Califórnia, é uma das maiores áreas protegidas do mundo e um lugar de beleza desértica e rústica. É mundialmente famosa pelas suas lagoas, onde as baleias-cinzentas dão à luz e criam seus filhotes todos os anos. O encontro com estes gigantes gentis nas lagoas de Ojo de Liebre e San Ignacio é uma das experiências naturais mais impressionantes do mundo. Além dos gigantes marinhos, a reserva abriga a antilocapra da Baixa Califórnia, ameaçada de extinção, e pinturas rupestres pré-históricas na Sierra de San Francisco. A paisagem é caracterizada por florestas de cactos exóticos e vastas planícies de sal, que formam um contraste extremo com o azul profundo do Pacífico. El Vizcaíno é um refúgio para a vida sob condições extremas e Patrimônio Mundial da UNESCO.",
      fr: "La réserve de biosphère El Vizcaíno, sur la péninsule de Basse-Californie, est l'une des plus grandes zones protégées de la planète et un lieu d'une beauté désertique sauvage. Elle est mondialement célèbre pour ses lagunes où les baleines grises donnent naissance et élèvent leurs baleineaux chaque année. La rencontre avec ces géants pacifiques dans les lagunes d'Ojo de Liebre et de San Ignacio compte parmi les expériences naturelles les plus impressionnantes au monde. Outre les géants des mers, la réserve abrite l'antilocapre de Basse-Californie, menacée, ainsi que des peintures rupestres préhistoriques dans la Sierra de San Francisco. Le paysage est marqué par de bizarres forêts de cactus et de vastes plaines salées qui forment un contraste extrême avec le bleu profond du Pacifique. El Vizcaíno est un refuge pour la vie dans des conditions extrêmes et est classé au patrimoine mondial de l'UNESCO.",
    },
    factsAdvanced: {
      de: ["Umfasst eine riesige Fläche von mehr als 2,5 Millionen Hektar.", "Die Lagunen sind die weltweit wichtigsten Fortpflanzungsstätten für Grauwale.", "Die Höhlenmalereien der Sierra de San Francisco sind über 10.000 Jahre alt.", "Beherbergt den endemischen Baja-California-Gabelbock, eine sehr seltene Antilopenart.", "Die jährliche Wanderung der Grauwale erstreckt sich über 10.000 Kilometer.", "Das Reservat umfasst sowohl Küstenwüsten als auch Hochgebirgsregionen.", "Wurde 1993 aufgrund seiner einzigartigen Biodiversität zum UNESCO-Welterbe erklärt."],
      hu: ["El Vizcaino sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "El Vizcaino egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["El Vizcaino ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "El Vizcaino este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "At over 2.5 million hectares, it is the largest protected area in Mexico.",
        "The reserve was designated a UNESCO World Heritage site in 1993, specifically for its whale sanctuaries.",
        "The Sierra de San Francisco within the reserve contains some of the most important prehistoric rock art in the world.",
        "The Peninsular pronghorn found here is one of the fastest land animals in the Western Hemisphere.",
        "The reserve's lagoons are also a critical wintering site for thousands of migratory waterfowl and shorebirds."
      ],
      es: ["Abarca una superficie enorme de más de 2,5 millones de hectáreas.", "Las lagunas son los sitios de reproducción de ballenas grises más importantes del mundo.", "Las pinturas rupestres de la Sierra de San Francisco tienen más de 10.000 años de antigüedad.", "Alberga al berrendo de Baja California, una especie de antílope muy rara.", "La migración anual de las ballenas grises se extiende por más de 10.000 kilómetros.", "La reserva incluye tanto desiertos costeros como regiones de alta montaña.", "Fue declarada Patrimonio de la Humanidad por la UNESCO en 1993 debido a su biodiversidad única."],
      pt: ["Abrange uma área enorme de mais de 2,5 milhões de hectares.", "As lagoas são os locais de reprodução de baleias-cinzentas mais importantes do mundo.", "As pinturas rupestres da Sierra de San Francisco têm mais de 10.000 anos.", "Abriga a antilocapra da Baixa Califórnia, uma espécie de antílope muito rara.", "A migração anual das baleias-cinzentas estende-se por mais de 10.000 quilômetros.", "A reserva inclui desertos costeiros e regiões de alta montanha.", "Foi declarada Patrimônio Mundial da UNESCO em 1993 devido à sua biodiversidade única."],
      fr: ["Couvre une surface immense de plus de 2,5 millions d'hectares.", "Les lagunes sont les sites de reproduction des baleines grises les plus importants au monde.", "Les peintures rupestres de la Sierra de San Francisco ont plus de 10 000 ans.", "Abrite l'antilocapre de Basse-Californie, une espèce d'antilope très rare.", "La migration annuelle des baleines grises s'étend sur plus de 10 000 kilomètres.", "La réserve comprend aussi bien des déserts côtiers que des régions de haute montagne.", "Inscrit au patrimoine mondial de l'UNESCO en 1993 pour sa biodiversité unique."],
    }, image: "/poi-images/el-vizcaino-life-v2.webp",
    description: {
      de: "Ein riesiges Biosphärenreservat auf der Halbinsel Baja California, berühmt für seine Wüstenlandschaften.",
      hu: "A Kaliforniai-félszigeten elterülő óriási rezervátum, amely a szürke bálnák legfontosabb szaporodóhelye.",
      ro: "Cea mai mare zonă protejată din Mexic, celebră pentru lagunele sale de coastă unde balenele cenușii vin să nască.",
      en: "Mexico's largest protected area, famous for its coastal lagoons that serve as the primary calving grounds for gray whales.",
      es: "Una enorme reserva de la biosfera en la península de Baja California, famosa por sus paisajes desérticos.",
      pt: "Uma enorme reserva da biosfera na península da Baixa Califórnia, famosa por suas paisagens desérticas.",
      fr: "Une immense réserve de biosphère sur la péninsule de Basse-Californie, célèbre pour ses paysages désertiques.",
    },
    facts: {
      de: ["UNESCO-Welterbe", "Lagunen zur Walbeobachtung", "Über 2,5 Millionen Hektar"],
      hu: ["UNESCO világörökségi helyszín", "2,5 millió hektáros terület", "10 000 éves barlangrajzok"],
      ro: ["Rezervație UNESCO din anul 1993", "Suprafață de peste 2,5 milioane hectare", "Include picturi rupestre preistorice"],
      en: ["UNESCO site since 1993", "Over 2.5 million hectares", "Includes prehistoric rock paintings"],
      es: ["Patrimonio de la Humanidad", "Lagunas para avistamiento de ballenas", "Más de 2,5 millones de hectáreas"],
      pt: ["Patrimônio Mundial da UNESCO", "Lagoas para observação de baleias", "Mais de 2,5 milhões de hectares"],
      fr: ["Patrimoine mondial de l'UNESCO", "Lagunes pour l'observation des baleines", "Plus de 2,5 millions d'hectares"],
    }
  },
  {
    id: "celestun-flamingo-reserve-life-v2", "sights": {
      "de": [
            {
                  "name": "14 000 flamencos - noches (ruta a la derecha)",
                  "text": "Abendlicher Aussichtspunkt für eine große Flamingokolonie, erreichbar über die rechte Straße.",
                  "category": "landmark",
                  "coords": [
                        -90.374217,
                        20.892555
                  ]
            },
            {
                  "name": "Celeste Vida",
                  "text": "Erholungsresort mit dem Namen ‚Himmelsblau Leben‘ im Flamingo-Reservat.",
                  "category": "recreational",
                  "coords": [
                        -90.394357,
                        20.871815
                  ]
            },
            {
                  "name": "Reserva de la Biósfera Celestún",
                  "text": "Geschütztes Feuchtgebiet, berühmt für tausende Flamingos und Mangrovenwälder.",
                  "category": "landmark",
                  "coords": [
                        -90.38339,
                        20.857625
                  ]
            },
            {
                  "name": "Parque Ecoturistico Jaltun",
                  "text": "Ökotourismus-Park mit Führungen durch Dschungel und Wasserwege.",
                  "category": "landmark",
                  "coords": [
                        -90.385848,
                        20.857892
                  ]
            },
            {
                  "name": "Flamencos - mañana",
                  "text": "Morgendlicher Beobachtungspunkt für Flamingos im Celestún-Reservat.",
                  "category": "landmark",
                  "coords": [
                        -90.403441,
                        20.836101
                  ]
            },
            {
                  "name": "Hacienda Real de Salinas",
                  "text": "Historische Salz-Hazienda mit Ruinen und Salinen in der Nähe.",
                  "category": "historical",
                  "coords": [
                        -90.400835,
                        20.820996
                  ]
            },
            {
                  "name": "Ruinas",
                  "text": "Alte Maya-Ruinen im Flamingo-Reservat.",
                  "category": "historical",
                  "coords": [
                        -90.405582,
                        20.836368
                  ]
            }
      ],
      "hu": [
            {
                  "name": "14 000 flamencos - noches (ruta a la derecha)",
                  "text": "Esti kilátóhely egy nagy flamingókolóniához, a jobb oldali úton érhető el.",
                  "category": "landmark",
                  "coords": [
                        -90.374217,
                        20.892555
                  ]
            },
            {
                  "name": "Celeste Vida",
                  "text": "Üdülőközpont a 'Mennyei kék élet' néven, a Flamingó-rezervátumban.",
                  "category": "recreational",
                  "coords": [
                        -90.394357,
                        20.871815
                  ]
            },
            {
                  "name": "Reserva de la Biósfera Celestún",
                  "text": "Védett vizes élőhely, híres ezernyi flamingójáról és mangroveerdőiről.",
                  "category": "landmark",
                  "coords": [
                        -90.38339,
                        20.857625
                  ]
            },
            {
                  "name": "Parque Ecoturistico Jaltun",
                  "text": "Ökoturisztikai park dzsungel- és vízi úti túrákkal.",
                  "category": "landmark",
                  "coords": [
                        -90.385848,
                        20.857892
                  ]
            },
            {
                  "name": "Flamencos - mañana",
                  "text": "Reggeli flamingómegfigyelő hely a Celestún-rezervátumban.",
                  "category": "landmark",
                  "coords": [
                        -90.403441,
                        20.836101
                  ]
            },
            {
                  "name": "Hacienda Real de Salinas",
                  "text": "Történelmi sóüzemű hacienda romokkal és sómezőkkel a közelben.",
                  "category": "historical",
                  "coords": [
                        -90.400835,
                        20.820996
                  ]
            },
            {
                  "name": "Ruinas",
                  "text": "Ősi maja romok a Flamingó-rezervátumban.",
                  "category": "historical",
                  "coords": [
                        -90.405582,
                        20.836368
                  ]
            }
      ],
      "ro": [
            {
                  "name": "14 000 flamencos - noches (ruta a la derecha)",
                  "text": "Punct de observare seara pentru o colonie mare de flamingi, accesibil pe drumul din dreapta.",
                  "category": "landmark",
                  "coords": [
                        -90.374217,
                        20.892555
                  ]
            },
            {
                  "name": "Celeste Vida",
                  "text": "Stațiune de agrement cu numele 'Viața Cerul-Albastru' în Rezervația Flamingo.",
                  "category": "recreational",
                  "coords": [
                        -90.394357,
                        20.871815
                  ]
            },
            {
                  "name": "Reserva de la Biósfera Celestún",
                  "text": "Zonă umedă protejată, renumită pentru miile de flamingi și pădurile de mangrove.",
                  "category": "landmark",
                  "coords": [
                        -90.38339,
                        20.857625
                  ]
            },
            {
                  "name": "Parque Ecoturistico Jaltun",
                  "text": "Parc ecoturistic cu excursii ghidate prin junglă și căi navigabile.",
                  "category": "landmark",
                  "coords": [
                        -90.385848,
                        20.857892
                  ]
            },
            {
                  "name": "Flamencos - mañana",
                  "text": "Punct de observare dimineața pentru flamingi în Rezervația Celestún.",
                  "category": "landmark",
                  "coords": [
                        -90.403441,
                        20.836101
                  ]
            },
            {
                  "name": "Hacienda Real de Salinas",
                  "text": "Hacienda istorică de sare, cu ruine și saline în apropiere.",
                  "category": "historical",
                  "coords": [
                        -90.400835,
                        20.820996
                  ]
            },
            {
                  "name": "Ruinas",
                  "text": "Ruine mayașe antice în Rezervația Flamingo.",
                  "category": "historical",
                  "coords": [
                        -90.405582,
                        20.836368
                  ]
            }
      ],
      "en": [
            {
                  "name": "14 000 flamingos - evening (take the road at your right)",
                  "text": "Evening viewing spot for a large flock of flamingos, accessed via the right-hand road.",
                  "category": "landmark",
                  "coords": [
                        -90.374217,
                        20.892555
                  ]
            },
            {
                  "name": "Celeste Vida",
                  "text": "Recreational resort named 'Sky Blue Life' in the Flamingo Reserve.",
                  "category": "recreational",
                  "coords": [
                        -90.394357,
                        20.871815
                  ]
            },
            {
                  "name": "Reserva de la Biósfera Celestún",
                  "text": "Protected wetland reserve famous for its thousands of flamingos and mangrove forests.",
                  "category": "landmark",
                  "coords": [
                        -90.38339,
                        20.857625
                  ]
            },
            {
                  "name": "Parque Ecoturistico Jaltun",
                  "text": "Ecotourism park offering guided tours of the surrounding jungle and waterways.",
                  "category": "landmark",
                  "coords": [
                        -90.385848,
                        20.857892
                  ]
            },
            {
                  "name": "Flamingos - morning",
                  "text": "Morning viewing area for flamingos in the Celestún Reserve.",
                  "category": "landmark",
                  "coords": [
                        -90.403441,
                        20.836101
                  ]
            },
            {
                  "name": "Hacienda Real de Salinas",
                  "text": "Historic salt-producing hacienda with ruins and salt flats nearby.",
                  "category": "historical",
                  "coords": [
                        -90.400835,
                        20.820996
                  ]
            },
            {
                  "name": "Ruinas",
                  "text": "Ancient Maya ruins located within the Flamingo Reserve.",
                  "category": "historical",
                  "coords": [
                        -90.405582,
                        20.836368
                  ]
            }
      ]
},
    type: "animal-habitat",
    parent: "MX-YUC",
    coords: [-90.39, 20.86],
    name: { de: "Celestun-Reservat", hu: "Celestun rezervatum", ro: "Rezervatia Celestun", en: "Celestun Reserve"},
    image: "/poi-images/celestun-flamingo-reserve-life-v2.webp",
    descriptionAdvanced: {
      de: "Das Biosphärenreservat Celestún an der Westküste von Yucatán ist ein Paradies für Vogelliebhaber und berühmt für seine riesigen Kolonien von rosa Flamingos. Die Lagune, in der sich Süßwasser aus unterirdischen Cenoten mit dem Salzwasser des Golfs von Mexiko vermischt, bietet ideale Bedingungen für diese eleganten Vögel. Bei einer Bootsfahrt durch die dichten Mangroventunnel können Besucher zudem Krokodile, Leguane und hunderte andere Vogelarten entdecken. Die intensive rosa Farbe der Flamingos, die durch ihre Nahrung aus kleinen Krebstieren entsteht, bildet einen fantastischen Kontrast zum blauen Wasser. Celestún ist ein wichtiges Schutzgebiet für Zugvögel und ein wunderbares Beispiel für die empfindlichen Küstenökosysteme der Halbinsel. Die Ruhe des Fischerdorfes Celestún rundet das Naturerlebnis perfekt ab. Biologie K6 - Nahrungsketten und Küstenhabitate.",
      hu: "Celestun rezervatum egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Celestun rezervatum kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Rezervatia Celestun este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Rezervatia Celestun este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Ría Celestún Biosphere Reserve, located on the western coast of the Yucatán Peninsula, is a stunning wetland ecosystem famous for its vibrant pink inhabitants. The mix of freshwater from underground springs and saltwater from the Gulf of Mexico creates an ideal habitat for the American flamingo, which gathers here in the thousands. Visitors typically explore the reserve by small boat, navigating through narrow mangrove tunnels to reach the open lagoons where the flamingos feed. The reserve is also a paradise for birdwatchers, with over 300 species recorded, including pelicans, egrets, and kingfishers. The surrounding town of Celestún offers a glimpse into traditional coastal life, known for its fresh seafood and tranquil beaches. The protection of these wetlands is essential for maintaining the regional water quality and providing a nursery for many marine species. The ethereal beauty of the pink flamingos against the emerald green of the mangroves is one of Yucatán's most iconic sights.",
      es: "La Reserva de la Biosfera de Celestún, en la costa oeste de Yucatán, es un paraíso para los amantes de las aves y famosa por sus inmensas colonias de flamencos rosas. La laguna, donde el agua dulce de los cenotes subterráneos se mezcla con el agua salada del Golfo de México, ofrece condiciones ideales para estas elegantes aves. En un paseo en bote por los densos túneles de manglar, los visitantes pueden descubrir además cocodrilos, iguanas y cientos de otras especies de aves. El intenso color rosa de los flamencos, que surge de su alimentación a base de pequeños crustáceos, forma un contraste fantástico con el agua azul. Celestún es un área de protección importante para aves migratorias y un maravilloso ejemplo de los sensibles ecosistemas costeros de la península.",
      pt: "A Reserva da Biosfera de Celestún, na costa oeste de Yucatán, é um paraíso para os amantes de aves e famosa pelas suas enormes colónias de flamingos cor-de-rosa. A lagoa, onde a água doce dos cenotes subterrâneos se mistura com a água salgada do Golfo do México, oferece condições ideais para estas elegantes aves. Num passeio de barco pelos densos túneis de mangais, os visitantes podem também descobrir crocodilos, iguanas e centenas de outras espécies de aves. A cor rosa intensa dos flamingos, que resulta da sua alimentação à base de pequenos crustáceos, forma um contraste fantástico com a água azul. Celestún é uma importante área de proteção para aves migratórias e um exemplo maravilhoso dos sensíveis ecossistemas costeiros da península.",
      fr: "La réserve de la biosphère de Celestún, sur la côte ouest du Yucatán, est un paradis pour les ornithologues et célèbre pour ses immenses colonies de flamants roses. La lagune, où l'eau douce des cénotes souterrains se mélange à l'eau salée du golfe du Mexique, offre des conditions idéales pour ces oiseaux élégants. Lors d'une balade en bateau à travers les denses tunnels de mangrove, les visiteurs peuvent également apercevoir des crocodiles, des iguanas et des centaines d'autres espèces d'oiseaux. La couleur rose intense des flamants, due à leur alimentation composée de petits crustacés, crée un contraste fantastique avec l'eau bleue. Celestún est une zone de protection majeure pour les oiseaux migrateurs et un exemple merveilleux des écosystèmes côtiers fragiles de la péninsule.",
    },
    factsAdvanced: {
      de: ["Beherbergt im Winter bis zu 35.000 Rosa Flamingos gleichzeitig.", "Das Reservat erstreckt sich über eine Fläche von rund 60.000 Hektar.", "Die Mangrovenwälder schützen die Küste vor Erosion und Stürmen.", "Es wurden über 300 verschiedene Vogelarten im Schutzgebiet registriert.", "Die Lagune von Celestún ist ein wichtiges Brutgebiet für Meeresschildkröten.", "Die rosa Färbung der Flamingos stammt von Carotinoiden in ihrer Nahrung.", "Süßwasserquellen im Meer, sogenannte 'Ojos de Agua', speisen die Lagune."],
      hu: ["Celestun rezervatum sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Celestun rezervatum egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Rezervatia Celestun ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Rezervatia Celestun este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The reserve covers approximately 60,000 hectares and was established in 1979.",
        "The intense pink color of the flamingos comes from the carotenoids in the shrimp and algae they eat.",
        "Celestún is home to several 'petenes,' which are circular islands of jungle growing around freshwater springs.",
        "It is one of the few places in the world where flamingos can be seen in their natural habitat year-round.",
        "The reserve also protects important nesting sites for the hawksbill and green sea turtles."
      ],
      es: ["Alberga hasta 35.000 flamencos rosas simultáneamente en invierno.", "La reserva se extiende por una superficie de unas 60.000 hectáreas.", "Los manglares protegen la costa de la erosión y las tormentas.", "Se han registrado más de 300 especies diferentes de aves en el área protegida.", "La laguna de Celestún es una importante zona de cría para tortugas marinas.", "La coloración rosa de los flamencos proviene de los carotenoides en su comida.", "Fuentes de agua dulce en el mar, llamadas 'Ojos de Agua', alimentan la laguna."],
      pt: ["Abriga até 35.000 flamingos cor-de-rosa simultaneamente no inverno.", "A reserva estende-se por uma área de cerca de 60.000 hectares.", "As florestas de mangais protegem a costa da erosão e das tempestades.", "Foram registadas mais de 300 espécies diferentes de aves na área protegida.", "A lagoa de Celestún é uma importante área de reprodução para tartarugas marinhas.", "A coloração rosa dos flamingos provém dos carotenoides na sua alimentação.", "Fontes de água doce no mar, os chamados 'Ojos de Agua', alimentam a lagoa."],
      fr: ["Accueille jusqu'à 35 000 flamants roses simultanément en hiver.", "La réserve s'étend sur une superficie d'environ 60 000 hectares.", "Les mangroves protègent la côte de l'érosion et des tempêtes.", "Plus de 300 espèces d'oiseaux différentes ont été enregistrées dans la réserve.", "La lagune de Celestún est un site de reproduction majeur pour les tortues marines.", "La coloration rose des flamants provient des caroténoïdes de leur nourriture.", "Des sources d'eau douce en mer, appelées 'Ojos de Agua', alimentent la lagune."],
    }, image: "/poi-images/celestun-flamingo-reserve-life-v2.webp",
    description: {
      de: "Ein Küstenreservat in Yucatán, das für seine riesigen Kolonien rosa Flamingos weltberühmt ist.",
      hu: "Yucatán-félsziget nyugati partján fekvő vizes élőhely, amely a rózsás flamingók kolóniáiról híres.",
      ro: "Zonă umedă de coastă în Yucatán, faimoasă pentru miile de flamingo roz și ecosistemele sale diverse de mangrove.",
      en: "A coastal wetland in Yucatán famous for its thousands of pink flamingos and diverse mangrove ecosystems.",
      es: "Una reserva costera en Yucatán, famosa mundialmente por sus enormes colonias de flamencos rosas.",
      pt: "Uma reserva costeira em Yucatán, mundialmente famosa pelas suas enormes colónias de flamingos cor-de-rosa.",
      fr: "Une réserve côtière au Yucatán, célèbre dans le monde entier pour ses immenses colonies de flamants roses.",
    },
    facts: {
      de: ["Rosa Flamingos im Winter", "Bootstouren durch Mangroven", "Mündung des Rio Esperanza"],
      hu: ["Akár 35 000 flamingó telente", "300-nál több madárfaj", "Tengeri teknősök fészkelőhelye"],
      ro: ["Peste 300 de specii de păsări înregistrate", "Izvoare unice de apă dulce în mare", "Loc important de cuibărit pentru țestoase"],
      en: ["Over 300 bird species recorded", "Features unique freshwater springs", "Important sea turtle nesting area"],
      es: ["Flamencos rosas en invierno", "Paseos en bote por manglares", "Desembocadura del río Esperanza"],
      pt: ["Flamingos cor-de-rosa no inverno", "Passeios de barco pelos mangais", "Foz do Rio Esperanza"],
      fr: ["Flamants roses en hiver", "Tours en bateau dans la mangrove", "Embouchure du Rio Esperanza"],
    }
  },
  {
    id: "la-ventanilla-turtle-reserve-life-v2", "sights": {
      "de": [
            {
                  "name": "Playa Camarón",
                  "text": "Sandstrand namens ‚Garnelenstrand‘, bekannt für Garnelen, Teil der Ventanilla-Schildkrötenreservat.",
                  "category": "recreational",
                  "coords": [
                        -96.525803,
                        15.662727
                  ]
            },
            {
                  "name": "Letras gigantes Zipolite",
                  "text": "Riesenbuchstaben, die Zipolite buchstabieren, ein Fotospot im Gebiet von La Ventanilla.",
                  "category": "landmark",
                  "coords": [
                        -96.511691,
                        15.66214
                  ]
            },
            {
                  "name": "Zapotengo beach",
                  "text": "Sandstrand gesäumt von Zapote-Bäumen, beliebt für Schildkröten-Nistplätze.",
                  "category": "recreational",
                  "coords": [
                        -96.40539,
                        15.692642
                  ]
            },
            {
                  "name": "Casa de Cultura Petra Valle",
                  "text": "Kulturzentrum benannt nach Petra Valle, das lokale Kunst und Geschichte zeigt.",
                  "category": "cultural",
                  "coords": [
                        -96.465384,
                        15.746578
                  ]
            },
            {
                  "name": "Playa Estacahuite",
                  "text": "Abgelegener Strand mit ruhigem Wasser, ideal zum Schwimmen und Kajakfahren.",
                  "category": "recreational",
                  "coords": [
                        -96.482102,
                        15.668763
                  ]
            },
            {
                  "name": "Rancho Los Mangos",
                  "text": "Ehemalige Mangoplantage, heute ein Natur-Rückzugsort.",
                  "category": "landmark",
                  "coords": [
                        -96.520484,
                        15.665498
                  ]
            },
            {
                  "name": "Playa Shambhala",
                  "text": "Spirituell gestalteter Strand, bekannt für Yoga-Retreats und friedliche Atmosphäre.",
                  "category": "recreational",
                  "coords": [
                        -96.522944,
                        15.663189
                  ]
            },
            {
                  "name": "Playa Zipolite",
                  "text": "Playa Zipolite ist ein FKK-Strand mit starken Wellen nahe der Ventanilla-Schildkrötenreservat.",
                  "category": "landmark",
                  "coords": [
                        -96.515942,
                        15.66256
                  ]
            },
            {
                  "name": "Playa del amor",
                  "text": "Liebesstrand – romantischer Ort mit Sonnenuntergängen und sanften Wellen.",
                  "category": "recreational",
                  "coords": [
                        -96.509372,
                        15.660052
                  ]
            },
            {
                  "name": "Playa Panteon",
                  "text": "Strand neben einem Friedhof, der ein einzigartiges kulturelles Erlebnis bietet.",
                  "category": "recreational",
                  "coords": [
                        -96.495612,
                        15.664923
                  ]
            },
            {
                  "name": "Faro de Puerto Ângel",
                  "text": "Leuchtturm von Puerto Ángel, der die Schiffe entlang der oaxacanischen Küste führt.",
                  "category": "landmark",
                  "coords": [
                        -96.500222,
                        15.658436
                  ]
            },
            {
                  "name": "La Boquilla",
                  "text": "‚Der kleine Mund‘ – eine schmale Bucht, perfekt zum Schwimmen.",
                  "category": "recreational",
                  "coords": [
                        -96.465128,
                        15.683337
                  ]
            },
            {
                  "name": "playa Cabanas abandonadas",
                  "text": "Strand mit verlassenen Hütten, beliebt für Fotografie.",
                  "category": "recreational",
                  "coords": [
                        -96.390553,
                        15.689066
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Playa Camarón",
                  "text": "Homokos 'Garnélarák-part', garnélájáról ismert, a Ventanilla Teknős-rezervátum része.",
                  "category": "recreational",
                  "coords": [
                        -96.525803,
                        15.662727
                  ]
            },
            {
                  "name": "Letras gigantes Zipolite",
                  "text": "Óriásbetűk, amelyek Zipolitét írják ki, egy fotópont a La Ventanilla körzetben.",
                  "category": "landmark",
                  "coords": [
                        -96.511691,
                        15.66214
                  ]
            },
            {
                  "name": "Zapotengo beach",
                  "text": "Homokos part Zapote-fákkal szegélyezve, népszerű teknős fészkelőhely.",
                  "category": "recreational",
                  "coords": [
                        -96.40539,
                        15.692642
                  ]
            },
            {
                  "name": "Casa de Cultura Petra Valle",
                  "text": "Petra Valle-ról elnevezett kulturális központ, helyi művészetet és történelmet bemutatva.",
                  "category": "cultural",
                  "coords": [
                        -96.465384,
                        15.746578
                  ]
            },
            {
                  "name": "Playa Estacahuite",
                  "text": "Félreeső strand nyugodt vízzel, ideális úszáshoz és kajakozáshoz.",
                  "category": "recreational",
                  "coords": [
                        -96.482102,
                        15.668763
                  ]
            },
            {
                  "name": "Rancho Los Mangos",
                  "text": "Egykori mangóültetvény, ma természetes menedékhely.",
                  "category": "landmark",
                  "coords": [
                        -96.520484,
                        15.665498
                  ]
            },
            {
                  "name": "Playa Shambhala",
                  "text": "Spirituális tematikájú part, híres jóga-elvonulásairól és békés légköréről.",
                  "category": "recreational",
                  "coords": [
                        -96.522944,
                        15.663189
                  ]
            },
            {
                  "name": "Playa Zipolite",
                  "text": "A Playa Zipolite egy textil nélküli strand erős hullámokkal a Ventanilla Teknősrezervátum közelében.",
                  "category": "landmark",
                  "coords": [
                        -96.515942,
                        15.66256
                  ]
            },
            {
                  "name": "Playa del amor",
                  "text": "Szeretet-part – romantikus hely naplementékkel és lassú hullámokkal.",
                  "category": "recreational",
                  "coords": [
                        -96.509372,
                        15.660052
                  ]
            },
            {
                  "name": "Playa Panteon",
                  "text": "Temető melletti part, egyedi kulturális élményt nyújtva.",
                  "category": "recreational",
                  "coords": [
                        -96.495612,
                        15.664923
                  ]
            },
            {
                  "name": "Faro de Puerto Ângel",
                  "text": "Puerto Ángel világítótorony, amely a hajókat vezeti Oaxaca partjai mentén.",
                  "category": "landmark",
                  "coords": [
                        -96.500222,
                        15.658436
                  ]
            },
            {
                  "name": "La Boquilla",
                  "text": "'A kis száj' – egy keskeny öböl, tökéletes úszáshoz.",
                  "category": "recreational",
                  "coords": [
                        -96.465128,
                        15.683337
                  ]
            },
            {
                  "name": "playa Cabanas abandonadas",
                  "text": "Part elhagyott kunyhókkal, népszerű fotózásra.",
                  "category": "recreational",
                  "coords": [
                        -96.390553,
                        15.689066
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Playa Camarón",
                  "text": "Plajă nisipoasă numită 'Plaja Creveților', cunoscută pentru creveți, parte a Rezervației de Țestoase Ventanilla.",
                  "category": "recreational",
                  "coords": [
                        -96.525803,
                        15.662727
                  ]
            },
            {
                  "name": "Letras gigantes Zipolite",
                  "text": "Litere uriașe care scriu Zipolite, un loc de fotografiat în zona La Ventanilla.",
                  "category": "landmark",
                  "coords": [
                        -96.511691,
                        15.66214
                  ]
            },
            {
                  "name": "Zapotengo beach",
                  "text": "Plajă nisipoasă mărginită de copaci Zapote, populară pentru cuibăritul țestoaselor.",
                  "category": "recreational",
                  "coords": [
                        -96.40539,
                        15.692642
                  ]
            },
            {
                  "name": "Casa de Cultura Petra Valle",
                  "text": "Centru cultural numit după Petra Valle, care prezintă arta și istoria locală.",
                  "category": "cultural",
                  "coords": [
                        -96.465384,
                        15.746578
                  ]
            },
            {
                  "name": "Playa Estacahuite",
                  "text": "Plajă izolată cu ape liniștite, ideală pentru înot și caiac.",
                  "category": "recreational",
                  "coords": [
                        -96.482102,
                        15.668763
                  ]
            },
            {
                  "name": "Rancho Los Mangos",
                  "text": "Fostă plantație de mango, acum un refugiu natural.",
                  "category": "landmark",
                  "coords": [
                        -96.520484,
                        15.665498
                  ]
            },
            {
                  "name": "Playa Shambhala",
                  "text": "Plajă cu temă spirituală, cunoscută pentru retragerile de yoga și atmosfera liniștită.",
                  "category": "recreational",
                  "coords": [
                        -96.522944,
                        15.663189
                  ]
            },
            {
                  "name": "Playa Zipolite",
                  "text": "Playa Zipolite este o plajă pentru nudism cu valuri puternice lângă Rezervația de Țestoase Ventanilla.",
                  "category": "landmark",
                  "coords": [
                        -96.515942,
                        15.66256
                  ]
            },
            {
                  "name": "Playa del amor",
                  "text": "Plaja Iubirii – loc romantic cu apusuri și valuri line.",
                  "category": "recreational",
                  "coords": [
                        -96.509372,
                        15.660052
                  ]
            },
            {
                  "name": "Playa Panteon",
                  "text": "Plajă adiacentă unui cimitir, oferind o experiență culturală unică.",
                  "category": "recreational",
                  "coords": [
                        -96.495612,
                        15.664923
                  ]
            },
            {
                  "name": "Faro de Puerto Ângel",
                  "text": "Farul din Puerto Ángel, care ghidează navele de-a lungul coastei Oaxacăi.",
                  "category": "landmark",
                  "coords": [
                        -96.500222,
                        15.658436
                  ]
            },
            {
                  "name": "La Boquilla",
                  "text": "'Gurița' – un golf îngust, perfect pentru înot.",
                  "category": "recreational",
                  "coords": [
                        -96.465128,
                        15.683337
                  ]
            },
            {
                  "name": "playa Cabanas abandonadas",
                  "text": "Plajă cu cabane abandonate, populară pentru fotografie.",
                  "category": "recreational",
                  "coords": [
                        -96.390553,
                        15.689066
                  ]
            }
      ],
      "en": [
            {
                  "name": "Playa Camarón",
                  "text": "Sandy beach named 'Shrimp Beach', known for shrimp, part of the Ventanilla Turtle Reserve.",
                  "category": "recreational",
                  "coords": [
                        -96.525803,
                        15.662727
                  ]
            },
            {
                  "name": "Letras gigantes Zipolite",
                  "text": "Giant letters spelling Zipolite, a photo spot in the La Ventanilla area.",
                  "category": "landmark",
                  "coords": [
                        -96.511691,
                        15.66214
                  ]
            },
            {
                  "name": "Zapotengo beach",
                  "text": "Sandy beach lined with zapote trees, popular for turtle nesting.",
                  "category": "recreational",
                  "coords": [
                        -96.40539,
                        15.692642
                  ]
            },
            {
                  "name": "Casa de Cultura Petra Valle",
                  "text": "Cultural center named after Petra Valle, showcasing local art and history.",
                  "category": "cultural",
                  "coords": [
                        -96.465384,
                        15.746578
                  ]
            },
            {
                  "name": "Playa Estacahuite",
                  "text": "Secluded beach with calm waters, ideal for swimming and kayaking.",
                  "category": "recreational",
                  "coords": [
                        -96.482102,
                        15.668763
                  ]
            },
            {
                  "name": "Rancho Los Mangos",
                  "text": "Former mango plantation now a nature retreat.",
                  "category": "landmark",
                  "coords": [
                        -96.520484,
                        15.665498
                  ]
            },
            {
                  "name": "Playa Shambhala",
                  "text": "Spiritual-themed beach known for its yoga retreats and peaceful atmosphere.",
                  "category": "recreational",
                  "coords": [
                        -96.522944,
                        15.663189
                  ]
            },
            {
                  "name": "Playa Zipolite",
                  "text": "Playa Zipolite is a clothing-optional beach with powerful waves near the Ventanilla Turtle Reserve.",
                  "category": "landmark",
                  "coords": [
                        -96.515942,
                        15.66256
                  ]
            },
            {
                  "name": "Playa del amor",
                  "text": "Love Beach – a romantic spot with sunsets and gentle waves.",
                  "category": "recreational",
                  "coords": [
                        -96.509372,
                        15.660052
                  ]
            },
            {
                  "name": "Playa Panteon",
                  "text": "Beach adjacent to a local cemetery, offering a unique cultural experience.",
                  "category": "recreational",
                  "coords": [
                        -96.495612,
                        15.664923
                  ]
            },
            {
                  "name": "Faro de Puerto Ângel",
                  "text": "Lighthouse at Puerto Ángel, guiding ships along the Oaxacan coast.",
                  "category": "landmark",
                  "coords": [
                        -96.500222,
                        15.658436
                  ]
            },
            {
                  "name": "La Boquilla",
                  "text": "The Little Mouth – a narrow cove beach perfect for swimming.",
                  "category": "recreational",
                  "coords": [
                        -96.465128,
                        15.683337
                  ]
            },
            {
                  "name": "playa Cabanas abandonadas",
                  "text": "Abandoned Cabins Beach – a sandy shore with derelict structures, popular for photography.",
                  "category": "recreational",
                  "coords": [
                        -96.390553,
                        15.689066
                  ]
            }
      ]
},
    type: "animal-habitat",
    parent: "MX-OAX",
    coords: [-96.47, 15.67],
    name: { de: "La Ventanilla", hu: "La Ventanilla", ro: "La Ventanilla", en: "La Ventanilla"},
    image: "/poi-images/la-ventanilla-turtle-reserve-life-v2.webp",
    descriptionAdvanced: {
      de: "La Ventanilla an der Küste von Oaxaca ist ein inspirierendes Beispiel für gemeindebasierten Ökotourismus und den Schutz von Meeresschildkröten. Nach der Zerstörung durch Hurrikane haben sich die Dorfbewohner zusammengeschlossen, um ihre Mangroven und die Tierwelt, insbesondere Krokodile und Schildkröten, zu schützen. Besucher können in Ruderbooten durch die Lagunen glide und Leguane in den Bäumen sowie Krokodile beim Sonnenbaden beobachten. Der nahegelegene Strand ist ein wichtiger Nistplatz für die Oliv-Bastardschildkröte, deren Eier von Freiwilligen geschützt werden. Die Gemeinschaft betreibt auch eine Baumschule für Mangroven, um das Ökosystem aktiv wieder aufzuforsten. Ein Besuch in La Ventanilla zeigt, wie Menschen im Einklang mit der Natur leben und diese erfolgreich regenerieren können. Sachkunde K4 - Naturschutz und lokales Engagement.",
      hu: "La Ventanilla egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. La Ventanilla kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "La Ventanilla este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. La Ventanilla este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "La Ventanilla is a unique community-led ecotourism project and nature reserve located on the Pacific coast of Oaxaca, near Puerto Escondido. Named after a natural rock arch that looks like a small window, the area features a lush mangrove lagoon that was restored by local families after being devastated by hurricanes. Today, the lagoon is a thriving habitat for crocodiles, iguanas, and a vast array of bird species. The community operates a turtle sanctuary on the beach, where they protect the nests of olive ridley and leatherback turtles from poachers and predators. Visitors can take guided canoe tours through the mangroves, learning about the complex ecosystem and the community's reforestation efforts. This project is a shining example of how local empowerment and conservation can go hand-in-hand to protect natural resources and provide a sustainable livelihood. The lack of large-scale development ensures that La Ventanilla remains a peaceful and authentic natural escape.",
      es: "La Ventanilla, en la costa de Oaxaca, es un ejemplo inspirador de ecoturismo comunitario y protección de tortugas marinas. Tras la destrucción causada por los huracanes, los habitantes se unieron para proteger sus manglares y la fauna, especialmente cocodrilos y tortugas. Los visitantes pueden deslizarse por las lagunas en botes de remos y observar iguanas en los árboles, así como cocodrilos tomando el sol. La playa cercana es un importante lugar de anidación para la tortuga golfina, cuyos huevos son protegidos por voluntarios. La comunidad también gestiona un vivero de manglares para reforestar activamente el ecosistema. Una visita a La Ventanilla muestra cómo las personas pueden vivir en armonía con la naturaleza y regenerarla con éxito. Conocimiento K4 - Conservación y compromiso local.",
      pt: "La Ventanilla, na costa de Oaxaca, é um exemplo inspirador de ecoturismo comunitário e proteção de tartarugas marinhas. Após a destruição causada por furacões, os moradores se uniram para proteger seus manguezais e a vida selvagem, especialmente crocodilos e tartarugas. Os visitantes podem deslizar pelas lagoas em barcos a remo e observar iguanas nas árvores, bem como crocodilos tomando sol. A praia próxima é um importante local de desova para a tartaruga-oliva, cujos ovos são protegidos por voluntários. A comunidade também opera um viveiro de mangues para reflorestar ativamente o ecossistema. Uma visita a La Ventanilla mostra como as pessoas podem viver em harmonia com a natureza e regenerá-la com sucesso. Conhecimento K4 - Conservação e engajamento local.",
      fr: "La Ventanilla, sur la côte d'Oaxaca, est un exemple inspirant d'écotourisme communautaire et de protection des tortues marines. Après les destructions causées par les ouragans, les villageois se sont unis pour protéger leurs mangroves et la faune, en particulier les crocodiles et les tortues. Les visiteurs peuvent glisser sur les lagunes en barques à rames et observer des iguanes dans les arbres ainsi que des crocodiles se chauffant au soleil. La plage voisine est un site de nidification important pour la tortue olivâtre, dont les œufs sont protégés par des bénévoles. La communauté gère également une pépinière de mangroves pour reboiser activement l'écosystème. Une visite à La Ventanilla montre comment les hommes peuvent vivre en harmonie avec la nature et la régénérer avec succès. Connaissances K4 - Conservation et engagement local.",
    },
    factsAdvanced: {
      de: ["Wurde von einer lokalen Kooperative gegründet, um den Wildtierhandel zu stoppen.", "Das Reservat schützt die gefährdete Oliv-Bastardschildkröte (Lepidochelys olivacea).", "Besucher können an nächtlichen Patrouillen zum Schutz der Schildkröteneier teilnehmen.", "Die Mangrovenwälder beherbergen zwei Arten: die Rote und die Weiße Mangrove.", "Krokodile werden hier in ihrer natürlichen Umgebung ohne Zäune beobachtet.", "Die Gemeinschaft nutzt Solarenergie und betreibt ökologische Abwasserreinigung.", "La Ventanilla hat sich zu einem Modell für nachhaltigen Tourismus in Mexiko entwickelt."],
      hu: ["La Ventanilla sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "La Ventanilla egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["La Ventanilla ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "La Ventanilla este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The project is entirely run by a local cooperative of about 20 families who dedicated themselves to conservation.",
        "The lagoon is home to more than 300 crocodiles that are protected and monitored by the community.",
        "During the nesting season, volunteers and community members patrol the beach every night to rescue turtle eggs.",
        "The mangroves at La Ventanilla help protect the coastline from erosion and storm surges.",
        "The area was officially designated as a protected community-managed zone to prevent industrial tourism."
      ],
      es: ["Fue fundada por una cooperativa local para detener el comercio de vida silvestre.", "La reserva protege a la tortuga golfina (Lepidochelys olivacea), especie en peligro.", "Los visitantes pueden participar en patrullas nocturnas para proteger los huevos de tortuga.", "Los manglares albergan dos especies: el mangle rojo y el blanco.", "Los cocodrilos se observan aquí en su entorno natural sin vallas.", "La comunidad utiliza energía solar y opera un tratamiento ecológico de aguas residuales.", "La Ventanilla se ha convertido en un modelo de turismo sostenible en México."],
      pt: ["Foi fundada por uma cooperativa local para interromper o comércio de vida selvagem.", "A reserva protege a tartaruga-oliva (Lepidochelys olivacea), uma espécie ameaçada.", "Os visitantes podem participar de patrulhas noturnas para proteger os ovos de tartaruga.", "Os manguezais abrigam duas espécies: o mangue-vermelho e o mangue-branco.", "Os crocodilos são observados aqui em seu ambiente natural, sem cercas.", "A comunidade utiliza energia solar e opera o tratamento ecológico de águas residuais.", "La Ventanilla tornou-se um modelo de turismo sustentável no México."],
      fr: ["A été fondée par une coopérative locale pour stopper le commerce de la faune sauvage.", "La réserve protège la tortue olivâtre (Lepidochelys olivacea), une espèce menacée.", "Les visiteurs peuvent participer à des patrouilles nocturnes pour protéger les œufs de tortue.", "Les forêts de mangroves abritent deux espèces : la mangrove rouge et la blanche.", "Les crocodiles y sont observés dans leur environnement naturel sans clôtures.", "La communauté utilise l'énergie solaire et gère un traitement écologique des eaux usées.", "La Ventanilla est devenue un modèle de tourisme durable au Mexique."],
    }, image: "/poi-images/la-ventanilla-turtle-reserve-life-v2.webp",
    description: {
      de: "Ein ökologisches Projekt an der Küste von Oaxaca, das sich dem Schutz von Schildkröten und Krokodilen widmet.",
      hu: "Oaxaca partjainál fekvő közösségi ökoturisztikai projekt, amely a teknősök védelmére összpontosít.",
      ro: "Rezervație naturală gestionată de comunitate în Oaxaca, dedicată restaurării mangrovelor și protecției țestoaselor marine.",
      en: "A community-managed nature reserve in Oaxaca dedicated to the restoration of mangroves and the protection of sea turtles.",
      es: "Un proyecto ecológico en la costa de Oaxaca dedicado a la protección de tortugas y cocodrilos.",
      pt: "Um projeto ecológico na costa de Oaxaca dedicado à proteção de tartarugas e crocodilos.",
      fr: "Un projet écologique sur la côte d'Oaxaca dédié à la protection des tortues et des crocodiles.",
    },
    facts: {
      de: ["Gemeindebasiertes Ökoprojekt", "Lagune mit Krokodilen", "Nistplatz für Meeresschildkröten"],
      hu: ["Közösségi kezelésű rezervátum", "Több mint 300 védett krokodil", "Olajzöld fattyúteknősök fészkelőhelye"],
      ro: ["Administrată de familii locale", "Peste 300 de crocodili protejați", "Sanctuar pentru țestoasa măslinie"],
      en: ["Run by local families", "Home to over 300 crocodiles", "Sanctuary for olive ridley turtles"],
      es: ["Proyecto ecológico comunitario", "Laguna con cocodrilos", "Lugar de anidación de tortugas marinas"],
      pt: ["Projeto ecológico comunitário", "Lagoa com crocodilos", "Local de desova de tartarugas marinhas"],
      fr: ["Projet écologique communautaire", "Lagune avec des crocodiles", "Lieu de nidification des tortues marines"],
    }
  },
  {
    id: "los-tuxtlas-reserve-life-v2",
    type: "forest",
    parent: "MX-VER",
    coords: [-95.2, 18.57],
    name: { de: "Los Tuxtlas", hu: "Los Tuxtlas", ro: "Los Tuxtlas", en: "Los Tuxtlas"},
    image: "/poi-images/los-tuxtlas-reserve-life-v2.webp",
    descriptionAdvanced: {
      de: "Das Biosphärenreservat Los Tuxtlas im Bundesstaat Veracruz ist ein einzigartiges Gebiet, in dem tropischer Regenwald direkt auf vulkanische Gebirge und den Golf von Mexiko trifft. Diese besondere geografische Lage sorgt für eine immense Artenvielfalt und macht es zu einem der wichtigsten Zentren für die Erforschung der Tropenbiologie. Das Reservat beherbergt Brüllaffen, Tukane und seltene Orchideen, während die Küstenabschnitte wichtige Nistplätze für Schildkröten bieten. Spektakuläre Wasserfälle wie Eyipantla und tiefblaue Lagunen wie Catemaco prägen das Landschaftsbild. Die Region ist auch für ihre spirituellen Traditionen und die Verbindung von Naturheilkunde und indigenem Wissen bekannt. Los Tuxtlas ist ein grünes Juwel am Golf, das trotz des menschlichen Drucks seine wilde Schönheit bewahrt hat. Biologie K7 - Biodiversität und vulkanische Böden.",
      hu: "Los Tuxtlas egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Los Tuxtlas kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Los Tuxtlas este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Los Tuxtlas este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Los Tuxtlas Biosphere Reserve in southern Veracruz is a remarkable 'ecological island' where the northernmost tropical rainforest in the Americas meets ancient volcanic peaks. This unique landscape, characterized by dramatic elevation changes, ranges from the shores of the Gulf of Mexico to the summits of the San Martín and Santa Martha volcanoes. The reserve is a hotspot for biodiversity, housing over 500 species of birds and more than 100 species of mammals, including the mantled howler monkey. It is also famous for its connection to the Olmec civilization, with numerous archaeological finds having been discovered in its fertile soils. The region's heavy rainfall feeds countless rivers and spectacular waterfalls, such as Eyipantla Falls. Despite historical pressure from cattle ranching, Los Tuxtlas remains a critical area for biological research and a bastion of wild nature in the state of Veracruz. Its cloud forests and lowland jungles offer a glimpse into the incredible natural wealth of Mexico's humid tropics.",
      es: "La Reserva de la Biosfera Los Tuxtlas, en el estado de Veracruz, es un área única donde la selva tropical se encuentra directamente con las montañas volcánicas y el Golfo de México. Esta ubicación geográfica especial genera una inmensa biodiversidad y la convierte en uno de los centros más importantes para la investigación de la biología tropical. La reserva alberga monos aulladores, tucanes y orquídeas raras, mientras que los tramos costeros ofrecen importantes lugares de anidación para tortugas. Cascadas espectaculares como Eyipantla y lagunas de color azul profundo como Catemaco definen el paisaje. La región también es conocida por sus tradiciones espirituales y la combinación de medicina natural y conocimiento indígena. Los Tuxtlas es una joya verde en el Golfo que ha conservado su belleza salvaje a pesar de la presión humana. Biología K7 - Biodiversidad y suelos volcánicos.",
      pt: "A Reserva da Biosfera Los Tuxtlas, no estado de Veracruz, é uma área única onde a floresta tropical encontra diretamente as montanhas vulcânicas e o Golfo do México. Esta localização geográfica especial proporciona uma imensa biodiversidade e torna-a um dos centros mais importantes para a pesquisa da biologia tropical. A reserva abriga bugios, tucanos e orquídeas raras, enquanto os trechos costeiros oferecem locais importantes de desova para tartarugas. Cachoeiras espetaculares como Eyipantla e lagoas azul-profundo como Catemaco definem a paisagem. A região também é conhecida por suas tradições espirituais e pela combinação de medicina natural e conhecimento indígena. Los Tuxtlas é uma joia verde no Golfo que preservou sua beleza selvagem, apesar da pressão humana. Biologia K7 - Biodiversidade e solos vulcânicos.",
      fr: "La réserve de biosphère de Los Tuxtlas, dans l'État de Veracruz, est une zone unique où la forêt tropicale rencontre directement les montagnes volcaniques et le golfe du Mexique. Cette situation géographique particulière assure une immense biodiversité et en fait l'un des centres les plus importants pour la recherche en biologie tropicale. La réserve abrite des singes hurleurs, des toucans et des orchidées rares, tandis que les sections côtières offrent des sites de nidification importants pour les tortues. Des cascades spectaculaires comme Eyipantla et des lagunes d'un bleu profond comme Catemaco marquent le paysage. La région est également connue pour ses traditions spirituelles et l'alliance de la médecine naturelle et du savoir indigène. Los Tuxtlas est un joyau vert sur le Golfe qui a conservé sa beauté sauvage malgré la pression humaine. Biologie K7 - Biodiversité et sols volcaniques.",
    },
    factsAdvanced: {
      de: ["Ist das nördlichste tropische Regenwaldgebiet auf dem amerikanischen Kontinent.", "Umfasst eine Fläche von über 155.000 Hektar in einer vulkanischen Zone.", "Beherbergt die wissenschaftliche Station 'Los Tuxtlas' der Universität UNAM.", "Über 500 Vogelarten wurden in diesem verhältnismäßig kleinen Gebiet gezählt.", "Die Lagune von Catemaco ist berühmt für ihre 'Affeninsel' mit Makaken.", "Enthält seltene endemische Arten wie die Los-Tuxtlas-Wachteltaube.", "Die vulkanischen Böden sind extrem fruchtbar und unterstützen das dichte Pflanzenwachstum."],
      hu: ["Los Tuxtlas sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Los Tuxtlas egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Los Tuxtlas ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Los Tuxtlas este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Los Tuxtlas was the site of Mexico's first biological research station, Catemaco, established in 1967.",
        "The reserve is home to the 'giant Olmec heads,' the largest of which was found in the nearby Tres Zapotes.",
        "It contains the San Martín Tuxtla volcano, which last erupted in 1793.",
        "Approximately 40% of all the bird species found in Mexico can be observed in this reserve.",
        "The region is one of the few places in the world where you can see rainforest growing right up to the edge of the sea."
      ],
      es: ["Es el área de selva tropical más al norte del continente americano.", "Abarca una superficie de más de 155.000 hectáreas en una zona volcánica.", "Alberga la estación científica 'Los Tuxtlas' de la universidad UNAM.", "Se han contado más de 500 especies de aves en esta área relativamente pequeña.", "La laguna de Catemaco es famosa por su 'isla de los monos' con macacos.", "Contiene especies endémicas raras como la paloma perdiz de Los Tuxtlas.", "Los suelos volcánicos son extremadamente fértiles y sustentan el denso crecimiento vegetal."],
      pt: ["É a área de floresta tropical mais ao norte do continente americano.", "Abrange uma área de mais de 155.000 hectares em uma zona vulcânica.", "Abriga a estação científica 'Los Tuxtlas' da universidade UNAM.", "Mais de 500 espécies de aves foram contadas nesta área relativamente pequena.", "A lagoa de Catemaco é famosa pela sua 'ilha dos macacos' com macacos.", "Contém espécies endêmicas raras, como a pomba-perdiz de Los Tuxtlas.", "Os solos vulcânicos são extremamente férteis e sustentam o crescimento vegetal denso."],
      fr: ["C'est la zone de forêt tropicale la plus septentrionale du continent américain.", "Couvre une superficie de plus de 155 000 hectares dans une zone volcanique.", "Abrite la station scientifique 'Los Tuxtlas' de l'université UNAM.", "Plus de 500 espèces d'oiseaux ont été recensées dans cette zone relativement petite.", "La lagune de Catemaco est célèbre pour son 'île aux singes' avec des macaques.", "Contient des espèces endémiques rares comme la colombe de Tuxtla.", "Les sols volcaniques sont extrêmement fertiles et favorisent la croissance dense des plantes."],
    }, image: "/poi-images/los-tuxtlas-reserve-life-v2.webp",
    description: {
      de: "Ein Biosphärenreservat in Veracruz mit üppigen Regenwäldern, Wasserfällen und Vulkanlandschaften.",
      hu: "Veracruz államban található bioszféra-rezervátum, ahol az esőerdő vulkanikus hegyekkel találkozik.",
      ro: "Rezervație a biosferei în Veracruz unde pădurea tropicală întâlnește munții vulcanici și apele Golfului Mexic.",
      en: "An ecological hotspot in Veracruz where tropical rainforests meet volcanic mountains and the shores of the Gulf of Mexico.",
      es: "Una reserva de la biosfera en Veracruz con exuberantes selvas tropicales, cascadas y paisajes volcánicos.",
      pt: "Uma reserva da biosfera em Veracruz com florestas tropicais exuberantes, cachoeiras e paisagens vulcânicas.",
      fr: "Une réserve de biosphère à Veracruz avec des forêts tropicales luxuriantes, des cascades et des paysages volcaniques.",
    },
    facts: {
      de: ["Nördlichster Regenwald Amerikas", "Über 500 Vogelarten", "Sitz der Olmeken-Kultur"],
      hu: ["Amerika legészakibb trópusi erdője", "500-nál több madárfaj", "Az olmék kultúra egykori központja"],
      ro: ["Cea mai nordică pădure tropicală din America", "Peste 500 de specii de păsări înregistrate", "Regiune a vechii civilizații olmece"],
      en: ["Northernmost rainforest in the Americas", "Contains over 500 bird species", "Region of the ancient Olmec culture"],
      es: ["Selva tropical más septentrional de América", "Más de 500 especies de aves", "Sede de la cultura Olmeca"],
      pt: ["Floresta tropical mais setentrional da América", "Mais de 500 espécies de aves", "Sede da cultura Olmeca"],
      fr: ["Forêt tropicale la plus septentrionale d'Amérique", "Plus de 500 espèces d'oiseaux", "Siège de la culture olmèque"],
    }
  },
  {
    id: "sierra-gorda-reserve-life-v2",
    type: "forest",
    parent: "MX-QUE",
    coords: [-99.73, 21.2],
    name: { de: "Sierra Gorda", hu: "Sierra Gorda", ro: "Sierra Gorda", en: "Sierra Gorda"},
    image: "/poi-images/sierra-gorda-reserve-life-v2.webp",
    descriptionAdvanced: {
      de: "Das Biosphärenreservat Sierra Gorda im Bundesstaat Querétaro ist eines der ökologisch vielfältigsten Gebiete Mexikos und wird oft als 'grünes Herz' des Landes bezeichnet. Es umfasst extreme Gegensätze, von trockenen Wüstenlandschaften in den Tälern bis hin zu nebligen Wolkenwäldern auf den Berggipfeln. Diese Vielfalt ermöglicht das Überleben zahlreicher bedrohter Arten wie Jaguare, Militäraras und den geheimnisvollen Puma. Neben der Natur ist die Region für die fünf Franziskanermissionen bekannt, die von der UNESCO zum Weltkulturerbe erklärt wurden und sich harmonisch in die Landschaft einfügen. Das Reservat wird durch ein preisgekröntes Modell des bürgerschaftlichen Naturschutzes verwaltet, das die lokale Bevölkerung aktiv einbezieht. Sierra Gorda ist ein Paradies für Wanderer und Naturliebhaber, die die Stille der Berge suchen. Geographie K8 - Klimazonen und nachhaltiges Management.",
      hu: "Sierra Gorda egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Sierra Gorda kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Sierra Gorda este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Sierra Gorda este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Sierra Gorda Biosphere Reserve, located in the northern part of Querétaro, is one of Mexico's most diverse and spectacular protected areas. Often described as a 'green jewel' in the heart of the country, it encompasses a stunning range of ecosystems, from semi-arid deserts to lush cloud forests and rugged mountain peaks. The reserve is a haven for hikers and nature lovers, featuring deep canyons, hidden caves, and the spectacular Sotano del Barro, one of the world's deepest natural pits. Beyond its natural wonders, the Sierra Gorda is famous for the five 18th-century Franciscan missions, a UNESCO World Heritage site, which showcase a unique blend of European and indigenous artistic styles. The reserve is managed with a strong focus on community participation and sustainable agriculture, aiming to protect the landscape while improving local livelihoods. Its complex topography creates numerous microclimates, making it home to rare species like the jaguar and the military macaw. It is a place of profound beauty and cultural richness, representing the best of Mexico's natural diversity.",
      es: "La Reserva de la Biosfera Sierra Gorda, en el estado de Querétaro, es una de las áreas más diversas desde el punto de vista ecológico de México y a menudo se le llama el 'corazón verde' del país. Abarca contrastes extremos, desde paisajes desérticos secos en los valles hasta bosques de niebla en las cimas de las montañas. Esta diversidad permite la supervivencia de numerosas especies amenazadas como jaguares, guacamayas militares y el misterioso puma. Además de la naturaleza, la región es conocida por las cinco misiones franciscanas que fueron declaradas Patrimonio de la Humanidad por la UNESCO y se integran armoniosamente en el paisaje. La reserva se gestiona mediante un modelo galardonado de conservación cívica que involucra activamente a la población local. Sierra Gorda es un paraíso para senderistas y amantes de la naturaleza que buscan la paz de las montañas. Geografía K8 - Zonas climáticas y gestión sostenible.",
      pt: "A Reserva da Biosfera Sierra Gorda, no estado de Querétaro, é uma das áreas ecologicamente mais diversas do México e é frequentemente referida como o 'coração verde' do país. Abrange contrastes extremos, desde paisagens desérticas secas nos vales até florestas de nuvens nebulosas nos cumes das montanhas. Esta diversidade permite a sobrevivência de inúmeras espécies ameaçadas, como jaguares, araras-militares e o misterioso puma. Além da natureza, a região é conhecida pelas cinco missões franciscanas que foram declaradas Património Mundial pela UNESCO e que se integram harmoniosamente na paisagem. A reserva é gerida através de um modelo premiado de conservação cívica, que envolve ativamente a população local. Sierra Gorda é um paraíso para caminhantes e amantes da natureza que procuram o silêncio das montanhas. Geografia K8 - Zonas climáticas e gestão sustentável.",
      fr: "La réserve de biosphère de la Sierra Gorda, dans l'État de Querétaro, est l'une des zones les plus diversifiées du Mexique sur le plan écologique et est souvent appelée le 'cœur vert' du pays. Elle englobe des contrastes extrêmes, allant de paysages désertiques secs dans les vallées aux forêts de nuages brumeuses sur les sommets. Cette diversité permet la survie de nombreuses espèces menacées telles que les jaguars, les aras militaires et le mystérieux puma. Outre la nature, la région est connue pour ses cinq missions franciscaines, classées au patrimoine mondial de l'UNESCO, qui s'intègrent harmonieusement dans le paysage. La réserve est gérée selon un modèle de conservation citoyenne primé qui implique activement la population locale. La Sierra Gorda est un paradis pour les randonneurs et les amoureux de la nature qui recherchent le calme des montagnes. Géographie K8 - Zones climatiques et gestion durable.",
    },
    factsAdvanced: {
      de: ["Umfasst etwa 383.567 Hektar und deckt ein Drittel des Staates Querétaro ab.", "Beherbergt 15 verschiedene Vegetationsarten aufgrund der extremen Höhenunterschiede.", "Ist die Heimat der kleinsten Magnolienart der Welt und zahlreicher Orchideen.", "Die UNESCO-Weltkulturerbe-Missionen wurden im 18. Jahrhundert erbaut.", "Gilt als eines der am besten verwalteten Biosphärenreservate Lateinamerikas.", "Bietet spektakuläre Aussichtspunkte wie den 'Cuatro Palos' über die Wolken.", "Das Projekt 'Bosque Sustentable' zahlt Bauern für den Erhalt ihrer Wälder."],
      hu: ["Sierra Gorda sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Sierra Gorda egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Sierra Gorda ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Sierra Gorda este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The reserve covers 383,567 hectares, which is about one-third of the entire state of Querétaro.",
        "The Sotano del Barro is a massive collapse sinkhole with a vertical drop of 410 meters.",
        "It was designated a UNESCO Biosphere Reserve in 1997 due to its high level of endemism.",
        "The five Franciscan missions were founded by Junípero Serra between 1750 and 1760.",
        "The reserve is home to the second-largest population of military macaws in Mexico."
      ],
      es: ["Abarca unas 383.567 hectáreas y cubre un tercio del estado de Querétaro.", "Alberga 15 tipos de vegetación diferentes debido a las extremas diferencias de altitud.", "Es el hogar de la especie de magnolia más pequeña del mundo y de numerosas orquídeas.", "Las misiones Patrimonio de la Humanidad de la UNESCO fueron construidas en el siglo XVIII.", "Es considerada una de las reservas de la biosfera mejor gestionadas de América Latina.", "Ofrece miradores espectaculares como el 'Cuatro Palos' por encima de las nubes.", "El proyecto 'Bosque Sustentable' paga a los campesinos por la conservación de sus bosques."],
      pt: ["Abrange cerca de 383.567 hectares e cobre um terço do estado de Querétaro.", "Abriga 15 tipos diferentes de vegetação devido às diferenças extremas de altitude.", "É o lar da menor espécie de magnólia do mundo e de inúmeras orquídeas.", "As missões Património Mundial da UNESCO foram construídas no século XVIII.", "É considerada uma das reservas da biosfera mais bem geridas da América Latina.", "Oferece miradouros espetaculares, como o 'Cuatro Palos', acima das nuvens.", "O projeto 'Bosque Sustentable' paga aos agricultores pela preservação das suas florestas."],
      fr: ["Couvre environ 383 567 hectares et représente un tiers de l'État de Querétaro.", "Abrite 15 types de végétation différents en raison des écarts d'altitude extrêmes.", "Elle accueille la plus petite espèce de magnolia au monde et de nombreuses orchidées.", "Les missions classées à l'UNESCO ont été construites au XVIIIe siècle.", "Est considérée comme l'une des réserves de biosphère les mieux gérées d'Amérique latine.", "Offre des points de vue spectaculaires comme le 'Cuatro Palos' au-dessus des nuages.", "Le projet 'Bosque Sustentable' rémunère les paysans pour la préservation de leurs forêts."],
    }, image: "/poi-images/sierra-gorda-reserve-life-v2.webp",
    description: {
      de: "Ein ökologisch vielfältiges Gebirge in Querétaro mit tiefen Schluchten und dichten Wäldern.",
      hu: "Querétaro állam északi részén fekvő változatos táj, amely a sivatagtól a köderdőkig terjed.",
      ro: "Rezervație a biosferei spectaculoasă în Querétaro, variind de la deșerturi aride la păduri de nori de mare altitudine.",
      en: "A stunningly diverse biosphere reserve in Querétaro, ranging from arid deserts to lush, high-altitude cloud forests.",
      es: "Una montaña ecológicamente diversa en Querétaro con cañones profundos y bosques densos.",
      pt: "Uma montanha ecologicamente diversa em Querétaro, com desfiladeiros profundos e florestas densas.",
      fr: "Une montagne écologiquement diversifiée à Querétaro avec des canyons profonds et des forêts denses.",
    },
    facts: {
      de: ["Fünf Franziskanermissionen", "Enorme Höhenunterschiede", "Schutzgebiet für den Puma"],
      hu: ["UNESCO világörökségi missziók", "15 különböző növénytársulás", "A puma egyik fontos menedékhelye"],
      ro: ["Include cinci misiuni franciscane UNESCO", "Ocupă o treime din statul Querétaro", "Refugiu important pentru pume"],
      en: ["Covers one-third of Querétaro", "Includes five 18th-century missions", "UNESCO site for nature and culture"],
      es: ["Cinco misiones franciscanas", "Enormes diferencias de altitud", "Área protegida para el puma"],
      pt: ["Cinco missões franciscanas", "Enormes diferenças de altitude", "Área protegida para o puma"],
      fr: ["Cinq missions franciscaines", "Écarts d'altitude énormes", "Zone de protection pour le puma"],
    }
  },
  {
    id: "isla-holbox-life-v2", "sights": {
      "de": [
            {
                  "name": "Punta Mosquito",
                  "text": "Punta Mosquito ist ein Aussichtspunkt auf der Isla Holbox mit malerischem Blick.",
                  "category": "natural",
                  "coords": [
                        -87.326734,
                        21.558309
                  ]
            },
            {
                  "name": "Crossing point.",
                  "text": "Fährüberfahrtspunkt zur Insel Holbox von Chiquilá aus.",
                  "category": "landmark",
                  "coords": [
                        -87.344825,
                        21.54082
                  ]
            },
            {
                  "name": "Playa mosquito",
                  "text": "Moskitostrand – ein abgelegener Sandstrand, bekannt für starke Winde und Kitesurfen.",
                  "category": "landmark",
                  "coords": [
                        -87.355819,
                        21.545631
                  ]
            },
            {
                  "name": "Punta Mosquito",
                  "text": "Punkt der Insel mit Panoramablick und einer flachen Sandbank.",
                  "category": "landmark",
                  "coords": [
                        -87.353117,
                        21.543074
                  ]
            },
            {
                  "name": "Bonita Playa",
                  "text": "Unberührter weißer Sandstrand mit türkisfarbenem Wasser.",
                  "category": "natural",
                  "coords": [
                        -87.360938,
                        21.536906
                  ]
            },
            {
                  "name": "Refugio Animal",
                  "text": "Das Tierheim auf Isla Holbox bietet Schutz für Tiere.",
                  "category": "landmark",
                  "coords": [
                        -87.382115,
                        21.521826
                  ]
            },
            {
                  "name": "Punta Cocos",
                  "text": "Felsige Landzunge mit Kokospalmen und einem kleinen Leuchtturm.",
                  "category": "landmark",
                  "coords": [
                        -87.392467,
                        21.50826
                  ]
            },
            {
                  "name": "Tortugas",
                  "text": "Der Aussichtspunkt Tortugas auf Isla Holbox bietet Blick auf Schildkröten.",
                  "category": "natural",
                  "coords": [
                        -87.396523,
                        21.510625
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Punta Mosquito",
                  "text": "A Punta Mosquito egy kilátó Isla Holboxon, festői kilátással.",
                  "category": "natural",
                  "coords": [
                        -87.326734,
                        21.558309
                  ]
            },
            {
                  "name": "Crossing point.",
                  "text": "Komphajó-átkelőhely Holbox szigetére Chiquilából.",
                  "category": "landmark",
                  "coords": [
                        -87.344825,
                        21.54082
                  ]
            },
            {
                  "name": "Playa mosquito",
                  "text": "Szúnyog-part – egy távoli homokos part, erős széléről és kitesurfingjéről ismert.",
                  "category": "landmark",
                  "coords": [
                        -87.355819,
                        21.545631
                  ]
            },
            {
                  "name": "Punta Mosquito",
                  "text": "A sziget pontja panorámás kilátással és sekély homokzátonnyal.",
                  "category": "landmark",
                  "coords": [
                        -87.353117,
                        21.543074
                  ]
            },
            {
                  "name": "Bonita Playa",
                  "text": "Érintetlen fehér homokos part türkizkék vízzel.",
                  "category": "natural",
                  "coords": [
                        -87.360938,
                        21.536906
                  ]
            },
            {
                  "name": "Refugio Animal",
                  "text": "Az állatmenhely Isla Holbox szigetén állatok védelmét szolgálja.",
                  "category": "landmark",
                  "coords": [
                        -87.382115,
                        21.521826
                  ]
            },
            {
                  "name": "Punta Cocos",
                  "text": "Sziklás földnyelv kókuszpálmákkal és egy kis világítótoronnyal.",
                  "category": "landmark",
                  "coords": [
                        -87.392467,
                        21.50826
                  ]
            },
            {
                  "name": "Tortugas",
                  "text": "A Tortugas kilátópont Isla Holboxon teknősök megfigyelésére alkalmas.",
                  "category": "natural",
                  "coords": [
                        -87.396523,
                        21.510625
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Punta Mosquito",
                  "text": "Punta Mosquito este un punct de belvedere pe Isla Holbox, cu priveliști pitorești.",
                  "category": "natural",
                  "coords": [
                        -87.326734,
                        21.558309
                  ]
            },
            {
                  "name": "Crossing point.",
                  "text": "Punct de traversare cu feribotul către insula Holbox din Chiquilá.",
                  "category": "landmark",
                  "coords": [
                        -87.344825,
                        21.54082
                  ]
            },
            {
                  "name": "Playa mosquito",
                  "text": "Plaja Țânțarilor – o întindere îndepărtată de nisip, cunoscută pentru vânturi puternice și kitesurfing.",
                  "category": "landmark",
                  "coords": [
                        -87.355819,
                        21.545631
                  ]
            },
            {
                  "name": "Punta Mosquito",
                  "text": "Punct al insulei cu vederi panoramice și o bancă de nisip mică.",
                  "category": "landmark",
                  "coords": [
                        -87.353117,
                        21.543074
                  ]
            },
            {
                  "name": "Bonita Playa",
                  "text": "Plajă curată de nisip alb cu ape turcoaz.",
                  "category": "natural",
                  "coords": [
                        -87.360938,
                        21.536906
                  ]
            },
            {
                  "name": "Refugio Animal",
                  "text": "Adăpostul pentru animale de pe insula Isla Holbox oferă protecție animalelor.",
                  "category": "landmark",
                  "coords": [
                        -87.382115,
                        21.521826
                  ]
            },
            {
                  "name": "Punta Cocos",
                  "text": "Promontoriu stâncos cu palmieri de cocos și un far mic.",
                  "category": "landmark",
                  "coords": [
                        -87.392467,
                        21.50826
                  ]
            },
            {
                  "name": "Tortugas",
                  "text": "Punctul de belvedere Tortugas de pe Isla Holbox oferă vedere la țestoase.",
                  "category": "natural",
                  "coords": [
                        -87.396523,
                        21.510625
                  ]
            }
      ],
      "en": [
            {
                  "name": "Punta Mosquito",
                  "text": "Punta Mosquito is a viewpoint on Isla Holbox offering scenic views.",
                  "category": "natural",
                  "coords": [
                        -87.326734,
                        21.558309
                  ]
            },
            {
                  "name": "Crossing point.",
                  "text": "Ferry crossing point to Holbox island from Chiquilá.",
                  "category": "landmark",
                  "coords": [
                        -87.344825,
                        21.54082
                  ]
            },
            {
                  "name": "Playa mosquito",
                  "text": "Mosquito Beach – a remote stretch of sand known for strong winds and kite surfing.",
                  "category": "landmark",
                  "coords": [
                        -87.355819,
                        21.545631
                  ]
            },
            {
                  "name": "Mosquito Point",
                  "text": "Point of the island with panoramic views and a shallow sandbar.",
                  "category": "landmark",
                  "coords": [
                        -87.353117,
                        21.543074
                  ]
            },
            {
                  "name": "Bonita Playa",
                  "text": "Pristine white-sand beach with turquoise waters.",
                  "category": "natural",
                  "coords": [
                        -87.360938,
                        21.536906
                  ]
            },
            {
                  "name": "Animal Refuge",
                  "text": "Animal refuge on Isla Holbox providing shelter for animals.",
                  "category": "landmark",
                  "coords": [
                        -87.382115,
                        21.521826
                  ]
            },
            {
                  "name": "Punta Cocos",
                  "text": "Rocky outcrop with coconut palms and a small lighthouse.",
                  "category": "landmark",
                  "coords": [
                        -87.392467,
                        21.50826
                  ]
            },
            {
                  "name": "Tortugas",
                  "text": "Tortugas viewpoint on Isla Holbox, where turtles can be seen.",
                  "category": "natural",
                  "coords": [
                        -87.396523,
                        21.510625
                  ]
            }
      ]
},
    type: "island",
    parent: "MX-ROO",
    coords: [-87.38, 21.52],
    name: { de: "Holbox-Insel", hu: "Holbox sziget", ro: "Insula Holbox", en: "Holbox Island"},
    image: "/poi-images/isla-holbox-life-v2.webp",
    descriptionAdvanced: {
      de: "Holbox ist eine verträumte Insel vor der Nordküste der Halbinsel Yucatán, die für ihre entspannte Atmosphäre und ihre sandigen, autofreien Straßen bekannt ist. Die Insel gehört zum Naturschutzgebiet Yum Balam und bietet unberührte Strände mit flachem, türkisblauem Wasser. Ein weltweit bekanntes Highlight ist die Möglichkeit, zwischen Juni und September mit den sanften Walhaien zu schnorcheln, die sich hier versammeln. Flamingos, Pelikane und andere Wasservögel sind in den flachen Lagunen der Insel allgegenwärtig. Die bunten Wandmalereien im Dorf und das Fehlen von großen Hotelketten verleihen Holbox einen einzigartigen, fast magischen Charme. Es ist ein ort, an dem man den Rhythmus der Natur wiederfindet und die Schönheit der mexikanischen Karibik in ihrer reinsten Form genießen kann. Biologie K6 - Meeresbiologie und Ökotourismus.",
      hu: "Holbox sziget egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Holbox sziget kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Insula Holbox este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Insula Holbox este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Isla Holbox is a serene, car-free paradise located just off the northern coast of the Yucatán Peninsula, where the Gulf of Mexico meets the Caribbean Sea. Part of the Yum Balam Biosphere Reserve, the island is famous for its shallow, turquoise waters, vibrant street art, and powdery white sand beaches. Holbox remains a sanctuary for wildlife, particularly the thousands of flamingos that feed in its lagoons and the whale sharks that visit its waters every summer. The town itself has a relaxed, bohemian vibe, with sandy streets and colorful wooden houses that emphasize the island's connection to nature. Visitors can explore the island by golf cart, bicycle, or on foot, discovering hidden sandbars and watching spectacular sunsets over the sea. The island's culinary scene is also a draw, particularly the famous lobster pizza. Holbox offers a tranquil alternative to the bustling tourist hubs of the Riviera Maya, inviting travelers to slow down and enjoy the simple pleasures of island life.",
      es: "Holbox es una isla de ensueño frente a la costa norte de la península de Yucatán, conocida por su atmósfera relajada y sus calles de arena sin automóviles. La isla pertenece a la reserva natural de Yum Balam y ofrece playas vírgenes con aguas turquesas poco profundas. Un punto culminante de fama mundial es la posibilidad de nadar con los gentiles tiburones ballena entre junio y septiembre, cuando se congregan aquí. Flamencos, pelícanos y otras aves acuáticas son omnipresentes en las lagunas poco profundas de la isla. Los coloridos murales en el pueblo y la ausencia de grandes cadenas hoteleras le dan a Holbox un encanto único y casi mágico. Es un lugar donde se puede redescubrir el ritmo de la naturaleza y disfrutar de la belleza del Caribe mexicano en su forma más pura.",
      pt: "Holbox é uma ilha paradisíaca na costa norte da península de Yucatán, conhecida pela sua atmosfera relaxante e pelas suas ruas de areia sem carros. A ilha pertence à reserva natural Yum Balam e oferece praias imaculadas com águas azul-turquesa rasas. Um destaque mundialmente famoso é a possibilidade de mergulhar com os gentis tubarões-baleia entre junho e setembro, que se reúnem aqui. Flamingos, pelicanos e outras aves aquáticas são onipresentes nas lagoas rasas da ilha. Os murais coloridos na aldeia e a ausência de grandes cadeias de hotéis conferem a Holbox um charme único e quase mágico. É um lugar onde se pode reencontrar o ritmo da natureza e apreciar a beleza do Caribe mexicano em sua forma mais pura.",
      fr: "Holbox est une île de rêve située au large de la côte nord de la péninsule du Yucatán, connue pour son atmosphère décontractée et ses rues sablonneuses sans voitures. L'île fait partie de la réserve naturelle de Yum Balam et offre des plages vierges aux eaux turquoise peu profondes. Un moment fort mondialement connu est la possibilité de nager avec les requins-baleines entre juin et septembre. Les flamants roses, les pélicans et d'autres oiseaux aquatiques sont omniprésents dans les lagunes peu profondes de l'île. Les peintures murales colorées du village et l'absence de grandes chaînes hôtelières confèrent à Holbox un charme unique, presque magique. C'est un endroit où l'on retrouve le rythme de la nature et où l'on savoure la beauté des Caraïbes mexicaines dans sa forme la plus pure.",
    },
    factsAdvanced: {
      de: ["Holbox ist etwa 42 Kilometer lang und nur 2 Kilometer breit.", "Es gibt keine Autos auf der Insel; Golfcarts und Fahrräder sind die Hauptverkehrsmittel.", "Die Insel liegt am Zusammenfluss von Golf von Mexiko und Karibischem Meer.", "Die Walhai-Saison zieht Forscher und Touristen aus aller Welt an.", "Große Teile der Insel bestehen aus geschützten Mangrovenwäldern.", "Das Phänomen der Biolumineszenz kann nachts an den Stränden beobachtet werden.", "Der Name 'Holbox' bedeutet in der Maya-Sprache 'Schwarzes Loch'."],
      hu: ["Holbox sziget sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Holbox sziget egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Insula Holbox ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Insula Holbox este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The name 'Holbox' comes from the Maya language and means 'black hole,' referring to the dark waters of the lagoon.",
        "The island is approximately 42 kilometers long and only about 1.5 kilometers wide.",
        "From June to September, Holbox becomes one of the world's best places to swim with whale sharks.",
        "There are no paved roads on the island, and the main mode of transportation is by electric golf cart.",
        "The island is separated from the mainland by the Yalahau Lagoon, which is home to many dolphins and flamingos."
      ],
      es: ["Holbox tiene unos 42 kilómetros de largo y solo 2 kilómetros de ancho.", "No hay autos en la isla; los carritos de golf y las bicicletas son los principales medios de transporte.", "La isla se encuentra en la confluencia del Golfo de México y el Mar Caribe.", "La temporada del tiburón ballena atrae a investigadores y turistas de todo el mundo.", "Gran parte de la isla consiste en bosques de manglares protegidos.", "El fenómeno de la bioluminiscencia se puede observar de noche en las playas.", "El nombre 'Holbox' significa 'Hoyo Negro' en la lengua maya."],
      pt: ["Holbox tem cerca de 42 quilômetros de comprimento e apenas 2 quilômetros de largura.", "Não há carros na ilha; carrinhos de golfe e bicicletas são os principais meios de transporte.", "A ilha está localizada na confluência do Golfo do México e do Mar do Caribe.", "A temporada do tubarão-baleia atrai pesquisadores e turistas de todo o mundo.", "Grande parte da ilha consiste em florestas de manguezais protegidas.", "O fenômeno da bioluminescência pode ser observado à noite nas praias.", "O nome 'Holbox' significa 'Buraco Negro' na língua maia."],
      fr: ["Holbox mesure environ 42 kilomètres de long et seulement 2 kilomètres de large.", "Il n'y a pas de voitures sur l'île ; les voiturettes de golf et les vélos sont les principaux moyens de transport.", "L'île est située au confluent du golfe du Mexique et de la mer des Caraïbes.", "La saison des requins-baleines attire des chercheurs et des touristes du monde entier.", "Une grande partie de l'île est constituée de forêts de mangroves protégées.", "Le phénomène de la bioluminescence peut être observé la nuit sur les plages.", "Le nom 'Holbox' signifie 'Trou Noir' en langue maya."],
    }, image: "/poi-images/isla-holbox-life-v2.webp",
    description: {
      de: "Eine entspannte, autofreie Insel im Norden Yucatáns, bekannt für ihre Sandstraßen und Flamingos.",
      hu: "Autómentes sziget Yucatán északi partjainál, amely homokos utcáiról és nyugodt hangulatáról ismert.",
      ro: "O insulă liniștită fără mașini în nordul peninsulei Yucatán, cunoscută pentru străzile cu nisip și apusurile spectaculoase.",
      en: "A tranquil, car-free island in northern Yucatán known for its sandy streets, vibrant murals, and summer whale shark sightings.",
      es: "Una isla relajada y sin autos en el norte de Yucatán, conocida por sus calles de arena y flamencos.",
      pt: "Uma ilha relaxante e sem carros no norte de Yucatán, conhecida pelas suas ruas de areia e flamingos.",
      fr: "Une île décontractée et sans voiture au nord du Yucatán, connue pour ses rues de sable et ses flamants roses.",
    },
    facts: {
      de: ["Walhaie im Sommer", "Türkisfarbenes Wasser", "Teil des Yum Balam Reservats"],
      hu: ["Nincsenek aszfaltozott utak", "A Yum Balam rezervátum része", "Híres a homár-pizzájáról"],
      ro: ["Fără drumuri asfaltate pe insulă", "Loc ideal pentru înot cu rechinii balenă", "Parte a rezervației Yum Balam"],
      en: ["No paved roads on the island", "Part of Yum Balam reserve", "Famous for its lobster pizza"],
      es: ["Tiburones ballena en verano", "Aguas color turquesa", "Parte de la reserva Yum Balam"],
      pt: ["Tubarões-baleia no verão", "Água azul-turquesa", "Parte da reserva Yum Balam"],
      fr: ["Requins-baleines en été", "Eaux turquoise", "Fait partie de la réserve Yum Balam"],
    }
  },
  {
    id: "isla-mujeres-life-v2", "sights": {
      "de": [
            {
                  "name": "Gran Puerto",
                  "text": "Hauptfährenhafen, der Isla Mujeres mit Cancún verbindet.",
                  "category": "landmark",
                  "coords": [
                        -86.807071,
                        21.183749
                  ]
            },
            {
                  "name": "Playa Tiburón",
                  "text": "Haistrand – benannt nach den Walhaien, die in der Nähe vorkommen.",
                  "category": "landmark",
                  "coords": [
                        -86.733482,
                        21.223499
                  ]
            },
            {
                  "name": "Escalera Arcoiris",
                  "text": "Bunt bemalte Regenbogentreppe an der Hauptstraße der Insel.",
                  "category": "landmark",
                  "coords": [
                        -86.727079,
                        21.226262
                  ]
            },
            {
                  "name": "Monumento a los Maestros",
                  "text": "Denkmal für Lehrer – eine Skulptur zu Ehren von Pädagogen auf dem Hauptplatz.",
                  "category": "historical",
                  "coords": [
                        -86.728074,
                        21.228084
                  ]
            },
            {
                  "name": "Marina Puerto Isla Mujeres - Villa Vera",
                  "text": "Jachthafen mit Anlegestegen und Bootsverleih, Teil des Villa-Vera-Resorts.",
                  "category": "landmark",
                  "coords": [
                        -86.736879,
                        21.235227
                  ]
            },
            {
                  "name": "Varadero Puerto Isla Mujeres",
                  "text": "Werft für Bootsreparaturen und Lagerung nahe der Marina.",
                  "category": "landmark",
                  "coords": [
                        -86.738175,
                        21.237181
                  ]
            },
            {
                  "name": "Dolphin Discovery",
                  "text": "Dolphin Discovery bietet Delfinbegegnungen in Isla Mujeres.",
                  "category": "landmark",
                  "coords": [
                        -86.741671,
                        21.234624
                  ]
            },
            {
                  "name": "Museo historico fotografico",
                  "text": "Historisches Fotomuseum mit Bildern der Inselvergangenheit.",
                  "category": "museum",
                  "coords": [
                        -86.745628,
                        21.256697
                  ]
            },
            {
                  "name": "Esduma La Perla Hotel Isla Mujeres",
                  "text": "Hotel mit Swimmingpool und Strandzugang, genannt ‚Die Perle‘.",
                  "category": "recreational",
                  "coords": [
                        -86.746425,
                        21.258594
                  ]
            },
            {
                  "name": "Playa Norte",
                  "text": "Zentraler Strand – beliebter öffentlicher Strand mit ruhigem Wasser und Einrichtungen.",
                  "category": "natural",
                  "coords": [
                        -86.751237,
                        21.256847
                  ]
            },
            {
                  "name": "Playa Norte",
                  "text": "Nordstrand – breiter Sandstrand mit flachem Wasser, ideal für Familien.",
                  "category": "recreational",
                  "coords": [
                        -86.750993,
                        21.260648
                  ]
            },
            {
                  "name": "Playa Media Luna",
                  "text": "Ein halbmondförmiger Strand auf Isla Mujeres, bekannt für sein ruhiges Wasser.",
                  "category": "landmark",
                  "coords": [
                        -86.747267,
                        21.259619
                  ]
            },
            {
                  "name": "Benito Juárez Garcia",
                  "text": "Ein Denkmal auf Isla Mujeres zu Ehren des mexikanischen Präsidenten Benito Juárez.",
                  "category": "historical",
                  "coords": [
                        -86.743042,
                        21.251764
                  ]
            },
            {
                  "name": "Monumento Tiburon Ballena",
                  "text": "Eine Skulptur auf Isla Mujeres, die die Walhaie feiert, die in den umliegenden Gewässern vorkommen.",
                  "category": "landmark",
                  "coords": [
                        -86.741594,
                        21.247379
                  ]
            },
            {
                  "name": "Temple of Ixchel",
                  "text": "Maya-Ruinen auf Isla Mujeres, die der Fruchtbarkeitsgöttin Ixchel gewidmet sind.",
                  "category": "historical",
                  "coords": [
                        -86.710859,
                        21.202012
                  ]
            },
            {
                  "name": "Iguana",
                  "text": "Große Leguan-Skulptur, ein Wahrzeichen der Insel.",
                  "category": "landmark",
                  "coords": [
                        -86.713077,
                        21.204085
                  ]
            },
            {
                  "name": "Parque Garrafón",
                  "text": "Ein Naturpark auf Isla Mujeres mit Schnorchelmöglichkeiten und Klippenspringen.",
                  "category": "landmark",
                  "coords": [
                        -86.714469,
                        21.204064
                  ]
            },
            {
                  "name": "Snorkelling",
                  "text": "Schnorchelplatz am Korallenriff mit vielen tropischen Fischen.",
                  "category": "landmark",
                  "coords": [
                        -86.717591,
                        21.205636
                  ]
            },
            {
                  "name": "Zona Sujeta a Conservación Ecológica Refugio Estatal de Flora y Fauna Sistema Lagunar Chacmochuch",
                  "text": "Ein geschütztes ökologisches Reservat auf Isla Mujeres mit vielfältiger Flora und Fauna.",
                  "category": "natural",
                  "coords": [
                        -86.826245,
                        21.231106
                  ]
            },
            {
                  "name": "Garrafón de Castilla Beach Park",
                  "text": "Ein Strandclub auf Isla Mujeres mit Wasseraktivitäten und Gastronomie.",
                  "category": "recreational",
                  "coords": [
                        -86.718579,
                        21.206489
                  ]
            },
            {
                  "name": "Zona Sujeta a Conservación Ecológica Refugio Estatal de Flora y Fauna Laguna Manatí",
                  "text": "Ein Naturschutzgebiet auf Isla Mujeres, das das Ökosystem der Manatí-Lagune schützt.",
                  "category": "natural",
                  "coords": [
                        -86.813411,
                        21.191492
                  ]
            },
            {
                  "name": "Playa El Niño",
                  "text": "Kinderstrand – kleine geschützte Bucht mit sanften Wellen, sicher für Kinder.",
                  "category": "recreational",
                  "coords": [
                        -86.805971,
                        21.193286
                  ]
            },
            {
                  "name": "El Meco",
                  "text": "Eine Maya-archäologische Stätte an der Nordküste von Isla Mujeres.",
                  "category": "historical",
                  "coords": [
                        -86.803929,
                        21.211486
                  ]
            },
            {
                  "name": "Tern Sanctuary",
                  "text": "Ein Schutzgebiet auf Isla Mujeres für Seeschwalbenpopulationen.",
                  "category": "natural",
                  "coords": [
                        -86.748693,
                        21.264138
                  ]
            },
            {
                  "name": "Green Demon Beach Club",
                  "text": "Ein Strandclub auf Isla Mujeres mit Annehmlichkeiten und Unterhaltung.",
                  "category": "recreational",
                  "coords": [
                        -86.750178,
                        21.261778
                  ]
            },
            {
                  "name": "Parque de los Sueños",
                  "text": "Park der Träume – Meerespark mit Skulpturen, Kunstinstallationen und einem Spielplatz.",
                  "category": "recreational",
                  "coords": [
                        -86.721596,
                        21.210372
                  ]
            },
            {
                  "name": "MUSA Cancun Underwater Museum of Art",
                  "text": "Ein Unterwasser-Museum in der Nähe von Isla Mujeres mit Hunderten von versunkenen Skulpturen.",
                  "category": "museum",
                  "coords": [
                        -86.725435,
                        21.200921
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Gran Puerto",
                  "text": "Fő kompkikötő, amely Isla Mujerest Cancúnnal köti össze.",
                  "category": "landmark",
                  "coords": [
                        -86.807071,
                        21.183749
                  ]
            },
            {
                  "name": "Playa Tiburón",
                  "text": "Cápa-part – a közelben előforduló bálna- cápákról elnevezve.",
                  "category": "landmark",
                  "coords": [
                        -86.733482,
                        21.223499
                  ]
            },
            {
                  "name": "Escalera Arcoiris",
                  "text": "Szivárvány színűre festett lépcső a sziget főutcáján.",
                  "category": "landmark",
                  "coords": [
                        -86.727079,
                        21.226262
                  ]
            },
            {
                  "name": "Monumento a los Maestros",
                  "text": "Tanárok emlékműve – szobor a pedagógusok tiszteletére a főtéren.",
                  "category": "historical",
                  "coords": [
                        -86.728074,
                        21.228084
                  ]
            },
            {
                  "name": "Marina Puerto Isla Mujeres - Villa Vera",
                  "text": "Kikötő dokkokkal és hajókölcsönzéssel, a Villa Vera üdülő része.",
                  "category": "landmark",
                  "coords": [
                        -86.736879,
                        21.235227
                  ]
            },
            {
                  "name": "Varadero Puerto Isla Mujeres",
                  "text": "Hajójavító és tároló műhely a kikötő közelében.",
                  "category": "landmark",
                  "coords": [
                        -86.738175,
                        21.237181
                  ]
            },
            {
                  "name": "Dolphin Discovery",
                  "text": "Dolphin Discovery delfintalálkozásokat kínál Isla Mujeresen.",
                  "category": "landmark",
                  "coords": [
                        -86.741671,
                        21.234624
                  ]
            },
            {
                  "name": "Museo historico fotografico",
                  "text": "Történelmi fotómúzeum, amely a sziget múltját bemutató képekkel rendelkezik.",
                  "category": "museum",
                  "coords": [
                        -86.745628,
                        21.256697
                  ]
            },
            {
                  "name": "Esduma La Perla Hotel Isla Mujeres",
                  "text": "Szálloda úszómedencével és strandhozzáféréssel, 'A Gyöngy' néven.",
                  "category": "recreational",
                  "coords": [
                        -86.746425,
                        21.258594
                  ]
            },
            {
                  "name": "Playa Norte",
                  "text": "Központi strand – népszerű nyilvános part nyugodt vízzel és szolgáltatásokkal.",
                  "category": "natural",
                  "coords": [
                        -86.751237,
                        21.256847
                  ]
            },
            {
                  "name": "Playa Norte",
                  "text": "Északi strand – széles homokos part sekély vízzel, családok számára kiváló.",
                  "category": "recreational",
                  "coords": [
                        -86.750993,
                        21.260648
                  ]
            },
            {
                  "name": "Playa Media Luna",
                  "text": "Egy félhold alakú strand Isla Mujeres szigetén, nyugodt vizéről ismert.",
                  "category": "landmark",
                  "coords": [
                        -86.747267,
                        21.259619
                  ]
            },
            {
                  "name": "Benito Juárez Garcia",
                  "text": "Egy szobor Isla Mujeres szigetén, amely Benito Juárez mexikói elnököt tiszteli.",
                  "category": "historical",
                  "coords": [
                        -86.743042,
                        21.251764
                  ]
            },
            {
                  "name": "Monumento Tiburon Ballena",
                  "text": "Egy szobor Isla Mujeres szigetén, amely a környező vizekben élő cetcápákat ünnepli.",
                  "category": "landmark",
                  "coords": [
                        -86.741594,
                        21.247379
                  ]
            },
            {
                  "name": "Temple of Ixchel",
                  "text": "Maja romok Isla Mujeres szigetén, a termékenység istennőjének, Ixchelnek szentelve.",
                  "category": "historical",
                  "coords": [
                        -86.710859,
                        21.202012
                  ]
            },
            {
                  "name": "Iguana",
                  "text": "Nagy leguánszobor, a sziget egyik jelképe.",
                  "category": "landmark",
                  "coords": [
                        -86.713077,
                        21.204085
                  ]
            },
            {
                  "name": "Parque Garrafón",
                  "text": "Egy természeti park Isla Mujeres szigetén, sznorkelezési lehetőségekkel és szikláról ugrálással.",
                  "category": "landmark",
                  "coords": [
                        -86.714469,
                        21.204064
                  ]
            },
            {
                  "name": "Snorkelling",
                  "text": "Búvárkodó hely a korallzátonynál, rengeteg trópusi hallal.",
                  "category": "landmark",
                  "coords": [
                        -86.717591,
                        21.205636
                  ]
            },
            {
                  "name": "Zona Sujeta a Conservación Ecológica Refugio Estatal de Flora y Fauna Sistema Lagunar Chacmochuch",
                  "text": "Egy védett ökológiai rezervátum Isla Mujeres szigetén, változatos növény- és állatvilággal.",
                  "category": "natural",
                  "coords": [
                        -86.826245,
                        21.231106
                  ]
            },
            {
                  "name": "Garrafón de Castilla Beach Park",
                  "text": "Egy strandklub Isla Mujeres szigetén, vízi tevékenységekkel és étkezési lehetőségekkel.",
                  "category": "recreational",
                  "coords": [
                        -86.718579,
                        21.206489
                  ]
            },
            {
                  "name": "Zona Sujeta a Conservación Ecológica Refugio Estatal de Flora y Fauna Laguna Manatí",
                  "text": "Egy természetvédelmi terület Isla Mujeres szigetén, amely a Manatí-lagúna ökoszisztémáját védi.",
                  "category": "natural",
                  "coords": [
                        -86.813411,
                        21.191492
                  ]
            },
            {
                  "name": "Playa El Niño",
                  "text": "Gyermekpart – kis védett öböl enyhe hullámokkal, gyerekek számára biztonságos.",
                  "category": "recreational",
                  "coords": [
                        -86.805971,
                        21.193286
                  ]
            },
            {
                  "name": "El Meco",
                  "text": "Egy maja régészeti lelőhely Isla Mujeres északi partján.",
                  "category": "historical",
                  "coords": [
                        -86.803929,
                        21.211486
                  ]
            },
            {
                  "name": "Tern Sanctuary",
                  "text": "Egy szentély Isla Mujeres szigetén a csér madárpopulációk védelmére.",
                  "category": "natural",
                  "coords": [
                        -86.748693,
                        21.264138
                  ]
            },
            {
                  "name": "Green Demon Beach Club",
                  "text": "Egy strandklub Isla Mujeres szigetén, szolgáltatásokkal és szórakozással.",
                  "category": "recreational",
                  "coords": [
                        -86.750178,
                        21.261778
                  ]
            },
            {
                  "name": "Parque de los Sueños",
                  "text": "Álmok parkja – tengerparti park szobrokkal, művészeti installációkkal és játszótérrel.",
                  "category": "recreational",
                  "coords": [
                        -86.721596,
                        21.210372
                  ]
            },
            {
                  "name": "MUSA Cancun Underwater Museum of Art",
                  "text": "Egy víz alatti múzeum Isla Mujeres közelében, több száz elsüllyedt szoborral.",
                  "category": "museum",
                  "coords": [
                        -86.725435,
                        21.200921
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Gran Puerto",
                  "text": "Port principal de feribot care leagă Isla Mujeres de Cancun.",
                  "category": "landmark",
                  "coords": [
                        -86.807071,
                        21.183749
                  ]
            },
            {
                  "name": "Playa Tiburón",
                  "text": "Plaja Rechinilor – numită după rechinii-balenă care frecventează apele din apropiere.",
                  "category": "landmark",
                  "coords": [
                        -86.733482,
                        21.223499
                  ]
            },
            {
                  "name": "Escalera Arcoiris",
                  "text": "Scară pictată în culorile curcubeului pe strada principală a insulei.",
                  "category": "landmark",
                  "coords": [
                        -86.727079,
                        21.226262
                  ]
            },
            {
                  "name": "Monumento a los Maestros",
                  "text": "Monumentul Profesorilor – o sculptură care onorează educatorii, în piața principală.",
                  "category": "historical",
                  "coords": [
                        -86.728074,
                        21.228084
                  ]
            },
            {
                  "name": "Marina Puerto Isla Mujeres - Villa Vera",
                  "text": "Marină cu docuri și închirieri de bărci, parte a complexului Villa Vera.",
                  "category": "landmark",
                  "coords": [
                        -86.736879,
                        21.235227
                  ]
            },
            {
                  "name": "Varadero Puerto Isla Mujeres",
                  "text": "Șantier naval pentru repararea și depozitarea ambarcațiunilor lângă marină.",
                  "category": "landmark",
                  "coords": [
                        -86.738175,
                        21.237181
                  ]
            },
            {
                  "name": "Dolphin Discovery",
                  "text": "Dolphin Discovery oferă întâlniri cu delfinii în Isla Mujeres.",
                  "category": "landmark",
                  "coords": [
                        -86.741671,
                        21.234624
                  ]
            },
            {
                  "name": "Museo historico fotografico",
                  "text": "Muzeu istoric de fotografie care prezintă imagini ale trecutului insulei.",
                  "category": "museum",
                  "coords": [
                        -86.745628,
                        21.256697
                  ]
            },
            {
                  "name": "Esduma La Perla Hotel Isla Mujeres",
                  "text": "Hotel cu piscină și acces la plajă, numit 'Perla'.",
                  "category": "recreational",
                  "coords": [
                        -86.746425,
                        21.258594
                  ]
            },
            {
                  "name": "Playa Norte",
                  "text": "Plaja Centrală – plajă publică populară, cu ape liniștite și facilități.",
                  "category": "natural",
                  "coords": [
                        -86.751237,
                        21.256847
                  ]
            },
            {
                  "name": "Playa Norte",
                  "text": "Plaja de Nord – plajă largă nisipoasă cu apă mică, perfectă pentru familii.",
                  "category": "recreational",
                  "coords": [
                        -86.750993,
                        21.260648
                  ]
            },
            {
                  "name": "Playa Media Luna",
                  "text": "O plajă în formă de semilună pe Insula Isla Mujeres, cunoscută pentru apele sale calme.",
                  "category": "landmark",
                  "coords": [
                        -86.747267,
                        21.259619
                  ]
            },
            {
                  "name": "Benito Juárez Garcia",
                  "text": "Un monument pe Insula Isla Mujeres în onoarea președintelui mexican Benito Juárez.",
                  "category": "historical",
                  "coords": [
                        -86.743042,
                        21.251764
                  ]
            },
            {
                  "name": "Monumento Tiburon Ballena",
                  "text": "O sculptură pe Insula Isla Mujeres care celebrează rechinii-balenă din apele din jur.",
                  "category": "landmark",
                  "coords": [
                        -86.741594,
                        21.247379
                  ]
            },
            {
                  "name": "Temple of Ixchel",
                  "text": "Ruine mayașe pe Insula Isla Mujeres dedicate zeiței fertilității Ixchel.",
                  "category": "historical",
                  "coords": [
                        -86.710859,
                        21.202012
                  ]
            },
            {
                  "name": "Iguana",
                  "text": "Sculptură mare de iguana, simbol al insulei.",
                  "category": "landmark",
                  "coords": [
                        -86.713077,
                        21.204085
                  ]
            },
            {
                  "name": "Parque Garrafón",
                  "text": "Un parc natural pe Insula Isla Mujeres cu snorkeling și sărituri de pe stânci.",
                  "category": "landmark",
                  "coords": [
                        -86.714469,
                        21.204064
                  ]
            },
            {
                  "name": "Snorkelling",
                  "text": "Loc de snorkeling lângă reciful de corali, abundent în pești tropicali.",
                  "category": "landmark",
                  "coords": [
                        -86.717591,
                        21.205636
                  ]
            },
            {
                  "name": "Zona Sujeta a Conservación Ecológica Refugio Estatal de Flora y Fauna Sistema Lagunar Chacmochuch",
                  "text": "O rezervație ecologică protejată pe Insula Isla Mujeres, cu floră și faună diversă.",
                  "category": "natural",
                  "coords": [
                        -86.826245,
                        21.231106
                  ]
            },
            {
                  "name": "Garrafón de Castilla Beach Park",
                  "text": "Un club de plajă pe Insula Isla Mujeres, cu activități acvatice și restaurante.",
                  "category": "recreational",
                  "coords": [
                        -86.718579,
                        21.206489
                  ]
            },
            {
                  "name": "Zona Sujeta a Conservación Ecológica Refugio Estatal de Flora y Fauna Laguna Manatí",
                  "text": "O rezervație naturală pe Insula Isla Mujeres care protejează ecosistemul lagunei Manatí.",
                  "category": "natural",
                  "coords": [
                        -86.813411,
                        21.191492
                  ]
            },
            {
                  "name": "Playa El Niño",
                  "text": "Plaja Copiilor – mic golf adăpostit cu valuri line, sigur pentru copii.",
                  "category": "recreational",
                  "coords": [
                        -86.805971,
                        21.193286
                  ]
            },
            {
                  "name": "El Meco",
                  "text": "Un sit arheologic mayaș pe coasta de nord a Insulei Isla Mujeres.",
                  "category": "historical",
                  "coords": [
                        -86.803929,
                        21.211486
                  ]
            },
            {
                  "name": "Tern Sanctuary",
                  "text": "Un sanctuar pe Insula Isla Mujeres pentru protejarea populațiilor de păsări tern.",
                  "category": "natural",
                  "coords": [
                        -86.748693,
                        21.264138
                  ]
            },
            {
                  "name": "Green Demon Beach Club",
                  "text": "Un club de plajă pe Insula Isla Mujeres cu facilități și divertisment.",
                  "category": "recreational",
                  "coords": [
                        -86.750178,
                        21.261778
                  ]
            },
            {
                  "name": "Parque de los Sueños",
                  "text": "Parcul Visurilor – parc pe malul mării cu sculpturi, instalații de artă și un loc de joacă.",
                  "category": "recreational",
                  "coords": [
                        -86.721596,
                        21.210372
                  ]
            },
            {
                  "name": "MUSA Cancun Underwater Museum of Art",
                  "text": "Un muzeu subacvatic lângă Insula Isla Mujeres, cu sute de sculpturi scufundate.",
                  "category": "museum",
                  "coords": [
                        -86.725435,
                        21.200921
                  ]
            }
      ],
      "en": [
            {
                  "name": "Gran Puerto",
                  "text": "Main ferry port connecting Isla Mujeres to Cancun.",
                  "category": "landmark",
                  "coords": [
                        -86.807071,
                        21.183749
                  ]
            },
            {
                  "name": "Playa Tiburón",
                  "text": "Shark Beach – named after the whale sharks that frequent nearby waters.",
                  "category": "landmark",
                  "coords": [
                        -86.733482,
                        21.223499
                  ]
            },
            {
                  "name": "Rainbow Steps",
                  "text": "Painted rainbow-colored staircase on the island's main street.",
                  "category": "landmark",
                  "coords": [
                        -86.727079,
                        21.226262
                  ]
            },
            {
                  "name": "Monumento a los Maestros",
                  "text": "Monument to Teachers – a sculpture honoring educators in the town square.",
                  "category": "historical",
                  "coords": [
                        -86.728074,
                        21.228084
                  ]
            },
            {
                  "name": "Marina Puerto Isla Mujeres - Villa Vera",
                  "text": "Marina with docks and boat rentals, part of the Villa Vera resort.",
                  "category": "landmark",
                  "coords": [
                        -86.736879,
                        21.235227
                  ]
            },
            {
                  "name": "Shipyard Puerto Isla Mujeres",
                  "text": "Shipyard for boat repairs and storage near the marina.",
                  "category": "landmark",
                  "coords": [
                        -86.738175,
                        21.237181
                  ]
            },
            {
                  "name": "Dolphin Discovery",
                  "text": "Dolphin Discovery offers dolphin encounters in Isla Mujeres.",
                  "category": "landmark",
                  "coords": [
                        -86.741671,
                        21.234624
                  ]
            },
            {
                  "name": "Museo historico fotografico",
                  "text": "Historical Photography Museum displaying images of the island's past.",
                  "category": "museum",
                  "coords": [
                        -86.745628,
                        21.256697
                  ]
            },
            {
                  "name": "Esduma La Perla Hotel Isla Mujeres",
                  "text": "Hotel with a swimming pool and beach access, named 'The Pearl'.",
                  "category": "recreational",
                  "coords": [
                        -86.746425,
                        21.258594
                  ]
            },
            {
                  "name": "Playa Centro",
                  "text": "Central Beach – a popular public beach with calm waters and amenities.",
                  "category": "natural",
                  "coords": [
                        -86.751237,
                        21.256847
                  ]
            },
            {
                  "name": "North Beach",
                  "text": "North Beach – a wide sandy shore with shallow water, great for families.",
                  "category": "recreational",
                  "coords": [
                        -86.750993,
                        21.260648
                  ]
            },
            {
                  "name": "Playa Media Luna",
                  "text": "A crescent-shaped beach on Isla Mujeres known for its calm waters.",
                  "category": "landmark",
                  "coords": [
                        -86.747267,
                        21.259619
                  ]
            },
            {
                  "name": "Benito Juárez Garcia",
                  "text": "A monument on Isla Mujeres honoring the Mexican president Benito Juárez.",
                  "category": "historical",
                  "coords": [
                        -86.743042,
                        21.251764
                  ]
            },
            {
                  "name": "Whale Shark Monument",
                  "text": "A sculpture on Isla Mujeres celebrating the whale sharks that frequent the surrounding waters.",
                  "category": "landmark",
                  "coords": [
                        -86.741594,
                        21.247379
                  ]
            },
            {
                  "name": "Temple of Ixchel",
                  "text": "Mayan ruins on Isla Mujeres dedicated to the goddess of fertility Ixchel.",
                  "category": "historical",
                  "coords": [
                        -86.710859,
                        21.202012
                  ]
            },
            {
                  "name": "Iguana",
                  "text": "A large sculpture of an iguana, a symbol of the island.",
                  "category": "landmark",
                  "coords": [
                        -86.713077,
                        21.204085
                  ]
            },
            {
                  "name": "Parque Garrafón",
                  "text": "A natural park on Isla Mujeres with snorkeling and cliff jumping.",
                  "category": "landmark",
                  "coords": [
                        -86.714469,
                        21.204064
                  ]
            },
            {
                  "name": "Snorkelling",
                  "text": "Snorkeling spot near the coral reef, abundant with tropical fish.",
                  "category": "landmark",
                  "coords": [
                        -86.717591,
                        21.205636
                  ]
            },
            {
                  "name": "Zona Sujeta a Conservación Ecológica Refugio Estatal de Flora y Fauna Sistema Lagunar Chacmochuch",
                  "text": "A protected ecological reserve on Isla Mujeres with diverse flora and fauna.",
                  "category": "natural",
                  "coords": [
                        -86.826245,
                        21.231106
                  ]
            },
            {
                  "name": "Garrafón de Castilla Beach Park",
                  "text": "A beach club on Isla Mujeres offering water activities and dining.",
                  "category": "recreational",
                  "coords": [
                        -86.718579,
                        21.206489
                  ]
            },
            {
                  "name": "Zona Sujeta a Conservación Ecológica Refugio Estatal de Flora y Fauna Laguna Manatí",
                  "text": "A nature reserve on Isla Mujeres protecting the Manatí lagoon ecosystem.",
                  "category": "natural",
                  "coords": [
                        -86.813411,
                        21.191492
                  ]
            },
            {
                  "name": "Playa El Niño",
                  "text": "Child Beach – a small sheltered cove with gentle waves, safe for children.",
                  "category": "recreational",
                  "coords": [
                        -86.805971,
                        21.193286
                  ]
            },
            {
                  "name": "El Meco",
                  "text": "A Mayan archaeological site on the northern coast of Isla Mujeres.",
                  "category": "historical",
                  "coords": [
                        -86.803929,
                        21.211486
                  ]
            },
            {
                  "name": "Tern Sanctuary",
                  "text": "A sanctuary on Isla Mujeres protecting tern bird populations.",
                  "category": "natural",
                  "coords": [
                        -86.748693,
                        21.264138
                  ]
            },
            {
                  "name": "Green Demon Beach Club",
                  "text": "A beach club on Isla Mujeres with amenities and entertainment.",
                  "category": "recreational",
                  "coords": [
                        -86.750178,
                        21.261778
                  ]
            },
            {
                  "name": "Parque de los Sueños",
                  "text": "Park of Dreams – a seaside park with sculptures, art installations, and a playground.",
                  "category": "recreational",
                  "coords": [
                        -86.721596,
                        21.210372
                  ]
            },
            {
                  "name": "MUSA Cancun Underwater Museum of Art",
                  "text": "An underwater museum near Isla Mujeres with hundreds of submerged sculptures.",
                  "category": "museum",
                  "coords": [
                        -86.725435,
                        21.200921
                  ]
            }
      ]
},
    type: "island",
    parent: "MX-ROO",
    coords: [-86.73, 21.23],
    name: { de: "Isla Mujeres", hu: "Isla Mujeres", ro: "Isla Mujeres", en: "Isla Mujeres"},
    image: "/poi-images/isla-mujeres-life-v2.webp",
    descriptionAdvanced: {
      de: "Isla Mujeres, die 'Insel der Frauen', liegt nur eine kurze Fährfahrt von Cancún entfernt und bietet eine völlig andere, ruhigere Welt. Bekannt für die Playa Norte, einen der schönsten Strände der Welt, besticht die Insel durch kristallklares Wasser und feinen weißen Sand. Am südlichen Ende der Insel, Punta Sur, befinden sich die Überreste eines antiken Maya-Tempels, der der Göttin Ixchel gewidmet war. Die Insel ist ein Paradies für Taucher und Schnorchler, besonders im Garrafón-Park oder beim Besuch des Unterwassermuseums MUSA. Trotz ihrer Beliebtheit hat sich die Insel einen gewissen Kleinstadtcharme mit bunten Häusern und freundlichen Bewohnern bewahrt. Isla Mujeres ist der Ort, an dem die ersten Sonnenstrahlen Mexikos das Land berühren. Geographie K7 - Küstenmorphologie und Inselkultur.",
      hu: "Isla Mujeres egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Isla Mujeres kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Isla Mujeres este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Isla Mujeres este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Isla Mujeres, or the 'Island of Women,' is a charming Caribbean gem located just a short ferry ride from the bustling shores of Cancún. Despite its proximity to the mainland, the island maintains a distinctly laid-back and authentic atmosphere. It is world-renowned for Playa Norte, often cited as one of the most beautiful beaches in the world, with its calm, waist-deep crystal waters and swaying palms. At the island's southern tip, Punta Sur, visitors can find the ruins of a temple dedicated to the Maya goddess Ixchel and a spectacular sculpture park overlooking the sea. The surrounding coral reefs, including the Manchones Reef, are a paradise for snorkelers and divers, featuring a portion of the famous MUSA (Underwater Museum of Art). Whether you're cruising around in a golf cart, exploring the colorful town center, or simply relaxing by the sea, Isla Mujeres offers a perfect blend of natural beauty and Mexican charm.",
      es: "Isla Mujeres, situada a un corto viaje en ferry desde Cancún, ofrece un mundo completamente diferente y más tranquilo. Conocida por Playa Norte, una de las playas más hermosas del mundo, la isla impresiona por sus aguas cristalinas y su fina arena blanca. En el extremo sur de la isla, Punta Sur, se encuentran los restos de un antiguo templo maya dedicado a la diosa Ixchel. La isla es un paraíso para buceadores y aficionados al esnórquel, especialmente en el Parque Garrafón o visitando el museo submarino MUSA. A pesar de su popularidad, la isla ha conservado cierto encanto de pueblo pequeño con casas coloridas y habitantes amigables. Isla Mujeres es el lugar donde los primeros rayos de sol de México tocan la tierra.",
      pt: "Isla Mujeres, localizada a uma curta viagem de balsa de Cancún, oferece um mundo completamente diferente e mais tranquilo. Conhecida pela Playa Norte, uma das praias mais bonitas do mundo, a ilha impressiona pelas suas águas cristalinas e areia branca fina. Na extremidade sul da ilha, em Punta Sur, encontram-se os restos de um antigo templo maia dedicado à deusa Ixchel. A ilha é um paraíso para mergulhadores e praticantes de snorkel, especialmente no Parque Garrafón ou ao visitar o museu subaquático MUSA. Apesar da sua popularidade, a ilha manteve um certo charme de cidade pequena, com casas coloridas e moradores amigáveis. Isla Mujeres é o lugar onde os primeiros raios de sol do México tocam a terra.",
      fr: "Isla Mujeres, l'île des femmes, se trouve à une courte traversée en ferry de Cancún et offre un monde totalement différent et plus calme. Connue pour Playa Norte, l'une des plus belles plages du monde, l'île séduit par ses eaux cristallines et son sable blanc et fin. À l'extrémité sud de l'île, à Punta Sur, se trouvent les vestiges d'un ancien temple maya dédié à la déesse Ixchel. L'île est un paradis pour les plongeurs, notamment au parc Garrafón ou lors d'une visite au musée sous-marin MUSA. Malgré sa popularité, l'île a conservé un certain charme de petite ville avec ses maisons colorées et ses habitants accueillants. Isla Mujeres est l'endroit où les premiers rayons de soleil du Mexique touchent le pays.",
    },
    factsAdvanced: {
      de: ["Die Insel ist nur etwa 7 Kilometer lang und 650 Meter breit.", "An der Punta Sur steht die erste Skulptur, die morgens vom Sonnenlicht getroffen wird.", "Ixchel war die Maya-Göttin der Fruchtbarkeit und der Heilung.", "Das MUSA-Unterwassermuseum hilft bei der Regeneration der natürlichen Korallenriffe.", "Die Gewässer um die Insel sind berühmt für die Sichtung von Segelfischen im Winter.", "Playa Norte zeichnet sich durch extrem flaches Wasser und minimale Wellen aus.", "In der Nähe befindet sich das 'Schildkrötenhaus' (Tortugranja) für den Artenschutz."],
      hu: ["Isla Mujeres sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Isla Mujeres egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Isla Mujeres ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Isla Mujeres este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The island was named by Spanish conquistadors who found many statues of the Maya goddess Ixchel there.",
        "Isla Mujeres is only about 7 kilometers long and 650 meters wide at its widest point.",
        "The eastern side of the island is rugged and rocky, while the western side features calm, sandy beaches.",
        "Punta Sur is the easternmost point of Mexico, where the first rays of the sun hit the country each morning.",
        "The MUSA underwater museum near the island features over 500 life-sized sculptures designed to promote reef growth."
      ],
      es: ["La isla tiene solo unos 7 kilómetros de largo y 650 metros de ancho.", "En Punta Sur se encuentra la primera escultura que recibe la luz del sol por la mañana.", "Ixchel era la diosa maya de la fertilidad y la medicina.", "El museo submarino MUSA ayuda a la regeneración de los arrecifes de coral naturales.", "Las aguas alrededor de la isla son famosas por el avistamiento de pez vela en invierno.", "Playa Norte se caracteriza por aguas extremadamente poco profundas y olas mínimas.", "Cerca se encuentra la 'Tortugranja' para la protección de las tortugas marinas."],
      pt: ["A ilha tem apenas cerca de 7 quilômetros de comprimento e 650 metros de largura.", "Na Punta Sur encontra-se a primeira escultura a ser atingida pela luz do sol de manhã.", "Ixchel era a deusa maia da fertilidade e da cura.", "O museu subaquático MUSA ajuda na regeneração dos recifes de coral naturais.", "As águas ao redor da ilha são famosas pelo avistamento de peixes-vela no inverno.", "Playa Norte caracteriza-se por águas extremamente rasas e ondas mínimas.", "Nas proximidades encontra-se a 'Tortugranja' para a proteção de espécies de tartarugas."],
      fr: ["L'île ne mesure que 7 kilomètres de long et 650 mètres de large.", "À Punta Sur se trouve la première sculpture touchée par la lumière du soleil le matin.", "Ixchel était la déesse maya de la fertilité et de la guérison.", "Le musée sous-marin MUSA aide à la régénération des récifs coralliens naturels.", "Les eaux autour de l'île sont célèbres pour l'observation des voiliers en hiver.", "Playa Norte se caractérise par une eau extrêmement peu profonde et des vagues minimales.", "À proximité se trouve la 'Tortugranja' pour la protection des espèces."],
    }, image: "/poi-images/isla-mujeres-life-v2.webp",
    description: {
      de: "Eine malerische Insel vor Cancún, die für ihre Traumstrände und das kristallklare Wasser geschätzt wird.",
      hu: "Cancún közelében fekvő festői sziget, amely kristálytiszta vizéről és strandjairól híres.",
      ro: "O insulă pitorească din Caraibe lângă Cancún, faimoasă pentru apele calme ale Playa Norte și farmecul său boem.",
      en: "A picturesque Caribbean island near Cancún, famous for the calm, turquoise waters of Playa Norte and its laid-back charm.",
      es: "Una pintoresca isla frente a Cancún, apreciada por sus playas de ensueño y aguas cristalinas.",
      pt: "Uma ilha pitoresca em frente a Cancún, valorizada pelas suas praias paradisíacas e águas cristalinas.",
      fr: "Une île pittoresque au large de Cancún, appréciée pour ses plages de rêve et ses eaux cristallines.",
    },
    facts: {
      de: ["Tempel der Göttin Ixchel", "Bekannter Strand Playa Norte", "Ideal zum Schnorcheln"],
      hu: ["7 kilométer hosszú sziget", "Víz alatti művészeti múzeum", "Ixchel maja istennő szentélye"],
      ro: ["Punctul cel mai estic al Mexicului", "Templul zeiței Ixchel la Punta Sur", "Include un muzeu de artă subacvatic"],
      en: ["Temple of goddess Ixchel at Punta Sur", "Only 7 kilometers long", "Features an underwater art museum"],
      es: ["Templo de la diosa Ixchel", "Famosa playa Playa Norte", "Ideal para el esnórquel"],
      pt: ["Templo da deusa Ixchel", "Praia famosa Playa Norte", "Ideal para snorkel"],
      fr: ["Temple de la déesse Ixchel", "Plage célèbre de Playa Norte", "Idéal pour le snorkeling"],
    }
  },
  {
    id: "janitzio-island-life-v2", "sights": {
      "de": [
            {
                  "name": "Cementerio",
                  "text": "Friedhof auf der Insel mit bunten Gräbern mit Blick auf das Meer.",
                  "category": "landmark",
                  "coords": [
                        -101.696852,
                        19.555959
                  ]
            },
            {
                  "name": "Monumento José María Morelos Pavón",
                  "text": "Eine große Statue von José María Morelos, einem prominenten mexikanischen Unabhängigkeitsführer, auf der Insel Janitzio.",
                  "category": "historical",
                  "coords": [
                        -101.651943,
                        19.574242
                  ]
            },
            {
                  "name": "Museo de las Artes e Industrias Populares",
                  "text": "Ein Museum für Volkskunst und -industrie auf der Insel Janitzio.",
                  "category": "museum",
                  "coords": [
                        -101.606954,
                        19.51369
                  ]
            },
            {
                  "name": "salon Robal",
                  "text": "Salon Robal ist ein markantes Gebäude im Stadtzentrum von Island mit traditioneller Fassade.",
                  "category": "landmark",
                  "coords": [
                        -101.604346,
                        19.522178
                  ]
            },
            {
                  "name": "Centro de Interpretación Ruta Don Vasco",
                  "text": "Ein Interpretationszentrum zur Route Don Vasco auf der Insel Janitzio.",
                  "category": "museum",
                  "coords": [
                        -101.615643,
                        19.536345
                  ]
            },
            {
                  "name": "Zona Arqueológica Ihuatzio",
                  "text": "Eine prähispanische Purépecha-archäologische Stätte nahe der Insel Janitzio.",
                  "category": "landmark",
                  "coords": [
                        -101.621159,
                        19.577989
                  ]
            },
            {
                  "name": "Área Natural Protegida Cerro del Estribo Grande",
                  "text": "Das Schutzgebiet Cerro del Estribo Grande ist ein Hügel, dessen Form an einen Steigbügel erinnert.",
                  "category": "natural",
                  "coords": [
                        -101.642995,
                        19.510939
                  ]
            },
            {
                  "name": "Humedales del lago de Pátzcuaro",
                  "text": "Die Feuchtgebiete des Pátzcuaro-Sees sind ein wichtiger Lebensraum für Zugvögel.",
                  "category": "natural",
                  "coords": [
                        -101.675191,
                        19.560234
                  ]
            },
            {
                  "name": "Casa de los once patios",
                  "text": "Eine Galerie im Haus der elf Innenhöfe auf der Insel Janitzio.",
                  "category": "museum",
                  "coords": [
                        -101.60816,
                        19.511793
                  ]
            },
            {
                  "name": "Basilica de Nuestra Señora de la Salud",
                  "text": "Eine Basilika, die der Jungfrau der Gesundheit geweiht ist, auf der Insel Janitzio.",
                  "category": "religious",
                  "coords": [
                        -101.606012,
                        19.514999
                  ]
            },
            {
                  "name": "Teatro Emperador Caltzontzin",
                  "text": "Ein nach Kaiser Caltzontzin benanntes Theater auf der Insel Janitzio.",
                  "category": "cultural",
                  "coords": [
                        -101.608569,
                        19.516864
                  ]
            },
            {
                  "name": "Posada de Don Vasco",
                  "text": "Die Posada de Don Vasco ist eine historische Herberge, benannt nach dem Bischof Don Vasco de Quiroga.",
                  "category": "landmark",
                  "coords": [
                        -101.611855,
                        19.532
                  ]
            },
            {
                  "name": "Zona de Restauración Ambiental Estribito del Cerro Colorado",
                  "text": "Ein Umweltrestaurierungsgebiet am Estribito del Cerro Colorado nahe Janitzio.",
                  "category": "natural",
                  "coords": [
                        -101.609022,
                        19.53796
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Cementerio",
                  "text": "Temető a szigeten színes sírokkal, tengerre néző kilátással.",
                  "category": "landmark",
                  "coords": [
                        -101.696852,
                        19.555959
                  ]
            },
            {
                  "name": "Monumento José María Morelos Pavón",
                  "text": "José María Morelos, a mexikói függetlenségi harcos nagy szobra Janitzio szigetén.",
                  "category": "historical",
                  "coords": [
                        -101.651943,
                        19.574242
                  ]
            },
            {
                  "name": "Museo de las Artes e Industrias Populares",
                  "text": "Népi művészetek és iparok múzeuma Janitzio szigetén.",
                  "category": "museum",
                  "coords": [
                        -101.606954,
                        19.51369
                  ]
            },
            {
                  "name": "salon Robal",
                  "text": "A Salon Robal egy jellegzetes épület Island városközpontjában, hagyományos homlokzattal.",
                  "category": "landmark",
                  "coords": [
                        -101.604346,
                        19.522178
                  ]
            },
            {
                  "name": "Centro de Interpretación Ruta Don Vasco",
                  "text": "Egy interpretációs központ a Don Vasco útvonalról Janitzio szigetén.",
                  "category": "museum",
                  "coords": [
                        -101.615643,
                        19.536345
                  ]
            },
            {
                  "name": "Zona Arqueológica Ihuatzio",
                  "text": "Egy prehispanikus Purépecha régészeti lelőhely Janitzio szigete közelében.",
                  "category": "landmark",
                  "coords": [
                        -101.621159,
                        19.577989
                  ]
            },
            {
                  "name": "Área Natural Protegida Cerro del Estribo Grande",
                  "text": "A Cerro del Estribo Grande védett terület egy domb, amelynek alakja egy kengyelre emlékeztet.",
                  "category": "natural",
                  "coords": [
                        -101.642995,
                        19.510939
                  ]
            },
            {
                  "name": "Humedales del lago de Pátzcuaro",
                  "text": "A Pátzcuaro-tó mocsarai fontos élőhelyet biztosítanak a vándormadaraknak.",
                  "category": "natural",
                  "coords": [
                        -101.675191,
                        19.560234
                  ]
            },
            {
                  "name": "Casa de los once patios",
                  "text": "Egy galéria a Tizenegy Udvar Házában Janitzio szigetén.",
                  "category": "museum",
                  "coords": [
                        -101.60816,
                        19.511793
                  ]
            },
            {
                  "name": "Basilica de Nuestra Señora de la Salud",
                  "text": "Egy bazilika, amelyet az Egészség Szűzanyjának szenteltek, Janitzio szigetén.",
                  "category": "religious",
                  "coords": [
                        -101.606012,
                        19.514999
                  ]
            },
            {
                  "name": "Teatro Emperador Caltzontzin",
                  "text": "Egy Caltzontzin császárról elnevezett színház Janitzio szigetén.",
                  "category": "cultural",
                  "coords": [
                        -101.608569,
                        19.516864
                  ]
            },
            {
                  "name": "Posada de Don Vasco",
                  "text": "A Posada de Don Vasco egy történelmi fogadó, amely Don Vasco de Quiroga püspökről kapta a nevét.",
                  "category": "landmark",
                  "coords": [
                        -101.611855,
                        19.532
                  ]
            },
            {
                  "name": "Zona de Restauración Ambiental Estribito del Cerro Colorado",
                  "text": "Egy környezeti helyreállítási terület az Estribito del Cerro Colorado-n Janitzio közelében.",
                  "category": "natural",
                  "coords": [
                        -101.609022,
                        19.53796
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Cementerio",
                  "text": "Cimitir pe insulă, cu morminte viu colorate cu vedere la mare.",
                  "category": "landmark",
                  "coords": [
                        -101.696852,
                        19.555959
                  ]
            },
            {
                  "name": "Monumento José María Morelos Pavón",
                  "text": "O statuie mare a lui José María Morelos, un important lider al independenței mexicane, pe insula Janitzio.",
                  "category": "historical",
                  "coords": [
                        -101.651943,
                        19.574242
                  ]
            },
            {
                  "name": "Museo de las Artes e Industrias Populares",
                  "text": "Un muzeu al artelor și industriilor populare pe insula Janitzio.",
                  "category": "museum",
                  "coords": [
                        -101.606954,
                        19.51369
                  ]
            },
            {
                  "name": "salon Robal",
                  "text": "Salon Robal este o clădire emblematică în centrul orașului Island, cu o fațadă tradițională.",
                  "category": "landmark",
                  "coords": [
                        -101.604346,
                        19.522178
                  ]
            },
            {
                  "name": "Centro de Interpretación Ruta Don Vasco",
                  "text": "Un centru de interpretare a Rutei Don Vasco pe insula Janitzio.",
                  "category": "museum",
                  "coords": [
                        -101.615643,
                        19.536345
                  ]
            },
            {
                  "name": "Zona Arqueológica Ihuatzio",
                  "text": "Un sit arheologic prehispanic Purépecha lângă insula Janitzio.",
                  "category": "landmark",
                  "coords": [
                        -101.621159,
                        19.577989
                  ]
            },
            {
                  "name": "Área Natural Protegida Cerro del Estribo Grande",
                  "text": "Ária naturală protejată Cerro del Estribo Grande este un deal în formă de scară de șa.",
                  "category": "natural",
                  "coords": [
                        -101.642995,
                        19.510939
                  ]
            },
            {
                  "name": "Humedales del lago de Pátzcuaro",
                  "text": "Zonele umede ale Lacului Pátzcuaro sunt un habitat important pentru păsările migratoare.",
                  "category": "natural",
                  "coords": [
                        -101.675191,
                        19.560234
                  ]
            },
            {
                  "name": "Casa de los once patios",
                  "text": "O galerie în Casa celor unsprezece curți pe insula Janitzio.",
                  "category": "museum",
                  "coords": [
                        -101.60816,
                        19.511793
                  ]
            },
            {
                  "name": "Basilica de Nuestra Señora de la Salud",
                  "text": "O bazilică dedicată Maicii Domnului a Sănătății pe insula Janitzio.",
                  "category": "religious",
                  "coords": [
                        -101.606012,
                        19.514999
                  ]
            },
            {
                  "name": "Teatro Emperador Caltzontzin",
                  "text": "Un teatru numit după Împăratul Caltzontzin pe insula Janitzio.",
                  "category": "cultural",
                  "coords": [
                        -101.608569,
                        19.516864
                  ]
            },
            {
                  "name": "Posada de Don Vasco",
                  "text": "Posada de Don Vasco este un han istoric numit după episcopul Don Vasco de Quiroga.",
                  "category": "landmark",
                  "coords": [
                        -101.611855,
                        19.532
                  ]
            },
            {
                  "name": "Zona de Restauración Ambiental Estribito del Cerro Colorado",
                  "text": "O zonă de restaurare ecologică pe Estribito del Cerro Colorado lângă Janitzio.",
                  "category": "natural",
                  "coords": [
                        -101.609022,
                        19.53796
                  ]
            }
      ],
      "en": [
            {
                  "name": "Cementery",
                  "text": "Cemetery on the island, with brightly colored tombs overlooking the sea.",
                  "category": "landmark",
                  "coords": [
                        -101.696852,
                        19.555959
                  ]
            },
            {
                  "name": "Monumento José María Morelos Pavón",
                  "text": "A large statue of José María Morelos, a prominent Mexican independence leader, on Janitzio Island.",
                  "category": "historical",
                  "coords": [
                        -101.651943,
                        19.574242
                  ]
            },
            {
                  "name": "Museo de las Artes e Industrias Populares",
                  "text": "A museum of popular arts and industries on Janitzio Island.",
                  "category": "museum",
                  "coords": [
                        -101.606954,
                        19.51369
                  ]
            },
            {
                  "name": "salon Robal",
                  "text": "Salon Robal is a landmark building in the center of Island with a traditional façade.",
                  "category": "landmark",
                  "coords": [
                        -101.604346,
                        19.522178
                  ]
            },
            {
                  "name": "Centro de Interpretación Ruta Don Vasco",
                  "text": "An interpretation center about the Don Vasco Route on Janitzio Island.",
                  "category": "museum",
                  "coords": [
                        -101.615643,
                        19.536345
                  ]
            },
            {
                  "name": "Zona Arqueológica Ihuatzio",
                  "text": "A pre-Hispanic Purépecha archaeological site near Janitzio Island.",
                  "category": "landmark",
                  "coords": [
                        -101.621159,
                        19.577989
                  ]
            },
            {
                  "name": "Área Natural Protegida Cerro del Estribo Grande",
                  "text": "The Cerro del Estribo Grande protected area is a hill shaped like a stirrup.",
                  "category": "natural",
                  "coords": [
                        -101.642995,
                        19.510939
                  ]
            },
            {
                  "name": "Humedales del lago de Pátzcuaro",
                  "text": "The wetlands of Lake Pátzcuaro are a key habitat for migratory birds.",
                  "category": "natural",
                  "coords": [
                        -101.675191,
                        19.560234
                  ]
            },
            {
                  "name": "Casa de los once patios",
                  "text": "A gallery in the House of the Eleven Patios on Janitzio Island.",
                  "category": "museum",
                  "coords": [
                        -101.60816,
                        19.511793
                  ]
            },
            {
                  "name": "Basilica de Nuestra Señora de la Salud",
                  "text": "A basilica dedicated to Our Lady of Health on Janitzio Island.",
                  "category": "religious",
                  "coords": [
                        -101.606012,
                        19.514999
                  ]
            },
            {
                  "name": "Teatro Emperador Caltzontzin",
                  "text": "A theatre named after Emperor Caltzontzin on Janitzio Island.",
                  "category": "cultural",
                  "coords": [
                        -101.608569,
                        19.516864
                  ]
            },
            {
                  "name": "Posada de Don Vasco",
                  "text": "Posada de Don Vasco is a historic inn named after Bishop Don Vasco de Quiroga.",
                  "category": "landmark",
                  "coords": [
                        -101.611855,
                        19.532
                  ]
            },
            {
                  "name": "Zona de Restauración Ambiental Estribito del Cerro Colorado",
                  "text": "An environmental restoration area on Estribito del Cerro Colorado near Janitzio.",
                  "category": "natural",
                  "coords": [
                        -101.609022,
                        19.53796
                  ]
            }
      ]
},
    type: "island",
    parent: "MX-MIC",
    coords: [-101.69, 19.55],
    name: { de: "Janitzio", hu: "Janitzio", ro: "Janitzio", en: "Janitzio"},
    image: "/poi-images/janitzio-island-life-v2.webp",
    descriptionAdvanced: {
      de: "Janitzio ist die bekannteste der fünf Inseln im Pátzcuaro-See im Bundesstaat Michoacán und ein Zentrum der Purépecha-Kultur. Schon von weitem ist die monumentale Statue von José María Morelos sichtbar, die den höchsten Punkt der Insel krönt. Besonders berühmt ist Janitzio für seine Feierlichkeiten zum Tag der Toten (Día de Muertos), wenn der See von den Kerzen der Fischer in ihren traditionellen Booten erleuchtet wird. Die engen, steilen Gassen der Insel sind gesäumt von Kunsthandwerksläden und Restaurants, die lokale Spezialitäten wie 'Pescado Blanco' anbieten. Die Fischer mit ihren charakteristischen Schmetterlingsnetzen sind ein ikonisches Symbol für die Identität dieser Region. Janitzio bietet eine tiefgehende Erfahrung der lebendigen Traditionen und des spirituellen Erbes Mexikos. Geschichte K7 - Indigene Traditionen und Unabhängigkeitskampf.",
      hu: "Janitzio egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Janitzio kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Janitzio este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Janitzio este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Janitzio is the most iconic of the five islands in Lake Pátzcuaro, in the state of Michoacán, and is a central symbol of Purépecha culture. The island is famous for its unique butterfly net fishermen, who perform a traditional dance with their large, wing-like nets that have become a national symbol of Mexico. Rising high above the island's red-tiled roofs is a massive 40-meter statue of José María Morelos, a hero of Mexican Independence; visitors can climb inside the statue to see murals depicting his life and enjoy panoramic views of the lake. Janitzio is especially vibrant during the Day of the Dead (Noche de Muertos), when thousands of candles illuminate the cemetery and the lake, reflecting a deep spiritual connection to the ancestors. The island's steep, narrow streets are lined with shops selling local crafts and delicious regional food, such as 'charales' (dried fish). A visit to Janitzio is an immersion into the living traditions and resilient spirit of the Purépecha people.",
      es: "Janitzio es la más conocida de las cinco islas del lago de Pátzcuaro en el estado de Michoacán y un centro de la cultura purépecha. Desde lejos se ve la estatua monumental de José María Morelos que corona el punto más alto de la isla. Janitzio es especialmente famosa por sus celebraciones del Día de Muertos, cuando el lago se ilumina con las velas de los pescadores en sus botes tradicionales. Las calles estrechas y empinadas de la isla están llenas de tiendas de artesanías y restaurantes que ofrecen especialidades locales como el 'pescado blanco'. Los pescadores con sus características redes de mariposa son un símbolo icónico de la identidad de esta región. Janitzio ofrece una experiencia profunda de las tradiciones vivas y el legado espiritual de México.",
      pt: "Janitzio é a mais conhecida das cinco ilhas do Lago Pátzcuaro, no estado de Michoacán, e um centro da cultura Purépecha. De longe, é visível a estátua monumental de José María Morelos, que coroa o ponto mais alto da ilha. Janitzio é especialmente famosa pelas suas celebrações do Dia dos Mortos (Día de Muertos), quando o lago é iluminado pelas velas dos pescadores em seus barcos tradicionais. As ruas estreitas e íngremes da ilha estão repletas de lojas de artesanato e restaurantes que oferecem especialidades locais, como o 'Pescado Blanco'. Os pescadores com as suas características redes de borboleta são um símbolo icônico da identidade desta região. Janitzio oferece uma experiência profunda das tradições vivas e do legado espiritual do México.",
      fr: "Janitzio est la plus connue des cinq îles du lac Pátzcuaro, dans l'État de Michoacán, et un centre de la culture Purépecha. De loin, on aperçoit la statue monumentale de José María Morelos qui couronne le point culminant de l'île. Janitzio est particulièrement célèbre pour ses célébrations du Jour des Morts (Día de Muertos), lorsque le lac est illuminé par les bougies des pêcheurs dans leurs barques traditionnelles. Les ruelles étroites et escarpées de l'île sont bordées de boutiques d'artisanat et de restaurants proposant des spécialités locales comme le 'Pescado Blanco'. Les pêcheurs avec leurs filets papillons caractéristiques sont un symbole emblématique de l'identité de cette région. Janitzio offre une expérience profonde des traditions vivantes et de l'héritage spirituel du Mexique.",
    },
    factsAdvanced: {
      de: ["Die Statue von Morelos ist 40 Meter hoch und im Inneren begehbar.", "Die Wandgemälde im Inneren der Statue zeigen das Leben des Nationalhelden.", "Die Schmetterlingsnetze der Fischer werden heute meist für Zeremonien genutzt.", "Die Purépecha-Sprache wird auf der Insel noch aktiv gesprochen.", "Día de Muertos in Janitzio gilt als einer der authentischsten in ganz Mexiko.", "Die Insel ist nur mit kleinen Motorbooten vom Festland aus erreichbar.", "Der Pátzcuaro-See liegt auf einer beeindruckenden Höhe von 2.140 Metern."],
      hu: ["Janitzio sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Janitzio egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Janitzio ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Janitzio este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The island can only be reached by boat from the docks in the town of Pátzcuaro.",
        "The statue of Morelos on the island's summit is taller than the Christ the Redeemer in Rio de Janeiro.",
        "Lake Pátzcuaro is located at an elevation of over 2,100 meters above sea level.",
        "The butterfly nets (mariposas) were traditionally used to catch a specific type of small white fish native to the lake.",
        "The name 'Janitzio' means 'where it rains' or 'silk hair' in the Purépecha language."
      ],
      es: ["La estatua de Morelos mide 40 metros de altura y se puede entrar en su interior.", "Los murales dentro de la estatua muestran la vida del héroe nacional.", "Las redes de mariposa de los pescadores se utilizan hoy principalmente para ceremonias.", "El idioma purépecha todavía se habla activamente en la isla.", "El Día de Muertos en Janitzio es considerado uno de los más auténticos de todo México.", "La isla solo es accesible en pequeñas lanchas de motor desde tierra firme.", "El lago de Pátzcuaro se encuentra a una altura impresionante de 2.140 metros."],
      pt: ["A estátua de Morelos tem 40 metros de altura e é acessível por dentro.", "Os murais no interior da estátua mostram a vida do herói nacional.", "As redes de borboleta dos pescadores são usadas hoje principalmente para cerimônias.", "A língua Purépecha ainda é falada ativamente na ilha.", "O Día de Muertos em Janitzio é considerado um dos mais autênticos de todo o México.", "A ilha só é acessível por pequenos barcos a motor a partir do continente.", "O Lago Pátzcuaro está localizado a uma altitude impressionante de 2.140 metros."],
      fr: ["La statue de Morelos mesure 40 mètres de haut et l'intérieur est accessible.", "Les peintures murales à l'intérieur de la statue retracent la vie du héros national.", "Les filets papillons des pêcheurs sont aujourd'hui surtout utilisés pour les cérémonies.", "La langue Purépecha est encore activement parlée sur l'île.", "Le Día de Muertos à Janitzio est considéré comme l'un des plus authentiques du Mexique.", "L'île n'est accessible que par de petits bateaux à moteur depuis le continent.", "Le lac Pátzcuaro est situé à une altitude impressionnante de 2 140 mètres."],
    }, image: "/poi-images/janitzio-island-life-v2.webp",
    description: {
      de: "Die berühmteste Insel im Pátzcuaro-See, bekannt für ihre Fischer und die riesige Morelos-Statue.",
      hu: "A Pátzcuaro-tó legismertebb szigete, amely a purépecha kultúra központja.",
      ro: "Cea mai faimoasă insulă din lacul Pátzcuaro, recunoscută pentru pescarii cu plase \"fluture\" și statuia monumentală a lui Morelos.",
      en: "The most famous island in Lake Pátzcuaro, recognized for its iconic butterfly net fishermen and monumental statue.",
      es: "La isla más famosa del lago de Pátzcuaro, conocida por sus pescadores y la enorme estatua de Morelos.",
      pt: "A ilha mais famosa do Lago Pátzcuaro, conhecida pelos seus pescadores e pela enorme estátua de Morelos.",
      fr: "L'île la plus célèbre du lac Pátzcuaro, connue pour ses pêcheurs et sa statue monumentale de Morelos.",
    },
    facts: {
      de: ["Zentrum des Tags der Toten", "Schmetterlingsfischer-Tradition", "Gigantische Statue auf der Spitze"],
      hu: ["40 méter magas Morelos-szobor", "A halottak napja központi helyszíne", "Hagyományos purépecha közösség"],
      ro: ["Statuia lui Morelos are 40 de metri înălțime", "Centru major pentru Ziua Morților", "Comunitate tradițională Purépecha"],
      en: ["Statue of Morelos is 40 meters high", "Heart of Day of the Dead traditions", "Purépecha cultural center"],
      es: ["Centro del Día de Muertos", "Tradición de pesca con redes de mariposa", "Estatua gigante en la cima"],
      pt: ["Centro do Dia dos Mortos", "Tradição de pesca com redes de borboleta", "Estátua gigantesca no topo"],
      fr: ["Centre du Jour des Morts", "Tradition des pêcheurs aux filets papillons", "Statue géante au sommet"],
    }
  },
  {
    id: "isla-espiritu-santo-life-v2", "sights": {
      "de": [
            {
                  "name": "El Corralito",
                  "text": "El Corralito ist ein Erholungsgebiet mit einem natürlichen, von Felsen umgebenen Pool.",
                  "category": "recreational",
                  "coords": [
                        -110.371542,
                        24.446339
                  ]
            },
            {
                  "name": "Stacked Rocks",
                  "text": "Stacked Rocks ist eine Felsformation aus übereinander geschichteten Felsbrocken an der Küste.",
                  "category": "landmark",
                  "coords": [
                        -110.305808,
                        24.504619
                  ]
            },
            {
                  "name": "Arch",
                  "text": "Der Bogen (Arch) ist ein natürliches, von Wellen geformtes Felsentor an der Küste von Espiritu Santo.",
                  "category": "landmark",
                  "coords": [
                        -110.335185,
                        24.523529
                  ]
            },
            {
                  "name": "Baja Expeditions Eco Camp",
                  "text": "Das Baja Expeditions Eco Camp bietet nachhaltige Unterkünfte und geführte Touren auf der Insel an.",
                  "category": "recreational",
                  "coords": [
                        -110.387393,
                        24.516098
                  ]
            },
            {
                  "name": "The Mask",
                  "text": "The Mask ist eine Felsformation, die einem menschlichen Gesicht gleicht, an den Klippen von Espiritu Santo.",
                  "category": "landmark",
                  "coords": [
                        -110.389759,
                        24.499141
                  ]
            },
            {
                  "name": "Isla Gallina",
                  "text": "Isla Gallina ist eine kleine unbewohnte Insel vor Espiritu Santo, bekannt für Seevögel.",
                  "category": "natural",
                  "coords": [
                        -110.383988,
                        24.457093
                  ]
            },
            {
                  "name": "Parque nacional marina del Archipiélago de Espíritu Santo",
                  "text": "Der Nationale Meerespark des Archipiélago de Espíritu Santo ist ein Naturschutzgebiet im Espíritu Santo-Archipel.",
                  "category": "natural",
                  "coords": [
                        -110.348894,
                        24.50785
                  ]
            },
            {
                  "name": "Isla Gallo",
                  "text": "Isla Gallo ist eine felsige Insel nahe Espiritu Santo, ein beliebter Ort zum Tauchen.",
                  "category": "natural",
                  "coords": [
                        -110.385426,
                        24.466064
                  ]
            },
            {
                  "name": "Isla Ballena",
                  "text": "Isla Ballena ist eine walähnlich geformte Insel, Teil des Espiritu Santo Archipels.",
                  "category": "natural",
                  "coords": [
                        -110.403527,
                        24.483031
                  ]
            },
            {
                  "name": "Quieta",
                  "text": "Quieta ist eine ruhige Bucht in Espiritu Santo, ideal zum Kajakfahren und Schnorcheln.",
                  "category": "recreational",
                  "coords": [
                        -110.401355,
                        24.565971
                  ]
            },
            {
                  "name": "Área de Protección de Flora y Fauna Islas del Golfo de California",
                  "text": "Das Área de Protección de Flora y Fauna Islas del Golfo de California ist ein Naturschutzgebiet, das Inseln im Golf von Kalifornien umfasst, einschließlich der Insel Espíritu Santo.",
                  "category": "natural",
                  "coords": [
                        -110.414302,
                        24.571648
                  ]
            }
      ],
      "hu": [
            {
                  "name": "El Corralito",
                  "text": "Az El Corralito egy természetes, sziklákkal körülvett medencével rendelkező pihenőhely.",
                  "category": "recreational",
                  "coords": [
                        -110.371542,
                        24.446339
                  ]
            },
            {
                  "name": "Stacked Rocks",
                  "text": "A Stacked Rocks egy tengerparti sziklafal, amely egymásra rakott szikladarabokból áll.",
                  "category": "landmark",
                  "coords": [
                        -110.305808,
                        24.504619
                  ]
            },
            {
                  "name": "Arch",
                  "text": "Az Arch (Ív) egy természetes tengeri sziklaív, amelyet a hullámok formáltak Espiritu Santo partjainál.",
                  "category": "landmark",
                  "coords": [
                        -110.335185,
                        24.523529
                  ]
            },
            {
                  "name": "Baja Expeditions Eco Camp",
                  "text": "A Baja Expeditions Eco Camp fenntartható szállásokat és vezetett túrákat kínál a szigeten.",
                  "category": "recreational",
                  "coords": [
                        -110.387393,
                        24.516098
                  ]
            },
            {
                  "name": "The Mask",
                  "text": "A The Mask egy emberi archoz hasonlító sziklaformáció Espiritu Santo sziklafalain.",
                  "category": "landmark",
                  "coords": [
                        -110.389759,
                        24.499141
                  ]
            },
            {
                  "name": "Isla Gallina",
                  "text": "Az Isla Gallina egy kis lakatlan sziget Espiritu Santo partjainál, tengeri madarak élőhelye.",
                  "category": "natural",
                  "coords": [
                        -110.383988,
                        24.457093
                  ]
            },
            {
                  "name": "Parque nacional marina del Archipiélago de Espíritu Santo",
                  "text": "Az Archipiélago de Espíritu Santo tengeri nemzeti parkja természetvédelmi terület az Espíritu Santo-szigetvilágban.",
                  "category": "natural",
                  "coords": [
                        -110.348894,
                        24.50785
                  ]
            },
            {
                  "name": "Isla Gallo",
                  "text": "Az Isla Gallo egy sziklás sziget Espiritu Santo közelében, népszerű merülőhely.",
                  "category": "natural",
                  "coords": [
                        -110.385426,
                        24.466064
                  ]
            },
            {
                  "name": "Isla Ballena",
                  "text": "Az Isla Ballena egy bálna alakú sziget, az Espiritu Santo szigetcsoport része.",
                  "category": "natural",
                  "coords": [
                        -110.403527,
                        24.483031
                  ]
            },
            {
                  "name": "Quieta",
                  "text": "A Quieta egy csendes öböl Espiritu Santóban, ideális kajakozáshoz és snorkelezéshez.",
                  "category": "recreational",
                  "coords": [
                        -110.401355,
                        24.565971
                  ]
            },
            {
                  "name": "Área de Protección de Flora y Fauna Islas del Golfo de California",
                  "text": "Az Área de Protección de Flora y Fauna Islas del Golfo de California egy természetvédelmi terület, amely a Kaliforniai-öböl szigeteit foglalja magában, beleértve az Espíritu Santo-szigetet is.",
                  "category": "natural",
                  "coords": [
                        -110.414302,
                        24.571648
                  ]
            }
      ],
      "ro": [
            {
                  "name": "El Corralito",
                  "text": "El Corralito este o zonă de agrement cu o piscină naturală înconjurată de stânci.",
                  "category": "recreational",
                  "coords": [
                        -110.371542,
                        24.446339
                  ]
            },
            {
                  "name": "Stacked Rocks",
                  "text": "Stacked Rocks este o formațiune stâncoasă formată din bolovani suprapuși pe coastă.",
                  "category": "landmark",
                  "coords": [
                        -110.305808,
                        24.504619
                  ]
            },
            {
                  "name": "Arch",
                  "text": "Arch este un arc natural de piatră sculptat de valuri pe țărmul insulei Espiritu Santo.",
                  "category": "landmark",
                  "coords": [
                        -110.335185,
                        24.523529
                  ]
            },
            {
                  "name": "Baja Expeditions Eco Camp",
                  "text": "Baja Expeditions Eco Camp oferă cazare ecologică și tururi ghidate pe insulă.",
                  "category": "recreational",
                  "coords": [
                        -110.387393,
                        24.516098
                  ]
            },
            {
                  "name": "The Mask",
                  "text": "The Mask este o formațiune stâncoasă ce seamănă cu o față umană pe stâncile din Espiritu Santo.",
                  "category": "landmark",
                  "coords": [
                        -110.389759,
                        24.499141
                  ]
            },
            {
                  "name": "Isla Gallina",
                  "text": "Isla Gallina este o insulă mică nelocuită lângă Espiritu Santo, cunoscută pentru coloniile de păsări marine.",
                  "category": "natural",
                  "coords": [
                        -110.383988,
                        24.457093
                  ]
            },
            {
                  "name": "Parque nacional marina del Archipiélago de Espíritu Santo",
                  "text": "Parcul național marin al Arhipelagului Espíritu Santo este o rezervație naturală în arhipelagul Espíritu Santo.",
                  "category": "natural",
                  "coords": [
                        -110.348894,
                        24.50785
                  ]
            },
            {
                  "name": "Isla Gallo",
                  "text": "Isla Gallo este o insulă stâncoasă lângă Espiritu Santo, loc popular pentru scufundări.",
                  "category": "natural",
                  "coords": [
                        -110.385426,
                        24.466064
                  ]
            },
            {
                  "name": "Isla Ballena",
                  "text": "Isla Ballena este o insulă în formă de balenă, parte a arhipelagului Espiritu Santo.",
                  "category": "natural",
                  "coords": [
                        -110.403527,
                        24.483031
                  ]
            },
            {
                  "name": "Quieta",
                  "text": "Quieta este o golfuleț liniștit în Espiritu Santo, ideal pentru caiac și snorkeling.",
                  "category": "recreational",
                  "coords": [
                        -110.401355,
                        24.565971
                  ]
            },
            {
                  "name": "Área de Protección de Flora y Fauna Islas del Golfo de California",
                  "text": "Área de Protección de Flora y Fauna Islas del Golfo de California este o rezervație naturală care cuprinde insulele din Golful Californiei, inclusiv insula Espíritu Santo.",
                  "category": "natural",
                  "coords": [
                        -110.414302,
                        24.571648
                  ]
            }
      ],
      "en": [
            {
                  "name": "El Corralito",
                  "text": "El Corralito is a recreational area with a natural pool surrounded by rocks.",
                  "category": "recreational",
                  "coords": [
                        -110.371542,
                        24.446339
                  ]
            },
            {
                  "name": "Stacked Rocks",
                  "text": "Stacked Rocks is a rock formation of piled boulders along the coast.",
                  "category": "landmark",
                  "coords": [
                        -110.305808,
                        24.504619
                  ]
            },
            {
                  "name": "Arch",
                  "text": "The Arch is a natural sea arch carved by waves on Espiritu Santo's shoreline.",
                  "category": "landmark",
                  "coords": [
                        -110.335185,
                        24.523529
                  ]
            },
            {
                  "name": "Baja Expeditions Eco Camp",
                  "text": "Baja Expeditions Eco Camp offers sustainable lodging and guided tours on the island.",
                  "category": "recreational",
                  "coords": [
                        -110.387393,
                        24.516098
                  ]
            },
            {
                  "name": "The Mask",
                  "text": "The Mask is a rock formation resembling a human face on the cliffs of Espiritu Santo.",
                  "category": "landmark",
                  "coords": [
                        -110.389759,
                        24.499141
                  ]
            },
            {
                  "name": "Isla Gallina",
                  "text": "Isla Gallina is a small uninhabited island off Espiritu Santo, known for seabird colonies.",
                  "category": "natural",
                  "coords": [
                        -110.383988,
                        24.457093
                  ]
            },
            {
                  "name": "Parque nacional marina del Archipiélago de Espíritu Santo",
                  "text": "The national marine park of the Archipiélago de Espíritu Santo is a nature reserve in the Espíritu Santo Archipelago.",
                  "category": "natural",
                  "coords": [
                        -110.348894,
                        24.50785
                  ]
            },
            {
                  "name": "Isla Gallo",
                  "text": "Isla Gallo is a rocky islet near Espiritu Santo, a popular diving spot.",
                  "category": "natural",
                  "coords": [
                        -110.385426,
                        24.466064
                  ]
            },
            {
                  "name": "Isla Ballena",
                  "text": "Isla Ballena is a whale-shaped island within the Espiritu Santo archipelago.",
                  "category": "natural",
                  "coords": [
                        -110.403527,
                        24.483031
                  ]
            },
            {
                  "name": "Quieta",
                  "text": "Quieta is a calm cove in Espiritu Santo, perfect for kayaking and snorkeling.",
                  "category": "recreational",
                  "coords": [
                        -110.401355,
                        24.565971
                  ]
            },
            {
                  "name": "Área de Protección de Flora y Fauna Islas del Golfo de California",
                  "text": "Área de Protección de Flora y Fauna Islas del Golfo de California is a nature reserve encompassing islands in the Gulf of California, including Espíritu Santo Island.",
                  "category": "natural",
                  "coords": [
                        -110.414302,
                        24.571648
                  ]
            }
      ]
},
    type: "island",
    parent: "MX-BCS",
    coords: [-110.34, 24.52],
    name: { de: "Isla Espiritu Santo", hu: "Espiritu Santo sziget", ro: "Insula Espiritu Santo", en: "Espiritu Santo Island"},
    image: "/poi-images/isla-espiritu-santo-life-v2.webp",
    descriptionAdvanced: {
      de: "Isla Espíritu Santo im Golf von Kalifornien ist eine unbewohnte Insel von wilder, fast surrealer Schönheit und gehört zum UNESCO-Weltnaturerbe. Die kontrastreiche Landschaft aus rötlichen Vulkanfelsen, türkisfarbenem Wasser und weißen Sandstränden macht sie zu einem Juwel der Baja California. Ein absolutes Highlight ist das Schnorcheln mit den verspielten Seelöwen in der Kolonie von Los Islotes. Die Gewässer um die Insel wimmeln von Leben, von bunten Tropenfischen bis hin zu vorbeiziehenden Walen und Delfinen. Als Naturschutzgebiet bietet die Insel wichtige Nistplätze für Seevögel und ist ein beliebtes Ziel für Seekajakfahrer und Camper, die die absolute Einsamkeit suchen. Isla Espíritu Santo zeigt den Golf von Kalifornien als das 'Aquarium der Welt', wie Jacques Cousteau ihn einst nannte. Biologie K8 - Marine Biodiversität und Inselökologie.",
      hu: "Espiritu Santo sziget egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Espiritu Santo sziget kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Insula Espiritu Santo este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Insula Espiritu Santo este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Isla Espíritu Santo is a breathtaking, uninhabited island located in the Sea of Cortez, just off the coast of La Paz in Baja California Sur. Often called the 'Galápagos of Mexico,' it is a place of stark, desert beauty, with red volcanic cliffs, turquoise bays, and white sand beaches. The island and its surrounding waters are a UNESCO-protected biosphere reserve, housing an incredible diversity of marine life, including dolphins, whales, and rays. The highlight for many visitors is the sea lion colony at Los Islotes, where it is possible to snorkel and swim with playful young pups in their natural habitat. The island's numerous coves, such as Ensenada Grande, offer perfect conditions for kayaking, paddleboarding, and camping under the stars. With no permanent human residents, the island remains a pristine wilderness where the silence is only broken by the sound of the waves and the calls of seabirds. It is a must-visit destination for those seeking an authentic encounter with the wild beauty of the Gulf of California.",
      es: "Isla Espíritu Santo, en el Golfo de California, es una isla deshabitada de belleza salvaje y casi surrealista, declarada Patrimonio de la Humanidad por la UNESCO. El paisaje contrastante de rocas volcánicas rojizas, aguas turquesas y playas de arena blanca la convierte en una joya de Baja California. Un punto culminante absoluto es el esnórquel con los juguetones lobos marinos en la colonia de Los Islotes. Las aguas alrededor de la isla bullen de vida, desde coloridos peces tropicales hasta ballenas y delfines que pasan por allí. Como reserva natural, la isla ofrece sitios de anidación vitales para aves marinas y es un destino popular para kayakistas de mar y campistas que buscan la soledad absoluta. Isla Espíritu Santo muestra el Golfo de California como el 'Acuario del Mundo', como Jacques Cousteau lo llamó alguna vez.",
      pt: "Isla Espíritu Santo, no Golfo da Califórnia, é uma ilha desabitada de beleza selvagem e quase surreal, sendo Patrimônio Mundial da UNESCO. A paisagem contrastante de rochas vulcânicas avermelhadas, águas turquesa e praias de areia branca torna-a uma joia da Baixa Califórnia. Um destaque absoluto é o mergulho com snorkel com os leões-marinhos brincalhões na colônia de Los Islotes. As águas ao redor da ilha fervilham de vida, desde peixes tropicais coloridos até baleias e golfinhos que passam. Como reserva natural, a ilha oferece locais de nidificação importantes para aves marinhas e é um destino popular para canoístas e campistas que buscam a solidão absoluta. Isla Espíritu Santo mostra o Golfo da Califórnia como o 'Aquário do Mundo', como Jacques Cousteau o chamou uma vez.",
      fr: "L'île Espiritu Santo, dans le golfe de Californie, est une île inhabitée d'une beauté sauvage, presque surréaliste, classée au patrimoine mondial de l'UNESCO. Le paysage contrasté de roches volcaniques rougeâtres, d'eaux turquoise et de plages de sable blanc en fait un joyau de la Basse-Californie. Un moment fort absolu est le snorkeling avec les otaries joueuses de la colonie de Los Islotes. Les eaux entourant l'île regorgent de vie, des poissons tropicaux colorés aux baleines et dauphins de passage. En tant que réserve naturelle, l'île offre des sites de nidification importants pour les oiseaux marins et constitue une destination prisée des kayakistes et des campeurs en quête de solitude absolue. L'île Espiritu Santo illustre le golfe de Californie comme 'l'aquarium du monde', comme Jacques Cousteau l'appelait autrefois.",
    },
    factsAdvanced: {
      de: ["Die Insel ist durch einen schmalen Kanal von der größeren Isla Partida getrennt.", "Beherbergt eine permanente Kolonie von über 300 Kalifornischen Seelöwen.", "Wurde 1995 zusammen mit anderen Inseln im Golf zum UNESCO-Welterbe erklärt.", "Enthält prähistorische Fundstätten der indigenen Pericú-Kultur.", "Über 500 Fischarten wurden in den Gewässern rund um die Insel gezählt.", "Die Insel ist vulkanischen Ursprungs, was die rötliche Farbe der Felsen erklärt.", "Es gibt keine permanenten menschlichen Siedlungen oder Hotels auf der Insel."],
      hu: ["Espiritu Santo sziget sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Espiritu Santo sziget egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Insula Espiritu Santo ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Insula Espiritu Santo este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The island was declared a part of a UNESCO Biosphere Reserve in 1995.",
        "It is separated from the smaller Isla Partida by a narrow, shallow channel that can sometimes be crossed on foot at low tide.",
        "Espíritu Santo is home to several endemic species, including the black jackrabbit.",
        "The island was once a site for pearl oyster cultivation in the late 19th and early 20th centuries.",
        "Jacques Cousteau famously referred to the Sea of Cortez around the island as the 'World's Aquarium'."
      ],
      es: ["La isla está separada de la más grande Isla Partida por un canal estrecho.", "Alberga una colonia permanente de más de 300 lobos marinos de California.", "Fue declarada Patrimonio de la Humanidad por la UNESCO en 1995 junto con otras islas del golfo.", "Contiene sitios arqueológicos prehistóricos de la cultura indígena Pericú.", "Se han contado más de 500 especies de peces en las aguas que rodean la isla.", "La isla es de origen volcánico, lo que explica el color rojizo de las rocas.", "No hay asentamientos humanos permanentes ni hoteles en la isla."],
      pt: ["A ilha é separada da maior Isla Partida por um canal estreito.", "Abriga uma colônia permanente de mais de 300 leões-marinhos da Califórnia.", "Foi declarada Patrimônio Mundial da UNESCO em 1995, juntamente com outras ilhas do golfo.", "Contém sítios arqueológicos pré-históricos da cultura indígena Pericú.", "Mais de 500 espécies de peixes foram contadas nas águas ao redor da ilha.", "A ilha é de origem vulcânica, o que explica a cor avermelhada das rochas.", "Não há assentamentos humanos permanentes ou hotéis na ilha."],
      fr: ["L'île est séparée de la plus grande île Partida par un étroit canal.", "Abrite une colonie permanente de plus de 300 otaries de Californie.", "Inscrite au patrimoine mondial de l'UNESCO en 1995 avec d'autres îles du golfe.", "Contient des sites préhistoriques de la culture indigène Pericú.", "Plus de 500 espèces de poissons ont été dénombrées dans les eaux entourant l'île.", "L'île est d'origine volcanique, ce qui explique la couleur rougeâtre des rochers.", "Il n'y a pas d'installations humaines permanentes ni d'hôtels sur l'île."],
    }, image: "/poi-images/isla-espiritu-santo-life-v2.webp",
    description: {
      de: "Eine unbewohnte, felsige Insel im Golf von Kalifornien, ein Paradies für Taucher und Kajakfahrer.",
      hu: "Lakatlan, vörös szikláiról ismert sziget a Kaliforniai-öbölben, amely UNESCO világörökségi helyszín.",
      ro: "Insulă deșertică nelocuită în Marea lui Cortez, celebră pentru stâncile vulcanice roșii și colonia de lei de mare prietenoși.",
      en: "An uninhabited desert island in the Sea of Cortez, celebrated for its red volcanic cliffs and friendly sea lion colony.",
      es: "Una isla rocosa y deshabitada en el Golfo de California, un paraíso para buceadores y kayakistas.",
      pt: "Uma ilha rochosa e desabitada no Golfo da Califórnia, um paraíso para mergulhadores e canoístas.",
      fr: "Une île rocheuse inhabitée dans le golfe de Californie, un paradis pour les plongeurs et les kayakistes.",
    },
    facts: {
      de: ["UNESCO-Weltnaturerbe", "Kolonie von Seelöwen", "Kontrast aus Wüste und Meer"],
      hu: ["Állandó oroszlánfóka-kolónia", "UNESCO világörökségi helyszín", "Lakatlan, érintetlen terület"],
      ro: ["Patrimoniu Mondial UNESCO din anul 1995", "Colonii permanente de lei de mare", "Numită \"Acvariul Lumii\" de Jacques Cousteau"],
      en: ["UNESCO Biosphere Reserve", "Home to a permanent sea lion colony", "Uninhabited and pristine"],
      es: ["Patrimonio Mundial de la UNESCO", "Colonia de lobos marinos", "Contraste de desierto y mar"],
      pt: ["Patrimônio Mundial da UNESCO", "Colônia de leões-marinhos", "Contraste entre deserto e mar"],
      fr: ["Patrimoine mondial de l'UNESCO", "Colonie d'otaries", "Contraste entre désert et mer"],
    }
  },
  {
    id: "isla-tiburon-life-v2",
    type: "island",
    parent: "MX-SON",
    coords: [-112.8, 28.98],
    name: { de: "Isla Tiburon", hu: "Tiburon sziget", ro: "Insula Tiburon", en: "Tiburon Island"},
    image: "/poi-images/isla-tiburon-life-v2.webp",
    descriptionAdvanced: {
      de: "Isla Tiburón, die größte Insel Mexikos, liegt im Golf von Kalifornien und ist das heilige angestammte Land des Seri-Volkes (Comcaac). Die Insel ist ein streng geschütztes Naturreservat und beherbergt eine einzigartige Wüstenfauna, darunter Dickhornschafe und Maultierhirsche. Für die Seri ist die Insel nicht nur ein physischer Ort, sondern ein spirituelles Zentrum ihrer Kultur und Identität. Da der Zugang streng kontrolliert wird, hat Isla Tiburón eine fast unberührte Wildnis bewahrt, die einen tiefen Einblick in die ursprünglichen Ökosysteme der Sonora-Wüste bietet. Die rauen Berge der Insel und die umliegenden fischreichen Gewässer bilden eine Landschaft von archaischer Schönheit. Geographie K8 - Indigene Landrechte und Wüstenbiotope.",
      hu: "Tiburon sziget egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Tiburon sziget kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Insula Tiburon este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Insula Tiburon este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Isla Tiburón, or 'Shark Island,' is the largest island in Mexico and is located in the Gulf of California, separated from the state of Sonora by a narrow channel. This rugged and arid island is the ancestral territory of the Seri (Comca'ac) people, who consider it a sacred place and still manage it today as a communal ecological reserve. The island's landscape is dominated by two mountain ranges and features unique desert flora, including the giant cardon cactus and numerous endemic plant species. It is a critical habitat for desert bighorn sheep and mule deer, which were reintroduced to the island to ensure their survival. Access to Isla Tiburón is strictly controlled by the Seri people, and visitors must obtain permission and often hire a local guide to explore its pristine shores and interior. The island remains one of the most untouched and culturally significant wilderness areas in Mexico, reflecting the deep connection between the Seri people and their natural environment.",
      es: "Isla Tiburón, la isla más grande de México, se encuentra en el Golfo de California y es la tierra ancestral sagrada del pueblo Seri (Comcaac). La isla es una reserva natural estrictamente protegida y alberga una fauna desértica única, que incluye borregos cimarrones y venados bura. Para los Seri, la isla no es solo un lugar físico, sino un centro espiritual de su cultura e identidad. Dado que el acceso está estrictamente controlado, Isla Tiburón ha conservado una naturaleza casi virgen que ofrece una visión profunda de los ecosistemas originales del desierto de Sonora. Las montañas escarpadas de la isla y las aguas circundantes ricas en peces forman un paisaje de belleza arcaica.",
      pt: "Isla Tiburón, a maior ilha do México, está localizada no Golfo da Califórnia e é a terra ancestral sagrada do povo Seri (Comcaac). A ilha é uma reserva natural estritamente protegida e abriga uma fauna desértica única, incluindo carneiros-selvagens e veados-mula. Para os Seri, a ilha não é apenas um lugar físico, mas um centro espiritual da sua cultura e identidade. Como o acesso é estritamente controlado, Isla Tiburón preservou uma natureza quase intocada, oferecendo uma visão profunda dos ecossistemas originais do deserto de Sonora. As montanhas escarpadas da ilha e as águas circundantes ricas em peixes formam uma paisagem de beleza arcaica.",
      fr: "L'île Tiburón, la plus grande île du Mexique, se trouve dans le golfe de Californie et est la terre ancestrale sacrée du peuple Seri (Comcaac). L'île est une réserve naturelle strictement protégée et abrite une faune désertique unique, notamment des mouflons canadiens et des cerfs mulets. Pour les Seri, l'île n'est pas seulement un lieu physique, mais un centre spirituel de leur culture et de leur identité. Comme l'accès est strictement contrôlé, l'île Tiburón a conservé une nature sauvage presque intacte, offrant un aperçu profond des écosystèmes originaux du désert de Sonora. Les montagnes escarpées de l'île et les eaux poissonneuses environnantes forment un paysage d'une beauté archaïque.",
    },
    factsAdvanced: {
      de: ["Ist mit einer Fläche von 1.200 Quadratkilometern die größte Insel Mexikos.", "Wird ausschließlich vom Seri-Volk verwaltet und geschützt.", "Dient als wichtiges Reservoir für die Wiederansiedlung des Dickhornschafes.", "Der Name bedeutet 'Haifischinsel' aufgrund der vielen Haie in der Region.", "Ist durch den Canal del Infiernillo vom mexikanischen Festland getrennt.", "Verfügt über keine permanenten menschlichen Siedlungen oder Infrastruktur.", "Wurde 1963 zum Naturschutzgebiet erklärt."],
      hu: ["Tiburon sziget sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Tiburon sziget egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Insula Tiburon ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Insula Tiburon este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "At 1,200 square kilometers, it is slightly larger than the island of Martinique.",
        "The island was decreed a nature reserve in 1963 and later became part of the 'Islands of the Gulf of California' UNESCO site.",
        "There are no permanent settlements on the island, as the Seri people maintain it as a wild preserve.",
        "The channel separating the island from the mainland is called the Canal del Infiernillo (Little Hell Channel) due to its strong currents.",
        "The desert bighorn sheep population on the island is one of the healthiest and most genetically pure in North America."
      ],
      es: ["Con una superficie de 1.200 kilómetros cuadrados, es la isla más grande de México.", "Es administrada y protegida exclusivamente por el pueblo Seri.", "Sirve como reserva importante para la reintroducción del borrego cimarrón.", "El nombre significa 'Isla del Tiburón' debido a los muchos tiburones en la región.", "Está separada del territorio continental mexicano por el Canal del Infiernillo.", "No cuenta con asentamientos humanos permanentes ni infraestructura.", "Fue declarada reserva natural en 1963."],
      pt: ["Com uma área de 1.200 quilômetros quadrados, é a maior ilha do México.", "É gerida e protegida exclusivamente pelo povo Seri.", "Serve como um reservatório importante para a reintrodução do carneiro-selvagem.", "O nome significa 'Ilha do Tubarão' devido aos muitos tubarões na região.", "É separada do continente mexicano pelo Canal del Infiernillo.", "Não possui assentamentos humanos permanentes ou infraestrutura.", "Foi declarada reserva natural em 1963."],
      fr: ["Avec une surface de 1 200 kilomètres carrés, c'est la plus grande île du Mexique.", "Gérée et protégée exclusivement par le peuple Seri.", "Sert de réservoir important pour la réintroduction du mouflon canadien.", "Son nom signifie 'île aux requins' en raison des nombreux requins dans la région.", "Séparée du continent mexicain par le canal del Infiernillo.", "Ne possède aucune installation humaine permanente ni infrastructure.", "Déclarée réserve naturelle en 1963."],
    }, image: "/poi-images/isla-tiburon-life-v2.webp",
    description: {
      de: "Die größte Insel Mexikos, gelegen im Golf von Kalifornien und heiliges Land der Seri-Indianer.",
      hu: "Mexikó legnagyobb szigete, amely a seri indiánok szent területe és védett rezervátum.",
      ro: "Cea mai mare insulă din Mexic, menținută ca rezervație ecologică sacră de către poporul indigen Seri din Sonora.",
      en: "Mexico's largest island, maintained as a wild and sacred ecological reserve by the indigenous Seri people of Sonora.",
      es: "La isla más grande de México, ubicada en el Golfo de California y tierra sagrada de los indios Seri.",
      pt: "A maior ilha do México, localizada no Golfo da Califórnia e terra sagrada dos índios Seri.",
      fr: "La plus grande île du Mexique, située dans le golfe de Californie et terre sacrée des Indiens Seri.",
    },
    facts: {
      de: ["Größte mexikanische Insel", "Keine dauerhafte Besiedlung", "Eigentum der Comcaac-Nation"],
      hu: ["Mexikó legnagyobb szigete", "A seri (comcaac) nép tulajdona", "Vadon élő kanadai vadjuhok otthona"],
      ro: ["Teritoriul ancestral al poporului Seri", "Nu are așezări umane permanente", "Refugiu pentru oile sălbatice de deșert"],
      en: ["Ancestral land of the Seri people", "Largest island in Mexico", "Refuge for desert bighorn sheep"],
      es: ["Isla mexicana más grande", "Sin asentamiento permanente", "Propiedad de la nación Comcaac"],
      pt: ["Maior ilha mexicana", "Sem ocupação permanente", "Propriedade da nação Comcaac"],
      fr: ["Plus grande île mexicaine", "Pas de peuplement permanent", "Propriété de la nation Comcaac"],
    }
  },
  {
    id: "marietas-islands-life-v2", "sights": {
      "de": [
            {
                  "name": "Playa del amor",
                  "text": "Playa del Amor ist ein versteckter Strand auf Isla Mujeres, zugänglich durch einen Felstunnel.",
                  "category": "landmark",
                  "coords": [
                        -105.566719,
                        20.703981
                  ]
            },
            {
                  "name": "Snorkelling",
                  "text": "Dieser Schnorchelplatz bietet klares Wasser und eine bunte Unterwasserwelt rund um die Marietas-Inseln.",
                  "category": "landmark",
                  "coords": [
                        -105.577358,
                        20.696632
                  ]
            },
            {
                  "name": "Playa Escondida",
                  "text": "Versteckter Strand auf den Marietas-Inseln, bekannt für seinen geheimen Zugang durch eine Höhle.",
                  "category": "recreational",
                  "coords": [
                        -105.564958,
                        20.704162
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Playa del amor",
                  "text": "A Playa del Amor egy rejtett strand Isla Mujeresen, amely egy sziklaalagúton keresztül érhető el.",
                  "category": "landmark",
                  "coords": [
                        -105.566719,
                        20.703981
                  ]
            },
            {
                  "name": "Snorkelling",
                  "text": "Ez a snorkelezőhely tiszta vizet és színes vízi élővilágot kínál a Marietas-szigetek körül.",
                  "category": "landmark",
                  "coords": [
                        -105.577358,
                        20.696632
                  ]
            },
            {
                  "name": "Playa Escondida",
                  "text": "Rejtett strand a Marietas-szigeteken, amely egy barlangon keresztüli titkos bejáratáról ismert.",
                  "category": "recreational",
                  "coords": [
                        -105.564958,
                        20.704162
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Playa del amor",
                  "text": "Playa del Amor este o plajă ascunsă pe Isla Mujeres, accesibilă printr-un tunel de stâncă.",
                  "category": "landmark",
                  "coords": [
                        -105.566719,
                        20.703981
                  ]
            },
            {
                  "name": "Snorkelling",
                  "text": "Acest loc de snorkeling oferă apă clară și o lume subacvatică colorată în jurul Insulelor Marietas.",
                  "category": "landmark",
                  "coords": [
                        -105.577358,
                        20.696632
                  ]
            },
            {
                  "name": "Playa Escondida",
                  "text": "Plajă ascunsă pe insulele Marietas, cunoscută pentru accesul secret printr-o peșteră.",
                  "category": "recreational",
                  "coords": [
                        -105.564958,
                        20.704162
                  ]
            }
      ],
      "en": [
            {
                  "name": "Playa del amor",
                  "text": "Playa del Amor is a hidden beach on Isla Mujeres, accessed through a rock tunnel.",
                  "category": "landmark",
                  "coords": [
                        -105.566719,
                        20.703981
                  ]
            },
            {
                  "name": "Snorkelling",
                  "text": "This snorkeling spot offers clear water and colorful marine life around the Marietas Islands.",
                  "category": "landmark",
                  "coords": [
                        -105.577358,
                        20.696632
                  ]
            },
            {
                  "name": "Hidden Beach",
                  "text": "Hidden Beach on the Marietas Islands, famous for its secret entrance through a cave.",
                  "category": "recreational",
                  "coords": [
                        -105.564958,
                        20.704162
                  ]
            }
      ]
},
    type: "island",
    parent: "MX-NAY",
    coords: [-105.57, 20.7],
    name: { de: "Marietas-Inseln", hu: "Marietas szigetek", ro: "Insulele Marietas", en: "Marietas Islands"},
    image: "/poi-images/marietas-islands-life-v2.webp",
    descriptionAdvanced: {
      de: "Die Islas Marietas sind eine Gruppe kleiner, unbewohnter Inseln vor der Küste von Nayarit, die für ihren 'versteckten Strand' (Playa del Amor) weltberühmt sind. Dieser Strand befindet sich in einem eingestürzten Vulkankrater und ist nur durch einen schmalen Wassertunnel erreichbar. Die Inseln sind vulkanischen Ursprungs und beherbergen eine enorme Vielfalt an Meeresvögeln, darunter den seltenen Blaufußtölpel. Jacques Cousteau setzte sich massiv für den Schutz dieser Inseln ein, was schließlich zu ihrer Ernennung zum Nationalpark führte. Um das empfindliche Ökosystem und die Korallenriffe zu schützen, ist der Zugang heute streng limitiert. Die Marietas sind ein herausragendes Beispiel für die Regenerationskraft der Natur nach militärischer Nutzung in der Vergangenheit. Biologie K6 - Vulkaninseln und Artenschutz.",
      hu: "Marietas szigetek egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Marietas szigetek kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Insulele Marietas este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Insulele Marietas este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Marietas Islands are a group of small, uninhabited volcanic islands located a few miles off the coast of Punta de Mita in Nayarit. They are famous worldwide for the 'Playa del Amor,' also known as the Hidden Beach, which is a stunning sandy cavern with a massive open-air roof created by volcanic activity and erosion. The islands are a protected national park and a UNESCO Biosphere Reserve, serving as an important nesting site for over 90 species of birds, including the rare blue-footed booby. The surrounding waters are part of a thriving marine reserve, offering exceptional snorkeling and diving opportunities where visitors can see giant mantas, sea turtles, and a variety of colorful tropical fish. During the winter months, the area around the islands is a prime spot for whale watching, as humpback whales congregate in the Bay of Banderas to mate and give birth. To protect the delicate ecosystem, the number of visitors is strictly limited, and access to the Hidden Beach requires swimming through a short tunnel at low tide.",
      es: "Las Islas Marietas son un grupo de pequeñas islas deshabitadas frente a la costa de Nayarit, mundialmente famosas por su 'playa escondida' (Playa del Amor). Esta playa se encuentra dentro de un cráter volcánico colapsado y solo es accesible a través de un estrecho túnel de agua. Las islas son de origen volcánico y albergan una enorme diversidad de aves marinas, incluido el raro piquero patiazul. Jacques Cousteau abogó intensamente por la protección de estas islas, lo que finalmente llevó a su designación como Parque Nacional. Para proteger el frágil ecosistema y los arrecifes de coral, el acceso está hoy estrictamente limitado. Las Marietas son un ejemplo destacado del poder de regeneración de la naturaleza tras el uso militar en el pasado. Biología K6 - Islas volcánicas y protección de especies.",
      pt: "As Ilhas Marietas são um grupo de pequenas ilhas desabitadas ao largo da costa de Nayarit, mundialmente famosas pela sua 'praia escondida' (Playa del Amor). Esta praia situa-se num seio de uma cratera vulcânica colapsada e só é acessível através de um túnel estreito de água. As ilhas são de origem vulcânica e abrigam uma enorme diversidade de aves marinhas, incluindo o raro atobá-de-pés-azuis. Jacques Cousteau defendeu intensamente a proteção destas ilhas, o que acabou por levar à sua designação como Parque Nacional. Para proteger o ecossistema frágil e os recifes de coral, o acesso é hoje estritamente limitado. As Marietas são um exemplo notável do poder de regeneração da natureza após o uso militar no passado. Biologia K6 - Ilhas vulcânicas e proteção de espécies.",
      fr: "Les îles Marietas sont un groupe de petites îles inhabitées au large de la côte de Nayarit, mondialement célèbres pour leur 'plage cachée' (Playa del Amor). Cette plage se trouve dans un cratère volcanique effondré et n'est accessible que par un étroit tunnel aquatique. Les îles sont d'origine volcanique et abritent une énorme diversité d'oiseaux marins, dont le rare fou à pieds bleus. Jacques Cousteau s'est massivement engagé pour la protection de ces îles, ce qui a finalement conduit à leur nomination comme parc national. Pour protéger l'écosystème fragile et les récifs coralliens, l'accès est aujourd'hui strictement limité. Les Marietas sont un exemple exceptionnel de la force de régénération de la nature après une utilisation militaire passée. Biologie K6 - Îles volcaniques et protection des espèces.",
    },
    factsAdvanced: {
      de: ["Der 'Hidden Beach' entstand vermutlich durch Testbombenangriffe im frühen 20. Jahrhundert.", "Wurden 2005 zum Nationalpark 'Islas Marietas' erklärt.", "Sind einer der wenigen Orte in Mexiko, an denen Blaufußtölpel nisten.", "Die Gewässer um die Inseln sind im Winter ein wichtiger Ort für Buckelwale.", "Der Zugang zum Hidden Beach ist auf eine begrenzte Anzahl von Personen pro Tag limitiert.", "Gehören seit 2008 zum Biosphärenreservat der UNESCO.", "Bieten exzellente Bedingungen zum Schnorcheln in intakten Korallengärten."],
      hu: ["Marietas szigetek sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Marietas szigetek egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Insulele Marietas ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Insulele Marietas este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "In the early 20th century, the islands were used as a military testing ground, which contributed to some of their unique rock formations.",
        "Jacques Cousteau led a campaign in the 1960s to protect the islands from further military and industrial use.",
        "The islands are home to one of the few colonies of blue-footed boobies outside of the Galápagos Islands.",
        "Access to the 'Hidden Beach' is limited to a specific number of people per day to prevent environmental degradation.",
        "The islands are part of the 'Islas del Golfo de California' UNESCO World Heritage site."
      ],
      es: ["La 'Playa Escondida' se formó probablemente por bombardeos de prueba a principios del siglo XX.", "Fueron declaradas Parque Nacional 'Islas Marietas' en 2005.", "Son uno de los pocos lugares en México donde anidan los piqueros patiazul.", "Las aguas alrededor de las islas son un sitio importante para las ballenas jorobadas en invierno.", "El acceso a la Playa Escondida está limitado a un número restringido de personas por día.", "Pertenecen desde 2008 a la Reserva de la Biosfera de la UNESCO.", "Ofrecen condiciones excelentes para el esnórquel en jardines de coral intactos."],
      pt: ["A 'Praia Escondida' formou-se provavelmente devido a bombardeamentos de teste no início do século XX.", "Foram declaradas Parque Nacional 'Islas Marietas' em 2005.", "São um dos poucos locais no México onde os atobás-de-pés-azuis fazem ninhos.", "As águas em redor das ilhas são um local importante para as baleias-jubarte no inverno.", "O acesso à Praia Escondida está limitado a um número restrito de pessoas por dia.", "Pertencem desde 2008 à Reserva da Biosfera da UNESCO.", "Oferecem excelentes condições para a prática de snorkeling em jardins de coral intactos."],
      fr: ["La 'Plage Cachée' a probablement été créée par des essais de bombardements au début du XXe siècle.", "Ont été déclarées parc national 'Islas Marietas' en 2005.", "Sont l'un des rares endroits au Mexique où nichent les fous à pieds bleus.", "Les eaux autour des îles sont un lieu important pour les baleines à bosse en hiver.", "L'accès à la plage cachée est limité à un nombre restreint de personnes par jour.", "Font partie de la réserve de biosphère de l'UNESCO depuis 2008.", "Offrent d'excellentes conditions pour la plongée en apnée dans des jardins de corail intacts."],
    }, image: "/poi-images/marietas-islands-life-v2.webp",
    description: {
      de: "Eine Gruppe kleiner, unbewohnter Inseln vor Puerto Vallarta, berühmt für den versteckten 'Hidden Beach'.",
      hu: "Lakatlan vulkanikus szigetek Nayarit partjainál, amelyek a rejtett strandjukról híresek.",
      ro: "Insule vulcanice mici lângă coasta Nayarit, celebre în întreaga lume pentru \"Plaja Ascunsă\" aflată în interiorul unui crater.",
      en: "Small volcanic islands off the coast of Nayarit, world-famous for the 'Hidden Beach' nestled inside a collapsed crater.",
      es: "Un grupo de pequeñas islas deshabitadas frente a Puerto Vallarta, famosas por la 'Playa Escondida'.",
      pt: "Um grupo de pequenas ilhas desabitadas ao largo de Puerto Vallarta, famosas pela 'Praia Escondida'.",
      fr: "Un groupe de petites îles inhabitées au large de Puerto Vallarta, célèbres pour la 'Plage Cachée'.",
    },
    facts: {
      de: ["Nationalpark und Vogelschutz", "Berühmter Playa del Amor", "Einstiges militärisches Testgelände"],
      hu: ["Szigorúan korlátozott látogatószám", "Kéklábú szulák fészkelőhelye", "Egykori katonai teszthelyszín"],
      ro: ["Adăpost pentru pasărea Sula cu picioare albastre", "Acces strict limitat pentru turiști", "Fost poligon de testare militară"],
      en: ["Home to blue-footed boobies", "Access is strictly limited", "Former military testing ground"],
      es: ["Parque Nacional y protección de aves", "Famosa Playa del Amor", "Antiguo campo de pruebas militares"],
      pt: ["Parque Nacional e proteção de aves", "Famosa Playa del Amor", "Antigo campo de testes militares"],
      fr: ["Parc national et protection des oiseaux", "Célèbre Playa del Amor", "Ancien terrain d'essais militaires"],
    }
  },
  {
    id: "contoy-island-life-v2", "sights": {
      "de": [
            {
                  "name": "Contoy Lighthouse",
                  "text": "Ein historischer Leuchtturm auf Contoy Island, der Schiffe in der Karibik leitet.",
                  "category": "landmark",
                  "coords": [
                        -86.802702,
                        21.528928
                  ]
            },
            {
                  "name": "Isla Contoy",
                  "text": "Ein geschütztes Insel-Naturschutzgebiet, bekannt für sein Vogelschutzgebiet und unberührte Strände.",
                  "category": "natural",
                  "coords": [
                        -86.794721,
                        21.494437
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Contoy Lighthouse",
                  "text": "Egy történelmi világítótorony Contoy szigetén, amely hajókat irányít a Karib-térségben.",
                  "category": "landmark",
                  "coords": [
                        -86.802702,
                        21.528928
                  ]
            },
            {
                  "name": "Isla Contoy",
                  "text": "Egy védett szigeti természetvédelmi terület, amely madárrezervátumáról és érintetlen strandjairól ismert.",
                  "category": "natural",
                  "coords": [
                        -86.794721,
                        21.494437
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Contoy Lighthouse",
                  "text": "Un far istoric pe Insula Contoy, care ghidează navele în Caraibe.",
                  "category": "landmark",
                  "coords": [
                        -86.802702,
                        21.528928
                  ]
            },
            {
                  "name": "Isla Contoy",
                  "text": "O rezervație naturală insulară protejată, cunoscută pentru sanctuarul de păsări și plajele nealterate.",
                  "category": "natural",
                  "coords": [
                        -86.794721,
                        21.494437
                  ]
            }
      ],
      "en": [
            {
                  "name": "Contoy Lighthouse",
                  "text": "A historic lighthouse on Contoy Island guiding ships in the Caribbean.",
                  "category": "landmark",
                  "coords": [
                        -86.802702,
                        21.528928
                  ]
            },
            {
                  "name": "Isla Contoy",
                  "text": "A protected island nature reserve known for its bird sanctuary and pristine beaches.",
                  "category": "natural",
                  "coords": [
                        -86.794721,
                        21.494437
                  ]
            }
      ]
},
    type: "island",
    parent: "MX-ROO",
    coords: [-86.78, 21.49],
    name: { de: "Isla Contoy", hu: "Contoy sziget", ro: "Insula Contoy", en: "Isla Contoy"},
    image: "/poi-images/contoy-island-life-v2.webp",
    descriptionAdvanced: {
      de: "Isla Contoy ist ein unberührtes Naturparadies im Norden der Riviera Maya und gilt als das wichtigste Vogelschutzgebiet der mexikanischen Karibik. Die kleine Insel ist permanent unbewohnt und der Zugang für Touristen ist auf nur 200 Personen pro Tag begrenzt, um das ökologische Gleichgewicht zu wahren. Hier treffen das Karibische Meer und der Golf von Mexiko aufeinander, was zu einem nährstoffreichen Wasser führt, das hunderte von Vogelarten und Seekühe anzieht. Die dichten Mangrovenwälder der Insel bieten sichere Nistplätze für Fregattvögel, Pelikane und Kormorane. Ein kleiner Aussichtsturm und ein ökologisches Museum informieren über die Bemühungen zum Schutz dieses fragilen Ökosystems. Isla Contoy ist ein Ort der Stille und ein Rückzugsort für das Leben in seiner reinsten Form. Geographie K7 - Küstenschutz und marine Reservate.",
      hu: "Contoy sziget egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Contoy sziget kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Insula Contoy este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Insula Contoy este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Isla Contoy is a tiny, pristine island located north of Isla Mujeres, where the Caribbean Sea meets the Gulf of Mexico. Known as the 'Island of Birds,' it was declared a national park in 1998 and is the most important nesting site for seabirds in the Mexican Caribbean. The island is strictly protected, with only a limited number of visitors allowed each day, and no permanent residents other than a few scientists and park rangers. Its shallow, crystal-clear bays are perfect for spotting rays and sea turtles, while the inland lagoons are filled with thousands of frigates, pelicans, and cormorants. The island also features a small museum and an observation tower that provides panoramic views of the surrounding turquoise waters and the Ixcaché reef, which marks the beginning of the Great Mesoamerican Reef. Isla Contoy remains one of the last true virgin paradises in the region, offering a glimpse into what the Caribbean looked like before the era of mass tourism.",
      es: "Isla Contoy es un paraíso natural virgen en el norte de la Riviera Maya y se considera el santuario de aves más importante del Caribe mexicano. La pequeña isla está permanentemente deshabitada y el acceso para turistas está limitado a solo 200 personas por día para preservar el equilibrio ecológico. Aquí se encuentran el Mar Caribe y el Golfo de México, lo que resulta en aguas ricas en nutrientes que atraen a cientos de especies de aves y manatíes. Los densos manglares de la isla ofrecen sitios de anidación seguros para fragatas, pelícanos y cormoranes. Una pequeña torre de observación y un museo ecológico informan sobre los esfuerzos para proteger este frágil ecosistema. Isla Contoy es un lugar de silencio y un refugio para la vida en su forma más pura.",
      pt: "Isla Contoy é um paraíso natural intocado no norte da Riviera Maya e é considerada o santuário de aves mais importante do Caribe mexicano. A pequena ilha é permanentemente desabitada e o acesso para turistas é limitado a apenas 200 pessoas por dia para preservar o equilíbrio ecológico. Aqui, o Mar do Caribe e o Golfo do México se encontram, resultando em águas ricas em nutrientes que atraem centenas de espécies de aves e peixes-boi. Os densos manguezais da ilha oferecem locais de nidificação seguros para fragatas, pelicanos e cormorões. Uma pequena torre de observação e um museu ecológico informam sobre os esforços para proteger este frágil ecossistema. Isla Contoy é um lugar de silêncio e um refúgio para a vida em sua forma mais pura.",
      fr: "Isla Contoy est un paradis naturel vierge au nord de la Riviera Maya et est considérée comme la plus importante réserve ornithologique des Caraïbes mexicaines. La petite île est inhabitée en permanence et l'accès pour les touristes est limité à seulement 200 personnes par jour afin de préserver l'équilibre écologique. C'est ici que la mer des Caraïbes et le golfe du Mexique se rejoignent, ce qui donne une eau riche en nutriments qui attire des centaines d'espèces d'oiseaux et des lamantins. Les denses forêts de mangroves de l'île offrent des sites de nidification sûrs pour les frégates, les pélicans et les cormorans. Une petite tour d'observation et un musée écologique informent sur les efforts de protection de cet écosystème fragile. Isla Contoy est un lieu de silence et un refuge pour la vie dans sa forme la plus pure.",
    },
    factsAdvanced: {
      de: ["Die Insel ist nur etwa 8,5 Kilometer lang und an der schmalsten Stelle 20 Meter breit.", "Wurde bereits 1961 zum Naturschutzgebiet erklärt, das älteste in Quintana Roo.", "Beherbergt über 150 verschiedene Vogelarten, die hier nisten oder rasten.", "Schildkröten wie die Echte Karettschildkröte nutzen die Strände zur Eiablage.", "Das Ixlaché-Riff im Süden markiert den Beginn des Großen Maya-Riffs.", "Es gibt keine Hotels auf der Insel, nur eine kleine Forschungsstation.", "Regenwasser wird auf der Insel gesammelt und Solarenergie für den Betrieb genutzt."],
      hu: ["Contoy sziget sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Contoy sziget egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Insula Contoy ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Insula Contoy este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The island is only 8.5 kilometers long and covers an area of about 317 hectares.",
        "More than 150 different species of birds have been recorded on the island during various seasons.",
        "It is a major nesting site for three species of sea turtles: the hawksbill, green, and loggerhead.",
        "The number of visitors is strictly capped at 200 per day to preserve the island's fragile ecosystem.",
        "Isla Contoy is located at the point where the Caribbean current begins its journey northward as the Gulf Stream."
      ],
      es: ["La isla tiene solo unos 8,5 kilómetros de largo y 20 metros de ancho en su punto más estrecho.", "Fue declarada reserva natural en 1961, la más antigua de Quintana Roo.", "Alberga más de 150 especies de aves diferentes que anidan o descansan aquí.", "Tortugas como la tortuga carey utilizan las playas para desovar.", "El arrecife Ixlaché al sur marca el comienzo del Gran Arrecife Maya.", "No hay hoteles en la isla, solo una pequeña estación de investigación.", "Se recolecta agua de lluvia en la isla y se utiliza energía solar para su funcionamiento."],
      pt: ["A ilha tem apenas cerca de 8,5 quilômetros de comprimento e 20 metros de largura no ponto mais estreito.", "Foi declarada reserva natural em 1961, a mais antiga de Quintana Roo.", "Abriga mais de 150 espécies de aves diferentes que aninham ou descansam aqui.", "Tartarugas como a tartaruga-de-pente utilizam as praias para a postura de ovos.", "O recife Ixlaché, ao sul, marca o início do Grande Recife Maia.", "Não há hotéis na ilha, apenas uma pequena estação de pesquisa.", "A água da chuva é coletada na ilha e a energia solar é usada para operação."],
      fr: ["L'île ne mesure que 8,5 kilomètres de long et 20 mètres de large à l'endroit le plus étroit.", "Déclarée réserve naturelle dès 1961, la plus ancienne du Quintana Roo.", "Abrite plus de 150 espèces d'oiseaux différentes qui y nichent ou s'y reposent.", "Des tortues comme la tortue imbriquée utilisent les plages pour la ponte.", "Le récif d'Ixlaché au sud marque le début du Grand Récif Maya.", "Il n'y a pas d'hôtels sur l'île, seulement une petite station de recherche.", "L'eau de pluie est collectée sur l'île et l'énergie solaire est utilisée pour le fonctionnement."],
    }, image: "/poi-images/contoy-island-life-v2.webp",
    description: {
      de: "Eine geschützte Insel nördlich von Isla Mujeres, die als eines der wichtigsten Vogelschutzgebiete gilt.",
      hu: "Szigorúan védett nemzeti park Isla Mujeres-től északra, fontos madárfészkelő hely.",
      ro: "Parc național strict protejat la nord de Isla Mujeres, servind drept cel mai important loc de cuibărit pentru păsările marine.",
      en: "A strictly protected national park north of Isla Mujeres, serving as the most important nesting site for seabirds in the region.",
      es: "Una isla protegida al norte de Isla Mujeres, considerada uno de los santuarios de aves más importantes.",
      pt: "Uma ilha protegida ao norte de Isla Mujeres, considerada um dos santuários de aves mais importantes.",
      fr: "Une île protégée au nord d'Isla Mujeres, considérée comme l'une des plus importantes réserves ornithologiques.",
    },
    facts: {
      de: ["Nationalpark seit 1961", "Begrenzte Besucherzahl täglich", "Nistplatz für über 150 Vogelarten"],
      hu: ["Napi 200 fős látogatói korlát", "Több mint 150 madárfaj fészkel itt", "Fontos tengeri teknős fészkelőhely"],
      ro: ["Nelocuită de oameni", "Limită de 200 de vizitatori pe zi", "Peste 150 de specii de păsări înregistrate"],
      en: ["Known as the 'Island of Birds'", "Limited to 200 visitors per day", "Uninhabited by humans"],
      es: ["Parque Nacional desde 1961", "Número limitado de visitantes diarios", "Sitio de anidación para más de 150 aves"],
      pt: ["Parque Nacional desde 1961", "Número limitado de visitantes diários", "Local de nidificação para mais de 150 aves"],
      fr: ["Parc national depuis 1961", "Nombre limité de visiteurs par jour", "Site de nidification pour plus de 150 espèces"],
    }
  },
  {
    id: "tequila-agave-fields-life-v2", "sights": {
      "de": [
            {
                  "name": "Campo Agavero Turístico",
                  "text": "Das Campo Agavero Turístico liegt in den Agavenfeldern und bietet einen Panoramablick auf die blauen Agaven.",
                  "category": "landmark",
                  "coords": [
                        -103.819371,
                        20.878666
                  ]
            },
            {
                  "name": "Chacuaco",
                  "text": "Chacuaco ist ein historischer Mezcal-Brennofen aus Stein im Agavengebiet.",
                  "category": "historical",
                  "coords": [
                        -103.833031,
                        20.881973
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Campo Agavero Turístico",
                  "text": "A Campo Agavero Turístico az agavemezőkön fekszik, panorámás kilátással a kék agávékra.",
                  "category": "landmark",
                  "coords": [
                        -103.819371,
                        20.878666
                  ]
            },
            {
                  "name": "Chacuaco",
                  "text": "A Chacuaco egy történelmi kőből készült mezcalégető kemence az agaveterületen.",
                  "category": "historical",
                  "coords": [
                        -103.833031,
                        20.881973
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Campo Agavero Turístico",
                  "text": "Campo Agavero Turístico este situat în câmpurile de agave, oferind vedere panoramică asupra agavelor albastre.",
                  "category": "landmark",
                  "coords": [
                        -103.819371,
                        20.878666
                  ]
            },
            {
                  "name": "Chacuaco",
                  "text": "Chacuaco este un cuptor istoric de piatră pentru mezcal în zona agavelor.",
                  "category": "historical",
                  "coords": [
                        -103.833031,
                        20.881973
                  ]
            }
      ],
      "en": [
            {
                  "name": "Campo Agavero Turístico",
                  "text": "Campo Agavero Turístico is a tourist camp in the agave fields with panoramic views of blue agave.",
                  "category": "landmark",
                  "coords": [
                        -103.819371,
                        20.878666
                  ]
            },
            {
                  "name": "Chacuaco",
                  "text": "Chacuaco is a historic stone mezcal oven used for roasting agave in the agave fields.",
                  "category": "historical",
                  "coords": [
                        -103.833031,
                        20.881973
                  ]
            }
      ]
},
    type: "agriculture",
    parent: "MX-JAL",
    coords: [-103.83, 20.88],
    name: { de: "Agavenfelder von Tequila", hu: "Tequila agave-foldei", ro: "Campurile de agave din Tequila", en: "Tequila Agave Fields"},
    image: "/poi-images/tequila-agave-fields-life-v2.webp",
    descriptionAdvanced: {
      de: "Die Agavenlandschaft von Tequila im Bundesstaat Jalisco ist ein UNESCO-Welterbe und das Herzstück der mexikanischen Identität. Kilometerlange Felder mit der Blauen Agave (Agave tequilana) überziehen die Hänge des Vulkans Tequila und bilden ein faszinierendes blaugrünes Meer. Hier wird seit Jahrhunderten die Kunst der Tequila-Herstellung perfektioniert, die eng mit der Geschichte und den Traditionen der Region verknüpft ist. Die Arbeit der 'Jimadores', die die schweren Agavenherzen mit traditionellen Werkzeugen ernten, ist ein beeindruckendes Zeugnis menschlicher Ausdauer. Die Landschaft umfasst auch die alten Industrieanlagen und Brennereien, die oft in prächtigen Haciendas untergebracht sind. Tequila ist nicht nur ein Getränk, sondern ein Symbol für das kulturelle Erbe Mexikos. Geschichte K8 - Landwirtschaftliche Traditionen und Welterbe.",
      hu: "Tequila agave-foldei egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Tequila agave-foldei kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Campurile de agave din Tequila este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Campurile de agave din Tequila este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The Agave Landscape of Tequila, located in the state of Jalisco, is a stunning UNESCO World Heritage site that celebrates the cultural and agricultural heritage of Mexico's most famous spirit. The region is characterized by vast, undulating fields of blue agave (Agave tequilana), whose distinctive blue-green leaves create a striking contrast against the red volcanic soil. These fields have been cultivated for centuries, and the traditional methods of harvesting, performed by skilled laborers known as 'jimadores,' remain largely unchanged. The landscape also includes the town of Tequila itself, with its historic distilleries and haciendas that date back to the 18th century. Visitors can tour the fields to learn about the long growth cycle of the agave plant, which takes 7 to 10 years to reach maturity. The entire region is a testament to the fusion of indigenous traditions and European distillation techniques, which together created a drink that is now a global symbol of Mexican identity.",
      es: "El paisaje agavero de Tequila, en el estado de Jalisco, es Patrimonio de la Humanidad por la UNESCO y la pieza central de la identidad mexicana. Kilómetros de campos con Agave Azul (Agave tequilana) cubren las laderas del volcán Tequila, formando un fascinante mar azul verdoso. Aquí se ha perfeccionado durante siglos el arte de la producción de tequila, íntimamente ligado a la historia y las tradiciones de la región. El trabajo de los 'jimadores', que cosechan los pesados corazones de agave con herramientas tradicionales, es un impresionante testimonio de la resistencia humana. El paisaje también incluye antiguas instalaciones industriales y destilerías, a menudo alojadas en magníficas haciendas. El tequila no es solo una bebida, sino un símbolo del patrimonio cultural de México.",
      pt: "A paisagem de agave de Tequila, no estado de Jalisco, é Patrimônio Mundial da UNESCO e a peça central da identidade mexicana. Quilômetros de campos com o Agave Azul (Agave tequilana) cobrem as encostas do vulcão Tequila, formando um fascinante mar azul-esverdeado. Aqui, a arte da produção de tequila tem sido aperfeiçoada há séculos, intimamente ligada à história e às tradições da região. O trabalho dos 'jimadores', que colhem os pesados corações de agave com ferramentas tradicionais, é um testemunho impressionante da resistência humana. A paisagem também inclui as antigas instalações industriais e destilarias, muitas vezes instaladas em magníficas fazendas. Tequila não é apenas uma bebida, mas um símbolo da herança cultural do México.",
      fr: "Le paysage d'agaves de Tequila, dans l'État du Jalisco, est classé au patrimoine mondial de l'UNESCO et constitue le cœur de l'identité mexicaine. Des kilomètres de champs d'agave bleu (Agave tequilana) recouvrent les flancs du volcan Tequila, formant une fascinante mer bleu-vert. C'est ici que l'art de la fabrication de la tequila est perfectionné depuis des siècles, étroitement lié à l'histoire et aux traditions de la région. Le travail des « jimadores », qui récoltent les lourds cœurs d'agave avec des outils traditionnels, est un témoignage impressionnant de l'endurance humaine. Le paysage comprend également les anciennes installations industrielles et les distilleries, souvent installées dans de magnifiques haciendas. La tequila n'est pas seulement une boisson, mais un symbole du patrimoine culturel du Mexique.",
    },
    factsAdvanced: {
      de: ["Die Agavenlandschaft wurde 2006 in die Liste des UNESCO-Welterbes aufgenommen.", "Nur Destillate aus der Blauen Agave dürfen die geschützte Bezeichnung 'Tequila' tragen.", "Eine Agave benötigt zwischen 6 und 10 Jahre, um die Erntereife zu erreichen.", "Das Agavenherz, aus dem der Saft gewonnen wird, wird 'Piña' (Ananas) genannt.", "Die Region umfasst die Städte Tequila, Arenal und Amatitán.", "Der Vulkan Tequila lieferte die fruchtbaren Obsidianböden für die Pflanzen.", "Tequila war das erste Produkt Mexikos mit einer geschützten Herkunftsbezeichnung."],
      hu: ["Tequila agave-foldei sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Tequila agave-foldei egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Campurile de agave din Tequila ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Campurile de agave din Tequila este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The Agave Landscape and Ancient Industrial Facilities of Tequila were inscribed as a UNESCO site in 2006.",
        "The red volcanic soil of the region is particularly rich in iron and minerals, which gives Tequila its unique flavor profile.",
        "A single agave plant can weigh between 40 and 100 kilograms when it is ready for harvest.",
        "The 'piña,' or heart of the agave, is the part used to produce the spirit after the leaves are trimmed away.",
        "Tequila is a protected 'Designation of Origin,' meaning it can only be legally produced in certain regions of Mexico."
      ],
      es: ["El paisaje agavero fue incluido en la lista del Patrimonio Mundial de la UNESCO en 2006.", "Solo los destilados de Agave Azul pueden llevar la denominación protegida 'Tequila'.", "Un agave necesita entre 6 y 10 años para alcanzar su madurez de cosecha.", "El corazón del agave, del cual se extrae el jugo, se llama 'piña'.", "La región incluye las ciudades de Tequila, Arenal y Amatitán.", "El volcán Tequila proporcionó los fértiles suelos de obsidiana para las plantas.", "El tequila fue el primer producto de México con una denominación de origen protegida."],
      pt: ["A paisagem do agave foi incluída na lista do Patrimônio Mundial da UNESCO em 2006.", "Apenas destilados de Agave Azul podem ostentar a denominação protegida 'Tequila'.", "Um agave necessita entre 6 e 10 anos para atingir a maturidade para a colheita.", "O coração do agave, do qual é extraído o suco, é chamado de 'piña' (abacaxi).", "A região abrange as cidades de Tequila, Arenal e Amatitán.", "O vulcão Tequila forneceu os solos férteis de obsidiana para as plantas.", "O tequila foi o primeiro produto do México com uma denominação de origem protegida."],
      fr: ["Le paysage d'agaves a été inscrit sur la liste du patrimoine mondial de l'UNESCO en 2006.", "Seuls les distillats issus de l'agave bleu peuvent porter l'appellation protégée 'Tequila'.", "Un agave a besoin de 6 à 10 ans pour atteindre la maturité de récolte.", "Le cœur de l'agave, dont on extrait le jus, est appelé 'piña' (ananas).", "La région comprend les villes de Tequila, Arenal et Amatitán.", "Le volcan Tequila a fourni les sols d'obsidienne fertiles pour les plantes.", "La tequila a été le premier produit mexicain doté d'une appellation d'origine protégée."],
    }, image: "/poi-images/tequila-agave-fields-life-v2.webp",
    description: {
      de: "Die weiten, blauen Agavenfelder in Jalisco, die das Herz der Tequila-Produktion bilden.",
      hu: "Jalisco állam kék agávé ültetvényei, amelyek az UNESCO világörökség részét képezik.",
      ro: "Peisaj înscris în patrimoniul UNESCO în Jalisco, unde agava albastră este cultivată de secole pentru producția de tequila.",
      en: "A UNESCO World Heritage landscape in Jalisco where the blue agave has been cultivated for centuries to produce tequila.",
      es: "Los extensos campos de agave azul en Jalisco, que constituyen el corazón de la producción de Tequila.",
      pt: "Os vastos campos de agave azul em Jalisco, que formam o coração da produção de Tequila.",
      fr: "Les vastes champs d'agave bleu dans le Jalisco, qui constituent le cœur de la production de Tequila.",
    },
    facts: {
      de: ["UNESCO-Weltkulturerbe", "Anbau der Blauen Agave", "Vulkanische Bodenbeschaffenheit"],
      hu: ["UNESCO világörökségi kultúrtáj", "7-10 év az agávé beérése", "Jellegzetes vörös vulkáni talaj"],
      ro: ["Sol vulcanic roșu bogat în minerale", "Agava are nevoie de 7-10 ani pentru maturare", "Sit UNESCO din anul 2006"],
      en: ["Rich volcanic red soil", "Agave takes 7-10 years to mature", "UNSECO site since 2006"],
      es: ["Patrimonio de la Humanidad UNESCO", "Cultivo del Agave Azul", "Suelo de origen volcánico"],
      pt: ["Patrimônio Mundial da UNESCO", "Cultivo do Agave Azul", "Composição do solo vulcânico"],
      fr: ["Patrimoine mondial de l'UNESCO", "Culture de l'agave bleu", "Composition volcanique du sol"],
    }
  },
  {
    id: "valle-de-guadalupe-vineyards-life-v2",
    type: "agriculture",
    parent: "MX-BCN",
    coords: [-116.52, 32.06],
    name: { de: "Valle de Guadalupe", hu: "Valle de Guadalupe", ro: "Valea Guadalupe", en: "Valle de Guadalupe"},
    image: "/poi-images/valle-de-guadalupe-vineyards-life-v2.webp",
    descriptionAdvanced: {
      de: "Das Valle de Guadalupe in Baja California ist die wichtigste Weinregion Mexikos und wird oft als das 'Napa Valley von Mexiko' bezeichnet. In diesem sonnenverwöhnten Tal werden über 90 % des mexikanischen Weins produziert, wobei das mediterrane Klima ideale Bedingungen für Rebsorten wie Cabernet Sauvignon, Nebbiolo und Tempranillo bietet. Die Landschaft ist geprägt von endlosen Reihen von Weinstöcken, modernen Boutique-Weingütern und rustikalen Haciendas vor der Kulisse trockener Berge. Neben dem Weinbau hat sich das Tal zu einem kulinarischen Zentrum für die 'Baja Med'-Küche entwickelt, die lokale Meeresfrüchte mit mediterranen Einflüssen kombiniert. Ein Besuch im Tal bietet eine genussvolle Reise durch die moderne mexikanische Weinkultur und zeigt die Innovationskraft der lokalen Winzer. Geographie K7 - Agrarregionen und spezialisierter Anbau.",
      hu: "Valle de Guadalupe egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Valle de Guadalupe kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Valea Guadalupe este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Valea Guadalupe este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Valle de Guadalupe, located in the northern part of Baja California near Ensenada, is the premier wine-producing region of Mexico, often referred to as the 'Napa Valley of Mexico.' This Mediterranean-style valley produces nearly 90% of the country's wine, benefitting from a unique microclimate of warm days and cool, misty nights from the Pacific Ocean. The region is home to over 150 wineries, ranging from large, established producers to small, experimental boutique vineyards. Beyond the wine, Valle de Guadalupe has become a major culinary destination, famous for its 'Baja-Med' cuisine that emphasizes fresh, locally sourced ingredients. The landscape is dotted with stylish tasting rooms, eco-lofts, and outdoor restaurants that offer stunning views of the vine-covered hills. The annual 'Fiestas de la Vendimia' in August celebrate the harvest with concerts, tastings, and gala dinners, attracting wine enthusiasts from around the world. It is a place where modern sophistication and rustic charm blend seamlessly in a spectacular natural setting.",
      es: "El Valle de Guadalupe en Baja California es la región vinícola más importante de México y a menudo se le llama el 'Napa Valley de México'. En este valle bañado por el sol se produce más del 90 % del vino mexicano, donde el clima mediterráneo ofrece condiciones ideales para variedades como Cabernet Sauvignon, Nebbiolo y Tempranillo. El paisaje está marcado por interminables hileras de vides, modernas bodegas boutique y haciendas rústicas con montañas secas de fondo. Además de la viticultura, el valle se ha convertido en un centro culinario para la cocina 'Baja Med', que combina mariscos locales con influencias mediterráneas. Una visita al valle ofrece un viaje placentero por la moderna cultura del vino mexicano y muestra la capacidad de innovación de los productores locales.",
      pt: "O Valle de Guadalupe, na Baja California, é a região vinícola mais importante do México e é frequentemente chamado de 'Napa Valley do México'. Neste vale ensolarado, são produzidos mais de 90% do vinho mexicano, onde o clima mediterrâneo oferece condições ideais para variedades como Cabernet Sauvignon, Nebbiolo e Tempranillo. A paisagem é marcada por fileiras intermináveis de videiras, vinícolas boutique modernas e fazendas rústicas com montanhas secas ao fundo. Além da viticultura, o vale tornou-se um centro culinário para a cozinha 'Baja Med', que combina frutos do mar locais com influências mediterrâneas. Uma visita ao vale oferece uma jornada prazerosa pela moderna cultura vinícola mexicana e mostra a força inovadora dos produtores locais.",
      fr: "Le Valle de Guadalupe, en Basse-Californie, est la principale région viticole du Mexique, souvent surnommée la 'Napa Valley du Mexique'. Plus de 90 % du vin mexicain est produit dans cette vallée ensoleillée, où le climat méditerranéen offre des conditions idéales pour des cépages tels que le Cabernet Sauvignon, le Nebbiolo et le Tempranillo. Le paysage est caractérisé par des rangées infinies de vignes, des domaines viticoles modernes et des haciendas rustiques sur fond de montagnes arides. Outre la viticulture, la vallée est devenue un centre culinaire pour la cuisine 'Baja Med', qui combine les produits de la mer locaux avec des influences méditerranéennes. Une visite de la vallée propose un voyage savoureux à travers la culture viticole mexicaine moderne.",
    },
    factsAdvanced: {
      de: ["Das Tal produziert jährlich mehrere Millionen Liter Wein höchster Qualität.", "Die ersten Weinreben wurden hier bereits im 18. Jahrhundert von Jesuiten gepflanzt.", "Russische Molokanen-Siedler spielten im frühen 20. Jahrhundert eine wichtige Rolle.", "Es beherbergt über 150 Weingüter, von großen Kellereien bis zu Mikro-Winzern.", "Die 'Ruta del Vino' ist eine der bekanntesten Tourismusrouten Nordmexikos.", "Das Klima ist durch heiße Tage und kühle Nächte vom nahen Pazifik geprägt.", "Im Tal befindet sich das mexikanische Weinmuseum (Museo de la Vid y el Vino)."],
      hu: ["Valle de Guadalupe sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Valle de Guadalupe egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Valea Guadalupe ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Valea Guadalupe este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "The first vineyards in the valley were established by Jesuit missionaries in the 18th century.",
        "The region is known for its incredible variety of grapes, including Nebbiolo, Tempranillo, Cabernet Sauvignon, and Chenin Blanc.",
        "Valle de Guadalupe is located about 30 kilometers inland from the Pacific port city of Ensenada.",
        "The valley produces approximately 2 million cases of wine per year.",
        "The 'Wine Route' (Ruta del Vino) through the valley is one of the most popular tourist drives in Baja California."
      ],
      es: ["El valle produce anualmente varios millones de litros de vino de la más alta calidad.", "Las primeras vides fueron plantadas aquí en el siglo XVIII por jesuitas.", "Los colonos rusos Molokanes jugaron un papel importante a principios del siglo XX.", "Alberga más de 150 bodegas, desde grandes cavas hasta microvinicultores.", "La 'Ruta del Vino' es una de las rutas turísticas más conocidas del norte de México.", "El clima se caracteriza por días calurosos y noches frescas gracias al cercano Pacífico.", "En el valle se encuentra el Museo de la Vid y el Vino."],
      pt: ["O vale produz anualmente vários milhões de litros de vinho da mais alta qualidade.", "As primeiras videiras foram plantadas aqui já no século XVIII por Jesuítas.", "Colonos russos Molokans desempenharam um papel importante no início do século XX.", "Abriga mais de 150 vinícolas, desde grandes caves até microprodutores.", "A 'Ruta del Vino' é uma das rotas turísticas mais conhecidas do norte do México.", "O clima é caracterizado por dias quentes e noites frias vindas do Pacífico próximo.", "No vale localiza-se o museu do vinho mexicano (Museo de la Vid y el Vino)."],
      fr: ["La vallée produit chaque année plusieurs millions de litres de vin de haute qualité.", "Les premières vignes ont été plantées ici dès le XVIIIe siècle par des Jésuites.", "Les colons russes Molokans ont joué un rôle important au début du XXe siècle.", "Elle abrite plus de 150 domaines, des grandes caves aux micro-vignerons.", "La 'Ruta del Vino' est l'un des itinéraires touristiques les plus célèbres du nord du Mexique.", "Le climat est marqué par des journées chaudes et des nuits fraîches grâce au Pacifique proche.", "La vallée abrite le musée du vin mexicain (Museo de la Vid y el Vino)."],
    }, image: "/poi-images/valle-de-guadalupe-vineyards-life-v2.webp",
    description: {
      de: "Das wichtigste Weinanbaugebiet Mexikos in Baja California, bekannt für seine erstklassigen Weingüter.",
      hu: "Mexikó legfontosabb borvidéke Alsó-Kaliforniában, a hazai bortermelés központja.",
      ro: "Principala regiune viticolă a Mexicului situată în Baja California, producând aproape 90% din vinul țării.",
      en: "The premier wine region of Mexico, located in Baja California and producing nearly 90% of the country's wine.",
      es: "La región vitivinícola más importante de México en Baja California, famosa por sus bodegas de clase mundial.",
      pt: "A região vinícola mais importante do México na Baja California, conhecida pelas suas vinícolas de classe mundial.",
      fr: "La plus importante région viticole du Mexique en Basse-Californie, réputée pour ses domaines de classe mondiale.",
    },
    facts: {
      de: ["Produziert 90% des mexikanischen Weins", "Mediterranes Mikroklima", "Zentrum der Valle-Gastronomie"],
      hu: ["Mediterrán mikroklíma", "Több mint 150 borászat", "Mexikó gasztronómiai központja"],
      ro: ["Microclimat de tip mediteranean", "Peste 150 de crame în regiune", "Destinație culinară de top în Mexic"],
      en: ["Mediterranean-style microclimate", "Home to over 150 wineries", "Mexico's leading culinary destination"],
      es: ["Produce el 90% del vino mexicano", "Microclima mediterráneo", "Centro de la gastronomía del Valle"],
      pt: ["Produz 90% do vinho mexicano", "Microclima mediterrâneo", "Centro da gastronomia do Vale"],
      fr: ["Produit 90 % du vin mexicain", "Microclimat méditerranéen", "Centre de la gastronomie de la vallée"],
    }
  },
  {
    id: "veracruz-coffee-farms-life-v2",
    type: "agriculture",
    parent: "MX-VER",
    coords: [-96.13, 19.52],
    name: { de: "Kaffeefarmen von Veracruz", hu: "Veracruz kavefarmjai", ro: "Fermele de cafea din Veracruz", en: "Veracruz Coffee Farms"},
    image: "/poi-images/veracruz-coffee-farms-life-v2.webp",
    descriptionAdvanced: {
      de: "Die Kaffeefarmen von Veracruz, insbesondere rund um die Städte Coatepec und Xalapa, gehören zu den traditionsreichsten und besten Anbaugebieten Mexikos. In diesen nebligen Hochlandregionen gedeiht der Kaffee im Schatten hoher Urwaldbäume, was ihm ein einzigartiges Aroma und eine feine Säure verleiht. Der Anbau ist eng mit der lokalen Geschichte verknüpft, und viele der alten Fincas stammen noch aus dem 19. Jahrhundert. Besucher können den gesamten Prozess von der Ernte der roten Kaffeekirschen bis zur Röstung miterleben. Coatepec wurde aufgrund seiner bedeutenden Kaffeekultur als 'Pueblo Mágico' ausgezeichnet. Der Duft von frisch geröstetem Kaffee liegt hier ständig in der Luft und erzählt von der Leidenschaft der Menschen für ihr wichtigstes Exportgut. Sachkunde K5 - Nutzpflanzen und Welthandel.",
      hu: "Veracruz kavefarmjai egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Veracruz kavefarmjai kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Fermele de cafea din Veracruz este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Fermele de cafea din Veracruz este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The coffee-growing regions of Veracruz, particularly around towns like Coatepec and Xalapa, are famous for producing some of the smoothest and most aromatic beans in Mexico. The high altitude, volcanic soil, and frequent cloud cover of the Sierra Madre Oriental provide the perfect conditions for 'coffee under shade,' a traditional method that preserves the local biodiversity. Veracruz was the first place in Mexico where coffee was planted in the late 18th century, and the region has maintained a deep-rooted coffee culture ever since. Visitors can explore historic 'fincas' (estates) to see the entire process from flowering and cherry picking to roasting and tasting. The architecture of the region, with its colonial houses and lush gardens, reflects the wealth and history brought by the coffee trade. A cup of Veracruz coffee is typically characterized by a balanced body with notes of chocolate and fruit, making it a favorite both domestically and abroad. The commitment of local farmers to quality and tradition ensures that Veracruz remains at the heart of Mexico's coffee identity.",
      es: "Las fincas cafetaleras de Veracruz, especialmente alrededor de las ciudades de Coatepec y Xalapa, pertenecen a las regiones de cultivo más tradicionales y mejores de México. En estas brumosas regiones de tierras altas, el café crece a la sombra de altos árboles de la selva, lo que le otorga un aroma único y una acidez fina. El cultivo está estrechamente vinculado a la historia local, y muchas de las antiguas fincas datan del siglo XIX. Los visitantes pueden experimentar todo el proceso, desde la cosecha de las cerezas rojas de café hasta el tostado. Coatepec fue distinguido como 'Pueblo Mágico' debido a su significativa cultura cafetalera. El aroma del café recién tostado flota constantemente en el aire y habla de la pasión de la gente por su principal producto de exportación.",
      pt: "As fazendas de café de Veracruz, especialmente em torno das cidades de Coatepec e Xalapa, pertencem às regiões de cultivo mais tradicionais e melhores do México. Nestas regiões montanhosas nubladas, o café cresce à sombra de altas árvores da floresta, o que lhe confere um aroma único e uma acidez fina. O cultivo está estreitamente ligado à história local, e muitas das antigas fazendas datam do século XIX. Os visitantes podem vivenciar todo o processo, desde a colheita das cerejas vermelhas do café até a torrefação. Coatepec foi distinguido como 'Pueblo Mágico' devido à sua significativa cultura cafeeira. O aroma de café acabado de torrar está constantemente no ar aqui, contando sobre a paixão das pessoas pelo seu produto de exportação mais importante.",
      fr: "Les fermes de café de Veracruz, notamment autour des villes de Coatepec et Xalapa, comptent parmi les zones de culture les plus traditionnelles et les meilleures du Mexique. Dans ces régions montagneuses brumeuses, le café s'épanouit à l'ombre de grands arbres tropicaux, ce qui lui confère un arôme unique et une acidité fine. La culture est étroitement liée à l'histoire locale, et bon nombre des anciennes fincas datent encore du XIXe siècle. Les visiteurs peuvent suivre l'ensemble du processus, de la récolte des cerises de café rouges à la torréfaction. Coatepec a été classé « Pueblo Mágico » en raison de son importante culture du café. L'odeur du café fraîchement torréfié flotte constamment dans l'air, témoignant de la passion des habitants pour leur principal produit d'exportation.",
    },
    factsAdvanced: {
      de: ["Veracruz war der erste Ort in Mexiko, an dem im 18. Jahrhundert Kaffee gepflanzt wurde.", "Coatepec ist weltweit bekannt für seinen hochwertigen Arabica-Kaffee.", "Der Kaffee wird hier meist auf einer Höhe zwischen 800 und 1.200 Metern angebaut.", "Der Schattenanbau schützt die Biodiversität und bietet Lebensraum für Vögel.", "Mexiko ist einer der weltweit größten Produzenten von biologisch angebautem Kaffee.", "Die Erntezeit in Veracruz liegt traditionell zwischen November und März.", "Das 'Museo del Café' in Coatepec zeigt antike Maschinen zur Kaffeeverarbeitung."],
      hu: ["Veracruz kavefarmjai sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Veracruz kavefarmjai egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Fermele de cafea din Veracruz ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Fermele de cafea din Veracruz este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Coatepec, known as the 'Coffee Capital of Mexico,' was designated a 'Pueblo Mágico' for its beauty and tradition.",
        "Veracruz is the second-largest producer of coffee in Mexico, after Chiapas.",
        "The first coffee seeds were brought to Veracruz from Cuba in 1790.",
        "Much of the coffee in Veracruz is grown at elevations between 800 and 1,500 meters above sea level.",
        "The region is known for its high-quality Arabica beans, which are often exported to gourmet markets worldwide."
      ],
      es: ["Veracruz fue el primer lugar en México donde se plantó café en el siglo XVIII.", "Coatepec es mundialmente conocido por su café arábica de alta calidad.", "El café se cultiva aquí principalmente a una altitud de entre 800 y 1,200 metros.", "El cultivo bajo sombra protege la biodiversidad y ofrece hábitat a las aves.", "México es uno de los mayores productores mundiales de café orgánico.", "La temporada de cosecha en Veracruz es tradicionalmente entre noviembre y marzo.", "El 'Museo del Café' en Coatepec muestra maquinaria antigua para el procesamiento del café."],
      pt: ["Veracruz foi o primeiro lugar no México onde o café foi plantado no século XVIII.", "Coatepec é mundialmente conhecido pelo seu café arábica de alta qualidade.", "O café é cultivado aqui principalmente a uma altitude entre 800 e 1.200 metros.", "O cultivo à sombra protege a biodiversidade e oferece habitat para aves.", "O México é um dos maiores produtores mundiais de café cultivado organicamente.", "A época da colheita em Veracruz é tradicionalmente entre novembro e março.", "O 'Museo del Café' em Coatepec exibe máquinas antigas para o processamento de café."],
      fr: ["Veracruz a été le premier endroit au Mexique où le café a été planté au XVIIIe siècle.", "Coatepec est mondialement connue pour son café Arabica de haute qualité.", "Le café y est principalement cultivé à une altitude comprise entre 800 et 1 200 mètres.", "La culture sous ombre protège la biodiversité et offre un habitat aux oiseaux.", "Le Mexique est l'un des plus grands producteurs mondiaux de café biologique.", "La période de récolte à Veracruz se situe traditionnellement entre novembre et mars.", "Le 'Museo del Café' à Coatepec expose des machines anciennes pour le traitement du café."],
    }, image: "/poi-images/veracruz-coffee-farms-life-v2.webp",
    description: {
      de: "Traditionelle Kaffeeplantagen in den feuchten Bergregionen von Veracruz, bekannt für ihren Hochlandkaffee.",
      hu: "Veracruz ködös hegyvidékein fekvő tradicionális kávéültetvények, ahol árnyékban termett kávét állítanak elő.",
      ro: "Ferme istorice de cafea în zonele muntoase din Veracruz, unde se produce una dintre cele mai fine cafele de umbră din Mexic.",
      en: "Historic coffee-growing estates in the misty highlands of Veracruz, where some of Mexico's finest shade-grown coffee is produced.",
      es: "Plantaciones de café tradicionales en las húmedas regiones montañosas de Veracruz, famosas por su café de altura.",
      pt: "Plantações de café tradicionais nas regiões montanhosas úmidas de Veracruz, conhecidas pelo seu café de altitude.",
      fr: "Plantations de café traditionnelles dans les régions montagneuses humides de Veracruz, réputées pour leur café d'altitude.",
    },
    facts: {
      de: ["Anbau in Coatepec und Xalapa", "Schattenkaffee-Tradition", "Reiche koloniale Fincas"],
      hu: ["Coatepec a kávé fővárosa", "Mexikó első kávéültetvényei", "Magaslati, vulkanikus talaj"],
      ro: ["Coatepec este capitala cafelei mexicane", "Primul loc din Mexic unde s-a cultivat cafea", "Cultivată la altitudini de peste 800 de metri"],
      en: ["Coatepec is the 'Coffee Capital'", "First place in Mexico to grow coffee", "Grown in the Sierra Madre Oriental"],
      es: ["Cultivo en Coatepec y Xalapa", "Tradición de café de sombra", "Ricas fincas coloniales"],
      pt: ["Cultivo em Coatepec e Xalapa", "Tradição de café sombreado", "Ricas fazendas coloniais"],
      fr: ["Culture à Coatepec et Xalapa", "Tradition du café d'ombre", "Riches fincas coloniales"],
    }
  },
  {
    id: "chiapas-coffee-fincas-life-v2",
    type: "agriculture",
    parent: "MX-CHP",
    coords: [-92.25, 15.85],
    name: { de: "Kaffeefincas in Chiapas", hu: "Chiapas kavefincai", ro: "Fincile de cafea din Chiapas", en: "Chiapas Coffee Fincas"},
    image: "/poi-images/chiapas-coffee-fincas-life-v2.webp",
    descriptionAdvanced: {
      de: "Chiapas ist das größte Kaffeeanbaugebiet Mexikos und bekannt für seine kräftigen, oft biologisch zertifizierten Kaffeesorten aus dem Hochland der Sierra Madre. Die Fincas liegen oft versteckt in nebligen Bergwäldern und werden häufig von indigenen Kooperativen bewirtschaftet. Diese Gemeinschaften kombinieren traditionelles Wissen mit modernen ökologischen Standards, um einen Kaffee von Weltruf zu produzieren. Die Region Soconusco im Süden von Chiapas gilt als Wiege des mexikanischen Kaffees und beherbergt historische Farmen, die teilweise sogar Gästezimmer für Agrotourismus anbieten. Der Kaffee aus Chiapas ist nicht nur ein Genussmittel, sondern eine lebenswichtige Einkommensquelle, die den Erhalt der Wälder unterstützt. Ein Besuch auf einer Finca bietet tiefe Einblicke in das soziale und ökologische Gefüge der Region. Biologie K7 - Nachhaltige Landwirtschaft und Hochlandökosysteme.",
      hu: "Chiapas kavefincai egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Chiapas kavefincai kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Fincile de cafea din Chiapas este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Fincile de cafea din Chiapas este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Chiapas is the powerhouse of Mexican coffee production, with its rugged southern highlands providing an ideal environment for high-quality Arabica beans. The region is famous for its 'Strictly High Grown' (SHG) coffee, which matures slowly at elevations above 1,200 meters, resulting in a complex flavor profile with bright acidity and floral notes. Much of the coffee in Chiapas is produced by smallholder farmers and indigenous cooperatives, many of which have become global leaders in organic and fair-trade certification. The historic fincas of the Soconusco region, near the Guatemalan border, offer a glimpse into the late 19th-century coffee boom, with some estates still operating as both farms and boutique hotels. The humid tropical climate and volcanic soil of the Sierra Madre de Chiapas contribute to the region's exceptional biodiversity, where coffee is often grown under a canopy of native trees. Drinking Chiapas coffee is not just a sensory experience but also a way to support the sustainable livelihoods of thousands of mountain communities.",
      es: "Chiapas es la región cafetalera más grande de México, conocida por sus variedades de café fuertes y a menudo certificadas como orgánicas de las tierras altas de la Sierra Madre. Las fincas suelen estar escondidas en bosques nubosos y frecuentemente son gestionadas por cooperativas indígenas. Estas comunidades combinan el conocimiento tradicional con estándares ecológicos modernos para producir un café de renombre mundial. La región del Soconusco, al sur de Chiapas, es considerada la cuna del café mexicano y alberga granjas históricas que a veces ofrecen alojamiento para el agroturismo. El café de Chiapas no es solo un producto de consumo, sino una fuente vital de ingresos que apoya la conservación de los bosques. Una visita a una finca ofrece una visión profunda del tejido social y ecológico de la región.",
      pt: "Chiapas é a maior região produtora de café do México, conhecida por suas variedades de café fortes e frequentemente com certificação orgânica das terras altas da Sierra Madre. As fazendas costumam estar escondidas em florestas nubladas e são frequentemente geridas por cooperativas indígenas. Estas comunidades combinam o conhecimento tradicional com padrões ecológicos modernos para produzir um café de renome mundial. A região de Soconusco, no sul de Chiapas, é considerada o berço do café mexicano e abriga fazendas históricas que, por vezes, oferecem quartos para agroturismo. O café de Chiapas não é apenas um produto de consumo, mas uma fonte vital de renda que apoia a preservação das florestas. Uma visita a uma fazenda oferece uma visão profunda da estrutura social e ecológica da região.",
      fr: "Le Chiapas est la plus grande région productrice de café du Mexique, connue pour ses variétés de café corsées et souvent certifiées biologiques, issues des hautes terres de la Sierra Madre. Les fincas sont souvent cachées dans des forêts de nuages brumeuses et sont fréquemment exploitées par des coopératives indigènes. Ces communautés combinent le savoir traditionnel avec des normes écologiques modernes pour produire un café de renommée mondiale. La région du Soconusco, au sud du Chiapas, est considérée comme le berceau du café mexicain et abrite des fermes historiques qui proposent parfois des chambres d'hôtes pour l'agrotourisme. Le café du Chiapas n'est pas seulement un produit de plaisir, mais une source de revenus vitale qui soutient la préservation des forêts. Une visite dans une finca offre un aperçu profond du tissu social et écologique de la région.",
    },
    factsAdvanced: {
      de: ["Chiapas trägt etwa ein Drittel zur gesamten Kaffeeproduktion Mexikos bei.", "Viele Farmen liegen auf einer Höhe von bis zu 1.600 Metern über dem Meeresspiegel.", "Die 'Ruta del Café' führt zu historischen Fincas wie Irlandia oder Argovia.", "Vulkanische Böden der Region verleihen dem Kaffee seine besondere Note.", "Der Anbau erfolgt fast ausschließlich in Handarbeit an den steilen Berghängen.", "Chiapas ist ein weltweit führendes Zentrum für Fair-Trade-zertifizierten Kaffee.", "Die Farmen sind oft wichtige Rückzugsorte für Zugvögel aus Nordamerika."],
      hu: ["Chiapas kavefincai sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Chiapas kavefincai egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Fincile de cafea din Chiapas ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Fincile de cafea din Chiapas este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Chiapas produces more than 40% of all the coffee grown in Mexico.",
        "The state is a world leader in the production of organic coffee, with thousands of certified small producers.",
        "The Soconusco region has a unique history of German and Italian immigrants who established the first large coffee estates.",
        "The coffee harvest in Chiapas typically runs from November to March.",
        "Coffee from the El Triunfo Biosphere Reserve in Chiapas is highly prized by specialty roasters for its environmental and flavor credentials."
      ],
      es: ["Chiapas aporta aproximadamente un tercio de la producción total de café en México.", "Muchas granjas están situadas a una altitud de hasta 1.600 metros sobre el nivel del mar.", "La 'Ruta del Café' conduce a fincas históricas como Irlandia o Argovia.", "Los suelos volcánicos de la región otorgan al café su nota especial.", "El cultivo se realiza casi exclusivamente a mano en las laderas empinadas.", "Chiapas es un centro líder mundial para el café certificado de comercio justo.", "Las granjas suelen ser refugios importantes para aves migratorias de América del Norte."],
      pt: ["Chiapas contribui com cerca de um terço da produção total de café no México.", "Muitas fazendas estão situadas a uma altitude de até 1.600 metros acima do nível do mar.", "A 'Ruta del Café' leva a fazendas históricas como Irlandia ou Argovia.", "Os solos vulcânicos da região conferem ao café sua nota especial.", "O cultivo é realizado quase exclusivamente à mão nas encostas íngremes das montanhas.", "Chiapas é um centro líder mundial para café com certificação de comércio justo.", "As fazendas são frequentemente refúgios importantes para aves migratórias da América do Norte."],
      fr: ["Le Chiapas contribue à environ un tiers de la production totale de café au Mexique.", "De nombreuses fermes sont situées à une altitude allant jusqu'à 1 600 mètres.", "La 'Ruta del Café' mène à des fincas historiques comme Irlandia ou Argovia.", "Les sols volcaniques de la région donnent au café sa note particulière.", "La culture se fait presque exclusivement à la main sur les pentes escarpées.", "Le Chiapas est un centre mondial de premier plan pour le café certifié équitable.", "Les fermes sont souvent des refuges importants pour les oiseaux migrateurs d'Amérique du Nord."],
    }, image: "/poi-images/chiapas-coffee-fincas-life-v2.webp",
    description: {
      de: "Kaffeeplantagen im Hochland von Chiapas, die für ihren qualitativ hochwertigen Bio-Kaffee geschätzt werden.",
      hu: "Chiapas hegyvidéki kávéfarmjai, a világ egyik vezető biokávé-termelő vidéke.",
      ro: "Moșii vaste de cafea în munții din Chiapas, lider mondial în producția de cafea organică și certificată fair-trade.",
      en: "Expansive coffee estates in the rugged mountains of Chiapas, a world leader in organic and fair-trade coffee production.",
      es: "Plantaciones de café en las tierras altas de Chiapas, apreciadas por su café orgánico de alta calidad.",
      pt: "Plantações de café nas terras altas de Chiapas, valorizadas pelo seu café orgânico de alta qualidade.",
      fr: "Plantations de café dans les hautes terres du Chiapas, appréciées pour leur café biologique de haute qualité.",
    },
    facts: {
      de: ["Größter Kaffeeproduzent Mexikos", "Fokus auf fairen Handel", "Ideal durch vulkanische Böden"],
      hu: ["Mexikó legnagyobb kávétermelője", "Világelső a biokávé-termesztésben", "1200 méter feletti ültetvények"],
      ro: ["Cel mai mare producător de cafea din Mexic", "Sol montan vulcanic ideal", "Lider în agricultura ecologică"],
      en: ["Largest coffee producer in Mexico", "Ideal volcanic mountain soil", "Major center for organic farming"],
      es: ["Mayor productor de café de México", "Enfoque en el comercio justo", "Ideal por suelos volcánicos"],
      pt: ["Maior produtor de café do México", "Foco no comércio justo", "Ideal devido aos solos vulcânicos"],
      fr: ["Plus grand producteur de café du Mexique", "Accent sur le commerce équitable", "Idéal grâce aux sols volcaniques"],
    }
  },
  {
    id: "oaxaca-mezcal-agave-life-v2", "sights": {
      "de": [
            {
                  "name": "Balneario El Cerrito",
                  "text": "Balneario El Cerrito ist ein natürliches Quellbad mit tropischer Vegetation und Pools.",
                  "category": "recreational",
                  "coords": [
                        -96.759869,
                        16.960822
                  ]
            },
            {
                  "name": "Zona Arqueologica",
                  "text": "Die Zona Arqueologica beherbergt präkolumbische Ruinen und eine Pyramide.",
                  "category": "historical",
                  "coords": [
                        -96.749651,
                        16.952654
                  ]
            },
            {
                  "name": "Taller de artes plásticas \"Manos a la obra\"",
                  "text": "Der Workshop 'Manos a la obra' bietet Kurse in traditionellen Kunsthandwerkstechniken an.",
                  "category": "cultural",
                  "coords": [
                        -96.745617,
                        16.954074
                  ]
            },
            {
                  "name": "Centro de Difusión de la Grana Cochinilla Fina Tlapanochestli",
                  "text": "Das Museum zeigt die Geschichte der Cochenilleschildlaus (Grana Cochinilla) und ihrer Nutzung als Farbstoff.",
                  "category": "museum",
                  "coords": [
                        -96.710646,
                        16.963536
                  ]
            },
            {
                  "name": "Servicio De Banda Musical La Tremenda Aventurera",
                  "text": "Servicio De Banda Musical La Tremenda Aventurera bietet Live-Musik mit traditionellen mexikanischen Klängen.",
                  "category": "cultural",
                  "coords": [
                        -96.7102,
                        17.012366
                  ]
            },
            {
                  "name": "Museo de Arte de Santa María Coyotepec",
                  "text": "Das Museo de Arte de Santa María Coyotepec zeigt zeitgenössische Kunst lokaler Oaxaca-Künstler.",
                  "category": "museum",
                  "coords": [
                        -96.706546,
                        16.966797
                  ]
            },
            {
                  "name": "Fuerza Verde Grupo Musical",
                  "text": "Fuerza Verde Grupo Musical unterhält das Publikum mit populären mexikanischen Musikgenres.",
                  "category": "cultural",
                  "coords": [
                        -96.741513,
                        17.007247
                  ]
            },
            {
                  "name": "Balneario La Gloria",
                  "text": "Balneario La Gloria ist ein Wasserpark mit mehreren Pools, Rutschen und Erholungsbereichen.",
                  "category": "recreational",
                  "coords": [
                        -96.778928,
                        16.985627
                  ]
            },
            {
                  "name": "tatro 600 años (el queso)",
                  "text": "Das Theater 'El Queso' (600 Jahre) ist ein Veranstaltungsort für kulturelle Aufführungen.",
                  "category": "cultural",
                  "coords": [
                        -96.750263,
                        16.95147
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Balneario El Cerrito",
                  "text": "A Balneario El Cerrito egy természetes forrásvízű fürdő trópusi növényzettel és medencékkel.",
                  "category": "recreational",
                  "coords": [
                        -96.759869,
                        16.960822
                  ]
            },
            {
                  "name": "Zona Arqueologica",
                  "text": "A Zona Arqueologica prekolumbián romokat és egy piramist foglal magában.",
                  "category": "historical",
                  "coords": [
                        -96.749651,
                        16.952654
                  ]
            },
            {
                  "name": "Taller de artes plásticas \"Manos a la obra\"",
                  "text": "A 'Manos a la obra' műhely hagyományos kézműves technikák tanfolyamait kínálja.",
                  "category": "cultural",
                  "coords": [
                        -96.745617,
                        16.954074
                  ]
            },
            {
                  "name": "Centro de Difusión de la Grana Cochinilla Fina Tlapanochestli",
                  "text": "A múzeum a bíbortetű (Grana Cochinilla) történetét és festékként való felhasználását mutatja be.",
                  "category": "museum",
                  "coords": [
                        -96.710646,
                        16.963536
                  ]
            },
            {
                  "name": "Servicio De Banda Musical La Tremenda Aventurera",
                  "text": "A Servicio De Banda Musical La Tremenda Aventurera élőzenét kínál hagyományos mexikói hangzással.",
                  "category": "cultural",
                  "coords": [
                        -96.7102,
                        17.012366
                  ]
            },
            {
                  "name": "Museo de Arte de Santa María Coyotepec",
                  "text": "A Museo de Arte de Santa María Coyotepec helyi oaxacai művészek kortárs művészetét mutatja be.",
                  "category": "museum",
                  "coords": [
                        -96.706546,
                        16.966797
                  ]
            },
            {
                  "name": "Fuerza Verde Grupo Musical",
                  "text": "A Fuerza Verde Grupo Musical népszerű mexikói zenei műfajokkal szórakoztatja a közönséget.",
                  "category": "cultural",
                  "coords": [
                        -96.741513,
                        17.007247
                  ]
            },
            {
                  "name": "Balneario La Gloria",
                  "text": "A Balneario La Gloria egy vízipark több medencével, csúszdákkal és pihenőhelyekkel.",
                  "category": "recreational",
                  "coords": [
                        -96.778928,
                        16.985627
                  ]
            },
            {
                  "name": "tatro 600 años (el queso)",
                  "text": "Az 'El Queso' (600 év) színház kulturális előadások helyszíne.",
                  "category": "cultural",
                  "coords": [
                        -96.750263,
                        16.95147
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Balneario El Cerrito",
                  "text": "Balneario El Cerrito este o baie naturală cu izvoare, vegetație tropicală și piscine.",
                  "category": "recreational",
                  "coords": [
                        -96.759869,
                        16.960822
                  ]
            },
            {
                  "name": "Zona Arqueologica",
                  "text": "Zona Arqueologica conține ruine precolumbiene și o piramidă.",
                  "category": "historical",
                  "coords": [
                        -96.749651,
                        16.952654
                  ]
            },
            {
                  "name": "Taller de artes plásticas \"Manos a la obra\"",
                  "text": "Atelierul 'Manos a la obra' oferă cursuri de meșteșuguri tradiționale.",
                  "category": "cultural",
                  "coords": [
                        -96.745617,
                        16.954074
                  ]
            },
            {
                  "name": "Centro de Difusión de la Grana Cochinilla Fina Tlapanochestli",
                  "text": "Muzeul prezintă istoria coșenilei (Grana Cochinilla) și utilizarea sa ca vopsea.",
                  "category": "museum",
                  "coords": [
                        -96.710646,
                        16.963536
                  ]
            },
            {
                  "name": "Servicio De Banda Musical La Tremenda Aventurera",
                  "text": "Servicio De Banda Musical La Tremenda Aventurera oferă muzică live cu sunete tradiționale mexicane.",
                  "category": "cultural",
                  "coords": [
                        -96.7102,
                        17.012366
                  ]
            },
            {
                  "name": "Museo de Arte de Santa María Coyotepec",
                  "text": "Muzeul de Artă Santa María Coyotepec expune arta contemporană a artiștilor locali din Oaxaca.",
                  "category": "museum",
                  "coords": [
                        -96.706546,
                        16.966797
                  ]
            },
            {
                  "name": "Fuerza Verde Grupo Musical",
                  "text": "Fuerza Verde Grupo Musical distrează publicul cu genuri muzicale mexicane populare.",
                  "category": "cultural",
                  "coords": [
                        -96.741513,
                        17.007247
                  ]
            },
            {
                  "name": "Balneario La Gloria",
                  "text": "Balneario La Gloria este un parc acvatic cu mai multe piscine, tobogane și zone de relaxare.",
                  "category": "recreational",
                  "coords": [
                        -96.778928,
                        16.985627
                  ]
            },
            {
                  "name": "tatro 600 años (el queso)",
                  "text": "Teatrul 'El Queso' (600 de ani) este un loc pentru spectacole culturale.",
                  "category": "cultural",
                  "coords": [
                        -96.750263,
                        16.95147
                  ]
            }
      ],
      "en": [
            {
                  "name": "Balneario El Cerrito",
                  "text": "Balneario El Cerrito is a natural spring pool surrounded by lush tropical vegetation.",
                  "category": "recreational",
                  "coords": [
                        -96.759869,
                        16.960822
                  ]
            },
            {
                  "name": "Zona Arqueologica",
                  "text": "The Zona Arqueologica features pre-Columbian ruins and a pyramid.",
                  "category": "historical",
                  "coords": [
                        -96.749651,
                        16.952654
                  ]
            },
            {
                  "name": "Taller de artes plásticas \"Manos a la obra\"",
                  "text": "The 'Manos a la obra' workshop offers classes in traditional handicraft techniques.",
                  "category": "cultural",
                  "coords": [
                        -96.745617,
                        16.954074
                  ]
            },
            {
                  "name": "Centro de Difusión de la Grana Cochinilla Fina Tlapanochestli",
                  "text": "The museum exhibits the history of the cochineal insect (Grana Cochinilla) and its use as a dye.",
                  "category": "museum",
                  "coords": [
                        -96.710646,
                        16.963536
                  ]
            },
            {
                  "name": "Servicio De Banda Musical La Tremenda Aventurera",
                  "text": "Servicio De Banda Musical La Tremenda Aventurera provides live music with traditional Mexican sounds.",
                  "category": "cultural",
                  "coords": [
                        -96.7102,
                        17.012366
                  ]
            },
            {
                  "name": "Museo de Arte de Santa María Coyotepec",
                  "text": "The Museo de Arte de Santa María Coyotepec displays contemporary art by local Oaxacan artists.",
                  "category": "museum",
                  "coords": [
                        -96.706546,
                        16.966797
                  ]
            },
            {
                  "name": "Fuerza Verde Grupo Musical",
                  "text": "Fuerza Verde Grupo Musical entertains audiences with popular Mexican music genres.",
                  "category": "cultural",
                  "coords": [
                        -96.741513,
                        17.007247
                  ]
            },
            {
                  "name": "Balneario La Gloria",
                  "text": "Balneario La Gloria is a water park with multiple pools, slides, and relaxation areas.",
                  "category": "recreational",
                  "coords": [
                        -96.778928,
                        16.985627
                  ]
            },
            {
                  "name": "tatro 600 años (el queso)",
                  "text": "The 'El Queso' (600 Years) theater is a venue for cultural performances.",
                  "category": "cultural",
                  "coords": [
                        -96.750263,
                        16.95147
                  ]
            }
      ]
},
    type: "agriculture",
    parent: "MX-OAX",
    coords: [-96.72, 16.97],
    name: { de: "Mezcal-Agave in Oaxaca", hu: "Oaxaca mezcal-agavei", ro: "Agava pentru mezcal din Oaxaca", en: "Oaxaca Mezcal Agave"},
    image: "/poi-images/oaxaca-mezcal-agave-life-v2.webp",
    descriptionAdvanced: {
      de: "Oaxaca ist die unbestrittene Hauptstadt des Mezcals, einer traditionellen Spirituose, die aus verschiedenen Agavenarten (Maguey) gewonnen wird. Anders als Tequila wird Mezcal oft in kleinen, familiären 'Palenques' nach jahrhundertealten Methoden handwerklich hergestellt. Die Landschaft um Santiago Matatlán ist geprägt von weiten Agavenfeldern, die in der trockenen Erde des Tals gedeihen. Die Herzen der Agaven werden in erdigen Grubenöfen geröstet, was dem Mezcal sein charakteristisches rauchiges Aroma verleiht. Die Vielfalt der verwendeten Agavenarten sorgt für ein breites Spektrum an Geschmacksnoten, von süßlich-mild bis würzig-erdig. Mezcal ist tief in der rituellen und sozialen Kultur Oaxacas verwurzelt und gilt als der 'Geist' der Region. Sachkunde K6 - Kulturelle Traditionen und Handwerk.",
      hu: "Oaxaca mezcal-agavei egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Oaxaca mezcal-agavei kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Agava pentru mezcal din Oaxaca este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Agava pentru mezcal din Oaxaca este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Oaxaca is the undisputed heart of mezcal production, a spirit that has become a powerful symbol of the state's cultural and biological diversity. Unlike tequila, which is made only from blue agave, mezcal can be produced from dozens of different agave species, many of which grow wild in the rugged Oaxacan landscape. The landscape is dotted with small, family-run distilleries known as 'palenques,' where traditional methods like pit-roasting the agave hearts and crushing them with a horse-drawn stone wheel (tahona) are still practiced. The valley of Tlacolula and the town of Santiago Matatlán are particularly famous for their dense concentration of mezcal producers. The process is deeply artisanal and reflects the unique 'terroir' of each village, with flavors ranging from smoky and earthy to floral and sweet. Mezcal is more than just a drink in Oaxaca; it is a vital part of social life, used in celebrations, rituals, and as a gesture of hospitality. The recent global surge in mezcal's popularity has brought new attention and economic opportunities to these rural agave-growing communities.",
      es: "Oaxaca es la capital indiscutible del mezcal, una bebida espirituosa tradicional que se obtiene de diversas especies de agave (maguey). A diferencia del tequila, el mezcal se produce a menudo en pequeños 'palenques' familiares siguiendo métodos centenarios y artesanales. El paisaje alrededor de Santiago Matatlán está marcado por extensos campos de agave que crecen en la tierra seca del valle. Los corazones de los agaves se tuestan en hornos de foso de tierra, lo que le da al mezcal su característico aroma ahumado. La variedad de especies de agave utilizadas proporciona un amplio espectro de notas de sabor, desde dulces y suaves hasta especiadas y terrosas. El mezcal está profundamente arraigado en la cultura ritual y social de Oaxaca y se considera el 'espíritu' de la región. Conocimiento K6 - Tradiciones culturales y artesanía.",
      pt: "Oaxaca é a capital indiscutível do mezcal, uma bebida espirituosa tradicional obtida de diversas espécies de agave (maguey). Ao contrário da tequila, o mezcal é frequentemente produzido em pequenos 'palenques' familiares, seguindo métodos seculares e artesanais. A paisagem em redor de Santiago Matatlán é marcada por vastos campos de agave que crescem na terra seca do vale. Os corações dos agaves são assados em fornos de cova na terra, o que confere ao mezcal o seu característico aroma fumado. A variedade de espécies de agave utilizadas proporciona um amplo espetro de notas de sabor, desde o doce e suave ao picante e terroso. O mezcal está profundamente enraizado na cultura ritual e social de Oaxaca e é considerado o 'espírito' da região. Conhecimento K6 - Tradições culturais e artesanato.",
      fr: "Oaxaca est la capitale incontestée du mezcal, un spiritueux traditionnel tiré de différentes espèces d'agaves (maguey). Contrairement à la tequila, le mezcal est souvent fabriqué artisanalement dans de petits 'palenques' familiaux selon des méthodes séculaires. Le paysage autour de Santiago Matatlán est marqué par de vastes champs d'agaves qui poussent dans la terre sèche de la vallée. Les cœurs d'agaves sont rôtis dans des fours creusés dans la terre, ce qui donne au mezcal son arôme fumé caractéristique. La diversité des espèces d'agaves utilisées offre une large gamme de saveurs, du doux-sucré à l'épicé-terreux. Le mezcal est profondément enraciné dans la culture rituelle et sociale d'Oaxaca et est considéré comme 'l'esprit' de la région. Connaissances K6 - Traditions culturelles et artisanat.",
    },
    factsAdvanced: {
      de: ["Santiago Matatlán wird oft als die 'Welthauptstadt des Mezcals' bezeichnet.", "Im Gegensatz zum Tequila darf Mezcal aus über 30 verschiedenen Agavenarten hergestellt werden.", "Die Agave 'Espadín' ist die am häufigsten für Mezcal genutzte Sorte.", "Wildwachsende Agaven wie 'Tobalá' werden besonders geschätzt und sind seltener.", "Das Rösten der Agavenkerne dauert traditionell drei bis fünf Tage.", "Mezcal hat eine geschützte Herkunftsbezeichnung (Denominación de Origen).", "Ein traditioneller Spruch lautet: 'Para todo mal, Mezcal; para todo bien, también'."],
      hu: ["Oaxaca mezcal-agavei sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Oaxaca mezcal-agavei egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Agava pentru mezcal din Oaxaca ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Agava pentru mezcal din Oaxaca este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Santiago Matatlán is often called the 'World Capital of Mezcal' due to its hundreds of small distilleries.",
        "While tequila must be at least 51% blue agave, authentic mezcal is usually 100% agave.",
        "The 'espadín' agave is the most commonly cultivated species for mezcal due to its high sugar content and yield.",
        "The smoky flavor of mezcal comes from roasting the agave hearts in underground stone-lined pits for several days.",
        "Oaxaca accounts for over 90% of all mezcal production in Mexico."
      ],
      es: ["Santiago Matatlán es a menudo llamada la 'capital mundial del mezcal'.", "A diferencia del tequila, el mezcal puede elaborarse con más de 30 especies diferentes de agave.", "El agave 'Espadín' es la variedad más utilizada para el mezcal.", "Los agaves silvestres como el 'Tobalá' son especialmente apreciados y más raros.", "El tostado de los núcleos de agave dura tradicionalmente de tres a cinco días.", "El mezcal tiene una Denominación de Origen protegida.", "Un dicho tradicional dice: 'Para todo mal, Mezcal; para todo bien, también'."],
      pt: ["Santiago Matatlán é frequentemente chamada de 'capital mundial do mezcal'.", "Ao contrário da tequila, o mezcal pode ser feito a partir de mais de 30 espécies diferentes de agave.", "O agave 'Espadín' é a variedade mais utilizada para o mezcal.", "Agaves de crescimento selvagem, como o 'Tobalá', são particularmente valorizados e mais raros.", "A torrefação dos núcleos de agave demora tradicionalmente de três a cinco dias.", "O mezcal possui uma Denominação de Origem protegida.", "Um provérbio tradicional diz: 'Para todo mal, Mezcal; para todo bien, también'."],
      fr: ["Santiago Matatlán est souvent appelée la 'capitale mondiale du mezcal'.", "Contrairement à la tequila, le mezcal peut être produit à partir de plus de 30 espèces d'agaves différentes.", "L'agave 'Espadín' est la variété la plus couramment utilisée pour le mezcal.", "Les agaves sauvages comme le 'Tobalá' sont particulièrement prisés et plus rares.", "Le rôtissage des cœurs d'agaves dure traditionnellement de trois à cinq jours.", "Le mezcal possède une Appellation d'Origine Protégée (Denominación de Origen).", "Un dicton traditionnel dit : 'Para todo mal, Mezcal ; para todo bien, también'."],
    }, image: "/poi-images/oaxaca-mezcal-agave-life-v2.webp",
    description: {
      de: "Regionen in Oaxaca, in denen verschiedene Agavenarten für die handwerkliche Herstellung von Mezcal kultiviert werden.",
      hu: "Oaxaca vidékein termesztett különféle agávéfajok, amelyekből kézműves mezcal készül.",
      ro: "Centrul tradițional al producției de mezcal din Oaxaca, unde diverse specii de agavă sunt recoltate pentru băuturi artizanale.",
      en: "The traditional heartland of mezcal production in Oaxaca, where diverse agave species are harvested for artisanal spirits.",
      es: "Regiones en Oaxaca donde se cultivan diversas especies de agave para la producción artesanal de mezcal.",
      pt: "Regiões em Oaxaca onde são cultivadas diversas espécies de agave para a produção artesanal de mezcal.",
      fr: "Régions d'Oaxaca où différentes espèces d'agave sont cultivées pour la fabrication artisanale du mezcal.",
    },
    facts: {
      de: ["Hauptregion des Mezcals", "Handwerkliche Destillation", "Vielfalt wilder Agavenarten"],
      hu: ["Mezcal world fővárosa", "Több mint 30 agávéfaj használata", "Hagyományos, föld alatti pörkölés"],
      ro: ["Oaxaca produce 90% din mezcalul mexican", "Folosește peste 30 de specii diferite de agavă", "Proces artizanal de coacere în gropi"],
      en: ["Oaxaca produces 90% of Mexico's mezcal", "Uses over 30 different agave species", "Artisanal pit-roasting process"],
      es: ["Principal región del mezcal", "Destilación artesanal", "Diversidad de especies de agave silvestre"],
      pt: ["Principal região do mezcal", "Destilação artesanal", "Diversidade de espécies de agave selvagem"],
      fr: ["Principale région du mezcal", "Distillation artisanale", "Diversité d'espèces d'agaves sauvages"],
    }
  },
  {
    id: "michoacan-avocado-orchards-life-v2", "sights": {
      "de": [
            {
                  "name": "Gral. Lázaro Cárdenas",
                  "text": "Das Denkmal für General Lázaro Cárdenas ehrt den ehemaligen mexikanischen Präsidenten.",
                  "category": "historical",
                  "coords": [
                        -102.050652,
                        19.417989
                  ]
            },
            {
                  "name": "La Huátapera",
                  "text": "La Huátapera ist ein historisches Gebäude, das ein Museum für indigene Kultur und Kunst beherbergt.",
                  "category": "museum",
                  "coords": [
                        -102.062108,
                        19.421055
                  ]
            },
            {
                  "name": "Pérgola Municipal",
                  "text": "Die Pérgola Municipal ist eine historische überdachte Struktur auf dem Hauptplatz für öffentliche Veranstaltungen.",
                  "category": "historical",
                  "coords": [
                        -102.063304,
                        19.421018
                  ]
            },
            {
                  "name": "Casa mas chiquita del mundo",
                  "text": "Das Smaller house in the world ist ein Museum in einem Haus, das als eines der kleinsten der Welt gilt, in michoacan-avocado-orchards-life-v2.",
                  "category": "museum",
                  "coords": [
                        -102.066048,
                        19.425471
                  ]
            },
            {
                  "name": "Foro Cultural La Morada",
                  "text": "Foro Cultural La Morada ist ein Kulturzentrum, das Konzerte und Ausstellungen veranstaltet.",
                  "category": "cultural",
                  "coords": [
                        -102.06654,
                        19.423036
                  ]
            },
            {
                  "name": "Monumento a La Bandera",
                  "text": "Das Monumento a La Bandera ist eine Skulptur, die der mexikanischen Flagge gewidmet ist.",
                  "category": "historical",
                  "coords": [
                        -102.06391,
                        19.42228
                  ]
            },
            {
                  "name": "Mural de los Orígenes de Uruapan",
                  "text": "Das Mural de los Orígenes de Uruapan zeigt in einem Wandgemälde die Gründungsgeschichte der Stadt.",
                  "category": "historical",
                  "coords": [
                        -102.074792,
                        19.433321
                  ]
            },
            {
                  "name": "Mansión del Cupatitzio",
                  "text": "Die Mansión del Cupatitzio ist ein Hotel und Herrenhaus, benannt nach dem Fluss Cupatitzio, in michoacan-avocado-orchards-life-v2.",
                  "category": "landmark",
                  "coords": [
                        -102.074191,
                        19.429717
                  ]
            },
            {
                  "name": "La Tzararacua",
                  "text": "La Tzararacua ist ein Naturschutzgebiet mit einem bekannten Wasserfall in michoacan-avocado-orchards-life-v2.",
                  "category": "natural",
                  "coords": [
                        -102.073256,
                        19.352721
                  ]
            },
            {
                  "name": "Monumento al Padre Ochoa",
                  "text": "Das Monumento al Padre Ochoa erinnert an den Priester und Pädagogen Padre Ochoa.",
                  "category": "historical",
                  "coords": [
                        -102.066382,
                        19.405226
                  ]
            },
            {
                  "name": "Taller Escuela de Artes Plásticas y Artesanías MAPECO",
                  "text": "Der Taller Escuela de Artes Plásticas y Artesanías MAPECO ist ein Kunstzentrum und eine Werkstattschule für Bildende Kunst und Handwerk in michoacan-avocado-orchards-life-v2.",
                  "category": "cultural",
                  "coords": [
                        -102.046986,
                        19.408919
                  ]
            },
            {
                  "name": "Acceso Caltzontzin",
                  "text": "Acceso Caltzontzin ist der Eingang zu einem archäologischen Gebiet nahe Uruapan.",
                  "category": "historical",
                  "coords": [
                        -102.010434,
                        19.424152
                  ]
            },
            {
                  "name": "Balneario Villa Paraíso",
                  "text": "Das Balneario Villa Paraíso ist ein Wasserpark und Spa-Resort in michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.018294,
                        19.418761
                  ]
            },
            {
                  "name": "Centro de Espectáculos 'La Macarena'",
                  "text": "Das Centro de Espectáculos 'La Macarena' ist eine Veranstaltungshalle für Shows und Konzerte.",
                  "category": "cultural",
                  "coords": [
                        -102.042796,
                        19.416149
                  ]
            },
            {
                  "name": "Parque de Animales San Miguelito",
                  "text": "Der Parque de Animales San Miguelito ist ein kleiner Zoo und Tierpark in michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.124672,
                        19.416619
                  ]
            },
            {
                  "name": "Parque Nacional Barranca del Cupatitzio",
                  "text": "Der Parque Nacional Barranca del Cupatitzio ist ein Nationalpark und Naturschutzgebiet, das die Schlucht des Cupatitzio-Flusses umfasst, in michoacan-avocado-orchards-life-v2.",
                  "category": "natural",
                  "coords": [
                        -102.104795,
                        19.429228
                  ]
            },
            {
                  "name": "Zona de Protección Ambiental El Zapién",
                  "text": "Das Schutzgebiet El Zapién bewahrt eine vielfältige Flora und Fauna mit Wanderwegen.",
                  "category": "natural",
                  "coords": [
                        -102.100784,
                        19.445452
                  ]
            },
            {
                  "name": "Balneario Juntas del Cupatitzio",
                  "text": "Das Balneario Juntas del Cupatitzio ist ein Wasserpark am Zusammenfluss des Cupatitzio-Flusses in michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.061513,
                        19.387956
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Gral. Lázaro Cárdenas",
                  "text": "A Gral. Lázaro Cárdenas emlékmű a volt mexikói elnök előtt tiszteleg.",
                  "category": "historical",
                  "coords": [
                        -102.050652,
                        19.417989
                  ]
            },
            {
                  "name": "La Huátapera",
                  "text": "A La Huátapera egy történelmi épület, amely az őslakos kultúra és művészet múzeumának ad otthont.",
                  "category": "museum",
                  "coords": [
                        -102.062108,
                        19.421055
                  ]
            },
            {
                  "name": "Pérgola Municipal",
                  "text": "A Pérgola Municipal egy történelmi fedett építmény a főtéren, nyilvános eseményekhez.",
                  "category": "historical",
                  "coords": [
                        -102.063304,
                        19.421018
                  ]
            },
            {
                  "name": "Casa mas chiquita del mundo",
                  "text": "A Smaller house in the world egy múzeum, amely a világ egyik legkisebb házának tartott épületben található a michoacan-avocado-orchards-life-v2 területén.",
                  "category": "museum",
                  "coords": [
                        -102.066048,
                        19.425471
                  ]
            },
            {
                  "name": "Foro Cultural La Morada",
                  "text": "A Foro Cultural La Morada egy kulturális központ, amely koncerteknek és kiállításoknak ad otthont.",
                  "category": "cultural",
                  "coords": [
                        -102.06654,
                        19.423036
                  ]
            },
            {
                  "name": "Monumento a La Bandera",
                  "text": "A Monumento a La Bandera egy szobor, amely a mexikói zászlónak állít emléket.",
                  "category": "historical",
                  "coords": [
                        -102.06391,
                        19.42228
                  ]
            },
            {
                  "name": "Mural de los Orígenes de Uruapan",
                  "text": "A Mural de los Orígenes de Uruapan egy falfestmény, amely a város alapításának történetét ábrázolja.",
                  "category": "historical",
                  "coords": [
                        -102.074792,
                        19.433321
                  ]
            },
            {
                  "name": "Mansión del Cupatitzio",
                  "text": "A Mansión del Cupatitzio egy szálloda és kúria, amely a Cupatitzio folyóról kapta a nevét, a michoacan-avocado-orchards-life-v2 területén.",
                  "category": "landmark",
                  "coords": [
                        -102.074191,
                        19.429717
                  ]
            },
            {
                  "name": "La Tzararacua",
                  "text": "La Tzararacua egy természetvédelmi terület egy híres vízeséssel a michoacan-avocado-orchards-life-v2 területén.",
                  "category": "natural",
                  "coords": [
                        -102.073256,
                        19.352721
                  ]
            },
            {
                  "name": "Monumento al Padre Ochoa",
                  "text": "A Monumento al Padre Ochoa a Padre Ochoa papra és pedagógusra emlékezik.",
                  "category": "historical",
                  "coords": [
                        -102.066382,
                        19.405226
                  ]
            },
            {
                  "name": "Taller Escuela de Artes Plásticas y Artesanías MAPECO",
                  "text": "A Taller Escuela de Artes Plásticas y Artesanías MAPECO egy művészeti központ és műhelyiskola képzőművészet és kézművesség számára a michoacan-avocado-orchards-life-v2 területén.",
                  "category": "cultural",
                  "coords": [
                        -102.046986,
                        19.408919
                  ]
            },
            {
                  "name": "Acceso Caltzontzin",
                  "text": "Az Acceso Caltzontzin a Uruapan közelében lévő régészeti terület bejárata.",
                  "category": "historical",
                  "coords": [
                        -102.010434,
                        19.424152
                  ]
            },
            {
                  "name": "Balneario Villa Paraíso",
                  "text": "A Balneario Villa Paraíso egy vízipark és gyógyfürdő üdülőhely a michoacan-avocado-orchards-life-v2 területén.",
                  "category": "recreational",
                  "coords": [
                        -102.018294,
                        19.418761
                  ]
            },
            {
                  "name": "Centro de Espectáculos 'La Macarena'",
                  "text": "A Centro de Espectáculos 'La Macarena' egy rendezvénycsarnok előadások és koncertek számára.",
                  "category": "cultural",
                  "coords": [
                        -102.042796,
                        19.416149
                  ]
            },
            {
                  "name": "Parque de Animales San Miguelito",
                  "text": "A Parque de Animales San Miguelito egy kis állatkert és állatpark a michoacan-avocado-orchards-life-v2 területén.",
                  "category": "recreational",
                  "coords": [
                        -102.124672,
                        19.416619
                  ]
            },
            {
                  "name": "Parque Nacional Barranca del Cupatitzio",
                  "text": "A Parque Nacional Barranca del Cupatitzio egy nemzeti park és természetvédelmi terület, amely magában foglalja a Cupatitzio folyó szurdokát, a michoacan-avocado-orchards-life-v2 területén.",
                  "category": "natural",
                  "coords": [
                        -102.104795,
                        19.429228
                  ]
            },
            {
                  "name": "Zona de Protección Ambiental El Zapién",
                  "text": "Az El Zapién védett terület sokféle növény- és állatvilágot őriz, túraútvonalakkal.",
                  "category": "natural",
                  "coords": [
                        -102.100784,
                        19.445452
                  ]
            },
            {
                  "name": "Balneario Juntas del Cupatitzio",
                  "text": "A Balneario Juntas del Cupatitzio egy vízipark a Cupatitzio folyó összefolyásánál a michoacan-avocado-orchards-life-v2 területén.",
                  "category": "recreational",
                  "coords": [
                        -102.061513,
                        19.387956
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Gral. Lázaro Cárdenas",
                  "text": "Monumentul Gral. Lázaro Cárdenas îl onorează pe fostul președinte mexican.",
                  "category": "historical",
                  "coords": [
                        -102.050652,
                        19.417989
                  ]
            },
            {
                  "name": "La Huátapera",
                  "text": "La Huátapera este o clădire istorică ce găzduiește un muzeu al culturii și artei indigene.",
                  "category": "museum",
                  "coords": [
                        -102.062108,
                        19.421055
                  ]
            },
            {
                  "name": "Pérgola Municipal",
                  "text": "Pérgola Municipală este o structură istorică acoperită în piața principală, pentru evenimente publice.",
                  "category": "historical",
                  "coords": [
                        -102.063304,
                        19.421018
                  ]
            },
            {
                  "name": "Casa mas chiquita del mundo",
                  "text": "Smaller house in the world este un muzeu găzduit într-o casă despre care se spune că este una dintre cele mai mici din lume, în michoacan-avocado-orchards-life-v2.",
                  "category": "museum",
                  "coords": [
                        -102.066048,
                        19.425471
                  ]
            },
            {
                  "name": "Foro Cultural La Morada",
                  "text": "Foro Cultural La Morada este un centru cultural care găzduiește concerte și expoziții.",
                  "category": "cultural",
                  "coords": [
                        -102.06654,
                        19.423036
                  ]
            },
            {
                  "name": "Monumento a La Bandera",
                  "text": "Monumento a La Bandera este o sculptură dedicată drapelului mexican.",
                  "category": "historical",
                  "coords": [
                        -102.06391,
                        19.42228
                  ]
            },
            {
                  "name": "Mural de los Orígenes de Uruapan",
                  "text": "Mural de los Orígenes de Uruapan este o pictură murală care ilustrează originile orașului.",
                  "category": "historical",
                  "coords": [
                        -102.074792,
                        19.433321
                  ]
            },
            {
                  "name": "Mansión del Cupatitzio",
                  "text": "Mansión del Cupatitzio este un hotel tip conac, numit după râul Cupatitzio, în michoacan-avocado-orchards-life-v2.",
                  "category": "landmark",
                  "coords": [
                        -102.074191,
                        19.429717
                  ]
            },
            {
                  "name": "La Tzararacua",
                  "text": "La Tzararacua este o rezervație naturală cu o cascadă renumită în michoacan-avocado-orchards-life-v2.",
                  "category": "natural",
                  "coords": [
                        -102.073256,
                        19.352721
                  ]
            },
            {
                  "name": "Monumento al Padre Ochoa",
                  "text": "Monumento al Padre Ochoa comemorează preotul și educatorul Padre Ochoa.",
                  "category": "historical",
                  "coords": [
                        -102.066382,
                        19.405226
                  ]
            },
            {
                  "name": "Taller Escuela de Artes Plásticas y Artesanías MAPECO",
                  "text": "Taller Escuela de Artes Plásticas y Artesanías MAPECO este un centru de artă și o școală-atelier pentru arte plastice și meșteșuguri în michoacan-avocado-orchards-life-v2.",
                  "category": "cultural",
                  "coords": [
                        -102.046986,
                        19.408919
                  ]
            },
            {
                  "name": "Acceso Caltzontzin",
                  "text": "Acceso Caltzontzin este intrarea într-o zonă arheologică lângă Uruapan.",
                  "category": "historical",
                  "coords": [
                        -102.010434,
                        19.424152
                  ]
            },
            {
                  "name": "Balneario Villa Paraíso",
                  "text": "Balneario Villa Paraíso este un parc acvatic și o stațiune spa în michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.018294,
                        19.418761
                  ]
            },
            {
                  "name": "Centro de Espectáculos 'La Macarena'",
                  "text": "Centro de Espectáculos 'La Macarena' este o sală de spectacole pentru concerte și show-uri.",
                  "category": "cultural",
                  "coords": [
                        -102.042796,
                        19.416149
                  ]
            },
            {
                  "name": "Parque de Animales San Miguelito",
                  "text": "Parque de Animales San Miguelito este o grădină zoologică mică și un parc animalier în michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.124672,
                        19.416619
                  ]
            },
            {
                  "name": "Parque Nacional Barranca del Cupatitzio",
                  "text": "Parque Nacional Barranca del Cupatitzio este un parc național și rezervație naturală care cuprinde canionul râului Cupatitzio, în michoacan-avocado-orchards-life-v2.",
                  "category": "natural",
                  "coords": [
                        -102.104795,
                        19.429228
                  ]
            },
            {
                  "name": "Zona de Protección Ambiental El Zapién",
                  "text": "Zona de protecție El Zapién păstrează o floră și faună diversă, cu trasee de drumeție.",
                  "category": "natural",
                  "coords": [
                        -102.100784,
                        19.445452
                  ]
            },
            {
                  "name": "Balneario Juntas del Cupatitzio",
                  "text": "Balneario Juntas del Cupatitzio este un parc acvatic situat la confluența râului Cupatitzio, în michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.061513,
                        19.387956
                  ]
            }
      ],
      "en": [
            {
                  "name": "Gral. Lázaro Cárdenas",
                  "text": "The monument to General Lázaro Cárdenas honors the former Mexican president.",
                  "category": "historical",
                  "coords": [
                        -102.050652,
                        19.417989
                  ]
            },
            {
                  "name": "La Huátapera",
                  "text": "La Huátapera is a historic building housing a museum of indigenous culture and art.",
                  "category": "museum",
                  "coords": [
                        -102.062108,
                        19.421055
                  ]
            },
            {
                  "name": "Pérgola Municipal",
                  "text": "The Pérgola Municipal is a historic covered structure on the main square for public events.",
                  "category": "historical",
                  "coords": [
                        -102.063304,
                        19.421018
                  ]
            },
            {
                  "name": "Smaller house in the world",
                  "text": "The Smaller house in the world is a museum housed in what is claimed to be one of the smallest houses, located in michoacan-avocado-orchards-life-v2.",
                  "category": "museum",
                  "coords": [
                        -102.066048,
                        19.425471
                  ]
            },
            {
                  "name": "Foro Cultural La Morada",
                  "text": "Foro Cultural La Morada is a cultural center hosting concerts and exhibitions.",
                  "category": "cultural",
                  "coords": [
                        -102.06654,
                        19.423036
                  ]
            },
            {
                  "name": "Monumento a La Bandera",
                  "text": "The Monumento a La Bandera is a sculpture dedicated to the Mexican flag.",
                  "category": "historical",
                  "coords": [
                        -102.06391,
                        19.42228
                  ]
            },
            {
                  "name": "Mural de los Orígenes de Uruapan",
                  "text": "The Mural de los Orígenes de Uruapan depicts the founding history of the city in a mural.",
                  "category": "historical",
                  "coords": [
                        -102.074792,
                        19.433321
                  ]
            },
            {
                  "name": "Mansión del Cupatitzio",
                  "text": "Mansión del Cupatitzio is a hotel mansion named after the Cupatitzio River in michoacan-avocado-orchards-life-v2.",
                  "category": "landmark",
                  "coords": [
                        -102.074191,
                        19.429717
                  ]
            },
            {
                  "name": "La Tzararacua",
                  "text": "La Tzararacua is a nature reserve featuring a famous waterfall in michoacan-avocado-orchards-life-v2.",
                  "category": "natural",
                  "coords": [
                        -102.073256,
                        19.352721
                  ]
            },
            {
                  "name": "Monumento al Padre Ochoa",
                  "text": "The Monumento al Padre Ochoa commemorates the priest and educator Father Ochoa.",
                  "category": "historical",
                  "coords": [
                        -102.066382,
                        19.405226
                  ]
            },
            {
                  "name": "Taller Escuela de Artes Plásticas y Artesanías MAPECO",
                  "text": "Taller Escuela de Artes Plásticas y Artesanías MAPECO is an arts centre and workshop school for plastic arts and crafts in michoacan-avocado-orchards-life-v2.",
                  "category": "cultural",
                  "coords": [
                        -102.046986,
                        19.408919
                  ]
            },
            {
                  "name": "Acceso Caltzontzin",
                  "text": "Acceso Caltzontzin is the entrance to an archaeological area near Uruapan.",
                  "category": "historical",
                  "coords": [
                        -102.010434,
                        19.424152
                  ]
            },
            {
                  "name": "Balneario Villa Paraíso",
                  "text": "Balneario Villa Paraíso is a water park and spa resort in michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.018294,
                        19.418761
                  ]
            },
            {
                  "name": "Centro de Espectáculos 'La Macarena'",
                  "text": "Centro de Espectáculos 'La Macarena' is an entertainment venue for shows and concerts.",
                  "category": "cultural",
                  "coords": [
                        -102.042796,
                        19.416149
                  ]
            },
            {
                  "name": "Parque de Animales San Miguelito",
                  "text": "Parque de Animales San Miguelito is a small zoo and animal park in michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.124672,
                        19.416619
                  ]
            },
            {
                  "name": "Parque Nacional Barranca del Cupatitzio",
                  "text": "Parque Nacional Barranca del Cupatitzio is a national park and nature reserve encompassing the Cupatitzio River canyon in michoacan-avocado-orchards-life-v2.",
                  "category": "natural",
                  "coords": [
                        -102.104795,
                        19.429228
                  ]
            },
            {
                  "name": "Zona de Protección Ambiental El Zapién",
                  "text": "El Zapién protected area preserves diverse flora and fauna with hiking trails.",
                  "category": "natural",
                  "coords": [
                        -102.100784,
                        19.445452
                  ]
            },
            {
                  "name": "Balneario Juntas del Cupatitzio",
                  "text": "Balneario Juntas del Cupatitzio is a water park located at the confluence of the Cupatitzio River in michoacan-avocado-orchards-life-v2.",
                  "category": "recreational",
                  "coords": [
                        -102.061513,
                        19.387956
                  ]
            }
      ]
},
    type: "agriculture",
    parent: "MX-MIC",
    coords: [-102.05, 19.41],
    name: { de: "Avocadoplantagen in Michoacan", hu: "Michoacan avokado-ultetvenyek", ro: "Livezi de avocado din Michoacan", en: "Michoacan Avocado Orchards"},
    image: "/poi-images/michoacan-avocado-orchards-life-v2.webp",
    descriptionAdvanced: {
      de: "Michoacán ist das weltweite Zentrum der Avocadoproduktion und wird oft als der 'Avocado-Obstgarten der Welt' bezeichnet. Die fruchtbaren vulkanischen Böden und das milde Klima bieten ideale Bedingungen für die Sorte Hass, die hier das ganze Jahr über geerntet werden kann. Riesige Plantagen erstrecken sich über die sanften Hügel der Region, insbesondere rund um die Stadt Uruapan. Die Avocado ist für Mexiko das 'grüne Gold', da sie ein bedeutender Wirtschaftsfaktor und ein Symbol der mexikanischen Küche weltweit ist. Der massive Anbau stellt die Region jedoch auch vor ökologische Herausforderungen wie hohen Wasserverbrauch und den Erhalt der Wälder. Dennoch ist die Professionalität und Skalierung dieses Wirtschaftszweiges in Michoacán weltweit einzigartig und beeindruckend. Geographie K8 - Globale Märkte und Monokulturen.",
      hu: "Michoacan avokado-ultetvenyek egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Michoacan avokado-ultetvenyek kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Livezi de avocado din Michoacan este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Livezi de avocado din Michoacan este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "Michoacán is the 'avocado capital of the world,' a fertile region where the rich volcanic soil and temperate climate of the highlands create the perfect conditions for the 'green gold' to thrive. The state accounts for the vast majority of Mexico's avocado production, with hundreds of thousands of hectares of orchards stretching across its undulating landscape. The avocado industry is a massive economic driver for the state, supporting the livelihoods of thousands of farmers, harvesters, and exporters. The Hass variety, known for its creamy texture and rich flavor, is the dominant crop and is exported in massive quantities, particularly to the United States. Driving through the 'Meseta Purépecha,' visitors can see endless rows of dark green trees climbing the hillsides, often interspersed with pine forests. Despite the economic success, the industry faces challenges related to water use and forest conservation, prompting a move toward more sustainable farming practices. The avocado remains a source of immense pride and a staple of the local culinary tradition, essential for everything from daily meals to world-famous guacamole.",
      es: "Michoacán es el centro mundial de la producción de aguacate y a menudo se le llama el 'huerto de aguacates del mundo'. Los fértiles suelos volcánicos y el clima templado ofrecen condiciones ideales para la variedad Hass, que puede cosecharse aquí durante todo el año. Enormes plantaciones se extienden por las suaves colinas de la región, especialmente alrededor de la ciudad de Uruapan. El aguacate es el 'oro verde' de México, ya que es un factor económico importante y un símbolo de la cocina mexicana en todo el mundo. Sin embargo, el cultivo masivo también plantea desafíos ecológicos a la región, como el alto consumo de agua y la preservación de los bosques. No obstante, la profesionalidad y la escala de este sector económico en Michoacán son únicas e impresionantes a nivel mundial. Geografía K8 - Mercados globales y monocultivos.",
      pt: "Michoacán é o centro mundial da produção de abacate e é frequentemente referido como o 'pomar de abacate do mundo'. Os solos vulcânicos férteis e o clima ameno oferecem condições ideais para a variedade Hass, que pode ser colhida aqui durante todo o ano. Enormes plantações estendem-se pelas colinas suaves da região, especialmente em redor da cidade de Uruapan. O abacate é o 'ouro verde' para o México, pois é um importante fator económico e um símbolo da cozinha mexicana em todo o mundo. No entanto, o cultivo massivo também coloca desafios ecológicos à região, como o elevado consumo de água e a preservação das florestas. No entanto, o profissionalismo e a escala deste setor económico em Michoacán são únicos e impressionantes a nível mundial. Geografia K8 - Mercados globais e monoculturas.",
      fr: "Le Michoacán est le centre mondial de la production d'avocats et est souvent appelé le 'verger d'avocats du monde'. Les sols volcaniques fertiles et le climat doux offrent des conditions idéales pour la variété Hass, qui peut être récoltée ici toute l'année. D'immenses plantations s'étendent sur les collines douces de la région, particulièrement autour de la ville d'Uruapan. L'avocat est 'l'or vert' du Mexique, car il constitue un facteur économique majeur et un symbole de la cuisine mexicaine dans le monde entier. Cependant, la culture massive pose aussi des défis écologiques à la région, tels qu'une consommation d'eau élevée et la préservation des forêts. Néanmoins, le professionnalisme et l'ampleur de ce secteur économique dans le Michoacán sont uniques et impressionnants à l'échelle mondiale. Géographie K8 - Marchés mondiaux et monocultures.",
    },
    factsAdvanced: {
      de: ["Mexiko liefert etwa 45 % aller weltweit konsumierten Avocados.", "Michoacán ist der einzige Staat weltweit, der das ganze Jahr über exportieren darf.", "Die Stadt Uruapan gilt als das globale Epizentrum des Avocadohandels.", "Ein einzelner Baum kann unter idealen Bedingungen bis zu 200 Kilo Früchte tragen.", "Die Avocado stammt ursprünglich aus Zentralmexiko und wurde schon von den Azteken genutzt.", "Der Name stammt vom Aztekischen Wort 'ahuacatl', was Hoden bedeutet.", "Die Branche schafft direkt und indirekt über 300.000 Arbeitsplätze in der Region."],
      hu: ["Michoacan avokado-ultetvenyek sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Michoacan avokado-ultetvenyek egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Livezi de avocado din Michoacan ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Livezi de avocado din Michoacan este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Michoacán is the only place in the world where avocado trees can bloom four times a year, allowing for year-round harvest.",
        "Mexico provides approximately 45% of the world's avocado exports, with Michoacán leading the way.",
        "The town of Uruapan is considered the center of the avocado trade and hosts a massive avocado festival.",
        "Avocados have been cultivated in Mexico for nearly 10,000 years, according to archaeological evidence.",
        "The industry generates billions of dollars in revenue for the Mexican economy each year."
      ],
      es: ["México suministra aproximadamente el 45% de todos los aguacates consumidos en el mundo.", "Michoacán es el único estado del mundo que puede exportar durante todo el año.", "La ciudad de Uruapan es considerada el epicentro global del comercio de aguacate.", "Un solo árbol puede producir hasta 200 kilos de fruta en condiciones ideales.", "El aguacate es originario del centro de México y ya era utilizado por los aztecas.", "El nombre proviene de la palabra azteca 'ahuacatl', que significa testículo.", "La industria genera más de 300.000 empleos directos e indirectos en la región."],
      pt: ["O México fornece cerca de 45% de todos os abacates consumidos mundialmente.", "Michoacán é o único estado do mundo que pode exportar durante todo o ano.", "A cidade de Uruapan é considerada o epicentro global do comércio de abacate.", "Uma única árvore pode produzir até 200 quilos de fruta em condições ideais.", "O abacate é originário do centro do México e já era utilizado pelos astecas.", "O nome provém da palavra asteca 'ahuacatl', que significa testículo.", "A indústria cria mais de 300.000 empregos diretos e indiretos na região."],
      fr: ["Le Mexique fournit environ 45 % de tous les avocats consommés dans le monde.", "Le Michoacán est le seul État au monde autorisé à exporter toute l'année.", "La ville d'Uruapan est considérée comme l'épicentre mondial du commerce de l'avocat.", "Un seul arbre peut porter jusqu'à 200 kilos de fruits dans des conditions idéales.", "L'avocat est originaire du centre du Mexique et était déjà utilisé par les Aztèques.", "Le nom vient du mot aztèque 'ahuacatl', qui signifie testicule.", "La filière crée directement et indirectement plus de 300 000 emplois dans la région."],
    }, image: "/poi-images/michoacan-avocado-orchards-life-v2.webp",
    description: {
      de: "Die weltweit größten Avocado-Anbaugebiete, die Michoacán zum globalen Zentrum dieser Frucht machen.",
      hu: "A világ legnagyobb avokádótermelő vidéke Michoacán államban, a \"zöld arany\" hazája.",
      ro: "Regiunea lider mondial în producția de avocado, cu livezi vaste întinse pe dealurile vulcanice fertile din Michoacán.",
      en: "The world's leading avocado-producing region, with vast orchards stretching across the fertile volcanic hills of Michoacán.",
      es: "Las áreas de cultivo de aguacate más grandes del mundo, que hacen de Michoacán el centro global de esta fruta.",
      pt: "As maiores áreas de cultivo de abacate do mundo, que tornam Michoacán o centro global desta fruta.",
      fr: "Les plus grandes zones de culture d'avocats au monde, faisant du Michoacán le centre mondial de ce fruit.",
    },
    facts: {
      de: ["Weltmarktführer im Export", "Fruchtbare Vulkanerde", "Ganzjährige Ernte möglich"],
      hu: ["A világ avokádótermelésének központja", "Egész évben zajló betakarítás", "Termékeny vulkanikus dombvidék"],
      ro: ["Furnizează 45% din exporturile globale", "Cunoscut sub numele de \"Aurul Verde\"", "Copacii pot înflori de patru ori pe an"],
      en: ["Known as 'Green Gold'", "Supplies 45% of global exports", "Trees can bloom four times a year"],
      es: ["Líder mundial en exportación", "Tierra volcánica fértil", "Cosecha posible todo el año"],
      pt: ["Líder mundial em exportação", "Solo vulcânico fértil", "Colheita possível durante todo o ano"],
      fr: ["Leader mondial de l'exportation", "Terre volcanique fertile", "Récolte possible toute l'année"],
    }
  },
  {
    id: "sinaloa-mango-orchards-life-v2", "sights": {
      "de": [
            {
                  "name": "CUM Los Mochis",
                  "text": "Das CUM Los Mochis ist der Campus einer Universität inmitten von Mango-Obstgärten.",
                  "category": "landmark",
                  "coords": [
                        -109.028632,
                        25.746627
                  ]
            },
            {
                  "name": "Parroquia de Nuestra Señora del Rosario",
                  "text": "Die Parroquia de Nuestra Señora del Rosario ist eine historische Kirche aus dem 16. Jahrhundert.",
                  "category": "castle",
                  "coords": [
                        -108.994859,
                        25.7683
                  ]
            },
            {
                  "name": "Trapiche Museo Interactivo",
                  "text": "Das Trapiche Museo Interactivo ist ein interaktives Museum zur Geschichte der Zuckerproduktion.",
                  "category": "museum",
                  "coords": [
                        -108.999103,
                        25.788559
                  ]
            },
            {
                  "name": "Entrada",
                  "text": "Der zentrale Eingang (Entrada) führt in das Gebiet der Mango-Obstgärten und -Plantagen.",
                  "category": "landmark",
                  "coords": [
                        -109.001832,
                        25.788583
                  ]
            },
            {
                  "name": "La Pergola",
                  "text": "Natürliche Pergola aus Mangobäumen in Mango Orchards.",
                  "category": "natural",
                  "coords": [
                        -108.972815,
                        25.805474
                  ]
            },
            {
                  "name": "Casa de La Cultura Conrado Espinoza",
                  "text": "Kulturhausmuseum, benannt nach dem Schriftsteller Conrado Espinoza, in Mango Orchards.",
                  "category": "museum",
                  "coords": [
                        -108.972525,
                        25.80447
                  ]
            },
            {
                  "name": "Museo Del Valle del Fuerte",
                  "text": "Museum des Fuerte-Tals mit regionalhistorischen Exponaten in Mango Orchards.",
                  "category": "museum",
                  "coords": [
                        -108.999588,
                        25.790209
                  ]
            },
            {
                  "name": "Ruinas de la Casa Grande",
                  "text": "Ruinen eines großen Kolonialhauses in Mango Orchards.",
                  "category": "historical",
                  "coords": [
                        -109.003358,
                        25.789639
                  ]
            },
            {
                  "name": "Jardín Botánico Benjamin Francis Johnston (Parque Sinaloa)",
                  "text": "Botanischer Garten Benjamin Francis Johnston, Teil des Parque Sinaloa, mit einheimischen Pflanzen.",
                  "category": "landmark",
                  "coords": [
                        -109.005038,
                        25.789143
                  ]
            },
            {
                  "name": "Teatro Ingenio",
                  "text": "Theater Ingenio, eine kulturelle Spielstätte in Mango Orchards.",
                  "category": "cultural",
                  "coords": [
                        -109.000382,
                        25.788697
                  ]
            }
      ],
      "hu": [
            {
                  "name": "CUM Los Mochis",
                  "text": "A CUM Los Mochis egy egyetemi kampusz a mangóültetvények között.",
                  "category": "landmark",
                  "coords": [
                        -109.028632,
                        25.746627
                  ]
            },
            {
                  "name": "Parroquia de Nuestra Señora del Rosario",
                  "text": "A Parroquia de Nuestra Señora del Rosario egy 16. századi történelmi templom.",
                  "category": "castle",
                  "coords": [
                        -108.994859,
                        25.7683
                  ]
            },
            {
                  "name": "Trapiche Museo Interactivo",
                  "text": "A Trapiche Museo Interactivo egy interaktív múzeum a cukortermelés történetéről.",
                  "category": "museum",
                  "coords": [
                        -108.999103,
                        25.788559
                  ]
            },
            {
                  "name": "Entrada",
                  "text": "A központi bejárat (Entrada) vezet a mangóültetvények és -termesztő területekbe.",
                  "category": "landmark",
                  "coords": [
                        -109.001832,
                        25.788583
                  ]
            },
            {
                  "name": "La Pergola",
                  "text": "Természetes pergola mangófákból Mango Orchardsban.",
                  "category": "natural",
                  "coords": [
                        -108.972815,
                        25.805474
                  ]
            },
            {
                  "name": "Casa de La Cultura Conrado Espinoza",
                  "text": "Conrado Espinoza íróról elnevezett kultúrház-múzeum Mango Orchardsban.",
                  "category": "museum",
                  "coords": [
                        -108.972525,
                        25.80447
                  ]
            },
            {
                  "name": "Museo Del Valle del Fuerte",
                  "text": "A Fuerte-völgy múzeuma regionális történeti kiállításokkal Mango Orchardsban.",
                  "category": "museum",
                  "coords": [
                        -108.999588,
                        25.790209
                  ]
            },
            {
                  "name": "Ruinas de la Casa Grande",
                  "text": "Egy nagy gyarmati ház romjai Mango Orchardsban.",
                  "category": "historical",
                  "coords": [
                        -109.003358,
                        25.789639
                  ]
            },
            {
                  "name": "Jardín Botánico Benjamin Francis Johnston (Parque Sinaloa)",
                  "text": "Benjamin Francis Johnston Botanikus Kert, a Parque Sinaloa része, őshonos növényekkel.",
                  "category": "landmark",
                  "coords": [
                        -109.005038,
                        25.789143
                  ]
            },
            {
                  "name": "Teatro Ingenio",
                  "text": "Ingenio Színház, kulturális helyszín Mango Orchardsban.",
                  "category": "cultural",
                  "coords": [
                        -109.000382,
                        25.788697
                  ]
            }
      ],
      "ro": [
            {
                  "name": "CUM Los Mochis",
                  "text": "CUM Los Mochis este un campus universitar situat printre livezile de mango.",
                  "category": "landmark",
                  "coords": [
                        -109.028632,
                        25.746627
                  ]
            },
            {
                  "name": "Parroquia de Nuestra Señora del Rosario",
                  "text": "Parroquia de Nuestra Señora del Rosario este o biserică istorică din secolul al XVI-lea.",
                  "category": "castle",
                  "coords": [
                        -108.994859,
                        25.7683
                  ]
            },
            {
                  "name": "Trapiche Museo Interactivo",
                  "text": "Trapiche Museo Interactivo este un muzeu interactiv despre istoria producției de zahăr.",
                  "category": "museum",
                  "coords": [
                        -108.999103,
                        25.788559
                  ]
            },
            {
                  "name": "Entrada",
                  "text": "Intrarea principală (Entrada) duce în zona livezilor și plantațiilor de mango.",
                  "category": "landmark",
                  "coords": [
                        -109.001832,
                        25.788583
                  ]
            },
            {
                  "name": "La Pergola",
                  "text": "Pergolă naturală formată din mango în Mango Orchards.",
                  "category": "natural",
                  "coords": [
                        -108.972815,
                        25.805474
                  ]
            },
            {
                  "name": "Casa de La Cultura Conrado Espinoza",
                  "text": "Muzeu de cultură numit după scriitorul Conrado Espinoza în Mango Orchards.",
                  "category": "museum",
                  "coords": [
                        -108.972525,
                        25.80447
                  ]
            },
            {
                  "name": "Museo Del Valle del Fuerte",
                  "text": "Muzeul Văii Fuerte cu exponate de istorie regională în Mango Orchards.",
                  "category": "museum",
                  "coords": [
                        -108.999588,
                        25.790209
                  ]
            },
            {
                  "name": "Ruinas de la Casa Grande",
                  "text": "Ruinele unei mari case coloniale în Mango Orchards.",
                  "category": "historical",
                  "coords": [
                        -109.003358,
                        25.789639
                  ]
            },
            {
                  "name": "Jardín Botánico Benjamin Francis Johnston (Parque Sinaloa)",
                  "text": "Grădina Botanică Benjamin Francis Johnston, parte a Parcului Sinaloa, cu plante native.",
                  "category": "landmark",
                  "coords": [
                        -109.005038,
                        25.789143
                  ]
            },
            {
                  "name": "Teatro Ingenio",
                  "text": "Teatrul Ingenio, un loc cultural în Mango Orchards.",
                  "category": "cultural",
                  "coords": [
                        -109.000382,
                        25.788697
                  ]
            }
      ],
      "en": [
            {
                  "name": "CUM Los Mochis",
                  "text": "CUM Los Mochis is a university campus set among mango orchards.",
                  "category": "landmark",
                  "coords": [
                        -109.028632,
                        25.746627
                  ]
            },
            {
                  "name": "Parroquia de Nuestra Señora del Rosario",
                  "text": "Parroquia de Nuestra Señora del Rosario is a historic 16th-century church.",
                  "category": "castle",
                  "coords": [
                        -108.994859,
                        25.7683
                  ]
            },
            {
                  "name": "Trapiche Museo Interactivo",
                  "text": "Trapiche Museo Interactivo is an interactive museum about the history of sugar production.",
                  "category": "museum",
                  "coords": [
                        -108.999103,
                        25.788559
                  ]
            },
            {
                  "name": "Entrada",
                  "text": "The entrance (Entrada) leads into the area of mango orchards and plantations.",
                  "category": "landmark",
                  "coords": [
                        -109.001832,
                        25.788583
                  ]
            },
            {
                  "name": "La Pergola",
                  "text": "Natural pergola formed by mango trees in Mango Orchards.",
                  "category": "natural",
                  "coords": [
                        -108.972815,
                        25.805474
                  ]
            },
            {
                  "name": "Casa de La Cultura Conrado Espinoza",
                  "text": "Cultural house museum named after writer Conrado Espinoza in Mango Orchards.",
                  "category": "museum",
                  "coords": [
                        -108.972525,
                        25.80447
                  ]
            },
            {
                  "name": "Museo Del Valle del Fuerte",
                  "text": "Museum of the Fuerte Valley featuring regional history exhibits in Mango Orchards.",
                  "category": "museum",
                  "coords": [
                        -108.999588,
                        25.790209
                  ]
            },
            {
                  "name": "Ruinas de la Casa Grande",
                  "text": "Ruins of a large colonial house in Mango Orchards.",
                  "category": "historical",
                  "coords": [
                        -109.003358,
                        25.789639
                  ]
            },
            {
                  "name": "Jardín Botánico Benjamin Francis Johnston (Parque Sinaloa)",
                  "text": "Benjamin Francis Johnston Botanical Garden, part of Parque Sinaloa, with native plants.",
                  "category": "landmark",
                  "coords": [
                        -109.005038,
                        25.789143
                  ]
            },
            {
                  "name": "Teatro Ingenio",
                  "text": "Teatro Ingenio, a cultural venue in Mango Orchards.",
                  "category": "cultural",
                  "coords": [
                        -109.000382,
                        25.788697
                  ]
            }
      ]
},
    type: "agriculture",
    parent: "MX-SIN",
    coords: [-108.98, 25.79],
    name: { de: "Mangohaine in Sinaloa", hu: "Sinaloa mango-ultetvenyek", ro: "Livezi de mango din Sinaloa", en: "Sinaloa Mango Orchards"},
    image: "/poi-images/sinaloa-mango-orchards-life-v2.webp",
    descriptionAdvanced: {
      de: "Sinaloa ist der führende Produzent von Mangos in Mexiko und bekannt für die hohe Qualität seiner Früchte, die weltweit exportiert werden. Die weiten Ebenen des Bundesstaates sind im Sommer von den tiefgrünen Kronen der Mangobäume bedeckt, die Sorten wie Ataulfo, Kent und Keitt hervorbringen. Besonders die Region um Mazatlán und Rosario ist für ihre süßen und saftigen Früchte berühmt. Der Mangoanbau ist ein entscheidender Faktor für die ländliche Wirtschaft und erfordert während der Erntezeit tausende fleißige Hände. Die reifen Mangos aus Sinaloa sind ein Inbegriff des tropischen Mexikos und finden ihren Weg in unzählige Gerichte und Märkte weltweit. Die Modernisierung der Bewässerungssysteme hat Sinaloa zu einer agrarindustriellen Macht im Nordwesten des Landes gemacht. Geographie K7 - Bewässerungslandwirtschaft und Exportwirtschaft.",
      hu: "Sinaloa mango-ultetvenyek egy figyelemre melto termeszeti es elmenyhelyszin Mexikoban, amely egyedi hangulataval es kulturalis jelentosegevel vonzza a latogatokat. A helyszin es kornyezete autentikus betekintest nyujt a regio identitasaba, ahol a hagyomanyos eletmod es a modern fejlodes talalkozik. Az utazok ertekelik a tortenelem, a termeszet es a hetkoznapi kultura valtozatos elegyet, amely meghatarozza ezt a helyet. A kornyek bejarasa soran tipikus epiteszet, helyi piacok es jellegzetes tajformak fedezhetok fel. Sinaloa mango-ultetvenyek kivaloan alkalmas egy olyan kirandulasra, amely egyszerre kinal ismeretszerzest es kikapcsolodast. Egy latogatas melyebb betekintest nyujt Mexiko sokszinusegebe, es maradando elmennyel ajandekozza meg az utazokat.",
      ro: "Livezi de mango din Sinaloa este un loc natural si de experienta din Mexic remarcabil, care atrage vizitatorii prin atmosfera sa unica si semnificatia culturala deosebita. Locatia si imprejurimile ofera o privire autentica asupra identitatii regionale, in care modul de viata traditional se intalneste cu dezvoltarile moderne. Calatorii apreciaza amestecul variat de istorie, natura si cultura cotidiana care defineste acest loc. Zona inconjuratoare invita la explorare, oferind arhitectura tipica, piete locale si forme de relief caracteristice. Livezi de mango din Sinaloa este potrivit pentru o excursie care imbina invatarea cu relaxarea. O vizita ofera o intelegere mai profunda a diversitatii Mexicului si ramane o experienta memorabila.",
      en: "The coastal plains of Sinaloa are one of Mexico's most productive agricultural regions, particularly famous for their vast and sun-drenched mango orchards. The state's warm, tropical climate and extensive irrigation systems make it a leading producer and exporter of several mango varieties, including the popular Kent, Keitt, and the sweet, buttery Ataulfo. During the harvest season, which peaks in the summer months, the region is a hive of activity as thousands of tons of fruit are carefully picked, packed, and shipped to markets across North America. The orchards form a verdant canopy over the landscape, particularly around the southern part of the state near towns like Rosario and Escuinapa. Beyond their economic value, mangoes are a beloved part of the local culture, celebrated in festivals and enjoyed in countless fresh and spicy preparations. The Sinaloa mango industry is known for its high safety and quality standards, ensuring that its delicious fruit reaches international consumers at the peak of ripeness. It remains a vital pillar of the state's robust agricultural economy.",
      es: "Sinaloa es el principal productor de mangos en México y es conocido por la alta calidad de sus frutas, que se exportan a todo el mundo. Las vastas llanuras del estado se cubren en verano con las copas verde oscuro de los mangos, que producen variedades como Ataulfo, Kent y Keitt. Especialmente la región de Mazatlán y Rosario es famosa por sus frutos dulces y jugosos. El cultivo de mango es un factor decisivo para la economía rural y requiere miles de manos trabajadoras durante la cosecha. Los mangos maduros de Sinaloa son un símbolo del México tropical y llegan a innumerables platos y mercados internacionales. La modernización de los sistemas de riego ha convertido a Sinaloa en una potencia agroindustrial en el noroeste del país.",
      pt: "Sinaloa é o principal produtor de mangas no México e é conhecido pela alta qualidade de seus frutos, que são exportados para todo o mundo. As vastas planícies do estado são cobertas no verão pelas copas verde-escuras das mangueiras, que produzem variedades como Ataulfo, Kent e Keitt. Especialmente a região de Mazatlán e Rosario é famosa por seus frutos doces e suculentos. O cultivo da manga é um fator decisivo para a economia rural e exige milhares de mãos trabalhadoras durante a colheita. As mangas maduras de Sinaloa são um símbolo do México tropical e encontram seu caminho em inúmeros pratos e mercados mundiais. A modernização dos sistemas de irrigação tornou Sinaloa uma potência agroindustrial no noroeste do país.",
      fr: "Sinaloa est le principal producteur de mangues au Mexique et est réputé pour la haute qualité de ses fruits, exportés dans le monde entier. Les vastes plaines de l'État se couvrent en été des frondaisons vert foncé des manguiers, produisant des variétés telles que l'Ataulfo, le Kent et le Keitt. La région de Mazatlán et Rosario est particulièrement célèbre pour ses fruits sucrés et juteux. La culture de la mangue est un facteur décisif pour l'économie rurale et nécessite des milliers de bras pendant la période de récolte. Les mangues mûres de Sinaloa sont l'incarnation du Mexique tropical et se retrouvent dans d'innombrables plats et marchés mondiaux. La modernisation des systèmes d'irrigation a fait de Sinaloa une puissance agro-industrielle dans le nord-ouest du pays.",
    },
    factsAdvanced: {
      de: ["Mexiko ist der weltweit führende Exporteur von frischen Mangos.", "Sinaloa produziert jährlich mehr als 350.000 Tonnen dieser Tropenfrucht.", "Die Sorte 'Ataulfo' ist eine rein mexikanische Züchtung mit cremiger Textur.", "Mangobäume können über 100 Jahre alt werden und immer noch Früchte tragen.", "Die Haupterntezeit in Sinaloa liegt zwischen den Monaten Mai und August.", "Moderne Verpackungsanlagen in Sinaloa erfüllen höchste internationale Standards.", "Mangos sind reich an Vitamin A und C und ein wichtiger Bestandteil der lokalen Ernährung."],
      hu: ["Sinaloa mango-ultetvenyek sokszinu elmenyt kinal a termeszetkedvelok szamara.", "A letesitmeny kiemelt figyelmet fordit a kornyezeti nevelesre es a fajvedelemre.", "A csaladok szamos gyermekbarat lehetoseget talalnak itt.", "Hazai es egzotikus fajok egyutt jelennek meg a bemutatokban.", "Vezetett seta soran az allatok elohelyei es viselkedese is megismerheto.", "Sinaloa mango-ultetvenyek egesz evben kedvelt kirandulocel.", "Specialis programok melyitik a Mexiko biodiverzitasarol szerzett ismereteket."],
      ro: ["Livezi de mango din Sinaloa ofera experiente variate pentru iubitorii naturii.", "Institutia pune accent pe educatia ecologica si protectia speciilor.", "Familiile gasesc aici numeroase activitati potrivite copiilor.", "Sunt prezentate atat specii locale, cat si exotice.", "Tururile ghidate explica habitatele si comportamentul animalelor.", "Livezi de mango din Sinaloa este o destinatie populara pe tot parcursul anului.", "Programe speciale aprofundeaza cunostintele despre biodiversitatea Mexicului."],
      en: [
        "Sinaloa is one of the top mango-producing states in Mexico, along with Guerrero and Nayarit.",
        "The Ataulfo mango, also known as the 'honey' or 'champagne' mango, is a native Mexican variety with a non-fibrous texture.",
        "Mango trees can grow up to 35-40 meters tall and live for over 100 years.",
        "The state of Sinaloa has a rigorous 'fruit fly-free' certification program to facilitate international exports.",
        "Mexico is the world's leading exporter of fresh mangoes, with a significant portion coming from Sinaloa's orchards."
      ],
      es: ["México es el principal exportador mundial de mangos frescos.", "Sinaloa produce anualmente más de 350,000 toneladas de esta fruta tropical.", "La variedad 'Ataulfo' es una raza puramente mexicana con textura cremosa.", "Los manguiers pueden vivir más de 100 años y seguir dando frutos.", "La temporada principal de cosecha en Sinaloa es entre mayo y agosto.", "Las modernas plantas de empaque en Sinaloa cumplen con los más altos estándares internacionales.", "Los mangos son ricos en vitamina A y C y un componente importante de la dieta local."],
      pt: ["O México é o maior exportador mundial de mangas frescas.", "Sinaloa produz anualmente mais de 350.000 toneladas desta fruta tropical.", "A variedade 'Ataulfo' é uma linhagem puramente mexicana com textura cremosa.", "As mangueiras podem viver mais de 100 anos e ainda dar frutos.", "A principal época de colheita em Sinaloa ocorre entre os meses de maio e agosto.", "As modernas instalações de embalagem em Sinaloa cumprem os mais altos padrões internacionais.", "As mangas são ricas em vitamina A e C e um componente importante da dieta local."],
      fr: ["Le Mexique est le premier exportateur mondial de mangues fraîches.", "Sinaloa produit plus de 350 000 tonnes de ce fruit tropical par an.", "La variété 'Ataulfo' est une sélection purement mexicaine à la texture crémeuse.", "Les manguiers peuvent vivre plus de 100 ans et continuer à porter des fruits.", "La principale période de récolte à Sinaloa se situe entre mai et août.", "Les usines d'emballage modernes de Sinaloa respectent les normes internationales les plus strictes.", "Les mangues sont riches en vitamines A et C et constituent un élément important de l'alimentation locale."],
    },
    description: {
      de: "Ausgedehnte Mango-Plantagen in Sinaloa, einer der produktivsten Agrarregionen Mexikos.",
      hu: "Sinaloa tengerparti síkságain elterülő trópusi gyümölcsösök, Mexikó jelentős mangóexport-központja.",
      ro: "Livezi tropicale luxuriante în câmpiile de coastă din Sinaloa, unul dintre cele mai productive centre de export de mango din Mexic.",
      en: "Lush tropical orchards in the coastal plains of Sinaloa, one of Mexico's most productive agricultural and mango-exporting hubs.",
      es: "Extensas plantaciones de mango en Sinaloa, una de las regiones agrícolas más productivas de México.",
      pt: "Extensas plantações de manga em Sinaloa, uma das regiões agrícolas mais produtivas do México.",
      fr: "Vastes plantations de manguiers à Sinaloa, l'une des régions agricoles les plus productives du Mexique.",
    },
    facts: {
      de: ["Wichtiger Exporteur in die USA", "Mehrere Sorten wie Ataulfo", "Tropisches Küstenklima"],
      hu: ["Ataulfo mangó fő exportőre", "Trópusi tengerparti klíma", "Nyári csúcsszezon a betakarításkor"],
      ro: ["Lider în exportul de mango Ataulfo", "Climă de coastă caldă și tropicală", "Vârful recoltei are loc în lunile de vară"],
      en: ["Leading exporter of Ataulfo mangoes", "Warm, tropical coastal climate", "Peak harvest in summer months"],
      es: ["Importante exportador a los EE. UU.", "Varias variedades como Ataulfo", "Clima costero tropical"],
      pt: ["Importante exportador para os EUA", "Várias variedades como Ataulfo", "Clima costeiro tropical"],
      fr: ["Important exportateur vers les États-Unis", "Plusieurs variétés comme l'Ataulfo", "Climat côtier tropical"],
    }, image: "/poi-images/sinaloa-mango-orchards-life-v2.webp"},
];

