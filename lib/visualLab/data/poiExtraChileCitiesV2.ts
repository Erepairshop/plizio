// @ts-nocheck
import type { POI } from "./poi";

export const poiExtraChileCitiesV2: POI[] = [
  {
    id: "santiago-cities-v2",
    type: "state-capital",
    parent: "CL-RM",
    coords: [-70.6693, -33.4489],
    name: { de: "Santiago", hu: "Santiago", ro: "Santiago", en: "Santiago"},
    image: "/poi-images/santiago-cities-v2.webp",
    description: {
      de: "Die Hauptstadt und größte Metropole von Chile, gelegen in einem zentralen Becken.",
      hu: "Chile fővárosa és legnagyobb metropolisza, amely egy központi medencében fekszik.",
      ro: "Capitala și cea mai mare metropolă a statului Chile, situată într-un bazin central.",
      en: "The capital and largest metropolis of Chile, located in a central basin.",
      es: "La capital y metrópolis más grande de Chile, situada en una cuenca central.",
      pt: "A capital e maior metrópole do Chile, situada numa bacia central.",
      fr: "La capitale et plus grande métropole du Chili, située dans un bassin central.",
    },
    descriptionAdvanced: {
      de: "Santiago de Chile wurde 1541 von Pedro de Valdivia am Ufer des Flusses Mapocho gegründet und liegt in einem zentralen Becken auf einer Höhe von etwa 520 Metern über dem Meeresspiegel. Die Stadt ist das unbestrittene politische, kulturelle und wirtschaftliche Zentrum Chiles und beherbergt fast 40 Prozent der Gesamtbevölkerung des Landes. Die Architektur im historischen Zentrum wird durch prachtvolle Gebäude aus der Kolonialzeit wie die Metropolitan-Kathedrale und den Palacio de La Moneda geprägt, der heute als Amtssitz des Präsidenten dient. Im Osten der Stadt ragen die majestätischen Anden auf, deren Gipfel oft ganzjährig schneebedeckt sind und das urbane Stadtbild auf einzigartige Weise dominieren. Santiago hat sich im 21. Jahrhundert zu einer hochmodernen Metropole mit gläsernen Wolkenkratzern wie dem Gran Torre Santiago entwickelt, der mit 300 Metern Höhe das höchste Gebäude Südamerikas ist. Das Klima ist mediterran geprägt, was den Weinbau in den umliegenden Tälern wie dem Maipo-Tal begünstigt, welches für seine erstklassigen Rotweine weltberühmt ist. Die Stadt verfügt zudem über eines der modernsten und effizientesten Metrosysteme des Kontinents.",
      hu: "Santiagót 1541-ben alapította Pedro de Valdivia a Mapocho-folyó partján, és ma Chile vitathatatlan politikai, gazdasági és kulturális központja. A város egy termékeny központi völgyben helyezkedik el, átlagosan 520 méteres tengerszint feletti magasságban, az Andok hófödte csúcsainak lábánál, amelyek látványa meghatározza a városképet. Santiago történelmi központjában számos koloniális épület maradt fenn, köztük a Metropolitan katedrális és a La Moneda palota, amely az elnöki székhelyként szolgál. A 20. század végétől kezdve a város gyors modernizáción ment keresztül, aminek eredményeként megépült a 300 méter magas Gran Torre Santiago, Dél-Amerika egyik legmagasabb felhőkarcolója. A metropolisz lakossága a környező elővárosokkal együtt meghaladja a 6 millió főt, ami az ország népességének jelentős részét teszi ki. A mediterrán éghajlat ideális feltételeket biztosít a közeli Maipo-völgy híres bortermeléséhez, amely világszerte elismert kiváló minőségű vörösborairól. A város emellett a kontinens egyik legfejlettebb és legtisztább metróhálózatával büszkélkedhet.",
      ro: "Santiago de Chile a fost fondat în 1541 de exploratorul Pedro de Valdivia pe malurile râului Mapocho, devenind centrul politic, cultural și economic suprem al țării. Orașul este situat într-o vale centrală fertilă, la o altitudine de aproximativ 520 de metri, fiind flancat la est de crestele înalte ale Anzilor, care oferă un fundal spectaculos metropolei. Centrul istoric păstrează monumente coloniale importante, precum Catedrala Metropolitană și Palatul La Moneda, sediul guvernului chilian și un simbol al istoriei naționale. În ultimele decenii, Santiago s-a transformat într-o metropolă modernă, găzduind Gran Torre Santiago, care la 300 de metri este cea mai înaltă clădire din America de Sud. Populația zonei metropolitane depășește 6 milioane de locuitori, reprezentând aproape 40% din totalul locuitorilor statului Chile. Climatul mediteranean al regiunii permite dezvoltarea agriculturii și a viticulturii de renume mondial în văile adiacente, precum Valea Maipo. Orașul dispune de o infrastructură avansată, inclusiv unul dintre cele mai eficiente sisteme de metrou din America Latină.",
      en: "Santiago was founded in 1541 by Pedro de Valdivia on the banks of the Mapocho River and serves as the political, cultural, and economic heart of Chile. The city is situated in a central basin at an elevation of approximately 520 meters, dominated by the majestic peaks of the Andes Mountains to the east. The historical center features colonial-era landmarks such as the Metropolitan Cathedral and the La Moneda Palace, the seat of the Chilean president. Since the late 20th century, Santiago has evolved into a modern metropolis, exemplified by the 300-meter-tall Gran Torre Santiago, the highest building in South America. The metropolitan area is home to over 6 million people, concentrating nearly 40 percent of the nation's total population. Its Mediterranean climate supports extensive vineyards in the surrounding valleys, making the region a global leader in wine production. The city is also known for its sophisticated public transport system, including the continent's most modern metro network, which connects its diverse residential and financial districts.",
    },
    factsAdvanced: {
      de: ["Gründung: 12. Februar 1541", "Höhe des Gran Torre Santiago: 300 Meter", "Bevölkerung der Metropolregion: ca. 6,2 Millionen", "Durchschnittliche Höhe: 520 m ü. d. M.", "Metrosystem: Ältestes und größtes in Südamerika", "Entfernung zum Pazifik: ca. 100 Kilometer"],
      hu: ["Alapítás dátuma: 1541. február 12.", "Gran Torre Santiago magassága: 300 méter", "Metropolisz lakossága: kb. 6,2 millió fő", "Tengerszint feletti magasság: 520 méter", "Metróhálózat hossza: több mint 140 kilométer", "Éghajlat: Mediterrán jellegű száraz nyárral"],
      ro: ["Data fondării: 12 februarie 1541", "Înălțimea Gran Torre Santiago: 300 metri", "Populația zonei metropolitane: aprox. 6,2 milioane", "Altitudine medie: 520 metri", "Rețea de metrou: Cea mai extinsă din America de Sud", "Climat: Mediteranean cu influențe oceanice"],
      en: ["Date founded: February 12, 1541", "Gran Torre Santiago height: 300 meters", "Metropolitan population: ~6.2 million", "Average elevation: 520 meters", "Metro system: Longest and most advanced in SA", "Distance to Pacific coast: ~100 km"],
      es: [],
      pt: [],
      fr: [],
    },
    
    
    facts: {
      de: ["Wurde 1541 gegründet.", "Das Zentrum der chilenischen Wirtschaft."],
      hu: ["1541-ben alapították.", "A chilei gazdaság központja."],
      ro: ["A fost fondată în 1541.", "Centrul economiei chiliene."],
      en: ["Founded in 1541.", "The center of the Chilean economy."],
      es: ["Fue fundada en 1541.", "El centro de la economía chilena."],
      pt: ["Foi fundada em 1541.", "O centro da economia chilena."],
      fr: ["Fondée en 1541.", "Le centre de l'économie chilienne."],
    },
     image: "/poi-images/santiago-cities-v2.webp"},
  {
    id: "puente-alto-cities-v2",
    type: "city",
    parent: "CL-RM",
    coords: [-70.5753, -33.6117],
    name: { de: "Puente Alto", hu: "Puente Alto", ro: "Puente Alto", en: "Puente Alto"},
    image: "/poi-images/puente-alto-cities-v2.webp",
    description: {
      de: "Eine der bevölkerungsreichsten Städte Chiles, südlich der Hauptstadt gelegen.",
      hu: "Chile egyik legnépesebb városa, a fővárostól délre helyezkedik el.",
      ro: "Una dintre cele mai populate orașe din Chile, situată la sud de capitală.",
      en: "One of the most populous cities in Chile, located south of the capital.",
      es: "Una de las ciudades más pobladas de Chile, situada al sur de la capital.",
      pt: "Uma das cidades mais povoadas do Chile, situada ao sul da capital.",
      fr: "L'une des villes les plus peuplées du Chili, située au sud de la capitale.",
    },
    descriptionAdvanced: {
      de: "Puente Alto liegt am südöstlichen Rand der Metropolregion Santiago und ist einer der bevölkerungsreichsten Bezirke Chiles. Die Stadt wurde offiziell im Jahr 1898 gegründet und hat sich von einer ländlichen Siedlung zu einem bedeutenden städtischen Zentrum entwickelt, das heute als Wohnort für hunderttausende Pendler dient. Geografisch wird Puente Alto vom Fluss Maipo im Süden begrenzt, der eine entscheidende Rolle für die lokale Wasserversorgung und die Bewässerung der umliegenden Agrarflächen spielt. In der Region ist die traditionsreiche Weinkellerei Concha y Toro ansässig, eines der weltweit größten Weinexportunternehmen, dessen historische Weinberge das Landschaftsbild prägen. Die Anbindung an das Zentrum von Santiago erfolgt über die moderne Linie 4 der Metro, was die Stadt eng in das wirtschaftliche Gefüge der Hauptstadt integriert. Trotz der dichten Besiedlung bietet Puente Alto durch seine Nähe zu den Ausläufern der Anden Zugang zu natürlichen Erholungsgebieten und Wanderwegen. Die Stadt kombiniert industrielle Zonen mit ausgedehnten Wohngebieten und einer wachsenden kommerziellen Infrastruktur.",
      hu: "Puente Alto Santiago metropolisz területének délkeleti szélén található, és Chile egyik legnépesebb kerületeként ismert. A várost hivatalosan 1898-ban alapították, és azóta egy mezőgazdasági jellegű településből jelentős városi központtá fejlődött, amely ma százezrek számára biztosít lakóhelyet. Földrajzilag délről a Maipo-folyó határolja, amely alapvető fontosságú a helyi vízellátás és az öntözéses gazdálkodás szempontjából a régióban. Itt található a híres Concha y Toro borászat központja, amely a világ egyik legnagyobb bortermelője és exportőre, pincészete pedig népszerű turisztikai célpont. A város közlekedését nagyban segíti a Santiago-i metró 4-es vonala, amely közvetlen kapcsolatot biztosít a főváros üzleti negyedeivel és egyetemeivel. Puente Alto lakossága az utóbbi évtizedekben robbanásszerűen megnőtt, miközben megőrizte ipari jelentőségét a papírgyártás és a textilipar területén. A közeli Andok közelsége különleges mikroklímát és számos kirándulóhelyet biztosít a lakók számára.",
      ro: "Puente Alto este situat la periferia sud-estică a regiunii metropolitane Santiago și este una dintre cele mai populate comune din Chile. Fondat oficial în 1898, orașul a evoluat spectaculos de la o comunitate rurală la un centru urban dens, esențial pentru dinamica demografică a capitalei. Din punct de vedere geografic, este delimitat la sud de râul Maipo, care furnizează resursa vitală de apă pentru consum și pentru vastele terenuri agricole din jur. În această zonă își are sediul renumita cramă Concha y Toro, unul dintre cei mai mari producători și exportatori de vin la nivel mondial, atrăgând anual mii de vizitatori. Accesibilitatea către centrul orașului Santiago este facilitată de Linia 4 a metroului, transformând comuna într-o importantă zonă rezidențială pentru clasa muncitoare și cea de mijloc. Deși este intens urbanizat, Puente Alto beneficiază de proximitatea față de Anzi, oferind acces rapid către peisaje montane și zone de recreere naturală. Economia locală este diversificată, incluzând sectoare precum comerțul, industria hârtiei și servicii.",
      en: "Puente Alto is located on the southeastern edge of the Santiago Metropolitan Region and ranks as one of the most populous communes in Chile. Officially established in 1898, the city transitioned from a rural settlement into a major urban center that now serves as a key residential hub for the capital's workforce. Geographically, it is bordered to the south by the Maipo River, which is vital for the local water supply and agricultural irrigation in the surrounding valley. The area is home to the world-renowned Concha y Toro winery, one of the largest wine producers and exporters globally, whose historic cellars are a major attraction. Public transportation is anchored by Line 4 of the Santiago Metro, providing a direct and efficient link to the capital's financial and educational districts. Despite its high population density, Puente Alto maintains a connection to nature due to its location at the foothills of the Andes Mountains. The commune has seen rapid commercial development in recent years, complementing its established industrial sectors such as paper manufacturing.",
    },
    factsAdvanced: {
      de: ["Gründungsjahr: 1898", "Einwohnerzahl: über 560.000", "Heimat von Concha y Toro (gegr. 1883)", "Südliche Grenze: Fluss Maipo", "Anbindung: Metro-Linie 4", "Durchschnittliche Höhe: 700 m ü. d. M."],
      hu: ["Alapítás éve: 1898", "Lakosság: több mint 560 000 fő", "Borászat: Concha y Toro központja", "Folyó: Maipo határolja délről", "Közlekedés: Metro 4-es vonal végállomása", "Ipar: Jelentős papírgyártás"],
      ro: ["Anul fondării: 1898", "Populație: peste 560.000 locuitori", "Economie: Sediul cramei Concha y Toro", "Geografie: Limitat de râul Maipo", "Transport: Conexiune prin Linia 4 de metrou", "Suprafață: 88,2 km²"],
      en: ["Year founded: 1898", "Population: over 560,000", "Home to Concha y Toro winery (est. 1883)", "Border: Maipo River to the south", "Transport: Connected by Metro Line 4", "Elevation: ~700 meters above sea level"],
      es: [],
      pt: [],
      fr: [],
    },
    
    
    facts: {
      de: ["Bekannt für große Wohngebiete.", "Wichtiges urbanes Zentrum."],
      hu: ["Hatalmas lakóövezeteiről ismert.", "Fontos városi központ."],
      ro: ["Cunoscută pentru zonele rezidențiale extinse.", "Centru urban important."],
      en: ["Known for large residential areas.", "Important urban center."],
      es: ["Conocida por sus grandes zonas residenciales.", "Importante centro urbano."],
      pt: ["Conhecida pelas suas grandes zonas residenciais.", "Importante centro urbano."],
      fr: ["Connue pour ses grandes zones résidentielles.", "Important centre urbain."],
    },
     image: "/poi-images/puente-alto-cities-v2.webp"},
  {
    id: "maipu-cities-v2",
    type: "city",
    parent: "CL-RM",
    coords: [-70.7578, -33.5100],
    name: { de: "Maipú", hu: "Maipú", ro: "Maipú", en: "Maipú"},
    image: "/poi-images/maipu-cities-v2.webp",
    description: {
      de: "Ein historischer Vorort von Santiago, berühmt für die Schlacht von Maipú.",
      hu: "Santiago történelmi külvárosa, amely a maipúi csatáról híres.",
      ro: "O suburbie istorică a orașului Santiago, faimoasă pentru bătălia de la Maipú.",
      en: "A historic suburb of Santiago, famous for the Battle of Maipú.",
      es: "Un suburbio histórico de Santiago, famoso por la Batalla de Maipú.",
      pt: "Um subúrbio histórico de Santiago, famoso pela Batalha de Maipú.",
      fr: "Une banlieue historique de Santiago, célèbre pour la bataille de Maipú.",
    },
    descriptionAdvanced: {
      de: "Maipú ist ein historisch bedeutsamer Bezirk im Südwesten der Metropolregion Santiago und bekannt als der Ort, an dem Chile seine endgültige Unabhängigkeit sicherte. Am 5. April 1818 besiegten die chilenischen und argentinischen Streitkräfte unter der Führung von José de San Martín und Bernardo O'Higgins die spanischen Royalisten in der entscheidenden Schlacht von Maipú. Zum Gedenken an diesen Sieg wurde der Templo Votivo de Maipú errichtet, eine monumentale Basilika, die heute ein nationales Wahrzeichen und einer der wichtigsten Wallfahrtsorte des Landes ist. Geografisch erstreckt sich der Bezirk über eine fruchtbare Ebene, die früher intensiv für den Getreideanbau genutzt wurde, bevor die Urbanisierung einsetzte. Heute ist Maipú ein dicht besiedeltes Wohn- und Industriegebiet mit einer modernen Infrastruktur, einschließlich einer direkten Anbindung an das Metronetz über die Linie 5. Die Stadt verbindet ihre reiche militärische Tradition mit einem dynamischen Bevölkerungswachstum und einer Vielzahl von Bildungseinrichtungen. Jedes Jahr finden hier große patriotische Feierlichkeiten statt, die an die Gründung der Republik erinnern.",
      hu: "Maipú Santiago metropolisz területének délnyugati részén fekszik, és Chile történelmének egyik legfontosabb helyszíne. 1818. április 5-én itt zajlott le a sorsdöntő maipúi csata, ahol a José de San Martín és Bernardo O'Higgins vezette egyesült seregek döntő győzelmet arattak a spanyol királyi erők felett, végleg biztosítva Chile függetlenségét. A győzelem tiszteletére épült a Templo Votivo de Maipú, egy monumentális bazilika, amely az ország egyik legfontosabb vallási és nemzeti jelképe, és 90 méteres magasságával messziről látható. A kerület eredetileg mezőgazdasági terület volt, de a 20. század második felében gyorsan iparosodott és Chile egyik legnépesebb negyedévé vált. Ma Maipú egy modern lakóövezet, amely kiváló közlekedési kapcsolatokkal, köztük a metró 5-ös vonalával rendelkezik a főváros központja felé. A város büszkén őrzi történelmi örökségét, miközben modern bevásárlóközpontokkal és kiterjedt parkokkal, például a Municipal Parkkal szolgálja ki lakóit.",
      ro: "Maipú este o comună cu o semnificație istorică profundă, situată în partea de sud-vest a regiunii metropolitane Santiago. La 5 aprilie 1818, pe câmpiile de aici a avut loc Bătălia de la Maipú, momentul în care forțele conduse de José de San Martín și Bernardo O'Higgins au învins definitiv armata regalistă spaniolă, pecetluind independența statului Chile. Pentru a onora acest eveniment crucial, a fost construit Templo Votivo de Maipú, o bazilică impozantă care servește drept simbol național și principal loc de pelerinaj catolic. Din punct de vedere geografic, zona a fost inițial un centru agricol fertil, transformându-se treptat într-un pol rezidențial și industrial major al capitalei. Astăzi, Maipú este una dintre cele mai populate zone din țară, beneficiind de o infrastructură modernă și conexiuni rapide asigurate de Linia 5 a metroului. Orașul îmbină monumentele sale istorice cu o dezvoltare urbană accelerată, găzduind numeroase centre comerciale și instituții de învățământ superior. Evenimentele anuale dedicate independenței atrag mii de turiști și localnici deopotrivă.",
      en: "Maipú is a commune of profound historical significance located in the southwestern part of the Santiago Metropolitan Region. It is famously known as the site of the Battle of Maipú, fought on April 5, 1818, where the combined forces of José de San Martín and Bernardo O'Higgins decisively defeated the Spanish Royalists, effectively securing Chilean independence. To commemorate this victory, the Templo Votivo de Maipú was constructed; this monumental basilica stands as a major national landmark and a prominent religious site. Geographically, the area occupies a flat, fertile plain that was dominated by wheat fields before undergoing rapid urbanization in the 20th century. Today, Maipú is a bustling residential and industrial hub with a sophisticated transport network, including a vital connection via Line 5 of the Santiago Metro. The commune remains a symbol of national pride, blending its military history with modern amenities such as large parks, university campuses, and commercial centers. It ranks as one of the most populous urban districts in Chile.",
    },
    factsAdvanced: {
      de: ["Schlacht von Maipú: 5. April 1818", "Höhe des Templo Votivo: ca. 90 Meter", "Einwohnerzahl: ca. 520.000", "Metro-Anbindung: Linie 5", "Status: Unabhängigkeit Chiles hier besiegelt", "Größter Park: Parque Municipal de Maipú"],
      hu: ["Maipúi csata dátuma: 1818. április 5.", "Templom magassága: kb. 90 méter", "Lakosság: kb. 520 000 fő", "Közlekedés: Metro 5-ös vonal", "Történelmi szerep: Chilei függetlenség helyszíne", "Fő látnivaló: Templo Votivo de Maipú"],
      ro: ["Bătălia de la Maipú: 5 aprilie 1818", "Înălțimea templului: aprox. 90 metri", "Populație: peste 520.000 locuitori", "Transport: Conexiune prin Linia 5 de metrou", "Semnificație: Locul independenței chiliene", "Obiectiv turistic: Muzeul Carmele"],
      en: ["Date of Battle of Maipú: April 5, 1818", "Templo Votivo height: ~90 meters", "Population: ~520,000", "Metro access: Line 5 (Green Line)", "Historical role: Birthplace of Chilean independence", "Park: Parque Municipal de Maipú"],
      es: [],
      pt: [],
      fr: [],
    },
    
    
    facts: {
      de: ["Hier steht der Templo Votivo.", "Schauplatz der Unabhängigkeit."],
      hu: ["Itt található a Templo Votivo.", "A függetlenség helyszíne."],
      ro: ["Aici se află Templo Votivo.", "Scena independenței."],
      en: ["Home to the Templo Votivo.", "Site of independence."],
      es: ["Aquí se encuentra el Templo Votivo.", "Escenario de la independencia."],
      pt: ["Aqui encontra-se o Templo Votivo.", "Cenário da independência."],
      fr: ["Lieu du Templo Votivo.", "Théâtre de l'indépendance."],
    },
     image: "/poi-images/maipu-cities-v2.webp"},
  {
    id: "la-florida-cities-v2",
    type: "city",
    parent: "CL-RM",
    coords: [-70.5258, -33.5256],
    name: { de: "La Florida", hu: "La Florida", ro: "La Florida", en: "La Florida"},
    image: "/poi-images/la-florida-cities-v2.webp",
    description: {
      de: "Eine dicht besiedelte Gemeinde im südöstlichen Teil der Hauptstadtregion.",
      hu: "Egy sűrűn lakott község a fővárosi régió délkeleti részén.",
      ro: "O comună dens populată în partea de sud-est a regiunii capitalei.",
      en: "A densely populated commune in the southeastern part of the capital region.",
      es: "Una comuna densamente poblada en la parte sureste de la región metropolitana.",
      pt: "Uma comuna densamente povoada na parte sudeste da região metropolitana.",
      fr: "Une commune densément peuplée dans la partie sud-est de la région métropolitaine.",
    },
    descriptionAdvanced: {
      de: "La Florida ist ein weitläufiger Wohnbezirk im Südosten von Santiago de Chile und gilt als eines der wichtigsten kommerziellen Zentren der Hauptstadt. Der Bezirk wurde im Jahr 1899 gegründet und entwickelte sich besonders in den 1980er und 1990er Jahren zu einer bevorzugten Wohngegend für die aufstrebende Mittelschicht des Landes. Ein markantes Merkmal ist die Mall Plaza Vespucio, eines der größten und erfolgreichsten Einkaufszentren Chiles, das als zentraler Treffpunkt und Wirtschaftsmotor der Region dient. Geografisch erstreckt sich La Florida von den flachen Ebenen im Westen bis hin zu den Ausläufern der Anden im Osten, was zu signifikanten Höhenunterschieden innerhalb des Stadtteils führt. Hier befindet sich auch der Panul-Wald, einer der letzten verbliebenen naturbelassenen Wälder in der Nähe des Stadtgebiets. Die medizinische Versorgung und Bildungsinfrastruktur sind durch zahlreiche Kliniken und moderne Universitätsstandorte überdurchschnittlich gut ausgebaut. Dank der Anbindung an mehrere Metrolinien ist der Bezirk heute einer der bestvernetzten Teile der Metropolregion Santiago.",
      hu: "La Florida Santiago de Chile délkeleti részén elhelyezkedő nagyméretű kerület, amely a főváros egyik legjelentősebb kereskedelmi és lakóövezete. Az 1899-ben alapított település a 20. század végén indult gyors fejlődésnek, amikor a középosztály számára vonzó modern lakónegyedek és társasházak épültek ki. Itt található a Mall Plaza Vespucio, Chile egyik legnagyobb és legforgalmasabb bevásárlóközpontja, amely a régió gazdasági életének és szórakoztatásának központja. A kerület földrajzilag rendkívül változatos, hiszen a nyugati síkságoktól egészen az Andok keleti lábaiig nyúlik, magában foglalva a Panul-erdőt, amely a város egyik utolsó természetes erdőmaradványa. La Florida kiváló oktatási és egészségügyi intézményekkel rendelkezik, és három metróvonal (4, 4A, 5) is érinti, biztosítva a gyors összeköttetést a város többi részével. Modern arculata, kiterjedt szolgáltatási szektora és zöldövezetei miatt az egyik legkeresettebb lakóhely a chilei fővárosban.",
      ro: "La Florida este o comună extinsă situată în sud-estul capitalei Santiago, fiind recunoscută drept unul dintre principalele centre comerciale și rezidențiale ale orașului. Fondată în 1899, localitatea a cunoscut o expansiune masivă în ultimele decenii ale secolului XX, devenind un simbol al creșterii și prosperității clasei de mijloc chiliene. Un punct de reper major este Mall Plaza Vespucio, unul dintre cele mai mari centre comerciale din țară, care atrage zilnic zeci de mii de vizitatori și susține economia locală. Geografic, comuna se întinde de la câmpia centrală până la poalele Munților Anzi, incluzând pădurea El Panul, un ecosistem nativ vital pentru calitatea aerului din metropolă. Infrastructura educațională și medicală este foarte dezvoltată, găzduind numeroase sedii universitare și clinici private de înaltă performanță. Conexiunea cu restul capitalei este excelentă, fiind asigurată de mai multe linii de metrou care facilitează tranzitul rapid. Comuna este, de asemenea, gazda unor evenimente sportive importante pe stadionul Bicentenario.",
      en: "La Florida is an expansive residential and commercial commune located in the southeastern part of Santiago, Chile. Established in 1899, it experienced explosive growth during the late 20th century, evolving into a premier hub for the country's middle class and a vital urban node. A prominent feature of the district is the Mall Plaza Vespucio, one of Chile’s largest and most iconic shopping centers, which serves as a major economic driver for the area. Geographically, La Florida stretches from the central plains to the rugged foothills of the Andes, encompassing the El Panul forest, the city's last remaining native Mediterranean forest ecosystem. The commune is exceptionally well-served by healthcare facilities and educational institutions, hosting various university satellite campuses and medical centers. Its integration with the rest of Santiago is unparalleled, as it is served by three major metro lines (4, 4A, and 5). La Florida continues to be a dynamic district characterized by its balance of commercial activity and suburban living.",
    },
    factsAdvanced: {
      de: ["Gründungsdatum: 28. November 1899", "Einwohnerzahl: ca. 360.000", "Haupteinkaufszentrum: Mall Plaza Vespucio", "Naturdenkmal: Panul-Wald (letzter Stadtwald)", "Metro-Linien: 4, 4A und 5", "Stadion: Estadio Bicentenario (12.000 Plätze)"],
      hu: ["Alapítás napja: 1899. november 28.", "Lakosság: kb. 360 000 fő", "Kereskedelem: Itt épült az ország egyik első plázája", "Természeti kincs: El Panul őshonos erdő", "Közlekedés: Három metróvonal csomópontja", "Sport: Estadio Bicentenario de La Florida"],
      ro: ["Data înființării: 28 noiembrie 1899", "Populație: aprox. 360.000 locuitori", "Comerț: Găzduiește Mall Plaza Vespucio", "Natură: Include pădurea nativă El Panul", "Metrou: Deservit de liniile 4, 4A și 5", "Sport: Stadionul Bicentenario (12.000 locuri)"],
      en: ["Date founded: November 28, 1899", "Population: ~360,000", "Shopping: Home to Mall Plaza Vespucio", "Environment: Contains El Panul native forest", "Metro service: Connected by lines 4, 4A, and 5", "Stadium capacity: 12,000 (Bicentenario)"],
      es: [],
      pt: [],
      fr: [],
    },
    
    
    facts: {
      de: ["Wichtiges Einkaufszentrum.", "Hat viele Grünflächen."],
      hu: ["Fontos bevásárlóközpont.", "Sok zöldterülettel rendelkezik."],
      ro: ["Centru comercial important.", "Are multe spații verzi."],
      en: ["Important shopping center.", "Has many green spaces."],
      es: ["Importante centro comercial.", "Tiene muchas zonas verdes."],
      pt: ["Importante centro comercial.", "Possui muitas áreas verdes."],
      fr: ["Important centre commercial.", "Possède de nombreux espaces verts."],
    },
     image: "/poi-images/la-florida-cities-v2.webp"},
  {
    id: "antofagasta-cities-v2",
    type: "state-capital",
    parent: "CL-AN",
    coords: [-70.4000, -23.6500],
    name: { de: "Antofagasta", hu: "Antofagasta", ro: "Antofagasta", en: "Antofagasta"},
    image: "/poi-images/antofagasta-cities-v2.webp",
    description: {
      de: "Eine Hafenstadt im Norden Chiles, die stark vom Bergbau geprägt ist.",
      hu: "Kikötőváros Észak-Chilében, amelyet erősen meghatároz a bányászat.",
      ro: "Un oraș portuar din nordul statului Chile, marcat puternic de minerit.",
      en: "A port city in northern Chile heavily influenced by mining.",
      es: "Una ciudad portuaria en el norte de Chile, fuertemente influenciada por la minería.",
      pt: "Uma cidade portuária no norte do Chile, fortemente influenciada pela mineração.",
      fr: "Une ville portuaire au nord du Chili, fortement marquée par l'industrie minière.",
    },
    descriptionAdvanced: {
      de: "Antofagasta ist eine bedeutende Hafenstadt im Norden Chiles und dient als Hauptstadt der gleichnamigen Region in der Atacama-Wüste. Die Stadt wurde 1868 gegründet und erlebte während des Salpeterbooms im späten 19. Jahrhundert einen rasanten Aufstieg, bevor sie nach dem Salpeterkrieg unter chilenische Kontrolle kam. Heute ist sie das weltweit wichtigste Logistikzentrum für den Kupferbergbau, wobei ein Großteil des weltweit produzierten Kupfers aus Minen wie Escondida über ihren Hafen verschifft wird. Die Geografie Antofagastas ist durch einen schmalen Küstenstreifen zwischen dem Pazifischen Ozean und dem steil aufragenden Küstengebirge geprägt, was der Stadt eine langgestreckte Form verleiht. Zu den architektonischen Sehenswürdigkeiten gehören die historischen Gebäude des Barrio Histórico und der Torre del Reloj auf der Plaza Colón, ein Geschenk der britischen Gemeinde. In der Nähe der Stadt befindet sich das berühmte Naturdenkmal La Portada, ein spektakulärer natürlicher Steinbogen im Meer. Das extrem trockene Klima macht die Stadt fast vollständig von der Entsalzung von Meerwasser abhängig.",
      hu: "Antofagasta Chile északi részének legjelentősebb kikötővárosa, amely az Atacama-sivatag szélén, a Csendes-óceán partján fekszik. Az 1868-ban alapított város a 19. század végi salétrom-láz idején vált fontos gazdasági központtá, ma pedig a globális rézbányászat logisztikai kapujának tekintik. A világ réztermelésének jelentős részét a város korszerű kikötőjén keresztül exportálják, ami meghatározza a település ipari és kereskedelmi karakterét. Földrajzi elhelyezkedése különleges: a város egy keskeny sávban nyúlik el az óceán és a parti hegyláncok között, hossza meghaladja a 20 kilométert. Antofagasta jelképe a La Portada természetvédelmi terület, ahol egy hatalmas természetes sziklakapu emelkedik ki a tengerből, különleges látványt nyújtva. A város történelmi negyedében brit építészeti hatások is felfedezhetőek, mint például a Plaza Colón óratornya, amely a Big Ben kicsinyített mása. A rendkívül száraz sivatagi éghajlat miatt a lakosság vízellátását hatalmas tengervíz-lepárló üzemek biztosítják.",
      ro: "Antofagasta este un oraș portuar major situat în nordul Chile, servind drept capitală a regiunii omonime în inima aridă a deșertului Atacama. Fondat în 1868 sub jurisdicție boliviană, orașul a trecut sub control chilian după Războiul Pacificului și a prosperat inițial datorită exploatării salpetrului. În prezent, este considerat „capitala mondială a cuprului”, fiind punctul principal de export pentru minereurile extrase din cele mai mari mine de suprafață din lume, precum Escondida. Orașul este situat pe o fâșie îngustă de pământ între Oceanul Pacific și Munții de Coastă, creând un peisaj urban liniar spectaculos. Printre atracțiile sale naturale se numără Monumentul Natural La Portada, un arc natural masiv de piatră format prin eroziune în largul mării. Din cauza absenței surselor de apă dulce, Antofagasta depinde strategic de una dintre cele mai mari uzine de desalinizare din America Latină pentru a-și susține populația și industria. Centrul orașului păstrează clădiri coloniale și un turn cu ceas donat de comunitatea britanică.",
      en: "Antofagasta is a major port city in northern Chile and serves as the capital of the Antofagasta Region within the Atacama Desert. Founded in 1868, the city rose to global prominence during the nitrate boom and is now the primary logistical hub for Chile's massive copper mining industry. It handles a significant portion of the world's copper exports, which are shipped through its modernized port facilities from nearby mines like Escondida. The geography of Antofagasta is characterized by a narrow coastal strip wedged between the Pacific Ocean and the steep Coastal Range, resulting in a long, linear urban layout. Notable landmarks include the historic buildings of the Barrio Histórico and the Clock Tower in Plaza Colón, which was a gift from the British community in the early 20th century. Just north of the city lies the La Portada Natural Monument, a stunning natural rock arch rising from the ocean waves. Due to its hyper-arid desert climate, the city relies heavily on advanced seawater desalination plants for its entire water supply.",
    },
    factsAdvanced: {
      de: ["Gründungsjahr: 1868", "Spitzname: Perle des Nordens", "Höhe von La Portada: 43 Meter", "Einwohnerzahl: ca. 360.000", "Wirtschaft: Kupfer-Exporthauptstadt", "Wasserversorgung: >60% durch Meerwasserentsalzung"],
      hu: ["Alapítás éve: 1868", "Becenév: Az észak gyöngye", "La Portada magassága: 43 méter", "Lakosság: kb. 360 000 fő", "Ipar: A világ egyik legnagyobb rézkikötője", "Vízellátás: Tengervíz-lepárlással megoldva"],
      ro: ["Anul fondării: 1868", "Supranume: Perla Nordului", "Înălțimea La Portada: 43 metri", "Populație: aprox. 360.000 locuitori", "Resurse: Cel mai mare port de export al cuprului", "Climat: Unul dintre cele mai aride din lume"],
      en: ["Year founded: 1868", "Nickname: Pearl of the North", "La Portada height: 43 meters", "Population: ~360,000", "Economy: Copper export capital of the world", "Water source: Seawater desalination plants"],
      es: [],
      pt: [],
      fr: [],
    },
    
    
    facts: {
      de: ["Bekannt als die Perle des Nordens.", "Zentrum der Kupferindustrie."],
      hu: ["Észak gyöngyeként ismert.", "A rézipar központja."],
      ro: ["Cunoscută ca Perla Nordului.", "Centrul industriei cuprului."],
      en: ["Known as the Pearl of the North.", "Center of the copper industry."],
      es: ["Conocida como la Perla del Norte.", "Centro de la industria del cobre."],
      pt: ["Conhecida como a Pérola do Norte.", "Centro da indústria do cobre."],
      fr: ["Connue comme la Perle du Nord.", "Centre de l'industrie du cuivre."],
    },
    
  }
];

