// @ts-nocheck
import type { POI } from "./poi";

export const poiExtraKenyaCitiesV2: POI[] = [
  {
    id: "nairobi-cities-v2",
    type: "state-capital",
    parent: "KE-47",
    coords: [36.8219, -1.2921],
    name: {
      de: "Nairobi",
      hu: "Nairobi",
      ro: "Nairobi",
      en: "Nairobi",
image: "/poi-images/nairobi-cities-v2.webp"
},
    descriptionAdvanced: {
      en: "Established in 1899 as a simple rail depot on the Uganda Railway, Nairobi has rapidly transformed into one of Africa's most prominent metropolises. Earning the moniker 'Green City in the Sun', it stands out globally by hosting the Nairobi National Park, a sprawling wildlife reserve situated merely 7 kilometers from its bustling central business district. The city serves as a crucial hub for international diplomacy, housing the headquarters of the United Nations Environment Programme (UNEP) since 1972. Sitting at an elevation of roughly 1,795 meters above sea level, Nairobi enjoys a remarkably temperate climate despite its proximity to the equator. Its skyline is defined by modern architectural marvels like the 200-meter-tall Britam Tower, completed in 2017. As the financial heartbeat of East Africa, the city drives innovation in mobile banking and tech startups. Geography K7 — African metropolises.",
      de: "Nairobi, die Hauptstadt Kenias, ist eine Metropole voller Kontraste, die oft als das „grüne Herz Afrikas“ bezeichnet wird. Gegründet 1899 als Eisenbahndepot, hat sich die Stadt zu einem bedeutenden Wirtschafts- und Politikzentrum Ostafrikas entwickelt. Einzigartig ist die unmittelbare Nähe zum Nairobi-Nationalpark, wo Wildtiere nur wenige Kilometer vom Stadtzentrum entfernt beobachtet werden können. Die Stadt ist zudem Sitz wichtiger internationaler Organisationen wie dem Umweltprogramm der Vereinten Nationen (UNEP). Der Name „Nairobi“ stammt aus der Massai-Sprache und bedeutet „kühler Wasserplatz“, was auf die Flüsse in der Region hinweist. Mit einer Bevölkerung von über 4,8 Millionen Menschen ist sie ein Schmelztiegel der Kulturen und ein dynamischer Ort, der Tradition und Moderne verbindet.",
      hu: "Nairobi, Kenya fővárosa és legnagyobb városa, egyben Kelet-Afrika egyik legfontosabb gazdasági és politikai központja. Az 1899-ben alapított település neve a maszáj „Enkare Nyirobi” kifejezésből ered, ami „hűvös vizek helyét” jelenti. A város beceneve, a „Zöld város a nap alatt” a számos parkjára és zöldterületére utal. Különlegessége, hogy a világ egyetlen fővárosa, amelynek határain belül egy nemzeti park is található. A Nairobi Nemzeti Park mindössze néhány kilométerre fekszik a nyüzsgő városközponttól, páratlan lehetőséget nyújtva a vadvilág megfigyelésére. A város otthont ad az ENSZ Környezetvédelmi Programjának (UNEP) és más nemzetközi szervezeteknek, ami tovább erősíti globális jelentőségét.",
      ro: "Nairobi, capitala și cel mai mare oraș din Kenya, este un centru vibrant al Africii de Est, fondat în 1899. Numele său provine din expresia Maasai „Enkare Nyirobi”, care se traduce prin „locul apelor reci”. Orașul este supranumit „Orașul Verde sub Soare” datorită parcurilor sale extinse și a vegetației luxuriante. O caracteristică unică a Nairobiului este prezența Parcului Național Nairobi în limitele sale administrative, fiind singura capitală din lume cu un parc național atât de aproape de centrul urban. Acesta găzduiește o faună sălbatică diversă la doar câțiva kilometri de zgârie-nori. Nairobi este, de asemenea, un important centru diplomatic, adăpostind sediul Programului Națiunilor Unite pentru Mediu (UNEP)."
    },
    factsAdvanced: {
      en: ["Founded in 1899 as a Uganda Railway supply depot.", "Replaced Machakos as the capital of the East Africa Protectorate in 1907.", "Situated at an altitude of approximately 1,795 meters (5,889 feet).", "Nairobi National Park was established in 1946, covering 117 square kilometers.", "The Britam Tower, completed in 2017, reaches a height of 200 meters.", "Hosts the UN Environment Programme (UNEP) headquarters, established in 1972.", "The city's population surpassed 4.3 million during the 2019 national census."],
      de: ["Gegründet 1899 als Versorgungsdepot der Uganda-Eisenbahn.", "Ersetzte 1907 Machakos als Hauptstadt des britischen Ostafrika-Protektorats.", "Liegt auf einer Höhe von ca. 1.795 Metern.", "Der Nairobi-Nationalpark wurde 1946 gegründet.", "Die Bevölkerung der Metropolregion beträgt etwa 5,7 Millionen Menschen.", "Beherbergt seit 1972 den Hauptsitz des UN-Umweltprogramms (UNEP)."],
      hu: [],
      ro: []
    },
    description: {
      de: "Nairobi ist die Hauptstadt und größte Stadt Kenias, bekannt als die 'grüne Stadt in der Sonne'.",
      hu: "Nairobi Kenya fővárosa és legnagyobb városa, amelyet 'zöld város a nap alatt' néven ismernek.",
      ro: "Nairobi este capitala și cel mai mare oraș din Kenya, cunoscut ca 'orașul verde sub soare'.",
      en: "Nairobi is the capital and largest city of Kenya, known as the 'Green City in the Sun'."
    },
    facts: {
      de: [
        "Wurde 1899 als Eisenbahndepot gegründet.",
        "Der Name stammt vom Massai-Wort 'Enkare Nyrobi' ab, was kühles Wasser bedeutet."
      ],
      hu: [
        "1899-ben alapították vasúti depóként.",
        "A név a maszáj 'Enkare Nyrobi' szóból ered, ami hűvös vizet jelent."
      ],
      ro: [
        "Fondat în 1899 ca un depou feroviar.",
        "Numele provine din cuvântul maasai 'Enkare Nyrobi', însemnând apă rece."
      ],
      en: [
        "Founded in 1899 as a rail depot.",
        "The name comes from Maasai 'Enkare Nyrobi', meaning cool water."
      ]
    },
    
     image: "/poi-images/nairobi-cities-v2.webp"},
  {
    id: "mombasa-cities-v2",
    type: "city",
    parent: "KE-01",
    coords: [39.6682, -4.0435],
    name: {
      de: "Mombasa",
      hu: "Mombasa",
      ro: "Mombasa",
      en: "Mombasa",
image: "/poi-images/mombasa-cities-v2.webp"
},
    descriptionAdvanced: {
      en: "Strategically positioned on the Indian Ocean coast, Mombasa is Kenya's oldest city, boasting a documented history that stretches back to at least the 12th century. The city's layout spans Mombasa Island and the adjacent mainland, connected by the Nyali Bridge built in 1980. Its deep-water port, Kilindini Harbour, handles millions of tonnes of cargo annually, serving not only Kenya but also landlocked neighbors like Uganda and Rwanda. The historic Fort Jesus, constructed by the Portuguese between 1593 and 1596, dominates the coastline and was designated a UNESCO World Heritage site in 2001. Mombasa's cultural fabric is deeply influenced by centuries of Swahili, Arab, and Omani trade networks, reflected in the narrow, winding alleys of its Old Town. The city experiences a tropical wet and dry climate, making it a pivotal center for East African tourism. History K8 — Colonial trade routes.",
      de: "Mombasa, die älteste und zweitgrößte Stadt Kenias, liegt an der Küste des Indischen Ozeans. Die Stadt hat eine reiche Geschichte als Handelszentrum, das über Jahrhunderte von arabischen, portugiesischen und britischen Einflüssen geprägt wurde. Sie war die erste Hauptstadt von Britisch-Ostafrika, bevor Nairobi 1907 diesen Status übernahm. Das historische Zentrum, die Altstadt, besticht durch enge Gassen und traditionelle Swahili-Architektur. Ein Wahrzeichen ist das Fort Jesus, eine von den Portugiesen im 16. Jahrhundert erbaute Festung, die heute zum UNESCO-Weltkulturerbe gehört. Die Gebäude im zentralen Geschäftsviertel sind in den Farben Blau und Weiß gehalten, die den Indischen Ozean symbolisieren. Mit rund 1,2 Millionen Einwohnern ist Mombasa ein wichtiger Hafen und ein beliebtes Touristenziel.",
      hu: "Mombasa Kenya legrégebbi és második legnagyobb városa, amely az Indiai-óceán partján fekszik. Történelme során fontos kereskedelmi központ volt, amelyet arab, portugál és brit hatások formáltak. 1907-ig Brit Kelet-Afrika fővárosa volt, amikor is Nairobi vette át ezt a szerepet. Az óváros szűk utcái és a szuahéli építészet egyedülálló hangulatot árasztanak. A város egyik legfontosabb nevezetessége a 16. században épült Fort Jesus erőd, amely az UNESCO Világörökség része. A központi üzleti negyed kék-fehér épületei az óceánt idézik. Mombasa ma fontos kikötőváros és népszerű turisztikai célpont, lakossága meghaladja az 1,2 millió főt.",
      ro: "Mombasa, cel mai vechi și al doilea cel mai mare oraș din Kenya, este situat pe coasta Oceanului Indian. Cu o istorie bogată ca centru comercial, orașul a fost modelat de influențe arabe, portugheze și britanice de-a lungul secolelor. A fost prima capitală a Africii de Est Britanice până în 1907, când Nairobi a preluat acest rol. Centrul istoric, cunoscut sub numele de Orașul Vechi, impresionează prin străduțele sale înguste și arhitectura tradițională Swahili. Un punct de reper emblematic este Fort Jesus, o fortăreață construită de portughezi în secolul al XVI-lea, inclusă astăzi în Patrimoniul Mondial UNESCO. Clădirile din centrul de afaceri sunt vopsite în albastru și alb, simbolizând Oceanul Indian."
    },
    factsAdvanced: {
      en: ["Documented history dates back to around 1150 AD by geographer al-Idrisi.", "Fort Jesus was built by Portuguese architect João Batista Cairato in 1593.", "Kilindini Harbour was officially inaugurated for deep-water shipping in 1896.", "Mombasa served as the capital of the British East Africa Protectorate until 1907.", "The iconic Mombasa Tusks were erected in 1952 to commemorate Princess Elizabeth's visit.", "Fort Jesus was recognized as a UNESCO World Heritage site in 2001.", "The New Nyali Bridge, linking the island to the north mainland, opened in 1980."],
      de: ["Die älteste Stadt Kenias mit einer über 900-jährigen Geschichte.", "War bis 1907 die Hauptstadt von Britisch-Ostafrika.", "Die Bevölkerung lag 2019 bei etwa 1,2 Millionen Menschen.", "Fort Jesus wurde zwischen 1593 und 1596 erbaut.", "Die Gebäude im CBD sind als Symbol für den Ozean blau und weiß gestrichen.", "Ein wichtiger Hafen für ganz Ostafrika."],
      hu: [],
      ro: []
    },
    description: {
      de: "Mombasa ist die älteste und zweitgrößte Stadt in Kenia, gelegen an der Küste des Indischen Ozeans.",
      hu: "Mombasa Kenya legrégebbi és második legnagyobb városa az Indiai-óceán partján.",
      ro: "Mombasa este cel mai vechi și al doilea oraș ca mărime din Kenya, situat pe coasta Oceanului Indian.",
      en: "Mombasa is the oldest and second-largest city in Kenya, located on the Indian Ocean coast."
    },
    facts: {
      de: [
        "Historisch ein wichtiges Handelszentrum.",
        "Bekannt für seine weißen Sandstrände und Fort Jesus."
      ],
      hu: [
        "Történelmileg fontos kereskedelmi központ volt.",
        "Híres fehér homokos strandjairól és a Fort Jesus erődítményről."
      ],
      ro: [
        "Istoric un centru comercial important.",
        "Cunoscut pentru plajele sale cu nisip alb și Fort Jesus."
      ],
      en: [
        "Historically an important trading center.",
        "Known for its white sand beaches and Fort Jesus."
      ]
    },
    
     image: "/poi-images/mombasa-cities-v2.webp"},
  {
    id: "kisumu-cities-v2",
    type: "city",
    parent: "KE-42",
    coords: [34.7617, -0.0917],
    name: {
      de: "Kisumu",
      hu: "Kisumu",
      ro: "Kisumu",
      en: "Kisumu",
image: "/poi-images/kisumu-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kisumu, am Ufer des riesigen Viktoriasees gelegen, ist die drittgrößte Stadt Kenias und ein wichtiges wirtschaftliches Zentrum in der westlichen Region des Landes. Ursprünglich im Jahr 1901 unter dem Namen Port Florence als Endstation der Uganda-Bahn gegründet, hat sich die Stadt zu einem dynamischen Handelsknotenpunkt entwickelt. Der Hafen von Kisumu ist ein entscheidender Umschlagplatz für den internationalen Binnenhandel mit den Nachbarländern Uganda und Tansania. Neben der Fischereiindustrie, insbesondere dem Nilbarsch, ist die Region auch für die Landwirtschaft bekannt. Kulturell ist Kisumu das Zentrum der Luo-Gemeinschaft. Besucher zieht es oft zum Kisumu Impala Sanctuary oder in das Kisumu Museum, das die reiche lokale Geschichte und Naturgeschichte dokumentiert. Diese Stadt verbindet städtisches Wachstum mit der Nähe zu bedeutenden Naturräumen (Geografie K7).",
      hu: "Kisumu Kenya harmadik legnagyobb városa és a Viktória-tó menti legfontosabb belföldi kikötője. A város a 19. század végén indult fejlődésnek, amikor a britek megépítették az Ugandai Vasutat. Ma Kisumu a nyugat-kenyai kereskedelem és az agráripar központja, különösen a halászat és a cukornád-feldolgozás területén. A közeli Kisumu Impala Rezervátum a természetkedvelők kedvelt úti célja. Földrajzi fekvése miatt az egyenlítői éghajlat jellemzi, gyakori délutáni záporokkal. (Földrajz K7 — afrikai tavak és városok)",
      ro: "Kisumu este al treilea oraș ca mărime din Kenya, situat pitoresc pe malul estic al imensului Lac Victoria. Fondat în anul 1901 sub numele de Port Florence, orașul a devenit rapid un nod comercial vital pentru Africa de Est. Datorită poziției sale strategice, Kisumu servește ca principal port intern al Kenyei, facilitând transportul mărfurilor și pescuitul pe cel mai mare lac tropical din lume. Economia locală se bazează pe pescuit, agricultură și comerțul transfrontalier cu Uganda și Tanzania. Vizitatorii sunt adesea atrași de Sanctuarul Impala și de piața aglomerată Kibuye, una dintre cele mai mari piețe în aer liber din regiune.",
      en: "Anchoring the eastern shores of Lake Victoria, Kisumu serves as the principal inland port city of Kenya and the administrative capital of Kisumu County. Originally established in 1901 as Port Florence, the settlement was the vital terminus of the Uganda Railway, facilitating trade across the expansive inland waters. The city sits at an elevation of 1,131 meters and is characterized by a warm, humid climate ideal for regional agriculture. Kisumu is culturally significant as the heartland of the Luo community and hosts the Kisumu Museum, founded in 1980, which preserves intricate ethnographic exhibits. In recent decades, the modernization of the Kisumu port facilities has rejuvenated regional shipping networks connecting Kenya, Uganda, and Tanzania. The nearby Impala Sanctuary offers critical conservation space for the endangered sitatunga antelope. Geography K7 — Inland water transport hubs."
    },
    factsAdvanced: {
      en: ["Founded in 1901 and originally named Port Florence after Florence Preston.", "Located on the shores of Lake Victoria at an elevation of 1,131 meters.", "The Kisumu Museum officially opened its doors to the public in 1980.", "Served as the western terminus of the Uganda Railway upon its completion in 1901.", "The city's population was recorded at approximately 397,000 in the 2019 census.", "Features the Ndere Island National Park, gazetted in 1986.", "Major renovations of the Kisumu Port were completed in 2019 to boost regional trade."]
    },
    
    description: {
      de: "Kisumu ist die drittgrößte Stadt in Kenia und dient als wichtiger Hafen am Viktoriasee.",
      hu: "Kisumu Kenya harmadik legnagyobb városa, és fontos kikötő a Viktória-tavon.",
      ro: "Kisumu este al treilea oraș ca mărime din Kenya și servește ca un port major pe Lacul Victoria.",
      en: "Kisumu is the third-largest city in Kenya and serves as a major port on Lake Victoria."
    },
    facts: {
      de: [
        "Ursprünglich als Port Florence bekannt.",
        "Es ist das Handelszentrum von Westkenia."
      ],
      hu: [
        "Eredetileg Port Florence néven ismerték.",
        "Ez Nyugat-Kenya kereskedelmi központja."
      ],
      ro: [
        "Cunoscut inițial ca Port Florence.",
        "Este centrul comercial din vestul Kenyei."
      ],
      en: [
        "Originally known as Port Florence.",
        "It is the commercial center of western Kenya."
      ]
    }, image: "/poi-images/kisumu-cities-v2.webp"},
  {
    id: "nakuru-cities-v2",
    type: "city",
    parent: "KE-32",
    coords: [36.08, -0.3031],
    name: {
      de: "Nakuru",
      hu: "Nakuru",
      ro: "Nakuru",
      en: "Nakuru",
image: "/poi-images/nakuru-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Nakuru liegt im faszinierenden Großen Afrikanischen Grabenbruch und ist die viertgrößte Stadt Kenias sowie die Hauptstadt des gleichnamigen Nakuru Countys. Die Stadt ist vor allem als Ausgangspunkt für den berühmten Lake-Nakuru-Nationalpark bekannt, der 1961 gegründet wurde und weltweite Berühmtheit für seine riesigen Populationen von Zwergflamingos erlangte. Nakuru wuchs historisch als wichtiges Agrar- und Landwirtschaftszentrum, insbesondere während der Kolonialzeit in den \"White Highlands\". Heute ist es ein lebendiger Knotenpunkt für Handel, Bildung und Tourismus. Etwas außerhalb der Stadt befindet sich der Menengai-Krater, ein massiver Schildvulkan mit einer 90 Quadratkilometer großen Caldera, der ebenfalls viele Besucher anzieht. Die Region bietet hervorragende Beispiele für vulkanische Aktivitäten und Ökosysteme im Rift Valley (Geografie K8).",
      hu: "Nakuru a Nagy-hasadékvölgy egyik legfontosabb mezőgazdasági és kereskedelmi csomópontja, 2021-ben kapott hivatalos városi rangot. A város leginkább a Nakuru-tóról híres, amely egykor több millió flamingó otthona volt, és a környék a rinocéroszok fontos menedékhelye. A város gazdasága jelentős mértékben támaszkodik a turizmusra és a környező farmok termékeinek feldolgozására. A 20. század elején a brit telepesek kedvelt mezőgazdasági központjává vált. (Földrajz K6 — hasadékvölgyek és mezőgazdaság)",
      ro: "Nakuru, situat în inima Marelui Rift African, este al patrulea cel mai mare centru urban din Kenya și un important hub agricol și turistic. Orașul s-a dezvoltat inițial ca o așezare pe calea ferată Uganda în anul 1904. Nakuru este faimos la nivel mondial datorită Parcului Național Lacul Nakuru, care atrage anual mii de vizitatori dornici să vadă uriașele stoluri de păsări flamingo roz și populațiile de rinoceri protejați. Pe lângă turism, economia locală este susținută de prelucrarea produselor agricole, cultivarea porumbului și a cafelei, precum și de industriile manufacturiere.",
      en: "Nestled within the Great Rift Valley at an altitude of 1,850 meters, Nakuru is the fourth-largest urban center in Kenya and officially attained city status in 2021. Established in 1904 as a critical node on the evolving railway network, the city developed rapidly into a major agricultural hub, surrounded by expansive farmlands producing coffee, wheat, and dairy. The area is globally renowned for Lake Nakuru National Park, created in 1961, which historically attracted millions of lesser flamingos feeding on the lake's alkaline algae. Nakuru is also distinguished by the massive Menengai Crater, an extinct shield volcano located just north of the city limits, featuring a caldera spanning 12 kilometers in diameter. The region's geological volatility provides substantial geothermal energy potential, shaping its modern industrial landscape. Earth Sciences K7 — Rift Valley geology."
    },
    factsAdvanced: {
      en: ["Established by the British in 1904 along the Uganda Railway route.", "Officially elevated from municipality to city status in December 2021.", "Lake Nakuru National Park was established as a bird sanctuary in 1961.", "Situated at an elevation of roughly 1,850 meters (6,070 feet).", "The nearby Menengai Crater spans approximately 12 kilometers in diameter.", "Hyrax Hill Prehistoric Site, discovered in 1926, is located on the city outskirts.", "The city's population reached around 393,000 according to the 2019 census."]
    },
    
    description: {
      de: "Nakuru ist die viertgrößte Stadt Kenias und liegt im Großen Afrikanischen Grabenbruch.",
      hu: "Nakuru Kenya negyedik legnagyobb városa a Nagy-hasadékvölgyben.",
      ro: "Nakuru este al patrulea oraș ca mărime din Kenya, situat în Marea Vale a Riftului.",
      en: "Nakuru is the fourth-largest city in Kenya, situated in the Great Rift Valley."
    },
    facts: {
      de: [
        "Wurde 2021 in den Stadtstatus erhoben.",
        "Liegt in der Nähe des Nakurusees, der für seine Flamingos berühmt ist."
      ],
      hu: [
        "2021-ben kapott városi rangot.",
        "A flamingóiról híres Nakuru-tó közelében található."
      ],
      ro: [
        "Elevat la statutul de oraș în 2021.",
        "Situat lângă Lacul Nakuru, faimos pentru păsările flamingo."
      ],
      en: [
        "Elevated to city status in 2021.",
        "Located near Lake Nakuru, famous for its flamingos."
      ]
    }, image: "/poi-images/nakuru-cities-v2.webp"},
  {
    id: "eldoret-cities-v2",
    type: "city",
    parent: "KE-27",
    coords: [35.2699, 0.5143],
    name: {
      de: "Eldoret",
      hu: "Eldoret",
      ro: "Eldoret",
      en: "Eldoret",
image: "/poi-images/eldoret-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Eldoret, die fünftgrößte Stadt Kenias und Hauptstadt des Uasin Gishu Countys, liegt malerisch in den fruchtbaren Hügeln des Rift Valley auf über 2.000 Metern Höhe. Diese Höhenlage in Verbindung mit dem milden Klima bietet nicht nur exzellente Bedingungen für die Landwirtschaft, sondern hat die Stadt auch weltweit als Heimat der kenianischen Langstreckenläufer berühmt gemacht. Zahlreiche Olympiasieger und Weltmeister stammen aus dieser Region oder trainieren in den zahlreichen High-Altitude-Trainingscamps rund um die Stadt. Wirtschaftlich ist Eldoret ein Zentrum für Textilindustrie, Weizen- und Maisanbau sowie die Milchproduktion. Die Stadt entwickelte sich ursprünglich nach 1910 aus einer Farm für südafrikanische Siedler und beherbergt heute die renommierte Moi University, eine der wichtigsten Bildungseinrichtungen des Landes (Sport & Geografie K6).",
      hu: "Eldoret Kenya ötödik legnagyobb városa, mely az Uasin Gishu fennsíkon fekszik. A települést 1910-ben dél-afrikai búr telepesek alapították, akik nagyüzemi mezőgazdaságot honosítottak meg a térségben. Ma a város a kenyai atlétika fővárosaként ismert, mivel innen származik a híres hosszútávfutók többsége. A hűvösebb klíma és a magaslati levegő ideális edzéskörülményeket biztosít. Jelentős a sajt- és textilipara is, az egyetemei miatt pedig fontos oktatási központ. (Földrajz K7 — magasföldek és éghajlat)",
      ro: "Eldoret este un centru urban vibrant situat în vestul Kenyei, renumit la nivel global ca fiind «capitala alergătorilor». Aflat la o altitudine de peste 2.100 de metri, climatul său răcoros și aerul rarefiat au creat condiții perfecte pentru antrenamentul atleților de talie mondială, mulți campioni olimpici provenind din această regiune. Orașul a fost fondat în anul 1910 de fermierii de origine afrikaans. Astăzi, Eldoret este un important centru agricol și universitar, dominat de cultivarea grâului, a porumbului și de producția de lactate, având o economie în plină expansiune.",
      en: "Situated on the Uasin Gishu Plateau at a commanding elevation of 2,100 meters, Eldoret is one of Kenya's fastest-growing urban centers, historically shaped by Afrikaner settlement in the early 20th century. Established around 1910 under the name 'Farm 64', the town officially became Eldoret in 1912, named after the Maasai word 'eldore' meaning stony river. The high altitude and temperate climate create optimal conditions for agriculture, making the region a massive producer of maize and wheat. Furthermore, this unique geography has transformed Eldoret into a world-renowned training ground for elite long-distance runners, producing multiple Olympic gold medalists. The city's industrial sector expanded significantly with the establishment of the Rivatex textile mill in 1975, contributing to its modern economic vitality. Social Studies K6 — High-altitude demographics."
    },
    factsAdvanced: {
      en: ["Founded around 1910 and initially referred to as 'Farm 64' by settlers.", "Officially named Eldoret in 1912, deriving from the local term for 'stony river'.", "Located at a high altitude of approximately 2,100 meters (6,900 feet).", "The Uganda Railway extension reached Eldoret in 1924, boosting its growth.", "Moi University, a major regional institution, was established here in 1984.", "Rivatex East Africa, a major textile facility, commenced operations in 1975.", "The town's population was approximately 475,000 in the 2019 national census."]
    },
    
    description: {
      de: "Eldoret ist eine Großstadt in der Rift-Valley-Region, bekannt als die 'Stadt der Champions' aufgrund ihrer berühmten Athleten.",
      hu: "Eldoret egy nagyváros a Rift Valley régióban, melyet híres atlétái miatt a 'Bajnokok városának' is neveznek.",
      ro: "Eldoret este un oraș major în regiunea Rift Valley, cunoscut ca 'Orașul Campionilor' datorită atleților săi celebri.",
      en: "Eldoret is a major city in the Rift Valley region, known as the 'City of Champions' due to its famous athletes."
    },
    facts: {
      de: [
        "Ein wichtiges landwirtschaftliches Handelszentrum.",
        "Beherbergt ein Höhentrainingslager für Marathonläufer."
      ],
      hu: [
        "Fontos mezőgazdasági kereskedelmi központ.",
        "Maratonfutók magaslati edzőtábora található itt."
      ],
      ro: [
        "Un centru cheie de comerț agricol.",
        "Găzduiește un centru de antrenament la mare altitudine pentru maratoniști."
      ],
      en: [
        "A key agricultural trade center.",
        "Home to a high altitude training camp for marathon runners."
      ]
    }, image: "/poi-images/eldoret-cities-v2.webp"},
  {
    id: "thika-cities-v2",
    type: "city",
    parent: "KE-22",
    coords: [37.0693, -1.0333],
    name: {
      de: "Thika",
      hu: "Thika",
      ro: "Thika",
      en: "Thika",
image: "/poi-images/thika-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Thika ist eine bedeutende Industriestadt im Kiambu County, die etwa 40 Kilometer nordöstlich von Kenias Hauptstadt Nairobi liegt. Die Stadt erlangte internationale Bekanntheit unter anderem durch Elspeth Huxleys berühmtes Buch \"Die Flammenbäume von Thika\". Wirtschaftlich ist Thika ein Schwergewicht in Kenia: Hier befinden sich große Fabriken für die Textil-, Fahrzeugmontage- und Lebensmittelindustrie. Besonders berühmt ist die Stadt für den Anbau und die Verarbeitung von Ananas durch das Unternehmen Del Monte Kenya, das Tausende Hektar Land bewirtschaftet. Geografisch liegt Thika an den malerischen Chania Falls und dem Thika River, die als wichtige Wasserquellen dienen. Mit dem Ausbau der Thika-Superhighway ist die Stadt noch stärker mit dem Großraum Nairobi verschmolzen (Wirtschaft K8).",
      hu: "Thika egy ipari fókuszú város a Középső Tartományban, Nairobitól mintegy 40 kilométerre északkeletre. A térség a kávé-, ananász- és makadámdió-ültetvényeiről ismert, itt működik a híres Del Monte konzervgyár is. Thika a 20. század elején indult gyors növekedésnek az európai telepesek ültetvényeinek köszönhetően. A város melletti Chania és Thika folyók több lenyűgöző vízesést is táplálnak, amelyek a helyiek körében is népszerű kirándulóhelyek. (Földrajz K6 — ipar és mezőgazdaság)",
      ro: "Thika este un important oraș industrial și comercial situat în județul Kiambu, la nord-est de capitala Nairobi. Cunoscut drept un pol major al procesării agro-alimentare, orașul găzduiește vaste plantații de ananas, printre care și celebrele ferme Del Monte. Thika a jucat un rol esențial în dezvoltarea industrială a Kenyei post-independență, având numeroase fabrici de textile, asamblare de vehicule și procesare a fructelor. Din punct de vedere turistic, orașul este faimos pentru cascadele Thika și Fourteen Falls, care atrag vizitatori pasionați de natură și drumeții.",
      en: "Located just 42 kilometers northeast of Nairobi, Thika is a critical industrial town situated in Kiambu County along the Thika River. The settlement gained formal township status in 1924 and rapidly evolved into an agricultural processing powerhouse, surrounded by extensive pineapple and coffee plantations. It is most famously home to the Del Monte pineapple cannery, established in the mid-20th century, which exports massive quantities of canned fruit and juice globally. The town is geographically defined by the nearby Chania Falls and Thika Falls, which provide scenic natural boundaries and local water resources. In 2012, the completion of the eight-lane Thika Superhighway drastically reduced travel time to the capital, accelerating suburban expansion and real estate development in the area. Economics K8 — Agro-industrial processing."
    },
    factsAdvanced: {
      en: ["Gazetted as a formal township by the colonial government in 1924.", "Located at an elevation of 1,631 meters above sea level.", "Home to the massive Del Monte Kenya pineapple processing facility.", "The eight-lane Thika Superhighway connecting it to Nairobi was completed in 2012.", "Mount Kenya University established its main campus in Thika in 2008.", "Features the historic Blue Posts Hotel, established in 1908 near the Chania Falls.", "The town recorded a population of about 251,000 in the 2019 census."],
      de: ["1924 von der Kolonialregierung als formelle Gemeinde anerkannt.", "Liegt auf einer Höhe von 1.631 Metern über dem Meeresspiegel.", "Heimat der riesigen Ananasverarbeitungsanlage von Del Monte Kenya.", "Die achtspurige Thika Superhighway nach Nairobi wurde 2012 fertiggestellt.", "Die Mount Kenya University gründete 2008 ihren Hauptcampus in Thika.", "Bevölkerung von etwa 279.429 laut der Volkszählung 2019."]
    },
    
    description: {
      de: "Thika ist eine Industriestadt und ein wichtiges Handelszentrum im Kiambu County nahe Nairobi.",
      hu: "Thika egy iparváros és fontos kereskedelmi központ a Kiambu megyében, Nairobi közelében.",
      ro: "Thika este un oraș industrial și un hub major de comerț în comitatul Kiambu, lângă Nairobi.",
      en: "Thika is an industrial town and major commerce hub in Kiambu County, near Nairobi."
    },
    facts: {
      de: [
        "Bekannt für seine riesigen Ananasplantagen.",
        "Beherbergt die Chania- und Thika-Wasserfälle."
      ],
      hu: [
        "Hatalmas ananászültetvényeiről ismert.",
        "Itt találhatóak a Chania és a Thika vízesések."
      ],
      ro: [
        "Cunoscut pentru vastele sale plantații de ananas.",
        "Găzduiește cascadele Chania și Thika."
      ],
      en: [
        "Known for its vast pineapple plantations.",
        "Hosts the Chania and Thika Falls."
      ]
    }, image: "/poi-images/thika-cities-v2.webp"},
  {
    id: "malindi-cities-v2",
    type: "city",
    parent: "KE-03",
    coords: [40.1166, -3.2166],
    name: {
      de: "Malindi",
      hu: "Malindi",
      ro: "Malindi",
      en: "Malindi",
image: "/poi-images/malindi-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Malindi ist eine historische Küstenstadt in Kenia, die eine faszinierende Mischung aus alter Swahili-Kultur und modernem Tourismus bietet. Bereits im 13. Jahrhundert war Malindi eine wohlhabende Handelsniederlassung. Als der portugiesische Entdecker Vasco da Gama 1498 hier landete, wurde er freundlich empfangen, und er errichtete ein Korallenkreuz, die Vasco-da-Gama-Säule, die noch heute an der Küste steht und ein bedeutendes Wahrzeichen ist. Heute ist die Stadt für ihre weißen Sandstrände, luxuriösen Resorts und den Malindi Marine National Park bekannt, der ein beliebtes Ziel für Taucher und Schnorchler ist. Die starke Präsenz italienischer Investoren und Touristen hat der Stadt den Spitznamen \"Little Italy\" in Kenia eingebracht. Die Architektur der Altstadt spiegelt die jahrhundertelangen Einflüsse aus dem Oman, Portugal und der lokalen Swahili-Kultur wider (Geschichte K7).",
      hu: "Malindi egy ősi partmenti város, amely a 14. század óta fontos szuahéli kereskedelmi központ. Amikor Vasco da Gama 1498-ban ideérkezett, barátságos fogadtatásban részesült, és a város fontos bázis lett a portugálok számára az Indiába vezető úton. Ma Malindi népszerű üdülőhely, különösen az olasz turisták körében, csodálatos korallzátonyokkal és tengeri nemzeti parkokkal. A város a történelmi emlékek, a fehér homokos strandok és az élénk éjszakai élet keveréke. (Történelem K6 — európai felfedezések Afrika partjainál)",
      ro: "Malindi este un oraș de coastă istoric situat la Oceanul Indian, cunoscut pentru plajele sale spectaculoase și recifele de corali vibrante. Cu o istorie care datează din secolul al IX-lea, Malindi a fost un important port comercial swahili și un punct de oprire pentru exploratorul portughez Vasco da Gama în anul 1498. Astăzi, stâlpul Vasco da Gama rămâne unul dintre cele mai vechi monumente europene din Africa. Orașul atrage mii de turiști, în special din Italia, fiind un centru principal pentru pescuit la mare adâncime, scufundări și relaxare tropicală.",
      en: "Malindi is a historically rich coastal town situated at the mouth of the Galana River on the Indian Ocean, characterized by pristine white beaches and ancient coral reefs. The settlement has been a prominent Swahili trading port since the 13th century, hosting the Portuguese explorer Vasco da Gama in 1498, an event commemorated by a coral pillar still standing today. During the late 20th century, Malindi transformed into a major tourism hub, notably attracting a large Italian expatriate community that heavily influenced local architecture and cuisine. The adjacent Malindi Marine National Park, established in 1968, was the first of its kind in Africa, protecting extensive seagrass beds and marine biodiversity. Despite modern development, the town preserves its historical layout, including the Juma Mosque dating back to the 15th century. History K7 — Indian Ocean trade networks."
    },
    factsAdvanced: {
      en: ["Has functioned as a major Swahili coastal settlement since the 13th century.", "Vasco da Gama visited in 1498 and erected a coral pillar near the shore.", "Malindi Marine National Park was established in 1968, a first for Africa.", "The historic Juma Mosque on the seafront dates back to the late 15th century.", "Located 120 kilometers northeast of Mombasa at the Galana River estuary.", "Features the nearby Gedi Ruins, a 12th-century abandoned Swahili city.", "The urban population was recorded at approximately 119,000 in the 2019 census."]
    },
    
    description: {
      de: "Malindi ist eine Küstenstadt im Kilifi County, bekannt für ihre schönen Strände und historische Swahili-Architektur.",
      hu: "Malindi egy tengerparti város a Kilifi megyében, gyönyörű strandjairól és történelmi szuahéli építészetéről ismert.",
      ro: "Malindi este un oraș de coastă în comitatul Kilifi, cunoscut pentru plajele sale frumoase și arhitectura istorică swahili.",
      en: "Malindi is a coastal town in Kilifi County, known for its beautiful beaches and historical Swahili architecture."
    },
    facts: {
      de: [
        "Vasco da Gama besuchte die Stadt 1498.",
        "Ein beliebtes Reiseziel für italienische Touristen."
      ],
      hu: [
        "Vasco da Gama 1498-ban látogatta meg a várost.",
        "Népszerű célpont az olasz turisták körében."
      ],
      ro: [
        "Vasco da Gama a vizitat orașul în 1498.",
        "O destinație populară pentru turiștii italieni."
      ],
      en: [
        "Vasco da Gama visited the town in 1498.",
        "A popular destination for Italian tourists."
      ]
    }, image: "/poi-images/malindi-cities-v2.webp"},
  {
    id: "kitale-cities-v2",
    type: "city",
    parent: "KE-26",
    coords: [35.0062, 1.0157],
    name: {
      de: "Kitale",
      hu: "Kitale",
      ro: "Kitale",
      en: "Kitale",
image: "/poi-images/kitale-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kitale, gelegen im westlichen Kenia zwischen Mount Elgon und den Cherangani-Hills, ist das administrative und kommerzielle Zentrum des Trans-Nzoia Countys. Die Stadt wurde 1920 von europäischen Siedlern gegründet und entwickelte sich schnell zu einem der wichtigsten landwirtschaftlichen Knotenpunkte des Landes. Die fruchtbaren Böden und das gemäßigte Klima machen die Region zu einem Hauptproduktionsgebiet für Mais, Sonnenblumen, Bohnen und Milchprodukte. Für Besucher ist Kitale ein hervorragender Ausgangspunkt für Expeditionen in den Mount Elgon Nationalpark oder den Saiwa Swamp Nationalpark, der als kleinster Nationalpark Kenias bekannt ist und die seltene Sitatunga-Antilope beheimatet. In Kitale befindet sich auch das Kitale Museum, das sich intensiv mit den ethnografischen und naturgeschichtlichen Besonderheiten Westkenias beschäftigt (Biologie K7).",
      hu: "Kitale mezőgazdasági csomópont Nyugat-Kenyában, az Elgon-hegy és a Cherangani-dombság között. A 20. század elején európai farmerek alapították, akik rájöttek, hogy a terület rendkívül alkalmas kukorica- és napraforgó-termesztésre, valamint tejtermelésre. A város gazdaságát ma is a nagyüzemi farmok határozzák meg, ezért a régiót Kenya magtáraként emlegetik. A város múzeuma a térség egykori mezőgazdasági történetét és az őslakos kultúrákat mutatja be. (Földrajz K6 — élelmiszertermelés és domborzat)",
      ro: "Kitale este un oraș agricol pitoresc din nord-vestul Kenyei, servind drept capitală a județului Trans-Nzoia. Fondat în 1920 de către coloniștii europeni pe o ramificație a căii ferate, Kitale se află într-o regiune extrem de fertilă, între Muntele Elgon și Dealurile Cherangani. Este adesea numit «coșul de pâine» al Kenyei, datorită producției masive de porumb, floarea-soarelui și produse lactate. Atracțiile notabile includ Muzeul Național Kitale și Parcul Național Muntele Elgon, care oferă peisaje uimitoare și ocazia de a explora faimoasele peșteri cu elefanți sălbatici.",
      en: "Positioned in the fertile northern Rift Valley between Mount Elgon and the Cherangani Hills, Kitale functions as the agricultural nerve center of Trans-Nzoia County. Founded in 1920 by European settlers drawn to its high elevation of 1,900 meters and rich volcanic soils, the town quickly became the primary trading hub for Kenya's 'breadbasket' region, producing vast quantities of maize and sunflower seeds. The arrival of the railway branch line in 1926 catalyzed the export of agricultural goods. Kitale is distinguished by the Kitale Museum, opened in 1924, which was the first domestic museum in Kenya and houses extensive natural history collections and ethnographic artifacts. The surrounding terrain also serves as a critical base for expeditions into the Mount Elgon National Park and the Saiwa Swamp reserve. Geography K6 — Agricultural heartlands."
    },
    factsAdvanced: {
      en: ["Founded by European colonial settlers in 1920.", "Located at a high altitude of 1,900 meters (6,200 feet).", "The Kitale branch of the railway line was completed in 1926.", "The National Museum of Western Kenya (Kitale Museum) was opened in 1924.", "Serves as the administrative capital of Trans-Nzoia County.", "Acts as the primary gateway to the Saiwa Swamp National Park, created in 1974.", "The urban area had a population of roughly 162,000 in the 2019 census."]
    },
    
    description: {
      de: "Kitale ist eine landwirtschaftliche Stadt im nördlichen Rift Valley, gelegen zwischen Mount Elgon und den Cherangani-Hügeln.",
      hu: "Kitale egy mezőgazdasági város a Rift Valley északi részén, a Mount Elgon és a Cherangani-hegység között.",
      ro: "Kitale este un oraș agricol în nordul Văii Rift, situat între Muntele Elgon și Dealurile Cherangani.",
      en: "Kitale is an agricultural town in northern Rift Valley, situated between Mount Elgon and the Cherangani Hills."
    },
    facts: {
      de: [
        "Wurde 1920 von weißen Siedlern gegründet.",
        "Berühmt für das National Museum of Western Kenya."
      ],
      hu: [
        "1920-ban alapították fehér telepesek.",
        "Híres a Nyugat-Kenyai Nemzeti Múzeumról."
      ],
      ro: [
        "Fondat în 1920 de coloniști albi.",
        "Faimos pentru Muzeul Național al Kenyei de Vest."
      ],
      en: [
        "Founded in 1920 by white settlers.",
        "Famous for the National Museum of Western Kenya."
      ]
    }, image: "/poi-images/kitale-cities-v2.webp"},
  {
    id: "garissa-cities-v2",
    type: "city",
    parent: "KE-07",
    coords: [39.6385, -0.4536],
    name: {
      de: "Garissa",
      hu: "Garissa",
      ro: "Garissa",
      en: "Garissa",
image: "/poi-images/garissa-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Garissa ist die Hauptstadt des Garissa Countys und ein wesentlicher urbaner Knotenpunkt im semiariden Nordosten Kenias. Die Stadt liegt strategisch günstig am Ostufer des Tana-Flusses, des längsten Flusses in Kenia, der als lebenswichtige Wasserquelle für diese ansonsten trockene Region dient. Die Bevölkerung von Garissa ist vorwiegend somalischer Abstammung, was der Stadt eine ausgeprägte kulturelle Prägung in Sprache, Traditionen und Handel verleiht. Wirtschaftlich stützt sich die Stadt stark auf die nomadische und semi-nomadische Viehwirtschaft, wobei große Märkte für Kamele, Rinder und Ziegen eine zentrale Rolle spielen. In der Nähe befindet sich das Bour-Algi-Giraffen-Schutzgebiet, das die seltene Netzgiraffe beheimatet und ökologisch von großer Bedeutung ist (Geografie K8).",
      hu: "Garissa egy sivatagi és félsivatagi oázisváros, Északkelet-Kenya legnépesebb települése. A város a Tana-folyó mentén fekszik, amely létfontosságú vízforrást biztosít a jórészt szomáli származású lakosságnak és az állattenyésztésnek. A gazdaság alapját a nomád pásztorkodás és a szomszédos Szomáliával folytatott kereskedelem adja. A forró és száraz klíma ellenére a folyóparti zöld övezetben még gyümölcsöt és zöldséget is termesztenek. (Földrajz K8 — sivatagi éghajlat és folyók szerepe)",
      ro: "Garissa este cel mai important centru urban din regiunea de nord-est a Kenyei, situat pe malurile fierbinți ale râului Tana. Datorită climatului său arid, economia locală este dominată de păstoritul nomad și de comerțul transfrontalier cu Somalia. Orașul este un punct vital de tranzit și un hub administrativ, găzduind o populație majoritar etnică somaleză. Garissa se confruntă cu provocări climatice, dar râul Tana susține o agricultură irigată limitată și o biodiversitate unică, inclusiv Sanctuarul Girafelor Bour-Algi, dedicat protejării girafelor reticulate.",
      en: "Garissa serves as the administrative headquarters of Garissa County, situated along the eastern banks of the Tana River in the expansive, semi-arid North Eastern Province. Characterized by a hot and dry desert climate, the city is a critical commercial and livestock trading hub connecting coastal regions with the interior arid lands. Historically, Garissa evolved as a crucial watering point and market center for nomadic Somali pastoralists who dominate the local demographic. The landscape around the city is defined by resilient scrubland, with the Tana River providing the sole reliable water source, sustaining localized agriculture and riverine ecosystems. In 2013, the establishment of Garissa University expanded the city's role as an educational center in a historically marginalized region. Social Studies K7 — Arid region urban development."
    },
    factsAdvanced: {
      en: ["Functions as the capital of Garissa County in northeastern Kenya.", "Situated directly on the eastern banks of the Tana River.", "Features a hot desert climate with average annual rainfall below 300 mm.", "Garissa University was officially established as a public institution in 2013.", "The Bour-Algi Giraffe Sanctuary is located 5 kilometers south of the town.", "A major 50-megawatt solar power plant was commissioned near Garissa in 2019.", "The town's population was recorded at approximately 163,000 in 2019."]
    },
    
    description: {
      de: "Garissa ist die Hauptstadt des Garissa County, einer trockenen Region am Tana-Fluss.",
      hu: "Garissa a Garissa megye fővárosa, amely egy száraz vidéken fekszik a Tana folyó mentén.",
      ro: "Garissa este capitala comitatului Garissa, o regiune aridă situată pe râul Tana.",
      en: "Garissa is the capital of Garissa County, an arid region located on the Tana River."
    },
    facts: {
      de: [
        "Die Landschaft ist überwiegend eine trockene Wüste.",
        "Beherbergt eine große somalische Gemeinde."
      ],
      hu: [
        "A tájat elsősorban a száraz sivatag uralja.",
        "Jelentős szomáliai közösség él itt."
      ],
      ro: [
        "Peisajul este predominant deșert arid.",
        "Găzduiește o mare comunitate somaleză."
      ],
      en: [
        "The landscape is predominantly arid dessert.",
        "It hosts a large Somali community."
      ]
    }, image: "/poi-images/garissa-cities-v2.webp"},
  {
    id: "kakamega-cities-v2",
    type: "city",
    parent: "KE-37",
    coords: [34.7523, 0.2827],
    name: {
      de: "Kakamega",
      hu: "Kakamega",
      ro: "Kakamega",
      en: "Kakamega",
image: "/poi-images/kakamega-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kakamega, die Hauptstadt des gleichnamigen Kakamega Countys, liegt im Westen Kenias und ist untrennbar mit dem berühmten Kakamega-Wald verbunden. Dieser Wald ist der letzte verbliebene Überrest des einst riesigen guineo-kongolesischen Regenwaldes in Ostafrika und bietet eine unvergleichliche Biodiversität, darunter viele endemische Vogel-, Insekten- und Primatenarten. Die Stadt selbst entwickelte sich in den 1930er Jahren infolge des Kakamega-Goldrausches sprunghaft, als europäische Goldsucher in die Region strömten. Heute ist Kakamega das kulturelle Zentrum der Luhya-Gemeinschaft. Ein weiteres bekanntes Wahrzeichen in der Nähe ist der \"Crying Stone of Ilesi\", eine markante Felsformation, die aufgrund von abfließendem Wasser wie ein weinendes Gesicht aussieht und tief in der lokalen Mythologie verwurzelt ist (Geografie & Biologie K7).",
      hu: "Kakamega Nyugat-Kenya luhja közösségeinek kulturális és közigazgatási központja. A város neve szorosan összefonódik az 1930-as évek aranylázával, amikor a környéken rövid ideig tartó bányászati fellendülés indult. Ma a legfőbb nevezetessége a Kakamega-erdő, amely az utolsó maradványa a Guineo-Kongói esőerdő rendszernek Kelet-Afrikában, gazdag madár- és majomvilággal. A gazdaságot a cukornád és a kukorica uralja. (Természetismeret K5 — esőerdők és biodiverzitás)",
      ro: "Kakamega, capitala județului omonim, este un oraș aglomerat situat în vestul Kenyei, recunoscut pentru peisajele sale luxuriante și moștenirea culturală a poporului Luhya. Orașul s-a format în timpul goanei după aur din anii 1930, atrăgând prospectori europeni, însă astăzi economia se bazează pe cultivarea trestiei de zahăr și a porumbului. Bijuteria absolută a regiunii este Pădurea Kakamega, singura rămășiță din Kenya a pădurilor tropicale guineo-congoleze care odinioară acopereau centrul Africii. Această pădure oferă adăpost pentru sute de specii rare de păsări, fluturi și primate.",
      en: "Kakamega, the headquarters of Kakamega County, is a prominent urban center in western Kenya, historically defined by the brief but intense 1930s gold rush that catalyzed its early expansion. Located at an elevation of 1,535 meters, the city experiences high annual rainfall, creating an exceptionally green and fertile environment. It is intrinsically linked to the Kakamega Forest, the easternmost remnant of the Guineo-Congolian rainforest, established as a reserve in 1933 and renowned for its unique primate and avian biodiversity. Today, the local economy is largely driven by agricultural commerce, particularly sugarcane farming, and serves as a major cultural epicenter for the Luhya people. The establishment of Masinde Muliro University in 2007 has further accelerated the city's modernization and demographic growth. Geography K7 — Rainforest fringe settlements."
    },
    factsAdvanced: {
      en: ["Experienced a significant gold rush starting in 1931.", "Located at an altitude of approximately 1,535 meters (5,036 feet).", "The adjacent Kakamega Forest was declared a strict nature reserve in 1933.", "Masinde Muliro University of Science and Technology was founded in 2007.", "Serves as the administrative center and cultural heart for the Luhya community.", "The region receives high annual rainfall, often exceeding 2,000 millimeters.", "The urban population reached approximately 107,000 in the 2019 census."]
    },
    
    description: {
      de: "Kakamega ist eine bedeutende Stadt in Westkenia, bekannt für den nahe gelegenen tropischen Kakamega-Wald.",
      hu: "Kakamega egy jelentős város Nyugat-Kenyában, amely a közeli trópusi Kakamega-erdőről híres.",
      ro: "Kakamega este un oraș proeminent în vestul Kenyei, cunoscut pentru pădurea tropicală Kakamega din apropiere.",
      en: "Kakamega is a prominent town in western Kenya, known for the nearby tropical Kakamega Forest."
    },
    facts: {
      de: [
        "Verwaltungssitz des Kakamega County.",
        "Der Wald ist Kenias einziger tropischer Regenwald."
      ],
      hu: [
        "Kakamega megye székhelye.",
        "Az erdő Kenya egyetlen trópusi esőerdeje."
      ],
      ro: [
        "Sediul comitatului Kakamega.",
        "Pădurea este singura pădure tropicală tropicală din Kenya."
      ],
      en: [
        "Headquarters of Kakamega County.",
        "The forest is Kenya's only tropical rainforest."
      ]
    }, image: "/poi-images/kakamega-cities-v2.webp"},
  {
    id: "nyeri-cities-v2",
    type: "city",
    parent: "KE-19",
    coords: [36.9452, -0.4168],
    name: {
      de: "Nyeri",
      hu: "Nyeri",
      ro: "Nyeri",
      en: "Nyeri",
image: "/poi-images/nyeri-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Nyeri ist eine historisch und landwirtschaftlich bedeutende Stadt im zentralen Hochland von Kenia und dient als Hauptstadt des Nyeri Countys. Die Stadt liegt malerisch am Fuße der Aberdare-Bergkette und unweit der Hänge des mächtigen Mount Kenya. Gegründet 1902 als britischer Kolonialposten, bewahrt Nyeri viel von seinem historischen Charme. Bekannt ist Nyeri als die letzte Ruhestätte von Lord Robert Baden-Powell, dem Gründer der weltweiten Pfadfinderbewegung, dessen Grabstätte jährlich Pilger aus aller Welt anzieht. Auch die berühmte kenianische Friedensnobelpreisträgerin Wangari Maathai wurde in dieser Region geboren. Wirtschaftlich dominiert in der Umgebung der Anbau von erstklassigem Arabica-Kaffee und Tee, die in den kühlen, fruchtbaren Hochebenen exzellent gedeihen (Geschichte K8).",
      hu: "Nyeri a Kenya-hegy nyugati lejtőinél fekvő történelmi város, a Középső Tartomány központja. Az enyhe éghajlatú település a kávé- és teatermesztéséről híres, és fontos kiindulópontja a Kenya-hegyre induló expedícióknak. A város szorosan kapcsolódik a brit gyarmati múlthoz; itt található Robert Baden-Powell, a cserkészmozgalom alapítójának sírja, amely ma is sok látogatót vonz. Nyeri egyben a kikuju nép egyik fő kulturális bástyája is. (Történelem K7 — gyarmati korszak és mozgalmak)",
      ro: "Nyeri este un oraș istoric situat în inima regiunii centrale a Kenyei, la poalele spectaculoase ale Muntelui Kenya și ale lanțului Aberdare. Înființat în anul 1902 ca un post militar britanic, orașul a evoluat într-un prosper centru agricol, fiind renumit pentru plantațiile extinse de cafea și ceai de înaltă calitate. Nyeri are o semnificație istorică deosebită: este locul de odihnă al lui Robert Baden-Powell, fondatorul mișcării mondiale a cercetașilor, mormântul său atrăgând anual mii de pelerini. Orașul oferă acces facil la Parcurile Naționale Muntele Kenya și Aberdare.",
      en: "Nestled in the central highlands at a cool elevation of 1,750 meters, Nyeri is situated between the majestic massifs of Mount Kenya and the Aberdare Range. Established by the British in 1902 as a military post, the town quickly developed into a prosperous agricultural center specializing in high-quality Arabica coffee and tea production. Nyeri holds profound historical significance; it was a focal point during the Mau Mau uprising in the 1950s and contains the gravesite of Robert Baden-Powell, the founder of the Scout movement, who spent his final years here until his death in 1941. The city serves as a crucial staging point for mountaineering expeditions and wildlife tourism directed toward the Aberdare National Park. Its red volcanic soils and consistent rainfall make it one of Kenya's most productive agronomic zones. History K8 — Colonial legacy and agriculture."
    },
    factsAdvanced: {
      en: ["Founded as a British military post in December 1902.", "Located at an elevation of 1,750 meters (5,740 feet) in the central highlands.", "Serves as the administrative headquarters for Nyeri County.", "Lord Baden-Powell, founder of the Scouts, was buried here in 1941.", "Dedan Kimathi University of Technology was granted its charter in 2012.", "Situated between Mount Kenya to the east and the Aberdare Range to the west.", "The town's population was roughly 140,000 according to the 2019 national census."]
    },
    
    description: {
      de: "Nyeri liegt im zentralen Hochland von Kenia und ist ein wichtiges Zentrum für den Kaffee- und Teeanbau.",
      hu: "Nyeri Kenya központi hegyvidékén található, és a kávé- valamint teatermesztés fontos központja.",
      ro: "Nyeri este situat în zonele înalte centrale din Kenya și este un centru important pentru cultivarea cafelei și ceaiului.",
      en: "Nyeri is located in the central highlands of Kenya and is an important center for coffee and tea farming."
    },
    facts: {
      de: [
        "Heimat der Grabstätte von Lord Baden-Powell, dem Gründer der Pfadfinder.",
        "Liegt an den Ausläufern des Mount Kenya."
      ],
      hu: [
        "Itt található Lord Baden-Powellnek, a cserkészek alapítójának a sírja.",
        "A Mount Kenya lábánál fekszik."
      ],
      ro: [
        "Găzduiește locul de înmormântare al lui Lord Baden-Powell, fondatorul cercetașilor.",
        "Situat la poalele Muntelui Kenya."
      ],
      en: [
        "Home to the burial site of Lord Baden-Powell, founder of the Scouts.",
        "Situated at the foothills of Mount Kenya."
      ]
    }, image: "/poi-images/nyeri-cities-v2.webp"},
  {
    id: "machakos-cities-v2",
    type: "city",
    parent: "KE-16",
    coords: [37.2634, -1.5177],
    name: {
      de: "Machakos",
      hu: "Machakos",
      ro: "Machakos",
      en: "Machakos",
image: "/poi-images/machakos-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Machakos, benannt nach dem Kamba-Häuptling Masaku, ist eine der ältesten Verwaltungsstädte Kenias. Sie wurde bereits 1887, ein volles Jahrzehnt vor der Gründung von Nairobi, von der Imperial British East Africa Company als erster britischer Verwaltungsposten im Binnenland gegründet. Bis 1899 diente Machakos sogar als Hauptstadt des britischen Protektorats, bevor Nairobi diese Rolle übernahm. Heute ist Machakos die pulsierende Hauptstadt des Machakos Countys und ein wichtiges wirtschaftliches Zentrum, das sich durch seine Nähe zu Nairobi (nur 63 km entfernt) rasant entwickelt hat. Die hügelige Landschaft bietet eine atemberaubende Kulisse, und die Region ist ein Zentrum für Landwirtschaft und Handel. Die Stadt beherbergt auch den Machakos People's Park, eine der modernsten Freizeitanlagen des Landes (Geschichte K7).",
      hu: "Machakos egy dombvidéki város Nairobitól délkeletre, amely 1889-ben rövid ideig a Brit Kelet-Afrika első fővárosa is volt, mielőtt a közigazgatást Nairobiba helyezték át. A várost a szárazabb éghajlathoz alkalmazkodó kamba nép lakja. Machakos a közelmúltban jelentős modernizáción ment keresztül; felújított parkjai, új konferencia-központjai és sportlétesítményei révén Nairobi tehetős agglomerációs vonzáskörzetévé vált. Főbb terménye a kukorica és a gyümölcsök. (Történelem K6 — városok fejlődése és közigazgatás)",
      ro: "Machakos este un oraș pitoresc situat la sud-est de Nairobi, înconjurat de dealuri spectaculoase și peisaje rurale. Fondat în anul 1887 cu un deceniu înaintea capitalei Nairobi, Machakos a fost primul centru administrativ britanic din Africa de Est. Astăzi, este inima administrativă a județului Machakos și un hub cultural pentru comunitatea Akamba, cunoscuți pentru sculpturile lor din lemn și abilitățile de prelucrare a meșteșugurilor. Orașul se dezvoltă rapid, cu proiecte imobiliare moderne, dar își păstrează farmecul datorită parcurilor tematice precum Machakos People's Park.",
      en: "Machakos holds the distinction of being the first administrative capital of the British East Africa Protectorate, established in 1887 before the colonial headquarters were relocated to Nairobi in 1899. Nestled within a hilly terrain at an altitude of 1,715 meters, the city is surrounded by the picturesque Mua Hills and serves as the primary commercial and cultural nucleus for the Kamba people. Despite the semi-arid climate of the broader region, the hilly topography surrounding the town supports the cultivation of drought-resistant crops, fruits, and subsistence farming. In recent years, Machakos has undergone significant infrastructural modernization, including the construction of the Kenyatta Stadium, completed in 2013, and the development of the Machakos People's Park, which has revitalized local tourism and recreational commerce. History K7 — Early colonial administrative centers."
    },
    factsAdvanced: {
      en: ["Established by the Imperial British East Africa Company in 1887.", "Served as the capital of the East Africa Protectorate until 1899.", "Located at an elevation of 1,715 meters (5,626 feet).", "The modern 10,000-seat Kenyatta Stadium was completed in 2013.", "Machakos University was officially chartered as an independent institution in 2016.", "Features the anti-gravity phenomen of Kyamwilu, located a few kilometers outside the city.", "The urban population was recorded at approximately 63,000 in the 2019 census."]
    },
    
    description: {
      de: "Machakos ist eine schnell wachsende Stadt südöstlich von Nairobi und dient als wichtiges Verwaltungs- und Handelszentrum.",
      hu: "Machakos egy gyorsan növekvő város Nairobitól délkeletre, amely fontos közigazgatási és kereskedelmi csomópont.",
      ro: "Machakos este un oraș în creștere rapidă situat la sud-est de Nairobi, servind ca un hub administrativ și comercial cheie.",
      en: "Machakos is a fast-growing town located southeast of Nairobi, serving as a key administrative and commercial hub."
    },
    facts: {
      de: [
        "War die erste Hauptstadt des britischen Protektorats Ostafrika.",
        "Berühmt für seine wunderschön geschnitzten Holzkunstwerke."
      ],
      hu: [
        "Ez volt a Brit Kelet-afrikai Protektorátus első fővárosa.",
        "Híres a gyönyörűen faragott fa kézműves termékeiről."
      ],
      ro: [
        "A fost prima capitală a Protectoratului Britanic al Africii de Est.",
        "Faimos pentru obiectele sale de artizanat din lemn frumos sculptate."
      ],
      en: [
        "Was the first capital of the British East Africa Protectorate.",
        "Famous for its beautifully carved wooden handicrafts."
      ]
    }, image: "/poi-images/machakos-cities-v2.webp"},
  {
    id: "meru-cities-v2",
    type: "city",
    parent: "KE-12",
    coords: [37.6496, 0.0463],
    name: {
      de: "Meru",
      hu: "Meru",
      ro: "Meru",
      en: "Meru",
image: "/poi-images/meru-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Meru, im Osten der mächtigen Mount-Kenya-Region gelegen, ist das florierende Zentrum des Meru Countys. Die Stadt verdankt ihren Wohlstand den extrem fruchtbaren vulkanischen Böden und dem günstigen Klima an den Nordosthängen des Berges. Weltweit ist Meru als das Epizentrum für den Anbau von Miraa (Khat) bekannt, einer Pflanze, die wegen ihrer stimulierenden Wirkung angebaut und in ganz Ostafrika und in den Nahen Osten exportiert wird. Neben Khat gedeihen hier auch Tee, Kaffee und Bananen prächtig. Die Stadt fungiert als Tor zum faszinierenden Meru-Nationalpark, in dem einst die berühmte Löwin Elsa aus dem Buch \"Frei geboren\" (Born Free) von Joy Adamson lebte. Kulturell wird die Stadt vom Volk der Ameru geprägt, das eine reiche Tradition bewahrt (Geografie & Biologie K8).",
      hu: "Meru a Kenya-hegy északkeleti oldalán épült mezőgazdasági és kereskedelmi központ. A város és környéke egyedülálló mikroklímával rendelkezik, amely bőséges csapadékot biztosít a kávé-, tea- és banánültetvények, valamint a helyben népszerű 'miraa' (khat) termesztéséhez. Meru a meru népcsoport kulturális szíve, és fontos kiindulópontja a Meru Nemzeti Park szafarijainak. A helyi fafeldolgozás és a kereskedelem adja a gazdaság gerincét. (Földrajz K6 — mikroklíma és mezőgazdaság)",
      ro: "Meru este un oraș înfloritor situat pe pantele nord-estice ale Muntelui Kenya, înconjurat de păduri dense și terenuri agricole fertile. Cunoscut ca centrul comercial al județului Meru, orașul este faimos în întreaga țară pentru cultivarea extinsă a plantei miraa (khat), o cultură comercială extrem de profitabilă care susține economia locală. De asemenea, clima favorabilă permite cultivarea intensivă a cafelei, ceaiului și a diverselor legume. Turiștii folosesc Meru ca poartă de acces către Parcul Național Meru, locul unde a trăit celebra leoaică Elsa, detaliată în cartea «Născută Liberă».",
      en: "Perched on the northeastern slopes of Mount Kenya at an altitude of approximately 1,580 meters, Meru is a vital commercial and agricultural center in the eastern highlands. The city was formally established in 1911 as a colonial administrative post, strategically positioned in a region characterized by exceptionally fertile volcanic soils and reliable rainfall. Meru is historically renowned as a major production zone for coffee, tea, and notably khat (miraa), which forms a massive segment of the local export economy. The proximity to the Meru National Park, established in 1968 and famous for the conservation of the lioness Elsa, positions the town as an important logistical hub for northern tourism circuits. The urban layout reflects a blend of traditional market dynamics and expanding modern retail infrastructure. Economics K8 — Cash crop economies."
    },
    factsAdvanced: {
      en: ["Formally established as a colonial administrative post in 1911.", "Located on the northeastern slopes of Mount Kenya at 1,580 meters.", "Serves as the headquarters for Meru County.", "A major national hub for the cultivation and export of khat (miraa).", "Meru University of Science and Technology was chartered in 2013.", "Meru National Park, located to the east, was established in 1968.", "The town's population reached approximately 110,000 in the 2019 census."]
    },
    
    description: {
      de: "Meru ist eine Großstadt im Osten Kenias, gelegen an den nordöstlichen Hängen des Mount Kenya.",
      hu: "Meru egy nagyváros Kelet-Kenyában, a Mount Kenya északkeleti lejtőin.",
      ro: "Meru este un oraș major în estul Kenyei, situat pe pantele nord-estice ale Muntelui Kenya.",
      en: "Meru is a major town in eastern Kenya, situated on the northeastern slopes of Mount Kenya."
    },
    facts: {
      de: [
        "Ein wichtiges landwirtschaftliches und kommerzielles Zentrum.",
        "Bekannt für die großflächige Produktion von Miraa (Khat)."
      ],
      hu: [
        "Fontos mezőgazdasági és kereskedelmi központ.",
        "A miraa (khat) nagyszabású termesztéséről ismert."
      ],
      ro: [
        "Un centru agricol și comercial important.",
        "Cunoscut pentru producția pe scară largă de miraa (khat)."
      ],
      en: [
        "An important agricultural and commercial center.",
        "Known for the large-scale production of miraa (khat)."
      ]
    }, image: "/poi-images/meru-cities-v2.webp"},
  {
    id: "embu-cities-v2",
    type: "city",
    parent: "KE-14",
    coords: [37.4527, -0.5311],
    name: {
      de: "Embu",
      hu: "Embu",
      ro: "Embu",
      en: "Embu",
image: "/poi-images/embu-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Embu ist eine geschichtsträchtige Stadt in Zentral-Ost-Kenia, die traditionell als Hauptstadt der ehemaligen Eastern Province fungierte. Am südöstlichen Fuß des Mount Kenya auf rund 1.350 Metern Höhe gelegen, profitiert Embu von reichhaltigen vulkanischen Böden und zuverlässigen Regenfällen. Dies macht die Region zu einem landwirtschaftlichen Kraftzentrum, in dem vor allem Macadamia-Nüsse, Kaffee, Tee und Mais im großen Stil angebaut werden. Die Bevölkerung besteht hauptsächlich aus den ethnischen Gruppen der Embu und Mbeere. In der Umgebung befinden sich wichtige Staudämme entlang des Tana-Flusses, wie der Masinga- und der Kiambere-Damm, die einen Großteil des kenianischen Wasserkraftstroms produzieren. Die Stadt bietet eine ruhige Atmosphäre und dient als Knotenpunkt für Reisen in die nordöstlichen Gebiete Kenias (Geografie K7).",
      hu: "Embu egy nyüzsgő város a Kenya-hegy délkeleti lejtőin, az embu népcsoport központja. A terület híres kiváló minőségű, magashegyi kávéjáról és makadámdió-ültetvényeiről. A város fekvése miatt az éghajlat hűvösebb és csapadékosabb, mint az alacsonyabban fekvő síkságokon. Embu fontos oktatási központ is számos középiskolával és főiskolával, továbbá közlekedési csomópont a főváros, Nairobi és az északabbi megyék között. (Földrajz K7 — hegyvidéki mezőgazdaság)",
      ro: "Embu, capitala județului cu același nume, este un important centru agricol așezat pe pantele sud-estice ale Muntelui Kenya. Orașul beneficiază de precipitații abundente și soluri vulcanice fertile, ceea ce îl face un producător major de nuci de macadamia, ceai negru de calitate superioară și cafea. Regiunea este locuită predominant de grupurile etnice Embu și Mbeere. În plus, Embu este recunoscut pentru barajele sale masive de pe râul Tana, cum ar fi Barajul Masinga și Barajul Kiambere, care furnizează o mare parte din energia hidroelectrică a Kenyei.",
      en: "Embu is a robust agricultural town situated on the southeastern slopes of Mount Kenya, acting as the administrative headquarters for Embu County. Founded in 1906 by British colonial administrators, the settlement leveraged its favorable altitude of 1,350 meters and rich volcanic loam soils to become a major center for macadamia nut, coffee, and tea cultivation. The landscape is intensely green, bisected by the Rupingazi and Kapingazi rivers, which provide vital irrigation and contribute to the region's hydroelectric potential, notably linked to the nearby Seven Forks dam cascade on the Tana River. The town boasts a strong educational legacy, hosting institutions like the University of Embu, which was fully chartered in 2016. The local economy is tightly woven into the intensive terraced farming of the surrounding hillsides. Geography K6 — Slope agriculture and hydrology."
    },
    factsAdvanced: {
      en: ["Founded as a colonial administrative center in 1906.", "Located at an altitude of 1,350 meters (4,429 feet).", "Serves as the administrative capital of Embu County.", "University of Embu received its official charter in 2016.", "The nearby Seven Forks Hydroelectric project began construction in the 1960s.", "A major regional producer of macadamia nuts and Arabica coffee.", "The town's population was recorded at roughly 64,000 in the 2019 census."]
    },
    
    description: {
      de: "Embu ist eine bedeutende Stadt in der östlichen Provinz, gelegen an den Ausläufern des Mount Kenya.",
      hu: "Embu egy jelentős város a keleti tartományban, a Mount Kenya lábánál.",
      ro: "Embu este un oraș proeminent în Provincia de Est, situat la poalele Muntelui Kenya.",
      en: "Embu is a prominent town in the Eastern Province, situated at the foothills of Mount Kenya."
    },
    facts: {
      de: [
        "Umgeben von üppigen, fruchtbaren Tee- und Kaffeefarmen.",
        "Historisch ein wichtiger Verwaltungssitz der Provinz."
      ],
      hu: [
        "Buja, termékeny tea- és kávéültetvények veszik körül.",
        "Történelmileg egy fontos tartományi székhely volt."
      ],
      ro: [
        "Înconjurat de ferme luxuriante și fertile de ceai și cafea.",
        "Istoric un important sediu provincial."
      ],
      en: [
        "Surrounded by lush, fertile tea and coffee farms.",
        "Historically an important provincial headquarters."
      ]
    }, image: "/poi-images/embu-cities-v2.webp"},
  {
    id: "bungoma-cities-v2",
    type: "city",
    parent: "KE-39",
    coords: [34.5583, 0.5635],
    name: {
      de: "Bungoma",
      hu: "Bungoma",
      ro: "Bungoma",
      en: "Bungoma",
image: "/poi-images/bungoma-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Bungoma, gelegen in Westkenia nahe der Grenze zu Uganda, ist die Hauptstadt des Bungoma Countys. Die Stadt ist das wichtigste landwirtschaftliche und kommerzielle Zentrum der Region und eng mit der Bukusu-Untergruppe des Luhya-Volkes verbunden. In Bungoma dominiert der Zuckerrohranbau, angeführt von riesigen Verarbeitungsanlagen wie der Nzoia Sugar Company. Eine bedeutende historische Sehenswürdigkeit in der Nähe ist das Chetambe-Fort (Chetambe's Fort), wo 1895 die letzte große Widerstandsschlacht der Bukusu gegen die britischen Kolonialtruppen stattfand. Die Gegend ist auch für ihre traditionellen Beschneidungszeremonien (Khuketwa) bekannt, die ein wichtiges kulturelles Ereignis darstellen. Die Nähe zum Mount Elgon macht Bungoma zudem zu einem Ausgangspunkt für Exkursionen (Geschichte K8).",
      hu: "Bungoma Nyugat-Kenyában, az ugandai határ közelében található forgalmas kereskedelmi város, a bukusu luhja alcsoport központja. A város gazdasága erősen függ az Uganda felé irányuló tranzitkereskedelemtől és a helyi cukornád-termesztéstől. Bungoma híres a kerékpáros közlekedés, a 'boda-boda' széleskörű használatáról is, amely a mindennapi élet része. A környék sűrűn lakott és intenzíven művelt mezőgazdasági terület. (Földrajz K6 — határon átnyúló kereskedelem)",
      ro: "Bungoma este un oraș vibrant situat în vestul Kenyei, aproape de granița cu Uganda. Este centrul administrativ al județului Bungoma și inima culturală a poporului Bukusu, un subgrup al comunității Luhya. Economia regiunii se bazează pe agricultură, fiind faimoasă pentru vastitatea plantațiilor de trestie de zahăr, care alimentează numeroase fabrici de rafinare, și pentru culturile de porumb. Zona este adesea asociată cu peisaje dramatice, fiind un punct de plecare pentru explorarea Muntelui Elgon și a cascadei Nabuyole, un important sit de patrimoniu cultural și natural.",
      en: "Located in western Kenya near the Uganda border, Bungoma is an expanding commercial hub serving the densely populated agrarian regions of Bungoma County. The town evolved around the railway line constructed in the 1920s, which connected the coastal ports to the Ugandan interior, facilitating the export of regional agricultural produce. Bungoma's economy is heavily anchored in sugarcane farming, feeding the massive Nzoia Sugar Company factory located nearby, which was established in 1978. Situated at an elevation of 1,385 meters, the climate is highly favorable for both cash crops and extensive maize production. Culturally, it is the stronghold of the Bukusu sub-tribe of the Luhya people, maintaining deep-rooted traditions and annual circumcision ceremonies that shape the social calendar. Economics K7 — Border trade and agriculture."
    },
    factsAdvanced: {
      en: ["Developed primarily following the arrival of the railway in the late 1920s.", "Serves as the administrative headquarters of Bungoma County.", "Located at an elevation of roughly 1,385 meters (4,543 feet).", "The nearby Nzoia Sugar Company was established in 1978.", "Kibabii University, located on the outskirts, was chartered in 2015.", "Functions as a major transit route for cargo moving into neighboring Uganda.", "The urban center had a population of approximately 114,000 in 2019."]
    },
    
    description: {
      de: "Bungoma ist ein geschäftiges landwirtschaftliches und kommerzielles Zentrum in Westkenia.",
      hu: "Bungoma egy nyüzsgő mezőgazdasági és kereskedelmi központ Nyugat-Kenyában.",
      ro: "Bungoma este un centru agricol și comercial plin de viață în vestul Kenyei.",
      en: "Bungoma is a bustling agricultural and commercial center in western Kenya."
    },
    facts: {
      de: [
        "Die lokale Wirtschaft stützt sich stark auf den Anbau von Zuckerrohr.",
        "Ein wichtiger Knotenpunkt auf der Route nach Uganda."
      ],
      hu: [
        "A helyi gazdaság nagymértékben a cukornádtermesztésre támaszkodik.",
        "Fontos csomópont az Ugandába vezető úton."
      ],
      ro: [
        "Economia locală se bazează puternic pe cultivarea trestiei de zahăr.",
        "Un hub major pe ruta către Uganda."
      ],
      en: [
        "The local economy relies heavily on farming sugarcane.",
        "A major hub on the route to Uganda."
      ]
    }, image: "/poi-images/bungoma-cities-v2.webp"},
  {
    id: "naivasha-cities-v2",
    type: "city",
    parent: "KE-32",
    coords: [36.4357, -0.7167],
    name: {
      de: "Naivasha",
      hu: "Naivasha",
      ro: "Naivasha",
      en: "Naivasha",
image: "/poi-images/naivasha-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Naivasha ist eine aufstrebende Stadt im Nakuru County, die am Ufer des Lake Naivasha, einem der wenigen Süßwasserseen im kenianischen Teil des Rift Valley, liegt. In den letzten Jahrzehnten hat sich Naivasha zu einem der wichtigsten Zentren für den Anbau von Schnittblumen entwickelt; riesige Gewächshäuser säumen den See und exportieren Rosen in die ganze Welt. Die Stadt ist zudem ein Hotspot für den Tourismus. Der benachbarte Hell's-Gate-Nationalpark, bekannt für seine dramatischen Schluchten und die Möglichkeit, dort gefahrlos zu wandern und Fahrrad zu fahren, zog sogar die Macher des Films \"Der König der Löwen\" zur Inspiration an. Zudem liegt das Geothermiekraftwerk Olkaria, eines der größten in Afrika, im Süden des Sees (Wirtschaft & Geografie K8).",
      hu: "Naivasha egy gyorsan fejlődő turisztikai és ipari város a Nagy-hasadékvölgyben. Hírnevét a Naivasha-tóról kapta, amely gazdag madárvilágával és vízilovak sokaságával vonzza a szafarizókat. A város környéke Kenya virágiparának epicentruma; az itt található hatalmas üvegházakból származik az Európába exportált kenyai rózsák jelentős része. A közelben található Olkaria geotermikus erőmű az ország energiaellátásának egyik alappillére. (Földrajz K8 — geotermikus energia és export)",
      ro: "Naivasha este un oraș turistic și agricol în plină ascensiune, așezat pe țărmul faimosului Lac Naivasha, în Valea Marelui Rift. Orașul este recunoscut pe plan internațional ca un epicentru al floriculturii; sere imense produc milioane de trandafiri care sunt exportați zilnic către piețele din Europa. În plus, Naivasha atrage nenumărați turiști dornici să exploreze Parcul Național Hell's Gate, unul dintre puținele parcuri din Kenya unde se poate merge pe jos sau cu bicicleta printre animale sălbatice, și Parcul Național Longonot, unde pasionații pot escalada vulcanul latent.",
      en: "Naivasha is a bustling market town situated at an altitude of 1,900 meters on the floor of the Great Rift Valley, historically positioned on the shores of Lake Naivasha. Originating as a colonial settler hub and a station on the Uganda Railway in 1899, the town has dramatically transformed into the epicenter of Kenya's multi-billion-shilling floriculture industry. Dozens of vast greenhouses line the lake, producing roses and carnations exported daily to European markets via Nairobi. The region is characterized by intense geothermal activity, with the Olkaria Geothermal Power Station, first commissioned in 1981, providing a significant percentage of the national grid's electricity. Naivasha also serves as the primary gateway to Hell's Gate National Park, renowned for its dramatic volcanic scenery and geothermal features. Economics K8 — Floriculture and renewable energy."
    },
    factsAdvanced: {
      en: ["Established as a railway station on the Uganda Railway in 1899.", "Situated at an altitude of 1,900 meters (6,233 feet) in the Rift Valley.", "The epicenter of Kenya's export floriculture, specializing in roses.", "Olkaria Geothermal Power Station I was commissioned south of the town in 1981.", "Serves as the gateway to Hell's Gate National Park, established in 1984.", "Lake Naivasha is a freshwater lake, unlike most other Rift Valley lakes.", "The urban population was recorded at approximately 198,000 in the 2019 census."],
      de: ["1899 als Bahnhof der Uganda-Eisenbahn gegründet.", "Liegt auf einer Höhe von 1.900 Metern im Rift Valley.", "Epizentrum der kenianischen Blumenexportindustrie, spezialisiert auf Rosen.", "Das Geothermiekraftwerk Olkaria I wurde 1981 südlich der Stadt in Betrieb genommen.", "Tor zum Hell's-Gate-Nationalpark, der 1984 gegründet wurde.", "Der Name leitet sich vom Massai-Wort ɛnaɨpɔ́sha ab, was „das, was wogt“ bedeutet."]
    },
    
    description: {
      de: "Naivasha ist eine Marktstadt im Rift Valley, berühmt für ihre florierende Blumenindustrie und ihren wunderschönen See.",
      hu: "Naivasha egy piacváros a Rift Valley-ben, amely a virágzó virágkertészetéről és gyönyörű taváról híres.",
      ro: "Naivasha este un oraș-târg în Valea Rift, renumit pentru industria sa floricolă în plină expansiune și lacul său frumos.",
      en: "Naivasha is a market town in the Rift Valley, renowned for its booming floriculture industry and beautiful lake."
    },
    facts: {
      de: [
        "Ein führendes Zentrum für den Export von Schnittblumen.",
        "Der Naivashasee ist ein beliebtes Reiseziel für Touristen."
      ],
      hu: [
        "A vágott virág exportjának vezető központja.",
        "A Naivasha-tó népszerű turisztikai célpont."
      ],
      ro: [
        "Un centru lider pentru exporturile de flori tăiate.",
        "Lacul Naivasha este o destinație populară pentru turiști."
      ],
      en: [
        "A leading center for cut-flower exports.",
        "Lake Naivasha is a popular destination for tourists."
      ]
    }, image: "/poi-images/naivasha-cities-v2.webp"},
  {
    id: "kericho-cities-v2",
    type: "city",
    parent: "KE-35",
    coords: [35.2831, -0.3708],
    name: {
      de: "Kericho",
      hu: "Kericho",
      ro: "Kericho",
      en: "Kericho",
image: "/poi-images/kericho-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kericho ist die unangefochtene Teehauptstadt Kenias. In den Hochlagen des westlichen Rift Valley auf über 2.000 Metern gelegen, profitiert die Stadt von einem kühlen Klima und reichlichen Niederschlägen, die perfekte Bedingungen für den Teeanbau bieten. Kenia ist einer der größten Tee-Exporteure der Welt, und Kericho bildet das Epizentrum dieser Industrie, umgeben von schier endlosen, sattgrünen Teeplantagen, die internationalen Konzernen wie Finlays oder Unilever gehören. Die Stadt hat eine lange koloniale Geschichte, was sich in der Architektur und den gepflegten Golfplätzen widerspiegelt. Die mehrheitlich von der Kipsigis-Ethnie bewohnte Region ist nicht nur landwirtschaftlich bedeutsam, sondern auch kulturell, und beheimatet die größte Gurdwara (Sikh-Tempel) in ganz Afrika (Wirtschaft K8).",
      hu: "Kericho neve egyet jelent a kenyai teával. A Viktória-tó medencéje fölé magasodó dombokon fekvő város egyedülálló mikroklímával bír: a bőséges és egyenletes esőzések ideálisak a teacserjék számára. Itt találhatók a világ legnagyobb egybefüggő teaültetvényei. A város maga egy csendes, zöld oázis, melyet évtizedek óta a teatermesztő vállalatok és a kipsigis népcsoport alakít, szigorú rendben tartott mezőgazdasági farmokkal. (Földrajz K7 — globális agrárpiac és tea)",
      ro: "Kericho este renumit pe plan mondial ca centrul incontestabil al industriei ceaiului din Kenya. Aflat în regiunea de sud-vest, orașul este înconjurat de dealuri acoperite cu plantații de ceai cu un verde strălucitor și impecabil îngrijite, care se întind până la orizont. Datorită precipitațiilor zilnice regulate și solurilor bogate, Kericho produce un ceai negru considerat printre cele mai bune din lume. Pe lângă importanța sa economică enormă, orașul oferă peisaje uimitoare și un aer răcoros, fiind un refugiu perfect pentru iubitorii de natură și agro-turism.",
      en: "Kericho is synonymous with Kenya's globally renowned tea industry, situated in the verdant highlands west of the Great Rift Valley. Founded in 1902, the town sits at a commanding elevation of 2,000 meters, an environment characterized by consistent daily rainfall and deep, well-drained volcanic soils perfect for Camellia sinensis cultivation. The landscape is dominated by vast, manicured tea estates established in the 1920s by multinational corporations, making Kericho the largest tea-producing region in Africa. The local economy revolves almost entirely around tea picking, processing, and export logistics. Beyond agriculture, the town hosts the massive Holy Trinity Cathedral, completed in 2015, which stands as a landmark of modern architectural design in the region. Economics K7 — Monoculture and global exports."
    },
    factsAdvanced: {
      en: ["Founded as an administrative and trading post in 1902.", "Located at an elevation of 2,000 meters (6,560 feet) in the western highlands.", "The first commercial tea was planted in the Kericho area in 1924.", "Serves as the headquarters for Kericho County.", "The striking Holy Trinity Cathedral in Kericho was completed and consecrated in 2015.", "The region receives near-daily rainfall, often exceeding 1,700 mm annually.", "The town's population was recorded at roughly 104,000 in the 2019 census."]
    },
    
    description: {
      de: "Kericho ist die unbestrittene Tee-Hauptstadt Kenias, geprägt von sanften Hügeln, die vollständig von Teeplantagen bedeckt sind.",
      hu: "Kericho Kenya vitathatatlan teafővárosa, melynek dombjait teljesen teacserjék borítják.",
      ro: "Kericho este capitala incontestabilă a ceaiului din Kenya, având dealuri complet acoperite de plantații de ceai.",
      en: "Kericho is the undisputed tea capital of Kenya, featuring rolling hills completely covered by tea plantations."
    },
    facts: {
      de: [
        "Die Region produziert einige der feinsten Teesorten der Welt.",
        "Verfügt über ein sehr kühles und nasses Hochlandklima."
      ],
      hu: [
        "A régió a világ legkiválóbb teáit termeli.",
        "Nagyon hűvös és csapadékos hegyvidéki éghajlattal rendelkezik."
      ],
      ro: [
        "Regiunea produce o parte din cel mai bun ceai din lume.",
        "Are un climat de munte foarte răcoros și umed."
      ],
      en: [
        "The region produces some of the world's finest tea.",
        "Features a very cool and wet highland climate."
      ]
    }, image: "/poi-images/kericho-cities-v2.webp"},
  {
    id: "lamu-cities-v2",
    type: "city",
    parent: "KE-05",
    coords: [40.902, -2.2717],
    name: {
      de: "Lamu",
      hu: "Lamu",
      ro: "Lamu",
      en: "Lamu",
image: "/poi-images/lamu-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Lamu Town, gelegen auf der Insel Lamu vor der Nordküste Kenias, ist die älteste erhaltene Swahili-Siedlung in Ostafrika. Gegründet im 14. Jahrhundert, ist die Stadt seit 2001 ein UNESCO-Weltkulturerbe. Die Architektur ist durch enge, verwinkelte Gassen, kunstvoll geschnitzte Holztüren und imposante Korallensteinhäuser geprägt. Bemerkenswert ist, dass es in Lamu fast keine Autos gibt; der Transport erfolgt traditionell mit Eseln oder Daus (traditionelle Segelboote). Die Stadt war einst ein blühendes Zentrum für den Handel mit Elfenbein, Gewürzen und Sklaven und ist bis heute ein wichtiges Zentrum islamischer Gelehrsamkeit in Ostafrika. Das Lamu Fort, das Lamu Museum und das jährliche Maulidi-Festival ziehen Forscher und Touristen gleichermaßen an, um diese unberührte Kultur zu erleben (Geschichte K8).",
      hu: "Lamu egy történelmi szigetváros az Indiai-óceán partvidékén, Kelet-Afrika legrégebbi és legjobban megőrzött szuahéli települése, az UNESCO Világörökség része. A város szűk, kanyargós utcáin nincsenek autók, a közlekedést szamarakon és gyalogosan oldják meg. Gazdag, évszázados építészete a korallkő házakkal, faragott faajtókkal a 14. századig nyúlik vissza. Gazdasága főként a kulturális turizmusból és az ősi dhow-hajóépítésből áll. (Történelem K8 — szuahéli kultúra és világörökség)",
      ro: "Lamu este una dintre cele mai vechi și mai bine conservate așezări swahili din Africa de Est, situată pe insula cu același nume de pe coasta Oceanului Indian. Declarat Patrimoniu Mondial UNESCO în 2001, orașul vechi fascinează prin arhitectura sa unică, strâmtele ulițe labirintice și porțile din lemn masiv bogat sculptate. Traficul motorizat este interzis pe insulă, transportul fiind asigurat aproape exclusiv cu măgari și bărci cu pânze dhow. Lamu este un centru vibrant al culturii și religiei islamice, găzduind un faimos festival cultural anual.",
      en: "Lamu Town, situated on Lamu Island off the northern coast of Kenya, is the oldest continuously inhabited Swahili settlement in East Africa, dating back to at least the 14th century. Designated a UNESCO World Heritage site in 2001, the town is a masterful preservation of Swahili architecture, characterized by narrow, winding streets, intricately carved wooden doors, and coral stone buildings. With no motorized vehicles allowed, transport relies exclusively on donkeys, dhows, and foot traffic, preserving an atmosphere untouched by modern urbanization. The town historically served as a major center for the timber, ivory, and spice trades. Today, its economy is heavily reliant on cultural tourism and traditional maritime activities, while the nearby Lamu Port (LAPSSET) project on the mainland introduces a stark contrast of 21st-century infrastructure. History K8 — Swahili coastal civilization."
    },
    factsAdvanced: {
      en: ["Established as a Swahili trading settlement in the 14th century.", "Lamu Old Town was inscribed as a UNESCO World Heritage site in 2001.", "The historic Lamu Fort was constructed between 1813 and 1821.", "Motorized vehicles are largely banned; transport relies on donkeys and boats.", "The Riyadha Mosque in Lamu, a major Islamic learning center, was built in 1900.", "Serves as the administrative center of Lamu County.", "The town features an annual cultural festival celebrating traditional dhow racing."]
    },
    
    description: {
      de: "Lamu ist eine historische Stadt auf der Insel Lamu und gilt als die älteste kontinuierlich bewohnte Siedlung in Kenia.",
      hu: "Lamu egy történelmi város a Lamu-szigeten, amely Kenya legrégebbi, folyamatosan lakott települése.",
      ro: "Lamu este un oraș istoric pe insula Lamu, recunoscut ca cea mai veche așezare locuită continuu din Kenya.",
      en: "Lamu is a historic town on Lamu Island, recognized as the oldest continuously inhabited settlement in Kenya."
    },
    facts: {
      de: [
        "Wurde zum UNESCO-Weltkulturerbe erklärt.",
        "Motorisierte Fahrzeuge sind weitgehend verboten; Esel sind allgegenwärtig."
      ],
      hu: [
        "Az UNESCO Világörökség részévé nyilvánították.",
        "A motoros járművek nagyjából be vannak tiltva; a szamarak mindennaposak."
      ],
      ro: [
        "Desemnat ca sit al Patrimoniului Mondial UNESCO.",
        "Vehiculele motorizate sunt în mare parte interzise; măgarii sunt comuni."
      ],
      en: [
        "Designated as a UNESCO World Heritage site.",
        "Motorized vehicles are largely banned; donkeys are common."
      ]
    }, image: "/poi-images/lamu-cities-v2.webp"},
  {
    id: "isiolo-cities-v2",
    type: "city",
    parent: "KE-11",
    coords: [37.5833, 0.35],
    name: {
      de: "Isiolo",
      hu: "Isiolo",
      ro: "Isiolo",
      en: "Isiolo",
image: "/poi-images/isiolo-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Isiolo liegt im geografischen Zentrum Kenias und dient oft als Grenze zwischen dem fruchtbaren Hochland im Süden und den weiten, halbtrockenen Wüstenregionen im Norden. Die Stadt hat eine diverse Bevölkerung, zu der Borana, Somali, Samburu und Meru gehören. Isiolo entwickelte sich historisch aus Militär- und Verwaltungslagern der Briten nach dem Ersten Weltkrieg. In den letzten Jahren hat Isiolo als geplantes Zentrum des ehrgeizigen LAPSSET-Infrastrukturkorridors (Lamu Port-South Sudan-Ethiopia Transport) enorm an strategischer Bedeutung gewonnen, was zum Bau des Isiolo International Airport führte. Für Touristen ist Isiolo das Tor zu den berühmten Wildreservaten Samburu, Buffalo Springs und Shaba National Reserve (Geografie K7).",
      hu: "Isiolo egy stratégiai fekvésű határváros, amely átjárót képez Kenya zöldebb hegyvidékei és az északi sivatagos síkságok között. A települést eredetileg brit katonai bázisként alapították a nomád törzsek, például a szomálik és a boranák találkozási pontján. Ma Isiolo egy gyorsan fejlődő logisztikai csomópont a LAPSSET-folyosó részeként, új nemzetközi repülőtérrel és aszfaltozott utakkal. A várostól északra találhatók a híres Samburu és Buffalo Springs nemzeti rezervátumok. (Földrajz K8 — infrastrukturális fejlesztés és logisztika)",
      ro: "Isiolo este considerat poarta principală de intrare către vasta regiune sălbatică a Kenyei de Nord. Originar dezvoltat în jurul taberelor militare de după Primul Război Mondial, orașul multicultural este un punct de intersecție vital pentru comercianți și păstori din comunitățile Borana, Samburu, Meru și Somali. Odată cu dezvoltarea recentă a coridorului LAPSSET și modernizarea aeroportului internațional Isiolo, localitatea cunoaște o transformare rapidă. Din punct pentru turiști, Isiolo oferă acces rapid la spectaculoasele rezerve naționale Samburu, Shaba și Buffalo Springs.",
      en: "Isiolo functions as the crucial geographical and cultural gateway between the fertile central highlands of Kenya and the expansive, arid northern frontier. Established in the 1920s as a military post by the British to monitor pastoralist movements, the town has grown into a major transport logistics hub, highly diverse and populated largely by Borana, Somali, and Meru communities. The landscape transitions sharply here from green slopes to acacia scrubland. As a central node in the ambitious LAPSSET corridor project, Isiolo's infrastructure was drastically upgraded, marked by the opening of the Isiolo International Airport in 2017. The town also serves as the primary staging ground for tourism into the nearby Buffalo Springs, Shaba, and Samburu National Reserves. Geography K7 — Ecotone transitions and transport corridors."
    },
    factsAdvanced: {
      en: ["Established as a British military outpost in the 1920s.", "Located at an altitude of 1,145 meters (3,756 feet).", "Isiolo International Airport was officially opened in July 2017.", "Serves as the administrative capital of Isiolo County.", "Positioned as a central hub in the LAPSSET (Lamu Port-South Sudan-Ethiopia-Transport) corridor.", "Acts as the gateway to the Samburu National Reserve, established in 1948.", "The urban population was roughly 80,000 according to the 2019 national census."]
    },
    
    description: {
      de: "Isiolo ist eine kulturell vielfältige Stadt in der oberen östlichen Region Kenias und dient als Tor zum Norden.",
      hu: "Isiolo egy kulturálisan sokszínű város Kenya felső keleti régiójában, amely kapuként szolgál az északi területek felé.",
      ro: "Isiolo este un oraș cu o mare diversitate culturală situat în regiunea de est superioară a Kenyei, acționând ca o poartă către nord.",
      en: "Isiolo is a culturally diverse town located in the upper eastern region of Kenya, acting as a gateway to the north."
    },
    facts: {
      de: [
        "Dient als wichtiger Verkehrsknotenpunkt nach Moyale.",
        "Hier befindet sich ein neu entwickelter internationaler Flughafen."
      ],
      hu: [
        "Fontos tranzit csomópont Moyale felé.",
        "Egy újonnan épült nemzetközi repülőtér is található itt."
      ],
      ro: [
        "Servește ca un nod major de tranzit către Moyale.",
        "Aici este situat un aeroport internațional recent dezvoltat."
      ],
      en: [
        "Serves as a major transit hub to Moyale.",
        "A newly developed international airport is located here."
      ]
    }, image: "/poi-images/isiolo-cities-v2.webp"},
  {
    id: "homa-bay-cities-v2",
    type: "city",
    parent: "KE-43",
    coords: [34.4531, -0.5273],
    name: {
      de: "Homa Bay",
      hu: "Homa Bay",
      ro: "Homa Bay",
      en: "Homa Bay",
image: "/poi-images/homa-bay-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Homa Bay liegt am malerischen südlichen Ufer des Viktoriasees und ist die Verwaltungshauptstadt des Homa Bay Countys. Die Wirtschaft der Stadt ist stark von der Fischerei geprägt, insbesondere dem Fang von Nilbarsch und Tilapia, sowie der Landwirtschaft auf dem fruchtbaren Boden der Umgebung. Homa Bay ist der ideale Ausgangspunkt für Besuche im Ruma-Nationalpark, dem einzigen Ort in Kenia, an dem die seltene Pferdeantilope (Roan Antelope) in freier Wildbahn beobachtet werden kann. Die Stadt wird im Hintergrund von den Homa-Bergen (Mount Homa) überragt, einer Reihe erloschener vulkanischer Hügel. Homa Bay spielt eine zentrale Rolle in der Luo-Kultur und ist ein wichtiges Zentrum für den Binnenhandel auf dem Viktoriasee (Biologie & Geografie K7).",
      hu: "Homa Bay egy pezsgő partmenti város a Viktória-tó keleti öblében, a tókörnyéki halászat egyik fellegvára. A város a Homa-hegy, egy régi vulkáni hegy lábánál épült. A gazdaság jelentős része a tilápia és a nílusi sügér halászatán alapul, amelyeket belföldre és exportra is szánnak. A közeli Ruma Nemzeti Park miatt a régió egyre vonzóbb a turisták számára, a helyi luo közösség pedig gazdag folklórral és mesterségekkel várja a látogatókat. (Természetismeret K5 — halászat és tóvidékek)",
      ro: "Homa Bay este un oraș pitoresc situat pe malul estic al Golfului Winam, în extremitatea estică a Lacului Victoria. Datorită poziției sale, economia orașului este profund ancorată în industria pescuitului, capturile de biban de Nil și tilapia susținând o mare parte din populația locală a comunității Luo. Orașul servește drept punct de plecare excelent pentru explorarea atracțiilor naturale uimitoare din regiune, cum ar fi Muntele Homa, situl paleontologic de la Kanam și Parcul Național Ruma, singurul loc din Kenya unde poate fi admirată rara antilopă roană.",
      en: "Homa Bay is a prominent port town situated on the southern shores of the Winam Gulf of Lake Victoria, framed by the backdrop of the volcanic Mount Homa. Established in the mid-20th century, the town has grown primarily as a fishing and agricultural center, heavily reliant on the lake's resources, particularly the Nile perch and tilapia fisheries. The surrounding topography is distinguished by the nearby Ruma National Park, gazetted in 1983, which protects one of the last remaining populations of the endangered roan antelope in Kenya. The local climate is warm and semi-humid, supporting the cultivation of sorghum, maize, and cotton in the hinterlands. Recent infrastructural investments have sought to revive passenger and cargo ferry services across the lake to Kisumu and neighboring countries. Economics K6 — Lake economies and fishing."
    },
    factsAdvanced: {
      en: ["Located on the southern shores of Lake Victoria's Winam Gulf.", "Serves as the administrative headquarters of Homa Bay County.", "Mount Homa, a broad volcanic complex, is located 20 kilometers north.", "Ruma National Park, known for roan antelopes, was gazetted in 1983.", "The local economy relies heavily on the Nile perch and tilapia fishing industries.", "Situated at an elevation of approximately 1,135 meters (3,724 feet).", "The town's population was recorded at roughly 112,000 in the 2019 census."]
    },
    
    description: {
      de: "Homa Bay ist eine Küstenstadt am südlichen Ufer des Viktoriasees.",
      hu: "Homa Bay egy tóparti város, amely a Viktória-tó déli partján helyezkedik el.",
      ro: "Homa Bay este un oraș pe malul lacului, poziționat pe țărmul sudic al Lacului Victoria.",
      en: "Homa Bay is a lakeside town positioned on the southern shore of Lake Victoria."
    },
    facts: {
      de: [
        "Fischfang ist eine wichtige Einnahmequelle.",
        "Der Mount Homa und der Ruma-Nationalpark liegen in der Nähe."
      ],
      hu: [
        "A halászat az egyik fő bevételi forrás.",
        "A Mount Homa és a Ruma Nemzeti Park is a közelben van."
      ],
      ro: [
        "Pescuitul este o sursă majoră de venit.",
        "Muntele Homa și Parcul Național Ruma sunt în apropiere."
      ],
      en: [
        "Fishing is a major source of income.",
        "Mount Homa and Ruma National Park are nearby."
      ]
    }, image: "/poi-images/homa-bay-cities-v2.webp"},
  {
    id: "migori-cities-v2",
    type: "city",
    parent: "KE-44",
    coords: [34.4716, -1.0634],
    name: {
      de: "Migori",
      hu: "Migori",
      ro: "Migori",
      en: "Migori",
image: "/poi-images/migori-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Migori, gelegen nahe der Grenze zu Tansania, ist ein geschäftiger Handelsknotenpunkt im Südwesten Kenias. Die Stadt profitiert enorm von dem grenzüberschreitenden Handel und verbindet Kenia über den Grenzübergang Isebania mit Tansania. Die Region ist reich an Bodenschätzen, und in den ländlichen Gebieten rund um Migori wird seit Jahrzehnten handwerklicher Goldabbau betrieben, was ihr den Ruf als Goldgräberstadt eingebracht hat. Neben dem Bergbau dominiert die Landwirtschaft, insbesondere der Anbau von Zuckerrohr und Tabak. Kulturell ist Migori bemerkenswert vielfältig, da hier Bevölkerungsgruppen wie die Luo, Kuria und Abasuba eng zusammenleben. Ein historisches Highlight in der Nähe ist Thimlich Ohinga, eine archäologische Stätte mit steinernen Trockenmauern aus dem 16. Jahrhundert (Geschichte K7).",
      hu: "Migori a kenyai-tanzániai határ mentén fekvő dinamikus kereskedőváros. A térség aranybányászatáról is ismert; a helyi folyók mentén már a gyarmati idők óta folytatnak kézi bányászatot. A város mezőgazdasága is kiemelkedő, különösen a cukornád, a dohány és a kukorica termesztése révén. Határvárosi jellege miatt Migori kulturálisan sokszínű, többek között luo, kuria és tanzániai bevándorlók olvasztótégelye. (Földrajz K7 — bányászat és határvárosok)",
      ro: "Migori este un oraș comercial extrem de dinamic, situat în sud-vestul Kenyei, aproape de granița cu Tanzania. Fiind un oraș de tranzit pe ruta majoră Isebania-Mwanza, economia sa prosperă din comerțul transfrontalier, agricultură și minerit la scară mică. Cultivarea trestiei de zahăr, a tutunului și pescuitul pe Lacul Victoria sunt pilonii principali ai regiunii. Mai mult, dealurile din jurul Migoriului sunt faimoase pentru zăcămintele lor de aur, atrăgând numeroși mineri artizanali în căutarea unor surse alternative de venit, transformând orașul într-un pol efervescent de activitate.",
      en: "Migori is a bustling commercial center located in southwestern Kenya, positioned strategically just 20 kilometers from the Tanzanian border crossing at Isebania. The town sits at an elevation of 1,400 meters within the fertile Lake Victoria basin, facilitating robust agricultural production including sugarcane, tobacco, and maize. Migori developed rapidly in the late 20th century as a critical transit point for cross-border trade, fostering a highly cosmopolitan demographic comprising Luo, Kuria, and Kisii communities. The region has also seen significant artisanal gold mining activity along the Macalder greenstone belt, which heavily influences the local informal economy. The nearby Thimlich Ohinga, a remarkable 16th-century dry-stone walled settlement, adds immense historical gravity to the broader county landscape. Economics K7 — Border economies and artisanal mining."
    },
    factsAdvanced: {
      en: ["Located 20 kilometers from the Isebania border crossing into Tanzania.", "Serves as the administrative capital of Migori County.", "Situated at an altitude of approximately 1,400 meters (4,593 feet).", "The region experiences significant artisanal gold mining along the Macalder belt.", "The nearby Thimlich Ohinga archaeological site was built in the 16th century.", "Cross-border commerce heavily drives the local retail and transport sectors.", "The urban population reached approximately 100,000 in the 2019 national census."]
    },
    
    description: {
      de: "Migori ist eine lebhafte Grenzstadt nahe der tansanischen Grenze im Südwesten Kenias.",
      hu: "Migori egy élénk határváros a tanzániai határ közelében, Kenya délnyugati részén.",
      ro: "Migori este un oraș de graniță plin de viață, situat lângă granița cu Tanzania, în partea de sud-vest a Kenyei.",
      en: "Migori is a lively border town located near the Tanzanian border in the southwestern part of Kenya."
    },
    facts: {
      de: [
        "Ein bedeutendes Zentrum für den grenzüberschreitenden Handel.",
        "In den umliegenden Regionen wird Gold abgebaut."
      ],
      hu: [
        "A határon átnyúló kereskedelem jelentős központja.",
        "A környező régiókban aranybányászat is folyik."
      ],
      ro: [
        "Un hub semnificativ pentru comerțul transfrontalier.",
        "Activități de exploatare a aurului au loc în regiunile înconjurătoare."
      ],
      en: [
        "A significant hub for cross-border trade.",
        "Gold mining activities occur in the surrounding regions."
      ]
    }, image: "/poi-images/migori-cities-v2.webp"},
  {
    id: "kajiado-cities-v2",
    type: "city",
    parent: "KE-34",
    coords: [36.7772, -1.8546],
    name: {
      de: "Kajiado",
      hu: "Kajiado",
      ro: "Kajiado",
      en: "Kajiado",
image: "/poi-images/kajiado-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kajiado, südlich von Nairobi gelegen, ist das administrative Zentrum des Kajiado Countys und eine Stadt, die tief im Herzen des Maasai-Landes liegt. Während die Stadt in der Vergangenheit ein kleiner Handelsposten war, ist sie heute stark in den Pendlergürtel von Nairobi integriert, was zu rasantem Bevölkerungswachstum und Immobilienentwicklung geführt hat. Dennoch bleibt Kajiado ein wichtiges Zentrum für die Kultur der Maasai, wo traditionelle Rinderzucht auf moderne Urbanisierung trifft. Die Stadt liegt in einer halbtrockenen Region in den Kitengela-Ebenen und hat historisch vom Bau der Magadi-Eisenbahnlinie profitiert, die bis heute Soda-Asche aus dem Lake Magadi transportiert (Geografie K7).",
      hu: "Kajiado egy viszonylag száraz, sík területen fekvő város Nairobitól délre, amely a maszáj pásztorkultúra egyik legfontosabb központja. A város hagyományosan a szarvasmarha-kereskedelemre támaszkodik, de a főváros közelsége miatt jelentős ingázó településsé és ingatlanfejlesztési célponttá is vált. A környéken található Ngong-hegység és az Amboseli Nemzeti Park felé vezető út miatt a turisztikai forgalom is élénk. (Földrajz K6 — pásztorkodás és szuburbanizáció)",
      ro: "Kajiado, capitala județului cu același nume, este un oraș cu o identitate culturală puternică, aflându-se în inima teritoriilor poporului Maasai. Aflat la sud de Nairobi, pe calea ferată către Magadi, Kajiado are o economie tradițională centrată pe păstorit și comerțul cu vite, dar experimentează o rapidă expansiune imobiliară datorită apropierii sale de capitală. Orașul este recunoscut pentru piețele sale vibrante de animale, unde comercianți și păstori se întâlnesc în haine tradiționale colorate, reflectând echilibrul dintre urbanizarea modernă și moștenirea ancestrală Maasai.",
      en: "Kajiado is the administrative heart of Kajiado County, positioned in the semi-arid plains south of Nairobi at an elevation of 1,700 meters. Evolving from a small railway stop on the Magadi branch line built in 1915 to transport soda ash, the town has deep roots in the pastoralist culture of the Maasai people, who have historically dominated the region. In recent decades, Kajiado has experienced rapid demographic expansion due to urban spillover from Nairobi, leading to a transition from pure pastoralism to mixed land-use and real estate development. Despite modernization, the town remains a critical livestock trading hub. The surrounding savannah landscape is characterized by acacia scrub and serves as a dispersal area for wildlife moving between the Amboseli and Nairobi national parks. Geography K6 — Urban sprawl and pastoralism."
    },
    factsAdvanced: {
      en: ["Grew as a station on the Magadi Railway line, completed in 1915.", "Serves as the administrative headquarters of Kajiado County.", "Located at an elevation of roughly 1,700 meters (5,577 feet).", "A major cultural and administrative center for the Maasai people.", "Experiences a semi-arid climate with distinct wet and dry seasons.", "Functions as a major livestock trading market for the southern region.", "The urban center had a population of approximately 24,000 in the 2019 census."]
    },
    
    description: {
      de: "Kajiado ist das Verwaltungszentrum des Kajiado County, das überwiegend vom Volk der Massai bewohnt wird.",
      hu: "Kajiado Kajiado megye közigazgatási központja, melyet leginkább a maszájok laknak.",
      ro: "Kajiado este inima administrativă a comitatului Kajiado, populată în principal de poporul Maasai.",
      en: "Kajiado is the administrative heart of Kajiado County, populated mostly by the Maasai people."
    },
    facts: {
      de: [
        "Hauptsächlich bekannt für Rinderzucht und Weidewirtschaft.",
        "Erlebt ein schnelles Wachstum aufgrund der Nähe zu Nairobi."
      ],
      hu: [
        "Elsősorban szarvasmarha-tenyésztéséről és pásztorkodásról ismert.",
        "Gyors növekedést tapasztal Nairobi közelsége miatt."
      ],
      ro: [
        "Cunoscut în primul rând pentru creșterea vitelor și păstorit.",
        "Cunoaște o creștere rapidă datorită apropierii sale de Nairobi."
      ],
      en: [
        "Primarily known for cattle rearing and pastoralism.",
        "Experiencing rapid growth due to its proximity to Nairobi."
      ]
    }
  },
  {
    id: "wajir-cities-v2",
    type: "city",
    parent: "KE-08",
    coords: [40.0573, 1.7471],
    name: {
      de: "Wajir",
      hu: "Wajir",
      ro: "Wajir",
      en: "Wajir",
image: "/poi-images/wajir-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Wajir ist eine abgelegene und faszinierende Stadt im nordöstlichen Kenia, die in einer extrem trockenen, flachen Wüstenlandschaft liegt. Die Stadt ist bekannt für ihre traditionellen, runden weißen Häuser, die ihr einen einzigartigen architektonischen Charakter verleihen. Da in Wajir Wasser die wertvollste Ressource ist, dreht sich das Überleben der nomadischen Bevölkerung (überwiegend Somali) um Brunnen, von denen einige jahrhundertelang in Betrieb sind. Wajir wurde 1912 unter britischer Kolonialherrschaft formell gegründet und erhielt während des Zweiten Weltkriegs durch den Bau eines Flugplatzes strategische Bedeutung. Eines der bekanntesten Wahrzeichen ist das von den Italienern im Zweiten Weltkrieg errichtete Wajir Fort. Die raue Umgebung macht die Stadt zu einem Symbol für Widerstandsfähigkeit (Geografie K8).",
      hu: "Wajir egy elszigetelt, oázis jellegű város Kenya északkeleti, sivatagi régiójában, lakossága túlnyomórészt szomáli pásztorokból áll. A város egy sűrű kút-hálózatra épült, amely az évszázadok során biztosította a túlélést a zord, száraz klímában. Az 1912-ben emelt Wajir-erőd ma is áll, amely a brit gyarmati jelenlét emléke. A város gazdasága elsősorban a tevetenyésztésre és a határkereskedelemre épül, és a nomád kultúra hű őrzője. (Történelem K6 — sivatagi oázisok és nomád kultúra)",
      ro: "Wajir este un oraș istoric din nord-estul extrem al Kenyei, recunoscut pentru climatul său arid și peisajele deșertice. Așezat pe o câmpie plată și nisipoasă, orașul este faimos pentru puțurile sale antice săpate manual, care atrag mii de păstori nomazi în sezonul uscat. Aceste fântâni reprezintă o sursă vitală de viață pentru turmele de cămile și capre ale comunității predominant somaleze. Wajir a fost înființat oficial ca fort colonial britanic în anii 1920 și și-a păstrat un caracter arhitectural distinctiv, marcat de clădiri cu influențe swahili și islamice, care rezistă căldurii extreme din regiune.",
      en: "Situated in the deep arid heartland of Kenya's North Eastern Province, Wajir is an isolated yet crucial administrative and commercial hub. The settlement is geologically unique, built upon a limestone bed with a high water table that allows for numerous shallow wells, a feature that has made it a vital watering point for Somali pastoralists for centuries. Established formally as a colonial outpost in 1912, Wajir features distinctive architecture, including the Wajir Fort built in the 1920s and numerous whitewashed, dome-shaped structures adapted to the severe desert heat. The city's economy is almost entirely dependent on the livestock trade, particularly camels and goats. In 2016, the expansion of the Wajir Airport to handle commercial flights significantly reduced the town's geographic isolation. Earth Sciences K7 — Groundwater oases in deserts."
    },
    factsAdvanced: {
      en: ["Formally established as a colonial administrative post in 1912.", "Built on a limestone plain featuring numerous shallow groundwater wells.", "The historic Wajir Fort was constructed by the British in the 1920s.", "Serves as the administrative headquarters for Wajir County.", "Features a hot desert climate with extremely low annual rainfall.", "Wajir Airport was upgraded to accommodate commercial civilian flights in 2016.", "The town's population was recorded at approximately 90,000 in the 2019 census."]
    },
    
    description: {
      de: "Wajir ist eine trockene Siedlung in der ehemaligen Nordostprovinz, die von rauem Wetter und einer starken somalischen Kultur geprägt ist.",
      hu: "Wajir egy száraz település az egykori Északkeleti tartományban, melyet a zord időjárás és az erős szomáliai kultúra jellemez.",
      ro: "Wajir este o așezare aridă în fosta Provincie de Nord-Est, caracterizată prin vreme aspră și o puternică cultură somaleză.",
      en: "Wajir is an arid settlement in the former North Eastern Province, characterized by harsh weather and a strong Somali culture."
    },
    facts: {
      de: [
        "Beherbergt historische architektonische Relikte aus dem Zweiten Weltkrieg.",
        "Der Viehhandel ist das wirtschaftliche Rückgrat."
      ],
      hu: [
        "Itt találhatók a második világháborúból származó történelmi építészeti emlékek.",
        "A gazdaság gerincét az állatkereskedelem adja."
      ],
      ro: [
        "Găzduiește relicve arhitecturale istorice din al Doilea Război Mondial.",
        "Comerțul cu animale este coloana vertebrală economică."
      ],
      en: [
        "Home to historical architectural relics from World War II.",
        "Livestock trading is the economic backbone."
      ]
    }, image: "/poi-images/wajir-cities-v2.webp"},
  {
    id: "narok-cities-v2",
    type: "city",
    parent: "KE-33",
    coords: [35.86, -1.0833],
    name: {
      de: "Narok",
      hu: "Narok",
      ro: "Narok",
      en: "Narok",
image: "/poi-images/narok-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Narok ist die Hauptstadt des Narok Countys und das wichtigste städtische Zentrum der Maasai-Volksgruppe im südwestlichen Kenia. Weltweit ist Narok als letzter großer Zwischenstopp und \"Tor\" zur weltberühmten Maasai Mara bekannt. Die Stadt liegt entlang des Großen Afrikanischen Grabenbruchs und wird von der saisonalen Flutung des Enkare Narok Rivers geprägt. Wirtschaftlich lebt Narok enorm vom Wildtiertourismus in der Mara sowie vom großflächigen Weizen- und Gerstenanbau in den fruchtbaren Gebieten rund um Mau Narok. In Narok verschmelzen traditionelles Maasai-Leben mit modernen landwirtschaftlichen und touristischen Dienstleistungen, was die Stadt zu einem der reichsten Countys in Kenia macht (Wirtschaft & Geografie K7).",
      hu: "Narok a világhírű Masai Mara Nemzeti Rezervátum kapuja és a maszáj nép egyik legfőbb közigazgatási és gazdasági bázisa. A város kettős gazdasággal bír: a hatalmas turisztikai bevételek mellett a régió Kenya legnagyobb búzatermő területe. Narok a hagyományos maszáj kultúra és a modern, gépesített mezőgazdaság lenyűgöző találkozási pontja, ahol a piacokon turisták és hagyományos ruhás pásztorok egyaránt megfordulnak. (Természetismeret K6 — szafari turizmus és vadvilág)",
      ro: "Narok servește drept principala poartă de acces către celebra Rezervație Națională Maasai Mara, fiind un oraș prosper și efervescent. Economia orașului beneficiază masiv de pe urma turismului ecologic, primind sute de mii de vizitatori anual care vin să asiste la Marea Migrație a animalelor. În afara turismului, Narok este un colos agricol; câmpiile vaste din jurul orașului sunt renumite pentru cultivarea grâului și a orzului, transformând județul într-unul dintre cei mai importanți producători de cereale din Kenya. Cultura bogată a poporului Maasai se reflectă pretutindeni în piețele și festivalurile locale.",
      en: "Narok is the primary economic and administrative center of Narok County, positioned on the edge of the Great Rift Valley at an altitude of 1,827 meters. The town has evolved from a small trading outpost into a bustling commercial hub, serving as the essential gateway to the world-famous Maasai Mara National Reserve located to the southwest. Surrounded by expansive, fertile highlands, the local economy is a dual engine of massive wheat farming and robust tourism logistics. Culturally, it remains a vital nexus for the Maasai community, balancing rapid agricultural modernization with traditional pastoralist heritage. The town sits along the Enkare Narok river, which occasionally causes severe flash flooding due to heavy rains in the nearby Mau Escarpment. Economics K7 — Tourism gateways and agriculture."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters of Narok County.", "Located at an altitude of approximately 1,827 meters (5,994 feet).", "The primary urban gateway for tourists traveling to the Maasai Mara National Reserve.", "A major national center for large-scale commercial wheat production.", "Situated along the Enkare Narok river, originating from the Mau Forest.", "Maasai Mara University, located in the town, was fully chartered in 2013.", "The town recorded a population of roughly 119,000 in the 2019 national census."]
    },
    
    description: {
      de: "Narok dient als Tor zum weltberühmten Maasai Mara Nationalreservat und ist ein landwirtschaftliches Zentrum.",
      hu: "Narok a világhírű Masai Mara Nemzeti Rezervátum kapuja, és egyben mezőgazdasági központ is.",
      ro: "Narok servește ca poartă către celebra Rezervație Națională Maasai Mara și este un centru agricol.",
      en: "Narok serves as the gateway to the world-famous Maasai Mara National Reserve and is an agricultural center."
    },
    facts: {
      de: [
        "Ein bedeutender Weizenproduzent in Kenia.",
        "Die Stadt liegt an den Verwerfungslinien des Großen Afrikanischen Grabenbruchs."
      ],
      hu: [
        "A búza jelentős termelője Kenyában.",
        "A város a Nagy-hasadékvölgy törésvonalai mentén fekszik."
      ],
      ro: [
        "Producător major de grâu din Kenya.",
        "Orașul se află de-a lungul liniilor de falie ale Marii Văi a Riftului."
      ],
      en: [
        "Major producer of wheat in Kenya.",
        "The town lies along the Great Rift Valley fault lines."
      ]
    }, image: "/poi-images/narok-cities-v2.webp"},
  {
    id: "lodwar-cities-v2",
    type: "city",
    parent: "KE-23",
    coords: [35.5962, 3.1199],
    name: {
      de: "Lodwar",
      hu: "Lodwar",
      ro: "Lodwar",
      en: "Lodwar",
image: "/poi-images/lodwar-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Lodwar, die Hauptstadt des riesigen Turkana Countys im Nordwesten Kenias, ist eine Wüstenstadt mit einer rauen, aber fesselnden Ausstrahlung. Sie liegt in einem der trockensten und heißesten Gebiete des Landes, westlich des Turkana-Sees. Die historische Bedeutung von Lodwar ist groß, denn während der britischen Kolonialzeit wurde hier 1959 Jomo Kenyatta, der erste Präsident Kenias, politisch inhaftiert; sein Haus steht heute noch als nationales Denkmal. In Lodwar dominiert die Kultur des Turkana-Volkes, bekannt für traditionelle Korbflechterei und Viehzucht. In den letzten Jahrzehnten hat die Entdeckung von Ölvorkommen in der Ngamia-1-Bohrung in der Nähe große Erwartungen geweckt und Lodwar in den Fokus der kenianischen Energiepolitik gerückt (Geschichte & Wirtschaft K8).",
      hu: "Lodwar Északnyugat-Kenya legnagyobb városa, a félsivatagos Turkana megye székhelye. A város perzselő hőségéről és pálmafáiról ismert, és a Turkana-tó közelében fekszik, ami némiképp enyhíti a klímát. Történelmileg is jelentős: a gyarmati időkben politikai foglyok börtöne volt, itt tartották fogva Jomo Kenyattát, a független Kenya első elnökét. A lakosság többsége a nomád turkana népből kerül ki, akik a kosárfonásról és a pásztorkodásról ismertek. (Történelem K8 — függetlenségi harcok és politikai foglyok)",
      ro: "Lodwar este cel mai mare oraș din nord-vestul arid al Kenyei și capitala vastului județ Turkana. Aflat la vest de imensul Lac Turkana, orașul se mândrește cu un climat deșertic necruțător, dar cu un farmec aparte. Este considerat centrul cultural al poporului Turkana, cunoscuți pentru abilitățile lor de supraviețuire în condiții extreme și pentru împletirea măiestrită a coșurilor tradiționale. Lodwar a dobândit importanță națională și pentru că aici, în timpul erei coloniale, primul președinte al Kenyei, Jomo Kenyatta, a fost ținut în detenție de autoritățile britanice.",
      en: "Lodwar is the largest town in northwestern Kenya and serves as the administrative capital of the vast Turkana County. Located in a hyper-arid desert environment west of Lake Turkana, the settlement experiences extreme heat year-round and is defined by the ephemeral Turkwel River, which provides a critical green corridor of doum palms through the barren landscape. Originally established in 1933 as a remote colonial outpost, it later became infamous as the site where Jomo Kenyatta and other political leaders were detained in 1959 during the Mau Mau state of emergency. Today, Lodwar is a growing frontier town, its economy historically reliant on nomadic pastoralism and fishing from the lake, but recently energized by oil exploration in the nearby South Lokichar basin. History K8 — Political exile and frontier towns."
    },
    factsAdvanced: {
      en: ["Established as a British colonial outpost in 1933.", "Jomo Kenyatta was detained here by colonial authorities in 1959.", "Serves as the administrative headquarters for Turkana County.", "Located near the ephemeral Turkwel River in a hot desert climate.", "The nearby South Lokichar basin saw significant oil discoveries starting in 2012.", "Lodwar Airport connects the remote region to Nairobi and Eldoret.", "The urban population was recorded at approximately 82,000 in the 2019 census."]
    },
    
    description: {
      de: "Lodwar ist die größte Stadt im Nordwesten Kenias und die Hauptstadt des Turkana County.",
      hu: "Lodwar Kenya északnyugati részének legnagyobb városa, és Turkana megye fővárosa.",
      ro: "Lodwar este cel mai mare oraș din nord-vestul Kenyei și capitala comitatului Turkana.",
      en: "Lodwar is the largest town in northwestern Kenya and the capital of Turkana County."
    },
    facts: {
      de: [
        "Verzeichnet das ganze Jahr über extreme Hitze.",
        "Wurde während der Kolonialzeit historisch als Internierungslager genutzt."
      ],
      hu: [
        "Egész évben extrém hőség uralkodik.",
        "A gyarmati korszakban történelmileg fogolytáborként használták."
      ],
      ro: [
        "Experimentează căldură extremă pe tot parcursul anului.",
        "A fost folosit istoric ca tabără de detenție în timpul erei coloniale."
      ],
      en: [
        "Experiences extreme heat throughout the year.",
        "Historically used as a detention camp during the colonial era."
      ]
    }, image: "/poi-images/lodwar-cities-v2.webp"},
  {
    id: "busia-cities-v2",
    type: "city",
    parent: "KE-40",
    coords: [34.1084, 0.4608],
    name: {
      de: "Busia",
      hu: "Busia",
      ro: "Busia",
      en: "Busia",
image: "/poi-images/busia-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Busia ist eine vitale Grenzstadt im Westen Kenias, die direkt an die gleichnamige Stadt in Uganda grenzt. Sie ist die Hauptstadt des Busia Countys und der zweitwichtigste Grenzübergang zwischen Kenia und Uganda (nach Malaba). Dieser Status als Grenzstadt macht Busia zu einem ununterbrochen summenden Zentrum für internationalen Handel, Logistik und Warentransport im Rahmen der East African Community (EAC). Die Region wird hauptsächlich von den Luhya- und Teso-Gemeinschaften bewohnt. Landwirtschaft, insbesondere Fischerei im nahen Viktoriasee, sowie der Anbau von Maniok, Mais und Baumwolle bilden das Rückgrat der ländlichen Wirtschaft rund um die Stadt. Busia bietet authentische Einblicke in das dynamische Leben einer afrikanischen Grenzregion (Geografie K7).",
      hu: "Busia egy pezsgő ikerváros, amely pontosan a kenyai-ugandai határon fekszik, és Kelet-Afrika egyik legforgalmasabb tranzitpontja. A teherautók és a kereskedők mindennapos látványa jellemzi a határátkelőt, ahol mind legális, mind informális kereskedelem zajlik az országok között. A helyiek többsége luhja és iteso származású. A kereskedelem mellett a város határában napraforgót és dohányt is termesztenek. (Földrajz K6 — nemzetközi kereskedelmi korridorok)",
      ro: "Busia este un oraș comercial extrem de aglomerat, funcționând ca principalul punct de trecere a frontierei terestre dintre Kenya și Uganda. Datorită poziției sale strategice pe coridorul de tranzit est-african, orașul trăiește aproape exclusiv din comerțul transfrontalier, logistică și transportul mărfurilor grele. Miile de camioane care traversează zilnic vama aduc o activitate frenetică, susținând hotelurile și piețele locale. Pe lângă importanța sa comercială, regiunea înconjurătoare se ocupă cu agricultura de subzistență, cultivând porumb, manioc și sorg.",
      en: "Busia is a pivotal border town situated in western Kenya, acting as the primary gateway into neighboring Uganda. Positioned at an elevation of 1,220 meters, it is the administrative and commercial headquarters of Busia County. The local economy is intensely dominated by cross-border trade, logistics, and transportation, handling a massive percentage of Kenya's overland exports to the East African interior. Beyond transit commerce, the surrounding hinterland is agriculturally rich, supporting significant cultivation of sugarcane, cassava, and millet. The town is predominantly inhabited by the Luhya and Iteso communities, contributing to a vibrant, binational cultural atmosphere. In 2018, the opening of a modern One-Stop Border Post drastically reduced cargo clearance times, further solidifying Busia's status as a critical node in regional economic integration. Economics K7 — Border crossings and logistics."
    },
    factsAdvanced: {
      en: ["Functions as a major border crossing point between Kenya and Uganda.", "Serves as the administrative capital of Busia County.", "Located at an elevation of approximately 1,220 meters (4,003 feet).", "A modern One-Stop Border Post was officially opened in 2018 to facilitate trade.", "The surrounding agricultural region heavily cultivates cassava and sugarcane.", "Handles a significant portion of overland freight traffic into landlocked East Africa.", "The urban population was recorded at roughly 111,000 in the 2019 national census."]
    },
    
    description: {
      de: "Busia ist eine geschäftige Stadt direkt an der Grenze zu Uganda.",
      hu: "Busia egy nyüzsgő város közvetlenül az ugandai határ mellett.",
      ro: "Busia este un oraș plin de viață situat direct la granița cu Uganda.",
      en: "Busia is a bustling town situated directly on the border with Uganda."
    },
    facts: {
      de: [
        "Dient als riesiger Kontrollpunkt für transafrikanische Lkw-Routen.",
        "Handel und Gewerbe florieren aufgrund seiner Lage."
      ],
      hu: [
        "Hatalmas ellenőrzőpont a transzafrikai teherautó-útvonalakon.",
        "A kereskedelem fellendült a kedvező elhelyezkedésnek köszönhetően."
      ],
      ro: [
        "Servește ca un punct de control masiv pentru rutele de camioane trans-africane.",
        "Comerțul prosperă datorită locației sale."
      ],
      en: [
        "Serves as a massive checkpoint for trans-African truck routes.",
        "Trade and commerce thrive due to its location."
      ]
    }, image: "/poi-images/busia-cities-v2.webp"},
  {
    id: "voi-cities-v2",
    type: "city",
    parent: "KE-06",
    coords: [38.563, -3.3957],
    name: {
      de: "Voi",
      hu: "Voi",
      ro: "Voi",
      en: "Voi",
image: "/poi-images/voi-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Voi ist die größte Stadt im Taita-Taveta County und ein entscheidender Verkehrsknotenpunkt im südöstlichen Kenia. Gegründet am Ufer des Voi-Flusses, entwickelte sich die Stadt rasant durch den Bau der historischen Uganda-Eisenbahn, die Mombasa mit dem Inland verband. Voi ist heute der wichtigste Knotenpunkt an der Fernstraße Nairobi-Mombasa und berühmt als das Tor zum riesigen Tsavo-East-Nationalpark, einem der größten Wildschutzgebiete Afrikas. Neben dem Wildtiertourismus spielt der Bergbau eine große Rolle; die umliegenden Taita-Hügel sind bekannt für Vorkommen seltener Edelsteine wie dem Tsavorit. Die strategische Lage macht Voi zudem zu einem Umschlagsplatz für landwirtschaftliche Produkte wie Sisal, die in der trockenen Region gedeihen (Wirtschaft & Geografie K8).",
      hu: "Voi egy stratégiai fontosságú közlekedési csomópont a Tsavo East Nemzeti Park szélén. Itt ágazik el a Nairobi-Mombasa főútvonal Tanzánia irányába. A várost a 19. század végén az Ugandai Vasút építése során alapították munkástáborként. Fekvése miatt a turizmusból is jelentősen profitál, mivel számtalan szafari-expedíció indul innen a környező hatalmas vadvédelmi területekre. Klímája száraz, a szavannai növényzet dominál. (Történelem K6 — vasútépítés és infrastruktúra)",
      ro: "Voi este un oraș pitoresc din sudul Kenyei, situat strategic pe autostrada principală care leagă capitala Nairobi de portul Mombasa. Este poarta ideală pentru vizitarea faimosului Parc Național Tsavo East, unul dintre cele mai mari și mai vechi parcuri naturale din țară, renumit pentru elefanții săi acoperiți de praf roșiatic. Pe lângă turism, Voi are o economie tradițională susținută de întinsele plantații de sisal și de cultivarea de fructe pe pantele fertile ale Dealurilor Taita. Orașul atrage vizitatori pasionați de fotografie sălbatică și exploratori.",
      en: "Voi is a crucial transit and commercial hub situated in Taita-Taveta County along the edge of the Tsavo East National Park. Established originally as a prominent trading post for the local Taita people, the town expanded significantly in the late 19th century as a key station on the Uganda Railway connecting Mombasa to the interior. Located at a relatively low altitude of 580 meters, Voi experiences a hot, semi-arid climate. Today, its economy is heavily anchored in tourism logistics, serving as the primary access point for safaris heading into the vast Tsavo conservation areas. Furthermore, Voi has benefited immensely from the modern Standard Gauge Railway (SGR), featuring a major passenger terminus that has revitalized local trade and connectivity to the coast. Economics K7 — Transport nodes and tourism."
    },
    factsAdvanced: {
      en: ["Evolved significantly as a major station on the Uganda Railway in the late 1890s.", "Serves as the principal gateway to Tsavo East National Park.", "Located at an elevation of approximately 580 meters (1,900 feet).", "Features a major station on the modern Standard Gauge Railway (SGR).", "The largest urban center within Taita-Taveta County by population.", "Historically a vital trading post between coastal merchants and the interior Taita people.", "The town's population reached approximately 53,000 in the 2019 census."]
    },
    
    description: {
      de: "Voi ist die größte Stadt im Taita-Taveta County, am Rande des Tsavo-East-Nationalparks gelegen.",
      hu: "Voi a Taita-Taveta megye legnagyobb városa, amely a Tsavo East Nemzeti Park szélén helyezkedik el.",
      ro: "Voi este cel mai mare oraș din comitatul Taita-Taveta, situat la marginea Parcului Național Tsavo East.",
      en: "Voi is the largest town in Taita-Taveta County, located on the edge of the Tsavo East National Park."
    },
    facts: {
      de: [
        "Ein wichtiger Verkehrsknotenpunkt an der Autobahn Nairobi-Mombasa.",
        "Bekannt als landwirtschaftliches Marktzentrum für Sisal und andere Nutzpflanzen."
      ],
      hu: [
        "Fontos csomópont a Nairobi-Mombasa autópályán.",
        "A szizál és más növények mezőgazdasági piacközpontjaként ismert."
      ],
      ro: [
        "O intersecție crucială pe autostrada Nairobi-Mombasa.",
        "Cunoscut ca un centru de piață agricolă pentru sisal și alte culturi."
      ],
      en: [
        "A crucial junction on the Nairobi-Mombasa highway.",
        "Known as an agricultural market center for sisal and other crops."
      ]
    }, image: "/poi-images/voi-cities-v2.webp"},
  {
    id: "ruiru-cities-v2",
    type: "city",
    parent: "KE-22",
    coords: [36.96, -1.1467],
    name: {
      de: "Ruiru",
      hu: "Ruiru",
      ro: "Ruiru",
      en: "Ruiru",
image: "/poi-images/ruiru-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Ruiru, im stark bevölkerten Kiambu County gelegen, ist eine der am schnellsten wachsenden Städte Kenias und liegt nur etwa 30 Kilometer nördlich von Nairobi. Ursprünglich war Ruiru von weiten Kaffeeplantagen geprägt, die in der Kolonialzeit angelegt wurden. Heute verwandeln sich viele dieser Flächen in große Wohn- und Industriegebiete, insbesondere seit dem Ausbau des Thika Superhighway. Ein herausragendes Projekt ist der Bau der Tatu City, einer riesigen, planmäßig angelegten Satellitenstadt in Ruiru, die Wohnraum für über 150.000 Menschen und hochmoderne Industriezonen bieten soll. Die Stadt profitiert enorm von der Infrastruktur Nairobis und zieht Unternehmen und Pendler gleichermaßen an, was sie zu einem Symbol für Kenias rasante Urbanisierung macht (Wirtschaft & Geografie K7).",
      hu: "Ruiru a Középső Tartományban, Nairobitól mindössze 30 kilométerre fekvő ipari és ingázóváros. A várost eredetileg a környező hatalmas kávéültetvények szolgálták ki, és történelme során is fontos mezőgazdasági bázis maradt. Ma Kenya egyik leggyorsabban növekvő települése, ami főként a Nairobi körüli szuburbanizációnak és az új ipari parkok, köztük a Tatu City közelségének köszönhető. Számos főiskola és magánegyetem campusa is itt található. (Földrajz K6 — agglomeráció és iparosodás)",
      ro: "Ruiru este un oraș rezidențial și industrial în plină expansiune, situat la nord de Nairobi, în județul Kiambu. Inițial un centru liniștit înconjurat de vaste plantații de cafea deținute de coloniști europeni, orașul a cunoscut o transformare masivă, devenind un «oraș-dormitor» preferat pentru profesioniștii care lucrează în capitală. Creșterea populației a adus o explozie a dezvoltărilor imobiliare și industriale, de la prelucrarea cafelei la fabrici textile și oțelării. Cu toate acestea, Ruiru își păstrează o parte din farmecul verde datorită fermelor rămase la periferie.",
      en: "Ruiru is a rapidly expanding industrial and residential town located within Kiambu County, functioning effectively as a major satellite city to Nairobi. Historically dominated by vast sisal and coffee plantations established by colonial settlers, the town has transitioned dramatically into an industrial powerhouse. Positioned at an elevation of 1,565 meters, Ruiru now hosts numerous major manufacturing facilities, notably the massive Brookside Dairy processing plant and the Devki Ruiru steel mills. The completion of the Thika Superhighway and the Eastern Bypass has triggered explosive real estate development, transforming former agricultural lands into modern housing estates. In addition to industry, the town has become a prominent educational center, hosting the main campus of Kenyatta University, one of Kenya's largest public institutions. Geography K7 — Urban sprawl and industrialization."
    },
    factsAdvanced: {
      en: ["Historically known for extensive sisal and coffee farming prior to industrialization.", "Located along the Thika Superhighway just 25 kilometers from Nairobi.", "Home to the massive Brookside Dairy processing facility.", "Hosts the main campus of Kenyatta University.", "Positioned at an elevation of roughly 1,565 meters (5,135 feet).", "The Eastern Bypass highway intersects here, connecting to Jomo Kenyatta International Airport.", "Registered a massive population of over 371,000 in the 2019 census."],
      de: ["Historisch bekannt für ausgedehnten Sisal- und Kaffeeanbau.", "Liegt an der Thika Superhighway, nur 25 Kilometer von Nairobi entfernt.", "Heimat der riesigen Milchverarbeitungsanlage von Brookside Dairy.", "Beherbergt den Hauptcampus der Kenyatta University.", "Liegt auf einer Höhe von rund 1.565 Metern.", "Verzeichnete bei der Volkszählung 2019 eine massive Bevölkerung von über 371.000."]
    },
    
    description: {
      de: "Ruiru ist eine schnell wachsende Schlafstadt im Kiambu County, ganz in der Nähe von Nairobi.",
      hu: "Ruiru egy gyorsan bővülő alvóváros a Kiambu megyében, nagyon közel Nairobihoz.",
      ro: "Ruiru este un oraș dormitor în expansiune rapidă, situat în comitatul Kiambu, foarte aproape de Nairobi.",
      en: "Ruiru is a rapidly expanding dormitory town situated in Kiambu County, very close to Nairobi."
    },
    facts: {
      de: [
        "Umgeben von großen Kaffeeplantagen.",
        "Verfügt über einen wachsenden Immobilien- und Industriesektor."
      ],
      hu: [
        "Hatalmas kávéültetvények veszik körül.",
        "Növekvő ingatlan- és ipari szektorral rendelkezik."
      ],
      ro: [
        "Înconjurat de mari plantații de cafea.",
        "Dispune de un sector imobiliar și industrial în creștere."
      ],
      en: [
        "Surrounded by large coffee estates.",
        "Features a growing real estate and industrial sector."
      ]
    }, image: "/poi-images/ruiru-cities-v2.webp"},
  {
    id: "kilifi-cities-v2",
    type: "city",
    parent: "KE-03",
    coords: [39.85, -3.6333],
    name: {
      de: "Kilifi",
      hu: "Kilifi",
      ro: "Kilifi",
      en: "Kilifi",
image: "/poi-images/kilifi-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kilifi ist eine malerische Küstenstadt und die Hauptstadt des Kilifi Countys, berühmt für den spektakulären Kilifi Creek, einen tiefen Meeresarm, der die Stadt teilt. Bis zum Bau der imposanten Kilifi-Brücke im Jahr 1991, die zu den längsten in Ostafrika zählt, war die Überquerung nur per Fähre möglich. Die Stadt ist stark vom Swahili-Einfluss und der Mijikenda-Kultur, insbesondere der Giriama, geprägt. Wirtschaftlich stützt sich Kilifi auf Tourismus, Cashewnuss-Anbau und Fischerei. Zudem befindet sich hier die Pwani University, eine wichtige Forschungseinrichtung, sowie das Kenya Medical Research Institute (KEMRI). Nahe der Stadt liegen die mystischen Mnarani-Ruinen aus dem 14. Jahrhundert und der üppige Arabuko-Sokoke-Wald, der für seine seltene Tierwelt bekannt ist (Geografie & Biologie K7).",
      hu: "Kilifi egy festői fekvésű történelmi tengerparti város, a Kilifi-patak torkolatánál, Mombasától északra. Az egykori csendes halászfalu a Mnarani romok közelsége miatt fontos régészeti jelentőséggel is bír, ahol a korai szuahéli civilizáció nyomai láthatók. A város gazdasága ma a turizmuson, a halászaton, és a kesudió-termesztésen alapul. Az itt áthaladó hatalmas Kilifi-híd köti össze a déli és az északi partvidéket, ami nagyban fellendítette a helyi kereskedelmet. (Történelem K6 — parti kereskedelem és hidak)",
      ro: "Kilifi este un idilic oraș de coastă situat la estuarul pitorescului râu Goshi, pe malul Oceanului Indian. Faimos pentru plajele sale albe și liniștite, precum Bofa Beach, Kilifi este o destinație turistică preferată pentru sporturi nautice, sailing și pescuit. Orașul este de asemenea un nod istoric, cu ruinele vechiului oraș swahili Mnarani datând din secolul al XIV-lea. Astăzi, Kilifi pulsează de tinerețe, găzduind Universitatea Pwani și un institut major de cercetare medicală, fiind dominat de prezența iconicilor copaci de baobab care străjuiesc estuarul.",
      en: "Kilifi is a picturesque coastal resort town and the administrative capital of Kilifi County, located midway between Mombasa and Malindi. Situated on the estuary of the Goshi River where it empties into the Indian Ocean, the town is historically significant, featuring the nearby Mnarani Ruins, which date back to the 14th century and highlight early Swahili coastal architecture. Today, Kilifi is renowned for its pristine Bofa Beach and deep-water creek, which serves as a premier destination for yachting and water sports. The town's modern infrastructure is defined by the Kilifi Bridge, completed in 1991, which forms a vital link along the coastal highway. The local economy uniquely blends tourism, sisal farming, and higher education, notably hosting Pwani University. Geography K8 — Coastal estuaries and tourism."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters for Kilifi County.", "Located on the Goshi River estuary, creating a deep natural creek.", "The nearby Mnarani Ruins showcase 14th-century Swahili civilization.", "The Kilifi Bridge, spanning 420 meters, was completed in 1991.", "Home to Pwani University, originally an agricultural research institute.", "The region is a major national producer of sisal and cashew nuts.", "The urban population was roughly 122,000 according to the 2019 census."]
    },
    
    description: {
      de: "Kilifi ist ein Ferienort an der Küste, berühmt für seine malerische Flussmündung, den Kilifi Creek.",
      hu: "Kilifi egy tengerparti üdülőváros, amely festői torkolatáról, a Kilifi Creekről híres.",
      ro: "Kilifi este o stațiune balneară situată pe coastă, faimoasă pentru estuarul său pitoresc, Pârâul Kilifi.",
      en: "Kilifi is a resort town located on the coast, famous for its picturesque estuary, the Kilifi Creek."
    },
    facts: {
      de: [
        "Sitz der Pwani University.",
        "Beherbergt antike Ruinen in Mnarani."
      ],
      hu: [
        "A Pwani Egyetem otthona.",
        "Mnarani ősi romjai itt találhatóak."
      ],
      ro: [
        "Găzduiește Universitatea Pwani.",
        "Conține ruine antice la Mnarani."
      ],
      en: [
        "Home to the Pwani University.",
        "Contains ancient ruins at Mnarani."
      ]
    }, image: "/poi-images/kilifi-cities-v2.webp"},
  {
    id: "mumias-cities-v2",
    type: "city",
    parent: "KE-37",
    coords: [34.4855, 0.3392],
    name: {
      de: "Mumias",
      hu: "Mumias",
      ro: "Mumias",
      en: "Mumias",
image: "/poi-images/mumias-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Mumias, im Kakamega County in Westkenia gelegen, ist historisch und wirtschaftlich von enormer Bedeutung. Benannt nach Nabongo Mumia, dem bedeutendsten König (Nabongo) des vorkolonialen Wanga-Königreichs, war die Stadt einst das Zentrum des mächtigsten zentralisierten Reiches der Luhya-Nation. Im späten 19. Jahrhundert schloss König Mumia Verträge mit britischen Entdeckern, was die Stadt zu einem frühen Verwaltungszentrum machte. Wirtschaftlich dominierte Mumias jahrzehntelang Kenias Zuckerindustrie, angetrieben von der Mumias Sugar Company, einst der größte Zuckerproduzent des Landes. Heute zeugen Kulturstätten wie das Nabongo Cultural Centre, in dem die Gräber vergangener Wanga-Könige ruhen, von der stolzen vorkolonialen Geschichte der Stadt (Geschichte K8).",
      hu: "Mumias neve évtizedeken át egyet jelentett a kenyai cukoriparral. A Nyugat-Kenyában fekvő város egykor az ősi Wanga Királyság központja volt, ahol Mumia király uralkodott a brit érkezés idején. Az 1970-es években itt alapították a Mumias Sugar Company-t, amely Afrika egyik legnagyobb cukorgyára lett, és fellendítette a helyi luhja lakosság gazdaságát. Jelenleg is a cukornádültetvények dominálják a környező, rendkívül zöld és csapadékos vidéket. (Történelem K7 — hagyományos királyságok és ipar)",
      ro: "Mumias este un oraș tradițional situat în vestul Kenyei, recunoscut de decenii drept inima industriei de zahăr din națiune. Fondat ca reședință a Regelui Nabongo Mumia al comunității Wanga, orașul a prosperat pe seama agriculturii și comerțului timpuriu de pe ruta caravanelor arabe. Moștenirea islamică lăsată de acei comercianți este încă evidentă astăzi, Mumias având o populație musulmană semnificativă și moschei importante. Centrul economic al orașului a fost dominat mult timp de Mumias Sugar Company, una dintre cele mai mari fabrici de rafinare din Africa de Est.",
      en: "Mumias is a prominent town in western Kenya, historically serving as the capital of the Wanga Kingdom under the influential Paramount Chief Nabongo Mumia during the late 19th century. Located at an altitude of 1,314 meters, the town is geographically and economically defined by its highly fertile soils and consistent rainfall, which make it ideal for extensive agriculture. For decades, Mumias was synonymous with the Mumias Sugar Company, established in 1971, which was once the largest sugar producer in East Africa and the absolute backbone of the local economy. While the sugar industry has faced severe economic challenges recently, the town remains a vital commercial center for Kakamega County. The historical legacy of the Wanga Kingdom is preserved locally, maintaining a unique cultural identity within the broader Luhya community. History K8 — Pre-colonial kingdoms and agro-industry."
    },
    factsAdvanced: {
      en: ["Historically the capital of the Wanga Kingdom under Chief Nabongo Mumia.", "Located in Kakamega County at an elevation of 1,314 meters.", "The Mumias Sugar Company was established here in 1971.", "At its peak, the local sugar factory was the largest in East Africa.", "The first British colonial administrative post in western Kenya was set up here in 1894.", "The town features a prominent mosque built in the early 1900s, reflecting early Islamic influence.", "The population was recorded at approximately 116,000 in the 2019 census."]
    },
    
    description: {
      de: "Mumias ist eine Stadt im Kakamega County, historisch mit dem Wanga-Königreich verbunden.",
      hu: "Mumias egy város Kakamega megyében, történelmileg a Wanga Királysághoz kötődik.",
      ro: "Mumias este un oraș în comitatul Kakamega, legat istoric de Regatul Wanga.",
      en: "Mumias is a town in Kakamega County, historically tied to the Wanga Kingdom."
    },
    facts: {
      de: [
        "Einst das Zentrum der kenianischen Zuckerindustrie.",
        "Benannt nach Nabongo Mumia, einem mächtigen lokalen König."
      ],
      hu: [
        "Egykor a kenyai cukoripar központja volt.",
        "Nabongo Mumia, egy hatalmas helyi király után kapta a nevét."
      ],
      ro: [
        "A fost odată centrul industriei zahărului din Kenya.",
        "Numit după Nabongo Mumia, un rege local puternic."
      ],
      en: [
        "Once the center of Kenya's sugar industry.",
        "Named after Nabongo Mumia, a powerful local king."
      ]
    }, image: "/poi-images/mumias-cities-v2.webp"},
  {
    id: "kapenguria-cities-v2",
    type: "city",
    parent: "KE-24",
    coords: [35.1166, 1.2333],
    name: {
      de: "Kapenguria",
      hu: "Kapenguria",
      ro: "Kapenguria",
      en: "Kapenguria",
image: "/poi-images/kapenguria-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kapenguria ist die Hauptstadt des West Pokot Countys und liegt in den zerklüfteten Hügeln des Rift Valleys in Westkenia. Die Stadt nimmt einen festen Platz in Kenias Geschichtsbüchern ein: Im Jahr 1952 fand hier der berüchtigte Prozess gegen die \"Kapenguria Six\" statt, darunter der spätere Präsident Jomo Kenyatta. Sie wurden von den britischen Kolonialherren fälschlicherweise beschuldigt, den Mau-Mau-Aufstand zu leiten. Die Zellen und der Gerichtssaal, in denen sie inhaftiert waren, sind heute als Kapenguria Museum erhalten. Die Bevölkerung der Region besteht größtenteils aus dem Volk der Pokot, die traditionell Viehzucht betreiben. Nahe der Stadt befindet sich der Turkwel-Staudamm (Turkwel Gorge Dam), Kenias höchster Staudamm, der entscheidend für die Stromerzeugung ist (Geschichte K8).",
      hu: "Kapenguria egy hegyvidéki kisváros a Cherangani-dombság közelében, amely a kenyai történelem egyik legfontosabb helyszíne. Az 1952-es híres 'Kapenguriai hatok' pere során itt ítélték el Jomo Kenyattát és öt másik függetlenségi vezetőt a brit gyarmati hatóságok. Ma a város múzeuma – amely az egykori börtönépületben kapott helyet – ennek a sorsdöntő eseménynek és a helyi pokot kultúrának állít emléket. A vidék gazdaságát az állattenyésztés uralja. (Történelem K8 — dekolonizáció és perek)",
      ro: "Kapenguria este un oraș situat într-o regiune montană cu peisaje uluitoare în nord-vestul Kenyei, servind ca sediu al județului West Pokot. Așezarea a intrat în manualele de istorie datorită celebrului proces «Kapenguria Six» din anul 1952. În acel moment, liderii independenței kenyene, inclusiv Jomo Kenyatta, au fost judecați și închiși de către forțele coloniale britanice pentru presupusa lor implicare în revolta Mau Mau. Astăzi, Muzeul Kapenguria comemorează acest eveniment crucial. Economia orașului se bazează pe agricultura de subzistență și creșterea animalelor de către poporul Pokot.",
      en: "Kapenguria is the administrative capital of West Pokot County, situated in the rugged northern Rift Valley region at the foothills of the Cherangani Hills. The town sits at an elevation of 2,130 meters, experiencing a cool climate that supports the cultivation of maize and dairy farming, sharply contrasting with the arid plains located further north. Kapenguria holds a profound place in Kenyan history; it was here in 1953 that colonial authorities held the infamous trial of the 'Kapenguria Six', including future president Jomo Kenyatta, accusing them of managing the Mau Mau rebellion. This history is meticulously preserved at the Kapenguria Museum, which occupies the former detention cells. Today, it serves as a critical commercial link between the agricultural highlands and the pastoralist communities of the deep north. History K8 — Anti-colonial resistance."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters of West Pokot County.", "The site of the historic 1953 trial of the 'Kapenguria Six', including Jomo Kenyatta.", "The Kapenguria Museum was opened in 1993 in the former prison cells.", "Located at a high altitude of approximately 2,130 meters (6,988 feet).", "Functions as a transition zone between fertile highlands and arid pastoral lands.", "Situated at the western foothills of the Cherangani Hills.", "The urban population reached approximately 71,000 in the 2019 national census."]
    },
    
    description: {
      de: "Kapenguria ist eine Stadt im West Pokot County, bekannt für ihre historische Bedeutung während Kenias Unabhängigkeitskampf.",
      hu: "Kapenguria egy város West Pokot megyében, amely a kenyai függetlenségi harc során betöltött történelmi jelentőségéről ismert.",
      ro: "Kapenguria este un oraș în comitatul West Pokot, cunoscut pentru semnificația sa istorică în timpul luptei pentru independență a Kenyei.",
      en: "Kapenguria is a town in West Pokot County, known for its historical significance during Kenya's independence struggle."
    },
    facts: {
      de: [
        "Die Kapenguria Six wurden hier 1952 inhaftiert und vor Gericht gestellt.",
        "Das Kapenguria Museum bewahrt diese Geschichte."
      ],
      hu: [
        "A Kapenguria Hatokat itt tartották fogva és ítélték el 1952-ben.",
        "A Kapenguria Múzeum őrzi ezt a történelmet."
      ],
      ro: [
        "Cei Șase din Kapenguria au fost reținuți și judecați aici în 1952.",
        "Muzeul Kapenguria păstrează această istorie."
      ],
      en: [
        "The Kapenguria Six were detained and tried here in 1952.",
        "The Kapenguria Museum preserves this history."
      ]
    }
  },
  {
    id: "nanyuki-cities-v2",
    type: "city",
    parent: "KE-31",
    coords: [37.0728, -0.0163],
    name: {
      de: "Nanyuki",
      hu: "Nanyuki",
      ro: "Nanyuki",
      en: "Nanyuki",
image: "/poi-images/nanyuki-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Nanyuki, in Laikipia County gelegen, liegt exakt auf dem Äquator, was oft durch Hinweisschilder markiert ist und Touristen anzieht. Die 1907 von britischen Siedlern gegründete Stadt dient als wichtigster Ausgangspunkt für Expeditionen und Bergbesteigungen der nordwestlichen Routen des Mount Kenya. Nanyuki hat eine starke militärische Präsenz: Hier befindet sich ein wichtiger Stützpunkt der kenianischen Luftwaffe (KAF) sowie die BATUK (British Army Training Unit Kenya), die die weiten, trockenen Ebenen von Laikipia für Militärübungen nutzt. Wirtschaftlich ist Nanyuki ein Zentrum für den Handel mit Rindfleisch und Wolle. Zudem liegt die Stadt umgeben von weltbekannten privaten Wildschutzgebieten wie Ol Pejeta (Heimat seltener Nashörner) und Lewa, was den Tourismus ankurbelt (Geografie & Politik K7).",
      hu: "Nanyuki az Egyenlítőn fekvő város, amely a Kenya-hegy megmászására induló turisták fő bázisa. A várost eredetileg a 20. század elején alapították brit telepesek farmközpontként, és a brit hadsereg máig fenntart itt egy gyakorlóbázist (BATUK). Nanyuki gazdaságának motorja a szafariturizmus, az Ol Pejeta és a Lewa vadvédelmi rezervátumok közelsége miatt. A tiszta hegyi levegő és a lenyűgöző kilátás miatt sok külföldi is ide költözött. (Földrajz K6 — hegységek és az Egyenlítő)",
      ro: "Nanyuki este un oraș fermecător și cosmopolit așezat chiar pe linia Ecuatorului, la poalele nord-vestice ale maiestuosului Munte Kenya. Fondat în 1907 de fermierii britanici de origine afrikaans, Nanyuki rămâne o bază importantă pentru armata britanică, care folosește regiunea pentru antrenament. Datorită poziției sale excelente, orașul a devenit un hub de prim rang pentru safariuri ecologice și pentru conservarea faunei sălbatice, zeci de rezervații private (cum ar fi Ol Pejeta) fiind situate în apropiere. Turiștii pornesc de aici în expediții epice de alpinism spre vârfurile Batian și Nelion.",
      en: "Nanyuki is a vibrant market town in Laikipia County, uniquely positioned exactly on the equator, a geographical fact marked by iconic roadside signposts popular with tourists. Founded by British settlers in 1907 at an elevation of 1,947 meters, the town was established as a major center for cattle ranching and large-scale farming on the fertile northwestern slopes of Mount Kenya. Today, Nanyuki is famous as the primary base for mountaineering expeditions tackling the Sirimon and Burguret routes up the mountain. Furthermore, the town is a major logistical hub for the numerous private wildlife conservancies spanning the Laikipia Plateau, balancing robust tourism with agricultural output. It also hosts the main base of the British Army Training Unit Kenya (BATUK), adding a distinct international dimension to the local economy. Geography K7 — Equator settlements and tourism."
    },
    factsAdvanced: {
      en: ["The town is famously bisected precisely by the equator.", "Founded by British colonial settlers in 1907 as a farming and ranching hub.", "Located at an altitude of 1,947 meters (6,388 feet) near Mount Kenya.", "Serves as the primary logistics base for British Army Training Unit Kenya (BATUK).", "The central gateway for expeditions climbing Mount Kenya's Sirimon route.", "Surrounded by vast private wildlife conservancies like Ol Pejeta.", "The town recorded a population of roughly 72,000 in the 2019 census."]
    },
    
    description: {
      de: "Nanyuki ist eine Marktstadt im Laikipia County, berühmt dafür, genau auf dem Äquator zu liegen.",
      hu: "Nanyuki egy piacváros a Laikipia megyében, amely arról híres, hogy pontosan az Egyenlítőn fekszik.",
      ro: "Nanyuki este un oraș-târg în comitatul Laikipia, faimos pentru că este situat exact pe Ecuator.",
      en: "Nanyuki is a market town in Laikipia County, famously situated exactly on the Equator."
    },
    facts: {
      de: [
        "Dient als Hauptstützpunkt für Bergsteiger des Mount Kenya.",
        "Beherbergt eine große Trainingseinrichtung der britischen Armee."
      ],
      hu: [
        "Fő bázisként szolgál a Mount Kenya hegymászói számára.",
        "A brit hadsereg jelentős kiképzőközpontjának ad otthont."
      ],
      ro: [
        "Servește ca o bază principală pentru alpiniștii de pe Muntele Kenya.",
        "Găzduiește o importantă facilitate de antrenament pentru Armata Britanică."
      ],
      en: [
        "Serves as a main base for Mount Kenya climbers.",
        "Hosts a major training facility for the British Army."
      ]
    }, image: "/poi-images/nanyuki-cities-v2.webp"},
  {
    id: "kikuyu-cities-v2",
    type: "city",
    parent: "KE-22",
    coords: [36.6575, -1.247],
    name: {
      de: "Kikuyu",
      hu: "Kikuyu",
      ro: "Kikuyu",
      en: "Kikuyu",
image: "/poi-images/kikuyu-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kikuyu Town, benannt nach Kenias größter ethnischer Gruppe, liegt im Kiambu County, nur wenige Kilometer von Nairobi entfernt. Die Stadt hat eine tiefe historische Bedeutung, da hier in den späten 1890er Jahren eine der ersten Missionarsstationen Ostafrikas durch die Church of Scotland Mission (CSM) gegründet wurde. Kikuyu besitzt einige der ältesten Bildungseinrichtungen Kenias, darunter die berühmte Alliance High School und das PCEA Kikuyu Hospital, die beide maßgeblich zur Entwicklung des Landes beitrugen. Geografisch zeichnet sich Kikuyu durch kühles Klima, fruchtbare Böden und hügeliges Terrain aus, was den intensiven Anbau von Gemüse und Kaffee ermöglicht. Durch den Ausbau der südlichen Umgehungsstraße ist die Stadt stark in das Wirtschaftssystem Nairobis integriert (Geschichte K7).",
      hu: "Kikuyu városa Nairobitól északnyugatra fekszik, és a legelső misszionárius állomások egyike volt a térségben, amit még 1898-ban alapítottak. Nevét Kenya legnagyobb etnikai csoportjáról, a kikujukról kapta. Történelmi jelentősége a korai brit missziókból, kórházakból és iskolákból ered, amelyek ma is meghatározzák a települést. A fővároshoz való közelsége miatt mára nagyrészt Nairobi elővárosává vált, de mezőgazdasági múltja a környező zöldségeskertekben még tetten érhető. (Történelem K6 — misszionáriusok és városiasodás)",
      ro: "Kikuyu este unul dintre cele mai vechi așezăminte din regiunea centrală, localizat la vest de Nairobi și înconjurat de dealurile înverzite ale județului Kiambu. Numele său provine de la cel mai mare grup etnic din Kenya. Orașul deține o profundă semnificație istorică și religioasă, fiind locul în care misionarii din Biserica Scoției și-au stabilit cartierul general în anul 1898. Astăzi, Kikuyu este un amestec de centre educaționale prestigioase, clădiri coloniale istorice și dezvoltări rezidențiale moderne, atrăgând familii care doresc o alternativă liniștită la agitația capitalei Nairobi.",
      en: "Located just 20 kilometers northwest of Nairobi, Kikuyu is one of Kenya's oldest modern towns, deeply intertwined with the early history of the Uganda Railway and Christian missionary work. Established around a railway station in the late 1890s, the town quickly became a prominent center for the Church of Scotland, which founded the iconic PCEA Church of the Torch in 1909 and the highly influential Alliance High School in 1926. Situated at an elevation of 2,000 meters, Kikuyu features a cool climate that historically supported extensive wattle and coffee plantations. Today, fueled by the expansion of the Nairobi Southern Bypass, it has largely transformed into a rapidly growing commuter suburb, blending historical missionary architecture with modern real estate developments. History K8 — Missionary education and railway towns."
    },
    factsAdvanced: {
      en: ["Evolved around an early station built on the Uganda Railway in the 1890s.", "The PCEA Church of the Torch was completed here by missionaries in 1909.", "Alliance High School, a premier national institution, was founded here in 1926.", "Located at a high elevation of approximately 2,000 meters (6,561 feet).", "Home to the Presbyterian University of East Africa.", "Has rapidly developed into a major residential suburb of the Nairobi metropolis.", "The urban center had a population of roughly 323,000 in the 2019 census."]
    },
    
    description: {
      de: "Kikuyu ist eine Stadt im Kiambu County, die als eine der ältesten Missionssiedlungen in Kenia gilt.",
      hu: "Kikuyu egy város a Kiambu megyében, amelyet széles körben Kenya egyik legrégebbi misszionárius településének tartanak.",
      ro: "Kikuyu este un oraș în comitatul Kiambu, considerat pe scară largă una dintre cele mai vechi așezări misionare din Kenya.",
      en: "Kikuyu is a town in Kiambu County, widely considered one of the oldest missionary settlements in Kenya."
    },
    facts: {
      de: [
        "Beherbergt einige der ältesten Kolonialgebäude.",
        "Ist aufgrund der Ausdehnung Nairobis stark gewachsen."
      ],
      hu: [
        "A legrégebbi gyarmati épületek egy részének ad otthont.",
        "Nairobi határának kitolódása miatt jelentősen megnőtt."
      ],
      ro: [
        "Găzduiește unele dintre cele mai vechi clădiri coloniale.",
        "A crescut semnificativ datorită granițelor în expansiune ale Nairobiului."
      ],
      en: [
        "Hosts some of the oldest colonial buildings.",
        "Grown significantly due to Nairobi's expanding boundaries."
      ]
    }, image: "/poi-images/kikuyu-cities-v2.webp"},
  {
    id: "nyahururu-cities-v2",
    type: "city",
    parent: "KE-31",
    coords: [36.368, -0.0405],
    name: {
      de: "Nyahururu",
      hu: "Nyahururu",
      ro: "Nyahururu",
      en: "Nyahururu",
image: "/poi-images/nyahururu-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Nyahururu, in der Kolonialzeit als Thomson's Falls bekannt, ist die höchstgelegene Großstadt Kenias (etwa 2.360 Meter über dem Meeresspiegel). Sie liegt auf dem Laikipia-Plateau an der Grenze zum Nyandarua County. Die Hauptattraktion der Stadt sind die spektakulären Thomson-Wasserfälle (Thomson's Falls) des Ewaso-Narok-Flusses, die über 70 Meter in die Tiefe stürzen. Die Stadt wurde 1883 vom britischen Entdecker Joseph Thomson entdeckt und diente lange als Knotenpunkt auf der alten Eisenbahnstrecke von Gilgil. Das kalte Klima macht Nyahururu zu einer florierenden Agrarregion für den Anbau von Kartoffeln, Kohl und Schnittblumen. Die extreme Höhe wird zudem von vielen der besten Leichtathleten Kenias für das Höhentraining genutzt (Geografie K7).",
      hu: "Nyahururu, korábbi nevén Thomson's Falls, egy magasan fekvő város az Aberdare-hegység északi peremén. Legfőbb nevezetessége a városon áthaladó Ewaso Narok folyón található 74 méteres vízesés. A 2300 méteres tengerszint feletti magasság Kenya egyik leghűvösebb klímájú városává teszi, ahol a hőmérséklet éjszaka gyakran 10°C alá süllyed. A hűvös éghajlat miatt a város a maratoni futók népszerű edzőközpontja, de kiterjedt virág- és zöldségültetvényekkel is rendelkezik. (Földrajz K7 — magaslati éghajlat és vízesések)",
      ro: "Nyahururu, situat în județul Laikipia, este cel mai înalt oraș din Kenya, oferind aer proaspăt și peisaje spectaculoase. Fondat inițial sub numele de Thomson's Falls, localitatea este cel mai bine cunoscută pentru superba cascadă de 74 de metri de pe râul Ewaso Ng'iro, numită după exploratorul scoțian Joseph Thomson, care a descoperit-o în 1883. Economia orașului prosperă din agricultură intensivă și creșterea ovinelor, dar altitudinea sa extremă a făcut din Nyahururu și un loc de antrenament renumit pentru atleții fondiști care aleargă pe distanțe lungi.",
      en: "Nyahururu, formerly known as Thomson's Falls, is a high-altitude town situated at 2,303 meters on the edge of the Laikipia Plateau and the Aberdare Range. The town was founded in the 1920s around the dramatic 74-meter Thomson's Falls on the Ewaso Narok river, named after the Scottish explorer Joseph Thomson who reached them in 1883. Benefiting from exceptionally cool temperatures and consistent rainfall, Nyahururu is a premier agricultural hub, producing massive quantities of potatoes, maize, and dairy products. Similar to Eldoret and Iten, its extreme high altitude makes it a highly favored training base for elite long-distance athletes from around the globe. The town remains heavily reliant on domestic tourism driven by the waterfalls, alongside its robust agricultural commerce. Geography K6 — High-altitude farming and waterfalls."
    },
    factsAdvanced: {
      en: ["Originally named Thomson's Falls after explorer Joseph Thomson (1883).", "Located at an extreme altitude of 2,303 meters (7,555 feet).", "Features the spectacular 74-meter-high Thomson's Falls on the Ewaso Narok river.", "A major training center for marathon runners due to its thin, high-altitude air.", "The town was a prominent filming location for the 1980 movie 'The Gods Must Be Crazy'.", "A critical regional hub for large-scale potato and dairy farming.", "The population was recorded at approximately 51,000 in the 2019 national census."]
    },
    
    description: {
      de: "Nyahururu ist eine hoch gelegene Stadt in Zentralkenia, die historisch als Thomson's Falls bekannt ist.",
      hu: "Nyahururu egy magaslati város Közép-Kenyában, történelmileg Thomson's Falls néven ismerték.",
      ro: "Nyahururu este un oraș situat la mare altitudine în centrul Kenyei, cunoscut istoric ca Thomson's Falls.",
      en: "Nyahururu is a high-altitude town in central Kenya, historically known as Thomson's Falls."
    },
    facts: {
      de: [
        "Der nahegelegene 74 m hohe Wasserfall ist eine große Touristenattraktion.",
        "Mit 2360 Metern ist sie eine der höchstgelegenen Städte Kenias."
      ],
      hu: [
        "A közeli 74 méteres vízesés jelentős turisztikai látványosság.",
        "2360 méteres magasságával Kenya egyik legmagasabban fekvő városa."
      ],
      ro: [
        "Cascada din apropiere de 74 m este o atracție turistică majoră.",
        "La 2360 de metri, este unul dintre cele mai înalte orașe din Kenya."
      ],
      en: [
        "The nearby 74m waterfall is a major tourist attraction.",
        "At 2360 meters, it is one of Kenya's highest towns."
      ]
    }, image: "/poi-images/nyahururu-cities-v2.webp"},
  {
    id: "siaya-cities-v2",
    type: "city",
    parent: "KE-41",
    coords: [34.2882, 0.0617],
    name: {
      de: "Siaya",
      hu: "Siaya",
      ro: "Siaya",
      en: "Siaya",
image: "/poi-images/siaya-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Siaya, die Hauptstadt des gleichnamigen Siaya Countys, liegt in Westkenia unweit des Viktoriasees. Die Stadt und das umgebende County sind das kulturelle und politische Zentrum der Luo-Bevölkerung. Weltweite Bekanntheit erlangte die Region durch das nahegelegene Dorf Kogelo, das der historische Stammsitz der Familie des ehemaligen US-Präsidenten Barack Obama ist. Wirtschaftlich ist die Region stark von der Subsistenzwirtschaft und der Fischerei im Viktoriasee geprägt, obwohl der Boden in vielen Teilen als weniger fruchtbar gilt als in den benachbarten Countys. Siaya ist auch bekannt für seine dichte Ansammlung von Schulen und Colleges, was die historische Priorisierung von Bildung in der lokalen Luo-Kultur widerspiegelt (Politik & Geografie K8).",
      hu: "Siaya a Viktória-tó északi öbleinek közelében fekszik, a kenyai luo közösség kulturális és politikai központja. A város történelmi jelentőségét adja, hogy számos neves kenyai akadémikus és politikus, köztük Barack Obama apjának szülőföldje, Kogelo falu is ebben a régióban található. A lakosság elsősorban mezőgazdaságból, cirok- és kukoricatermesztésből él. Bár Siaya iparilag kevésbé fejlett, az erős oktatási intézmények és a kulturális örökség miatt jelentős szereplője az országnak. (Történelem K6 — kulturális központok és származás)",
      ro: "Siaya, un oraș liniștit situat la nord de Lacul Victoria, servește ca centru administrativ al județului omonim și este o enclavă culturală profundă a poporului Luo. Orașul a câștigat o vizibilitate uriașă pe scena mondială după ce s-a aflat că satul Kogelo, aflat în apropiere, este locul natal al lui Barack Obama Sr., tatăl celui de-al 44-lea președinte american. Economia se concentrează pe agricultura de subzistență, cultivându-se manioc și porumb. Turismul patrimonial a luat avânt odată cu sosirea curioșilor dornici să exploreze originile familiei Obama și tradițiile unice locale.",
      en: "Siaya is the administrative headquarters of Siaya County, located in the gently rolling landscape of the Lake Victoria basin in western Kenya. Situated at an elevation of 1,240 meters, the town is a major cultural and political center for the Luo community. The local economy is primarily agrarian, focusing on the cultivation of sorghum, maize, and cotton, supplemented heavily by fishing activities from the nearby shores of Lake Victoria. Siaya holds significant historical gravity, closely associated with prominent Kenyan political figures, notably the Odinga family. While traditionally a quieter administrative center, the town has seen recent modernization with the establishment of Jaramogi Oginga Odinga University of Science and Technology in nearby Bondo, which has stimulated educational and commercial growth across the entire county. Social Studies K7 — Regional administrative centers."
    },
    factsAdvanced: {
      en: ["Serves as the administrative capital of Siaya County.", "Located at an elevation of roughly 1,240 meters (4,068 feet) near Lake Victoria.", "Functions as a major cultural and political heartland for the Luo people.", "The surrounding economy is driven by subsistence agriculture and lake fishing.", "Positioned near the historic Alego-Kogelo village, the ancestral home of Barack Obama Sr.", "The region is known for producing robust harvests of sorghum and cotton.", "The urban population was roughly 33,000 according to the 2019 census."]
    },
    
    description: {
      de: "Siaya ist eine bedeutende Stadt in der Region Nyanza, tief verwurzelt in der Kultur des Luo-Volkes.",
      hu: "Siaya jelentős város a Nyanza régióban, amely mélyen gyökerezik a luo nép kultúrájában.",
      ro: "Siaya este un oraș semnificativ în regiunea Nyanza, adânc înrădăcinat în cultura poporului Luo.",
      en: "Siaya is a significant town in Nyanza region, deeply rooted in the culture of the Luo people."
    },
    facts: {
      de: [
        "Bekannt als die angestammte Heimat von Barack Obama Sr.",
        "Stützt sich hauptsächlich auf Subsistenzlandwirtschaft."
      ],
      hu: [
        "Idősebb Barack Obama ősi otthonaként ismert.",
        "Elsősorban az önellátó mezőgazdaságra támaszkodik."
      ],
      ro: [
        "Cunoscut ca reședința ancestrală a lui Barack Obama Sr.",
        "Se bazează în principal pe agricultura de subzistență."
      ],
      en: [
        "Known as the ancestral home of Barack Obama Sr.",
        "Primarily relies on subsistence agriculture."
      ]
    }, image: "/poi-images/siaya-cities-v2.webp"},
  {
    id: "bomet-cities-v2",
    type: "city",
    parent: "KE-36",
    coords: [35.341, -0.7818],
    name: {
      de: "Bomet",
      hu: "Bomet",
      ro: "Bomet",
      en: "Bomet",
image: "/poi-images/bomet-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Bomet ist das pulsierende Zentrum des Bomet Countys in der südlichen Rift-Valley-Region. Die Stadt liegt in den weiten, sanft hügeligen Highlands und ist das kulturelle Herz der Kipsigis, einer großen Untergruppe der Kalenjin-Ethnie. Landwirtschaftlich ist Bomet ein wahrer Riese: Die Region produziert enorme Mengen an Tee und profitiert von fruchtbaren Böden und regelmäßigen Niederschlägen. Große multinationale und lokale Teefabriken säumen die Umgebung. Zudem ist die Milchviehhaltung ein wesentlicher wirtschaftlicher Pfeiler für unzählige Kleinbauern in der Umgebung. Bomet liegt verkehrsgünstig an der Hauptstraße, die in die südliche Maasai Mara führt, was der Stadt einen stetigen Fluss an Handel und lokalen Tourismus einbringt (Wirtschaft K7).",
      hu: "Bomet egy lendületes mezőgazdasági város a Dél-Rift régióban, a Mau-erdő szomszédságában. A kipsigis népcsoport által lakott terület kiváló természeti adottságokkal bír a teatermesztés és a tejelő szarvasmarha-tartás számára. Bomet a környező vidék kereskedelmi agya, ahol az agrártermékeket begyűjtik és továbbítják a nagyobb piacok felé. A várost buja, zöld dombvidék veszi körül, amelyet az egyenletes, egész éven át tartó esőzések táplálnak. (Földrajz K6 — tejtermelés és tea)",
      ro: "Bomet este un oraș situat strategic în regiunea South Rift, în mijlocul peisajelor ondulate și verzi din județul cu același nume. Această așezare dinamică joacă un rol esențial în producția și procesarea ceaiului, plantațiile luxuriante dominând împrejurimile. Bomet este și un centru cultural pentru grupul Kalenjin, popor faimos pentru performanțele lor atletice la nivel global. Climatul favorabil, marcat de ploi regulate, face ca orașul să fie extrem de productiv și în ceea ce privește agricultura de lactate și horticultura comercială, susținând puternic economia locală.",
      en: "Bomet is a bustling agricultural center and the administrative headquarters of Bomet County, situated in the fertile highlands of the South Rift region. Located at an altitude of 1,950 meters, the town benefits from abundant rainfall and deep volcanic soils, making it an absolute powerhouse in Kenya's tea industry. The surrounding landscape is characterized by endless, rolling tea plantations, largely managed by smallholder farmers supplying local processing factories. Beyond tea, dairy farming forms a massive pillar of the local economy. The town has expanded rapidly as a commercial transit point connecting the southern Rift Valley to the Lake Victoria basin. The establishment of Bomet University College has recently introduced a new educational dimension to the town's predominantly agrarian character. Economics K7 — Smallholder cash crop farming."
    },
    factsAdvanced: {
      en: ["Serves as the administrative capital of Bomet County.", "Located at a high altitude of 1,950 meters (6,397 feet) in the South Rift.", "A major national production hub for tea, predominantly via smallholder farms.", "The town's economy is heavily supported by intensive dairy farming cooperatives.", "Bomet University College was established to expand regional higher education.", "Acts as a crucial transport link connecting the Rift Valley to Nyanza Province.", "The town recorded a population of roughly 110,000 in the 2019 census."]
    },
    
    description: {
      de: "Bomet ist ein Handelszentrum im South Rift, das stark in der Milchwirtschaft und im Teeanbau engagiert ist.",
      hu: "Bomet egy kereskedelmi csomópont a Déli-hasadékvölgyben, ahol a tej- és teatermelés a domináns.",
      ro: "Bomet este un hub comercial în Valea Riftului de Sud, puternic implicat în producția de lactate și ceai.",
      en: "Bomet is a commercial hub in the South Rift, heavily involved in dairy and tea farming."
    },
    facts: {
      de: [
        "Dient als wichtiger Verkehrsknotenpunkt in Richtung Maasai Mara.",
        "Hat eine überwiegend kalenjinsprachige Bevölkerung."
      ],
      hu: [
        "Fontos csomópontként szolgál a Masai Mara felé vezető úton.",
        "A lakosság túlnyomórészt kalendzsin."
      ],
      ro: [
        "Servește ca un nod major care duce spre Maasai Mara.",
        "Are o populație predominant Kalenjin."
      ],
      en: [
        "Serves as a major junction leading towards the Maasai Mara.",
        "Has a predominantly Kalenjin population."
      ]
    }, image: "/poi-images/bomet-cities-v2.webp"},
  {
    id: "athi-river-cities-v2",
    type: "city",
    parent: "KE-16",
    coords: [36.9806, -1.4485],
    name: {
      de: "Athi River",
      hu: "Athi River",
      ro: "Athi River",
      en: "Athi River",
image: "/poi-images/athi-river-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Athi River, auch bekannt als Mavoko, ist eine boomende Industriestadt im Machakos County, südöstlich von Nairobi. Die Stadt verdankt ihren Namen dem Athi-Fluss, an dessen Ufern sie wuchs. Athi River ist das Herzstück der kenianischen Zement- und Bauindustrie; hier befinden sich Fabriken gigantischer Konzerne wie Bamburi Cement, Mombasa Cement und East African Portland Cement, da die Region reich an Kalksteinvorkommen ist. Darüber hinaus beherbergt Athi River eine große Export Processing Zone (EPZ), die Zehntausende von Arbeitsplätzen in der Textil- und Fertigungsindustrie bietet. Die Stadt liegt verkehrsgünstig an der SGR-Eisenbahnstrecke und der Mombasa-Autobahn, was sie zum ultimativen Logistikzentrum macht (Wirtschaft K8).",
      hu: "Athi River Nairobitól délkeletre található ipari város, nevét a rajta keresztülfolyó Athi-folyóról kapta. Az ország legnagyobb ipari csomópontjainak egyike; itt koncentrálódik Kenya cementgyártása, köszönhetően a helyi gazdag mészkőlelőhelyeknek. Továbbá egy hatalmas Exportfeldolgozó Övezetnek (EPZ) is otthont ad, ami számos textil- és könnyűipari cég működését biztosítja. A Mombasa felé vezető vasút és autópálya kulcsfontosságú logisztikai bázissá teszi a települést. (Földrajz K7 — nehézipar és exportzónák)",
      ro: "Athi River este inima industrială a Kenyei, situată strategic la sud-est de Nairobi, pe calea ferată și autostrada spre Mombasa. Numele său provine de la râul omonim care o străbate, dar astăzi orașul este recunoscut în primul rând pentru fabricile sale gigantice de ciment, care au alimentat boom-ul construcțiilor din întreaga țară. Mai mult, aici se găsește una dintre cele mai mari Zone de Procesare pentru Export (EPZ), unde mii de muncitori asamblează bunuri, îmbrăcăminte și electronice destinate piețelor internaționale, aducând un aport uriaș economiei naționale.",
      en: "Athi River, also known as Mavoko, is a heavily industrialized town in Machakos County, located just 30 kilometers southeast of Nairobi. Positioned along the Nairobi-Mombasa highway and the railway line, its strategic location spurred massive industrial development beginning in the mid-20th century. The town is most famous as Kenya's premier hub for cement manufacturing, hosting colossal plants for Bamburi, East African Portland, and Mombasa Cement. Additionally, Athi River is home to the expansive Export Processing Zone (EPZ), established in 1990, which houses dozens of textile and manufacturing firms producing goods specifically for international export. This intense industrialization has driven rapid demographic growth, transforming the town into a densely populated, working-class urban center highly integrated into Nairobi's broader metropolitan economy. Economics K8 — Export processing and heavy industry."
    },
    factsAdvanced: {
      en: ["A major industrial center formally known as Mavoko municipality.", "Located along the primary transport corridor between Nairobi and Mombasa.", "The epicenter of Kenya's cement industry, hosting multiple massive factories.", "Home to the Athi River Export Processing Zone (EPZ), established in 1990.", "Features a major station on the modern Standard Gauge Railway (SGR).", "The Kenya Meat Commission established its massive processing facility here in 1953.", "The urban population exploded to over 322,000 according to the 2019 census."]
    },
    
    description: {
      de: "Athi River, offiziell als Mavoko bekannt, ist eine Industriestadt außerhalb von Nairobi.",
      hu: "Athi River, hivatalos nevén Mavoko, egy iparosodott város Nairobi külvárosában.",
      ro: "Athi River, cunoscut oficial sub numele de Mavoko, este un oraș industrializat situat în afara orașului Nairobi.",
      en: "Athi River, officially known as Mavoko, is an industrialized town located outside of Nairobi."
    },
    facts: {
      de: [
        "Berühmt für seine großen Zementproduktionsanlagen.",
        "Wächst schnell als Teil der Metropolregion Nairobi."
      ],
      hu: [
        "Híres nagy cementgyártó üzemeiről.",
        "A Nairobi nagyvárosi régió részeként gyorsan növekszik."
      ],
      ro: [
        "Faimos pentru marile sale fabrici de producție a cimentului.",
        "Crește rapid ca parte a Regiunii Metropolitane Nairobi."
      ],
      en: [
        "Famous for its large cement manufacturing plants.",
        "Rapidly growing as part of the Nairobi Metropolitan Region."
      ]
    }, image: "/poi-images/athi-river-cities-v2.webp"},
  {
    id: "vihiga-cities-v2",
    type: "city",
    parent: "KE-38",
    coords: [34.7214, 0.0817],
    name: {
      de: "Vihiga",
      hu: "Vihiga",
      ro: "Vihiga",
      en: "Vihiga",
image: "/poi-images/vihiga-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Vihiga, die größte Stadt des gleichnamigen Vihiga Countys in Westkenia, besticht durch ihre felsige Landschaft und ihre dichte Besiedlung. Sie liegt in den westlichen Ausläufern des Rift Valleys, nicht weit vom Äquator entfernt. Vihiga ist vor allem die kulturelle Heimat der Maragoli, einer prominenten Untergruppe der Luhya-Nation. Die Region zeichnet sich durch riesige, dramatische Granitfelsen aus, die in den üppig grünen Hügeln verstreut liegen, darunter die berühmten Maragoli-Hills. Aufgrund der sehr hohen Bevölkerungsdichte sind die landwirtschaftlichen Flächen klein, was die Bewohner zu intensiver Subsistenzwirtschaft (Mais, Bohnen, Tee) zwingt. Kulturell ist Vihiga berühmt für das jährliche Maragoli-Kulturfestival, das traditionell am zweiten Weihnachtstag stattfindet (Geografie & Gesellschaft K7).",
      hu: "Vihiga a Nyugat-kenyai régió egyik legsűrűbben lakott, rendkívül csapadékos és termékeny területe, a maragori luhja közösség otthona. A város egy hatalmas gránitszikla árnyékában, buja trópusi környezetben fekszik, a Kakamega-erdő és Kisumu között. A terület gazdaságát a kis parcellás, rendkívül intenzív földművelés – kukorica, banán és bab termesztése – jellemzi. Vihiga híres hagyományos zenéjéről és élénk kulturális fesztiváljairól is. (Földrajz K6 — népsűrűség és intenzív agrárium)",
      ro: "Vihiga este o localitate colinară situată la câțiva kilometri sud de Ecuator, fiind remarcabilă prin densitatea extremă a populației sale rurale. Așezat printre pitoreștile dealuri Maragoli și înconjurat de formațiuni stâncoase uriașe (bolovani de granit), orașul este centrul tradițional al subgrupului Maragoli din cadrul poporului Luhya. Din cauza terenurilor agricole limitate și a fragmentării parcelelor, locuitorii practică o agricultură intensivă pe terase, cultivând porumb, banane și ceai pe suprafețe mici. Orașul este recunoscut și pentru festivalul cultural anual Maragoli.",
      en: "Vihiga is the administrative center of Vihiga County, located in the densely populated, hilly terrain of western Kenya, just north of the equator. The town sits at an elevation of 1,550 meters and is characterized by exceptionally high agricultural intensity, driven by rich soils and abundant rainfall. However, due to severe population density, land fragmentation is high, pushing the local economy toward intensive small-scale farming of tea, maize, and bananas, alongside a strong culture of out-migration for employment. Vihiga is deeply rooted in the Maragoli cultural heritage, uniquely famous for its massive annual Maragoli cultural festivals and vibrant community traditions. The landscape is dotted with dramatic granitic rock formations, creating a distinct and picturesque topography amid the dense green vegetation. Geography K7 — High population density agriculture."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters for Vihiga County.", "Located just five kilometers north of the equator at an altitude of 1,550 meters.", "One of the most densely populated rural municipalities in Kenya.", "The local economy relies on intensive smallholder tea and maize farming.", "Features dramatic granitic boulder outcrops scattered across the hilly terrain.", "A major cultural epicenter for the Maragoli sub-tribe of the Luhya people.", "The town's population was recorded at approximately 118,000 in the 2019 census."]
    },
    
    description: {
      de: "Vihiga ist eine dicht besiedelte Stadt in Westkenia, gelegen zwischen massiven felsigen Hügeln.",
      hu: "Vihiga egy sűrűn lakott város Nyugat-Kenyában, hatalmas sziklás dombok között.",
      ro: "Vihiga este un oraș dens populat din vestul Kenyei, situat printre dealuri stâncoase masive.",
      en: "Vihiga is a densely populated town in western Kenya, situated among massive rocky hills."
    },
    facts: {
      de: [
        "Heimat des Maragoli-Volkes.",
        "Landwirtschaft und kleinbäuerliche Teeproduktion sind die wichtigsten wirtschaftlichen Aktivitäten."
      ],
      hu: [
        "A maragoli nép otthona.",
        "A mezőgazdaság és a kisüzemi teatermelés a fő gazdasági tevékenység."
      ],
      ro: [
        "Căminul poporului Maragoli.",
        "Agricultura și producția de ceai la scară mică sunt principalele activități economice."
      ],
      en: [
        "Home to the Maragoli people.",
        "Agriculture and small-scale tea production are the main economic activities."
      ]
    }
  },
  {
    id: "chuka-cities-v2",
    type: "city",
    parent: "KE-13",
    coords: [37.6488, -0.3323],
    name: {
      de: "Chuka",
      hu: "Chuka",
      ro: "Chuka",
      en: "Chuka",
image: "/poi-images/chuka-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Chuka ist die größte städtische Siedlung im Tharaka-Nithi County und schmiegt sich an die östlichen Hänge des Mount Kenya. Die Stadt ist benannt nach den Chuka, einer Untergruppe des Ameru-Volkes. Traditionell und historisch bekannt für ihre mutigen Krieger (die einst die \"Mau Mau\"-Rebellen in den dichten Wäldern unterstützten), ist Chuka heute ein florierendes Agrarzentrum. Die vulkanischen Böden ermöglichen den lukrativen Anbau von Kaffee, Tee, Macadamia-Nüssen und Bananen. Die Chuka University, eine rapidly wachsende Bildungseinrichtung im Vorort Ndagani, hat das wirtschaftliche Profil der Stadt massiv verändert und Tausende Studenten angezogen. Chuka dient auch als Zugangspunkt zur östlichen Mount-Kenya-Route (Biologie & Gesellschaft K7).",
      hu: "Chuka a Kenya-hegy keleti, dúsan erdősült lejtőin megbúvó mezőgazdasági kisváros. A település híres a tharaka-nithi népcsoport hagyományos dobosairól és táncosairól, akik látványos előadásaikkal gyakran szerepelnek nemzeti ünnepségeken. Chuka az ország egyik legfontosabb kávétermelő bázisa, a vulkáni talaj különleges aromát ad az itteni termésnek. Az újonnan alapított Chuka Egyetem jelentősen felgyorsította a település fejlődését, ami a modernizáció felé mozdította a várost. (Földrajz K6 — kultúra és kávétermesztés)",
      ro: "Chuka este un oraș aflat pe versanții estici, împăduriți ai Muntelui Kenya, îndeplinind un rol comercial major în județul Tharaka-Nithi. Cunoscut pentru ritmurile sale de tobe specifice poporului Chuka, orașul a crescut datorită terenurilor vulcanice profund fertile, favorabile agriculturii intense. Cultivatorii locali produc unele dintre cele mai apreciate soiuri de cafea și ceai, dar și nuci de macadamia. Drumurile care șerpuiesc prin pădure de la Chuka sunt deseori folosite de drumeți curajoși care vor să ajungă pe rutele mai puțin aglomerate ale Parcului Național Muntele Kenya.",
      en: "Chuka is a prominent agricultural and educational town situated on the eastern slopes of Mount Kenya in Tharaka-Nithi County. Located at an altitude of 1,500 meters, the town is blessed with fertile volcanic soils and consistent rainfall, fostering a robust agrarian economy focused on coffee, tea, and macadamia nut production. Chuka is historically significant as the traditional heartland of the Chuka people, a sub-group of the larger Meru community, known for their unique drumming and cultural dances. In recent years, the town has undergone a profound transformation from a quiet farming center to a vibrant university town following the establishment and rapid expansion of Chuka University in 2004. This institutional growth has spurred massive real estate and retail development, modernizing the local economy. Social Studies K7 — University-driven urban growth."
    },
    factsAdvanced: {
      en: ["Located on the eastern slopes of Mount Kenya at an elevation of 1,500 meters.", "The traditional cultural and commercial center for the Chuka people.", "A major regional producer of Arabica coffee and macadamia nuts.", "Chuka University was established in 2004, driving rapid urban expansion.", "Situated along the primary Meru-Nairobi highway corridor.", "The town is famous for the traditional, acrobatic Chuka drummers.", "The urban population reached approximately 43,000 in the 2019 national census."]
    },
    
    description: {
      de: "Chuka ist eine lebendige Landwirtschafts- und Universitätsstadt an den Osthängen des Mount Kenya.",
      hu: "Chuka egy nyüzsgő mezőgazdasági és egyetemi város, amely a Mount Kenya keleti lejtőin található.",
      ro: "Chuka este un oraș universitar și agricol plin de viață situat pe pantele estice ale Muntelui Kenya.",
      en: "Chuka is a lively agricultural and university town located on the eastern slopes of Mount Kenya."
    },
    facts: {
      de: [
        "Sitz der Chuka University.",
        "Bekannt für seine lebhaften lokalen Märkte, auf denen frische Produkte verkauft werden."
      ],
      hu: [
        "A Chuka Egyetemnek ad otthont.",
        "Híres nyüzsgő helyi piacairól, ahol friss termékeket árulnak."
      ],
      ro: [
        "Găzduiește Universitatea Chuka.",
        "Cunoscut pentru piețele sale locale vibrante unde se vând produse proaspete."
      ],
      en: [
        "Hosts Chuka University.",
        "Known for its vibrant local markets selling fresh produce."
      ]
    }, image: "/poi-images/chuka-cities-v2.webp"},
  {
    id: "kiambu-cities-v2",
    type: "city",
    parent: "KE-22",
    coords: [36.83, -1.1667],
    name: {
      de: "Kiambu",
      hu: "Kiambu",
      ro: "Kiambu",
      en: "Kiambu",
image: "/poi-images/kiambu-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kiambu Town ist die Hauptstadt des Kiambu Countys und liegt in unmittelbarer Nähe nördlich von Nairobi. Sie wurde um 1900 von britischen Siedlern gegründet und entwickelte sich rasch zu einem der wichtigsten Zentren für den Kaffeeanbau in Kenia. Jahrhundertelang bildete diese wohlhabende Region das Herzstück des Gikuyu-Landes. Historisch gesehen stammen viele der einflussreichsten Politiker und Geschäftsleute Kenias (inklusive des ersten Präsidenten Jomo Kenyatta) aus dem Kiambu County, was der Stadt den Ruf einer \"politischen Elite-Schmiede\" einbrachte. Mit der massiven Expansion Nairobis wandeln sich die einst endlosen Kaffeeplantagen um Kiambu Town zunehmend in hochklassige Wohngebiete, Einkaufszentren und Golfclubs (Geschichte & Wirtschaft K8).",
      hu: "Kiambu Nairobitól alig néhány kilométerre északra fekvő ősi kikuju közigazgatási központ. A város és környezete egykor sűrű erdőség volt, amit a 20. század elején hatalmas kávé- és teaültetvényekké alakítottak át. Napjainkban a város jelentős átalakuláson megy keresztül, mivel az egykori farmok helyén elit lakóövezetek, golfklubok és bevásárlóközpontok épülnek a növekvő nairobi középosztály számára. Ennek ellenére történelmi épületei és zöld környezete megmaradt. (Földrajz K7 — agglomerációs terjeszkedés)",
      ro: "Kiambu, așezat la nord de capitala Nairobi, a fost din punct de vedere istoric inima elitelor britanice producătoare de cafea. Astăzi, orașul își păstrează aura de lux și prestigiu, caracterizându-se prin terenuri de golf impecabile, reședințe palatiale și plantații tradiționale de cafea de înaltă calitate, care sunt rapid înlocuite de proiecte imobiliare scumpe. Acționează ca centrul administrativ al județului Kiambu și este renumit pentru peisajele deluroase și aerul rece. De asemenea, a jucat un rol vital în perioada premergătoare independenței, mulți lideri politici kenyeni provenind de aici.",
      en: "Kiambu is the historic administrative capital of Kiambu County, situated just 14 kilometers north of Nairobi at a cool elevation of 1,720 meters. Established deeply within the colonial era as a premier center for European coffee farming, the town is surrounded by some of the most lucrative and extensive coffee estates in East Africa. While it retains a strong agricultural backbone, Kiambu's extreme proximity to the capital has subjected it to intense suburbanization, with old plantations steadily yielding to high-end real estate and gated communities. The town remains a critical political and economic powerhouse, deeply influential in Kenya's national history as a core settlement for the Kikuyu people. Despite the creeping concrete, the adjacent Kiambu Golf Club, founded in 1916, preserves the town's old-world heritage. Geography K8 — Suburbanization of agricultural land."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters of Kiambu County.", "Located at an elevation of roughly 1,720 meters (5,643 feet).", "Historically one of the most prominent coffee-growing hubs in East Africa.", "Experiencing massive suburban expansion due to its proximity to Nairobi.", "The Kiambu Golf Club was established by colonial settlers in 1916.", "Functions as a major cultural and political center for the Kikuyu community.", "The urban center had a population of approximately 147,000 in the 2019 census."]
    },
    
    description: {
      de: "Kiambu ist eine wichtige Landwirtschafts- und Verwaltungsstadt, die als großer Vorort von Nairobi fungiert.",
      hu: "Kiambu egy fontos mezőgazdasági és közigazgatási város, amely Nairobi egyik jelentős külvárosa.",
      ro: "Kiambu este un important oraș agricol și administrativ care acționează ca o suburbie majoră pentru Nairobi.",
      en: "Kiambu is an important agricultural and administrative town acting as a major suburb to Nairobi."
    },
    facts: {
      de: [
        "Umgeben von tiefgrünen Kaffeefeldern.",
        "Die Hauptstadt des Kiambu County."
      ],
      hu: [
        "Mélyzöld kávéföldek veszik körül.",
        "Kiambu megye fővárosa."
      ],
      ro: [
        "Înconjurat de câmpuri de cafea de un verde închis.",
        "Capitala comitatului Kiambu."
      ],
      en: [
        "Surrounded by deep green coffee fields.",
        "The capital of Kiambu County."
      ]
    }, image: "/poi-images/kiambu-cities-v2.webp"},
  {
    id: "mandera-cities-v2",
    type: "city",
    parent: "KE-09",
    coords: [41.8614, 3.9372],
    name: {
      de: "Mandera",
      hu: "Mandera",
      ro: "Mandera",
      en: "Mandera",
image: "/poi-images/mandera-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Mandera ist eine der abgelegensten Städte Kenias und liegt extrem im Nordosten, genau an der Dreiländerecke (dem sogenannten Triangle) zwischen Kenia, Äthiopien und Somalia. Die Stadt zeichnet sich durch ein glühend heißes und wüstenhaftes Klima aus. Mandera ist ein lebendiger Schmelztiegel somalischer und äthiopischer Kulturen und lebt wirtschaftlich fast ausschließlich vom grenzüberschreitenden Viehhandel, vor allem mit Kamelen, Ziegen und Rindern. Aufgrund ihrer Grenzlage sah sich Mandera historisch oft mit sicherheitspolitischen Herausforderungen konfrontiert, insbesondere im Zusammenhang mit Schmuggel und militanten Gruppierungen. Dennoch hat die Stadt eine widerstandsfähige Gemeinschaft bewahrt und dient als wichtigstes Handelszentrum der entlegenen Nordost-Region (Geografie & Politik K8).",
      hu: "Mandera a kenyai, szomáliai és etiópiai határok találkozásánál (hármashatár) fekvő poros, szárazföldi sivatagi város. A távoli és elszigetelt település lakói főként nomád szomáli pásztorok, akik számára a teve és a szarvasmarha jelenti a megélhetést. Wajirhoz és Garissához hasonlóan a klíma itt rendkívül meleg és száraz, a víz a legnagyobb kincs. A város gazdasága a három ország közötti informális árucserére és a jószágkereskedelemre támaszkodik, dacolva a térség biztonsági kihívásaival. (Földrajz K8 — hármashatárok és sivatagi túlélés)",
      ro: "Mandera este cel mai îndepărtat oraș din colțul de nord-est al Kenyei, situat fix la punctul de intersecție a granițelor cu Somalia și Etiopia. Acest oraș deșertic se află pe malurile râului Daua, o sursă rară și vitală de apă care susține puțină agricultură irigată într-o regiune predominant aridă. Economia sa tradițională se concentrează pe comerțul transfrontalier și păstoritul cămilelor și caprelor de către poporul de etnie somaleză. În ciuda provocărilor legate de securitate și secetă, Mandera rămâne un centru comercial vibrant în Cornul Africii.",
      en: "Mandera is an incredibly remote frontier town situated at the absolute northeastern tip of Kenya, where the borders of Kenya, Ethiopia, and Somalia converge. Defined by a harsh, arid desert climate, the town sits at a low elevation of just 200 meters near the Daua River, which forms the physical boundary with Ethiopia. Historically and currently, Mandera is a crucial trading post for Somali pastoralists, its economy revolving entirely around cross-border livestock commerce, particularly camels and goats. Due to its isolated location and porous borders, the town is highly cosmopolitan yet frequently challenged by regional security dynamics. Despite its arid nature, localized agriculture thrives along the banks of the Daua River, providing essential subsistence crops to the nomadic population. Geography K7 — Tri-border desert settlements."
    },
    factsAdvanced: {
      en: ["Located at the tri-point border where Kenya, Ethiopia, and Somalia meet.", "Serves as the administrative headquarters of Mandera County.", "Situated at an elevation of roughly 200 meters (656 feet) in a hot desert climate.", "The Daua River forms the northern boundary and supports localized irrigation.", "The economy is overwhelmingly driven by the cross-border pastoralist livestock trade.", "Considered one of the most geographically remote administrative centers in Kenya.", "The urban population was recorded at approximately 114,000 in the 2019 census."]
    },
    
    description: {
      de: "Mandera ist eine Grenzstadt im äußersten Nordosten, wo die Grenzen von Kenia, Äthiopien und Somalia aufeinandertreffen.",
      hu: "Mandera egy szélső északkeleti határváros, amely ott található, ahol Kenya, Etiópia és Szomália határai találkoznak.",
      ro: "Mandera este un oraș de graniță din nord-estul extrem situat la intersecția granițelor Kenyei, Etiopiei și Somaliei.",
      en: "Mandera is an extreme northeastern border town situated where the Kenyan, Ethiopian, and Somali borders meet."
    },
    facts: {
      de: [
        "Liegt in einem rauen, trockenen Wüstenklima.",
        "Weidewirtschaft ist die primäre Lebensweise."
      ],
      hu: [
        "Zord, száraz sivatagi éghajlaton fekszik.",
        "A pásztorkodás az elsődleges életmód."
      ],
      ro: [
        "Situat într-un climat de deșert uscat și aspru.",
        "Păstoritul este modul principal de viață."
      ],
      en: [
        "Located in a harsh, dry desert climate.",
        "Pastoralism is the primary way of life."
      ]
    }, image: "/poi-images/mandera-cities-v2.webp"},
  {
    id: "marsabit-cities-v2",
    type: "city",
    parent: "KE-10",
    coords: [37.9782, 2.3312],
    name: {
      de: "Marsabit",
      hu: "Marsabit",
      ro: "Marsabit",
      en: "Marsabit",
image: "/poi-images/marsabit-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Marsabit, eine Oasenstadt im weiten Norden Kenias, ist eine bemerkenswerte geografische Anomalie. Die Stadt liegt isoliert auf dem erloschenen Schildvulkan Mount Marsabit in einer Höhe von rund 1.700 Metern. Während die umgebende Wüste Chalbi flach, heiß und trocken ist, fängt der Berg die Feuchtigkeit ein und ist von dichtem, nebeligem Regenwald umgeben. Der direkt an die Stadt angrenzende Marsabit-Nationalpark ist Heimat großer Elefantenherden – historisch berühmt durch \"Ahmed\", einen Elefanten mit riesigen Stoßzähnen, der in den 1970er Jahren persönlichen Schutz durch Präsident Kenyatta erhielt. Marsabit ist auch ein faszinierendes kulturelles Zentrum, in dem Rendille, Borana, Gabbra und Burji in einem rauen Umfeld zusammenleben (Geografie K7).",
      hu: "Marsabit egy megdöbbentő, vulkáni oázis Észak-Kenya végtelen sivatagjában. A város a Marsabit vulkán kialudt lejtőin, sűrű, ködbe burkolózó esőerdő szélén épült, éles kontrasztban az őt körülvevő száraz síkságokkal. A klíma itt hűvös és párás. Marsabit a nomád borana, rendille, gabbra és turkana törzsek találkozóhelye és piaca. A közeli Marsabit Nemzeti Park krátertavai – mint például a Lake Paradise – elefántok és bivalyok menedéke. (Természetismeret K6 — vulkáni oázisok és nomád piacok)",
      ro: "Marsabit este o oază neobișnuită situată în mijlocul deșertului din nordul Kenyei. Construit pe un vulcan stins dens împădurit, orașul beneficiază de un micro-climat rece și umed, o anomalie absolută comparativ cu regiunile prăfuite din jur. Parcul Național Marsabit înconjoară așezarea și este faimos pentru elefanții săi gigantici care au fildeși uriași. Atracțiile includ spectaculosul Lac Paradise, un lac de crater imaculat ce oferă apă animalelor sălbatice. Orașul este un melting-pot cultural izolat, adăpostind triburi Rendille, Borana și Gabra care trăiesc în armonie.",
      en: "Marsabit is a spectacular geographical anomaly, functioning as a lush, green oasis rising dramatically from the surrounding hyper-arid deserts of northern Kenya. Situated at an altitude of 1,400 meters on an extinct shield volcano, the town is frequently shrouded in thick mist, creating a microclimate that supports a dense montane forest. This forest is protected within the Marsabit National Park, established in 1949 and historically famous for its massive-tusked elephants, including the legendary 'Ahmed'. The town is highly diverse, serving as a peaceful trading nexus for the Borana, Rendille, Gabra, and Turkana communities. Recently, the paving of the Isiolo-Moyale highway has broken Marsabit's historical isolation, accelerating commercial growth and positioning it as a major stopover on the Cape-to-Cairo route. Earth Sciences K8 — Volcanic microclimates."
    },
    factsAdvanced: {
      en: ["Situated on an isolated extinct volcano at an elevation of 1,400 meters.", "Serves as the administrative capital of the vast Marsabit County.", "Features a cool, misty microclimate surrounded by harsh, low-altitude deserts.", "Marsabit National Park, home to Lake Paradise, was established in 1949.", "Historically famous for 'Ahmed', an elephant with extraordinarily large tusks.", "A major cultural melting pot for Rendille, Borana, and Gabra pastoralists.", "The town's population reached approximately 38,000 according to the 2019 census."]
    },
    
    description: {
      de: "Marsabit ist eine bewaldete Oasenstadt, die sich abrupt aus der umliegenden Wüste im Norden Kenias erhebt.",
      hu: "Marsabit egy erdős oázisváros, amely hirtelen emelkedik ki a környező sivatagból Észak-Kenyában.",
      ro: "Marsabit este un oraș oază împădurit ce se ridică brusc din deșertul înconjurător din nordul Kenyei.",
      en: "Marsabit is a forested oasis town emerging abruptly from the surrounding desert in northern Kenya."
    },
    facts: {
      de: [
        "Sitzt auf einem erloschenen Schildvulkan.",
        "Bekannt für große Elefanten mit massiven Stoßzähnen."
      ],
      hu: [
        "Egy kihalt pajzsvulkán tetején fekszik.",
        "Hatalmas agyarú, nagy elefántjairól ismert."
      ],
      ro: [
        "Se află deasupra unui vulcan scut dispărut.",
        "Cunoscut pentru elefanții mari cu colți masivi."
      ],
      en: [
        "Sits atop an extinct shield volcano.",
        "Known for large elephants with massive tusks."
      ]
    }, image: "/poi-images/marsabit-cities-v2.webp"},
  {
    id: "nyamira-cities-v2",
    type: "city",
    parent: "KE-46",
    coords: [34.935, -0.5638],
    name: {
      de: "Nyamira",
      hu: "Nyamira",
      ro: "Nyamira",
      en: "Nyamira",
image: "/poi-images/nyamira-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Nyamira, gelegen in der südwestlichen Ecke Kenias, ist die Hauptstadt des Nyamira Countys und bildet zusammen mit Kisii das traditionelle Hochland-Territorium der Gusii-Bevölkerung. Nyamira zeichnet sich durch seine atemberaubenden, saftig grünen Hügel, eine sehr hohe Bevölkerungsdichte und ein kühles, feuchtes Klima aus, das das ganze Jahr über üppige Ernten garantiert. Tee und Bananen sind die Hauptstützen der lokalen Wirtschaft, wobei kleine Parzellen intensiv bewirtschaftet werden. Obwohl Nyamira im Schatten seines größeren Nachbarn Kisii steht, fungiert die Stadt als wichtiges landwirtschaftliches Verwaltungszentrum. Die tiefen Täler und steilen Hügel machen Nyamira zu einer der landschaftlich reizvollsten, aber infrastrukturell anspruchsvollsten Regionen Kenias (Geografie K7).",
      hu: "Nyamira egy termékeny, dimbes-dombos mezőgazdasági kisváros a Kisii-hegyvidéken, amelyet jórészt a kisii (gusii) etnikai csoport lakik. A vidék vulkáni talaja és az egész évben gyakori csapadék ideálissá teszi a tea, a banán és a passion fruit (maracuja) termesztésére, amely a lakosság fő jövedelemforrása. A terület népsűrűsége rendkívül magas, ezért a farmok többsége nagyon apró. Nyamira nyugodt, dolgos atmoszférája ellenére jelentős mezőgazdasági exportőr a belföldi piacon. (Földrajz K6 — gyümölcstermesztés és vulkáni talaj)",
      ro: "Nyamira este un oraș agricol situat în peisajele deluroase și luxuriante din sud-vestul Kenyei, servind drept capitală a județului cu același nume. Această zonă înaltă, parte a teritoriului tradițional al poporului Gusii (Kisii), beneficiază de ploi frecvente și soluri extrem de fertile. Prin urmare, economia locală este dominată de agricultura pe suprafețe mici, cultivându-se intens banane, ceai, porumb și o mare varietate de legume proaspete. Terenurile sunt terasate pentru a maximiza producția, creând un peisaj rural pitoresc, dar care se confruntă cu o presiune demografică tot mai ridicată.",
      en: "Nyamira is a highly productive agricultural town and the headquarters of Nyamira County, located in the verdant highlands of southwestern Kenya. Situated at an altitude of 1,700 meters, the town is characterized by steep hills, deep valleys, and exceptional rainfall, making it a prime zone for intensive cash crop cultivation. The local economy is robustly anchored in the massive production of tea and bananas, alongside dairy farming, largely executed by smallholder farmers. Culturally, Nyamira is a core settlement for the Gusii people, renowned for their industrious farming practices. Despite its high population density, the town retains a deeply rural character, serving primarily as a collection and processing point for the surrounding agrarian hinterland. Economics K6 — Intensive highland agriculture."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters for Nyamira County.", "Located at a high altitude of approximately 1,700 meters (5,577 feet).", "A major national producer of high-quality tea and bananas.", "The local topography is defined by extremely steep, terraced hillsides.", "Functions as a primary cultural and commercial hub for the Gusii community.", "The region experiences near-daily rainfall, supporting year-round agriculture.", "The urban population was recorded at roughly 36,000 in the 2019 census."]
    },
    
    description: {
      de: "Nyamira ist ein landwirtschaftliches Zentrum im Kisii-Hochland, das für den Tee- und Bananenanbau berühmt ist.",
      hu: "Nyamira egy mezőgazdasági csomópont a Kisii-fennsíkon, amely tea- és banántermesztéséről híres.",
      ro: "Nyamira este un hub agricol în zonele înalte Kisii, renumit pentru cultivarea ceaiului și a bananelor.",
      en: "Nyamira is an agricultural hub in the Kisii highlands, renowned for tea and banana farming."
    },
    facts: {
      de: [
        "Geprägt von sehr fruchtbarem vulkanischen Boden.",
        "Eine der am dichtesten besiedelten ländlichen Gegenden Kenias."
      ],
      hu: [
        "Rendkívül termékeny vulkáni talaj jellemzi.",
        "Kenya egyik legsűrűbben lakott vidéki területe."
      ],
      ro: [
        "Caracterizat printr-un sol vulcanic foarte fertil.",
        "Una dintre cele mai dens populate zone rurale din Kenya."
      ],
      en: [
        "Characterized by highly fertile volcanic soil.",
        "One of the most densely populated rural areas in Kenya."
      ]
    }, image: "/poi-images/nyamira-cities-v2.webp"},
  {
    id: "kwale-cities-v2",
    type: "city",
    parent: "KE-02",
    coords: [39.46, -4.1738],
    name: {
      de: "Kwale",
      hu: "Kwale",
      ro: "Kwale",
      en: "Kwale",
image: "/poi-images/kwale-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kwale ist eine ruhige Stadt in den Küstenhügeln Südkenias und das Verwaltungszentrum des Kwale Countys. Im Gegensatz zu den heißen Küstenorten wie Diani Beach, das ebenfalls zum County gehört, profitiert Kwale von der kühleren, erhöhten Lage in den Shimba Hills. Direkt neben der Stadt erstreckt sich das Shimba-Hills-Nationalreservat, ein Biodiversitäts-Hotspot, der als letzter Zufluchtsort der stark bedrohten Rappenantilope (Sable Antelope) in Kenia bekannt ist. Wirtschaftlich erlebt die Region seit 2013 einen massiven Aufschwung durch das Base-Titanium-Bergbauprojekt, das enorme Mengen an Titanmineralien wie Rutil und Ilmenit fördert. Dennoch dominieren für die einheimische Digo-Bevölkerung weiterhin Landwirtschaft und Kleinhandel (Geografie & Biologie K8).",
      hu: "Kwale egy partmenti dombvidéki város Mombasától délre, amely a híres Shimba-hegység lábánál fekszik. A város zöld és párás környezete menedéket nyújt a tengerparti hőség elől, és az őslakos digo nép közigazgatási központja. A közelmúltban a gazdaság hatalmas fellendülést tapasztalt a Base Titanium bányavállalat megnyitásával, amely nehézásványokat termel ki a város közelében. A Shimba Hills Nemzeti Rezervátum a ritka fakó antilopok egyik utolsó menedéke. (Földrajz K7 — ritkaföldfém-bányászat és természetvédelem)",
      ro: "Kwale este un oraș liniștit situat la sud-vest de Mombasa, îndeplinind rolul de capitală administrativă a județului de coastă omonim. Deși se află aproape de faimoasele plaje cu nisip alb de la Diani, orașul în sine este cocoțat pe colinele împădurite, bucurându-se de o climă mai răcoroasă. Kwale se bazează economic pe turism, pe vastul proiect de minerit al titanului din regiune și pe agricultură tropicală, cultivându-se nucă de cocos și citrice. O atracție naturală majoră în apropiere este Rezervația Națională Shimba Hills, renumită pentru populația sa rară de antilope sabre.",
      en: "Kwale is the administrative capital of Kwale County, uniquely situated 30 kilometers inland from the Indian Ocean on the eastern slopes of the Shimba Hills. Positioned at an elevation of roughly 400 meters, the town enjoys a slightly cooler and less humid climate than the adjacent coastal strip. Kwale's economy is highly diversified; it acts as an agricultural market for the surrounding fruit and cashew nut farmers, a logistical base for tourism directed at the pristine Diani Beach, and a center for environmental conservation linked to the Shimba Hills National Reserve, famous for its Sable antelopes. Furthermore, the local economy was significantly boosted by the establishment of the Kwale Titanium Mine in 2013, which introduced major industrial infrastructure to this historically quiet administrative post. Geography K7 — Coastal hinterlands and mining."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters for Kwale County.", "Located inland on the eastern slopes of the Shimba Hills at 400 meters elevation.", "Positioned just 30 kilometers southwest of Mombasa city.", "The adjacent Shimba Hills National Reserve was established in 1968.", "A major logistical hub for the nearby Diani Beach tourism sector.", "The massive Kwale Titanium Mine commenced operations nearby in 2013.", "The town recorded a population of approximately 10,000 in the 2019 census."]
    },
    
    description: {
      de: "Kwale ist eine Stadt auf einem Hügel nahe der Küste, die als Verwaltungssitz des Kwale County dient.",
      hu: "Kwale egy dombtetőn épült város a tengerpart közelében, amely a Kwale megye székhelye.",
      ro: "Kwale este un oraș pe un deal lângă coastă, servind ca sediu pentru comitatul Kwale.",
      en: "Kwale is a hilltop town near the coast, serving as the headquarters for Kwale County."
    },
    facts: {
      de: [
        "Nahe dem Shimba Hills Nationalreservat.",
        "Bietet tropisches Küstenwetter kombiniert mit einer etwas kühleren Brise auf dem Hügel."
      ],
      hu: [
        "A Shimba Hills Nemzeti Rezervátum közelében található.",
        "Trópusi parti időjárás és egy kissé hűvösebb dombtetői szellő jellemzi."
      ],
      ro: [
        "Aproape de Rezervația Națională Shimba Hills.",
        "Dispune de o vreme tropicală de coastă combinată cu o briză de deal ușor mai rece."
      ],
      en: [
        "Close to the Shimba Hills National Reserve.",
        "Features coastal tropical weather combined with a slightly cooler hilltop breeze."
      ]
    }, image: "/poi-images/kwale-cities-v2.webp"},
  {
    id: "kangundo-cities-v2",
    type: "city",
    parent: "KE-16",
    coords: [37.3468, -1.2981],
    name: {
      de: "Kangundo",
      hu: "Kangundo",
      ro: "Kangundo",
      en: "Kangundo",
image: "/poi-images/kangundo-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kangundo ist eine historische und landwirtschaftlich bedeutende Stadt im Machakos County, die in den malerischen Kangundo- und Matungulu-Hügeln liegt. Die Stadt entwickelte sich während der Kolonialzeit stark, vor allem weil christliche Missionare (wie die der African Inland Mission) hier früh Schulen und Krankenhäuser etablierten, darunter das berühmte Kangundo Level 4 Hospital. In der Mitte des 20. Jahrhunderts wurde Kangundo als \"Millionärsstadt\" der Kamba-Bevölkerung bekannt, da die hier ansässigen Kaffeebauern dank des boomenden globalen Kaffeepreises zu großem Reichtum kamen. Heute ist Kangundo noch immer stark agrarisch geprägt und produziert neben Kaffee auch Bohnen, Mais und Papayas für den Markt in Nairobi (Geschichte & Landwirtschaft K7).",
      hu: "Kangundo a kamba régió egyik legrégebbi közigazgatási és oktatási központja a Machakos megyében. A gyarmati időkben a misszionáriusok itt építették fel az első erős téglaiskolákat és kórházakat a vidéken. Bár a terület félszáraz, Kangundo magaslati fekvése lehetővé teszi a kávé és a teraszos kukorica termesztését. A város a történelmi kereskedelmi útvonalak metszéspontján áll, és máig a kamba kultúra büszke bástyája, amely híres a kosárfonásról és fafaragásról. (Történelem K6 — misszionárius oktatás)",
      ro: "Kangundo este o așezare deluroasă și pitorească aflată în județul Machakos, situată la est de capitala Nairobi. Datorită altitudinii sale ridicate comparativ cu câmpiile înconjurătoare, orașul beneficiază de un micro-climat prielnic pentru agricultură comercială. Terenurile terasate din zona Kangundo sunt acoperite cu plantații bogate de cafea de calitate și livezi întinse de pomi fructiferi, reprezentând principala sursă de venit a fermierilor locali din tribul Akamba. Dincolo de agricultură, apropierea relativă de Nairobi atrage investiții în dezvoltarea infrastructurii locale și în imobiliare.",
      en: "Kangundo is a vibrant agricultural town situated within the fertile Mua Hills in Machakos County, located approximately 70 kilometers east of Nairobi. Sitting at a high elevation of 1,600 meters, the town benefits from a distinctly cooler and wetter microclimate compared to the surrounding semi-arid Ukambani plains. This geographical advantage has historically made Kangundo a premier hub for high-quality Arabica coffee production, managed largely by robust local farming cooperatives. The town is deeply culturally significant to the Kamba people and is renowned for its strong educational institutions, particularly Kangundo High School. The steep, terraced landscapes around the town not only support intensive agriculture but also provide striking scenic views of the Rift Valley margins. Geography K6 — Microclimates and cooperative farming."
    },
    factsAdvanced: {
      en: ["Located in the Mua Hills of Machakos County at an elevation of 1,600 meters.", "Historically a premier center for Arabica coffee cultivation in the region.", "The local economy is heavily driven by agricultural farming cooperatives.", "Features a cooler, wetter microclimate than the broader Ukambani region.", "Located approximately 70 kilometers east of the capital, Nairobi.", "Acts as a major educational and cultural center for the Kamba community.", "The combined Kangundo-Tala urban area had a population of roughly 12,000 in 2019."]
    },
    
    description: {
      de: "Kangundo ist eine lebhafte Agrarstadt im Machakos County, eng verbunden mit Tala.",
      hu: "Kangundo egy élénk mezőgazdasági város a Machakos megyében, amely szorosan kapcsolódik Talához.",
      ro: "Kangundo este un oraș agricol vibrant din comitatul Machakos, strâns legat de Tala.",
      en: "Kangundo is a vibrant agricultural town in Machakos County, closely linked with Tala."
    },
    facts: {
      de: [
        "Ein wichtiger Kaffeeproduzent in der Region.",
        "Hat eine starke lokale Wirtschaft, die vom Handel angetrieben wird."
      ],
      hu: [
        "A régió egyik fontos kávétermelője.",
        "Erős helyi gazdasággal rendelkezik, amelyet a kereskedelem mozgat."
      ],
      ro: [
        "Un important producător de cafea din regiune.",
        "Are o economie locală puternică condusă de comerț."
      ],
      en: [
        "An important producer of coffee in the region.",
        "Has a strong local economy driven by trade."
      ]
    }, image: "/poi-images/kangundo-cities-v2.webp"},
  {
    id: "gilgil-cities-v2",
    type: "city",
    parent: "KE-32",
    coords: [36.3268, -0.4951],
    name: {
      de: "Gilgil",
      hu: "Gilgil",
      ro: "Gilgil",
      en: "Gilgil",
image: "/poi-images/gilgil-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Gilgil ist eine strategisch wichtige Stadt im Nakuru County, eingebettet zwischen Naivasha und Nakuru im Rift Valley. Während der britischen Kolonialzeit in den 1920er Jahren wurde Gilgil als riesiger Eisenbahnknotenpunkt gegründet, der die Hauptstrecke mit einer Abzweigung nach Nyahururu verband. Heute ist Gilgil vor allem als die wichtigste Garnisonsstadt Kenias bekannt. Hier befinden sich große Kasernen der Kenya Defence Forces (KDF), der National Youth Service (NYS) und die Anti-Stock Theft Unit. Neben dem Militär zieht die Stadt Touristen an, die zum nahegelegenen Lake Elementaita (Heimat von Flamingos und Pelikanen) oder zum Kariandusi-Prähistorik-Museum, einer bedeutenden Fundstätte von Faustkeilen der frühen Hominiden, reisen (Geografie & Geschichte K7).",
      hu: "Gilgil egy nyüzsgő közlekedési és katonai kisváros a Nagy-hasadékvölgyben, Naivasha és Nakuru között. Története mélyen összefonódik a hadsereggel; a brit időkben jelentős katonai garnizon volt, ma pedig a Kenyai Védelmi Erők egyik fő bázisa. A vasútvonal mellett épült város ma már a környező hatalmas tej- és vágómarha farmok kereskedelmi központjaként is szolgál. Klímája száraz, a növényzet jellegzetesen akáciás szavanna. (Történelem K6 — katonai helyőrségek)",
      ro: "Gilgil este un oraș plin de viață din județul Nakuru, așezat în Valea Marelui Rift, de-a lungul autostrăzii Nairobi-Nakuru. De-a lungul istoriei, a servit ca o bază militară esențială atât pentru forțele britanice în perioada colonială, cât și, în prezent, pentru Forțele de Apărare din Kenya. Cu toate acestea, Gilgil a dobândit faimă națională datorită zonei Kekopey, cunoscută ca \"centrul fripturii de carne\" (nyama choma), unde zeci de grătare fumegânde atrag călătorii zilnic. În plus, peisajul înconjurător este presărat cu ferme de lactate de top și facilități de floricultură.",
      en: "Gilgil is a prominent transit and military town situated in Nakuru County on the floor of the Great Rift Valley, between Naivasha and Nakuru. Established in the early 20th century as a crucial station on the Uganda Railway, it sits at an elevation of 2,000 meters. The town's modern identity is heavily defined by its massive military installations; it hosts the headquarters of the Kenya Defence Forces' Western Command, the Anti-Stock Theft Unit, and the National Youth Service training college, injecting substantial state-driven commerce into the local economy. Beyond the military presence, Gilgil is an important agricultural market for nearby wheat and dairy farms, and serves as the primary turn-off point for tourists traveling toward Nyahururu and the Aberdare ranges. Social Studies K7 — Garrison towns and rail history."
    },
    factsAdvanced: {
      en: ["Evolved around a railway station established on the Uganda Railway line.", "Located at an altitude of approximately 2,000 meters (6,561 feet).", "Hosts the headquarters for the Kenya Defence Forces' Western Command.", "Home to the primary training college for the National Youth Service (NYS).", "Positioned on the main Nairobi-Nakuru highway in the Great Rift Valley.", "Serves as the main junction for traffic heading north to Nyahururu.", "The urban population was recorded at roughly 60,000 in the 2019 census."]
    },
    
    description: {
      de: "Gilgil ist eine alte Eisenbahnstadt zwischen Naivasha und Nakuru im Rift Valley.",
      hu: "Gilgil egy régi vasúti város Naivasha és Nakuru között a Rift Valley-ben.",
      ro: "Gilgil este un vechi oraș feroviar situat între Naivasha și Nakuru în Valea Riftului.",
      en: "Gilgil is an old railway town located between Naivasha and Nakuru in the Rift Valley."
    },
    facts: {
      de: [
        "Heimat einer großen Militärkaserne.",
        "Bekannt für eine große Telekommunikations-Satelliten-Erdfunkstelle."
      ],
      hu: [
        "Egy nagy katonai laktanya található itt.",
        "Egy hatalmas távközlési műholdas földi állomásról ismert."
      ],
      ro: [
        "Găzduiește o importantă cazarmă militară.",
        "Cunoscut pentru o mare stație de pământ prin satelit pentru telecomunicații."
      ],
      en: [
        "Home to a major military barracks.",
        "Known for a large telecommunications satellite earth station."
      ]
    }, image: "/poi-images/gilgil-cities-v2.webp"},
  {
    id: "kapsabet-cities-v2",
    type: "city",
    parent: "KE-29",
    coords: [35.105, 0.2038],
    name: {
      de: "Kapsabet",
      hu: "Kapsabet",
      ro: "Kapsabet",
      en: "Kapsabet",
image: "/poi-images/kapsabet-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kapsabet ist die Hauptstadt des Nandi Countys und thront auf dem weiten, üppigen Nandi-Plateau in Westkenia. Die Stadt ist die unbestrittene kulturelle Hochburg des Nandi-Volkes. International bekannt ist Kapsabet als \"die Brutstätte der Champions\". Nirgendwo sonst auf der Welt gibt es eine so hohe Dichte an Olympiasiegern und Weltrekordhaltern im Lang- und Mittelstreckenlauf wie in Kapsabet und den umliegenden Dörfern. Die Höhenlage von rund 2.000 Metern, das perfekte Klima und eine genetische Veranlagung machen die Athleten unschlagbar. Neben dem Sport ist die Region eines der Hauptanbaugebiete für Tee in Kenia; die Teeplantagen der Nandi-Hills prägen das Landschaftsbild maßgeblich und sichern das wirtschaftliche Überleben (Sport & Biologie K8).",
      hu: "Kapsabet a nandi nép fővárosa a Rift-völgy magasföldjén, híres buja, smaragdzöld teaültetvényeiről. A város, akárcsak Eldoret, a kenyai hosszútávfutók bölcsője; tucatnyi olimpiai bajnok indult útjára Kapsabet híres edzőtáboraiból és középiskoláiból. Az 1900 méter feletti tengerszint feletti magasság biztosítja a szükséges ritkás levegőt az állóképesség fejlesztéséhez. A tea mellett az erdészet és a tejtermelés a helyi gazdaság gerince. (Földrajz K6 — magasföldek és sport)",
      ro: "Kapsabet este un oraș vibrant, așezat în regiunea colinară și mereu verde din Valea Riftului, servind ca nucleu administrativ al județului Nandi. Pe plan internațional, Kapsabet este adesea denumit pepiniera campionilor, zeci de deținători ai recordurilor mondiale la maraton având originile sau bazele de antrenament aici. Orașul este de asemenea epicentrul unei vaste regiuni agricole, fiind faimos pentru fabricile sale de procesare a frunzelor de ceai și pentru producția extinsă de lactate, care susțin fermierii din comunitatea Kalenjin. Aerul curat, combinat cu solurile vulcanice, asigură prosperitatea locală.",
      en: "Kapsabet is the administrative capital of Nandi County, nestled high in the Nandi Hills at a commanding elevation of 2,000 meters. The town's geography is characterized by rolling, intensely green landscapes shaped by deep volcanic soils and heavy, consistent rainfall. This environment makes Kapsabet an absolute powerhouse for commercial tea production, with numerous sprawling estates completely surrounding the urban center. Similar to Eldoret, Kapsabet is globally celebrated as a 'cradle of champions', having produced an extraordinary number of Olympic and world-record-breaking long-distance runners who utilize the high altitude for rigorous training. The town serves as the cultural and political heartbeat of the Nandi people, balancing its deeply agrarian economy with expanding educational and athletic infrastructure. Social Studies K6 — Athletics and high-altitude tea."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters of Nandi County.", "Located in the Nandi Hills at a high elevation of roughly 2,000 meters.", "The surrounding region is one of Kenya's largest commercial tea producers.", "Globally renowned for producing dozens of elite, world-class marathon runners.", "Acts as the primary cultural and political center for the Nandi community.", "Experiences an exceptionally cool and wet climate year-round.", "The urban population reached approximately 104,000 in the 2019 national census."]
    },
    
    description: {
      de: "Kapsabet ist das Verwaltungszentrum des Nandi County, stark geprägt vom Teeanbau.",
      hu: "Kapsabet a Nandi megye közigazgatási központja, amely nagy mértékben a teatermesztésre támaszkodik.",
      ro: "Kapsabet este centrul administrativ al comitatului Nandi, profund implicat în cultivarea ceaiului.",
      en: "Kapsabet is the administrative center of Nandi County, heavily immersed in tea cultivation."
    },
    facts: {
      de: [
        "Gilt als Heimat legendärer Langstreckenläufer.",
        "Zeichnet sich durch üppige, sanft gewellte grüne Landschaften aus."
      ],
      hu: [
        "A legendás hosszútávfutók szülőföldjének tartják.",
        "Buja, hullámzó zöld tájak jellemzik."
      ],
      ro: [
        "Considerată o patrie a legendarilor alergători de fond.",
        "Prezintă peisaje verzi luxuriante și deluroase."
      ],
      en: [
        "Considered a homeland for legendary long-distance runners.",
        "Features lush, rolling green landscapes."
      ]
    }, image: "/poi-images/kapsabet-cities-v2.webp"},
  {
    id: "limuru-cities-v2",
    type: "city",
    parent: "KE-22",
    coords: [36.6433, -1.107],
    name: {
      de: "Limuru",
      hu: "Limuru",
      ro: "Limuru",
      en: "Limuru",
image: "/poi-images/limuru-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Limuru ist eine bezaubernde, nebelverhangene Stadt im Kiambu County, die am östlichen Rand des Rift Valleys auf einer erstaunlichen Höhe von über 2.200 Metern liegt. Das eisige Klima, kombiniert mit fruchtbarer vulkanischer Asche, macht Limuru zum unbestrittenen Zentrum des Teeanbaus in Zentral-Kenia (im Gegensatz zu Kericho im Westen). Im Jahr 1903 wurde hier der erste Tee Kenias durch den Europäer AB McDonell gepflanzt, und heute produzieren riesige Güter wie Mabroukie Tee von Weltklasse. Industriell ist Limuru untrennbar mit der Schuhfabrik Bata verbunden, deren gigantisches Werk seit den 1930er Jahren Generationen von Kenianern mit Schuhen versorgt. Diese Mischung aus Industrie und idyllischen Teefeldern macht die Stadt einzigartig (Wirtschaft K8).",
      hu: "Limuru Nairobitól nyugatra található hűvös, magaslati város, amely a brit gyarmati tea-bárók egyik legkorábbi központja volt. A vastag, sűrű ködök és a 2200 méteres tengerszint feletti magasság miatt az ide látogató európaiak Skóciához hasonlították a vidéket. Ma Limuru ad otthont a történelmi teaüzemeknek (például a híres Mabroukie teabirtok), valamint az ország egyik legrégibb cipőgyárának, a Bata üzemnek. A tájat ma is az elegáns kúriák és a teamezők uralják. (Földrajz K7 — magashegyi ipar és tea)",
      ro: "Limuru este un oraș recunoscut național pentru micro-climatul său extrem de rece și adesea cețos, fiind cocoțat pe marginea estică a Văii Riftului în județul Kiambu. Aflat la doar câțiva kilometri de Nairobi, peisajul său este definit de nesfârșite plantații de ceai cu o nuanță de verde smarald, fiind prima locație din Kenya unde ceaiul a fost cultivat comercial la începutul secolului XX. În plus față de turismul agricol, Limuru se remarcă drept un puternic pol industrial; din 1940 găzduiește sediul uriașei fabrici de încălțăminte Bata, un pilon vital al angajării forței de muncă în zonă.",
      en: "Limuru is a historically significant, high-altitude town located on the eastern edge of the Great Rift Valley in Kiambu County, just 30 kilometers northwest of Nairobi. Positioned at an imposing elevation of 2,250 meters, it is famous for its chilly, frequently foggy climate. This unique geography led colonial settlers to establish Kenya's very first commercial tea plantations here in 1903, an industry that continues to dominate the landscape today alongside vast dairy farms. Limuru also features the massive Bata shoe factory, established in 1939, which has been a pillar of local industrial employment. Today, the town serves as a prominent commuter suburb, while its dramatic escarpment views and historic tea estates attract significant domestic tourism and real estate investment. Geography K8 — Escarpment climates and early industrialization."
    },
    factsAdvanced: {
      en: ["Located on the edge of the Great Rift Valley at an altitude of 2,250 meters.", "The site of Kenya's first commercial tea plantations, established in 1903.", "Home to the massive Bata shoe manufacturing plant, opened in 1939.", "Features a famously cool, misty climate year-round due to its elevation.", "Located just 30 kilometers northwest of the capital, Nairobi.", "A major national hub for large-scale commercial dairy farming.", "The town recorded a population of roughly 159,000 in the 2019 census."]
    },
    
    description: {
      de: "Limuru ist eine kalte, neblige Stadt im Kiambu County, historisch berühmt für die Produktion von Kenias bestem Tee und Schuhen.",
      hu: "Limuru egy hideg, ködös város Kiambu megyében, amely történelmileg Kenya legjobb teájának és cipőinek gyártásáról híres.",
      ro: "Limuru este un oraș rece și cețos din comitatul Kiambu, renumit istoric pentru producția celui mai bun ceai și a pantofilor din Kenya.",
      en: "Limuru is a cold, misty town in Kiambu County, historically famous for producing Kenya's best tea and shoes."
    },
    facts: {
      de: [
        "Heimat der Bata-Schuhfabrik in Kenia.",
        "Berüchtigt für extrem kühles und nebliges Wetter."
      ],
      hu: [
        "Itt található a Bata cipőgyár Kenyában.",
        "Rendkívül hűvös és ködös időjárásáról hírhedt."
      ],
      ro: [
        "Găzduiește Compania de Pantofi Bata din Kenya.",
        "Notoriu pentru vremea sa extrem de rece și cețoasă."
      ],
      en: [
        "Home to the Bata Shoe Company in Kenya.",
        "Notorious for extremely chilly and foggy weather."
      ]
    }
  },
  {
    id: "molo-cities-v2",
    type: "city",
    parent: "KE-32",
    coords: [35.7333, -0.2486],
    name: {
      de: "Molo",
      hu: "Molo",
      ro: "Molo",
      en: "Molo",
image: "/poi-images/molo-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Molo liegt tief in den dichten Wäldern und Highlands des Nakuru Countys und ist berüchtigt für sein eisiges Klima – oft fallen die Temperaturen hier auf Werte knapp über dem Gefrierpunkt. Früher war Molo umgeben vom endlosen Mau-Wald, Kenias größtem Wassereinzugsgebiet, doch Landwirtschaft und Holzeinschlag haben die Landschaft stark verändert. Historisch etablierten europäische Siedler in Molo eine blühende Industrie für Schafzucht und Molkereien; das \"Molo-Lamm\" (Molo Lamb) gilt bis heute in kenianischen Restaurants als eine seltene und teure Delikatesse. Zudem ist die Region das größte Anbaugebiet für Pyrethrum (Wucherblumen), eine Pflanze, aus der biologische Insektizide gewonnen werden. Trotz dunkler politischer Episoden in den 1990er Jahren ist die Stadt ein Agrar-Gigant geblieben (Geografie K7).",
      hu: "Molo egy nyugodt hegyvidéki kisváros Nakuru közelében, a Mau-erdő magasföldjein. A 2500 méteres magasság miatt a hőmérséklet gyakran megközelíti a fagypontot, ami teljesen egyedi flórát eredményez Kenyában. A britek itt honosították meg a juh- és burgonyatenyésztést; a 'molo-i bárány' fogalom a minőségi hús szinonimája a kenyai piacon. A város faipara is jelentős volt a hatalmas, mára sokat ritkult tűlevelű erdőségeknek köszönhetően. (Földrajz K6 — hegyvidéki pásztorkodás és fagypont)",
      ro: "Molo este un oraș montan liniștit din regiunea Marelui Rift, faimos în toată Kenya pentru temperaturile sale de îngheț și pentru importanța sa în agricultura la mare altitudine. Înconjurat de pădurea istorică Mau, orașul a fost puternic colonizat de europeni, care i-au valorificat solul extrem de fertil. Astăzi, Molo este supranumit «capitala cartofilor» a Kenyei, o mare parte a producției naționale de cartofi, mazăre, pyrethrum și morcovi provenind de pe fermele locale. Mai mult, pășunile sale abundente fac din regiune un centru de top pentru creșterea ovinelor de înaltă calitate.",
      en: "Molo is a prominent agricultural town situated high on the Mau Escarpment within Nakuru County. Benefiting from an exceptionally high elevation of 2,500 meters and rich volcanic loam soils, the town is widely regarded as the capital of Kenya's potato and pyrethrum industries. Molo grew significantly in the early 20th century as a key station on the Uganda Railway, facilitating the rapid export of massive agricultural yields and high-quality timber from the adjacent Mau Forest. The consistently cold climate historically made it a favored settlement for European farmers concentrating on sheep rearing and wheat. Today, Molo remains a critical agrarian hub, though it occasionally faces environmental challenges linked to the deforestation of the vital Mau water tower that borders the town. Economics K7 — High-altitude cash crops."
    },
    factsAdvanced: {
      en: ["Located on the Mau Escarpment at an extreme altitude of 2,500 meters (8,202 feet).", "Historically developed as a major agricultural stop on the Uganda Railway.", "Nationally famous as the primary hub for potato and pyrethrum cultivation.", "The surrounding region is characterized by extensive, cool-climate sheep farming.", "Borders the Mau Forest complex, Kenya's largest closed-canopy forest ecosystem.", "Known for producing high-quality timber and wool products.", "The town's population was recorded at approximately 163,000 in the 2019 census."]
    },
    
    description: {
      de: "Molo ist eine hoch gelegene Agrarstadt entlang der alten Eisenbahnstrecke zwischen Kenia und Uganda.",
      hu: "Molo egy magaslati mezőgazdasági város a régi Kenya-Uganda vasútvonal mentén.",
      ro: "Molo este un oraș agricol de mare altitudine situat de-a lungul vechii căi ferate Kenya-Uganda.",
      en: "Molo is a high-altitude agricultural town positioned along the old Kenya-Uganda railway."
    },
    facts: {
      de: [
        "Berühmt für die Produktion von hochwertigen Schafen und Lämmern (Molo-Lamm).",
        "Die Holzproduktion ist ein wichtiger lokaler Wirtschaftszweig."
      ],
      hu: [
        "Kiváló minőségű juh- és bárányhúsáról hírhedt (Molo bárány).",
        "A fatermelés az egyik legfontosabb helyi iparág."
      ],
      ro: [
        "Faimos pentru producerea ovinelor și mieilor de înaltă calitate (Miel de Molo).",
        "Producția de lemn este o industrie locală majoră."
      ],
      en: [
        "Famed for producing high-quality sheep and lamb (Molo Lamb).",
        "Timber production is a major local industry."
      ]
    }, image: "/poi-images/molo-cities-v2.webp"},
  {
    id: "ngong-cities-v2",
    type: "city",
    parent: "KE-34",
    coords: [36.6566, -1.3621],
    name: {
      de: "Ngong",
      hu: "Ngong",
      ro: "Ngong",
      en: "Ngong",
image: "/poi-images/ngong-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Ngong liegt malerisch am Fuße der spektakulären Ngong-Hügel (Ngong Hills) im Kajiado County. Einst war es ein kleines Dorf der Maasai und ein beliebtes Erholungsgebiet für europäische Kolonialisten (berühmt geworden durch Karen Blixens Buch \"Jenseits von Afrika\"). Heute ist Ngong faktisch ein sehr beliebter, rapidly wachsender Vorort von Nairobi. Die steilen Hügelkuppen der Ngong Hills erheben sich bis auf 2.460 Meter, bieten einen atemberaubenden Blick über das Rift Valley im Westen und Nairobi im Osten und sind heute Standort von Kenias größtem Windpark. Trotz der massiven Urbanisierung bleibt Ngong ein wichtiger Knotenpunkt für Vieh- und Fleischhandel und ein Trainingsparadies für Langstreckenläufer, die das anspruchsvolle Terrain nutzen (Geografie & Literatur K8).",
      hu: "Ngong a híres Ngong-hegyek lábánál épült város, Nairobi délnyugati határában. A név maszáj nyelven 'bütyköket' jelent, utalva a hegyek négy látványos csúcsára, amelyek uralják a láthatárt. Ngong az angol telepesek idején kedvelt lovas- és vadászterület volt, amit Karen Blixen 'Volt egy farmom Afrikában' című könyve is megörökített. Mára dinamikus ingázóvárossá vált, de a környező hegyek peremén ma is szélerőművek és sétaútvonalak vonzzák a kirándulókat. (Történelem K7 — gyarmati irodalom és agglomeráció)",
      ro: "Ngong este un oraș dinamic și rezidențial aflat chiar la marginea sud-vestică a capitalei Nairobi, situat la baza faimoaselor Dealuri Ngong. Orașul este martorul unei tranziții uriașe; dintr-o așezare rustică, dominată de cultura pastorală Maasai, a devenit un oraș de navetiști cu o creștere explozivă. Dealurile cu același nume, cu cele patru vârfuri distinctive, reprezintă un magnet pentru drumeții pasionați de escapade de weekend și pentru alergătorii profesioniști care vin să se antreneze la mare altitudine. Orașul se remarcă printr-un amestec inedit de influențe tribale și urbanizare corporativă modernă.",
      en: "Ngong is a rapidly expanding commuter town located in Kajiado County, positioned at the foothills of the scenic Ngong Hills just southwest of Nairobi. Historically a deeply rural Maasai settlement, its name derives from the Maasai word 'enkong'u' meaning 'knuckles', describing the distinct shape of the adjacent hills. Sitting at an elevation of 1,960 meters, Ngong enjoys a cool climate and spectacular panoramic views of the Great Rift Valley. Over the last two decades, the town has transitioned explosively into a major residential suburb, absorbing massive spillover populations from the capital. Despite this urbanization, the nearby Ngong Hills remain a highly protected forest reserve, famous as a training ground for elite athletes and the site of a major wind power generation farm. Geography K7 — Topography and suburban expansion."
    },
    factsAdvanced: {
      en: ["Located at the eastern foothills of the Ngong Hills at an elevation of 1,960 meters.", "The name 'Ngong' translates to 'knuckles' in the local Maasai language.", "A major, rapidly growing commuter suburb for the Nairobi metropolitan area.", "The adjacent Ngong Hills host a significant national wind power generation facility.", "Historically the home of Danish author Karen Blixen during the colonial era.", "The hills serve as a famous high-altitude training ground for long-distance runners.", "The urban population surged to approximately 156,000 in the 2019 census."]
    },
    
    description: {
      de: "Ngong ist eine Stadt, die in die Hügel südwestlich von Nairobi eingebettet ist und als Wohnvorort sehr beliebt ist.",
      hu: "Ngong egy Nairobitól délnyugatra, a dombok között megbúvó város, amely lakóövezetként nagyon kedvelt.",
      ro: "Ngong este un oraș cuibărit în dealurile la sud-vest de Nairobi, foarte favorizat ca suburbie rezidențială.",
      en: "Ngong is a town nested in the hills southwestern of Nairobi, highly favored as a residential suburb."
    },
    facts: {
      de: [
        "Die nahegelegenen Ngong-Hügel bieten malerische Wanderwege.",
        "Windkraftanlagen auf den Hügeln erzeugen Strom."
      ],
      hu: [
        "A közeli Ngong-dombok festői túraútvonalakat kínálnak.",
        "A dombokon szélenergia termel áramot."
      ],
      ro: [
        "Dealurile Ngong din apropiere oferă trasee de drumeție pitorești.",
        "Energia eoliană generează electricitate pe dealuri."
      ],
      en: [
        "The nearby Ngong Hills offer scenic hiking trails.",
        "Wind power generates electricity on the hills."
      ]
    }, image: "/poi-images/ngong-cities-v2.webp"},
  {
    id: "kisii-cities-v2",
    type: "city",
    parent: "KE-45",
    coords: [34.7733, -0.6811],
    name: {
      de: "Kisii",
      hu: "Kisii",
      ro: "Kisii",
      en: "Kisii",
image: "/poi-images/kisii-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kisii, auch liebevoll als \"Bosongo\" bekannt, ist das kommerzielle und kulturelle Herz des Gusii-Landes in Südwestkenia. Die Stadt zeichnet sich durch extrem fruchtbare Böden und ständigen Regenfall aus, was sie zu einem landwirtschaftlichen Powerhouse macht. Kisii ist das absolute Zentrum für den Anbau von Bananen in Kenia und ein Großproduzent von Tee und Kaffee. Eine Besonderheit der Region ist das im Vorort Tabaka abgbaute Speckstein-Mineral (Soapstone). Die Kisii-Specksteinschnitzereien – handgemachte Skulpturen in rosa, weißen und schwarzen Tönen – sind weltweit begehrt. Mit einer enorm hohen Bevölkerungsdichte wächst die Stadt rasant und ist ein Zentrum für Bildungseinrichtungen wie die Kisii University sowie Krankenhäuser (Wirtschaft & Kunst K7).",
      hu: "Kisii a Délnyugat-kenyai régió legjelentősebb gazdasági és közigazgatási motorja. A folyamatosan esős, rendkívül buja és zöld dombvidéken épült város a kisii (gusii) etnikum fővárosa. Kenya szappanokkő-faragásának (szteatit) egyetlen központja; az itt készült finom szobrokat a világ minden tájára exportálják. A város emellett hatalmas mennyiségű teát, kávét és banánt termel, dinamikus egyetemi élettel és magas népsűrűséggel jellemezhető. (Földrajz K7 — kőfaragás és trópusi dombvidék)",
      ro: "Kisii este principalul centru urban din regiunea deluroasă sud-vestică a Kenyei și inima economică a grupului etnic Gusii. Orașul se mândrește cu unele dintre cele mai mari densități de populație din Africa de Est, ceea ce aduce o efervescență comercială constantă pe străzile sale. Agricultura joacă un rol colosal; piețele imense din Kisii sunt inundate cu recolte locale de banane, avocado, ceai verde și cafea. Din punct de vedere cultural și industrial, orașul a câștigat apreciere mondială pentru măiestria artei cioplirii în piatră de săpun (soapstone), extrasă direct din carierele Tabaka situate în apropiere.",
      en: "Kisii is the bustling commercial and administrative capital of Kisii County, located in the fertile highlands of southwestern Kenya at an elevation of 1,700 meters. The town experiences robust, year-round rainfall and features deep volcanic soils, creating an intensely productive agricultural environment dominated by smallholder tea, coffee, and massive banana plantations. Renowned as a major cultural epicenter for the industrious Gusii people, the town is characterized by its extremely high population density and frantic commercial activity. It serves as a primary collection point for fresh produce destined for Kisumu and Nairobi. Kisii is also highly famous for its soapstone carving industry, notably centered in the nearby Tabaka region, which exports intricate sculptures globally. Economics K7 — Cash crops and artisanal crafts."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters for Kisii County.", "Located in the southwestern highlands at an elevation of 1,700 meters.", "Nationally famous for the extensive soapstone carving industry in nearby Tabaka.", "The surrounding region is characterized by exceptionally high rural population density.", "A major collection hub for bananas, tea, and coffee from Gusii smallholders.", "Kisii University was fully chartered as an independent institution in 2013.", "The urban center had a recorded population of approximately 112,000 in 2019."]
    },
    
    description: {
      de: "Kisii ist das städtische und kommerzielle Hauptzentrum des Gusii-Hochlands im Südwesten Kenias.",
      hu: "Kisii a Gusii-fennsík fő városi és kereskedelmi központja Délnyugat-Kenyában.",
      ro: "Kisii este principalul centru urban și comercial al zonelor înalte Gusii din sud-vestul Kenyei.",
      en: "Kisii is the main urban and commercial center of the Gusii Highlands in southwestern Kenya."
    },
    facts: {
      de: [
        "Bekannt für seine Speckstein-Schnitzerei-Industrie.",
        "Eine der am schnellsten wachsenden Städte in Westkenia."
      ],
      hu: [
        "Zsírkőfaragó iparáról ismert.",
        "Nyugat-Kenya egyik leggyorsabban növekvő városa."
      ],
      ro: [
        "Cunoscut pentru industria sculpturii în steatit.",
        "Unul dintre orașele cu cea mai rapidă creștere din vestul Kenyei."
      ],
      en: [
        "Known for its soapstone carving industry.",
        "One of the fastest growing towns in western Kenya."
      ]
    }, image: "/poi-images/kisii-cities-v2.webp"},
  {
    id: "hola-cities-v2",
    type: "city",
    parent: "KE-04",
    coords: [40.032, -1.4988],
    name: {
      de: "Hola",
      hu: "Hola",
      ro: "Hola",
      en: "Hola",
image: "/poi-images/hola-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Hola ist eine heiße, flache Wüstenstadt und das Verwaltungszentrum des Tana River Countys. Die Siedlung liegt direkt am Ufer des Tana-Flusses, der Kenias längster Fluss ist und als grüne Lebensader durch die sonst völlig semiaride Region fließt. Hola erlangte in den 1950er Jahren während des Mau-Mau-Aufstandes traurige Berühmtheit durch das Hola-Massaker in einem britischen Internierungslager, ein Ereignis, das den Unabhängigkeitskampf Kenias maßgeblich beeinflusste. Heute ist die Wirtschaft stark vom Bura und Hola Irrigation Scheme abhängig, riesigen Bewässerungsprojekten, die den Anbau von Baumwolle und Mais in der Wüste ermöglichen. Die Bevölkerung, dominiert von den Pokomo (Bauern) und den Orma und Wardei (Viehhirten), ist stark von den unberechenbaren Überschwemmungen des Flusses abhängig (Geschichte K8).",
      hu: "Hola a száraz és elzárt Tana River megye székhelye, amely a Tana-folyó forró, sivatagos alföldjén található. A település hírhedt történelmi szerepe az 1959-es 'Hola-tábori incidens' volt, amikor a brit őrök a Mau Mau lázadás több tucatnyi gyanúsítottját verték halálra a büntetőtáborban; ez a botrány gyorsította fel Kenya függetlenedését. Ma a város csendes mezőgazdasági központ, ahol a helyi lakosok a folyó vizét használva próbálnak termőföldeket művelni az aszályos környezetben. (Történelem K8 — gyarmati túlkapások és függetlenség)",
      ro: "Hola este un oraș arid dar istoric, situat pe malurile fluviului Tana, servind ca centru administrativ al județului Tana River. Este un oraș așezat într-o zonă fierbinte și uscată, populată în principal de comunitățile Pokomo, Wardey și Orma, al căror mijloc de trai variază între pescuitul în râu și păstoritul cămilelor. Orașul este adânc încrustat în istoria Kenyei din cauza \"Masacrului de la Hola\" din 1959, când deținuții politici din timpul rebeliunii Mau Mau au fost uciși brutal într-un lagăr de detenție britanic. Astăzi, guvernul susține regiunea prin proiecte masive de irigații (Tana River Primate Reserve).",
      en: "Hola is an isolated, culturally diverse town functioning as the administrative capital of Tana River County in eastern Kenya. Positioned on the western banks of the massive Tana River at an elevation of just 55 meters, the settlement endures a hot, semi-arid climate. Hola is historically infamous as the site of the Hola massacre in 1959, where British colonial authorities beat to death 11 detained Mau Mau fighters, an event that profoundly accelerated the push for Kenyan independence. Today, the town's economy is deeply intertwined with the Tana River, supporting irrigated agriculture, notably the Bura Irrigation and Settlement Project located slightly to the north. Culturally, it acts as a peaceful confluence for the Pokomo agriculturalists and Orma pastoralists. History K8 — Anti-colonial struggles."
    },
    factsAdvanced: {
      en: ["Serves as the administrative headquarters of Tana River County.", "Located directly on the banks of the Tana River at just 55 meters elevation.", "The site of the tragic Hola massacre in 1959 during the Mau Mau uprising.", "The local economy relies heavily on river-fed irrigated agriculture.", "Features a hot and semi-arid climate typical of the lower Tana basin.", "Acts as a cultural boundary between the Pokomo and Orma communities.", "The town's population was recorded at roughly 17,000 in the 2019 census."]
    },
    
    description: {
      de: "Hola, auch bekannt als Galole, ist die Hauptstadt des Tana River County in einer halbtrockenen Region.",
      hu: "Hola, más néven Galole, a Tana River megye fővárosa egy félszáraz régióban.",
      ro: "Hola, cunoscut și sub numele de Galole, este capitala comitatului Tana River într-o regiune semi-aridă.",
      en: "Hola, also known as Galole, is the capital of Tana River County in a semi-arid region."
    },
    facts: {
      de: [
        "Liegt am Ufer des Tana-Flusses.",
        "Ort eines berüchtigten kolonialen Internierungslagers im Jahr 1959."
      ],
      hu: [
        "A Tana folyó partján fekszik.",
        "Egy hírhedt gyarmati fogolytábor helyszíne 1959-ben."
      ],
      ro: [
        "Situat pe malurile râului Tana.",
        "Locul unei faimoase tabere de detenție colonială în 1959."
      ],
      en: [
        "Located on the banks of the Tana River.",
        "Site of a notorious colonial detention camp in 1959."
      ]
    }, image: "/poi-images/hola-cities-v2.webp"},
  {
    id: "webuye-cities-v2",
    type: "city",
    parent: "KE-39",
    coords: [34.7667, 0.6167],
    name: {
      de: "Webuye",
      hu: "Webuye",
      ro: "Webuye",
      en: "Webuye",
image: "/poi-images/webuye-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Webuye, in der Kolonialzeit als Broderick Falls bekannt, ist eine Industriestadt im Bungoma County im Westen Kenias. Die Stadt verdankt ihre Existenz weitgehend dem gewaltigen Nzoia-Fluss, der hier über die beeindruckenden Nabuyole-Wasserfälle stürzt. Jahrzehntelang war Webuye das Zentrum der kenianischen Papierindustrie durch die gigantische Pan African Paper Mills Fabrik. Als die Fabrik 2009 kollabierte, geriet die Stadt in eine schwere wirtschaftliche Depression, aus der sie sich durch landwirtschaftliche Diversifizierung (Zuckerrohr und Mais) langsam wieder erholt. Kulturell ist Webuye eng mit dem Volk der Bukusu (Luhya) verbunden und dient als wichtiges Transitdreieck auf dem Weg zur ugandischen Grenze bei Malaba (Wirtschaft & Geografie K8).",
      hu: "Webuye ipari és mezőgazdasági város Nyugat-Kenyában, a Nzoia-folyó mentén. A város nevét évtizedekig a Pan African Paper Mills papírgyár tette ismertté, amely Afrika egyik legnagyobb papírfeldolgozó üzeme volt, és a helyi gazdaság legfőbb motorjaként szolgált. Bár a gyár a 2000-es években gazdasági nehézségekkel küzdött, Webuye jelentősége megmaradt, mint a Kampala-Mombasa tranzitútvonal kulcsfontosságú állomása. A környéket sűrű luhja lakosság és cukornádültetvények jellemzik. (Földrajz K7 — nehézipar és tranzit)",
      ro: "Webuye este un oraș industrial localizat în județul Bungoma, având un trecut economic formidabil legat de fabricarea hârtiei. Pe malurile zgomotoase ale râului Nzoia, orașul s-a extins masiv după înființarea Pan African Paper Mills, cea mai mare fabrică de hârtie din regiunea est-africană a anilor '70. Deși industria grea a întâmpinat greutăți în ultimii ani, orașul rămâne un hub de transport datorită intersecției cu Marea Șosea de Nord și liniei ferate spre Uganda. Dincolo de industrie, turiștii sunt fermecați de spectaculoasa cascadă Nabuyole (Broderick Falls), un loc mistic în tradițiile locale Bukusu.",
      en: "Webuye is a heavily industrialized town located in Bungoma County, western Kenya, historically synonymous with the massive Pan African Paper Mills established in 1974. Situated along the major highway connecting Kenya to Uganda at an elevation of 1,525 meters, the town is physically divided by the fast-flowing Nzoia River, which powers local hydroelectric infrastructure. For decades, the paper mill was the absolute economic engine of the region, utilizing the nearby Kakamega Forest and massive pine plantations for pulp before falling into severe financial decline in the 2000s. Despite the industrial slump, Webuye remains a vital transport and agricultural transit hub, famous locally for the scenic Broderick Falls located just outside the urban center. Economics K8 — Industrial boom and decline."
    },
    factsAdvanced: {
      en: ["Historically dominated by the Pan African Paper Mills, opened in 1974.", "Located along the primary highway corridor to the Ugandan border.", "Bisected by the Nzoia River, featuring the scenic Broderick Falls.", "Situated at an altitude of approximately 1,525 meters (5,003 feet).", "The local economy has shifted toward agriculture and transit logistics.", "Features the nearby Chetambe's Fort, a historical Bukusu defensive site.", "The urban population was roughly 43,000 according to the 2019 census."]
    },
    
    description: {
      de: "Webuye ist eine Industriestadt im Bungoma County, hauptsächlich bekannt für ihre stillgelegte Papierfabrik.",
      hu: "Webuye egy ipari város Bungoma megyében, amelyet elsősorban megszűnt papírgyáráról ismernek.",
      ro: "Webuye este un oraș industrial în comitatul Bungoma, recunoscut în principal pentru fabrica sa de hârtie desființată.",
      en: "Webuye is an industrial town in Bungoma County, primarily recognized for its defunct paper mill."
    },
    facts: {
      de: [
        "War einst das Epizentrum der Papierproduktion in Ostafrika.",
        "Liegt in der Nähe der Broderick Falls."
      ],
      hu: [
        "Egykor a kelet-afrikai papírgyártás epicentruma volt.",
        "A Broderick-vízesés közelében fekszik."
      ],
      ro: [
        "A fost odată epicentrul producției de hârtie în Africa de Est.",
        "Situat lângă Cascada Broderick."
      ],
      en: [
        "Was once the epicenter of paper production in East Africa.",
        "Located near the Broderick Falls."
      ]
    }, image: "/poi-images/webuye-cities-v2.webp"},
  {
    id: "kimilili-cities-v2",
    type: "city",
    parent: "KE-39",
    coords: [34.7214, 0.7831],
    name: {
      de: "Kimilili",
      hu: "Kimilili",
      ro: "Kimilili",
      en: "Kimilili",
image: "/poi-images/kimilili-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Kimilili ist ein lebhaftes Agrar- und Handelszentrum im Bungoma County, eingebettet an den südlichen Ausläufern des mächtigen Mount Elgon. Die Stadt profitiert massiv von den extrem fruchtbaren vulkanischen Böden des Berges und den sehr zuverlässigen, hohen Niederschlägen. Kimilili ist das landwirtschaftliche Herzstück der Region und produziert riesige Mengen an Mais, Bohnen, Zwiebeln und zunehmend auch Kaffee. Historisch war die Region stark vom traditionellen Handel der lokalen Bukusu-Bevölkerung geprägt. Der nahegelegene Mount Elgon bietet eine dramatische Kulisse und versorgt die zahllosen Flüsse und Bäche, die durch das Gebiet fließen, ganzjährig mit frischem Wasser (Geografie K7).",
      hu: "Kimilili egy lüktető mezőgazdasági központ az Elgon-hegy déli lejtőinek lábánál, Bungoma megyében. A vulkáni eredetű, mély és rendkívül termékeny talaj, valamint a folyamatos esőzések ideálissá teszik a kávé, a kukorica és a napraforgó termesztésére. A város gazdasága szinte teljes egészében ezen agrártermékek begyűjtésén, feldolgozásán és szállításán alapul. Kimilili a bukusu luhják egyik legfontosabb szellemi központja, hagyományos rítusaik és kultúrájuk máig meghatározó. (Földrajz K6 — vulkáni talajok és mezőgazdaság)",
      ro: "Kimilili este un oraș agricol pitoresc situat în nordul județului Bungoma, sprijinit chiar de pantele fertile și abrupte ale impresionantului Munte Elgon. Acesta este un oraș esențial pentru comunitatea Bukusu, un subgrup al populației Luhya. Vastele zone din jurul Kimililiului sunt acoperite cu plantații verzi de porumb, boabe de cafea și tutun, fermierii beneficiind de ploi montane frecvente și un sol bogat. Deși relativ mic, orașul pulsează de activitate în timpul campaniilor de recoltare și este cunoscut pentru ceremoniile anuale tradiționale, colorate, care celebrează circumcizia tinerilor băieți.",
      en: "Kimilili is a highly productive agricultural town situated on the lush southern slopes of Mount Elgon within Bungoma County. Enjoying a cool, high-altitude climate at roughly 1,700 meters, the town sits in one of Kenya's most fertile agricultural zones. It is a massive collection point for regional maize, beans, and coffee, acting as a crucial food basket for the broader western province. Culturally, Kimilili is deeply rooted in the traditions of the Bukusu people, a prominent sub-tribe of the Luhya, and serves as a focal point for their elaborate biennial circumcision ceremonies. Despite its rural setting, the town has experienced rapid demographic and infrastructural growth, bolstered by the paving of the main road connecting it to the larger towns of Webuye and Kitale. Geography K6 — Slope agriculture and food security."
    },
    factsAdvanced: {
      en: ["Located on the fertile southern slopes of Mount Elgon.", "Positioned at a high elevation of roughly 1,700 meters (5,577 feet).", "A major regional collection point for massive maize and coffee harvests.", "Functions as a profound cultural center for the Bukusu community.", "Famous for hosting massive, highly synchronized biennial circumcision festivals.", "Benefits from deep volcanic soils and extremely consistent annual rainfall.", "The town recorded a population of roughly 56,000 in the 2019 census."]
    },
    
    description: {
      de: "Kimilili ist eine florierende Agrarstadt in der Nähe der Hänge des Mount Elgon.",
      hu: "Kimilili egy virágzó mezőgazdasági város a Mount Elgon lejtőinek közelében.",
      ro: "Kimilili este un oraș agricol înfloritor în apropierea pantelor Muntelui Elgon.",
      en: "Kimilili is a thriving agricultural town near the slopes of Mount Elgon."
    },
    facts: {
      de: [
        "Dient als wichtiges Handelszentrum für Mais und Bohnen.",
        "Hat im letzten Jahrzehnt ein schnelles kommerzielles Wachstum verzeichnet."
      ],
      hu: [
        "Jelentős kukorica- és babkereskedelmi központként szolgál.",
        "Az elmúlt évtizedben gyors kereskedelmi növekedést tapasztalt."
      ],
      ro: [
        "Servește ca un important centru comercial pentru porumb și fasole.",
        "A cunoscut o creștere comercială rapidă în ultimul deceniu."
      ],
      en: [
        "Serves as a major maize and bean trading center.",
        "Has seen rapid commercial growth over the last decade."
      ]
    }, image: "/poi-images/kimilili-cities-v2.webp"},
  {
    id: "makuyu-cities-v2",
    type: "city",
    parent: "KE-21",
    coords: [37.28, -0.7497],
    name: {
      de: "Makuyu",
      hu: "Makuyu",
      ro: "Makuyu",
      en: "Makuyu",
image: "/poi-images/makuyu-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Makuyu ist eine landwirtschaftlich geprägte Stadt im Murang'a County in der Zentralregion Kenias. Die Region liegt an der wichtigen Thika-Nyeri Autobahn und zeichnet sich durch ein etwas trockeneres Klima aus als die höher gelegenen Teezonen von Murang'a. Historisch und bis in die Gegenwart ist Makuyu weltberühmt für seine endlosen Sisalplantagen und massiven Ananasfarmen, von denen viele zum multinationalen Konzern Del Monte gehören. Neben diesen Cash-Crops wird auf den Großfarmen auch Macadamia und Kaffee angebaut. Makuyu hat sich als logistisches Bindeglied zwischen Nairobi und dem zentralen kenianischen Hochland entwickelt und profitiert stark vom stetigen Ausbau der Straßeninfrastruktur (Wirtschaft K7).",
      hu: "Makuyu egy csendesebb mezőgazdasági település a Murang'a megyében, a Középső Tartomány szívében. Történelmileg a hatalmas, európai kézben lévő szizál- és kávéültetvények tették híressé, amelyek közül sok még ma is működik. A Nairobiból északra, a Kenya-hegy felé tartó fő autópálya mentén fekszik, így fontos pihenőhely a teherforgalom számára. A város lakossága, elsősorban kikujuk, a nagyüzemi farmokon dolgozik vagy saját kisbirtokain gazdálkodik. (Földrajz K6 — szizál és kávé termesztés)",
      ro: "Makuyu este un orășel situat pe pantele ondulate ale județului Murang'a, la nord-est de Nairobi, pe principala autostradă Nairobi-Nyeri. Spre deosebire de alte orașe ale regiunii centrale, care sunt renumite pentru ceai, Makuyu a dobândit renume național datorită întinselor livezi de fructe; culturile masive de mango, macadamia, avocado și portocale colorează peisajul înconjurător. Orașul a fost un punct de focalizare în epoca colonială pentru marile exploatații de cafea și sisal, iar astăzi reprezintă un model vibrant al tranziției agrare spre exportul fructelor tropicale proaspete către piețele internaționale.",
      en: "Makuyu is an agricultural settlement positioned prominently along the bustling Nairobi-Nyeri highway in Murang'a County, central Kenya. Historically established as a center for massive European-owned coffee and sisal plantations during the colonial era, the town's landscape is still heavily defined by vast, meticulously organized estates. Located at an altitude of 1,400 meters, Makuyu enjoys a moderately cool climate that transitions smoothly from the intense highland cold to the warmer plains stretching eastward. In recent years, the town has become a critical logistical node due to its location on the expanded dual-carriageway, attracting modern warehousing and agro-processing facilities. It is also home to the massive Kakuzi Limited farm, one of Kenya's largest exporters of avocados and macadamia nuts. Economics K7 — Large-scale estate farming."
    },
    factsAdvanced: {
      en: ["Positioned strategically along the main Nairobi-Nyeri highway.", "Located at an elevation of roughly 1,400 meters (4,593 feet).", "Historically developed around massive colonial sisal and coffee plantations.", "Home to sections of Kakuzi Limited, a massive avocado exporting corporation.", "Functions as a transition zone between the central highlands and eastern plains.", "The local economy relies heavily on wage labor on adjacent agricultural estates.", "The urban center had an estimated population of 43,000 in the 2019 census."]
    },
    
    description: {
      de: "Makuyu ist eine kleine, ruhige Stadt entlang der Autobahn Nairobi-Nyeri im Murang'a County.",
      hu: "Makuyu egy kis, csendes város a Nairobi-Nyeri autópálya mentén Murang'a megyében.",
      ro: "Makuyu este un oraș mic și liniștit situat de-a lungul autostrăzii Nairobi-Nyeri din comitatul Murang'a.",
      en: "Makuyu is a small, quiet town located along the Nairobi-Nyeri highway in Murang'a County."
    },
    facts: {
      de: [
        "Umgeben von riesigen Macadamianuss- und Kaffeefarmen.",
        "Ein häufiger Zwischenstopp für Reisende auf dem Weg zum Mount Kenya."
      ],
      hu: [
        "Hatalmas makadámdió- és kávéültetvények veszik körül.",
        "Gyakori megállóhely a Mount Kenya felé tartó utazók számára."
      ],
      ro: [
        "Înconjurat de vaste ferme de nuci macadamia și cafea.",
        "Un punct de oprire comun pentru călătorii care se îndreaptă spre Muntele Kenya."
      ],
      en: [
        "Surrounded by vast macadamia nut and coffee farms.",
        "A common stopover point for travelers heading to Mount Kenya."
      ]
    }, image: "/poi-images/makuyu-cities-v2.webp"},
  {
    id: "runyenjes-cities-v2",
    type: "city",
    parent: "KE-14",
    coords: [37.556, -0.4283],
    name: {
      de: "Runyenjes",
      hu: "Runyenjes",
      ro: "Runyenjes",
      en: "Runyenjes",
image: "/poi-images/runyenjes-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Runyenjes ist eine malerische und historisch gewachsene Stadt im Embu County, die an den üppigen östlichen Hängen des Mount Kenya liegt. Die Stadt wurde nach einem berühmten vorkolonialen Chief der Embu, Chief Runyenje, benannt. Dank der vulkanischen Böden und der hohen, verlässlichen Niederschlagsmengen ist die Region extrem fruchtbar. Die lokalen Kleinbauern konzentrieren sich stark auf den Anbau von hochwertigem Arabica-Kaffee, Tee und Macadamia-Nüssen, die direkt für den Exportmarkt bestimmt sind. Die hügelige Topografie, die von tiefen Tälern und eiskalten Flüssen durchzogen ist, verleiht der Stadt einen ruhigen, ländlichen Charakter, der im starken Kontrast zu den Ebenen im Osten steht (Landwirtschaft & Geschichte K7).",
      hu: "Runyenjes egy tipikus hegyvidéki agrárváros az Embu megyében, a Kenya-hegy keleti lábánál. A település hírnevét kiváló minőségű, árnyékban termesztett kávéjának és makadámdió-termésének köszönheti. A meredek, sűrű növényzetű domboldalakon kisparcellás gazdálkodás zajlik, amely az embu nép hagyományos életmódját tükrözi. A város központjában nyüzsgő piac működik, ahol a hegyről érkező friss terményeket azonnal értékesítik a felvásárlóknak. (Földrajz K6 — dombvidéki piacok)",
      ro: "Runyenjes este o așezare agitată din județul Embu, înrădăcinată adânc pe versanții estici extrem de productivi ai Muntelui Kenya. Orașul funcționează ca inima pulsantă a activității agricole din regiune, fermierii locali bazându-se pe cultura arborelui de cafea, a ceaiului negru aromat și a nucilor de macadamia. Datorită altitudinii ridicate, aerul este pur, iar solul vulcanic fertilizează nenumărate parcele terasate. Piața din oraș este renumită pentru abundența de produse proaspete. Mai mult, Runyenjes acționează ca un nucleu istoric vital pentru grupul etnic Embu, păstrând o moștenire bogată a tradițiilor rurale.",
      en: "Runyenjes is a picturesque agricultural town located on the intensely green eastern slopes of Mount Kenya within Embu County. Sitting at an altitude of approximately 1,450 meters, the town is characterized by its steeply terraced hillsides, meticulously cultivated by the local Embu community. Benefiting from exceptionally fertile volcanic soils and high annual rainfall fed by the mountain's microclimate, Runyenjes is an absolute powerhouse for smallholder coffee, tea, and macadamia nut production. The town also serves as a crucial staging point for the less-frequented eastern climbing routes up Mount Kenya. Historically a quiet farming community, it was elevated to municipal status in the late 20th century and has since seen steady infrastructural development, anchoring the county's agricultural wealth. Geography K6 — Terraced slope agriculture."
    },
    factsAdvanced: {
      en: ["Located on the steep eastern slopes of Mount Kenya at 1,450 meters.", "A premier regional center for high-quality Arabica coffee production.", "The surrounding terrain is characterized by intense, terraced smallholder farming.", "Serves as an access point for the Chogoria climbing route up Mount Kenya.", "The local economy is heavily bolstered by macadamia nut cultivation.", "Positioned roughly 25 kilometers from the county capital, Embu town.", "The town's population was recorded at approximately 61,000 in the 2019 census."]
    },
    
    description: {
      de: "Runyenjes ist eine Agrarstadt im Embu County, geprägt durch steile Hügel und fruchtbare Täler.",
      hu: "Runyenjes egy mezőgazdasági város Embu megyében, amelyet meredek dombok és termékeny völgyek jellemeznek.",
      ro: "Runyenjes este un oraș agricol din comitatul Embu, caracterizat de dealuri abrupte și văi fertile.",
      en: "Runyenjes is an agricultural town in Embu County, characterized by steep hills and fertile valleys."
    },
    facts: {
      de: [
        "Die lokale Wirtschaft stützt sich auf Tee und Kaffee.",
        "Benannt nach einem legendären lokalen Häuptling."
      ],
      hu: [
        "A helyi gazdaság a teára és a kávéra támaszkodik.",
        "Egy legendás helyi törzsfőnökről nevezték el."
      ],
      ro: [
        "Economia locală se bazează pe ceai și cafea.",
        "Numit după o căpetenie locală legendară."
      ],
      en: [
        "The local economy relies on tea and coffee.",
        "Named after a legendary local chief."
      ]
    }
  },
  {
    id: "mariakani-cities-v2",
    type: "city",
    parent: "KE-03",
    coords: [39.4678, -3.8642],
    name: {
      de: "Mariakani",
      hu: "Mariakani",
      ro: "Mariakani",
      en: "Mariakani",
image: "/poi-images/mariakani-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Mariakani ist ein boomendes Transit- und Industriezentrum, das an der Grenze zwischen dem Kilifi und dem Kwale County im Hinterland der kenianischen Küste liegt. Die Stadt ist landesweit bekannt für die Mariakani Weighbridge, eine der wichtigsten und geschäftigsten LKW-Waagen auf der Mombasa-Nairobi Autobahn. Aufgrund dieser Waage halten täglich tausende von Lastwagen in Mariakani, was eine florierende Dienstleistungs- und Handelswirtschaft (Restaurants, Werkstätten, Unterkünfte) entstehen ließ. Neben dem Logistiksektor entwickelt sich Mariakani zunehmend zu einem industriellen Vorort von Mombasa; Unternehmen der Stahl- und Zementindustrie haben sich hier angesiedelt, da das Land günstiger ist als im Hafengebiet. Die traditionelle Bevölkerung besteht vorwiegend aus der Giriama-Ethnie (Wirtschaft K8).",
      hu: "Mariakani egy dinamikus logisztikai és ingázóváros Mombasától nem messze a szárazföld belseje felé. Kulcsfontosságú stratégiai helyszín, itt található a Mariakani hídmérleg állomás, ahol minden Mombasa kikötőjéből induló nehézgépjárművet megmérnek. Ez a teherforgalom határozza meg a város életét és gazdaságát. Emellett a Kenyai Védelmi Erők jelentős katonai bázist is fenntartanak a város szélén. (Földrajz K7 — kikötői logisztika és teherforgalom)",
      ro: "Mariakani este un oraș de importanță strategică deosebită, situat fix pe granița a două județe de coastă, Kilifi și Kwale. Având o așezare crucială pe marea Autostradă Mombasa-Nairobi, orașul este celebru pe plan național pentru faimoasa stație de cântărire a vehiculelor de mare tonaj, unde sute de camioane așteaptă zilnic verificările. În plus, Mariakani găzduiește o bază militară terestră și se laudă cu un sector de producție al lactatelor destul de neobișnuit pentru climatul costier arid, mulțumită fermelor de vite ale comunităților de păstori nomazi ce trăiesc în zonele lăturalnice.",
      en: "Mariakani is a vital logistical and industrial town situated approximately 36 kilometers inland from Mombasa along the primary Nairobi-Mombasa highway. Straddling the border between Kilifi and Kwale counties, the town serves as a crucial weighbridge and inspection checkpoint for thousands of cargo trucks departing the Port of Mombasa daily. This immense volume of transit traffic forms the absolute backbone of the local informal economy. In recent decades, Mariakani has industrialized significantly, hosting major steel rolling mills, cement packaging facilities, and a massive military garrison belonging to the Kenya Defence Forces. The landscape is notably drier and hotter than the immediate coastline, characterized by resilient scrubland and baobab trees. Economics K7 — Freight logistics and weighbridges."
    },
    factsAdvanced: {
      en: ["Located 36 kilometers northwest of Mombasa on the primary transport corridor.", "Hosts the critical Mariakani Weighbridge, inspecting all port-bound/departing cargo.", "Straddles the administrative boundary between Kilifi and Kwale counties.", "Features a major military barracks for the Kenya Defence Forces.", "Has rapidly industrialized with the establishment of steel and cement plants.", "The climate is significantly drier and hotter than the immediate coastal strip.", "The urban population surged to roughly 89,000 in the 2019 national census."]
    },
    
    description: {
      de: "Mariakani ist eine Transit- und Industriestadt an der Grenze zwischen den Countys Kilifi und Kwale.",
      hu: "Mariakani egy tranzit- és ipari város a Kilifi és Kwale megyék határán.",
      ro: "Mariakani este un oraș de tranzit și industrial care se întinde pe granița dintre comitatele Kilifi și Kwale.",
      en: "Mariakani is a transit and industrial town straddling the border between Kilifi and Kwale counties."
    },
    facts: {
      de: [
        "Verfügt über eine große Waage für schwere Nutzfahrzeuge.",
        "Beherbergt eine große Militärkaserne."
      ],
      hu: [
        "Egy fontos hídmérleggel rendelkezik nehéz haszongépjárművek számára.",
        "Egy nagy katonai laktanya található itt."
      ],
      ro: [
        "Dispune de un cântar auto major pentru vehicule comerciale grele.",
        "Găzduiește o mare cazarmă militară."
      ],
      en: [
        "Features a major weighbridge for heavy commercial vehicles.",
        "Hosts a large military barracks."
      ]
    }, image: "/poi-images/mariakani-cities-v2.webp"},
  {
    id: "awendo-cities-v2",
    type: "city",
    parent: "KE-44",
    coords: [34.5269, -0.9067],
    name: {
      de: "Awendo",
      hu: "Awendo",
      ro: "Awendo",
      en: "Awendo",
image: "/poi-images/awendo-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Awendo, oft als \"Green Town\" bezeichnet, liegt im Migori County im Südwesten Kenias und ist untrennbar mit der kenianischen Zuckerindustrie verbunden. Die gesamte Stadtentwicklung basiert auf der South Nyanza Sugar Company (SONY Sugar), deren massive Fabrik und endlose Zuckerrohrfelder das Landschaftsbild und die Wirtschaft dominieren. SONY Sugar wurde in den späten 1970er Jahren gegründet und bietet Tausenden von Einheimischen direkte und indirekte Beschäftigung. Awendo ist kulturell divers, obwohl es primär im Gebiet der Luo liegt, arbeiten hier Menschen aus allen Teilen des Landes. Die grünen Zuckerrohrplantagen erstrecken sich über weite Hügel und machen die Stadt zum landwirtschaftlichen Rückgrat des Countys (Wirtschaft K8).",
      hu: "Awendo egy monokultúrás iparváros Migori megyében, amelynek teljes élete a cukornádra épül. A Sony Sugar Company, Dél-Nyanza legnagyobb cukorgyára a város központjában üzemel, és ezreknek biztosít közvetlen vagy közvetett megélhetést. A környező lankás dombvidéket, ameddig a szem ellát, hatalmas cukornádültetvények borítják. Bár a város gyorsan növekszik a feldolgozóiparnak köszönhetően, jelentős kihívást jelent a helyi kisbirtokosok és az iparigigász gazdasági egyensúlyának fenntartása. (Földrajz K6 — feldolgozóipar és cukornád)",
      ro: "Awendo este un orășel situat în provincia Nyanza de Sud, în cadrul județului Migori, și este universal asociat cu un singur produs: zahărul. Peisajul din împrejurimi este complet înghițit de plantațiile masive și fermele mici pline de trestie de zahăr, care alimentează neîntrerupt utilajele fabricii de rafinare South Nyanza Sugar Company (Sony Sugar). Fabrica reprezintă fundamentul economiei locale, generând locuri de muncă pentru mii de tineri și susținând școlile și infrastructura sportivă a comunității Luo. În timpul sezonului ploios, micul oraș prinde viață, transformându-se într-un labirint plin cu tractoare.",
      en: "Awendo is an agriculturally dominant town located in Migori County, western Kenya, historically defined by the massive South Nyanza Sugar Company (SONY Sugar). Established in 1979, the sugar milling factory is the undeniable economic engine of the town, processing sugarcane harvested from thousands of surrounding smallholder farms. Situated at an elevation of roughly 1,450 meters within the fertile Lake Victoria basin, the town enjoys a warm, humid climate ideal for continuous sugarcane cultivation. The town's layout is heavily influenced by the industrial infrastructure, with vast company-owned estates blending into private farmlands. Beyond sugar, the local economy is supported by robust retail commerce serving the densely populated, predominantly Luo rural hinterland. Economics K7 — Mono-crop industrial reliance."
    },
    factsAdvanced: {
      en: ["The local economy is almost entirely anchored by the SONY Sugar Company.", "The massive sugar milling factory was established by the government in 1979.", "Located in Migori County at an elevation of roughly 1,450 meters.", "Surrounded by thousands of acres of smallholder and corporate sugarcane farms.", "Situated along the primary highway connecting Kisii to the Tanzanian border.", "The town serves as a commercial center for the predominantly Luo local population.", "The urban area recorded a population of roughly 17,000 in the 2019 census."]
    },
    
    description: {
      de: "Awendo ist eine zuckerproduzierende Stadt im Migori County im Westen Kenias.",
      hu: "Awendo egy cukortermelő város Migori megyében, Nyugat-Kenyában.",
      ro: "Awendo este un oraș producător de zahăr situat în comitatul Migori din vestul Kenyei.",
      en: "Awendo is a sugar-producing town located in Migori County in western Kenya."
    },
    facts: {
      de: [
        "Erbaut um die Fabrik der Sony Sugar Company.",
        "Umgeben von riesigen Zuckerrohrfeldern."
      ],
      hu: [
        "A Sony Sugar Company gyára köré épült.",
        "Hatalmas cukornádföldek veszik körül."
      ],
      ro: [
        "Construit în jurul fabricii Sony Sugar Company.",
        "Înconjurat de vaste câmpuri de trestie de zahăr."
      ],
      en: [
        "Built around the Sony Sugar Company factory.",
        "Surrounded by vast fields of sugarcane."
      ]
    }, image: "/poi-images/awendo-cities-v2.webp"},
  {
    id: "litein-cities-v2",
    type: "city",
    parent: "KE-35",
    coords: [35.1833, -0.5333],
    name: {
      de: "Litein",
      hu: "Litein",
      ro: "Litein",
      en: "Litein",
image: "/poi-images/litein-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Litein ist eine florierende Agrarstadt im Kericho County, die tief in den üppigen Teegebieten des westlichen Rift Valleys liegt. Umgeben von atemberaubenden, sattgrünen Hügeln, profitiert Litein von einem kühlen Hochlandklima, das perfekte Bedingungen für den Anbau von schwarzem Tee bietet. Die Stadt wuchs in den letzten Jahrzehnten von einem kleinen Handelsposten zu einem wichtigen regionalen Zentrum heran, das von zahlreichen Tee-Fabriken der Kenya Tea Development Agency (KTDA) flankiert wird. Die Bevölkerung besteht primär aus den Kipsigis, die eine starke Tradition im genossenschaftlichen Kleinbauern-Modell pflegen. Litein ist nicht nur wirtschaftlich vital, sondern auch ein Zentrum für erstklassige Schulen und medizinische Einrichtungen in der Region (Wirtschaft K7).",
      hu: "Litein egy magasan fekvő, folyamatosan zöldellő város Kericho megyében, a teakörzet szívében. A települést végtelen teaültetvények veszik körül, amelyek biztosítják a helyi gazdaság alapját. A kipsigis közösség számára fontos oktatási és egészségügyi központ is egyben. Bár kisebb, mint Kericho, Litein intenzív kereskedelmi élettel bír, különösen a teafeldolgozó gyárakba (például a szomszédos Kaplitein vagy a Kiptagich üzemekbe) áramló napi levélszállítmányok miatt. (Földrajz K6 — agrárkereskedelem és teafeldolgozás)",
      ro: "Litein este o așezare mică, dar extrem de importantă din regiunea South Rift, situată în județul Kericho, la scurtă distanță de orașul Bomet. Ca majoritatea localităților din acest platou înalt, Litein trăiește și respiră prin cultivarea ceaiului; este complet înconjurat de văi de un verde intens, cu tufe de ceai așezate perfect la rând, ce seamănă cu un covor imens de catifea. Regiunea este renumită pentru recoltele de calitate superioară pe care marile fabrici multinaționale le exportă în Europa și Asia. Pe lângă importanța agricolă, orașul prezintă o demografie majoritar formată din tribul atletic Kipsigis.",
      en: "Litein is a rapidly expanding commercial and agricultural town situated in Kericho County within the highly fertile South Rift region. Positioned at a high elevation of approximately 1,900 meters, the town is surrounded by a breathtaking landscape of rolling hills entirely blanketed by intensely green tea plantations. Unlike the massive corporate estates near Kericho town, the tea around Litein is predominantly grown by thousands of independent smallholder farmers who supply local processing factories like the Litein Tea Factory. This decentralized agricultural wealth has fostered a highly robust local retail and banking sector, making Litein one of the wealthiest rural towns in the region. The climate is notoriously cool and exceptionally wet, raining nearly year-round. Economics K7 — Decentralized cash crop wealth."
    },
    factsAdvanced: {
      en: ["Located in the highly fertile South Rift highlands of Kericho County.", "Positioned at a cool, high elevation of approximately 1,900 meters.", "The local economy is driven massively by smallholder tea farming.", "Home to the prominent Litein Tea Factory, managed by local cooperatives.", "Experiences an exceptionally wet microclimate with near-daily rainfall.", "Has developed into a major regional hub for banking and agricultural retail.", "The urban population was roughly 93,000 according to the 2019 census."]
    },
    
    description: {
      de: "Litein ist eine wachsende Stadt im Kericho County, die stark von den umliegenden Teeplantagen abhängig ist.",
      hu: "Litein egy növekvő város Kericho megyében, amely nagymértékben függ a környező teaültetvényektől.",
      ro: "Litein este un oraș în creștere în comitatul Kericho, puternic dependent de plantațiile de ceai din jur.",
      en: "Litein is a growing town in Kericho County, heavily dependent on the surrounding tea estates."
    },
    facts: {
      de: [
        "Aufgrund der Höhenlage oft in kühlen Nebel gehüllt.",
        "Ein wichtiges lokales Handelszentrum für Teebauern."
      ],
      hu: [
        "A nagy magasság miatt gyakran hűvös köd borítja.",
        "Fontos helyi kereskedelmi központ a teatermelők számára."
      ],
      ro: [
        "Adesea acoperit de o ceață rece datorită altitudinii ridicate.",
        "Un important centru de comerț local pentru cultivatorii de ceai."
      ],
      en: [
        "Often covered in a cool mist due to high altitude.",
        "An important local commerce center for tea farmers."
      ]
    }, image: "/poi-images/litein-cities-v2.webp"},
  {
    id: "oyugis-cities-v2",
    type: "city",
    parent: "KE-43",
    coords: [34.7317, -0.5061],
    name: {
      de: "Oyugis",
      hu: "Oyugis",
      ro: "Oyugis",
      en: "Oyugis",
image: "/poi-images/oyugis-cities-v2.webp"
},
    descriptionAdvanced: {
      de: "Oyugis ist die zweitgrößte Stadt im Homa Bay County und liegt an einer wichtigen Kreuzung in der dicht besiedelten Nyanza-Region nahe dem Viktoriasee. Anders als die direkt am See gelegenen Städte ist Oyugis ein landwirtschaftlicher Knotenpunkt. Die fruchtbaren Böden in der hügeligen Umgebung eignen sich hervorragend für den Anbau von Mais, Bohnen, Süßkartoffeln und Ziegelherstellung aus lokalem Ton. Historisch ist die Stadt bekannt für den riesigen Freiluftmarkt, der Händler aus Kisii, Kisumu und Homa Bay anzieht. Nahe der Stadt liegt der mystische \"Simbi Nyaima\", ein vulkanischer Kratersee, der in der Luo-Mythologie als Überrest eines versunkenen Dorfes gilt und heute seltene Vogelarten wie Flamingos anzieht (Geografie & Kultur K7).",
      hu: "Oyugis a dél-nyanzai régió egyik legfontosabb kereskedelmi és agrár-csomópontja Homa Bay megyében. A buja növényzetű, dombos vidék hagyományosan a banán, az édesburgonya és a különféle babok termesztésének központja, de a napraforgó is egyre népszerűbb. A város az ország egyik legnagyobb nyitott piacával büszkélkedhet, ahová a környék luo gazdái hozzák be terményeiket. A közeli vulkáni dombok festői hátteret biztosítanak az élénk mezővárosnak. (Földrajz K6 — regionális piacok)",
      ro: "Oyugis, al doilea oraș ca mărime din județul Homa Bay, este un important centru de legătură agricol localizat pe traseul aglomerat dintre Kisii și Kisumu. Orașul este recunoscut în toată regiunea pentru faimoasa Piață Pelican, o structură comercială modernă ce reunește zilnic sute de comercianți. Aici se vând pește proaspăt adus de la Lacul Victoria, fructe și textile, formând coloana vertebrală a economiei micro-regionale. Cultura tradițională a comunității Luo se reflectă în festivalurile muzicale și religioase care au loc frecvent pe dealurile stâncoase înconjurătoare.",
      en: "Oyugis is a prominent commercial and agricultural town located in Homa Bay County, nestled in the fertile hills of the southern Nyanza region. Sitting at an altitude of approximately 1,450 meters, the town benefits from excellent rainfall and deep soils, supporting a highly diversified agricultural output that includes maize, beans, bananas, and increasingly, sweet potatoes. Oyugis is nationally famous for its massive open-air market, which operates twice a week and draws thousands of traders from across western Kenya, forming the absolute core of the local economy. Culturally, it is a deeply significant center for the local Luo community and serves as a critical transport junction connecting Kisii, Homa Bay, and Kisumu. Economics K6 — Regional open-air markets."
    },
    factsAdvanced: {
      en: ["Located in the fertile highland region of Homa Bay County.", "Situated at an elevation of approximately 1,450 meters (4,757 feet).", "Hosts one of the largest and busiest bi-weekly open-air markets in western Kenya.", "A major regional production zone for sweet potatoes and bananas.", "Serves as a crucial transport junction linking Kisii to Lake Victoria towns.", "Functions as a major cultural and commercial hub for the local Luo people.", "The town's population was recorded at approximately 42,000 in the 2019 census."]
    },
    
    description: {
      de: "Oyugis ist eine Landwirtschafts- und Handelsstadt im Homa Bay County nahe dem Viktoriasee.",
      hu: "Oyugis egy mezőgazdasági és kereskedelmi város Homa Bay megyében, a Viktória-tó közelében.",
      ro: "Oyugis este un oraș agricol și comercial din comitatul Homa Bay, lângă Lacul Victoria.",
      en: "Oyugis is an agricultural and commercial town in Homa Bay County, near Lake Victoria."
    },
    facts: {
      de: [
        "Ein bedeutendes Zentrum für den Handel mit Mais, Bohnen und Fisch.",
        "Historisch ein Rastplatz für Händler auf dem Weg zum See."
      ],
      hu: [
        "A kukorica, a bab és a hal kereskedelmének jelentős központja.",
        "Történelmileg a tóhoz tartó kereskedők pihenőhelye volt."
      ],
      ro: [
        "Un centru semnificativ pentru comerțul cu porumb, fasole și pește.",
        "Istoric un punct de odihnă pentru comercianții care se îndreptau spre lac."
      ],
      en: [
        "A significant center for trading maize, beans, and fish.",
        "Historically a resting point for traders heading to the lake."
      ]
    }
  }
];

