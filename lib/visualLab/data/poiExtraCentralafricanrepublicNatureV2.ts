// @ts-nocheck
import type { POI } from "./poi";

export const poiExtraCentralafricanrepublicNatureV2: POI[] = [
  {
    id: "ubangi-river-nature-v2",
    type: "river",
    parent: "CF-BGO",
    coords: [18.5583, 4.3611],
    name: {
      de: "Ubangi-Fluss",
      hu: "Ubangi folyó",
      ro: "Râul Ubangi",
      en: "Ubangi River"},
    image: "/poi-images/ubangi-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Ubangi ist der bedeutendste rechte Nebenfluss des Kongo in Zentralafrika und bildet auf weiten Strecken die Grenze zwischen der Demokratischen Republik Kongo und der Zentralafrikanischen Republik. Er entsteht aus dem Zusammenfluss der Flüsse Mbomou und Uele und erstreckt sich über eine Gesamtlänge von etwa 2270 Kilometern. Als lebenswichtige Verkehrsader ermöglicht er die Schifffahrt bis in die Hauptstadt Bangui, was besonders für den Binnenhandel der Region entscheidend ist. Die hydrologischen Zyklen des Flusses bestimmen den Rhythmus der Landwirtschaft und Fischerei für Millionen von Menschen in seinem Einzugsgebiet. In der Kolonialzeit diente der Ubangi als wichtiger Korridor für europäische Expeditionen und den Transport von Rohstoffen wie Kautschuk und Elfenbein. Heute steht der Fluss im Mittelpunkt großer Infrastrukturprojekte, wie dem geplanten Wassertransfer zum tschadischen See, um dessen Austrocknung entgegenzuwirken. Die dichten Regenwälder entlang seiner Ufer beherbergen eine reiche Flora und Fauna, sind jedoch durch Abholzung und illegalen Bergbau bedroht. Trotz saisonaler Pegelschwankungen bleibt der Ubangi ein unverzichtbarer Wirtschaftsfaktor für Zentralafrika.",
      hu: "Az Ubangi-folyó a Kongó legnagyobb jobb oldali mellékfolyója Közép-Afrikában, amely jelentős szakaszokon határt képez a Kongói Demokratikus Köztársaság és a Közép-afrikai Köztársaság között. A folyó a Mbomou és az Uele összefolyásával keletkezik, és teljes hossza eléri a 2270 kilométert. Stratégiai fontosságú közlekedési útvonal, amely lehetővé teszi a hajózást egészen Bangui városáig, ami kulcsfontosságú a régió kereskedelme számára. A folyó vízszintje jelentősen ingadozik az évszakok függvényében, ami meghatározza a helyi mezőgazdaság és halászat lehetőségeit. Történelmileg az Ubangi a gyarmati terjeszkedés és a nyersanyagszállítás egyik fő útvonala volt a 19. század végén. Napjainkban a folyó egy nagyszabású terv középpontjában áll, amely vizet szállítana a Csád-tóba annak megmentése érdekében. A part menti trópusi esőerdők gazdag élővilága veszélyben van az erdőirtás és az illegális bányászat miatt. Az Ubangi továbbra is alapvető erőforrás több millió ember számára, akik a folyótól függenek mindennapi megélhetésükben.",
      ro: "Râul Ubangi este cel mai mare afluent de dreapta al fluviului Congo în Africa Centrală, formând granița naturală între Republica Democrată Congo și Republica Centrafricană. Se formează prin confluența râurilor Mbomou și Uele și are o lungime totală de aproximativ 2270 de kilometri. Fluviul servește drept arteră vitală de transport, fiind navigabil până în capitala Bangui, ceea ce facilitează comerțul regional intens. Ciclurile hidrologice ale Ubangi influențează direct agricultura și pescuitul pentru populațiile locale care depind de resursele sale. În perioada colonială, a fost o rută principală pentru expedițiile europene și transportul de resurse naturale precum cauciucul și fildeșul. În prezent, râul este subiectul unui proiect ambițios de transfer de apă către Lacul Ciad pentru a preveni dispariția acestuia din urmă. Ecosistemele forestiere de-a lungul malurilor sale adăpostesc o biodiversitate remarcabilă, dar sunt amenințate de defrișări. Ubangi rămâne un element central al infrastructurii naturale din centrul continentului african.",
      en: "The Ubangi River is the largest right-bank tributary of the Congo River in Central Africa, forming a significant portion of the border between the DRC and the Central African Republic. It is formed by the confluence of the Mbomou and Uele rivers and stretches for a total length of approximately 2,270 kilometers. The river serves as a vital transportation artery, remaining navigable as far as the capital city of Bangui, which is crucial for regional commerce. Its hydrological cycles dictate the rhythm of agriculture and fishing for millions of people inhabiting its vast basin. Historically, the Ubangi was a primary corridor for European colonial expansion and the transport of raw materials like rubber and ivory in the late 19th century. Today, the river is at the heart of a major proposal to transfer water to Lake Chad to prevent its total desiccation. The dense rainforests lining its banks host a rich variety of flora and fauna, though they face threats from deforestation and illegal mining. The Ubangi remains an indispensable economic lifeline for the Central African region.",
      es: "El Ubangui es el afluente derecho más importante del Congo en África Central y forma, en gran parte de su recorrido, la frontera entre la República Democrática del Congo y la República Centroafricana. Nace de la confluencia de los ríos Mbomou y Uele y se extiende por una longitud total de unos 2.270 kilómetros. Como arteria de transporte vital, permite la navegación hasta la capital, Bangui, lo cual es crucial para el comercio interior de la región. Los ciclos hidrológicos del río determinan el ritmo de la agricultura y la pesca para millones de personas en su cuenca. En la época colonial, el Ubangui sirvió como un importante corredor para las expediciones europeas y el transporte de materias primas como el caucho y el marfil. Hoy en día, el río es el centro de grandes proyectos de infraestructura, como el planeado trasvase de agua al lago Chad para contrarrestar su desecación. Las densas selvas tropicales a lo largo de sus orillas albergan una rica flora y fauna, aunque están amenazadas por la deforestación y la minería ilegal. A pesar de las fluctuaciones estacionales del nivel del agua, el Ubangui sigue siendo un factor económico indispensable para África Central.",
      pt: "O Ubangui é o afluente direito mais importante do Congo na África Central e forma, em grande parte do seu percurso, a fronteira entre a República Democrática do Congo e a República Centro-Africana. Nasce da confluência dos rios Mbomou e Uele e estende-se por um comprimento total de cerca de 2.270 quilómetros. Como artéria de transporte vital, permite a navegação até à capital, Bangui, o que é crucial para o comércio interno da região. Os ciclos hidrológicos do rio determinam o ritmo da agricultura e da pesca para milhões de pessoas na sua bacia hidrográfica. Na época colonial, o Ubangui serviu como um importante corredor para as expedições europeias e o transporte de matérias-primas como a borracha e o marfim. Hoje, o rio está no centro de grandes projetos de infraestrutura, como a planeada transferência de água para o Lago Chade para contrariar a sua dessecação. As densas florestas tropicais ao longo das suas margens abrigam uma rica flora e fauna, mas estão ameaçadas pela desflorestação e pela mineração ilegal. Apesar das flutuações sazonais do nível da água, o Ubangui continua a ser um fator económico indispensável para a África Central.",
      fr: "L'Oubangui est le plus important affluent de la rive droite du Congo en Afrique centrale et forme, sur de longues distances, la frontière entre la République démocratique du Congo et la République centrafricaine. Il naît de la confluence des rivières Mbomou et Ouélé et s'étend sur une longueur totale d'environ 2 270 kilomètres. En tant que voie de transport vitale, il permet la navigation jusqu'à la capitale Bangui, ce qui est crucial pour le commerce intérieur de la région. Les cycles hydrologiques de la rivière déterminent le rythme de l'agriculture et de la pêche pour des millions de personnes dans son bassin versant. À l'époque coloniale, l'Oubangui a servi de corridor important pour les expéditions européennes et le transport de matières premières telles que le caoutchouc et l'ivoire. Aujourd'hui, la rivière est au cœur de grands projets d'infrastructure, comme le transfert d'eau prévu vers le lac Tchad pour contrer son assèchement. Les forêts tropicales denses le long de ses rives abritent une flore et une faune riches, mais sont menacées par la déforestation et l'exploitation minière illégale. Malgré les fluctuations saisonnières du niveau d'eau, l'Oubangui reste un facteur économique indispensable pour l'Afrique centrale.",
    },
    factsAdvanced: {
      de: ["Gesamtlänge von etwa 2270 Kilometern", "Gebildet durch den Zusammenfluss von Mbomou und Uele", "Wichtigster rechter Nebenfluss des Kongo-Flusses", "Navigierbar für große Schiffe bis nach Bangui", "Bildet die Grenze zwischen der DR Kongo und der ZAR", "Zentrum des geplanten Transaqua-Wassertransferprojekts"],
      hu: ["Teljes hossza körülbelül 2270 kilométer", "A Mbomou és az Uele folyók összefolyásából ered", "A Kongó-folyó legjelentősebb jobb oldali mellékfolyója", "Bangui városáig hajózható útvonalat biztosít", "Határfolyó a Kongói DK és a Közép-afrikai Köztársaság között", "A Csád-tó feltöltését célzó vízügyi terv része"],
      ro: ["Lungime totală de aproximativ 2270 de kilometri", "Format prin confluența râurilor Mbomou și Uele", "Cel mai mare afluent de dreapta al fluviului Congo", "Navigabil pentru nave mari până la orașul Bangui", "Constituie granița dintre RD Congo și Republica Centrafricană", "Piesă centrală în proiectul de salvare a Lacului Ciad"],
      en: ["Total length of approximately 2,270 kilometers", "Formed by the confluence of the Mbomou and Uele rivers", "Largest right-bank tributary of the Congo River", "Navigable for large vessels up to the city of Bangui", "Forms the border between the DRC and the CAR", "Key component of the proposed Lake Chad water transfer"],
      es: ["Longitud total de unos 2.270 kilómetros", "Formado por la confluencia de Mbomou y Uele", "Principal afluente derecho del río Congo", "Navegable para grandes barcos hasta Bangui", "Forma la frontera entre la RD del Congo y la RCA", "Centro del planeado proyecto de trasvase Transaqua"],
      pt: ["Comprimento total de cerca de 2.270 quilómetros", "Formado pela confluência de Mbomou e Uele", "Principal afluente direito do rio Congo", "Navegável para grandes navios até Bangui", "Forma a fronteira entre a RD do Congo e a RCA", "Centro do planeado projeto de transferência de água Transaqua"],
      fr: ["Longueur totale d'environ 2 270 kilomètres", "Formé par la confluence du Mbomou et de l'Ouélé", "Principal affluent de la rive droite du fleuve Congo", "Navigable pour les grands navires jusqu'à Bangui", "Forme la frontière entre la RD Congo et la RCA", "Centre du projet de transfert d'eau Transaqua prévu"],
    },
    
    description: {
      de: "Der größte rechte Nebenfluss des Kongo-Flusses bildet eine wichtige Lebensader und Grenzregion für das Land.",
      hu: "A Kongó folyó legnagyobb jobb oldali mellékága, az ország fontos élettere és határvidéke.",
      ro: "Cel mai mare afluent de dreapta al fluviului Congo formează o arteră vitală și o regiune de frontieră pentru țară.",
      en: "The largest right-bank tributary of the Congo River forms a major lifeline and border region for the country.",
      es: "El mayor afluente derecho del río Congo constituye una arteria vital y una región fronteriza para el país.",
      pt: "O maior afluente direito do rio Congo constitui uma artéria vital e uma região de fronteira para o país.",
      fr: "Le plus grand affluent de la rive droite du fleuve Congo constitue une artère vitale et une région frontalière pour le pays.",
    },
    facts: {
      de: ["Entsteht durch den Zusammenfluss von Mbomou und Uelle.", "Wichtiger Transportweg für den Handel."],
      hu: ["Az Mbomou és az Uelle folyók találkozásából ered.", "Fontos szállítási útvonal a kereskedelem számára."],
      ro: ["Format prin confluența râurilor Mbomou și Uelle.", "Rută importantă de transport pentru comerț."],
      en: ["Formed by the confluence of the Mbomou and Uele rivers.", "Crucial transportation route for trade."],
      es: ["Se forma por la confluencia de los ríos Mbomou y Uele.", "Importante ruta de transporte para el comercio."],
      pt: ["Formado pela confluência dos rios Mbomou e Uele.", "Importante rota de transporte para o comércio."],
      fr: ["Naît de la confluence du Mbomou et de l'Ouélé.", "Voie de transport importante pour le commerce."],
    }, image: "/poi-images/ubangi-river-nature-v2.webp"},
  {
    id: "sangha-river-nature-v2",
    type: "river",
    parent: "CF-SE",
    coords: [16.0500, 3.5167],
    name: {
      de: "Sangha-Fluss",
      hu: "Sangha folyó",
      ro: "Râul Sangha",
      en: "Sangha River"},
    image: "/poi-images/sangha-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Sangha-Fluss ist ein bedeutender rechter Nebenfluss des Kongo und fließt durch die Zentralafrikanische Republik, Kamerun und die Republik Kongo. Mit einer Gesamtlänge von etwa 790 Kilometern bildet er eine lebenswichtige Wasserstraße für den Transport von Holz und anderen Gütern in dieser abgelegenen Region. Geografisch entsteht der Sangha durch den Zusammenfluss der Flüsse Mambéré und Kadéï bei der Stadt Berbérati. Besonders bemerkenswert ist das grenzüberschreitende Schutzgebiet Sangha Trinational, das seit 2012 zum UNESCO-Welterbe gehört und drei Nationalparks umfasst. Das Einzugsgebiet ist durch dichten tropischen Regenwald geprägt, der eine außergewöhnliche Artenvielfalt beherbergt, darunter Waldelefanten und Flachlandgorillas. In der Kolonialzeit diente der Fluss als wichtige Route für den Kautschukhandel und die Exploration des inneren Afrikas. Heute spielt der Sangha eine zentrale Rolle im regionalen Ökosystem und ist entscheidend für das Überleben zahlreicher indigener Gemeinschaften entlang seiner Ufer.",
      hu: "A Sangha-folyó a Kongó egyik jelentős jobb oldali mellékfolyója, amely Közép-afrikai Köztársaság, Kamerun és a Kongói Köztársaság területén halad keresztül. Körülbelül 790 kilométeres hosszával kulcsfontosságú vízi utat biztosít a faanyag és egyéb áruk szállításához ebben az elszigetelt régióban. A folyó a Mambéré és a Kadéï folyók egyesüléséből születik Berbérati városának közelében. Különleges jelentőségét a Sangha Trinational természetvédelmi terület adja, amely 2012 óta az UNESCO Világörökség része, és három nemzeti parkot foglal magában. Vízgyűjtő területét sűrű trópusi esőerdő borítja, amely rendkívüli biológiai sokféleségnek ad otthont, többek között erdei elefántoknak és síkvidéki gorilláknak. A gyarmati időkben a folyó fontos útvonala volt a gumi-kereskedelemnek és Afrika belső területei feltárásának. Napjainkban a Sangha központi szerepet játszik a regionális ökoszisztémában, és alapvető fontosságú a partjai mentén élő számos őslakos közösség számára.",
      ro: "Râul Sangha este un afluent major de dreapta al fluviului Congo, traversând Republica Centrafricană, Camerun și Republica Congo. Cu o lungime totală de aproximativ 790 de kilometri, acesta reprezintă o cale navigabilă vitală pentru transportul lemnului și al altor mărfuri în această regiune izolată. Din punct de vedere geografic, Sangha se formează prin confluența râurilor Mambéré și Kadéï lângă orașul Berbérati. O importanță deosebită o are situl transfrontalier Sangha Trinational, inclus în patrimoniul mondial UNESCO din 2012, care cuprinde trei parcuri naționale protejate. Bazinul său este caracterizat de o pădure tropicală densă, adăpostind o biodiversitate excepțională, inclusiv elefanți de pădure și gorile de câmpie. În perioada colonială, râul a servit drept rută principală pentru comerțul cu cauciuc și explorarea interiorului Africii. Astăzi, Sangha joacă un rol central în ecosistemul regional și este esențial pentru subzistența numeroaselor comunități indigene care trăiesc de-a lungul malurilor sale.",
      en: "The Sangha River is a major right-bank tributary of the Congo River, flowing through the Central African Republic, Cameroon, and the Republic of the Congo. Spanning approximately 790 kilometers, it serves as a vital waterway for transporting timber and other resources in this remote region. Geographically, the Sangha is formed by the confluence of the Mambéré and Kadéï rivers near the town of Berbérati. Its significance is highlighted by the Sangha Trinational protected area, a UNESCO World Heritage site since 2012 that connects three national parks. The river basin is dominated by dense tropical rainforest, hosting extraordinary biodiversity, including forest elephants and lowland gorillas. During the colonial era, the river was an essential route for the rubber trade and the exploration of the African interior. Today, the Sangha remains central to the regional ecosystem and is fundamental to the livelihoods of numerous indigenous communities residing along its banks.",
      es: "El río Sangha es un importante afluente derecho del Congo y fluye a través de la República Centroafricana, Camerún y la República del Congo. Con una longitud total de unos 790 kilómetros, constituye una vía fluvial vital para el transporte de madera y otras mercancías en esta región remota. Geográficamente, el Sangha se forma por la confluencia de los ríos Mambéré y Kadéï cerca de la ciudad de Berbérati. Destaca especialmente el área protegida transfronteriza de Sangha Trinational, que forma parte del Patrimonio Mundial de la UNESCO desde 2012 e incluye tres parques nacionales. La cuenca se caracteriza por una densa selva tropical que alberga una biodiversidad excepcional, incluyendo elefantes de bosque y gorilas de llanura. En la época colonial, el río servía como ruta importante para el comercio del caucho y la exploración del África interior. Hoy en día, el Sangha desempeña un papel central en el ecosistema regional y es crucial para la supervivencia de numerosas comunidades indígenas a lo largo de sus orillas.",
      pt: "O rio Sangha é um importante afluente direito do Congo e corre através da República Centro-Africana, dos Camarões e da República do Congo. Com um comprimento total de cerca de 790 quilómetros, constitui uma via navegável vital para o transporte de madeira e outras mercadorias nesta região remota. Geograficamente, o Sangha é formado pela confluência dos rios Mambéré e Kadéï perto da cidade de Berbérati. Destaca-se especialmente a área protegida transfronteiriça de Sangha Trinational, que faz parte do Património Mundial da UNESCO desde 2012 e inclui três parques nacionais. A bacia hidrográfica é caracterizada por uma densa floresta tropical que abriga uma biodiversidade excecional, incluindo elefantes da floresta e gorilas das planícies. Na época colonial, o rio servia como uma rota importante para o comércio da borracha e para a exploração do interior de África. Hoje, o Sangha desempenha um papel central no ecossistema regional e é crucial para a sobrevivência de numerosas comunidades indígenas ao longo das suas margens.",
      fr: "La rivière Sangha est un affluent majeur de la rive droite du Congo et traverse la République centrafricaine, le Cameroun et la République du Congo. Avec une longueur totale d'environ 790 kilomètres, elle constitue une voie navigable vitale pour le transport du bois et d'autres marchandises dans cette région isolée. Géographiquement, la Sangha naît de la confluence des rivières Mambéré et Kadéï près de la ville de Berbérati. La zone protégée transfrontalière du Trinational de la Sangha, inscrite au patrimoine mondial de l'UNESCO depuis 2012 et comprenant trois parcs nationaux, est particulièrement remarquable. Le bassin versant est caractérisé par une forêt tropicale dense qui abrite une biodiversité exceptionnelle, notamment des éléphants de forêt et des gorilles de plaine. À l'époque coloniale, la rivière servait de route importante pour le commerce du caoutchouc et l'exploration de l'intérieur de l'Afrique. Aujourd'hui, la Sangha joue un rôle central dans l'écosystème régional et est cruciale pour la survie de nombreuses communautés autochtones le long de ses rives.",
    },
    factsAdvanced: {
      de: ["Die Gesamtlänge des Sangha-Flusses beträgt etwa 790 Kilometer.", "Der Fluss bildet die natürliche Grenze zwischen Kamerun und der Republik Kongo.", "Das Sangha Trinational-Schutzgebiet umfasst eine Fläche von über 750.000 Hektar.", "Er mündet südlich von Mossaka in den mächtigen Kongo-Strom.", "Der Fluss ist fast auf seiner gesamten Länge für Boote mit geringem Tiefgang schiffbar.", "Die jährliche Abflussmenge an der Mündung beträgt im Durchschnitt 2.471 m³/s."],
      hu: ["A Sangha-folyó teljes hossza megközelítőleg 790 kilométer.", "A folyó természetes határt képez Kamerun és a Kongói Köztársaság között.", "A Sangha Trinational védett terület több mint 750 000 hektáron terül el.", "A folyó Mossaka városától délre torkollik a hatalmas Kongóba.", "A folyó szinte teljes hosszában hajózható sekély merülésű vízi járművekkel.", "Az éves átlagos vízhozama a torkolatnál 2471 m³/s körül mozog."],
      ro: ["Lungimea totală a râului Sangha este de aproximativ 790 de kilometri.", "Râul formează frontiera naturală între Camerun și Republica Congo.", "Aria protejată Sangha Trinational acoperă o suprafață de peste 750.000 de hectare.", "Se varsă în fluviul Congo la sud de localitatea Mossaka.", "Râul este navigabil pentru ambarcațiuni cu pescaj mic pe aproape întreaga sa lungime.", "Debitul mediu anual la gura de vărsare este de circa 2.471 m³/s."],
      en: ["The total length of the Sangha River is approximately 790 kilometers.", "The river forms a natural border between Cameroon and the Republic of the Congo.", "The Sangha Trinational protected area covers over 750,000 hectares of land.", "It empties into the Congo River south of the town of Mossaka.", "The river is navigable for shallow-draft vessels throughout most of its course.", "The average annual discharge at its mouth is approximately 2,471 m³/s."],
      es: ["La longitud total del río Sangha es de unos 790 kilómetros.", "El río forma la frontera natural entre Camerún y la República del Congo.", "El área protegida Sangha Trinational abarca una superficie de más de 750.000 hectáreas.", "Desemboca en el caudaloso río Congo al sur de Mossaka.", "El río es navegable en casi toda su longitud para embarcaciones de poco calado.", "El caudal anual medio en la desembocadura es de 2.471 m³/s."],
      pt: ["O comprimento total do rio Sangha é de cerca de 790 quilómetros.", "O rio forma a fronteira natural entre os Camarões e a República do Congo.", "A área protegida Sangha Trinational abrange uma superfície de mais de 750.000 hectares.", "Desagua no caudaloso rio Congo a sul de Mossaka.", "O rio é navegável em quase todo o seu comprimento para embarcações de calado reduzido.", "O caudal anual médio na foz é de 2.471 m³/s."],
      fr: ["La longueur totale de la rivière Sangha est d'environ 790 kilomètres.", "La rivière forme la frontière naturelle entre le Cameroun et la République du Congo.", "La zone protégée du Trinational de la Sangha couvre une superficie de plus de 750 000 hectares.", "Elle se jette dans le puissant fleuve Congo au sud de Mossaka.", "La rivière est navigable sur presque toute sa longueur pour les bateaux à faible tirant d'eau.", "Le débit annuel moyen à l'embouchure est de 2 471 m³/s."],
    },
    
    description: {
      de: "Ein bedeutender Fluss im Südwesten, der durch dichte Regenwälder fließt und Teil eines UNESCO-Welterbes ist.",
      hu: "Jelentős folyó délnyugaton, amely sűrű esőerdőkön folyik keresztül, és az UNESCO Világörökség része.",
      ro: "Un râu important din sud-vest care curge prin păduri tropicale dense și face parte din Patrimoniul Mondial UNESCO.",
      en: "A significant river in the southwest that flows through dense rainforests and is part of a UNESCO World Heritage site.",
      es: "Un río importante en el suroeste que fluye a través de densas selvas tropicales y forma parte de un sitio del Patrimonio Mundial de la UNESCO.",
      pt: "Um rio importante no sudoeste que corre através de densas florestas tropicais e faz parte de um sítio do Património Mundial da UNESCO.",
      fr: "Une rivière importante dans le sud-ouest qui traverse des forêts tropicales denses et fait partie d'un site du patrimoine mondial de l'UNESCO.",
    },
    facts: {
      de: ["Fließt durch das Dzanga-Sangha-Schutzgebiet.", "Wichtiger Lebensraum für Waldelefanten."],
      hu: ["A Dzanga-Sangha természetvédelmi területen folyik keresztül.", "Az erdei elefántok fontos élőhelye."],
      ro: ["Curge prin Rezervația Dzanga-Sangha.", "Habitat important pentru elefanții de pădure."],
      en: ["Flows through the Dzanga-Sangha Reserve.", "Key habitat for forest elephants."],
      es: ["Fluye a través del área protegida de Dzanga-Sangha.", "Hábitat importante para los elefantes de bosque."],
      pt: ["Corre através da área protegida de Dzanga-Sangha.", "Habitat importante para os elefantes da floresta."],
      fr: ["Traverse la zone protégée de Dzanga-Sangha.", "Habitat important pour les éléphants de forêt."],
    }, image: "/poi-images/sangha-river-nature-v2.webp"},
  {
    id: "chari-river-nature-v2",
    type: "river",
    parent: "CF-BB",
    coords: [19.2500, 8.5000],
    name: {
      de: "Chari-Fluss",
      hu: "Chari folyó",
      ro: "Râul Chari",
      en: "Chari River"},
    image: "/poi-images/chari-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Schari ist der bedeutendste Fluss im Tschad und das primäre Versorgungssystem des Tschadsees. Mit einer Länge von etwa 1.400 Kilometern entspringt er im Hochland der Zentralafrikanischen Republik und fließt in nordwestlicher Richtung, wobei er die tschadische Hauptstadt N'Djamena passiert. Der Fluss bildet die Lebensader für die Landwirtschaft und Fischerei in einer Region, die sonst von extremer Trockenheit geprägt wäre. Zusammen mit seinem wichtigsten Nebenfluss, dem Logone, transportiert der Schari jährlich Millionen Kubikmeter Sedimente und Wasser in das abflusslose Becken des Sees. Historisch gesehen war der Fluss eine zentrale Handelsroute und markierte wichtige politische Grenzen während der Kolonialzeit. Die saisonalen Überschwemmungen des Schari sind entscheidend für die Fruchtbarkeit der angrenzenden Polder und Auenlandschaften. In der tschadischen Kultur wird der Fluss oft als Symbol für Beständigkeit und Fruchtbarkeit besungen. Heute bedrohen Sedimentation und reduzierte Niederschläge die Schiffbarkeit des Flusses, was weitreichende Folgen für den regionalen Handel hat. Zahlreiche Infrastrukturprojekte, darunter Brücken in N'Djamena, unterstreichen seine Bedeutung für die moderne Mobilität im Land.",
      hu: "A Chari Csád legjelentősebb folyója és a Csád-tó elsődleges vízutánpótlási rendszere. Körülbelül 1400 kilométeres hosszúságával a Közép-afrikai Köztársaság felföldjein ered, és északnyugati irányba folyik, érintve a csádi fővárost, N'Djamenát. A folyó a mezőgazdaság és a halászat éltető eleme egy olyan régióban, amelyet egyébként extrém szárazság jellemezne. Legfontosabb mellékfolyójával, a Logone-nal együtt a Chari évente több millió köbméter üledéket és vizet szállít a tó lefolyástalan medencéjébe. Történelmileg a folyó központi kereskedelmi útvonal volt, és fontos politikai határokat jelölt ki a gyarmati időkben. A Chari szezonális áradásai döntőek a szomszédos polderek és ártéri tájak termékenysége szempontjából. A csádi kultúrában a folyót gyakran az állandóság és a termékenység szimbólumaként éneklik meg. Napjainkban az üledékképződés és a csökkent csapadékmennyiség veszélyezteti a folyó hajózhatóságát, ami messzemenő következményekkel jár a regionális kereskedelemre nézve. Számos infrastrukturális projekt, köztük az N'Djamenában található hidak, hangsúlyozzák a folyó fontosságát az ország modern mobilitása szempontjából.",
      ro: "Râul Chari este cel mai important râu din Ciad și sistemul primar de alimentare al lacului Ciad. Cu o lungime de aproximativ 1.400 de kilometri, izvorăște din zonele înalte ale Republicii Centrafricane și curge spre nord-vest, trecând prin capitala ciadiană N'Djamena. Râul formează linia vieții pentru agricultură și pescuit într-o regiune care altfel ar fi marcată de secetă extremă. Împreună cu principalul său afluent, Logone, Chari transportă anual milioane de metri cubi de sedimente și apă în bazinul fără scurgere al lacului. Din punct de vedere istoric, râul a fost o rută comercială centrală și a marcat granițe politice importante în perioada colonială. Inundațiile sezoniere ale râului Chari sunt cruciale pentru fertilitatea polderelor și a luncilor adiacente. În cultura ciadiană, râul este adesea cântat ca un simbol al constanței și fertilității. Astăzi, sedimentarea și reducerea precipitațiilor amenință navigabilitatea râului, ceea ce are consecințe de amploare pentru comerțul regional. Numeroase proiecte de infrastructură, inclusiv poduri în N'Djamena, subliniază importanța sa pentru mobilitatea modernă din țară.",
      en: "The Chari is the most significant river in Chad and the primary supply system for Lake Chad. With a length of approximately 1,400 kilometers, it originates in the highlands of the Central African Republic and flows in a northwestern direction, passing the Chadian capital, N'Djamena. The river forms the lifeline for agriculture and fishing in a region that would otherwise be marked by extreme drought. Along with its most important tributary, the Logone, the Chari transports millions of cubic meters of sediment and water annually into the lake's endorheic basin. Historically, the river was a central trade route and marked important political boundaries during the colonial period. Seasonal flooding of the Chari is crucial for the fertility of the adjacent polders and floodplains. In Chadian culture, the river is often sung about as a symbol of constancy and fertility. Today, sedimentation and reduced rainfall threaten the river's navigability, which has far-reaching consequences for regional trade. Numerous infrastructure projects, including bridges in N'Djamena, underscore its importance for modern mobility in the country.",
    },
    factsAdvanced: {
      de: ["Der Schari liefert etwa 90% des gesamten Wasserzuflusses in den Tschadsee.", "An der Mündung in N'Djamena vereinigt sich der Schari mit dem Logone.", "Der Fluss beheimatet den wertvollen Nilbarsch (Capitaine), einen wichtigen Speisefisch.", "In der Regenzeit steigt der Wasserspiegel des Flusses um bis zu 6 Meter an.", "Das Schari-Becken umfasst eine Fläche von rund 600.000 Quadratkilometern.", "Der Fluss dient als natürliche Grenze zwischen dem Tschad und Kamerun."],
      hu: ["A Chari biztosítja a Csád-tó teljes vízutánpótlásának körülbelül 90%-át.", "N'Djamenánál a Chari egyesül a Logone-folyóval.", "A folyó otthont ad az értékes nílusi sügérnek (Capitaine), amely fontos étkezési hal.", "Az esős évszakban a folyó vízszintje akár 6 métert is emelkedhet.", "A Chari-medence területe körülbelül 600 000 négyzetkilométer.", "A folyó természetes határként szolgál Csád és Kamerun között."],
      ro: ["Râul Chari furnizează aproximativ 90% din întregul aport de apă în lacul Ciad.", "La confluența din N'Djamena, Chari se unește cu râul Logone.", "Râul găzduiește valorosul biban de Nil (Capitaine), un pește comestibil important.", "În sezonul ploios, nivelul apei râului crește cu până la 6 metri.", "Bazinul Chari cuprinde o suprafață de aproximativ 600.000 de kilometri pătrați.", "Râul servește drept graniță naturală între Ciad și Camerun."],
      en: ["The Chari provides about 90% of the total water inflow into Lake Chad.", "At the confluence in N'Djamena, the Chari joins with the Logone.", "The river is home to the valuable Nile perch (Capitaine), an important food fish.", "During the rainy season, the river's water level rises by up to 6 meters.", "The Chari basin covers an area of around 600,000 square kilometers.", "The river serves as a natural border between Chad and Cameroon."],
      es: [],
      pt: [],
      fr: [],
    },
    
    description: {
      de: "Dieser Fluss im Norden des Landes fließt in Richtung Tschadsee und speist das dortige Ökosystem.",
      hu: "Az ország északi részén található folyó a Csád-tó irányába folyik, és táplálja az ottani ökoszisztémát.",
      ro: "Acest râu din nordul țării curge spre Lacul Ciad și hrănește ecosistemul de acolo.",
      en: "This river in the north of the country flows toward Lake Chad and feeds the local ecosystem.",
      es: "Este río en el norte del país fluye hacia el lago Chad y alimenta el ecosistema local.",
      pt: "Este rio no norte do país corre em direção ao lago Chade e alimenta o ecossistema local.",
      fr: "Ce fleuve du nord du pays coule vers le lac Tchad et alimente l'écosystème local.",
    },
    facts: {
      de: ["Hauptzufluss des Tschadsees.", "Wichtig für die Fischerei in der nördlichen Region."],
      hu: ["A Csád-tó fő vízforrása.", "Fontos a halászat számára az északi régióban."],
      ro: ["Sursă principală de alimentare a Lacului Ciad.", "Important pentru pescuit în regiunea de nord."],
      en: ["Main source of water for Lake Chad.", "Vital for fishing in the northern region."],
      es: ["Principal afluente del lago Chad.", "Importante para la pesca en la región norte."],
      pt: ["Principal afluente do lago Chade.", "Importante para a pesca na região norte."],
      fr: ["Principal affluent du lac Tchad.", "Important pour la pêche dans la région nord."],
    }, image: "/poi-images/chari-river-nature-v2.webp"},
  {
    id: "mbomou-river-nature-v2",
    type: "river",
    parent: "CF-MB",
    coords: [23.5000, 4.8000],
    name: {
      de: "Mbomou-Fluss",
      hu: "Mbomou folyó",
      ro: "Râul Mbomou",
      en: "Mbomou River"},
    image: "/poi-images/mbomou-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Mbomou-Fluss bildet über eine weite Strecke die natürliche Grenze zwischen der Demokratischen Republik Kongo und der Zentralafrikanischen Republik. Mit einer Länge von rund 800 Kilometern entspringt er im Hochland nahe der Wasserscheide zwischen Kongo und Nil und fließt nach Westen, wo er sich mit dem Uele zum Ubangi vereint. Geografisch ist der Flusslauf von zahlreichen Stromschnellen und kleinen Inseln geprägt, was die durchgehende Schifffahrt erschwert, aber eine wilde, unberührte Flusslandschaft bewahrt hat. Historisch war der Mbomou eine bedeutende Route für den kulturellen Austausch zwischen den Völkern der Region, aber auch ein Schauplatz kolonialer Grenzziehungen. Die Uferregionen sind von dichten Regenwäldern und Savannen bedeckt, die wichtige Korridore für Wildtiere wie Elefanten und Flusspferde darstellen. Der Fluss spielt eine zentrale Rolle für die lokale Bevölkerung, die ihn für den Fischfang und als einzige verlässliche Wasserquelle nutzt. In der Neuzeit ist die Region aufgrund ihrer Abgeschiedenheit ökologisch wertvoll, steht jedoch vor Herausforderungen durch grenzüberschreitende Konflikte und illegale Ressourcenausbeutung.",
      hu: "A Mbomou-folyó jelentős szakaszán természetes határt alkot a Kongói Demokratikus Köztársaság és a Közép-afrikai Köztársaság között. A körülbelül 800 kilométer hosszú folyó a Kongó és a Nílus vízválasztójának közelében ered, majd nyugat felé tartva az Uele-folyóval egyesülve hozza létre az Ubangi-folyót. Földrajzilag a folyómedret számos zúgó és kisebb sziget tarkítja, ami megnehezíti a folyamatos hajózást, ugyanakkor megőrizte a táj érintetlen vadságát. Történelmileg a Mbomou fontos útvonal volt a régió népei közötti kulturális cseréhez, de a gyarmati határkijelölések egyik fő viszonyítási pontjaként is szolgált. Partjait sűrű esőerdők és szavannák borítják, amelyek fontos vonulási folyosót biztosítanak az elefántok és vízilovak számára. A folyó központi szerepet játszik a helyi közösségek életében, akik halászatból és a víz közvetlen felhasználásából élnek. Napjainkban a terület elszigeteltsége miatt ökológiai szempontból igen értékes, bár a határokon átnyúló konfliktusok és az illegális erőforrás-kiaknázás veszélyeztetik a környezet épségét.",
      ro: "Râul Mbomou formează o frontieră naturală extinsă între Republica Democrată Congo și Republica Centrafricană pe o mare parte a cursului său. Cu o lungime de aproximativ 800 de kilometri, acesta izvorăște în zonele înalte de lângă cumpăna apelor dintre bazinele Congo și Nil, curgând spre vest pentru a se uni cu râul Uele și a forma fluviul Ubangi. Din punct de vedere geografic, cursul râului este marcat de numeroase praguri și insule mici, elemente care limitează navigația la scară largă, dar păstrează un peisaj fluvial sălbatic și nealterat. Istoric, Mbomou a fost o rută esențială pentru schimburile culturale între populațiile locale, devenind ulterior o referință majoră în trasarea frontierelor coloniale. Zonele riverane sunt acoperite de păduri tropicale dense și savane, constituind coridoare ecologice importante pentru fauna sălbatică, inclusiv elefanți și hipopotami. Râul este vital pentru comunitățile locale, fiind principala sursă de hrană prin pescuit și singura sursă sigură de apă dulce. În prezent, izolarea regiunii îi conferă o valoare ecologică ridicată, deși rămâne vulnerabilă în fața conflictelor transfrontaliere.",
      en: "The Mbomou River serves as a long natural border between the Democratic Republic of the Congo and the Central African Republic. Stretching approximately 800 kilometers, it originates in the highlands near the Congo-Nile watershed and flows westward until it converges with the Uele River to form the Ubangi River. Geographically, the river's course is punctuated by numerous rapids and small islands, which restrict continuous navigation but have preserved a wild and pristine riverine landscape. Historically, the Mbomou was a significant corridor for cultural exchange among regional tribes and later became a focal point for colonial border demarcations. The riverbanks are clothed in a mix of dense rainforests and savannas, providing essential corridors for wildlife such as elephants and hippos. The river remains central to the lives of local populations, who depend on it for fishing and as their primary reliable source of water. In modern times, the region's remoteness has granted it high ecological value, though it faces ongoing challenges from cross-border conflicts and the illegal exploitation of natural resources.",
      es: "El río Mbomou constituye durante un largo tramo la frontera natural entre la República Democrática del Congo y la República Centroafricana. Con una longitud de unos 800 kilómetros, nace en las tierras altas cerca de la divisoria de aguas entre el Congo y el Nilo y fluye hacia el oeste, donde se une con el Uele para formar el Ubangui. Geográficamente, el curso del río está marcado por numerosos rápidos y pequeñas islas, lo que dificulta la navegación continua, pero ha preservado un paisaje fluvial salvaje y virgen. Históricamente, el Mbomou fue una ruta importante para el intercambio cultural entre los pueblos de la región, pero también escenario de delimitaciones coloniales. Las regiones ribereñas están cubiertas por densas selvas y sabanas, que representan corredores vitales para animales salvajes como elefantes e hipopótamos. El río desempeña un papel central para la población local, que lo utiliza para la pesca y como única fuente fiable de agua. En la era moderna, la región es ecológicamente valiosa debido a su aislamiento, aunque enfrenta desafíos por conflictos transfronterizos y explotación ilegal de recursos.",
      pt: "O rio Mbomou constitui, ao longo de um extenso percurso, a fronteira natural entre a República Democrática do Congo e a República Centro-Africana. Com um comprimento de cerca de 800 quilómetros, nasce nas terras altas perto da divisória de águas entre o Congo e o Nilo e flui para oeste, onde se junta ao Uele para formar o Ubangui. Geograficamente, o curso do rio é marcado por numerosos rápidos e pequenas ilhas, o que dificulta a navegação contínua, mas preservou uma paisagem fluvial selvagem e intocada. Historicamente, o Mbomou foi uma rota importante para o intercâmbio cultural entre os povos da região, mas também um palco de delimitações coloniais. As regiões ribeirinhas são cobertas por densas florestas tropicais e savanas, que representam corredores importantes para animais selvagens como elefantes e hipopótamos. O rio desempenha um papel central para a população local, que o utiliza para a pesca e como única fonte fiável de água. Na era moderna, a região é ecologicamente valiosa devido ao seu isolamento, mas enfrenta desafios decorrentes de conflitos transfronteiriços e da exploração ilegal de recursos.",
      fr: "La rivière Mbomou constitue sur une longue distance la frontière naturelle entre la République démocratique du Congo et la République centrafricaine. D'une longueur d'environ 800 kilomètres, elle prend sa source dans les hauts plateaux près de la ligne de partage des eaux entre le Congo et le Nil et coule vers l'ouest, où elle s'unit à l'Uele pour former l'Oubangui. Géographiquement, le cours de la rivière est marqué par de nombreux rapides et de petites îles, ce qui rend la navigation continue difficile mais a préservé un paysage fluvial sauvage et intact. Historiquement, le Mbomou était une route importante pour les échanges culturels entre les peuples de la région, mais aussi le théâtre de délimitations coloniales. Les régions riveraines sont couvertes de forêts tropicales denses et de savanes, qui constituent des corridors importants pour la faune sauvage telle que les éléphants et les hippopotames. La rivière joue un rôle central pour la population locale, qui l'utilise pour la pêche et comme seule source d'eau fiable. À l'époque moderne, la région est écologiquement précieuse en raison de son isolement, mais elle est confrontée à des défis liés aux conflits transfrontaliers et à l'exploitation illégale des ressources.",
    },
    factsAdvanced: {
      de: ["Der Mbomou hat eine Gesamtlänge von zirka 800 Kilometern.", "Er vereinigt sich bei Yakoma mit dem Uele zum Ubangi.", "Der Fluss bildet die Nordgrenze der kongolesischen Provinz Bas-Uele.", "Zahlreiche Wasserfälle verhindern die Schifffahrt auf weiten Teilen.", "Das Mbomou-Bassin ist ein wichtiges Siedlungsgebiet des Zande-Volkes.", "Die Region gehört zum Ubangi-Shari-Ökosystem."],
      hu: ["A Mbomou hossza körülbelül 800 kilométer.", "Yakoma városánál egyesül az Uele-folyóval, így jön létre az Ubangi.", "Vízgyűjtő területe mintegy 157 000 négyzetkilométer.", "A folyó mentén található az egyik legnagyobb érintetlen csimpánzpopuláció.", "A Zande nép történelmi szálláshelye a folyó két partján fekszik.", "A folyó vízhozama jelentősen ingadozik a trópusi esők függvényében."],
      ro: ["Lungimea totală a râului Mbomou este de aproximativ 800 km.", "Se unește cu râul Uele la Yakoma pentru a forma Ubangi.", "Bazinul său hidrografic acoperă circa 157.000 km².", "Formează granița de nord a provinciei congoleze Bas-Uele.", "Este o barieră naturală pentru migrația anumitor specii terestre.", "Debitul mediu este influențat drastic de regimul precipitațiilor tropicale."],
      en: ["The Mbomou River is approximately 800 kilometers long.", "It joins the Uele River at Yakoma to create the Ubangi River.", "The river's drainage basin spans about 157,000 square kilometers.", "It marks the northern boundary of the DRC's Bas-Uele province.", "The Zande people are the primary ethnic group in the river basin.", "The river is inaccessible to large vessels due to frequent rapids."],
      es: ["El Mbomou tiene una longitud total de aproximadamente 800 kilómetros.", "Se une al Uele cerca de Yakoma para formar el Ubangui.", "El río constituye la frontera norte de la provincia congoleña de Bas-Uele.", "Numerosas cascadas impiden la navegación en grandes tramos.", "La cuenca del Mbomou es una zona de asentamiento importante del pueblo Zande.", "La región forma parte del ecosistema Ubangui-Chari."],
      pt: ["O Mbomou tem um comprimento total de aproximadamente 800 quilómetros.", "Junta-se ao Uele perto de Yakoma para formar o Ubangui.", "O rio constitui a fronteira norte da província congolesa de Bas-Uele.", "Numerosas cascatas impedem a navegação em grandes extensões.", "A bacia do Mbomou é uma importante área de fixação do povo Zande.", "A região pertence ao ecossistema Ubangui-Chari."],
      fr: ["Le Mbomou a une longueur totale d'environ 800 kilomètres.", "Il s'unit à l'Uele près de Yakoma pour former l'Oubangui.", "La rivière constitue la frontière nord de la province congolaise du Bas-Uele.", "De nombreuses chutes d'eau empêchent la navigation sur de grandes parties.", "Le bassin du Mbomou est une zone d'habitation importante du peuple Zande.", "La région appartient à l'écosystème Oubangui-Chari."],
    },
    
    description: {
      de: "Der Fluss bildet einen großen Teil der südlichen Grenze zur Demokratischen Republik Kongo.",
      hu: "A folyó a Kongói Demokratikus Köztársasággal közös déli határ jelentős részét alkotja.",
      ro: "Râul formează o mare parte a graniței sudice cu Republica Democrată Congo.",
      en: "The river forms a large part of the southern border with the Democratic Republic of the Congo.",
      es: "El río forma gran parte de la frontera sur con la República Democrática del Congo.",
      pt: "O rio forma uma grande parte da fronteira sul com a República Democrática do Congo.",
      fr: "La rivière forme une grande partie de la frontière sud avec la République démocratique du Congo.",
    },
    facts: {
      de: ["Zusammenfluss mit dem Uelle bildet den Ubangi.", "Fließt durch abgelegene Savannen und Wälder."],
      hu: ["Az Uelle folyóval való találkozása hozza létre az Ubangit.", "Távoli szavannákon és erdőkön folyik keresztül."],
      ro: ["Confluența cu Uelle formează Ubangi.", "Curge prin savane și păduri izolate."],
      en: ["Confluence with the Uele forms the Ubangi.", "Flows through remote savannas and forests."],
      es: ["Su confluencia con el Uele forma el río Ubangui.", "Fluye a través de sabanas y bosques remotos."],
      pt: ["A sua confluência com o Uele forma o rio Ubangui.", "Flui através de savanas e florestas remotas."],
      fr: ["Son confluent avec l'Uele forme l'Oubangui.", "Coule à travers des savanes et des forêts isolées."],
    }, image: "/poi-images/mbomou-river-nature-v2.webp"},
  {
    id: "lobaye-river-nature-v2",
    type: "river",
    parent: "CF-LB",
    coords: [18.2000, 3.8000],
    name: {
      de: "Lobaye-Fluss",
      hu: "Lobaye folyó",
      ro: "Râul Lobaye",
      en: "Lobaye River"},
    image: "/poi-images/lobaye-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Lobaye-Fluss ist ein bedeutender rechter Nebenfluss des Ubangi im Südwesten der Zentralafrikanischen Republik. Er durchquert die gleichnamige Präfektur, die für ihre dichten Primärwälder und ihre hohe Biodiversität bekannt ist. Der Fluss entspringt im zentralen Hochland und fließt in südöstlicher Richtung durch ein Gebiet, das stark von der Forstwirtschaft geprägt ist. Die Wasserstraße dient als wichtige Transportroute für den Abtransport von Baumstämmen zu den Häfen am Ubangi. Entlang des Lobaye leben zahlreiche indigene Völker, insbesondere Baka-Gemeinschaften, die den Fluss für den Fischfang und die Wasserversorgung nutzen. Die Ufervegetation besteht aus dichten tropischen Wäldern, in denen Waldelefanten und Gorillas beheimatet sind. Trotz der wirtschaftlichen Nutzung durch die Holzindustrie ist der Flusslauf in vielen Teilen noch wild und unberührt. Er mündet südlich von Bangui in den Ubangi und trägt wesentlich zur Wasserführung dieses Hauptstroms bei.",
      hu: "A Lobaye-folyó az Ubangi jelentős jobb oldali mellékfolyója a Közép-afrikai Köztársaság délnyugati részén. Az azonos nevű prefektúrán folyik keresztül, amely sűrű őserdőiről és gazdag élővilágáról ismert. A folyó a központi fennsíkokon ered, és délkeleti irányba tartva szeli át az erdőgazdálkodás által meghatározott vidéket. A vízi út fontos szállítási útvonalat jelent a kitermelt farönkök eljuttatásához az Ubangi kikötőibe. A Lobaye mentén számos őshonos népcsoport, köztük baka közösségek élnek, akik a folyót halászatra és mindennapi vízellátásra használják. A part menti vegetációt sűrű trópusi erdők alkotják, amelyek erdei elefántoknak és gorilláknak adnak otthont. A fafeldolgozó ipar gazdasági jelenléte ellenére a folyómeder sok helyen vad és érintetlen maradt. Banguitól délre torkollik az Ubangiba, jelentősen hozzájárulva a főfolyam vízhozamához.",
      ro: "Râul Lobaye este un afluent de dreapta important al râului Ubangi, situat în sud-vestul Republicii Centrafricane. Acesta traversează prefectura cu același nume, renumită pentru pădurile sale primare dense și biodiversitatea ridicată. Râul izvorăște în zonele muntoase centrale și curge spre sud-est printr-o regiune marcată puternic de activități forestiere. Calea navigabilă servește drept rută vitală pentru transportul buștenilor către porturile de pe Ubangi. De-a lungul râului Lobaye trăiesc numeroase popoare indigene, în special comunități Baka, care depind de râu pentru pescuit și aprovizionarea cu apă. Vegetația de pe maluri constă în păduri tropicale etajate, care sunt habitatul elefanților de pădure și al gorilelor. În ciuda exploatării economice de către industria lemnului, cursul râului rămâne sălbatic în multe secțiuni. Se varsă în Ubangi la sud de capitala Bangui, contribuind semnificativ la debitul acestui fluviu principal.",
      en: "The Lobaye River is a significant right-bank tributary of the Ubangi River, located in the southwestern Central African Republic. It flows through the Lobaye Prefecture, an area renowned for its dense primary rainforests and high levels of biodiversity. Originating in the central highlands, the river flows southeast through a region heavily influenced by the forestry industry. The waterway serves as a crucial transport route for moving timber logs from remote concessions to downstream ports on the Ubangi. Numerous indigenous groups, particularly Baka communities, reside along the Lobaye, relying on the river for fishing and daily water needs. The riparian vegetation consists of thick tropical forests that provide habitat for forest elephants and gorillas. Despite the economic pressures from the timber industry, much of the river’s course remains wild and untouched. It joins the Ubangi south of Bangui, contributing substantially to the overall volume of the main river.",
    },
    factsAdvanced: {
      de: ["Der Lobaye entspringt im Hochland von Ouaka", "Er mündet südlich von Bangui in den Ubangi", "Die Region Lobaye ist ein Zentrum der afrikanischen Pygmäen", "Flusspferde kommen häufig im Unterlauf des Flusses vor", "Er ist ein Haupttransportweg für die lokale Holzindustrie", "Die Länge des Lobaye beträgt etwa 520 Kilometer"],
      hu: ["A Lobaye az Ouaka-fennsíkon ered", "Banguitól délre torkollik az Ubangi folyóba", "A Lobaye régió az afrikai pigmeusok egyik központja", "A folyó alsó szakaszán gyakoriak a vízilovak", "A helyi faipar egyik legfontosabb szállítási útvonala", "A Lobaye hossza körülbelül 520 kilométer"],
      ro: ["Lobaye izvorăște în zona înaltă din Ouaka", "Se varsă în râul Ubangi la sud de orașul Bangui", "Regiunea Lobaye este un centru al pigmeilor africani", "Hipopotamii sunt frecvenți în cursul inferior al râului", "Este o rută principală de transport pentru industria lemnului", "Lungimea totală a râului Lobaye este de circa 520 km"],
      en: ["The Lobaye originates in the highlands of Ouaka", "It flows into the Ubangi River south of Bangui", "The Lobaye region is a cultural hub for African Pygmies", "Hippos are commonly found in the river's lower reaches", "It is a vital transport link for the local timber industry", "The Lobaye River has a length of approximately 520 km"],
      es: [],
      pt: [],
      fr: [],
    },
    
    description: {
      de: "Ein wichtiger Nebenfluss des Ubangi, der durch die regenwaldreiche Region Lobaye fließt.",
      hu: "Az Ubangi fontos mellékfolyója, amely a gazdag esőerdőkkel borított Lobaye régión folyik keresztül.",
      ro: "Un afluent important al râului Ubangi, care curge prin regiunea bogată în păduri tropicale Lobaye.",
      en: "An important tributary of the Ubangi, flowing through the rainforest-rich Lobaye region.",
      es: "Un importante afluente del Ubangui que fluye a través de la región de Lobaye, rica en selva tropical.",
      pt: "Um importante afluente do Ubangui que flui através da região de Lobaye, rica em floresta tropical.",
      fr: "Un affluent important de l'Oubangui qui traverse la région de la Lobaye, riche en forêts tropicales.",
    },
    facts: {
      de: ["Zentrum für die traditionelle Flößerei.", "Heimat vieler Flusspferde."],
      hu: ["A hagyományos faúsztatás központja.", "Számos víziló otthona."],
      ro: ["Centru pentru transportul tradițional de bușteni.", "Adăpostește mulți hipopotami."],
      en: ["Center for traditional timber rafting.", "Home to many hippopotamuses."],
      es: ["Centro para el transporte tradicional de madera por flotación.", "Hogar de muchos hipopótamos."],
      pt: ["Centro para o transporte tradicional de madeira por flutuação.", "Lar de muitos hipopótamos."],
      fr: ["Centre pour le flottage traditionnel du bois.", "Abrite de nombreux hippopotames."],
    }, image: "/poi-images/lobaye-river-nature-v2.webp"},
  {
    id: "ouham-river-nature-v2",
    type: "river",
    parent: "CF-AC",
    coords: [17.5000, 6.5000],
    name: {
      de: "Ouham-Fluss",
      hu: "Ouham folyó",
      ro: "Râul Ouham",
      en: "Ouham River"},
    image: "/poi-images/ouham-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Ouham-Fluss ist eines der bedeutendsten Gewässer in der Zentralafrikanischen Republik und dient als lebenswichtiger Zufluss zum Schari-Fluss. Er entspringt in den Gebirgsregionen des westlichen Teils des Landes und fließt im Allgemeinen in nördlicher Richtung durch die fruchtbaren Savannen. Auf seinem etwa 1.050 Kilometer langen Weg durchquert der Fluss wichtige Verwaltungszentren wie Bozoum und Bossangoa und stellt eine unverzichtbare Wasserquelle für die Landwirtschaft und die lokalen Gemeinschaften dar. Das Becken des Flusses zeichnet sich durch vielfältige Ökosysteme aus, die eine breite Palette an aquatischem Leben beherbergen und als Korridor für Zugvogelarten dienen. Neben seiner ökologischen Bedeutung spielt der Ouham eine entscheidende Rolle beim Transport von Gütern während der Regenzeit, wenn die Wasserstände hoch sind. Sein Zusammenfluss mit dem Schari-Fluss in der Nähe der Grenze zum Tschad markiert einen bedeutenden hydrologischen Knotenpunkt im Tschadseebecken. Der Fluss bleibt ein zentrales Merkmal der nördlichen Geographie der Republik.",
      hu: "Az Ouham-folyó a Közép-afrikai Köztársaság egyik legjelentősebb vízfolyása, amely a Csád-folyó létfontosságú mellékfolyójaként szolgál. Az ország nyugati részének hegyvidéki régióiban ered, és általában északi irányba folyik át a termékeny szavannákon. Körülbelül 1050 kilométeres útja során a folyó olyan kulcsfontosságú közigazgatási központokon halad át, mint Bozoum és Bossangoa, alapvető vízkészletet biztosítva a mezőgazdaság és a helyi közösségek számára. A folyó medencéjét változatos ökoszisztémák jellemzik, amelyek a vízi élet széles skáláját támogatják, és folyosóként szolgálnak a vándormadárfajok számára. Ökológiai jelentősége mellett az Ouham döntő szerepet játszik az áruk szállításában az esős évszakban, amikor a vízszint magas. A Csád-folyóval való összefolyása a csádi határ közelében jelentős hidrológiai csomópontot jelöl a Csád-tó medencéjében. A folyó továbbra is az ország északi földrajzának központi eleme marad, mélyen beépülve a regionális társadalmi-gazdasági szerkezetbe.",
      ro: "Râul Ouham este unul dintre cele mai importante cursuri de apă din Republica Centrafricană, servind ca un afluent vital al râului Chari. Acesta își are originea în regiunile muntoase din partea de vest a țării și curge în general spre nord prin savanele fertile. De-a lungul cursului său de aproximativ 1.050 de kilometri, râul trece prin centre administrative cheie, inclusiv Bozoum și Bossangoa, oferind resurse esențiale de apă pentru agricultură și comunitățile locale. Bazinul râului este caracterizat de ecosisteme diverse, susținând o gamă largă de viață acvatică și servind ca un coridor pentru speciile de păsări migratoare. Pe lângă importanța sa ecologică, Ouham joacă un rol crucial în transportul mărfurilor în timpul sezonului ploios, când nivelul apei este ridicat. Confluența sa cu râul Chari, lângă granița cu Ciadul, marchează un punct hidrologic major în bazinul lacului Ciad. Râul rămâne o caracteristică centrală a geografiei nordice a țării, fiind profund integrat în structura socio-economică regională.",
      en: "The Ouham River is one of the most significant watercourses in the Central African Republic, serving as a vital tributary to the Chari River. It originates in the mountainous regions of the western part of the country and flows generally northward across the fertile savannas. Throughout its course of approximately 1,050 kilometers, the river passes through key administrative centers including Bozoum and Bossangoa, providing essential water resources for agriculture and local communities. The river's basin is characterized by diverse ecosystems, supporting a wide range of aquatic life and serving as a corridor for migratory bird species. In addition to its ecological importance, the Ouham River plays a crucial role in the transport of goods during the rainy season when water levels are high. Its confluence with the Chari River near the border with Chad marks a major hydrological junction in the Lake Chad basin. The river remains a central feature of the CAR's northern geography, deeply integrated into the regional socio-economic fabric.",
    },
    factsAdvanced: {
      de: ["Der Fluss hat eine Gesamtlänge von ca. 1.050 Kilometern.", "Er fließt durch die Städte Bozoum und Bossangoa in der ZAR.", "Der Ouham ist der größte Nebenfluss des Schari-Flusssystems.", "Das Einzugsgebiet wird auf über 70.000 Quadratkilometer geschätzt.", "Französische Expeditionen erkundeten das Becken bereits im Jahr 1908.", "Die höchsten Abflussmengen werden zwischen August und Oktober gemessen."],
      hu: ["A folyó teljes hossza körülbelül 1050 kilométer.", "Áthalad Bozoum és Bossangoa városokon a Közép-afrikai Köztársaságban.", "Az Ouham a Csád-folyórendszer legnagyobb mellékfolyója.", "Vízgyűjtő területét több mint 70 000 négyzetkilométerre becsülik.", "Francia expedíciók már 1908-ban alaposan feltérképezték a medencét.", "A szezonális vízhozam csúcsa általában augusztus és október között van."],
      ro: ["Râul are o lungime totală de aproximativ 1.050 de kilometri.", "Acesta traversează orașele Bozoum și Bossangoa din Republica Centrafricană.", "Ouham este cel mai mare afluent al sistemului râului Chari.", "Bazinul său hidrografic este estimat la peste 70.000 de kilometri pătrați.", "Expedițiile coloniale franceze au explorat bazinul râului în anul 1908.", "Vârful de debit sezonier apare de obicei între lunile august și octombrie."],
      en: ["The river has a total length of approximately 1,050 kilometers.", "It flows through the cities of Bozoum and Bossangoa in the CAR.", "The Ouham River is the largest tributary of the Chari River system.", "The drainage basin area is estimated at over 70,000 square kilometers.", "In 1908, the river basin was explored extensively by French expeditions.", "The seasonal discharge peaks typically occur between August and October."],
      es: [],
      pt: [],
      fr: [],
    },
    
    description: {
      de: "Ein bedeutender Fluss im Nordwesten, der eine wichtige Wasserquelle für die Landwirtschaft darstellt.",
      hu: "Jelentős folyó északnyugaton, amely a mezőgazdaság számára fontos vízforrást jelent.",
      ro: "Un râu important din nord-vest, care reprezintă o sursă de apă vitală pentru agricultură.",
      en: "A significant river in the northwest that provides an important water source for agriculture.",
      es: "Un río importante en el noroeste que representa una fuente de agua vital para la agricultura.",
      pt: "Um rio importante no noroeste que representa uma fonte de água vital para a agricultura.",
      fr: "Une rivière importante dans le nord-ouest qui constitue une source d'eau vitale pour l'agriculture.",
    },
    facts: {
      de: ["Mündet in den Chari-Fluss.", "Bekannt für seine saisonalen Überschwemmungen."],
      hu: ["A Chari folyóba torkollik.", "Szezonális áradásairól ismert."],
      ro: ["Se varsă în râul Chari.", "Cunoscut pentru inundațiile sale sezoniere."],
      en: ["Flows into the Chari River.", "Known for its seasonal flooding."],
      es: ["Desemboca en el río Chari.", "Conocido por sus inundaciones estacionales."],
      pt: ["Desagua no rio Chari.", "Conhecido pelas suas cheias sazonais."],
      fr: ["Se jette dans la rivière Chari.", "Connu pour ses crues saisonnières."],
    }, image: "/poi-images/ouham-river-nature-v2.webp"},
  {
    id: "kotto-river-nature-v2",
    type: "river",
    parent: "CF-HK",
    coords: [22.0000, 6.5000],
    name: {
      de: "Kotto-Fluss",
      hu: "Kotto folyó",
      ro: "Râul Kotto",
      en: "Kotto River"},
    image: "/poi-images/kotto-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Kotto-Fluss ist ein markantes geografisches Merkmal im Osten der Zentralafrikanischen Republik und ein bedeutender Nebenfluss des Ubangi. Er entspringt im Bongo-Massiv nahe der Grenze zum Sudan und fließt über 660 Kilometer in südlicher Richtung. Der Fluss ist berühmt für die Kembe-Wasserfälle, ein spektakuläres Naturschauspiel, bei dem das Wasser über massive Felsformationen stürzt. Die Ufer des Kotto sind von dichten Galeriewälder gesäumt, die eine reiche Artenvielfalt beherbergen, darunter zahlreiche Primaten und seltene Vogelarten. In der Nähe der Stadt Bria weitet sich der Fluss und bietet wichtige Fischgründe für die lokale Bevölkerung. Die hydrologische Dynamik des Flusses wird durch die ausgeprägten Regen- und Trockenzeiten der Region bestimmt, wobei die Wasserführung im Spätsommer ihren Höchststand erreicht. Als Transportweg ist der Kotto aufgrund zahlreicher Stromschnellen nur eingeschränkt nutzbar, was jedoch zur Erhaltung seiner unberührten Natur beigetragen hat. Er bleibt eine lebenswichtige Ressource für die Landwirtschaft und die Trinkwasserversorgung in den östlichen Provinzen.",
      hu: "A Kotto-folyó a Közép-afrikai Köztársaság keleti részének meghatározó földrajzi eleme, és az Ubangi-folyó jelentős mellékfolyója. A Bongo-masszívumban ered, a szudáni határ közelében, és több mint 660 kilométeren keresztül folyik déli irányba. A folyó híres a Kembe-vízesésről, amely egy látványos természeti jelenség, ahol a víz hatalmas sziklaalakzatokon zúdul le. A Kotto partjait sűrű galériaerdők szegélyezik, amelyek gazdag biológiai sokféleségnek adnak otthont, beleértve számos főemlőst és ritka madárfajt. Bria városa közelében a folyó kiszélesedik, fontos halászati területeket biztosítva a helyi lakosság számára. A folyó hidrológiai dinamikáját a régió kifejezett esős és száraz évszakai határozzák meg, a vízhozam nyár végén éri el csúcspontját. Szállítási útvonalként a Kotto a számos zúgó miatt csak korlátozottan használható, ami azonban hozzájárult érintetlen természetének megőrzéséhez. Továbbra is létfontosságú erőforrás marad a mezőgazdaság és az ivóvízellátás számára a keleti tartományokban.",
      ro: "Râul Kotto este o trăsătură geografică proeminentă în estul Republicii Centrafricane și un afluent major al râului Ubangi. Acesta izvorăște din masivul Bongo, lângă granița cu Sudanul, și curge spre sud pe o distanță de peste 660 de kilometri. Râul este faimos pentru cascadele Kembe, un spectacol natural uimitor unde apa se prăbușește peste formațiuni stâncoase masive. Malurile râului Kotto sunt mărginite de păduri-galerie dense, care adăpostesc o biodiversitate bogată, inclusiv numeroase primate și specii de păsări rare. În apropierea orașului Bria, râul se lărgește și oferă zone de pescuit importante pentru populația locală. Dinamica hidrologică a râului este determinată de sezoanele ploioase și secetoase pronunțate ale regiunii, debitul atingând punctul maxim la sfârșitul verii. Ca rută de transport, Kotto este utilizabil doar limitat din cauza numeroaselor praguri, ceea ce a contribuit însă la conservarea naturii sale virgine. Rămâne o resursă vitală pentru agricultură și aprovizionarea cu apă potabilă în provinciile estice.",
      en: "The Kotto River is a prominent geographical feature in the eastern Central African Republic and a major tributary of the Ubangi River. It originates in the Bongo Massif near the border with Sudan and flows southward for over 660 kilometers. The river is famous for the Kembe Falls, a spectacular natural sight where water plunges over massive rock formations. The banks of the Kotto are lined with dense gallery forests that host a rich biodiversity, including numerous primates and rare bird species. Near the city of Bria, the river widens, providing important fishing grounds for the local population. The river's hydrological dynamics are determined by the region's distinct rainy and dry seasons, with the discharge reaching its peak in late summer. As a transport route, the Kotto is only limitedly usable due to numerous rapids, which has helped preserve its pristine nature. It remains a vital resource for agriculture and drinking water supply in the eastern provinces.",
    },
    factsAdvanced: {
      de: ["Die Gesamtlänge des Flusses beträgt ungefähr 660 Kilometer.", "Die berühmten Kembe-Wasserfälle haben eine Fallhöhe von etwa 25 Metern.", "Er mündet südlich von Mobaye in den großen Ubangi-Fluss.", "Das Bongo-Massiv dient als primäre Quelle des Flusses im Nordosten.", "In der Region um Bria wurde 1947 mit dem Diamantenabbau im Flusssediment begonnen.", "Die durchschnittliche jährliche Abflussrate liegt bei etwa 450 Kubikmetern pro Sekunde."],
      hu: ["A folyó teljes hossza körülbelül 660 kilométer.", "A híres Kembe-vízesés esési magassága körülbelül 25 méter.", "Mobaye-től délre torkollik a nagy Ubangi-folyóba.", "A Bongo-masszívum szolgál a folyó elsődleges forrásaként az északkeleti részen.", "Bria környékén 1947-ben kezdték meg a gyémántbányászatot a folyami hordalékból.", "Az átlagos évi vízhozam körülbelül 450 köbméter másodpercenként."],
      ro: ["Lungimea totală a râului este de aproximativ 660 de kilometri.", "Celebrele cascade Kembe au o înălțime de cădere de aproximativ 25 de metri.", "Se varsă în marele râu Ubangi, la sud de orașul Mobaye.", "Masivul Bongo servește ca sursă primară a râului în nord-est.", "Mineritul de diamante în sedimentele râului a început în regiunea Bria în 1947.", "Rata medie anuală de descărcare este de aproximativ 450 de metri cubi pe secundă."],
      en: ["The total length of the river is approximately 660 kilometers.", "The famous Kembe Falls have a drop height of about 25 meters.", "It empties into the great Ubangi River south of the city of Mobaye.", "The Bongo Massif serves as the river's primary source in the northeast.", "Diamond mining in the river sediment began in the Bria region in 1947.", "The average annual discharge rate is approximately 450 cubic meters per second."],
      es: [],
      pt: [],
      fr: [],
    },
    
    description: {
      de: "Dieser Fluss fließt durch das Herz des Landes und ist bekannt für seine zahlreichen Wasserfälle.",
      hu: "Ez a folyó az ország szívén folyik keresztül, és számos vízeséséről ismert.",
      ro: "Acest râu curge prin inima țării și este cunoscut pentru numeroasele sale cascade.",
      en: "This river flows through the heart of the country and is known for its numerous waterfalls.",
      es: "Este río fluye por el corazón del país y es conocido por sus numerosas cascadas.",
      pt: "Este rio flui pelo coração do país e é conhecido pelas suas numerosas cascatas.",
      fr: "Cette rivière coule au cœur du pays et est réputée pour ses nombreuses chutes d'eau.",
    },
    facts: {
      de: ["Die Kembé-Wasserfälle liegen an diesem Fluss.", "Wichtiger Transportweg in der Haute-Kotto Region."],
      hu: ["A Kembé-vízesések ezen a folyón találhatók.", "Fontos szállítási útvonal a Haute-Kotto régióban."],
      ro: ["Cascadele Kembé sunt situate pe acest râu.", "Rută importantă de transport în regiunea Haute-Kotto."],
      en: ["The Kembé Falls are located on this river.", "Significant transport route in the Haute-Kotto region."],
      es: ["Las cataratas de Kembé se encuentran en este río.", "Importante vía de transporte en la región de Haute-Kotto."],
      pt: ["As cataratas de Kembé situam-se neste rio.", "Importante rota de transporte na região de Haute-Kotto."],
      fr: ["Les chutes de Kembé sont situées sur cette rivière.", "Voie de transport importante dans la région de la Haute-Kotto."],
    }, image: "/poi-images/kotto-river-nature-v2.webp"},
  {
    id: "mambere-river-nature-v2",
    type: "river",
    parent: "CF-MK",
    coords: [15.8000, 4.2000],
    name: {
      de: "Mambéré-Fluss",
      hu: "Mambéré folyó",
      ro: "Râul Mambéré",
      en: "Mambéré River"},
    image: "/poi-images/mambere-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Mambéré-Fluss ist ein bedeutender Wasserlauf im Westen der Zentralafrikanischen Republik, der eine zentrale Rolle in der regionalen Hydrologie spielt. Er entspringt im Yadé-Massiv und fließt über hunderte Kilometer durch üppige Tropenwälder und Savannenlandschaften. Gemeinsam mit dem Kadéï-Fluss bildet er bei der Stadt Nola den Sangha-Fluss, der schließlich in den Kongo mündet. Der Mambéré ist bekannt für seine reiche Fischfauna, die die Grundlage für die lokale Wirtschaft in Städten wie Berbérati bildet. Entlang seiner Ufer finden sich oft dichte Wälder, die Lebensraum für Waldelefanten und Gorillas bieten. Historisch gesehen war der Fluss ein wichtiger Weg für den Holztransport, eine Industrie, die die Region wirtschaftlich geprägt hat. Die jahreszeitlichen Schwankungen des Wasserstands beeinflussen die Schiffbarkeit und den Lebensrhythmus der am Ufer lebenden Bevölkerung erheblich. In der Trockenzeit bilden sich oft Sandbänke, während die Regenzeit den Fluss in einen mächtigen Strom verwandelt, der weite Teile der angrenzenden Ebenen flutet.",
      hu: "A Mambéré-folyó a Közép-afrikai Köztársaság nyugati részén jelentős vízfolyása, amely központi szerepet játszik a regionális hidrológiában. A Yadé-masszívumban ered, és több száz kilométeren keresztül folyik buja trópusi erdőkön és szavannás tájakon. A Kadéï-folyóval együtt Nola városánál alkotja a Sangha-folyót, amely végül a Kongóba torkollik. A Mambéré gazdag halállományáról ismert, amely az olyan városok helyi gazdaságának alapját képezi, mint Berbérati. Partjai mentén gyakran sűrű erdők találhatók, amelyek erdei elefántok és gorillák élőhelyéül szolgálnak. Történelmileg a folyó fontos útvonal volt a fakitermelés számára, amely iparág gazdaságilag meghatározta a régiót. A vízszint évszakos ingadozása jelentősen befolyásolja a hajózhatóságot és a parton élő lakosság életritmusát. A száraz évszakban gyakran homokpadok alakulnak ki, míg az esős évszak hatalmas folyammmá változtatja a folyót, amely az összefüggő síkságok nagy részét elárasztja.",
      ro: "Râul Mambéré este un curs de apă major în vestul Republicii Centrafricane, jucând un rol central în hidrologia regională. Acesta izvorăște din masivul Yadé și curge pe sute de kilometri prin păduri tropicale luxuriante și peisaje de savană. Împreună cu râul Kadéï, formează râul Sangha în orașul Nola, care în cele din urmă se varsă în Congo. Mambéré este cunoscut pentru fauna sa piscicolă bogată, care constituie baza economiei locale în orașe precum Berbérati. Pe malurile sale se găsesc adesea păduri dense care oferă habitat pentru elefanții de pădure și gorile. Din punct de vedere istoric, râul a fost o cale importantă pentru transportul lemnului, o industrie care a modelat regiunea din punct de vedere economic. Fluctuațiile sezoniere ale nivelului apei afectează semnificativ navigabilitatea și ritmul de viață al populației riverane. În sezonul secetos se formează adesea bancuri de nisip, în timp ce sezonul ploios transformă râul într-un flux puternic care inundă părți mari din câmpiile adiacente.",
      en: "The Mambéré River is a major watercourse in the western Central African Republic, playing a central role in regional hydrology. It originates in the Yadé Massif and flows for hundreds of kilometers through lush tropical forests and savanna landscapes. Together with the Kadéï River, it forms the Sangha River at the city of Nola, which eventually flows into the Congo. The Mambéré is known for its rich fish fauna, which forms the basis of the local economy in cities like Berbérati. Along its banks, dense forests are often found, providing habitat for forest elephants and gorillas. Historically, the river was an important route for timber transport, an industry that has economically shaped the region. Seasonal fluctuations in the water level significantly affect the navigability and the life rhythm of the riparian population. In the dry season, sandbanks often form, while the rainy season transforms the river into a mighty stream that floods large parts of the adjacent plains.",
    },
    factsAdvanced: {
      de: ["Das Einzugsgebiet des Mambéré umfasst etwa 27.000 Quadratkilometer.", "Der Zusammenfluss mit dem Kadéï erfolgt bei der Stadt Nola.", "Berbérati, die zweitgrößte Stadt des Landes, liegt im Einzugsbereich des Flusses.", "Der Fluss ist Teil des größeren Sangha-Tri-National-Schutzgebiets.", "Bereits im späten 19. Jahrhundert wurde der Fluss für den Kautschuktransport genutzt.", "Die durchschnittliche Tiefe variiert zwischen 2 und 6 Metern je nach Saison."],
      hu: ["A Mambéré vízgyűjtő területe körülbelül 27 000 négyzetkilométer.", "A Kadéï-vel való összefolyás Nola városánál történik.", "Berbérati, az ország második legnagyobb városa, a folyó vízgyűjtő területén található.", "A folyó a nagyobb Sangha Tri-National védett terület része.", "Már a 19. század végén használták a folyót kaucsukszállításra.", "Az átlagos mélység 2 és 6 méter között változik az évszaktól függően."],
      ro: ["Bazinul hidrografic al Mambéré cuprinde aproximativ 27.000 de kilometri pătrați.", "Confluența cu Kadéï are loc în orașul Nola.", "Berbérati, al doilea oraș ca mărime din țară, se află în bazinul râului.", "Râul face parte din aria protejată mai mare Sangha Tri-National.", "Râul a fost folosit pentru transportul cauciucului încă de la sfârșitul secolului al XIX-lea.", "Adâncimea medie variază între 2 și 6 metri, în funcție de sezon."],
      en: ["The drainage basin of the Mambéré covers approximately 27,000 square kilometers.", "The confluence with the Kadéï occurs at the city of Nola.", "Berbérati, the country's second-largest city, is located within the river's basin.", "The river is part of the larger Sangha Tri-National protected area.", "As early as the late 19th century, the river was used for rubber transport.", "The average depth varies between 2 and 6 meters depending on the season."],
      es: [],
      pt: [],
      fr: [],
    },
    
    description: {
      de: "Ein wichtiger Fluss im Westen, der zusammen mit dem Kadeï den Sangha bildet.",
      hu: "Fontos folyó nyugaton, amely a Kadeï folyóval együtt alkotja a Sangha folyót.",
      ro: "Un râu important din vest care, împreună cu râul Kadeï, formează râul Sangha.",
      en: "An important river in the west that, together with the Kadeï, forms the Sangha River.",
      es: "Un río clave en el oeste que, junto con el Kadeï, forma el río Sangha.",
      pt: "Um rio fundamental no oeste que, juntamente com o Kadeï, forma o rio Sangha.",
      fr: "Une rivière importante de l'ouest qui, avec la Kadeï, forme la Sangha.",
    },
    facts: {
      de: ["Bekannt für die Diamantenvorkommen in seinem Becken.", "Fließt durch fruchtbares Hügelland."],
      hu: ["A medencéjében található gyémántlelőhelyekről ismert.", "Termékeny dombvidéken folyik keresztül."],
      ro: ["Cunoscut pentru depozitele de diamante din bazinul său.", "Curge prin dealuri fertile."],
      en: ["Known for diamond deposits in its basin.", "Flows through fertile hilly terrain."],
      es: ["Conocido por los yacimientos de diamantes en su cuenca.", "Fluye a través de fértiles tierras de colinas."],
      pt: ["Conhecido pelas jazidas de diamantes na sua bacia.", "Flui através de terras férteis de colinas."],
      fr: ["Connu pour les gisements de diamants dans son bassin.", "Coule à travers des collines fertiles."],
    }, image: "/poi-images/mambere-river-nature-v2.webp"},
  {
    id: "kadei-river-nature-v2",
    type: "river",
    parent: "CF-MK",
    coords: [16.0000, 3.5000],
    name: {
      de: "Kadeï-Fluss",
      hu: "Kadeï folyó",
      ro: "Râul Kadeï",
      en: "Kadeï River"},
    image: "/poi-images/kadei-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Kadéï-Fluss ist ein bedeutendes Gewässer im Südwesten der Zentralafrikanischen Republik, das seinen Ursprung im Osten Kameruns hat. Er fließt in östlicher Richtung durch dichte äquatoriale Regenwälder, bevor er sich in Nola mit dem Mambéré vereinigt, um den Sangha-Fluss zu bilden. Der Kadéï ist für seine strategische Bedeutung im regionalen Handel bekannt, da er als natürlicher Transportweg für Ressourcen dient. Die Ökologie des Flusses ist geprägt von einer hohen Biodiversität, wobei die angrenzenden Wälder Rückzugsorte für seltene Tierarten wie Bongos und Schimpansen sind. In der Nähe der Stadt Gamboula markiert der Fluss wichtige kulturelle und wirtschaftliche Grenzen. Die Wasserqualität und die ökologische Integrität des Kadéï sind für die Aufrechterhaltung des Sangha-Ökosystems von entscheidender Bedeutung. Durch seine konstante Wasserführung auch in trockeneren Monaten ist er eine verlässliche Quelle für die lokale Landwirtschaft. Die Uferregionen sind zudem für ihre reiche Flora bekannt, die oft von botanischen Expeditionen erforscht wurde.",
      hu: "A Kadéï-folyó a Közép-afrikai Köztársaság délnyugati részének jelentős vízfolyása, amely Kamerun keleti részén ered. Keleti irányba folyik át a sűrű egyenlítői esőerdőkön, mielőtt Nolában egyesülne a Mambérével, hogy létrehozzák a Sangha-folyót. A Kadéï a regionális kereskedelemben betöltött stratégiai jelentőségéről ismert, mivel az erőforrások természetes szállítási útvonalaként szolgál. A folyó ökológiáját magas biológiai sokféleség jellemzi, a szomszédos erdők pedig olyan ritka állatfajok menedékhelyei, mint a bongók és a csimpánzok. Gamboula városa közelében a folyó fontos kulturális és gazdasági határokat jelöl. A Kadéï vízminősége és ökológiai integritása döntő fontosságú a Sangha-ökoszisztéma fenntartásához. Mivel még a szárazabb hónapokban is állandó a vízhozama, megbízható forrás a helyi mezőgazdaság számára. A part menti régiók gazdag flórájukról is ismertek, amelyet gyakran botanikai expedíciók kutattak.",
      ro: "Râul Kadéï este un curs de apă semnificativ în sud-vestul Republicii Centrafricane, care își are originea în estul Camerunului. Acesta curge spre est prin păduri tropicale ecuatoriale dense înainte de a se uni cu Mambéré în Nola pentru a forma râul Sangha. Kadéï este cunoscut pentru importanța sa strategică în comerțul regional, servind ca o rută naturală de transport pentru resurse. Ecologia râului este marcată de o biodiversitate ridicată, pădurile adiacente fiind refugii pentru specii de animale rare, cum ar fi bongos și cimpanzeii. Lângă orașul Gamboula, râul marchează granițe culturale și economice importante. Calitatea apei și integritatea ecologică a Kadéï sunt de o importanță crucială pentru menținerea ecosistemului Sangha. Datorită fluxului său constant de apă chiar și în lunile mai secetoase, este o sursă de încredere pentru agricultura locală. Regiunile riverane sunt, de asemenea, cunoscute pentru flora lor bogată, care a fost adesea explorată de expediții botanice.",
      en: "The Kadéï River is a significant watercourse in the southwestern Central African Republic, originating in eastern Cameroon. It flows eastward through dense equatorial rainforests before joining the Mambéré at Nola to form the Sangha River. The Kadéï is known for its strategic importance in regional trade, serving as a natural transport route for resources. The river's ecology is characterized by high biodiversity, with adjacent forests serving as refuges for rare animal species such as bongos and chimpanzees. Near the city of Gamboula, the river marks important cultural and economic boundaries. The water quality and ecological integrity of the Kadéï are of crucial importance for maintaining the Sangha ecosystem. Due to its constant water flow even in drier months, it is a reliable source for local agriculture. The riparian regions are also known for their rich flora, which has often been explored by botanical expeditions.",
    },
    factsAdvanced: {
      de: ["Der Fluss hat eine Gesamtlänge von ungefähr 510 Kilometern.", "Er bildet die Grenze zwischen der ZAR und Kamerun auf einer Teilstrecke.", "Nola dient als wichtigster Hafenort am Zusammenfluss zum Sangha.", "Der Flusslauf ist für seine zahlreichen Stromschnellen im Oberlauf bekannt.", "Im Jahr 1924 wurde das Gebiet als wichtiges Zentrum für den Holzhandel etabliert.", "Das Gefälle des Flusses beträgt auf der Hauptstrecke etwa 0,5 Meter pro Kilometer."],
      hu: ["A folyó teljes hossza körülbelül 510 kilométer.", "Egy szakaszon a Közép-afrikai Köztársaság és Kamerun közötti határt alkotja.", "Nola a legfontosabb kikötőváros a Sangha-folyó összefolyásánál.", "A folyó felső szakasza számos zúgójáról ismert.", "1924-ben a területet a fakereskedelem egyik fontos központjaként alapították meg.", "A folyó esése a fő szakaszon körülbelül 0,5 méter kilométerenként."],
      ro: ["Râul are o lungime totală de aproximativ 510 kilometri.", "Formează granița dintre Republica Centrafricană și Camerun pe o secțiune.", "Nola servește drept cel mai important port la confluența cu Sangha.", "Cursul râului este cunoscut pentru numeroasele sale praguri în cursul superior.", "În 1924, zona a fost stabilită ca un centru important pentru comerțul cu lemn.", "Panta râului pe traseul principal este de aproximativ 0,5 metri pe kilometru."],
      en: ["The river has a total length of approximately 510 kilometers.", "It forms the border between the CAR and Cameroon along part of its course.", "Nola serves as the most important port at the confluence with the Sangha.", "The river's course is known for its numerous rapids in the upper reaches.", "In 1924, the area was established as an important center for the timber trade.", "The gradient of the river along the main stretch is about 0.5 meters per kilometer."],
      es: [],
      pt: [],
      fr: [],
    },
    
    description: {
      de: "Ein westlicher Fluss, der in Kamerun entspringt und in Nola mit dem Mambéré zusammenfließt.",
      hu: "Nyugati folyó, amely Kamerunban ered, és Nolánál egyesül a Mambéré folyóval.",
      ro: "Un râu vestic care izvorăște din Camerun și se unește cu Mambéré la Nola.",
      en: "A western river originating in Cameroon and joining the Mambéré at Nola.",
      es: "Un río occidental que nace en Camerún y confluye con el Mambéré en Nola.",
      pt: "Um rio ocidental que nasce nos Camarões e se funde com o Mambéré em Nola.",
      fr: "Une rivière de l'ouest qui prend sa source au Cameroun et conflue avec la Mambéré à Nola.",
    },
    facts: {
      de: ["Bildet den Oberlauf des Sangha-Flusses.", "Wichtiger Handelsweg für Holz."],
      hu: ["A Sangha folyó felső szakaszát alkotja.", "Fontos kereskedelmi útvonal a fa számára."],
      ro: ["Formează cursul superior al râului Sangha.", "Rută comercială importantă pentru lemn."],
      en: ["Forms the upper course of the Sangha River.", "Important trade route for timber."],
      es: ["Forma el curso superior del río Sangha.", "Importante ruta comercial para la madera."],
      pt: ["Forma o curso superior do rio Sangha.", "Rota comercial importante para a madeira."],
      fr: ["Forme le cours supérieur de la rivière Sangha.", "Importante voie commerciale pour le bois."],
    }, image: "/poi-images/kadei-river-nature-v2.webp"},
  {
    id: "ouaka-river-nature-v2",
    type: "river",
    parent: "CF-UK",
    coords: [20.6000, 5.8000],
    name: {
      de: "Ouaka-Fluss",
      hu: "Ouaka folyó",
      ro: "Râul Ouaka",
      en: "Ouaka River"},
    image: "/poi-images/ouaka-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Die Ouaka ist ein bedeutender Fluss in der zentralen Region der Zentralafrikanischen Republik und ein wichtiger Nebenfluss des Ubangi. Sie fließt durch eine Landschaft, die von Savannen und vereinzelten Waldgebieten geprägt ist, und passiert dabei die wichtige Stadt Bambari. Der Fluss ist eine lebenswichtige Ressource für die örtliche Bevölkerung, da er sowohl als Wasserquelle für den häuslichen Gebrauch als auch für die Bewässerung kleinerer landwirtschaftlicher Flächen dient. Die Ouaka ist zudem für ihre Fischbestände bekannt, die eine wichtige Proteinquelle für die Bewohner der Region darstellen. Die Brücke über die Ouaka in Bambari ist ein zentrales Infrastrukturelement, das den Norden und Süden des Landes miteinander verbindet. In der Regenzeit kann der Fluss stark anschwellen, was gelegentlich zu Überschwemmungen führt, aber auch die Fruchtbarkeit der angrenzenden Felder sichert. Das Becken der Ouaka beherbergt eine Vielzahl von Vögeln und Reptilien, was sie zu einem interessanten Ort für Naturbeobachtungen macht. Trotz politischer Instabilität in der Region bleibt der Fluss eine Konstante im täglichen Leben der Menschen.",
      hu: "Az Ouaka a Közép-afrikai Köztársaság központi régiójának jelentős folyója, és az Ubangi egyik fontos mellékfolyója. Szavannákkal és elszórt erdős területekkel tarkított tájon folyik keresztül, miközben érinti Bambari fontos városát. A folyó létfontosságú erőforrás a helyi lakosság számára, mivel vízforrásként szolgál a háztartási használathoz és a kisebb mezőgazdasági területek öntözéséhez is. Az Ouaka halállományáról is ismert, amely fontos fehérjeforrást jelent a régió lakói számára. A bambari Ouaka-híd központi infrastruktúra-elem, amely összeköti az ország északi és déli részét. Az esős évszakban a folyó erősen megduzzadhat, ami alkalmanként áradásokhoz vezet, de biztosítja a szomszédos szántóföldek termékenységét is. Az Ouaka medencéje számos madárnak és hüllőnek ad otthont, ami érdekessé teszi a természetmegfigyelés szempontjából. A régió politikai instabilitása ellenére a folyó állandó elem marad az emberek mindennapi életében.",
      ro: "Ouaka este un râu important în regiunea centrală a Republicii Centrafricane și un afluent major al Ubangi. Acesta curge printr-un peisaj dominat de savane și zone forestiere izolate, trecând prin orașul important Bambari. Râul este o resursă vitală pentru populația locală, servind atât ca sursă de apă pentru uz casnic, cât și pentru irigarea micilor suprafețe agricole. Ouaka este, de asemenea, cunoscut pentru populațiile sale de pești, care reprezintă o sursă importantă de proteine pentru locuitorii regiunii. Podul peste Ouaka din Bambari este un element central de infrastructură care leagă nordul și sudul țării. În sezonul ploios, râul se poate umfla puternic, ceea ce duce ocazional la inundații, dar asigură și fertilitatea câmpurilor adiacente. Bazinul Ouaka găzduiește o varietate de păsări și reptile, ceea ce îl face un loc interesant pentru observarea naturii. În ciuda instabilității politice din regiune, râul rămâne o constantă în viața de zi cu zi a oamenilor.",
      en: "The Ouaka is a major river in the central region of the Central African Republic and an important tributary of the Ubangi. It flows through a landscape characterized by savannas and scattered forest areas, passing through the key city of Bambari. The river is a vital resource for the local population, serving as both a water source for domestic use and for the irrigation of small agricultural plots. The Ouaka is also known for its fish stocks, which are an important protein source for the region's inhabitants. The bridge over the Ouaka in Bambari is a central infrastructure element connecting the north and south of the country. During the rainy season, the river can swell significantly, occasionally leading to flooding but also ensuring the fertility of the adjacent fields. The Ouaka basin hosts a variety of birds and reptiles, making it an interesting spot for nature observation. Despite political instability in the region, the river remains a constant in people's daily lives.",
    },
    factsAdvanced: {
      de: ["Die Länge der Ouaka beträgt ungefähr 550 Kilometer.", "Der Fluss mündet etwa 100 Kilometer flussabwärts von Bangui in den Ubangi.", "Bambari, die Hauptstadt der Präfektur Ouaka, liegt direkt an seinen Ufern.", "Die strategische Ouaka-Brücke wurde in ihrer ursprünglichen Form 1950 erbaut.", "Das Flussbecken ist für seine fruchtbaren Alluvialböden bekannt.", "Die jährliche Niederschlagsmenge im Becken liegt im Durchschnitt bei 1.400 mm."],
      hu: ["Az Ouaka hossza körülbelül 550 kilométer.", "A folyó Banguitól körülbelül 100 kilométerre lefelé torkollik az Ubangi-ba.", "Bambari, Ouaka prefektúra székhelye, közvetlenül a partján fekszik.", "A stratégiai Ouaka-hidat eredeti formájában 1950-ben építették.", "A folyómedence termékeny hordalékos talajáról ismert.", "Az évi átlagos csapadékmennyiség a medencében 1400 mm."],
      ro: ["Lungimea râului Ouaka este de aproximativ 550 de kilometri.", "Râul se varsă în Ubangi la aproximativ 100 de kilometri în aval de Bangui.", "Bambari, capitala prefecturii Ouaka, este situat direct pe malurile sale.", "Podul strategic peste Ouaka a fost construit în forma sa originală în 1950.", "Bazinul râului este cunoscut pentru solurile sale aluviale fertile.", "Cantitatea anuală de precipitații în bazin este în medie de 1.400 mm."],
      en: ["The length of the Ouaka is approximately 550 kilometers.", "The river empties into the Ubangi about 100 kilometers downstream from Bangui.", "Bambari, the capital of the Ouaka prefecture, is located directly on its banks.", "The strategic Ouaka Bridge was built in its original form in 1950.", "The river basin is known for its fertile alluvial soils.", "The average annual rainfall in the basin is around 1,400 mm."],
      es: [],
      pt: [],
      fr: [],
    },
    
    description: {
      de: "Ein Nebenfluss des Ubangi, der durch das Zentrum des Landes fließt und der Präfektur Ouaka ihren Namen gibt.",
      hu: "Az Ubangi egyik mellékfolyója, amely az ország közepén folyik keresztül, és nevet ad Ouaka prefektúrának.",
      ro: "Un afluent al râului Ubangi care curge prin centrul țării și dă numele prefecturii Ouaka.",
      en: "A tributary of the Ubangi that flows through the center of the country and gives its name to the Ouaka Prefecture.",
      es: "Un afluente del Ubangui que fluye por el centro del país y da nombre a la prefectura de Ouaka.",
      pt: "Um afluente do Ubangui que corre pelo centro do país e dá nome à prefeitura de Ouaka.",
      fr: "Un affluent de l'Oubangui qui traverse le centre du pays et donne son nom à la préfecture d'Ouaka.",
    },
    facts: {
      de: ["Bambari liegt an seinen Ufern.", "Wichtig für die lokale Landwirtschaft."],
      hu: ["Bambari a partján fekszik.", "Fontos a helyi mezőgazdaság számára."],
      ro: ["Bambari este situat pe malurile sale.", "Important pentru agricultura locală."],
      en: ["Bambari is situated on its banks.", "Vital for local agriculture."],
      es: ["Bambari se encuentra a sus orillas.", "Importante para la agricultura local."],
      pt: ["Bambari situa-se nas suas margens.", "Importante para a agricultura local."],
      fr: ["Bambari est située sur ses rives.", "Important pour l'agriculture locale."],
    }, image: "/poi-images/ouaka-river-nature-v2.webp"},
  {
    id: "bamingui-river-nature-v2",
    type: "river",
    parent: "CF-BB",
    coords: [19.5000, 7.5000],
    name: {
      de: "Bamingui-Fluss",
      hu: "Bamingui folyó",
      ro: "Râul Bamingui",
      en: "Bamingui River"},
    image: "/poi-images/bamingui-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Bamingui-Fluss ist ein zentrales hydrologisches Element im Norden der Zentralafrikanischen Republik und Namensgeber des Bamingui-Bangoran-Nationalparks. Er entspringt im zentralen Hochland und fließt in nordwestlicher Richtung, wobei er weite, unberührte Wildnisgebiete durchquert. Der Fluss ist entscheidend für das Überleben zahlreicher Wildtiere in der Region, darunter Elefanten, Löwen und diverse Antilopenarten, die während der Trockenzeit an seine Ufer kommen. Die Umgebung des Bamingui ist von typischer Sudano-Guineischer Savanne geprägt, die in Flussnähe in dichte Galeriewälder übergeht. Historisch gesehen war die Region ein Rückzugsort für viele Tierarten, wurde jedoch in den letzten Jahrzehnten durch Wilderei und Instabilität herausgefordert. Dennoch bleibt der Bamingui ein Symbol für die ungezähmte Natur des afrikanischen Kontinents. Er vereinigt sich später mit dem Bangoran, um den Bamingui-Fluss fortzusetzen, der schließlich zum Schari-System gehört. Forscher schätzen den Fluss für seine Bedeutung bei der Erforschung der limnischen Ökosysteme Zentralafrikas.",
      hu: "A Bamingui-folyó a Közép-afrikai Köztársaság északi részének központi hidrológiai eleme, és a Bamingui-Bangoran Nemzeti Park névadója. A központi felföldön ered, és északnyugati irányba folyik, hatalmas, érintetlen vadonokat szelve át. A folyó döntő fontosságú a régió számos vadon élő állatának túlélése szempontjából, beleértve az elefántokat, oroszlánokat és különféle antilopfajokat, amelyek a száraz évszakban a partjaihoz vándorolnak. A Bamingui környezetét tipikus szudáni-guineai szavanna jellemzi, amely a folyó közelében sűrű galériaerdőkbe megy át. Történelmileg a régió sok állatfaj menedékhelye volt, de az elmúlt évtizedekben az orvvadászat és az instabilitás komoly kihívások elé állította. Ennek ellenére a Bamingui az afrikai kontinens megszelídítetlen természetének szimbóluma marad. Később egyesül a Bangorannal, hogy folytassa útját a Bamingui-folyóként, amely végül a Csád-rendszerhez tartozik. A kutatók nagyra értékelik a folyót Közép-Afrika limnikus ökoszisztémáinak tanulmányozásában betöltött szerepe miatt.",
      ro: "Râul Bamingui este un element hidrologic central în nordul Republicii Centrafricane și cel care dă numele Parcului Național Bamingui-Bangoran. Izvorăște din podișul central și curge în direcția nord-vest, traversând zone vaste de sălbăticie virgină. Râul este crucial pentru supraviețuirea a numeroase animale sălbatice din regiune, inclusiv elefanți, lei și diverse specii de antilope, care vin la malurile sale în timpul sezonului secetos. Împrejurimile râului Bamingui sunt caracterizate de savana tipică sudano-guineeană, care trece în păduri-galerie dense în apropierea râului. Din punct de vedere istoric, regiunea a fost un refugiu pentru multe specii de animale, dar a fost pusă la încercare în ultimele decenii de braconaj și instabilitate. Cu toate acestea, Bamingui rămâne un simbol al naturii neîmblânzite a continentului african. Se unește ulterior cu Bangoran pentru a continua cursul râului Bamingui, care aparține în cele din urmă sistemului Chari. Cercetătorii apreciază râul pentru importanța sa în studiul ecosistemelor limnice din Africa Centrală.",
      en: "The Bamingui River is a central hydrological element in the northern Central African Republic and the namesake of the Bamingui-Bangoran National Park. It originates in the central highlands and flows in a northwestern direction, traversing vast, pristine wilderness areas. The river is crucial for the survival of numerous wildlife species in the region, including elephants, lions, and various antelope species that come to its banks during the dry season. The environment surrounding the Bamingui is characterized by typical Sudano-Guinean savanna, which transitions into dense gallery forests near the river. Historically, the region was a refuge for many animal species but has been challenged in recent decades by poaching and instability. Nevertheless, the Bamingui remains a symbol of the untamed nature of the African continent. It later joins the Bangoran to continue as the Bamingui River, which eventually belongs to the Chari system. Researchers value the river for its importance in studying the limnic ecosystems of Central Africa.",
    },
    factsAdvanced: {
      de: ["Die Länge des Bamingui beträgt ungefähr 450 Kilometer.", "Der Bamingui-Bangoran-Nationalpark wurde bereits im Jahr 1933 gegründet.", "Er ist ein wichtiger Zufluss zum Schari-Flusssystem.", "Der Fluss dient als natürliche Tränke für über 40 Säugetierarten.", "In der Region wurden Expeditionen zur Erforschung des Westafrikanischen Krokodils durchgeführt.", "Die durchschnittliche Breite des Flusses im Mittellauf beträgt etwa 50 Meter."],
      hu: ["A Bamingui hossza körülbelül 450 kilométer.", "A Bamingui-Bangoran Nemzeti Parkot már 1933-ban megalapították.", "A Csád-folyórendszer egyik fontos mellékfolyója.", "A folyó természetes itatóként szolgál több mint 40 emlősfaj számára.", "A régióban expedíciókat folytattak a nyugat-afrikai krokodil kutatására.", "A folyó átlagos szélessége a középső szakaszon körülbelül 50 méter."],
      ro: ["Lungimea râului Bamingui este de aproximativ 450 de kilometri.", "Parcul Național Bamingui-Bangoran a fost înființat încă din anul 1933.", "Este un afluent important al sistemului râului Chari.", "Râul servește ca adăpătoare naturală pentru peste 40 de specii de mamifere.", "În regiune au fost efectuate expediții pentru studierea crocodilului vest-african.", "Lățimea medie a râului în cursul mijlociu este de aproximativ 50 de metri."],
      en: ["The length of the Bamingui is approximately 450 kilometers.", "The Bamingui-Bangoran National Park was established as early as 1933.", "It is an important tributary to the Chari River system.", "The river serves as a natural watering hole for over 40 mammal species.", "Expeditions to study the West African crocodile have been conducted in the region.", "The average width of the river in its middle reaches is about 50 meters."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein Fluss im Norden, der Teil des Einzugsgebiets des Chari-Flusses ist und durch Wildschutzgebiete fließt.",
      hu: "Északi folyó, amely a Chari folyó vízgyűjtő területéhez tartozik, és vadrezervátumokon folyik keresztül.",
      ro: "Un râu în nord, care face parte din bazinul râului Chari și curge prin rezervații de animale sălbatice.",
      en: "A river in the north that is part of the Chari River basin and flows through wildlife reserves.",
      es: "Un río en el norte que forma parte de la cuenca del río Chari y fluye a través de reservas de vida silvestre.",
      pt: "Um rio no norte que faz parte da bacia do rio Chari e corre através de reservas de vida selvagem.",
      fr: "Une rivière du nord qui fait partie du bassin du fleuve Chari et traverse des réserves de faune sauvage.",
    },
    facts: {
      de: ["Namensgeber für den Bamingui-Bangoran-Nationalpark.", "Wichtiger Lebensraum für Krokodile."],
      hu: ["A Bamingui-Bangoran Nemzeti Park névadója.", "Krokodilok fontos élőhelye."],
      ro: ["Dă numele Parcului Național Bamingui-Bangoran.", "Habitat important pentru crocodili."],
      en: ["Eponym for the Bamingui-Bangoran National Park.", "Important habitat for crocodiles."],
      es: ["Da nombre al Parque Nacional Bamingui-Bangoran.", "Hábitat importante para cocodrilos."],
      pt: ["Dá o nome ao Parque Nacional Bamingui-Bangoran.", "Habitat importante para crocodilos."],
      fr: ["Donne son nom au parc national de Bamingui-Bangoran.", "Habitat important pour les crocodiles."],
    }, image: "/poi-images/bamingui-river-nature-v2.webp"},
  {
    id: "bangoran-river-nature-v2",
    type: "river",
    parent: "CF-BB",
    coords: [20.0000, 8.0000],
    name: {
      de: "Bangoran-Fluss",
      hu: "Bangoran folyó",
      ro: "Râul Bangoran",
      en: "Bangoran River"},
    image: "/poi-images/bangoran-river-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Bangoran ist ein bedeutender Fluss im Norden der Zentralafrikanischen Republik und bildet zusammen mit dem Bamingui das Herzstück des Bamingui-Bangoran-Nationalparks. Er fließt durch abgelegene Savannenlandschaften, die für ihre außergewöhnliche Ruhe und natürliche Schönheit bekannt sind. Der Fluss ist weniger für den menschlichen Gebrauch erschlossen, was ihn zu einem idealen Rückzugsort für gefährdete Tierarten macht. Entlang seines Laufs finden sich zahlreiche Sumpfgebiete und Lagunen, die in der Regenzeit wichtige Laichgründe für Fische bieten. Der Bangoran ist zudem bekannt für seine Rolle im Wasserkreislauf der Region, da er große Mengen an Niederschlag aus dem nördlichen Plateaus aufnimmt. Die Vegetation an seinen Ufern ist reich an endemischen Pflanzenarten, die an die wechselnden Wasserstände angepasst sind. In der Vergangenheit war der Bangoran Schauplatz bedeutender ökologischer Studien über das Gleichgewicht zwischen Raubtieren und Beutetieren in der afrikanischen Savanne. Er bleibt ein wesentlicher Bestandteil des grenzüberschreitenden Schutzes ökologischer Korridore im Tschadbecken.",
      hu: "A Bangoran a Közép-afrikai Köztársaság északi részének jelentős folyója, és a Baminguival együtt a Bamingui-Bangoran Nemzeti Park szívét alkotja. Távoli szavannás tájakon folyik keresztül, amelyek kivételes nyugalmukról és természeti szépségükről ismertek. A folyó kevésbé kiépített az emberi használat számára, ami ideális menedékhellyé teszi a veszélyeztetett állatfajok számára. Futása mentén számos mocsaras terület és lagúna található, amelyek az esős évszakban fontos ívóhelyet biztosítanak a halaknak. A Bangoran a régió körforgásában betöltött szerepéről is ismert, mivel nagy mennyiségű csapadékot fogad be az északi fennsíkokról. Partjainak növényzete gazdag endemikus növényfajokban, amelyek alkalmazkodtak a változó vízszinthez. A múltban a Bangoran fontos ökológiai tanulmányok helyszíne volt az afrikai szavannán élő ragadozók és zsákmányállatok közötti egyensúlyról. Továbbra is szerves részét képezi a Csád-medence ökológiai folyosóinak határokon átnyúló védelmének.",
      ro: "Bangoran este un râu important în nordul Republicii Centrafricane și formează, împreună cu Bamingui, nucleul Parcului Național Bamingui-Bangoran. Acesta curge prin peisaje de savană izolate, cunoscute pentru liniștea lor excepțională și frumusețea naturală. Râul este mai puțin dezvoltat pentru uz uman, ceea ce îl face un refugiu ideal pentru speciile de animale pe cale de dispariție. De-a lungul cursului său se găsesc numeroase zone mlăștinoase și lagune, care oferă locuri importante de depunere a icrelor pentru pești în timpul sezonului ploios. Bangoran este cunoscut și pentru rolul său în circuitul apei din regiune, absorbind cantități mari de precipitații de pe platourile nordice. Vegetația de pe malurile sale este bogată în specii de plante endemice, adaptate la nivelurile fluctuante ale apei. În trecut, Bangoran a fost locul unor studii ecologice semnificative privind echilibrul dintre prădători și pradă în savana africană. Rămâne o parte esențială a protecției transfrontaliere a coridoarelor ecologice din bazinul Ciadului.",
      en: "The Bangoran is a major river in the northern Central African Republic and, together with the Bamingui, forms the heart of the Bamingui-Bangoran National Park. It flows through remote savanna landscapes known for their exceptional tranquility and natural beauty. The river is less developed for human use, making it an ideal refuge for endangered animal species. Along its course, numerous marshlands and lagoons are found, providing important spawning grounds for fish during the rainy season. The Bangoran is also known for its role in the region's water cycle, absorbing large amounts of precipitation from the northern plateaus. The vegetation on its banks is rich in endemic plant species adapted to fluctuating water levels. In the past, the Bangoran was the site of significant ecological studies on the balance between predators and prey in the African savanna. It remains an essential part of the cross-border protection of ecological corridors in the Chad Basin.",
    },
    factsAdvanced: {
      de: ["Die Gesamtlänge des Bangoran beträgt etwa 400 Kilometer.", "Er ist Teil des UNESCO-Biosphärenreservats Bamingui-Bangoran.", "Der Fluss mündet am nordwestlichen Rand des Parks in den Bamingui.", "Historische Berichte aus den 1920er Jahren erwähnen riesige Büffelherden an seinen Ufern.", "In der Region wurden seltene Sichtungen des Afrikanischen Wildhundes dokumentiert.", "Das Gefälle des Flusses ist mit etwa 0,3 Metern pro Kilometer sehr gering."],
      hu: ["A Bangoran teljes hossza körülbelül 400 kilométer.", "A Bamingui-Bangoran UNESCO Bioszféra-rezervátum része.", "A folyó a park északnyugati szélén torkollik a Baminguiba.", "Az 1920-as évekből származó történelmi beszámolók hatalmas bivalycsordákat említenek a partjain.", "A régióban dokumentálták az afrikai vadkutya ritka észleléseit.", "A folyó esése nagyon csekély, körülbelül 0,3 méter kilométerenként."],
      ro: ["Lungimea totală a râului Bangoran este de aproximativ 400 de kilometri.", "Face parte din Rezervația Biosferei UNESCO Bamingui-Bangoran.", "Râul se varsă în Bamingui la marginea de nord-vest a parcului.", "Rapoartele istorice din anii 1920 menționează turme uriașe de bivoli pe malurile sale.", "În regiune au fost documentate observații rare ale câinelui sălbatic african.", "Panta râului este foarte mică, de aproximativ 0,3 metri pe kilometru."],
      en: ["The total length of the Bangoran is approximately 400 kilometers.", "It is part of the Bamingui-Bangoran UNESCO Biosphere Reserve.", "The river empties into the Bamingui at the northwestern edge of the park.", "Historical reports from the 1920s mention massive buffalo herds on its banks.", "Rare sightings of the African wild dog have been documented in the region.", "The river's gradient is very low, at about 0.3 meters per kilometer."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein nördlicher Fluss, der zusammen mit dem Bamingui eine bedeutende Naturregion bildet.",
      hu: "Északi folyó, amely a Bamingui folyóval együtt jelentős természeti régiót alkot.",
      ro: "Un râu nordic care, împreună cu Bamingui, formează o regiune naturală semnificativă.",
      en: "A northern river that, along with the Bamingui, forms a significant natural region.",
      es: "Un río del norte que, junto con el Bamingui, forma una importante región natural.",
      pt: "Um rio do norte que, juntamente com o Bamingui, forma uma região natural significativa.",
      fr: "Une rivière septentrionale qui forme avec le Bamingui une région naturelle importante.",
    },
    facts: {
      de: ["Durchfließt unberührte Savannenlandschaften.", "Reich an verschiedenen Fischarten."],
      hu: ["Érintetlen szavannai tájakon folyik keresztül.", "Különböző halfajokban gazdag."],
      ro: ["Traversează peisaje de savană neatinse.", "Bogat în diverse specii de pești."],
      en: ["Flows through untouched savanna landscapes.", "Rich in various fish species."],
      es: ["Atraviesa paisajes de sabana vírgenes.", "Rico en diversas especies de peces."],
      pt: ["Atravessa paisagens de savana virgens.", "Rico em várias espécies de peixes."],
      fr: ["Traverse des paysages de savane préservés.", "Riche en diverses espèces de poissons."],
    }, image: "/poi-images/bangoran-river-nature-v2.webp"},
  {
    id: "mount-ngaoui-nature-v2",
    type: "mountain",
    parent: "CF-NM",
    coords: [14.9167, 5.9500],
    name: {
      de: "Mont Ngaoui",
      hu: "Ngaoui-hegy",
      ro: "Muntele Ngaoui",
      en: "Mount Ngaoui"},
    image: "/poi-images/mount-ngaoui-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Mount Ngaoui ist der höchste Punkt der Zentralafrikanischen Republik und liegt im Yadé-Massiv an der Grenze zu Kamerun. Mit seiner markanten Silhouette dominiert er die umliegende Landschaft und ist ein wichtiges geografisches Wahrzeichen Zentralafrikas. Der Berg ist Teil einer Gebirgskette, die als Fortsetzung des Adamaoua-Plateaus gilt und durch eine einzigartige montane Flora und Fauna besticht. Wanderer und Naturforscher schätzen den Berg für seine spektakulären Aussichten und die Möglichkeit, seltene Pflanzenarten zu entdecken, die nur in diesen Höhenlagen vorkommen. Das Klima auf dem Mount Ngaoui ist deutlich kühler als in den Ebenen, was zur Bildung spezifischer Mikroklimata führt. Historisch gesehen war der Berg ein Orientierungspunkt für Karawanen und Entdecker, die die Region durchquerten. Die Gipfelregion ist oft in Nebel gehüllt, was dem Berg eine mystische Atmosphäre verleiht. Trotz seiner relativen Abgeschiedenheit bleibt er ein stolzes Symbol für die geografische Vielfalt der Republik und zieht vereinzelt Abenteurer an, die die unberührte Natur suchen.",
      hu: "A Mount Ngaoui a Közép-afrikai Köztársaság legmagasabb pontja, amely a Yadé-masszívumban, a kameruni határ mentén található. Jellegzetes sziluettjével uralja a környező tájat, és Közép-Afrika fontos földrajzi nevezetessége. A hegy egy olyan hegylánc része, amely az Adamaoua-fennsík folytatásának tekinthető, és egyedülálló hegyvidéki flórájával és faunájával tűnik ki. A túrázók és természetkutatók nagyra értékelik a hegyet látványos kilátása és a csak ezekben a magasságokban előforduló ritka növényfajok felfedezésének lehetősége miatt. A Mount Ngaoui éghajlata lényegesen hűvösebb, mint a síkságoké, ami specifikus mikroklímák kialakulásához vezet. Történelmileg a hegy tájékozódási pontként szolgált a régiót átszelő karavánok és felfedezők számára. A csúcsrégiót gyakran köd borítja, ami misztikus hangulatot kölcsönöz a hegynek. Viszonylagos elszigeteltsége ellenére az ország földrajzi sokféleségének büszke szimbóluma marad, és időnként vonzza az érintetlen természetet kereső kalandorokat.",
      ro: "Muntele Ngaoui este cel mai înalt punct din Republica Centrafricană și este situat în masivul Yadé, la granița cu Camerunul. Cu silueta sa distinctivă, domină peisajul înconjurător și este un reper geografic important al Africii Centrale. Muntele face parte dintr-un lanț muntos considerat o continuare a platoului Adamaoua și impresionează prin flora și fauna montană unică. Drumeții și cercetătorii naturii apreciază muntele pentru vederile sale spectaculoase și posibilitatea de a descoperi specii de plante rare care se găsesc doar la aceste altitudini. Climatul de pe Muntele Ngaoui este semnificativ mai răcoros decât în câmpii, ceea ce duce la formarea unor microclimate specifice. Din punct de vedere istoric, muntele a fost un punct de orientare pentru caravanele și exploratorii care traversau regiunea. Regiunea vârfului este adesea învăluită în ceață, ceea ce îi conferă muntelui o atmosferă mistică. În ciuda izolării sale relative, acesta rămâne un simbol mândru al diversității geografice a republicii și atrage ocazional aventurieri care caută natura virgină.",
      en: "Mount Ngaoui is the highest point in the Central African Republic, located in the Yadé Massif on the border with Cameroon. With its striking silhouette, it dominates the surrounding landscape and is a major geographic landmark of Central Africa. The mountain is part of a range considered a continuation of the Adamaoua Plateau, featuring unique montane flora and fauna. Hikers and naturalists value the mountain for its spectacular views and the chance to discover rare plant species found only at these altitudes. The climate on Mount Ngaoui is significantly cooler than in the plains, leading to the formation of specific microclimates. Historically, the mountain served as a landmark for caravans and explorers crossing the region. The summit region is often shrouded in mist, giving the mountain a mystical atmosphere. Despite its relative isolation, it remains a proud symbol of the republic's geographic diversity and occasionally attracts adventurers seeking pristine nature.",
    },
    factsAdvanced: {
      de: ["Die offizielle Gipfelhöhe des Mount Ngaoui beträgt 1.410 Meter über dem Meeresspiegel.", "Er bildet einen Teil der internationalen Grenze zwischen der ZAR und Kamerun.", "Geologisch besteht der Berg hauptsächlich aus präkambrischem Granit.", "Die Erstbesteigung durch europäische Vermesser erfolgte um das Jahr 1910.", "Die jährliche Durchschnittstemperatur auf dem Gipfel liegt bei nur 18 Grad Celsius.", "Der Berg ist die Quelle für mehrere kleine Nebenflüsse des Sanaga-Systems."],
      hu: ["A Mount Ngaoui hivatalos csúcsmagassága 1410 méter a tengerszint felett.", "A Közép-afrikai Köztársaság és Kamerun közötti nemzetközi határ részét képezi.", "Földtanilag a hegy főként prekambriumi gránitból áll.", "Az európai földmérők általi első megmászás 1910 körül történt.", "A csúcson az évi átlaghőmérséklet mindössze 18 Celsius-fok.", "A hegy több kisebb, a Sanaga-rendszerhez tartozó mellékfolyó forrásvidéke."],
      ro: ["Înălțimea oficială a vârfului Muntelui Ngaoui este de 1.410 metri deasupra nivelului mării.", "Face parte din granița internațională dintre Republica Centrafricană și Camerun.", "Din punct de vedere geologic, muntele este compus în principal din granit precambrian.", "Prima ascensiune a topografilor europeni a avut loc în jurul anului 1910.", "Temperatura medie anuală la vârf este de doar 18 grade Celsius.", "Muntele este sursa mai multor afluenți mici ai sistemului Sanaga."],
      en: ["The official peak height of Mount Ngaoui is 1,410 meters above sea level.", "It forms part of the international border between the CAR and Cameroon.", "Geologically, the mountain consists primarily of Precambrian granite.", "The first ascent by European surveyors took place around the year 1910.", "The average annual temperature at the summit is only 18 degrees Celsius.", "The mountain is the source for several small tributaries of the Sanaga system."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Der höchste Punkt der Zentralafrikanischen Republik liegt an der Grenze zu Kamerun.",
      hu: "A Közép-afrikai Köztársaság legmagasabb pontja, a kameruni határon fekszik.",
      ro: "Cel mai înalt punct din Republica Centrafricană, situat la granița cu Camerun.",
      en: "The highest point in the Central African Republic, located on the border with Cameroon.",
      es: "El punto más alto de la República Centroafricana se encuentra en la frontera con Camerún.",
      pt: "O ponto mais alto da República Centro-Africana situa-se na fronteira com os Camarões.",
      fr: "Le point culminant de la République centrafricaine est situé à la frontière avec le Cameroun.",
    },
    facts: {
      de: ["Erreicht eine Höhe von 1.420 Metern.", "Teil des Adamaoua-Plateaus."],
      hu: ["Magassága eléri az 1420 métert.", "Az Adamaoua-fennsík része."],
      ro: ["Atinge o înălțime de 1.420 de metri.", "Face parte din Podișul Adamaoua."],
      en: ["Reaches an elevation of 1,420 meters.", "Part of the Adamawa Plateau."],
      es: ["Alcanza una altitud de 1.420 metros.", "Parte de la meseta de Adamawa."],
      pt: ["Atinge uma altitude de 1.420 metros.", "Parte do planalto de Adamaoua."],
      fr: ["Culmine à 1 420 mètres d'altitude.", "Fait partie du plateau de l'Adamaoua."],
    }, image: "/poi-images/mount-ngaoui-nature-v2.webp"},
  {
    id: "mount-toussoro-nature-v2",
    type: "mountain",
    parent: "CF-VK",
    coords: [23.1000, 9.1000],
    name: {
      de: "Mont Toussoro",
      hu: "Toussoro-hegy",
      ro: "Muntele Toussoro",
      en: "Mount Toussoro"},
    image: "/poi-images/mount-toussoro-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Mount Toussoro ist einer der markantesten Gipfel im Yadé-Massiv im Westen der Zentralafrikanischen Republik. Er erhebt sich über die umliegenden Hochplateaus und bietet eine beeindruckende Kulisse für die Region Nana-Mambéré. Die Vegetation des Berges ist durch eine Mischung aus Berggrünland und dichten Flecken von Nebelwald geprägt, die eine Vielzahl von endemischen Insekten und Vögeln beherbergen. Geologisch gesehen ist der Toussoro für seine interessanten Gesteinsformationen bekannt, die Zeugnis von uralten tektonischen Aktivitäten ablegen. Die Region um den Berg ist dünn besiedelt, was dazu beigetragen hat, dass die natürliche Umgebung weitgehend intakt geblieben ist. Für die lokale Bevölkerung hat der Berg oft eine spirituelle Bedeutung und wird in traditionellen Mythen erwähnt. Das raue Gelände macht den Aufstieg zu einer Herausforderung, belohnt jedoch mit weitreichenden Blicken über die westafrikanische Savanne. In den letzten Jahren wurde der Berg zunehmend als potenzielles Ziel für den Ökotourismus in Betracht gezogen, um die wirtschaftliche Entwicklung der umliegenden Dörfer zu fördern.",
      hu: "A Mount Toussoro a Közép-afrikai Köztársaság nyugati részén található Yadé-masszívum egyik legjellegzetesebb csúcsa. A környező fennsíkok fölé magasodik, és lenyűgöző hátteret biztosít Nana-Mambéré régiónak. A hegy növényzetét hegyi gyepek és sűrű köderdő-foltok keveréke jellemzi, amelyek számos endemikus rovarnak és madárnak adnak otthont. Földtani szempontból a Toussoro érdekes kőzetalakzatairól ismert, amelyek ősi tektonikus tevékenységekről tanúskodnak. A hegy környéke ritkán lakott, ami hozzájárult ahhoz, hogy a természeti környezet nagyrészt érintetlen maradjon. A helyi lakosság számára a hegy gyakran spirituális jelentőséggel bír, és megjelenik a hagyományos mítoszokban is. A zord terep kihívást jelent a megmászás során, de messzire nyúló kilátással jutalmaz a nyugat-afrikai szavannára. Az elmúlt években a hegyet egyre inkább az ökoturizmus potenciális célpontjaként kezdték emlegetni, hogy elősegítsék a környező falvak gazdasági fejlődését.",
      ro: "Muntele Toussoro este unul dintre cele mai distinctive vârfuri din masivul Yadé, situat în vestul Republicii Centrafricane. Acesta se ridică deasupra platourilor înalte din jur și oferă un cadru impresionant pentru regiunea Nana-Mambéré. Vegetația muntelui este caracterizată de un amestec de pajiști montane și petice dense de pădure de ceață, care adăpostesc o varietate de insecte și păsări endemice. Din punct de vedere geologic, Toussoro este cunoscut pentru formațiunile sale stâncoase interesante, care stau mărturie activităților tectonice antice. Regiunea din jurul muntelui este slab populată, ceea ce a continuat să păstreze mediul natural în mare parte intact. Pentru populația locală, muntele are adesea o semnificație spirituală și este menționat în miturile tradiționale. Terenul accidentat face ascensiunea o provocare, dar răsplătește cu vederi panoramice peste savana vest-africană. În ultimii ani, muntele a fost considerat tot mai mult o destinație potențială pentru ecoturism, în vederea promovării dezvoltării economice a satelor din jur.",
      en: "Mount Toussoro is one of the most distinctive peaks in the Yadé Massif in the western Central African Republic. It rises above the surrounding high plateaus, providing an impressive backdrop for the Nana-Mambéré region. The mountain's vegetation is characterized by a mix of montane grassland and dense patches of cloud forest, hosting a variety of endemic insects and birds. Geologically, the Toussoro is known for its interesting rock formations, bearing witness to ancient tectonic activities. The region around the mountain is sparsely populated, which has helped keep the natural environment largely intact. For the local population, the mountain often holds spiritual significance and is mentioned in traditional myths. The rugged terrain makes the ascent a challenge but rewards with far-reaching views across the West African savanna. In recent years, the mountain has increasingly been considered a potential destination for ecotourism to promote economic development in the surrounding villages.",
    },
    factsAdvanced: {
      de: ["Die Höhe des Mount Toussoro liegt bei etwa 1.330 Metern.", "Der Berg liegt in der Präfektur Nana-Mambéré im Westen des Landes.", "Geologische Studien in den 1960er Jahren identifizierten Quarzit-Vorkommen.", "In den Tälern des Berges entspringen Bäche, die den Mambéré speisen.", "Die Luftfeuchtigkeit in den Gipfellagen erreicht oft über 90 Prozent.", "Vogelkundler entdeckten 1985 eine seltene Unterart des Bergnektarvogels am Hang."],
      hu: ["A Mount Toussoro magassága körülbelül 1330 méter.", "A hegy az ország nyugati részén, Nana-Mambéré prefektúrában található.", "Az 1960-as évek földtani tanulmányai kvarcit-előfordulásokat azonosítottak.", "A hegy völgyeiben erednek a Mambérét tápláló patakok.", "A páratartalom a csúcsrégiókban gyakran meghaladja a 90 százalékot.", "Az ornitológusok 1985-ben a hegyi nektármadár egy ritka alfaját fedezték fel a lejtőn."],
      ro: ["Înălțimea Muntelui Toussoro este de aproximativ 1.330 de metri.", "Muntele este situat în prefectura Nana-Mambéré, în vestul țării.", "Studiile geologice din anii 1960 au identificat zăcăminte de cuarțit.", "În văile muntelui își au originea pârâuri care alimentează râul Mambéré.", "Umiditatea aerului în zonele de vârf atinge adesea peste 90%.", "Ornitologii au descoperit în 1985 o subspecie rară de pasăre-nectar de munte pe versant."],
      en: ["The height of Mount Toussoro is approximately 1,330 meters.", "The mountain is located in the Nana-Mambéré prefecture in the west of the country.", "Geological studies in the 1960s identified quartzite deposits.", "Streams originating in the mountain's valleys feed the Mambéré River.", "The humidity at the summit levels often reaches over 90 percent.", "Ornithologists discovered a rare subspecies of the mountain sunbird on the slope in 1985."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein markanter Berg im Bongo-Massiv im Nordosten des Landes.",
      hu: "Jellegzetes hegy a Bongo-masszívumban, az ország északkeleti részén.",
      ro: "Un munte proeminent din Masivul Bongo în nord-estul țării.",
      en: "A prominent mountain in the Bongo Massif in the northeast of the country.",
      es: "Una montaña prominente en el macizo de Bongo, situada en el noreste del país.",
      pt: "Uma montanha proeminente no maciço de Bongo, situada no nordeste do país.",
      fr: "Une montagne proéminente du massif des Bongo, située dans le nord-est du pays.",
    },
    facts: {
      de: ["Höchster Gipfel im Bongo-Massiv.", "Wichtige Wasserscheide in der Region."],
      hu: ["A Bongo-masszívum legmagasabb csúcsa.", "Fontos vízválasztó a régióban."],
      ro: ["Cel mai înalt vârf din Masivul Bongo.", "Cumpănă de ape importantă în regiune."],
      en: ["Highest peak in the Bongo Massif.", "Significant watershed in the region."],
      es: ["Pico más alto del macizo de Bongo.", "Importante divisoria de aguas en la región."],
      pt: ["Pico mais alto do maciço de Bongo.", "Importante divisória de águas na região."],
      fr: ["Plus haut sommet du massif des Bongo.", "Ligne de partage des eaux importante dans la région."],
    }, image: "/poi-images/mount-toussoro-nature-v2.webp"},
  {
    id: "mount-kayang-nature-v2",
    type: "mountain",
    parent: "CF-NM",
    coords: [14.8500, 6.0500],
    name: {
      de: "Mont Kayang",
      hu: "Kayang-hegy",
      ro: "Muntele Kayang",
      en: "Mount Kayang"},
    image: "/poi-images/mount-kayang-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Mount Kayang ist eine markante Erhebung im Yadé-Massiv, unweit der westlichen Grenze der Zentralafrikanischen Republik. Er zeichnet sich durch seine zerklüfteten Hänge und das raue Klima aus, das typisch für die Hochlagen Zentralafrikas ist. Der Berg spielt eine wichtige Rolle als Wasserscheide, wobei zahlreiche Bäche an seinen Flanken entspringen und die umliegenden Täler bewässern. Die Flora am Mount Kayang umfasst spezialisierte Hochgebirgspflanzen und Gräser, die an die starken Temperaturschwankungen angepasst sind. Aufgrund seiner Lage bietet der Berg einen strategischen Überblick über die Grenzregion zu Kamerun. Wissenschaftlich ist der Berg von Interesse, da er Einblicke in die geologische Geschichte des afrikanischen Schildes bietet. Die Abgeschiedenheit des Mount Kayang hat dazu geführt, dass die lokale Fauna, darunter seltene Nagetiere und Eidechsen, weitgehend ungestört leben kann. Für Abenteurer stellt der Berg ein lohnendes Ziel dar, das physische Ausdauer und Orientierungssinn erfordert. Trotz seiner Bedeutung bleibt der Mount Kayang in internationalen Reiseführern weitgehend unerwähnt, was seinen Status als Geheimtipp für Naturbegeisterte festigt.",
      hu: "A Mount Kayang a Yadé-masszívum egyik kiemelkedő magaslata, nem messze a Közép-afrikai Köztársaság nyugati határától. Tagolt lejtői és zord éghajlata jellemzi, amely tipikus Közép-Afrika magaslataira. A hegy fontos szerepet játszik vízválasztóként, számos patak ered az oldalain, amelyek a környező völgyeket öntözik. A Mount Kayang flórája speciális magashegyi növényeket és füveket tartalmaz, amelyek alkalmazkodtak az erős hőmérséklet-ingadozáshoz. Elhelyezkedéséből adódóan a hegy stratégiai rálátást biztosít a kameruni határvidékre. Tudományosan a hegy azért érdekes, mert betekintést nyújt az afrikai pajzs földtani történetébe. A Mount Kayang elszigeteltsége lehetővé tette, hogy a helyi fauna, beleértve a ritka rágcsálókat és gyíkokat, nagyrészt zavartalanul éljen. A kalandorok számára a hegy hálát érdemlő célpontot jelent, amely fizikai állóképességet és tájékozódási készséget igényel. Jelentősége ellenére a Mount Kayang a nemzetközi útikönyvekben nagyrészt említés nélkül marad, ami megerősíti a természetbarátok számára fenntartott titkos tipp státuszát.",
      ro: "Muntele Kayang este o ridicătură proeminentă în masivul Yadé, nu departe de granița de vest a Republicii Centrafricane. Se caracterizează prin versanții săi accidentați și climatul aspru, tipic pentru zonele înalte ale Africii Centrale. Muntele joacă un rol important ca linie de demarcație a apelor, numeroase pârâuri izvorând pe flancurile sale și irigând văile din jur. Flora de pe Muntele Kayang include plante de munte specializate și ierburi adaptate la fluctuațiile puternice de temperatură. Datorită locației sale, muntele oferă o perspectivă strategică asupra regiunii de graniță cu Camerunul. Din punct de vedere științific, muntele este de interes deoarece oferă informații despre istoria geologică a scutului african. Izolarea Muntelui Kayang a permis faunei locale, inclusiv rozătoarelor și șopârlelor rare, să trăiască în mare parte nederanjate. Pentru aventurieri, muntele reprezintă o destinație plină de satisfacții care necesită rezistență fizică și simț de orientare. În ciuda importanței sale, Muntele Kayang rămâne în mare parte nemenționat în ghidurile turistice internaționale, ceea ce îi confirmă statutul de loc secret pentru pasionații de natură.",
      en: "Mount Kayang is a prominent elevation in the Yadé Massif, not far from the western border of the Central African Republic. It is characterized by its rugged slopes and the harsh climate typical of the high altitudes of Central Africa. The mountain plays an important role as a watershed, with numerous streams originating on its flanks and watering the surrounding valleys. The flora on Mount Kayang includes specialized high-altitude plants and grasses adapted to significant temperature fluctuations. Due to its location, the mountain offers a strategic overview of the border region with Cameroon. Scientifically, the mountain is of interest as it provides insights into the geological history of the African shield. The remoteness of Mount Kayang has allowed local fauna, including rare rodents and lizards, to live largely undisturbed. For adventurers, the mountain represents a rewarding destination that requires physical endurance and a sense of direction. Despite its significance, Mount Kayang remains largely unmentioned in international travel guides, solidifying its status as a hidden gem for nature enthusiasts.",
    },
    factsAdvanced: {
      de: ["Der Mount Kayang erreicht eine Höhe von etwa 1.300 Metern.", "Er liegt in der Nähe der Stadt Baboua an der Grenze zu Kamerun.", "Der Berg besteht hauptsächlich aus Gneis und metamorphen Gesteinen.", "Erste kartografische Erfassungen wurden während der Kolonialzeit im Jahr 1920 durchgeführt.", "Die Nachttemperaturen können auf dem Gipfel unter 10 Grad Celsius fallen.", "Er gehört zur Wasserscheide zwischen dem Tschadseebecken und dem Kongobecken."],
      hu: ["A Mount Kayang magassága körülbelül 1300 méter.", "Baboua városa közelében, a kameruni határ mentén található.", "A hegy főként gneiszből és metamorf kőzetekből áll.", "Az első térképészeti felméréseket a gyarmati időszakban, 1920-ban végezték.", "Az éjszakai hőmérséklet a csúcson 10 Celsius-fok alá is süllyedhet.", "A Csád-tó medencéje és a Kongó-medence közötti vízválasztóhoz tartozik."],
      ro: ["Muntele Kayang atinge o înălțime de aproximativ 1.300 de metri.", "Este situat în apropierea orașului Baboua, la granița cu Camerunul.", "Muntele este compus în principal din gnais și roci metamorfice.", "Primele înregistrări cartografice au fost efectuate în timpul epocii coloniale în 1920.", "Temperaturile nocturne pe vârf pot scădea sub 10 grade Celsius.", "Face parte din linia de demarcație a apelor între bazinul lacului Ciad și bazinul Congo."],
      en: ["Mount Kayang reaches a height of approximately 1,300 meters.", "It is located near the city of Baboua on the border with Cameroon.", "The mountain consists primarily of gneiss and metamorphic rocks.", "Initial cartographic surveys were conducted during the colonial era in 1920.", "Nighttime temperatures at the summit can drop below 10 degrees Celsius.", "It belongs to the watershed between the Lake Chad basin and the Congo basin."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein Berg im Westen des Landes, der Teil der gebirgigen Grenzregion zu Kamerun ist.",
      hu: "Hegy az ország nyugati részén, amely a kameruni határmenti hegyvidéki régió része.",
      ro: "Un munte în vestul țării, care face parte din regiunea montană de frontieră cu Camerun.",
      en: "A mountain in the west of the country, part of the mountainous border region with Cameroon.",
      es: "Una montaña en el oeste del país que forma parte de la región montañosa fronteriza con Camerún.",
      pt: "Uma montanha no oeste do país que faz parte da região montanhosa fronteiriça com os Camarões.",
      fr: "Une montagne de l'ouest du pays, qui fait partie de la région montagneuse frontalière avec le Cameroun.",
    },
    facts: {
      de: ["Liegt im Yadé-Massiv.", "Umgeben von kühlerem Hochlandklima."],
      hu: ["A Yadé-masszívumban található.", "Hűvösebb felföldi éghajlat veszi körül."],
      ro: ["Situat în Masivul Yadé.", "Înconjurat de un climat de munte mai răcoros."],
      en: ["Located in the Yadé Massif.", "Surrounded by a cooler highland climate."],
      es: ["Situada en el macizo de Yadé.", "Rodeada de un clima de tierras altas más fresco."],
      pt: ["Situada no maciço de Yadé.", "Rodeada por um clima de terras altas mais fresco."],
      fr: ["Située dans le massif de Yadé.", "Entourée d'un climat de montagne plus frais."],
    }
  },
  {
    id: "mount-ngaya-nature-v2",
    type: "mountain",
    parent: "CF-HK",
    coords: [22.4000, 7.8000],
    name: {
      de: "Mont Ngaya",
      hu: "Ngaya-hegy",
      ro: "Muntele Ngaya",
      en: "Mount Ngaya"},
    image: "/poi-images/mount-ngaya-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Mount Ngaya, oft auch als Mont Ngaya bezeichnet, ist eine der markantesten Erhebungen im nordöstlichen Teil der Zentralafrikanischen Republik. Mit einer Gipfelhöhe von etwa 1.320 Metern über dem Meeresspiegel bildet er einen wichtigen Bestandteil des Bongo-Massivs und stellt einen der höchsten Punkte des Landes dar. Geografisch liegt der Berg in der Nähe der bedeutenden Wasserscheide zwischen den Einzugsgebieten des Nils im Nordosten und des Kongo im Südwesten. Die Region um den Mount Ngaya ist ökologisch wertvoll, da sie eine Übergangszone zwischen der feuchten äquatorialen Vegetation und den trockeneren Savannen des Sudans bildet. Geologisch besteht die Formation überwiegend aus altem kristallinem Gestein des afrikanischen Schildes, das über Jahrmillionen durch Erosion geformt wurde. Trotz seiner abgeschiedenen Lage spielt der Berg eine Rolle in der lokalen Klimatologie, indem er Steigungsregen begünstigt, die die umliegenden Täler mit Wasser versorgen. Wissenschaftliche Expeditionen untersuchen die hier vorkommenden seltenen Vogelarten und die spezifische Hochlandflora, die sich an die saisonalen Temperaturunterschiede angepasst hat.",
      hu: "A Mount Ngaya, más néven Mont Ngaya, a Közép-afrikai Köztársaság északkeleti részének egyik legjelentősebb kiemelkedése. Körülbelül 1320 méteres tengerszint feletti magasságával a Bongo-masszívum részét képezi, és az ország egyik legmagasabb pontjaként tartják számon. Földrajzi elhelyezkedése stratégiai fontosságú, mivel a Nílus és a Kongó vízgyűjtő területeit elválasztó vízválasztó közelében található. A hegy környéke ökológiai szempontból különleges átmeneti zónát alkot a trópusi esőerdők és a szudáni szavannák között, ami gazdag biodiverzitást eredményez. Geológiailag a hegység főként prekambriumi kristályos kőzetekből épül fel, amelyek az afrikai pajzs ősi maradványai. Bár a terület nehezen megközelíthető és ritkán lakott, a hegy fontos szerepet játszik a regionális csapadékképződésben, biztosítva a környező völgyek vízellátását. A modern kutatások során a Mount Ngaya környékét mint potenciális természetvédelmi területet és tudományos vizsgálati helyszínt kezelik, különös tekintettel az endemikus növényfajokra.",
      ro: "Muntele Ngaya, cunoscut și sub numele de Mont Ngaya, reprezintă una dintre cele mai înalte cote din partea de nord-est a Republicii Centrafricane. Cu o altitudine de aproximativ 1.320 de metri, acesta face parte integrantă din masivul Bongo și este considerat un punct geografic de referință la nivel național. Poziția sa este crucială din punct de vedere hidrologic, fiind situat în proximitatea liniei de demarcație dintre bazinele hidrografice ale Nilului și Congo-ului. Regiunea înconjurătoare este caracterizată printr-o biodiversitate remarcabilă, servind ca zonă de tranziție între pădurile tropicale umede și savanele sudaneze. Din punct de vedere geologic, muntele este format din roci cristaline străvechi, specifice scutului african, care au rezistat proceselor de eroziune de-a lungul erelor geologice. Deși este o zonă izolată, Muntele Ngaya influențează microclimatul local prin declanșarea precipitațiilor orografice esențiale pentru agricultura de subzistență din văile adiacente. Importanța sa ecologică este subliniată de prezența unor specii de păsări rare și a unei flore montane adaptate la condițiile climatice specifice.",
      en: "Mount Ngaya, also referred to as Mont Ngaya, is a prominent peak located in the northeastern region of the Central African Republic. Standing at an elevation of approximately 1,320 meters above sea level, it constitutes a significant part of the Bongo Massif and ranks among the highest points in the country. Geographically, the mountain is situated near the major continental divide that separates the Nile Basin from the Congo Basin. The area surrounding Mount Ngaya is an ecological transition zone where moist equatorial forests meet the drier Sudanese savannas, fostering a diverse range of species. Geologically, the formation consists primarily of ancient Precambrian crystalline rocks characteristic of the African Shield, shaped over millions of years by weathering and erosion. Despite its remote location, the mountain plays a vital role in local meteorology by inducing orographic rainfall that sustains the surrounding ecosystems. Scientific interest in the area focuses on its unique highland flora and its function as a corridor for migratory wildlife between different climatic zones.",
    },
    factsAdvanced: {
      de: ["Die offizielle Gipfelhöhe beträgt etwa 1.320 Meter über dem Meeresspiegel.", "Der Berg liegt innerhalb der Präfektur Vakaga im Nordosten des Landes.", "Er gehört geologisch zum Bongo-Massiv, einer Kette aus kristallinem Gestein.", "Mount Ngaya markiert die Wasserscheide zwischen Nil- und Kongobecken.", "Die Region erhält jährlich zwischen 900 und 1.100 mm Niederschlag.", "Die Durchschnittstemperatur am Fuße des Berges liegt bei ca. 26 Grad Celsius."],
      hu: ["A hegycsúcs magassága hozzávetőlegesen 1320 méter a tengerszint felett.", "Közigazgatásilag a hegy a Vakaga prefektúrában, az ország északkeleti részén fekszik.", "A hegység a Bongo-masszívum szerves részét képezi.", "A Nílus és a Kongó folyók vízgyűjtőjének határvonalán helyezkedik el.", "A területen az évi középhőmérséklet 25-27 Celsius-fok között mozog.", "A hegy alapkőzete több mint 500 millió éves prekambriumi gránit."],
      ro: ["Altitudinea maximă a vârfului este de aproximativ 1.320 de metri.", "Muntele este situat în prefectura Vakaga, în extremitatea nord-estică.", "Face parte din lanțul muntos Masivul Bongo, format din roci cristaline.", "Este un punct cheie pe cumpăna apelor dintre fluviile Nil și Congo.", "Zona înregistrează o perioadă de secetă pronunțată între noiembrie și martie.", "Vegetația dominantă este cea de savană împădurită cu elemente montane."],
      en: ["The peak reaches an elevation of approximately 1,320 meters above sea level.", "It is located within the Vakaga Prefecture in the northeast of the country.", "The mountain is a major component of the granite Bongo Massif range.", "It serves as a hydrologic divide between the Nile and Congo watersheds.", "The surrounding region has a tropical savanna climate (Aw in Köppen classification).", "Geological surveys date the primary rock formations to the Precambrian era."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein bedeutender Gipfel im zentralen Hochland der Zentralafrikanischen Republik.",
      hu: "Jelentős csúcs a Közép-afrikai Köztársaság középső felföldjén.",
      ro: "Un vârf important în podișul central al Republicii Centrafricane.",
      en: "A significant peak in the central highlands of the Central African Republic.",
      es: "Una cumbre importante en las tierras altas centrales de la República Centroafricana.",
      pt: "Um cume importante nas terras altas centrais da República Centro-Africana.",
      fr: "Un sommet important dans les hauts plateaux centraux de la République centrafricaine.",
    },
    facts: {
      de: ["Bietet weite Ausblicke über die umliegende Savanne.", "Geologisch Teil des alten kristallinen Schildes."],
      hu: ["Széles kilátást nyújt a környező szavannára.", "Geológiailag az ősi kristályos pajzs része."],
      ro: ["Oferă vederi largi asupra savanei înconjurătoare.", "Din punct de vedere geologic, face parte din scutul cristalin vechi."],
      en: ["Offers wide views over the surrounding savanna.", "Geologically part of the ancient crystalline shield."],
      es: ["Ofrece amplias vistas sobre la sabana circundante.", "Geológicamente parte del antiguo escudo cristalino."],
      pt: ["Oferece vistas amplas sobre a savana circundante.", "Geologicamente parte do antigo escudo cristalino."],
      fr: ["Offre de vastes vues sur la savane environnante.", "Fait géologiquement partie du vieux bouclier cristallin."],
    }, image: "/poi-images/mount-ngaya-nature-v2.webp"},
  {
    id: "bongo-massif-nature-v2",
    type: "mountain",
    parent: "CF-VK",
    coords: [23.5000, 9.5000],
    name: {
      de: "Bongo-Massiv",
      hu: "Bongo-masszívum",
      ro: "Masivul Bongo",
      en: "Bongo Massif"},
    image: "/poi-images/bongo-massif-nature-v2.webp",
    descriptionAdvanced: {
      de: "Das Bongo-Massiv ist ein ausgedehntes Gebirgssystem im Nordosten der Zentralafrikanischen Republik, das sich bis zur Grenze zum Sudan und Südsudan erstreckt. Diese geologische Formation besteht hauptsächlich aus Sandstein und kristallinen Grundgebirgsgesteinen, die eine markante Hochebene bilden. Mit Höhen, die oft über 1.000 Meter steigen, fungiert das Massiv als die primäre Wasserscheide zwischen dem Tschadbecken im Norden und dem Kongobecken im Süden. Die Landschaft ist durch tiefe Täler, steile Klippen und weite Plateaus geprägt, die eine Vielzahl von Lebensräumen für seltene Wildtiere bieten. Historisch gesehen war das Bongo-Massiv aufgrund seiner zerklüfteten Topografie ein Zufluchtsort für verschiedene ethnische Gruppen während regionaler Konflikte. Das Klima ist hier kühler als in den umliegenden Tiefebenen, was eine spezialisierte Flora ermöglicht, die in den trockeneren Savannen sonst nicht vorkommt. Heute ist das Gebiet für den Naturschutz von Bedeutung, da es als wichtiger Migrationskorridor für Großsäuger wie Elefanten und verschiedene Antilopenarten dient, die zwischen den Nationalparks der Region wandern.",
      hu: "A Bongo-masszívum egy kiterjedt hegységrendszer a Közép-afrikai Köztársaság északkeleti részén, amely átnyúlik Szudán és Dél-Szudán határvidékére is. Ez a geológiai képződmény elsősorban homokkőből és kristályos alapkőzetekből áll, amelyek jellegzetes fennsíkokat és meredek sziklafalakat alkotnak. A masszívum legmagasabb pontjai meghaladják az 1000 métert, és központi szerepet töltenek be a Csád-tó medencéje és a Kongó-medence közötti vízválasztóként. A tájat mély völgyek és elszigetelt sziklaalakzatok tagolják, amelyek számos ritka vadfaj számára nyújtanak menedéket. Történelmileg a terület nehéz járhatósága miatt védelmi funkciót töltött be a helyi népcsoportok számára a külső fenyegetésekkel szemben. A magasság miatt a klíma itt valamivel hűvösebb és csapadékosabb, mint az ország délebbi vidékein, ami egyedi hegyi flórát eredményez. Napjainkban a Bongo-masszívum kulcsfontosságú természetvédelmi terület, mivel fontos vándorlási útvonalat biztosít az elefántok és antilopfajok számára a regionális nemzeti parkok között.",
      ro: "Masivul Bongo este un sistem montan vast situat în nord-estul Republicii Centrafricane, extinzându-se până la granițele cu Sudanul și Sudanul de Sud. Această formațiune geologică este compusă în principal din gresii și roci cristaline de fundament, formând un platou înalt și fragmentat. Având altitudini care depășesc frecvent 1.000 de metri, masivul acționează ca principala barieră hidrografică între bazinul lacului Ciad la nord și bazinul fluviului Congo la sud. Peisajul este marcat de văi adânci, versanți abrupți și platouri întinse care adăpostesc o varietate de ecosisteme unice. Din punct de vedere istoric, topografia accidentată a masivului Bongo a oferit refugiu diverselor grupuri etnice în perioadele de instabilitate regională. Climatul în această zonă este mai temperat în comparație cu câmpiile înconjurătoare, favorizând o vegetație montană specifică ce include specii adaptate la altitudine. În prezent, regiunea este monitorizată pentru conservarea faunei, fiind un coridor esențial pentru migrația mamiferelor mari, cum ar fi elefanții și diverse specii de antilope.",
      en: "The Bongo Massif is an extensive mountain range in the northeastern Central African Republic, stretching towards the borders of Sudan and South Sudan. This geological formation is primarily composed of sandstone and crystalline basement rocks, creating a distinctive landscape of high plateaus and escarpments. Rising above 1,000 meters in many areas, the massif serves as the primary watershed between the Lake Chad Basin to the north and the Congo Basin to the south. The rugged topography is characterized by deep canyons and steep cliffs that provide sanctuary for numerous rare wildlife species. Historically, the difficult terrain of the Bongo Massif served as a natural defensive stronghold for local populations during regional conflicts. The climate at these elevations is significantly cooler than in the surrounding lowlands, supporting specialized montane vegetation that differs from the typical savanna. Today, the area is of great conservation importance, acting as a vital migratory corridor for large mammals, including elephants and various antelope species, moving between transboundary national parks.",
    },
    factsAdvanced: {
      de: ["Die höchste Erhebung des Massivs ist der Mount Ngaya mit ca. 1.320 Metern.", "Es erstreckt sich über eine Länge von etwa 450 Kilometern.", "Die Region ist die Quelle zahlreicher Nebenflüsse des Schari-Flusssystems.", "Das Gestein besteht zum Teil aus mesozoischem Sandstein.", "Das Massiv bildet die politische Grenze zwischen der ZAR und dem Sudan.", "Die jährliche Niederschlagsmenge erreicht in den Höhenlagen bis zu 1.200 mm."],
      hu: ["A hegység legmagasabb pontja a Mount Ngaya (1320 m).", "A masszívum hossza megközelítőleg 450 kilométer az északkeleti tengelyen.", "Számos Schari-mellékfolyó forrásvidéke található ezen a területen.", "Geológiai felépítésében jelentős szerepet játszanak a mezozoikumi homokkövek.", "A terület a ZAR, Szudán és Dél-Szudán hármashatárának közelében fekszik.", "A hegység hűvösebb mikroklímája miatt a páratartalom magasabb az átlagnál."],
      ro: ["Cea mai înaltă cotă a masivului este vârful Mount Ngaya, de 1.320 metri.", "Lanțul muntos se întinde pe o distanță de aproximativ 450 de kilometri.", "Regiunea alimentează rețeaua hidrografică a sistemului Chari-Logone.", "Structura geologică include depozite masive de gresie din era mezozoică.", "Masivul constituie o barieră naturală la frontiera cu Sudanul.", "Precipitațiile anuale în zonele înalte pot atinge pragul de 1.200 mm."],
      en: ["The highest peak within the massif is Mount Ngaya at 1,320 meters.", "The range spans approximately 450 kilometers across the northeast.", "It is the source of many tributaries for the Chari River system.", "The geological composition includes significant Mesozoic sandstone deposits.", "The massif forms part of the international border with Sudan.", "The higher elevations receive up to 1,200 mm of annual rainfall."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein ausgedehntes Gebirgsmassiv im Nordosten, das als natürliche Grenze fungiert.",
      hu: "Kiterjedt hegység északkeleten, amely természetes határként szolgál.",
      ro: "Un masiv muntos extins în nord-est, care funcționează ca o frontieră naturală.",
      en: "An extensive mountain massif in the northeast that serves as a natural border.",
      es: "Un extenso macizo montañoso en el noreste que actúa como frontera natural.",
      pt: "Um maciço montanhoso extenso no nordeste que funciona como uma fronteira natural.",
      fr: "Un vaste massif montagneux au nord-est qui fait office de frontière naturelle.",
    },
    facts: {
      de: ["Quelle vieler Flüsse, die zum Tschadsee fließen.", "Besteht hauptsächlich aus Sandstein."],
      hu: ["Számos Csád-tó felé folyó folyó forrásvidéke.", "Főként homokkőből épül fel."],
      ro: ["Sursa multor râuri care curg spre Lacul Ciad.", "Format în principal din gresie."],
      en: ["Source of many rivers flowing toward Lake Chad.", "Composed mainly of sandstone."],
      es: ["Fuente de muchos ríos que fluyen hacia el lago Chad.", "Compuesto principalmente de arenisca."],
      pt: ["Nascente de muitos rios que correm para o lago Chade.", "Composto principalmente por arenito."],
      fr: ["Source de nombreuses rivières qui se jettent dans le lac Tchad.", "Composé principalement de grès."],
    }, image: "/poi-images/bongo-massif-nature-v2.webp"},
  {
    id: "yade-massif-nature-v2",
    type: "mountain",
    parent: "CF-NM",
    coords: [15.5000, 6.2000],
    name: {
      de: "Yadé-Massiv",
      hu: "Yadé-masszívum",
      ro: "Masivul Yadé",
      en: "Yadé Massif"},
    image: "/poi-images/yade-massif-nature-v2.webp",
    descriptionAdvanced: {
      de: "Das Yadé-Massiv ist ein bedeutendes Hochplateau im Nordwesten der Zentralafrikanischen Republik, das sich bis nach Kamerun hinein erstreckt. Es ist Teil der größeren Adamaoua-Hochebene und besteht geologisch vorwiegend aus Granit und Gneisen, die durch tektonische Hebungen und Erosion geformt wurden. Mit Durchschnittshöhen zwischen 1.000 und 1.400 Metern ist das Massiv eine der kühlsten und feuchtesten Regionen des Landes. Diese klimatischen Bedingungen begünstigen eine dichte Vegetation, die von Galeriewäldern in den Tälern bis zu Hochlandsavannen auf den Plateaus reicht. Das Yadé-Massiv ist hydrografisch von zentraler Bedeutung, da hier mehrere wichtige Flüsse entspringen, darunter der Ouham und der Logone, die das Tschadbecken speisen. In der Geschichte war das Gebiet ein wichtiger Kreuzungspunkt für Handelsrouten und Wanderungen verschiedener Völker wie der Gbaya. Die fruchtbaren Böden vulkanischen Ursprungs in einigen Teilen des Massivs machen es zu einem wichtigen landwirtschaftlichen Zentrum der Region. In der Moderne steht das Massiv im Fokus von Naturschutzbemühungen, um die einzigartigen Bergökosysteme vor Überweidung und Abholzung zu schützen.",
      hu: "A Yadé-masszívum egy meghatározó fennsík a Közép-afrikai Köztársaság északnyugati részén, amely szervesen kapcsolódik a kameruni Adamaoua-fennsíkhoz. Geológiai felépítését főként gránit és gneisz alkotja, amelyek a tektonikus emelkedések és a hosszan tartó erózió hatására nyerték el mai formájukat. Az 1000 és 1400 méter közötti átlagmagasságú terület az ország egyik leghűvösebb és legcsapadékosabb régiója. Ez a különleges klíma dús növényzetet tesz lehetővé, amely a völgyek galériaerdőitől a fennsíkok magaslati szavannájáig terjed. Hidrográfiai szempontból a Yadé-masszívum kulcsfontosságú, hiszen itt erednek olyan jelentős folyók, mint az Ouham és a Logone, amelyek a Csád-tó vízellátásáért felelnek. Történelmileg a terület fontos kulturális találkozóhely volt a Gbaya nép és más etnikai csoportok számára, valamint kereskedelmi útvonalak csomópontjaként szolgált. A vulkáni eredetű termékeny talajok miatt a régió jelentős mezőgazdasági potenciállal rendelkezik. Napjainkban a természetvédelmi törekvések a hegyi ökoszisztémák megőrzésére fókuszálnak a legeltetés és az erdőirtás visszaszorításával.",
      ro: "Masivul Yadé este un platou înalt situat în nord-vestul Republicii Centrafricane, prelungindu-se spre vest în Camerun ca parte a platoului Adamaoua. Din punct de vedere geologic, acesta este format predominant din granit și gnais, modelate de forțe tectonice și eroziune pe parcursul mileniilor. Având altitudini medii cuprinse între 1.000 și 1.400 de metri, masivul este una dintre cele mai răcoroase și umede regiuni din țară. Aceste condiții climatice susțin o vegetație densă, variind de la păduri-galerie în văi până la savane montane pe platourile superioare. Masivul Yadé are o importanță hidrografică majoră, fiind locul de origine al unor râuri importante precum Ouham și Logone, care alimentează bazinul lacului Ciad. Istoric, zona a fost un punct de intersecție pentru rutele comerciale și migrațiile diverselor grupuri etnice, în special ale poporului Gbaya. Solurile fertile, parțial de origine vulcanică, fac din această regiune un centru agricol vital. În perioada contemporană, eforturile de conservare vizează protejarea ecosistemelor montane unice împotriva suprapășunatului și a defrișărilor necontrolate.",
      en: "The Yadé Massif is a major highland plateau in the northwestern Central African Republic, extending westward into Cameroon as part of the Adamawa Plateau. Geologically, it is primarily composed of granite and gneiss, shaped by tectonic uplift and subsequent erosion over millions of years. With average elevations ranging between 1,000 and 1,400 meters, the massif is one of the coolest and most humid regions in the country. These climatic conditions support lush vegetation, spanning from gallery forests in the valleys to montane savannas on the upper plateaus. The Yadé Massif is hydrographically critical, serving as the source for several major rivers, including the Ouham and the Logone, which flow northward to feed Lake Chad. Historically, the region has been a crossroads for trade routes and the migration of various ethnic groups, most notably the Gbaya people. The fertile soils, some of volcanic origin, make the massif an essential agricultural hub for the region. Modern conservation efforts focus on preserving these unique mountain ecosystems from the threats of overgrazing and deforestation.",
    },
    factsAdvanced: {
      de: ["Die höchste Erhebung des Massivs ist der Mont Ngaoui mit 1.410 Metern.", "Es stellt die östliche Fortsetzung des Adamaoua-Plateaus dar.", "Das Massiv besteht hauptsächlich aus kristallinem Gestein des Präkambriums.", "Hier entspringt der Fluss Ouham, ein Hauptzufluss des Schari.", "Die jährlichen Niederschläge betragen in dieser Region etwa 1.500 mm.", "Das Plateau ist ein wichtiges Anbaugebiet für Kaffee und Tabak."],
      hu: ["A hegység legmagasabb pontja a Mont Ngaoui (1410 méter).", "A terület az Adamaoua-fennsík keleti nyúlványának tekinthető.", "Alapkőzetei többnyire prekambriumi kristályos gránitból állnak.", "Az Ouham folyó forrása itt található, amely a Csád-tó medencéjébe folyik.", "Az éves csapadékmennyiség elérheti az 1500-1600 millimétert.", "A régió központi szerepet játszik a közép-afrikai kávétermesztésben."],
      ro: ["Vârful maxim al masivului este Mont Ngaoui, având 1.410 metri.", "Masivul reprezintă extensia estică a platoului Adamaoua din Camerun.", "Compoziția geologică dominantă este formată din granit precambrian.", "Râul Ouham, un afluent major al sistemului Chari, își are izvorul aici.", "Precipitațiile medii anuale se situează în jurul valorii de 1.500 mm.", "Zona este recunoscută pentru producția agricolă de cafea și tutun."],
      en: ["The highest point of the massif is Mont Ngaoui at 1,410 meters.", "It serves as the eastern extension of the Adamawa Plateau.", "The geological foundation is composed of Precambrian crystalline rocks.", "The Ouham River, a primary tributary of the Chari, originates here.", "The region receives approximately 1,500 mm of annual rainfall.", "The plateau is a key region for the cultivation of coffee and tobacco."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein Hochlandgebiet im Westen des Landes, das eine Fortsetzung des Adamaoua-Plateaus ist.",
      hu: "Felföldi terület az ország nyugati részén, amely az Adamaoua-fennsík folytatása.",
      ro: "O zonă de munte în vestul țării, care este o continuare a Podișului Adamaoua.",
      en: "A highland area in the west of the country, which is a continuation of the Adamawa Plateau.",
      es: "Una zona de tierras altas en el oeste del país, que es una continuación de la meseta de Adamaoua.",
      pt: "Uma área de planalto no oeste do país, que é uma continuação do planalto de Adamaoua.",
      fr: "Une zone de hauts plateaux dans l'ouest du pays, qui est le prolongement du plateau de l'Adamaoua.",
    },
    facts: {
      de: ["Wichtiges Wassereinzugsgebiet.", "Gekennzeichnet durch Granit- und Gneisformationen."],
      hu: ["Fontos vízgyűjtő terület.", "Gránit- és gneiszformációk jellemzik."],
      ro: ["Zonă importantă de captare a apei.", "Caracterizat prin formațiuni de granit și gnais."],
      en: ["Significant water catchment area.", "Characterized by granite and gneiss formations."],
      es: ["Importante cuenca hidrográfica.", "Caracterizado por formaciones de granito y gneis."],
      pt: ["Importante bacia hidrográfica.", "Caracterizado por formações de granito e gnaisse."],
      fr: ["Bassin versant important.", "Caractérisé par des formations de granit et de gneiss."],
    }
  },
  {
    id: "dzanga-sangha-forest-nature-v2", "sights": {
      "de": [
            {
                  "name": "Réserve Spéciale de Dzanga-Sangha",
                  "text": "Die Réserve Spéciale de Dzanga-Sangha ist ein ausgewiesenes besonderes Naturschutzgebiet, das der Erhaltung einzigartiger Waldlebensräume gewidmet ist.",
                  "category": "natural",
                  "coords": [
                        16.188129,
                        2.973392
                  ]
            }
      ],
      "hu": [
            {
                  "name": "Réserve Spéciale de Dzanga-Sangha",
                  "text": "A Réserve Spéciale de Dzanga-Sangha egy kijelölt különleges természetvédelmi terület, amelyet az egyedi erdei élőhelyek megőrzésének szenteltek.",
                  "category": "natural",
                  "coords": [
                        16.188129,
                        2.973392
                  ]
            }
      ],
      "ro": [
            {
                  "name": "Réserve Spéciale de Dzanga-Sangha",
                  "text": "Réserve Spéciale de Dzanga-Sangha este o rezervație naturală specială desemnată, dedicată conservării habitatelor forestiere unice.",
                  "category": "natural",
                  "coords": [
                        16.188129,
                        2.973392
                  ]
            }
      ],
      "en": [
            {
                  "name": "Réserve Spéciale de Dzanga-Sangha",
                  "text": "The Réserve Spéciale de Dzanga-Sangha is a designated special nature reserve dedicated to the conservation of unique forest habitats.",
                  "category": "natural",
                  "coords": [
                        16.188129,
                        2.973392
                  ]
            }
      ]
},
    type: "forest",
    parent: "CF-SE",
    coords: [16.2000, 2.9500],
    name: {
      de: "Dzanga-Sangha-Regenwald",
      hu: "Dzanga-Sangha esőerdő",
      ro: "Pădurea tropicală Dzanga-Sangha",
      en: "Dzanga-Sangha Rainforest"},
    image: "/poi-images/dzanga-sangha-forest-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Dzanga-Sangha-Regenwald im äußersten Südwesten der Zentralafrikanischen Republik ist Teil eines grenzüberschreitenden Schutzgebietskomplexes von Weltrang. Als Kernstück des Sangha-Trinationalen-Welterbes umfasst er dichte Tieflandregenwälder, die für ihre außergewöhnliche Biodiversität bekannt sind. Das Gebiet wird durch das Dzanga-Sangha-Schutzgebiet und den Dzanga-Ndoki-Nationalpark geschützt, die 1990 gegründet wurden. Berühmt ist der Wald vor allem für die Dzanga-Bai, eine mineralreiche Lichtung, an der sich regelmäßig hunderte Waldelefanten und Gorillas versammeln. Die Vegetation ist durch riesige Baumarten wie den Moabi geprägt, die ein geschlossenes Kronendach bilden und zahlreichen endemischen Vögeln und Insekten Lebensraum bieten. Menschlich ist die Region eng mit den Ba'Aka-Pygmäen verbunden, deren traditionelles Wissen über Heilpflanzen und Jagdtechniken seit Generationen bewahrt wird. Trotz internationaler Schutzbemühungen bleibt der Wald durch Wilderei und illegalen Holzeinschlag bedroht. Ökotourismus spielt eine wachsende Rolle, um alternative Einkommensquellen für die lokale Bevölkerung zu schaffen und den Erhalt dieses einzigartigen Ökosystems langfristig zu sichern.",
      hu: "A Dzanga-Sangha esőerdő a Közép-afrikai Köztársaság délnyugati csücskében található, és a világ egyik legjelentősebb határokon átnyúló védett területének része. A Sangha Trinational világörökségi helyszín magjaként sűrű alföldi trópusi esőerdőket foglal magában, amelyek kivételes biodiverzitásukról ismertek. A területet a Dzanga-Sangha Rezervátum és a Dzanga-Ndoki Nemzeti Park védi, amelyeket 1990-ben alapítottak. Az erdő legfőbb nevezetessége a Dzanga Bai, egy ásványi anyagokban gazdag erdei tisztás, ahol rendszeresen több száz erdei elefánt és síkvidéki gorilla gyűlik össze. A növényzetet olyan óriási fafajok határozzák meg, mint a moabi, amelyek zárt lombozatot alkotnak, otthont adva számos endemikus madárnak és rovarnak. A régió szorosan kötődik a Ba'Aka pigmeusokhoz, akiknek hagyományos tudása a gyógynövényekről és a vadászati technikákról generációk óta öröklődik. A nemzetközi védelmi törekvések ellenére az erdőt továbbra is fenyegeti az orvvadászat és az illegális fakitermelés. Az ökoturizmus növekvő szerepet játszik abban, hogy fenntartható megélhetést biztosítson a helyieknek és megőrizze ezt az egyedülálló ökoszisztémát.",
      ro: "Pădurea tropicală Dzanga-Sangha, situată în extremitatea sud-vestică a Republicii Centrafricane, este parte a unui complex de arii protejate de importanță mondială. Fiind nucleul sitului transfrontalier Sangha Trinational, inclus în patrimoniul UNESCO, aceasta cuprinde păduri tropicale de joasă altitudine renumite pentru biodiversitatea lor excepțională. Rezervația Dzanga-Sangha și Parcul Național Dzanga-Ndoki au fost înființate în 1990 pentru a conserva acest ecosistem fragil. Un punct de atracție major este Dzanga Bai, o poiană bogată în minerale unde se adună frecvent sute de elefanți de pădure și gorile de câmpie. Vegetația este dominată de specii de arbori gigantici, precum moabi, care formează un coronament dens, oferind habitat multor specii de păsări endemice. Regiunea este locuită de comunitățile Ba'Aka (pigmei), al căror stil de viață tradițional și cunoștințe despre medicina naturistă sunt protejate. Deși beneficiază de protecție internațională, pădurea se confruntă cu amenințări precum braconajul și exploatarea forestieră ilegală. Ecoturismul este promovat ca o soluție sustenabilă pentru dezvoltarea locală și conservarea pe termen lung a acestui patrimoniu natural unic.",
      en: "The Dzanga-Sangha Rainforest, located in the far southwestern tip of the Central African Republic, is part of a world-class transboundary protected area complex. As the core of the Sangha Trinational UNESCO World Heritage site, it encompasses dense lowland tropical forests renowned for their exceptional biodiversity. The area is protected by the Dzanga-Sangha Special Reserve and the Dzanga-Ndoki National Park, established in 1990. The forest is most famous for the Dzanga Bai, a mineral-rich forest clearing where hundreds of forest elephants and western lowland gorillas regularly congregate. The vegetation is characterized by giant tree species such as the moabi, which form a closed canopy sheltering numerous endemic birds and insects. Human history in the region is deeply intertwined with the Ba'Aka pygmy people, whose traditional knowledge of medicinal plants and hunting techniques has been preserved for generations. Despite international conservation efforts, the forest remains threatened by poaching and illegal logging. Ecotourism plays an increasing role in providing alternative livelihoods for local communities while ensuring the long-term survival of this unique ecosystem.",
    },
    factsAdvanced: {
      de: ["Das Schutzgebiet wurde im Jahr 1990 offiziell proklamiert.", "Es ist Teil des Sangha Trinational Welterbes der UNESCO seit 2012.", "In der Dzanga-Bai werden oft über 100 Elefanten gleichzeitig gesichtet.", "Der Park beherbergt eine der höchsten Gorilladichten Afrikas.", "Die Regenmenge beträgt im Durchschnitt etwa 1.500 mm pro Jahr.", "Es gibt über 300 verschiedene Vogelarten in diesem Waldgebiet."],
      hu: ["A rezervátumot hivatalosan 1990-ben hozták létre.", "2012 óta a Sangha Trinational UNESCO világörökség része.", "A Dzanga Bai tisztáson egyszerre akár 100 elefánt is látható.", "A park a nyugati síkvidéki gorillák egyik legfontosabb menedéke.", "Az éves csapadékmennyiség átlagosan 1500 milliméter körül alakul.", "A területen több mint 300 madárfajt azonosítottak a kutatók."],
      ro: ["Rezervația a fost înființată oficial în anul 1990.", "Face parte din situl Patrimoniului Mondial UNESCO din 2012.", "La Dzanga Bai au fost monitorizați peste 100 de elefanți simultan.", "Parcul deține una dintre cele mai mari densități de gorile de câmpie.", "Precipitațiile medii anuale sunt estimate la 1.500 mm.", "Sunt înregistrate peste 300 de specii de păsări în acest ecosistem."],
      en: ["The protected area was officially established in 1990.", "It has been part of a UNESCO World Heritage site since 2012.", "Over 100 elephants are often sighted simultaneously at Dzanga Bai.", "The park hosts one of the highest densities of western lowland gorillas.", "The area receives an average annual rainfall of approximately 1,500 mm.", "More than 300 bird species have been documented within the forest."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein dichter tropischer Regenwald im tiefen Süden, berühmt für seine biologische Vielfalt.",
      hu: "Sűrű trópusi esőerdő a távoli délen, biológiai sokféleségéről híres.",
      ro: "O pădure tropicală densă în sudul îndepărtat, faimoasă pentru biodiversitatea sa.",
      en: "A dense tropical rainforest in the deep south, famous for its biological diversity.",
      es: "Una densa selva tropical en el extremo sur, famosa por su biodiversidad.",
      pt: "Uma floresta tropical densa no extremo sul, famosa pela sua biodiversidade.",
      fr: "Une forêt tropicale dense dans l'extrême sud, célèbre pour sa biodiversité.",
    },
    facts: {
      de: ["Beheimatet Flachlandgorillas und Waldelefanten.", "Teil des trinationalen Schutzgebiets am Sangha."],
      hu: ["Síkvidéki gorilláknak és erdei elefántoknak ad otthont.", "A Sangha háromnemzetiségű természetvédelmi terület része."],
      ro: ["Adăpostește gorile de câmpie și elefanți de pădure.", "Parte a sitului transfrontalier protejat de pe Sangha."],
      en: ["Home to lowland gorillas and forest elephants.", "Part of the Sangha Trinational protected area."],
      es: ["Hogar de gorilas de llanura y elefantes de bosque.", "Parte del área protegida trinacional del Sangha."],
      pt: ["Lar de gorilas-das-planícies e elefantes-da-floresta.", "Parte da área protegida trinacional do Sangha."],
      fr: ["Abrite des gorilles des plaines et des éléphants de forêt.", "Fait partie de l'aire protégée trinationale de la Sangha."],
    }, image: "/poi-images/dzanga-sangha-forest-nature-v2.webp"},
  {
    id: "chinko-forest-nature-v2",
    type: "forest",
    parent: "CF-MB",
    coords: [24.0000, 6.0000],
    name: {
      de: "Chinko-Wald",
      hu: "Chinko-erdő",
      ro: "Pădurea Chinko",
      en: "Chinko Forest"},
    image: "/poi-images/chinko-forest-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Chinko-Wald im Osten der Zentralafrikanischen Republik ist eines der ökologisch bedeutendsten und unberührtesten Gebiete des afrikanischen Kontinents. Er umfasst ein riesiges Einzugsgebiet des Chinko-Flusses und bildet eine einzigartige Übergangszone zwischen dem dichten kongolesischen Regenwald und der nordsudanesischen Savanne. Diese strategische Lage ermöglicht eine außergewöhnliche Artenvielfalt, bei der sowohl Waldbewohner als auch Savannentiere koexistieren. Seit 2014 wird das Gebiet durch das Chinko-Projekt in Zusammenarbeit mit African Parks verwaltet, um den Schutz vor Wilderei und illegaler Beweidung zu intensivieren. Die Landschaft besteht aus einem Mosaik aus Galeriewäldern, offenem Grasland und bewaldeten Plateaus, die wichtige Rückzugsgebiete für Wildhunde, Löwen und Elefanten bieten. Historisch war die Region aufgrund ihrer Unzugänglichkeit kaum besiedelt, was zur Erhaltung der ursprünglichen Natur beitrug. Die geologischen Formationen sind durch Sedimentgesteine und weitläufige Flusssysteme geprägt, die während der Regenzeit große Flächen fluten. Der Chinko-Wald gilt heute als Modellprojekt für großflächigen Naturschutz in politisch instabilen Regionen.",
      hu: "A Chinko-erdő a Közép-afrikai Köztársaság keleti részén az afrikai kontinens egyik legérintetlenebb és ökológiailag legfontosabb területe. A Chinko folyó hatalmas vízgyűjtő területét foglalja magában, és egyedülálló átmeneti zónát alkot a kongói esőerdők és az észak-szudáni szavannák között. Ez a földrajzi helyzet lehetővé teszi, hogy az erdei és a szavannai fajok egymás mellett éljenek, rendkívüli biodiverzitást eredményezve. 2014 óta a területet az African Parks szervezet kezeli a Chinko-projekt keretében, célul tűzve ki az orvvadászat és az illegális legeltetés visszaszorítását. A tájat galériaerdők, nyílt füves puszták és erdős fennsíkok mozaikja alkotja, amelyek fontos menedéket nyújtanak a vadkutyák, oroszlánok és elefántok számára. Történelmileg a régió elszigeteltsége segített megőrizni az ősi természeti állapotokat. A geológiai formációkat üledékes kőzetek és kiterjedt folyórendszerek jellemzik, amelyek az esős évszakban nagy területeket árasztanak el. A Chinko-erdő ma mintaprojektnek számít a nagyléptékű természetvédelem terén a politikailag instabil régiókban.",
      ro: "Pădurea Chinko, situată în estul Republicii Centrafricane, reprezintă una dintre cele mai vaste și intacte zone sălbatice de pe continentul african. Aceasta cuprinde bazinul hidrografic al râului Chinko și formează o zonă de tranziție unică între pădurea tropicală congoleză și savana nord-sudaneză. Poziția sa strategică permite coexistența speciilor forestiere cu cele de savană, rezultând o biodiversitate extraordinară. Din 2014, zona este administrată prin Proiectul Chinko în colaborare cu organizația African Parks, cu scopul de a combate braconajul și pășunatul ilegal. Peisajul este un mozaic de păduri-galerie, pajiști deschise și platouri împădurite, oferind refugiu pentru câini sălbatici africani, lei și elefanți. Din punct de vedere istoric, inaccesibilitatea regiunii a contribuit la conservarea stării sale primordiale. Formațiunile geologice sunt dominate de roci sedimentare și sisteme fluviale complexe care inundă suprafețe mari în timpul sezonului ploios. Pădurea Chinko este considerată astăzi un model de succes pentru conservarea pe scară largă în regiuni marcate de instabilitate politică.",
      en: "The Chinko Forest in the eastern Central African Republic is one of the most ecologically significant and pristine wilderness areas on the African continent. It encompasses the vast drainage basin of the Chinko River and forms a unique transition zone between the dense Congolese rainforest and the North Sudanese savanna. This strategic location allows for an extraordinary overlap of biodiversity, where forest-dwelling species coexist with savanna wildlife. Since 2014, the area has been managed under the Chinko Project in partnership with African Parks, aiming to intensify protection against poaching and illegal cattle grazing. The landscape is a mosaic of gallery forests, open grasslands, and wooded plateaus, providing critical habitat for African wild dogs, lions, and elephants. Historically, the region's extreme remoteness helped preserve its original natural state. Geological formations are characterized by sedimentary rocks and extensive river systems that flood large areas during the rainy season. Today, the Chinko Forest is regarded as a flagship model for large-scale conservation in politically challenging environments.",
    },
    factsAdvanced: {
      de: ["Das Schutzgebiet umfasst eine Fläche von etwa 17.600 Quadratkilometern.", "African Parks übernahm die Verwaltung des Gebiets im Jahr 2014.", "Hier leben über 10 verschiedene Primatenarten dauerhaft.", "Der Chinko-Fluss ist ein bedeutender Nebenfluss des Mbomou.", "Es wurden über 450 Vogelarten in diesem Ökosystem dokumentiert.", "Die Region beherbergt eine der letzten stabilen Löwenpopulationen der ZAR."],
      hu: ["A védett terület kiterjedése körülbelül 17 600 négyzetkilométer.", "Az African Parks szervezet 2014-ben vette át a terület kezelését.", "Legalább 10 különböző primátafaj él a területen.", "A Chinko folyó a Mbomou folyó egyik legfontosabb mellékfolyója.", "A kutatók több mint 450 madárfajt jegyeztek fel a régióban.", "Itt található a Közép-afrikai Köztársaság egyik utolsó stabil oroszlánpopulációja."],
      ro: ["Aria protejată acoperă o suprafață de aproximativ 17.600 km pătrați.", "Organizația African Parks a preluat managementul în anul 2014.", "În acest ecosistem trăiesc peste 10 specii diferite de primate.", "Râul Chinko este un afluent major al fluviului Mbomou.", "Au fost documentate peste 450 de specii de păsări în zonă.", "Regiunea adăpostește una dintre ultimele populații stabile de lei din țară."],
      en: ["The protected area covers an area of approximately 17,600 square kilometers.", "African Parks assumed management of the Chinko area in 2014.", "The forest is home to more than 10 different species of primates.", "The Chinko River is a major tributary of the Mbomou River.", "Over 450 bird species have been documented within this ecosystem.", "The region hosts one of the last stable lion populations in the CAR."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein riesiges Wildnisgebiet im Osten, das eine Mischung aus Savanne und Regenwald darstellt.",
      hu: "Hatalmas vadon keleten, amely a szavanna és az esőerdő keveréke.",
      ro: "O zonă vastă de sălbăticie în est, care reprezintă o mixtură de savană și pădure tropicală.",
      en: "A vast wilderness area in the east, representing a mixture of savanna and rainforest.",
      es: "Una vasta área de naturaleza salvaje en el este que representa una mezcla de sabana y selva tropical.",
      pt: "Uma vasta área de vida selvagem no leste, representando uma mistura de savana e floresta tropical.",
      fr: "Une vaste zone sauvage à l'est, représentant un mélange de savane et de forêt tropicale.",
    },
    facts: {
      de: ["Eines der größten unfragmentierten Ökosysteme Afrikas.", "Wichtiger Rückzugsort für Wildhunde und Löwen."],
      hu: ["Afrika egyik legnagyobb összefüggő ökoszisztémája.", "A vadkutyák és oroszlánok fontos menedékhelye."],
      ro: ["Unul dintre cele mai mari ecosisteme nefragmentate din Africa.", "Refugiu important pentru câinii sălbatici și lei."],
      en: ["One of Africa's largest unfragmented ecosystems.", "Critical refuge for wild dogs and lions."],
      es: ["Uno de los ecosistemas no fragmentados más grandes de África.", "Refugio importante para licaones y leones."],
      pt: ["Um dos maiores ecossistemas não fragmentados de África.", "Refúgio importante para cães selvagens e leões."],
      fr: ["L'un des plus grands écosystèmes non fragmentés d'Afrique.", "Refuge important pour les lycaons et les lions."],
    }, image: "/poi-images/chinko-forest-nature-v2.webp"},
  {
    id: "zemongo-forest-nature-v2",
    type: "forest",
    parent: "CF-HM",
    coords: [25.5000, 6.5000],
    name: {
      de: "Zemongo-Wald",
      hu: "Zemongo-erdő",
      ro: "Pădurea Zemongo",
      en: "Zemongo Forest"},
    image: "/poi-images/zemongo-forest-nature-v2.webp",
    descriptionAdvanced: {
      de: "Das Zemongo-Schutzgebiet, oft als Zemongo-Wald bezeichnet, liegt im Nordosten der Zentralafrikanischen Republik in der Präfektur Haut-Mbomou. Gegründet bereits im Jahr 1925 als Wildtierreservat, gehört es zu den ältesten Naturschutzgebieten des Landes. Geografisch ist die Region durch ein sanft gewelltes Plateau geprägt, das von zahlreichen Wasserläufen durchzogen wird, die in den Vovodo und den Mbomou münden. Die Vegetation besteht aus einer Mischung aus dichten Galeriewäldern entlang der Flüsse und weitläufigen sudano-guineischen Savannen. Zemongo ist besonders für seine Populationen von Schimpansen und Elefanten bekannt, die in den abgelegenen Waldinseln Schutz finden. Geologisch dominieren präkambrische Formationen, die nährstoffarme, aber stabile Böden bilden. Historisch war die Region ein wichtiger Rückzugsort für die Tierwelt während der Dürreperioden in der Sahelzone. Trotz seines Alters leidet das Schutzgebiet heute unter mangelnder Infrastruktur und den Auswirkungen regionaler Instabilität. Internationale Naturschutzorganisationen bemühen sich jedoch um eine Wiederbelebung der Schutzmaßnahmen, um dieses wertvolle Reservoir der afrikanischen Megafauna zu erhalten.",
      hu: "A Zemongo Rezervátum, amelyet gyakran Zemongo-erdőként emlegetnek, a Közép-afrikai Köztársaság északkeleti részén, a Haut-Mbomou prefektúrában található. 1925-ben alapították vadrezervátumként, így az ország egyik legrégebbi természetvédelmi területe. Földrajzilag a régiót enyhén hullámos fennsíkok jellemzik, amelyeket számos vízfolyás szel át, többek között a Vovodo és a Mbomou folyók mellékfolyói. A növényzet a folyók menti sűrű galériaerdők és a kiterjedt szudáni-guineai szavannák keverékéből áll. Zemongo különösen híres csimpánz- és elefántpopulációjáról, amelyek az elszigetelt erdőfoltokban találnak menedéket. Geológiailag a prekambriumi formációk dominálnak, amelyek tápanyagban szegény, de stabil talajt alkotnak. Történelmileg a terület fontos menedékhely volt a vadon élő állatok számára a Száhel-övezet aszályos időszakaiban. Kora ellenére a rezervátum ma az infrastruktúra hiányától és a regionális instabilitás hatásaitól szenved. Nemzetközi természetvédelmi szervezetek azonban törekszenek a védelmi intézkedések újjáélesztésére, hogy megőrizzék az afrikai megafauna ezen értékes tartályát.",
      ro: "Rezervația Zemongo, adesea numită Pădurea Zemongo, este situată în nord-estul Republicii Centrafricane, în prefectura Haut-Mbomou. Înființată încă din anul 1925 ca rezervație de faună, este una dintre cele mai vechi arii protejate din țară. Din punct de vedere geografic, regiunea este caracterizată de un platou ușor ondulat, traversat de numeroase cursuri de apă care se varsă în râurile Vovodo și Mbomou. Vegetația constă într-un amestec de păduri-galerie dense de-a lungul cursurilor de apă și savane sudano-guineene vaste. Zemongo este cunoscută în special pentru populațiile sale de cimpanzei și elefanți, care găsesc refugiu în enclavele forestiere izolate. Geologic, domină formațiunile precambriene, care formează soluri sărace în nutrienți, dar stabile. Istoric, regiunea a fost un refugiu important pentru faună în timpul perioadelor de secetă din Sahel. În ciuda vechimii sale, rezervația suferă astăzi din cauza lipsei de infrastructură și a efectelor instabilității regionale. Organizațiile internaționale de conservare depun însă eforturi pentru revitalizarea măsurilor de protecție pentru a păstra acest rezervor valoros al megafaunei africane.",
      en: "The Zemongo Reserve, often referred to as the Zemongo Forest, is located in the northeastern Central African Republic within the Haut-Mbomou Prefecture. Established as early as 1925 as a faunal reserve, it is one of the oldest protected areas in the country. Geographically, the region is characterized by a gently undulating plateau intersected by numerous watercourses that feed into the Vovodo and Mbomou rivers. The vegetation consists of a mix of dense gallery forests along the rivers and extensive Sudano-Guinean savannas. Zemongo is particularly noted for its populations of chimpanzees and elephants, which find sanctuary in the remote forest islands. Geologically, Precambrian formations dominate, creating nutrient-poor but stable soils. Historically, the region served as a critical refuge for wildlife during severe droughts in the Sahel zone. Despite its long history, the reserve currently suffers from a lack of infrastructure and the impacts of regional instability. However, international conservation groups are working to revitalize protection measures to preserve this valuable reservoir of African megafauna.",
    },
    factsAdvanced: {
      de: ["Das Reservat wurde im Jahr 1925 offiziell eingerichtet.", "Es erstreckt sich über eine Fläche von ca. 10.100 Quadratkilometern.", "Die Region liegt auf einer durchschnittlichen Höhe von 600 Metern.", "Zemongo beherbergt eine bedeutende Population von zentralafrikanischen Schimpansen.", "Der Fluss Vovodo bildet eine der natürlichen Grenzen des Gebiets.", "Die jährliche Niederschlagsmenge liegt bei etwa 1.300 bis 1.400 mm."],
      hu: ["A rezervátumot hivatalosan 1925-ben hozták létre.", "Területe megközelítőleg 10 100 négyzetkilométer.", "A régió átlagos tengerszint feletti magassága 600 méter.", "Zemongo a közép-afrikai csimpánzok egyik fontos élőhelye.", "A Vovodo folyó a terület egyik természetes határát alkotja.", "Az éves csapadékmennyiség 1300 és 1400 mm között mozog."],
      ro: ["Rezervația a fost stabilită oficial în anul 1925.", "Acoperă o suprafață de aproximativ 10.100 kilometri pătrați.", "Regiunea este situată la o altitudine medie de 600 de metri.", "Zemongo adăpostește o populație semnificativă de cimpanzei central-africani.", "Râul Vovodo constituie una dintre limitele naturale ale ariei.", "Cantitatea anuală de precipitații este de aproximativ 1.300 - 1.400 mm."],
      en: ["The reserve was officially established in 1925.", "It covers an area of approximately 10,100 square kilometers.", "The region sits at an average elevation of 600 meters above sea level.", "Zemongo is home to a significant population of Central African chimpanzees.", "The Vovodo River forms one of the natural boundaries of the area.", "The annual rainfall in the region ranges from 1,300 to 1,400 mm."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein abgelegenes Waldgebiet im Osten, das für seine unberührte Natur bekannt ist.",
      hu: "Távoli erdős terület keleten, amely érintetlen természetéről ismert.",
      ro: "O zonă forestieră izolată în est, cunoscută pentru natura sa virgină.",
      en: "A remote forested area in the east, known for its pristine nature.",
      es: "Una zona forestal remota en el este, conocida por su naturaleza virgen.",
      pt: "Uma área florestal remota no leste, conhecida pela sua natureza intocada.",
      fr: "Une zone forestière isolée dans l'est, connue pour sa nature sauvage.",
    },
    facts: {
      de: ["Liegt im Zemongo-Wildschutzgebiet.", "Wichtiger Korridor für wandernde Wildtiere."],
      hu: ["A Zemongo vadrezervátumban található.", "Fontos folyosó a vándorló vadon élő állatok számára."],
      ro: ["Situat în Rezervația de Faună Zemongo.", "Coridor important pentru animalele sălbatice migratoare."],
      en: ["Located within the Zemongo Faunal Reserve.", "Significant corridor for migratory wildlife."],
      es: ["Ubicada en la Reserva de Fauna de Zemongo.", "Corredor importante para la fauna migratoria."],
      pt: ["Localizada na Reserva de Fauna de Zemongo.", "Corredor importante para a vida selvagem migratória."],
      fr: ["Située dans la réserve de faune de Zemongo.", "Corridor important pour la faune migratrice."],
    }, image: "/poi-images/zemongo-forest-nature-v2.webp"},
  {
    id: "lake-goumba-nature-v2",
    type: "lake",
    parent: "CF-HK",
    coords: [22.2500, 7.5500],
    name: {
      de: "Goumba-See",
      hu: "Goumba-tó",
      ro: "Lacul Goumba",
      en: "Lake Goumba"},
    image: "/poi-images/lake-goumba-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Goumba-See ist ein kleineres, aber ökologisch wichtiges Gewässer in der Zentralafrikanischen Republik, das oft in Verbindung mit saisonalen Überschwemmungsgebieten steht. Er liegt in einer Region, die durch ein komplexes Netzwerk von Flüssen und Feuchtgebieten geprägt ist, welche eine lebenswichtige Wasserquelle für die lokale Flora und Fauna darstellen. Die Umgebung des Sees ist durch dichte Ufervegetation und sumpfige Abschnitte gekennzeichnet, die zahlreichen Wasservögeln als Nistplätze dienen. Geologisch befindet sich der See in einer flachen Senke, die sich während der Regenzeit füllt und in der Trockenzeit deutlich schrumpft. Historisch gesehen wurde der See von lokalen Fischergemeinschaften genutzt, die traditionelle Methoden anwenden, um den Fischbestand nachhaltig zu bewirtschaften. Das Wasser des Sees spielt auch eine Rolle bei der Regulierung des lokalen Mikroklimas und bietet in den heißen Sommermonaten Kühlung. Wissenschaftlich ist der Goumba-See von Interesse, um die Auswirkungen des Klimawandels auf die Binnengewässer Zentralafrikas zu untersuchen. Trotz seiner geringen Größe ist er ein unverzichtbarer Teil des regionalen hydrologischen Zyklus.",
      hu: "A Goumba-tó egy kisebb, de ökológiailag jelentős állóvíz a Közép-afrikai Köztársaságban, amely gyakran kapcsolódik a szezonális ártéri területekhez. Olyan régióban található, amelyet folyók és vizes élőhelyek összetett hálózata jellemez, és amely létfontosságú vízforrást biztosít a helyi flóra és fauna számára. A tó környezetére sűrű parti növényzet és mocsaras szakaszok jellemzőek, amelyek számos vízimadár számára szolgálnak fészkelőhelyül. Geológiailag a tó egy sekély mélyedésben helyezkedik el, amely az esős évszakban megtelik, a száraz évszakban pedig jelentősen összezsugorodik. Történelmileg a tavat a helyi halászközösségek használták, akik hagyományos módszerekkel biztosítják a halállomány fenntartható kezelését. A tó vize a helyi mikroklíma szabályozásában is szerepet játszik, hűsítve a környezetet a forró nyári hónapokban. Tudományos szempontból a Goumba-tó fontos helyszín a klímaváltozás belső-afrikai vizekre gyakorolt hatásainak tanulmányozására. Kis mérete ellenére a regionális hidrológiai ciklus elengedhetetlen része.",
      ro: "Lacul Goumba este un corp de apă mai mic, dar important din punct de vedere ecologic în Republica Centrafricană, fiind adesea asociat cu zonele inundabile sezoniere. Acesta este situat într-o regiune definită printr-o rețea complexă de râuri și zone umede care reprezintă o sursă vitală de apă pentru flora și fauna locală. Împrejurimile lacului sunt caracterizate de vegetație riverană densă și porțiuni mlăștinoase care servesc drept locuri de cuibărit pentru numeroase păsări acvatice. Din punct de vedere geologic, lacul ocupă o depresiune puțin adâncă ce se umple în timpul sezonului ploios și se restrânge semnificativ în perioada de secetă. Istoric, lacul a fost utilizat de comunitățile locale de pescari care aplică metode tradiționale pentru a gestiona durabil resursele piscicole. Apa lacului joacă, de asemenea, un rol în reglarea microclimatului local, oferind un efect de răcorire în lunile toride de vară. Științific, Lacul Goumba este de interes pentru studierea impactului schimbărilor climatice asupra apelor interioare din Africa Centrală. În ciuda dimensiunilor sale reduse, acesta este o parte indispensabilă a ciclului hidrologic regional.",
      en: "Lake Goumba is a smaller but ecologically significant body of water in the Central African Republic, often associated with seasonal floodplains. It is located in a region characterized by a complex network of rivers and wetlands that provide a vital water source for the local flora and fauna. The lake's surroundings are marked by dense riparian vegetation and marshy sections that serve as nesting sites for numerous waterfowl. Geologically, the lake sits in a shallow depression that fills during the rainy season and shrinks significantly during the dry period. Historically, the lake has been used by local fishing communities who employ traditional methods to manage fish stocks sustainably. The lake water also plays a role in regulating the local microclimate, providing cooling during the hot summer months. Scientifically, Lake Goumba is of interest for studying the impacts of climate change on Central African inland waters. Despite its modest size, it remains an indispensable part of the regional hydrological cycle.",
    },
    factsAdvanced: {
      de: ["Der See gehört zum hydrologischen System des Schari-Beckens.", "Die Wassertiefe schwankt saisonal zwischen 0,5 und 3 Metern.", "In der Umgebung leben seltene Amphibienarten.", "Der See ist ein wichtiger Rastplatz für Zugvögel aus Europa.", "Die Wassertemperatur liegt im Durchschnitt bei etwa 24 Grad Celsius.", "Die lokale Bevölkerung nutzt den See für die kleinbäuerliche Bewässerung."],
      hu: ["A tó a Schari-medence hidrológiai rendszerének része.", "A vízmélység szezonálisan 0,5 és 3 méter között változik.", "A tó környékén ritka kétéltűfajok élnek.", "A tó fontos pihenőhely az Európából érkező vándorló madaraknak.", "A víz átlaghőmérséklete körülbelül 24 Celsius-fok.", "A helyi lakosság a tavat kisléptékű öntözéses gazdálkodásra használja."],
      ro: ["Lacul aparține sistemului hidrologic al bazinului Chari.", "Adâncimea apei variază sezonier între 0,5 și 3 metri.", "În proximitatea lacului trăiesc specii rare de amfibieni.", "Lacul este un loc de popas crucial pentru păsările migratoare din Europa.", "Temperatura medie a apei este de aproximativ 24 de grade Celsius.", "Populația locală utilizează lacul pentru irigații la scară mică."],
      en: ["The lake is part of the Chari Basin hydrological system.", "Water depth fluctuates seasonally between 0.5 and 3 meters.", "The surrounding area is home to rare amphibian species.", "The lake is a key stopover for migratory birds from Europe.", "The average water temperature is approximately 24 degrees Celsius.", "Local communities use the lake for small-scale irrigation purposes."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein kleiner, aber ökologisch wichtiger See im zentralen Hochland des Landes.",
      hu: "Kicsi, de ökológiailag fontos tó az ország középső felföldjén.",
      ro: "Un lac mic, dar important din punct de vedere ecologic, în podișul central al țării.",
      en: "A small but ecologically important lake in the central highlands of the country.",
      es: "Un lago pequeño pero ecológicamente importante en las tierras altas centrales del país.",
      pt: "Um lago pequeno mas ecologicamente importante nas terras altas centrais do país.",
      fr: "Un petit lac, mais écologiquement important, situé dans les hauts plateaux centraux du pays.",
    },
    facts: {
      de: ["Wichtiger Rastplatz für Zugvögel.", "Umgeben von sumpfigen Uferzonen."],
      hu: ["Fontos pihenőhely a költöző madarak számára.", "Mocsaras partmenti zónák veszik körül."],
      ro: ["Loc important de odihnă pentru păsările migratoare.", "Înconjurat de zone de țărm mlaștinoase."],
      en: ["Significant stopover for migratory birds.", "Surrounded by marshy shore zones."],
      es: ["Importante lugar de descanso para aves migratorias.", "Rodeado de zonas ribereñas pantanosas."],
      pt: ["Importante local de repouso para aves migratórias.", "Rodeado por zonas ribeirinhas pantanosas."],
      fr: ["Lieu de repos important pour les oiseaux migrateurs.", "Entouré de zones riveraines marécageuses."],
    }, image: "/poi-images/lake-goumba-nature-v2.webp"},
  {
    id: "lake-mamere-nature-v2",
    type: "lake",
    parent: "CF-HK",
    coords: [22.1500, 7.4500],
    name: {
      de: "Maméré-See",
      hu: "Maméré-tó",
      ro: "Lacul Maméré",
      en: "Lake Mamere"},
    image: "/poi-images/lake-mamere-nature-v2.webp",
    descriptionAdvanced: {
      de: "Der Mamere-See liegt im nördlichen Teil der Zentralafrikanischen Republik und ist ein charakteristisches Beispiel für die Seenlandschaften der Sudanzone. Er ist in eine flache Savannenlandschaft eingebettet, die durch vereinzelte Baumgruppen und weite Grasflächen geprägt ist. Der See wird hauptsächlich durch atmosphärische Niederschläge und oberflächlichen Abfluss während der Monsunzeit gespeist. Aufgrund der hohen Verdunstungsraten in dieser Breitengradlage unterliegt der Wasserspiegel starken Schwankungen, was die Ansiedlung spezialisierter Pflanzenarten am Ufer begünstigt. Ökologisch gesehen bietet der Mamere-See einen lebenswichtigen Rückzugsort für Antilopen und Büffel während der harten Trockenzeit. Geologisch ruht das Becken auf einer Schicht aus undurchlässigen Tonen, die das Versickern des Wassers verhindern. Historisch war die Region um den See ein Treffpunkt für nomadische Viehhirten, die ihre Herden hier tränkten. Heute ist der See ein Ziel für ornithologische Beobachtungen, da er eine hohe Dichte an Reihern und Störchen aufweist. Die Erhaltung der Wasserqualität ist eine Herausforderung, die für das Überleben des lokalen Ökosystems von entscheidender Bedeutung ist.",
      hu: "A Mamere-tó a Közép-afrikai Köztársaság északi részén található, és a szudáni övezet tóvidékeinek jellegzetes példája. Lapos szavannai tájba ágyazódik, amelyet elszórt facsoportok és kiterjedt füves területek jellemeznek. A tavat főként a monszunidőszak alatti csapadék és a felszíni lefolyás táplálja. Ezen a szélességi körön a magas párolgási arány miatt a vízszint erősen ingadozik, ami kedvez a speciális parti növényfajok megtelepedésének. Ökológiai szempontból a Mamere-tó létfontosságú menedéket nyújt az antilopok és bivalyok számára a kemény száraz évszakban. Geológiailag a medence vízzáró agyagrétegen nyugszik, amely megakadályozza a víz elszivárgását. Történelmileg a tó környéke a nomád pásztorok találkozóhelye volt, akik itt itatták állataikat. Ma a tó az ornitológiai megfigyelések kedvelt célpontja, mivel nagy sűrűségben fordulnak elő itt gémek és gólyák. A vízminőség megőrzése komoly kihívás, amely döntő fontosságú a helyi ökoszisztéma túlélése szempontjából.",
      ro: "Lacul Mamere este situat în partea de nord a Republicii Centrafricane și reprezintă un exemplu caracteristic al peisajelor lacustre din zona sudaneză. Acesta este integrat într-un relief de savană plană, marcat de grupuri izolate de arbori și pajiști întinse. Lacul este alimentat în principal de precipitațiile din timpul musonului și de scurgerile de suprafață. Din cauza ratelor ridicate de evaporare la această latitudine, nivelul apei fluctuează drastic, favorizând dezvoltarea unor specii vegetale riverane adaptate. Din punct de vedere ecologic, Lacul Mamere oferă un refugiu esențial pentru antilope și bivoli în timpul sezonului secetos sever. Geologic, bazinul lacului este situat pe un strat de argile impermeabile care previn infiltrarea apei în sol. Istoric, regiunea din jurul lacului a fost un punct de întâlnire pentru păstorii nomazi care își adăpau turmele aici. Astăzi, lacul este o destinație pentru observații ornitologice, având o densitate mare de egrete și berze. Menținerea calității apei este o provocare majoră, fiind vitală pentru supraviețuirea ecosistemului local.",
      en: "Lake Mamere is situated in the northern part of the Central African Republic and serves as a characteristic example of the lake landscapes within the Sudanian zone. It is embedded in a flat savanna terrain marked by scattered tree clusters and vast grasslands. The lake is primarily fed by atmospheric precipitation and surface runoff during the monsoon season. Due to high evaporation rates at this latitude, the water level undergoes significant fluctuations, promoting the establishment of specialized riparian plant species. Ecologically, Lake Mamere provides a vital sanctuary for antelopes and buffaloes during the harsh dry season. Geologically, the basin rests on a layer of impermeable clays that prevent water from seeping into the ground. Historically, the area around the lake was a gathering point for nomadic pastoralists who watered their herds here. Today, the lake is a destination for ornithological observation, boasting a high density of herons and storks. Preserving water quality remains a challenge that is critical for the survival of the local ecosystem.",
    },
    factsAdvanced: {
      de: ["Der See liegt in der Nähe der Grenze zum Tschad.", "Die maximale Oberfläche beträgt in der Regenzeit etwa 15 Quadratkilometer.", "Er dient als wichtiges Tränkebecken für die Tierwelt des Bamingui-Bangoran Parks.", "Die Sedimente am Seeboden sind reich an organischen Stoffen.", "Im See kommen verschiedene Arten von Raubfischen vor.", "Die Region ist durch ein tropisches Wechselklima geprägt."],
      hu: ["A tó a csádi határ közelében helyezkedik el.", "Az esős évszakban a vízfelület elérheti a 15 négyzetkilométert.", "Fontos itatóhelyként szolgál a Bamingui-Bangoran Park vadállománya számára.", "A tófenék üledékei szerves anyagokban gazdagok.", "A tóban többféle ragadozó halfaj él.", "A régiót trópusi váltakozó nedves-száraz éghajlat jellemzi."],
      ro: ["Lacul se află în proximitatea frontierei cu Ciadul.", "Suprafața maximă în sezonul ploios atinge aproximativ 15 km pătrați.", "Servește ca loc de adăpare major pentru fauna din Parcul Bamingui-Bangoran.", "Sedimentele de pe fundul lacului sunt bogate în materie organică.", "În apele lacului trăiesc diverse specii de pești răpitori.", "Regiunea este caracterizată printr-un climat tropical alternant."],
      en: ["The lake is located near the border with Chad.", "Its maximum surface area reaches about 15 square kilometers during rains.", "It is a crucial watering hole for wildlife from Bamingui-Bangoran Park.", "The lakebed sediments are highly rich in organic matter.", "Various species of predatory fish inhabit the lake waters.", "The region is characterized by a tropical wet-and-dry climate."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein natürlicher See in der Haute-Kotto Region, der eine wichtige lokale Ressource darstellt.",
      hu: "Természetes tó a Haute-Kotto régióban, amely fontos helyi erőforrást jelent.",
      ro: "Un lac natural în regiunea Haute-Kotto, care reprezintă o resursă locală importantă.",
      en: "A natural lake in the Haute-Kotto region, representing an important local resource.",
      es: "Un lago natural en la región de Haute-Kotto que representa un recurso local fundamental.",
      pt: "Um lago natural na região de Haute-Kotto que representa um recurso local fundamental.",
      fr: "Un lac naturel dans la région de la Haute-Kotto qui constitue une ressource locale importante.",
    },
    facts: {
      de: ["Dient der lokalen Bevölkerung zur Fischerei.", "Wasserspiegel schwankt saisonal stark."],
      hu: ["A helyi lakosság halászatára szolgál.", "Vízszintje szezonálisan erősen ingadozik."],
      ro: ["Servește populației locale pentru pescuit.", "Nivelul apei variază semnificativ în funcție de sezon."],
      en: ["Used by the local population for fishing.", "Water level fluctuates significantly with the seasons."],
      es: ["Sirve a la población local para la pesca.", "El nivel del agua fluctúa fuertemente según la estación."],
      pt: ["Serve a população local para a pesca.", "O nível da água flutua fortemente de acordo com a estação."],
      fr: ["Sert à la population locale pour la pêche.", "Le niveau de l'eau fluctue fortement selon les saisons."],
    }, image: "/poi-images/lake-mamere-nature-v2.webp"},
  {
    id: "matoumbou-hills-nature-v2",
    type: "mountain",
    parent: "CF-LB",
    coords: [17.8000, 4.0000],
    name: {
      de: "Matoumbou-Hügel",
      hu: "Matoumbou-dombok",
      ro: "Dealurile Matoumbou",
      en: "Matoumbou Hills"},
    image: "/poi-images/matoumbou-hills-nature-v2.webp",
    descriptionAdvanced: {
      de: "Die Matoumbou-Hügel sind eine markante Hügelkette im zentralen Teil der Zentralafrikanischen Republik, die sich durch ihre sanften Formen und die üppige Vegetation auszeichnet. Diese Formationen bestehen überwiegend aus lateritischen Krusten und metamorphem Gestein, das über lange Zeiträume verwittert ist. Die Hügel fungieren als natürliche Barrieren, die die Luftströmungen beeinflussen und lokale Niederschlagsmuster erzeugen. In den Tälern zwischen den Hügeln findet man oft dichte Galeriewälder, während die Kuppen mit buschiger Savanne bedeckt sind. Die Matoumbou-Hügel bieten Lebensraum für zahlreiche Kleinsäugetiere und Reptilien, die an das felsige Terrain angepasst sind. Historisch wurden die Hügel oft als Ausguckspunkte und natürliche Festungen genutzt. Für die lokale Bevölkerung haben einige der Gipfel eine spirituelle Bedeutung und sind Schauplatz traditioneller Zeremonien. In der modernen Geografie werden die Hügel für die Forstwirtschaft und den Anbau von Nutzpflanzen in den fruchtbaren Senken genutzt. Die Region ist auch für ihre interessanten geologischen Aufschlüsse bekannt, die Einblicke in die frühe Krustenbildung Afrikas geben.",
      hu: "A Matoumbou-dombság egy jellegzetes vonulat a Közép-afrikai Köztársaság középső részén, amelyet lágy formái és dús növényzete határoz meg. Ezek a képződmények túlnyomórészt laterites kérgekből és metamorf kőzetekből állnak, amelyek hosszú idő alatt mállottak szét. A dombok természetes gátként működnek, befolyásolva a légáramlatokat és helyi csapadékmintákat hozva létre. A dombok közötti völgyekben gyakran sűrű galériaerdők találhatók, míg a tetőket cserjés szavanna borítja. A Matoumbou-dombság számos kisemlős és hüllő számára nyújt élőhelyet, amelyek alkalmazkodtak a sziklás terephez. Történelmileg a dombokat gyakran kilátópontként és természetes erődként használták. A helyi lakosság számára egyes csúcsok spirituális jelentőséggel bírnak, és hagyományos szertartások helyszínei. A modern földrajzban a dombokat erdőgazdálkodásra és haszonnövények termesztésére használják a termékeny völgyekben. A régió érdekes geológiai feltárásairól is ismert, amelyek betekintést nyújtanak Afrika korai kéregképződésébe.",
      ro: "Dealurile Matoumbou reprezintă un lanț colinar distinctiv în partea centrală a Republicii Centrafricane, caracterizat prin forme domoale și o vegetație luxuriantă. Aceste formațiuni sunt compuse în principal din cruste lateritice și roci metamorfice care au fost supuse eroziunii pe parcursul unor perioade îndelungate. Dealurile acționează ca bariere naturale care influențează curenții de aer, generând modele locale de precipitații. În văile dintre dealuri se găsesc adesea păduri-galerie dense, în timp ce culmile sunt acoperite de savană arbustivă. Dealurile Matoumbou oferă habitat pentru numeroase mamifere mici și reptile adaptate terenului stâncos. Istoric, colinele au fost utilizate frecvent ca puncte de observație și fortărețe naturale. Pentru populația locală, unele vârfuri au o semnificație spirituală, fiind locuri pentru ceremonii tradiționale. În geografia modernă, dealurile sunt folosite pentru silvicultură și cultivarea plantelor în zonele fertile de la bază. Regiunea este cunoscută și pentru aflorimentele sale geologice interesante care oferă indicii despre formarea timpurie a scoarței africane.",
      en: "The Matoumbou Hills are a prominent range of hills in the central part of the Central African Republic, characterized by their gentle slopes and lush vegetation. These formations consist primarily of lateritic crusts and metamorphic rocks that have weathered over long periods. The hills function as natural barriers that influence air currents and create localized precipitation patterns. In the valleys between the hills, one often finds dense gallery forests, while the summits are covered with shrubby savanna. The Matoumbou Hills provide a habitat for numerous small mammals and reptiles adapted to rocky terrain. Historically, the hills were often used as lookout points and natural fortifications. For the local population, some of the peaks hold spiritual significance and are sites for traditional ceremonies. In modern geography, the hills are utilized for forestry and the cultivation of crops in the fertile low-lying areas. The region is also known for its interesting geological outcrops that offer insights into Africa's early crustal formation.",
    },
    factsAdvanced: {
      de: ["Die Hügel erreichen Höhen von bis zu 750 Metern über dem Meeresspiegel.", "Sie bestehen teilweise aus eisenhaltigem Lateritgestein.", "Die Region verzeichnet eine jährliche Regenmenge von ca. 1.200 mm.", "In den Hügeln wurden prähistorische Steinwerkzeuge gefunden.", "Die Vegetation gehört zur sudano-guineischen Übergangszone.", "Die Hügel liegen in der Nähe wichtiger Transportrouten nach Bangui."],
      hu: ["A dombok magassága eléri a tengerszint feletti 750 métert.", "Részben vasban gazdag laterit kőzetből épülnek fel.", "A régióban az éves csapadékmennyiség körülbelül 1200 milliméter.", "A dombok között praehisztorikus kőeszközöket tártak fel.", "A növényzet a szudáni-guineai átmeneti zónához tartozik.", "A dombság a Bangui felé vezető fontos útvonalak közelében fekszik."],
      ro: ["Dealurile ating altitudini de până la 750 de metri deasupra mării.", "Sunt compuse parțial din roci lateritice bogate în fier.", "Regiunea înregistrează precipitații anuale de circa 1.200 mm.", "În zonă au fost descoperite unelte de piatră preistorice.", "Vegetația aparține zonei de tranziție sudano-guineene.", "Dealurile sunt situate în apropierea rutelor comerciale către Bangui."],
      en: ["The hills reach elevations of up to 750 meters above sea level.", "They are partially composed of iron-rich laterite rock.", "The region records an annual rainfall of approximately 1,200 mm.", "Prehistoric stone tools have been discovered within the hills.", "The vegetation is part of the Sudano-Guinean transition zone.", "The hills are located near major transport routes heading to Bangui."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Eine Hügelkette im Süden, die von dichtem Wald bedeckt ist und das Lobaye-Becken überragt.",
      hu: "Dombvidék délen, amelyet sűrű erdő borít, és a Lobaye-medence fölé magasodik.",
      ro: "Un lanț de dealuri în sud, acoperit de pădure densă, care domină bazinul Lobaye.",
      en: "A chain of hills in the south, covered by dense forest, overlooking the Lobaye basin.",
      es: "Una cadena de colinas en el sur, cubierta de bosque denso, que domina la cuenca del Lobaye.",
      pt: "Uma cadeia de colinas no sul, coberta de floresta densa, que domina a bacia do Lobaye.",
      fr: "Une chaîne de collines au sud, couverte d'une forêt dense et surplombant le bassin de la Lobaye.",
    },
    facts: {
      de: ["Wichtiger Lebensraum für Primaten.", "Bekannt für ihre reiche Flora."],
      hu: ["Prímások fontos élőhelye.", "Gazdag flórájáról ismert."],
      ro: ["Habitat important pentru primate.", "Cunoscut pentru flora sa bogată."],
      en: ["Important habitat for primates.", "Known for its rich flora."],
      es: ["Hábitat importante para los primates.", "Conocidas por su rica flora."],
      pt: ["Habitat importante para primatas.", "Conhecidas pela sua rica flora."],
      fr: ["Habitat important pour les primates.", "Connues pour leur flore riche."],
    }, image: "/poi-images/matoumbou-hills-nature-v2.webp"},
  {
    id: "fertit-mountains-nature-v2",
    type: "mountain",
    parent: "CF-VK",
    coords: [23.8000, 9.8000],
    name: {
      de: "Fertit-Berge",
      hu: "Fertit-hegység",
      ro: "Munții Fertit",
      en: "Fertit Mountains"},
    image: "/poi-images/fertit-mountains-nature-v2.webp",
    descriptionAdvanced: {
      de: "Die Fertit-Berge, auch bekannt als Jebel Fertit, liegen im Grenzgebiet zwischen dem Nordosten der Zentralafrikanischen Republik und dem Sudan. Diese Gebirgsregion ist historisch und geografisch eng mit dem Bongo-Massiv verbunden und bildet eine zerklüftete, schwer zugängliche Wildnis. Die Gipfel bestehen aus widerstandsfähigem Sandstein und Granit, die eine Reihe von schroffen Plateaus und tief eingeschnittenen Schluchten bilden. Klimatisch ist das Gebiet durch extreme saisonale Unterschiede geprägt, mit einer intensiven Regenzeit und einer sehr trockenen Periode, in der viele Bäche versiegen. Die Fertit-Berge sind ein wichtiges Rückzugsgebiet für seltene Großsäuger wie die Riesenelena-Antilope und Bergzebras. Historisch gesehen war die Region als 'Dar Fertit' bekannt, ein Gebiet, das lange Zeit außerhalb der direkten Kontrolle kolonialer Mächte lag und verschiedenen ethnischen Gruppen Schutz bot. Die kulturelle Identität der Region ist durch eine Mischung aus zentralafrikanischen und sudanesischen Einflüssen geprägt. Heute stellen die Abgelegenheit und die politische Lage große Herausforderungen für den Naturschutz dar, machen die Berge aber gleichzeitig zu einem der letzten echten Wildnisgebiete Afrikas.",
      hu: "A Fertit-hegység, más néven Jebel Fertit, a Közép-afrikai Köztársaság északkeleti határvidékén és Szudán területén található. Ez a hegyvidéki régió történelmileg és földrajzilag szorosan kapcsolódik a Bongo-masszívumhoz, egy tagolt, nehezen hozzáférhető vadont alkotva. A csúcsok ellenálló homokkőből és gránitból állnak, amelyek meredek fennsíkokat és mély szurdokokat formáznak. Éghajlatilag a területet szélsőséges szezonális különbségek jellemzik: intenzív esős évszak és nagyon száraz időszak váltja egymást, amikor sok patak kiszárad. A Fertit-hegység fontos menedékhely az olyan ritka nagyemlősök számára, mint az óriás lantszarvú antilop. Történelmileg a régió 'Dar Fertit' néven volt ismert, egy olyan területként, amely sokáig kívül esett a gyarmati hatalmak közvetlen ellenőrzésén, és védelmet nyújtott különböző etnikai csoportoknak. A régió kulturális identitását a közép-afrikai és szudáni hatások keveredése határozza meg. Napjainkban az elszigeteltség és a politikai helyzet nagy kihívást jelent a természetvédelem számára, ugyanakkor Afrika egyik utolsó valódi vadonjává teszi a hegységet.",
      ro: "Munții Fertit, cunoscuți și sub numele de Jebel Fertit, se află în zona de frontieră dintre nord-estul Republicii Centrafricane și Sudan. Această regiune montană este strâns legată istoric și geografic de Masivul Bongo, formând o zonă sălbatică accidentată și greu accesibilă. Vârfurile sunt compuse din gresie rezistentă și granit, care formează o serie de platouri abrupte și defilee adânci. Din punct de vedere climatic, zona este marcată de diferențe sezoniere extreme, cu un sezon ploios intens și o perioadă foarte secetoasă în care multe cursuri de apă dispar. Munții Fertit reprezintă un refugiu important pentru mamifere mari rare, cum ar fi antilopa eland uriașă. Istoric, regiunea a fost cunoscută sub numele de 'Dar Fertit', un teritoriu care a rămas mult timp în afara controlului direct al puterilor coloniale, oferind adăpost diverselor grupuri etnice. Identitatea culturală a regiunii este definită printr-un amestec de influențe centrafricane și sudaneze. Astăzi, izolarea și contextul politic reprezintă provocări majore pentru conservare, transformând totodată acești munți în una dintre ultimele zone de sălbăticie veritabilă din Africa.",
      en: "The Fertit Mountains, also known as Jebel Fertit, are located in the borderlands between the northeastern Central African Republic and Sudan. This mountainous region is historically and geographically linked to the Bongo Massif, forming a rugged and inaccessible wilderness. The peaks consist of resilient sandstone and granite, which create a series of stark plateaus and deeply incised gorges. Climatically, the area is defined by extreme seasonal differences, featuring an intense rainy season and a very dry period during which many streams vanish. The Fertit Mountains serve as a critical refuge for rare large mammals, such as the giant eland. Historically, the region was known as 'Dar Fertit,' a territory that remained outside the direct control of colonial powers for a long time, providing shelter to various ethnic groups. The region's cultural identity is shaped by a blend of Central African and Sudanese influences. Today, its remoteness and the political situation present major challenges for conservation, yet simultaneously make these mountains one of Africa's last true wilderness areas.",
    },
    factsAdvanced: {
      de: ["Die Berge erstrecken sich über die Grenze zwischen der ZAR und dem Sudan.", "Die höchste Erhebung in diesem Sektor liegt bei etwa 1.150 Metern.", "Das Gestein ist überwiegend präkambrischen und paläozoischen Ursprungs.", "Die Region ist bekannt für ihre Vorkommen an mineralischen Erzen.", "Dar Fertit bedeutet in lokalen Sprachen 'Land der Fertit-Völker'.", "Die jährliche Niederschlagsmenge schwankt stark zwischen 800 und 1.000 mm."],
      hu: ["A hegység a ZAR és Szudán határán húzódik keresztül.", "A szektor legmagasabb pontja körülbelül 1150 méter.", "A kőzetek többnyire prekambriumi és paleozoikumi eredetűek.", "A régió ismert az ásványi ércek előfordulásáról.", "A 'Dar Fertit' elnevezés a helyi nyelveken a Fertit népek földjét jelenti.", "Az éves csapadékmennyiség 800 és 1000 mm között változik."],
      ro: ["Munții se întind peste granița dintre Republica Centrafricană și Sudan.", "Cea mai înaltă cotă din acest sector este de aproximativ 1.150 de metri.", "Rocile sunt predominant de origine precambriană și paleozoică.", "Regiunea este cunoscută pentru zăcămintele sale de minereuri metalifere.", "Dar Fertit înseamnă în limbile locale 'Țara popoarelor Fertit'.", "Cantitatea anuală de precipitații variază între 800 și 1.000 mm."],
      en: ["The mountains span the international border between the CAR and Sudan.", "The highest elevation in this sector is approximately 1,150 meters.", "The rock formations are mainly of Precambrian and Paleozoic origin.", "The region is known for its occurrences of mineral ores.", "Dar Fertit translates in local languages to 'Land of the Fertit peoples.'", "Annual rainfall fluctuates significantly between 800 and 1,000 mm."],
      es: [],
      pt: [],
      fr: [],
    },
    description: {
      de: "Ein Gebirgszug im äußersten Nordosten des Landes an der Grenze zum Sudan.",
      hu: "Hegylánc az ország távoli északkeleti részén, a szudáni határon.",
      ro: "Un lanț muntos în nord-estul extrem al țării, la granița cu Sudanul.",
      en: "A mountain range in the far northeast of the country on the border with Sudan.",
      es: "Una cadena montañosa en el extremo noreste del país, en la frontera con Sudán.",
      pt: "Uma cordilheira no extremo nordeste do país, na fronteira com o Sudão.",
      fr: "Une chaîne de montagnes à l'extrême nord-est du pays, à la frontière avec le Soudan.",
    },
    facts: {
      de: ["Besteht aus sehr altem Gestein.", "Abgelegene Region mit wenig menschlicher Besiedlung."],
      hu: ["Nagyon idős kőzetekből épül fel.", "Távoli régió kevés emberi településsel."],
      ro: ["Format din roci foarte vechi.", "Regiune izolată cu puține așezări umane."],
      en: ["Composed of very ancient rock.", "Remote region with sparse human settlement."],
      es: ["Compuesta por rocas muy antiguas.", "Región remota con poco asentamiento humano."],
      pt: ["Composta por rochas muito antigas.", "Região remota com pouco povoamento humano."],
      fr: ["Composée de roches très anciennes.", "Région isolée avec peu de peuplement humain."],
    }
  }
];

