import type { StarholdModuleId, StarholdResources, StarholdState } from "./types";

export function clamp(value: number, min = 0, max = 100) {
  return Math.max(min, Math.min(max, value));
}

export function withAlert(state: StarholdState, alert: string | null): StarholdState {
  return { ...state, alert };
}
...
export function pushSystemJournal(state: StarholdState, line: string): Pick<StarholdState, "journal"> {
