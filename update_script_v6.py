...
def get_desc_for_poi(poi_id):
    if 'incles' in poi_id:
        ...
        # A bit more generic but distinct enough
...
def update_poi_block(block):
    # Update descriptionAdvanced.en if empty
    # Looking for: en: ""
    # But notice some might have other characters inside. Let's use a regex that matches empty or near-empty strings.
    
    # We target descriptionAdvanced field specifically
    if 'descriptionAdvanced' in block:
        # Use regex to replace the empty en field
        block = re.sub(r'(descriptionAdvanced:\s*\{[^}]*?en:\s*")(")', 
                       lambda m: m.group(1) + get_desc_for_poi(re.search(r'id:\s*"([^"]+)"', block).group(1)) + m.group(2), 
                       block)

    # Update factsAdvanced.en if empty
    if 'factsAdvanced' in block and 'en: []' in block:
        # get_facts_for_poi same as v5
        poi_id_match = re.search(r'id:\s*"([^"]+)"', block)
        if poi_id_match:
            poi_id = poi_id_match.group(1)
            # facts logic (same as v5)
            if 'incles' in poi_id:
