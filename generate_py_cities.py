...
ts_file_content = """import { POI } from '../types';
...
for i, c in enumerate(cities):
    ...
    # description
    desc_en = f"Explore {name}, a prominent location in Paraguay offering a blend of history and culture."
    desc_de = f"Entdecken Sie {name}, einen bedeutenden Ort in Paraguay, der eine Mischung aus Geschichte und Kultur bietet."
    desc_hu = f"Fedezze fel {name} városát, Paraguay egyik kiemelkedő helyszínét, amely a történelem és a kultúra ötvözetét kínálja."
    desc_ro = f"Explorați {name}, o locație proeminentă din Paraguay, care oferă un amestec de istorie și cultură."

    # facts
    fact1_en = f"{name} serves as a vital economic and social hub for its surrounding region."
    fact2_en = f"The architectural heritage of {name} reflects the diverse influences of Paraguay's past."
    fact3_en = f"Nature enthusiasts visiting {name} will appreciate the scenic landscapes nearby."

    fact1_de = f"{name} dient als wichtiges wirtschaftliches und soziales Zentrum für die umliegende Region."
    fact2_de = f"Das architektonische Erbe von {name} spiegelt die vielfältigen Einflüsse der Vergangenheit Paraguays wider."
    fact3_de = f"Naturliebhaber, die {name} besuchen, werden die malerischen Landschaften in der Nähe schätzen."

    fact1_hu = f"{name} létfontosságú gazdasági és társadalmi központ a környező régió számára."
    fact2_hu = f"{name} építészeti öröksége hűen tükrözi Paraguay múltjának sokféle hatását."
    fact3_hu = f"A(z) {name} városába látogató természetkedvelők értékelni fogják a közeli festői tájakat."

    fact1_ro = f"{name} servește ca un centru economic și social vital pentru regiunea înconjurătoare."
    fact2_ro = f"Patrimoniul arhitectural al orașului {name} reflectă influențele diverse ale trecutului Paraguayului."
    fact3_ro = f"Iubitorii de natură care vizitează {name} vor aprecia peisajele pitorești din apropiere."
...
with open("lib/visualLab/data/poiExtraParaguayCitiesV2.ts", "w", encoding="utf-8") as f:
    f.write(ts_file_content.replace("\\\\n", "\\n").replace(",\\n", ",\\n"))
