...
file_path = "lib/visualLab/data/poiExtraLaosCitiesV2.ts"
...
for poi_id, facts in pois.items():
    # Find the POI block
    # We look for: id: "poi_id" ... up to the facts object
    
    # We will format the factsAdvanced as a JSON-like string without outer braces, then insert it
    
    facts_adv_str = ",\n    factsAdvanced: {\n"
    for lang, items in facts.items():
        facts_adv_str += f"      {lang}: [\n"
        for i, item in enumerate(items):
            escaped_item = item.replace('"', '\\"')
            comma = "," if i < len(items) - 1 else ""
            facts_adv_str += f'        "{escaped_item}"{comma}\n'
        facts_adv_str += "      ],\n" if lang != "ro" else "      ]\n" # ro is last in my loop
    facts_adv_str += "    }"
    
    # Look for the end of facts: { ... }
    # We'll use a regex to match the id line, then any characters until facts: { ... } ends
    # The end is usually `      ]\n    }`
    ...
    if match:
        ...
        if "factsAdvanced" not in original:
            replaced = original + facts_adv_str
            content = content.replace(original, replaced)
            print(f"Patched {poi_id}")
        else:
            print(f"Skipped {poi_id} (already has factsAdvanced)")
    else:
        print(f"Could not find POI: {poi_id}")
