...
updates = {
    ...
    "cult-design-district": {
        ...
        "factsAdvanced": [
            "The Design District officially established in 2005 to promote Helsinki's creative industries.",
...
def update_poi(match):
    ...
    if 'descriptionAdvanced' in u:
        ...
        # Look for descriptionAdvanced block
        if 'descriptionAdvanced:' in poi_block:
            # Check if en is empty
            if re.search(r'descriptionAdvanced:\s*\{[^}]*en:\s*""', poi_block):
                poi_block = re.sub(r'(descriptionAdvanced:\s*\{[^}]*en:\s*)""', r'\1"' + desc_escaped + r'"', poi_block)
            elif 'en:' not in re.search(r'descriptionAdvanced:\s*\{([^}]*)\}', poi_block).group(1):
                # Add en field if missing
                poi_block = re.sub(r'(descriptionAdvanced:\s*\{)', r'\1\n      en: "' + desc_escaped + r'",', poi_block)
        else:
            # Add the whole block
            new_block = '    descriptionAdvanced: {\n      en: "' + desc_escaped + r'",\n      de: "",\n      hu: "",\n      ro: ""\n    },\n'
            poi_block = re.sub(r'(name: \{[^}]+\},)', r'\1\n' + new_block, poi_block)
    ...
    if 'factsAdvanced' in u:
        ...
        if 'factsAdvanced:' in poi_block:
             # Check if en is empty
            if re.search(r'factsAdvanced:\s*\{[^}]*en:\s*\[\s*\]', poi_block):
                poi_block = re.sub(r'(factsAdvanced:\s*\{[^}]*en:\s*)\[\s*\]', r'\1' + facts_block_str, poi_block)
            elif 'en:' not in re.search(r'factsAdvanced:\s*\{([^}]*)\}', poi_block).group(1):
                 # Add en field if missing
                poi_block = re.sub(r'(factsAdvanced:\s*\{)', r'\1\n      en: ' + facts_block_str + r',', poi_block)
        else:
            # Add the whole block
            new_block = '    factsAdvanced: {\n      en: ' + facts_block_str + r',\n      de: [],\n      hu: [],\n      ro: []\n    },\n'
            # Insert before image or end of object
            if 'image:' in poi_block:
                poi_block = re.sub(r'(image:)', new_block + r'    \1', poi_block)
            else:
                poi_block = re.sub(r'(\s*\},?\s*)$', r'\n' + new_block + r'\1', poi_block)

    return poi_block

# This regex tries to match the whole POI object more accurately
new_content = re.sub(r'\{\s+id: "([^"]+)",.*?\n\s+\},?', update_poi, content, flags=re.DOTALL)
