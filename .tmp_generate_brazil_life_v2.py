...
for file_path in glob.glob("lib/visualLab/data/brazilPoi.ts") + glob.glob("lib/visualLab/data/poiExtraBrazil*.ts"):
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as f:
            content = f.read()
            # Find names: name: { de: "Some Name", ... }
            matches = re.findall(r'name:\s*\{\s*de:\s*"([^"]+)"', content)
            for m in matches:
                existing_de_names.add(m.lower())

# Filter our list
filtered_pois = []
for p in pois:
    if p["name"]["de"].lower() not in existing_de_names:
        filtered_pois.append(p)

if len(filtered_pois) != 30:
    print(f"Error: expected 30 POIs, but got {len(filtered_pois)} due to duplicates.")
    exit(1)

# Write TS file
with open("lib/visualLab/data/poiExtraBrazilLifeV2.ts", "w", encoding="utf-8") as f:
    f.write('import type { POI } from "./poi";\n\n')
    f.write('export const poiExtraBrazilLifeV2: POI[] = [\n')
    ...
    for i, poi in enumerate(filtered_pois):
        ...
        name_str = f'de: "{poi["name"]["de"]}", hu: "{poi["name"]["hu"]}", ro: "{poi["name"]["ro"]}", en: "{poi["name"]["en"]}"'
        f.write(f'    name: {{ {name_str} }},\n')
        
        desc_str = f'de: "{poi["description"]["de"]}", hu: "{poi["description"]["hu"]}", ro: "{poi["description"]["ro"]}", en: "{poi["description"]["en"]}"'
        f.write(f'    description: {{ {desc_str} }},\n')
        
        f.write('    facts: {\n')
        for lang in ['de', 'hu', 'ro', 'en']:
            facts = [f'"{fact}"' for fact in poi["facts"][lang]]
            f.write(f'      {lang}: [{", ".join(facts)}]{"," if lang != "en" else ""}\n')
        f.write('    }\n')
        
        if i < len(filtered_pois) - 1:
            f.write('  },\n')
        else:
            f.write('  }\n')
...
print(f"Success! {len(filtered_pois)} POIs generated.")
print(f"Blocklist size: {len(existing_de_names)}")
print(f"File size: {os.path.getsize('lib/visualLab/data/poiExtraBrazilLifeV2.ts')} bytes")
