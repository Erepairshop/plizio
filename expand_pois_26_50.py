...
new_pois = []
for poi_id, data in advanced_data.items():
    pattern = r'id: "' + poi_id + r'",(.*?)(?=\n  \{|\n\])'
    match = re.search(pattern, content, re.DOTALL)
    if match:
        inner = match.group(1)
        type_val = re.search(r'type: "(.*?)",', inner).group(1)
        parent_val = re.search(r'parent: "(.*?)",', inner).group(1)
        coords_val = re.search(r'coords: \[(.*?)\],', inner).group(1)
        name_val = re.search(r'name: \{(.*?)\},', inner, re.DOTALL).group(1).strip()
        
        desc_match = re.search(r'description: \{(.*?)\},', inner, re.DOTALL)
        if desc_match:
            desc_val = desc_match.group(1).strip()
        else:
            desc_val = re.search(r'description: \{(.*?)(?=descriptionAdvanced)', inner, re.DOTALL).group(1).strip()
            desc_val = desc_val.rstrip().rstrip(',')
            
        facts_match = re.findall(r'facts: \{(.*?)\}', inner, re.DOTALL)
        if facts_match:
            facts_val = facts_match[-1].strip()
        else:
            facts_val = ""

        desc_adv = data["description"]
    ...
    factsAdvanced: {{
      ...
      en: []
        ...
        new_pois.append((poi_id, new_poi))
...
if start_idx != -1 and end_idx != -1:
    new_block = start_marker + "\n" + "\n".join([p[1] for p in new_pois]) + "\n\n  "
    new_content = content[:start_idx] + new_block + content[end_idx:]
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Successfully updated POIs 26-50")
else:
