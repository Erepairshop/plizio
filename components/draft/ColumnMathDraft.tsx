...
function ColumnMathDraft({
  ...
  // ─── REF GRID: store refs in a Map keyed by "row_col" ───
  ...
  const handleCellInput = useCallback(
    (row: number, col: number, value: string) => {
      setState((prev) => {
        ...
        const s: ColumnDraftState = { ...prev, rows: newRows, focusRow: row, focusCol: col };
        syncToProvider(s);
        return s;
      });

      // Auto-advance left after digit entry (right-to-left for column math)
      if (value !== "") {
        const nextCol = col - 1;
        if (nextCol >= 0) {
    ...
    (row: number, col: number, e: React.KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case "Enter":
          ...
          break;
        case "Backspace": {
          const cellVal = state.rows[row]?.cells[col]?.value;
          if (cellVal === "") {
            e.preventDefault();
            // Move right (backtrack in right-to-left mode) and clear that cell
            const prevCol = col + 1;
            if (prevCol < cols) {
              setState((prev) => {
                const newRows = prev.rows.map((r, ri) =>
        ...
        default:
          // Block non-digit character keys
      ...
      <div className="flex items-center justify-between mb-3 gap-2">
        ...
        <div className="flex items-center gap-0.5">
          {(["+", "-", "×", "÷"] as Operation[]).map((op) => (
            <button
              key={op}
              onClick={() => setOperation((prev) => (prev === op ? "" : op))}
              className={`
                w-7 h-7 rounded text-sm font-black transition-colors
                ${operation === op
        ...
        <div className="inline-block p-2 bg-white rounded-lg border border-gray-100">
          {state.rows.map((row, ri) => (
            <div key={ri} className="flex items-center mt-[2px] first:mt-0">
              {/* Operation symbol on the left of the second row */}
