...
interface MultiplicationDraftProps {
  ...
  cols?: number;
  /** Initial number of rows (default 6) */
  initialRows?: number;
}
...
const MulCell = memo(function MulCell({
  value,
  rowIdx,
  colIdx,
  isFocused,
...
function MultiplicationDraft({
  ...
  const handleInput = useCallback(
    (row: number, col: number, value: string) => {
      setState((prev) => {
        ...
        const newState: MultiplicationDraftState = {
          ...prev,
          rows: newRows,
          focusRow: row,
          focusCol: nextCol,
  ...
  const handleClear = useCallback(() => {
    const newState = createMultiplicationDraft(cols, initialRows);
    setState(newState);
    syncToProvider(newState);
  ...
  return (
    <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-200 shadow-sm">
      ...
      {/* Grid - multiplication uses right-to-left entry, same as column math */}
          ...
          >
            {row.cells.map((cell, colIdx) => (
              <MulCell
                ...
                rowIdx={rowIdx}
                colIdx={colIdx}
                isFocused={state.focusRow === rowIdx && state.focusCol === colIdx}
                onInput={handleInput}
      ...
      <p className="mt-2 text-[10px] text-gray-400">
        Szorzásnál: 1. sor = szorzandó, 2. sor = szorzó, alatta részeredmények
      </p>
    </div>
  );
}
