...
import { HistoryTestDiagram, type HistoryDiagramId } from "./HistoryTestDiagrams";

type Props = {
  mode: "choice" | "order";
  diagramId: HistoryDiagramId;
  prompt: string;
  title: string;
  clue: string;
  options?: string[];
...
export default function HistoryMissionQuestion(props: Props) {
  const {
  ...
  const correct = mode === "choice"
    ...
    : selected.join(SEP) === correctOrder.join(SEP);
  ...
  return (
    <section className="mx-1 my-2 p-2 sm:p-3">
      <div className="grid gap-3 sm:grid-cols-[190px_1fr] sm:items-center">
        <HistoryTestDiagram kind={diagramId} label={`${title}. ${clue}`} className="mx-auto h-auto w-full max-w-[190px]" />
        <div className="min-w-0">
          <p className="text-xs font-bold uppercase tracking-wide text-amber-700">{prompt}</p>
      ...
      {mode === "choice" ? (
        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
          {options.map(option => {
            ...
            const state = submitted
              ...
              : chosen ? "border-amber-500 bg-amber-500 text-white" : "border-slate-300 bg-white text-slate-700 hover:border-amber-400 hover:bg-amber-50";
        ...
        <>
          <div className="mt-3 min-h-14 rounded-xl border border-dashed border-amber-300 bg-amber-50/40 p-2">
            ...
            {selected.length === 0 ? <p className="py-1 text-sm italic text-slate-400">{tapLabel}</p> : (
              <div className="flex flex-wrap items-center gap-1.5">
                {selected.map((item, index) => <span key={item} className="flex items-center gap-1">{index > 0 && <span className="font-bold text-amber-500">→</span>}<span className="rounded-lg border border-amber-200 bg-white px-2 py-1 text-sm font-bold text-slate-800">{item}</span></span>)}
              </div>
            )}
          </div>
          {!submitted && <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
            {available.map(item => <button key={item} type="button" aria-pressed={selected.includes(item)} onClick={() => toggle(item)} className={`min-h-12 rounded-xl border-2 px-3 py-2 text-left text-sm font-bold transition ${selected.includes(item) ? "border-amber-500 bg-amber-500 text-white" : "border-slate-300 bg-white text-slate-700 hover:border-amber-400 hover:bg-amber-50"}`}>{item}</button>)}
          </div>}
          {!submitted && selected.length > 0 && <button type="button" onClick={() => { setSelected([]); onAnswer(""); }} className="mt-2 min-h-10 rounded-lg px-3 text-sm font-semibold text-slate-500 hover:bg-amber-50 hover:text-amber-700">↺ {resetLabel}</button>}
        </>
      )}
