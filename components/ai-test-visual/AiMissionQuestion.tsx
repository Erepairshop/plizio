...
import { AiTestDiagram, type AiTestDiagramId } from "./AiTestDiagrams";

type Props = {
  mode: "choice" | "order";
  diagramId: AiTestDiagramId;
  prompt: string;
  title: string;
  clue: string;
  options?: string[];
...
export default function AiMissionQuestion(props: Props) {
  const {
  ...
  return <section className="rounded-2xl border border-indigo-100 bg-white/80 p-3 shadow-sm sm:p-4">
    <div className="grid gap-3 sm:grid-cols-[190px_1fr] sm:items-center">
      <AiTestDiagram kind={diagramId} label={`${title}. ${clue}`} className="mx-auto h-auto w-full max-w-[190px]" />
      <div className="min-w-0">
        <p className="text-xs font-bold uppercase tracking-wide text-indigo-700">{prompt}</p>
    ...
    {mode === "choice" ? <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">{options.map(option => {
      ...
      const state = submitted
        ? answer ? "border-emerald-500 bg-emerald-500 text-white" : chosen ? "border-rose-400 bg-rose-100 text-rose-700" : "border-slate-200 bg-white text-slate-400"
        : chosen ? "border-indigo-600 bg-indigo-600 text-white" : "border-slate-300 bg-white text-slate-700 hover:border-indigo-400 hover:bg-indigo-50";
      ...
      <div className="mt-3 min-h-14 rounded-xl border border-dashed border-indigo-300 bg-indigo-50/40 p-2">
        <p className="mb-1 text-xs font-bold uppercase tracking-wide text-slate-500">{orderLabel}</p>
        {selected.length === 0 ? <p className="py-1 text-sm italic text-slate-400">{tapLabel}</p> : <div className="flex flex-wrap items-center gap-1.5">{selected.map((item, index) => <span key={item} className="flex items-center gap-1">{index > 0 && <span className="font-bold text-indigo-600">→</span>}<span className="rounded-lg border border-indigo-200 bg-white px-2 py-1 text-sm font-bold text-slate-800">{item}</span></span>)}</div>}
      </div>
