...
import type { PhysicsGameId, PhysicsLevel } from "@/lib/visualLab/physicsCurriculum";
import { PhysicsLevelBar, usePhysicsGameProgress } from "./PhysicsGameProgress";
...
export default function PhysicsRoundSession<T extends { id: string }>({ gameId, grade, lang, rounds, renderRound }: {
  gameId: PhysicsGameId;
  grade: number;
  lang: Lang;
  rounds: T[] | ((level: PhysicsLevel, count: number) => T[]);
  renderRound: (round: T, onDone: (score: number, total?: number) => void, key: string) => ReactNode;
}) {
  const t = COPY[lang] ?? COPY.en;
  const { progress, rounds: targetRounds, mastery, selectLevel, recordAnswer, advanceToUnlockedLevel } = usePhysicsGameProgress(gameId, grade, lang);
  ...
  return (
    <div>
      <PhysicsLevelBar grade={grade} lang={lang} progress={progress} mastery={mastery} onSelect={selectLevel} />
