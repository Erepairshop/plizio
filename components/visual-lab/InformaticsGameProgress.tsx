...
import {
  clampInformaticsLevel,
  requiredCorrectForInformaticsLevel,
  type InformaticsGameId,
  type InformaticsLevel,
} from "@/lib/visualLab/informaticsCurriculum";

type Lang = "de" | "hu" | "ro" | "en";

export interface StoredInformaticsProgress {
  unlockedLevel: InformaticsLevel;
  selectedLevel: InformaticsLevel;
...
function emptyProgress(): StoredInformaticsProgress {
...
function normalizeProgress(value: unknown): StoredInformaticsProgress {
  if (!value || typeof value !== "object") return emptyProgress();
  const input = value as Partial<StoredInformaticsProgress>;
  const correctByLevel = normalizeCounts(input.correctByLevel);
  let unlockedLevel = clampInformaticsLevel(Number(input.unlockedLevel));
  while (unlockedLevel < 5 && correctByLevel[unlockedLevel - 1] >= requiredCorrectForInformaticsLevel(unlockedLevel)) {
    unlockedLevel = (unlockedLevel + 1) as InformaticsLevel;
  }
  return {
    unlockedLevel,
    selectedLevel: Math.min(unlockedLevel, clampInformaticsLevel(Number(input.selectedLevel))) as InformaticsLevel,
    correctByLevel,
...
export function useInformaticsGameProgress(gameId: InformaticsGameId, grade: number, lang: Lang) {
  const storageKey = `plizio:informatics-progress:v1:${lang}:g${grade}:${gameId}`;
  const [progress, setProgress] = useState<StoredInformaticsProgress>(() => emptyProgress());
  ...
  const persist = useCallback((next: StoredInformaticsProgress) => {
  ...
  const selectLevel = useCallback((levelInput: number) => {
    setProgress((current) => persist({
      ...current,
      selectedLevel: Math.min(current.unlockedLevel, clampInformaticsLevel(levelInput)) as InformaticsLevel,
    }));
  }, [persist]);
  ...
  const recordAnswer = useCallback((correct: boolean) => {
    setProgress((current) => {
      ...
      if (correct && current.selectedLevel === unlockedLevel && unlockedLevel < 5
        && correctByLevel[index] >= requiredCorrectForInformaticsLevel(current.selectedLevel)) {
        unlockedLevel = (unlockedLevel + 1) as InformaticsLevel;
      }
  ...
  const required = requiredCorrectForInformaticsLevel(progress.selectedLevel);
  return {
    progress,
    rounds: required,
...
export function InformaticsLevelBar({ grade, lang, progress, mastery, onSelect }: {
  grade: number;
  lang: Lang;
  progress: StoredInformaticsProgress;
  mastery: number;
  onSelect: (level: InformaticsLevel) => void;
}) {
  const t = COPY[lang] ?? COPY.en;
  return (
    <div className="mb-3 rounded-xl border border-cyan-300/15 bg-black/25 p-2.5 text-white">
      <div className="mb-2 flex items-center justify-between gap-3 text-xs font-bold">
        <span>{t.grade} {grade}</span>
        <span>{t.level} {progress.selectedLevel}/5 · ★ {progress.stars}</span>
      </div>
      <div className="grid grid-cols-5 gap-1.5">
        {([1, 2, 3, 4, 5] as InformaticsLevel[]).map((level) => {
          ...
          return (
            <button key={level} type="button" disabled={!unlocked} onClick={() => onSelect(level)}
              ...
              className={`min-h-9 rounded-lg border text-sm font-black transition active:scale-95 ${selected ? "border-cyan-300 bg-cyan-500/30 text-cyan-50" : unlocked ? "border-white/15 bg-white/5 text-white/75" : "border-white/5 bg-white/[.02] text-white/20"}`}>
              {unlocked ? level : "·"}
            </button>
          );
        })}
      ...
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10" aria-label={`${t.next}: ${Math.round(mastery * 100)}%`}>
        <div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-indigo-400 transition-all" style={{ width: `${Math.round(mastery * 100)}%` }} />
      </div>
    </div>
  );
}
