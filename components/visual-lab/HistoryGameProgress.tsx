"use client";

import { useCallback, useEffect, useState } from "react";
import {
  clampHistoryLevel,
  requiredCorrectForHistoryLevel,
  type HistoryGameId,
  type HistoryLevel,
} from "@/lib/visualLab/historyCurriculum";

type Lang = "de" | "hu" | "ro" | "en";

export interface StoredHistoryProgress {
  unlockedLevel: HistoryLevel;
  selectedLevel: HistoryLevel;
  correctByLevel: number[];
  attemptsByLevel: number[];
  stars: number;
}
const COPY: Record<Lang, { level: string; grade: string; next: string; locked: string }> = {
  de: { level: "Level", grade: "Klasse", next: "Nächstes Level", locked: "Noch gesperrt" },
  hu: { level: "Szint", grade: "Osztály", next: "Következő szint", locked: "Még zárva" },
  ro: { level: "Nivel", grade: "Clasa", next: "Nivelul următor", locked: "Încă blocat" },
  en: { level: "Level", grade: "Grade", next: "Next level", locked: "Still locked" },
};

function emptyProgress(): StoredHistoryProgress {
  return { unlockedLevel: 1, selectedLevel: 1, correctByLevel: [0, 0, 0, 0, 0], attemptsByLevel: [0, 0, 0, 0, 0], stars: 0 };
}

function normalizeCounts(value: unknown): number[] {
  return Array.from({ length: 5 }, (_, index) => {
    const item = Array.isArray(value) ? Number(value[index]) : 0;
    return Number.isFinite(item) ? Math.max(0, Math.floor(item)) : 0;
  });
}

function normalizeProgress(value: unknown): StoredHistoryProgress {
  if (!value || typeof value !== "object") return emptyProgress();
  const input = value as Partial<StoredHistoryProgress>;
  const correctByLevel = normalizeCounts(input.correctByLevel);
  let unlockedLevel = clampHistoryLevel(Number(input.unlockedLevel));
  while (unlockedLevel < 5 && correctByLevel[unlockedLevel - 1] >= requiredCorrectForHistoryLevel(unlockedLevel)) {
    unlockedLevel = (unlockedLevel + 1) as HistoryLevel;
  }
  return {
    unlockedLevel,
    selectedLevel: Math.min(unlockedLevel, clampHistoryLevel(Number(input.selectedLevel))) as HistoryLevel,
    correctByLevel,
    attemptsByLevel: normalizeCounts(input.attemptsByLevel),
    stars: Math.max(0, Math.floor(Number(input.stars) || 0)),
  };
}

export function useHistoryGameProgress(gameId: HistoryGameId, grade: number, lang: Lang) {
  const storageKey = `plizio:history-progress:v1:${lang}:g${grade}:${gameId}`;
  const [progress, setProgress] = useState<StoredHistoryProgress>(() => emptyProgress());

  useEffect(() => {
    try {
      const stored = localStorage.getItem(storageKey);
      setProgress(stored ? normalizeProgress(JSON.parse(stored)) : emptyProgress());
    } catch {
      setProgress(emptyProgress());
    }
  }, [storageKey]);

  const persist = useCallback((next: StoredHistoryProgress) => {
    try { localStorage.setItem(storageKey, JSON.stringify(next)); } catch {}
    return next;
  }, [storageKey]);

  const selectLevel = useCallback((levelInput: number) => {
    setProgress((current) => persist({
      ...current,
      selectedLevel: Math.min(current.unlockedLevel, clampHistoryLevel(levelInput)) as HistoryLevel,
    }));
  }, [persist]);

  const recordAnswer = useCallback((correct: boolean) => {
    setProgress((current) => {
      const index = current.selectedLevel - 1;
      const correctByLevel = [...current.correctByLevel];
      const attemptsByLevel = [...current.attemptsByLevel];
      attemptsByLevel[index] += 1;
      if (correct) correctByLevel[index] += 1;
      let unlockedLevel = current.unlockedLevel;
      if (correct && current.selectedLevel === unlockedLevel && unlockedLevel < 5
        && correctByLevel[index] >= requiredCorrectForHistoryLevel(current.selectedLevel)) {
        unlockedLevel = (unlockedLevel + 1) as HistoryLevel;
      }
      return persist({ ...current, unlockedLevel, correctByLevel, attemptsByLevel, stars: current.stars + (correct ? 1 : 0) });
    });
  }, [persist]);

  const advanceToUnlockedLevel = useCallback(() => {
    setProgress((current) => current.selectedLevel >= current.unlockedLevel
      ? current
      : persist({ ...current, selectedLevel: current.unlockedLevel }));
  }, [persist]);

  const levelIndex = progress.selectedLevel - 1;
  const required = requiredCorrectForHistoryLevel(progress.selectedLevel);
  return {
    progress,
    rounds: required,
    mastery: Math.min(1, progress.correctByLevel[levelIndex] / required),
    selectLevel,
    recordAnswer,
    advanceToUnlockedLevel,
  };
}

export function HistoryLevelBar({ grade, lang, progress, mastery, onSelect }: {
  grade: number;
  lang: Lang;
  progress: StoredHistoryProgress;
  mastery: number;
  onSelect: (level: HistoryLevel) => void;
}) {
  const t = COPY[lang] ?? COPY.en;
  return (
    <div className="mb-3 rounded-xl border border-amber-300/15 bg-black/25 p-2.5 text-white">
      <div className="mb-2 flex items-center justify-between gap-3 text-xs font-bold">
        <span>{t.grade} {grade}</span>
        <span>{t.level} {progress.selectedLevel}/5 · ★ {progress.stars}</span>
      </div>
      <div className="grid grid-cols-5 gap-1.5">
        {([1, 2, 3, 4, 5] as HistoryLevel[]).map((level) => {
          const unlocked = level <= progress.unlockedLevel;
          const selected = level === progress.selectedLevel;
          return (
            <button key={level} type="button" disabled={!unlocked} onClick={() => onSelect(level)}
              aria-label={unlocked ? `${t.level} ${level}` : `${t.level} ${level}: ${t.locked}`}
              aria-pressed={selected}
              className={`min-h-9 rounded-lg border text-sm font-black transition active:scale-95 ${selected ? "border-amber-300 bg-amber-500/30 text-amber-50" : unlocked ? "border-white/15 bg-white/5 text-white/75" : "border-white/5 bg-white/[.02] text-white/20"}`}>
              {unlocked ? level : "·"}
            </button>
          );
        })}
      </div>
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10" aria-label={`${t.next}: ${Math.round(mastery * 100)}%`}>
        <div className="h-full rounded-full bg-gradient-to-r from-amber-400 to-rose-400 transition-all" style={{ width: `${Math.round(mastery * 100)}%` }} />
      </div>
    </div>
  );
}
