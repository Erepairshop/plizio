...
import type { LanguageGameId, LanguageLevel } from "@/lib/visualLab/languageCurriculum";
import { LanguageLevelBar, useLanguageGameProgress } from "./LanguageGameProgress";
...
const COPY = {
  ...
  ro: { complete: "Nivel finalizat", practice: "Runda de exercițiu s-a închis", correct: "corecte", retry: "Din nou", next: "Nivelul următor" },
  en: { complete: "Level complete", practice: "Practice round complete", correct: "correct", retry: "Try again", next: "Next level" },
};

function isWinningResult(score: number, total: number | undefined, gameId: LanguageGameId): boolean {
...
function buildSequence<T>(rounds: T[], count: number, seed: string): T[] {
  ...
  return Array.from({ length: safeCount }, (_, index) => rounds[(offset + index) % rounds.length]);
}

export default function LanguageRoundSession<T extends { id: string }>({ gameId, grade, lang, rounds, renderRound }: {
  gameId: LanguageGameId;
  grade: number;
  lang: Lang;
  rounds: T[] | ((level: LanguageLevel, count: number) => T[]);
  renderRound: (round: T, onDone: (score: number, total?: number) => void, key: string) => ReactNode;
}) {
  const t = COPY[lang] ?? COPY.en;
  const { progress, rounds: targetRounds, mastery, selectLevel, recordAnswer, advanceToUnlockedLevel } = useLanguageGameProgress(gameId, grade, lang);
  ...
  const finishRound = useCallback((score: number, total?: number) => {
    if (isAdvancingRef.current || isFinishedRef.current) return;
    isAdvancingRef.current = true;

    const wasCorrect = isWinningResult(score, total, gameId);
  ...
  }, [recordAnswer, roundIndex, targetRounds, gameId]);
  ...
  return (
    <div>
      <LanguageLevelBar grade={grade} lang={lang} progress={progress} mastery={mastery} onSelect={selectLevel} />
      ...
      {finished ? (
        <div className="rounded-2xl border border-cyan-400/25 bg-slate-950/90 p-6 text-center text-white">
          ...
          <p className="mt-2 text-cyan-100/75">{correct}/{targetRounds} {t.correct}</p>
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            ...
            {unlockedNext && (
              <button type="button" onClick={() => { advanceToUnlockedLevel(); restart(); }} className="min-h-12 rounded-xl bg-cyan-400 px-4 font-black text-slate-950 active:scale-[.98]">{t.next}</button>
            )}
          </div>
        </div>
