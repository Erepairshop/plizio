...
import {
  clampChemistryLevel,
  requiredCorrectForChemistryLevel,
  type ChemistryGameId,
  type ChemistryLevel,
} from "@/lib/visualLab/chemistryCurriculum";

type Lang = "de" | "hu" | "ro" | "en";

export interface StoredChemistryProgress {
  unlockedLevel: ChemistryLevel;
  selectedLevel: ChemistryLevel;
...
function emptyProgress(): StoredChemistryProgress {
...
function normalizeProgress(value: unknown): StoredChemistryProgress {
  if (!value || typeof value !== "object") return emptyProgress();
  const input = value as Partial<StoredChemistryProgress>;
  const correctByLevel = normalizeCounts(input.correctByLevel);
  let unlockedLevel = clampChemistryLevel(Number(input.unlockedLevel));
  while (unlockedLevel < 5 && correctByLevel[unlockedLevel - 1] >= requiredCorrectForChemistryLevel(unlockedLevel)) {
    unlockedLevel = (unlockedLevel + 1) as ChemistryLevel;
  }
  return {
    unlockedLevel,
    selectedLevel: Math.min(unlockedLevel, clampChemistryLevel(Number(input.selectedLevel))) as ChemistryLevel,
    correctByLevel,
...
export function useChemistryGameProgress(gameId: ChemistryGameId, grade: number, lang: Lang) {
  const storageKey = `plizio:chemistry-progress:v1:${lang}:g${grade}:${gameId}`;
  const [progress, setProgress] = useState<StoredChemistryProgress>(() => emptyProgress());
  ...
  const persist = useCallback((next: StoredChemistryProgress) => {
  ...
  const selectLevel = useCallback((levelInput: number) => {
    setProgress((current) => persist({
      ...current,
      selectedLevel: Math.min(current.unlockedLevel, clampChemistryLevel(levelInput)) as ChemistryLevel,
    }));
  }, [persist]);
  ...
  const recordAnswer = useCallback((correct: boolean) => {
    setProgress((current) => {
      ...
      if (correct && current.selectedLevel === unlockedLevel && unlockedLevel < 5
        && correctByLevel[index] >= requiredCorrectForChemistryLevel(current.selectedLevel)) {
        unlockedLevel = (unlockedLevel + 1) as ChemistryLevel;
      }
  ...
  const required = requiredCorrectForChemistryLevel(progress.selectedLevel);
  return {
    progress,
    rounds: required,
...
export function ChemistryLevelBar({ grade, lang, progress, mastery, onSelect }: {
  grade: number;
  lang: Lang;
  progress: StoredChemistryProgress;
  mastery: number;
  onSelect: (level: ChemistryLevel) => void;
}) {
  const t = COPY[lang] ?? COPY.en;
  return (
    <div className="mb-3 rounded-xl border border-orange-300/15 bg-black/25 p-2.5 text-white">
      <div className="mb-2 flex items-center justify-between gap-3 text-xs font-bold">
        <span>{t.grade} {grade}</span>
        <span>{t.level} {progress.selectedLevel}/5 · ★ {progress.stars}</span>
      </div>
      <div className="grid grid-cols-5 gap-1.5">
        {([1, 2, 3, 4, 5] as ChemistryLevel[]).map((level) => {
          ...
          return (
            <button key={level} type="button" disabled={!unlocked} onClick={() => onSelect(level)}
              ...
              className={`min-h-9 rounded-lg border text-sm font-black transition active:scale-95 ${selected ? "border-orange-300 bg-orange-500/30 text-orange-50" : unlocked ? "border-white/15 bg-white/5 text-white/75" : "border-white/5 bg-white/[.02] text-white/20"}`}>
              {unlocked ? level : "·"}
            </button>
          );
        })}
      ...
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10" aria-label={`${t.next}: ${Math.round(mastery * 100)}%`}>
        <div className="h-full rounded-full bg-gradient-to-r from-orange-400 to-fuchsia-400 transition-all" style={{ width: `${Math.round(mastery * 100)}%` }} />
      </div>
    </div>
  );
}
