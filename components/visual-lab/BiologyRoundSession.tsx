...
import type { BiologyGameId, BiologyLevel } from "@/lib/visualLab/biologyCurriculum";
import { BiologyLevelBar, useBiologyGameProgress } from "./BiologyGameProgress";
...
export default function BiologyRoundSession<T extends { id: string }>({ gameId, grade, lang, rounds, renderRound }: {
  gameId: BiologyGameId;
  grade: number;
  lang: Lang;
  rounds: T[] | ((level: BiologyLevel, count: number) => T[]);
  renderRound: (round: T, onDone: (score: number, total?: number) => void, key: string) => ReactNode;
}) {
  const t = COPY[lang] ?? COPY.en;
  const { progress, rounds: targetRounds, mastery, selectLevel, recordAnswer, advanceToUnlockedLevel } = useBiologyGameProgress(gameId, grade, lang);
  ...
  return (
    <div>
      <BiologyLevelBar grade={grade} lang={lang} progress={progress} mastery={mastery} onSelect={selectLevel} />
      ...
      {finished ? (
        <div className="rounded-2xl border border-lime-400/25 bg-slate-950/90 p-6 text-center text-white">
          ...
          <p className="mt-2 text-lime-100/75">{correct}/{targetRounds} {t.correct}</p>
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            ...
            {unlockedNext && (
              <button type="button" onClick={() => { advanceToUnlockedLevel(); restart(); }} className="min-h-12 rounded-xl bg-lime-300 px-4 font-black text-slate-950 active:scale-[.98]">{t.next}</button>
            )}
          </div>
        </div>
