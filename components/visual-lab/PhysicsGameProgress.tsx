...
import {
  clampPhysicsLevel,
  requiredCorrectForPhysicsLevel,
  type PhysicsGameId,
  type PhysicsLevel,
...
export interface StoredPhysicsProgress {
  unlockedLevel: PhysicsLevel;
  selectedLevel: PhysicsLevel;
...
function emptyProgress(): StoredPhysicsProgress {
...
function normalizeProgress(value: unknown): StoredPhysicsProgress {
  if (!value || typeof value !== "object") return emptyProgress();
  const input = value as Partial<StoredPhysicsProgress>;
  const correctByLevel = normalizeCounts(input.correctByLevel);
  let unlockedLevel = clampPhysicsLevel(Number(input.unlockedLevel));
  while (unlockedLevel < 5 && correctByLevel[unlockedLevel - 1] >= requiredCorrectForPhysicsLevel(unlockedLevel)) {
    unlockedLevel = (unlockedLevel + 1) as PhysicsLevel;
  }
  return {
    unlockedLevel,
    selectedLevel: Math.min(unlockedLevel, clampPhysicsLevel(Number(input.selectedLevel))) as PhysicsLevel,
    correctByLevel,
...
export function usePhysicsGameProgress(gameId: PhysicsGameId, grade: number, lang: Lang) {
  const storageKey = `plizio:physics-progress:v1:${lang}:g${grade}:${gameId}`;
  const [progress, setProgress] = useState<StoredPhysicsProgress>(() => emptyProgress());
  ...
  const persist = useCallback((next: StoredPhysicsProgress) => {
  ...
  const selectLevel = useCallback((levelInput: number) => {
    setProgress((current) => persist({
      ...current,
      selectedLevel: Math.min(current.unlockedLevel, clampPhysicsLevel(levelInput)) as PhysicsLevel,
    }));
  }, [persist]);
  ...
  const recordAnswer = useCallback((correct: boolean) => {
    setProgress((current) => {
      ...
      if (correct && current.selectedLevel === unlockedLevel && unlockedLevel < 5
        && correctByLevel[index] >= requiredCorrectForPhysicsLevel(current.selectedLevel)) {
        unlockedLevel = (unlockedLevel + 1) as PhysicsLevel;
      }
  ...
  const required = requiredCorrectForPhysicsLevel(progress.selectedLevel);
  return {
    progress,
    rounds: required,
...
export function PhysicsLevelBar({ grade, lang, progress, mastery, onSelect }: {
  grade: number;
  lang: Lang;
  progress: StoredPhysicsProgress;
  mastery: number;
  onSelect: (level: PhysicsLevel) => void;
}) {
  ...
  return (
    <div className="mb-3 rounded-xl border border-cyan-300/15 bg-black/25 p-2.5 text-white">
      ...
      <div className="grid grid-cols-5 gap-1.5">
        {([1, 2, 3, 4, 5] as PhysicsLevel[]).map((level) => {
      ...
      <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10" aria-label={`${t.next}: ${Math.round(mastery * 100)}%`}>
        <div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-emerald-400 transition-all" style={{ width: `${Math.round(mastery * 100)}%` }} />
      </div>
    </div>
  );
}
