...
import { northAmericaOverview, northAmericaOverviewViewBox, NaCountry } from "@/lib/visualLab/maps/northamericaOverview";
...
const COUNTRY_BINDINGS: Record<string, string> = {
  US: '/usa-map', CA: '/canada-map', MX: '/mexico-map',
  GT: '/guatemala-map', HN: '/honduras-map', NI: '/nicaragua-map',
  CR: '/costarica-map', PA: '/panama-map', CU: '/cuba-map',
  DO: '/dominicanrepublic-map', HT: '/haiti-map', JM: '/jamaica-map',
...
export default function NorthAmericaMap({ lang }: NorthAmericaMapProps) {
  ...
  const [vbX, vbY, vbW, vbH] = northAmericaOverviewViewBox.split(" ").map(Number);
  ...
  const handleCountryClick = (country: NaCountry) => {
    ...
    <div className="relative w-full flex-1 min-h-[70vh] bg-slate-900 overflow-hidden shadow-2xl border border-white/10">
      ...
      >
        <g transform={`translate(${view.x} ${view.y}) scale(${view.scale})`}>
          {/* Country paths */}
          {northAmericaOverview.map((country) => {
