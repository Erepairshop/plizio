...
(function () {
  ...
  var PIN_SVG = '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0z"/><circle cx="12" cy="10" r="3"/></svg>';

  function mapsUrl(data, r, mode) {
    var fmt = function (x) { return x[1].toFixed(6) + "," + x[0].toFixed(6); }; // x=[lon,lat] -> "lat,lng"
    var pts = r.map(function (s) { return fmt(s.p.x); });
    var url = "https://www.google.com/maps/dir/?api=1&travelmode=" + (mode === "car" ? "driving" : "walking") +
      "&origin=" + encodeURIComponent(fmt(data.center)) +
      "&destination=" + encodeURIComponent(pts[pts.length - 1]);
    var wp = pts.slice(0, -1);
  ...
  function render(el, data, t, st) {
    ...
    var mb = el.querySelector(".plz-pl-maps");
    if (mb) mb.setAttribute("data-url", mapsUrl(data, r, st.mode));
  }
  ...
  function build(el) {
    ...
    el.innerHTML =
      ...
      '<div class="plz-pl-acts"><button class="plz-pl-act plz-pl-maps" data-url="">' + PIN_SVG + " " + t.maps + '</button><button class="plz-pl-act plz-pl-save">' + t.save + '</button><button class="plz-pl-act plz-pl-clr">' + t.clear + '</button></div><div class="plz-pl-note"></div></div>';
    ...
    el.querySelector(".plz-pl-maps").addEventListener("click", function () {
      var u = this.getAttribute("data-url"); if (u) window.open(u, "_blank", "noopener");
    });
