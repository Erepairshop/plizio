...
(function () {
  ...
  function route(data, budget, vibe) {
    ...
    while (guard++ < 40) {
      ...
      if (best === null) break;
      var p2 = pool[best], dur = (TYPE_META[p2.t] || { m: 30 }).m, walk = Math.round(bd * WALK_MIN_PER_KM);
      if (out.length && left - (dur + walk) < 0) break;
      used[best] = 1;
      out.push({ p: p2, walk: out.length ? walk : 0, dur: dur });
      left -= dur + (out.length > 1 ? walk : 0);
      cur = p2.x;
    }
    return out;
  }
  ...
  function render(el, data, t, st) {
    var r = route(data, st.budget, st.vibe);
  ...
  function build(el) {
    ...
    var st = { budget: 240, vibe: "culture" };
    el.innerHTML =
    ...
    el.querySelectorAll(".plz-pl-opts").forEach(function (g) {
      g.addEventListener("click", function (e) {
        ...
        if (g.getAttribute("data-g") === "budget") st.budget = parseInt(b.getAttribute("data-val"), 10);
    ...
    el.querySelector(".plz-pl-save").addEventListener("click", function () {
      try { localStorage.setItem("plizio-plan-" + (data.id || location.pathname), JSON.stringify({ budget: st.budget, vibe: st.vibe })); el.querySelector(".plz-pl-note").textContent = t.saved; } catch (e) {}
    });
