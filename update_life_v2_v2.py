...
# Define the new content for each POI (shortened for the script, but I'll use full in execution)
# I will use the same 'updates' dictionary as before.
updates = {
...
# We split by "  }," which is a common separator for POIs in this repo
blocks = re.split(r'\n  \},\n', content)

new_blocks = []
for i, block in enumerate(blocks):
    # Special handling for the last block which might end with ];
    ...
    if id_match:
        ...
        if poi_id in updates:
            ...
            # Insert before the last } of the block
            # If it's the last block, it might have ]; at the end
            if is_last:
                # Find the last } that belongs to the POI
