...
# Define the new content for the 12 POIs
new_content = {
...
for i in range(1, len(pois), 2):
    ...
    # Clean up duplicated fields and comma-splitting issues
    # Fix descriptionAdvanced
    desc_adv_matches = list(re.finditer(r'descriptionAdvanced:\s*\{[^}]+\},?', body))
    if desc_adv_matches:
        # Collect all RO content
        ro_content = ""
        for m in desc_adv_matches:
            ro_match = re.search(r'ro:\s*"([^"]+)"', m.group(0))
            if ro_match and len(ro_match.group(1)) > len(ro_content):
                ro_content = ro_match.group(1)
        
        # Determine EN content
        en_content = ""
        if poi_id in new_content:
            en_content = new_content[poi_id]["desc"]
        else:
            for m in desc_adv_matches:
                en_match = re.search(r'en:\s*"([^"]+)"', m.group(0))
                if en_match and len(en_match.group(1)) > len(en_content):
                    en_content = en_match.group(1)
        
        # Remove all existing descriptionAdvanced
        body = re.sub(r'descriptionAdvanced:\s*\{[^}]+\},?\s*', '', body)
        
        # Add back one clean descriptionAdvanced
        new_desc_adv = f'descriptionAdvanced: {{\n      de: "",\n      hu: "",\n      ro: "{ro_content}",\n      en: "{en_content}"\n    }},\n    '
        body = re.sub(r'(description:\s*\{[^}]+\},?\s*)', r'\1' + new_desc_adv, body, count=1)

    # Fix factsAdvanced
    facts_adv_matches = list(re.finditer(r'factsAdvanced:\s*\{[^}]+\},?', body))
    if facts_adv_matches:
        # Collect all RO content
        ro_facts = []
        for m in facts_adv_matches:
            ro_match = re.search(r'ro:\s*\[(.*?)\]', m.group(0), re.DOTALL)
            if ro_match:
                facts = re.findall(r'"([^"]+)"', ro_match.group(1))
                # Merge if they look like they were split by commas
                merged_facts = []
                for f in facts:
                    if merged_facts and (f.startswith('000') or f.startswith(' ') or f[0].islower()):
                         merged_facts[-1] += ", " + f
                    else:
                         merged_facts.append(f)
                if len(merged_facts) > len(ro_facts):
                    ro_facts = merged_facts
        
        # Determine EN content
        en_facts = []
        if poi_id in new_content:
            en_facts = new_content[poi_id]["facts"]
        else:
            for m in facts_adv_matches:
                en_match = re.search(r'en:\s*\[(.*?)\]', m.group(0), re.DOTALL)
                if en_match:
                    ...
                    for f in facts:
                        if merged_facts and (f.startswith('000') or f.startswith(' ') or f[0].islower()):
                             ...
                             merged_facts.append(f)
                    if len(merged_facts) > len(en_facts):
                        en_facts = merged_facts
        
        # Remove all existing factsAdvanced
        body = re.sub(r'factsAdvanced:\s*\{[^}]+\},?\s*', '', body)
        
        # Add back one clean factsAdvanced
        ro_facts_str = '",\n        "'.join(ro_facts)
        en_facts_str = '",\n        "'.join(en_facts)
        new_facts_adv = f'factsAdvanced: {{\n      de: [],\n      hu: [],\n      ro: [\n        "{ro_facts_str}"\n      ],\n      en: [\n        "{en_facts_str}"\n      ]\n    }},\n    '
        body = re.sub(r'(facts:\s*\{[^}]+\},?\s*)', r'\1' + new_facts_adv, body, count=1)
...
# Global comma-split fix for any remaining arrays (just in case)
def fix_split_arrays(match):
    array_content = match.group(1)
    # Match sequences like "1", "000" and replace with "1,000"
    fixed = re.sub(r'"([^"]+)",\s*"(\d{3})"', r'"\1,\2"', array_content)
    return f'[{fixed}]'

final_content = re.sub(r'\[(.*?)\]', fix_split_arrays, final_content, flags=re.DOTALL)
