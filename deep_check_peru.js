...
files.forEach(file => {
  ...
  poiBlocks.forEach(block => {
    ...
    if (block.includes('descriptionAdvanced: {') && !block.includes('en:')) {
       console.log(`NO EN FIELD in descriptionAdvanced: ${id} in ${file}`);
    }
    if (!block.includes('descriptionAdvanced:')) {
       console.log(`NO descriptionAdvanced AT ALL: ${id} in ${file}`);
    }
  });
});
