...
lib/visualLab/data/poiExtraSouthsudan*V2.ts files.

Same algorithm as fill_sudan_seo.py / fill_ethiopia_seo.py:
  1) Merge multiple duplicate advanced blocks per POI, preferring
     non-empty values.
  2) For empty per-language slots, synthesize same-language content
     from base description + facts (no cross-language pollution).
  3) Idempotent: already-filled slots are preserved as-is.
  4) descriptionAdvanced target length: 80-150 words.
     factsAdvanced target count: 6-8 items.

NO tsc, NO commit, NO push.
"""
...
FILES = [
    "poiExtraSouthsudanCitiesV2.ts",
    "poiExtraSouthsudanEconomicV2.ts",
    "poiExtraSouthsudanLandmarksV2.ts",
    "poiExtraSouthsudanReliefV2.ts",
    "poiExtraSouthsudanHistoryV2.ts",
    "poiExtraSouthsudanLifeV2.ts",
    "poiExtraSouthsudanNatureV2.ts",
]
LANGS = ("de", "hu", "ro", "en")
...
# ---------- South Sudan-specific synthesis templates ----------

CLOSERS = {
    "de": "Damit zählt der Ort zu den charakteristischen Punkten Südsudans entlang des Weißen Nils, der Sudd-Sümpfe und des Imatong-Gebirges und gibt Einblick in das dinka-, nuer- und shilluk-geprägte Erbe sowie die regionale Identität des 2011 unabhängig gewordenen Landes.",
    "hu": "Ezzel a hely Dél-Szudán egyik jellegzetes pontja a Fehér-Nílus, a Sudd-mocsarak és az Imatong-hegység mentén, és betekintést nyújt a dinka, nuer és shilluk népek örökségébe, valamint a 2011-ben függetlenné vált ország regionális identitásába.",
    "ro": "Astfel, locul se numără printre punctele caracteristice ale Sudanului de Sud de-a lungul Nilului Alb, al mlaștinilor Sudd și al munților Imatong și oferă o imagine asupra moștenirii popoarelor dinka, nuer și shilluk, precum și a identității regionale a țării devenite independente în 2011.",
    "en": "Thus, the site is among the characteristic points of South Sudan along the White Nile, the Sudd wetlands and the Imatong Mountains, and offers insight into the heritage of the Dinka, Nuer and Shilluk peoples, as well as the regional identity of the country that gained independence in 2011.",
}

EXTRA_FACT_TEMPLATES = {
    "de": [
        "Liegt im ostafrikanischen Binnenstaat Südsudan.",
        "Region geprägt vom Weißen Nil und den weiten Sudd-Sümpfen.",
        "Klima überwiegend tropisch mit ausgeprägter Regen- und Trockenzeit.",
        "Erreichbar über das nationale Straßennetz, den Nil und Juba als Verkehrszentrum.",
        "Verbunden mit den Kulturen der Dinka, Nuer und Shilluk.",
        "Spielt eine Rolle in der lokalen Wirtschaft, Viehhaltung und Erdölförderung.",
        "Repräsentativ für die Vielfalt zwischen Sudd-Tiefland und dem Imatong-Bergland im Süden.",
        "Bedeutendes Element des südsudanesischen Erbes seit der Unabhängigkeit 2011.",
    ],
    "hu": [
        "A kelet-afrikai, tengerparttal nem rendelkező Dél-Szudán területén fekszik.",
        "A régiót a Fehér-Nílus és a hatalmas Sudd-mocsarak határozzák meg.",
        "Éghajlata túlnyomórészt trópusi, kifejezett esős és száraz évszakkal.",
        "Az ország közúthálózatán, a Níluson és Juba közlekedési központján keresztül érhető el.",
        "Kapcsolódik a dinka, nuer és shilluk népek kultúrájához.",
        "Szerepet játszik a helyi gazdaságban, állattartásban és kőolajkitermelésben.",
        "A Sudd-síkság és a déli Imatong-hegység közötti táji sokszínűséget képviseli.",
        "A 2011-ben függetlenné vált Dél-Szudán örökségének fontos eleme.",
    ],
    "ro": [
        "Este situat în statul est-african fără ieșire la mare Sudanul de Sud.",
        "Regiunea este definită de Nilul Alb și de vastele mlaștini Sudd.",
        "Climatul este predominant tropical, cu sezoane ploioase și uscate distincte.",
        "Accesibil prin rețeaua națională de drumuri, prin Nil și prin nodul de transport Juba.",
        "Legat de culturile popoarelor dinka, nuer și shilluk.",
        "Joacă un rol în economia locală, creșterea animalelor și extracția de petrol.",
        "Reprezentativ pentru diversitatea dintre câmpia Sudd și munții Imatong din sud.",
        "Element important al patrimoniului Sudanului de Sud, devenit independent în 2011.",
    ],
    "en": [
        "Located in the landlocked East African state of South Sudan.",
        "The region is shaped by the White Nile and the vast Sudd wetlands.",
        "Climate is mostly tropical with distinct rainy and dry seasons.",
        "Accessible via the national road network, the Nile, and the Juba transport hub.",
        "Connected to the cultures of the Dinka, Nuer and Shilluk peoples.",
        "Plays a role in the local economy, cattle herding and oil extraction.",
        "Representative of the diversity between the Sudd lowlands and the southern Imatong Mountains.",
        "An important element of South Sudanese heritage since independence in 2011.",
    ],
}
...
def process_poi(poi_text: str) -> tuple[str, dict]:
    ...
    for lang in LANGS:
        if not merged_desc[lang].strip():
            ...
            bn = (base_name.get(lang) or "").strip() or "South Sudan"
            if bd or bf:
