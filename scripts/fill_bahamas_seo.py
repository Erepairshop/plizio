...
lib/visualLab/data/poiExtraBahamas*.ts files.

Adapted from fill_senegal_seo.py. Same-language synthesis only
...
FILES = [
    "poiExtraBahamasCitiesV2.ts",
    "poiExtraBahamasEconomicV2.ts",
    "poiExtraBahamasHistoryV2.ts",
    "poiExtraBahamasLandmarksV2.ts",
    "poiExtraBahamasLifeV2.ts",
    "poiExtraBahamasNatureV2.ts",
    "poiExtraBahamasReliefV2.ts",
    "poiExtraBahamasRefill.ts",
]
LANGS = ("de", "hu", "ro", "en")
...
# ---------- Bahamas-specific synthesis (per language) ----------

CLOSERS = {
    "de": "Damit zählt der Ort zu den charakteristischen Punkten der Bahamas und bietet einen Einblick in die regionale Identität der Inselgruppe.",
    "hu": "Ezzel a hely a Bahama-szigetek egyik jellegzetes pontja, és betekintést nyújt a szigetcsoport regionális identitásába.",
    "ro": "Astfel, locul se numără printre punctele caracteristice ale Bahamas și oferă o imagine asupra identității regionale a arhipelagului.",
    "en": "Thus, the site is among the characteristic points of the Bahamas and offers insight into the regional identity of the archipelago.",
}

EXTRA_FACT_TEMPLATES = {
    "de": [
        "Liegt im karibischen Inselstaat Bahamas.",
        "Region geprägt von tropisch-maritimem Klima.",
        "Erreichbar über das nationale Verkehrsnetz der Inseln.",
        "Spielt eine Rolle in der lokalen Wirtschaft und Kultur.",
        "Repräsentativ für die Vielfalt der bahamaischen Landschaft.",
        "Bedeutendes Element des bahamaischen Erbes.",
    ],
    "hu": [
        "A karibi térségben fekvő Bahama-szigetek területén található.",
        "A régiót trópusi-tengeri éghajlat jellemzi.",
        "A szigetek közlekedési hálózatán keresztül érhető el.",
        "Szerepet játszik a helyi gazdaságban és kultúrában.",
        "A bahamai táj sokszínűségét képviseli.",
        "A bahamai örökség fontos eleme.",
    ],
    "ro": [
        "Este situat în statul-arhipelag caraibian Bahamas.",
        "Regiunea este caracterizată de un climat tropical-maritim.",
        "Accesibil prin rețeaua națională de transport a insulelor.",
        "Joacă un rol în economia și cultura locală.",
        "Reprezentativ pentru diversitatea peisajului bahamian.",
        "Element important al patrimoniului bahamian.",
    ],
    "en": [
        "Located in the Caribbean island nation of the Bahamas.",
        "The region is marked by a tropical-maritime climate.",
        "Accessible via the national transport network of the islands.",
        "Plays a role in the local economy and culture.",
        "Representative of the diversity of the Bahamian landscape.",
        "An important element of Bahamian heritage.",
    ],
}
...
def process_poi(poi_text: str):
    ...
    for lang in LANGS:
        if not merged_desc[lang].strip():
            ...
            bn = (base_name.get(lang) or "").strip() or "Bahamas"
            if bd or bf:
