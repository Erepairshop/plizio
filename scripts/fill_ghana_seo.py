...
lib/visualLab/data/poiExtraGhana*.ts files.
...
FILES = [
    "poiExtraGhanaCitiesV2.ts",
    "poiExtraGhanaEconomicV2.ts",
    "poiExtraGhanaLandmarksV2.ts",
    "poiExtraGhanaReliefV2.ts",
    "poiExtraGhanaHistoryV2.ts",
    "poiExtraGhanaLifeV2.ts",
    "poiExtraGhanaNatureV2.ts",
]
LANGS = ("de", "hu", "ro", "en")
...
CLOSERS = {
    "de": "Damit zählt der Ort zu den charakteristischen Punkten Ghanas — eines Landes, das von der Atlantikküste, dem Volta-Stromsystem, dem Erbe des Ashanti-Königreichs sowie dem Kakao- und Goldreichtum geprägt ist — und bietet einen Einblick in die regionale Identität.",
    "hu": "Ezzel a hely Ghána egyik jellegzetes pontja — egy olyan országé, amelyet az atlanti partvidék, a Volta-folyó vízrendszere, az Ashanti királyság öröksége, valamint a kakaó- és aranygazdagság formált —, és betekintést nyújt a regionális identitásba.",
    "ro": "Astfel, locul se numără printre punctele caracteristice ale Ghanei — o țară modelată de coasta atlantică, de sistemul fluvial Volta, de moștenirea regatului Ashanti și de bogăția în cacao și aur — și oferă o imagine asupra identității regionale.",
    "en": "Thus, the site is among the characteristic points of Ghana — a country shaped by its Atlantic coastline, the Volta river system, the legacy of the Ashanti Kingdom, and its cocoa and gold wealth — and offers insight into the regional identity.",
}

EXTRA_FACT_TEMPLATES = {
    "de": [
        "Liegt im westafrikanischen Staat Ghana am Golf von Guinea.",
        "Die Region zählt zum tropischen Klimagürtel mit ausgeprägter Regen- und Trockenzeit.",
        "Erreichbar über das nationale Straßennetz, das von Accra ausgeht.",
        "Spielt eine Rolle in der ghanaischen Wirtschaft, die von Kakao-, Gold- und Erdölexport geprägt ist.",
        "Repräsentativ für die Vielfalt der Landschaft zwischen Atlantikküste, Voltabecken und Savanne.",
        "Bedeutendes Element des kulturellen Erbes, das vom Ashanti-Königreich und küstennahen Forts geformt ist.",
    ],
    "hu": [
        "A nyugat-afrikai Ghána területén fekszik, a Guineai-öböl partvidékén.",
        "A régiót trópusi éghajlat jellemzi, határozott esős és száraz évszakkal.",
        "Az Accrából kiinduló közúthálózaton keresztül érhető el.",
        "Szerepet játszik a kakaó-, arany- és kőolajexportra épülő ghánai gazdaságban.",
        "Az atlanti partvidék, a Volta-medence és a szavannaöv sokszínűségét képviseli.",
        "Az Ashanti királyság és a parti erődök formálta ghánai örökség fontos eleme.",
    ],
    "ro": [
        "Este situat în statul vest-african Ghana, pe coasta Golfului Guineei.",
        "Regiunea aparține zonei tropicale, cu sezoane ploioase și secetoase distincte.",
        "Accesibil prin rețeaua națională de drumuri care pornește din Accra.",
        "Joacă un rol în economia ghaneză, bazată pe exportul de cacao, aur și petrol.",
        "Reprezentativ pentru diversitatea peisajului dintre coasta atlantică, bazinul Volta și savană.",
        "Element important al patrimoniului ghanez, modelat de regatul Ashanti și de forturile de pe coastă.",
    ],
    "en": [
        "Located in the West African state of Ghana along the Gulf of Guinea coast.",
        "The region lies within the tropical belt, with marked rainy and dry seasons.",
        "Accessible via the national road network radiating from Accra.",
        "Plays a role in the Ghanaian economy, driven by cocoa, gold and oil exports.",
        "Representative of the diversity between Atlantic coast, Volta basin and savanna belt.",
        "An important element of Ghanaian heritage, shaped by the Ashanti Kingdom and coastal forts.",
    ],
}
...
def process_poi(poi_text: str) -> tuple[str, dict]:
    ...
    for lang in LANGS:
        if not merged_desc[lang].strip():
            ...
            bn = (base_name.get(lang) or "").strip() or "Ghana"
            if bd or bf:
