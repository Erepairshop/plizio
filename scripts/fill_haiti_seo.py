...
lib/visualLab/data/poiExtraHaiti*V2.ts files.

Mirrors fill_cuba_seo.py / fill_bahamas_seo.py behavior:
  1) Merge multiple duplicate `descriptionAdvanced` / `factsAdvanced`
     blocks per POI (later non-empty wins).
  2) For each language slot still empty, synthesize from same-language
     `description` + `facts` (no cross-language pollution).
  3) descriptionAdvanced target length: 80-150 words.
     factsAdvanced target count: 6-8 items.
  4) Idempotent: running again on a filled POI is a no-op (no empty
     slots remain to synthesize).

Haiti-specific closers / extra facts highlight: western half of
Hispaniola, Caribbean Sea, Port-au-Prince, Cap-Haïtien, mountainous
relief, the historical slave revolt and 1804 independence, French
colonial heritage, coffee / mango / sugarcane economy.
"""
...
FILES = [
    "poiExtraHaitiCitiesV2.ts",
    "poiExtraHaitiEconomicV2.ts",
    "poiExtraHaitiHistoryV2.ts",
    "poiExtraHaitiLandmarksV2.ts",
    "poiExtraHaitiLifeV2.ts",
    "poiExtraHaitiNatureV2.ts",
    "poiExtraHaitiReliefV2.ts",
]
LANGS = ("de", "hu", "ro", "en")
...
# ---------- Haiti-specific closers + extra facts ----------

CLOSERS = {
    "de": "Damit zählt der Ort zu den prägenden Punkten Haitis, des westlichen Teils der Karibikinsel Hispaniola, und spiegelt die Verbindung von französischem Kolonialerbe, Unabhängigkeitsgeschichte, gebirgiger Landschaft sowie Kaffee-, Mango- und Zuckerrohranbau wider.",
    "hu": "Ezzel a hely Haiti, a Hispaniola-sziget nyugati felének egyik jellegzetes pontja, amely a francia gyarmati örökség, a függetlenségi történelem, a hegyvidéki táj, valamint a kávé-, mangó- és cukornádtermesztés összefonódását tükrözi.",
    "ro": "Astfel, locul se numără printre punctele caracteristice ale statului Haiti, jumătatea vestică a insulei Hispaniola, reflectând împletirea dintre moștenirea colonială franceză, istoria independenței, relieful muntos și cultivarea cafelei, a fructelor de mango și a trestiei de zahăr.",
    "en": "Thus, the site is among the characteristic points of Haiti, the western half of Hispaniola, reflecting the interplay of French colonial heritage, the country's revolutionary history, mountainous terrain, and the cultivation of coffee, mangoes and sugarcane.",
}

EXTRA_FACT_TEMPLATES = {
    "de": [
        "Liegt im Westteil der Karibikinsel Hispaniola, dem Staat Haiti.",
        "Tropisches Klima mit Regen- und Trockenzeit, Hurrikangefahr von Juni bis November.",
        "Hauptstadt Port-au-Prince ist das politische, wirtschaftliche und kulturelle Zentrum.",
        "Cap-Haïtien im Norden ist die zweitgrößte Stadt und bedeutender Hafen.",
        "Über die Hälfte des Landes ist gebirgig, prägend ist das Massif de la Selle.",
        "Erstes unabhängiges Land Lateinamerikas (1804) nach erfolgreichem Sklavenaufstand.",
        "Französisches Kolonialerbe lebt in Sprache, Architektur und Kreolkultur weiter.",
        "Wichtige Kulturen sind Kaffee, Mango, Zuckerrohr, Kakao und Bohnen.",
    ],
    "hu": [
        "A Hispaniola-sziget nyugati felén, a Haiti Köztársaság területén fekszik.",
        "Trópusi éghajlat jellemzi, esős és száraz évszakkal, június és november között hurrikánveszéllyel.",
        "Az ország politikai, gazdasági és kulturális központja a főváros, Port-au-Prince.",
        "Az északi Cap-Haïtien az ország második legnagyobb városa és fontos kikötője.",
        "A terület több mint fele hegyvidékes, meghatározó a Massif de la Selle vonulata.",
        "Latin-Amerika első független állama (1804), a sikeres rabszolgafelkelés nyomán.",
        "A francia gyarmati örökség a nyelvben, az építészetben és a kreol kultúrában él tovább.",
        "Fő mezőgazdasági termékei a kávé, a mangó, a cukornád, a kakaó és a bab.",
    ],
    "ro": [
        "Este situat în jumătatea vestică a insulei Hispaniola, în Republica Haiti.",
        "Climat tropical, cu sezon ploios și sezon secetos, expus uraganelor între iunie și noiembrie.",
        "Capitala Port-au-Prince este centrul politic, economic și cultural al țării.",
        "Cap-Haïtien, în nord, este al doilea oraș ca mărime și un port important.",
        "Peste jumătate din teritoriu este muntos, dominat de masivul Massif de la Selle.",
        "Primul stat independent din America Latină (1804), în urma revoltei sclavilor.",
        "Moștenirea colonială franceză se regăsește în limbă, arhitectură și cultura creolă.",
        "Principalele culturi sunt cafeaua, mango, trestia de zahăr, cacaoa și fasolea.",
    ],
    "en": [
        "Located on the western half of the Caribbean island of Hispaniola, in the Republic of Haiti.",
        "Tropical climate with wet and dry seasons; the country lies in the Atlantic hurricane belt (June-November).",
        "Port-au-Prince, the capital, is Haiti's political, economic and cultural hub.",
        "Cap-Haïtien on the north coast is the country's second-largest city and a key port.",
        "More than half of Haiti's territory is mountainous, with the Massif de la Selle range standing out.",
        "The first independent state in Latin America (1804), born of a successful slave revolt.",
        "French colonial heritage survives in language, architecture and Haitian Creole culture.",
        "Key crops include coffee, mangoes, sugarcane, cocoa and beans.",
    ],
}
...
def process_poi(poi_text: str):
    ...
    for lang in LANGS:
        if not merged_desc[lang].strip():
            ...
            bn = (base_name.get(lang) or "").strip() or "Haiti"
            if bd or bf:
