...
lib/visualLab/data/poiExtraTanzania*V2.ts files.
...
FILES = [
    "poiExtraTanzaniaCitiesV2.ts",
    "poiExtraTanzaniaEconomicV2.ts",
    "poiExtraTanzaniaHistoryV2.ts",
    "poiExtraTanzaniaLandmarksV2.ts",
    "poiExtraTanzaniaLifeV2.ts",
    "poiExtraTanzaniaNatureV2.ts",
    "poiExtraTanzaniaReliefV2.ts",
]
LANGS = ("de", "hu", "ro", "en")
...
CLOSERS = {
    ...
    "en": "Thus, the site is among the characteristic points of Tanzania — an East African country shaped by Mount Kilimanjaro, the Serengeti, the Ngorongoro Crater, Lake Victoria and Lake Tanganyika, the Zanzibar archipelago along the Indian Ocean coast, and its wealth in coffee, cloves and gold mining — and offers insight into the regional identity.",
}

EXTRA_FACT_TEMPLATES = {
    "de": [
        "Liegt im ostafrikanischen Staat Tansania zwischen dem Indischen Ozean und der Region der Großen Seen.",
        "Die Region zählt zum tropischen Klimagürtel mit ausgeprägter Regen- und Trockenzeit.",
        "Erreichbar über das nationale Straßennetz, das von Dar es Salaam und Dodoma ausgeht.",
        "Spielt eine Rolle in der tansanischen Wirtschaft, die von Kaffee-, Gewürznelken-, Tee- und Goldexport geprägt ist.",
        "Repräsentativ für die landschaftliche Vielfalt zwischen Kilimandscharo, Serengeti, Ngorongoro-Krater und der Küste des Indischen Ozeans.",
        "Bedeutendes Element des kulturellen Erbes, das von Suaheli-Kultur, Sansibar und den ostafrikanischen Handelsrouten geformt ist.",
    ],
    "hu": [
        "A kelet-afrikai Tanzánia területén fekszik, az Indiai-óceán és a Nagy-tavak vidéke között.",
        "A régiót trópusi éghajlat jellemzi, határozott esős és száraz évszakkal.",
        "A Dar es Salaamból és Dodomából kiinduló közúthálózaton keresztül érhető el.",
        "Szerepet játszik a kávé-, szegfűszeg-, tea- és aranyexportra épülő tanzániai gazdaságban.",
        "A Kilimandzsáró, a Serengeti, a Ngorongoro-kráter és az Indiai-óceáni partvidék közötti tájak sokszínűségét képviseli.",
        "A szuahéli kultúra, Zanzibár és a kelet-afrikai kereskedelmi útvonalak formálta tanzániai örökség fontos eleme.",
    ],
    "ro": [
        ...
        "Accesibil prin rețeaua națională de drumuri care pornește din Dar es Salaam și Dodoma.",
        "Joacă un rol în economia tanzaniană, bazată pe exportul de cafea, cuișoare, ceai și aur.",
        "Reprezentativ pentru diversitatea peisajului dintre Kilimanjaro, Serengeti, craterul Ngorongoro și coasta Oceanului Indian.",
        "Element important al patrimoniului tanzanian, modelat de cultura swahili, de Zanzibar și de rutele comerciale est-africane.",
    ],
    "en": [
        ...
        "Representative of the diversity between Mount Kilimanjaro, the Serengeti, the Ngorongoro Crater and the Indian Ocean coast.",
        "An important element of Tanzanian heritage, shaped by Swahili culture, Zanzibar and the East African trade routes.",
    ],
}
...
def process_poi(poi_text: str) -> tuple[str, dict]:
    ...
    for lang in LANGS:
        if not merged_desc[lang].strip():
            ...
            bn = (base_name.get(lang) or "").strip() or "Tanzania"
            if bd or bf:
