...
lib/visualLab/data/poiExtraGuinea*V2.ts files (Guinea ONLY,
NOT Guineabissau / Equatorialguinea).
...
FILES = [
    "poiExtraGuineaCitiesV2.ts",
    "poiExtraGuineaEconomicV2.ts",
    "poiExtraGuineaLandmarksV2.ts",
    "poiExtraGuineaReliefV2.ts",
    "poiExtraGuineaHistoryV2.ts",
    "poiExtraGuineaLifeV2.ts",
    "poiExtraGuineaNatureV2.ts",
]
LANGS = ("de", "hu", "ro", "en")
...
CLOSERS = {
    "de": "Damit zählt der Ort zu den charakteristischen Punkten Guineas — eines Landes, das vom Fouta-Djallon-Hochland als Quellgebiet von Niger, Senegal und Gambia, von der Atlantikküste mit der Hauptstadt Conakry, vom tropischen Regenwald sowie vom Bauxit- und Eisenerzreichtum geprägt ist — und bietet einen Einblick in die regionale Identität.",
    "hu": "Ezzel a hely Guinea egyik jellegzetes pontja — egy olyan országé, amelyet a Fouta Djallon hegyvidék mint a Niger, a Szenegál és a Gambia folyók forrásvidéke, az atlanti partvidék Conakryval, a trópusi esőerdő, valamint a bauxit- és vasérckészletek formáltak —, és betekintést nyújt a regionális identitásba.",
    "ro": "Astfel, locul se numără printre punctele caracteristice ale Guineei — o țară modelată de podișul Fouta Djallon ca zonă de izvor a fluviilor Niger, Senegal și Gambia, de coasta atlantică cu capitala Conakry, de pădurea tropicală ecuatorială și de bogăția în bauxită și minereu de fier — și oferă o imagine asupra identității regionale.",
    "en": "Thus, the site is among the characteristic points of Guinea — a country shaped by the Fouta Djallon highlands as the source region of the Niger, Senegal and Gambia rivers, by its Atlantic coastline with the capital Conakry, by tropical rainforest, and by its bauxite and iron-ore wealth — and offers insight into the regional identity.",
}

EXTRA_FACT_TEMPLATES = {
    "de": [
        "Liegt im westafrikanischen Staat Guinea zwischen Atlantikküste und dem Fouta-Djallon-Hochland.",
        "Die Region zählt zum tropischen Klimagürtel mit ausgeprägter Regen- und Trockenzeit.",
        "Erreichbar über das nationale Straßennetz, das von Conakry ausgeht.",
        "Spielt eine Rolle in der guineischen Wirtschaft, die maßgeblich von Bauxit-, Eisenerz- und Goldförderung geprägt ist.",
        "Repräsentativ für die Vielfalt der Landschaft zwischen Küstenebene, Fouta Djallon, Oberguinea-Savanne und Waldguinea.",
        "Bedeutendes Element des Erbes Guineas, dem Quellgebiet von Niger, Senegal und Gambia.",
    ],
    "hu": [
        "A nyugat-afrikai Guinea területén fekszik, az atlanti partvidék és a Fouta Djallon hegyvidék között.",
        "A régiót trópusi éghajlat jellemzi, határozott esős és száraz évszakkal.",
        "A Conakryból kiinduló közúthálózaton keresztül érhető el.",
        "Szerepet játszik a guineai gazdaságban, amelyet a bauxit-, vasérc- és aranybányászat határoz meg.",
        "A part menti síkság, a Fouta Djallon, a felső-guineai szavanna és az erdős guineai övezet sokszínűségét képviseli.",
        "Guinea örökségének fontos eleme, amely egyben a Niger, a Szenegál és a Gambia folyók forrásvidéke.",
    ],
    "ro": [
        "Este situat în statul vest-african Guineea, între coasta atlantică și podișul Fouta Djallon.",
        "Regiunea aparține zonei tropicale, cu sezoane ploioase și secetoase distincte.",
        "Accesibil prin rețeaua națională de drumuri care pornește din Conakry.",
        "Joacă un rol în economia Guineei, bazată în mare parte pe extracția de bauxită, minereu de fier și aur.",
        "Reprezentativ pentru diversitatea peisajului dintre câmpia litorală, Fouta Djallon, savana Guineei Superioare și pădurea ecuatorială.",
        "Element important al patrimoniului Guineei, regiune de izvor a fluviilor Niger, Senegal și Gambia.",
    ],
    "en": [
        "Located in the West African state of Guinea, between the Atlantic coast and the Fouta Djallon highlands.",
        "The region lies within the tropical belt, with marked rainy and dry seasons.",
        "Accessible via the national road network radiating from Conakry.",
        "Plays a role in the Guinean economy, which is largely driven by bauxite, iron-ore and gold mining.",
        "Representative of the diversity between coastal plain, Fouta Djallon, Upper Guinea savanna and Forest Guinea.",
        "An important element of Guinea's heritage as the source region of the Niger, Senegal and Gambia rivers.",
    ],
}
...
def process_poi(poi_text: str) -> tuple[str, dict]:
    ...
    for lang in LANGS:
        if not merged_desc[lang].strip():
            ...
            bn = (base_name.get(lang) or "").strip() or "Guinea"
            if bd or bf:
