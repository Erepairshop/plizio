#!/usr/bin/env python3
"""Translate exact-key German chemistry-test batches through Azure OpenAI."""
...
INSTRUCTIONS = """Translate every supplied German school chemistry string into natural {language_name}.
The strings belong to grade 5-8 test questions, answer choices, short answers, topic names and explanations.
Translate faithfully and use standard school terminology. Do not add, remove or simplify scientific facts.
Preserve chemical formulae, element symbols, reaction equations, arrows, numbers, units, pH values,
mathematical notation and placeholders exactly. Proper names may use their established target-language form.
Return only one JSON object with this schema: {{"translations":{{"input_key":"translated text"}}}}.
Every input key must occur exactly once; no extra key is allowed. Do not add commentary or markdown.
Use correct UTF-8 spelling and punctuation for the target language."""
