...
function render(c: CountryCfg, lang: Lang, top: POI[]): string {
  ...
  const itemList = { "@context":"https://schema.org","@type":"ItemList","name":t.titleTpl(country),"numberOfItems":top.length,"itemListElement": top.map((p,i)=>({ "@type":"ListItem","position":i+1,"item":{ "@type":"TouristAttraction","name":localizedName(p,lang),...(poiHref(p,lang)?{"url":SITE+poiHref(p,lang)}:{}),...((()=>{const iu=resolveImg(p);return iu?{"image":SITE+iu}:{};})()) } })) };
