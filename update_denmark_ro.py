...
poi_data = {
    ...
    "city-copenhagen": {
        "descriptionAdvanced": {
            "ro": "Copenhaga, capitala vibrantă a Danemarcei, este o combinație armonioasă de farmec istoric și inovație modernă de ultimă oră. Orașul este celebru pentru portul său pitoresc Nyhavn, cu case multicolore, și pentru emblematica statuie Mica Sirenă, care veghează malul apei. Fiind un lider global în sustenabilitate, Copenhaga impresionează prin infrastructura sa extinsă pentru biciclete și prin curățenia portului său, unde se poate înota în siguranță. Vizitatorii pot explora palate regale impunătoare, precum Amalienborg, și se pot bucura de atmosfera magică a Grădinilor Tivoli, unul dintre cele mai vechi parcuri de distracții din lume. Scena gastronomică este, de asemenea, remarcabilă, găzduind numeroase restaurante cu stele Michelin care definesc noua bucătărie nordică."
        },
        "factsAdvanced": {
            "ro": [
    ...
    "city-odense": {
        ...
        "factsAdvanced": {
            "ro": [
                ...
                "Festivalul H.C. Andersen, desfășurat anual în luna august, transformă întregul oraș într-un cadru magic plin de spectacole și artă."
            ]
        }
    },
    "reg-jutland": {
        "descriptionAdvanced": {
            "ro": "Iutlanda este peninsula principală care formează partea continentală a Danemarcei, oferind o diversitate peisagistică uimitoare. Coasta de vest este marcată de plaje sălbatice, dune de nisip impunătoare și briza aspră a Mării Nordului, fiind un paradis pentru iubitorii de natură. În contrast, coasta de est este mai blândă, cu fiorduri liniștite, dealuri verzi și păduri dese care invită la explorare. Regiunea este bogată în istorie, adăpostind monumente vikinge de o valoare inestimabilă, cum ar fi Pietrele de la Jelling, parte a patrimoniului UNESCO. Iutlanda este, de asemenea, centrul agricol și industrial al țării, păstrând în același timp un puternic sentiment al tradiției și ospitalității locale."
        },
        "factsAdvanced": {
            "ro": [
    ...
    "land-statens-museum-for-kunst": {
        ...
        "factsAdvanced": {
            "ro": [
                ...
                "Clădirea veche și cea nouă sunt separate de o 'Stradă a Sculpturilor' acoperită cu sticlă, care servește drept spațiu expozițional și de tranzit."
            ]
        }
    },
    ...
    "hist-roskilde-cathedral": {
        ...
        "factsAdvanced": {
            "ro": [
                ...
                "De-a lungul secolelor, catedrala a suferit numeroase extinderi, rezultând un amestec fascinant de stiluri, de la romanic la gotic, baroc și neoclasic.",
...
# We use a non-greedy match for the content of the object
...
def update_poi(match):
    ...
    desc_adv_match = re.search(r'descriptionAdvanced:\s*\{([^}]*)\}', poi_text)
    if desc_adv_match:
        desc_content = desc_adv_match.group(1)
        if '"ro":' not in desc_content and "'ro':" not in desc_content and 'ro:' not in desc_content:
            # Insert ro content
            ro_text = data["descriptionAdvanced"]["ro"]
            new_desc_content = desc_content.rstrip()
            if new_desc_content and not new_desc_content.endswith(','):
                new_desc_content += ','
            new_desc_content += f'\n    ro: "{ro_text}"'
            poi_text = poi_text.replace(desc_content, new_desc_content)
        elif 'ro: ""' in desc_content or 'ro: \'\'' in desc_content:
             # Replace empty ro
             ro_text = data["descriptionAdvanced"]["ro"]
             poi_text = poi_text.replace('ro: ""', f'ro: "{ro_text}"')
             poi_text = poi_text.replace("ro: ''", f'ro: "{ro_text}"')
    else:
        # Need to add descriptionAdvanced object
        ro_text = data["descriptionAdvanced"]["ro"]
        new_obj = f',\n  descriptionAdvanced: {{\n    de: "",\n    hu: "",\n    ro: "{ro_text}",\n    en: ""\n  }}'
        # Insert before image or facts or at the end
        if 'image:' in poi_text:
            poi_text = re.sub(r'(\s*)image:', fr'\2{new_obj},\1image:', poi_text)
        elif 'facts:' in poi_text:
             poi_text = re.sub(r'(\s*)facts:', fr'\2{new_obj},\1facts:', poi_text)
        else:
            # Insert before the last closing brace
            poi_text = re.sub(r'(\s*)\}\s*$', fr'\1{new_obj}\1}}', poi_text)

    # Update factsAdvanced
    facts_adv_match = re.search(r'factsAdvanced:\s*\{([^}]*)\}', poi_text)
    if facts_adv_match:
        facts_content = facts_adv_match.group(1)
        if '"ro":' not in facts_content and "'ro':" not in facts_content and 'ro:' not in facts_content:
            ro_facts = json.dumps(data["factsAdvanced"]["ro"], ensure_ascii=False)
            new_facts_content = facts_content.rstrip()
            if new_facts_content and not new_facts_content.endswith(','):
                new_facts_content += ','
            new_facts_content += f'\n    ro: {ro_facts}'
            poi_text = poi_text.replace(facts_content, new_facts_content)
        elif 'ro: []' in facts_content:
            ro_facts = json.dumps(data["factsAdvanced"]["ro"], ensure_ascii=False)
            poi_text = poi_text.replace('ro: []', f'ro: {ro_facts}')
    else:
        # Need to add factsAdvanced object
        ro_facts = json.dumps(data["factsAdvanced"]["ro"], ensure_ascii=False)
        new_obj = f',\n  factsAdvanced: {{\n    de: [],\n    hu: [],\n    ro: {ro_facts},\n    en: []\n  }}'
        if 'image:' in poi_text:
            poi_text = re.sub(r'(\s*)image:', fr'\2{new_obj},\1image:', poi_text)
...
# Use regex to find and replace POI objects
# This is tricky because objects can contain other objects. 
# But in this file they are flat within the arrays.

# Find the start and end of each object in the arrays
# We'll split the file by variable exports and then process each part.

updated_content = content

# Process denmarkCountry separately as it's not in an array
country_match = re.search(r'export const denmarkCountry: POI = (\{.*?\n\};)', updated_content, re.DOTALL)
if country_match:
    original = country_match.group(1)
    updated = update_poi(re.search(r'\{.*\}', original, re.DOTALL))
    updated_content = updated_content.replace(original, updated + ';')

# For the arrays, we find each object { ... }
# We use a pattern that matches an object starting with { and ending with }, but not spanning across multiple POIs.
# Since each POI starts with id:, we can use that.

poi_pattern = r'\{[^{]*?id:\s*["\'][^"\']+?["\'].*?\n\s*\}'
updated_content = re.sub(poi_pattern, update_poi, updated_content, flags=re.DOTALL)

with open(file_path, "w", encoding="utf-8") as f:
    f.write(updated_content)

print("Successfully updated denmarkPoi.ts")
