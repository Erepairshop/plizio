...
import { K7_ISLAND_SVGS } from "../islands-k7";
...
import {
  K7_ISLANDS, K7_CHECKPOINT_MAP,
  loadK7Progress, saveK7Progress,
...
const K7_LABEL: Record<string, string> = {
  en: "Klasse 7 · Deutsch Adventure",
  hu: "7. osztály · Deutsch kaland",
  de: "Klasse 7 · Deutsch-Abenteuer",
  ro: "Clasa 7 · Aventură germană",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowD7" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowD7" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1d7" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#B44DFF" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#B44DFF" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2d7" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1d7)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2d7)" />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8}
        filter="url(#pathGlowD7)" strokeLinecap="round" />
          ...
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            {unlocked ? (
              <foreignObject x={island.svgX - 30} y={island.svgY - 30} width={60} height={60}>
                {K7_ISLAND_SVGS[island.id] ? React.createElement(K7_ISLAND_SVGS[island.id], { size: 60 }) : <span style={{ fontSize: 20 }}>{island.icon}</span>}
              </foreignObject>
            ) : (
              <>
...
export default function AstroDeutschK7Page() {
  const { lang } = useLang();
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{K7_LABEL[lang] ?? K7_LABEL.de}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #B44DFF, #00D4FF)" }}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK7: Record<string, Record<string, string>> = {
      en: { test1: "Konjunktiv I, Werden-Passive & Modal Passive", test2: "Figures of Speech, Causal & Conditional Clauses", test3: "Concessive, Final, Temporal Clauses & Infinitive Constructions" },
      hu: { test1: "Konjunktív I, werden-szenvedő & modális szenvedő", test2: "Stíluseszközök, okhatározói & feltételes mellékmondatok", test3: "Megengedő, célhatározói, időhatározói & infinitív szerkezetek" },
      de: { test1: "Konjunktiv I, Werden-Passiv & Passiv mit Modalverb", test2: "Stilmittel, Kausalsatz & Konditionalsatz", test3: "Konzessivsatz, Finalsatz, Temporalsatz & Infinitivkonstruktionen" },
      ro: { test1: "Conjunctiv I, pasiv cu werden & pasiv cu modal", test2: "Figuri de stil, propoziții cauzale & condiționale", test3: "Concesive, finale, temporale & construcții infinitivale" },
    };
    const topicDesc = (testTopicsK7[lang] ?? testTopicsK7.de)[activeTestId] ?? "";
    return (
