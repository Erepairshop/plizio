...
import {
  ...
  type DeutschProgress,
  loadK1Progress, saveK1Progress,
  isMissionDoneK1, isIslandDoneK1, isIslandUnlockedK1,
  isCheckpointUnlockedK1, isCheckpointDoneK1,
  completeMissionK1, completeTestK1, islandTotalStarsK1,
  generateIslandQuestionsK1, generateCheckpointQuestionsK1,
} from "@/lib/astroDeutsch";
...
const K1_LABEL: Record<string, string> = {
  en: "Klasse 1 · Deutsch Adventure",
  hu: "1. osztály · Deutsch kaland",
  de: "Klasse 1 · Deutsch-Abenteuer",
  ro: "Clasa 1 · Aventură germană",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowD1" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowD1" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1d1" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF2D78" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#FF2D78" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2d1" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1d1)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2d1)" />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8}
        filter="url(#pathGlowD1)" strokeLinecap="round" />
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedK1(progress, testId);
      ...
      {K1_ISLANDS.map((island, idx) => {
        ...
        const done = isIslandDoneK1(progress, island.id);
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneK1(progress, island.id, m.id);
                  return (
...
export default function AstroDeutschK1Page() {
  const { lang } = useLang();
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = K1_ISLANDS.find(i => i.id === id);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsK1(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsK1(activeTestId, lang as Lang, 10);
    setQuestions(qs);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{K1_LABEL[lang] ?? K1_LABEL.de}</p>
          </div>
      ...
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneK1(progress, activeIsland.id, mission.id);
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK1: Record<string, Record<string, string>> = {
      en: { test1: "Letters, Syllables, Words", test2: "Sentences, Rhymes, Articles", test3: "Capitalization, Vocabulary, Colors & Numbers" },
      hu: { test1: "Betűk, szótagok, szavak", test2: "Mondatok, rímek, névelők", test3: "Nagybetűírás, szókincs, színek & számok" },
      de: { test1: "Buchstaben, Silben, Wörter", test2: "Sätze, Reime, Artikel", test3: "Großschreibung, Wortschatz, Farben & Zahlen" },
      ro: { test1: "Litere, silabe, cuvinte", test2: "Propoziții, rime, articole", test3: "Majuscule, vocabular, culori & cifre" },
    };
    const topicDesc = (testTopicsK1[lang] ?? testTopicsK1.de)[activeTestId] ?? "";
    return (
