// SVG Island illustrations for AstroDeutsch K2 map
...
// i1 — NomenArtikel: Blue floating island with label tags and der/die/das badges
export function K2IslandNomenArtikel({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      ...
      {/* Main label tag — der */}
      <rect x="26" y="28" width="14" height="9" rx="2" fill="#DBEAFE" stroke="#3B82F6" strokeWidth="1.2" />
      <circle cx="26" cy="32.5" r="1.5" fill="#93C5FD" />
      <line x1="22" y1="32.5" x2="24.5" y2="32.5" stroke="#93C5FD" strokeWidth="1" />
      <text x="33" y="35" textAnchor="middle" fontSize="6" fontWeight="bold" fill="#1D4ED8" fontFamily="monospace">der</text>
      {/* Label tag — die */}
      <rect x="43" y="24" width="14" height="9" rx="2" fill="#EFF6FF" stroke="#60A5FA" strokeWidth="1.2" />
      <circle cx="43" cy="28.5" r="1.5" fill="#93C5FD" />
      <line x1="39" y1="28.5" x2="41.5" y2="28.5" stroke="#93C5FD" strokeWidth="1" />
      <text x="50" y="31" textAnchor="middle" fontSize="6" fontWeight="bold" fill="#1D4ED8" fontFamily="monospace">die</text>
      {/* Label tag — das (small, floating) */}
      <rect x="28" y="18" width="13" height="8" rx="2" fill="#BFDBFE" stroke="#60A5FA" strokeWidth="1" opacity="0.85" />
      <text x="34.5" y="24" textAnchor="middle" fontSize="5.5" fontWeight="bold" fill="#1E40AF" fontFamily="monospace">das</text>
      {/* String line from das tag down */}
      <line x1="34.5" y1="26" x2="34.5" y2="29" stroke="#93C5FD" strokeWidth="0.8" strokeDasharray="1 1" />
      {/* Floating sparkles */}
...
// i2 — Verben: Red dynamic island with running figure silhouette and action arrows
export function K2IslandVerben({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      {/* Glow beneath */}
...
// i3 — Adjektive: Emerald island with color palette, paint brush and colorful splashes
export function K2IslandAdjektive({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      {/* Glow beneath */}
...
// i4 — Satzarten: Amber island with three floating punctuation marks (? ! .)
export function K2IslandSatzarten({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      {/* Glow beneath */}
      ...
      {/* Period / full stop — small floating bubble */}
...
// i5 — Grossschreibung: Indigo island with big letter A wearing a crown
export function K2IslandGrossschreibung({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      {/* Glow beneath */}
      ...
      {/* Small lowercase a beside with strike-through idea */}
      ...
      {/* Arrow pointing up from lowercase to uppercase */}
...
// i6 — Rechtschreibung1: Pink island with quill pen, ink drops and tz/ck letter pairs
export function K2IslandRechtschreibung1({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      {/* Glow beneath */}
      ...
      <text x="56" y="37" textAnchor="middle" fontSize="7.5" fontWeight="bold" fill="#BE185D" fontFamily="monospace">tz</text>
      {/* Letter badge: ck */}
      <rect x="49" y="42" width="14" height="9" rx="2.5" fill="#FCE7F3" stroke="#F9A8D4" strokeWidth="1" />
      <text x="56" y="49" textAnchor="middle" fontSize="7.5" fontWeight="bold" fill="#9D174D" fontFamily="monospace">ck</text>
      {/* Sparkles */}
...
// i7 — Rechtschreibung2: Teal island with letter blocks, double consonants, magnifying glass
export function K2IslandRechtschreibung2({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      {/* Glow beneath */}
      ...
      <text x="26" y="36" textAnchor="middle" fontSize="9" fontWeight="bold" fill="#0F766E" fontFamily="monospace">ll</text>
      {/* Letter block — "mm" */}
      <rect x="34" y="22" width="12" height="12" rx="2" fill="#99F6E4" stroke="#2DD4BF" strokeWidth="1.2" />
      <text x="40" y="32" textAnchor="middle" fontSize="8" fontWeight="bold" fill="#0F766E" fontFamily="monospace">mm</text>
      {/* Letter block — "ss" */}
      <rect x="48" y="26" width="12" height="12" rx="2" fill="#CCFBF1" stroke="#14B8A6" strokeWidth="1.2" />
      <text x="54" y="36" textAnchor="middle" fontSize="9" fontWeight="bold" fill="#0F766E" fontFamily="monospace">ss</text>
      {/* Magnifying glass */}
      ...
      {/* Focus highlight in magnifying glass */}
      <text x="30" y="41" textAnchor="middle" fontSize="5" fontWeight="bold" fill="#0F766E">ff</text>
      {/* Sparkles */}
...
// i8 — Wortfelder: Purple globe island with connected word web/network
export function K2IslandWortfelder({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      {/* Glow beneath */}
      ...
      <text x="20" y="22" textAnchor="middle" fontSize="5" fontWeight="bold" fill="#EDE9FE" fontFamily="monospace">Tier</text>
      <line x1="27" y1="22" x2="29" y2="24" stroke="#C084FC" strokeWidth="0.9" />
      {/* Node: Haus */}
      <circle cx="60" cy="20" r="7" fill="#7C3AED" stroke="#C084FC" strokeWidth="1" />
      <text x="60" y="22" textAnchor="middle" fontSize="5" fontWeight="bold" fill="#EDE9FE" fontFamily="monospace">Haus</text>
      <line x1="53" y1="22" x2="51" y2="24" stroke="#C084FC" strokeWidth="0.9" />
      {/* Node: Baum */}
      <circle cx="22" cy="37" r="7" fill="#6D28D9" stroke="#C084FC" strokeWidth="1" />
      <text x="22" y="39" textAnchor="middle" fontSize="5" fontWeight="bold" fill="#EDE9FE" fontFamily="monospace">Baum</text>
      <line x1="29" y1="36" x2="29" y2="33" stroke="#C084FC" strokeWidth="0.9" />
      {/* Node: See */}
      <circle cx="58" cy="37" r="7" fill="#6D28D9" stroke="#C084FC" strokeWidth="1" />
      <text x="58" y="39" textAnchor="middle" fontSize="5" fontWeight="bold" fill="#EDE9FE" fontFamily="monospace">See</text>
...
// i9 — GrossePruefung: Teal final exam island with trophy, crown and star burst (K2 variant)
export function K2IslandGrossePruefung({ size = 80 }: IP) {
  return (
    <svg width={size} height={size} viewBox="0 0 80 80" fill="none">
      {/* Glow beneath */}
      <ellipse cx="40" cy="74" rx="21" ry="5" fill="rgba(78,205,196,0.25)" />
      {/* Rocky underside stalactites — slightly different shape from K1 */}
      ...
      {/* Star burst rays — static lines replacing K1 .map() */}
      ...
      {/* K2 badge on base */}
      <text x="40" y="42" textAnchor="middle" fontSize="5" fontWeight="bold" fill="#99F6E4" fontFamily="monospace">K2</text>
      {/* Sparkle above */}
...
export const K2_ISLAND_SVGS: Record<string, React.FC<{ size?: number }>> = {
  i1: K2IslandNomenArtikel,
  i2: K2IslandVerben,
  i3: K2IslandAdjektive,
  i4: K2IslandSatzarten,
  i5: K2IslandGrossschreibung,
  i6: K2IslandRechtschreibung1,
  i7: K2IslandRechtschreibung2,
  i8: K2IslandWortfelder,
  i9: K2IslandGrossePruefung,
};
