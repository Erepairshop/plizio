...
import { K6_ISLAND_SVGS } from "../islands-k6";
...
const K6_LABEL: Record<string, string> = {
  en: "Klasse 6 · Deutsch Adventure",
  hu: "6. osztály · Deutsch kaland",
  de: "Klasse 6 · Deutsch-Abenteuer",
  ro: "Clasa 6 · Aventură germană",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowD6" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowD6" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1d6" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF9500" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#FF9500" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2d6" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1d6)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2d6)" />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8}
        filter="url(#pathGlowD6)" strokeLinecap="round" />
          ...
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            {unlocked ? (
              <foreignObject x={island.svgX - 30} y={island.svgY - 30} width={60} height={60}>
                {K6_ISLAND_SVGS[island.id] ? React.createElement(K6_ISLAND_SVGS[island.id], { size: 60 }) : <span style={{ fontSize: 20 }}>{island.icon}</span>}
              </foreignObject>
            ) : (
              <>
...
export default function AstroDeutschK6Page() {
  const { lang } = useLang();
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{K6_LABEL[lang] ?? K6_LABEL.de}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #FF9500, #00D4FF)" }}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK6: Record<string, Record<string, string>> = {
      en: { test1: "Active & Passive, Konjunktiv II & Infinitive", test2: "Synonyms, Antonyms, Foreign Words & Clauses", test3: "Word Formation, Punctuation & Causal Clauses" },
      hu: { test1: "Aktív & szenvedő, Konjunktív II & infinitív", test2: "Szinonimák, antonimák, idegen szavak & mellékmondatok", test3: "Szóalkotás, írásjelek & okhatározói mondat" },
      de: { test1: "Aktiv & Passiv, Konjunktiv II & Infinitiv", test2: "Synonyme, Antonyme, Fremdwörter & Nebensätze", test3: "Wortbildung, Zeichensetzung & Kausalsatz" },
      ro: { test1: "Activ & pasiv, conjunctiv II & infinitiv", test2: "Sinonime, antonime, cuvinte străine & subordonate", test3: "Formare de cuvinte, punctuație & subordonate cauzale" },
    };
    const topicDesc = (testTopicsK6[lang] ?? testTopicsK6.de)[activeTestId] ?? "";
    return (
