...
import { K2_ISLAND_SVGS } from "@/app/astrodeutsch/islands-k2";
...
const K2_LABEL: Record<string, string> = {
  en: "Klasse 2 · Deutsch Adventure",
  hu: "2. osztály · Deutsch kaland",
  de: "Klasse 2 · Deutsch-Abenteuer",
  ro: "Clasa 2 · Aventură germană",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowD2" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowD2" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1d2" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#3B82F6" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#3B82F6" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2d2" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1d2)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2d2)" />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8}
        filter="url(#pathGlowD2)" strokeLinecap="round" />
...
export default function AstroDeutschK2Page() {
  const { lang } = useLang();
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsK2(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsK2(activeTestId, lang as Lang, 10);
    setQuestions(qs);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{K2_LABEL[lang] ?? K2_LABEL.de}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #3B82F6, #00D4FF)" }}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK2: Record<string, Record<string, string>> = {
      en: { test1: "Nouns, Articles & Verbs", test2: "Adjectives, Pronouns & Sentence Types", test3: "Capitalization, Spelling & Vocabulary" },
      hu: { test1: "Főnév, névelő & igék", test2: "Melléknevek, névmások & mondattípusok", test3: "Nagybetűírás, helyesírás & szókincs" },
      de: { test1: "Nomen, Artikel & Verben", test2: "Adjektive, Pronomen & Satzarten", test3: "Großschreibung, Rechtschreibung & Wortschatz" },
      ro: { test1: "Substantive, articole & verbe", test2: "Adjective, pronume & tipuri de propoziții", test3: "Majuscule, ortografie & vocabular" },
    };
    const topicDesc = (testTopicsK2[lang] ?? testTopicsK2.de)[activeTestId] ?? "";
    return (
