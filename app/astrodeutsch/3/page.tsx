...
import {
  K3_ISLANDS, K3_CHECKPOINT_MAP,
  loadK3Progress, saveK3Progress,
...
const K3_LABEL: Record<string, string> = {
  en: "Klasse 3 · Deutsch Adventure",
  hu: "3. osztály · Deutsch kaland",
  de: "Klasse 3 · Deutsch-Abenteuer",
  ro: "Clasa 3 · Aventură germană",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowD3" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowD3" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1d3" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#10B981" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#10B981" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2d3" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1d3)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2d3)" />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8}
        filter="url(#pathGlowD3)" strokeLinecap="round" />
...
export default function AstroDeutschK3Page() {
  const { lang } = useLang();
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{K3_LABEL[lang] ?? K3_LABEL.de}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #10B981, #00D4FF)" }}
  ...
  if (["orbit-quiz", "black-hole", "star-match", "speed-round", "deutsch-visual", "deutsch-explore", "sentence-scramble", "m2", "m3"].includes(screen)) return (
    <>
      {gameScreen}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK3: Record<string, Record<string, string>> = {
      en: { test1: "Plural, Word Families & Verb Conjugations", test2: "Adjectives, Sentence Parts & Objects", test3: "Past Tense, Direct Speech & Spelling" },
      hu: { test1: "Többes szám, szócsaládok & igeragozás", test2: "Melléknevek, mondatrészek & tárgy", test3: "Múlt idő, egyenes idézet & helyesírás" },
      de: { test1: "Plural, Wortfamilien & Verbkonjugation", test2: "Adjektive, Satzglieder & Objekte", test3: "Vergangenheit, direkte Rede & Rechtschreibung" },
      ro: { test1: "Plural, familii de cuvinte & conjugarea verbelor", test2: "Adjective, părti de propoziție & complemente", test3: "Trecut, vorbire directă & ortografie" },
    };
    const topicDesc = (testTopicsK3[lang] ?? testTopicsK3.de)[activeTestId] ?? "";
    return (
