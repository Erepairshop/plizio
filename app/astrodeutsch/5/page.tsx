...
} from "@/lib/astroDeutsch5";

const AvatarCompanion = dynamic(() => import("@/components/AvatarCompanion"), { ssr: false });

// K5 Explorer imports
const AdjectiveComparisonExplorerK5 = dynamic(() => import("./../../astrodeutsch/games/k5/AdjectiveComparisonExplorerK5"), { ssr: false });
const PronounExplorerK5 = dynamic(() => import("./../../astrodeutsch/games/k5/PronounExplorerK5"), { ssr: false });
const PrepositionExplorerK5 = dynamic(() => import("./../../astrodeutsch/games/k5/PrepositionExplorerK5"), { ssr: false });
const SentenceAnalysisK5Explorer = dynamic(() => import("./../../astrodeutsch/games/k5/SentenceAnalysisK5Explorer"), { ssr: false });
const PunctuationK5Explorer = dynamic(() => import("./../../astrodeutsch/games/k5/PunctuationK5Explorer"), { ssr: false });
const ParticipleK5Explorer = dynamic(() => import("./../../astrodeutsch/games/k5/ParticipleK5Explorer"), { ssr: false });
const PassiveK5Explorer = dynamic(() => import("./../../astrodeutsch/games/k5/PassiveK5Explorer"), { ssr: false });
const SpellingVocabK5Explorer = dynamic(() => import("./../../astrodeutsch/games/k5/SpellingVocabK5Explorer"), { ssr: false });
const ReviewK5Explorer = dynamic(() => import("./../../astrodeutsch/games/k5/ReviewK5Explorer"), { ssr: false });
...
const K5_LABEL: Record<string, string> = {
  en: "Klasse 5 · Deutsch Adventure",
  hu: "5. osztály · Deutsch kaland",
  de: "Klasse 5 · Deutsch-Abenteuer",
  ro: "Clasa 5 · Aventură germană",
};
...
type Screen =
  ...
  | "spell-race"
  | "k5-adjective-comparison-explorer"
  | "k5-pronoun-explorer"
  | "k5-preposition-explorer"
  | "k5-sentence-analysis-explorer"
  | "k5-punctuation-explorer"
  | "k5-participle-explorer"
  | "k5-passive-explorer"
  | "k5-spelling-vocab-explorer"
  | "k5-review-explorer"
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowD5" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowD5" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1d5" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#6366F1" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#6366F1" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2d5" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1d5)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2d5)" />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8}
        filter="url(#pathGlowD5)" strokeLinecap="round" />
...
export default function AstroDeutschK5Page() {
  const { lang } = useLang();
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{K5_LABEL[lang] ?? K5_LABEL.de}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #6366F1, #00D4FF)" }}
  ...
  const gameScreen = (
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "k5-adjective-comparison-explorer" && (
          <AdjectiveComparisonExplorerK5 color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "k5-pronoun-explorer" && (
          <PronounExplorerK5 color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "k5-preposition-explorer" && (
          <PrepositionExplorerK5 color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "k5-sentence-analysis-explorer" && (
          <SentenceAnalysisK5Explorer color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "k5-punctuation-explorer" && (
          <PunctuationK5Explorer color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "k5-participle-explorer" && (
          <ParticipleK5Explorer color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "k5-passive-explorer" && (
          <PassiveK5Explorer color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "k5-spelling-vocab-explorer" && (
          <SpellingVocabK5Explorer color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "k5-review-explorer" && (
          <ReviewK5Explorer color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
      </div>
    </div>
  );
  ...
  if ([
    "orbit-quiz", "black-hole", "star-match", "speed-round", "spell-race",
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK5: Record<string, Record<string, string>> = {
      en: { test1: "Adjectives, Pronouns & Prepositions", test2: "Sentence Parts, Punctuation & Passive Voice", test3: "Participle, Past Perfect, Spelling & Vocabulary" },
      hu: { test1: "Melléknevek, névmások & elöljárók", test2: "Mondatrészek, írásjelek & szenvedő szerkezet", test3: "Igenév, régmúlt, helyesírás & szókincs" },
      de: { test1: "Adjektive, Pronomen & Präpositionen", test2: "Satzglieder, Zeichensetzung & Passiv", test3: "Partizip, Plusquamperfekt, Rechtschreibung & Wortschatz" },
      ro: { test1: "Adjective, pronume & prepoziții", test2: "Părti de propoziție, punctuație & diateza pasivă", test3: "Participiu, mai-mult-ca-perfect, ortografie & vocabular" },
    };
    const topicDesc = (testTopicsK5[lang] ?? testTopicsK5.de)[activeTestId] ?? "";
    return (
