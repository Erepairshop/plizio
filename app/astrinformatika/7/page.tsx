...
import {
  INFO_K7_ISLANDS as K7_ISLANDS, INFO_K7_CHECKPOINT_MAP as K7_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadInfoK7Progress as loadK7Progress, saveInfoK7Progress as saveK7Progress,
  type InformatikaProgress as K7Progress,
  isMissionDoneInfoK7 as isMissionDoneK7, isIslandDoneInfoK7 as isIslandDoneK7,
  isIslandUnlockedInfoK7 as isIslandUnlockedK7,
  isCheckpointUnlockedInfoK7 as isCheckpointUnlockedK7, isCheckpointDoneInfoK7 as isCheckpointDoneK7,
  completeMissionInfoK7 as completeMissionK7, completeTestInfoK7 as completeTestK7,
  islandTotalStarsInfoK7 as islandTotalStarsK7,
  generateIslandQuestionsInfoK7 as generateIslandQuestionsK7, generateCheckpointQuestionsInfoK7 as generateCheckpointQuestionsK7,
...
const K7_LABEL: Record<string, string> = {
  en: "Grade 7 · Informatika",
  hu: "7. osztály · Informatika",
  de: "Klasse 7 · Informatik",
  ro: "Clasa 7 · Informatică",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint, lang }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }} aria-label={mapText.map}>
      <defs>
        <filter id="pathGlowInfo7" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowInfo7" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1info7" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#22C55E" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#22C55E" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2info7" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1info7)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2info7)" />

      <path d={pathD} fill="none" stroke="rgba(100,200,100,0.25)" strokeWidth={8}
        filter="url(#pathGlowInfo7)" strokeLinecap="round" />
...
export default function AstroInformatikaK7Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsK7(activeIsland, qCount, countryCode);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const countryCode = { de: "DE", hu: "HU", ro: "RO", en: "US" }[lang as Lang] ?? "US";
    const qs = generateCheckpointQuestionsK7(testId, 7, countryCode);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const countryCode = { de: "DE", hu: "HU", ro: "RO", en: "US" }[lang as Lang] ?? "US";
    const qs = generateCheckpointQuestionsK7(activeTestId, 15, countryCode);
    setQuestions(qs);
    ...
    return (
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          <button onClick={() => router.push("/astrinformatika")}
      ...
      <VisualLab subject="informatika" grade={7} lang={lang} open={visualLabOpen} onClose={() => setVisualLabOpen(false)} />
      </>
    );
  }
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "info-explore" && activeIsland && (
          <K7Explorer island={activeIsland} grade={7} onDone={handleMissionDone} color={bgColor} lang={lang} />
        )}
        {screen === "m2" && activeMission?.gameKey && INFORMATIKA_M2_POOLS[activeMission.gameKey] && (
          <M2Engine gameKey={activeMission.gameKey} rounds={INFORMATIKA_M2_POOLS[activeMission.gameKey]} color={bgColor} lang={lang as any} onDone={handleMissionDone} onCorrect={() => { setAvatarMood("happy"); setJumpTrigger({ reaction: "happy", timestamp: Date.now() }); }} onWrong={() => setAvatarMood("disappointed")} />
        )}
        {screen === "m3" && activeMission?.gameKey && INFORMATIKA_M3_POOLS[activeMission.gameKey] && (
          <M3Engine gameKey={activeMission.gameKey} rounds={INFORMATIKA_M3_POOLS[activeMission.gameKey]} color={bgColor} lang={lang as any} onDone={handleMissionDone} onCorrect={() => { setAvatarMood("happy"); setJumpTrigger({ reaction: "happy", timestamp: Date.now() }); }} onWrong={() => setAvatarMood("disappointed")} />
        )}
  ...
  if (screen === "reward" && earnedCard) {
    return (
      <>
        <RewardReveal rarity={earnedCard} game="astro-informatika"
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK7: Record<string, Record<string, string>> = {
      en: { test1: "Spreadsheets & Data Analysis", test2: "Computer Networks & Security", test3: "Advanced Programming Concepts" },
      hu: { test1: "Táblázatkezelés & Adatelemzés", test2: "Számítógépes hálózatok & Biztonság", test3: "Haladó programozási fogalmak" },
      de: { test1: "Tabellenkalkulation & Datenanalyse", test2: "Computernetzwerke & Sicherheit", test3: "Fortgeschrittene Programmierkonzepte" },
      ro: { test1: "Foi de calcul și analiza datelor", test2: "Rețele de calculatoare și securitate", test3: "Concepte avansate de programare" },
    };
