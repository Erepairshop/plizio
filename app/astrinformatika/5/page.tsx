...
import {
  INFO_K5_ISLANDS as K5_ISLANDS, INFO_K5_CHECKPOINT_MAP as K5_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadInfoK5Progress as loadK5Progress, saveInfoK5Progress as saveK5Progress,
  type InformatikaProgress as K5Progress,
  isMissionDoneInfoK5 as isMissionDoneK5, isIslandDoneInfoK5 as isIslandDoneK5,
  isIslandUnlockedInfoK5 as isIslandUnlockedK5,
  isCheckpointUnlockedInfoK5 as isCheckpointUnlockedK5, isCheckpointDoneInfoK5 as isCheckpointDoneK5,
  completeMissionInfoK5 as completeMissionK5, completeTestInfoK5 as completeTestK5,
  islandTotalStarsInfoK5 as islandTotalStarsK5,
  generateIslandQuestionsInfoK5 as generateIslandQuestionsK5, generateCheckpointQuestionsInfoK5 as generateCheckpointQuestionsK5,
...
const K5_LABEL: Record<string, string> = {
  en: "Grade 5 · Informatika",
  hu: "5. osztály · Informatika",
  de: "Klasse 5 · Informatik",
  ro: "Clasa 5 · Informatică",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint, lang }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }} aria-label={mapText.map}>
      <defs>
        <filter id="pathGlowInfo5" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowInfo5" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1info5" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#22C55E" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#22C55E" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2info5" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1info5)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2info5)" />

      <path d={pathD} fill="none" stroke="rgba(100,200,100,0.25)" strokeWidth={8}
        filter="url(#pathGlowInfo5)" strokeLinecap="round" />
...
export default function AstroInformatikaK5Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsK5(activeIsland, qCount, countryCode);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const countryCode = { de: "DE", hu: "HU", ro: "RO", en: "US" }[lang as Lang] ?? "US";
    const qs = generateCheckpointQuestionsK5(testId, 7, countryCode);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const countryCode = { de: "DE", hu: "HU", ro: "RO", en: "US" }[lang as Lang] ?? "US";
    const qs = generateCheckpointQuestionsK5(activeTestId, 15, countryCode);
    setQuestions(qs);
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "info-explore" && activeIsland && (
          <K5Explorer island={activeIsland} grade={5} onDone={handleMissionDone} color={bgColor} lang={lang} />
        )}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK5: Record<string, Record<string, string>> = {
      en: { test1: "Hardware Basics & Operating Systems", test2: "Internet Safety & Web Browsing", test3: "Algorithms & Programming Introduction" },
      hu: { test1: "Hardver alapok & Operációs rendszerek", test2: "Internetbiztonság & Böngészés", test3: "Algoritmusok & Bevezetés a programozásba" },
      de: { test1: "Hardware-Grundlagen & Betriebssysteme", test2: "Internetsicherheit & Web-Browsing", test3: "Algorithmen & Einführung in die Programmierung" },
      ro: { test1: "Bazele hardware și sisteme de operare", test2: "Siguranța pe internet și navigarea pe web", test3: "Algoritmi și introducere în programare" },
    };
