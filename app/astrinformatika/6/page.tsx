...
import {
  ...
  loadInfoK6Progress as loadK6Progress, saveInfoK6Progress as saveK6Progress,
  type InformatikaProgress as K6Progress,
  isMissionDoneInfoK6 as isMissionDoneK6, isIslandDoneInfoK6 as isIslandDoneK6,
  isIslandUnlockedInfoK6 as isIslandUnlockedK6,
  isCheckpointUnlockedInfoK6 as isCheckpointUnlockedK6, isCheckpointDoneInfoK6 as isCheckpointDoneK6,
  completeMissionInfoK6 as completeMissionK6, completeTestInfoK6 as completeTestK6,
  islandTotalStarsInfoK6 as islandTotalStarsK6,
  generateIslandQuestionsInfoK6 as generateIslandQuestionsK6, generateCheckpointQuestionsInfoK6 as generateCheckpointQuestionsK6,
...
const K6_LABEL: Record<string, string> = {
  en: "Grade 6 · Informatika",
  hu: "6. osztály · Informatika",
  de: "Klasse 6 · Informatik",
  ro: "Clasa 6 · Informatică",
};
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        ...
        <filter id="islandGlowInfo6" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1info6" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#22C55E" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#22C55E" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2info6" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1info6)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2info6)" />

      <path d={pathD} fill="none" stroke="rgba(100,200,100,0.25)" strokeWidth={8}
        filter="url(#pathGlowInfo6)" strokeLinecap="round" />
...
export default function AstroInformatikaK6Page() {
  const { lang } = useLang();
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "info-explore" && activeIsland && (
          <K6Explorer island={activeIsland} grade={6} onDone={handleMissionDone} color={bgColor} lang={lang} />
        )}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK6: Record<string, Record<string, string>> = {
      en: { test1: "Word Processing & Formatting", test2: "Presentations & Multimedia", test3: "Logic Gates & Basic Coding" },
      hu: { test1: "Szövegszerkesztés & Formázás", test2: "Prezentációk & Multimédia", test3: "Logikai kapuk & Alapvető kódolás" },
      de: { test1: "Textverarbeitung & Formatierung", test2: "Präsentationen & Multimedia", test3: "Logikgatter & Grundlegende Codierung" },
      ro: { test1: "Procesare de text și formatare", test2: "Prezentări și multimedia", test3: "Porți logice și codare de bază" },
    };
