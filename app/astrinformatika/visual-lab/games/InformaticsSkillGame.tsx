...
import { CheckCircle2, Lightbulb, Cpu } from "lucide-react";
import type { InformaticsSkillLang, InformaticsSkillRound } from "@/lib/visualLab/informaticsSkillContent";
import InformaticsSkillMechanic from "./InformaticsSkillMechanics";

const COPY: Record<InformaticsSkillLang, { correct: string; retry: string; continue: string; topic: string }> = {
  de: { correct: "Richtig", retry: "Prüfe den digitalen Zusammenhang noch einmal", continue: "Weiter", topic: "Thema" },
  hu: { correct: "Helyes", retry: "Vizsgáld meg újra a digitális összefüggést", continue: "Tovább", topic: "Téma" },
  ro: { correct: "Corect", retry: "Verifică din nou legătura digitală", continue: "Continuă", topic: "Temă" },
  en: { correct: "Correct", retry: "Check the digital connection once more", continue: "Continue", topic: "Topic" },
};

export default function InformaticsSkillGame({ lang, round, onDone }: {
  lang: InformaticsSkillLang;
  round: InformaticsSkillRound;
  ...
  return (
    <section className="relative min-h-[clamp(430px,72dvh,680px)] overflow-hidden rounded-2xl border border-cyan-300/20 bg-[radial-gradient(circle_at_top,#164e6355,#020617_58%)] p-4 text-white shadow-2xl sm:p-6">
      ...
      <div className="relative z-10 mx-auto max-w-2xl">
        <div className="mb-4 h-1.5 overflow-hidden rounded-full bg-black/25">
          <div className="h-full rounded-full bg-gradient-to-r from-cyan-400 to-indigo-300" style={{ width: `${Math.max(12, round.level * 20)}%` }} />
        </div>
        <header className="mb-4 flex items-start gap-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-cyan-200/25 bg-cyan-300/10 text-cyan-200"><Cpu size={24} aria-hidden="true" /></span>
          <div>
        ...
        <InformaticsSkillMechanic key={round.id} gameId={round.gameId} lang={lang} level={round.level} context={round.context}
        ...
        <AnimatePresence mode="wait">
          ...
          {solved && (
            <motion.div key="solved" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-4 rounded-xl border border-emerald-300/20 bg-emerald-500/10 p-4">
              ...
              <p className="mt-2 flex items-start gap-2 text-sm leading-relaxed text-white/75"><Lightbulb size={18} className="mt-0.5 shrink-0 text-cyan-300" aria-hidden="true" /><span>{round.explanation}</span></p>
