...
import {
  INFO_K8_ISLANDS as K8_ISLANDS, INFO_K8_CHECKPOINT_MAP as K8_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadInfoK8Progress as loadK8Progress, saveInfoK8Progress as saveK8Progress,
  type InformatikaProgress as K8Progress,
  isMissionDoneInfoK8 as isMissionDoneK8, isIslandDoneInfoK8 as isIslandDoneK8,
  isIslandUnlockedInfoK8 as isIslandUnlockedK8,
  isCheckpointUnlockedInfoK8 as isCheckpointUnlockedK8, isCheckpointDoneInfoK8 as isCheckpointDoneK8,
  completeMissionInfoK8 as completeMissionK8, completeTestInfoK8 as completeTestK8,
  islandTotalStarsInfoK8 as islandTotalStarsK8,
  generateIslandQuestionsInfoK8 as generateIslandQuestionsK8, generateCheckpointQuestionsInfoK8 as generateCheckpointQuestionsK8,
...
const K8_LABEL: Record<string, string> = {
  en: "Grade 8 · Informatika",
  hu: "8. osztály · Informatika",
  de: "Klasse 8 · Informatik",
  ro: "Clasa 8 · Informatică",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint, lang }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }} aria-label={mapText.map}>
      <defs>
        <filter id="pathGlowInfo8" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowInfo8" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1info8" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#22C55E" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#22C55E" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2info8" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1info8)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2info8)" />

      <path d={pathD} fill="none" stroke="rgba(100,200,100,0.25)" strokeWidth={8}
        filter="url(#pathGlowInfo8)" strokeLinecap="round" />
...
export default function AstroInformatikaK8Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsK8(activeIsland, qCount, countryCode);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const countryCode = { de: "DE", hu: "HU", ro: "RO", en: "US" }[lang as Lang] ?? "US";
    const qs = generateCheckpointQuestionsK8(testId, 7, countryCode);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const countryCode = { de: "DE", hu: "HU", ro: "RO", en: "US" }[lang as Lang] ?? "US";
    const qs = generateCheckpointQuestionsK8(activeTestId, 15, countryCode);
    setQuestions(qs);
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "info-explore" && activeIsland && (
          <K8Explorer island={activeIsland} grade={8} onDone={handleMissionDone} color={bgColor} lang={lang} />
        )}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK8: Record<string, Record<string, string>> = {
      en: { test1: "Database Systems", test2: "Web Development & HTML/CSS", test3: "Robotics & AI Introduction" },
      hu: { test1: "Adatbázisrendszerek", test2: "Webfejlesztés & HTML/CSS", test3: "Robotika & MI bevezetés" },
      de: { test1: "Datenbanksysteme", test2: "Webentwicklung & HTML/CSS", test3: "Robotik & KI Einführung" },
      ro: { test1: "Sisteme de baze de date", test2: "Dezvoltare web și HTML/CSS", test3: "Robotică și introducere în IA" },
    };
