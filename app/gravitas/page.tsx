...
import { FlaskConical, Eye, ArrowLeftRight, Users, Calendar, Book, Bell, Swords, Compass, Fuel, Radio } from "lucide-react";
...
export default function GravitasPage() {
  ...
  const [interiorView, setInteriorView] = useState<StarholdModuleId | "galaxy" | "warroom" | "research" | "espionage" | "trade" | "repairbay" | "weekly" | "codex" | "factionwars" | "expeditions" | "concord" | "npcstations" | null>(null);
    ...
    <main className={`fixed inset-0 overflow-hidden transition-colors duration-1000 ${isLockdown ? "bg-[#0a0a0a]" : state.crisis ? "bg-[#1a0505]" : "bg-[#050816]"} ${impactFlash ? "animate-shake" : ""} text-white flex flex-col`}>
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden p-2.5 pt-2">
        ...
        >
          ...
          <MapMiniButton
            icon={<Shield size={14} />}
            active={interiorView === "concord"}
            onClick={() => setInteriorView(interiorView === "concord" ? null : "concord")}
            showDot={state.concord.status === "candidate"}
          />
          <MapMiniButton
            icon={<Radio size={14} />}
            active={interiorView === "npcstations"}
            onClick={() => setInteriorView(interiorView === "npcstations" ? null : "npcstations")}
            showDot={state.npcStations.stations.some((station) => station.status !== "active")}
          />
          <MapMiniButton
