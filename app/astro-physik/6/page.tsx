...
import {
  ...
  loadPhysikK6Progress as loadProgress, savePhysikK6Progress as saveProgress,
  isMissionDonePhysikK6 as isMissionDone, isIslandDonePhysikK6 as isIslandDone,
  isIslandUnlockedPhysikK6 as isIslandUnlocked, isCheckpointUnlockedPhysikK6 as isCheckpointUnlocked,
  isCheckpointDonePhysikK6 as isCheckpointDone,
  completeMissionPhysikK6 as completeMission, completeTestPhysikK6 as completeTest,
  islandTotalStarsPhysikK6 as islandTotalStars,
  generateIslandQuestionsPhysikK6 as generateIslandQuestions, generateCheckpointQuestionsPhysikK6 as generateCheckpointQuestions,
} from "@/lib/astroPhysik6";
...
// ─── Grade 2 label ─────────────────────────────────────────────────────────────
const SK_G2_LABEL: Record<string, Record<string, string | Record<string, string>>> = {
  en: { title: "AstroSachkunde", grade: "Grade 6", back: "Back", start: "Start", next: "Next Island",
        ...
        testTopics: { test1: "Healthy Body · Habitats & Animals", test2: "Plants & Water · Jobs", test3: "Safety & Science Experiments" },
  ...
  hu: { title: "AstroSachkunde", grade: "6. osztály", back: "Vissza", start: "Indítás", next: "Következő sziget",
        ...
        testTopics: { test1: "Egészséges test · Élőhelyek és állatok", test2: "Növények & Víz · Foglalkozások", test3: "Biztonság & Tudományos kísérletek" },
  ...
  de: { title: "AstroSachkunde", grade: "Klasse 6", back: "Zurück", start: "Start", next: "Nächste Insel",
        ...
        testTopics: { test1: "Gesunder Körper · Lebensräume und Tiere", test2: "Pflanzen & Wasser · Berufe", test3: "Sicherheit & Wissenschafts-Experimente" },
  ...
  ro: { title: "AstroSachkunde", grade: "Clasa 6", back: "Înapoi", start: "Start", next: "Insula următoare",
        ...
        testTopics: { test1: "Corp sănătos · Habitate și animale", test2: "Plante & Apă · Locuri de muncă", test3: "Siguranță & Experimente științifice" },
...
function buildSmoothPath(islands: typeof ISLANDS): string {
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function AstroPhysikK6Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestions(activeIsland, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland]);
  ...
  const handleIslandAnimDone = useCallback(() => {
    ...
    saveCard({ id: generateCardId(), game: "astro-physik", rarity, score: missionScore.score, total: missionScore.total, date: new Date().toISOString() });
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestions(testId, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestions(activeTestId, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    saveCard({ id: generateCardId(), game: "astro-physik", rarity, score, total, date: new Date().toISOString() });
    window.dispatchEvent(new Event("plizio-cards-changed"));
    incrementTotalGames();
    checkNewMilestones();
    setEarnedCard(rarity);
    ...
    return (
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          <button onClick={() => router.push("/astro-physik")}
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #3B82F6, #81C784)" }}
