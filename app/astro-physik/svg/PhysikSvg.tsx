...
const DIAGRAM_LABELS = {
  de: { mass: "Masse (m)", focalPoint: "Brennpunkt", amplitude: "Amplitude", wavelength: "Wellenlänge (λ)", force: "Kraft (F)" },
  en: { mass: "Mass (m)", focalPoint: "Focal point", amplitude: "Amplitude", wavelength: "Wavelength (λ)", force: "Force (F)" },
  hu: { mass: "Tömeg (m)", focalPoint: "Fókuszpont", amplitude: "Amplitúdó", wavelength: "Hullámhossz (λ)", force: "Erő (F)" },
  ro: { mass: "Masă (m)", focalPoint: "Focar", amplitude: "Amplitudine", wavelength: "Lungime de undă (λ)", force: "Forță (F)" },
};

function diagramLabels(lang: string) {
  return DIAGRAM_LABELS[lang as keyof typeof DIAGRAM_LABELS] ?? DIAGRAM_LABELS.en;
}

const SPACE_STARS = Array.from({ length: 20 }, (_, index) => ({
  cx: ((index * 37) % 239) + 0.5,
  cy: ((index * 53) % 139) + 0.5,
}));
...
export const ForceSvg = memo(({ lang = "hu" }: { lang?: string }) => {
  ...
  const diagram = diagramLabels(lang);
  return (
    <ScopedSvg viewBox="0 0 240 140" className="w-full h-auto max-h-36">
      ...
      <text x="120" y="110" fontSize="8" fill="#64748b" textAnchor="middle">{diagram.mass}</text>
    </ScopedSvg>
  );
});
...
export const OpticsSvg = memo(({ lang = "hu" }: { lang?: string }) => {
  const l = diagramLabels(lang);
  return (
    <ScopedSvg viewBox="0 0 240 140" className="w-full h-auto max-h-36">
      ...
      <text x="200" y="85" fontSize="8" fill="#ef4444" textAnchor="middle">{l.focalPoint}</text>
    </ScopedSvg>
  );
});
  ...
  return (
    <ScopedSvg viewBox="0 0 240 140" className="w-full h-auto max-h-36">
      ...
      {SPACE_STARS.map((star, index) => (
        <circle key={index} cx={star.cx} cy={star.cy} r="0.5" fill="white" opacity="0.5" />
      ))}
...
export const WaveSvg = memo(({ lang = "hu" }: { lang?: string }) => {
  const l = diagramLabels(lang);
  return (
    <ScopedSvg viewBox="0 0 240 140" className="w-full h-auto max-h-36">
      ...
      <text x="60" y="45" fontSize="8" fill="#be185d">{l.amplitude}</text>

      <path d="M 55 20 L 105 20" fill="none" stroke="#be185d" strokeWidth="1" markerStart="url(#dot)" markerEnd="url(#dot)" />
      <text x="80" y="15" fontSize="8" fill="#be185d" textAnchor="middle">{l.wavelength}</text>
...
export const MechanicsSvg = memo(({ lang = "hu" }: { lang?: string }) => {
  const l = diagramLabels(lang);
  return (
    <ScopedSvg viewBox="0 0 240 140" className="w-full h-auto max-h-36">
      <rect width="240" height="140" fill="#f8fafc" rx="12" />
      ...
      <text x="190" y="55" fontSize="10" fontWeight="bold" fill="#16a34a" textAnchor="middle">{l.force}</text>
    </ScopedSvg>
  );
});
