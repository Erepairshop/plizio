...
function LanguageTestEngineInner({ config }: { config: LanguageTestEngineConfig }) {
  ...
  const handlePrintBlank = () => {
    ...
    const buildChemistryVisualSvg = (type: string, diagramId?: string): string => {
      if (!diagramId || !type.startsWith("chemie-")) return "";
      const circles = (points: [number, number][], color: string) => points.map(([x, y]) => `<circle cx="${x}" cy="${y}" r="5" fill="${color}" stroke="#334155" stroke-width="1.5"/>`).join("");
      let body = "";
      if (type === "chemie-teilchenbild") {
        const solid: [number, number][] = [[48,38],[70,38],[92,38],[114,38],[48,60],[70,60],[92,60],[114,60],[48,82],[70,82],[92,82],[114,82]];
        const liquid: [number, number][] = [[42,73],[61,66],[81,76],[101,68],[121,77],[51,91],[73,94],[96,89],[118,96]];
        const gas: [number, number][] = [[29,28],[73,20],[132,31],[48,61],[111,59],[23,94],[82,99],[150,88]];
        if (diagramId === "solid") body = circles(solid, "#2dd4bf");
        else if (diagramId === "liquid") body = `<path d="M28 96h124" stroke="#38bdf8"/>${circles(liquid, "#38bdf8")}`;
        else if (diagramId === "gas") body = circles(gas, "#a78bfa");
        else if (diagramId === "dissolving") body = `<path d="M42 24v65c0 17 96 17 96 0V24"/><path d="M43 72c24-7 70 7 94 0v19c0 14-94 14-94 0Z" fill="#bae6fd" stroke="none"/>${circles([[58,43],[75,54],[94,45],[116,57]], "#f59e0b")}`;
        else if (diagramId === "diffusion") body = circles([[35,36],[42,58],[31,82],[55,91]], "#fb7185") + circles([[123,32],[145,51],[126,74],[149,94]], "#38bdf8") + `<path d="M65 61h50m-11-9 11 9-11 9" stroke="#7c3aed"/>`;
        else body = circles(liquid, "#fb923c") + `<path d="M48 105c7-18 14 8 21-10 7-18 14 8 21-10 7-18 14 8 21-10" stroke="#f97316"/><path d="M62 56V30m-7 10 7-10 7 10M105 52V22m-7 10 7-10 7 10" stroke="#7c3aed"/>`;
      } else if (type === "chemie-trennmethode") {
        const drawings: Record<string, string> = {
          filtration: `<path d="M50 20h42L79 52v35h35v20H68V87"/><path d="M53 27h36l-5 13H58Z" fill="#fed7aa"/><path d="M79 55v20" stroke="#38bdf8" stroke-dasharray="3 4"/>`,
          evaporation: `<path d="M48 76h84l-10 25H58Z" fill="#bae6fd"/><path d="M67 66c-10-12 8-15 0-27m25 27c-10-12 8-15 0-27m25 27c-10-12 8-15 0-27" stroke="#7c3aed"/>`,
          magnet: `<path d="M40 26v45c0 33 49 33 49 0V26H70v45c0 10-11 10-11 0V26Z" fill="#fecaca"/><circle cx="126" cy="48" r="5" fill="#94a3b8"/><circle cx="139" cy="68" r="5" fill="#94a3b8"/>`,
          decanting: `<path d="M44 28h46v55c0 20-46 20-46 0Z" transform="rotate(-25 67 64)" fill="#bae6fd"/><path d="M111 69h42v32h-42Z"/><path d="M93 58c17 4 23 11 27 20" stroke="#38bdf8"/>`,
          distillation: `<path d="M34 92c0-28 9-48 24-48s24 20 24 48Z" fill="#dbeafe"/><path d="M58 44V26h45v19l34 25v25M103 35h43v20h-43"/>`,
          chromatography: `<path d="M55 18h70v88H55Z" fill="#f8fafc"/><path d="M62 88h56" stroke="#38bdf8"/><circle cx="74" cy="46" r="5" fill="#fb7185"/><circle cx="91" cy="59" r="5" fill="#f59e0b"/><circle cx="108" cy="34" r="5" fill="#38bdf8"/>`,
        };
        body = drawings[diagramId] ?? "";
      } else if (type === "chemie-laborsymbol") {
        const icons: Record<string, string> = {
          flammable: `<path d="M91 92c-25 0-30-24-14-39 4 12 12 13 13-2 1-13 9-23 16-29 3 16 17 24 17 43 0 16-12 27-32 27Z" fill="#fb923c"/>`,
          toxic: `<circle cx="90" cy="50" r="19" fill="#fff"/><circle cx="83" cy="47" r="3" fill="#334155"/><circle cx="97" cy="47" r="3" fill="#334155"/><path d="m83 58h14M58 88l64-35M58 53l64 35"/>`,
          corrosive: `<path d="M50 33h36v10H50Zm49 0h32v10H99Z" fill="#cbd5e1"/><path d="m68 43 10 24m37-24-11 24M43 88h52m11 0h35"/>`,
          irritant: `<path d="M90 31v40" stroke-width="8"/><circle cx="90" cy="88" r="5" fill="#334155"/>`,
          oxidizing: `<circle cx="90" cy="78" r="17"/><path d="M90 63c-18-8-12-25 2-36 0 13 17 17 9 31" fill="#fb923c"/>`,
          explosive: `<path d="m90 61 13-31m-8 35 30-20M82 58 61 31m18 37-35-5m43 9-20 28m29-29 31 21"/><path d="m70 55 15-13 18 9 8 20-19 14-23-9Z" fill="#f59e0b"/>`,
        };
        body = `<g transform="translate(90 60) rotate(45)"><rect x="-39" y="-39" width="78" height="78" fill="#fff" stroke="#ef4444" stroke-width="5"/></g>${icons[diagramId] ?? ""}`;
      } else {
        const changes: Record<string, string> = {
          melting: `<path d="M49 34h47v43H49Z" fill="#bae6fd"/><path d="M45 92c16-13 34 9 51-3 14-10 30 4 42-4" stroke="#38bdf8"/>`,
          rusting: `<path d="M48 82h85l-15 17H60Z" fill="#cbd5e1"/><path d="M59 65h62l-8 17H67Z" fill="#b45309"/><circle cx="74" cy="73" r="3" fill="#f97316"/>`,
          dissolving: `<path d="M48 29v62c0 18 84 18 84 0V29"/><path d="M49 69c21-6 61 6 82 0v24c0 14-82 14-82 0Z" fill="#bae6fd" stroke="none"/>`,
          burning: `<path d="M42 94h96M58 88l57-49M72 98l56-48" stroke="#92400e" stroke-width="7"/><path d="M95 66c-23-13-12-35 5-49 0 18 23 23 14 45-4 10-11 14-19 4Z" fill="#fb923c"/>`,
          fizzing: `<path d="M54 24h40l-7 23v48c0 14-26 14-26 0V47Z" fill="#fef3c7"/><circle cx="74" cy="66" r="4" fill="#38bdf8"/><circle cx="104" cy="45" r="5" fill="#38bdf8"/>`,
          precipitate: `<path d="M53 22h74v69c0 22-74 22-74 0Z"/><circle cx="71" cy="87" r="6" fill="#f59e0b"/><circle cx="91" cy="94" r="7" fill="#f59e0b"/><circle cx="111" cy="84" r="5" fill="#f59e0b"/>`,
        };
        body = changes[diagramId] ?? "";
      }
      return body ? `<svg viewBox="0 0 180 120" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="#334155" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" style="width:100%;height:auto;max-height:90px;">${body}</svg>` : "";
    };
    const renderQuestionPrint = (q: TestQuestion, qi: number): string => {
      ...
      switch (q.type as string) {
        ...
        case "hu-kép-felismerés": {
          ...
          const opts = q.options ?? [];
          parts.push(`<div class="visual-hint-box">`);
          const chemistrySvg = buildChemistryVisualSvg(q.type as string, (q as any).diagramId);
          const registeredPrintSvg = typeof (q as any).printSvg === "string" ? (q as any).printSvg : "";
          if ((q.type as string) === "kraft-richtung") {
            const direction = opts[(q as any).correctIndex] ?? "→";
            parts.push(`<div style="text-align:center; margin-bottom:8px;">${buildKraftVektorSvg(direction)}</div>`);
          } else if (registeredPrintSvg) {
            parts.push(`<div style="text-align:center; margin-bottom:8px;">${registeredPrintSvg}</div>`);
          } else if (chemistrySvg) {
            parts.push(`<div style="text-align:center; margin-bottom:8px;">${chemistrySvg}</div>`);
