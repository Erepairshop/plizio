...
import { AI_M2_POOLS, AI_M3_POOLS } from "@/lib/astro/aiGameRegistry";
import K5Explorer from "@/app/astro-ai/games/k5/K5Explorer";
...
import {
  AI_K5_ISLANDS, AI_K5_CHECKPOINT_TOPICS, type AIProgress,
  loadAIK5Progress, saveAIK5Progress,
  isMissionDoneAIK5, isIslandDoneAIK5, isIslandUnlockedAIK5,
  isCheckpointUnlockedAIK5, isCheckpointDoneAIK5,
  completeMissionAIK5, completeTestAIK5, islandTotalStarsAIK5,
  generateIslandQuestionsAIK5, generateCheckpointQuestionsAIK5,
} from "@/lib/astroAI5";
import {
  AI_K6_ISLANDS, AI_K6_CHECKPOINT_TOPICS,
  loadAIK6Progress, saveAIK6Progress,
  isMissionDoneAIK6, isIslandDoneAIK6, isIslandUnlockedAIK6,
  isCheckpointUnlockedAIK6, isCheckpointDoneAIK6,
  completeMissionAIK6, completeTestAIK6, islandTotalStarsAIK6,
  generateIslandQuestionsAIK6, generateCheckpointQuestionsAIK6,
} from "@/lib/astroAI6";
import {
  AI_K7_ISLANDS, AI_K7_CHECKPOINT_TOPICS,
  loadAIK7Progress, saveAIK7Progress,
  isMissionDoneAIK7, isIslandDoneAIK7, isIslandUnlockedAIK7,
  isCheckpointUnlockedAIK7, isCheckpointDoneAIK7,
  completeMissionAIK7, completeTestAIK7, islandTotalStarsAIK7,
  generateIslandQuestionsAIK7, generateCheckpointQuestionsAIK7,
} from "@/lib/astroAI7";
import {
  AI_K8_ISLANDS, AI_K8_CHECKPOINT_TOPICS,
  loadAIK8Progress, saveAIK8Progress,
  isMissionDoneAIK8, isIslandDoneAIK8, isIslandUnlockedAIK8,
  isCheckpointUnlockedAIK8, isCheckpointDoneAIK8,
  completeMissionAIK8, completeTestAIK8, islandTotalStarsAIK8,
  generateIslandQuestionsAIK8, generateCheckpointQuestionsAIK8,
} from "@/lib/astroAI8";

const AvatarCompanion = dynamic(() => import("@/components/AvatarCompanion"), { ssr: false });

// ─── Grade config (single source of grade-specific data + behaviour) ─────────────
interface GradeCfg {
  grade: 5 | 6 | 7 | 8;
  islands: IslandDef[];
  checkpointTopics: Record<string, string[]>;
  load: () => AIProgress;
  save: (p: AIProgress) => void;
  isMissionDone: (p: AIProgress, islandId: string, missionId: string) => boolean;
  isIslandDone: (p: AIProgress, islandId: string) => boolean;
  isIslandUnlocked: (p: AIProgress, islandId: string) => boolean;
  isCheckpointUnlocked: (p: AIProgress, testId: string) => boolean;
  isCheckpointDone: (p: AIProgress, testId: string) => boolean;
  completeMission: (p: AIProgress, islandId: string, missionId: string, stars?: number) => AIProgress;
  completeTest: (p: AIProgress, testId: string) => AIProgress;
  islandTotalStars: (p: AIProgress, islandId: string) => number;
  generateIslandQuestions: (island: IslandDef, count?: number, lang?: string) => MathQuestion[];
  generateCheckpointQuestions: (testId: string, count?: number, lang?: string) => MathQuestion[];
  Explorer: React.ComponentType<{ island: IslandDef; grade: number; onDone: (s: number, t: number) => void; color?: string; lang?: string }>;
  label: Record<string, string>;
}

const GRADE_CFG: Record<string, GradeCfg> = {
  "5": {
    grade: 5, islands: AI_K5_ISLANDS, checkpointTopics: AI_K5_CHECKPOINT_TOPICS,
    load: loadAIK5Progress, save: saveAIK5Progress,
    isMissionDone: isMissionDoneAIK5, isIslandDone: isIslandDoneAIK5, isIslandUnlocked: isIslandUnlockedAIK5,
    isCheckpointUnlocked: isCheckpointUnlockedAIK5, isCheckpointDone: isCheckpointDoneAIK5,
    completeMission: completeMissionAIK5, completeTest: completeTestAIK5, islandTotalStars: islandTotalStarsAIK5,
    generateIslandQuestions: generateIslandQuestionsAIK5, generateCheckpointQuestions: generateCheckpointQuestionsAIK5,
    Explorer: K5Explorer,
    label: { en: "Grade 5 · AI", hu: "5. osztály · MI", de: "Klasse 5 · KI", ro: "Clasa 5 · IA" },
  },
  "6": {
    grade: 6, islands: AI_K6_ISLANDS, checkpointTopics: AI_K6_CHECKPOINT_TOPICS,
    load: loadAIK6Progress, save: saveAIK6Progress,
    isMissionDone: isMissionDoneAIK6, isIslandDone: isIslandDoneAIK6, isIslandUnlocked: isIslandUnlockedAIK6,
    isCheckpointUnlocked: isCheckpointUnlockedAIK6, isCheckpointDone: isCheckpointDoneAIK6,
    completeMission: completeMissionAIK6, completeTest: completeTestAIK6, islandTotalStars: islandTotalStarsAIK6,
    generateIslandQuestions: generateIslandQuestionsAIK6, generateCheckpointQuestions: generateCheckpointQuestionsAIK6,
    Explorer: K6Explorer,
    label: { en: "Grade 6 · AI", hu: "6. osztály · MI", de: "Klasse 6 · KI", ro: "Clasa 6 · IA" },
  },
  "7": {
    grade: 7, islands: AI_K7_ISLANDS, checkpointTopics: AI_K7_CHECKPOINT_TOPICS,
    load: loadAIK7Progress, save: saveAIK7Progress,
    isMissionDone: isMissionDoneAIK7, isIslandDone: isIslandDoneAIK7, isIslandUnlocked: isIslandUnlockedAIK7,
    isCheckpointUnlocked: isCheckpointUnlockedAIK7, isCheckpointDone: isCheckpointDoneAIK7,
    completeMission: completeMissionAIK7, completeTest: completeTestAIK7, islandTotalStars: islandTotalStarsAIK7,
    generateIslandQuestions: generateIslandQuestionsAIK7, generateCheckpointQuestions: generateCheckpointQuestionsAIK7,
    Explorer: K7Explorer,
    label: { en: "Grade 7 · AI", hu: "7. osztály · MI", de: "Klasse 7 · KI", ro: "Clasa 7 · IA" },
  },
  "8": {
    grade: 8, islands: AI_K8_ISLANDS, checkpointTopics: AI_K8_CHECKPOINT_TOPICS,
    load: loadAIK8Progress, save: saveAIK8Progress,
    isMissionDone: isMissionDoneAIK8, isIslandDone: isIslandDoneAIK8, isIslandUnlocked: isIslandUnlockedAIK8,
    isCheckpointUnlocked: isCheckpointUnlockedAIK8, isCheckpointDone: isCheckpointDoneAIK8,
    completeMission: completeMissionAIK8, completeTest: completeTestAIK8, islandTotalStars: islandTotalStarsAIK8,
    generateIslandQuestions: generateIslandQuestionsAIK8, generateCheckpointQuestions: generateCheckpointQuestionsAIK8,
    Explorer: K8Explorer,
    label: { en: "Grade 8 · AI", hu: "8. osztály · MI", de: "Klasse 8 · KI", ro: "Clasa 8 · IA" },
  },
};
...
function IslandMapSVG({ G, progress, onIsland, onCheckpoint, lang }: {
  G: GradeCfg;
  progress: AIProgress;
  ...
  const mapText = {
    de: { done: "Fertig!", ready: "Test!", locked: "Test", checkpoint: "Checkpoint", island: "Insel", map: "KI-Lernkarte" },
    hu: { done: "Kész!", ready: "Teszt!", locked: "Teszt", checkpoint: "Ellenőrző", island: "Sziget", map: "MI-tanulótérkép" },
    ro: { done: "Gata!", ready: "Test!", locked: "Test", checkpoint: "Verificare", island: "Insulă", map: "Harta de învățare IA" },
    en: { done: "Done!", ready: "Test!", locked: "Test", checkpoint: "Checkpoint", island: "Island", map: "AI learning map" },
  }[lang];

  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }} aria-label={mapText.map}>
      <defs>
        <filter id="pathGlowAI" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowAI" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1ai" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#22C55E" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#22C55E" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2ai" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1ai)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2ai)" />

      <path d={pathD} fill="none" stroke="rgba(100,200,100,0.25)" strokeWidth={8}
        filter="url(#pathGlowAI)" strokeLinecap="round" />
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = G.isCheckpointUnlocked(progress, testId);
        const done = G.isCheckpointDone(progress, testId);
      ...
      {islands.map((island, idx) => {
        const unlocked = G.isIslandUnlocked(progress, island.id);
        const done = G.isIslandDone(progress, island.id);
        const total = G.islandTotalStars(progress, island.id);
        ...
        return (
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m: MissionDef, mi: number) => {
                  const mdone = G.isMissionDone(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
export default function AstroAIGradeClient({ params }: { params: Promise<{ grade: string }> }) {
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = G.generateIslandQuestions(activeIsland, qCount, lang);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, G, lang]);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = G.completeMission(progress, activeIsland.id, activeMission.id, stars);
  ...
  }, [activeIsland, activeMission, progress, G]);
  ...
  const handleIslandAnimDone = useCallback(() => {
    ...
    saveCard({ id: generateCardId(), game: "astro-ai", rarity, score: missionScore.score, total: missionScore.total, date: new Date().toISOString() });
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = G.generateCheckpointQuestions(testId, 7, lang);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = G.generateCheckpointQuestions(activeTestId, 15, lang);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = G.completeTest(progress, activeTestId);
    G.save(newProgress);
    setProgress(newProgress);
    ...
    saveCard({ id: generateCardId(), game: "astro-ai", rarity, score, total, date: new Date().toISOString() });
    incrementTotalGames();
    checkNewMilestones();
    setEarnedCard(rarity);
    setRewardScore({ score, total });
    setScreen("reward");
  }, [activeTestId, progress, G]);
  ...
  const bgColor = activeIsland?.color ?? "#4CAF50";
    ...
    return (
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          <button onClick={() => router.push("/astro-ai")}
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{G.label[lang] ?? G.label.en}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #4CAF50, #00FF88)" }}
        ...
        <div className="relative z-10 flex-1 min-h-0 overflow-y-auto" ref={attachAutoScrollToBottom}>
          <div className="max-w-sm mx-auto px-2 pb-6" style={{ minHeight: MAP_H + 40 }}>
            <div className="relative">
              <IslandMapSVG G={G} progress={progress} onIsland={handleIslandSelect} onCheckpoint={startCheckpoint} lang={lang as Lang} />
              <motion.div
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = G.islandTotalStars(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = G.isMissionDone(progress, activeIsland.id, mission.id);
  ...
  const gameScreen = (
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "info-explore" && activeIsland && (
          <Explorer island={activeIsland} grade={G.grade} onDone={handleMissionDone} color={bgColor} lang={lang} />
        )}
        {screen === "m2" && activeMission?.gameKey && AI_M2_POOLS[activeMission.gameKey] && (
          <M2Engine gameKey={activeMission.gameKey} rounds={AI_M2_POOLS[activeMission.gameKey]} color={bgColor} lang={lang as any} onDone={handleMissionDone} onCorrect={() => { setAvatarMood("happy"); setJumpTrigger({ reaction: "happy", timestamp: Date.now() }); }} onWrong={() => setAvatarMood("disappointed")} />
        )}
        {screen === "m3" && activeMission?.gameKey && AI_M3_POOLS[activeMission.gameKey] && (
          <M3Engine gameKey={activeMission.gameKey} rounds={AI_M3_POOLS[activeMission.gameKey]} color={bgColor} lang={lang as any} onDone={handleMissionDone} onCorrect={() => { setAvatarMood("happy"); setJumpTrigger({ reaction: "happy", timestamp: Date.now() }); }} onWrong={() => setAvatarMood("disappointed")} />
        )}
  ...
  if (screen === "reward" && earnedCard) {
    return (
      <>
        <RewardReveal rarity={earnedCard} game="astro-ai"
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    // Derive the topic description from the islands belonging to this checkpoint
    const testIslandRanges: Record<string, [number, number]> = { test1: [0, 3], test2: [3, 6], test3: [6, 9] };
    const range = testIslandRanges[activeTestId] ?? [0, 3];
    const topicDesc = ISLANDS.slice(range[0], range[1])
      .map((isl) => isl.name[lang as Lang] ?? isl.name.en)
      .join(" · ");
    return (
