...
const GRADE_LABEL: Record<string, string> = {
  en: "Grade 6 · Earth Layers & Water",
  de: "Klasse 6 · Erdschichten & Wasser",
  hu: "6. osztály · A Föld rétegei és a víz",
  ro: "Clasa 6 · Straturile Pământului și Apa",
};
...
export default function AstroGeographieK6Page() {
  ...
  useEffect(() => {
    const p = loadK6Progress(variant.id);
    setProgress(p);
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (mission.gameType === "geographie-explore") {
      // no question generation needed for K6Explorer
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    saveK6Progress(newProgress, variant.id);
    ...
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden bg-[#060614]">
        ...
        <div className="relative z-10 flex-1">
          ...
          {screen === "geographie-explore" && activeIsland && <GeographieK6Explorer island={activeIsland} grade={6} color={bgColor} lang={lang} variantId={variant.id} onDone={handleMissionDone} />}
        </div>
