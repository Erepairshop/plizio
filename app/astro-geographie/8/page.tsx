...
export default function AstroGeographieK8Page() {
  ...
  useEffect(() => {
    const p = loadK8Progress(variant.id);
    setProgress(p);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    saveK8Progress(newProgress, variant.id);
    ...
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden bg-[#060614]">
        ...
        <div className="relative z-10 flex-1">
          ...
          {screen === "geographie-explore" && activeIsland && <GeographieK8Explorer island={activeIsland} grade={8} color={bgColor} lang={lang} variantId={variant.id} onDone={handleMissionDone} />}
        </div>
  ...
  if (screen === "island-complete-anim") return <IslandCompleteAnimation islandIcon={activeIsland!.icon} islandColor={activeIsland!.color} islandName={activeIsland!.name[lang as Lang] ?? activeIsland!.name.de} lang={lang} grade={8} score={missionScore.score} total={missionScore.total} onDone={handleIslandAnimDone} />;
