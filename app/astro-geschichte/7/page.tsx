...
import { getK7VariantContent } from "@/lib/astroGeschichte7";
...
} from "@/lib/astroGeschichte7";

const AvatarCompanion = dynamic(() => import("@/components/AvatarCompanion"), { ssr: false });
import VisualLab, { VisualLabFab } from "@/components/VisualLab";
const K7Explorer = dynamic(() => import("@/app/astro-geschichte/games/k7/K7Explorer"), { ssr: false });
...
const CATEGORY_CONFIG: Record<string, {
  ...
  "geschichte-explore": {
    ...
    desc: {
      ...
      hu: "Ismerd meg a történelmet interaktív küldetéseken!",
      de: "Lerne Geschichte durch interaktive Missionen!",
      ro: "Învață istoria prin misiuni interactive!",
    },
...
const GRADE_LABEL: Record<string, string> = {
  en: "Grade 7 · Early Modern Era",
  de: "Klasse 7 · Frühe Neuzeit",
  hu: "7. osztály · Kora újkor",
  ro: "Clasa 7 · Epoca Modernă Timpurie",
};
...
function IslandMapSVG({ islands, progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowGK7" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowGK7" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebulaGK7" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor="#3B82F6" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#3B82F6" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={160} cy={200} rx={200} ry={250} fill="url(#nebulaGK7)" />
      <path d={pathD} fill="none" stroke="rgba(59,130,246,0.22)" strokeWidth={8}
        filter="url(#pathGlowGK7)" strokeLinecap="round" />
      ...
      {islands.map((island, idx) => {
        ...
        return (
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            {unlocked && (
              <circle cx={island.svgX} cy={island.svgY} r={35} fill={island.color} opacity={0.1} filter="url(#islandGlowGK7)" />
            )}
...
export default function AstroGeschichteK7Page() {
  const { lang } = useLang();
  const router = useRouter();
  const t = T[lang as keyof typeof T] ?? T.en;
  const content = useMemo(() => getK7VariantContent(lang as Lang), [lang]);
  ...
  useEffect(() => {
    const p = loadK7Progress(content.variant.id);
    setProgress(p);
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (mission.gameType === "geschichte-explore") {
      // no question generation needed for K7Explorer
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    saveK7Progress(newProgress, content.variant.id);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsK7(testId, lang as Lang, 10, content.checkpointTopics);
    setQuestions(qs);
  ...
  if (screen === "island-map") {
    return (
      ...
      <VisualLab subject="geschichte" grade={7} lang={lang} open={visualLabOpen} onClose={() => setVisualLabOpen(false)} />
      </>
    );
  }
      ...
      <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 justify-center">
          {activeIsland.missions.map((m, i) => {
            ...
            const done = isMissionDoneK7(progress, activeIsland.id, m.id);
            return (
    ...
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden bg-[#060614]">
        ...
        <div className="relative z-10 flex-1">
          ...
          {screen === "geschichte-explore" && activeIsland && content.usesCountryContent && <CountryHistoryExplorer island={activeIsland} grade={7} color={bgColor} lang={lang} onDone={handleMissionDone} onClose={() => setScreen("mission-select")} />}
          {screen === "geschichte-explore" && activeIsland && !content.usesCountryContent && <K7Explorer island={activeIsland} grade={7} color={bgColor} lang={lang} variantId={content.variant.id} onDone={handleMissionDone} />}
        </div>
