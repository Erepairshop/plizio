...
import { getK8VariantContent } from "@/lib/astroGeschichte8";
...
export default function AstroGeschichteK8Page() {
  const { lang } = useLang();
  ...
  useEffect(() => {
    const p = loadK8Progress(content.variant.id);
    setProgress(p);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    saveK8Progress(newProgress, content.variant.id);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsK8(testId, lang as Lang, 10, content.checkpointTopics);
    setQuestions(qs);
    ...
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden bg-[#060614]">
        ...
        <div className="relative z-10 flex-1">
          ...
          {screen === "geschichte-explore" && activeIsland && content.usesCountryContent && <CountryHistoryExplorer island={activeIsland} grade={8} color={bgColor} lang={lang} onDone={handleMissionDone} onClose={() => setScreen("mission-select")} />}
          {screen === "geschichte-explore" && activeIsland && !content.usesCountryContent && <K8Explorer island={activeIsland} grade={8} color={bgColor} lang={lang} variantId={content.variant.id} onDone={handleMissionDone} />}
        </div>
