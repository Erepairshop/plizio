...
import {
  ...
  loadK4Progress, saveK4Progress, type EnglishProgress,
...
const K4_LABEL: Record<string, string> = {
  en: "Grade 4 · Rhetoric Galaxy",
  hu: "4. osztály · Retorika Galaxis",
  de: "Klasse 4 · Rhetorik-Galaxie",
  ro: "Clasa 4 · Galaxia retoricii",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowK4" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowK4" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1k4" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#6420C8" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#6420C8" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2k4" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1k4)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2k4)" />

      <path d={pathD} fill="none" stroke="rgba(150,100,255,0.25)" strokeWidth={8}
        filter="url(#pathGlowK4)" strokeLinecap="round" />
...
export default function AstroEnglishK4Page() {
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{K4_LABEL[lang] ?? K4_LABEL.en}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #F59E0B, #EC4899)" }}
        ...
        <VisualLab subject="english" grade={4} lang={lang as "de" | "hu" | "ro" | "en"} open={visualLabOpen} onClose={() => setVisualLabOpen(false)} />
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "phonics" && activeIsland && (
          <PhonicsExplorer rounds={getExplorerContentK4(activeIsland.id, "phonics")} color={bgColor} onDone={handleMissionDone} lang={lang} />
        )}
        {screen === "picture-vocab" && activeIsland && (
          <PictureVocabExplorer rounds={getExplorerContentK4(activeIsland.id, "picture-vocab")} color={bgColor} onDone={handleMissionDone} lang={lang} />
        )}
        {screen === "rhyme-match" && activeIsland && (
          <RhymeMatchExplorer rounds={getExplorerContentK4(activeIsland.id, "rhyme-match")} color={bgColor} onDone={handleMissionDone} lang={lang} />
        )}
        {screen === "word-build" && activeIsland && (
          <WordBuildExplorer rounds={getExplorerContentK4(activeIsland.id, "word-build")} color={bgColor} onDone={handleMissionDone} lang={lang} />
        )}
        {screen === "reading-comp" && activeIsland && (
          <ReadingCompExplorer rounds={getExplorerContentK4(activeIsland.id, "reading-comp")} color={bgColor} onDone={handleMissionDone} lang={lang} />
        )}
        {screen === "tense-explorer" && activeIsland && (
          <TenseExplorer rounds={getExplorerContentK4(activeIsland.id, "tense-explorer")} color={bgColor} onDone={handleMissionDone} lang={lang} />
        )}
        {screen === "memory-pair" && activeIsland && (
          <MemoryPairExplorer pairs={getExplorerContentK4(activeIsland.id, "memory-pair")} color={bgColor} onDone={handleMissionDone} lang={lang} />
        )}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK4: Record<string, Record<string, string>> = {
      en: { test1: "Pronouns, Modals, Progressive Tenses", test2: "Adjectives, Prepositions, Sentences", test3: "Direct Speech, Spelling, Vocabulary" },
      hu: { test1: "Névmások, modális igék, progresszív igeidők", test2: "Melléknevek, előljárószók, mondatok", test3: "Közvetlen beszéd, helyesírás, szókincs" },
      de: { test1: "Pronomen, Modalverben, Progressiv", test2: "Adjektive, Präpositionen, Sätze", test3: "Direkte Rede, Rechtschreibung, Wortschatz" },
      ro: { test1: "Pronume, verbe modale, progresiv", test2: "Adjectiv, prepoziții, propoziții", test3: "Vorbire directă, ortografie, vocabular" },
    };
    const topicDesc = (testTopicsK4[lang] ?? testTopicsK4.en)[activeTestId] ?? "";
    return (
