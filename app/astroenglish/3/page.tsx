...
export default function AstroEnglishK3Page() {
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = K3_ISLANDS.find(i => i.id === id);
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (isExplorer) {
      ...
      const noQuestionsTypes: string[] = ["gravity-sort", "m2", "m3"];
  ...
  if (screen === "island-map") {
    ...
    return (
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <VisualLab subject="english" grade={3} lang={lang as "de" | "hu" | "ro" | "en"} open={visualLabOpen} onClose={() => setVisualLabOpen(false)} />
      </div>
    );
  }
