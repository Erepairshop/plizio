...
import {
  C5_ISLANDS as K5_ISLANDS, C5_CHECKPOINT_MAP as K5_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadC5Progress as loadK5Progress, saveC5Progress as saveK5Progress,
  type RomanaProgress as BioK5Progress,
  isMissionDoneC5 as isMissionDoneK5, isIslandDoneC5 as isIslandDoneK5,
  isIslandUnlockedC5 as isIslandUnlockedK5,
  isCheckpointUnlockedC5 as isCheckpointUnlockedK5, isCheckpointDoneC5 as isCheckpointDoneK5,
  completeMissionC5 as completeMissionK5, completeTestC5 as completeTestK5,
  islandTotalStarsC5 as islandTotalStarsK5,
  generateIslandQuestionsC5 as generateIslandQuestionsK5, generateCheckpointQuestionsC5 as generateCheckpointQuestionsK5,
} from "@/lib/astroRomana5";
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowRomana5" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowRomana5" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1romana5" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF2D78" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#FF2D78" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2romana5" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1romana5)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2romana5)" />

      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.25)" strokeWidth={8}
        filter="url(#pathGlowRomana5)" strokeLinecap="round" />
...
export default function AstroRomanaK5Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsK5(activeIsland!, lang as any, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsK5(testId, lang as any, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsK5(activeTestId, lang as any, 15);
    setQuestions(qs);
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "romana-explore" && activeIsland && (
          <C5Explorer island={activeIsland} grade={5} color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
