...
export interface AstroRomanaGradeConfig {
  grade: number;
  label: Record<string, string>;
  islands: IslandDef[];
  checkpointMap: Record<string, string[]>;
  loadProgress: () => RomanaProgress;
  saveProgress: (progress: RomanaProgress) => void;
  isMissionDone: (progress: RomanaProgress, islandId: string, missionId: string) => boolean;
  isIslandDone: (progress: RomanaProgress, islandId: string) => boolean;
  isIslandUnlocked: (progress: RomanaProgress, islandId: string) => boolean;
  isCheckpointUnlocked: (progress: RomanaProgress, testId: string) => boolean;
  isCheckpointDone: (progress: RomanaProgress, testId: string) => boolean;
  completeMission: (progress: RomanaProgress, islandId: string, missionId: string, stars?: number) => RomanaProgress;
  islandTotalStars: (progress: RomanaProgress, islandId: string) => number;
  completeTest: (progress: RomanaProgress, testId: string) => RomanaProgress;
  generateIslandQuestions: (island: IslandDef, lang: Lang, count?: number) => MathQuestion[];
  generateCheckpointQuestions: (testId: string, lang: Lang, count?: number) => MathQuestion[];
  ExplorerComponent: React.ComponentType<ExplorerProps>;
  checkpointTopics: Record<string, Record<string, string>>;
  noQuestionsTypes?: RomanaGameScreen[];
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{
          background: `${island.color}22`,
          border: `3px solid ${island.color}`,
          boxShadow: `0 0 30px ${island.color}55`,
        }}
      ...
      <div>
        ...
        <h2 className="text-2xl font-black text-white">{mission.label[lang as Lang] ?? mission.label.ro}</h2>
      </div>
    ...
    >
      ...
      <div>
        ...
        <h2 className="text-2xl font-black" style={{ color: island.color }}>
          {island.name[lang as Lang] ?? island.name.ro}
        </h2>
      </div>
      <div className="flex gap-1 text-3xl">
        {["⭐", "⭐", "⭐"].map((s, i) => (
          <motion.span key={i} initial={{ scale: 0, rotate: -20 }} animate={{ scale: 1, rotate: 0 }} transition={{ delay: i * 0.12, type: "spring" }}>
            {s}
          </motion.span>
        ))}
      </div>
...
function IslandMapSVG({ config, progress, onIsland, onCheckpoint }: {
  config: AstroRomanaGradeConfig;
  progress: RomanaProgress;
  onIsland: (island: IslandDef) => void;
  onCheckpoint: (testId: string) => void;
}) {
  const { lang } = useLang();
  const pathD = buildSmoothPath(config.islands);
  const grade = String(config.grade);
  const pathGlowId = `pathGlowR${grade}`;
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id={pathGlowId} x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id={islandGlowId} x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id={nebula1Id} cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#6366F1" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#6366F1" stopOpacity="0" />
        </radialGradient>
        <radialGradient id={nebula2Id} cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill={`url(#${nebula1Id})`} />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill={`url(#${nebula2Id})`} />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8} filter={`url(#${pathGlowId})`} strokeLinecap="round" />
      <path d={pathD} fill="none" stroke="rgba(255,255,255,0.10)" strokeWidth={2.5} strokeDasharray="10 7" strokeLinecap="round" />
      {config.islands.slice(0, -1).map((island, i) => {
        const next = config.islands[i + 1];
      ...
      {Object.keys(config.checkpointMap).map((testId) => {
        const pos = CP_POS[testId];
        const unlocked = config.isCheckpointUnlocked(progress, testId);
        const done = config.isCheckpointDone(progress, testId);
        ...
        return (
          <g key={testId} onClick={() => unlocked && !done && onCheckpoint(testId)} style={{ cursor: unlocked && !done ? "pointer" : "default" }}>
            ...
            <rect x={pos.x - 48} y={pos.y - 16} width={96} height={32} rx={16} fill={fillAlpha} stroke={color} strokeWidth={done ? 1.5 : 2} />
            <text x={pos.x - 32} y={pos.y + 5} textAnchor="middle" fontSize={13}>{done ? "✅" : unlocked ? "🚀" : "🔒"}</text>
            <text x={pos.x + 8} y={pos.y + 5} textAnchor="middle" fontSize={10} fontWeight="bold" fill={color}>
              {done ? "Gata!" : unlocked ? "Test!" : "Test"}
            </text>
          </g>
        );
      })}
      {config.islands.map((island, idx) => {
        const unlocked = config.isIslandUnlocked(progress, island.id);
        const done = config.isIslandDone(progress, island.id);
        ...
        return (
          <g key={island.id} onClick={() => unlocked && onIsland(island)} style={{ cursor: unlocked ? "pointer" : "default" }}>
            ...
            {unlocked ? (
              <text x={island.svgX} y={island.svgY + 7} textAnchor="middle" fontSize={20} opacity={done ? 0.85 : 1}>
                {island.icon}
              </text>
            ) : (
              <>
            ...
            {!unlocked && (
              <text x={island.svgX} y={island.svgY + 42} textAnchor="middle" fontSize={9} fill="rgba(255,255,255,0.2)" fontWeight="bold">
                {idx + 1}
              </text>
            )}
            {done && (
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((mission, mi) => {
                  const missionDone = config.isMissionDone(progress, island.id, mission.id);
                  return (
                    <circle
                      key={mi}
                      cx={island.svgX - 8 + mi * 8}
                      cy={island.svgY + 34}
                      r={4}
                      fill={missionDone ? island.color : "rgba(255,255,255,0.08)"}
                      stroke={missionDone ? island.color : "rgba(255,255,255,0.2)"}
                      strokeWidth={1}
                    />
                  );
                })}
            ...
            {unlocked && (
              ...
              >
                {total > 0 ? `${total}/9 ⭐` : island.name.ro?.split(" ")[0] ?? String(idx + 1)}
              </text>
            )}
            <title>{island.name[lang as Lang] ?? island.name.ro}</title>
          </g>
        );
      })}
    </svg>
...
export default function AstroRomanaGradePage({ config }: { config: AstroRomanaGradeConfig }) {
  const { lang } = useLang();
  const router = useRouter();
  const t = T[lang as keyof typeof T] ?? T.en;
  const ExplorerComponent = config.ExplorerComponent;
  const totalIslands = Math.max(config.islands.length, 1);

  const [screen, setScreen] = useState<Screen>("island-map");
  const [progress, setProgress] = useState<RomanaProgress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const [avatarIslandId, setAvatarIslandId] = useState<string>(config.islands[0]?.id ?? "i1");
  ...
  useEffect(() => {
    const nextProgress = config.loadProgress();
    setProgress(nextProgress);
    const lastDone = [...config.islands].reverse().find((island) => nextProgress.completedIslands.includes(island.id));
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (!noQuestionsTypes.has(mission.gameType as RomanaGameScreen)) {
      const qCount = mission.gameType === "star-match" ? 15 : 10;
      setQuestions(config.generateIslandQuestions(activeIsland, lang as Lang, qCount));
    }
    setScreen(mission.gameType as Screen);
  }, [activeIsland, config, lang, noQuestionsTypes]);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const nextProgress = config.completeMission(progress, activeIsland.id, activeMission.id, stars);
    const isNowIslandDone = nextProgress.completedIslands.includes(activeIsland.id);
    setJustUnlockedIsland(!wasIslandDone && isNowIslandDone);
    config.saveProgress(nextProgress);
    setProgress(nextProgress);
    setAvatarMood(pct >= 60 ? "victory" : "disappointed");
    setScreen("mission-done");
  }, [activeIsland, activeMission, config, progress]);

  const handleAfterMission = useCallback(() => {
    if (!activeIsland) return;
    setScreen(justUnlockedIsland ? "island-complete-anim" : "mission-select");
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    setQuestions(config.generateCheckpointQuestions(testId, lang as Lang, 7));
    setScreen("rocket-launch");
  }, [config, lang]);

  const startCheckpointQuiz = useCallback(() => {
    if (!activeTestId) return;
    setQuestions(config.generateCheckpointQuestions(activeTestId, lang as Lang, 10));
    setScreen("checkpoint-quiz");
  }, [activeTestId, config, lang]);

  const handleCheckpointDone = useCallback((score: number, total: number) => {
    if (!activeTestId) return;
    setCheckpointScore({ score, total });
    const nextProgress = config.completeTest(progress, activeTestId);
    config.saveProgress(nextProgress);
    setProgress(nextProgress);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{config.label[lang] ?? config.label.ro}</p>
          </div>
          <div className="w-9 h-9 flex items-center justify-center rounded-full bg-white/10 text-white/60 text-xs font-bold">
            {totalDone}/{config.islands.length}
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 gap-6 text-center pb-6">
          ...
          <div>
            <h2 className="text-2xl font-black text-white">{activeIsland.name[lang as Lang] ?? activeIsland.name.ro}</h2>
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = config.islandTotalStars(progress, activeIsland.id);
    ...
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((category, cardIdx) => {
            const mission = activeIsland.missions.find((m) => m.category === category);
            if (!mission) return null;
            const cfg = CATEGORY_CONFIG[category];
            const done = config.isMissionDone(progress, activeIsland.id, mission.id);
            const missionKey = `${activeIsland.id}_${mission.id}`;
            const bestStars = (progress.missionStars ?? {})[missionKey] ?? 0;
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black px-2.5 py-0.5 rounded-full" style={{ background: `${cfg.color}25`, color: cfg.color }}>
                    {cfg.label[lang] ?? cfg.label.en}
                  </span>
                  {done && (
                    <div className="flex gap-0.5">
                      {[1, 2, 3].map((star) => (
                        <span key={star} className="text-sm" style={{ opacity: star <= bestStars ? 1 : 0.2 }}>
                          {star <= bestStars ? "⭐" : "✩"}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-black text-white text-base leading-tight">{mission.label[lang as Lang] ?? mission.label.ro}</p>
                    <p className="text-xs mt-0.5" style={{ color: `${cfg.color}cc` }}>{cfg.desc[lang] ?? cfg.desc.en}</p>
                  </div>
                </div>
  ...
  if (screen === "island-complete-anim" && activeIsland) {
    return (
      <IslandCompleteAnimation
        ...
        islandName={activeIsland.name[lang as Lang] ?? activeIsland.name.ro}
        lang={lang}
        grade={config.grade}
        score={missionScore.score}
        total={missionScore.total}
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden px-5" style={{ background: `radial-gradient(ellipse at 50% 30%, ${bgColor}22 0%, #060614 60%)` }}>
        ...
        <div className="relative z-10 w-full">
          <MissionDoneScreen mission={activeMission} island={activeIsland} score={missionScore.score} total={missionScore.total} onContinue={handleAfterMission} />
        </div>
      </div>
    );
  }
    ...
    return (
      <>
        <RewardReveal
          ...
          onDone={() => { if (activeTestId) setScreen("checkpoint-done"); else setScreen("island-done"); }}
        />
        <MilestonePopup />
      </>
    );
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const topicDesc = (config.checkpointTopics[lang] ?? config.checkpointTopics.ro)[activeTestId] ?? "";
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden px-5 gap-6" style={{ background: "radial-gradient(ellipse at 50% 30%, rgba(255,215,0,0.12) 0%, #060614 60%)" }}>
        ...
        <div className="relative z-10 flex flex-col items-center gap-5 text-center w-full max-w-sm">
          ...
          <motion.button onClick={startCheckpointQuiz} className="w-full py-4 rounded-2xl font-black text-black text-base flex items-center justify-center gap-2" style={{ background: "linear-gradient(135deg, #FFD700, #FFA500)" }} whileTap={{ scale: 0.97 }}>
            {t.startTest} 🚀
          </motion.button>
        </div>
      </div>
  ...
  if (screen === "checkpoint-done" && activeTestId) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden px-5" style={{ background: "radial-gradient(ellipse at 50% 30%, rgba(255,215,0,0.12) 0%, #060614 60%)" }}>
        <Starfield />
        <div className="relative z-10 w-full">
          <CheckpointDoneScreen score={checkpointScore.score} total={checkpointScore.total} onContinue={goToMap} />
        </div>
        <MilestonePopup />
      </div>
    );
