...
import {
  C8_ISLANDS as K8_ISLANDS, C8_CHECKPOINT_MAP as K8_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadC8Progress as loadK8Progress, saveC8Progress as saveK8Progress,
  type RomanaProgress as BioK8Progress,
  isMissionDoneC8 as isMissionDoneK8, isIslandDoneC8 as isIslandDoneK8,
  isIslandUnlockedC8 as isIslandUnlockedK8,
  isCheckpointUnlockedC8 as isCheckpointUnlockedK8, isCheckpointDoneC8 as isCheckpointDoneK8,
  completeMissionC8 as completeMissionK8, completeTestC8 as completeTestK8,
  islandTotalStarsC8 as islandTotalStarsK8,
  generateIslandQuestionsC8 as generateIslandQuestionsK8, generateCheckpointQuestionsC8 as generateCheckpointQuestionsK8,
} from "@/lib/astroRomana8";
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowRomana8" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowRomana8" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1romana8" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF2D78" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#FF2D78" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2romana8" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1romana8)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2romana8)" />

      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.25)" strokeWidth={8}
        filter="url(#pathGlowRomana8)" strokeLinecap="round" />
...
export default function AstroRomanaK8Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (mission.gameType === "romana-explore") {
      setQuestions([]);
      setScreen("romana-explore");
      return;
    }

    if (mission.gameKey && ROMANA_M2_POOLS[mission.gameKey]) {
      setQuestions([]);
      setScreen("m2");
      return;
    }
    if (mission.gameKey && ROMANA_M3_POOLS[mission.gameKey]) {
      setQuestions([]);
      setScreen("m3");
      return;
    }
    ...
    const qs = generateIslandQuestionsK8(activeIsland, lang as any, qCount);
    setQuestions(qs);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsK8(testId, lang as any, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsK8(activeTestId, lang as any, 15);
    setQuestions(qs);
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "romana-explore" && activeIsland && (
          <C8Explorer island={activeIsland} grade={8} color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
