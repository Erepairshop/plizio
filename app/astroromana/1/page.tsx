...
import {
  ...
  isIslandDoneC1,
  isIslandUnlockedC1,
  isCheckpointUnlockedC1,
  isCheckpointDoneC1,
  completeMissionC1,
  islandTotalStarsC1,
  completeTestC1,
  generateIslandQuestionsC1,
  generateCheckpointQuestionsC1,
} from "@/lib/astroRomana";
...
type Screen =
  ...
  | "orbit-quiz" | "star-match" | "black-hole"
  | "romana-explore"
...
function buildSmoothPath(islands: typeof C1_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  const { lang } = useLang();
  const pathD = buildSmoothPath(C1_ISLANDS);
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowR1" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowR1" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1r1" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#6366F1" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#6366F1" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2r1" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1r1)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2r1)" />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8}
        filter="url(#pathGlowR1)" strokeLinecap="round" />
      <path d={pathD} fill="none" stroke="rgba(255,255,255,0.10)" strokeWidth={2.5}
        strokeDasharray="10 7" strokeLinecap="round" />
      {C1_ISLANDS.slice(0, -1).map((island, i) => {
        const next = C1_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedC1(progress, testId);
        const done = isCheckpointDoneC1(progress, testId);
      ...
      {C1_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedC1(progress, island.id);
        const done = isIslandDoneC1(progress, island.id);
        const total = islandTotalStarsC1(progress, island.id);
        return (
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneC1(progress, island.id, m.id);
                  return (
...
export default function AstroRomanaC1Page() {
  const { lang } = useLang();
  ...
  const avatarIsland = C1_ISLANDS.find((i) => i.id === avatarIslandId) ?? C1_ISLANDS[0];
  ...
  useEffect(() => {
    const p = loadC1Progress();
    setProgress(p);
    const lastDone = [...C1_ISLANDS].reverse().find((i) => p.completedIslands.includes(i.id));
  ...
  const noQuestionsTypes = new Set(["romana-explore"]);
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (!noQuestionsTypes.has(mission.gameType)) {
      ...
      const qs = generateIslandQuestionsC1(activeIsland, lang as Lang, qCount);
      setQuestions(qs);
    }
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeMissionC1(progress, activeIsland.id, activeMission.id, stars);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsC1(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsC1(activeTestId, lang as Lang, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeTestC1(progress, activeTestId);
    saveC1Progress(newProgress);
    setProgress(newProgress);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{C1_LABEL[lang] ?? C1_LABEL.ro}</p>
          </div>
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsC1(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneC1(progress, activeIsland.id, mission.id);
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "romana-explore" && activeIsland && (
          <C1Explorer island={activeIsland} grade={1} color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
      </div>
    </div>
  );

  if (["orbit-quiz", "black-hole", "star-match", "romana-explore"].includes(screen)) return (
    <>
      {gameScreen}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsC1: Record<string, Record<string, string>> = {
    ...
    const topicDesc = (testTopicsC1[lang] ?? testTopicsC1.ro)[activeTestId] ?? "";
    return (
