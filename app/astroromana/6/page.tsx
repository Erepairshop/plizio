...
import {
  C6_ISLANDS as K6_ISLANDS, C6_CHECKPOINT_MAP as K6_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadC6Progress as loadK6Progress, saveC6Progress as saveK6Progress,
  type RomanaProgress as BioK6Progress,
  isMissionDoneC6 as isMissionDoneK6, isIslandDoneC6 as isIslandDoneK6,
  isIslandUnlockedC6 as isIslandUnlockedK6,
  isCheckpointUnlockedC6 as isCheckpointUnlockedK6, isCheckpointDoneC6 as isCheckpointDoneK6,
  completeMissionC6 as completeMissionK6, completeTestC6 as completeTestK6,
  islandTotalStarsC6 as islandTotalStarsK6,
  generateIslandQuestionsC6 as generateIslandQuestionsK6, generateCheckpointQuestionsC6 as generateCheckpointQuestionsK6,
} from "@/lib/astroRomana6";
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowRomana6" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowRomana6" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1romana6" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF2D78" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#FF2D78" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2romana6" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1romana6)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2romana6)" />

      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.25)" strokeWidth={8}
        filter="url(#pathGlowRomana6)" strokeLinecap="round" />
...
export default function AstroRomanaK6Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsK6(activeIsland!, lang as any, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsK6(testId, lang as any, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsK6(activeTestId, lang as any, 15);
    setQuestions(qs);
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "romana-explore" && activeIsland && (
          <C6Explorer island={activeIsland} grade={6} color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
