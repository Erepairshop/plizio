...
import {
  ...
  isIslandDoneC2,
  isIslandUnlockedC2,
  isCheckpointUnlockedC2,
  isCheckpointDoneC2,
  completeMissionC2,
  islandTotalStarsC2,
  completeTestC2,
  generateIslandQuestionsC2,
  generateCheckpointQuestionsC2,
} from "@/lib/astroRomana2";
...
type Screen =
  ...
  | "orbit-quiz" | "star-match" | "black-hole" | "gravity-sort" | "speed-round"
  | "romana-explore"
...
function buildSmoothPath(islands: typeof C2_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  const { lang } = useLang();
  const pathD = buildSmoothPath(C2_ISLANDS);
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowR2" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowR2" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1r2" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#6366F1" stopOpacity="0.10" />
          <stop offset="100%" stopColor="#6366F1" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2r2" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.08" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>
      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1r2)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2r2)" />
      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.22)" strokeWidth={8}
        filter="url(#pathGlowR2)" strokeLinecap="round" />
      <path d={pathD} fill="none" stroke="rgba(255,255,255,0.10)" strokeWidth={2.5}
        strokeDasharray="10 7" strokeLinecap="round" />
      {C2_ISLANDS.slice(0, -1).map((island, i) => {
        const next = C2_ISLANDS[i + 1];
      ...
      {Object.entries(C2_CHECKPOINT_MAP).map(([testId]) => {
        const pos = CP_POS[testId];
        const unlocked = isCheckpointUnlockedC2(progress, testId);
        const done = isCheckpointDoneC2(progress, testId);
      ...
      {C2_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedC2(progress, island.id);
        const done = isIslandDoneC2(progress, island.id);
        const total = islandTotalStarsC2(progress, island.id);
        return (
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneC2(progress, island.id, m.id);
                  return (
...
export default function AstroRomanaC2Page() {
  const { lang } = useLang();
  ...
  const avatarIsland = C2_ISLANDS.find((i) => i.id === avatarIslandId) ?? C2_ISLANDS[0];
  ...
  useEffect(() => {
    const p = loadC2Progress();
    setProgress(p);
    const lastDone = [...C2_ISLANDS].reverse().find((i) => p.completedIslands.includes(i.id));
  ...
  const noQuestionsTypes = new Set(["romana-explore", "gravity-sort"]);
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (!noQuestionsTypes.has(mission.gameType)) {
      ...
      const qs = generateIslandQuestionsC2(activeIsland, lang as Lang, qCount);
      setQuestions(qs);
    }
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeMissionC2(progress, activeIsland.id, activeMission.id, stars);
  ...
  const handleIslandAnimDone = useCallback(() => {
    ...
    saveCard({ id: generateCardId(), game: "astroromana", rarity, score: missionScore.score, total: missionScore.total, date: new Date().toISOString() });
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsC2(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsC2(activeTestId, lang as Lang, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeTestC2(progress, activeTestId);
    saveC2Progress(newProgress);
    setProgress(newProgress);
    const rarity = calculateRarity(score, total, 0, false);
    saveCard({ id: generateCardId(), game: "astroromana", rarity, score, total, date: new Date().toISOString() });
    window.dispatchEvent(new Event("plizio-cards-changed"));
    incrementTotalGames();
    checkNewMilestones();
    setEarnedCard(rarity);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{C2_LABEL[lang] ?? C2_LABEL.ro}</p>
          </div>
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsC2(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneC2(progress, activeIsland.id, mission.id);
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-black text-white text-base leading-tight">
                      {mission.label[lang as Lang] ?? mission.label.ro}
                    </p>
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "romana-explore" && activeIsland && (
          <C2Explorer island={activeIsland} grade={2} color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
      </div>
    </div>
  );

  if (["orbit-quiz", "black-hole", "star-match", "gravity-sort", "speed-round", "romana-explore"].includes(screen)) return (
    <>
      {gameScreen}
  ...
  if (screen === "reward" && earnedCard) {
    return (
      <>
        <RewardReveal rarity={earnedCard} game="astroromana"
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsC2: Record<string, Record<string, string>> = {
    ...
    const topicDesc = (testTopicsC2[lang] ?? testTopicsC2.ro)[activeTestId] ?? "";
    return (
