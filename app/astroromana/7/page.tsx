...
import {
  C7_ISLANDS as K7_ISLANDS, C7_CHECKPOINT_MAP as K7_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadC7Progress as loadK7Progress, saveC7Progress as saveK7Progress,
  type RomanaProgress as BioK7Progress,
  isMissionDoneC7 as isMissionDoneK7, isIslandDoneC7 as isIslandDoneK7,
  isIslandUnlockedC7 as isIslandUnlockedK7,
  isCheckpointUnlockedC7 as isCheckpointUnlockedK7, isCheckpointDoneC7 as isCheckpointDoneK7,
  completeMissionC7 as completeMissionK7, completeTestC7 as completeTestK7,
  islandTotalStarsC7 as islandTotalStarsK7,
  generateIslandQuestionsC7 as generateIslandQuestionsK7, generateCheckpointQuestionsC7 as generateCheckpointQuestionsK7,
} from "@/lib/astroRomana7";
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowRomana7" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowRomana7" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1romana7" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF2D78" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#FF2D78" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2romana7" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1romana7)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2romana7)" />

      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.25)" strokeWidth={8}
        filter="url(#pathGlowRomana7)" strokeLinecap="round" />
          ...
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            {unlocked && (
              <text x={island.svgX} y={island.svgY + 48} textAnchor="middle" fontSize={9} fontWeight="bold"
                ...
                {total > 0 ? `${total}/9 ⭐` : (island.name.ro ?? island.name.en).split(" ")[0]}
              </text>
            )}
          </g>
        );
...
export default function AstroRomanaK7Page() {
  const { lang } = useLang();
  const router = useRouter();
  const t = T[lang as keyof typeof T] ?? T.en;

  const [screen, setScreen] = useState<Screen>("island-map");
  const [progress, setProgress] = useState<BioK7Progress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (mission.gameType === "m2" || mission.gameType === "m3" || mission.gameType === "romana-explore") {
      setQuestions([]);
    ...
    const qs = generateIslandQuestionsK7(activeIsland!, lang as any, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const handleIslandAnimDone = useCallback(() => {
    ...
    saveCard({ id: generateCardId(), game: "astro-romana", rarity, score: missionScore.score, total: missionScore.total, date: new Date().toISOString() });
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsK7(testId, lang as any, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsK7(activeTestId, lang as any, 15);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    saveCard({ id: generateCardId(), game: "astro-romana", rarity, score, total, date: new Date().toISOString() });
    incrementTotalGames();
    checkNewMilestones();
    setEarnedCard(rarity);
    ...
    return (
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          <button onClick={() => router.push("/astroromana")}
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #FF2D78, #FF9500)" }}
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "romana-explore" && activeIsland && (
          <C7Explorer island={activeIsland} grade={7} color={bgColor} lang={lang} onDone={handleMissionDone} />
        )}
        {screen === "m2" && activeMission?.gameKey && ROMANA_M2_POOLS[activeMission.gameKey] && (
          <M2Engine gameKey={activeMission.gameKey} rounds={ROMANA_M2_POOLS[activeMission.gameKey]} color={bgColor} lang={lang as any} onDone={handleMissionDone} onCorrect={() => { setAvatarMood("happy"); setJumpTrigger({ reaction: "happy", timestamp: Date.now() }); }} onWrong={() => setAvatarMood("disappointed")} />
        )}
        {screen === "m3" && activeMission?.gameKey && ROMANA_M3_POOLS[activeMission.gameKey] && (
          <M3Engine gameKey={activeMission.gameKey} rounds={ROMANA_M3_POOLS[activeMission.gameKey]} color={bgColor} lang={lang as any} onDone={handleMissionDone} onCorrect={() => { setAvatarMood("happy"); setJumpTrigger({ reaction: "happy", timestamp: Date.now() }); }} onWrong={() => setAvatarMood("disappointed")} />
        )}
  ...
  if (screen === "reward" && earnedCard) {
    return (
      <>
        <RewardReveal rarity={earnedCard} game="astro-romana"
