...
const GRADE_LABEL: Record<string, string> = {
  en: "Grade 8 · Math Space",
  hu: "8. osztály · Matek Űrkaland",
  de: "Klasse 8 · Mathe-Weltraum",
  ro: "Clasa 8 · Matematică în Spațiu",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  const pathD = buildSmoothPath(G8_ISLANDS);
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      ...
      {G8_ISLANDS.map((island) => {
        const unlocked = isIslandUnlockedG8(progress, island.id);
        const done = isIslandDoneG8(progress, island.id);
        const stars = islandTotalStarsG8(progress, island.id);
        const SvgIcon = G8_ISLAND_SVGS[island.id];
      ...
      {Object.entries(checkpointPos).map(([id, pos]) => {
        const unlocked = isCheckpointUnlockedG8(progress, id);
        const done = isCheckpointDoneG8(progress, id);
        return (
...
export default function AstroMathG8Page() {
  ...
  const avatarIsland = G8_ISLANDS.find(i => i.id === avatarIslandId) ?? G8_ISLANDS[0];
  ...
  useEffect(() => {
    const p = loadG8Progress();
    setProgress(p);
    const lastDone = [...G8_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = G8_ISLANDS.find(i => i.id === id);
  ...
  const startMission = (mission: MathMissionConfig) => {
    ...
    const qs = generateIslandQuestionsG8(activeIsland, lang as Lang);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  };
  ...
  const handleMissionDone = (score: number, total: number) => {
    ...
    const wasIslandDone = isIslandDoneG8(progress, activeIsland.id);
    const newProgress = completeMissionG8(progress, activeIsland.id, activeMission.id, stars);
    const isNowIslandDone = isIslandDoneG8(newProgress, activeIsland.id);

    setJustUnlockedIsland(!wasIslandDone && isNowIslandDone);
    saveG8Progress(newProgress);
  ...
  const startCheckpoint = (testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsG8(testId, lang as Lang);
    setQuestions(qs);
  ...
  const handleCheckpointDone = (score: number, total: number) => {
    ...
    const newProgress = completeTestG8(progress, activeTestId);
    saveG8Progress(newProgress);
    setProgress(newProgress);
  ...
  if (screen === "island-map") {
    const totalDone = G8_ISLANDS.filter(i => isIslandDoneG8(progress, i.id)).length;
    return (
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="w-9 h-9 flex items-center justify-center rounded-full bg-white/10 text-white/60 text-xs font-bold">{totalDone}/{G8_ISLANDS.length}</div>
        </div>
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #FF9500, #EC4899)" }} initial={{ width: 0 }} animate={{ width: `${(totalDone / G8_ISLANDS.length) * 100}%` }} transition={{ duration: 0.8 }} />
          </div>
        </div>
        ...
        <VisualLab subject="astromath" grade={8} lang={lang as any} open={visualLabOpen} onClose={() => setVisualLabOpen(false)} />
      </div>
    );
  }
  ...
  const gameContent = (
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}18 0%, #060614 55%)` }}>
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "gravity-sort" && <GravitySort sortRange={activeIsland?.sortRange || [0, 100]} color={bgColor} onDone={handleMissionDone} generateRound={activeIsland?.id === "i1" ? generateSqrtSortRound : undefined} />}
        ...
        {screen === "stat-explorer" && <StatExplorer8 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        ...
        {screen === "equation-explorer" && <EquationExplorer8 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "sqrt-explorer" && <SqrtExplorer8 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "quadratic-explorer" && <QuadraticExplorer8 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "system-explorer" && <SystemExplorer8 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "function-explorer" && <FunctionExplorer8 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "probability-explorer" && <ProbabilityExplorer8 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "transform-explorer" && <TransformExplorer8 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "visual-challenge" && (
          <VisualChallenge color={bgColor} rounds={5} onDone={handleMissionDone} renderVisual={(l, onAns) => {
            if (activeMission?.visualType === "cylinder-surface") return <CylinderSurface lang={l as any} onAnswer={onAns} />;
            if (activeMission?.visualType === "transformation") return <TransformationGrid lang={l as any} onAnswer={onAns} />;
            return null;
          }} />
        )}
  ...
  const gameScreens = ["orbit-quiz", "black-hole", "gravity-sort", "star-match", "number-duel", "speed-round", "equation-drill", "fraction-visual", "stat-explorer", "true-false-blitz", "equation-explorer", "sqrt-explorer", "quadratic-explorer", "system-explorer", "function-explorer", "probability-explorer", "transform-explorer", "visual-challenge", "m2", "m3"];
  if (gameScreens.includes(screen)) return gameContent;

  if (screen === "rocket-launch" && activeTestId) {
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(232,121,249,0.18) 0%, #060614 55%)" }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
          <RocketLaunch questions={questions} color="#E879F9" onDone={() => setScreen("checkpoint-intro")} />
        </div>
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    ...
    const topicDesc = testTopicsG8[lang as string]?.[activeTestId] || "";
    return (
      <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden px-5 gap-6" style={{ background: "radial-gradient(ellipse at 50% 30%, rgba(255,215,0,0.12) 0%, #060614 60%)" }}>
        ...
        <button onClick={() => { const qs = generateCheckpointQuestionsG8(activeTestId, lang as Lang); setQuestions(qs); setScreen("checkpoint-quiz"); }} className="w-full py-4 rounded-2xl font-black text-black" style={{ background: "linear-gradient(135deg, #FFD700, #FFA500)" }}>{t.startTest} 🚀</button></div>
      </div>
    );
  }
