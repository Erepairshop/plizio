...
import {
  G8_ISLANDS, G8_CHECKPOINT_MAP, type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadG8Progress, saveG8Progress, type G8Progress,
  isMissionDoneG8, isIslandDoneG8, isIslandUnlockedG8,
...
// ─── Grade 8 labels ────────────────────────────────────────────────────────────
const G8_LABEL: Record<string, string> = {
  en: "Grade 8 · Space Adventure",
  hu: "8. osztály · Űrkaland",
  de: "Klasse 8 · Weltraumabenteuer",
  ro: "Clasa 8 · Aventură spațială",
};
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <div className="absolute" style={{ left: "55%", top: "55%", width: 160, height: 160,
        background: "radial-gradient(ellipse, rgba(232,140,249,0.06) 0%, transparent 70%)", borderRadius: "50%" }} />
    </div>
  );
}
...
function buildSmoothPath(islands: typeof G8_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowG8" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowG8" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1g8" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#E879F9" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#E879F9" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2g8" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#A855F7" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#A855F7" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1g8)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2g8)" />

      <path d={pathD} fill="none" stroke="rgba(232,121,249,0.25)" strokeWidth={8}
        filter="url(#pathGlowG8)" strokeLinecap="round" />
      <path d={pathD} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth={2.5}
        strokeDasharray="10 7" strokeLinecap="round" />
      {G8_ISLANDS.slice(0, -1).map((island, i) => {
        const next = G8_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedG8(progress, testId);
        const done = isCheckpointDoneG8(progress, testId);
          ...
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            {unlocked ? (
              <foreignObject x={island.svgX - 30} y={island.svgY - 30} width={60} height={60}>
                {G8_ISLAND_SVGS[island.id] ? React.createElement(G8_ISLAND_SVGS[island.id], { size: 60 }) : <span style={{ fontSize: 20 }}>{island.icon}</span>}
              </foreignObject>
            ) : (
              <>
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneG8(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
export default function AstroMathG8Page() {
  ...
  const [progress, setProgress] = useState<G8Progress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const noQuestionsTypes: string[] = [
      "number-duel", "gravity-sort", "fraction-visual",
    ...
    const qs = generateIslandQuestionsG8(activeIsland, lang as Lang, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsG8(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsG8(activeTestId, lang as Lang, 10);
    setQuestions(qs);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{G8_LABEL[lang] ?? G8_LABEL.en}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #E879F9, #A855F7)" }}
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsG8(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneG8(progress, activeIsland.id, mission.id);
  ...
  const ALL_GAME_SCREENS: string[] = [
    "orbit-quiz", "black-hole", "gravity-sort", "star-match", "number-duel",
    "speed-round", "equation-drill", "fraction-visual",
    "algebra-explorer", "stat-explorer", "true-false-blitz",
  ...
  if (screen === "rocket-launch" && activeTestId) {
    return (
      <>
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: "radial-gradient(ellipse at 50% 0%, rgba(232,121,249,0.18) 0%, #060614 55%)" }}>
        <Starfield />
        <div className="relative z-10 flex items-center gap-3 px-4 pt-5 pb-3">
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    ...
    const topicDesc = (testTopicsG8[lang] ?? testTopicsG8.en)[activeTestId] ?? "";
    return (
