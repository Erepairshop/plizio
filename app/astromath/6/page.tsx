...
import {
  G6_ISLANDS, G6_CHECKPOINT_MAP, type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadG6Progress, saveG6Progress, type G6Progress,
  isMissionDoneG6, isIslandDoneG6, isIslandUnlockedG6,
...
// ─── Grade 6 labels ────────────────────────────────────────────────────────────
const G6_LABEL: Record<string, string> = {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <div className="absolute" style={{ left: "10%", top: "20%", width: 200, height: 200,
        background: "radial-gradient(ellipse, rgba(255,149,0,0.07) 0%, transparent 70%)", borderRadius: "50%" }} />
      <div className="absolute" style={{ left: "55%", top: "55%", width: 160, height: 160,
        background: "radial-gradient(ellipse, rgba(255,100,0,0.06) 0%, transparent 70%)", borderRadius: "50%" }} />
    </div>
  );
}
...
function buildSmoothPath(islands: typeof G6_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowG6" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowG6" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1g6" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF9500" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#FF9500" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2g6" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#EC4899" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#EC4899" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1g6)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2g6)" />

      <path d={pathD} fill="none" stroke="rgba(255,149,0,0.25)" strokeWidth={8}
        filter="url(#pathGlowG6)" strokeLinecap="round" />
      <path d={pathD} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth={2.5}
        strokeDasharray="10 7" strokeLinecap="round" />
      {G6_ISLANDS.slice(0, -1).map((island, i) => {
        const next = G6_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedG6(progress, testId);
        const done = isCheckpointDoneG6(progress, testId);
          ...
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            {unlocked ? (
              <foreignObject x={island.svgX - 30} y={island.svgY - 30} width={60} height={60}>
                {G6_ISLAND_SVGS[island.id] ? React.createElement(G6_ISLAND_SVGS[island.id], { size: 60 }) : <span style={{ fontSize: 20 }}>{island.icon}</span>}
              </foreignObject>
            ) : (
              <>
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneG6(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
export default function AstroMathG6Page() {
  ...
  const [progress, setProgress] = useState<G6Progress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsG6(activeIsland, lang as Lang, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsG6(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsG6(activeTestId, lang as Lang, 10);
    setQuestions(qs);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{G6_LABEL[lang] ?? G6_LABEL.en}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #FF9500, #EC4899)" }}
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsG6(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneG6(progress, activeIsland.id, mission.id);
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    ...
    const topicDesc = (testTopicsG6[lang] ?? testTopicsG6.en)[activeTestId] ?? "";
    return (
