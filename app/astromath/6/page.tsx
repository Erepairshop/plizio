...
import {
  G6_ISLANDS, loadG6Progress, saveG6Progress,
...
const GRADE_LABEL: Record<string, string> = {
  en: "Grade 6 · Math Space",
  hu: "6. osztály · Matek Űrkaland",
  de: "Klasse 6 · Mathe-Weltraum",
  ro: "Clasa 6 · Matematică în Spațiu",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  const pathD = buildSmoothPath(G6_ISLANDS);
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      ...
      {G6_ISLANDS.map((island) => {
        const unlocked = isIslandUnlockedG6(progress, island.id);
        const done = isIslandDoneG6(progress, island.id);
        const stars = islandTotalStarsG6(progress, island.id);
        const SvgIcon = G6_ISLAND_SVGS[island.id];
      ...
      {Object.entries(checkpointPos).map(([id, pos]) => {
        const unlocked = isCheckpointUnlockedG6(progress, id);
        const done = isCheckpointDoneG6(progress, id);
        return (
...
export default function AstroMathG6Page() {
  ...
  const avatarIsland = G6_ISLANDS.find(i => i.id === avatarIslandId) ?? G6_ISLANDS[0];
  ...
  useEffect(() => {
    const p = loadG6Progress();
    setProgress(p);
    const lastDone = [...G6_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = G6_ISLANDS.find(i => i.id === id);
  ...
  const startMission = (mission: MathMissionConfig) => {
    ...
    const qs = generateIslandQuestionsG6(activeIsland, lang as Lang);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  };
  ...
  const handleMissionDone = (score: number, total: number) => {
    ...
    const wasIslandDone = isIslandDoneG6(progress, activeIsland.id);
    const newProgress = completeMissionG6(progress, activeIsland.id, activeMission.id, stars);
    const isNowIslandDone = isIslandDoneG6(newProgress, activeIsland.id);

    setJustUnlockedIsland(!wasIslandDone && isNowIslandDone);
    saveG6Progress(newProgress);
  ...
  const startCheckpoint = (testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsG6(testId, lang as Lang);
    setQuestions(qs);
  ...
  const handleCheckpointDone = (score: number, total: number) => {
    ...
    const newProgress = completeTestG6(progress, activeTestId);
    saveG6Progress(newProgress);
    setProgress(newProgress);
  ...
  if (screen === "island-map") {
    const totalDone = G6_ISLANDS.filter(i => isIslandDoneG6(progress, i.id)).length;
    return (
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="w-9 h-9 flex items-center justify-center rounded-full bg-white/10 text-white/60 text-xs font-bold">{totalDone}/{G6_ISLANDS.length}</div>
        </div>
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #FF9500, #EC4899)" }} initial={{ width: 0 }} animate={{ width: `${(totalDone / G6_ISLANDS.length) * 100}%` }} transition={{ duration: 0.8 }} />
          </div>
        </div>
        ...
        <VisualLab subject="astromath" grade={6} lang={lang as any} open={visualLabOpen} onClose={() => setVisualLabOpen(false)} />
      </div>
    );
  }
  ...
  const gameContent = (
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}18 0%, #060614 55%)` }}>
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "gravity-sort" && <GravitySort sortRange={activeIsland?.sortRange || [0, 100]} color={bgColor} onDone={handleMissionDone} />}
        ...
        {screen === "fraction-pro-explorer" && <FractionProExplorer6 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "concept-explorer" && <ConceptExplorer color={bgColor} lang={lang as Lang} grade={6} onDone={handleMissionDone} />}
        {screen === "word-problem-explorer" && <WordProblemExplorer color={bgColor} lang={lang as Lang} grade={6} onDone={handleMissionDone} />}
        {screen === "area-explorer" && <GeometryAreaExplorer6 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        ...
        {screen === "negative-number-line" && <NegativeNumbersExplorer6 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "percent-bar" && <PercentExplorer6 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "ratio-explorer" && <RatioExplorer6 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "algebra-explorer" && <AlgebraExplorer6 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "stat-explorer" && <DataExplorer6 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "visual-challenge" && (
          <VisualChallenge color={bgColor} rounds={5} onDone={handleMissionDone} renderVisual={(l, onAns) => {
            if (activeMission?.visualType === "trapezoid-area") return <TrapezoidAreaCalc language={l as any} onAnswer={onAns} />;
            if (activeMission?.visualType === "pie-chart") return <PieChartRead language={l as any} onAnswer={onAns} />;
            return null;
          }} />
        )}
  ...
  const gameScreens = ["orbit-quiz", "black-hole", "gravity-sort", "star-match", "number-duel", "speed-round", "equation-drill", "fraction-visual", "fraction-pro-explorer", "concept-explorer", "word-problem-explorer", "area-explorer", "true-false-blitz", "negative-number-line", "percent-bar", "ratio-explorer", "algebra-explorer", "stat-explorer", "visual-challenge", "m2", "m3"];
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    ...
    const topicDesc = testTopicsG6[lang as string]?.[activeTestId] || "";
    return (
      <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden px-5 gap-6" style={{ background: "radial-gradient(ellipse at 50% 30%, rgba(255,215,0,0.12) 0%, #060614 60%)" }}>
        ...
        <button onClick={() => { const qs = generateCheckpointQuestionsG6(activeTestId, lang as Lang); setQuestions(qs); setScreen("checkpoint-quiz"); }} className="w-full py-4 rounded-2xl font-black text-black" style={{ background: "linear-gradient(135deg, #FFD700, #FFA500)" }}>{t.startTest} 🚀</button></div>
      </div>
    );
  }
