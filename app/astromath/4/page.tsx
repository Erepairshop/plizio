...
import {
  G4_ISLANDS, G4_CHECKPOINT_MAP, type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadG4Progress, saveG4Progress, type G4Progress,
  isMissionDoneG4, isIslandDoneG4, isIslandUnlockedG4,
...
// ─── Grade 4 labels ────────────────────────────────────────────────────────────
const G4_LABEL: Record<string, string> = {
  en: "Grade 4 · Space Adventure",
  hu: "4. osztály · Ûrkaland",
  de: "Klasse 4 · Weltraumabenteuer",
  ro: "Clasa 4 · Aventură spațială",
};
...
function buildSmoothPath(islands: typeof G4_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowG4" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowG4" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1g4" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#6420C8" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#6420C8" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2g4" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1g4)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2g4)" />

      <path d={pathD} fill="none" stroke="rgba(150,100,255,0.25)" strokeWidth={8}
        filter="url(#pathGlowG4)" strokeLinecap="round" />
      <path d={pathD} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth={2.5}
        strokeDasharray="10 7" strokeLinecap="round" />
      {G4_ISLANDS.slice(0, -1).map((island, i) => {
        const next = G4_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedG4(progress, testId);
        const done = isCheckpointDoneG4(progress, testId);
      ...
      {G4_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedG4(progress, island.id);
        const done = isIslandDoneG4(progress, island.id);
        const total = islandTotalStarsG4(progress, island.id);
            ...
            {unlocked ? (
              <foreignObject x={island.svgX - 30} y={island.svgY - 30} width={60} height={60}
                ...
                <div style={{ width: 60, height: 60, opacity: done ? 0.85 : 1 }}>
                  {G4_ISLAND_SVGS[island.id] ? React.createElement(G4_ISLAND_SVGS[island.id], { size: 60 }) : <span style={{ fontSize: 20 }}>{island.icon}</span>}
                </div>
              </foreignObject>
            ) : (
              <>
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneG4(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
export default function AstroMathG4Page() {
  ...
  const [progress, setProgress] = useState<G4Progress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const avatarIsland = G4_ISLANDS.find(i => i.id === avatarIslandId) ?? G4_ISLANDS[0];
  ...
  useEffect(() => {
    const p = loadG4Progress();
    setProgress(p);
    const lastDone = [...G4_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsG4(activeIsland, lang as Lang, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeMissionG4(progress, activeIsland.id, activeMission.id, stars);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsG4(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsG4(activeTestId, lang as Lang, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeTestG4(progress, activeTestId);
    saveG4Progress(newProgress);
    setProgress(newProgress);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{G4_LABEL[lang] ?? G4_LABEL.en}</p>
          </div>
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsG4(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneG4(progress, activeIsland.id, mission.id);
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    ...
    const topicDesc = (testTopicsG4[lang] ?? testTopicsG4.en)[activeTestId] ?? "";
    return (
