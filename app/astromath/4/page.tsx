...
import {
  G4_ISLANDS, G4_CHECKPOINT_MAP, type IslandDef, type MissionDef, type Lang,
        ...
        return (
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            <circle cx={island.svgX} cy={island.svgY} r={24}
              ...
              filter={unlocked ? "url(#islandGlowG4)" : undefined}
  ...
  if (screen === "mission-select" && activeIsland) {
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        ...
        {(() => {
          const total = islandTotalStarsG4(progress, activeIsland.id);
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-3 pb-6">
          {activeIsland.missions.map((mission, idx) => {
            ...
            const prevDone = idx === 0 || isMissionDoneG4(progress, activeIsland.id, activeIsland.missions[idx - 1].id);
