...
import {
  G5_ISLANDS, G5_CHECKPOINT_MAP, type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadG5Progress, saveG5Progress, type G5Progress,
  isMissionDoneG5, isIslandDoneG5, isIslandUnlockedG5,
...
// ─── Grade 5 labels ────────────────────────────────────────────────────────────
const G5_LABEL: Record<string, string> = {
  en: "Grade 5 · Space Adventure",
  hu: "5. osztály · Űrkaland",
  de: "Klasse 5 · Weltraumabenteuer",
  ro: "Clasa 5 · Aventură spațială",
};
...
function buildSmoothPath(islands: typeof G5_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowG5" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowG5" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1g5" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#B44DFF" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#B44DFF" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2g5" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#14B8A6" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#14B8A6" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1g5)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2g5)" />

      <path d={pathD} fill="none" stroke="rgba(180,77,255,0.25)" strokeWidth={8}
        filter="url(#pathGlowG5)" strokeLinecap="round" />
      <path d={pathD} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth={2.5}
        strokeDasharray="10 7" strokeLinecap="round" />
      {G5_ISLANDS.slice(0, -1).map((island, i) => {
        const next = G5_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedG5(progress, testId);
        const done = isCheckpointDoneG5(progress, testId);
          ...
          <g key={island.id} onClick={() => unlocked && onIsland(island)}
            ...
            {unlocked ? (
              <foreignObject x={island.svgX - 30} y={island.svgY - 30} width={60} height={60}>
                {G5_ISLAND_SVGS[island.id] ? React.createElement(G5_ISLAND_SVGS[island.id], { size: 60 }) : <span style={{ fontSize: 20 }}>{island.icon}</span>}
              </foreignObject>
            ) : (
              <>
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneG5(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
export default function AstroMathG5Page() {
  ...
  const [progress, setProgress] = useState<G5Progress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsG5(activeIsland, lang as Lang, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsG5(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsG5(activeTestId, lang as Lang, 10);
    setQuestions(qs);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{G5_LABEL[lang] ?? G5_LABEL.en}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #B44DFF, #14B8A6)" }}
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsG5(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneG5(progress, activeIsland.id, mission.id);
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    ...
    const topicDesc = (testTopicsG5[lang] ?? testTopicsG5.en)[activeTestId] ?? "";
    return (
