...
const GRADE_LABEL: Record<string, string> = {
  en: "Grade 5 · Math Space",
  hu: "5. osztály · Matek Űrkaland",
  de: "Klasse 5 · Mathe-Weltraum",
  ro: "Clasa 5 · Matematică în Spațiu",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  const pathD = buildSmoothPath(G5_ISLANDS);
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      ...
      {G5_ISLANDS.map((island) => {
        const unlocked = isIslandUnlockedG5(progress, island.id);
        const done = isIslandDoneG5(progress, island.id);
        const stars = islandTotalStarsG5(progress, island.id);
        const SvgIcon = G5_ISLAND_SVGS[island.id];
      ...
      {Object.entries(checkpointPos).map(([id, pos]) => {
        const unlocked = isCheckpointUnlockedG5(progress, id);
        const done = isCheckpointDoneG5(progress, id);
        return (
...
export default function AstroMathG5Page() {
  ...
  const avatarIsland = G5_ISLANDS.find(i => i.id === avatarIslandId) ?? G5_ISLANDS[0];
  ...
  useEffect(() => {
    const p = loadG5Progress();
    setProgress(p);
    const lastDone = [...G5_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = G5_ISLANDS.find(i => i.id === id);
  ...
  const startMission = (mission: MathMissionConfig) => {
    ...
    const qs = generateIslandQuestionsG5(activeIsland, lang as Lang);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  };
  ...
  const handleMissionDone = (score: number, total: number) => {
    ...
    const wasIslandDone = isIslandDoneG5(progress, activeIsland.id);
    const newProgress = completeMissionG5(progress, activeIsland.id, activeMission.id, stars);
    const isNowIslandDone = isIslandDoneG5(newProgress, activeIsland.id);

    setJustUnlockedIsland(!wasIslandDone && isNowIslandDone);
    saveG5Progress(newProgress);
  ...
  const startCheckpoint = (testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsG5(testId, lang as Lang);
    setQuestions(qs);
  ...
  const handleCheckpointDone = (score: number, total: number) => {
    ...
    const newProgress = completeTestG5(progress, activeTestId);
    saveG5Progress(newProgress);
    setProgress(newProgress);
  ...
  if (screen === "island-map") {
    const totalDone = G5_ISLANDS.filter(i => isIslandDoneG5(progress, i.id)).length;
    return (
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="w-9 h-9 flex items-center justify-center rounded-full bg-white/10 text-white/60 text-xs font-bold">{totalDone}/{G5_ISLANDS.length}</div>
        </div>
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #FF9500, #EC4899)" }} initial={{ width: 0 }} animate={{ width: `${(totalDone / G5_ISLANDS.length) * 100}%` }} transition={{ duration: 0.8 }} />
          </div>
        </div>
  ...
  const gameContent = (
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}18 0%, #060614 55%)` }}>
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "concept-explorer" && <MulDivExplorer5 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "place-value-explorer" && <PlaceValueExplorer5 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "decimal-explorer" && <DecimalExplorer5 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "word-problem-explorer" && <WordProblemExplorer5 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "unit-explorer" && <UnitExplorer5 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "angle-explorer" && <GeometryExplorer5 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        ...
        {screen === "chain-calc" && <ChainCalc topicKeys={activeIsland?.topicKeys || []} color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "addsub-explorer" && <AddSubExplorer5 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
        {screen === "fraction-explorer-5" && <FractionExplorer5 color={bgColor} lang={lang as Lang} onDone={handleMissionDone} />}
  ...
  const gameScreens = ["orbit-quiz", "black-hole", "gravity-sort", "star-match", "number-duel", "speed-round", "equation-drill", "fraction-visual", "concept-explorer", "place-value-explorer", "decimal-explorer", "word-problem-explorer", "unit-explorer", "angle-explorer", "true-false-blitz", "chain-calc", "addsub-explorer", "fraction-explorer-5", "m2", "m3"];
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    ...
    const topicDesc = testTopicsG5[lang as string]?.[activeTestId] || "";
    return (
      <div className="min-h-screen flex flex-col items-center justify-center relative overflow-hidden px-5 gap-6" style={{ background: "radial-gradient(ellipse at 50% 30%, rgba(255,215,0,0.12) 0%, #060614 60%)" }}>
        ...
        <button onClick={() => { const qs = generateCheckpointQuestionsG5(activeTestId, lang as Lang); setQuestions(qs); setScreen("checkpoint-quiz"); }} className="w-full py-4 rounded-2xl font-black text-black" style={{ background: "linear-gradient(135deg, #FFD700, #FFA500)" }}>{t.startTest} 🚀</button></div>
      </div>
    );
  }
