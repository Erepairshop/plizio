...
import {
  G3_ISLANDS, G3_CHECKPOINT_MAP, type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadG3Progress, saveG3Progress, type G3Progress,
  isMissionDoneG3, isIslandDoneG3, isIslandUnlockedG3,
...
// ─── Grade 3 labels ────────────────────────────────────────────────────────────
const G3_LABEL: Record<string, string> = {
  en: "Grade 3 · Space Adventure",
  hu: "3. osztály · Űrkaland",
  de: "Klasse 3 · Weltraumabenteuer",
  ro: "Clasa 3 · Aventură spațială",
};
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <div className="absolute" style={{ left: "10%", top: "20%", width: 200, height: 200,
        background: "radial-gradient(ellipse, rgba(200,20,30,0.07) 0%, transparent 70%)", borderRadius: "50%" }} />
      <div className="absolute" style={{ left: "55%", top: "55%", width: 160, height: 160,
        background: "radial-gradient(ellipse, rgba(255,100,50,0.06) 0%, transparent 70%)", borderRadius: "50%" }} />
    </div>
  );
}
...
function buildSmoothPath(islands: typeof G3_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  const pathD = buildSmoothPath(G3_ISLANDS);
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        ...
        <radialGradient id="nebula1g3" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#C1121F" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#C1121F" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2g3" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#FF6B35" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#FF6B35" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1g3)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2g3)" />

      <path d={pathD} fill="none" stroke="rgba(200,50,50,0.25)" strokeWidth={8}
      ...
      {G3_ISLANDS.slice(0, -1).map((island, i) => {
        const next = G3_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedG3(progress, testId);
        const done = isCheckpointDoneG3(progress, testId);
      ...
      {G3_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedG3(progress, island.id);
        const done = isIslandDoneG3(progress, island.id);
        const total = islandTotalStarsG3(progress, island.id);
            ...
            {unlocked ? (
              <foreignObject x={island.svgX - 30} y={island.svgY - 30} width={60} height={60}
                ...
                <div style={{ width: 60, height: 60, opacity: done ? 0.85 : 1 }}>
                  {G3_ISLAND_SVGS[island.id] ? React.createElement(G3_ISLAND_SVGS[island.id], { size: 60 }) : <span style={{ fontSize: 20 }}>{island.icon}</span>}
                </div>
              </foreignObject>
            ) : (
              <>
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneG3(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
export default function AstroMathG3Page() {
  ...
  const [progress, setProgress] = useState<G3Progress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const avatarIsland = G3_ISLANDS.find(i => i.id === avatarIslandId) ?? G3_ISLANDS[0];
  ...
  useEffect(() => {
    const p = loadG3Progress();
    setProgress(p);
    const lastDone = [...G3_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const noQuestionsTypes: string[] = ["number-duel", "gravity-sort", "concept-explorer", "place-value-explorer", "division-explorer", "area-explorer", "word-problem-explorer", "unit-explorer", "true-false-blitz", "chain-calc"];
    ...
    const qs = generateIslandQuestionsG3(activeIsland, lang as Lang, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland, lang]);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeMissionG3(progress, activeIsland.id, activeMission.id, stars);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsG3(testId, lang as Lang, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsG3(activeTestId, lang as Lang, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeTestG3(progress, activeTestId);
    saveG3Progress(newProgress);
    setProgress(newProgress);
  ...
  const bgColor = activeIsland?.color ?? "#C1121F";
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{G3_LABEL[lang] ?? G3_LABEL.en}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #C1121F, #FF6B35)" }}
  ...
  // ─── MISSION SELECT — 3 category cards ────────────────────────────────────────
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsG3(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneG3(progress, activeIsland.id, mission.id);
  ...
  if (["orbit-quiz", "black-hole", "gravity-sort", "star-match", "number-duel", "speed-round", "equation-drill", "concept-explorer", "place-value-explorer", "division-explorer", "area-explorer", "word-problem-explorer", "unit-explorer", "true-false-blitz", "chain-calc"].includes(screen)) return (
    <>
      {gameScreen}
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    ...
    const topicDesc = (testTopicsG3[lang] ?? testTopicsG3.en)[activeTestId] ?? "";
    return (
