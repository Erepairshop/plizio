...
function getTaskConfig(topicKey: string): TaskConfig {
  ...
  // "betuk/..." → SzófajSorter
  if (topic === "betuk") {
    return { Component: SzófajSorter, generator: () => genSzófajSorter() };
  }
  ...
  if (topic === "szavak" && ["toldalek", "to_toldalek"].includes(subtopic)) {
    return { Component: ToldalékVálasztó, generator: () => genToldalékVálasztó() };
  }

  // "szavak/szocsaladok" → SzócsaládFa
  if (topic === "szavak" && subtopic === "szocsaladok") {
    return { Component: SzócsaládFa, generator: () => genSzócsaládFa() };
  }

  // "mondatok/..." | "szavak/wortstellung" → MondatRendezés
  if (topic === "mondatok" || (topic === "szavak" && subtopic === "wortstellung")) {
    return { Component: MondatRendezés, generator: () => genMondatRendezés() };
  }

  // "mondatok/mondatreszek" → MondatrészJelölés
  if (topic === "mondatok" && subtopic === "mondatreszek") {
    return { Component: MondatrészJelölés, generator: () => genMondatrészJelölés() };
  }

  // "helyesiras/..." | "hibakereses" → HibaKeresés
  if (topic === "helyesiras" || topic === "hibakereses") {
    return { Component: HibaKeresés, generator: () => genHibaKeresés() };
  }

  // "szokincs/..." → KépFelismerés
  if (topic === "szokincs") {
    return { Component: KépFelismerés, generator: () => genKépFelismerés() };
  }

  // "szovegertés/..." → TörténetRendezés
  if (topic === "szovegertés") {
    return { Component: TörténetRendezés, generator: () => genTörténetRendezés() };
  }

  // default → HiányosSzöveg
  return { Component: HiányosSzöveg, generator: () => genHiányosSzöveg() };
}
...
export default function LangExplore({ island, grade, onDone }: LangExploreProps) {
  ...
  const handleCheck = useCallback(() => {
    // Validate answer (simplified — actual validation depends on component)
    const correct = userAnswer !== null && userAnswer !== undefined && String(userAnswer).trim() !== "";
    ...
    setTimeout(() => {
      ...
      if (taskIndex < 7) {
        ...
        onDone(correct ? correct + 1 : correct, 8);
      }
    }, 1200);
  }, [userAnswer, taskIndex, onDone]);
