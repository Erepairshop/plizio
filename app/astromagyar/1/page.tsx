...
import {
  ...
  completeMissionO1, completeTestO1, islandTotalStarsO1,
...
// ─── Grade 1 labels ────────────────────────────────────────────────────────────
const O1_LABEL: Record<string, string> = {
  en: "Grade 1 · Hungarian Language Galaxy",
  hu: "1. osztály · Magyar Nyelv Galaxis",
  de: "Klasse 1 · Ungarische Sprache Galaxie",
  ro: "Clasa 1 · Galaxia Limba Maghiară",
};
...
function buildSmoothPath(islands: typeof O1_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  const pathD = buildSmoothPath(O1_ISLANDS);

  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowO1" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowO1" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1o1" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF2D78" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#FF2D78" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2o1" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#00D4FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#00D4FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1o1)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2o1)" />

      <path d={pathD} fill="none" stroke="rgba(255,45,120,0.25)" strokeWidth={8}
        filter="url(#pathGlowO1)" strokeLinecap="round" />
      <path d={pathD} fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth={2.5}
        strokeDasharray="10 7" strokeLinecap="round" />
      {O1_ISLANDS.slice(0, -1).map((island, i) => {
        const next = O1_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlocked(progress, O1_CHECKPOINT_MAP, testId);
      ...
      {O1_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedO1(progress, island.id);
        const done = isIslandDone(progress, island.id);
        const total = islandTotalStarsO1(progress, island.id);
...
export default function AstroMagyarO1Page() {
  ...
  const [progress, setProgress] = useState<MagyarProgress>(loadO1Progress());
  ...
  const handleMissionSuccess = useCallback((score: number, total: number) => {
    ...
    if (activeIsland && activeMission) {
      ...
      const updatedProg = completeMissionO1(progress, activeIsland.id, activeMission.id, score >= total * 0.8 ? 3 : score >= total * 0.6 ? 2 : 1);
  ...
  const handleCheckpointSuccess = useCallback((score: number, total: number) => {
    setMissionScore({ score, total });
    const updatedProg = completeTestO1(progress, checkpointId!);
    setProgress(updatedProg);
    saveO1Progress(updatedProg);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-[#0A0A1A] to-slate-950 relative overflow-hidden">
      ...
      {screen === "island-map" && (
        <motion.div className="relative z-30 flex flex-col h-screen"
          ...
          <div className="flex items-center justify-between px-4 py-4 sm:px-6">
            ...
            <h1 className="text-lg sm:text-xl font-black text-white">{O1_LABEL[lang] || O1_LABEL.en}</h1>
            <div className="w-8" />
          </div>
      ...
      {screen === "mission-select" && activeIsland && (() => {
        const totalStars = islandTotalStarsO1(progress, activeIsland.id);
        return (
          <div className="min-h-screen flex flex-col relative overflow-hidden"
            style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
            <Starfield />
      ...
      {screen === "reward" && earnedCard && (
        <RewardReveal rarity={earnedCard} game="astromagyar" score={missionScore.score} total={missionScore.total}
          onDone={() => {
            if (activeIsland && isIslandDone(progress, activeIsland.id)) {
