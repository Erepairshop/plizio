...
import { Trophy, Layers, ArrowLeft, Crosshair, Zap, Brain, Calendar, Mountain, Star, Info, ChevronDown, ChevronUp, GitBranch, Grid3x3, Navigation, Shuffle, Rocket, Timer, Ghost, Radio, type LucideIcon } from "lucide-react";
...
const T: Record<Language, {
  ...
  hu: {
    ...
    rarityRules: {
      legendary: "100% + 3 napos streak · Expedíciós játékok: 10. szint teljesítése",
      gold: "95%+ effektív pontszám — csak szintes játékokban az 5. szinttől. Egyszerű játékokban nem létezik.",
  ...
  de: {
    ...
    rarityRules: {
      legendary: "100% + 3 Tage Streak · Expedition-Spiele: Level 10 abschließen",
      gold: "95%+ effektive Punktzahl — nur in Level-Spielen ab Level 5. In einfachen Spielen nicht verfügbar.",
  ...
  en: {
    ...
    rarityRules: {
      legendary: "100% + 3-day streak · Expedition games: complete level 10",
      gold: "95%+ effective score — only in level-based games from level 5. Not available in simple games.",
  ...
  ro: {
    ...
    rarityRules: {
      legendary: "100% + streak 3 zile · Jocuri expediție: completează nivelul 10",
      gold: "95%+ scor efectiv — doar în jocuri cu niveluri de la nivelul 5. Indisponibil în jocuri simple.",
...
const GAME_NAMES: Record<Language, Record<string, string>> = {
  hu: {
    ...
    milliomos: "Milliomos", racetrack: "Versenyút", citydrive: "Városvezetés", numberpath: "Számút", pathbuilder: "Útvonalépítő", topicsort: "Témarendező", pipeflow: "Csőáramlás", deductiongrid: "Dedukciós rács", deductiongrid2: "Galaktikus Dedukció", timeecho: "Idővisszhang", shadowswitch: "Árnyékváltó", signaldecoder: "Jeldekódoló",
  },
  de: {
    quickpick: "Quick Pick", reflexgrid: "Reflex Grid", memoryflash: "Memory Flash",
    ...
    milliomos: "Milliomos", racetrack: "Rennstrecke", citydrive: "Stadtfahrt", numberpath: "Zahlenpfad", pathbuilder: "Pfadbauer", topicsort: "Themen-Sortierung", pipeflow: "Rohrfluss", deductiongrid: "Deduktionsraster", deductiongrid2: "Galaktische Deduktion", timeecho: "Zeitecho", shadowswitch: "Schattenschalter", signaldecoder: "Signal-Decoder",
  },
  en: {
    quickpick: "Quick Pick", reflexgrid: "Reflex Grid", memoryflash: "Memory Flash",
    ...
    milliomos: "Milliomos", racetrack: "Racetrack", citydrive: "City Drive", numberpath: "Number Path", pathbuilder: "Path Builder", topicsort: "Topic Sort", pipeflow: "Pipe Flow", deductiongrid: "Deduction Grid", deductiongrid2: "Galactic Deduction", timeecho: "Time Echo", shadowswitch: "Shadow Switch", signaldecoder: "Signal Decoder",
  },
  ro: {
    quickpick: "Quick Pick", reflexgrid: "Reflex Grid", memoryflash: "Memory Flash",
    ...
    milliomos: "Milliomos", racetrack: "Circuit", citydrive: "Condus prin oraș", numberpath: "Calea Numerelor", pathbuilder: "Constructor de Trasee", topicsort: "Sortare teme", pipeflow: "Fluxul Conductelor", deductiongrid: "Grilă de deducție", deductiongrid2: "Deducție Galactică", timeecho: "Ecou Temporal", shadowswitch: "Schimb de Umbre", signaldecoder: "Decodor de Semnale",
  },
};
...
const GAME_ICONS: Record<string, LucideIcon> = {
  ...
  daily: Calendar, skyclimb: Mountain, numberpath: Grid3x3, pathbuilder: Navigation, topicsort: Shuffle, pipeflow: GitBranch, deductiongrid: Grid3x3, deductiongrid2: Rocket, timeecho: Timer, shadowswitch: Ghost, signaldecoder: Radio,
};

const GAME_COLORS: Record<string, string> = {
  quickpick: "#FF2D78", reflexgrid: "#00D4FF", memoryflash: "#B44DFF",
  daily: "#FFD700", skyclimb: "#00FF88", mathtest: "#FF8C00", numberpath: "#00FF88", pathbuilder: "#2DD4BF", topicsort: "#14B8A6", pipeflow: "#22C55E", deductiongrid: "#8B5CF6", deductiongrid2: "#6366F1", timeecho: "#00FFFF", shadowswitch: "#F472B6", signaldecoder: "#00FFCC",
...
export default function CollectionPage() {
  ...
  useEffect(() => {
    ...
    refresh();
    // Re-read when tab regains focus (bfcache / back-navigation)
    const onVisible = () => { if (document.visibilityState === "visible") refresh(); };
    document.addEventListener("visibilitychange", onVisible);
    // Re-read when a card is saved/removed from any tab
    window.addEventListener("plizio-cards-changed", refresh);
    return () => {
      document.removeEventListener("visibilitychange", onVisible);
      window.removeEventListener("plizio-cards-changed", refresh);
    };
  }, []);
  ...
  const handleExchange = () => {
    // Always read fresh from localStorage — React state can be stale
    // (e.g. card earned in another tab, bfcache restore, etc.)
    const freshCards = getCards();
    const freshCounts: Record<CardRarity, number> = {
    ...
    // Greedy: highest rarity first, cover exactly freshExchangeable * 60 micro-pts
    ...
    // 1–2pt gap: bronze (2pt) closes it with 1pt overcoverage at most
    if (toFill > 0 && toFill <= 2 && toRemove.bronze < freshCounts.bronze) {
      toRemove.bronze += 1;
    }
