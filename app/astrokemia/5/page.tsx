...
import {
  ...
  loadKemiaK5Progress as loadK5Progress, saveKemiaK5Progress as saveK5Progress,
  type KemiaK5Progress as K5Progress,
  isMissionDoneKemiaK5 as isMissionDoneK5, isIslandDoneKemiaK5 as isIslandDoneK5,
  isIslandUnlockedKemiaK5 as isIslandUnlockedK5,
  isCheckpointUnlockedKemiaK5 as isCheckpointUnlockedK5, isCheckpointDoneKemiaK5 as isCheckpointDoneK5,
  completeMissionKemiaK5 as completeMissionK5, completeTestKemiaK5 as completeTestK5,
  islandTotalStarsKemiaK5 as islandTotalStarsK5,
  generateIslandQuestionsKemiaK5 as generateIslandQuestionsK5,
  generateCheckpointQuestionsKemiaK5 as generateCheckpointQuestionsK5,
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowKemia5" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowKemia5" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1kemia5" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#10B981" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#10B981" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2kemia5" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#3B82F6" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#3B82F6" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1kemia5)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2kemia5)" />

      <path d={pathD} fill="none" stroke="rgba(16,185,129,0.25)" strokeWidth={8}
        filter="url(#pathGlowKemia5)" strokeLinecap="round" />
...
export default function AstroKemiaK5Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (mission.gameType === "ck5-explorer" || mission.gameType === "m2" || mission.gameType === "m3") {
      setQuestions([]);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #10B981, #00FF88)" }}
  ...
  const gameScreen = (
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "ck5-explorer" && activeIsland && (
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK5: Record<string, Record<string, string>> = {
      en: { test1: "Everyday Materials & Particle Model", test2: "Mixtures & Water/Air", test3: "Chemical Changes & Lab Safety" },
      hu: { test1: "Mindennapi anyagok & Részecskemodell", test2: "Keverékek & Víz/Levegő", test3: "Kémiai változások & Laborbiztonság" },
      de: { test1: "Alltagsstoffe & Teilchenmodell", test2: "Gemische & Wasser/Luft", test3: "Chemische Veränderungen & Laborsicherheit" },
      ro: { test1: "Substanțe cotidiene & Modelul particulelor", test2: "Amestecuri & Apă/Aer", test3: "Schimbări chimice & Siguranța în laborator" },
    };
