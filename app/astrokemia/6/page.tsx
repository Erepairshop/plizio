...
import {
  KEMIA_K6_ISLANDS as K6_ISLANDS, KEMIA_K6_CHECKPOINT_MAP as K6_CHECKPOINT_MAP,
  KEMIA_K6_TOPIC_LABELS as K6_TOPIC_LABELS,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadKemiaK6Progress as loadK6Progress, saveKemiaK6Progress as saveK6Progress,
  type KemiaK6Progress as K6Progress,
  isMissionDoneKemiaK6 as isMissionDoneK6, isIslandDoneKemiaK6 as isIslandDoneK6,
  isIslandUnlockedKemiaK6 as isIslandUnlockedK6,
  isCheckpointUnlockedKemiaK6 as isCheckpointUnlockedK6, isCheckpointDoneKemiaK6 as isCheckpointDoneK6,
  completeMissionKemiaK6 as completeMissionK6, completeTestKemiaK6 as completeTestK6,
  islandTotalStarsKemiaK6 as islandTotalStarsK6,
  generateIslandQuestionsKemiaK6 as generateIslandQuestionsK6,
  generateCheckpointQuestionsKemiaK6 as generateCheckpointQuestionsK6,
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowKemia6" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowKemia6" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1kemia6" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#3B82F6" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#3B82F6" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2kemia6" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#8B5CF6" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#8B5CF6" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1kemia6)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2kemia6)" />

      <path d={pathD} fill="none" stroke="rgba(59,130,246,0.25)" strokeWidth={8}
        filter="url(#pathGlowKemia6)" strokeLinecap="round" />
...
export default function AstroKemiaK6Page() {
  const { lang } = useLang();
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (mission.gameType === "ck6-explorer" || mission.gameType === "m2" || mission.gameType === "m3") {
      setQuestions([]);
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #3B82F6, #00FF88)" }}
  ...
  const gameScreen = (
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "ck6-explorer" && activeIsland && (
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK6: Record<string, Record<string, string>> = {
      en: { test1: "Atoms, Elements & Compounds", test2: "Understanding Reactions & Acids/Bases", test3: "Solutions, Separation & Environment" },
      hu: { test1: "Atomok, elemek & vegyületek", test2: "Reakciók megértése & Savak/Bázisok", test3: "Oldatok, szétválasztás & Környezet" },
      de: { test1: "Atome, Elemente & Verbindungen", test2: "Reaktionen verstehen & Säuren/Basen", test3: "Lösungen, Trennung & Umwelt" },
      ro: { test1: "Atomi, elemente & compuși", test2: "Înțelegerea reacțiilor & Acizi/Baze", test3: "Soluții, separare & Mediu" },
    };
