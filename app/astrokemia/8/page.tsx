...
import {
  KEMIA_K8_ISLANDS as K8_ISLANDS, KEMIA_K8_CHECKPOINT_MAP as K8_CHECKPOINT_MAP,
  KEMIA_K8_TOPIC_LABELS as K8_TOPIC_LABELS,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadKemiaK8Progress as loadK8Progress, saveKemiaK8Progress as saveK8Progress,
  type KemiaK8Progress as K8Progress,
  isMissionDoneKemiaK8 as isMissionDoneK8, isIslandDoneKemiaK8 as isIslandDoneK8,
  isIslandUnlockedKemiaK8 as isIslandUnlockedK8,
  isCheckpointUnlockedKemiaK8 as isCheckpointUnlockedK8, isCheckpointDoneKemiaK8 as isCheckpointDoneK8,
  completeMissionKemiaK8 as completeMissionK8, completeTestKemiaK8 as completeTestK8,
  islandTotalStarsKemiaK8 as islandTotalStarsK8,
  generateIslandQuestionsKemiaK8 as generateIslandQuestionsK8,
  generateCheckpointQuestionsKemiaK8 as generateCheckpointQuestionsK8,
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowKemia8" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowKemia8" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1kemia8" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#10B981" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#10B981" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2kemia8" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#F97316" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#F97316" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1kemia8)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2kemia8)" />

      <path d={pathD} fill="none" stroke="rgba(16,185,129,0.25)" strokeWidth={8}
        filter="url(#pathGlowKemia8)" strokeLinecap="round" />
...
export default function AstroKemiaK8Page() {
  const { lang } = useLang();
  ...
  const [progress, setProgress] = useState<K8Progress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    if (mission.gameType === "ck8-explorer" || mission.gameType === "m2" || mission.gameType === "m3") {
      setQuestions([]);
    ...
    const qs = generateIslandQuestionsK8(activeIsland, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland]);
  ...
  const bgColor = activeIsland?.color ?? "#F97316";
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #F97316, #00FF88)" }}
  ...
  const gameScreen = (
    <div className="min-h-screen flex flex-col relative overflow-hidden"
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
        ...
        {screen === "ck8-explorer" && activeIsland && (
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK8: Record<string, Record<string, string>> = {
      en: { test1: "Organic Chemistry & Functional Groups", test2: "Reaction Rates & Electrochemistry", test3: "Calculations & Environment" },
      hu: { test1: "Szerves kémia & Funkciós csoportok", test2: "Reakciósebesség & Elektrokémia", test3: "Számítások & Környezet" },
      de: { test1: "Organische Chemie & Stoffklassen", test2: "Reaktionsgeschwindigkeit & Elektrochemie", test3: "Berechnungen & Umwelt" },
      ro: { test1: "Chimie organică & Grupe funcționale", test2: "Viteza reacției & Electrochimie", test3: "Calcule & Mediu" },
    };
