...
import {
  BIO_K7_ISLANDS as K7_ISLANDS, BIO_K7_CHECKPOINT_MAP as K7_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadBioK7Progress as loadK7Progress, saveBioK7Progress as saveK7Progress,
  type BioK7Progress,
  isMissionDoneBioK7 as isMissionDoneK7, isIslandDoneBioK7 as isIslandDoneK7,
  isIslandUnlockedBioK7 as isIslandUnlockedK7,
  isCheckpointUnlockedBioK7 as isCheckpointUnlockedK7, isCheckpointDoneBioK7 as isCheckpointDoneK7,
  completeMissionBioK7 as completeMissionK7, completeTestBioK7 as completeTestK7,
  islandTotalStarsBioK7 as islandTotalStarsK7,
  generateIslandQuestionsK7, generateCheckpointQuestionsK7,
} from "@/lib/astroBiologie7";
...
const K7_LABEL: Record<string, string> = {
  en: "Grade 7 · Biology",
  hu: "7. osztály · Biológia",
  de: "Klasse 7 · Biologie",
  ro: "Clasa 7 · Biologie",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowBio7" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowBio7" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1bio7" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#B44DFF" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#B44DFF" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2bio7" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#B44DFF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#B44DFF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1bio7)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2bio7)" />

      <path d={pathD} fill="none" stroke="rgba(180,77,255,0.25)" strokeWidth={8}
        filter="url(#pathGlowBio7)" strokeLinecap="round" />
...
export default function AstroBiologieK7Page() {
  const { lang } = useLang();
  ...
  const handleIslandAnimDone = useCallback(() => {
    ...
    saveCard({ id: generateCardId(), game: "astro-biologie", rarity, score: missionScore.score, total: missionScore.total, date: new Date().toISOString() });
    ...
    return (
      ...
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          <button onClick={() => router.push("/astro-biologie")}
      ...
      <VisualLab subject="biologie" grade={7} lang={lang} open={visualLabOpen} onClose={() => setVisualLabOpen(false)} />
      </>
    );
  }
  ...
  if (screen === "reward" && earnedCard) {
    return (
      <>
        <RewardReveal rarity={earnedCard} game="astro-biologie"
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK7: Record<string, Record<string, string>> = {
      en: { test1: "Cell Biology: Structure, Division & Photosynthesis", test2: "Cellular Respiration, Ecology & Symbiosis", test3: "Immune System, Nervous System & Evolution" },
      hu: { test1: "Sejtbiológia: Sejtek, Sejtoszlás & Fotoszintézis", test2: "Sejt-légzés, Ökológia & Szimbiózis", test3: "Immunrendszer, Idegrendszer & Evolúció" },
      de: { test1: "Zellbiologie: Zellstruktur, Zellteilung & Fotosynthese", test2: "Zellatmung, Ökologie & Symbiose", test3: "Immunsystem, Nervensystem & Evolution" },
      ro: { test1: "Biologie celulară: Structura celulei, Diviziune & Fotosinteză", test2: "Respirație celulară, Ecologie & Simbioză", test3: "Sistemul imunitar, Sistemul nervos & Evoluție" },
    };
