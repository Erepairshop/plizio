...
import {
  ...
  type BioK6Progress,
  isMissionDoneBioK6 as isMissionDoneK6, isIslandDoneBioK6 as isIslandDoneK6,
  isIslandUnlockedBioK6 as isIslandUnlockedK6,
  isCheckpointUnlockedBioK6 as isCheckpointUnlockedK6, isCheckpointDoneBioK6 as isCheckpointDoneK6,
  completeMissionBioK6 as completeMissionK6, completeTestBioK6 as completeTestK6,
  islandTotalStarsBioK6 as islandTotalStarsK6,
  generateIslandQuestionsK6, generateCheckpointQuestionsK6,
} from "@/lib/astroBiologie6";
...
const K6_LABEL: Record<string, string> = {
  en: "Grade 6 · Biology",
  hu: "6. osztály · Biológia",
  de: "Klasse 6 · Biologie",
  ro: "Clasa 6 · Biologie",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowBio6" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowBio6" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1bio6" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#FF9500" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#FF9500" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2bio6" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1bio6)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2bio6)" />

      <path d={pathD} fill="none" stroke="rgba(255,149,0,0.25)" strokeWidth={8}
        filter="url(#pathGlowBio6)" strokeLinecap="round" />
...
export default function AstroBiologieK6Page() {
  const { lang } = useLang();
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK6: Record<string, Record<string, string>> = {
      en: { test1: "Invertebrates & Ecosystems: Arthropods, Mollusks, Forest & Water", test2: "Human Body: Heart, Blood & Circulation", test3: "Respiration, Puberty & Biology Review" },
      hu: { test1: "Gerinctelenek & ökoszisztémák: Ízeltlábúak, Puhatestűek, Erdő, Víz", test2: "Emberi test: Szív, Vér & Vérkeringés", test3: "Légzés, Pubertás & Biológia összefoglalás" },
      de: { test1: "Wirbellose & Ökosysteme: Gliederfüßer, Weichtiere, Wald, Wasser", test2: "Menschlicher Körper: Herz, Blut & Kreislauf", test3: "Atmung, Pubertät & Biologie-Review" },
      ro: { test1: "Nevertebrate & Ecosisteme: Artropode, Moluște, Pădure, Apă", test2: "Corp uman: Inimă, Sânge & Circulație", test3: "Respirație, Pubertate & Recapitulare" },
    };
