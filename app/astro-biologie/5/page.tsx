...
import {
  ...
  loadBioK5Progress as loadK5Progress, saveBioK5Progress as saveK5Progress,
  type BioK5Progress,
  isMissionDoneBioK5 as isMissionDoneK5, isIslandDoneBioK5 as isIslandDoneK5,
  isIslandUnlockedBioK5 as isIslandUnlockedK5,
  isCheckpointUnlockedBioK5 as isCheckpointUnlockedK5, isCheckpointDoneBioK5 as isCheckpointDoneK5,
  completeMissionBioK5 as completeMissionK5, completeTestBioK5 as completeTestK5,
  islandTotalStarsBioK5 as islandTotalStarsK5,
  generateIslandQuestionsK5, generateCheckpointQuestionsK5,
} from "@/lib/astroBiologie5";
...
const K5_LABEL: Record<string, string> = {
  en: "Grade 5 · Biology",
  hu: "5. osztály · Biológia",
  de: "Klasse 5 · Biologie",
  ro: "Clasa 5 · Biologie",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowBio5" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowBio5" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1bio5" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#22C55E" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#22C55E" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2bio5" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#0080FF" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#0080FF" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1bio5)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2bio5)" />

      <path d={pathD} fill="none" stroke="rgba(100,200,100,0.25)" strokeWidth={8}
        filter="url(#pathGlowBio5)" strokeLinecap="round" />
...
export default function AstroBiologieK5Page() {
  const { lang } = useLang();
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK5: Record<string, Record<string, string>> = {
      en: { test1: "Vertebrates: Fish, Amphibians, Reptiles, Birds, Mammals", test2: "Plants & Human Body: Organs, Photosynthesis, Skeleton, Muscles", test3: "Senses, Nutrition & Digestion" },
      hu: { test1: "Gerincesek: Halak, Kétéltűek, Hüllők, Madarak, Emlősök", test2: "Növények & Emberi test: szervek, fotoszintézis, csontváz", test3: "Érzékszervek, Táplálkozás & Emésztés" },
      de: { test1: "Wirbeltiere: Fische, Amphibien, Reptilien, Vögel, Säugetiere", test2: "Pflanzen & Menschlicher Körper: Organe, Fotosynthese, Skelett", test3: "Sinnesorgane, Ernährung & Verdauung" },
      ro: { test1: "Vertebrate: Pești, Amfibieni, Reptile, Păsări, Mamifere", test2: "Plante & Corp uman: organe, fotosinteză, schelet", test3: "Organe de simț, Nutriție & Digestie" },
    };
