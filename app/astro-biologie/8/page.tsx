...
import {
  BIO_K8_ISLANDS as K8_ISLANDS, BIO_K8_CHECKPOINT_MAP as K8_CHECKPOINT_MAP,
  type IslandDef, type MissionDef, type Lang, type MissionCategory,
  loadBioK8Progress as loadK8Progress, saveBioK8Progress as saveK8Progress,
  type BioK8Progress,
  isMissionDoneBioK8 as isMissionDoneK8, isIslandDoneBioK8 as isIslandDoneK8,
  isIslandUnlockedBioK8 as isIslandUnlockedK8,
  isCheckpointUnlockedBioK8 as isCheckpointUnlockedK8, isCheckpointDoneBioK8 as isCheckpointDoneK8,
  completeMissionBioK8 as completeMissionK8, completeTestBioK8 as completeTestK8,
  islandTotalStarsBioK8 as islandTotalStarsK8,
  generateIslandQuestionsK8, generateCheckpointQuestionsK8,
} from "@/lib/astroBiologie8";
...
const K8_LABEL: Record<string, string> = {
  en: "Grade 8 · Biology",
  hu: "8. osztály · Biológia",
  de: "Klasse 8 · Biologie",
  ro: "Clasa 8 · Biologie",
};
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      <defs>
        <filter id="pathGlowBio8" x="-20%" y="-20%" width="140%" height="140%">
          <feGaussianBlur stdDeviation="3" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <filter id="islandGlowBio8" x="-40%" y="-40%" width="180%" height="180%">
          <feGaussianBlur stdDeviation="5" result="blur" />
          <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
        </filter>
        <radialGradient id="nebula1bio8" cx="30%" cy="60%" r="50%">
          <stop offset="0%" stopColor="#E879F9" stopOpacity="0.12" />
          <stop offset="100%" stopColor="#E879F9" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="nebula2bio8" cx="70%" cy="30%" r="40%">
          <stop offset="0%" stopColor="#E879F9" stopOpacity="0.09" />
          <stop offset="100%" stopColor="#E879F9" stopOpacity="0" />
        </radialGradient>
      </defs>

      <ellipse cx={100} cy={350} rx={160} ry={200} fill="url(#nebula1bio8)" />
      <ellipse cx={220} cy={100} rx={130} ry={160} fill="url(#nebula2bio8)" />

      <path d={pathD} fill="none" stroke="rgba(232,121,249,0.25)" strokeWidth={8}
        filter="url(#pathGlowBio8)" strokeLinecap="round" />
...
export default function AstroBiologieK8Page() {
  const { lang } = useLang();
  ...
  if (screen === "checkpoint-intro" && activeTestId) {
    const testTopicsK8: Record<string, Record<string, string>> = {
      en: { test1: "Genetics: DNA, Mutations & Gene Technology", test2: "Hormones, Reproduction & Sex Determination", test3: "Biotechnology, Systems Biology & Population Genetics" },
      hu: { test1: "Genetika: DNS, Mutációk & Géntechnika", test2: "Hormonok, Szaporodás & Nemi meghatározás", test3: "Biotechnológia, Rendszerbiológia & Populációgenetika" },
      de: { test1: "Genetik: DNA, Mutationen & Gentechnik", test2: "Hormone, Reproduktion & Geschlechtsbestimmung", test3: "Biotechnologie, Systemische Biologie & Populationsgenetik" },
      ro: { test1: "Genetică: ADN, Mutații & Biotehnologie", test2: "Hormoni, Reproducere & Determinarea sexului", test3: "Biotehnologie, Biologie Sistemică & Genetica populațiilor" },
    };
