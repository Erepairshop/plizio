...
import HabitatExplorer from "@/app/astro-sachkunde/games/k2/HabitatExplorer";
import WildAnimalExplorer from "@/app/astro-sachkunde/games/k2/WildAnimalExplorer";
import PlantExplorer from "@/app/astro-sachkunde/games/k2/PlantExplorer";
import WaterExplorer from "@/app/astro-sachkunde/games/k2/WaterExplorer";
import JobsExplorer from "@/app/astro-sachkunde/games/k2/JobsExplorer";
import SafetyExplorer from "@/app/astro-sachkunde/games/k2/SafetyExplorer";
import ScienceExplorer from "@/app/astro-sachkunde/games/k2/ScienceExplorer";
import GrandReviewK2Explorer from "@/app/astro-sachkunde/games/k2/GrandReviewK2Explorer";
import {
  ...
  isMissionDoneSKG2, isIslandDoneSKG2, isIslandUnlockedSKG2, isCheckpointUnlockedSKG2, isCheckpointDoneSKG2,
  completeMissionSKG2, completeTestSKG2, islandTotalStarsSKG2,
  generateIslandQuestionsSKG2, generateCheckpointQuestionsSKG2,
} from "@/lib/astroSachkunde2";
import { SK_K2_ISLAND_SVGS } from "@/app/astro-sachkunde/islands-k2";
...
const SK_G2_LABEL: Record<string, Record<string, string | Record<string, string>>> = {
  en: { title: "AstroSachkunde", grade: "Grade 2", back: "Back", start: "Start", next: "Next Island",
  ...
  hu: { title: "AstroSachkunde", grade: "2. osztály", back: "Vissza", start: "Indítás", next: "Következő sziget",
  ...
  de: { title: "AstroSachkunde", grade: "Klasse 2", back: "Zurück", start: "Start", next: "Nächste Insel",
  ...
  ro: { title: "AstroSachkunde", grade: "Clasa 2", back: "Înapoi", start: "Start", next: "Insula următoare",
...
function buildSmoothPath(islands: typeof SK_G2_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint }: {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      ...
      {SK_G2_ISLANDS.slice(0, -1).map((island, i) => {
        const next = SK_G2_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedSKG2(progress, testId);
        const done = isCheckpointDoneSKG2(progress, testId);
      ...
      {SK_G2_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedSKG2(progress, island.id);
        const done = isIslandDoneSKG2(progress, island.id);
        const total = islandTotalStarsSKG2(progress, island.id);
        const missionsDone = island.missions.filter((m) => isMissionDoneSKG2(progress, island.id, m.id)).length;
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneSKG2(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
export default function AstroSachkundeG2Page() {
  ...
  const avatarIsland = SK_G2_ISLANDS.find(i => i.id === avatarIslandId) ?? SK_G2_ISLANDS[0];
  ...
  useEffect(() => {
    ...
    setProgress(p);
    const lastDone = [...SK_G2_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsSKG2(activeIsland, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland]);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeMissionSKG2(progress, activeIsland.id, activeMission.id, stars);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsSKG2(testId, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsSKG2(activeTestId, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeTestSKG2(progress, activeTestId);
    saveSKG2Progress(newProgress);
    setProgress(newProgress);
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsSKG2(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneSKG2(progress, activeIsland.id, mission.id);
