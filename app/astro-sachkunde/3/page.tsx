...
import {
  ...
  isMissionDoneSKG3, isIslandDoneSKG3, isIslandUnlockedSKG3, isCheckpointUnlockedSKG3, isCheckpointDoneSKG3,
...
const CATEGORY_CONFIG: Record<string, {
  ...
  explore: {
    ...
    desc: {
      en: "Discover the concept — no wrong answers!",
      hu: "Fedezd fel vizuálisan — nincs hibás válasz!",
      de: "Entdecke das Konzept — keine falschen Antworten!",
      ro: "Descoperă conceptul — fără răspunsuri greșite!",
    },
    color: "#A78BFA", bg: "rgba(167,139,250,0.12)", border: "rgba(167,139,250,0.35)",
  },
  build: {
    label: { en: "Learn", hu: "Tanulás", de: "Lernen", ro: "Învață" },
    desc: {
  ...
  challenge: {
    ...
    desc: {
      en: "Fast — show what you know!",
      hu: "Gyors — mutasd meg tudásod!",
      de: "Schnell — zeig was du kannst!",
      ro: "Rapid — arată ce știi!",
    },
...
// ─── Grade 3 label ─────────────────────────────────────────────────────────────
const SK_G3_LABEL: Record<string, Record<string, string | Record<string, string>>> = {
  en: { title: "AstroSachkunde", grade: "Grade 3", back: "Back", start: "Start", next: "Next Island",
        missionDone: "Mission Complete!", islandDone: "Island Complete!", checkpointTitle: "Checkpoint",
        stars: "Stars", replay: "Replay", continue: "Continue", missions: "Missions",
        testTopics: { test1: "Body & Animal Kingdom · Forest Life", test2: "Plants & Pollination · Weather & Geography", test3: "Technology & Environment · Community & Rules" },
        islandMap: "Island Map", testDone: "Checkpoint Complete", yourScore: "Your Score",
        checkpointReady: "Ready for Checkpoint?", checkpointDesc: "Test what you've learned from the last 3 islands!",
        startTest: "Start Test", rocketTitle: "Warm-Up", rocketDesc: "Get ready for the checkpoint test!" },
  hu: { title: "AstroSachkunde", grade: "3. osztály", back: "Vissza", start: "Indítás", next: "Következő sziget",
        missionDone: "Küldetés teljesítve!", islandDone: "Sziget teljesítve!", checkpointTitle: "Ellenőrzőpont",
        stars: "Csillagok", replay: "Újra", continue: "Tovább", missions: "Küldetések",
        testTopics: { test1: "Testrendszerek & Állatok · Erdei élet", test2: "Növények & Pollináció · Időjárás & Földrajz", test3: "Technológia & Környezet · Közösség & Szabályok" },
        islandMap: "Sziget térkép", testDone: "Ellenőrzőpont teljesítve", yourScore: "Az Ön pontszáma",
        checkpointReady: "Készen állsz az ellenőrzőpontra?", checkpointDesc: "Tesztelj az utolsó 3 szigetről tanultakból!",
        startTest: "Teszt indítása", rocketTitle: "Bemelegítés", rocketDesc: "Készülj fel az ellenőrzőpont tesztre!" },
  de: { title: "AstroSachkunde", grade: "Klasse 3", back: "Zurück", start: "Start", next: "Nächste Insel",
        missionDone: "Mission geschafft!", islandDone: "Insel geschafft!", checkpointTitle: "Checkpoint",
        stars: "Sterne", replay: "Wiederholen", continue: "Weiter", missions: "Missionen",
        testTopics: { test1: "Körpersysteme & Tierwelt · Waldleben", test2: "Pflanzen & Bestäubung · Wetter & Geographie", test3: "Technik & Umwelt · Gemeinschaft & Regeln" },
        islandMap: "Insekarte", testDone: "Checkpoint geschafft", yourScore: "Deine Punktzahl",
        checkpointReady: "Bereit für den Checkpoint?", checkpointDesc: "Teste dein Wissen aus den letzten 3 Inseln!",
        startTest: "Test starten", rocketTitle: "Aufwärmung", rocketDesc: "Bereite dich auf den Checkpoint-Test vor!" },
  ro: { title: "AstroSachkunde", grade: "Clasa 3", back: "Înapoi", start: "Start", next: "Insula următoare",
        missionDone: "Misiune completă!", islandDone: "Insulă completă!", checkpointTitle: "Punct de control",
        stars: "Stele", replay: "Rejoacă", continue: "Continuă", missions: "Misiuni",
        testTopics: { test1: "Sisteme corporale & Animale · Viață în pădure", test2: "Plante & Polenizare · Vreme & Geografie", test3: "Tehnologie & Mediu · Comunitate & Reguli" },
        islandMap: "Hartă insule", testDone: "Punct de control completat", yourScore: "Scorul tău",
        checkpointReady: "Gata pentru punct de control?", checkpointDesc: "Testează ce ai învățat din ultimele 3 insule!",
        startTest: "Lansează testul", rocketTitle: "Încălzire", rocketDesc: "Pregătește-te pentru testul de punct de control!" },
};
...
function buildSmoothPath(islands: typeof SK_G3_ISLANDS): string {
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedSKG3(progress, testId);
        const done = isCheckpointDoneSKG3(progress, testId);
      ...
      {SK_G3_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedSKG3(progress, island.id);
        const done = isIslandDoneSKG3(progress, island.id);
        const total = islandTotalStarsSKG3(progress, island.id);
        const missionsDone = island.missions.filter((m) => isMissionDoneSKG3(progress, island.id, m.id)).length;
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneSKG3(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
export default function AstroSachkundeG3Page() {
  ...
  const avatarIsland = SK_G3_ISLANDS.find(i => i.id === avatarIslandId) ?? SK_G3_ISLANDS[0];
  ...
  useEffect(() => {
    ...
    setProgress(p);
    const lastDone = [...SK_G3_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = SK_G3_ISLANDS.find(i => i.id === id);
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const K3_EXPLORER_MAP: Record<string, string> = {
      i1: "k3-body-systems-explorer", i2: "k3-animal-kingdom-explorer", i3: "k3-forest-life-explorer",
      i4: "k3-pollination-explorer", i5: "k3-weather-climate-explorer", i6: "k3-geography-explorer",
      i7: "k3-tech-machines-explorer", i8: "k3-community-town-explorer", i9: "k3-environment-explorer",
    };
    if (mission.gameType === "sachkunde-k3-explore") {
      const explorerScreen = K3_EXPLORER_MAP[activeIsland.id] ?? "k3-body-systems-explorer";
      setQuestions([]);
    ...
    const qs = generateIslandQuestionsSKG3(activeIsland, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland]);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeMissionSKG3(progress, activeIsland.id, activeMission.id, stars);
  ...
  const handleIslandAnimDone = useCallback(() => {
    ...
    saveCard({ id: generateCardId(), game: "astrosachkunde", rarity, score: missionScore.score, total: missionScore.total, date: new Date().toISOString() });
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsSKG3(testId, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsSKG3(activeTestId, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeTestSKG3(progress, activeTestId);
    saveSKG3Progress(newProgress);
    setProgress(newProgress);

    const rarity = calculateRarity(score, total, 0, false);
    saveCard({ id: generateCardId(), game: "astrosachkunde", rarity, score, total, date: new Date().toISOString() });
    window.dispatchEvent(new Event("plizio-cards-changed"));
    incrementTotalGames();
    checkNewMilestones();
    setEarnedCard(rarity);
    ...
    return (
      <div className="min-h-screen bg-[#060614] flex flex-col relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between px-4 pt-5 pb-2 flex-shrink-0">
          <button onClick={() => router.push("/astro-sachkunde")}
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-white/50 font-medium uppercase tracking-widest">{(l.title as string)} · {(l.grade as string)}</p>
          </div>
        ...
        <div className="relative z-10 px-4 mb-2 flex-shrink-0">
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <motion.div className="h-full rounded-full" style={{ background: "linear-gradient(90deg, #4CAF50, #81C784)" }}
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsSKG3(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneSKG3(progress, activeIsland.id, mission.id);
  ...
  if (screen === "reward" && earnedCard) {
    return (
      <>
        <RewardReveal rarity={earnedCard} game="astrosachkunde"
    ...
    return (
      <>
        <div className="min-h-screen flex flex-col relative overflow-hidden"
          ...
          <div className="relative z-10 flex items-center gap-3 px-4 pt-5 pb-3">
            <button onClick={goToMap} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 text-white/70"><X size={14} /></button>
            <p className="text-white/70 text-sm font-bold flex-1">{t.checkpointTitle ?? "Test"}</p>
          </div>
          <div className="relative z-10 flex-1 flex flex-col justify-center px-4 pb-6">
