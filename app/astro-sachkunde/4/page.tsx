...
import {
  ...
  isMissionDoneSKG4, isIslandDoneSKG4, isIslandUnlockedSKG4, isCheckpointUnlockedSKG4, isCheckpointDoneSKG4,
  completeMissionSKG4, completeTestSKG4, islandTotalStarsSKG4,
...
// ─── Grade 1 label ─────────────────────────────────────────────────────────────
const SK_G4_LABEL: Record<string, Record<string, string | Record<string, string>>> = {
  en: { title: "AstroSachkunde", grade: "Grade 4", back: "Back", start: "Start", next: "Next Island",
  ...
  hu: { title: "AstroSachkunde", grade: "4. osztály", back: "Vissza", start: "Indítás", next: "Következő sziget",
  ...
  de: { title: "AstroSachkunde", grade: "Klasse 4", back: "Zurück", start: "Start", next: "Nächste Insel",
  ...
  ro: { title: "AstroSachkunde", grade: "Clasa 4", back: "Înapoi", start: "Start", next: "Insula următoare",
...
function buildSmoothPath(islands: typeof SK_G4_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint, lang }: {
  ...
  const pathD = buildSmoothPath(SK_G4_ISLANDS);
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      ...
      {SK_G4_ISLANDS.slice(0, -1).map((island, i) => {
        const next = SK_G4_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedSKG4(progress, testId);
        const done = isCheckpointDoneSKG4(progress, testId);
      ...
      {SK_G4_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedSKG4(progress, island.id);
        const done = isIslandDoneSKG4(progress, island.id);
        const total = islandTotalStarsSKG4(progress, island.id);
        const missionsDone = island.missions.filter((m) => isMissionDoneSKG4(progress, island.id, m.id)).length;
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneSKG4(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function AstroSachkundeG4Page() {
  const { lang } = useLang();
  ...
  const avatarIsland = SK_G4_ISLANDS.find(i => i.id === avatarIslandId) ?? SK_G4_ISLANDS[0];
  ...
  useEffect(() => {
    ...
    setProgress(p);
    const lastDone = [...SK_G4_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = SK_G4_ISLANDS.find(i => i.id === id);
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsSKG4(activeIsland, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland]);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeMissionSKG4(progress, activeIsland.id, activeMission.id, stars);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsSKG4(testId, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsSKG4(activeTestId, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeTestSKG4(progress, activeTestId);
    saveSKG4Progress(newProgress);
    setProgress(newProgress);
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsSKG4(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneSKG4(progress, activeIsland.id, mission.id);
  ...
  if ([...explorerScreens, "orbit-quiz", "black-hole", "gravity-sort", "star-match", "true-false-blitz", "m2", "m3"].includes(screen)) return (
    <>
      {gameScreen}
