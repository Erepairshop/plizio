...
import {
  ...
  isMissionDoneSKG1, isIslandDoneSKG1, isIslandUnlockedSKG1, isCheckpointUnlockedSKG1, isCheckpointDoneSKG1,
  completeMissionSKG1, completeTestSKG1, islandTotalStarsSKG1,
  generateIslandQuestionsSKG1, generateCheckpointQuestionsSKG1,
} from "@/lib/astroSachkunde1";
import { SK_K1_ISLAND_SVGS } from "@/app/astro-sachkunde/islands";
...
// ─── Grade 1 label ─────────────────────────────────────────────────────────────
const SK_G1_LABEL: Record<string, Record<string, string | Record<string, string>>> = {
  en: { title: "AstroSachkunde", grade: "Grade 1", back: "Back", start: "Start", next: "Next Island",
        ...
        testTopics: { test1: "Body & Senses · Animals & Nature", test2: "Weather & Home · Family & Safety", test3: "Materials & Recycling · Grand Review" },
  ...
  hu: { title: "AstroSachkunde", grade: "1. osztály", back: "Vissza", start: "Indítás", next: "Következő sziget",
        ...
        testTopics: { test1: "Test és érzékek · Háziallatások & Vadállatok", test2: "Időjárás & Otthon · Család & Biztonság", test3: "Anyagok & Újrahasznosítás · Végső döntő" },
  ...
  de: { title: "AstroSachkunde", grade: "Klasse 1", back: "Zurück", start: "Start", next: "Nächste Insel",
        ...
        testTopics: { test1: "Körper & Sinne · Haustiere & Wilde Tiere", test2: "Wetter & Zuhause · Familie & Sicherheit", test3: "Materialien & Recycling · Großes Finale" },
  ...
  ro: { title: "AstroSachkunde", grade: "Clasa 1", back: "Înapoi", start: "Start", next: "Insula următoare",
        ...
        testTopics: { test1: "Corp și simțuri · Animale de companie & Animale sălbatice", test2: "Vreme & Casă · Familie & Siguranță", test3: "Materiale & Reciclare · Marele Final" },
...
function buildSmoothPath(islands: typeof SK_G1_ISLANDS): string {
...
function IslandMapSVG({ progress, onIsland, onCheckpoint, lang }: {
  ...
  const pathD = buildSmoothPath(SK_G1_ISLANDS);
  ...
  return (
    <svg viewBox={`0 -${MAP_VB_OFFSET} ${MAP_W} ${MAP_H}`} width="100%" style={{ minHeight: MAP_H, display: "block" }}>
      ...
      {SK_G1_ISLANDS.slice(0, -1).map((island, i) => {
        const next = SK_G1_ISLANDS[i + 1];
      ...
      {Object.entries(CP_POS).map(([testId, pos]) => {
        const unlocked = isCheckpointUnlockedSKG1(progress, testId);
        const done = isCheckpointDoneSKG1(progress, testId);
      ...
      {SK_G1_ISLANDS.map((island, idx) => {
        const unlocked = isIslandUnlockedSKG1(progress, island.id);
        const done = isIslandDoneSKG1(progress, island.id);
        const total = islandTotalStarsSKG1(progress, island.id);
        const missionsDone = island.missions.filter((m) => isMissionDoneSKG1(progress, island.id, m.id)).length;
            ...
            {unlocked && !done && (
              <g>
                {island.missions.map((m, mi) => {
                  const mdone = isMissionDoneSKG1(progress, island.id, m.id);
                  return (
                    <g key={mi}>
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function AstroSachkundeG1Page() {
  const { lang } = useLang();
  const router = useRouter();
  const t = T[lang as keyof typeof T] ?? T.en;
  const l = SK_G1_LABEL[lang] ?? SK_G1_LABEL.en;

  const [screen, setScreen] = useState<Screen>("island-map");
  const [visualLabOpen, setVisualLabOpen] = useState(false);
  const [progress, setProgress] = useState<SachkundeG1Progress>({ completedMissions: [], completedIslands: [], completedTests: [], missionStars: {} });
  ...
  const avatarIsland = SK_G1_ISLANDS.find(i => i.id === avatarIslandId) ?? SK_G1_ISLANDS[0];
  ...
  useEffect(() => {
    const p = loadSKG1Progress();
    setProgress(p);
    const lastDone = [...SK_G1_ISLANDS].reverse().find(i => p.completedIslands.includes(i.id));
  ...
  useEffect(() => {
    ...
    if (id) {
      const target = SK_G1_ISLANDS.find(i => i.id === id);
  ...
  const startMission = useCallback((mission: MissionDef) => {
    ...
    const qs = generateIslandQuestionsSKG1(activeIsland, qCount);
    setQuestions(qs);
    setScreen(mission.gameType as Screen);
  }, [activeIsland]);
  ...
  const handleMissionDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeMissionSKG1(progress, activeIsland.id, activeMission.id, stars);
  ...
  const startCheckpoint = useCallback((testId: string) => {
    ...
    setAvatarMood("focused");
    const qs = generateCheckpointQuestionsSKG1(testId, 7);
    setQuestions(qs);
  ...
  const startCheckpointQuiz = useCallback(() => {
    ...
    const qs = generateCheckpointQuestionsSKG1(activeTestId, 10);
    setQuestions(qs);
  ...
  const handleCheckpointDone = useCallback((score: number, total: number) => {
    ...
    const newProgress = completeTestSKG1(progress, activeTestId);
    saveSKG1Progress(newProgress);
    setProgress(newProgress);
  ...
  if (screen === "mission-select" && activeIsland) {
    const totalStars = islandTotalStarsSKG1(progress, activeIsland.id);
    return (
      <div className="min-h-screen flex flex-col relative overflow-hidden"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${bgColor}22 0%, #060614 55%)` }}>
        <Starfield />
        ...
        <div className="relative z-10 flex-1 flex flex-col px-5 gap-4 pb-8 justify-center">
          {(["explore", "build", "challenge"] as MissionCategory[]).map((cat, cardIdx) => {
            ...
            const done = isMissionDoneSKG1(progress, activeIsland.id, mission.id);
